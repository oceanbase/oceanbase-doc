...
| MINVALUE int_value     | 更改序列的最小值，`int_value` 的取值范围是 \[-(10^27^-1) \~ (10^27^-1)\]。                                                                                                                                                                                                                                                                                           |
| NOMINVALUE             | 如果更改为 `NOMINVALUE`，则升序时最小值为 1，降序时最小值为 -(10^27^-1)。                                                                                                                                                                                                                                                                                                   |
| MAXVALUE int_value     | 更改序列的最大值，`int_value` 的取值范围是 \[(-10^27^+1) \~ (10^28^-1)\]。新指定的 `MAXVALUE` 不能小于当前序列号的值。 **注意**  * 同时指定 `MINVALUE` 和 `MAXVALUE` 时，`MINVALUE` 必须小于 `MAXVALUE`。   * `MAXVALUE` 和 `MINVALUE` 之差必须大于等于 INCREMENT BY 中 的 `int_value`。    |
| NOMAXVALUE             | 如果更改为 `NOMAXVALUE`，则升序时最大值为(10^28^-1)，降序时最大值为 -1。                                                                                                                                                                                                                                                                                                    |
