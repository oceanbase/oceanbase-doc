...
* tests 目录：目录中是一个 obcdc 的简单应用（obcdc_tailf），oblog_taif 简介具体参考 [obcdc_tailf](3.oblog_tailf.md)。tests 目录中包含以下内容：
...
obcdc 需要在编译 OceanBase 代码库（具体请参考 [OceanBase 代码编译](https://github.com/oceanbase/oceanbase/blob/master/README-CN.md#%E5%A6%82%E4%BD%95%E6%9E%84%E5%BB%BA)）时，添加参数 `OB_BUILD_obcdc=ON`。

命令如：`./build.sh debug --init -DOB_BUILD_obcdc=ON --make`
