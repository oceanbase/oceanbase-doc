...
本文例举 obcdc 经常使用的配置项。全部配置项请参考 [obcdc 配置项说明](../2.liboblog-parameters/2.liboblog-configuration-items.md)。
...
| timezone                                   | LogRecord 输出 | obcdc 所属时区：用于同步 timestamp 类型时刷出为指定时区的时间。                                                                                                       |
