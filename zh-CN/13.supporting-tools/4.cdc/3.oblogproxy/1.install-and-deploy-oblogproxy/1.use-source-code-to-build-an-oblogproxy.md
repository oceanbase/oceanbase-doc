...
1. 获取 libobcdc.so
   ...
   * 编译 obcdc
     ...
     您可参考 [obcdc 编译说明](../../2.liboblog/1.deploy-and-use-liboblog/1.install-and-deploy-liboblog.md)，在编译 [OceanBase 代码库](https://github.com/oceanbase/oceanbase#how-to-build) 时，添加参数 `OB_BUILD_LIBOBLOG=ON`。
     ...
     获得编译产出：
     * libobcdc.so：在 `build_release/src/liboblog/src/` 目录下。
     * libobcdc.h：在 `src/liboblog/src/` 目录下。
