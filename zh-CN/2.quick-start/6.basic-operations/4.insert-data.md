...
* 向表 `t1` 中插入多行数据（由于该表有主键，故需要先执行 `TRUNCATE` 操作，避免插入重复数据报错）。
...
更多 `INSERT `语句相关的语法请参见《SQL 参考》中 [INSERT](../../10.sql-reference/5.sql-statements/43.sql-statements-insert.md) 章节。
