...
* OBD
  ...
  OceanBase Deployer，OceanBase 开源软件的安装部署工具，简称为 OBD。详细信息请参考 [OBD 文档](https://www.oceanbase.com/docs/community-obd-cn-10000000001031920)。
  
* ODP

  OceanBase Database Proxy，OceanBase 高性能反向代理服务器，简称为 OBD（又称为 OBProxy）。详细信息请参考 [ODP 文档](https://www.oceanbase.com/docs/enterprise-odp-enterprise-cn-10000000000982768)
...
* 中控机器中已安装 OBD，建议安装最新版本，详细信息请参考 [安装 OBD](https://www.oceanbase.com/docs/community-obd-cn-10000000001031921)。
...
4. 选择配置文件
   ...
   <main id="notice" type='explain'>
     ...
     <ul>
     <li>小规格开发模式：适用于个人设备（内存不低于 8 GB），配置文件名中带有 <code>mini</code> 或 <code>min</code> 标识。</li>
     <li>专业开发模式：适用于高配置 ECS 或物理服务器（可用资源不低于 16 核 64 GB）。</li>
   </main>
   ...
   * 分布式部署 + ODP + 监控配置样例：all-components-min.yaml、all-components.yaml
...
5. 修改配置文件。
   ...
   2. 修改机器的 IP、端口和相关目录，并配置内存相关参数及密码。
      ...
      <main id="notice" type='explain'>
        <h4>说明</h4>
        <p>配置文件中对应 server 中的配置优先级高于 `global`。</p>
      </main>
...
2. 启动 OceanBase 数据库
   ...
   当安装了 Grafana 或 Prometheus 时，会输出 Grafana 或 Prometheus 的访问地址，类似以下输出。在阿里云或其他云环境下，因为安装程序可能无法获取公网 IP，可能输出的是内网地址（在本例中，为 10.10.10.4），此 IP 非公网地址，您需要使用正确的地址。
...
* 方法一：使用 OBD 创建用户租户。
  ...
  命令详细使用方法请参考 OBD 手册 [集群命令组](https://www.oceanbase.com/docs/community-obd-cn-10000000001031933) 中 `obd cluster tenant create` 命令。
