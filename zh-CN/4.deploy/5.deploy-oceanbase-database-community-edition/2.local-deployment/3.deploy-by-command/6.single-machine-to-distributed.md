...
OceanBase 数据库为单机分布式一体化架构，支持单机在线转分布式部署。本文以手动单机部署的数据库为例介绍如何通过命令行将单机部署的集群转为分布式集群。
...
* 服务器中存在 OceanBase 数据库对应版本的 RPM 包，可从 [OceanBase 软件下载中心](https://www.oceanbase.com/softwarecenter) 自行下载。
