...
* OBServer 服务器已完成相关配置。详细信息，请参见 [服务器配置](../../2.preparations-before-deploy/2.configure-servers.md)、[（可选）配置时钟源](../1.configure-a-deployment-environment-command-line/2.configure-host-information.md) 和 [使用 oatcli 初始化 OBServer 服务器](../1.configure-a-deployment-environment-command-line/1.initializing-the-observer-server-using-oatcli.md)。
...
可以在创建租户时即为租户开启仲裁服务，新建租户时为租户开启仲裁服务的详细信息，请参见 [CREATE TENANT](../../../../7.reference/5.sql-reference/1.sql-syntax/1.system-tenants/8.create-tenant.md)。
