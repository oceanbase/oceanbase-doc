...
![](https://obbusiness-private.oss-cn-shanghai.aliyuncs.com/doc/img/observer/ap/ap-intro-feature.png)
...
为了支持 HTAP 混合负载场景下 TP、AP 资源隔离与分析性能诉求，OceanBase 数据库提供列存副本（Column Store Replica，C 副本）。列存副本是一种只读副本类型，具有只读特性，且副本上所有的用户表的基线数据只存储列存格式数据。列存副本是独立 Zone 部署。

列存副本支持两种路由策略：一是通过独立的 ODP 访问列存副本；二是与行存副本共用一个 ODP，由优化器根据规则或代价自动选择数据副本。
支持两种读一致性级别：一是通过弱读方式访问列存副本，当副本延迟不超过 `max_stale_time_for_weak_consistency` 时，数据可正常读取；二是通过强读方式访问列存副本，系统通过 GTS 获取全局一致性快照版本，并检查列存副本回放状态，等待回放完成才会返回数据。

列式副本在副本分布策略上遵循只读副本的一般规则，核心区别在于基础数据存储结构。与普通只读副本类似，列式存储副本不参与主节点选举和日志同步投票过程，但同样完整包含静态数据表、提交日志和内存数据表等核心组件。

有关列存副本的使用指导，参见 [列存副本](../700.reference/100.oceanbase-database-concepts/500.distributed-database-objects/300.data-partitions-and-replicas/300.partition-replica-type/400.columnstore-replica.md)和 [部署与使用列存副本](150.obap-deploy/150.uesing-columnstore-replica.md)。
...
OceanBase 支持嵌套物化视图（Nested Materialized View, Nested MV）与级联刷新能力，可在已有物化视图基础上继续构建新的物化视图，适用于数仓分层加工与中间结果复用场景。结合增量刷新、MLOG 自动化维护与查询改写能力，可降低预计算链路的维护复杂度。
...
在生成式 AI 时代，多模态数据处理需求持续增长。OceanBase 持续增强向量能力与全文搜索能力，并支持通过混合搜索在同一查询中组合全文、向量与结构化过滤条件，以满足知识检索等场景的检索与排序需求。

![](https://obbusiness-private.oss-cn-shanghai.aliyuncs.com/doc/img/observer/ap/vector-skill.png)

### 特殊索引

在传统关系型数据库中，B-Tree、Hash 等索引主要针对结构化数据的精确值查询（如数字、日期等）。但在实际业务中，随着半结构化数据（如 JSON 文档）、非结构化文本（日志、长文本）以及混合检索场景的普及，OceanBase 提供以下特殊索引能力：

+ **全文索引：** 基于倒排索引实现，通过分词技术对文本内容建立关键词映射，适用于日志分析、文档检索等场景。
+ **多值索引：** 针对 JSON 数组字段建立元素级索引，将数组展开为虚拟行记录并构建 B-Tree 索引，提升集合数据查询效率。
+ **搜索索引（Search Index）：** 面向 JSON、单级数组及多种标量类型等列提供索引加速，常用于 索引混合搜索 中的半结构化过滤，以及分析型宽表上多列索引与多索引联合查询（Index Merge）等场景。

上述能力通过差异化数据结构与优化器协同，为文本、半结构化与混合检索场景提供针对性加速。
...
有关全文索引的详细介绍和使用指导，参见 [全文索引（MySQL 模式）](../700.reference/100.oceanbase-database-concepts/400.database-objects/200.database-objects-of-mysql-mode/300.index-of-mysql-mode/320.full-text-indexes-of-mysql-mode-in-concepts.md)。
...
有关多值索引的详细介绍和使用指导，参见 [多值索引](../700.reference/100.oceanbase-database-concepts/400.database-objects/200.database-objects-of-mysql-mode/300.index-of-mysql-mode/100.index-overview-of-mysql-mode.md)。

#### 搜索索引（Search Index）

搜索索引采用倒排思路，将被索引列中的取值（及 JSON 内路径—取值）组织为可快速过滤的索引项；同一索引可覆盖多列，JSON 列还可通过 `WITH` 精细控制路径与类型，适合半结构化过滤、宽表多列过滤，并可与 B-tree、全文、向量等索引配合，支持多索引的 intersect/union 计算，尽量避免非必要回表。

搜索索引常用于两类场景：

+ **混合搜索场景：** 当一张表上同时存在向量、全文多路查询，并包含标量、JSON / 单级数组等列上的过滤条件时，可以利用向量索引、全文索引、搜索索引的 index merge，提升混搜的整体性能。
+ **Ad-hoc 场景：** 搜索索引支持在多个列上创建，从能力上可对齐这类“多列乃至全列建索引”的建模习惯。

在此基础上，当同一张表上存在多个可用索引时，还可结合多索引联合查询：由优化器自动识别可参与的索引组合，选取较优的联合扫描方式，在多列谓词并存时减少全表扫描，贴近分析型产品中的组合查询体验。

有关搜索索引的详细介绍和使用指导，参见 [搜索索引（Search Index）](../700.reference/100.oceanbase-database-concepts/400.database-objects/200.database-objects-of-mysql-mode/300.index-of-mysql-mode/340.search-indexes-of-mysql-mode-in-concepts.md)。
