...
* **宽表**：分析场景常使用列数较多的宽表，单次查询多只访问部分列。采用**列存表**或**列存索引**可按列存储与扫描，减少 IO、利于向量化与 Skip Index；配合分区与表组，宽表可水平打散并与其他表对齐，便于 Join 与并行扫描。详见本文「表类型」「数据分布」及 [AP 场景下的数据表设计与查询优化实践](100.data-design-best-practice/100.strategies-optimizing-storage-structure-query-performance.md)。
...
* **分区策略**：通过**分区表**按分区键（如时间、地域、ID）将数据水平拆分，可做分区裁剪减少扫描量、按分区并行计算，并便于按分区做生命周期管理（归档、清理）。AP 场景建议分区键与常用过滤、聚合维度一致，并配合表组实现多表分区对齐以优化 Join。详见本文「数据分布」及 [OLAP 场景下的表分区设计实践](100.data-design-best-practice/300.best-practice-table-partition.md)。
...
结合**宽表、预聚合、分区策略**等 AP 表设计原则，下表给出典型场景下的表设计组合建议，便于按业务形态选型。具体分区键、分区数、存储格式与索引需根据数据量与查询模式再细化，详见 [AP 场景下的数据表设计与查询优化实践](100.data-design-best-practice/100.strategies-optimizing-storage-structure-query-performance.md)、[OLAP 场景下的表分区设计实践](100.data-design-best-practice/300.best-practice-table-partition.md) 与 [JSON 多值索引与全文索引实践](100.data-design-best-practice/200.best-practices-choosing-special-indexes.md)。
...
| HTAP、以 TP 为主兼有分析 | 行存基表 + 列存索引 + 分区表 | 写入与点查走行存；分析走列存索引，存储可控。详见 [AP 场景下的数据表设计与查询优化实践](100.data-design-best-practice/100.strategies-optimizing-storage-structure-query-performance.md) 中的 **行存基表+列存索引**。 |
