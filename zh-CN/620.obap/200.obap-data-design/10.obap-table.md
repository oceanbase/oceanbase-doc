...
* **delete_insert（全列更新模式）**
  ...
  以 **查询性能优先**，每次更新写入完整行（delete 旧行 + insert 新行），确保每行数据包含完整的列值。该模式支持增量数据（Memtable/Delta SSTable）的 Skip Index，查询时可对增量数据进行过滤下推，若过滤结果不涉及对基线数据的更新，则可分别批处理基线与增量数据，减少读放大。适用于 **增量数据比例较高、需要频繁执行复杂查询或批处理分析** 的 OLAP 场景，但增量数据会占用更多存储空间。

* **partial_update（部分更新模式）**

  每次更新仅记录修改的列（delta），存储空间更节省。查询时需合并多份数据以获取最新值，性能相对较低，但更适合 **高频更新但查询需求低** 的场景（如 OLTP 类业务），或对存储成本敏感的环境。

* **append_only（只 INSERT 更新模式）**

  表的更新模式只允许 `INSERT` 操作，禁止其他所有会产生存储数据修改的 DML 和 DDL 操作，适用于 IoT、监控、金融交易等场景。如果要做过期数据清理可以使用 TTL 功能。

| **特性类别** | **delete_insert 更新模式** | **partial_update 更新模式** | **append_only 更新模式** |
| --- | --- | --- | --- |
| **存储方式** | 每次更新在 SSTable 中写入两行（`DELETE` 和 `INSERT`），包含全列数据。 | 每次更新仅记录修改的列的值，存储空间更节省。 | 在 SSTable 中 `INSERT` 全新数据。 |
| **查询效率** | 支持 Delta SSTable Skip Index，查询时可在 Memtable/Delta SSTable 上计算下推过滤算子；若增量数据过滤结果不包含对基线数据的更新，则可分别批处理基线与增量数据，减少读放大；若过滤结果包含对基线数据更新，则合并增量/基线的记录获取最新值进行投影。 | 查询时需合并多个 Memtable/SSTable 的记录，以获取主键的最新值，可能影响性能。适用于对存储成本敏感且更新操作频繁的场景。 | 启用 `skip_index_level = 1`，实时写入的数据能被高效过滤 |
| **适用场景** | 增量数据比例较高，需要频繁执行复杂查询或批处理分析的 OLAP 场景。 | 高频更新但查询需求较低的 OLTP 场景。 | 不可变性、高效追加、简化查询。|
| **Skip Index 支持范围** | 基线 + 所有增量 SSTable（minor/mini） | 仅基线 SSTable | 基线 + 所有增量 SSTable |

* 更多有关表更新模式的介绍，可参见 [创建表（MySQL 模式）](../../700.reference/300.database-object-management/100.manage-object-of-mysql-mode/200.manage-tables-of-mysql-mode/200.create-a-table-for-mysql-tenant-of-mysql-mode.md) 或 [创建表（Oracle 模式）](../../700.reference/300.database-object-management/200.manage-object-of-oracle-mode/100.manage-tables-of-oracle-mode/200.create-a-table-for-oracle-tenant-of-oracle-mode.md) 下 **指定表的更新模型** 章节。
* 有关增量 Skip Index 与表更新模式的介绍，可参见 [列 Skip Index 属性（MySQL 模式）](../../700.reference/300.database-object-management/100.manage-object-of-mysql-mode/200.manage-tables-of-mysql-mode/250.identify-skip-index-properties-of-mysql-mode.md) 或 [列 Skip Index 属性（Oracle 模式）](../../700.reference/300.database-object-management/200.manage-object-of-oracle-mode/100.manage-tables-of-oracle-mode/250.identify-skip-index-properties-of-oracle-mode.md) 下 **增量 SSTable 生成 Skip Index 聚合信息** 章节。
...
* **全文索引**：OceanBase 支持了兼容 MySQL 的全文搜索能力，通过预先处理文本内容，建立关键词索引，有效提升全文搜索效率。更多有关全文索引的详细信息，请参见 [全文索引](../../700.reference/100.oceanbase-database-concepts/400.database-objects/200.database-objects-of-mysql-mode/300.index-of-mysql-mode/320.full-text-indexes-of-mysql-mode-in-concepts.md)。
...
更多关于索引的详细信息，参见[MySQL 模式选的索引简介](../../700.reference/100.oceanbase-database-concepts/400.database-objects/200.database-objects-of-mysql-mode/300.index-of-mysql-mode/100.index-overview-of-mysql-mode.md)和[Oracle 模式选的索引简介](../../700.reference/100.oceanbase-database-concepts/400.database-objects/100.database-objects-of-oracle-mode/300.index-of-oracle-mode/100.the-index-overview-of-oracle-mode.md)。
