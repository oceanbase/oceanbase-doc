...
* **宽表**：分析场景常使用列数较多的宽表，单次查询多只访问部分列。采用**列存表**或**列存索引**可按列存储与扫描，减少 IO、利于向量化与 Skip Index；配合分区与表组，宽表可水平打散并与其他表对齐，便于 Join 与并行扫描。详见本文「表类型」「数据分布」及 [使用列存与行列混存](100.column-store-overview.md)。
...
* 分区表：OceanBase 数据库可以把普通的表的数据按照一定的规则划分到不同的区块内，将同一区块的数据物理上存储在一起。这种划分区块的表叫做分区表。OceanBase 数据库的基本分区策略包括范围（Range）分区、列表（List）分区、哈希（Hash）分区和 Key 分区。
...
OceanBase支持一级分区表和二级分区表，支持 RANGE 分区、LIST 分区、HASH 分区和 KEY 分区这四种分区类型。
...
* **B-tree 索引（普通索引）**：默认的索引结构，包括 **唯一索引**、**非唯一索引** 和 **函数索引**。适用于等值查询、范围查询、排序和点查。B-tree 索引可以是 **局部索引** 或 **全局索引**，也可以采用行存或列存格式；列存格式的 B-tree 索引即常说的 **列存索引**，适用于 HTAP 场景下的分析型扫描。详见 [索引简介](../../700.reference/100.oceanbase-database-concepts/400.database-objects/200.database-objects-of-mysql-mode/300.index-of-mysql-mode/100.index-overview-of-mysql-mode.md)、[创建索引（MySQL 模式）](../../700.reference/300.database-object-management/100.manage-object-of-mysql-mode/500.manage-indexes-of-mysql-mode/200.create-an-index-of-mysql-mode.md)、[列存](../../700.reference/100.oceanbase-database-concepts/900.storage-architecture/200.data-storage/320.columnstore-engine.md)。其中，**函数索引** 适用于按表达式过滤或排序（如 `MONTH(date_col)`、`UPPER(name)`），避免逐行计算。参见 [函数索引支持的系统函数列表](../../700.reference/300.database-object-management/100.manage-object-of-mysql-mode/500.manage-indexes-of-mysql-mode/500.function-index-list-of-supported-functions-of-mysql-mode.md) 与 [不支持的函数列表](../../700.reference/300.database-object-management/100.manage-object-of-mysql-mode/500.manage-indexes-of-mysql-mode/600.function-index-list-of-not-supported-functions-of-mysql-mode.md)。
  * **适用场景**：等值/范围条件（`WHERE col = ?`、`BETWEEN`、`IN`）；排序与分页（`ORDER BY` 结合过滤）；在分区键或常用过滤列上建局部索引以配合分区裁剪；主键/唯一键或覆盖索引减少回表，适用于维表关联、小结果集筛选等。

* **全文索引**：基于倒排索引与分词，对 `CHAR`/`VARCHAR`/`TEXT` 列建立关键词映射。**仅支持局部索引**。详见 [全文索引](../../700.reference/100.oceanbase-database-concepts/400.database-objects/200.database-objects-of-mysql-mode/300.index-of-mysql-mode/320.full-text-indexes-of-mysql-mode-in-concepts.md)。
  * **适用场景**：适用于企业内部知识库与文档检索、新闻/法律文书等大段文本搜索、日志分析中按关键词定位等。执行计划中可见 `TEXT RETRIEVAL SCAN` 表示命中全文索引。

* **JSON 多值索引**：OceanBase 数据库的 MySQL 模式支持多值索引功能，可在数组或集合上创建索引，提高基于 JSON 数组元素搜索的查询效率。**仅支持局部索引**。详见 [多值索引](../../700.reference/100.oceanbase-database-concepts/400.database-objects/200.database-objects-of-mysql-mode/300.index-of-mysql-mode/100.index-overview-of-mysql-mode.md) 与 [创建索引（MySQL 模式）](../../700.reference/300.database-object-management/100.manage-object-of-mysql-mode/500.manage-indexes-of-mysql-mode/200.create-an-index-of-mysql-mode.md)。
...
AP 场景下 **列存索引**、**行存基表 + 列存索引** 等组合，参见 [列存表与列存索引实践](800.data-design-best-practice/500.column-store-index-and-table-usage.md)；**全文索引、JSON 多值索引、空间索引、向量索引** 等特殊索引选型，参见 [JSON 多值索引与全文索引实践](800.data-design-best-practice/200.best-practices-choosing-special-indexes.md)。更多语法与实现细节，参见 [MySQL 模式索引简介](../../700.reference/100.oceanbase-database-concepts/400.database-objects/200.database-objects-of-mysql-mode/300.index-of-mysql-mode/100.index-overview-of-mysql-mode.md) 与 [Oracle 模式索引简介](../../700.reference/100.oceanbase-database-concepts/400.database-objects/100.database-objects-of-oracle-mode/300.index-of-oracle-mode/100.the-index-overview-of-oracle-mode.md)。
...
结合**宽表、预聚合、分区策略**等 AP 表设计原则，下表给出典型场景下的表设计组合建议，便于按业务形态选型。具体分区键、分区数、存储格式与索引需根据数据量与查询模式再细化，详见 [列存与行列混存概述](100.column-store-overview.md)、[AP 场景下的主键设计实践](800.data-design-best-practice/100.strategies-optimizing-storage-structure-query-performance.md)、[OLAP 场景下的表分区设计实践](800.data-design-best-practice/300.best-practice-table-partition.md)、[列存表与列存索引实践](800.data-design-best-practice/500.column-store-index-and-table-usage.md) 与 [JSON 多值索引与全文索引实践](800.data-design-best-practice/200.best-practices-choosing-special-indexes.md)。
...
| HTAP、以 TP 为主兼有分析 | 行存基表 + 列存索引 + 分区表 | 写入与点查走行存；分析走列存索引，存储可控。详见 [列存表与列存索引实践](800.data-design-best-practice/500.column-store-index-and-table-usage.md)。 |
| 需要 TP/AP 物理隔离 | F/R 上行存或行列混存 + 列存副本（C 副本） | AP 流量走 C 副本弱读，TP 走 F/R；无需在 F/R 上为分析建列存索引也可获得列存扫描。部署见 [AP 部署概述](../150.obap-deploy/10.obap-deploy-overview.md)。 |
...
* [使用列存与行列混存](100.column-store-overview.md)
* [AP 场景下的主键设计实践](800.data-design-best-practice/100.strategies-optimizing-storage-structure-query-performance.md)
* [列存表与列存索引实践](800.data-design-best-practice/500.column-store-index-and-table-usage.md)
* [OLAP 场景下的表分区设计实践](800.data-design-best-practice/300.best-practice-table-partition.md)
* [JSON 多值索引与全文索引实践](800.data-design-best-practice/200.best-practices-choosing-special-indexes.md)
* [动态分区](500.data-distribution/200.dynamic-partition.md)
