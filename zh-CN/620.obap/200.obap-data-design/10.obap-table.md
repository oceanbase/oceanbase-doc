...
| **查询效率** | 支持 Delta SSTable Skip Index，查询时可在 Memtable/Delta SSTable 上计算下推过滤算子；若增量数据过滤结果不包含对基线数据的更新，则可分别批处理基线与增量数据，减少读放大；若过滤结果包含对基线数据更新，则合并增量/基线的记录获取最新值进行投影。 | 查询时需合并多个 Memtable/SSTable 的记录，以获取主键的最新值，可能影响性能。适用于对存储成本敏感且更新操作频繁的场景。 |
| **适用场景** | 增量数据比例较高，需要频繁执行复杂查询或批处理分析的 OLAP 场景。 | 高频更新但查询需求较低的 OLTP 场景。 |
| **Skip Index 支持范围** | 基线 + 所有增量 SSTable（minor/mini） | 仅基线 SSTable |
