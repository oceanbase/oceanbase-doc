...
OceanBase 数据库支持对内表（本地表）以及外部 Catalog 表（如 Hive、Iceberg 格式的外表）进行统计信息的收集与管理，涵盖表级、列级基础统计信息和直方图等类型，并提供定时自动收集、手动收集、在线收集、异步自动收集等多种策略。
