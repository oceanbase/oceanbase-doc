...
为满足 TP 与 AP 混合负载等场景，OceanBase 支持在独立 Zone 上部署**只读列存副本（C 副本）**。访问方式与 **OceanBase 版本** 有关：**V4.6.0 之前** 通常需经**独立 ODP** 配置弱读与路由项访问 C 副本；**V4.6.0 及以上** 可在部署 C 副本后，通过 **`ap_query_route_policy` 等自动路由**直连或经普通连接使用，**不强制**再为 C 副本单独部署 ODP。两种形态、操作步骤与自动路由说明，见 **[列存副本的使用与自动路由](150.uesing-columnstore-replica.md)**；架构选型还可对照 [典型 AP 架构部署指南](50.ap-architecture-deployment-guide.md)。

![](https://obbusiness-private.oss-cn-shanghai.aliyuncs.com/doc/img/observer/ap/ap-deploy-1.png)

从日志流角度看，C 副本上同一日志流的用户表在 major sstable 层次为纯列存；其余行为与只读副本类似（不参与选举与投票，具备 sstable/clog/memtable 等）。在创建或变更租户时，可通过 **Locality** 指定 C 副本；更多副本管理的详细信息，参见[列存副本的使用与自动路由](150.uesing-columnstore-replica.md)**；架构选型还可对照 [典型 AP 架构部署指南](50.ap-architecture-deployment-guide.md)。
