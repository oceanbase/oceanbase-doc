...
* V4.6.0 之前，需要配置独立的 ODP，并通过 ODP 的配置项设置路由策略。
  ...
  即配置**单独一套 ODP**，在它上面配弱读、路由和 `init_sql`（常见 **`ob_route_policy = COLUMN_STORE_ONLY`**），把这条链路 **固定打到 C 副本**

* V4.6.0 及以上，C 副本不需要部署独立的 ODP，由数据库自动选择路由策略，也就是 **列存副本自动路由**（也常叫 **自动路由**）。

  **和访问 F/R 共用 ODP** 或 **直连 OBServer**，由 **OBServer** 里的 **`ap_query_*`（如 `ap_query_route_policy`）** 在只读规划阶段决定是否 **尝试** C 副本上的 **列存计划**。
...
### V4.6.0 及以上：直连 OBServer 并使用自动路由
...
从 **OceanBase V4.6.0** 起，可通过系统变量与 Hint 控制：只读查询是否 **尝试** 在 **C 副本** 上规划并执行。**`ap_query_route_policy`** 为 **列存副本自动路由** 的主开关——在 **OBServer** 的只读路径规划阶段，控制优化器是否尝试识别符合条件的分析型查询并生成 **列存侧** 执行计划（详见 [路由判定规则](300.ap-query-column-replica-routing-rules.md)）。
