...
为满足 TP 与 AP 混合负载等场景，OceanBase 数据库提供**列存副本**（COLUMNSTORE，简称 **C 副本**）。C 副本可部署在独立 Zone 上；用户表包括复制表、不包括索引表、内部表、系统表）在该副本中以列存格式存储。

本篇文档将介绍 C 副本的基本概念与限制、在租户中的部署步骤，以及路由方式的选型与操作要点。从 **OceanBase V4.6.0** 起，列存副本支持自动路由。
...
选定后，请先阅读 **认识列存副本** 对照注意项，再阅读 **使用列存副本** 完成部署与访问配置。
...
更多关于副本的概念，参见[列存副本](../../700.reference/100.oceanbase-database-concepts/500.distributed-database-objects/300.data-partitions-and-replicas/300.partition-replica-type/400.columnstore-replica.md)和[日志流和副本概述](../../700.reference/100.oceanbase-database-concepts/500.distributed-database-objects/300.data-partitions-and-replicas/100.data-partitioning-and-partitioned-replica-overview.md)。

### 两种访问方式

按 **OceanBase 版本** 与 **是否需要专用 AP 入口**，常见两种访问方式如下。
...
**方式一：** 在 ODP（`proxysys`）上配置 **弱读**，并把弱读请求 **限定到 C 副本所在 Zone 或 ColumnStore 副本类型**，常配合会话初始化中的 **`ob_route_policy = COLUMN_STORE_ONLY`**，以便生成列存侧执行计划。具体命令见 **二、使用列存副本** 中方式一小节。

**方式二：** 业务使用常规连接串连接 **OBServer**，或经 **普通 ODP** 转发（只要 SQL 到达 OBServer）。是否在 C 副本上规划只读部分，由 OBServer 上的配置项 `ap_query_*` 及 [路由判定规则](300.ap-query-column-replica-routing-rules.md) 决定，**无需**再配置方式一中的 ODP 路由项。

### 使用限制

* C 副本与 **F** 或 **R** 副本 **不支持** 互相转换。
* **物理恢复** 不支持恢复 C 副本；租户的 Locality 若包含 C 副本，恢复将失败。
* R 副本上的查询 **不允许** 转发到 C 副本；C 副本上的用户表查询 **不允许** 转发到 F/R 副本；F 副本可以将查询转发到 C 副本。
...
**部署 C 副本**（本节步骤一）与 **是否部署独立 ODP** 无必然关系：完成 Locality 中的 C 副本后，V4.6.0 版本之前可以采用**方式一**，才需要继续部署并配置 **独立 ODP**。V4.6.0 版本及以后裁员**方式二** 无需为列存单独新增 ODP 步骤。
...
从 **OceanBase V4.6.0** 起，可通过系统变量与 Hint 控制只读查询是否在 **C 副本** 上规划与执行列存副本自动路由。配置项 `ap_query_route_policy` 为列存副本自动路由的主开关：在只读路径规划阶段，控制是否尝试识别分析型查询并生成列存侧计划。

**何时尝试列存路径**、与 AP 查询的关系、常见否例等，见 [AP 查询与列存副本路由判定规则](300.ap-query-column-replica-routing-rules.md)。

<main id="notice" type='notice'>
  <h4>说明</h4>
  <p>从 V4.6.0 起，<strong>不强制</strong>为使用 C 副本而单独部署 ODP 并配置二、方式一中的 ODP 项；直连 OBServer 或经普通 ODP 时，可在本节通过会话/全局变量控制列存选择。</p>
</main>
