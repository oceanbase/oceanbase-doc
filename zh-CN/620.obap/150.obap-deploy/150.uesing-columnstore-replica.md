...
* V4.6.0 及以上，C 副本不需要部署独立的 ODP，由数据库自动选择路由策略，也就是 **列存副本自动路由**（也常叫 **自动路由**）。
  ...
  **和访问 F/R 共用 ODP**，由 **OBServer** 里的 **`ap_query_*`（如 `ap_query_route_policy`）** 在只读规划阶段决定是否 **尝试** C 副本上的 **列存计划**。
...
适用于已完成 **C 副本部署**、且 **不采用** 上文 **「V4.6.0 之前：需经独立 ODP 访问 C 副本」** 所述独立 ODP 弱读与路由项（将流量固定打到 C 副本）的场景。请使用 **与访问 F/R 相同的连接方式**：经 **同一套业务 ODP**。
...
<main id="notice" type='notice'>
  <h4>说明</h4>
  <p>从 V4.6.0 起，<strong>不强制</strong>单独部署一套仅面向列存访问的独立 ODP，也不强制按上文 <strong>「V4.6.0 之前：需经独立 ODP 访问 C 副本」</strong> 配置其中的弱读与路由项；直连 OBServer 或经与 F/R 共用的业务 ODP 时，可在本节通过会话/全局变量控制列存选择。</p>
</main>
...
二者 **层次不同**，不是简单替代关系。**经独立 ODP 访问 C 副本** 时，常在 `init_sql` 里设置 **`ob_route_policy = COLUMN_STORE_ONLY`**，与 ODP 路由项共同把流量导向列存；**列存副本自动路由** 则以 OBServer 侧 **`ap_query_*`** 变量为主。若同时设置，**语义与优先级以当前 OceanBase 版本说明为准**。详见 [ob_route_policy](../../700.reference/800.configuration-items-and-system-variables/200.system-variable/300.global-system-variable/8900.ob_route_policy-global.md)。
