...
OceanBase 数据库在 V4.6.0 版本对增量旁路导入进行了优化，新增一种增量旁路导入策略。新的增量旁路导入策略把数据分成两部分，和已有数据主键冲突的部分写入 Mini SSTable，不冲突的部分写到另外一份 Major SSTable 中。这样对于冲突数据少的情况，新的增量旁路导入策略可以做到多次导入，查询性能也不会差。

OceanBase 数据库在 V4.6.0 版本新增隐藏配置项 `_enable_inc_major_direct_load`（默认值为 `false`），用于控制增量旁路导入使用新的策略还是使用原来（旧）的导入策略。

**使用示例：**

<main id="notice" type='notice'>
  <h4>注意</h4>
  <p>Oracle 模式下，设置隐藏配置项时，在语法上隐藏配置项需加双引号。</p>
</main>

* 使用旧的增量旁路导入策略。

    ```shell
    obclient> ALTER SYSTEM SET _enable_inc_major_direct_load = false;
    ```

* 开启新的增量旁路导入策略。

    ```shell
    obclient> ALTER SYSTEM SET _enable_inc_major_direct_load = true;
    ```
