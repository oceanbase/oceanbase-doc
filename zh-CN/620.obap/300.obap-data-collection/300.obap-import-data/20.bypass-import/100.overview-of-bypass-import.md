...
| 导入过程 | <ol><li>数据经过类型转换后，按主键排序（若表有主键）。</li><li>直接写入 Major SSTable。</li><li>不重用现有宏块，逐行处理数据，避免与现有数据冲突。</li></ol>| <ol><li>数据经过类型转换后，按主键排序（若表有主键）。</li><li>和已有数据主键冲突的部分写入 Mini SSTable，不冲突的部分写到另外一份 Major SSTable 中。</li><li>不重用现有宏块，逐行处理数据，避免与现有数据冲突。</li></ol>|
| 写入位置 | Major SSTable | 和已有数据主键冲突的部分写入 Mini SSTable，不冲突的部分写到另外一份 Major SSTable 中。|
| 性能特征 | 导入期间锁定表，避免其他操作干扰。| 适用于主键冲突较少的场景，主键冲突较多的时候，性能会下降。|
| 锁机制   | 对于指定表的导入会加表锁，指定分区的导入会加分区锁。| 对于指定表的导入会加表锁，指定分区的导入会加分区锁。|
| 查询性能 | 数据直接写基线，查询性能优。| 对于无冲突数据的情况下，数据直接写基线，查询性能优，对于有冲突数据的情况下，冲突数据会写增量，查询性能良好。|
