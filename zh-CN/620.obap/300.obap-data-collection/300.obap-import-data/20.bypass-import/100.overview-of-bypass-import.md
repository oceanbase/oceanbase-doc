...
| 导入过程 | <ol><li>数据经过类型转换后，按主键排序（若表有主键）。</li><li>直接写入 Major SSTable。</li><li>不重用现有宏块，逐行处理数据，避免与现有数据冲突。</li></ol>| 和已有数据主键冲突的部分写入 Mini SSTable，不冲突的部分写到另外一份 Major SSTable 中。|
