...
* **有 C 副本**：TP 流量走主 ODP，访问 F/R 副本（强读/写）；AP 流量走**独立 ODP**，配置弱读（如 `obproxy_read_consistency = 1`）并将请求仅路由到 C 副本所在 Zone（如 `proxy_primary_zone_name`、`route_target_replica_type = 'ColumnStore'`、`init_sql='set @@ob_route_policy = COLUMN_STORE_ONLY'` 等），实现 TP/AP 物理隔离。详见 [部署与访问列存副本](150.obap-deploy/150.uesing-columnstore-replica.md)。
