...
* **读一致性**：C 副本相对日志源存在追平滞后；经独立 ODP 等方式访问 C 副本的分析查询通常按 **弱读** 规划。若要求**与 TP 完全同一份日志、无 C 副本滞后**，分析宜优先在 **F/R 上的列存表或行列混存表** 上完成。
...
* **列存副本（C 副本）**：C 副本不参与 Paxos，通过**异步日志**追赶 F/R 并回放，数据相对日志源**存在滞后**；经独立 ODP 访问 C 副本的分析读多为**弱一致**规划。若要求**与 TP 完全同一份日志、无 C 副本滞后**，分析仍宜优先在 **F/R 上的列存表或行列混存表** 上完成。
...
* **有 C 副本**：典型做法是 TP 走访问 F/R 的入口（强读/写）；AP 走**独立 ODP**，在 ODP 侧配置弱读并将请求路由到 C 副本（如 `proxy_primary_zone_name`、`route_target_replica_type = 'ColumnStore'`、`init_sql='set @@ob_route_policy = COLUMN_STORE_ONLY'` 等），实现 TP/AP 与 C 副本的链路隔离。运维与副本管理参见 [部署与使用列存副本](150.obap-deploy/150.uesing-columnstore-replica.md)。
...
* 从 V4.3.5 BP1 起支持部署**多个 C 副本**（建议最多 3 个），可在多 Zone 部署 C 副本，通过多套独立 ODP 及路由配置分散 AP 读负载，实现冗余与高可用。

### 列存副本接入与路由 FAQ

#### 访问 C 副本时为何通常需要独立 ODP？

在 **OceanBase V4.3.5** 上，将 AP 流量与 TP 链路隔离、并稳定路由到 C 副本的常见做法，是为 C 副本**单独部署一套 ODP**，在该 ODP 上配置弱读与列存路由策略。详细步骤参见 [部署与使用列存副本](150.obap-deploy/150.uesing-columnstore-replica.md)。

#### 独立 ODP 应该部署在哪里？是否必须使用独占服务器？

不是强制要求。独立 ODP 可部署在任意与集群网络互通的主机上。文档建议单机部署，主要是为了降低与 OBServer 的资源争抢风险，而不是协议或产品限制。

#### `proxy_idc_name` 与列存副本路由是什么关系？

`proxy_idc_name` 主要用于机房亲和（LDC）路由；是否路由到 C 副本属于副本选择维度。两者可以同时使用，但应分别配置与验证。

详细信息参见 [ODP 读写分离与路由策略](../700.reference/1200.database-proxy/700.route-management/400.odp-routing-policy/100.read-write-splitting.md)、[LDC 路由](../700.reference/1200.database-proxy/700.route-management/300.ldc-route.md)。
...
旁路导入是一种加快数据导入，并且能够加速数据查询的数据导入方式。大表数据导入，推荐使用旁路导入方式。目前，`load data` 命令、`insert into select` 语句支持旁路导入。旁路导入的详细使用方式参见 [旁路导入概述](300.obap-data-transfer/300.data-import/100.data-import-overview.md)。
