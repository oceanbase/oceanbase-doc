...
与 [文件外部表](../400.file-external-table.md) 不同，ODPS 外部表不通过 `LOCATION` 指向文件路径，而是通过 MaxCompute API 连接远端 ODPS 项目中的表。OceanBase 在本地保存外表定义与列映射，查询或写入时调用 ODPS Storage API 或 Tunnel API 与 MaxCompute 交互。
