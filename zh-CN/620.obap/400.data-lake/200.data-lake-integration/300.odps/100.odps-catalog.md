...
OceanBase 从 **V4.3.5 BP2** 起（MySQL 模式）支持 **ODPS Catalog**。
...
环境依赖的详细信息，参见 [部署 OceanBase 数据库 JAVA SDK 环境](../../../../400.deploy/300.deploy-oceanbase-enterprise-edition/500.deploy-the-ob-java-sdk-environment.md)。
...
| API_MODE | 否 | 调用 API 模式：`tunnel_api`（默认）或 `storage_api`。V4.3.5 BP3 及以上支持。详见下文。 |
| SPLIT | 条件 | 使用 `storage_api` 时指定分片方式：`byte` 或 `row`。V4.3.5 BP3 及以上支持。 |
