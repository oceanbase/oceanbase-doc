...
* OceanBase 数据库：详细信息，参考 [快速入门指南](/zh-CN/2.quick-start/1.overview-of-quick-start.md)。
...
2. 部署成功后，新建执行 TPC-H 测试的租户及用户（请勿直接使用 sys 租户进行测试）。将租户的 `primary_zone` 设置为 `RANDOM`。租户详情，参考 [创建用户租户。](/zh-CN/6.administrator-guide/3.basic-database-management/5.tenant-management/2.create-a-user-tenant.md)
