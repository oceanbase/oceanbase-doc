...
* OceanBase 数据库：详细信息，参考 [快速入门指南](/en-US/2.quick-start/1.overview-of-quick-start.md)。
...
* 租户规格：
  ...
  * 上文的租户规格是基于 [OceanBase TPC-H 性能测试报告](/en-US/3.performance-whitepaper/2.tpc-h-benchmark-report-of-oceanbase-database.md) 中的硬件配置进行配置的，您需根据自身数据库的硬件配置进行动态调整。详细操作，参考 [修改租户](/en-US/6.administrator-guide/3.basic-database-management/5.tenant-management/5.modify-a-tenant.md)。
...
2. 部署成功后，新建执行 TPC-H 测试的租户及用户（请勿直接使用 sys 租户进行测试）。将租户的 `primary_zone` 设置为 `RANDOM`。租户详情，参考 [创建用户租户。](/en-US/6.administrator-guide/3.basic-database-management/5.tenant-management/2.create-a-user-tenant.md)
...
测试结果可参考 [OceanBase TPC-H 性能测试报告](/en-US/3.performance-whitepaper/2.tpc-h-benchmark-report-of-oceanbase-database.md)。
