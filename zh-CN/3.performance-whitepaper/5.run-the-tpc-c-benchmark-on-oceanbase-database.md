...
* 租户规格：
  ...
  > **注意**
  >
  > * 上文的租户规格是基于 [OceanBase TPC-C 性能测试报告](../3.performance-whitepaper/6.tpc-c-benchmark-report-of-oceanbase-database.md) 中的硬件配置进行配置的，您需根据自身数据库的硬件配置进行动态调整。详细操作，参考 [修改租户](../6.administrator-guide/2.basic-database-management/4.manage-tenants/4.administrator-guide-manage-tenants/2.modify-a-tenant.md)。
  >
  > * 进行性能测试时，租户需配置最大可用资源。
...
* OceanBase 集群规模为 1：1：1。部署成功后，新建执行 TPC-C 测试的租户及用户（sys租户是管理集群的内置系统租户，请勿直接使用 sys 租户进行测试）。将租户的 `primary_zone` 设置为 `RANDOM`。`RANDOM` 表示新建表分区的 Leader 随机到这 3 台机器。租户详情，参考 [创建用户。](../6.administrator-guide/2.basic-database-management/4.manage-tenants/5.manage-users-and-privileges/2.create-a-user.md)
...
由于 Benchmark SQL5 不支持 OceanBase 数据库的 TPC-C 测试，本节将详细介绍如何通过修改 Benchmark SQL5 部分源码支持 OceanBase 数据库。
...
1. ODP 调优，请在 sys 租户下执行。
   ...
   以 `obclient -h$host_ip -P$host_port -uroot@sys -A` 命令从操作系统登录，并执行如下 SQL 语句进行调优。
...
3. 在 OBServer 部署之后，在进行测试 TPCC 的租户下执行。
   ...
   以 `obclient -h$host_ip -P$host_port -u$user@$tenant -p$password -A` 命令从操作系统登录，并执行如下 SQL 语句进行设置。
...
> **说明**
>
> 测试结果可参考 [OceanBase TPC-C 性能测试报告](../3.performance-whitepaper/../3.performance-whitepaper/6.tpc-c-benchmark-report-of-oceanbase-database.md)。
...
* 如何停止 Benchmark SQL 工具。
  ...
  强行停止：ps auxf \| grep java -- 删除 Java 进程、删除 runBenchmark 进程。
