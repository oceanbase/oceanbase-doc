...
1. 编辑 `config/db.php` 文件，配置 OceanBase 数据库连接：
...
2. 在 `config/web.php` 中确保数据库组件已配置：
...
1. 创建 User 模型 `models/User.php`：
    ...
    class User extends ActiveRecord
    {
        ...
        {
            $db = Yii::$app->db;
            $tableName = $db->schema->getRawTableName('{{%user}}');
...
2. 在浏览器中访问 `http://localhost:8080/user` 或使用 curl 测试：
...
3. 查看 `runtime/logs/app.log` 文件，您应该能看到类似以下的日志输出：
    ...
    2025-06-10 11:30:16 [info][application] 查询数据: {"id":1,"username":"oceanbase_user1654857016","email":"user1654857016@oceanbase.com","age":35,"created_at":1654857016}
...
4. 响应结果示例：
    ...
    ```json
    {
        ...
        "data": {
            ...
            "sampleUser": {
                "id": 1,
                "username": "oceanbase_user1654857016",
                "email": "user1654857016@oceanbase.com",
                "age": 35,
                "created_at": 1654857016
            }
        }
    }
    ```
