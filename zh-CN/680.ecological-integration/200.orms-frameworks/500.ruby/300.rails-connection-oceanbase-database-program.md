...
2. 修改 Gemfile 文件，确保包含 MySQL2 gem：
...
4. 配置 `config/database.yml` 文件：
    ...
    development:
      <<: *default
      database: oceanbase_development

    test:
      <<: *default
      database: oceanbase_test

    production:
      <<: *default
      database: oceanbase_production
      username: oceanbase_prod
      password: <%= ENV['OCEANBASE_DATABASE_PASSWORD'] %>
    ```
