...
- MySQL 客户端开发库（如 `libmysqlclient-dev` 或 `mariadb-connector-c`）
...
1. 创建数据库和表：
   ...
   -- 连接到 OceanBase 数据库
   -- 创建数据库
   CREATE DATABASE IF NOT EXISTS diesel_demo;
...
```bash
export DATABASE_URL=mysql://username:password@localhost:2881/diesel_demo
```
