...
OceanBase 数据库从 V4.3.3 版本开始，提供了向量类型存储、向量索引、embedding 向量检索的能力。
可以利用 OpenAI 的 API 接口，将向量化后的数据存储在 OceanBase 数据库，然后使用 OceanBase 数据库的向量检索能力查询相关数据。
...
* 确保您已经在实例中设置了 `ob_vector_memory_limit_percentage` 配置项，以启用向量检索功能。推荐设置值为 `30`，如需更精确设置此配置项，请参考 [ob_vector_memory_limit_percentage](../../700.reference/800.configuration-items-and-system-variables/100.system-configuration-items/400.tenant-level-configuration-items/6150.ob_vector_memory_limit_percentage.md) 计算此值。
