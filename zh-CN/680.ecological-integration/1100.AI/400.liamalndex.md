...
# OceanBase 数据库Vector 与 LlamaIndex 集成

OceanBase 数据库从 V4.3.3 版本开始，提供了向量类型存储、向量索引、embedding 向量检索的能力。可以将向量化后的数据存储在 OceanBase 数据库，供下一步的检索使用。

LlamaIndex 是一个使用 LLM（包括代理和工作流）构建上下文增强生成 AI 应用程序的框架。它提供了数据连接器、数据索引、代理、可观察性/评估集成和工作流等能力。

本教程结合通义千问 API，演示如何将 OceanBase 数据库中的向量检索功能、通义千问与 LlamaIndex 集成实现文档问答。
...
* 准备通义千问 API 密钥。
* 保您已经在实例中设置了 `ob_vector_memory_limit_percentage` 配置项，以启用向量检索功能。推荐设置值为 `30`，如需更精确设置此配置项，请参考 [ob_vector_memory_limit_percentage](../../700.reference/800.configuration-items-and-system-variables/100.system-configuration-items/400.tenant-level-configuration-items/6150.ob_vector_memory_limit_percentage.md) 计算此值。
