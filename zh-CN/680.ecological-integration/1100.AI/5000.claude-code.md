...
[Claude Code](https://windsurf.com/) 是一款支持 MCP 协议的 AI 编码助手。
...
1. 打开 VS Code 工具终端，执行 claude mcp add-json 命令。
   ...
   `/path/to/your/mcp-oceanbase` 需要替换为 `mcp-oceanbase` 文件夹的绝对路径，`OB_HOST`、`OB_PORT`、`OB_USER`、`OB_PASSWORD`、`OB_DATABASE` 需要替换为自己数据库的对应信息即可：
