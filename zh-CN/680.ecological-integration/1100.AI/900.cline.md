...
3. 填写配置文件。
    ...
    在上一个步骤中打开的 `cline_mcp_settings.json` 文件中，输入下面的配置信息，然后保存该文件。`/path/to/your/mcp-oceanbase` 需要替换为 `mcp-oceanbase` 文件夹的绝对路径，`OB_HOST`、`OB_PORT`、`OB_USER`、`OB_PASSWORD`、`OB_DATABASE` 需要替换为自己数据库的对应信息。
