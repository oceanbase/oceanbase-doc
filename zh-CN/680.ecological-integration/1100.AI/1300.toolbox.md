...
# OceanBase MCP Server 与 MCP Toolbox 集成
...
[MCP Toolbox for Databases](https://github.com/googleapis/genai-toolbox) 是一个开源的数据库 MCP 服务器。它通过处理连接池、身份验证等复杂问题，让您能够更轻松、更快速、更安全地开发工具。

本文将展示如何使用 MCP Toolbox 无需编程构建 OceanBase MCP 服务器。

## 前提条件

* 您的环境中有可用的 OceanBase 数据库实例。

* 如果您目前没有可用的实例，可以 [开通 OB Cloud 云数据库的免费试用实例](https://www.oceanbase.com/free-trial#trial)，点击查看 [免费试用的规则和开通方法](https://www.oceanbase.com/docs/common-oceanbase-cloud-1000000000703494)。

* 或者，您可以使用 Docker 快速部署一个独立的 OceanBase 数据库进行测试。

* 您的环境中已存在可以使用的 MySQL 兼容模式租户和 MySQL 数据库和账号，并已对数据库账号授予读写权限。如需创建，详情参见 [创建账号](https://www.oceanbase.com/docs/common-oceanbase-cloud-1000000001018091)和 [创建数据库（仅 MySQL）](https://www.oceanbase.com/docs/common-oceanbase-cloud-1000000001018092)。
...
您可以使用以下命令快速部署一个独立的 OceanBase 数据库进行测试：
...
### 使用 OceanBase Cloud（推荐用于生产）

或者，您可以在 OceanBase Cloud 上 1 分钟内免费试用实例。

## 步骤二：下载 Toolbox 并开始使用
...
创建一个 `tools.yaml` 文件并写入以下内容。请将连接信息替换为您的 OceanBase 地址。此示例配置展示了如何使用 Toolbox 通过 MCP 工具获取 ASH 报告和集群信息。您可以自定义自己的工具。
...
4. 点击连接按钮。
5. 在工具选项卡中点击列出工具，尝试 `get-all-server-nodes` 工具，您应该看到如下输出：
...
测试成功后，您现在拥有了自己的 MCP 服务器，无需编写任何代码。您可以集成任何 MCP 客户端与大语言模型一起工作，让您的工作更高效。
...
* 自定义您的工具配置，添加更多 OceanBase 相关的 SQL 查询
* 集成到您的工作流程中，提高数据库管理和分析效率
* 探索更多 MCP 工具和功能

<main id="notice" type='notice'>
<h4>注意</h4>
<p>需要创建一个视频来展示如何使用 ASH 报告工具分析 OceanBase 性能。我不确定这篇文章将如何发布，或者是否支持视频。</p>
</main>