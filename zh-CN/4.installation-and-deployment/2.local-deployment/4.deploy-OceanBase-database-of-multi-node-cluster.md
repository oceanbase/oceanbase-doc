...
执行`which obd` 和`which obclient` 检测是否安装成功, 如果可以找到obd和obclient 则表示安装成功
...
1. 选择配置文件
    ...
    所有配置文件放置在~/.oceanbase-all-in-one/conf 目录下, 也可以从 [GitHub obdeploy](https://github.com/oceanbase/obdeploy/tree/master/example) 上下载最新的配置文件。根据您的实际情况选择相应的配置文件：
...
1. 部署 OceanBase 集群
    ...
    all-in-one package 提供的组件已经经过相互适配测试, 为官方推荐版本, 当用户需要安装一个历史版本或者自己编译的版本时, 则需要提前将安装的目标rpm clone 到OBD仓库当中 , 类似参考如下
