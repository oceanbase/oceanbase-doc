...
| Grafana    | 10.10.10.4 | 数据可视化工具，如果您已经安装或计划安装 OCP，则无需安装 Grafana     |
| Prometheus | 10.10.10.4 | 系统监控和报警系统，如果您已经安装或计划安装 OCP，则无需安装 Prometheus      |
...
4. 选择配置文件
   ...
   * 分布式部署 + ODP + OBAgent + Grafana + Prometheus 配置样例：all-components-min.yaml、all-components.yaml
...
5. 修改配置文件。
   ...
   4. 修改监控相关组件的 IP 和 home_path。
      ...
      | ob_monitor_status  | 必选     | active      | 设置 OceanBase 监控指标采集状态，active 表示开启，inactive 表示关闭。 |
      | login_password  | 必选       | 无        | Grafana 登录密码。 |
...
2. 启动 OceanBase 集群
   ...
   当安装了 Grafana 或 Prometheus 时，会输出 Grafana 或 Prometheus 的访问地址。在阿里云或其他云环境下，安装程序可能因无法获取公网 IP 而输出内网地址，此 IP 非公网地址，您需要使用正确的地址。
