...
4. 下载安装所需 `jar` 包：
   ...
   - [`spark-connector-oceanbase`](https://obbusiness-private.oss-cn-shanghai.aliyuncs.com/doc/img/observer/tutorial/spark/spark-connector-oceanbase-3.4_2.12-1.1.jar)
   - [`mysql-connector-j`](https://obbusiness-private.oss-cn-shanghai.aliyuncs.com/doc/img/observer/tutorial/spark/mysql-connector-j-8.2.0.jar)
...
1. **存储与计算分离**：  
   ...
   - **OceanBase** 负责海量数据的高并发存储与查询。  
   - **Spark** 负责分布式计算任务，提升分析效率。
...
3. **扩展性**：  
   ...
   - 可进一步集成流式计算（如 Spark Streaming）实现实时分析。  
   - 结合机器学习（如 Spark MLlib）构建预测模型。  
