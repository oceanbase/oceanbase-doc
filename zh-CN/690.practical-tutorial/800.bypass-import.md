...
[ClickBench](https://benchmark.clickhouse.com/) 是 ClickHouse 提出的一个AP benchmark。通过这个 AP Benchmark 可以测试 OceanBase 数据库在 AP 场景下的查询性能。在运行这个 Benchmark 之前，首先需要导入数据。**导入数据** 这一步可以体验 OceanBase 数据库导入数据的性能。当然在 TPCH，TPCDS 等 AP Benchmark 上都可以使用旁路导入准备数据。本篇教程将向您展示如何使用旁路导入性能导入数据，体验高效的性能。
...
- 传统路径：橙色箭头所示的传统路径需要经过 SQL 查询、事务处理、数据存储等一系列模块。
...
**环境要求**：您已部署 OceanBase V4.3 及以上版本的集群，并创建了一个 MySQL 模式的租户。要了解如何部署 OceanBase 集群，请参见 [部署概述](../400.deploy/100.deploy-overview.md)。完成部署后，请通过以下 SQL 验证集群和租户信息：
...
**说明**：执行环境有差异，执行时间也有出入，上述表格中的执行时间可做参考。

**结论**：旁路导入速度提升 **3倍以上**，适用于大规模数据导入场景（如 TPC-H/TPC-DS 测试数据准备）。

## 相关文档

- [旁路导入概述](../500.data-migration/1100.bypass-import/100.overview-of-bypass-import.md)  
