...
* 支持数据同步工具 Canal 和 DataX。支持将数据从 MySQL 迁移到 OceanBase 数据库。支持使用 DataX 向 OceanBase 数据库全量迁移数据。详细信息，参考 [使用 DataX 向 OceanBase 数据库全量迁移数据](../7.developer-guide/5.migrate-data-to-oceanbase-database/2.use-datax-to-perform-full-data-migration-to-oceanbase-database.md) 和 [使用 Canal 向 OceanBase 数据库增量迁移数据](t2091017.html#topic-2091017)。
