...
* 修复了释放中间结果集过程导致的 OBServer crash的问题。
...
* 修复因为 CLOG 日志堆积导致 OBServer 重启 SYS 租户内存消耗过多的问题。
...
* 修复 Oracle 模式下，指定 GBK 字符集创建存储过程时在 SQL Prepare 阶段 OBServer 出现非预期的挂起的问题。
...
* 支持通过 ob_admin 修改指定 OBServer 的集群级配置项，但不支持修改租户级配置项。
...
* 优化删除 OBServer 节点间 RPC 通信的无效日志信息。
