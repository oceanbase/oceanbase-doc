...
创建一级分区后，您可以对一级分区进行管理，例如：添加一级分区、删除一级分区、Truncate 一级分区等。分区管理的详细操作，请参见 [管理一级分区表](/zh-CN/6.administrator-guide/5.distributed-storage-management/2.manage-partitions/3.manage-partitioned-tables.md)。
