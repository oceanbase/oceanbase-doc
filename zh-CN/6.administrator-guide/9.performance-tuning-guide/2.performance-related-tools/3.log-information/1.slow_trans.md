...
[init] u=0 arg1:140542173395392, ctx_type:"scheduler", trans_type:2, trans_id:{hash:7103234888693285108, inc:532846, addr:{ip:"100.88.xx.xx", port:40000}, t:1633664744167658}, pkey:{tid:1000, partition_id:1000, part_cnt:1000}, arg1:false, uref:1073741824
...
| start_stmt | 语句开始执行 | - |
| end_stmt | 语句结束执行 | - |
| submit_commit | scheduler 向 coordinator 发出事务提交/回滚命令 | 该动作由 end_trans 触发 |
| end_trans | 客户端调用事务提交/回滚的命令 | - |
| exiting | 事务提交结束，设置退出状态 | exiting、handle_message、end_trans_cb、destroy 等四个动作是同一时间点触发 |
| handle_message | 收到 coordinator 发来的提交成功与否的消息 | exiting、handle_message、end_trans_cb、destroy 等四个动作是同一时间点触发 |
| end_trans_cb | 应答客户端事务提交结果 | exiting、handle_message、end_trans_cb、destroy 等四个动作是同一时间点触发 |
| destroy | 事务上下文退出 | exiting、handle_message、end_trans_cb、destroy 等四个动作是同一时间点触发 |
...
1. 上一条 SQL 执行结束之后，由于中间链路耗时较长（跨城、网络等）也会产生这种情况。
1. OBServer 端资源不足，SQL 请求在 OBServer 端排队。为了确认这一情况，`start_stmt` 这一 trace 后面带上了该请求到达 OBServer 端的时间点 start_time，根据这个信息，也能判断出是否为该情况。
...
[init] u=0 arg1:140559776221904, ctx_type:"participant", trans_type:2, trans_id:{hash:4872971512783667647, inc:524414, addr:{ip:"100.88.xx.xx", port:40000}, t:1633664737942500}, pkey:{tid:1102810162709333, partition_id:36, part_cnt:0}, arg1:false, uref:1073741824
...
[init] u=0 arg1:140543227771456, ctx_type:"coordinator", trans_type:2, trans_id:{hash:16764817863274798805, inc:532875, addr:{ip:"100.88.xx.xx", port:40000}, t:1633664744191993}, pkey:{tid:1102810162709332, partition_id:25, part_cnt:0}, arg1:false, uref:1073741824
