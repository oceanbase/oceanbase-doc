...
2. （可选）配置备份模式，并开启归档日志压缩功能。
   ...
   * 日志压缩目前支持的压缩算法有：zstd_1.3.8 和 lz4_1.0，默认使用压缩算法 lz4_1.0。
   ...
   ```sql
   obclient> ALTER SYSTEM SET backup_log_archive_option='mandatory optional= zstd_1.3.8';
   
   obclient> ALTER SYSTEM SET backup_log_archive_option='mandatory optional= lz4_1.0';
   ```
