...
配置项 `backup_dest_option` 中 `recovery_window` 参数的配置方法及说明信息请参见 [备份前准备](1.preparations-before-backup.md)。
...
除了通过手动的方式清理备份数据外，您也可以设置系统自动清理备份数据，详细信息参见 [自动清理备份的数据](6.automatically-clear-backup-data.md)。

OceanBase 数据库支持停止清理备份数据，详细操作参见 [停止清理备份数据](8.stop-clearing-backup-data.md)。
