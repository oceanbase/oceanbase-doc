...
2. 配置备份目的端。
   ...
   * 必须保证所有 OBServer 都挂载了同一个服务器的 NFS。同时，为保证备份的顺利进行，务必使用本文档中推荐的参数挂载 NFS。挂载 NFS 具体操作请参见 [部署 NFS 客户端](/zh-CN/6.administrator-guide/7.high-data-availability/2.backup-and-recovery-management/2.deploy-nfs.md)。
...
* 停止备份任务。
  ...
  具体操作请参见 [停止备份](t2043601.html#topic-2043601)。
  

* （建议）停止清理备份的任务。

  具体操作请参见 [停止清理备份数据](/zh-CN/6.administrator-guide/7.high-data-availability/2.backup-and-recovery-management/3.cluster-level-data-backup/8.stop-clearing-backup-data.md)。
...
2. 通过配置项 `backup_dest_option` 设置备份相关的参数。
   ...
   * `recovery_window` 参数
     ...
     对于 [自动清理](/zh-CN/6.administrator-guide/7.high-data-availability/2.backup-and-recovery-management/3.cluster-level-data-backup/6.automatically-clear-backup-data.md) 场景，备份的数据过期后，系统会按照一定的规则自动清理过期的数据。
   ...
   * `backup_copies` 参数
   ...
   更多配置项 `backup_dest_option` 的使用说明，请参见 [backup_dest_option](/zh-CN/12.reference-guide/3.system-configuration-items/18.backup_dest_option.md)。
