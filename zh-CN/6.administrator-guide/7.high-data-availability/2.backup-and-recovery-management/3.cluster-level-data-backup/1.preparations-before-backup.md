...
* （建议）停止清理备份的任务。
  ...
  具体操作请参见 [停止清理备份数据](8.stop-clearing-backup-data.md)。
...
2. 通过配置项 `backup_dest_option` 设置备份相关的参数。
   ...
   * `recovery_window` 参数
     ...
     对于 [自动清理](6.automatically-clear-backup-data.md) 场景，备份的数据过期后，系统会按照一定的规则自动清理过期的数据。
   ...
   * `backup_copies` 参数
   ...
   更多配置项 `backup_dest_option` 的使用说明，请参见 [backup_dest_option](../../../../18.backup_dest_option.md)。
