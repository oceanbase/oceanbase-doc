...
3. 根据使用场景，查看转储情况。
   ...
   * 系统租户查看转储后租户的 Active MemStore 是否释放，同时查看 `freeze_cnt` 的计数情况。
     ...
     * 内部表 `__all_virtual_tenant_memstore_info` 中 `major_freeze_trigger` 列对应的是转储和合并的阈值。
       ...
       更多配置项 `minor_freeze_times` 的说明请参见 [minor_freeze_times](https://code.alipay.com/oceanbase-docs/oceanbase-doc/blob/V3.1.2/zh-CN/12.reference-guide/3.system-configuration-items/134.minor_freeze_times.md)。
