...
2. 集群中当前仅 `z1`、`z2`、`z3` 三个 Zone，需要在集群中增加 `z4`、`z5` 两个 Zone。
   ...
   在集群中增加 Zone 的具体操作请参见 [增加或删除 Zone](/zh-CN/6.administrator-guide/2.basic-database-management/1.manage-clusters/4.manage-zones-in-a-cluster/2.add-or-delete-zones.md)。
   

3. 在 `z4`、`z5` 两个 Zone 上各添加一台 OBServer。

   向 Zone 内添加 OBServer 的具体操作请参见 [添加 OBServer](/zh-CN/6.administrator-guide/2.basic-database-management/1.manage-clusters/5.manage-observers/1.add-an-observer.md)。
