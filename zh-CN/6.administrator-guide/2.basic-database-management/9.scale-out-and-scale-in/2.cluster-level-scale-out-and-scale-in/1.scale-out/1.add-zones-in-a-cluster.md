...
2. 向集群中增加 2 个 Zone 同时在 Zone 内各添加 1 台 OBServer。
   ...
   通过 OCP 向集群中增加 Zone 的相关操作请参见 [增加或删除 Zone](/zh-CN/6.administrator-guide/2.basic-database-management/1.manage-clusters/4.manage-zones-in-a-cluster/2.add-or-delete-zones.md)。
   

3. 添加成功后，在租户内，依次为新增的 2 个 Zone 新增副本。

   通过 OCP 新增副本的相关操作请参见 [新增副本](/zh-CN/6.administrator-guide/5.distributed-storage-management/3.manage-replicas/2.add-replicas.md)。
