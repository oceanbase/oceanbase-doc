...
* `ss_id` 表示 ODP 内部标记每个 Server 端会话（ Server Session）的 `id` 号，可以从 `SHOW PROXYSESSION ATTRIBUTE id` 中获取。详细的获取操作请参见 [展示 Session 详细状态](/zh-CN/6.administrator-guide/3.database-connection-and-routing/3.logical-connection/2.show-detailed-session-status.md)。
