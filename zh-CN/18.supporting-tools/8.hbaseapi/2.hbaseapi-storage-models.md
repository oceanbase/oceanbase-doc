...
上述建表过程中没有包含分区方式，建立的表是一个非分区表，仅适用于数据量很小的场景。一般情况下 HBase 业务表数据量都比较大，所以我们需要建立 [分区表](../../15.oceanbase-database-overview/3.database-objects/2.mysql-mode-tables/6.mysql-mode-partitioned-tables.md)。目前，OceanBase 数据库的 HBase 模型不能像 Apache HBase 一样自动进行分区（region）分裂，所以需要在建表的时候选择一种分区方式。您可以参考如下流程决定使用哪一种分区方法。
...
数据删除的详细操作说明，请参考 [HBaseAPI 过期数据删除功能](4.use-of-ttl.md)。
