...
当前版本新增 case 的时候，mysql_test 目录下需要有 t 目录和 r/mysql 目录。如果没有，可以在本地通过以下命令新建：

```bash
cd oceanbase/test/mysql_test
...
>**说明**
>
>假设我们测试 create table like 这个功能，case 文件名最好能反映测试点，比如 create_table_like_syntax.test 为语法级别的测试。
...
2. 输入 case 文件的内容：
    ...
    ```bash
    vi mysql_test/t/sample.test
    ```

3. 生产结果文件：

    ```bash
    obd test mysqltest <deploy name> --test-set sample --record
    ```

4. 人工校验结果文件后（这步也可以通过执行与 MySQL 的对比来完成）:

    ```bash
    mv mysql_test/r/mysql/sample.record mysql_test/r/mysql/sample.result
    ```

5. 再运行检查一遍：

    ```bash
    obd test mysqltest <deploy name> --test-set sample
    ```

6. 提交时注意 git add 的时候要同时提交 test 文件和 result 文件。根据 case 的测试目标，建议将执行成功的 case 放到 test_suite 中对应类型的 case 集合下，test 文件放在 t 目录下，result 文件放在 r/mysql 下。
