...
使用 `SQL_TEXT` 创建 Outline 后，会生成一个 Key-Value 对存储在 Map 中，其中 Key 为绑定的 SQL 参数化后的文本，Value 为绑定的 Hint。具体参数化原则，请参见 [快速参数化](/en-US/12.sql-tuning-guide/2.sql-execution-plan/4.fast-parameterization.md)。
...
* Hint 格式为 `/*+ xxx */`，关于 Hint 说明的详细信息，请参见 [Optimizer Hint。](/en-US/12.sql-tuning-guide/4.sql-tuning/6.manage-execution-plans/1.optimizer-hints.md)
...
3. 确定生成的执行计划是否符合预期。
   ...
   [实时执行计划展示](/en-US/12.sql-tuning-guide/2.sql-execution-plan/5.real-time-execution-plan-display.md)。
