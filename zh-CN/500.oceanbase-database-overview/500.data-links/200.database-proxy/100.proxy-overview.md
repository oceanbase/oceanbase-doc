...
图中 APP 是我们的业务程序，APP 前面有三台 OBPoxy（ODP 的进程名叫做 obproxy），在实际部署中， OBPoxy 和 APP 之间一般会有一个负载均衡将请求分散到多台 OBProxy 上面，OBProxy 下面是 OBServer，图中有 6 台 OBServer。
...
* 数据路由：ODP 可以获取到 OBServer 中的数据分布信息，可以将用户 SQL 高效转发到数据所在机器，执行效率更高。如：表 t1 数据在图中 P1 内，表 t2 数据在图中 P2 内，表 t3 数据在图中 P3 内，深蓝色表示主副本，浅蓝色表示从副本。对于 insert into t1 语句 ODP 可以将 SQL 转发到 ZONE_2 中含有 P1 主副本的机器上。对于 update t2 语句 ODP 可以将 SQL 转发到 ZONE_1 中含有 P2 主副本的机器上。
