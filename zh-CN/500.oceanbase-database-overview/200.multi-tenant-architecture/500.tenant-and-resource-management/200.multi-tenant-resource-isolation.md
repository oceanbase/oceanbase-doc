...
* 内存完全隔离。
  ...
  * SQL 执行过程各种算子使用的内存是分离的，一个租户的内存耗尽不会影响到另一个租户。
    ...
    SQL 执行过程的各种算子的详细信息请参见 [用户接口和查询语言](../../600.user-interfaces-and-query-languages/100.sql/100.introduction-to-sql/100.sql-overview.md) 章节。
  * Block Cache 和 MEMTable 是分离的，一个租户的内存耗尽不会影响到另一个租户的写入和读取。

    Block Cache 的详细介绍信息请参见 [多级缓存](../../800.storage-architecture/400.multi-level-caches.md)。

    MEMTable 的详细介绍信息请参见 [MEMTable](../../800.storage-architecture/200.data-storage/200.memtable.md)。
* CPU 通过用户态调度实现隔离。
...
关于内存配置的更多信息，请参见 [内存管理](../../1100.observer-architecture/500.oceanbase-database-overview-memory-management/100.overview-of-memory-management.md) 章节。
