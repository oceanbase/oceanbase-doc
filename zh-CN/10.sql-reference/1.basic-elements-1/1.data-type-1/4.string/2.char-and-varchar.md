...
`CHAR` 类型声明的长度为可以存储的最大字符数。例如，`CHAR(30)` 最多可容纳 30 个字符。
...
`VARCHAR` 类型声明的长度为可以存储的最大字节数。例如，`VARCHAR(50)` 最多可容纳 50 个字节。
...
`VARCHAR` 列长度可以指定为 0 到 65,535 之间的值。`VARCHAR` 的最大有效长度取决于最大行的大小（所有列之间共享 65,535 字节）和所使用的字符集。

与 `CHAR` 相比，`VARCHAR` 值存储形式为 1 字节或 2 字节的长度前缀加数据。长度前缀表示值中的字节数。如果值不超过 255 个字节，则该列占用一个字节，如果值可能超过 255 个字节，则占用两个字节。
