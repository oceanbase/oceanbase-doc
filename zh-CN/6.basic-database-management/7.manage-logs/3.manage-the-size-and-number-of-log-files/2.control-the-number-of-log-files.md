...
1. 通过如下两种方法设置 `max_syslog_file_count` 系统配置项：
   ...
   当完成 `max_syslog_file_count` 系统配置项设置后，OceanBase 数据库会将每类日志文件中时间较新的日志文件的数量控制在所配置的值。当有新日志文件产生时，超过该范围的旧日志文件将被删除。
   a OBServer 运行过程中，设置系统配置项 `max_syslog_file_count` 的值。
   ...
   b 启动 OBServer 时，在 `-o` 参数中指定 `max_syslog_file_count` 的值。
