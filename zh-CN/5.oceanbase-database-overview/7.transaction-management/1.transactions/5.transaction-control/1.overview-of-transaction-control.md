...
在事务内部可以创建 Savepoint，它标记了事务内部的一个点，您可以在事务后续的执行过程中通过 `ROLLBACK TO SAVEPOINT` 语句回滚到该点。更多 Savepoint 相关信息请参见 [Savepoint](/zh-CN/5.oceanbase-database-overview/7.transaction-management/1.transactions/5.transaction-control/3.oceanbase-database-overview-savepoint.md)。
...
虚拟表 `__all_virtual_trans_stat` 可用于查询系统中当前所有的活跃事务。有关活跃事务的详细信息请参见 [活跃事务](/zh-CN/5.oceanbase-database-overview/7.transaction-management/1.transactions/5.transaction-control/2.active-transactions.md)。
