...
* 索引嵌套循环联接（Index Nested Loop Join）
  ...
  *************************** 1. row ***************************
  ...
  在 `outputs & filters` 的输出结果中 `nl_param` 出现参数 `[t2.c1]`，说明执行了条件下压优化。详细信息请参考 [JOIN](../../../../5.oceanbase-database-overview/7.sql-engine/3.oceanbase-database-overview-query-optimization/2.oceanbase-database-overview-join-algorithm/2.oceanbase-database-overview-join-algorithms.md)。
