...
从上述执行计划示例的输出结果可以发现，虽然 SQL 中含有 `ROWNUM`，但是经过 SQL 优化器改写之后，已经将涉及含有 `ROWNUM` 的表达式转换为了等价的 `LIMIT` 表达式，转换的好处在于可以做更多的优化，详细信息请参见 [LIMIT](../../../../5.oceanbase-database-overview/7.sql-engine/4.oceanbase-database-overview-sql-execution-plan/2.oceanbase-database-overview-execution-plan-operators/4.execution-plan-operators-count.md)。
