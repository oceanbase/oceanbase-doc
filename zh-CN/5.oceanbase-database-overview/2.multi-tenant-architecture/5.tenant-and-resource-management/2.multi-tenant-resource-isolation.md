...
* 内存完全隔离。
  ...
  * SQL 执行过程各种算子使用的内存是分离的，一个租户的内存耗尽不会影响到另一个租户。
    ...
    SQL 执行过程的各种算子的详细信息请参见 [用户接口和查询语言](/zh-CN/5.oceanbase-database-overview/6.user-interfaces-and-query-languages/1.sql/1.introduction-to-sql/1.sql-overview.md) 章节。
    
  
  * Block Cache 和 MEMTable 是分离的，一个租户的内存耗尽不会影响到另一个租户的写入和读取。

    Block Cache 的详细介绍信息请参见 [多级缓存](/zh-CN/5.oceanbase-database-overview/8.storage-architecture/4.multi-level-caches.md)。

    MEMTable 的详细介绍信息请参见 [MEMTable](/zh-CN/5.oceanbase-database-overview/8.storage-architecture/2.data-storage/2.memtable.md)。
...
关于内存配置的更多信息，请参见 [内存管理](/zh-CN/5.oceanbase-database-overview/11.observer-architecture/5.oceanbase-database-overview-memory-management/1.overview-of-memory-management.md) 章节。
