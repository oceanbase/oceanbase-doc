...
假设资源池 `rp1` 之前的资源配置为 `uc1`，则示例语句将 `rp1` 的资源配置从 `uc1` 变更为 `uc2`。理论上 OceanBase 数据库支持对资源规格 `MIN_CPU`、`MAX_CPU`、`MIN_MEMORY` 以及 `MAX_MEMORY` 同时修改。但通常情况下，修改资源配置和切换资源配置都能对租户的服务能力进行调整。对应到租户层面，实际上是调整了租户资源单元的规格。对资源规格的修改通常有以下两种场景：
