...
2. 修改启动配置文件` conf/application.yml`。
   ...
   其中，`outerAdapters` 中的 `rdb` 数据源表示目标数据库。OceanBase 数据库用户名的格式详情，参考 [通过 MySQL 客户端连接 OceanBase 租户](../2.connect-to-oceanbase-database/1.connect-to-an-oceanbase-database-tenant-through-a-mysql-client.md)。
