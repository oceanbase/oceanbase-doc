...
* `PARTITION BY RANGE ( expr )` 里的 `expr` 表达式的结果必须为整型。如果要按时间类型列做 Range 分区，则必须将时间类型转换为数值。
...
OceanBase 数据库目前支持 Hash、Range、 List、Key、Range Columns 和 List Columns 六种分区类型，二级分区为任意两种分区类型的组合。
