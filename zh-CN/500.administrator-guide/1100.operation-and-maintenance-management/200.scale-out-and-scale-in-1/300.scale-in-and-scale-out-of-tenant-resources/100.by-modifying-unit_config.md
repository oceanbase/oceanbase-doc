...
* 进行租户缩容前，建议进行一轮转储以便释放租户正在使用的内存。
  ...
  手动触发转储的相关操作请参见 [手动触发转储](../../../600.basic-database-management/500.manage-data-storage/100.dump-management-1/300.trigger-dump-manually-1.md)。
...
* `resource_hard_limit`：是一个配置项的值。
  ...
  更多配置项 `resource_hard_limit` 的说明信息请参见 [resource_hard_limit](../../../../700.reference/500.system-configuration-items/300.cluster-level-configuration-items-1/16800.resource_hard_limit-1-2-3.md)。
...
更多 `DBA_OB_UNIT_CONFIGS` 视图的字段及说明信息请参见 [DBA_OB_UNIT_CONFIGS](../../../../700.reference/300.system-views/200.dictionary-view-5/6000.oceanbase-dba_ob_unit_configs.md)。
