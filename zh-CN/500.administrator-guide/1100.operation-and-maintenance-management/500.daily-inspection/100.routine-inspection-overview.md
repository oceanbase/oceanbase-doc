...
* [检查集群状态](../500.daily-inspection/200.check-the-cluster-status.md)
...
* [检查集群参数](../500.daily-inspection/300.check-cluster-parameters.md)

  OceanBase 数据库支持对集群最佳实践的参数进行检查。在问题分析阶段，您可通过检查集群参数来进行问题排查，您可以修改日志打印级别来获取更多的参数日志信息，待问题确定并修复后，请修改回原来的参数。

* [检查主机状态](../500.daily-inspection/400.check-host-status.md)

  OceanBase 数据库该检查项支持对主机状态进行检查，包括不限于磁盘水位、主机时间和主机负载。

* [检查租户资源使用状态](../500.daily-inspection/500.check-the-resource-usage-status-of-a-tenant.md)

  OceanBase 数据库支持对租户资源使用进行检查，包括不限于 CPU 、内存、副本数和磁盘。

* [检查集群资源使用状态](../500.daily-inspection/600.check-cluster-resource-usage.md)

  OceanBase 数据库支持对集群资源使用进行检查，包括不限于 CPU 、内存、副本数和磁盘。

* [检查 OBServer 状态](../500.daily-inspection/700.check-observer-status.md)

  OceanBase 数据库支持对 OBServer 的状态检查，包括进程是否存在、副本数等。

* [检查 NTP 偏移量](../500.daily-inspection/800.check-ntp-offset.md)

  OceanBase 数据库支持对主机 NTP 偏移量的检查，OceanBase 集群主机节点的 NTP 偏移量如果相差太大，可能会导致选主异常。

* [检查数据备份](../../1000.high-data-availability/200.backup-and-restoration-management/400.data-backup/200.initiate-full-data-backup.md)

  OceanBase 数据库支持对备份的检查，包括备份成功与否、备份速度和备份进度。

* [检查 OBProxy 连接状态](../500.daily-inspection/700.check-observer-status.md)
