...
* 创建表时，指定表的编码格式和压缩方式
  ...
  更多 `CREATE TABLE` 语句的说明请参见 [CREATE TABLE](../../../700.database-object-management/200.manage-tables/200.create-a-table.md)。
...
* 修改表时，更改表的编码格式和压缩方式
  ...
  更多 `ALTER TABLE` 语句的说明请参见 [ALTER TABLE](../../../700.database-object-management/200.manage-tables/600.change-table.md)。
...
2. 执行以下语句，设置渐进合并轮次。
   ...
   >**说明**
   >
   >如果希望尽快完成数据的重写，可以将 `progressive_merge_num` 的值设置为 `1` 后，手动发起一次合并，手动发起合并的操作请参见 [手动触发合并](../200.merge-management-1/400.manually-trigger-a-merge-1.md)。
