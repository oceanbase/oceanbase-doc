...
```sql
obclient> SELECT tenant_id, ctx_name, hold FROM oceanbase.GV$OB_MEMORY WHERE svr_ip = 'xx.xx.xx.xx' AND svr_port=2882 AND tenant_id = 1002 ORDER BY ctx_name;
+-----------+-----------------+-----------+
...
+-----------+----------------+----------+-----------------+-------+-----------+-----------+-------------+------------+
| TENANT_ID | SVR_IP         | SVR_PORT | CTX_NAME        | COUNT | HOLD      | USED      | ALLOC_COUNT | FREE_COUNT |
+-----------+----------------+----------+-----------------+-------+-----------+-----------+-------------+------------+
