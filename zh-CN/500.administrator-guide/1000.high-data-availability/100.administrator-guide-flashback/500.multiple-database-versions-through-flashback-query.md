...
* 如果被查询的表已经被删除进了回收站，则需要先将该表从回收站中恢复。
  ...
  闪回表的相关操作请参见 [闪回回收站对象](../100.administrator-guide-flashback/400.flashback-recycle-bin-object.md)。
...
2. 执行以下语句，修改租户级配置项 `undo_retention` 的值。
   ...
   更多租户级配置项 `undo_retention` 的说明信息请参见 [undo_retention](../../../700.reference/500.system-configuration-items/400.tenant-level-configuration-items-1/4900.undo_retention.md)。
