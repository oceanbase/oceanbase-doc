...
2. 设置归档目的端参数。
   ...
   * 配置 `LOCATION` (必选)
       ...
       * NFS
            ...
            >
            > * 必须保证所有 OBServer 都挂载了同一个服务器的 NFS。同时，为保证归档顺利进行，务必使用本文档中建议的参数挂载 NFS。挂载 NFS 的具体操作，请参见 [部署 NFS 客户端](../200.deploy-nfs.md)。
       ...
       * 阿里云 OSS
            ...
            * `tagging`：表示清理模式为对满足清理要求的归档文件设置 `Tag`，归档文件将仍然保留。
            ...
            设置 `delete` 模式或 `tagging` 模式后，自动清理归档数据的相关操作请参见 [自动清理归档的数据](../500.cleaning-up-backed-up-data/100.cleaning-up-backed-up-data-automatically.md)。
