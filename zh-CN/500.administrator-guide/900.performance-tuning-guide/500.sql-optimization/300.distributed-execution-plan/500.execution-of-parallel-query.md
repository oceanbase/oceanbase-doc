...
# docslug#/oceanbase-database/oceanbase-database/V4.0.0/execution-of-parallel-query
...
<main id="notice" type='notice'>
  <h4>注意</h4>
  <p>当前版本不支持关闭并行查询 NO_USE_PX Hint。</p>
</main>
...
1. hint 方式
   ...
   ```sql
   obclient> CREATE TABLE lineitem(c1 INT , c2 INT) PARTITION BY HASH(c1) PARTITIONS 24;
   Query OK, 0 rows affected (0.070 sec)

   obclient> explain select /*+ PARALLEL(8) */ count(*) from lineitem;
    +-----------------------------------------------------------------------------------------------------------------------------------------------------------------+
    | Query Plan   |
    +-----------------------------------------------------------------------------------------------------------------------------------------------------------------+
    | ==================================================
...
2. 设置 session 环境变量
  ...
  ```sql
  obclient> set _force_parallel_query_dop = 3;
  Query OK, 0 rows affected (0.001 sec)

  obclient [test]> explain select count(*) from lineitem;
    +-----------------------------------------------------------------------------------------------------------------------------------------------------------------+
    | Query Plan     |
    +-----------------------------------------------------------------------------------------------------------------------------------------------------------------+
    | ==================================================
...
3. 建表时强制设置并行度
  ...
  ```sql
  obclient [test]> create table t1(a int, b int ) parallel 3;
  Query OK, 0 rows affected

  obclient [test]> explain select * from t1;
  +-------------------------------------------------------------------------------------------------------------------------------------------------------------------+
  | Query Plan      |
  +-------------------------------------------------------------------------------------------------------------------------------------------------------------------+
  | ================================================
...
详细信息请参考 [(G)V$OB_SQL_AUDIT](../400.sql-optimization-1/300.monitor-sql-execution-performance/100.gv-sql_audit-1.md)。
