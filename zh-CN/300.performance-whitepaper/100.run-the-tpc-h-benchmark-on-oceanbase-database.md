...
* OceanBase 数据库：详细信息，请参考 [快速入门指南](../200.quick-start/100.overview-of-quick-start.md)
...
* 租户规格：
  ...
  > **注意**
  >
  > * 上文的租户规格是基于 [OceanBase TPC-H 性能测试报告](../300.performance-whitepaper/200.tpc-h-benchmark-report-of-oceanbase-database.md) 中的硬件配置进行配置的，您需根据自身数据库的硬件配置进行动态调整。详细操作，请参考 [修改租户](../600.administrator-guide/200.basic-database-management/400.manage-tenants/400.administrator-guide-manage-tenants/200.modify-a-tenant.md)。
  >
  > * 进行性能测试时，租户需配置最大可用资源。
...
2. 部署成功后，新建执行 TPC-H 测试的租户及用户（请勿直接使用 sys 租户进行测试）。将租户的 `primary_zone` 设置为 `RANDOM`。租户详情，请参考 [修改租户](../600.administrator-guide/200.basic-database-management/400.manage-tenants/400.administrator-guide-manage-tenants/200.modify-a-tenant.md)。
...
* obd 运行 tpch，详细参数介绍请参考 [obd test 文档](https://www.oceanbase.com/docs/community-obd-cn-10000000000017243)。
  
* 在本例中，大幅参数使用的是默认参数，在用户场景下，可以根据自己的具体情况做一些参数上的调整。例如，在本例中使用的租户名是 `tpch_mysql`。
...
* 如果系统租户的密码通过终端登陆并修改，非默认空值，需要通过 [obd cluster edit-config](https://www.oceanbase.com/docs/community-obd-cn-10000000000017235) 将密码设置到配置文件，配置项是 ``` # root_password: # root user password ```

* 运行 obd test tpch 后，系统会详细列出运行步骤和输出，数据量越大耗时越久。

* 远程目录具备足够的容量能存储 tpch 的数据，建议单独一块盘存储加载测试数据。

* obd test tpch 会自动完成所有操作，无需其他额外任何操作，包含测试数据的生成、传送、OceanBase 参数优化、加载和测试。当中间环节出错时，您可参考 [obd test 文档](https://www.oceanbase.com/docs/community-obd-cn-10000000000017243) 进行重试。例如：跳过数据的生成和传送，直接进行加载和测试。
...
> **说明**
>
> 测试结果可参考 [OceanBase TPC-H 性能测试报告](../300.performance-whitepaper/200.tpc-h-benchmark-report-of-oceanbase-database.md)。
