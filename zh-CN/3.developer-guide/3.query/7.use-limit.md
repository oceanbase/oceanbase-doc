...
如果想获取订单金额最大的一行记录，可以先按照金额降序，然后取第一行记录。
...
```sql
obclient> SELECT order_id ID, fruit_price 订单金额 FROM fruit_order LIMIT 3,6;
+----+--------------+
| ID | 订单金额     |
+----+--------------+
|  4 |        22.21 |
...
* `pageSize` 表示每页显示多少行记录，范围为 \[1,+∞)。例如：page = 5，pageSize = 10，表示获取第 5 页 10 行数据。

如下示例为，按照订单金额降序排序，每页显示 2 行数据，依次获取所有订单数据、第 1 页、第 2 页和第 3 页的数据。
