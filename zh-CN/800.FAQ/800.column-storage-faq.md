...
OceanBase 数据库 V4.6.0 版本引入了列存副本自动选择功能。简单来说，这个功能能够：

* 自动识别：系统会自动识别哪些查询适合使用列存副本（AP 查询）。
* 智能路由：根据查询特征和代价，自动将合适的查询路由到列存副本执行。
* 统一入口：TP 和 AP 流量使用同一个入口，无需手动切换。

## 为什么需要列存副本自动选择功能？

在 HTAP（混合事务分析处理）场景中，业务通常包含两类不同的查询：

| **查询类型** | **特征** | **适合的存储** |
|--------------|----------|----------------|
| TP 类查询（事务处理） | 短查询、高 QPS、对延迟敏感、访问数据量小 | 行存副本（利用索引加速） |
| AP 类查询（分析处理） | 大查询、低 QPS、延迟不敏感、访问数据量大 | 列存副本（利用列存和并行化加速） |

在 OceanBase 数据库 V4.6.0 版本之前的版本中，如果要将查询路由到列存副本，需要手动设置 `ob_route_policy = 'COLUMN_STORE_ONLY'`，这种方式存在以下问题：

1. 使用门槛高：需要用户自己判断哪些查询是 AP 查询，然后手动配置。
2. 无法自适应：无法根据查询特征自动判断，配置后所有查询都会走列存副本。
3. 强读限制：不支持强一致性读，只能使用弱读，限制了使用场景。

## 如何判断查询是否路由到了列存副本？

1. 查看执行计划：如果一张表在 F 副本没有列存，扫描算子却是 `COLUMN TABLE FULL SCAN`，说明查询被路由到了列存副本。
2. 查看 Outline：更准确的方式是查看 `outline data`，如果出现 `OPT_PARAM('AP_QUERY_ROUTE_POLICY' 'FORCE')` 的 Hint，说明确实路由到了列存副本。例如：
