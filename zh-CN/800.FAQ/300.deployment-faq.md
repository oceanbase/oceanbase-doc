...
## OBServer 节点是否可以同时支持 IPv4 和 IPv6？

即一个 OBServer 节点，与其它 OBServer 之间的 RPC 通信使用 IPv4，对外的 SQL 通信使用 IPv6，或者内部用 IPV4，对外 SQL 用 IPv6 的情况。

不支持，因为：

* OBServer 在没有启用 IPv6 的情况，不会监听 IPv6。
* 在启用 IPv6 的情况下，OBServer 监听 SQL 端口时会设置 IPV6_V6ONLY，IPv4 无法建立连接。

## OceanBase 集群中是否可以同时混用 IPv4 和 IPv6？

即是否允许再集群中混合部署 IPv4 和 IPv6 地址，有的 OBServer 节点用 IPv4 地址启动，有的节点用 IPv6 地址这种情况。

支持。

<main id="notice" type='notice'>
  <h4>注意</h4>
  <p>使用 IPv4 启动的 OBServer 节点，要求其操作系统和底层网络支持 IPv6，不然无法和其它 IPv6 的 OBServer 节点正常通信。</p>
</main>