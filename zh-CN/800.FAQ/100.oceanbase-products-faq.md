...
### V4.4.1 版本是否完全不能使用共享存储？

V4.4.1 版本不再支持新建共享存储业务。已部署的存量共享存储集群原则上可继续运行，但不建议在该版本线上继续扩展新的共享存储业务。

### 为什么推荐 OceanBase Database AI？

共享存储相关能力已纳入 OceanBase Database AI 产品体系统一演进，该产品在共享存储架构、弹性能力与后续版本支持上更具长期保障。

### 存量共享存储业务何时需要迁移？

存量业务暂无强制下线时间表。若业务有扩容、新特性接入或版本升级需求，建议提前评估迁移至 OceanBase Database AI 的可行性，具体事宜请与 OceanBase 技术支持团队沟通。
...
- 工作区（work area）报内存不足
  ...
  可以通过适当调大工作区系统变量来规避，比如 `set global ob_sql_work_area_percentage = 10`。
...
- MemStore 内存用尽
  ...
  写入的速度大于转储的速度，导致 MemStore 耗尽。比如：大量高并发的数据导入，MemStore 的写操作过快，系统未能及时转储，MemStore 就被用完，返回用户报错 4030。
