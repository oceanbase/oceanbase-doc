...
OceanBase 数据库可以通过配置参数 `log_storage_compress_all` 开启 CLOG 存储压缩功能。参数 `log_storage_compress_func` 可用于配置 CLOG 存储压缩的压缩算法。在开启 CLOG 存储压缩功能后，Clog 会按照算法来对日志进行压缩落盘。默认是 lz4_1.0。
