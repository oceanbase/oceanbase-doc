...
### 在 V4.x 版本中，切主会 Kill 事务吗？

在 V4.x 版本中，切主不会 Kill 事务，除非发生了无主。

<main id="notice" type='explain'>
<h4>说明</h4>
<p>在 OceanBase 数据库 V3.x 版本中，切主 Kill 事务时的最长等待时间由集群级配置项 <a href="../700.reference/800.configuration-items-and-system-variables/100.system-configuration-items/300.cluster-level-configuration-items/22700.trx_force_kill_threshold.md">trx_force_kill_threshold</a> 控制。</p>
</main>

### 切主完成前，对于未执行完的大事务，会一直等待大事务执行完后再切主吗？

切主不需要等待事务结束。OceanBase 数据库会等待旧主将未完成的事务的事务状态搬迁到新主上后，在新主上继续执行未完成的事务。
...
OceanBase 数据库可以通过配置参数 `log_storage_compress_all` 开启 clog 的存储压缩功能，需要手动开启。参数 `log_storage_compress_func` 可用于配置 clog 存储压缩的压缩算法。在开启 clog 的存储压缩后，clog 会按照算法来对日志进行压缩落盘。默认是 lz4_1.0。

OceanBase 数据库可以通过配置参数 `log_transport_compress_all` 开启日志的同步压缩，需要手动开启，`log_transport_compress_func` 可以用于设置 clog 日志传输的压缩算法，默认是 lz4_1.0。
