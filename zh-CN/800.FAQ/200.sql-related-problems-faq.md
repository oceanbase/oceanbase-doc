...
- **解决“大小账号” SQL 带来的性能下降问题**
  ...
  + 限制资源消耗：为了防止“大账号”查询影响系统性能，可以采用以下方法来限制资源消耗：
    ...
    2. **SQL级资源隔离**：将大账号查询绑定到资源组，限制其执行中的资源消耗，示例：
       ...
       您可以通过 Hint 的方式，给特定的 SQL 语句指定资源组：
...
### 新增 `BINDING_RULE` 功能后，现有 Outline 是否需要重建？

不需要。不带 `BINDING_RULE` 子句的 Outline 行为完全保持旧版语义。

### 跨库 Outline 与当前库模板 Outline 同时命中怎么办？

匹配优先级从高到低为：

1. 当前库精确绑定，例如 `sql_text` 方式的精确绑定。
2. 当前库 `sql_id`。
3. `format outline`（`format_sql_text` 和 `format_sql_id`）。
4. 当前库模板 `BINDING_RULE`。
5. 跨库（租户）模板 `BINDING_RULE`。

<main id="notice" type='explain'>
  <h4>说明</h4>
  <p>当前库模板 <code>BINDING_RULE</code> 严格优先于跨库模板 <code>BINDING_RULE</code>。</p>
</main>

### 表名只匹配数字后缀，正则该怎么写？

`'orders_${S:[0-9]+}'`。同理字母为 `[a-z]+`、字母数字混合为 `[a-z0-9]+`。

### 多分表 JOIN 场景，如何让 Outline 对同分片的表组合都能生效？

在 `MAP` 中对多个表使用同一个变量名来匹配分片后缀，这样只需写一条规则就能覆盖所有分片。例如：`MAP = {orders_sh TO 'orders_${S:[a-z]+}', products_sh TO 'products_${S}'}` 变量 `S` 在同一次匹配中只取一个值，因此：

* `orders_sh JOIN products_sh → S=sh`，命中。
* `orders_hz JOIN products_hz → S=hz`，命中。
* `orders_hz JOIN products_bj → S` 无法同时取两个值，不会误命中。
