...
```shell
# 阿里云 OSS
LOAD DATA /*+ [APPEND | DIRECT(need_sort,max_error,'inc_replace')] parallel(N) */ REMOTE_OSS INFILE 'oss://$PATH/$FILENAME/?host=$HOST&access_id=$ACCESS_ID&access_key=$ACCESS_KEY' INTO TABLE table_name ...;

# AWS S3
LOAD DATA /*+ [APPEND | DIRECT(need_sort,max_error,'inc_replace')] parallel(N) */ REMOTE_OSS INFILE 's3://$PATH/$FILENAME/?host=$HOST&access_id=$ACCESS_ID&access_key=$ACCESS_KEY&s3_region=S3_REGION' INTO TABLE table_name ...;

# 兼容 S3 协议的对象存储
LOAD DATA /*+ [APPEND | DIRECT(need_sort,max_error,'inc_replace')] parallel(N) */ REMOTE_OSS INFILE 's3://$PATH/$FILENAME/?host=$HOST&access_id=$ACCESS_ID&access_key=$ACCESS_KEY' INTO TABLE table_name ...;
```
