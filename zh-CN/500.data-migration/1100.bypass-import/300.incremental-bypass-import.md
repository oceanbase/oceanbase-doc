...
3. 使用旁路导入将表 `tbl4` 中的数据导入到表 `tbl3`。
   ...
   * 指定 `INSERT INTO SELECT` 语句的 Hint。
     ...
     * （可选）指定分区导入。
         ...
         ```shell
         obclient [test]> INSERT /*+ DIRECT(true, 0, 'inc_replace') enable_parallel_dml parallel(16) */ INTO tbl3 partition(p0, p1) SELECT t2.col1,1 FROM tbl4 partition(p0s, p1) t2;
         Query OK, 3 rows affected
         Records: 3  Duplicates: 0  Warnings: 0
         ```
