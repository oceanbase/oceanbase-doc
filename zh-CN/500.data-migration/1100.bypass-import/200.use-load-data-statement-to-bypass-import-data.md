...
`LOAD DATA` 进行旁路数据导入时，对三种类型的存储设备：服务器磁盘（server_disk）、云对象存储（oss）和客户端磁盘（client_disk）通配符支持规则如下：

- 服务器磁盘（server_disk）：

  * 若不使用通配符，那么无论输入单个文件路径还是多个文件路径，需要由 `,` 分隔的文件路径，并且所有文件都必须实际存在。
  * 若使用通配符，单个文件路径至少要能匹配到一个存在的文件。
  * 多个文件路径不支持通配符。

- 云对象存储（oss）:

- 目录前缀不支持通配符。
- 文件名中的通配符只支持星号 `*` 和问号 `?` 两种。
