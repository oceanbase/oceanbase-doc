...
```sql
LOAD DATA /*+ direct(need_sort,max_error,load_mode) parallel(N) */ [REMOTE_OSS | LOCAL] INFILE 'file_name' ...
```
...
| load_mode | 表示使用全量或者增量导入。值为字符串类型：<ul><li>`inc_replace`：表示增量旁路导入。</li><li>`full`：表示全量旁路导入。</li></ul> 目前默认值是 full。|
