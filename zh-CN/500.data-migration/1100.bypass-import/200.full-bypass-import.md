...
OceanBase 数据库常用的对象存储格式如下：

```shell
# 阿里云 OSS
LOAD DATA /*+ [APPEND | DIRECT(need_sort,max_error,'full')] parallel(N) */ REMOTE_OSS INFILE 'oss://$PATH/$FILENAME/?host=$HOST&access_id=$ACCESS_ID&access_key=$ACCESS_KEY' INTO TABLE table_name ...;

# AWS S3
LOAD DATA /*+ [APPEND | DIRECT(need_sort,max_error,'full')] parallel(N) */ REMOTE_OSS INFILE 's3://$PATH/$FILENAME/?host=$HOST&access_id=$ACCESS_ID&access_key=$ACCESS_KEY&s3_region=S3_REGION' INTO TABLE table_name ...;

# 兼容 S3 协议的对象存储
LOAD DATA /*+ [APPEND | DIRECT(need_sort,max_error,'full')] parallel(N) */ REMOTE_OSS INFILE 's3://$PATH/$FILENAME/?host=$HOST&access_id=$ACCESS_ID&access_key=$ACCESS_KEY' INTO TABLE table_name ...;
```
...
* `S3_REGION`：Amazon S3 存储桶所在的区域。对象存储为 AWS S3 时，必须指定的参数。
...
2. 使用旁路导入将表 `t1_wen` 中的数据导入到表 `tbl1` 的 `p0`、`p1` 分区中。

   ```shell
   obclient [test]> insert /*+ direct(true, 0, 'full') enable_parallel_dml parallel(3) append */ into tbl1 partition(p0,p1) select * from t1_wen where col1 <20;
   ```
