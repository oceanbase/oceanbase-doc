...
* 在 `LOAD DATA` 语句和 `INSERT INTO SELECT` 语句中使用指定分区旁路导入时需要注意：
  ...
  * 如果最后一级分区为 hash/key 分区，则不支持分区旁路导入。
...
3. 使用旁路导入将表 `tbl2` 中的数据导入到表 `tbl1`。
   ...
   * 指定 `INSERT INTO SELECT` 语句的 Hint。
     ...
     * （可选）指定分区导入。
         ...
         ```shell
         obclient [test]> INSERT /*+ DIRECT(true, 0, 'full') enable_parallel_dml parallel(16) */ INTO tbl1 partition(p0, p1) SELECT t2.col1,1 FROM tbl2 partition(p0s, p1) t2;
         Query OK, 3 rows affected
         Records: 3  Duplicates: 0  Warnings: 0
         ```
...
3. 使用旁路导入将表 `tbl4` 中的数据导入到表 `tbl3`。
   ...
   * 指定 `INSERT INTO SELECT` 语句的 Hint。
     ...
     * （可选）指定分区导入。
         ...
         ```shell
         obclient [test]> INSERT /*+ DIRECT(true, 0, 'full') enable_parallel_dml parallel(16) */ INTO tbl3 partition(p0, p1) SELECT t2.col1,1 FROM tbl4 partition(p0s, p1) t2;
         Query OK, 3 rows affected
         Records: 3  Duplicates: 0  Warnings: 0
         ```
