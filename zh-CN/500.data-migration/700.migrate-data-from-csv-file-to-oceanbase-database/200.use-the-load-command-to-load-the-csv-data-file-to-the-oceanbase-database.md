...
OceanBase 数据库在 V4.3.5 BP2 版本，`LOAD DATA` 语句导入 URL 外表语法开始新增指定错误诊断模式。允许将失败的行记录下来（当前版本记录于 warning buffer 中），而不是让整个操作因第一个错误而终止。配合 `REJECT LIMIT` 子句，可以控制允许的错误行数。
