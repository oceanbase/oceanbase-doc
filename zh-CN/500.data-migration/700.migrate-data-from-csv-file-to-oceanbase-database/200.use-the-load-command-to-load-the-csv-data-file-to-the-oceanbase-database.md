...
<main id="notice" type='notice'>
  <h4>注意</h4>
  <p><ul><li>OceanBase 数据库 Oracle 模式只支持加载位于 OSS 和服务器端（OBServer 节点）的数据文件，即 CSV 文件必须位于 OBServer 节点的本地文件系统上，当前版本不支持从远程客户端加载数据。</li><li>OceanBase 数据库 MySQL 模式支持加载位于 OSS、服务器端（OBServer 节点）和客户端（本地）的数据文件。</li></ul></p>
</main>
