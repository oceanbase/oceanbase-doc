...
本文档主要介绍使用 Canal 和 OceanBase Binlog 从 OceanBase 数据库同步数据至 MySQL 数据库。
...
* OceanBase Binlog：一个轻量级的 OceanBase Binlog 日志代理服务，能够高效地解析 OceanBase 日志，并将其转化为 MySQL Binlog 协议格式。

* Canal：一款 MySQL 增量日志解析工具，可以实时解析、订阅和消费 MySQL Binlog 数据。
