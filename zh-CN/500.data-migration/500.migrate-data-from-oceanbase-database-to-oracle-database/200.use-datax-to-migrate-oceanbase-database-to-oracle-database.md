...
详细功能和参数说明请参考官方说明：[`oraclewriter` 插件](https://github.com/alibaba/DataX/blob/master/oraclewriter/doc/oraclewriter.md)。
...
|    参数    |          描述    |
|----------|--------------------|
| name | 描述的是连接数据库的 reader 或 writer 对应的数据库插件的名称。其中 MySQL 的 reader 插件为：mysqlreader，OceanBase 的 writer 插件为 oceanbasev10writer。具体 reader 和 writer 的插件可以参考 datax的文档：[DataX 数据源指南](https://github.com/alibaba/datax)。|
