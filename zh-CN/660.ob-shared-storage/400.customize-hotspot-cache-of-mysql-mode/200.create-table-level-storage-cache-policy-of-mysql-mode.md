...
|slug|create-table-level-hot-cold-storage-strategy|

# 创建表级冷热存储策略

本文将向您介绍如何使用 SQL 语句创建表级冷热存储策略（`STORAGE_CACHE_POLICY`）：
...
在共享存储模式下，才可以在创建表/索引时指定冷热存储策略。

## 创建普通表的冷热存储策略

创建表时指定表级别冷热存储策略是在 `CREATE TABLE` 语句后增加表选项 `STORAGE_CACHE_POLICY`，语法格式如下：
...
* 创建表 `tbl2_date`，指定 `DATE`（`DATE`/`DATETIME`/`TIMESTAMP`/`YEAR`/`DAY` 等）时间属性的列，热数据维持 1 个月后，直接变为冷数据。
...
* 创建表 `tbl2_int`，指定 `BIGINT`（`INT`/`BIGINT` 等）整型的列，需要指定 `BOUNDARY_COLUMN_UNIT`，默认按照 UTC 时间戳处理。热数据维持 1 个月后，变为 `auto` 数据，一周以后再转为冷数据。
...
* 创建索引时，指定索引的表级冷热存储策略。
* 创建表时创建索引，指定索引的表级冷热存储策略。
* 给已有表增加索引时，指定索引的表级冷热存储策略。

更多有关创建索引的详细介绍，参见 [创建索引](../../700.reference/300.database-object-management/100.manage-object-of-mysql-mode/500.manage-indexes-of-mysql-mode/200.create-an-index-of-mysql-mode.md)。
...
* [修改表级冷热存储策略](400.modify-table-level-storage-cache-policy-of-mysql-mode.md)
...
* [ALTER TABLE](../../700.reference/500.sql-reference/100.sql-syntax/200.common-tenant-of-mysql-mode/600.sql-statement-of-mysql-mode/1600.alter-table-of-mysql-mode.md)
* [CREATE INDEX](../../700.reference/500.sql-reference/100.sql-syntax/200.common-tenant-of-mysql-mode/600.sql-statement-of-mysql-mode/2200.create-index-of-mysql-mode.md)
