...
* 您已经规划磁盘。详细操作请参考 [规划磁盘](./7.plan-disks-optional.md)。
  
* 您已经配置时钟源。详细操作请参考 [配置时钟源](./6.configure-the-clock-source-optional.md)。
  
* 每台机器上已创建 admin 用户。详细操作请参考 [创建用户](./11.create-a-user-optional.md)。
