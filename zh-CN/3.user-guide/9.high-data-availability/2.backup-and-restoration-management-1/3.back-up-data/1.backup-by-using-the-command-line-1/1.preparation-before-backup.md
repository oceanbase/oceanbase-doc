...
#docslug#/oceanbase-database/oceanbase-database/V4.0.0/preparation-before-backup
...
2. 配置备份目的端。
   ...
   目前，OceanBase 数据库支持 NFS 和阿里云 OSS 作为备份目的端。
   ...
   * 阿里云 OSS
     ...
     * `tagging`：表示清理模式为对满足清理要求的备份文件设置 Tag，备份文件将仍然保留。
     ...
     设置 `delete` 模式或 `tagging` 模式后，自动清理备份数据的相关操作请参见 [自动清理备份的数据](../7.automatically-delete-backed-up-data-1.md)。
...
* （建议）停止清理备份的任务。
  ...
  具体操作请参见 [停止清理备份数据](../9.stop-cleaning-backup-data.md)。
