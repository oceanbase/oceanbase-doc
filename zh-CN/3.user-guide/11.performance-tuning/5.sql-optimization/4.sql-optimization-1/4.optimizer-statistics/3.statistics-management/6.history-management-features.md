...
OceanBase 数据库优化器使用如下内部表来保存统计信息历史记录：

* `__all_table_stat_v2_history` 用于保存表级的历史统计信息。

  

* `__all_column_stat_v2_history` 用于保存列级的基本历史统计信息。

  

* `__all_histogram_stat_v2_history` 用于保存列级的直方图历史统计信息。
