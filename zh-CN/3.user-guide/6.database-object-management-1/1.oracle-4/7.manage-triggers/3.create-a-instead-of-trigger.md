...
INSTEAD OF 触发器是在不可编辑视图或不可编辑视图的嵌套表的列上创建的 DML 触发器。数据库触发 INSTEAD OF 触发器而不是运行触发的 DML 语句。

INSTEAD OF 触发器不能带有条件。INSTEAD OF 触发器是更新不可编辑视图的唯一方法。

INSTEAD OF 触发器是一个行级触发器。INSTEAD OF DML 触发器可以读 OLD 和 NEW 的值，但不能修改 OLD 和 NEW 的值。
