# 登录失败处理策略
#docslug#/oceanbase-database/oceanbase-database/V4.0.0/logon-failure-handling-policy
...
1. 创建 Profile。
   ...
   * `integer | UNLIMITED | DEFAULT`：用于指定参数值的取值范围，其中：
   ...
   更多 `CREATE PROFILE` 语句的详细信息，请参见 [CREATE PROFILE](../../../../../4.development-guide-refactoring-1/5.sql-syntax/3.common-tenant-oracle-mode/9.sql-statement-1/1.DDL-1/19.create-profile.md)。
...
2. 应用 Profile 至用户。
   ...
   * 在创建用户时，直接指定已创建的 Profile 文件。
     ...
     更多 `CREATE USER` 语句的详细信息，请参见 [CREATE USER](../../../../../4.development-guide-refactoring-1/5.sql-syntax/3.common-tenant-oracle-mode/9.sql-statement-1/1.DDL-1/27.create-user-1.md) **。**
   ...
   * 如果用户已创建，则可以通过修改用户的 Profile 文件将已创建的 Profile 文件应用至用户。
     ...
     更多 `ALTER USER` 语句的详细信息，请参见 [ALTER USER](../../../../../4.development-guide-refactoring-1/5.sql-syntax/3.common-tenant-oracle-mode/9.sql-statement-1/1.DDL-1/12.alter-user-1.md) 。
