...
2. 通过 `ALTER SYSTEM RESTORE TENANT` 命令调度租户恢复任务, 恢复到一个`备租户`

   对于恢复到`备租户`来说，`RESTORE TENANT` 命令的内部流程如下：

   1. 创建恢复用的租户。

   2. 创建恢复用的租户的日志流。

   3. 恢复用户日志流数据,并回放用户表日志。
...
3. 通过 `ALTER SYSTEM ACTIVATE  STANDBY TENANT` 命令调度 `备租户` 转成 `主租户`。
