# 修改表的 Locality
#docslug#/oceanbase-database/oceanbase-database/V4.0.0/modify-the-locality-of-a-table
...
```sql
obclient> SELECT tenant_id, table_name, job_type, job_status 
...
>**说明**
>
>租户级 Locality 变更也可使用上述语句进行查询，返回结果中的 `tenant id` 列中会显示变更对象的具体值。对于表级 Locality 的变更，返回结果中的 `tenant_id` 为 NULL。
...
执行后等待 `oceanbase.DBA_OB_TENANT_JOBS` 视图中对应任务记录的状态变为 `SUCCESS`，即表示创建或修改完成。
