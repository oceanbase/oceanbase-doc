...
# docslug#/oceanbase-database/oceanbase-database/V4.0.0/create-a-partition-table-2
...
OceanBase 数据库 Oracle 模式的一级分区支持的分区类型如下：Range 分区、List 分区、Hash 分区和 Interval 分区。

OceanBase 数据库 Oracle 模式的二级分区支持情况如下：
...
| Hash + List   | 支持         | 支持          |
| Hash + Hash   | 支持         | 支持          |
|Interval + Range|支持|不支持|
|Interval + List |支持|不支持|
|Interval + Hash |支持|不支持|
...
| column_type    | 指定列的数据类型。 |
| expr           |一个有效的数字表达式。|
| STORE IN       |兼容从 Oracle 数据库导出的 DDL。`tablespace`表示 Oracle 数据库的表空间。|
| partition_name | 指定一级分区名称。 |

### 示例

创建 Interval 类型的按月分区的一级分区表 tbl_i。
...
partition_option:
  PARTITION BY 
     RANGE(expr){subpartition_option} (range_partition_list)
   | LIST(expr){subpartition_option} (list_partition_list)
   | HASH(expr){subpartition_option} { (hash_partition_list)
                                             | PARTITIONS partition_count }
   | PARTITION BY RANGE (column_name) INTERVAL ( expr )
        {subpartition_option} (interval_partition_list)

subpartition_option:
  SUBPARTITION BY 
    RANGE(expr) SUBPARTITION TEMPLATE(range_subpartition_list)
   | LIST(expr) SUBPARTITION TEMPLATE(list_subpartition_list)
   | HASH(expr) { SUBPARTITION TEMPLATE (hash_subpartition_list)
...
range_partition:
  PARTITION partition_name VALUES LESS THAN {(expr_list) | MAXVALUE}

range_subpartition_list:
  range_subpartition [, range_subpartition ...]

range_subpartition:
  SUBPARTITION subpartition_name VALUES LESS THAN {(expr_list) | MAXVALUE}

list_partition_list:
  list_partition [, list_partition ...]

list_partition:
  PARTITION partition_name VALUES {(expr_list) | DEFAULT}

list_subpartition_list:
  list_subpartition [, list_subpartition ...]

list_subpartition:
  SUBPARTITION subpartition_name VALUES {(expr_list) | DEFAULT}
...
### 创建 Interval + Range 分区的模板化二级分区表

示例如下：

```sql
obclient> CREATE TABLE tbl_ir( 
  id             NUMBER(6),
  create_time    DATE,
  type           CHAR(1),
  amount         NUMBER(6),
...
### 创建 Interval + List 分区的模板化二级分区表

示例如下：

obclient> CREATE TABLE tbl_il(
  id             NUMBER(6),
  create_time    DATE,
  type           CHAR(1),
  amount         NUMBER(6),
...
### 创建 Interval + Hash 分区的模板化二级分区表

示例如下：

```sql
obclient> CREATE TABLE tbl_ih( 
  id             NUMBER(6),
  create_time    DATE,
  type           CHAR(1),
  amount         NUMBER(6),
