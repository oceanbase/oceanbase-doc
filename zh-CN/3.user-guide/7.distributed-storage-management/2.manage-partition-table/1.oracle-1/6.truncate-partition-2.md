# Truncate 分区
#docslug#/oceanbase-database/oceanbase-database/V4.0.0/truncate-partition-2
...
* 对于一级分区表，当前仅支持对 Range 分区和 List 分区执行 Truncate 分区操作，暂不支持对 Hash 分区和 Interval 分区执行 Truncate 分区操作。
...
* Truncate 分区时，请尽量避免待 Truncate 的分区上存在活动的事务或查询，否则可能会导致 SQL 语句报错，或者出现一些异常情况。您可以在 `sys` 租户下，通过事务相关视图 `oceanbase.GV$OB_TRANSACTION_PARTICIPANTS` 查询当前还未结束的事务上下文状态。
