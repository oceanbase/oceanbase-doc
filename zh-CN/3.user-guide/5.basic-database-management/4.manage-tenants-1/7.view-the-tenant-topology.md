...
* 您可以通过 `oceanbase.v$ob_cluster` 视图查看当前集群的基本信息。
  ...
  `oceanbase.v$ob_cluster` 视图的更多说明信息请参见 [v$ob_cluster](../../13.system-reference/1.reference-mysql-mode/1.system-view-4/3.performance-view-5/77.v-ob_cluster-2.md)。

  `oceanbase.v$ob_cluster` 视图的更多说明信息请参见《参考指南（MySQL 模式）》文档中 **系统视图** 的 **v$ob_cluster** 。
