...
* 如果命令很快执行失败，那么可能是日志不同步，需要检查 `DBA_OB_ROOTSERVICE_EVENT_HISTORY` 视图，确认是否有 Stop Server 的记录。如果没有记录，则说明在 RS 没执行 Stop Server 操作前就失败了；如果有记录，则说明是日志不同步导致 Stop Server 操作执行失败。
...
* 执行 Stop Server 操作后，`DBA_OB_ROOTSERVICE_EVENT_HISTORY` 视图中该 Server 的 `status` 字段仍为 `Active` 不变，但 `stop_time` 字段的值由 `0` 变为 Stop Server 的时间点，同时 OBServer 处于 `stopped` 状态。
