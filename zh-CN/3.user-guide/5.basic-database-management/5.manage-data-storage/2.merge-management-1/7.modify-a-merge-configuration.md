...
| enable_manual_merge           | 集群级配置项，是否开启手动合并。 <blockquote>**说明**  </br>`enable_manual_merge` 参数设置为开启后，表示需要手动对集群的 Zone 逐个进行合并。</blockquote> | false | <ul><li>true：开启</li> <li>* false：不开启</li></ul>        |
| merger_check_interval         | 集群级配置项，每个 Zone 的合并进度检查间隔。         | 10m   | \[10s, 60m\]           |
| merger_completion_percentage  | 集群级配置项，合并好的分区数或数据大小比例。            | 100   | \[5, 100\]             |
