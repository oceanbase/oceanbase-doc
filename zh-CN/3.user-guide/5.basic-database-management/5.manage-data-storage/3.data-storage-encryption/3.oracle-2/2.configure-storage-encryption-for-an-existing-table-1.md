...
10. 设置表的 `progressive_merge_num` 值，给表做全量合并或者渐进合并。
    ...
    * 对表执行渐进合并
      ...
      2. 使用 `root` 用户登录数据库的 `sys` 租户，手动发起多轮渐进合并，使表和其上的索引的所有存量宏块完成加密。
         ...
         更多配置项 `max_kept_major_version_number` 的说明信息请参见 [max_kept_major_version_number]()。
...
11. 完成后，您可以查看以下视图，确认所有宏块是否已完成加密。
    ...
    更多视图 `v$encrypted_tables` 的字段及说明信息请参见 [V$ENCRYPTED_TABLES](../../../../13.system-reference/2.reference-oracle-mode/1.system-view-5/3.performance-view-6/77.v-encrypted_tables-4.md)。
