...
2. 验证主密钥配置。
   ...
   1. 使用 `root` 用户登录到数据库的 `sys` 租户后，执行以下语句，记录当前密钥。
      ...
      ```sql
      obclient> SELECT * FROM oceanbase.__all_virtual_tenant_keystore;
      ```
...
3. 确认表空间内的表是否标记为加密。
   ...
   2. 执行以下语句，查看 `encryptionalg` 是否为之前设置的 `aes-256`。

      ```sql
      obclient> SELECT table_name,encryptionalg,encrypted FROM sys.V$OB_ENCRYPTED_TABLES;
      +------------+---------------+-----------+
