...
| HIGH_VALUE            | NUMBER        | NO             | 子分区表达式                  |
| HIGH_VALUE_LENGTH     | NUMBER        | NO             | 子分区表达式的长度               |
| SUBPARTITION_POSITION | VARCHAR2(30)  | NO             | 子分区在分区中的位置              |
| TABLESPACE_NAME       | NUMBER        | NO             | 包含子分区的表空间的名称            |
...
| FLASH_CACHE           | VARCHAR2(3)   | NO             | 目前暂不支持该字段，当前该字段默认为 NULL |
| CELL_FLASH_CACHE      | VARCHAR2(3)   | NO             | 目前暂不支持该字段，当前该字段默认为 NULL |
