...
1. 增加租户内存配置。建议直接在 OCP 执行租户内存扩容操作，详见 OCP 使用指南。命令行修改详见如下步骤：
   ...
   2. 执行生成的 SQL 语句。
      ...
      ```sql
      ALTER resource unit unit_config_name min_cpu=2,max_cpu=2,max_iops=10000,min_iops=10000;
      ```
...
2. 调高转储线程数，加快转储，让内存尽快释放。
   ...
   您还可以在 OCP 界面通过修改转储策略调高转储线程数，具体参见[修改转储配置](../../../../5.basic-database-management/5.manage-data-storage/1.dump-management-1/5.modify-dump-configuration.md)。
