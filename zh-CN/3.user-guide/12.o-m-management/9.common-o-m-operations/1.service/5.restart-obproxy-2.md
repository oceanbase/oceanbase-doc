...
# docslug#/oceanbase-database/oceanbase-database/V4.0.0/restart-obproxy-2
...
4. 在 OBProxy 列表中选择需要重启的 OBProxy，单击 **操作** 列的 **重启** 。
   ...
   如需重启 OBProxy 集群中所有 OBProxy，可通过 [重启 OBProxy 集群](../../../5.database-connection-and-routing/2.obproxy-management/2.manage-obproxy-clusters/5.restarts-all-obproxy-nodes-in-a-obproxy-cluster.md) 实现。
...
* 当没有 OCP 提供 config server 服务时，proxy 可以依赖 observer 的 rslist 直接启动。
  ...
  ```sql
  ./bin/obproxy -p2883 -r'xx.xx.xx.xx:2881;xx.xx.xx.xx:2881;xx.xx.
  xx.xx:2881' -o proxy_idc_name='hz001' -c 'xxbank' -n trade
  ```
