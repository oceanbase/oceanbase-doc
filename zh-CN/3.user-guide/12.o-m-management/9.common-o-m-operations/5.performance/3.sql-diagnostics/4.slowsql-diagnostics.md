...
# docslug#/oceanbase-database/oceanbase-database/V4.0.0/slowsql-diagnostics
...
<!-- 1. 黑屏或使用 OceanBase 开发者中心（OceanBase Developer Center，ODC）执行如下 SQL 修改 SlowSQL 判定的默认阈值为 120000 微秒。
   ...
   ```sql
   obclient> SELECT collect_elapsed_threshold_us from oceanbase.ob_agent_collection_config_history 
...
修改 OCP 系统参数 `ocp.ob.slowsql.threshold` 的值为 120000 微秒。
...
4. 对 SlowSQL 进行筛选。
   ...
   1. 配置筛选条件
      ...
      * 时间范围：可在 **时间范围** 下拉框中选择近 5 分钟、近 10 分钟、近 20 分钟、近 30 分钟、近 1 小时、近 3小时来快速选择时间范围；也可在下拉框中选择自定义时间，并配置开始时间和结束时间。默认显示近 30 分钟内的信息。
