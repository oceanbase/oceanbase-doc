...
向 Zone 内添加 OBServer 节点的操作可以通过 SQL 语句和 OCP 平台来完成。
...
3. 添加成功后，根据业务实际情况，调整租户的资源配置，即调大 `UNIT_NUM`。
   ...
   本案例中，需要将 UNIT_NUM 数量修改为 3，示例如下：
...
假设当前集群中共包含 3 个可用区 `z1`、`z2`、`z3`，每个 Zone 内包含 2 台 OBServer。现在希望向每个 Zone 内添加 1 台 OBServer 来扩容，则扩容后，3 个 Zone 内均包含 3 台 OBServer。

>**注意**
>
>向 Zone 内添加 OBServer 节点时，需要保证集群中的每个 Zone 中的 OBserver 数量均相等。

### 添加主机操作

在扩容前，需要先添加主机，添加主机的相关操作请参见 [在集群中增加 Zone](1.add-a-zone-to-the-cluster.md) 中的 **添加主机操作**。
...
3. 添加成功后，再根据业务需要，修改租户的资源池，即在每个 Zone 内增加 Unit 数量。
