...
* 错误原因：FOLLOWS 或 PRECEDES 子句中引用的触发器是不同的类型或不是复合触发器。

* 解决方式：将引用的触发器创建为相同类型或复合触发器。
...
* 错误原因：创建 FORWARD CROSSEDITION 触发器或常规触发器时指定了 PRECEDES 子句。

* 解决方式：不要创建 FORWARD CROSSEDITION 或带有 PRECEDES 子句的常规触发器。
