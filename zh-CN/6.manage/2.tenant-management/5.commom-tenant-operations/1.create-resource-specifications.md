...
4. 通过 `CREATE RESOURCE UNIT` 语句，创建名为 `S1_unit_config` 的资源规格。
   ...
   * MAX_CPU：租户分配的最大的 CPU 规格，所有租户的 MAX_CPU 的总和不能超过该节点 CPU 总容量的超卖值：CPU_CAPACITY * resource_hard_limit。
   * MIN_CPU：租户分配的最小的CPU规格，所有租户的 MIN_CPU 的总和不能超过该节点 CPU 总容量 CPU_CAPACITY。
   * MEMORY_SIZE：内存规格，OceanBase 4.0 开始不支持内存超卖。
   * LOG_DISK_SIZE：日志盘空间，OceanBase 4.0 会按租户管理日志盘空间，系统为各个租户预留日志盘空间，实现按租户隔离。LOG_DISK_SIZE 默认值是 MEMORY_SIZE * 3，最小为 2 G。
