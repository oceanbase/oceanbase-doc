...
删除资源单元前必须确保当前资源单元未被使用。如果资源单元正在被使用，则需要先将资源单元从资源池中移出后再删除资源单元。详细操作，请参见 [修改资源池配置](9.modify-resource-pool-properties.md)。
...
3. 通过 `DBA_OB_UNIT_CONFIGS` 和 `DBA_OB_RESOURCE_POOLS` 视图，确认资源规格的使用信息。如果查询结果的 NAME 列中无该资源单元名称，则表示该资源单元未被指定给资源池；如果查询结果的 NAME 列中有该资源单元名称，则表示资源单元已被指定给资源池。示例如下，确认资源规格 `S2_unit_config` 已经不在使用中。

    ```sql
    obclient [oceanbase]> SELECT a.UNIT_CONFIG_ID, a.NAME, b.NAME FROM oceanbase.DBA_OB_UNIT_CONFIGS a,oceanbase.DBA_OB_RESOURCE_POOLS b WHERE b.UNIT_CONFIG_ID=a.UNIT_CONFIG_ID;
    +----------------+-------------------------------+--------------------------+
    | UNIT_CONFIG_ID | NAME                          | NAME                     |
    +----------------+-------------------------------+--------------------------+
...
4. 通过 `DROP RESOURCE UNIT` 语句，删除 `S2_unit_config` 资源规格。
    ...
    `DROP RESOURCE UNIT` 语句的详细说明，请参见 [DROP RESOURCE UNIT](../../../7.reference/4.development-guide-refactoring/1.sql-syntax/1.system-tenants/10.drop-resource-unit.md)。

5. 查看 `DBA_OB_UNIT_CONFIGS` 视图，确认删除成功。
