...
3. 通过 `DBA_OB_TENANTS` 视图，确认租户 mq_t1 已经被删除。
...
4. 通过 `show recyclebin` 语句，查看当前系统租户 recyclebin 中的回收站对象。
...
5. 通过 `FLASHBACK TENANT` 语句恢复回收站中的 `mq_t1` 租户。
...
6. 查看 `DBA_OB_TENANTS` 视图和回收站确认执行结果。
