...
4. 通过 `CREATE TENANT` 语句，创建租户。
   ...
   * ob_tcp_invited_nodes：用于指定租户连接的白名单，即允许哪些客户端 IP 连接该租户。示例中 `%` 表示所有客户端都可登录，如果不调整 ob_tcp_invited_nodes 的值，则默认租户的连接方式为只允许本机的 IP 连接数据库。白名单的详细介绍，参见 [查看和设置白名单](4.view-and-set-whitelist.md)。
