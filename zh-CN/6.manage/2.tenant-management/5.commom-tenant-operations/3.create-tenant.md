...
创建新租户时，如果当前的资源池均被使用（被其他租户使用），则需要创建新的资源池。在创建资源池前，需要先确定资源规格和资源使用范围。所以创建租户的顺序为：资源规格 -> 资源池 -> 租户。
  
有关创建资源池和资源单元的详细操作，参见 [创建资源规格](1.create-resource-specifications.md) 和 [创建资源池](2.create-resource-pool.md)。
