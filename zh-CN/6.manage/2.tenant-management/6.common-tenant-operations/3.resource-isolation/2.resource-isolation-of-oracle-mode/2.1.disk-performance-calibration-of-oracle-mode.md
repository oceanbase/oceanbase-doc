...
1. 使用 `root` 用户登录到集群的 `sys` 租户。
   ...
   有关更加详细的连接数据库的操作指引，请参见 [连接数据库概述](../../../../../3.develop/1.connect-to-oceanbase-database/1.connect-to-oceanbase-database-of-mysql-mode/1.connection-methods-overview-of-mysql-mode.md)。
...
2. 根据业务使用场景，选择合适的命令，主动刷新磁盘校准信息。
   ...
   * `CALIBRATION_INFO`：指定待刷新的磁盘校准信息。如果不指定，则默认从内部表刷新磁盘性能信息。
       ...
       * `mode`：指定 I/O 模式，支持 `r`、 `w`、`read` 或 `write`。指定 I/O 模式时，读、写模式都至少需要指定一条记录，否则系统会报错。
