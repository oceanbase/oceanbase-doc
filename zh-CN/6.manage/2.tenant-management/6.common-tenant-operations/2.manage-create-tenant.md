...
3. 通过 `DBA_OB_UNIT_CONFIGS` 视图，获取已有的资源规格信息。
   ...
   `DBA_OB_UNIT_CONFIGS` 视图的详细说明，参见 [DBA_OB_UNIT_CONFIGS](../../../7.reference/7.system-views/4.system-view-of-mysql-mode/2.dictionary-view-of-mysql-mode/60.oceanbase-dba_ob_unit_configs-of-mysql-mode.md)。
...
4. 通过 `CREATE RESOURCE UNIT` 语句，创建资源规格。
    ...
    * OceanBase 数据库 V4.0 支持租户间 IOPS 隔离，通过 `MAX_IOPS`、`MIN_IOPS`、`IOPS_WEIGHT` 三个参数决定。IOPS 规格参数默认不需要指定，系统根据 CPU 规格自动计算。
    ...
    `CREATE RESOURCE UNIT` 语句的详细说明，参见 [CREATE RESOURCE UNIT](../../../7.reference/5.sql-reference/1.sql-syntax/1.system-tenants/7.create-resource-unit.md)。
...
4. 通过 `CREATE RESOURCE POOL` 语句，创建资源池。
    ...
    * `ZONE_LIST`：资源池的 Zone 分布。OceanBase 数据库会在 `ZONE_LIST` 描述的每个 Zone 里面创建 `UNIT_NUM` 个 Unit，每个 Unit 的资源规格为 `UNIT` 参数设置的值。
    ...
    `CREATE RESOURCE POOL` 语句的详细说明，参见 [CREATE RESOURCE POOL](../../../7.reference/5.sql-reference/1.sql-syntax/1.system-tenants/6.create-resource-pool.md)。
...
3. 通过 `DBA_OB_TENANTS` 视图，查看所有的租户信息。
   ...
   `DBA_OB_TENANTS` 视图的详细说明，请参见 [DBA_OB_TENANTS](../../../7.reference/7.system-views/4.system-view-of-mysql-mode/2.dictionary-view-of-mysql-mode/193.oceanbase-dba_ob_tenants-of-mysql-mode.md)。
...
4. 通过 `CREATE TENANT` 语句，创建租户。
    ...
    * `ENABLE_ARBITRATION_SERVICE`：指定租户是否开启仲裁服务，如果不显示指定，默认创建的租户其仲裁服务为关闭状态。如果创建租户的时候未开启仲裁服务，可以在创建租户后再开启仲裁服务，详细操作请参见 [开启仲裁服务](../../4.high-availability/4.arbitration-high-availability/2.enable-the-arbitration-service.md)。
    ...
    * `system_var_name`：指定系统变量或配置项。
    ...
    `CREATE TENANT` 语句的详细说明，参见 [CREATE TENANT](../../../7.reference/5.sql-reference/1.sql-syntax/1.system-tenants/8.create-tenant.md)。
...
租户创建成功后，您可以使用管理员用户登录并连接数据库，更多连接数据库的方法和详细操作请参见 [连接数据库概述（MySQL 模式）](../../../3.develop/1.application-development-of-mysql-mode/1.connect-to-oceanbase-database-of-mysql-mode/1.connection-methods-overview-of-mysql-mode.md) 和 [连接数据库概述（Oracle 模式）](../../../3.develop/1.application-development-of-mysql-mode/1.connect-to-oceanbase-database-of-mysql-mode/1.connection-methods-overview-of-mysql-mode.md)。如果您需要使用普通用户登录，则需要为租户创建新的普通用户并授权，创建用户并授权的相关操作请参见 [用户和权限](../../5.security-and-permissions/3.access-control/2.user-and-permission/1.user-and-permission-overview.md)。
