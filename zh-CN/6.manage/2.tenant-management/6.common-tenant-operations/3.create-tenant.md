...
2. [创建资源池](#步骤二：创建资源池)：创建资源池时会实际创建 Unit，按照规格定义分配资源，如对应节点预留资源不够将会创建失败，通过 `GV$OB_SERVERS` 视图可以查看所有节点资源分配信息。如果创建成功可以通过 `DBA_OB_RESOURCES_POOLS` 视图和 `DBA_OB_UNITS` 视图查看资源池及其对应 Unit。资源池不能复用，成功创建租户后指定资源池将会分配给租户。
...
4. 通过 `CREATE TENANT` 语句，创建租户。
    ...
    * `system_var_name`：指定系统变量或配置项。
    ...
    `CREATE TENANT` 语句的详细说明，参见 [CREATE TENANT](../../../7.reference/4.development-reference/1.sql-syntax/1.system-tenants/8.create-tenant-sql.md)。
