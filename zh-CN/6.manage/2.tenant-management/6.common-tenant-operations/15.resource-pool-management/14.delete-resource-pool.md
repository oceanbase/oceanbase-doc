...
3. 通过 `DBA_OB_RESOURCE_POOLS` 视图，确认资源池的配置信息。其中 `TENANT_ID` 为 NULL，说明资源池未被分配，可以执行删除。
   ...
   以查询 `mq_pool_02` 的配置信息为例：
    ...
    `DBA_OB_RESOURCE_POOLS` 视图的详细说明，参见 [DBA_OB_RESOURCE_POOLS](../../../../7.reference/7.system-views/4.system-view-of-mysql-mode/2.dictionary-view-of-mysql-mode/47.oceanbase-dba_ob_resource_pools-of-mysql-mode.md)。
...
4. 通过 `DROP RESOURCE POOL` 语句，删除资源池。
    ...
    `DROP RESOURCE POOL` 语句的详细说明，参见 [DROP RESOURCE POOL](../../../../7.reference/5.sql-reference/1.sql-syntax/1.system-tenants/9.drop-resource-pool.md)。
