...
创建租户的更多信息请参见 [创建租户](../2.manage-create-tenant.md)。
...
1. 集群中当前仅 `z1`、`z2`、`z3` 三个 Zone，需要在集群中增加 `z4`、`z5` 两个 Zone。
   ...
   在集群中增加 Zone 的具体操作请参见 [添加 Zone](../../../1.cluster-management/3.common-cluster-operations/8.add-a-zone.md)。

2. 在 `z4`、`z5` 两个 Zone 上各添加一台 OBServer 节点。

   向 Zone 内添加 OBServer 节点的具体操作请参见 [添加节点](../../../1.cluster-management/3.common-cluster-operations/4.add-a-node.md)。
...
4. 通过修改租户 Locality 来增加副本。
   ...
   根据 Locality 的变更规则，每次只能增加一个 Zone 内的 Locality，Locality 的变更规则相关信息请参见 [修改 Locality](../../../3.replica-management/2.replica-distribution/2.locality-common-operations/2.modify-locality.md)。
