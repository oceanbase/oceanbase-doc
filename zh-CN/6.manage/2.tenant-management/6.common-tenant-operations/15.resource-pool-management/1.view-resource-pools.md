...
2. 执行以下语句，查看集群中的资源池信息。
    ...
    * `TENANT_ID`：该资源池归属的租户 ID，每个租户对应一个租户 ID，`sys` 租户的租户 ID 为 `1`，当前租户详细信息的查看方法，请参见 [查看租户信息](../../4.manage-tenants/4.view-a-tenant-information.md)。
    ...
    * `UNIT_CONFIG_ID`：资源池对应的资源单元规格 ID。一个资源单元对应一个资源单元规格 ID。
       ...
       资源单元对应的资源单元规格（即资源单元配置）的查看方法，请参见 [查看资源单元配置](../16.resource-specification-management/1.view-resource-unit-configuration.md)。
