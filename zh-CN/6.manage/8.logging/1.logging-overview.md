...
OceanBase 数据库的单个日志文件大小不超过 256MB，当日志文件大小达到 256MB 时，系统会进行日志轮转。原日志文件追加时间为后缀（格式为 `yyyyMMddHHmmss`），时间为本日志文件中最后一条日志的生成时间，并生成新的日志文件。发生日志轮转时，`.wf` 日志文件即使没到 256MB 也会一起轮转。也就是说 `xxx.log.wf` 文件和 `xxx.log` 文件总是一一对应的，`.wf` 文件一般情况下远小于 256M。
...
OceanBase 数据库原生支持日志归档（通过集群配置项 `enable_syslog_recycle控制`），当某种日志文件数达到 `max_syslog_file_count` 时，日志轮转时删除最老的日志文件。日志文件最大空间使用可按如下方式评估: `max_syslog_file_count * 265M * 3 * (enable_syslog_wf ? 2 : 1)`。
...
* header 部分：
    ...
    * log_used_time：OBServer 支持同步日志与异步日志，开启同步日志时，该参数语义表示"上一条日志消耗的时间"；开启异步日志时，该参数表示"本条日志格式化的时间"。
    * log_used_time：上一条日志打印耗时，同步日志场景表示上一条日志内容生成时间+日志文件写入时间。异步日志场景表示上一条日志内容生成时间+提交到异步队列时间（通过集群配置项 `enable_async_syslog` 控制是否开启异步日志打印功能）。
    --- 需要确认上边两条哪条正确
