...
1. 对于分布了多个租户的 OB 节点，首先考虑通过扩容节点+迁移 Unit 的方式均衡数据。
   ...
   * 扩容节点
     ...
     扩容节点，具体步骤请参见 [添加节点](../../../1.cluster-management/3.common-cluster-operations/4.add-a-node.md)。

   * 手动迁移 Unit 到扩容节点。

     手动迁移 Unit 的具体步骤请参见 [Unit 迁移](../../../3.replica-management/2.replica-distribution/2.locality-common-operations/7.unit-migration.md)。迁移只能在单个 Zone 内进行。
