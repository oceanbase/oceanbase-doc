...
由于当前仅 `sys` 租户可以删除租户，故 MySQL 模式和 Oracle 模式的租户被删除后，只能进入 `sys` 租户的回收站中。对于 `sys` 租户，除了能查看本租户的数据库、表和索引等回收站对象外，还可以查看系统内所有的租户回收站对象；对于 MySQL 用户租户，只能查看数据库、表和索引的回收站对象；对于 Oracle 用户租户，只能查看表和索引的回收站对象。
