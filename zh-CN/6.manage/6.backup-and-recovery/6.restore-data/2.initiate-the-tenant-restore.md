...
4. （可选）在待恢复租户对应的 Meta 租户创建成功后，设置恢复任务相关的性能参数。

   目前，与恢复任务相关的配置项主要有两个：集群级别配置项 `restore_concurrency` 和租户级别配置项 `ha_low_thread_score`, 两个配置项均用于控制恢复任务的并发参数，但是用途不同。
   ...
   2. 设置配置项 `restore_concurrency` 和 `ha_low_thread_score`。

      集群级配置项 `restore_concurrency` 用于控制恢复是否允许进行。默认值为 0，取值范围为 [0, 512]，且该配置项将在 4.1 版本后废弃不再使用。有关配置项 `restore_concurrency` 的更多介绍，请参见 [restore_concurrency](../../../../7.reference/5.system-reference/1.system-configuration-items/3.cluster-level-configuration-items/170.restore_concurrency.md)。
