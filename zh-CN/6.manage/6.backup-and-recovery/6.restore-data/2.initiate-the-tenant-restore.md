...
3. （可选）如果需要加快恢复，可以配置项 `restore_concurrency` 适当调整恢复任务的并发度。

   集群级配置项 `restore_concurrency` 用于设置从备份恢复租户数据时的最大并发度。默认值为 0，取值范围为 [0, 512]。有关配置项 `restore_concurrency` 的更多介绍，请参见 [restore_concurrency](../../../../7.reference/5.system-reference/1.system-configuration-items/3.cluster-level-configuration-items/170.restore_concurrency.md)。

   语句示例如下：

   ```sql
   obclient [(none)]> ALTER SYSTEM SET restore_concurrency = 20;
   ```
