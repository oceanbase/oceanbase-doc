...
* 发起了 2 次日志归档，其 `round_id` 分别为 1、2。

  由于日志归档可能发生中断，故在计算恢复区间时还需要排除中间不连续的时间段，这样就会呈现出分段的可恢复区间，即图中大括号划出的范围：[scn 300, scn 500] 和 [scn 900, scn 1500]。

* 发起了 3 次数据备份，有效数据备份分别为 `full_backup_set_1`、 `full_backup_set_2`、`inc_backup_set_3`。

根据当前保留的有效数据备份以及日志的所有可恢复区间，物理恢复的可恢复区间为：当前保留的有效数据备份与日志的所有可恢复区间的并集，具体对应的区间如下：

\[`backup_full_set=1` 的 `MIN_RESTORE_SCN`,`log_archive_piece:2` 的 `CHECKPOINT_SCN`\] 和 \[`backup_full_set=2` 的 `MIN_RESTORE_SCN`, `log_archive_piece:3` 的 `CHECKPOINT_SCN`\]
