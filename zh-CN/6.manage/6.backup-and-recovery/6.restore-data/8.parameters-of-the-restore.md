...
物理恢复的可恢复区间如下图所示。其中，数据备份的右边界均为 `MIN_RESTORE_SCN`（例如，数据备份 `full_backup_set_1` 的 `MIN_RESTORE_SCN` 对应图中的 `scn 300`）；日志归档的右边界均为 `CEHCKPOINT_SCN`（例如，日志备份 `round 1` 的 `CEHCKPOINT_SCN` 对应图中的 `scn 500`）。
