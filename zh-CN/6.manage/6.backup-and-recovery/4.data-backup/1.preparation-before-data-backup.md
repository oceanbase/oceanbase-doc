...
在进行数据备份前，您需要确认已开启日志归档并配置 `DATA_BACKUP_DEST` 参数。由于系统租户不包含用户数据，是集群管理用的租户，不支持备份恢复，故 `sys` 租户无需配置备份 `DATA_BACKUP_DEST` 参数。
...
在执行全量数据备份前，需要先开启归档模式，仅当日志归档任务的 `STATUS` 为 `DOING` 时，才能开始数据备份，查看日志归档任务状态的相关操作请参见 [查看归档进度](../3.log-archive/6.view-log-archive-progress.md)。如果确认未开启归档模式，则需要先开启日志归档模式后，再执行数据备份，开启归档模式的相关操作请参见 [开启归档模式](../3.log-archive/3.open-the-log-archive-mode.md)。
...
2. 配置备份目的端。
   ...
   * 阿里云 OSS
      ...
      * `tagging`：表示清理模式为对满足清理要求的备份文件设置 Tag，备份文件将仍然保留。
      ...
      设置 `delete` 模式或 `tagging` 模式后，自动清理备份数据的相关操作请参见 [自动清理过期备份](../5.clear-backup-data/1.cleaning-up-backed-up-data-automatically.md)。
