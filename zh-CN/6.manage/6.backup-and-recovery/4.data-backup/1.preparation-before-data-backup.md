...
2. 配置备份目的端。
   ...
   <main id="notice" type='explain'>
   <h4>说明</h4>
   <p>如果通过 <code>sys<code> 租户配置备份目的端，则需要在语句中指定待发起备份的用户租户名（不能指定是 <code>sys</code> 租户），例如：<code>ALTER SYSTEM SET data_backup_dest='file:///data/nfs/backup/data' TENANT = mysql_tenant;</code>。</p>
   </main>
