...
本文主要介绍 OceanBase 数据库登录用户的密码复杂度介绍
...
|validate_password_policy|设置密码检查策略| <ul><li>LOW 策略仅包含密码长度检测。</li><li>MEDIUM 策略包括密码长度检测、大写字母个数检测、小写字母个数检测、数字个数检测、特殊字符个数检测、用户名密码相同检测。|
...
4. 验证密码复杂度,创建 2 个用户，分别设置为符合要求的密码和不符合要求的密码。
