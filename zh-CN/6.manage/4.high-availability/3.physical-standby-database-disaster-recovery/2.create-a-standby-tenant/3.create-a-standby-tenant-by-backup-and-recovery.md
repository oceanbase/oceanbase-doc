...
1. 主租户或源端的备租户开启归档模式，并保证归档状态一直为 `DOING`。
   ...
   3. 确认归档状态是否为 `DOING`。
      ...
      ```sql
      SELECT DEST_ID, ROUND_ID, DEST_NO, STATUS, CHECKPOINT_SCN, CHECKPOINT_SCN_DISPLAY, PATH FROM DBA_OB_ARCHIVELOG;
      ```
...
2. 主租户或源端的备租户开启数据备份，并完成一次全量备份。
   ...
   2. 执行数据备份。
      ...
      发起租户全量数据备份的详细操作，请参见 [发起数据备份](../../../6.backup-and-recovery/4.data-backup/2.initiate-full-data-backup.md)。
   ...
   3. 确认数据备份状态。
      ...
      如果备份正在执行，则该视图中会显示备份任务的进度信息。查看数据备份进度的详细操作，请参见 [查看数据备份进度](6.manage\6.backup-and-recovery\4.data-backup\5.view-data-backup-progress.md)。
...
5. 确认恢复是否完成。
   ...
   有关查看恢复进度的更多详细介绍，请参见 [查看恢复进度](../../../6.backup-and-recovery/6.restore-data/4.view-the-restore-progress.md)。

## 步骤三：开启日志持续同步

使用默认的 `RESTORE` 命令创建的备租户，不会连续同步主租户或备租户归档的日志，需要手动再执行 `RECOVER` 命令重新设置备租户的恢复终点，开启日志持续同步模式。
...
3. 再次查询 `DBA_OB_TENANTS` 视图，通过 `SYNC_SCN` 字段确认新备租户的同步位点。
   ...
   其中，`SCN_TO_TIMESTAMP(SYNC_SCN)` 列显示了备租户当前的同步进度。
