...
当前仅 `sys` 租户可以查看租户的回收站对象；其他用户租户只能查看数据库、表和索引的回收站对象。
...
2. 执行以下语句，查看回收站对象。
   ...
   * `TYPE`：表示回收站对象所属的类型。其中：
      ...
      * MySQL 模式中，用户租户主要支持 `INDEX`、`TABLE` 和 `DATABASE` 等三种类型。系统租户（`sys` 租户）支持 `INDEX`、`TABLE`、`DATABASE` 和 `TENANT` 等四种类型。本示例中，`oracle001` 即为一个租户类型的回收站对象。

      * Oracle 模式中，主要支持 `INDEX` 和 `TABLE` 两种类型。
...
* [清理回收站](3.purge-the-recycle-bin.md)
