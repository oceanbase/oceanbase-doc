...
* 在使用 NFS 作为备份介质时，必须保证所有 OBServer 服务器都挂载了同一个服务器的 NFS。同时，为保证备份的顺利进行，务必使用本文档中建议的参数挂载 NFS。挂载 NFS 具体操作请参见本节 **部署 NFS 客户端**。

* 在重启 OBServer 服务器时，需要先启动 NFS，再启动 OBServer 服务器。

* 添加新的机器后，在启动 OBServer 服务器前，需要保证新的机器挂载 NFS 成功或者可以备份到其他介质。
...
部署 NFS 客户端时，需要在所有 OBServer 服务器上进行操作。

以下以在某一台 OBServer 服务器上的操作为例，提供操作指导。
