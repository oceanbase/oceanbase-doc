...
>**说明**
>
>删除节点会减少可用资源，如果同 Zone 中其他节点的剩余资源不足以容纳待删除节点上的 Unit，将会导致 Unit 迁移失败，故在执行删除节点之前建议先查询 `oceanbase.GV$OB_SERVERS` 视图判断 Zone 内各 OBServer 的资源使用情况。
...
2. （可选）手动迁移待删除节点上的 Unit。
   ...
   >**说明**
   >
   >建议您手动迁移待删除的节点上的 Unit 后再执行 Delete Server 操作。
