...
2. 对问题节点执行隔离节点操作。
   ...
   2. 对问题节点执行 Force Stop Server 操作。
      ...
      ```sql
      ALTER SYSTEM FORCE STOP SERVER 'svr_ip:svr_port'; 
      ```
   ...
   3. 对问题节点执行 Isolate Server 操作。
      ...
      ```sql
      ALTER SYSTEM ISOLATE SERVER 'svr_ip:svr_port'; 
      ```
      ...
      成功执行 Isolate Server 操作 后，仅表示该节点为 stopped 状态，RS 会尽力将问题节点上的 Leader 切走，此时不能执行第 3 步 Kill observer 进程。如果必须要执行 Kill observer 进程，可能会有如下问题：
