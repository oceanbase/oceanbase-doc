...
2. 对问题 Zone 执行隔离 Zone 操作。
   ...
   3. 对问题 Zone 执行 Isolate Zone 操作。
      ...
      成功执行 Isolate Zone 操作后，仅将问题 Zone 内的节点状态标记为 stopped 状态，RootServiceS 会尽力将问题 Zone 内所有节点上的 Leader 切走，此时不能执行第 3 步 Kill observer 进程。如果必须要执行 Kill observer 进程，可能会有如下问题：
