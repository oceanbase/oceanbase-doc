...
* 通过 `GV$OB_PARAMETERS` 视图查看集群配置项
  ...
  与 `SHOW` 语句一样，可以查看集群中除隐藏配置项（`_xx_xx` 格式的配置项）以外的所有配置项及其所有属性描述。

* 通过配置文件 `observer.config.bin` 查看集群配置项

   该方式查看的配置项中仅包含了单台机器的配置项，集群中每台机器的配置项不完全一致。此外，通过该方式查看到的配置项信息仅包含了配置项名和配置项值，无各配置项的属性描述。不推荐使用该方式查看配置项，建议使用 `SHOW` 语句或 `GV$OB_PARAMETERS` 视图查看集群级配置项。
...
<main id="notice" type='explain'>
<h4>说明</h4>
<p>由于 <code>observer.config.bin</code> 文件为只读，您无法对文件中的配置项值进行修改。</p>
</main>
...
* [系统配置项概述](../../../7.reference/5.system-reference/1.system-configuration-items/1.system-configuration-items-overview-2.md)

* [系统变量概述](../../../7.reference/5.system-reference/2.system-variable-for-mysql/1.overview-of-system-variables-3.m)

* [设置参数](../../../7.reference/2.administrator-guide/2.basic-database-management/2.configuration-management/2.set-parameters.md)

* [设置变量](../../../7.reference/2.administrator-guide/2.basic-database-management/2.configuration-management/3.set-variables.md)

有关每个配置项的详细介绍，请参见 《参考指南》 的 **系统参考** 章节。
