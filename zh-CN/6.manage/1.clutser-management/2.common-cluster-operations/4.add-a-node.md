...
* 增加节点前需要为所有 OBServer 服务器安装数据库软件，您可以从 OceanBase 官网的下载中心下载对应版本的 OceanBase 数据库软件安装包安装包，或者从 OceanBase 技术支持人员处获取。

## 操作步骤

1. （可选）如果需要增加 Zone 后再向该 Zone 中增加 OBServer 节点，请确认已增加 Zone，增加 Zone 的操作请参见 [增加 Zone](5.add-a-zone.md)。

2. 通过 `ssh` 登录到待添加的 OBServer，进行安装前检查。

   1. 使用 `root` 用户依次登录到待增加的 OBServer 服务器上。
...
3. 通过 `ssh` 登录到待添加的 OBServer，安装 OceanBase 数据库的 RPM 包。

   在增加节点前，需要为待增加的所有 OBServer 服务器安装数据库软件。
...
4. 通常情况下，使用 `/data/1` 作为数据目录，`/data/log1` 作为日志目录。需要在待增加的 OBServer 服务器上创建 `/data/1` 和 `/data/log1` 目录，并将目录的拥有者（Owner）修改为 `admin`。
   ...
   4. 执行以下命令，启动 observer 进程。
      ...
      * `-o`：用于指定集群的启动配置项。使用 `-o` 参数时，需满足以下条件：
        ...
        * 配置项之间使用英文逗号（,）进行分割。
        ...
        示例中，`memory_limit=100GB` 表示进程启动内存上限为 100 GB；`datafile_disk_percentage=85`表示数据盘的占用比率为 85%。
...
7. 向集群中添加节点。
   ...
   2. 执行以下命令，向集群的 Zone 中添加节点。
      ...
      ```sql
      obclient [none]> ALTER SYSTEM ADD SERVER 'ip:port' [,'ip:port'...] [ZONE [=] 'zone_name'];
      ```

      相关参数说明如下：

      * `ip`：表示待添加的 OBServer 的 IP。

      * `port`：表示待添加的 OBServer 的 RPC 端口。默认为 `2882`。
...
8. 执行成功后，可以查询 `oceanbase.DBA_OB_SERVERS` 视图进行确认。
   ...
   列表中有刚刚添加的 OBServer，则表示添加成功。

## 后续操作

* 如果是向已存在的 Zone 中增加 OBServer，添加 OBServer 成功后，如果需要进一步使用该资源，还需要调整租户的资源配置，具体操作可参见[向 Zone 内添加 OBServer 节点](../../../7.reference/2.administrator-guide/7.o-m-management/2.scale-out-and-scale-in-1/2.cluster-level-scale-out-and-scale-in/1.scale-out/2.add-an-observer-node-to-a-zone.md)。

* 如果是添加 zone 并向该 Zone 中增加 OBServer，添加 OBServer 成功后，如果需要进一步使用该资源，还需要重新分配资源，并增加副本信息，具体操作可参见 [在集群中增加 Zone](../../../7.reference/2.administrator-guide/7.o-m-management/2.scale-out-and-scale-in-1/2.cluster-level-scale-out-and-scale-in/1.scale-out/1.add-a-zone-to-the-cluster.md)。
