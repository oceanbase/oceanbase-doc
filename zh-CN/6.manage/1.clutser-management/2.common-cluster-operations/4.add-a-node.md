...
2. 通过 `ssh` 登录到待添加的节点，进行安装前检查。

   1. 使用 `root` 用户依次登录到待增加的节点所在的机器上。
...
3. 通过 `ssh` 登录到待添加的节点，安装 OceanBase 数据库的 RPM 包。

   在增加节点前，需要为待增加的所有节点安装数据库软件。
   ...
   rpm -i --prefix=/home/admin/oceanbase $rpm_name
   ```
...
6. 启动 observer 进程。
   ...
   4. 执行以下命令，启动 observer 进程。
      ...
      * `-o`：用于指定集群的启动配置项。使用 `-o` 参数时，需满足以下条件：
      ...
      ./bin/observer -i eth0 -c 20221216 -p 2881 -P 2882 -z 'zone4' -n 'obdemo' -d '/home/admin/oceanbase/store/obdemo' -r '10.10.10.1:2882:2881;10.10.10.2:2882:2881;10.10.10.3:2882:2881' -l WARN -o 'memory_limit=100GB,datafile_disk_percentage=85'
      ```
