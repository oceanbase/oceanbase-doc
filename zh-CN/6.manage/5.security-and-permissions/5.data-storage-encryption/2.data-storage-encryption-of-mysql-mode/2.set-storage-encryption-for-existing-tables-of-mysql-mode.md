...
2. 执行以下语句，开启 internal 方式的透明加密。
   ...
   更多配置项 `tde_method` 的说明信息，参见 [tde_method](../../../../7.reference/8.configuration-items-and-system-variables/1.system-configuration-items/4.tenant-level-configuration-items/34.tde_method.md)。
...
2. 设置表的 `progressive_merge_num` 值，给表做全量合并或渐进合并。
   ...
   * 对表执行全量合并
     ...
     2. 手动发起一轮合并。
        ...
        手动发起合并的操作请参见 [手动触发合并](../../../../7.reference/2.system-management/5.manage-data-storage/2.merge-management/4.manually-trigger-a-merge.md)。
...
3. 完成后，您可以查看以下视图，确认所有宏块是否已完成加密。
   ...
   更多视图 `V$OB_ENCRYPTED_TABLES` 的字段及说明信息，参见 [V$OB_ENCRYPTED_TABLES](../../../../7.reference/7.system-views/4.system-view-of-mysql-mode/3.performance-view-of-mysql-mode/91.v-encrypted_tables-of-mysql-mode.md)。

## 相关文档

* [为新创建的表设置透明加密](1.configure-internal-storage-encryption-of-mysql-mode.md)
