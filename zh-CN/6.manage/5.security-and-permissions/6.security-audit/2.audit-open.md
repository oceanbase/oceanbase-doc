...
4. 启用审计功能并设置审计记录的存储位置。
   ...
   * `DB,EXTENDED / DB_EXTENDED`：审计记录写入内部表且记录包含执行的 SQL 语句。
   ...
   关于 `audit_trail` 配置项的详细信息，参见 [audit_trail](../../../7.reference/8.configuration-items-and-system-variables/1.system-configuration-items/4.tenant-level-configuration-items/2.audit_trail.md)。
