...
* cachePrepStmts：控制 JDBC driver 是否开启 PS cache 来缓存 PreparedStatement，避免重复执行 prepare（client 端和 server 端）。cachePrepStmts=TRUE 对使用 useServerPrepStms=TRUE 并重复对同一条 SQL 做 batch execute 的场景有帮助。每一次 batch execute 都会包含 preare 和 executecachePrepStmts=TRUE 可以避免重复的 prepare 操作。
