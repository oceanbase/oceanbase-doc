...
事务在没有提交之前，可以使用 `ROLLBACK` 命令回滚事务。更多事务相关的说明，请参考 [事务简介](../../../7.reference/1.oceanbase-database-concepts/8.transaction-management-1/1.transaction-2/1.transaction-introduction.md)。
...
* [SAVEPOINT](3.transaction-savepoints.md)：可以在事务过程中标记一个"保存点"，事务可以事后选择回滚到这个保存点上。保存点是可选的，一个事务过程中也可以设置多个保存点。
