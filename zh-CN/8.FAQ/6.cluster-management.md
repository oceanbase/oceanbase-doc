...
需要注意的是从选举成功到集群服务恢复之间，还需要一系列额外的工作流程：如新 Leader 需进行 Paxos 的 reconfirm 阶段补全日志（该阶段的耗时取决于新Leader 落后于旧 Leader 的日志数量，选举保证尽可能不选到日志落后过多的副本）；在新 Leader 上恢复尚未提交的旧 Leader 的事务状态；向 RootService 汇报 Leader 的位置信息，如果配置了 ObProxy 还需要等待 Proxy 更新正确 Leader 位置信息后才能将后面的 SQL 语句路由到正确的 Leader 位置上。
...
在集群的正常运行过程中，不需要数据库管理员或者管理员进行任何人为的操作或者干预来控制或影响 OBServer 的选举模块完成其功能。在进行集群管理动作时，比如说停掉 OBServer，上线新的 OBServer，修改租户的 Primary Zone 设置等，都有可能会触发选举模块进行选举，以保证租户中的数据副本存在主副本作为 Leader 对外提供数据服务。目前为止没有任何的系统运行情况需要管理员直接对选举模块进行运维干预。
...
可以通过 `__all_server_event_history` 表来查看某个日志流选举模块发生的事件。

### 如何查看无主选举？

可以通过`__all_server_event_history` 表查看，表里记录了所有日志流的所有选举事件，包括无主选举、切主、成员变更等。
...
OceanBase 数据库将数据保存在数据库不同副本中，选举模块能够在多个全功能副本选举一个 Leader 作为主副本对外提供服务。OceanBase 数据库通过 Clog 作为核心模块提供数据的高可靠高可用的服务。在 OceanBase 数据库的分布式架构实现中，Clog 会主动地周期性地检查副本的状态以确保分区副本能够形成多数派且主副本能够正常对外提供服务；Clog 的周期性检查会在副本状态异常，在还有多数数派的情况下决定是否进行主动卸主动作（Leader Revoke)， 在发现主副本无法正常提供副本服务时会打印一条 ERROR 日志并主动卸任，在能够形成多数派中 Election 模块执行无主选举选主新主。

### Clog 在发生主动卸主后会重新选主么？

OceanBase 数据库 4.0.0 版本不再有 Clog 发起的主动卸任动作，如果 Clog 写盘 hung 住，将会记录 failure 事件，Leader 的优先级会被降低，其后会自动选择最优副本直接切主，切主事件将会记录在 `__all_server_event_history` 表中。

## Clog FAQ

### OceanBase 数据库安装路径下 store 目录中有 Clog, Ilog 和 Slog 这些都分别是什么？有什么作用？

OceanBase Clog 是事务的 Redo log。当有数据修改发生时，数据库请求会到达 Leader 副本所在节点，Leader 会通过 Paxos 协议先将日志同步写到其他 Follower 的副本节点上。在 Follower 副本中当有多数派节点的日志写盘成功后，数据修改成功，才会再插入到内存中的 MemTable 上。OceanBase 数据库在 LSM Tree 存储架构下，当 MemTable 到达阈值后，会触发冻结和转储，数据会在 SSTable 落盘，而此时的 Clog 回放位点会推进，类似于做了 Checkpoint。OceanBase Clog 位于 OBServer 安装的 Clog 目录下，默认安装路径是 /home/admin/oceanbase/store/<cluster_name>/clog。 Clog 的文件编号从 1 开始并连续递增，文件 ID 不会复用，单个日志文件的大小为 64M。这些日志文件记录数据库中数据所做的更改操作，提供数据持久性保证。
OceanBase Slog 是本地引擎日志。当在节点上创建修改或销毁分区、生成新 SSTable、移除旧 SSTable 等元数据结构修改发生时，都会先写 Slog，而后修改内存中的元数据。后台会定期将分区和 SSTable 的元数据以快照的形式写在 2M 块进行落盘。
OceanBase Ilog 保存的是 Clog 的索引信息。
整体上来说，Clog 是 OceanBase 数据库的数据修改日志，作为基础模块支撑了 OceanBase 数据库的分布式事务，需要在多个副本间协商和同步。而 Slog 是单机引擎的元数据日志，不涉及多节点的同步和一致性问题。所以在目前的实现上考虑到 Clog 和 Slog 的不同功能和特性，将 Clog 和 Slog 放置在了不同的目录路径。

### OceanBase 数据库将 Clog 从 Leader 副本同步至 Follower 副本有哪些方式？ OceanBase 数据库如何保证 Follower 副本会最大程度地与 Leader 副本的 Clog 保持同步？

Clog 从 Leader 副本同步至 Follower 副本有以下几种：

- 在 OceanBase 数据库事务提交中，分区的 Leader 副本和 Follower 副本组成了 Paxos组，将 Clog 日志从Leader 副本同步到 Follower 节点。
- Follower 副本会做周期性的检查，如果发现 Follower 副本间隔一段时间内最新确认的日志一直没有更新，Follower 副本会主动从 Clog 发起 Clog Fetch 动作，采取主动拉取地方式来将 Clog 按批同步到 Follower 副本上。
- 当节点发现 Clog 同步已经有比较大的延迟时， Follower 副本会从 Leader 副本来做 rebuild 基线的动作；这是因为 Clog 本身是循环复用的结构，当 Clog 延迟比较大的时候，需要从 Leader 副本的 SSTable 中拉取数据。

这三种方式并行存在的情况下能够保证 Follower 副本上的 Clog 会尽量大限度地与 Leader 节点同步，与此同时，在 OBServer 的最新版本上(OBServer 2.2.76 及以上）为了让 Clog 同步能够更好地应对和处理副本间网络带宽有限场景（比如说带宽有限的跨城副本），Clog 同步增加了慢传输场景下自适应降速同步的机制。整个Clog 同步的机制一同保证了在网络带宽良好的情况下， Clog 能尽快地到达 Follower 副本。且在网络情况一般或较差时造成同步延迟的情况下，不会因为 Clog 同步重试逻辑造成不必要的网络负载。
...
OceanBase 数据库可以通过配置参数 `clog_transport_compress_all` 开启日志同步的压缩算法，需要手动开启，`clog_transport_compress_func` 可以用于设置事务日志传输的压缩算法_，_默认是 lz4_1.0。
...
系统虚表 `__all_virtual_clog_stat` 记录的是各个分区的 Clog 以及 Paxos 状态，其中 `is_in_sync` 代表了副本是否同步的状态，合法值为 TRUE 或者 FALSE。默认当 Follower 副本与 Leader 副本的 Clog 差异在 5s以内时，Clog的 is_in_sync 字段值就为 TRUE， 表示副本处在同步状态，否则就会认定 Follower 副本已经和 Leader 副本已经处在 Clog 不同步的状态。

### 如何查看分区 Follower 会读到的版本号是多少? OceanBase 数据库如何保证读到的这个版本号是安全的，不是一个正进行到一半的事务？

在系统虚表 `__all_virtual_partition_info` 中记录了分区副本的事务相关关键信息：

- min_log_service_ts：记录的是该分区在 Clog 滑动窗口中下一条将要滑出或将要生成的日志 timestamp；
- min_replay_engine_ts：记录的是该分区尚未回放日志的 timestamp 最小值；
- min_trans_service_ts：记录的是该分区当前所有正在回放事务 prepare 日志的最小值。

该分区的 Follower 副本的安全版本号就是 `__all_virtual_partition_info` 的 `weak_read_timestamp` 值，为了标识已经在 Follower 副本上完成回放，最新可以提供弱一致读数据服务的安全版本号。`weak_read_timestamp` 的取值是 `min_trans_service_ts`，`min_log_service_ts` 和 `min_replay_engine_ts` 三个值中的最小值，从而保证了这个值是该分区在该副本的的可读安全版本号。在这个版本号之前的所有事务都已经完成了回放。
通过下面的 SQL 可以查询该副本在该分区（通过 `table_id`, `part_idex` 和 `tenant_id` 可确定）上的安全弱读版本与当前时间的差异值（下面的 SQL 剔除了下线的副本, 日志型副本和只读副本）。

```sql
select a.zone,a.svr_ip, b.table_id, b.partition_idx,b.tenant_id, b.delay/1000000 clog_sync_delay_s from __all_server a,
( select svr_ip,table_id, partition_idx, tenant_id, replace(unix_timestamp(current_timestamp(6)),'.','')-min(weak_read_timestamp) delay 
     from __all_virtual_partition_info 
     where partition_state not in ('offline','offlining','remove','invalid_state') and replica_type!=5 and replica_type!=16  srv_ip = 'xx.xx.xx.xx' and table_id =''
```
...
4. DML1 和DML2 在 SQL 层执行成功，应用程序发起 COMMIT，Tnx1 进入事务提交阶段；
...
### Clog 聚合后会将 Clog 放在 Buffer 中，对应的内存消耗会是多大？

Clog 聚合 Buffer 的个数是以分区为单位定义的，每个 Buffer 固定占用 64KB 内存，重复使用而不会回收。假设有如下场景：

- `_clog_aggregation_buffer_amount = 4`。
- 某个租户里一共有 10,000 个分区。

那么，每个 zone 内的 Clog 聚合 Buffer 所消耗的总内存固定为 `64KB*4*10000=2.5GB` 内存。假设租户的 `unit_num=2`，那么每台 OBServer 上所消耗的内存是 `2.5GB/2=1.25GB`。
...
那么，假设所有的分区都在同一时刻用满了滑动窗口，对于一个集中了所有分区的 Follower 副本的 OBServer 来说，在这一时刻会额外消耗 `100B*10000*2*1,000=2GB` 内存。这是可能达到的最大内存消耗了，但这种情况几乎不会出现，因为几乎不可能在同一时刻把 1,000 个分区的滑动窗口同时压满。

### 如果 OBServer 的滑动窗口卡住或者满掉, 会出现什么报错，会造成什么后果，应当如何调优？
...
单条 Clog reconfirm 过程的超时时间是 10s，即如果 10s 没有执行完会打印 ERROR 级别的超时日志，关键字为 `is_reconfirm_role_change_or_sync_timeout_`。Clog reconfirm 通常由于卡在如上各个阶段而超时，卡住的原因主要有：无法提交回放、网络不通、日志盘满等。
...
对于分区数多、写入压力大的集群，如果转储慢（卡住）、或者租户 Unit 规格异构，那么可能导致部分副本的 Clog 回收不及时导致盘空间达到 95%，此时 OBServer 会自动停写，这样这台机器上会有大量副本不同步。
...
3. 如果通过调大 `Clog_disk_usage_limit_percentage` 至 98% 的方式仍然不能缓解 Clog 持续写满的问题，还可以将 OBServer 停机后，将 Clog 文件拷贝到另一个存储空间更大的目录下，并将 Clog 用软链接的方式链接过去。在完成后重新启动 OBServer。

   > **注意：**
   >
   > 请注意该步骤存在一定的操作风险，是在集群无法使用后应急操作恢复系统可用的最后办法。请在拷贝日志文件时格外注意，避免操作错误 。

   如果 Clog 的使用降到了 95% 水位以下，说明 OBServer 恢复顺利。继续等待所有副本达到 sync 状态即可。
...
### 如果 OBServer 重启时在初始化 Clog 失败会报什么错，如果 OBServer 启动时初始化失败报错应当如何操作恢复系统?
