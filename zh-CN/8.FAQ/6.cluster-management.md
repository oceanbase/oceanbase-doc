...
- 在 OceanBase 数据库事务提交中，分区的 Leader 副本和 Follower 副本组成了 Paxos组，将 Clog 日志从 Leader 副本同步到 Follower 节点。
...
这三种方式并行存在的情况下能够保证 Follower 副本上的 Clog 会尽量大限度地与 Leader 节点同步，与此同时，在 OBServer 的最新版本上 (OBServer 2.2.76 及以上）为了让 Clog 同步能够更好地应对和处理副本间网络带宽有限场景（比如说带宽有限的跨城副本），Clog 同步增加了慢传输场景下自适应降速同步的机制。整个 Clog 同步的机制一同保证了在网络带宽良好的情况下， Clog 能尽快地到达 Follower 副本。且在网络情况一般或较差时造成同步延迟的情况下，不会因为 Clog 同步重试逻辑造成不必要的网络负载。
...
1. `is_in_sync= 0` 的副本数量降为 0 之后，将上述所有改过的配置项恢复原值。
