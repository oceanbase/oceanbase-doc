...
* 租户规格：
  ...
  * 上文的租户规格是基于 [OceanBase TPC-H 性能测试报告](/zh-CN/3.performance-white-paper/2.wtu4kv.md) 中的硬件配置进行配置的，您需根据自身数据库的硬件配置进行动态调整。详细操作，参考 [修改租户](/zh-CN/6.administrator-guide/3.basic-database-management/5.tenants/5.modify-tenant.md)。
...
测试结果可参考 [OceanBase TPC-H 性能测试报告](/zh-CN/3.performance-white-paper/2.wtu4kv.md)。
