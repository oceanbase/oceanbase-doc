...
* OceanBase 数据库：详细信息，参考 [快速入门指南](/en-US/2.quickstart/1.quick-start-guide-1.md)。
...
* 租户规格：
  ...
  * 上文的租户规格是基于 [OceanBase TPC-H 性能测试报告](/en-US/3.performance-white-paper/2.wtu4kv.md) 中的硬件配置进行配置的，您需根据自身数据库的硬件配置进行动态调整。详细操作，参考 [修改租户](/en-US/6.administrator-guide/3.basic-database-management/5.tenants/5.modify-tenant.md)。
...
2. 部署成功后，新建执行 TPC-H 测试的租户及用户（请勿直接使用 sys 租户进行测试）。将租户的 `primary_zone` 设置为 `RANDOM`。租户详情，参考 [创建用户租户。](/en-US/6.administrator-guide/3.basic-database-management/5.tenants/2.create-a-user-tenant.md)
...
测试结果可参考 [OceanBase TPC-H 性能测试报告](/en-US/3.performance-white-paper/2.wtu4kv.md)。
