...
* 索引嵌套循环联接（Index Nested Loop Join）
  ...
  *************************** 1. row ***************************
  ...
  在 `outputs & filters` 的输出结果中 `nl_param` 出现参数 `[t2.c1]`，说明执行了条件下压优化。详细信息请参考 [JOIN](../../../../1000.sql-reference/400.queries-and-subqueries/300.queries-and-subqueries-join.md)。
