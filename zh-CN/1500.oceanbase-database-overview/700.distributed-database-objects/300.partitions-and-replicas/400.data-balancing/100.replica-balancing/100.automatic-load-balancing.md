...
分区副本的自动负载均衡是指在租户拥有的 Unit 内调整分区副本的分布使得 Unit 的负载差值尽量小。分区副本的自动负载均衡是租户级别的行为，发生在单个可用区（Zone）内。即 Root Service 调度某个租户的数据副本在 Zone 内发生迁移，达到该租户在该 Zone 上的全部 Unit 的负载均衡。
...
* 第一类均衡组
  ...
  有关 Partition Group 的更多信息请参见 [Table Group](../../../../../100.users-guide/800.distributed-storage-management/200.manage-partitions/1300.add-subpartitioned-tables-to-a-table-group.md)。
