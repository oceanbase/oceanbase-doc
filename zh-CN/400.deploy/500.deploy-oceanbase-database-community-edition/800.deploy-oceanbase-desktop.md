...
OceanBase 数据库启动成功后，单击 OceanBase 桌面版中的 **连接** 按钮，复制弹出的连接串。同时按下键盘 [Win] + [R] 键，输入 `cmd` 并单击 **确定** 打开 cmd 命令行，或者同时按下键盘 [Win] + [Q] 键，搜索并打开 **powershell** 或 **Windows Powershell**。在命令行中执行复制的连接串即可连接启动的 OceanBase 桌面版，连接串示例如下：
