...
<main id="notice" type='explain'>
  <h4>说明</h4>
  <p>本文以 OBD V2.8.0 为例提供操作指导。不同 OBD 版本的配置文件可能不同，请以实际配置文件为准。更多 OBD 相关内容请参见 <a href="https://www.oceanbase.com/docs/obd-cn">OBD 文档</a>。</p>
</main>
...
5. 修改配置文件。
   ...
   2. 修改机器的 IP、端口和相关目录，并配置内存相关参数及密码。
      ...
      | redo_dir  | 可选      | 默认与 `data_dir` 相同       | clog 的目录，默认与 data_dir 值相同，建议配置为独立的磁盘。 |
