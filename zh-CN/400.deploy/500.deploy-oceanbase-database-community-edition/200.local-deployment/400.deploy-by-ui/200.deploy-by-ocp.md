...
2. 在 **创建集群** 页面，设置集群的基础信息。
   ...
   | **root@sys 密码** | 支持自定义或随机生成。密码需要满足以下复杂度条件：</br> <ul><li>长度：8 \~ 32 字符</li><li>包含以下四种类型字符至少三种及以上：数字（0~9）、大写字母（A~Z）、小写字母（a~z）、特殊符号 `~!@#%^&*_-+=|(){}[]:;,.?/`</li></ul> </br>输入密码后，您可以单击 **复制密码**，将自定义或随机生成的密码复制到剪贴板。 |
