...
| [default_table_organization](../700.reference/800.configuration-items-and-system-variables/100.system-configuration-items/400.tenant-level-configuration-items/2150.default_table_organization.md) | HEAP | 表中数据行的存储顺序。对于 AP 场景，表中的数据行没有特定的顺序存储。| tenant |
| [default_table_merge_engine](../700.reference/800.configuration-items-and-system-variables/100.system-configuration-items/400.tenant-level-configuration-items/2130.default_table_merge_engine.md) | PARTIAL_UPDATE | 对 OLAP 工作负载使用 `partial_update` 合并引擎，稍后修改为 `delete_insert`。| tenant |
