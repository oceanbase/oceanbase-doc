...
|       参数       | 允许的取值范围 | 推荐值   |  说明  |
| ---------------- | ------------ |  ------ | ------ |
| vm.max_map_count | [327680, 1310720] | 655360 | 用于设置一个进程可拥有的最大内存映射区域数量。|
...
| fs.file-max | 大于等于 6573688 | N/A | 用于设置系统允许打开的最大文件数。<main id="notice" type='explain'><h4>说明</h4><p><code>fs.file-max</code> 是对整个系统的文件描述符限制，而不是针对个别用户。</p></main> |
...
| ulimit.process | <ul><li>soft nproc 655300</li><li>hard nproc 655300</li></ul> | <ul><li>soft nproc 655300</li><li>hard nproc 655300</li></ul> | 用于设置用户可以创建的最大进程数。|
...
| current_clocksource | N/A | tsc | 用于显示当前系统使用的时钟源。有用户是 hpet，存储性能比较差。|
