...
3. 获取并确认绝对路径。
    ...
    **示例如下：**
    ...
    ```shell
    /home/yourname/hdfs_lib
    ```

4. 配置 OceanBase 数据库路径变量。

    登录 sys 租户后，执行 `ALTER SYSTEM SET _ob_additional_lib_path = '/path_name';` 命令将自定义路径注册到系统配置。

    <main id="notice" type='notice'>
      <h4>注意</h4>
      <p>路径必须是绝对路径，多个路径用冒号 <code>:</code> 分隔，不能有空格。</p>
    </main>

    **示例如下：**

    ```sql
    ALTER SYSTEM SET _ob_additional_lib_path = '/home/yourname/hdfs_lib';
    ```
