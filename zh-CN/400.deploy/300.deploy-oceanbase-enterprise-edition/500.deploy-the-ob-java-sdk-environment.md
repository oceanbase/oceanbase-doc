...
4. 配置 OceanBase 数据库路径变量。
    ...
    **示例如下：**
    ...
    <main id="notice" type='notice'>
      <h4>注意</h4>
      <p><ul><li>在执行以下语句时请将示例中的 <code>$path_name</code> 替换为实际的路径。</li><li>路径必须是绝对路径，多个路径用冒号 <code>:</code> 分隔，不能有空格。</li></ul></p>
    </main>

    ```sql
    ALTER SYSTEM SET _ob_additional_lib_path = '/$path_name/hdfs_lib';
    ```
