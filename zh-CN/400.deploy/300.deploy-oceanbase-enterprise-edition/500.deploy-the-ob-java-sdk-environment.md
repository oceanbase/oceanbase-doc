...
<main id="notice" type='notice'>
  <h4>注意</h4>
  <p>所有环境配置和依赖安装操作均需以 admin 用户身份 执行。请通过以下方式切换用户。</p>
</main>
...
1. 通过 [下载地址](https://www.openlogic.com/openjdk-downloads) 下载 OpenJDK JAVA 安装包。
    ...
    <main id="notice" type='notice'>
      <h4>注意</h4>
      <p>请使用 OpenJDK 11 最新版本。</p>
    </main>

    本文以下载 OpenJDK 11.0.29+7 版本为例，具体操作步骤如下：
    ...
    * 开源版用户可在 [阿里云 OceanBase 镜像](https://mirrors.aliyun.com/oceanbase/) 页面中点击 `development-kit/` 目录，进入开发工具资源目录，下载 devdeps-java-extensions RPM 安装包。
      ...
      <main id="notice" type='notice'>
        ...
        <p><ul>
          ...
          <li>OceanBase 数据库 V4.4.1 版本：请下载 1.0.2 版本 devdeps-java-extensions RPM 安装包。</li>
          <li>OceanBase 数据库 V4.4.2 及更高版本：请下载 1.0.4 版本 devdeps-java-extensions RPM 安装包。</li></ul></p>
      </main>
...
1. 若当前环境无法获取 `sudo` 权限执行安装命令，可通过 `rpm2cpio` 命令手动解压 devdeps-hdfs-sdk RPM 安装包，您可以可根据需要自行放置需要的路径。
...
2. 指定目标路径。
    ...
    解压安装包后，可以使用 `mv` 命令将解压后的文件移动至用户可访问的自定义路径（例如 `~/hdfs_lib`）。
...
4. 配置 OceanBase 数据库路径变量。
    ...
    **示例如下：**
    ...
    <main id="notice" type='notice'>
      <h4>注意</h4>
      <p><ul><li>在执行以下语句时请将示例中的 <code>${user_name}</code> 替换为实际的路径。</li><li>路径必须是绝对路径，多个路径用冒号 <code>:</code> 分隔，不能有空格。</li></ul></p>
    </main>
