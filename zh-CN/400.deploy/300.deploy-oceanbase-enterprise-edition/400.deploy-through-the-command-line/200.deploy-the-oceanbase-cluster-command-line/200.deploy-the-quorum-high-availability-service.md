...
3. 启动仲裁 server 进程。
    ...
    **示例如下：**
    ...
    可以通过下面命令查看仲裁 server 进程是否启动成功：

    * `netstat -ntlp` 命令，如果监听到了 `2882` 端口，则说明进程启动成功。
    * `ps -ef|grep observer` 命令可以返回仲裁 server 进程信息。
...
<main id="notice" type='explain'>
   <h4>说明</h4>
   <p>仲裁 server 进程地址将被记录到 <code>sys</code> 租户的一张内部表中，可以通过视图 <code>DBA_OB_ARBITRATION_SERVICE</code> 查看。</p>
</main>
