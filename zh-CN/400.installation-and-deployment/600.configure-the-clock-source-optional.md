...
2. 运行以下命令查看服务器的 NTP 连接：
   ...
   ==============================================================================
   *time6.aliyun.co xx.xx.xx.xx 2 u 712 1024 377 21.951 4.253 4.208
   ```

3. （可选）开启 NTP 服务端服务

   如果您已经有了 NTP 服务端服务器，则可跳过此步骤。本文档使用目标机器 `xx.xx.xx.114` 作为 NTP 服务端服务器。
   1. 编辑配置文件。
...
5. 运行以下命令验证配置是否成功：
   ...
   ```bash
   synchronised to NTP server (xx.xx.xx.44) at stratum 3
