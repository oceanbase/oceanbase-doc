...
在实际使用中，高并发场景下可能会出现两个事务对同一行数据进行操作，可以通过 `SELECT FOR UPDATE` 语句锁定查询结果，避免其他 DML 语句对该笔记录进行同时修改。语句的详细使用方法，请参考 [锁定查询结果 SELECT FOR UPDATE](/zh-CN/7.development-guide-refactoring-1/2.development-guide/2.mysql-based-application-development/4.query-1/9.use-operators-and-functions-in-queries-1/10.lock-query-results-select-for-update-1.md)。
