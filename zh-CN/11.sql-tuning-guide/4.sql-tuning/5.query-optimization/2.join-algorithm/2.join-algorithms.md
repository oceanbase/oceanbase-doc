...
* 索引嵌套循环联接（Index Nested Loop Join）
  ...
  *************************** 1. row ***************************
  ...
  在 `outputs & filters` 的输出结果中 `nl_param` 出现参数 `[t2.c1]`，说明执行了条件下压优化。详细信息请参考 [JOIN](/zh-CN/11.sql-tuning-guide/2.sql-execution-plan/2.execution-plan-operators/3.join.md)。
