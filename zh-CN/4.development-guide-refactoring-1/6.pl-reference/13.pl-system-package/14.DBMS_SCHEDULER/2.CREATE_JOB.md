CREATE_JOB 
===============================
#docslug#/oceanbase-database/oceanbase-database/V4.0.0/CREATE_JOB
...
| repeat_interval    | 指定作业重复的频率。可以使用日历表达式或 PL 表达式指定重复间隔。 通过对指定的表达式求值，来确定作业下次运行的时间。如果未指定 `repeat_interval`，则作业仅在指定的开始日期运行一次。 有关更多信息，请参见 日历语法。                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
| auto_drop          | 指定作业在完成或自动禁用后是否自动删除。 该属性可以设置为 `TRUE` or `FALSE`，如果设置为 `TRUE`，则会导致作业在完成后自动删除或自动禁用。 如果出现以下情况，则认为已完成作业： * 作业结束日期（或作业计划的结束日期）已过。   * 作业已运行 `max_runs` 次。必须使用 `SET_ATTRIBUTE` 设置 `max_runs`。   * 该作业非重复的作业，只运行过一次。    当作业执行失败 `max_failures` 次时，将被禁用。`max_failures` 也使用 `SET_ATTRIBUTE` 设置。 如果 `auto_drop` 设置为 `FALSE`，则不会删除作业，并保留其元数据，直到使用 `DROP_JOB` 过程显式删除作业。默认情况下，创建作业时将 `auto_drop` 设置为 `TRUE`。                                |
| comments           | 添加作业的注释。默认情况下，该属性为 `NULL`。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| job_style          | 指定正在创建的作业的样式。 此属性可能的取值如下： * `'REGULAR' `：创建常规工作，为默认设置。   * `'LIGHTWEIGHT'`：创建轻量级作业。仅当作业引用程序对象时，才允许使用此值。当您需要频繁运行许多短期作业时，建议使用轻量级作业。在某些情况下，使用轻量级作业可以带来较小的性能提升。   * `'IN_MEMORY_RUNTIME'`：创建内存内运行时作业。这些作业基于轻量级工作结构，它们通过保持内存内缓存进一步提高了性能，因此可以最大限度地减少运行前和运行后操作的磁盘访问。   * `'IN_MEMORY_FULL'`：创建内存内已满的作业。内存中的完整作业需要一个程序，不能有时间计划或重复间隔。当作业启用时，它们会自动运行，运行后会被丢弃。它们将所有作业信息保存在内存中，不会备份到磁盘上，也就是说在实例重新启动时这些信息会丢失。它们的目的是以尽可能少的开销运行必须立即执行的操作。    |
