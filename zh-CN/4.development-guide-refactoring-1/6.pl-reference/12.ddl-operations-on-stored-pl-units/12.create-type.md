...
| subprogram_spec        | member_or_static               | * MEMBER 与 ADT 关联的函数或存储过程，被称为成员函数。通常，调用 MEMBER 方法主体中具有一个隐式第一参数，称为 SELF 。该参数表示调用该方法的对象。   * STATIC 与 ADT 关联的函数或存储过程。与 MEMBER 方法不同，STATIC 方法没有任何隐式参数。您无法在主体引用 SELF。它们通常的调用形式为 `type_name.method（）`。    |
| subprogram_spec        | proc_or_func_spec              | 指定存储过程或函数的参数和数据类型。如果此子程序不包含存储过程或函数的do，则必须通过`CREATE TYPE BODY`命令做相应的定义。 如果要创建子类型，则存储过程或函数的名称不能与在超类型链中声明的任何属性的名称（无论是否继承）相同。                                                                                                                                                                               |
