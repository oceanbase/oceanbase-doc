...
| --user                 | -u   | 是    | MySQL 模式租户的用户名：用户名@租户名#集群名。<br></br> **说明**  </br>如果只填写用户名，执行会默认sys租户。                                   | -uroot@MySQL#cluster1                                    |
| --password             | -p   | 否    | MySQL 密码                                                                                                             | -p1qaz@WSX                                               |
| --databases            | 无    | 是    | 指定数据库。 --all-databases：导出所有数据库，不推荐使用，建议单独指定。 <br></br>**说明** </br>实际使用中，可以直接填写数据库名。                | --databases db1 db2 db3  |
