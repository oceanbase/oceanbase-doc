...
* [BEGIN](02.start-transaction.md)：显式开启一个事务。此语句在使用过程中可选：
  ...
  * 租户会话的参数 `autocommit` 值是 1 （开启自动提交），则每条 SQL 都是一个独立的事务。如果要多个 SQL 组成一个事务，可以通过 `BEGIN` 命令显式开启一个事务，同时会禁用自动提交功能，直到执行 `COMMIT` 或 `ROLLBACK` 语句，才会恢复到自动提交模式。

* [SAVEPOINT](03.transaction-savepoint.md)：在事务过程中标记一个"保存点"，事务可以事后选择回滚到这个点。保存点是可选的，一个事务过程中也可以有多个保存点。
