...
| priv_level                             | 指定授予权限的层级。权限可以分为以下几个层级：<ul><li>全局层级：适用于所有的数据库。使用 GRANT ALL ON \*.\* 授予全局权限。</li><li> 数据库层级：适用于一个给定数据库中的所有目标。使用 GRANT ALL ON db_name.\* 授予数据库权限。</li><li>表层级：表权限适用于一个给定表中的所有列。使用 GRANT ALL ON database_name.table_name 授予表权限。 </li></ul></br>   用 "\*" 代替 `table_name`，表示赋予全局权限，即对数据库中的所有表赋权。 |
