...
| restore_option        | 恢复选项，支持以下几种： * `backup_cluster_name`：源集群的名字，为必选项。   * `backup_cluster_id`：源集群的 `cluster_id`，为必选项。   * `pool_list`：用户的资源池，为必选项。   * `locality`：租户的 Locality 信息，为可选项。   * `kms_encrypt`：为可选项。默认为 `false`。如果为 `true` 则表示在恢复时需要指定 `kms_encrypt_info`。 有关指定 `kms_encrypt_info` 方式的详细信息，请参见 《OceanBase 数据库管理员指南》中 **恢复指定的表** 章节。    |
