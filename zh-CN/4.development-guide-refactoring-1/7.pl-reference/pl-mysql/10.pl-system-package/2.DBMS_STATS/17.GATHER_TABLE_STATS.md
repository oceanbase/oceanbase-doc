...
收集用户 `test` 的 `t_subpart` 表的统计信息。

```sql
CALL DBMS_STATS.GATHER_TABLE_STATS ('test', 't_subpart', method_opt=>'FOR ALL COLUMNS SIZE 5'
, granularity=>'ALL', degree=>4);
```
