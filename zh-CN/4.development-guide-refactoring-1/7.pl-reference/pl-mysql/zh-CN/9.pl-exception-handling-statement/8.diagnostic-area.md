...
标准 SQL 的每个条件区域都包含以下列表中的项目（除了` MYSQL_ERRNO` 是扩展项）：

* `CLASS_ORIGIN`：包含 `RETURNED_SQLSTATE` 类别值的字符串。如果 `RETURNED_SQLSTATE` 值以 SQL 标准文档 ISO 9075-2 中定义的类别值开头，则 `CLASS_ORIGIN` 为"ISO 9075"。否则，`CLASS_ORIGIN` 为"MySQL"。

  

* `SUBCLASS_ORIGIN`：包含 `RETURNED_SQLSTATE` 值的子类的字符串。如果 `CLASS_ORIGIN` 为"ISO 9075"或 `RETURNED_SQLSTATE` 以"000"结尾，则 `SUBCLASS_ORIGIN` 为"ISO 9075"。否则，`SUBCLASS_ORIGIN` 是"MySQL"。
...
* `CONSTRAINT_CATALOG`、`CONSTRAINT_SCHEMA`、`CONSTRAINT_NAME`：表示违反约束的目录、Schema 和名称的字符串。这些项总是空的。

  

* `CATALOG_NAME`、`SCHEMA_NAME`、`TABLE_NAME`、`COLUMN_NAME`：表示与条件相关的目录、Schema、表和列的字符串。这些项总是空的。

  

* `CURSOR_NAME`：表示游标名称的字符串。该项总是空的。
...
以上定义适用于不是由信号（即由 `SIGNAL` 或 `RESIGNAL` 语句）生成的条件。对于非信号条件，数据库仅填充未描述为始终为空的条件项。
...
ERROR 1193 (HY000): Unknown system variable 'var1'
...
* 执行 `RESIGNAL`
  ...
  在存储程序内的复合语句中执行条件处理程序期间，`RESIGNAL` 语句用于传递可用的错误条件信息。`RESIGNAL` 可能会在传递信息之前更改部分或全部信息，并修改堆栈诊断区域，详细信息请参见 [RESIGNAL](/zh-CN/9.pl-exception-handling-statement/5.RESIGNAL.md)。
