...
`BEGIN ... END` 块支持嵌套。`BEGIN ... END` 也可以被标记，详细信息请参考 [语句标签](#title-5h5-oi6-4wc)。
...
`DECLARE` 语句用于定义程序的局部变量、异常处理和游标，详细信息请分别参见 [存储程序的变量](/zh-CN/2.storage-object/2.variables-in-stored-programs.md)、[异常处理](/zh-CN/9.pl-exception-handling-statement/1.exception-handling-statement.md)和[游标](/zh-CN/3.cursor/1.overview-of-cursors.md)。
...
有关块内异常处理程序的声明，请参见 [DECLARE ... HANDLER](/zh-CN/9.pl-exception-handling-statement/3.declare-handler.md)。
