...
`IF NOT EXISTS` 防止创建已经存在的同名例程。如果存储名称与内置 SQL 函数的名称相同，则会发生语法错误，除非在定义例程或稍后调用例程时在名称和随后的括号之间使用空格。因此，请避免将现有 SQL 函数的名称用于用户自己的存储例程。

括号内的 `proc_parameter` 参数列表必须始终存在。如果没有参数，则应使用空括号 `()` 表示空参数列表。这里的参数名称不区分大小写。对于存储函数每个参数都是 `IN` 参数。
...
有关创建函数的详细示例信息，请参见 [存储函数](../2.storage-object/4.pl-storage-function.md)。