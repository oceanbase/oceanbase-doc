...
将用户 `testUser01` 的 `test_stat` 表中存储的用户 `testUser02` 的表 `tbl1` 中 `idx1` 索引的统计信息导入并存储到字典中。

```sql
CALL DBMS_STATS.IMPORT_INDEX_STATS('testUser02', 'tbl1','idx1', stattab=>'test_stat', statown=>'testUser01');
```