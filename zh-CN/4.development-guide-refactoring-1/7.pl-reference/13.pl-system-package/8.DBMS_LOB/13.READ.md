READ 
=========================
#docslug#/oceanbase-database/oceanbase-database/V4.0.0/READ
...
|  **参数**  |                                    **解释**                                     |
|----------|-------------------------------------------------------------------------------|
| lob_loc  | 要读取的 `LOB` 的定位器。详细信息请参见 [DBMS_LOB 使用说明](../8.DBMS_LOB/1.dbms_lob-overview.md)。 |
...
| INVALID_ARGVAL | 可能为以下几种情况： * `amount` 小于 1   * `amount` 大于 32767 字节 (或等效字符)   * `offset` 小于 1   * `offset` 大于 `LOBMAXSIZE`   * 以字节或字符为单位的 `amount` 大于缓冲区的容量    |
