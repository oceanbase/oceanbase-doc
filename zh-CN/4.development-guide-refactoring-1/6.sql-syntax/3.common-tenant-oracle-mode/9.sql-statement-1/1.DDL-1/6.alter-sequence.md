ALTER SEQUENCE 
===================================
#docslug#/oceanbase-database/oceanbase-database/V4.0.0/alter-sequence
...
| MAXVALUE int_value     | 更改序列的最大值，`int_value` 的取值范围是 \[-9223372036854775808， 9223372036854775807\]。新指定的 `MAXVALUE` 不能小于当前序列号的值。 <br>**注意**  <ul><li> 同时指定 `MINVALUE` 和 `MAXVALUE` 时，`MINVALUE` 必须小于 `MAXVALUE`。   </li><li> `MAXVALUE` 和 `MINVALUE` 之差必须大于等于 INCREMENT BY 中 的 `int_value`。    |
| NOMAXVALUE             | 如果更改为 `NOMAXVALUE`，则升序时最大值为9223372036854775807，降序时最大值为 -1。 </li></ul>                                                                                                                                                                                                                                                                                                           |
