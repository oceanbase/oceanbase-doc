ALTER TABLE 
================================
#docslug#/oceanbase-database/oceanbase-database/V4.0.0/alter-table-1
...
alter_table_action:
    ...
    | ADD CONSTRAINT constraint_name FOREIGN KEY(foreign_col_name) REFERENCES reference_tbl_name(column_name);
...
* 为非模板化二级分区表 `tbl3` 的一级分区 `p1` 下添加二级分区 `p1_r4`。
  ...
  ```sql
  obclient> ALTER TABLE tbl9 MODIFY PARTITION p1 ADD SUBPARTITION p1_r4 VALUES LESS THAN(2022);
  Query OK, 0 rows affected
  ```
