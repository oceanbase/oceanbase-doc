...
OceanBase数据库基于 LSM-Tree 结构开发了自己的存储引擎。其中数据大致被分为基线数据（SSTable）和增量数据（MemTable）两部分，基线数据被保存在磁盘中，增量修改在内存中进行。这使得一方面数据在磁盘中能够以更紧凑的方式存储。除此之外由于在磁盘中的基线数据不会频繁更新，OceanBase 又基于通用压缩算法对基线数据进行了再次压缩，使得数据存储在 OceanBase 中可以获得非常好的压缩比。同时这种数据压缩并未带来查询和写入性能的下降。下面我们介绍 OceanBase 导入大量外部数据并且观察数据压缩比的方法。

>**注意**
>
>编码压缩在当前社区版 V3.1.3 暂未发布，后续版本将会添加改部分特性。
...
>**注意**
>
>由于安全原因，以上授权 SQL 只能使用本地访问执行，不能使用远程 OBClient 执行。即需要在 observer 进程所在机器上登录 OBClient （或者 MySQL 客户端） 执行。
