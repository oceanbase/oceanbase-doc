...
>**说明**
>
>其中，`obtest`  为集群名称，这里只是示例，您可自行定义集群名称。一个集群只能有一个名称，且集群名称不能重复。`quick_start_demo.yaml` 是自定义的配置文件，需要用户自己创建并修改，这里只是示例。增加 -A 参数后，将自动用系统剩余资源创建 test 租户。
...
OBClient 默认使用端口 2881 来连接 OBServer 节点，如果您对端口做了更改，此处使用您实际的端口号。tenant_name 为您将要连接的租户名。
