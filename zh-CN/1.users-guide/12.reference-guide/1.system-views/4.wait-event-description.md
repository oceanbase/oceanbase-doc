...
- `V$SYSTEM_EVENT`：显示该视图中所有等待事件的总数。
...
| dag worker condition wait | 后台 dag 线程等待时间 | CONCURRENCY | address | - | - |
...
| latch: clog sw renew ms task lock wait | clog 备库更新成员变更信息的锁等待 | CONCURRENCY | address | number | tries |
...
| latch: election group trace recorder lock wait | 保护 Election Group Trace统计信息的锁 | CONCURRENCY | address | number | tries |
...
| latch: io queue lock wait | io queue 读取时的锁等待 | CONCURRENCY | address | number | tries |
