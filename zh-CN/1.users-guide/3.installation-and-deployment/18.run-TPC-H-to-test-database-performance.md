...
- OceanBase 数据库：详细信息，请参考 [快速入门](../2.quick-start/2.use-docker-to-deploy-oceanbase-database.md)
- 租户规格：
...
1. 部署成功后，新建执行 TPC-H 测试的租户及用户（请勿直接使用 sys 租户进行测试）。将租户的 `primary_zone` 设置为 `RANDOM`。租户详情，请参考 [修改租户](../6.basic-database-management/4.manage-tenants/4.administrator-guide-manage-tenants/2.modify-a-tenant.md)。
...
2. 执行测试脚本。
   ...
   > **说明**
   >
   > 测试结果可参考 [OceanBase TPC-H 性能测试报告](../9.performance-tuning-guide/8.performance-whitepaper/2.tpc-h-benchmark-report-of-oceanbase-database.md)。
