...
prometheus 启动的配置文件通过 configmap 来进行配置
```yaml
# configmap 中关键配置
scrape_configs:
...
### 验证:
浏览器打开 prometheus 的地址，查看 Status -> Targets, 验证 `proxy-monitor`, `obagent-monitor-basic`, `obagent-monitor-extra` 下的 Endpoint 都是 up 状态
...
### 配置说明：
grafana 的配置中会使用到 prometheus 的服务地址作为数据源的配置(datasource 中的 url 字段)，需要根据 prometheus 的实际部署情况来填写
...
* obproxy: 15354
导入之后，可以查看对应的监控图表
说明: obproxy 需要有实际请求之后，才会有监控数据展示
