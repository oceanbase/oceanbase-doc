...
* OBD
  ...
  OceanBase Deployer，OceanBase 开源软件的安装部署工具，简称为 OBD。详细信息请参考 [OBD 文档](https://www.oceanbase.com/docs/community-obd-cn-10000000001690019)。
...
* 生产环境下，您需要进行环境和配置检查，具体操作请参考 [部署前配置](../3.installation-and-deployment/4.environment-and-configuration-checks/1.view-resources-optional.md) 章节。
...
* 方法二：使用 SQL 语句创建用户租户。创建租户前需要先创建资源池，具体操作请参考 [创建资源池](../6.basic-database-management/3.manage-resources/4.create-a-resource-pool.md)。
  ...
  创建租户的详细命令介绍请参考 [新建租户](../6.basic-database-management/4.manage-tenants/2.create-a-tenant.md)。
