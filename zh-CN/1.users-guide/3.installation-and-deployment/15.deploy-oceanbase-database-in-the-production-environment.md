...
4. 下载配置文件
   ...
   * 如果您采用单机安装，即目标机器只有一台，请使用 `/usr/obd/example/autodeploy` 目录下的配置文件：single-example.yaml。

   * 如果您采用分布式安装，即目标机器有多台，请使用 `/usr/obd/example/autodeploy` 目录下的配置文件：distributed-with-obproxy-example.yaml。
