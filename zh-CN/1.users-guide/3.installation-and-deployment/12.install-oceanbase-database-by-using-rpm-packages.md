...
2. 运行如下命令，安装 OBD。
   ...
   ```bash
   yum install -y ob-deploy-*.rpm
   source /etc/profile.d/obd.sh
   ```

### 获取 OceanBase 数据库安装包

运行如下命令，下载 OceanBase 数据库安装包：
...
下载 OceanBase 数据库安装包后，您需要部署 OceanBase 数据库。详情参考 [生产环境部署 OceanBase 数据库](../../1.users-guide/3.installation-and-deployment/15.deploy-oceanbase-database-in-the-production-environment.md)。
...
您在成功构建 OceanBase 数据库之后，可以使用 OBD 部署 OceanBase 数据库。详细信息请参考 [生产环境部署 OceanBase 数据库](../../1.users-guide/3.installation-and-deployment/15.deploy-oceanbase-database-in-the-production-environment.md)。
