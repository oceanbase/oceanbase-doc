...
1. 本次测试需要用到 4 台机器，TPC-H、ODP 和 OBD 单独部署在一台机器上，作为客户端的压力机器。通过 OBD 部署 OceanBase 集群需要使用 3 台机器，OceanBase 集群规模为 1:1:1。

   > **说明**
   >
   > 在 TPC-H 测试中，部署 Sysbench、ODP 和 OBD 的机器只需 4 核 16G 即可。
...
* obd 运行 tpch，详细参数介绍请参考 [obd test 文档](https://open.oceanbase.com/docs/obd-cn/V1.3.3/10000000000182179)。
