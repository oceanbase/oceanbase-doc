...
为了安全性，ODP 默认禁止使用用户 `proxyro` 登录 OceanBase 集群，需要先测试其连通性。因为`root` 用户的权限过于大，所以这里推荐使用用户 `proxyro` 获取表的元数据信息。
...
```bash
[root@obce00 ~]# mysql -h xxx.xxx.xxx.xxx -P 2883 -u proxyro@sys#****-3zones -p****** -c -A
...
```sql
mysql -h xxx.xxx.xxx.xxx -u root@pro**** -P 2883 -p******
...
```bash
[root@obce00 ~]# mysql -h xxx.xxx.xxx.xxx -P 2883 -u proxyro@sys#****-3zones -p****** -c -A -Ns -e "show databases;"
oceanbase
information_schema
[root@obce00 ~]#
```
...
```bash
[root@obce00 ob-loader-dumper-2.1.13-SNAPSHOT]# bin/obloader -h xxx.xxx.xxx.xxx -P 2883 -u u_loader -p 123456 --sys-user=proxyro --sys-password=uY7Yf8zx -c obce-3zones -t obmysql -D test2 --csv --all -f /tmp/obdumper
