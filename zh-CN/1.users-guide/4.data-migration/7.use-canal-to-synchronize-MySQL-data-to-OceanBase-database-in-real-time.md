...
`Canal` 是 Alibaba 开源的一个产品，主要用途是基于 MySQL 数据库增量日志解析，提供增量数据订阅和消费。开源项目地址：[https://github.com/alibaba/canal](https://github.com/alibaba/canal)。
...
![1](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/8420219361/p366248.png#crop=0&crop=0&crop=1&crop=1&id=M9wvi&originHeight=252&originWidth=822&originalType=binary&ratio=1&rotation=0&showTitle=false&status=done&style=none&title=)
...
1. 下载 Canal Admin，访问地址：[https://github.com/alibaba/canal/releases](https://github.com/alibaba/canal/releases)
...
- 登录 web 界面
Canal Admin 的 web 访问地址：[http://127.0.0.1:8089/](http://127.0.0.1:8089/)
登录用户名：`admin`
登录密码：`123456`

### 部署 Canal Deployer

1. 下载 canal，访问地址：[https://github.com/alibaba/canal/releases](https://github.com/alibaba/canal/releases)。
...
5. （可选）图形化部署 Canal server 和 Canal instance
如果使用 Canal Admin 管理 Canal server，则登录 admin 的管理界面 [http://172.24.50.39:8089/#/canalServer/nodeServers](http://172.24.50.39:8089/#/canalServer/nodeServers)，选择 **Canal Server** > **Server 管理**，单击 **新建 Server**。

    > **说明**
    >  
    > 此处链接中的 IP 为示例中配置 Canal Admin 的服务器 IP，仅供参考。您需根据实际情况将其转换为自身配置 Canal Admin 的服务器 IP。

    ![1](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/8420219361/p367320.jpg#crop=0&crop=0&crop=1&crop=1&id=ddtHL&originHeight=300&originWidth=1911&originalType=binary&ratio=1&rotation=0&showTitle=false&status=done&style=none&title=)
    在弹出的 **新建Server信息** 界面单击 **确定** 按钮。
    之后，选择 **Canal Server** > **Instance 管理** ，单击 **新建 Instance**。
    ![1](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/8420219361/p367326.jpg#crop=0&crop=0&crop=1&crop=1&id=ujIEb&originHeight=263&originWidth=1897&originalType=binary&ratio=1&rotation=0&showTitle=false&status=done&style=none&title=)
    单击 **载入模板** ，显示一个配置文件，跟前面看到的类似。修改配置文件中的跟源端 `canal` 和数据库有关的信息。
    ![1](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/8420219361/p367355.jpg#crop=0&crop=0&crop=1&crop=1&id=anJFO&originHeight=967&originWidth=1079&originalType=binary&ratio=1&rotation=0&showTitle=false&status=done&style=none&title=)
    单击 **保存** ，命名为 `mariadb`。
    保存后的 instance 状态为 **停止** ，单击 **操作** > **启动** 可启动 instance，启动后，instance 状态为 **启动**。
    ![1](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/8420219361/p367338.jpg#crop=0&crop=0&crop=1&crop=1&id=eqSSw&originHeight=369&originWidth=1913&originalType=binary&ratio=1&rotation=0&showTitle=false&status=done&style=none&title=)
...
1. 下载 Canal Adapter，访问地址：[https://github.com/alibaba/canal/releases](https://github.com/alibaba/canal/releases)
...
3. 修改启动器配置：`conf/application.yml`，这里以 OceanBase 目标库为例。
