...
* 如果您采用本地安装，即中控机器和目标机器是同一台机器，请使用 `/usr/obd/example/` 目录下的配置文件：mini-local-example.yaml。

* 如果您采用单机安装，即目标机器只有一台，请使用 `/usr/obd/example/` 目录下的配置文件：mini-single-example.yaml。

* 如果您采用分布式安装，即目标机器有多台，请使用 `/usr/obd/example/` 目录下的配置文件：mini-distributed-with-obproxy-example.yaml。
...
| servers | 172.20.249.50 | 本示例是在中控机上部署 OBServer，所以写的是中控机 IP。 |
...
+---------------+---------+------+-------+--------+
| 172.20.249.50 | 3.1.4   | 2881 | zone1 | active |
+---------------+---------+------+-------+--------+
