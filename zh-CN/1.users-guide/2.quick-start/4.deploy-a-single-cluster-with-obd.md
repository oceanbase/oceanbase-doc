...
* 如果您采用本地安装，即中控机器和目标机器是同一台机器，请使用本地安装配置文件（/usr/obd/example/mini-local-example.yaml）。

* 如果您采用单机安装，即目标机器只有一台，请使用单机安装配置文件（/usr/obd/example/mini-single-example.yaml）。

* 如果您采用分布式安装，即目标机器有多台，请使用分布式安装配置文件（/usr/obd/example/mini-distributed-with-obproxy-example.yaml）。
