...
| datafile_size datafile_disk_percentage |  | 这两个参数 2 选 1。用来指定该节点数据文件（`block_file`）大小的。可以按大小指定，或者按磁盘空间的百分比指定。 这里示例磁盘空间很小，为精确控制，指定数据文件大小。<br>**注意**：当数据文件和事务日志文件共用一个磁盘时，必须指定数据文件具体大小，以避免日志文件目录空间不够的情况发生。
