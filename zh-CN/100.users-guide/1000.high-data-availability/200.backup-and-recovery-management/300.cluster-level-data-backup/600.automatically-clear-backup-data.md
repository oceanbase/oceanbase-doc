...
2. 在配置项 `backup_dest_option` 中添加备份数据的过期时间和清理策略的设置。
   ...
   * `recovery_window` 参数
   ...
   配置项 `backup_dest_option` 中`auto_delete_obsolete_backup` 参数和 `recovery_window` 参数的配置方法及更多说明信息请参见 [备份前准备](../300.cluster-level-data-backup/100.preparations-before-backup.md)。
...
除了通过自动的方式清理备份数据外，您也可以手动清理备份数据。手动清理支持清理指定的备份数据以及清理过期的备份数据。详细信息参见 [手动清理备份的数据](../300.cluster-level-data-backup/700.manually-clear-backup-data.md)。
