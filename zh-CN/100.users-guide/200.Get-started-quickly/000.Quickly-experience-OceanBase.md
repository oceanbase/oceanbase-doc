...
<main id="notice" type='notice'>
  <h4>注意</h4>
  <p>本文中的方法仅适用于 OceanBase 数据库快速上手体验，不适用生产环境，如需在生产环境中部署 OceanBase 数据库，请参考 <a href="../300.installation-and-deployment/100.deployment-overview.md">部署 OceanBase 社区版</a> 章节。</p>
</main>
...
* OBAgent
  ...
  OBAgent 是 OceanBase 数据库监控采集框架，支持推、拉两种数据采集模式，可以满足不同的应用场景。详细信息可参见 [OBAgent](../../1800.supporting-tools/200.ob-agent/100.about-obagent/100.what-is-obagent.md) 章节。
...
1. 运行以下命令部署集群。
   ...
   > **说明**
   >
   > 其中，`obtest` 为集群名称，这里只是示例，您可自行定义集群名称。一个集群只能有一个名称，且集群名称不能重复。`single-example.yaml` 是 OBD 提供的配置文件示例。增加 `-A` 参数后，将自动用系统剩余资源创建 test 租户。
...
2. 使用 OBClient 客户端连接 OceanBase 数据库。
   ...
   ```shell
   obclient -h10.10.10.1 -P2881 -uroot@test
   ```
