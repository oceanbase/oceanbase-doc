...
* 生产环境下，您需要进行环境和配置检查，具体操作请参考 [环境和配置检查](../300.installation-and-deployment/400.environment-and-configuration-checks.md)
...
在您执行了 `obd cluster deploy` 命令之后，OBD 将检查您的目标机器是否有 OceanBase 数据库安装包。如果没有安装包，OBD 将自动从 yum 源获取。如果您的机器不能访问公网，您可以从 OceanBase 官网下载安装包。详细信息，参考 [获取安装包](../300.installation-and-deployment/1200.install-oceanbase-database-by-using-rpm-packages.md)。

### 离线安装 OBD

如果您的机器不能访问公网，您可以从 OceanBase 官网下载 OBD，建议下载最新版本的安装包。详细信息，参考 [获取安装包](../300.installation-and-deployment/1200.install-oceanbase-database-by-using-rpm-packages.md)。下载完成后，将 OBD 复制到您的中控机器上。
...
如果您的机器不能访问公网，您可以从 OceanBase 官网下载 OBLibs，建议下载最新版本的安装包。详细信息，参考 [获取安装包](../300.installation-and-deployment/1200.install-oceanbase-database-by-using-rpm-packages.md)。下载完成后，将 OBLibs 复制到您的每台机器上。
...
>
> * 您必须首先下载 OBClient 的依赖包 LibOBClient。详细信息，参考 [获取安装包](../300.installation-and-deployment/1200.install-oceanbase-database-by-using-rpm-packages.md)。
...
如果您的机器不能访问公网，您可以从 OceanBase 官网 [下载中心](https://www.oceanbase.com/softwareCenter) 下载 OBClient，建议下载最新版本的安装包。下载完成后，将 OBClient 复制到您的中控机器上。
...
安装 OceanBase 数据库和 ODP之前，为了数据安全，建议您切换到非 root 用户。如果离线安装 OceanBase 数据库和 ODP，从 OceanBase 官网下载 OceanBase 数据库、OBLibs 和 ODP。详细信息，参考 [获取安装包](../300.installation-and-deployment/1200.install-oceanbase-database-by-using-rpm-packages.md)。下载完成后，将 OceanBase 数据库和 ODP 复制到您的中控机器上。如果在线安装，跳过步骤 1\~3。
...
4. 下载配置文件
   ...
   * 如果您采用分布式安装，即目标机器有多台，可使用分布式安装配置文件（/usr/obd/example/autodeploy/distributed-with-obproxy-example.yaml）。
   ...
   示例采用自动部署配置文件。自动部署将根据简易配置和目标机器资源生成最大规格配置并启动 OceanBase 数据库。OceanBase 数据库详细参数介绍，参考 [OceanBase 数据库参数](https://github.com/oceanbase/obdeploy/blob/master/plugins/oceanbase/3.1.0/parameter.yaml)。
...
* 使用 SQL 语句创建普通租户。创建普通租户前需要先创建资源池，具体操作请参考 [创建资源池](../600.basic-database-management/300.manage-resources/400.create-a-resource-pool.md)
