...
如果您计划部署分布式 OceanBase 集群，建议您在部署 OceanBase 集群时，所有的机器使用相同的软硬件配置。详细信息请参考 [软硬件要求](../200.download-installation-packages.md)。
...
# if current ip is xx.xx.xx.xx, and the ip's network adaptor's name is "eth0", please use "eth0"
devname: eth0
```
