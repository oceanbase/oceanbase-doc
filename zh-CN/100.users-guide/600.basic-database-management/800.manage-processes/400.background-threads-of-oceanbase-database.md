...
| PartWorker         | Trx      | 1    | 遍历本机所有的 Partition，执行以下操作：<br> 1. 每隔 50ms 尝试写事务层 Checkpoint Log。 <br>  2. 每隔 15s 计算迁移的快照点。  <br> 3. 每隔 10s 驱动本 Partition 上所有活跃事务，为其检查Scheduler 的状态，用于事务上下文的 GC。 <br>  4. 每隔 10s 检查该 Partition 复制表的状态。 <br>  5. 每隔 10s 驱动刷新本 Partition 上缓存的 Clog日志。    |
