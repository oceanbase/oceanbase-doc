...
OceanBase 数据库社区版仅支持一种租户： MySQL 租户（企业版同时支持 Oracle 和 MySQL 两种租户）。只有 `root` 用户连接到 `sys` 租户（`root@sys`）才能执行 `CREATE TENANT` 命令去创建租户。
...
7. 填写 **Zone 信息** 。
   ...
   1. 为 z1、z2 和 z3 设置副本类型、Unit 规格和 Unit 数量。
      ...
      * 指定该 zone 下的 Unit 数量。需要注意的是，unit 数量不能超过该 zone下 server 个数。
...
8. 填写 **基本设置** 。
   ...
   3. 设置 **字符集** 。
      ...
      对于 MySQL 模式，可选字符集有：utf8mb4, binary。缺省为 utf8mb4。
