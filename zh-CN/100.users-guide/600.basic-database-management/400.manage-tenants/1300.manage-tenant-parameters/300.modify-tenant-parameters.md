...
* `SCOPE` 用于指定本次配置项修改的生效范围，默认值为 `BOTH`。其中：
  ...
  * `MEMORY`：表示仅修改内存中的配置项，修改立即生效，且本修改在 OBServer重启以后会失效（当前暂无配置项支持这种方式）。

  * `SPFILE`：表示仅修改配置表中的配置项值，当OBServer重启以后才生效。

  * `BOTH`：表示既修改配置表，又修改内存值，修改后立即生效，且 Server 重启以后配置值仍然生效。

* `SERVER` 表示指定租户中要修改的OBServer，`ZONE` 表示指定租户中要修改的 Zone。
...
* 普通租户的租户管理员设置本租户的配置项。
  ...
  obclient> ALTER SYSTEM SET clog_max_unconfirmed_log_count=1600 SERVER='xx.xx.xx.xx:2882';
