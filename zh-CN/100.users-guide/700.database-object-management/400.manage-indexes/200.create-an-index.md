...
本节主要介绍如何在非分区表上创建索引，分区表上索引的创建请参见 [在分区表上建立索引](../../800.distributed-storage-management/200.manage-partitions/800.create-an-index-on-a-partitioned-table/100.administrator-guide-overview-of-indexes.md) 章节。
...
* 字段解释如下：
  ...
  * `GLOBAL` ：全局索引。在创建索引时如果不显式指定 `global` 或 `local` ，则默认是 `local` 。
...
>**说明**
>
>暂不支持倒序索引。比如：CREATE INDEX idx ON test (c1, c2 DESC);会报错Not supported feature or function;
>
