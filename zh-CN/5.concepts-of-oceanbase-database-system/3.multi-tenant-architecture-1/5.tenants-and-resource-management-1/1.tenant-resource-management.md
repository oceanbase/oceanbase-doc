...
```sql
obclient> CREATE RESOURCE UNIT uc1 MAX_CPU 5, MIN_CPU 4, MEMROY_SIZE '36G', MAX_IOPS 128000, MIN_IOPS 128000, LOG_DISK_SIZE '2T' ;
```
...
`MIN_CPU` 和 `MIN_IOPS` 表示使用该资源配置的资源单元能够提供的 CPU/MEMORY 的下限。
...
* CPU资源：Meta 租户和用户租户共享 CPU 资源，不做隔离。考虑到公有云最小 1c2G 的售卖模式，CPU 规格最小为 1c。在 `GV$OB_UNITS` 视图中展示的 Meta 租户的 CPU 资源规格为 `NULL`，表示与用户租户共享 CPU 资源。
