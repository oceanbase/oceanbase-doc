...
* CPU 通过用户态调度实现隔离。
  ...
  为了达到更好的隔离效果，从 V4.0.0.0 版本开始，支持配置 cgroup 来进行 CPU 隔离。
...
从设计上，OceanBase 数据库从 V4.0.0.0 版本支持租户间 CPU、Memory、IOPS 隔离，一个 Unit 可以指定 CPU、Memory、IOPS、log_disk_size 这 4 种资源。
...
OBServer 的内存资源实际上不支持超卖，引入内存超卖反而会导致租户工作不稳定，从 V4.0.0.0 版本开始，OceanBase 数据库不再支持内存超卖。废弃 `MIN_MEMORY` 和 `MAX_MEMORY` 配置，采用 `MEMORY_SIZE` 参数代替。
...
在当前 V4.0.0.0 版本，租户工作线程和大部分后台线程是分租户的，网络线程是共享线程的。
...
1. V4.0.0.0 版本的后台拆分了租户，每个租户下有对应的线程数控制。
