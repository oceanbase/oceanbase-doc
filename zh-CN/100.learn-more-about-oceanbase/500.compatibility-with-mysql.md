...
* 仅支持 `SQL_SMALL_RESULT`、`SQL_BIG_RESULT`、`SQL_BUFFER_RESULT` 和 `HIGH_PRIORITY` 关键字语法，功能不生效。
* OceanBase 数据库并不完全兼容 MySQL 数据库中的 `STRAIGHT_JOIN` 关键字语法。当 `STRAIGHT_JOIN` 能够明确指定表连接的顺序时，OceanBase 数据库会按照这个命令执行。如果 `STRAIGHT_JOIN` 无法确定唯一的连接顺序，OceanBase 数据库将会忽略这个命令。
