# OceanBase 数据库后台线程
#docslug#/oceanbase-database/oceanbase-database/V4.0.0/oceanbase-database-background-threads
...
| PartWorker       | Trx      | 1    | 遍历本机所有的 Partition，执行以下操作： <ol><li>每隔 50ms 尝试写事务层 Checkpoint Log。</li> <li>每隔 15s 计算迁移的快照点。</li> <li>每隔 10s 驱动本 Partition 上所有活跃事务，为其检查Scheduler 的状态，用于事务上下文的 GC。</li><li>每隔 10s 驱动刷新本 Partition 上缓存的 Clog日志。</li></ol>    |
