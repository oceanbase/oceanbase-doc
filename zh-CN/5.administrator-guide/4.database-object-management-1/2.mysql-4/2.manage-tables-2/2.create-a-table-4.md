...
* 基于性能和后期维护的需要，建议建表时为表设计主键或者唯一键。如果没有合适的字段作为主键，可以在创建表时不指定主键，待表创建成功后系统会为无主键表指定自增列作为隐藏主键。有关序列的介绍请参见 [管理序列](/zh-CN/5.administrator-guide/4.database-object-management-1/1.oracle-4/5.management-sequence-1.md)章节。
