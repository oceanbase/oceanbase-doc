...
* 外键约束（`FOREIGN KEY`）：要求约束的列的值取自于另外一个表的主键列。
  ...
  更多 `foreign_key_checks` 变量的说明，请参见 [foreign_key_checks](/zh-CN/13.reference-oracle-mode/2.system-variable-6/27.foreign_key_checks-1-2-3-4.md)。
...
更多创建约束的语法及详细介绍请参见 [CREATE TABLE](/zh-CN/10.sql-reference-mysql-mode/6.sql-statement/21.create-table.md)。
...
更多创建约束的语法及详细介绍请参见 [ALTER TABLE](/zh-CN/10.sql-reference-mysql-mode/6.sql-statement/8.alter-table.md)。
