...
* 新增列，支持在表中新增列，但不支持直接增加主键列。如果需要增加主键列，建议先增加列后再为列添加主键。添加主键的相关操作请参见 [定义列的约束类型](/zh-CN/5.administrator-guide/4.database-object-management-1/1.oracle-4/1.manage-tables-1/4.define-the-constraint-type-for-a-column-1.md)。
...
* 修改列类型。OceanBase 数据库支持多种列类型之间的转换。
  ...
  OceanBase 数据库中列类型的转换规则及详细信息请参见 [列类型变更规则](/zh-CN/11.sql-reference-oracle-mode/10.online-ddl-1/4.column-type-change-rule.md)。
...
创建表后，OceanBase 数据库支持添加或删除表的主键和外键，更改表的主键和外键的具体操作及说明请参见 [定义列的约束类型](/zh-CN/5.administrator-guide/4.database-object-management-1/1.oracle-4/1.manage-tables-1/4.define-the-constraint-type-for-a-column-1.md)。
...
更改表的表组相关操作请参考 [表组管理命令](/zh-CN/5.administrator-guide/4.database-object-management-1/2.mysql-4/3.manage-a-table-group-2/2.table-group-management-statements-2.md)。
