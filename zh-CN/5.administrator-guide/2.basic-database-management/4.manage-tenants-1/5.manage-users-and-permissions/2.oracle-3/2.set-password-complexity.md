...
2. 编写 `PASSWORD_VERIFY_FUNCTION`参数对应的函数并授权。
   ...
   * 如果修改密码，新密码与旧密码至少要有 3 个以上的字符不相同。
   ...
   创建函数的相关操作，请参见 [创建函数](/zh-CN/9.pl-reference/8.stored-procedure-and-functions/3.create-a-function.md)。
...
3. 在 User 的 Profile 中，指定 `PASSWORD_VERIFY_FUNCTION` 参数，启用密码复杂度验证。
   ...
   更多 `ALTER PROFILE` 语句的详细信息，请参见 [ALTER PROFILE](/zh-CN/11.sql-reference-oracle-mode/9.sql-statement-1/1.DDL-1/4.alter-profile.md)。
...
2. 通过 `DBA_SOURCE` 视图查找对应的函数。
   ...
   更多 `DBA_SOURCE` 视图及其字段的说明请参见 [DBA_SOURCE](/zh-CN/13.reference-oracle-mode/1.system-view-5/2.dictionary-view-6/99.DBA_SOURCE-1.md)。
...
3. 通过 `USER_SOURCE` 视图查看当前用户拥有的函数 `VERIFY_FUNCTION` 的内容。
   ...
   更多 `USER_SOURCE` 视图及其字段的说明请参见 [USER_SOURCE](/zh-CN/13.reference-oracle-mode/1.system-view-5/2.dictionary-view-6/168.USER_SOURCE-1.md)。
...
更多函数管理的相关操作，请参见 [管理存储过程和函数](/zh-CN/9.pl-reference/8.stored-procedure-and-functions/4.view-and-delete-procedures-and-functions.md)。
