...
10. 设置表的 `progressive_merge_num` 值，给表做全量合并或者渐进合并。
    ...
    * 对表执行全量合并
      ...
      2. 使用 `root` 用户登录到数据库的 `sys` 租户，手动发起一轮合并。
         ...
         手动发起合并的操作请参见 [手动触发合并](/zh-CN/5.administrator-guide/2.basic-database-management/5.manage-data-storage/2.merge-management-1/4.manually-trigger-a-merge-1.md)。
...
11. 完成后，您可以查看以下视图，确认所有宏块是否已完成加密。
    ...
    更多视图 `v$encrypted_tables` 的字段及说明信息请参见 [V$ENCRYPTED_TABLES](/zh-CN/13.reference-oracle-mode/1.system-view-5/3.performance-view-6/77.v-encrypted_tables-4.md)。
