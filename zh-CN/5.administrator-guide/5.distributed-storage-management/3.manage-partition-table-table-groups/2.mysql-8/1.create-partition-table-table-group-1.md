...
| opt_tg_partition_option    | 定义表组的分区规则。表组的分区规则需要与表组内的表使用相同的分区方式。 由于表组没有具体的列定义，故不需要写出具体的列，只需要指定列个数（`COLUMN_NUM`）即可。 分区方式主要表现在以下几点： * 分区类型相同。例如，要求表组与表都是 Range Columns 分区。   * 如果是 Range Columns分区，则要求引用的列数相同和分区数相同，且 Range 分割点相同（各 value 的规则相同）。    |
