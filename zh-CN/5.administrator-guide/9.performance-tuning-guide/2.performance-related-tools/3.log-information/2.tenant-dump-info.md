...
# docslug#/oceanbase-database/oceanbase-database/V3.2.3/tenant-dump

日志中搜索 "dump tenant info" 关键字，可看到租户的规格、线程、队列、请求统计等信息。

>**说明**
>
>在 OceanBase 数据库 V4.x 版本，该条日志每个租户每 30 s 打印一次;对于 3.x及以下版本，每 10 s 打印一次。
...
actives:8, //活跃工作线程数：租户活跃工作线程数 + 大查询挂起的线程数，一般和workers相等，它们的差为：租户非活跃并等待延迟回收的线程。
