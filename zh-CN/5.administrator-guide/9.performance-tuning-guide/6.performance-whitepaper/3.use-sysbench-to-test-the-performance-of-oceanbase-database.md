...
* OceanBase 数据库：建议单独部署 ODP。详细信息，参考 [部署 OceanBase 数据库](../../../4.installation-and-deployment/1.deployment-overview.md)
...
* 租户规格：
  ...
  > **注意**
  >
  > * 上文的租户规格是基于 [OceanBase Sysbench 性能测试报告](../6.performance-whitepaper/4.sysbench-benchmark-report-of-oceanbase-database.md) 中的硬件配置进行配置的，您需根据自身数据库的硬件配置进行动态调整。详细操作请参考 [修改租户](../../6.basic-database-management/4.manage-tenants-1/4.manage-tenants-2/2.modify-tenant.md)。
  >
...
3. 设置租户（进行测试的用户登录设置）
   ...
   /*
   parallel_server_target 推荐设置为测试租户分配的 resource unit cpu 数的 10 倍
   */
...
> **说明**
>
> 测试结果可参考 [OceanBase Sysbench 性能测试报告](../6.performance-whitepaper/4.sysbench-benchmark-report-of-oceanbase-database.md)。
