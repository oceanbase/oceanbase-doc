...
4. 检查是否安装成功。
   ...
   ```bash
   [admin@obtest004 benchmarksql]$ant -version
   ```
...
3. 重新编译修改后的源码。
   ...
   ```bash
   [admin@obtest004 test]# cd benchmarksql-5.0
   [admin@obtest004 benchmarksql-5.0]# ant
   ```
...
6. 添加 mysql java connector 驱动，推荐 mysql-connector-java-5.1.47.jar。
   ...
   [admin@obtest004 benchmarksql-5.0]# cp mysql-connector-java-5.1.47.jar benchmarksql-5.0/lib/oceanbase/
   ```
