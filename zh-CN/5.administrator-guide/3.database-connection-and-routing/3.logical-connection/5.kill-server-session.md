...
* 与 `KILL connection_id` 的作用一致。有关 `KILL` 语句的详细介绍，请参见 [KILL（MySQL 模式）](/zh-CN/10.sql-reference-mysql-mode/6.sql-statement/47.KILL.md) 或 [KILL（Oracle 模式）](/zh-CN/11.sql-reference-oracle-mode/9.sql-statement-1/3.DCL/8.KILL-1.md)。
...
* `ss_id` 表示 ODP 内部标记每个 Server 端会话（ Server Session）的 `id` 号，可以从 `SHOW PROXYSESSION ATTRIBUTE id` 中获取。详细的获取操作请参见 [展示 Session 详细状态](/zh-CN/5.administrator-guide/3.database-connection-and-routing/3.logical-connection/2.displays-the-status-of-a-session.md)。
