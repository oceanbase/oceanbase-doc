...
* X509 双向认证
  ...
  ```unknow
  $ obclient -h <observer-ip> -P2881 -u **user2**@mytenant -p***** --ssl-ca=/path/to/ca.pem --ssl-cert=/p
...
* 使用加密算法为 DHE-RSA-AES128-GCM-SHA256
  ...
  ```unknow
  $ obclient -h <observer-ip> -P2881 -u **user3**@mytenant -p***** --ssl-ca=/path/to/ca.pem --ssl-cert=/pat
