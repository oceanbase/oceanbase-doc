...
V4.3.2 通过以下优化措施提升了全量旁路导入性能约 20%：

- 减少数据加载过程中的列类型转换操作。
- 降低统计信息收集操作的 CPU 占用。
- `INSERT INTO SELECT` 去除 `SELECT` 端生成 pk 逻辑。
- 列存默认关闭 `sum skip index`，结合配置项 `micro_block_merge_verify_level` 设置为 0 关闭微块校验。

### IN/EXISTS 子查询提升改写

在 V4.3.2 中，OceanBase 支持将 `SELECT` 子句中的 `IN`/`EXISTS` 子查询改写为连接形态，提升了 SQL 执行性能，特别是在高并发场景下提升了查询效率。

### 存储过程 DDL 执行编译结果缓存和落盘

V4.3.2 版本支持在存储过程执行 DDL 后，将编译结果缓存到 PL Cache 并落盘，提升了后续执行存储过程时命中 PL Cache 缓存的概率，从而显著提高存储过程执行性能。
