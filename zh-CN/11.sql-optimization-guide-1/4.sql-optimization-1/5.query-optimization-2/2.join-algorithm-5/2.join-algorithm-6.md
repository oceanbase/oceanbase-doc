...
* 索引嵌套循环联接（INDEX NESTED LOOP JOIN）
  ...
  *************************** 1. row ***************************
  ...
  在 outputs \& filters 的输出结果中 `nl_param` 出现参数 `[t2.c1]`，说明执行了条件下压优化。详细信息请参考 [JOIN](/zh-CN/11.sql-optimization-guide-1/2.sql-execution-plan-3/2.execution-plan-operator-2/3.JOIN-1-2.md)。
