...
更多系统视图的说明信息请参考《参考指南》文档中 [系统视图](../7.reference/5.system-reference/4.system-view-for-mysql/1.system-view-for-mysql-overview.md) 章节。
...
* 字符串函数：`CHARACTER_LENGTH()`、`FROM_BASE64()`、`LOAD_FILE()`、`MATCH()`、`OCTET_LENGTH()`、`SOUNDEX()` 和 `TO_BASE64()`。
  ...
  > **功能适用性**
  >
  > OceanBase 数据库社区版暂不支持 `FROM_BASE64()`、`SOUNDEX()` 和 `TO_BASE64()`。
...
更多分区的说明及使用请参见 [分区管理](../6.user-guide/8.distributed-storage-management/2.manage-partition-table/2.mysql-1/1.partition-overview-6.md) 。
...
更多优化器的详细信息请参见 [SQL 调优指南](../6.user-guide/9.performance-tuning-guide/5.sql-optimization/1.sql-request-execution-process-1.md)。
