...
oblogproxy 的配置文件默认放在 `conf/conf.json` 中。您只需要关心 [安装部署](/zh-CN/9.ecological-tools/4.CDC/3.oblogproxy/1.installation-and-deployment-2/1.installation-and-deployment-1.md) 中描述过的参数，对于其他参数，在不完全了解参数用途的情况下，我们不建议您进行修改。
