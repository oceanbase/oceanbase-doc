...
* tests 目录：目录中是一个 liboblog 的简单应用（oblog_tailf），oblog_taif 简介具体参考 [oblog_tailf](/zh-CN/9.ecological-tools/4.CDC/2.oceanbase-cdc/1.install-cdc/4.oblog_tailf.md)。tests 目录中包含以下内容：
...
您在安装 oblogproxy 后，liboblog 相关的依赖会自动添加。如何安装 oblogproxy，请参考 [使用源码构建 oblogproxy](/zh-CN/9.ecological-tools/4.CDC/3.oblogproxy/1.installation-and-deployment-2/1.installation-and-deployment-1.md) 和 [使用安装包安装部署 oblogproxy](/zh-CN/9.ecological-tools/4.CDC/3.oblogproxy/1.installation-and-deployment-2/2.install-and-deploy-logproxy-by-using-the-installation-package.md)。
