...
表组创建后，可以使用 `ALTER TABLE table_name SET TABLEGROUP tablegroup_name;` 语句将表加入到表组中。管理非分区表组的详细操作请参见 [管理表组](7.reference\3.user-guide-old\7.database-object-management-1\2.mysql-4\3.manage-a-table-group-2\2.table-group-management-statements-2.md)。
...
表组创建后，可以 使用 `ALTER TABLE table_name SET TABLEGROUP tablegroup_name;` 将已创建的表加入到表组中，也可以先创建表组，再使用  [CREATE TABLE](7.reference\4.development-guide-refactoring-1-old\6.sql-syntax\2.common-tenant-mysql-mode\6.sql-statement\16.create-table.md) 语句创建与表组分区方式相同的表，并通过 `TABLEGROUP` 属性指定表所属的表组。

例如，本示例中，如果先创建了表组 `tbl_rc_group`，则可以使用以下语句创建表并指定表组，语句如下。

```sql
obclient> CREATE TABLE tb1_rc(col1 INT,col2 INT) 
      TABLEGROUP = tbl_rc_group
       PARTITION BY RANGE COLUMNS(col1)
        (PARTITION p0 VALUES LESS THAN(100),
         PARTITION p1 VALUES LESS THAN(200),
         PARTITION p2 VALUES LESS THAN(300)
...
如果表组需要存放二级分区表，则需要创建与分区表具有相同分区方式的二级分区表表组。

示例 3：创建与表 `tb1_m_rcr` 分区方式相同的 Range Columns + Range 分区的模板化二级分区表表组。
...
同样的，示例 3 中，创建表组的语句中，`PARTITION BY RANGE COLUMNS 1` 中的 `1` 表示引用的列数，Range Columns 分区需要指定该值。从 `SUBPARTITION BY RANGE` 可以看出 Range 分区不需要指定引用的列数。
...
示例 5：创建与表 `tbl2_l` 分区方式相同的 List 分区表表组。
...
从示例 5 中，可以看到，List 分区的表组与分区表的分区数相同，且 List 分割点相同。

如果表组需要存放二级分区表，则需要创建与分区表具有相同分区方式的二级分区表表组，示例如下。
...
示例 9 中，创建表组的语法中，`PARTITION BY HASH PARTITIONS 2;` 不需要指定指定引用的列数，只要保证分区数相同即可。同时，该语句中未指定分区名，系统将按照分区规则为分区命名。

如果需要存放二级分区表，则需要创建对应的二级分区表表组。由于 OceanBase 数据库暂不支持以 `Key/Hash + Key/Hash` 分区组合的二级分区表表组，下面示例以其他组合分区为例。

示例 10：创建与表 `tbl3_hrc` 分区方式相同的 Hash + Range Columns 分区的模板化二级分区表表组。
...
示例 10 中，`PARTITIONS 2` 语句与分区表一致，表示指定 Hash 分区的分区个数，未指定分区的名称。系统将按照分区规则为分区命名。一级分区的每个分区分别命名为 p0、p1、...、pn。如果创建模板化二级分区表表组时未指定分区名，则二级分区的命名规则为 `($part_name)s($subpart_name)`。

下面介绍以 Hash 分区/Key 分区作为二级分区时，表组的创建方法。

示例 12：创建与表 `tbl4_f_lk` 分区方式相同的 List + Key 分区非模板化二级分区表表组。
...
示例 12 中，表组创建语句中，`SUBPARTITION BY KEY 1` 中的数值 `1` 指定了需要引用的列数，其他信息与分区表保持一致即可。

更多表组的创建示例，请参见 [创建一级分区表表组](4.0暂不支持) 和 [创建二级分区表表组](4.0暂不支持)。
