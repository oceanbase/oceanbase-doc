...
表组创建后，可以 使用 `ALTER TABLE table_name SET TABLEGROUP tablegroup_name;` 将已创建的表加入到表组中，也可以先创建表组，再使用 [CREATE TABLE](../../../7.reference/6.sql-syntax/2.common-tenant-mysql-mode/6.sql-statement/16.create-table.md) 语句创建与表组分区方式相同的表，并通过 `TABLEGROUP` 属性指定表所属的表组。
