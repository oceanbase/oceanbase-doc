...
请使用 [CREATE TABLE](../../../7.reference/6.sql-syntax/2.common-tenant-mysql-mode/6.sql-statement/16.create-table.md) 语句创建表，并参考以下内容完成表的创建。
...
* 根据数据类型的特点，为待存储在列中的数据选择合适的数据类型。
  ...
  OceanBase 数据库 MySQL 模式中支持的数据类型及详细介绍请参见 [数据类型概述](../../../7.reference/6.sql-syntax/2.common-tenant-mysql-mode/1.basic-elements/1.data-type/1.data-type-overview.md)。
...
要指定某一列为主键列，请在该列的定义后添加 `PRIMARY KEY` 关键字。如果要在多列上定义主键约束，请在 [CREATE TABLE](../../../7.reference/6.sql-syntax/2.common-tenant-mysql-mode/6.sql-statement/16.create-table.md) 语句中所有列的列表之后再添加主键约束的定义。
...
更多主键约束相关的介绍，请参见 [主键约束](../../../7.reference/4.concepts-of-oceanbase-database-system/4.database-objects-1/2.mysql/7.data-integrity-1/2.integrity-constraint-type-1/4.primary-key-constraint-1.md)。
...
* 外键约束（`FOREIGN KEY`）：要求约束的列的值取自于另外一个表的主键列。
  ...
  OceanBase 数据库默认开启了外键约束检查，外键约束检查开关由租户变量 `foreign_key_checks` 来控制。`foreign_key_checks` 变量的更多说明，请参见 [foreign_key_checks](../../../7.reference/14.system-reference/2.system-variable-for-mysql/27.foreign_key_checks-1-2-3.md)。
...
要约束单个列，请在该列的定义中添加一个约束关键字，如果要约束多个列，请在 [CREATE TABLE](../../../7.reference/6.sql-syntax/2.common-tenant-mysql-mode/6.sql-statement/16.create-table.md) 语句中所有列的列表之后再添加整个约束的定义。
...
示例 6：创建表 `tbl4`，并设置列 `col1` 的值需大于 `0`。

```sql
obclient> CREATE TABLE tbl4(col1 INT CHECK(col1>0),col2 INT);
Query OK, 0 rows affected
...
| CONSTRAINT_CATALOG | CONSTRAINT_SCHEMA | CONSTRAINT_NAME               | TABLE_SCHEMA | TABLE_NAME | CONSTRAINT_TYPE | ENFORCED |
+--------------------+-------------------+-------------------------------+--------------+------------+-----------------+----------+
| def                | xxx               | tbl4_OBCHECK_1665387311228914 | xxx          | tbl4       | CHECK           | YES      |
| def                | xxx               | PRIMARY                       | xxx          | test       | PRIMARY KEY     | YES      |
| def                | xxx               | PRIMARY                       | xxx          | tbl3       | PRIMARY KEY     | YES      |
| def                | xxx               | FK_col2                       | xxx          | tbl3       | FOREIGN KEY     | YES      |
+--------------------+-------------------+-------------------------------+--------------+------------+-----------------+----------+
...
您可以根据业务需要，修改这三个系统变量的值，修改系统变量的操作请参见 [系统变量概述](../../../7.reference/14.system-reference/2.system-variable-for-mysql/1.overview-of-system-variables-3.md)。
...
在 OceanBase 数据库的 MySQL 模式中，按照分区策略，分区表可以分为三大类：

* Range 分区 和 Range Columns 分区
...
二级分区支持 Range 分区、Range Columns 分区、List 分区、List Columns 分区、Hash 分区 和 key 分区等几种分区策略的两两组合。OceanBase 数据库的 MySQL 模式支持的分区类型及详细介绍请参见 [分区](7.reference\3.user-guide-old\8.distributed-storage-management\2.manage-partition-table\2.mysql-1\1.partition-overview-6.md)。
...
示例 8：创建模板化 Range Columns + Range Columns 分区表。
...
如果需要进行二次拆分，可以创建二级分区表，示例如下。

示例 9：创建模板化 Range Columns + Range 分区表。
...
示例 9 中，由于 Range 分区的分区键要求为整数类型，二次分区可以直接使用 `col2` 列名作为分区键，分区表 `tb1_m_rcr` 将会在原来分区的基础上，按照指定的 `3、6、9` 的范围进行再次分区。

由于二次拆分可以与第一次的拆分策略不同，除了 Range 分区/Range Columns 分区，二级分区还可以拆分为 List 分区/List Columns 分区以及Hash 分区/Key 分区。更多分区表的创建示例请参见 [创建分区表](7.reference\3.user-guide-old\8.distributed-storage-management\2.manage-partition-table\2.mysql-1\2.create-a-partition-table.md)。
...
List 分区和 List Columns 分区使用 `VALUES IN(value_list)` 关键字来定义每个分区，其中 List 分区 中的 `value_list` 是一个通过逗号分隔的整数列表，且每个分区的整数值集合不能有交集。
...
示例 10 中，由于 List 分区的分区键必须是整数类型，使用 `col1` 列作为分区键，分区表 `tbl2_l` 将会按照 `100、200` 的范围对数值进行分区。

如果需要做二次拆分，可以创建一个二次分区表，示例如下。

示例 11：创建非模板化 List + List Columns 分区表。
...
由于二次拆分可以与第一次的拆分策略不同，除了 List 分区/List Columns 分区，二级分区还可以拆分为 Range 分区/Range Columns 分区以及Hash 分区/Key 分区。更多分区表的创建示例请参见 [创建分区表](7.reference\3.user-guide-old\8.distributed-storage-management\2.manage-partition-table\2.mysql-1\2.create-a-partition-table.md)。

**Hash 分区 和 key 分区**

Hash 分区需要指定分区键和分区个数。通过 Hash 的分区表达式计算得到一个整数，根据这个结果再跟分区个数取模可以得到具体某行数据属于哪个分区。
...
示例 12 中，由于 Hash 分区的分区键必须是整数，可以使用 col1 作为分区键，将表 `tbl3_h` 分为 2 个 区。由于创建表时，未指定分区的名称，分区命名由系统根据命名规则完成，即一级分区的每个分区分别命名为 p0、p1、...、pn。

如果需要对表进行二次拆分，则可以创建二级分区表，示例如下。
...
>**说明**
>
>Hash 分区/key 分区作为二级分区时，对于非模板化二级分区表，二级分区的分区名为自定义的分区名；对于模板化二级分区表，二级分区的命名规则为 `($part_name)s($subpart_name)`。

由于二次拆分可以与第一次的拆分策略不同，除了 Hash 分区/Key 分区，二级分区还可以拆分为 Range 分区/Range Columns 分区以及List 分区/List Columns 分区。更多分区表的创建示例及操作说明请参见 [创建分区表](7.reference\3.user-guide-old\8.distributed-storage-management\2.manage-partition-table\2.mysql-1\2.create-a-partition-table.md)。

## 后续处理

* [创建表组](2.create-a-tablegroup.md)

* [创建索引](4.create-a-index.md)

* [管理分区](7.reference\3.user-guide-old\8.distributed-storage-management\2.manage-partition-table\2.mysql-1\1.partition-overview-6.md)
