...
在 OceanBase 数据库的 MySQL 模式中，按照分区策略，可以分为三大类：
...
如果需要进行二次拆分，可以创建二级分区表。由于拆分策略可以不同，除了 Range 分区/Range Columns 分区，二级分区还可以拆分为 List 分区/List Columns 分区以及Hash 分区/Key 分区，示例如下。

示例 9：创建非模板化 Range Columns + List Columns 分区表。
...
* List 分区的分区键必须是整数类型，List Columns 分区的分区键不要求是整数，可以是任意类型。

* List 分区仅支持单分区键，其分区键可以是一列，也可以是表达式，List Columns 分区的分区键不能是表达式，但可以是多列（列向量）。

示例 10：创建一个 List Columns 分区表。
...
如果需要做二次拆分，可以创建一个二次分区表。由于二级分区的拆分策略可以不同，除了 List 分区/List Columns 分区，二级分区还可以拆分为 Range 分区/Range Columns 分区以及Hash 分区/Key 分区，示例如下。

示例 11：创建模板化 List Columns + Range Columns 分区表。
...
Hash 分区需要指定分区键和分区个数。通过 Hash 的分区表达式计算得到一个整数，根据这个结果再跟分区个数取模可以得到具体某行数据属于哪个分区。通常用于指定分区键的热点查询，例如，按照用户 `id` 来分区。Hash 分区通常能消除热点查询。
...
如果需要对表进行二次拆分，则可以创建二级分区表，由于二级分区的拆分策略可以不同，除了 Hash 分区/Key 分区，二级分区还可以拆分为 Range 分区/Range Columns 分区以及List 分区/List Columns 分区，示例如下。

示例 13：创建非模板化 Hash + List 分区表。
...
更多分区表的创建示例及操作说明请参见 [创建分区表](7.reference\3.user-guide-old\8.distributed-storage-management\2.manage-partition-table\2.mysql-1\2.create-a-partition-table.md)。
