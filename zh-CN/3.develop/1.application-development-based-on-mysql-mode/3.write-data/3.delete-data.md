...
示例 1：删除 `ordr` 表中符合 `o_w_id=2` 的所有行数据。

```sql
obclient> DELETE FROM ordr WHERE o_w_id=2;
```

### 删除所有数据

当表中的数据量较小时，可以直接使用 `DELETE` 语句删除所有行数据。

示例 2：删除 `ordr` 表中的所有行数据。

```sql
obclient> DELETE FROM ordr;
```

当表中数据记录多达百万以上，一次性执行删除可能会出现性能问题，建议根据表内的信息使用 `WHERE` 条件进行分批删除，或者直接 [使用 TRUNCATE TABLE 语句清空表数据](#使用-truncate-table-语句清空表数据)。

示例 3：筛选表 `ordr` 中 `o_w_id` 列数据，分别执行多条语句分批删除 `o_w_id < 100000`，`o_w_id < 200000`，`o_w_id < 300000` 的数据。

```sql
obclient> DELETE FROM ordr WHERE o_w_id < 100000;

obclient> DELETE FROM ordr WHERE o_w_id < 200000;

obclient> DELETE FROM ordr WHERE o_w_id < 300000;
```
...
示例 5：通过 `TRUNCATE TABLE` 语句清空表 `ordr` 表中的所有数据。
