...
可以使用 REPLACE INTO 语句来插入或者替换数据，本文介绍了相关语句的使用方法和示例。
...
示例 1：创建 `t_replace` 表后，使用 `REPLACE INTO` 语句插入数据。
...
obclient> REPLACE INTO t_replace(id, name, value, gmt_create) VALUES(2,'US',2002,current_timestamp ());
Query OK, 1 row affected 
...
示例 2 中，`t_replace` 表中已有一条记录，由于 `(2,'US',2002,current_timestamp ())` 数据与表内的记录不违反唯一性约束，故执行结果是在 `t_replace` 表中插入一条记录。

此外，也可以使用查询语句充当 `REPLACE INTO` 语句的 `VALUES` 子句来插入多条数据。

示例 3：使用 `REPLACE INTO` 语句将表 `t_insert` 中的数据插入到表 `t_replace` 中。
...
obclient> REPLACE INTO t_replace(id, name, value, gmt_create) VALUES(2,'EN',2002,current_timestamp ());
Query OK, 2 rows affected
...
示例 4 中，由于表 `t_replace` 的 `id` 列为主键列，需要满足唯一性约束，而插入的数据 `(2,'EN',2002,current_timestamp ())` 违反唯一性约束，系统删除了原来的行记录 `(2,'US',2002,current_timestamp ())`，插入 `(2,'EN',2002,current_timestamp ())` 作为新的行记录。
