...
请使用 [INSERT](../../../7.reference/6.sql-syntax/2.common-tenant-mysql-mode/4.functions/2.functions-1/2.string-functions/14.INSERT-1-2-3-4.md) 语句，再参考下面的建议，向表中插入数据。
...
其中，表的 `id` 列、`name` 列不能为空，且 `id` 列为主键列，满足唯一性约束要求，不同有重复的值； `gmt_create` 列指定了默认值。
...
obclient> CREATE TABLE t_insert_bak(
    id number NOT NULL PRIMARY KEY,
    name varchar(10) NOT NULL, 
    value number,
    gmt_create DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP
...
obclient> INSERT INTO t_insert_bak SELECT * FROM t_insert;
Query OK, 2 rows affected

obclient> SELECT * FROM t_insert_bak;
+----+------+-------+---------------------+
| id | name | value | gmt_create          |
+----+------+-------+---------------------+
|  1 | CN   | 10001 | 2022-10-12 15:17:17 |
...
### 使用 `INSERT IGNORE INTO` 语句或 `INSERT INTO ON DUPLICATE KEY UPDATE` 语句避免唯一性约束冲突
...
该报错可以通过 `INSERT IGNORE INTO` 语句或 `INSERT INTO ON DUPLICATE KEY UPDATE` 语句来避免。其中：

* 通过  [`INSERT IGNORE INTO`](../../../7.reference/6.sql-syntax/2.common-tenant-mysql-mode/4.functions/2.functions-1/2.string-functions/14.INSERT-1-2-3-4.md) 语句避免约束冲突时，`IGNORE` 关键字可以忽略由于约束冲突导致的 `INSERT` 失败的影响。
...
* 通过 [`INSERT INTO ON DUPLICATE KEY UPDATE`](../../../7.reference/6.sql-syntax/2.common-tenant-mysql-mode/4.functions/2.functions-1/2.string-functions/14.INSERT-1-2-3-4.md) 语句避免约束冲突时，可以指定对重复主键或唯一键的后续处理。
  ...
  ```sql
  obclient> INSERT INTO t_insert(id, name, value) VALUES (3,'UK', 10003),(5, 'CN', 10005) ON DUPLICATE KEY UPDATE name=VALUES(name);
  Query OK, 1 row affected 
...
除了 `INSERT` 语句，当表中无数据记录，或者表中有数据记录但无主键或唯一键冲突时，还可以使用 `REPLACE INTO` 语句代替 `INSERT` 语句插入数据。`REPLACE INTO` 语句的详细语法及说明请参见 [REPLACE](../../../7.reference/6.sql-syntax/2.common-tenant-mysql-mode/6.sql-statement/46.REPLACE.md)。

示例 4：创建 `t_replace` 表后，使用 `REPLACE INTO` 语句插入数据。
