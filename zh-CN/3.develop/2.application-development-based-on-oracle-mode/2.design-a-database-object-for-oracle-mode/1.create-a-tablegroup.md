...
* 创建分区表表组时，表组需要与表具备相同的分区方式，具体要求如下：
  ...
  * 如果是 Hash 分区，要求表组的引用列数、分区数与表相同。

  * 如果是 Range/List 分区，要求表组与表的分区数相同，且分割点相同，即各 value 的规则相同。
...
在创建分区表表组时，对于 Range 分区，无论是作为一级分区还是二级分区，在进行分区时，要求引用的列数、分区数、Range 分隔点均与分区表相同。
...
```sql
obclient> CREATE TABLE tb1_r(col1 NUMBER,col2 NUMBER) 
...
从示例 2 中可以看到，表组 `tb1_r_group` 指定了引用的列数为 `1`，表组 `tb1_r_group` 与 表 `tb1_r` 的分区数均为 3 个，Range 分隔点均为 `100、200、300`。

## List 分区

在创建分区表表组时，对于 List 分区，无论是作为一级分区还是二级分区，在进行分区时，要求引用的列数、分区数、分隔点均与分区表相同。

与分区表一致，List 分区使用 `VALUES(value_list)` 关键字来定义每个分区。
...
在创建分区表表组时，对于 Hash 分区，无论是作为一级分区还是二级分区，要求引用列数、分区数均与分区表相同。
...
与分区表一样，表组也支持二级分区，二级分区表组的分区支持情况请参见 [创建分区表表组](4.0 暂不支持表组)。

下面通过几个示例，简单介绍二级分区表表组的介绍方法，更多二级分区表表组的创建示例请参见 [创建分区表](4.0表组暂不支持)。

示例 7： 创建与表 `tbl4_m_rl` 分区方式相同的 Range + List 分区的模板化二级分区表表组。

```sql
obclient> CREATE TABLE tbl4_m_rl(col1 NUMBER,col2 VARCHAR2(50))
       PARTITION BY RANGE(col1)
...
obclient> CREATE TABLEGROUP tbl4_m_rl_group
...
示例 7 中，表组 `tbl4_m_rl_group` 与表 `tbl4_m_rl` 的分区方式相同。同时，对于 Range 分区 和 List 分区，均需要指定引用的列数 1。


示例 8：创建与表 tbl6_f_lh 分区方式相同的 List + Hash 分区的非模板化二级分区表表组。

```sql
obclient> CREATE TABLE tbl6_f_lh(col1 NUMBER,col2 VARCHAR2(50))
       PARTITION BY LIST(col1)
...
obclient> CREATE TABLEgroup tbl6_f_lh_group
       PARTITION BY LIST 1
