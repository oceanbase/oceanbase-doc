...
示例 9 中，使用了 `col1` 作为分区键，将表 `tbl3_h` 分为 2 个 区。由于创建表时，未指定 Hash 分区的名称，分区命名由系统根据命名规则完成，即一级分区的每个分区分别命名为 p0、p1、...、pn。
...
下面通过几个简单示例，简单介绍二级分区表的创建方法。本文仅提供部分建表示例，分区表的详细示例请参见 [管理分区](../../../7.reference/2.manage-guide/4.distributed-storage-management/2.manage-partition-table/1.oracle-1/2.create-a-partition-table-2.md)。
...
创建模板化二级分区时，对于 Hash 分区，如果在创建时未指定分区名，则分区名由系统根据命名规则自动生成，分区的命名均为 `p0、p1、...、pn`。示例如下。

示例 11：创建 List + Hash 分区的模板化二级分区表。

```sql
obclient> CREATE TABLE tbl5_m_lh(col1 INT,col2 VARCHAR2(50))
     PARTITION BY LIST(col1)
...
obclient> SHOW CREATE TABLE tbl5_m_lh;
+-----------+---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
| TABLE     | CREATE TABLE
...
示例 11 中，使用 Hash 分区进行二次分区时未指定分区名，系统根据命名规则自动生成分区名 `p0、p1、p2、p3、p4`。

>**注意**
>
>对于 Hash 分区，如果是通过指定分区数的方式（例如，`SUBPARTITIONS 5`）进行分区，则在创建模板化二级分区表时，不需要指定 `SUBPARTITION TEMPLATE` 关键字。
...
示例 11 中，创建非模板化二级分区表，不需要添加 `SUBPARTITION TEMPLATE` 关键字。非模板化二级分区表的每个一级分区下的二级分区均可以自由定义，即每个一级分区下的二级分区的定义可以相同也可以不同。本示例中，先使用 List 分区方式进行分区，再使用 Hash 方式进行二次分区。
