...
3. 插入数据的 SQL 语句。
    ...
    定义插入数据的 SQL 语句，向 `test_tbl1` 表中插入了三条数据，分别为 `(John, 20)`、`(Lucy, 25)` 和 `(Tom, 30)`。
...
3. 定义创建表的函数。
    ...
    定义一个名为 `create_table()` 的函数，输出一条 INFO 级别的日志信息，表示开始创建表。连接数据库，创建游标。执行创建表的 SQL 语句，提交事务并输出日志信息，或回滚事务并输出错误日志信息。最后关闭游标和数据库连接。
...
4. 定义插入数据的函数。
    ...
    定义一个名为 `insert_data()` 的函数，输出一条 INFO 级别的日志信息，表示开始插入数据。连接数据库，创建游标。执行插入数据的 SQL 语句，提交事务并输出日志信息，或回滚事务并输出错误日志信息。最后关闭游标和数据库连接。
...
2. 定义查询数据的函数。
    ...
    定义一个名为 `select_data()` 的函数，用于从数据库中查询数据。函数中的详细步骤包括：创建数据库连接，使用 `cursor()` 方法创建一个游标对象。使用 `execute()` 方法执行 `SELECT_DATA_SQL` 定义的 SQL 语句查询数据。使用 `fetchall()` 方法获取查询结果，并通过 `for` 循环逐行输出结果。关闭游标和数据库连接。
