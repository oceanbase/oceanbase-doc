...
请使用 [CREATE TABLEGROUP](../../../7.reference/6.sql-syntax/2.common-tenant-mysql-mode/6.sql-statement/17.create-tablegroup.md) 语句创建表组。然后根据表组创建建议，参考本文的示例来完成表组的创建。
...
表组创建后，可以使用 `ALTER TABLE table_name SET TABLEGROUP tablegroup_name;` 语句将已创建的表加入到表组中。也可以先创建表组，再使用 [CREATE TABLE](../../../7.reference/6.sql-syntax/2.common-tenant-mysql-mode/6.sql-statement/16.create-table.md) 语句创建与表组分区方式相同的表，并通过 `TABLEGROUP` 属性指定表所属的表组。管理非分区表组的详细操作请参见 [管理表组](7.reference\3.user-guide-old\7.database-object-management-1\2.mysql-4\3.manage-a-table-group-2\2.table-group-management-statements-2.md)。
