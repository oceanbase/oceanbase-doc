...
使用用户 `ny1` 登录到数据库的 `Oracle` 租户，多次输入不正确的密码后，系统报错，提示用户被锁。
...
OceanBase 数据库设置了登录失败处理策略，对于多次登录失败的用户，OceanBase 数据库会锁定该用户，以便防止恶意的密码攻击，从而保护数据库，提升数据库的安全性。登录失败处理策略相关操作及说明请参见 [登录失败处理策略](../../../7.reference/2.manage-guide/2.basic-database-management/4.manage-tenants-1/5.manage-users-and-permissions/2.oracle-3/3.logon-failure-handling-policy.md)。
