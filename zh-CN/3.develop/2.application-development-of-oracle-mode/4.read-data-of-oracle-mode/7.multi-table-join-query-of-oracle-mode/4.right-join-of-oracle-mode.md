...
本文主要提供右联接的示例指导，全联接相关示例请参见 [全联接](2.full-join-of-oracle-mode.md)，左联接相关示例请参见 [左联接](3.left-join-of-oracle-mode.md)。
...
如果 `JOIN` 返回的数据除了符合联接条件和过滤条件的数据外，还包括右表里满足右表的过滤条件但不满足联接条件的数据时，就可以使用联接。右联接返回的结果里属于左表的数据如果不存在，则该列返回 `NULL`。
...
* [INNER JOIN](1.inner-join-of-oracle-mode.md)

* [FULL JOIN](2.full-join-of-oracle-mode.md)

* [LEFT JOIN](4.right-join-of-oracle-mode.md)