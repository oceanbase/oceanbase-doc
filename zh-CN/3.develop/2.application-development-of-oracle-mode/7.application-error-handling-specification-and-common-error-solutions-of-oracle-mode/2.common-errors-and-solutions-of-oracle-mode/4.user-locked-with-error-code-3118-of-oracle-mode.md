...
OceanBase 数据库设置了登录失败处理策略，对于多次登录失败的用户，OceanBase 数据库会锁定该用户，以便防止恶意的密码攻击，从而保护数据库，提升数据库的安全性。登录失败处理策略相关操作及说明请参见 [登录失败处理](../../../6.manage/5.security-and-permissions/2.identification-and-authentication/2.identification-and-authentication-of-oracle-mode/3.login-failure-handling-of-oracle-mode.md)。
