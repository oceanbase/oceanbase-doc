...
* 请确认您已连接到 OceanBase 数据库，连接数据库的操作请参见 [连接数据库（MySQL 模式）](../1.database-connection-with-client-of-mysql-mode/1.connection-methods-overview-of-mysql-mode.md) 或者 [连接数据库（Oracle 模式）](../1.connect-to-oceanbase-database/2.connect-to-oceanbase-database-of-oracle-mode/1.connection-methods-overview-of-oracle-mode.md)。
...
* 需确认拥有创建表权限：
  ...
  * 如果您连接的是 MySQL 模式租户，创建外表需要当前用户拥有 `CREATE` 权限，查看当前用户权限的操作请参见 [查看用户权限（MySQL 模式）](../../6.manage/5.security-and-permissions/3.access-control/2.user-and-permission/2.permission-of-mysql-mode/4.view-user-permissions-of-mysql-mode.md)。

  * 如果您连接的是 Oracle 模式租户，创建外表需要当前用户拥有 `CREATE TABLE` 权限，查看当前用户权限的操作请参见 [查看用户权限（Oracle 模式）](../../6.manage/5.security-and-permissions/3.access-control/2.user-and-permission/3.permission-of-oracle-mode/6.view-user-permissions-of-oracle-mode.md)。
...
关于 `CREATE EXTERNAL TABLE` 语句的详细信息，请参见 [CREATE EXTERNAL TABLE（MySQL 模式）](../../7.reference/5.sql-reference/1.sql-syntax/2.common-tenant-of-mysql-mode/6.sql-statement-of-mysql-mode/22.create-external-table-of-mysql-mode.md) 和 [CREATE EXTERNAL TABLE（Oracle 模式）](../../7.reference/5.sql-reference/1.sql-syntax/3.common-tenant-of-oracle-mode/9.sql-statement-of-oracle-mode/1.ddl-of-oracle-mode/16.create-external-table-of-oracle-mode.md) 语句创建外表，并参考以下内容完成外表的创建。
...
* OceanBase 数据库 Oracle 模式中支持的数据类型及详细介绍请参见 [内建数据类型概述](../../7.reference/5.sql-reference/1.sql-syntax/3.common-tenant-of-oracle-mode/3.basic-elements-of-oracle-mode/1.built-in-data-types-of-oracle-mode/1.overview-of-built-in-data-types-of-oracle-mode.md)。
...
* 本地 Location 格式：`LOCATION = '[file://] local_file_path'`
  ...
  <main id="notice" type='notice'>
  <h4>注意</h4>
  <p>对于使用本地 Location 格式的场景，需使用管理员用户（MySQL 模式：root；Oracle 模式：SYS）登录到集群对应的租户，设置系统变量 <a href="../../7.reference/8.configuration-items-and-system-variables/2.system-variable/3.global-system-variable/115.secure_file_priv-global.md">secure_file_priv</a> <code></code>，配置可以访问的路径。</p>
  </main>
...
* `ENCODING`：指定文件的字符集编码格式
  
  * 当前 MySQL 模式支持的所有字符集请参见 [字符集](../../7.reference/5.sql-reference/1.sql-syntax/2.common-tenant-of-mysql-mode/1.basic-elements-of-mysql-mode/3.character-set-and-collation-of-mysql-mode/2.character-set-of-mysql-mode.md)。如果不指定，默认值为 UTF8MB4。
  * 当前 Oracle 模式支持的所有字符集请参见 [字符集](../../7.reference/5.sql-reference/1.sql-syntax/3.common-tenant-of-oracle-mode/3.basic-elements-of-oracle-mode/2.data-type-comparison-rules-of-oracle-mode/1.overview-of-data-type-comparison-rules-of-oracle-mode.md)。如果不指定，默认值为 UTF8MB4。
...
2. 设置导入的文件路径。
    ...
    2. 执行以下命令，通过直连的方式连接租户 `mysql001`。

        ```shell
        obclient -h127.1 -P2881 -uroot@mysql001 -p****** -A
        ```

        更多连接数据库的信息，请参见 [连接数据库](../1.database-connection-with-client-of-mysql-mode/1.connection-methods-overview-of-mysql-mode.md)。
...
3. 执行以下命令，通过直连的方式重新连接租户 `mysql001`。

    <main id="notice" type='notice'>
    <h4>注意</h4>
    <p>这一步连接 OceanBase 数据库时，<code>-h</code> 指的的 IP 需要是第一步时的 OBServer 节点的 IP。</p>
    </main>
...
* [连接数据库（MySQL 模式）](../1.database-connection-with-client-of-mysql-mode/1.connection-methods-overview-of-mysql-mode.md)
* [CREATE EXTERNAL TABLE（MySQL 模式）](../../7.reference/5.sql-reference/1.sql-syntax/2.common-tenant-of-mysql-mode/6.sql-statement-of-mysql-mode/22.create-external-table-of-mysql-mode.md)
