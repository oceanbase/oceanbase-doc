# 创建索引
#docslug#/oceanbase-database/oceanbase-database/V3.2.3/create-an-index
您可以通过 `CREATE INDEX` 语句为表创建索引。

## 背景信息

OceanBase 数据库支持在非分区表和分区表上创建索引。

本文主要介绍如何在非分区表上创建索引，分区表上索引的创建请参见 [分区表的索引](../800.create-and-manage-partition-table/700.partition-table-index.md)。
