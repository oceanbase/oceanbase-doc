...
`ISNULL()` 函数可以用来代替等号（=）判断一个值是否为 `NULL`。（使用等号将一个值与 `NULL` 比较总是返回 `NULL`）`ISNULL()` 函数同 `IS NULL` 比较操作符具有一些相同的特性，有关比较操作符，请参见 [比较运算符](../../../200.operators/500.comparison-operators.md)。
