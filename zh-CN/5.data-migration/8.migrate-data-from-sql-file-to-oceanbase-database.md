...
有关 OceanBase 数据库 SQL 语法的详细信息，请参见 [SQL 语法（MySQL 模式）](../7.reference/4.development-reference/1.sql-syntax/2.common-tenant-mysql-mode/6.sql-statement/1.general-syntax.md) 和 [SQL 语法（Oracle 模式）](../7.reference/1.oceanbase-database-concepts/7.user-interface-and-query-language/1.SQL/1.sql-introduction/1.sql-overview.md)。
