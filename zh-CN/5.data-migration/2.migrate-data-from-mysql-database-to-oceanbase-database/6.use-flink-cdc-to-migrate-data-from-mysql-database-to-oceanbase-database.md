...
以下将介绍使用 Fink CDC 从 MySQL 数据库同步数据到 OceanBase 数据库。

## Flink CDC 环境准备

下载 Flink 和所需要的依赖包：

1. 下载 [Flink 1.15.3](https://downloads.apache.org/flink/flink-1.15.3/flink-1.15.3-bin-scala_2.12.tgz) 并将其解压至目录 `/FLINK_HOME/flink-1.15.3`。
...
2. 使用下面的命令启动 Flink 集群。
  ...
  <main id="notice" type='explain'>
    ...
    <p>示例如下：</p>
    <p><code></code>`bash
    [xxx@xxx /FLINK_HOME/flink-1.15.3]</p>
    <h1>chmod -R 777 /FLINK_HOME/flink-1.15.3/*</h1>
    <p><code></code>`</p>
  </main>
