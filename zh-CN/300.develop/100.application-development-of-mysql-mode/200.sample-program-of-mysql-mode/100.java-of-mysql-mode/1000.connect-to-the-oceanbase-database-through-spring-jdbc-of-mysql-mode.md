...
请点击 [spring-jdbc-mysql-client](https://obbusiness-private.oss-cn-shanghai.aliyuncs.com/doc/img/observer-enterprise/V4.2.0/3.develop/demo/java/spring-jdbc-mysql-client/spring-jdbc-mysql-client.zip) 下载项目代码。
...
2. `spring-jdbc-mysql-client` 项目代码介绍。
3. 将 `spring-jdbc-mysql-client` 项目导入到 Eclipse 中。
4. 修改 `spring-jdbc-mysql-client` 项目中的数据库连接信息。
5. 运行 `spring-jdbc-mysql-client` 项目。
...
1. 联系 OceanBase 数据库部署人员或者管理员获取相应的数据库连接串。
    ...
    **示例如下：**
    ...
    ```shell
    obclient -hxxx.xxx.xxx.xxx -P2881 -utest_user001@mysql001 -p****** -A
    ```
...
### 步骤二：spring-jdbc-mysql-client 项目代码介绍
...
5. 配置项目所依赖组件。
    ...
    <main id="notice" type='explain'>
      <h4>说明</h4>
      <p>本部分代码定义项目所依赖组件是 MySQL Connector/J 的 V8.0.25 版本，如果需要了解其他版本的信息，请参见 <a href="https://downloads.mysql.com/archives/c-j/">MySQL Connector/J</a>。</p>
    </main>

    定义了两个依赖库：MySQL JDBC 和 spring-jdbc。MySQL JDBC，版本号为 8.0.25。Spring JDBC 是 Spring 框架中的一个模块，它提供了一组简单的 API，可以在 Java 应用程序中进行数据库操作，例如查询、插入、更新和删除数据，版本号为 `5.2.0.RELEASE`。这些依赖库将会被 Maven 自动下载并添加到项目中，以供项目使用。具体步骤如下：
...
4. 创建数据源。
    ...
    2. 设置驱动类名：通过调用 `setDriverClassName` 方法，设置数据库驱动类名为 `com.mysql.cj.jdbc.Driver`。
...
6. 创建表。
    ...
    1. 定义一个创建表的 SQL 语句：SQ 语句中使用了 `IF NOT EXISTS` 来避免重复创建表，如果表已经存在，则不会执行创建操作。表名为 `test_tbl1`，定义了两个列，一个是名为 `id` 的 `INT` 类型列，使用 `AUTO_INCREMENT` 关键字使其自增，并将其设置为主键。另一个是名为 `name` 的 `VARCHAR(20)` 类型列，长度为 20 个字符。
    2. 执行创建表的 SQL 语句：`jdbcTemplate` 对象通过调用 `execute` 方法，执行了上述定义的创建表的 SQL 语句。

    **代码如下：**

    ```java
            String createTableSql = "CREATE TABLE IF NOT EXISTS test_tbl1 (id INT AUTO_INCREMENT PRIMARY KEY, name VARCHAR(20))";
            jdbcTemplate.execute(createTableSql);
    ```

7. 插入数据。

    1. 定义插入数据的 SQL 语句：SQL 语句中指定了要插入的表为 `test_tbl1`，列为 `name`，插入了两行数据，分别是 `('A1')` 和 `('A2')`。
    2. 执行插入数据的 SQL 语句：`jdbcTemplate` 对象通过调用 `update` 方法，执行了上述定义的插入数据的 SQL 语句。

    **代码如下：**

    ```java
            String insertDataSql = "INSERT INTO test_tbl1 (name) VALUES ('A1'), ('A2')";
...
### 步骤三：将 spring-jdbc-mysql-client 项目导入到 Eclipse 中
...
### 步骤四：修改 spring-jdbc-mysql-client 项目中的数据库连接信息
...
```java
...
        String url = "jdbc:mysql://xxx.xxx.xxx.xxx:2881/test";
        String username = "test_user001@mysql001";
        String password = "******";
...
```

### 步骤五：运行 spring-jdbc-mysql-client 项目
...
3. 在 Eclipse 的控制台窗口中来查看输出结果。
    ...
    ![log](https://obbusiness-private.oss-cn-shanghai.aliyuncs.com/doc/img/observer-enterprise/V4.2.0/3.develop/demo/java/spring-jdbc-mysql-client/5log.png)
