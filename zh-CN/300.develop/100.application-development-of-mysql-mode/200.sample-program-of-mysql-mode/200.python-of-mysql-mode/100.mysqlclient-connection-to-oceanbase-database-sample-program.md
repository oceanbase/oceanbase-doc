...
在 Windows 上，您可以使用预编译的二进制文件来安装 mysqlclient 库。从 [Download files](https://pypi.org/project/mysqlclient/#files) 下载与你的 Python 版本和操作系统平台兼容的 `whl` 文件，并使用`pip install` 命令来安装。
