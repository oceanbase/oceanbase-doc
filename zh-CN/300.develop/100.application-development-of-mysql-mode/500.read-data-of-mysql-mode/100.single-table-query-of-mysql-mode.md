...
在使用 LIMIT 和 OFFSET 实现分页查询的场景中，排序稳定性可能受到诸如执行计划、并行处理等因素的影响，这可能导致结果输出的顺序不一致。为了获得有序且可预测的结果，通常需要在查询的外层添加 `ORDER BY` 子句。

为了避免 MySQL 模式中迁移业务分页场景中需要对分页逻辑进行大量修改的工作，您可以使用隐藏的配置项  `_preserve_order_for_pagination` 开启分页查询保序功能。默认不开启，请根据实际场景开启。

<main id="notice" >
  <h4>说明</h4>
  <p>在启用分页保序功能时，新增的排序操作会对性能产生一定影响。对于某些查询，引入排序计算可能导致性能的相对下降。</p>
</main>

此外，打开分页保序功能之后，查询输出结果可能与未开启分页保序功能时不同。

以下是一个未启用分页保序功能时的查询示例：

```sql
SELECT t1.c1, t1.c2 FROM t1, t2 WHERE t1.c1 = t2.c1 LIMIT 10, 10;
```

在这种情况下，查询的输出会随数据库执行计划的变化而变化，无法保证输出结果的稳定性。

然而，当分页保序功能开启后，OceanBase 会自动引入排序字段，并按照这些字段顺序重写查询，如下所示：

```sql
SELECT t1.c1, t1.c2 FROM t1, t2 WHERE t1.c1 = t2.c1
ORDER BY t1.c1, t2.c2 LIMIT 10, 10;
```
此时，无论执行计划如何变化，查询结果始终保持稳定一致。
