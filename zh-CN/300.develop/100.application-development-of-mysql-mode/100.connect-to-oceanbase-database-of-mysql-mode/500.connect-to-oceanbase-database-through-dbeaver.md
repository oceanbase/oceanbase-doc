...
3. 在 **选择您的数据库** 页面选择 **OceanBase**，单击 **下一步**，进入到 **连接设置** 页面，根据待连接的数据库信息进行填写，之后单击 **测试连接**。
   ...
   <main id="notice" type='notice'>
       <h4>说明</h4>
       <p>您可通过 IP 直连或 OBProxy 代理两种方式连接数据库。直连时 Port 填写配置的 `mysql_port` 进行连接，默认端口为 2881。使用 OBProxy 连接时，Port 填写配置的 `prometheus_listen_port`，默认端口为 2883。</p><p>推荐使用 OBProxy 进行连接，用户名格式为：`用户名@租户名#集群名`。</p>
   </main>
