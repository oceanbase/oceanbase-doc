...
请点击 [c3p0-mysql-jdbc](https://obbusiness-private.oss-cn-shanghai.aliyuncs.com/doc/img/observer-enterprise/V4.2.0/3.develop/connection-pool/c3p0-mysql-jdbc.zip) 下载项目代码。
...
2. `c3p0-mysql-jdbc` 项目代码介绍。
3. 将 `c3p0-mysql-jdbc` 项目导入到 Eclipse 中。
4. 修改 `c3p0-mysql-jdbc` 项目中的数据库连接信息。
5. 运行 `c3p0-mysql-jdbc` 项目。
...
2. 根据 OceanBase 数据库连接串信息填写下面 URL 的对应信息。
    ...
    * `password`：提供账户密码。
    ...
    更多有关 MySQL Connector/J 连接 URL 语法信息，请参见 [Connection URL Syntax](https://dev.mysql.com/doc/connector-j/8.1/en/connector-j-reference-jdbc-url-format.html)。
...
### 步骤三：将 c3p0-mysql-jdbc 项目导入到 Eclipse 中
...
2. 在弹出的对话框中，点击 **Directory** 按钮选择项目所在的目录，然后点击 **Finish** 完成导入。
    ...
    ![Import](https://obbusiness-private.oss-cn-shanghai.aliyuncs.com/doc/img/observer-enterprise/V4.2.0/3.develop/c3p0-mysql-jdbc/1Import.png)

3. 查看项目情况。

    ![p1](https://obbusiness-private.oss-cn-shanghai.aliyuncs.com/doc/img/observer-enterprise/V4.2.0/3.develop/c3p0-mysql-jdbc/2p1.png)

### 步骤四：修改 c3p0-mysql-jdbc 项目中的数据库连接信息
...
### 步骤五：运行 c3p0-mysql-jdbc 项目

1. 在项目导航器视图中，找到并展开 **src/main/java** 目录。

2. 右键点击 **Main.java** 文件，然后选择 **Run As** -> **Java Application**。

    ![run](https://obbusiness-private.oss-cn-shanghai.aliyuncs.com/doc/img/observer-enterprise/V4.2.0/3.develop/c3p0-mysql-jdbc/4run.png)

3. 在 Eclipse 的控制台窗口中来查看项目的日志信息和输出结果。

    ![log](https://obbusiness-private.oss-cn-shanghai.aliyuncs.com/doc/img/observer-enterprise/V4.2.0/3.develop/c3p0-mysql-jdbc/5log.png)
