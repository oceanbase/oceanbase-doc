...
2. `hikaricp-mysql-client` 项目代码介绍。
3. 将 `hikaricp-mysql-client` 项目导入到 Eclipse 中。
4. 修改 `hikaricp-mysql-client` 项目中的数据库连接信息。
5. 运行 `hikaricp-mysql-client` 项目。
...
### 步骤二：hikaricp-mysql-client 项目代码介绍
...
### 步骤三：将 hikaricp-mysql-client 项目导入到 Eclipse 中
...
### 步骤四：修改 hikaricp-mysql-client 项目中的数据库连接信息
...
username=test_user@mysql001
password=******
...
```

### 步骤五：运行 hikaricp-mysql-client 项目
