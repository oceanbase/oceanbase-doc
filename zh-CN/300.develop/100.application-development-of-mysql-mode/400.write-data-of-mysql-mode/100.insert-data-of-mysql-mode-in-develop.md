...
2. 在 `tbl1` 表中插入几条数据。
   ...
   ```shell
   obclient [test]> INSERT INTO tbl1 (col1, col2) VALUES (15, 2);
...
7. 使用 `insert overwrite` 将表 `tbl1` 中的数据导入到表 `tbl2` 中的指定分区 `p1` 中。

   ```shell
   obclient [test]> insert overwrite tbl1 partition (p1) select * from tbl2 partition (p1);
   ```
