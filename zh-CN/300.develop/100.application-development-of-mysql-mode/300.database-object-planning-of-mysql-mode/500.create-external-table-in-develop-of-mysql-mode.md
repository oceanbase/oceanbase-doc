...
<main id="notice" type='explain'>
  <h4>说明</h4>
  <p>OceanBase 数据库从 V4.6.0 版本开始：<ul><li>Parquet/ORC 文件格式外表支持读取复杂数据类型：<code>Array</code>、<code>Map</code>、<code>JSON</code>。外表读取需与 OceanBase 数据库支持的数据类型需要保持一致，详细介绍请参见 <a href="../../../700.reference/500.sql-reference/100.sql-syntax/200.common-tenant-of-mysql-mode/100.basic-elements-of-mysql-mode/100.data-type-of-mysql-mode/100.data-type-overview-of-mysql-mode.md">数据类型概述</a>。</li><li>OceanBase 数据库 Parquet/ORC 文件格式外表支持读取 Bloom Filter，根据查询的谓词来过滤数据，减少不必要的 IO，用于查询加速。</li></ul></p>
</main>
