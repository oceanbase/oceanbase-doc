...
* 远程 Location 格式如下：
   ...
   * `LOCATION = '{oss|cos|S3}://$ACCESS_ID:$ACCESS_KEY@$HOST:s3_region/remote_file_path'`：其中`$ACCESS_ID`、`$ACCESS_KEY` 和 `$HOST` 为访问阿里云 OSS、腾讯云 COS 和 S3 所需配置的访问信息，`s3_region` 为使用 S3 时选择的区域信息，这些敏感的访问信息会以加密的方式存放在数据库的系统表中。

  * `LOCATION = hdfs:// ${hdfs namenode address}: ${port}/PATH.localhost`：其中 `port` 指的是 HDFS 的端口号，`PATH`指 HDFS 中的目录路径。
