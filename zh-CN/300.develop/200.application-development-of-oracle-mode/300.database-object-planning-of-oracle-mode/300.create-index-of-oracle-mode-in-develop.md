...
创建表 `tbl7` 后，再创建列存索引 `idx1_tbl7_cg`。

1. 创建表 `tbl7`。

   ```sql
   CREATE TABLE tbl7(id NUMBER, name VARCHAR2(20), age NUMBER);
   ```

2. 在表 `tbl7` 上创建列存索引 `idx1_tbl7_cg`，并且在索引表上存储 `age` 列的冗余数据。

   ```sql
   CREATE INDEX idx1_tbl7_cg ON tbl7(name) STORING(age) WITH COLUMN GROUP(each column);
   ```
