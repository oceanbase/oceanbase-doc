...
```shell
obclient -h$host -P$port -u$user_name -p****** [-c] [-A] [$schema_name]
```
...
OceanBase 数据库支持通过指定 ODP 的 `IP:PORT` 以及 `SERVICE_NAME` 进行登陆。根据指定的 `SERVICE_NAME`，ODP 将连接串转换为 `-uuser_name@主租户名#主租户所在集群`，用户不需要主备切换后手动改变连接串。

<main id="notice" type='notice'>
  <h4>注意</h4>
  <p>对于 OceanBase 数据库 V4.2.1 版本，从 V4.2.1 BP9 版本开始支持 <code>SERVICE_NAME</code> 功能。该功能依赖于 OceanBase 云平台（OceanBase Cloud Platform, OCP）。要求 OCP 为 V4.3.1 及之后版本，并且需要 OCP 接管了 OceanBase 集群才能正常使用。</p>
</main>
...
2. 使用 OBClient 客户端通过 ODP 连接 OceanBase 数据库的 Oracle 模式租户。
   ...
   ```shell
   obclient -hxxx.xx.xxx.xxx -P2883 -usys@oracle001#test421 -p****** -A test_user001
   ```
   ...
   Server version: OceanBase 4.2.1.0 (r200000342024061210-c4c0c18741e45a1d40889b6147c3132574d6e7ea) (Built Jun 12 2024 10:58:58)
