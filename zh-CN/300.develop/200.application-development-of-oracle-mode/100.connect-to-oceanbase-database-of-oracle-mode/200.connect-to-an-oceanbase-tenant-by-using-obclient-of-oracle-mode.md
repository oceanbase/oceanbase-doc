...
* `$user_name`：指定 OceanBase 数据库的连接账户，格式有：`用户名@租户名#集群名`、`集群名:租户名:用户名`、`集群名-租户名-用户名` 或者 `集群名.租户名.用户名`。Oracle 租户的管理员用户名默认是 `SYS`。
...
* `$user_name`：指定 OceanBase 数据库的连接账户，格式为：`用户名@租户名`。Oracle 租户的管理员用户名默认是 `sys`。
* `******`: 指定连接 OceanBase 数据库的密码。`******` 应该被实际的密码字符串替换。
