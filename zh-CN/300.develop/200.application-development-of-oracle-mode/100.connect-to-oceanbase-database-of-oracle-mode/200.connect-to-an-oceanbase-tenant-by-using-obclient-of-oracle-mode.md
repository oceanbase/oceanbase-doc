...
2. 连接数据库。
   ...
   * 通过 ODP 连接的方式
      ...
      ```sql
      obclient -h10.10.10.1 -uusername@obtenant#obdemo -P2883 -p****** -c -A oceanbase --proxy_user[=]user_name
      ```

      或者

      ```sql
      obclient -h10.10.10.1 -uobdemo:obtenant:username -P2883 -p****** -c -A oceanbase --proxy_user[=]user_name
      ```
      ...
      * `--proxy_user`：可选项，该参数指定被代理的用户名，即目标用户名。更多有关使用代理用户的示例信息，请参见 [使用代理用户](链接待添加)。
   ...
   * 通过直连方式
      ...
      ```sql
      obclient -h10.10.10.1 -uusername@obtenant -P2881 -p****** -c -A oceanbase --proxy_user[=]user_name
      ```
