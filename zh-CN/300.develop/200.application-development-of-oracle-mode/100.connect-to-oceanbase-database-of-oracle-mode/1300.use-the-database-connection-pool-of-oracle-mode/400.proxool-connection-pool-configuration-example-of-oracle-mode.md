...
请点击 [proxool-oceanbase-client](https://obbusiness-private.oss-cn-shanghai.aliyuncs.com/doc/img/observer-enterprise/V4.2.0/3.develop/connection-pool/proxool-oceanbase-client/proxool-oceanbase-client.zip) 下载项目代码。
...
2. `proxool-oceanbase-client` 项目代码介绍。
3. 将 `proxool-oceanbase-client` 项目导入到 IntelliJ IDEA 中。
4. 修改 `proxool-oceanbase-client` 项目中的数据库连接信息。
5. 运行 `proxool-oceanbase-client` 项目。
...
### 步骤二：proxool-oceanbase-client 项目代码介绍
...
`db.properties` 是本文示例的连接池配置文件，其中包含了连接池的各种配置属性。
...
| prototype-count | 0 | 用于设置连接池的原型连接数。默认值为 0，表示连接池不会主动创建额外的连接。</br> 与 `minimum-connection-count` 不同的是，`prototype-count` 这个属性会考虑当前活动连接的数量。如果可用连接数少于设定的 `prototype-count`，连接池会创建更多的连接（前提是未达到最大连接数）。例如，如果有 3 个活动连接和 2 个可用连接，但 `prototype-count` 设定为 4，那么连接池将尝试创建另外 2 个连接。 </br> `prototype-count` 的目的是在当前活动连接的基础上维持一定数量的备用连接。这些备用连接可以避免在高负载期间由于连接池中可用连接不足而导致的连接请求被阻塞。通过创建更多的连接，连接池可以更好地应对并发请求，提高应用程序的性能和响应速度。 </br> 在调整这个属性时，需要考虑应用程序的并发需求和数据库的配置。如果应用程序预计会有较高的并发请求，可以适当增加 `prototype-count`；如果应用程序的并发请求较少，可以将该属性设置为 0，以减少资源消耗。|
...
<main id="notice" type='notice'>
  <h4>注意</h4>
  <p>每个参数的具体设置取决于您的应用程序的需求和数据库的要求。建议根据实际情况来选择配置参数。更多 Proxool 参数的信息，请参见 <a href="https://proxool.sourceforge.net/properties.html">Properties</a>。</p>
</main>
...
### 步骤三：将 proxool-oceanbase-client 项目导入到 IntelliJ IDEA 中
...
### 步骤四：修改 proxool-oceanbase-client 项目中的数据库连接信息
...
### 步骤五：运行 proxool-oceanbase-client 项目
