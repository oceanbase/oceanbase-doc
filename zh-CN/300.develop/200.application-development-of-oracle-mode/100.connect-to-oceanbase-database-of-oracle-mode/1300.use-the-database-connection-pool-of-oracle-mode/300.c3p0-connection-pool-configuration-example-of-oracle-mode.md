...
请点击 [c3p0-oceanbase-jdbc](https://obbusiness-private.oss-cn-shanghai.aliyuncs.com/doc/img/observer-enterprise/V4.2.0/3.develop/connection-pool/c3p0-oceanbase-jdbc.zip) 下载项目代码。
...
2. `c3p0-oceanbase-jdbc` 项目代码介绍。
3. 将 `c3p0-oceanbase-jdbc` 项目导入到 Eclipse 中。
4. 修改 `c3p0-oceanbase-jdbc` 项目中的数据库连接信息。
5. 运行 `c3p0-oceanbase-jdbc` 项目。
...
### 步骤二：c3p0-oceanbase-jdbc 项目代码介绍

点击 [c3p0-oceanbase-jdbc](https://obbusiness-private.oss-cn-shanghai.aliyuncs.com/doc/img/observer-enterprise/V4.2.0/3.develop/connection-pool/c3p0-oceanbase-jdbc.zip) 下载项目代码，是一个名称为 `c3p0-oceanbase-jdbc.zip` 的压缩包。
...
<main id="notice" type='notice'>
  <h4>注意</h4>
  <p>每个参数的具体设置取决于您的应用程序的需求和数据库的要求。建议根据实际情况来选择配置参数。更多 C3P0 配置项信息，请参见 <a href="https://www.mchange.com/projects/c3p0/">C3P0</a>。</p>
</main>
...
### 步骤三：将 c3p0-oceanbase-jdbc 项目导入到 Eclipse 中
...
2. 在弹出的对话框中，点击 **Directory** 按钮选择项目所在的目录，然后点击 **Finish** 完成导入。
    ...
    ![Import](https://obbusiness-private.oss-cn-shanghai.aliyuncs.com/doc/img/observer-enterprise/V4.2.0/3.develop/c3p0-oceanbase-jdbc/1.Import.png)

3. 查看项目情况。

    ![p1](https://obbusiness-private.oss-cn-shanghai.aliyuncs.com/doc/img/observer-enterprise/V4.2.0/3.develop/c3p0-oceanbase-jdbc/2p1.png)

### 步骤四：修改 c3p0-oceanbase-jdbc 项目中的数据库连接信息

根据 **步骤一：获取 OceanBase 数据库 URL** 中获取的信息修改 `c3p0-config.xml` 文件中的数据库连接信息。
...
### 步骤五：运行 c3p0-oceanbase-jdbc 项目

1. 在项目导航器视图中，找到并展开 **src/main/java** 目录。

2. 右键点击 **Main.java** 文件，然后选择 **Run As** -> **Java Application**。

    ![run](https://obbusiness-private.oss-cn-shanghai.aliyuncs.com/doc/img/observer-enterprise/V4.2.0/3.develop/c3p0-oceanbase-jdbc/4run.png)

3. 在 Eclipse 的控制台窗口中来查看项目的日志信息和输出结果。

    ![log](https://obbusiness-private.oss-cn-shanghai.aliyuncs.com/doc/img/observer-enterprise/V4.2.0/3.develop/c3p0-oceanbase-jdbc/5log.png)

4. 也可以在 OceanBase 客户端（OBClient）中执行以下 SQL 语句查看结果。

    ```shell
    obclient [SYS]> SELECT * FROM test_c3p0;
    ```
