|description|介绍Oracle模式下数据库连接池的配置方法和建议参数设置。|
|---|---|
|keywords|连接池,JDBC,Oracle模式,数据库连接|
|dir-name|数据库连接池配置|
...
* 设置连接空闲超时时间，推荐 30 分钟。
  ...
  默认 8 小时连接主动断开，客户端无法感知，导致存在脏连接。连接池可以通过心跳、testOnBorrow 等机制来校验连接是否存活，当超过这个时间连接没有使用直接断开。
