...
本文将介绍如何使用 Spring JDBC 框架、OceanBase Connector/J 和 OceanBase 数据库构建一个应用程序，实现基本的数据库操作，包括创建表、插入和查询数据等。

请点击 [spring-jdbc-oceanbase-client](https://obbusiness-private.oss-cn-shanghai.aliyuncs.com/doc/img/observer-enterprise/V4.2.0/3.develop/demo/java/spring-jdbc-oceanbase-client/spring-jdbc-oceanbase-client.zip) 下载项目代码。
...
5. 配置项目所依赖组件。
    ...
    定义了两个依赖库：oceanbase-client 和 spring-jdbc。oceanbase-client 是 OceanBase 数据库的客户端库，版本号为 `2.4.2`。Spring JDBC 是 Spring 框架中的一个模块，它提供了一组简单的 API，可以在 Java 应用程序中进行数据库操作，例如查询、插入、更新和删除数据，版本号为 `5.2.0.RELEASE`。这些依赖库将会被 Maven 自动下载并添加到项目中，以供项目使用。具体步骤如下：
...
6. 创建表。
    ...
    1. 定义一个创建表的 SQL 语句：SQL 语句中指定表名为 `test_tbl1`，并定义了两个列，一个是名为 `id` 的 `NUMBER` 类型列，另一个是名为 `name` 的 `VARCHAR2(20)` 类型列。
    2. 执行创建表的 SQL 语句：`jdbcTemplate` 对象通过调用 `execute` 方法，执行了上述定义的创建表的 SQL 语句。

    **代码如下：**

    ```java
            String createTableSql = "CREATE TABLE test_tbl1 (id NUMBER, name VARCHAR2(20))";
            jdbcTemplate.execute(createTableSql);
    ```

7. 插入数据。

    1. 定义插入数据的 SQL 语句：SQL 语句中指定了要插入的表为 `test_tbl1`，列为 `id` 和 `name`，插入了两行数据，分别是 `(1,'A1')` 和 `(2,'A2')`。
    2. 执行插入数据的 SQL 语句：`jdbcTemplate` 对象通过调用 `update` 方法，执行了上述定义的插入数据的 SQL 语句。

    **代码如下：**

    ```java
            String insertDataSql = "INSERT INTO test_tbl1 (id,name) VALUES (1,'A1'), (2,'A2')";
            jdbcTemplate.update(insertDataSql);
    ```

8. 查询数据。

    1. 定义查询数据的 SQL 语句：SQL 语句中使用了 `SELECT *` 来选择所有列，从 `test_tbl1` 表中查询数据。
    ...
    **代码如下：**
    ...
    ```java
            String selectDataSql = "SELECT * FROM test_tbl1";
...
3. 在 Eclipse 的控制台窗口中来查看输出结果。
    ...
    ![log](https://obbusiness-private.oss-cn-shanghai.aliyuncs.com/doc/img/observer-enterprise/V4.2.0/3.develop/demo/java/spring-jdbc-oceanbase-client/5log.png)
