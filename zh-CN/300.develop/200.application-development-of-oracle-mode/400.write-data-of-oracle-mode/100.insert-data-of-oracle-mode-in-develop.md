...
* `INSERT OVERWRITE SELECT` 数据写入操作是全量旁路导入方式，所以操作受全量旁路导入功能限制。有关旁路导入的信息，参见 [使用 INSERT INTO SELECT 语句旁路导入数据](../../../500.data-migration/1100.bypass-import/300.use-insert-into-select-statement-to-bypass-import-data.md) 中的 **使用限制** 章节。
* `INSERT OVERWRITE SELECT` 指定旁路导入 Hint 会报错。
