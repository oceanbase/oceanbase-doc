...
```sql
INSERT [/*+PARALLEL(N)*/] OVERWRITE table_name [PARTITION (partition_name, ...)] select_stmt;
```
...
5. 分别查询两张表中的数据。
    ...
    ```sql
    obclient [SYS]> select * from t1_ins_overwrt;
    +------+------+
    | C1   | C2   |
    +------+------+
    |   12 |   20 |
    ...
    obclient [SYS]> select * from t2_ins_overwrt;
    +------+------+
    | C1   | C2   |
    +------+------+
    |    3 |   60 |
...
6. 执行如下 SQL 将 `t1_ins_overwrt` 表中的数据插入到 `t2_ins_overwrt` 表指定的分区（p1, p3）中，如果 `t2_ins_overwrt` 表中已存在指定分区的数据，则这些数据将被覆盖。

    ```sql
    obclient [SYS]> insert overwrite t2_ins_overwrt partition(p1, p3) select * from t1_ins_overwrt;
    Query OK, 3 rows affected (1.360 sec)
    Records: 3  Duplicates: 0  Warnings: 0
    ```

7. 查询 `t2_ins_overwrt` 表，发现 p3 分区的数据 （142，1420）被覆盖了。
