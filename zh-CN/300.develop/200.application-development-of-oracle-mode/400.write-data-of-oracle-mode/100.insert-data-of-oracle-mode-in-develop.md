...
* 该语句数据写入操作是全量旁路导入方式，所以操作受全量旁路导入功能限制。有关旁路导入的信息，参见 [使用 INSERT INTO SELECT 语句旁路导入数据](../../../500.data-migration/1100.bypass-import/200.full-bypass-import.md) 中的 **使用限制** 章节。
* 该语句指定旁路导入 Hint 会报错。
...
* 该语句无法在多行事务中操作。因此，为确保操作顺利进行，需先执行 `SET autocommit = on;` 命令开启自动提交事务。
* 从 OceanBase 数据库 V4.3.5 BP1 版本开始，分区级的 `insert overwrite` 操作支持目标表为 Hash 或 Key 分区类型。
...
7. 使用 `insert overwrite` 将表 `tbl2` 中的数据导入到表 `tbl1` 中的指定分区 `p0` 和 `p1`。
