...
* 对写入表加表锁，不允许任何并发的 DML 和 DDL 操作。
* 当前版本仅支持对整个表进行数据覆盖插入，而不支持针对表的特定分区执行分区级别的数据覆盖操作。
* 受 PDML（Parallel Data Manipulation Language，并行数据操纵语言）框架限制，PDML 不支持的场景无法导入数据，会报错 not supported。有关并行 DML 的详细信息，参见 [并行 DML](../../../700.reference/1000.performance-tuning-guide/500.sql-optimization/300.distributed-execution-plan/1000.parallel-dml.md)。
