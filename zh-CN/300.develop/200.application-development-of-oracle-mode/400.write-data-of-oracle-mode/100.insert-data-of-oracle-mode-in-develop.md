...
## 使用 INSERT OVERWRITE 语句插入数据

`INSERT OVERWRITE` 语句用于将查询结果替换表中的现有数据，即该语句会先清空表中的旧数据，再将新数据批量写入到表中。

<main id="notice" type='explain'>
  <h4>说明</h4>
  <p>当前版本 <code>INSERT OVERWRITE</code> 语句仅支持对整个表进行数据覆盖插入，而不支持针对表的特定分区执行分区级别的数据覆盖操作。</p>
</main>

`INSERT OVERWRITE` 语句简单语法格式如下：

```sql
INSERT OVERWRITE table_name select_stmt;
```
...
<main id="notice" type='notice'>
  <h4>注意</h4>
  <p>该语句无法在事务中执行。因此，为确保操作顺利进行，需先执行 <code>SET autocommit = on;</code> 命令开启自动提交事务。</p>
</main>
...
6. 使用 `INSERT OVERWRITE` 语句，基于 `col3` 大于 25 从 `source_tbl1` 中筛选数据，并将这些数据插入到 `target_tbl1` 中，替换其原有内容。
