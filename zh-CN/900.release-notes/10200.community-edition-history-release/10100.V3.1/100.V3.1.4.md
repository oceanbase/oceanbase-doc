...
本文主要介绍 OceanBase 数据库 V3.1.4 版本的新特征，以及在之前版本数据库基础上新增的功能和做出的改动。
...
* 修复备份介质不可用时，日志归档导致副本迁移失败问题。
* 修复新建分区归档落后导致迁移复制卡住问题。
* 修复事务提交和切主并发场景下的事务卡住问题。
* 修复停止备份后，无效的 `piece_id` 导致日志卡在 Stopping 状态。
* 修复非 Mini 模式下 lock-wait-mgr 休眠时间过长问题。
* 修复 `insert into ignore` 执行卡住问题。
* 修复 Varbinary 类型校验字符集导致插入报错问题。
...
修复租户数超过 32 个时 OBServer 节点重启失败的问题。
