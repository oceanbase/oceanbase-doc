...
**系统资源调优**：该版本专项优化了公有云系统资源使用，包括用户租户内存、500 租户内存、操作系统内存等，典型的 128G 规格及以上的 ECS，系统资源使用的内存比例从 30% 降到 15% 左右。
...
**数据完整性保障**：信息系统商用密码应用安全性评估（简称密评）中对存储有数据的机密性和完整性保护要求。V4.2.1 之前版本，OceanBase 数据库透明加密已支持 `AES_ECD`、`SM4_CBC`（加密算法_工作模式），对存储数据提供了机密性保护。V4.2.1 版本扩展支持了 GCM 模式。在对数据加密的基础上，结合密钥计算密文的校验和，在解密时对密文做校验，判断密文是否发生变化，以支持数据的完整性保护。
