...
* 修复执行内部 SQL 返回 4018 触发 DDL 提前返回结果给客户端，实际未执行完成的问题。
* 修复某个租户显式设置 `_optimizer_better_inlist_costing=true`，重启后有可能导致节点 Core dump 或有其他异常的问题。
* 修复集群某个租户 Table History 过多，机器重启的时候 Schema 刷新持续报错 4013 的问题。
* 修复 PL 中 SubQuery 结果为 `varchar` 类型，COLLATION 与期待值不一致时未做类型转换，导致数据合并报错 4016 的问题。
* 修复 .NET 驱动查询 `select param_list, returns from mysql.proc` 时，驱动端报错 `Unable to cast object of type system.strings to system.Byte` 的问题。
* 修复在对 Number 类型进行格式化时，当缓冲区满时报错不正确，导致系统未能自动扩充缓冲区的问题。
* 修复带系统变量的过滤条件不能准确抽取 Query Range 时，导致执行计划变慢，性能降低的问题。
* 修复三阶段 Aggregation 在关闭自适应的情况下不能正常落盘，导致 SQL 报错 4013 的问题。
* 修复 Oracle 模式下，HAVING 关键字在 GROUP BY 关键字之前执行时，快速语法解析和普通语法解析参数化顺序不一致，导致 SQL 执行可能存在报错的问题。
* 修复 Oracle 租户下，跨用户访问 PL 时，仅检查了调用者权限，导致在 `USER_TAB_COLUMNS` 视图查询不到表数据的问题。
* 修复 Oracle 模式下，带有 UDF 的 SELECT 语句并行执行未生效的问题。
* 修复在使用 MySQL Connector 连接数据库时，当在 .NET 平台下执行 `change user` 后，通过 PX 获取系统变量时报错 `Object type error` 的问题。
* 修复 Oracle 模式下，CTE 临时表过大导致内存占用过高，报错 4013 内存不足的问题。
* 修复 Oracle 模式下，函数表达式作为字段值未能使用索引，而是进行了全表扫描，性能较差的问题。
* 修复 SQL 中包含大量 OR 条件，或经过改写后产生大量 OR 条件的情况下，可能造成 SQL 执行报错 `4019 size overflow` 的问题。
* 修复 OBServer 解析 `change user` 包时误读 `conn attr`, 导致连接断开，并不断重试最终导致请求队列满的问题。
* 修复使用 `WM_CONCAT/LISTAGG` 函数时，若触发了 `remove order by` 改写，会导致 `WM_CONCAT/LISTAGG` 的结果顺序与 Oracle 数据库不一致的问题。
* 修复 `OUTER JOIN` 包含 OR 条件，且连接表为内联视图时，若视图中 WHERE 条件使 SELECT 输出满足 UNIQUE 且 SELECT 中仅有 Lob 输出列，SQL 可能报错4016 的问题。
* 修复 Oracle 模式下，PL 中 FORALL 包含的 UPDATE 语句执行报错 `PLS-00435: DML statement without BULK In-BIND cannot be used inside FORALL` 的问题。
* 修复内存不足时，SQL 执行可能报错 6210 的问题。
* 修复 Oracle 模式下，自定义函数嵌套执行报错 4016 的问题。
...
* **稳定性增强**
   ...
   优化大事务处理能力、减少依赖 NTP 服务、扩展支持分区数量上限，扩展支持 LOB 存储规格上限。
...
* 减少依赖 NTP 服务。
   ...
   基于全新的自动选主协议，降低了对 NTP 时钟的依赖，打破原来早期版本对所有节点的时钟偏差控制在 100 ms 以内的强需求。OceanBase 数据库自 V4.1.0 版本开始允许的时钟偏差可以达到 2s，同时支持动态修改时钟，修改的间隔在 2s 以内时不会对数据的正确性和集群的稳定运行带来影响。
