...
### 步骤一：下载并安装 all-in-one package

1. 下载 all-in-one package 安装包，并将其上传到机器任一目录下。
...
8. 使用 OBClient 客户端连接 OceanBase 数据库，或者登录 OCP Express 白屏界面管理集群。
   ...
   * 通过 2881 端口直连数据库，以直连 10.10.10.1 节点为例
     ...
     ```shell
     obclient -h10.10.10.1 -P2881 -uroot@sys -Doceanbase -A
     ```
  
   * 通过 ODP 代理访问数据库，以 ODP 所在节点为 10.10.10.1 为例
  
     ```shell
     obclient -h10.10.10.1 -P2883 -uroot@sys -Doceanbase -A --proxy-mode
     ```
