...
从 V4.0.0 开始，OceanBase 提供统一的安装包 all-in-one package。您可以通过这个统一的安装包一次性完成 OBD、OceanBase 数据库、 OBProxy、obagent、Grafana、ocp-express 和 Prometheus 的安装；您可以根据实际需求选择部分组件安装或者指定组件的版本。

#### 离线安装 all-in-one package

1. 从 [OceanBase 下载中心](https://www.oceanbase.com/softwarecenter) 下载最新的企业版 all-in-one package 安装包。
...
`obd demo` 部署方式采用最小规格部署，部署的组件版本默认为最新版本，默认部署的组件包含 oceanbase、obproxy、obagent、grafana、prometheus。
