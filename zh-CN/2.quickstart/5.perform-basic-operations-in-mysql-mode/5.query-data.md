# 查询数据
#docslug#/oceanbase-database/oceanbase-database/V4.0.0/query-data
使用 `SELECT` 语句查询表中的内容。
...
3. 从表 `a` 中根据筛选条件 `name = 'a'` ，输出对应的 `id` 、`name` 和 `num`。
   ...
   更多 `SELECT` 语句相关的语法说明请参见《SQL 参考（MySQL 模式）》中 [SELECT](../../4.development-guide-refactoring-1/5.sql-syntax/2.common-tenant-mysql-mode/6.sql-statement/51.SELECT-1-2/1.SELECT.md)章节。
