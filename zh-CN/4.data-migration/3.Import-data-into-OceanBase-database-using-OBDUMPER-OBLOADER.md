# 使用 OBDUMPER / OBLOADER 导入 / 导出数据到 OceanBase 数据库
...
```bash
[root@obce00 ~]# mysql -h 172.20.249.52 -P 2883 -u proxyro@sys#****-3zones -puY7Yf8zx -c -A
...
```sql
mysql -h 172.20.249.52 -u root@pro**** -P 2883 -p0MdTv1tm
...
```bash
[root@obce00 ~]# mysql -h 172.20.249.52 -P 2883 -u proxyro@sys#****-3zones -puY7Yf8zx -c -A -Ns -e "show databases;"
oceanbase
information_schema
[root@obce00 ~]#
```
...
跟上面一样，导入的时候 OBLOADER 需要获取表结构元数据信息，也需要连接到 sys 租户下。推荐用用户 `proxyro` 。
...
导入大量数据时，可以开启并发，`--threads` 默认会根据 CPU 动态调整，也可以手动指定，以控制导入对主机性能的影响。
...
更多使用问题请参考官方 [产品文档](https://open.oceanbase.com/docs/community/oceanbase-database/V3.1.2/what-is-oceanbase)。
