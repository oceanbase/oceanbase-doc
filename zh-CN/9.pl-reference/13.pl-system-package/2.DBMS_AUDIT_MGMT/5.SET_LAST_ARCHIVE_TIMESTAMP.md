...
| rac_instance_number | 指定实例编号。默认值为 `NULL`。 当 `audit_trail_type` 为如下值时，不接受 `rac_instance_number` 参数： * `audit_trail_all`   * `audit_trail_aud_std`   * `audit_trail_db_std`    对于单实例数据库，将 `rac_instance_number` 设置为 1。 |
