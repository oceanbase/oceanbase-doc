...
OceanBase 数据库提供了 Database 级均衡能力，支持同一 Database 下的所有用户表自动聚合以及多个 Database 下的用户表聚合。

此外，OceanBase 数据库还支持了 Sharding 属性为 `NONE` 的表组的权重均衡。通过表组权重均衡，可以帮助自动聚合的 Database 按权重分布。
