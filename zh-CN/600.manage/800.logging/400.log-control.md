...
|配置项名|默认值|配置项介绍|
|---|---|---|
|`enable_syslog_recycle`|False|用于设置是否打开记录 OBServer 节点启动前的旧日志的开关。与 `max_syslog_file_count` 配合使用，决定回收逻辑是否考虑旧的日志文件。</li><li>False：关闭。</li></ul>|
...
|`syslog_level`|WDIAG|用于设置系统日志级别。</br>取值包括  `DEBUG`、`TRACE`、`WDIAG`、`EDIAG`、`INFO`、`WARN`、`ERROR`。|
