...
系统视角的 OBServer 进程的内存使用统计，每 10s 打印一次。
...
* **关键字段说明**
  ...
  | -------------- | ---------------------------------------------------- |
  | tenant_id      | 租户 ID                                              |
  | max_disk_size  | 租户的磁盘上限（4.x 版本前租户没有磁盘使用量的限制） |
...
* **关键字段说明**
  ...
  | recv_hp_rpc_cnt <br> recv_np_rpc_cnt <br> recv_lp_rpc_cnt | 租户累计收到不同级别的 rpc 请求数，hp( High ), np( Normal ), lp( Low )。                            |
  ...
  | req_queue                                                 | total_size：当前处于等待队列中的请求总数                                                            |
...
* **日志定位命令**
  ...
  * **关键字段说明**
      ...
      | <ul><li>user_read_io_stat</li><li>user_write_io_stat</li><li>sys_read_io_stat sys_write_io_stat</li><li>large_query_read_io_stat</li><li>large_query_write_io_stat</li><ul>               | 这些部分包含了不同类型的 I/O 的统计信息，包括平均大小 `average_size`，平均响应时间 `average_rt_us`，以及旧的和新的统计数据（包括 I/O 操作数 `io_cnt`，I/O 字节数 `io_bytes` 和 I/O 响应时间 `io_rt_us`）。                                                                                                                                                                 |
...
常用归档线程统计有日志归档状态统计和 clog split 线程统计，两种统计均每 10s 打印一次。
  ...
  * **搜索关键字：** `archive_sender statistic`
      ...
      **日志说明：** 单个 clog send 线程的统计，每10s 打印一次。
...
* ****
  ...
  **关键字段说明**
  ...
  | 日志信息               | 说明                                                   |
  | ---------------------- | ------------------------------------------------------ |
  | priority               | 后台任务的优先级，OceanBase 会优先调度优先级高的任务。 |
...
* ****
  ...
  **关键字段说明**
  ...
  | 日志信息                 | 说明                                                                                   |
  | ------------------------ | -------------------------------------------------------------------------------------- |
  | sys_bkgd_net_percentage_ | 系统配置项 sys_bkgd_net_percentage 的值，是后台线程的网络带宽占比，默认是网卡带宽的60% |
...
* ****
  ...
  **关键字段说明**
  ...
  | ------------- | ------------------------------------- |
  | ret           | 错误码                                |
  | partition_key | 相关的object（table_id+partition_id） |
