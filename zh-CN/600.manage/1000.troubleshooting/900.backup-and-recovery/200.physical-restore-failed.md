...
2. 执行以下命令，查询恢复任务的具体状态。
   ...
   根据查询结果，如果 `status` 的值为 `RESTORE_WAIT_LS` ，说明租户的恢复状态一直卡在 `RESTORE_WAIT_LS` 状态，需要执行下一步，继续确认待恢复租户的日志流副本的恢复状态。
...
4. 如果查询到的日志流副本的 `restore_status` 值为 `6` 或者 `8`，由于处于 `6`或 `8` 状态的日志流主要进行日志和转储的恢复，该阶段优先考虑是日志的恢复问题。
     ...
     1. 根据上一步中查询到的机器信息，登录到日志流副本所在的机器。
     ...
     4. 找到日志流副本 `restore_status` 值为 `6` 或 `8` 的日志流的 Leader。
...
执行 `ALTER SYSTEM RESTORE` 语句发起物理恢复后，排查虚拟表，发现租户的恢复状态不处于 `RESTORE_WAIT_LS` 而是卡在其他状态，并且仅系统租户存在残留时，只有可能是租户的 Schema 未刷新出来，导致 create_tenant_end 失败。
