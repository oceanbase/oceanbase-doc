...
3. 根据上一步获取到的 `RS_SVR_IP`，登录 Root Service 所在的机器，搜索 `rootservice.log` 日志，确认报错点。
   ...
   3. 执行以下命令，搜索 `ret=\result` 的日志，找到报错点。
      ...
      <h4>注意</h4>
      <p>如果在 <code>rootservice.log</code> 中 <code>grep</code> 不到相关日志，可能是因为 <code>rootservice.log</code> 切了文件，可以执行 <code>grep "ret=\-9040" rootservice.log.* | grep "ob_backup_data_scheduler"</code>。</p>
      </main>
      ...
      * 租户状态不处于 Normal
      ...
      对于其他情况导致的数据备份任务发起失败，请联系技术支持人员协助处理。
