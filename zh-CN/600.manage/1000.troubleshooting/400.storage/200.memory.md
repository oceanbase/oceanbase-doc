...
* 模块内存超限。
  ...
  内存模块超限的判断标准是：`MODULE_SUM` > 租户 min_memory - 租户 MemStore。模块内存超限，可能需要先调整单独模块的内存，例如，修改 [ob_sql_work_area_percentage](../../../700.reference/800.configuration-items-and-system-variables/200.system-variable/300.global-system-variable/9100.ob_sql_work_area_percentage-global.md) 参数来调整工作区内存。如果租户内存过小，也需要加租户内存。
