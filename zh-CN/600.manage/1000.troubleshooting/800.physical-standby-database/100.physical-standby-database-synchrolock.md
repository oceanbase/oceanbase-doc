...
通常情况下，通过物理恢复的方式恢复出备租户后，备租户的日志同步就会停留在指定的恢复终点。如果执行恢复命令时未指定恢复终点，则系统会选择发起恢复命令时的最大归档进度作为恢复终点。

例如，如下所示，租户 `restore_oracle_tenant` 的租户角色为 `STANDBY`，其同步进度卡在了某个固定时间点。造成该现象的原因是用户未执行 RECOVER 命令，其 `SYNC_SCN` 等于可恢复位点 `RECOVERY_UNTIL_SCN`，备租户无需再同步更多日志。
...
其中，租户的同步位点可通过`DBA_OB_TENANTS` 视图中的 `SYNC_SCN` 字段获取；所有日志流的同步位点可通过 `V$OB_LS_LOG_RESTORE_STATUS` 视图中的 `SYNC_SCN` 字段获取。
...
2. 根据租户的 `TENANT_ID` 或 `TENANT_NAME` 查询问题租户的基本状态信息。
...
2. 确认问题租户是否设置了日志恢复源。
   ...
   * `VALUE`：其值与 `TYPE` 的值对应，分别为源端租户的归档目录以及主租户的 Access Point。
   ...
   有关 `CDB_OB_LOG_RESTORE_SOURCE` 和 DBA_OB_LOG_RESTORE_SOURCE 视图的更多说明，请参见 [CDB_OB_LOG_RESTORE_SOURCE]() 和 [DBA_OB_LOG_RESTORE_SOURCE](../../../)。

### 步骤三：通过 V$OB_LS_LOG_RESTORE_STATUS 查看恢复状态
...
2. 查看问题租户的日志同步状态。
   ...
   * 问题租户查看本租户的日志恢复源
     ...
     查询结果中，重点关注 `SYNC_STATUS` 字段，该字段表示日志流的恢复状态，如果值为 `NORMAL` 则表示日志在正常同步中，其他状态则为异常，日志同步状态及其表示的含义如下表所示。

     有关 `V$OB_LS_LOG_RESTORE_STATUS` 视图的更多说明，请参见 [V$OB_LS_LOG_RESTORE_STATUS](../../../)。

     | 日志同步状态            | 含义                                                                                  |
     |------------------------|--------------------------------------------------------------------------------------|
     | NORMAL                 | 正常同步中                                                                            |
     | SOURCE HAS A GAP       | 备库与日志源端日志有gap，主库日志未同步到备库已回收                                      |
     | STANDBY LOG NOT MATCH  | 恢复日志与备库冲突，出现双 PRIMARY 或者主备库关系                                       |
     | CHECK USER OR PASSWORD | 复制账户用户名或密码不对，无法访问原主库                                                |
     | CHECK NETWORK          | 主库不可达，检查网络是否异常                                                           |
     | RESTORE SUSPEND        | 备库已经恢复到指定位点                                                                |
     | STANDBY NEED UPGRADE   | 备库 binary 版本落后，需要升级                                                        |
     | PRIMARY TENANT DROPPED | 主库已经被删除                                                                       |
     | FETCH LOG TIMEOUT      | 同步日志超时，需要排查网络健康状况或者调大备库日志同步超时的租户级配置项 `standby_db_fetch_log_rpc_timeout`   |
     | STANDBY IN THROTTLING  | 备库写入限速中                                                                       |
     | STANDBY LOG DISK IS FULL | 备库磁盘空间不足                                                                   |
     | WAIT LOG STREAM CREATED  | 等待备库创建日志流完成                                                              |
     | NOT AVAILABLE          | 其他异常导致日志同步不可用                                                            |
