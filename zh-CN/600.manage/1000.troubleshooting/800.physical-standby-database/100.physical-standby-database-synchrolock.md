...
2. 根据租户的 `TENANT_ID` 或 `TENANT_NAME` 查询问题租户的基本状态信息，包括租户角色、同步位点等信息。
   ...
   * `RECOVERY_UNTIL_SCN`：表示租户可恢复到的终点，如果值为 `4611686018427387903`，则表示恢复到无穷。
   ...
   有关视图 `DBA_OB_TENANTS` 的更多说明，请参见 [DBA_OB_TENANTS](../../../700.reference/700.system-views/400.system-view-of-mysql-mode/200.dictionary-view-of-mysql-mode/12000.o-dba_ob_tenants-of-mysql-mode.md)。
...
在 OceanBase 数据库中，备租户通过日志恢复源来拉取上游的日志。通过物理备份恢复（带日志）功能创建的备租户，用户在执行 `RECOVER` 命令后，默认会继续从恢复时指定的归档目录继续恢复日志；通过 `CREATE STANDBY TENANT` 命令创建的备租户，默认会通过网络直接从主租户处同步日志；而通过 `BACKUP DATABASE PLUS ARCHIVELOG` 命令创建的备租户未指定日志恢复源，无法直接开启日志持续同步。
...
2. 确认问题租户是否设置了日志恢复源。
   ...
   *************************** 1. row ***************************
   ...
   有关视图 `CDB_OB_LOG_RESTORE_SOURCE` 和视图 `DBA_OB_LOG_RESTORE_SOURCE` 的更多说明，请参见 [CDB_OB_LOG_RESTORE_SOURCE](../../../700.reference/700.system-views/300.system-view-of-sys-tenant/200.dictionary-view-of-sys-tenant/9000.o-cdb_ob_log_restore_source-of-sys-tenant.md) 和 [DBA_OB_LOG_RESTORE_SOURCE](../../../700.reference/700.system-views/500.system-view-of-oracle-mode/200.dictionary-view-of-oracle-mode/12500.dba_ob_log_restore_source-of-oracle-mode.md)。
...
2. 查看问题租户所有日志流 Leader 的同步状态。
   ...
   * 问题租户查看本租户
     ...
     * `SYNC_STATUS`：表示该日志流同步状态，其中，`NORMAL` 表示该日志流的日志同步状态正常，如果为其他状态则表示异常。日志同步状态及其含义如下表所示。
     ...
     有关视图 `V$OB_LS_LOG_RESTORE_STATUS` 的更多说明，请参见 [V$OB_LS_LOG_RESTORE_STATUS](../../../700.reference/700.system-views/400.system-view-of-mysql-mode/300.performance-view-of-mysql-mode/32200.v-ob_ls_log_restore_status-of-mysql-mode.md)。
...
2. 通过视图 `GV$OB_LOG_STAT` 查询租户的所有日志流的状态信息。
   ...
   * 问题租户查看本租户
   ...
   有关视图 `GV$OB_LOG_STAT` 的更多说明，请参见 [GV$OB_LOG_STAT](../../../700.reference/700.system-views/400.system-view-of-mysql-mode/300.performance-view-of-mysql-mode/1900.gv-ob_log_stat-of-mysql-mode.md)。
