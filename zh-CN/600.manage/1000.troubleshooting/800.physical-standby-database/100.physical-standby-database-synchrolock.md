...
2. 查看问题租户所有日志流 Leader 的同步状态。
   ...
   * 问题租户查看本租户
     ...
     * `SYNC_STATUS`：表示该日志流同步状态，其中，`NORMAL` 表示该日志流的日志同步状态正常，如果为其他状态则表示异常。日志同步状态及其含义如下表所示。
     ...
     有关视图 `V$OB_LS_LOG_RESTORE_STATUS` 的更多说明，请参见 [V$OB_LS_LOG_RESTORE_STATUS](../../../700.reference/700.system-views/400.system-view-of-mysql-mode/300.performance-view-of-mysql-mode/31600.v-ob_ls_log_restore_status-of-mysql-mode.md)。
