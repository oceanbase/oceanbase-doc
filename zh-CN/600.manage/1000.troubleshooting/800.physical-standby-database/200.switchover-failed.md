...
执行备切主操作时，要求当前租户不能 Unit 在临时下线的机器上。如果租户有 Unit 在临时下线的机器上，就会出现以上报错，需要等机器永久下线，并且保证租户没有副本在永久下线的机器上后，才能执行备切主操作。
...
4. 对于机器临时下线的场景，可以参考以下操作进行处理。
   ...
   1. 手动启动 observer 进程后，重新执行备切主操作。

      手动启动 observer 进程后，如果备切主操作仍然无法执行成功，则需要执行下一步。

   2. 调小配置项 `server_permanent_offline_time` 的值，使临时下线机器满足永久下线条件，然后重新执行备切主操作。

      修改配置项 `server_permanent_offline_time` 的值时，不可设置过小，推荐至少要在 20s 以上。
   ...
   3. 确认永久下线的机器上是否发起了移除副本的任务。
      ...
      3. 确认 `NOW()` 比 `LAST_OFFLINE_TIME + server_permanent_offline_time` 大至少两倍的 `balancer_idle_time` 时间后，执行以下 SQL，确认是否已经发起了移除副本任务。

         ```sql
         SELECT * FROM oceanbase.CDB_OB_LS_REPLICA_TASKS WHERE TENANT_ID = user_tenant_id;
         ```

         其中，`user_tenant_id` 需要替换为目标租户的 `TENANT_ID`。

         如果没有发起或者任务一直没有完成，请联系技术支持人员协助处理。

   4. 如果仍有其他问题，请联系技术支持人员协助处理。
