...
2. 查询租户内永久下线的机器的信息。
    ...
    1. 查询下线机器的 IP 信息及 `LAST_OFFLINE_TIME`。
        ...
        ```sql
        obclient [oceanbase]> SELECT SVR_IP, SVR_PORT, LAST_OFFLINE_TIME, NOW() FROM oceanbase.DBA_OB_SERVERS WHERE LAST_OFFLINE_TIME IS NOT NULL;
        ```
...
2. 确认该租户下日志流的 `ACCESS_MODE`。
    ...
    根据查询结果，如果有日志流的 `ACCESS_MODE` 不为 `RAW_WRITE`，可能是有些日志流正在等待被垃圾回收，需要等待 20s 后再重新执行 Switchover 操作将备租户切换为主租户。如果重新执行后仍然不成功，请联系技术支持人员协助处理。

## 报错信息 11 

`ERROR HY000: query primary failed(original error code: -4152), switchover to primary is not allowed`

<main id="notice" type='notice'>
<h4>注意</h4>
<p>报错信息 <code>original error code: -4152</code> 中的 <code> -4152</code>，仅代表其中一种可能出现的错误码，该报错还有可能显示为其它错误码。</p>
</main>

### 报错原因

基于网络的物理备库场景下，如果需要执行 Switchover 操作将备租户切换为主租户，要求当前备租户拥有日志恢复源对应租户中部分系统视图的查询权限。如果不满足该条件，就可能会出现以上报错。

### 处理方法

1. 使用设置恢复源时指定的专用用户登录当前备租户。

2. 查看当前用户所拥有的权限，确认当前租户是否拥有日志恢复源租户中部分系统视图的查询权限。
...
3. 完成授权后，重新执行 Switchover 操作将备租户切换为主租户。如果重新执行后仍然不成功，请联系技术支持人员协助处理。
...
基于网络的物理备库场景下，如果需要执行 Switchover 操作将备租户切换为主租户，要求日志恢复源对应的租户不能被锁定。如果日志恢复源对应的租户被锁定，就可能会出现以上报错。
...
3. 执行以下命令，解锁租户。
    ...
    其中，`tenant_name` 需要替换为日志恢复源对应租户的租户名。

4. 完成租户解锁操作后，重新执行 Switchover 操作将备租户切换为主租户。如果重新执行后仍然不成功，请联系技术支持人员协助处理。
