...
<h4>说明</h4>
<p>对于基于日志归档的物理备库场景，通常不会出现以上报错，如果出现了以上报错，建议您也可以参考以下操作登录归档源所在的集群进行确认。</p>
</main>
...
执行备切主操作时，要求当前租户的 Unit 不能在临时下线的机器上。如果租户有 Unit 在临时下线的机器上，就会出现以上报错，需要等机器永久下线，并且保证租户没有副本在永久下线的机器上后，才能执行备切主操作。
...
4. 对于机器临时下线的场景，可以参考以下操作进行处理。
   ...
   2. 调小配置项 `server_permanent_offline_time` 的值，使临时下线机器满足永久下线条件，然后重新执行备切主操作。
      ...
      重新执行备切主操作后，如果执行不成功并且提示有副本在永久下线的机器上，则需要执行下一步。

   3. 确认永久下线的机器上是否发起了移除副本的任务。
      ...
      2. 等待一段时间后，执行以下命令，再次获取该永久下线机器的 `LAST_OFFLINE_TIME`。
         ...
         根据查询结果，当 `NOW()` 比 `LAST_OFFLINE_TIME + server_permanent_offline_time` 大至少两倍的 `balancer_idle_time` 时间后，执行下一步。

      3. 查询视图 `CDB_OB_LS_LOCATIONS`，观察 `SVR_IP` 和 `SVR_PORT` 字段，确认日志流 `LEADER` 的 `MEMBER_LIST` 中是否还有永久下线的机器。

         ```sql
         SELECT * FROM oceanbase.CDB_OB_LS_LOCATIONS WHERE TENANT_ID = user_tenant_id AND SVR_IP = 'permanent_offline_server_ip' AND SVR_PORT = permanent_offline_server_port UNION SELECT * FROM oceanbase.CDB_OB_LS_LOCATIONS WHERE TENANT_ID = user_tenant_id AND ROLE = 'LEADER' AND MEMBER_LIST LIKE "%permanent_offline_server_ip:permanent_offline_server_port%";
         ```
         ...
         * `permanent_offline_server_port` 需要替换为永久下线机器的 RPC 端口号。
         ...
         如果查询结果中，日志流 `LEADER` 的 `MEMBER_LIST` 中没有永久下线的机器，请联系技术支持人员协助解决。
         
         如果查询结果中，日志流 `LEADER` 的 `MEMBER_LIST` 中有永久下线的机器，则需要执行下一步。
      ...
      4. 确认是否已经发起了移除副本任务。
         ...
         在查询结果中，查看最近的时间内是否有 `start_remove_ls_paxos_replica` 和 `finish_remove_ls_paxos_replica` 的事件：
         
         * 如果 `start_remove_ls_paxos_replica` 和 `finish_remove_ls_paxos_replica` 事件都没有，则表示还没有发起移除副本的任务，可以等待一段时间后，再确认。如果一直未发起移除副本的任务，请联系技术支持人员协助处理。

         * 如果只有 `start_remove_ls_paxos_replica` 事件，则表示移除副本的任务一直没有完成，请联系技术支持人员协助处理。

         * 如果有 `start_remove_ls_paxos_replica` 和 `finish_remove_ls_paxos_replica` 事件，则表示已经发起并完成了副本移除任务，重新执行备切主操作即可。

    4. 如果仍然有其他问题，请联系技术支持人员协助处理。
