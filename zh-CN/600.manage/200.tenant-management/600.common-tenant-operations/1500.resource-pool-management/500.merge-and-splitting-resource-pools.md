...
为了便于管理，您可以将租户内相同资源单元的多个资源池合并为一个资源池。
...
假设当前租户有资源池 `pool0` 的当前使用范围是 `z1`，对应的资源单元为 `uc0`；资源池 `pool1` 的当前使用范围是 `z2`，对应的资源单元为 `uc0`；资源池 `pool2` 的当前使用范围是 `z3`，对应的资源单元为 `uc0`。

由于 `pool0`、`pool1`、`pool2` 这 3 个资源池使用了相同的资源单元，可以将多个资源池合并为一个资源池，降低资源池的管理和运维成本。
...
资源池 `pool1` 的当前使用范围是 `z1`、`z2`、`z3`，且其对应的资源单元为 `uc0`，由于 `z1`、`z2`、`z3` 等 3 个 Zone 上的物理机规格可能有较大差别，3 个 Zone 内如果使用同一个资源规格，无法充分利用每个 Zone 内物理机的资源。分裂资源池可以将一个多 Zone 资源池分裂为多个单 Zone 资源池，再为每个单 Zone 资源池配置各自的资源配置规格。
...
2. 将资源池 `pool1` 分裂为 `pool10`、`pool11` 和 `pool12`，并为新的资源池指定新的资源单元。
   ...
   分裂完成后，分裂出来的资源池的默认资源规格仍然为原资源规格，您可以根据各 Zone 的资源使用情况自行调整各新资源池的资源规格。
