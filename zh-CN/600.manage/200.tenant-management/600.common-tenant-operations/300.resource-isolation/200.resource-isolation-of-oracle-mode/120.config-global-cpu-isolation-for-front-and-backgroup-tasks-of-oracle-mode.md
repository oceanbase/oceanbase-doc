...
* 对于 V4.2.1 版本，从 V4.2.1 BP7 版本开始支持全局 CPU 资源的前后台隔离功能。
...
* `enable_global_background_resource_isolation`
  ...
  有关配置项 `enable_global_background_resource_isolation` 的更多说明，参见 [enable_global_background_resource_isolation](../../../../../700.reference/800.configuration-items-and-system-variables/100.system-configuration-items/300.cluster-level-configuration-items/6850.enable_global_background_resource_isolation.md)。
  
* `global_background_cpu_quota`

  集群级配置项 `global_background_cpu_quota` 用于控制后台任务可使用的 CPU 配额。默认值为 `-1`，表示后台任务可使用的 CPU 资源不受 cgroup 限制。

  有关配置项 `global_background_cpu_quota` 的更多说明，参见 [global_background_cpu_quota](../../../../../700.reference/800.configuration-items-and-system-variables/100.system-configuration-items/300.cluster-level-configuration-items/10150.global_background_cpu_quota.md)。
...
通过开启全局 CPU 资源的前后台隔离功能可以达到在全局层面上限制后台任务的目的，如果还希望对后台任务做进一步精细的隔离调控，则需要通过 `DBMS_RESOURCE_MANAGER` 系统包来配置租户内的资源隔离。使用`DBMS_RESOURCE_MANAGER` 系统包配置租户内资源隔离的详细操作，参见 [配置租户内资源隔离](200.resource-isolation-at-user-level-of-oracle-mode.md)。
