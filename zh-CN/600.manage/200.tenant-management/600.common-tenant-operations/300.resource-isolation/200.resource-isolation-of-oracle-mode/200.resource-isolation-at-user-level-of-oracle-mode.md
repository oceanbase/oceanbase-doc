...
完成磁盘校准后，在配置资源隔离计划前，需要保证租户的资源规格中 `MAX_IOPS` 和 `MIN_IOPS` 的值为有效值，即是否为 16 KB 读对应的 IOPS 值。
...
4. 执行以下命令，确认待进行资源隔离的租户所在的 OBServer 节点上的磁盘校准值，使用 16 KB 读的磁盘校准值作为该节点 IOPS 设置的上限值。
   ...
   根据查询结果，将获取到的各节点的磁盘校准值作为上限值来规划租户可用的 IOPS。由于集群内可能有多个租户使用了相同的 OBServer 节点，您可以需要根据业务实际情况来分配这些 IOPS。
