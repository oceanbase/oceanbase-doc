...
使用 `DBMS_RESOURCE_MANAGER` 系统包配置租户内的 Function 级资源隔离时，如果开启了全局 CPU 资源的前后台隔离功能，则租户下后台任务的 CPU 使用上限会受到配置项 `global_background_cpu_quota` 值的限制，即租户下后台任务的 CPU 使用上限实际为 `min(global_background_cpu_quota, 租户的 MAX_CPU * 资源组内的 UTILIZATION_LIMIT)`。
...
4. 调用 `DBMS_RESOURCE_MANAGER` 系统包中的 `CREATE_PLAN_DIRECTIVE` 子程序，创建资源管理计划对应的资源管理计划内容，用于启用资源管理计划时，限制资源组所使用的 CPU 资源和 IOPS 资源。
   ...
   * 指定资源计划为 `plan_a`，绑定资源组 `big_group`，并指定系统满负载情况下，可分配给本资源组的 CPU 资源为 90%，可使用的 CPU 资源上限为租户 CPU 总资源的 60%。同时，当出现出现 IO 争用时，指定最少可用的 IOPS 资源为 20%，可使用的 IOPS 资源上限为 IOPS 总资源的 100%，IOPS 资源权重为 20。
   ...
   * 指定资源计划为 `plan_a`，绑定资源组 `small_group`，并指定系统满负载情况下，可分配给本资源组的 CPU 资源为 100%，可使用的 CPU 资源上限为租户 CPU 总资源的 40%。同时，当出现出现 IO 争用时，指定最少可用的 IOPS 资源为 10%，最大可使用的 IOPS 资源上限为 IOPS 总资源的 90%，IOPS 资源权重为 30。
