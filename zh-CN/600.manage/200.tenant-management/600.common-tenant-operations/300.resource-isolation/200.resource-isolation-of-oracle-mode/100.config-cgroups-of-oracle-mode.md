...
2. 手动建立 OceanBase 数据库软件的安装目录与 cgroup 系统目录的软链接。
   ...
   执行成功后，结果如下所示。不需要重启 observer 进程，软链接即可建立成功。
   ...
   软链接建立成功后，用户可以在 OceanBase 数据库软件的安装目录 `/home/admin/oceanbase` 下看到 cgroup 的目录结构。

### 步骤三：开启 cgroup 功能

OceanBase 数据库通过集群级配置项 `enable_cgroup` 来控制 OBServer 是否开启 cgroup 功能，其默认值为 `True`，表示开启 cgroup 功能。在上一步骤 OceanBase 数据库软件的安装目录与 cgroup 系统目录建立软链接后，如果 `enable_cgroup` 的值为 True，则 cgroup 功能已开启并生效。

如果未开启 cgroup 功能，您可以参考以下操作，开启 cgroup 功能。开启后，不需要重启集群即可生效。
