...
### （可选）步骤五：开启前后台任务 CPU 资源的全局隔离

为了进一步实现更好的隔离效果，OceanBase 数据库支持了对前后台任务 CPU 资源的全局隔离。该功能可以在全局层面上对所有租户的前台和后台任务进行 CPU 资源隔离，适用于租户数较多，并且不希望后台任务对前台业务造成严重 CPU 竞争的场景。

OceanBase 数据库通过以下两个配置项来实现前后台任务 CPU 资源的全局隔离：
...
建议使用 cgroups 中的目录文件调用 `DBMS_RESOURCE_MANAGER` 系统包中的子程序 `CREATE_CONSUMER_GROUP` 创建的资源组，来进行用户级或 SQL 级的资源隔离。配置资源隔离的详细操作，请参见 [配置资源隔离](200.resource-isolation-at-user-level-of-oracle-mode.md)。
