...
* 要使用 cgroup 功能，要求 Linux 内核版本必须为 Linux Kernel 5.10 及以上版本。
...
OceanBase 数据库通过集群级配置项 `enable_cgroup` 来控制 OBServer 是否开启 cgroup 功能，该配置项的默认值为 `True`，表示已开启。如果 cgroup 功能已开启，请跳过本步骤。
...
建立软链接并且开启 cgroup 功能后，需要重启 OBServer 节点，功能才会生效（如果有多台机器，则每台机器都需要重启一次），重启 OBServer 节点的详细操作，参见 [重启节点](../../../../100.cluster-management/300.common-cluster-operations/300.restart-a-node.md)。

observer 进程在启动时，检测到之前已经建立的软链接，就会在 `/sys/fs/cgroup/cpu/oceanbase/` 目录下继续创建 OceanBase 数据库 cgroup 目录。

重启成功后，可以在 OceanBase 数据库软件的安装目录 `/home/admin/oceanbase` 下看到 cgroup 的目录结构。用户可以登录集群的 `sys` 租户，查询视图 `V$OB_CGROUP_CONFIG` 或 `GV$OB_CGROUP_CONFIG`，查看各 OBServer 节点下的 cgroup 配置。
