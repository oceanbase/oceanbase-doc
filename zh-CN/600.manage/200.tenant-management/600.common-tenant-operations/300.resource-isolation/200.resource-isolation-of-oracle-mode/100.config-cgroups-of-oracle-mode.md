...
在开始配置 cgroup 前，建议先了解 cgroup 的概念，有关 cgroup 的介绍信息，参见 [资源隔离概述](../100.resource-isolation-overview.md)。
...
* 开启 cgroup 会导致 OceanBase 数据库的性能有所下降，需要权衡隔离性和性能后再确认是否开启。
...
cgroup 系统目录配置成功后，即可部署 OceanBase 数据库。 OceanBase 数据库的详细安装操作，参见 [部署数据库](../../../../../400.deploy/300.deploy-oceanbase-enterprise-edition/400.deploy-through-the-command-line/200.deploy-the-oceanbase-cluster-command-line/100.stand-alone-deployment-of-oceanbase-database.md)。
...
3. 重新启动 observer 进程。
   ...
   重启 observer 进程的具体操作，参见 [重启节点](../../../../100.cluster-management/300.common-cluster-operations/300.restart-a-node.md)。
...
OceanBase 数据库通过集群级配置项 `enable_cgroup` 来控制 OBServer 是否开启 cgroup 功能。默认该功能为 `True`，表示已开启，如果未开启，可以参考以下操作，开启 cgroup 功能。
...
成功配置 cgroups 系统目录并开启 cgroup 功能后，在应急情况下，可以通过各租户所在目录下的 `cpu.cfs_period_us`、`cpu.cfs_quota_us`、`cpu.shares` 等文件来控制租户内 CPU 资源的占用。一般不建议使用该方式进行资源隔离。

建议使用 cgroups 中的目录文件调用 `DBMS_RESOURCE_MANAGER` 系统包中的子程序 `CREATE_CONSUMER_GROUP` 创建的资源组，来进行用户级或 SQL 级的资源隔离。配置资源隔离的详细操作，参见 [配置租户内资源隔离](200.resource-isolation-at-user-level-of-oracle-mode.md)。
