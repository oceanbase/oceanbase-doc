...
OceanBase 数据库通过集群级配置项 `enable_cgroup` 来控制 OBServer 是否开启 cgroup 功能，其默认值为 `True`，表示开启 cgroup 功能。建立软链接并开启 cgroup 功能后，不需要重启 OBServer 节点，用户就可以在 OceanBase 数据库软件的安装目录 `/home/admin/oceanbase` 下看到 cgroup 的目录结构。
