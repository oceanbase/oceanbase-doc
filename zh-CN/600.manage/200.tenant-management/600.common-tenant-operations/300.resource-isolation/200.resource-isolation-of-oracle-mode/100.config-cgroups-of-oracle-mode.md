...
* 要使用 cgroup 功能，要求 Linux 内核版本必须为 Linux Kenel 4.19 及以上版本。
...
cgroup 系统目录配置成功后，需要建立 OceanBase 数据库软件的安装目录与 cgroup 系统目录的软链接。
...
<main id="notice" type='notice'>
<h4>注意</h4>
<p>在 V4.2.5 版本中，从 V4.2.5 BP2 版本开始，建立软链接并开启 cgroup 功能后，不需要重启 OBServer 节点，cgroup 功能就会自动生效；对于 V4.2.5 BP2（不含） 之前版本，仍需要重启 OBServer 节点，cgroup 功能才会生效。</p>
</main>

如果 cgroup 功能未开启，可以参考以下操作，开启 cgroup 功能。
...
cgroup 功能生效后，用户可以登录集群的 `sys` 租户，查询视图 `V$OB_CGROUP_CONFIG` 或 `GV$OB_CGROUP_CONFIG`，查看各 OBServer 节点下的 cgroup 配置。
...
建议使用 cgroup 中的目录文件调用 `DBMS_RESOURCE_MANAGER` 系统包中的子程序 `CREATE_CONSUMER_GROUP` 创建的资源组，来进行用户级或 SQL 级的资源隔离。配置资源隔离的详细操作，请参见 [配置租户内资源隔离](200.resource-isolation-at-user-level-of-oracle-mode.md)。
