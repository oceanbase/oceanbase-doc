...
cgroup 系统目录配置成功后，即可部署 OceanBase 数据库社区版。 OceanBase 数据库的详细安装操作请参见 [使用命令行部署 OceanBase 数据库生产环境](../../../../../400.deploy\500.deploy-oceanbase-database-community-edition\200.local-deployment\500.deploy-OceanBase-database-of-multi-node-cluster.md)。
