...
2. 查看当前配置项 `_enable_ddl_worker_isolation` 的值。
   ...
   ```shell
   obclient> SELECT * FROM oceanbase.GV$OB_PARAMETERS WHERE NAME LIKE '%_enable_ddl_worker_isolation%';
   ```

3. 修改配置项 `_enable_ddl_worker_isolation` 的值。

   ```shell
   obclient> ALTER SYSTEM SET _enable_ddl_worker_isolation = true;
   ```
