...
* 在进行 IOPS 资源隔离前，可以先进行磁盘性能校准，磁盘性能校准的相关操作请参见 [磁盘性能校准](../300.resource-isolation-of-mysql-mode/150.disk-performance-calibration-of-mysql-mode.md)。

   <main id="notice" type='explain'>
   <h4>说明</h4>
   <p>在当前版本中，IOPS 资源隔离不再强依赖磁盘性能校准操作。</p>
   </main>
...
* 如果需要配置 Database 级资源隔离，请确认已开启 Database 级资源隔离，即，租户级隐藏配置项 `_enable_database_isolation_mode` 的值已设置为 `True`。

  用户租户下，开启 Database 级资源隔离的语句如下：

  ```shell
  obclient(root@mysql001)[(none)]> ALTER SYSTEM SET _enable_database_isolation_mode = True;
  ```
...
5. 调用 `DBMS_RESOURCE_MANAGER` 系统包中的 `SET_CONSUMER_GROUP_MAPPING` 子程序，根据实际使用场景，创建资源隔离匹配规则。
   ...
   CALL DBMS_RESOURCE_MANAGER.SET_CONSUMER_GROUP_MAPPING(
      ATTRIBUTE => 'column | user | function | database', 
      VALUE => 'values',
      CONSUMER_GROUP => 'group_name');
   ```
   ...
   * `VALUE`：指定属性值。
     ...
     * 如果属性类型为 `database`，则此处需要指定为数据库名。当前仅支持指定一个数据库名。
   ...
   * 创建 Database 资源隔离匹配规则。

     * 指定在数据库 `db1` 上操作时，系统绑定到名为 `interactive_group` 的资源组上执行，并使用该资源组所限制的 CPU 资源和 IOPS 资源。
     ...
     * 指定在数据库 `db2` 上操作时，系统绑定到名为 `batch_group` 的资源组上执行，并使用该资源组所限制的 CPU 资源和 IOPS 资源。
...
* 同一租户下，如果同时配置了多种级别的隔离策略，其优先级如下：

  SQL 级资源隔离 > Database 级资源隔离 > User 级资源隔离 > Function 级资源隔离
