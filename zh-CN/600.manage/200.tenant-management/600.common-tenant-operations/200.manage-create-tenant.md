...
4. 通过 `CREATE RESOURCE POOL` 语句，创建资源池。
    ...
    * 创建一个名为 `mq_pool_01` 的资源池，在 zone1、zone 2 各创建 1 个 Unit，且 Unit 的资源规格为 `S1_unit_config`。
    ...
    * 创建一个名为 `mq_pool_02` 的资源池，在 zone3 里创建 2 个 Unit，每个 Unit 的资源规格为 `S1_unit_config`。
...
4. （可选）当前版本支持异构 Zone 部署的租户，由于创建租户的语句中，不支持通过配置项 `zone_deploy_mode` 来指定异构 Zone 模式（`hetero`），如果用户希望直接创建一个异构部署（异构 Zone 模式）的租户，可以通过以下方式，修改配置项模板。
    ...
    <main id="notice" type='explain'>
    <h4>说明</h4>
    <p>除了修改配置项模板，也可以先创建同构 Zone 部署的租户后，再修改租户级配置项 <code>zone_deploy_mode</code> 的值为<code>hetero</code>（异构 Zone 模式）。</p>
    </main>

    命令执行成功后，后续新建的所有租户均允许异构部署。

5. 通过 `CREATE TENANT` 语句，创建租户。
    ...
    * `RESOURCE_POOL_LIST`：指定分配给租户的资源池列表，必填。如果有多个资源池：

      * 同构 Zone 模式下，要求多个资源池的 `UNIT_NUM` 个数一致。
      * 异构 Zone 模式下，租户的多个资源池的 `UNIT_NUM` 个数可以相同也可以不同，但一个租户最多只有两种 `UNIT_NUM`。例如，一租户有 3 个资源池 <code>pool1</code>、<code>pool2</code>、<code>pool3</code>，其 <code>unit_num</code> 分别为 2、4、4，如果将 <code>pool3</code> 的 <code>unit_num</code> 修改为 1，则系统会报 <code>More than two distinct unit_num in a single tenant not allowed</code> 的错误。
    ...
    * 创建一个名为 `mq_t2` 的异构部署的租户（默认为 MySQL 模式租户），资源池指定为 `mq_pool_01` 和 `mq_pool_02`，Primary Zone 为 `zone1,zone2`，允许所有 IP 连接数据库。
...
6. 查询 `DBA_OB_TENANTS` 视图，确认租户创建成功。

    例如，确认租户 `mq_t1` 是否创建成功：
...
7. 租户创建成功后，可以尝试登录租户进行使用。

   默认管理员用户（MySQL 模式为 `root`，Oracle 模式为 `sys`）的密码为空，您需要及时修改管理员用户的密码。修改示例如下。
