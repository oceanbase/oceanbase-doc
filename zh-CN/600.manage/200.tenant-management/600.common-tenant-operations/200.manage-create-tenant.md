...
3. 通过 `DBA_OB_UNIT_CONFIGS` 视图，获取已有的资源规格信息。
   ...
   `DBA_OB_UNIT_CONFIGS` 视图的详细说明，参见 [DBA_OB_UNIT_CONFIGS](../../../700.reference/700.system-views/400.system-view-of-mysql-mode/200.dictionary-view-of-mysql-mode/6000.oceanbase-dba_ob_unit_configs-of-mysql-mode.md)。
...
4. 通过 `CREATE RESOURCE UNIT` 语句，创建资源规格。
    ...
    * `MAX_IOPS` 和 `MIN_IOPS`：`MAX_IOPS` 和 `MIN_IOPS` 参数的最小值为 1024，并且要求 `MAX_IOPS >= MIN_IOPS`。如果不指定，默认系统将根据 CPU 的规格自动计算。
