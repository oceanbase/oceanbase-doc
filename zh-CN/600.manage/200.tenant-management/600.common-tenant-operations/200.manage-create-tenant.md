...
4. 通过 `CREATE RESOURCE UNIT` 语句，创建资源规格。
    ...
    * 在为参数指定值时，可以采用纯数字不带引号的方式，也可以使用带单位加引号或不带单位加引号的方式（例如：`'1T'`、`'1G'`、`'1M'`、`'1K'`）。
      ...
      * 对于 `MEMORY_SIZE` 和 `LOG_DISK_SIZE` 这些容量参数，如果使用带单位加引号的方式，与纯数字不带引号一样，其单位含义为字节，即 '1K' = 1024，'1M' = 1024 * 1024。
...
3. 通过 `DBA_OB_TENANTS` 视图，查看所有的租户信息。
    ...
    `DBA_OB_TENANTS` 视图的详细说明，请参见 [DBA_OB_TENANTS](../../../700.reference/700.system-views/400.system-view-of-mysql-mode/200.dictionary-view-of-mysql-mode/12000.o-dba_ob_tenants-of-mysql-mode.md)。

4. （可选）V4.2.5 从 BP5 版本开始支持异构 Zone 部署的租户，由于创建租户的语句中，不支持通过配置项 `zone_deploy_mode` 来指定异构 Zone 模式（`hetero`），如果用户希望直接创建一个异构部署（异构 Zone 模式）的租户，可以通过以下方式，修改配置项模板。
...
5. 通过 `CREATE TENANT` 语句，创建租户。
    ...
    * 创建一个名为 `mq_t1` 的租户（默认为 MySQL 模式租户），副本数为 2，资源池指定为 `mq_pool_01`，Primary Zone 为 zone1，允许所有 IP 连接数据库。
