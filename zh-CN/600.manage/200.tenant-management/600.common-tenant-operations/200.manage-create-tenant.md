...
4. 通过 `CREATE RESOURCE UNIT` 语句，创建资源规格。
    ...
    * `MAX_IOPS` 和 `MIN_IOPS`： 指定当前 Unit 可使用的 IOPS 资源的上限和下限，最小值均为 1024，并且要求 `MAX_IOPS >= MIN_IOPS`。
       ...
       由于 `MAX_IOPS` 和 `MIN_IOPS` 设置后，OceanBase 数据库内部会统一按照 16 KB 读的 IOPS 值作为有效值进行处理，建议将 `MAX_IOPS` 和 `MIN_IOPS` 的值设置为当前云盘计算出来的 16 KB 读对应的 IOPS 值，以便后续进行租户内的资源隔离。有关租户内资源隔离的详细介绍及操作，请参见 [资源隔离概述](300.resource-isolation/100.resource-isolation-overview.md)。

       以 16 KB 读为例，为云盘计算 16 KB 读对应的 IOPS 值的方法如下。假设当前使用的云盘的限制带宽为 300 MB，IOPS 为 30000，则使用 16 KB 读可以计算出 300 MB 带宽最多能达到的 IOPS 为 `(300 * 1024 KB)/16 KB=19200`，故可以将 `MAX_IOPS` 设置为 19200。如果计算出来的 IOPS 值已经达到或超过了该云盘的 IOPS 上限，例如，某种云盘的 IOPS 最高为 10000，则以 10000 为基准来设置。
