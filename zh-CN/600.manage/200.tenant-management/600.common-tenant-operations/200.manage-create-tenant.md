...
4. 通过 `CREATE RESOURCE UNIT` 语句，创建资源规格。
    ...
    * `MAX_IOPS` 和 `MIN_IOPS`：指定当前 Unit 可使用的 IOPS 资源的上限和下限，最小值均为 1024，并且要求 `MAX_IOPS >= MIN_IOPS`。
      ...
      * 如果 `MIN_IOPS` 和 `MAX_IOPS` 均未指定，则：

        * 在 V4.1.0 版本中，对于 V4.1.0 BP4（不含该版本） 之前版本，默认由系统根据 CPU 的规格自动计算 `MIN_IOPS` 和 `MAX_IOPS` 的值。计算规则为 1 个 Core 对应 1 万 IOPS 的值，即 `MAX_IOPS = MIN_IOPS = MIN_CPU * 10000`。对于 V4.1.0 BP4（含该版本） 及之后版本，默认由系统根据 `IOPS_WEIGHT` 自动分配 IOPS 资源，且 `MIN_IOPS = MAX_IOPS = INT64_MAX`。
...
4. 通过 `CREATE TENANT` 语句，创建租户。
    ...
    * `ENABLE_ARBITRATION_SERVICE`：指定租户是否开启仲裁服务，如果不显式指定，默认创建的租户其仲裁服务为关闭状态。如果创建租户的时候未开启仲裁服务，可以在创建租户后再开启仲裁服务，详细操作请参见 [开启仲裁服务](../../400.high-availability/400.arbitration-high-availability/200.enable-the-arbitration-service.md)。
