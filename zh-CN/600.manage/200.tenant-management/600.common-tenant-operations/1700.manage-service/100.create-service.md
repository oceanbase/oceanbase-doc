...
* 为租户创建服务时，要求租户的 Unit 中没有临时下线的机器。如果有临时下线的机器，则执行 `CREATE SERVICE` 命令后，系统会返回 `OB_SERVICE_NOT_FULLY_STARTED` 的错误，表示租户的 Unit 中除了永久下线的机器，其他的仅部分机器支持通过指定 SERVICE_NAME 的方式来建立连接。
