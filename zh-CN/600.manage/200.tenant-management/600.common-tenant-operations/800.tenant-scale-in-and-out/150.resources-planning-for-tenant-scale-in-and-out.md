...
对于租户缩容的场景，也可以参考上述扩容场景中的方法，估算出业务运行所需的逻辑资源需求量后，再调用 `DBMS_OB_LIMIT_CALCULATOR` 系统包中的 `CALCULATE_MIN_PHY_RES_NEEDED_BY_LOGIC_RES` 子程序，计算出这些逻辑资源所需的物理资源量，然后调小租户相应的物理资源量即可。

此外，在通过调整 Unit Number 实现租户缩容时，如果租户有多个 Unit，且每个 Unit 均比较空闲，可以通过以下方法选择合适的 Unit 进行删除。
