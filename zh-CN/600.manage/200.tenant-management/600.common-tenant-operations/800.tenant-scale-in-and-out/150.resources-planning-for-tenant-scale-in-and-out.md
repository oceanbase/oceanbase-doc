...
4. 根据实际扩容场景，计算扩容后所需的逻辑资源。
    ...
    * 用户租户在每个节点上能够创建 Tablet 个数的上限
    ...
    根据上述计算公式，假设租户 1002 PRIMARY_ZONE 的个数为 1， Unit 个数为 2，有广播日志流。现在需要将租户的 PRIMARY_ZONE 个数修改为 3，则单个节点上达到稳定状态的日志流个数即为 5（3 + 2）个。如果这些日志流发生 Transfer，则同时存在最多的日志流个数 `MAX_UNTILIZATION` 可能需要达到 14，故需要对租户进行扩容，以便能够承载 14 个日志流。
