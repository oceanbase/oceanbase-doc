...
4. 根据实际扩容场景，计算扩容后所需的逻辑资源。
    ...
    * 由于任何一个日志流都可能将分区 Transfer 到其他日志流上，同一个节点内的日志流之间 Transfer 时不会新建日志流，不同节点的日志流之间 Transfer 时会新建一个日志流，故发生 Transfer 时，用户租户单个节点最大的临时日志流个数：`(Unit 个数 * PRIMARY_ZONE 个数) * ((Unit 个数 - 1) * PRIMARY_ZONE 个数) `

    * 用户租户在每个节点上能够创建 Tablet 个数的上限
    
      * 受配置项值限制的上限：`(MEMORY_SIZE/1G) * _max_tablet_cnt_per_gb`
