...
4. 根据实际扩容场景，计算扩容后所需的逻辑资源。
    ...
    * 用户租户在每个节点上能够创建 Tablet 个数的上限
    ...
    上述公式中，`U` 在同构 Zone 模式下表示租户的 `UNIT_NUM` 数值，异构 Zone 模式下表示租户两个不同 `UNIT_NUM` 的最小公倍数；`P` 表示租户 `PRIMARY_ZONE` 的第一优先级个数。有关上述公式中其他参数的详细说明，请参见 [查看租户和资源信息](../400.view-tenant-information.md)。
...
同样，对于异构 Zone 模式的租户，通过调整租户单个资源池的 Unit Number 来实现租户缩容时，如果每个 Uint 均比较空闲，不确定选择哪一个 UNIT_LIST 进行删除时，也可以通过以下方法来完成。

以下以同构 Zone 模式的租户为例，提供租户缩容场景下的资源规划操作指导。异构 Zone 模式的租户的操作类似。
...
4. 执行以下命令，分别计算租户各 Unit 所需的最小物理资源。
