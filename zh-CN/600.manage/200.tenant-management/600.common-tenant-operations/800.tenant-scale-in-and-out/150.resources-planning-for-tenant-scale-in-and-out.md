...
4. 根据实际扩容场景，计算扩容后所需的逻辑资源。
    ...
    * 用户租户在每个节点上能够创建 Tablet 个数的上限
    ...
    上述公式中，`U` 在同构 Zone 模式下，表示租户的 `UNIT_NUM` 数值，异构 Zone 模式下表示租户两个不同 `UNIT_NUM` 的最小公倍数；`P` 表示租户 `PRIMARY_ZONE` 的第一优先级个数。有关上述公式中其他参数的详细说明，请参见 [查看租户和资源信息](../400.view-tenant-information.md)。
    
    根据上述计算公式，假设一同构 Zone 模式的租户 1002 `PRIMARY_ZONE` 第一优先级的个数为 1， `UNIT_NUM` 数为 2，有广播日志流。现在需要将租户的 `PRIMARY_ZONE` 第一优先级个数修改为 3，则该租户所有节点上达到稳定状态的日志流个数即为 8（`2*3 + 2`）个。如果这些日志流发生 Transfer，则单个节点上同时存在的最大日志流个数 `MAX_UNTILIZATION` 可能需要达到 14（`((2 * 3) *（2-1）*3)/2 + 3 + 2`）个，故需要对租户进行扩容，以便能够承载 14 个日志流。
...
4. 执行以下命令，分别计算租户内各 Unit 所需的最小物理资源。
    ...
    ```shell
    obclient(root@sys)[Oceanbase]> CALL DBMS_OB_LIMIT_CALCULATOR.CALCULATE_MIN_PHY_RES_NEEDED_BY_UNIT(TENANT_ID => 1002, SERVER => "xxx.xx.xx.192:2882");
    ```
