...
* 不允许在增加 LS、删除 LS 或 LS 属性变化的过程中执行克隆租户的操作。
  ...
  可以通过视图 `CDB_OB_BALANCE_TASKS`（系统租户）或 `DBA_OB_BALANCE_TASKS`（用户租户）进行确认。

* 不允许在 Transfer 过程中执行克隆租户的操作。

  可以通过视图 `CDB_OB_TRANSFER_TASKS`（系统租户）或 `DBA_OB_TRANSFER_TASKS`（用户租户）进行确认。

* 不允许在 Unit 迁移过程中执行克隆租户的操作。

  可以通过视图 `DBA_OB_UNIT_JOBS`（系统租户）进行确认。
