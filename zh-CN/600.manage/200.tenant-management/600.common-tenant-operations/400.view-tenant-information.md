...
以查询结果的第三行数据为例，租户名称（`TENANT_NAME`）是 mysql001，租户 ID（`TENANT_ID`）是 1002，租户类型（TENANT_TYPE）是用户租户，租户创建时间是（CREATE_TIME）是 `2025-12-29 15:44:48`，租户的 PRIMARY_ZONE（`PRIMARY_ZONE`）是 `zone1`（高优先级）和 `zone2`，租户副本分布（LOCALITY）在 zone1、zone2、zone3，兼容模式（COMPATIBILITY_MODE）是 MySQL。
