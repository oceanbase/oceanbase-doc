...
| 资源名称      | 描述                                                                                                                     |
|--------------|--------------------------------------------------------------------------------------------------------------------------|
|  LS	         | 日志流，事务基础提交单位，租户在每个节点上能够创建的日志流个数由配置项（_max_ls_cnt_per_server）、租户内存: MAX(8, (MEM_SIZE - 4G) / 200MB + 8)、日志盘大小(512MB每个)共同决定。 |
| Tablet	     | 数据分片，数据迁移的基本单位，租户在每个节点上能够创建 Tablet 个数的上限为 `(MEM_SIZE/1G * _max_tablet_cnt_per_gb)`。           | 
| Session	     | 会话，租户在每个节点上能够创建的连接数的上限由配置项 `_resource_limit_max_session_num` 或租户内存(5%/100K)来决定，配置项设置的值优先级更高。|
...
sys 租户通过 `GV$OB_TENANT_RESOURCE_LIMIT` 视图，可以查看所有租户在每个 Unit 上逻辑资源的使用情况，示例如下：
