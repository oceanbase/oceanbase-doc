...
| 资源名称      | 描述                                                                                                                                                           |
|--------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------|
|  LS	         | 日志流，事务基础提交单位，租户在每个节点上能够创建的日志流个数由隐藏配置项（`_max_ls_cnt_per_server`）、租户内存（`MAX(8, (MEMORY_SIZE - 4G) / 200MB + 8)`）、日志盘大小（每个日志流需要 512 MB）共同决定。 |
| TABLET	     | 数据分片，数据迁移的基本单位，租户在每个节点上能够创建 Tablet 个数的上限为 `(MEMORY_SIZE/1G * _max_tablet_cnt_per_gb)`。                                             | 
| SESSION	     | 会话，租户在每个节点上能够创建的连接数的上限由隐藏配置项 `_resource_limit_max_session_num` 或租户内存（`租户内存的 5% / 100KB`）来决定，配置项设置的值优先级更高。       |
| THREAD	     | 线程，每个节点上的线程数是有上限的，由隐藏配置项 `_ob_max_thread_num` 和租户内存（`MEMORY_SIZE`）决定。                                                              |
...
计算出所需的逻辑资源后，由于租户在每个节点上能创建的逻辑资源会受到各种物理资源或配置项值的限制，可以通过视图 `GV$OB_TENANT_RESOURCE_LIMIT_DETAIL` 来查看租户逻辑资源具体所受的限制及各种限制的上限值。
