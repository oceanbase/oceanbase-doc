...
4. 通过 `CREATE RESOURCE UNIT` 语句，创建资源规格。
    ...
    * `MEMORY_SIZE`：内存规格，最小为 1G。OceanBase 数据库 4.0 开始不支持内存超卖。
    * `LOG_DISK_SIZE`：日志盘空间，OceanBase 数据库 4.0 会按租户管理日志盘空间，系统为各个租户预留日志盘空间，实现按租户隔离。当用户没有指定时，`LOG_DISK_SIZE` 默认值是内存规格的 3 倍大小，最小为 2G。
    * OceanBase 数据库 4.0 支持租户间 IOPS 隔离，通过 `MAX_IOPS`、`MIN_IOPS`、`IOPS_WEIGHT` 三个参数决定。IOPS 规格参数默认不需要指定，系统根据 CPU 规格自动计算。
...
3. 通过 `DBA_OB_TENANTS` 视图，查看所有的租户信息。
   ...
   `DBA_OB_TENANTS` 视图的详细说明，请参见 [oceanbase.DBA_OB_TENANTS](../../../700.reference/500.system-reference/400.system-view-of-mysql-mode/200.dictionary-view-of-mysql-mode/5800.oceanbase-dba_ob_tenants-of-mysql-mode.md)
...
4. 通过 `CREATE TENANT` 语句，创建租户。
    ...
    * `tenant_name`：指定租户名称。
    * `PRIMARY_ZONE`：指定租户的 Primary Zone。当前版本修改 Primary Zone 的功能并不完善，修改租户的 Primary Zone并不能实现租户下已有表和分区的读写服务能力的扩缩容。建议用户在创建租户时一次性配置好租户的 Primary Zone 属性。
    ...
    * `system_var_name`：指定系统变量或配置项。
    ...
    `CREATE TENANT` 语句的详细说明，参见 [CREATE TENANT](../../../700.reference/400.development-reference/100.sql-syntax/100.system-tenants/800.create-tenant-sql.md)。
