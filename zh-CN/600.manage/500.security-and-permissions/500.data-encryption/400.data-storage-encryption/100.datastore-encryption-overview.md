...
|`bkmi`|开启，主密钥由外部 BKMI 系统管理|
|`obcloud`|开启，主密钥由 OBCloud 公有云统一 KMS 代理服务管理（推荐用于 OBCloud）|
|`ocp`|开启，主密钥由 OCP KMS 代理服务管理（推荐用于私有云）|

各取值的配置约束、`external_kms_info` 及 clog 加密等说明，请参见 [tde_method](../../../../700.reference/800.configuration-items-and-system-variables/100.system-configuration-items/400.tenant-level-configuration-items/7800.tde_method.md)。[external_kms_info](../../../../700.reference/800.configuration-items-and-system-variables/100.system-configuration-items/400.tenant-level-configuration-items/3200.external_kms_info.md) 用于配置 `obcloud` / `ocp` 等外部 KMS 代理相关属性。

`internal` 方式下，主密钥信息主要在内部表中管理；为避免日志回放时的循环依赖，**clog 默认不加密**。`bkmi`、`obcloud` 与 `ocp` 方式下，可按 [tde_method](../../../../700.reference/800.configuration-items-and-system-variables/100.system-configuration-items/400.tenant-level-configuration-items/7800.tde_method.md) 与 [enable_clog_encryption](../../../../700.reference/800.configuration-items-and-system-variables/100.system-configuration-items/400.tenant-level-configuration-items/2390.enable_clog_encryption.md) 手动开启 clog 加密。
...
* `ocp` 取值自 V4.6.0 起提供；`bkmi`、`obcloud`、`ocp` 等取值的实际可用性及配套能力以部署形态与产品版本为准。
