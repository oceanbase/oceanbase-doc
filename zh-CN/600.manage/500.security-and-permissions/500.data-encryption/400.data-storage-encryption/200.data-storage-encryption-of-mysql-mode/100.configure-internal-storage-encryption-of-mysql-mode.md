...
本节以在加密表空间 `sectest_ts1` 上创建表 `t1` 为例，提供透明加密的操作指导，共两步：

1. 配置加密方式，本文给出 `internal` 方式和 `obcloud` 方式的配置示例；
2. 创建表空间并指定加密算法，本文以 `aes-256` 为例。
