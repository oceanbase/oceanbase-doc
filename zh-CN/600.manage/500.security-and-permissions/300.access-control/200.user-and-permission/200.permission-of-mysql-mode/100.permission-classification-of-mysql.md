...
| 全局权限  | CREATE LOCATION <main id="notice" type='explain'><h4>说明</h4><p>对于 OceanBase 数据库 V4.4.x 版本，从 V4.4.1 版本开始引入 <code>CREATE LOCATION</code> 权限。</p></main>| 确定用户是否可以执行 `CREATE LOCATION`、`DROP LOCATION` 语句。|
