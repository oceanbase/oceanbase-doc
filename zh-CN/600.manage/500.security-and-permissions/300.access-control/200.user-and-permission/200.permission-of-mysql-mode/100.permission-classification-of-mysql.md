...
|  全局权限 | SHUTDOWN               | 确定用户是否可以执行 `mysqladmin shutdown` 命令的权限。<main id="notice" type='explain'><h4>说明</h4><p>支持为用户授予该权限，但该权限暂无适用场景。</p></main> |
|  全局权限 | RELOAD                 | 确定用户是否可以执行 Flush 操作的权限。 <main id="notice" type='explain'><h4>说明</h4><p>支持为用户授予该权限，但该权限暂无适用场景。</p></main>|
