...
| 字段 | 说明 |
|------|-----|
| user_name   | 用于指定用户名称。<main id="notice" type='explain'><h4>说明</h4><p><code>AUDIT_LOG_FILTER_SET_USER</code> 指定的 <code>user@host</code> 不要求是已存在的用户，可以写通配符或不存在的用户。<code>user_name</code> 有以下限制：<ul><li>不支持指定 <code>host</code> 字段。</li><li><code>user</code> 只支持对整个字段进行通配，即 <code>%</code>，不支持 <code>test_%</code> 这种部分通配的写法。</li></ul></p></main>|
