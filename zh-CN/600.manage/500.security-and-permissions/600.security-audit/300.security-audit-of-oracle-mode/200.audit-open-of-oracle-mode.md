...
Oracle 租户在将审计记录写入磁盘文件（`audit_trail` 为 `LOG`）时，支持对归档日志以明文或加密方式落盘。加密通过 `audit_log_encryption` 配置项以及审计函数 `AUDIT_LOG_ENCRYPTION_PASSWORD_SET()` 和 `AUDIT_LOG_ENCRYPTION_PASSWORD_GET()` 实现。具体步骤参见文末配置文档。
