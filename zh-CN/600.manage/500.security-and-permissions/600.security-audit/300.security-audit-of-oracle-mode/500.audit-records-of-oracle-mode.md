...
在开启安全审计功能时，会同步设置审计记录的存储方式，支持写入内部表、写入本地磁盘文件和写入审计日志三种方式。您可以根据设置到对应的位置查看审计记录。
...
<main id="notice" type='explain'>
  <h4>说明</h4>
  <p>本功能从 V4.4.2 BP2 版本开始支持。</p>
</main>

当 `audit_trail` 设置为 `LOG` 时，审计记录写入审计日志文件，支持日志归档、压缩、轮转、清理及加密等管理策略。

审计日志文件默认存放在 `<安装目录>/log/` 下。你可以通过查看审计日志文件内容来查看审计记录。关于审计日志的加密、归档等管理操作，请参见[设置和查看审计规则](300.set-up-and-view-audit-rules-of-oracle-mode.md)。
