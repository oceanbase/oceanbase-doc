...
<main id="notice" >
    <h4>功能适用性</h4>
    <p>本文档仅适用于 OceanBase 数据库 Oracle 模式。目前 OceanBase 数据库社区版仅提供了 MySQL 模式，有关 MySQL 模式下的密码复杂度，参见 <a href="../1.identification-and-authentication-of-mysql-mode/2.password-complexity-of-mysql-mode.md">密码复杂度</a>。</p>
</main>
...
2. 编写 `PASSWORD_VERIFY_FUNCTION` 参数对应的函数并授权。
   ...
   * 如果修改密码，新密码与旧密码至少要有 3 个以上的字符不相同。
   ...
   创建函数的相关操作，请参见 [创建函数](../../../../7.reference/5.sql-reference/3.pl-reference/3.pl-oracle/7.stored-procedure-and-functions-oracle/3.create-a-function-oracle.md)。
...
2. 通过 `DBA_SOURCE` 视图查找对应的函数。
   ...
   更多 `DBA_SOURCE` 视图及其字段的说明，参见 [DBA_SOURCE](../../../../7.reference/7.system-views/5.system-view-of-oracle-mode/2.dictionary-view-of-oracle-mode/99.dba_source-of-oracle-mode.md)。
...
3. 通过 `USER_SOURCE` 视图查看当前用户拥有的函数 `VERIFY_FUNCTION` 的内容。
   ...
   更多 `USER_SOURCE` 视图及其字段的说明，参见 [USER_SOURCE](../../../../7.reference/7.system-views/5.system-view-of-oracle-mode/2.dictionary-view-of-oracle-mode/168.user_source-of-oracle-mode.md)。
...
更多函数管理的相关操作，参见 [管理存储过程和函数](../../../../7.reference/5.sql-reference/3.pl-reference/3.pl-oracle/7.stored-procedure-and-functions-oracle/4.view-and-delete-procedures-and-functions-oracle.md)。
