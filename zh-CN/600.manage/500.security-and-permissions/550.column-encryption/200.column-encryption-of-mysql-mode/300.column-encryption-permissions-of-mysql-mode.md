...
1. 授予用户级权限
    ...
    | 语法/参数 | 说明 | 示例 |
    |------|------|------|
    | `user_or_role_list` | 用户或角色列表参数，多个用户用逗号分隔。<br>示例：`u1, u2, r1` |
    | `*.*` | 授予用户级（全局）权限的固定语法，表示全局范围的所有数据库和表。 |
...
1. 撤销用户级权限
    ...
    |------|------|------|
    | `user_or_role_list` | 用户或角色列表参数，多个用户用逗号分隔。<br>示例：`u1, u2, r1` |
    | `*.*` | 撤销用户级（全局）权限的固定语法，表示全局范围的所有数据库和表。 |
...
* [创建列加密规则](200.create-column-encryption-rule-of-mysql-mode.md)
