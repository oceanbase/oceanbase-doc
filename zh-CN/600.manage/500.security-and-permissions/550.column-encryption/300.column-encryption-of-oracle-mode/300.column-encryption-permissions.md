...
-- 授予用户/角色对特定规则的明文访问权限
GRANT PLAINACCESS ANY SENSITIVE RULE ON *.* TO  <user_or_role_list>;
```
...
GRANT PLAINACCESS ANY SENSITIVE RULE ON *.* TO u1;
...
-- 撤回用户/角色对特定规则的明文访问权限
REVOKE PLAINACCESS ANY SENSITIVE RULE ON *.* FROM <user_or_role_list>;
```
...
REVOKE PLAINACCESS ANY SENSITIVE RULE ON *.* FROM u1;
