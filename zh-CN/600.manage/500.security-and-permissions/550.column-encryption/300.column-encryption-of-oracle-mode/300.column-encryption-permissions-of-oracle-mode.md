...
列加密功能采用基于规则的权限类型，通过数据保护规则控制用户对敏感列的访问权限。OceanBase 在 Oracle 权限体系中新增了专门的权限类型来支持列加密功能，包含用户级权限和敏感规则级权限。
...
Oracle 模式下不支持使用 `SHOW GRANTS` 语句查看用户权限，需要使用以下视图查看：
...
| `DBA_SYS_PRIVS`、`USER_SYS_PRIVS`、`ROLE_SYS_PRIVS` | 可查看被授予的用户级创建敏感规则（CREATE SENSITIVE RULE）权限和明文访问（PLAINACCESS）权限。 |
