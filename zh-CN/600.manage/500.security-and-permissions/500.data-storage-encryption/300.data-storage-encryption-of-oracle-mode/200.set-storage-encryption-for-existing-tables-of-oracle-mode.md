    ...
    * 对表执行全量合并
      ...
      2. 手动发起一轮合并。
         ...
         <main id="notice" type='explain'>
            <h4>说明</h4>
            <p>全量合并完成后，数据加密可能仍在进行中。此时查询 <code>V$OB_ENCRYPTED_TABLES</code> 视图，<code>STATUS</code> 可能为 <code>ENCRYPTING</code>（加密中），表示加密任务尚未结束，并不代表合并失败。请等待 <code>ENCRYPTED</code> 变为 <code>YES</code> 且 <code>STATUS</code> 变为 <code>NORMAL</code> 后，再确认加密已完成。</p>
         </main>
...
3. 完成后，您可以查看以下视图，确认所有宏块是否已完成加密。
    ...
    * `BLOCKS_DECRYPTED` 为 `0`。
    ...
    更多视图 `V$OB_ENCRYPTED_TABLES` 的字段及说明信息，参见 [V$OB_ENCRYPTED_TABLES](../../../../700.reference/700.system-views/500.system-view-of-oracle-mode/300.performance-view-of-oracle-mode/30300.v-encrypted_tables-of-oracle-mode.md)。
