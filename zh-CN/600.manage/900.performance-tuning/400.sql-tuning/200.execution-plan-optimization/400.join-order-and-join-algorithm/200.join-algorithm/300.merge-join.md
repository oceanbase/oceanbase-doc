...
Merge Join 其实是 Nested Loops Joins 的一种变种。如果联接的两个数据集尚未排序，则数据库会对它们进行排序。对于第一个数据集中的每一行，数据库会探测第二个数据集中的匹配行并将它们联接起来，其起始位置基于上一次迭代中的匹配。这一点与 Nested Loops Joins 不同，Nested Loops Joins 遍历第二个数据集时每次都是从第一行数据开始。

为了帮助阐明概念，下面提供的伪代码示例将简要展示 Merge Join 算法的联接过程。需要注意的是，实际上该算法的实现远比此示例更加复杂。Merge Join 的执行过程可以通过以下伪代码来简略理解：
