...
最后，先收集完 `T_PART` 的统计信息，然后重新查询 Q4 和 Q5 的计划如下，可以看见在 Q4 的计划中，估行是准确的，通过观察 Optimization Info 中的 `est_method:[OPTIMIZER STATISTICS]`，可以看到当前计划使用的估行方式是通过收集的统计信息进行行数估计的；同时可以看见在 Q5 的计划中，其估行结果也是准确的，因为使用了收集的统计信息计算了准确的谓词选择率，加上存储层估计行数，因此可以得到一个准确的估行结果。
