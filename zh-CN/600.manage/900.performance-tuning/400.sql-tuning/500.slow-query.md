...
对一个正在运行的数据库系统，有的时候一批慢查询会拖慢整个系统的运行效率。有的时候是某一类查询执行了非常长的时间，一直没有结束执行。本文给出一种利用 `GV$OB_PROCESSLIST`、`GV$OB_PLAN_CACHE_PLAN_STAT`、`GV$OB_PLAN_CACHE_PLAN_EXPLAIN` 来查找慢查询和对应执行计划的方法。
