...
2. 执行以下命令，查看集群配置项。
   ...
   * 指定 `TENANT` 关键字可以查看指定租户的配置项信息。
   ...
   | isdefault  | 当前值是否为默认值。<ul><li>0：表示当前值不是默认值。</li><li>1：表示当前值为默认值。</li></ul> |
...
3. 执行以下命令，查看集群配置项。
  ...
  有关 `GV$OB_PARAMETERS` 视图的更多信息，请参见 [GV$OB_PARAMETERS（MySQL 模式）](../../../700.reference/700.system-views/400.system-view-of-mysql-mode/300.performance-view-of-mysql-mode/2100.gv-ob_parameters-of-mysql-mode.md)。
