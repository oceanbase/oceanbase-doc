...
* 要确认升级的新版本和所有关联集群的版本都兼容。判断两个版本是否兼容需要参考 `oceanbase_upgrade_dep.yml` 中的版本描述，例如，A 版本的 `can_be_upgraded_to` 属性为 B（即 B 版本号大于 A），表示 B 版本可以兼容 A 版本，那么相应地 B 版本的仲裁服务也就可以兼容 A 版本的 OceanBase 集群。
