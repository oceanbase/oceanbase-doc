...
* 支持 V4.3.0 Beta 及其后 BP 版本、V4.3.1 Beta 及其后 BP 版本、V4.3.2 Beta Hotfix1 及其后 BP 版本原地升级到 V4.3.3。
* 暂不支持 V4.2.x 系列或更低版本升级到 V4.3.3，随着版本演进，后续会增加升级路径支持。
* 自 V4.3.2 重构了多源数据的持久化格式，升级过程中需要对新旧多源数据格式进行转换，在分区数较多时需要预留相对充足的升级时间。
* 升级到 V4.3.3 过程中，需要保证没有进行中的增量旁路导入任务，可通过查询 `oceanbase.__all_virtual_load_data_stat` 确认。计划在后续版本解决这个问题。
