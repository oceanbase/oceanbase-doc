...
* 在 ARM 架构和不支持 AVX2（Advanced Vector Extensions 2） 指令集的 x86 机型集群内，如果存在使用 CS 编码数据（Constrained Sequential encoding data）时，不能从 V4.3 的低版本（V4.3.0、V4.3.1 和 V4.3.2）升级到 V4.3.3 及往后的版本，以防止升级后出现数据正确性风险。

* 当有 V4.3 的低版本集群升级到 V4.3.3 及往后的版本集群时，对于备租户，低版本集群无法解析高版本集群生成的日志，此时会在备租户提交日志时报错（仅报错，对应日志不会持久化）。
