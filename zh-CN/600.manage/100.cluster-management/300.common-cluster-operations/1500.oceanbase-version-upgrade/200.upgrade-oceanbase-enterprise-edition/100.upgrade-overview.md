...
* 升级到 V4.4.2 时，如果升级前置检查报如下错误：
  ...
  * 如果当前是 V4.2.5 BP7 版本，则需要升级到 V4.2.5 BP7 hotfix3 以及以上的 V4.2.5 BP7 hotfix 版本后再升级到 V4.4.2。
  * 如果当前是 V4.3.5 BP2 以及之前的版本，升级遇到该问题，则表示暂时无法支持该升级。

## 升级说明

* 支持无共享（Shared-Nothing，SN）模式下从 V4.2.5 BP7/V4.4.0/V4.4.0 BP1/V4.4.1 平滑升级到 V4.4.2 版本。
* 不支持共享存储（Shared-Storage，SS）模式的集群升级到 V4.4.2 版本。
* 不支持使用了向量的集群升级到 V4.4.2 版本。

## 升级路径图

![4257-4355-442-450](https://obbusiness-private.oss-cn-shanghai.aliyuncs.com/doc/img/observer-enterprise/V4.3.5/1500.oceanbase-version-upgrade/6-4257-4355-442-450.png)

<main id="notice" type='explain'>
  <h4>说明</h4>
  <p><ul><li>本文升级路径图并未包含 OceanBase 数据库的所有版本，只保留推荐的升级路径（升级路径仅针对 SN 模式）。如需了解更多版本的升级路径，可以参考 <code>oceanbase_upgrade_dep.yml</code> 文件，该文件记录了 OceanBase 的版本升级拓扑图。</li><li>解压目标版本的 OceanBase RPM 包后，会在存放 RPM 包的目录下生成 <code>home</code> 和 <code>use</code> 两个目录。升级相关脚本和 <code>oceanbase_upgrade_dep.yml</code> 文件存放 <code>home/admin/oceanbase/etc</code> 目录下。</li></ul>使用 <code>oceanbase_upgrade_dep.yml</code> 文件确认升级路径的详细信息，参见 <a href="300.start-upgrade.md">升级 OceanBase 集群</a> 中的 <b>步骤二：确认升级流程</b> 章节。</p>
</main>
