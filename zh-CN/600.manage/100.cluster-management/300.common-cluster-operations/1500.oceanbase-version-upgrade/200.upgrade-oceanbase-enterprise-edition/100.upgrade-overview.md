...
* OceanBase V4.0 版升级至 V4.1 版本的过程中会停日志归档。
...
* 支持 OceanBase 数据库 V4.2.1 BP2 及之前的 V4.x 版本通过有效升级路径在线升级到 OceanBase 数据库 V4.2.4 版本；V4.2.1 BP3 及之后的 BP 版本不再支持升级到 V4.2.4 版本。
* 支持 OceanBase 数据库 V4.2.3 升级到 V4.2.4 版本。
* 集群 Tablet 比较多的情况下，升级过程中 OBServer 重启会比较耗时。Tablet 达到 300 万级别时，重启时长预计会达到 20 分钟。
* V4.2.1 及 V4.2.2 系列升级到 V4.2.4 时，如果 `_nlj_batching_enabled` 配置项处于开启状态，升级前需要关闭，升级完成后可打开。
