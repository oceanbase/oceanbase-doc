...
3. 分析 `oceanbase_upgrade_dep.yml` 文件。
    ...
    * `require_from_binary`：升级序列包含当前版本时是否需要先升级到当前版本，即当前版本是否是升级序列中的 binary 版本。
      * `value`：当值为 `true` 时，和 `when_come_from` 属性联合使用。
      * `when_come_from`：是一个列表，当 `when_come_from` 未定义时，表示任何版本升级到目标版本时，都要先升级到当前 binary 版本；当 `when_come_from` 已定义，表示列表中的版本号升级到目标版本时，需要先升级到当前 binary 版本。例如：示例中的 `4.0.0.0` 和 `4.1.0.0-100000982023031415` 版本的 OceanBase 集群在升级到 V4.2.0.0 版本时，都要先升级到 V4.1.0.1(binary) 版本。

    `oceanbase_upgrade_dep.yml` 文件中 OceanBase 集群各个版本升级依赖关系示例如下：
    ...
    * V4.0.0.0 > V4.1.0.0 > V4.1.0.1(binary) > V4.2.0.0
    * V4.1.0.0-100000982023031415 > V4.1.0.0 > V4.1.0.1(binary) > V4.2.0.0
    * V4.1.0.0 > V4.1.0.1 > V4.2.0.0
    * V4.1.0.1 > V4.2.0.0

    <main id="notice" type='notice'>
      <h4>注意</h4>
      <p>如果升级路径中存在 barrier 版本，那么您还需获取升级路径中所有 barrier 版本的 OceanBase RPM 包。</p>
    </main>
...
以下将以 1:1:1 部署模式的 OceanBase 集群为例介绍升级流程。

* 当前 OceanBase 集群版本是 V4.0.0.0 版本，升级流程如下：

  1. 您需要先将 Zone1 从 V4.0.0.0 版本升级到 V4.1.0.1(binary) 版本，再将 Zone2 从 V4.0.0.0 版本升级到 V4.1.0.1(binary) 版本，然后将 Zone3 从 V4.0.0.0 版本升级到 V4.1.0.1(binary) 版本；此时，整个集群已经从 V4.0.0.0 版本升级到了 V4.1.0.1（binary）版本。
  2. 您需再次按照 Zone 顺序依次将 Zone1、Zone2、Zone3 从 V4.1.0.1（binary）版本升级到 V4.2.0.0，等到所有 Zone 均升级到 V4.2.0.0，集群升级就完成了。
...
* [enable_rereplication](../../../../../700.reference/800.configuration-items-and-system-variables/100.system-configuration-items/300.cluster-level-configuration-items/200.enable_rereplication.md)
