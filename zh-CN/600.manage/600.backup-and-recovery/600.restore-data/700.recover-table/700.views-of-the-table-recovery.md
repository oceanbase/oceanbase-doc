...
`CDB_OB_RECOVER_TABLE_JOB` 用于展示表级恢复任务的任务记录。
...
`CDB_OB_IMPORT_TABLE_JOBS` 展示租户级别的导入任务的记录。是表级恢复任务的子任务。
...
`CDB_OB_IMPORT_TABLE_JOB_HISTORY` 展示租户级别的导入任务的历史记录。
...
`CDB_OB_IMPORT_TABLE_TASKS` 展示表级别的导入任务的记录。
...
`CDB_OB_IMPORT_TABLE_TASK_HISTORY` 展示表级别的导入任务的历史记录。
