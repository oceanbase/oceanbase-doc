...
2. （可选）执行 `ALTER SYSTEM RESTORE...PREVIEW` 命令，指定原始数据备份路径和日志归档路径及期望恢复的时间。
   ...
   * `uri`：指定原始数据备份路径 `backup_data_dest` 与日志归档路径 `log_archive_dest`。该值至少包含一条数据备份路径和一条日志归档路径，路径之间使用英文逗号（,）分隔。如果在备份过程中执行了归档日志备份（即配置了 `BACKUP_ARCHIVE_DEST`），则还需要在 `uri` 中补充归档备份目的端路径 `backup_archive_dest`，格式为 `backup_data_dest,log_archive_dest,backup_archive_dest`，多条路径必须按照该顺序排列。
     ...
     * 如果租户执行过带 `DELETE INPUT` 参数的归档备份，则格式为 `A,log_archive_dest,backup_archive_dest`。
      ...
      <h4>说明</h4>
      <p>对于 V4.4.2 版本，从 V4.4.2 BP3 版本开始，支持在 <code>uri</code> 中指定归档备份目的端路径 <code>backup_archive_dest</code>。只要配置了 <code>BACKUP_ARCHIVE_DEST</code>，即可始终在 <code>uri</code> 中携带归档备份路径，系统会按需读取对应的 Piece。当旧的 Piece 位于归档备份目的端、对应的日志归档目的端中的副本已被清理，新 Piece 仍在日志归档目的端时，恢复任务可同时从两个目的端读取日志。</p>
      </main>
...
4. 执行 `ADD RESTORE SOURCE` 命令，分别加载恢复所需的 backup_set 和 archivelog_piece。
   ...
   * `restore_path`：填写待加载的 backup_set、archivelog_piece 或备份归档目的端路径。可以是一个路径，也可以是多个路径。

      <main id="notice" type='notice'>
      <h4>注意</h4>
      <p>如果待加载的 backup_set、archivelog_piece 或备份归档数据存放在对象存储上，则要求每条路径中的 <code>host</code>、<code>access_id</code> 以及 <code>access_key</code> 等参数信息均相同。</p>
      </main>
   ...
   * 该语句在同一个会话中可以连续多次执行。在同一会话中多次执行时，系统会增量添加到恢复源列表中。
   ...
   3. 如果在备份过程中执行了归档日志备份（即配置了 `BACKUP_ARCHIVE_DEST`），还需加载备份归档目的端路径。
...
5. （可选）加载 backup_set、archivelog_piece 或备份归档路径过程中，如果发现输入了错误的路径，可以执行以下语句来撤销之前的输入后，再重新加载正确的恢复路径。
...
6. 所有路径加载成功后，调用恢复命令。
   ...
   <li>执行恢复命令后，无论是否恢复成功，之前加载的恢复源列表均会被清空。如果需要重新调用恢复命令，则需要重新加载恢复所需的 backup_set、archivelog_piece 以及备份归档路径。</li>
   <li>归档备份目的端仅保存已冻结（FROZEN）的 Piece，恢复到较新的位点时，通常仍需提供包含新 Piece 和 ACTIVE 尾部的日志归档路径，建议同时加载日志归档路径和归档备份路径。恢复期间，如果任意必要的 Piece 在日志归档路径和归档备份路径中均不存在，恢复将失败。</li>
   </ul>
   </main>
