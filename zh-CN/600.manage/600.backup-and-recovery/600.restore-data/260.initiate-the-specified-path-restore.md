...
2. （可选）执行 `ALTER SYSTEM RESTORE...PREVIEW` 命令，指定原始数据备份路径和日志归档路径及期望恢复的时间。
   ...
   ```sql
   -- 全量恢复示例
   ALTER SYSTEM RESTORE FROM 'uri' UNTIL { TIME='timestamp' | SCN=scn } WITH 'pool_list=pool_name&method=quick'  PREVIEW;
   ...
   * 指定原始数据备份路径和日志归档路径，并且期望恢复到恢复到指定 scn `1712650554000909004`。
      ...
      // 快速恢复
      obclient [oceanbase]> ALTER SYSTEM RESTORE FROM 'file:///ob_backup/data, file:///ob_backup/archive' UNTIL SCN = 1712650554000909004 WITH 'pool_list=pool_name&method=quick' PREVIEW;
      ```
   ...
   * 指定原始数据备份路径和日志归档路径，并且期望恢复到指定时间戳 `2024-04-09 16:15:54`。
      ...
      // 快速恢复
      obclient [oceanbase]> ALTER SYSTEM RESTORE FROM 'file:///ob_backup/data, file:///ob_backup/archive' UNTIL TIME = '2024-04-09 16:15:54' WITH 'pool_list=pool_name&method=quick' PREVIEW;
      ```
