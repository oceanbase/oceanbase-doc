...
3. （可选）执行 `SHOW RESTORE PREVIEW` 命令，展示恢复到指定时间所需的数据备份集和日志归档 Piece。
   ...
   例如，上述查询结果中，需要使用 `BACKUP_PIECE` 为 `1` 的日志归档 Piece 进行恢复，而该文件已经被手动转移到 `oss://test_bucket/archive/piece_1` 上，则在使用 `ADD RESTORE SOURCE` 命令加载备份路径时，需要选择新的存放路径 `oss://test_bucket/archive/piece_1`。

4. 执行 `ADD RESTORE SOURCE` 命令，分别加载恢复所需的数据备份集和日志归档 Piece。
   ...
   * `restore_path`：填写待加载的备份集或日志归档 Piece 的路径。可以是一个路径，也可以是多个路径。
      ...
      <main id="notice" type='notice'>
      <h4>注意</h4>
      <p>如果待加载的数据备份集或日志归档存放在对象存储上，则要求每个数据备份集或日志归档 Piece 路径中的 <code>host</code>、<code>access_id</code> 以及 <code>access_key</code> 等参数信息均相同。</p>
      </main>
...
5. （可选）加载备份集或日志归档 Piece 过程中，如果发现输入了错误的路径，可以执行以下语句来撤销之前的输入后，再重新加载正确的恢复路径。
...
6. 所有路径加载成功后，调用恢复命令。
   ...
   <li>执行恢复命令后，无论是否恢复成功，之前加载的恢复源列表均会被清空。如果您需要重新调用恢复命令，则需要重新加载恢复所需的数据备份集和日志归档 Piece。</li>
   </ul>
   </main>
   ...
   * `TIME='timestamp' | SCN=scn`：指定恢复到的时间戳或 SCN。支持不指定时间的恢复，不显示指定恢复时间时，系统会根据加载的备份集和日志归档 Piece 恢复到能恢复的最新位点。如果显示指定时间，建议与 `ALTER SYSTEM RESTORE...PREVIEW` 命令中指定的时间一致。
