...
当数据备份集或日志归档 Piece 由于业务原因不在原来的目的端下时，您可以通过先加载该数据备份集和日志归档 Piece 的路径再发起恢复命令的方式来恢复租户。

## 前提条件

* 请确认已获取待恢复的数据备份集和日志归档 Piece 的路径。
...
2. 通过 `ADD RESTORE SOURCE` 语句分别加载待恢复的数据备份集路径和日志归档 Piece 路径。
   ...
   * `restore_path` 用于填写待加载的备份集或日志归档 Piece 的路径列表，即可以是一个路径，也可以是多个路径。
   
     填写路径时，数据备份集路径和归档日志 Piece 路径可以与源备份路径不一致。
...
3. 路径加载成功后，执行以下语句，调用恢复命令。
   ...
   * `restore_option`：支持指定 `pool_list`、`locality`、`primary_zone`、`concurrency`，不同参数之间通过 `&` 分隔。在指定 `locality` 和 `primary_zone` 时，建议尽量与源租户保持同构。如果不同构，待租户恢复后激活为主租户可能会产生负载均衡操作，影响性能。
   ...
   ```sql
   obclient> ALTER SYSTEM RESTORE mysql UNTIL '2020-06-01 00:00:00' WITH 'pool_list=restore_pool&locality=F@z1,F@z2,F@z3&primary_zone=z1';
   ```
