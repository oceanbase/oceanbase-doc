...
4. 执行 `ADD RESTORE SOURCE` 命令，分别加载恢复所需的数据备份集和日志归档 Piece。
   ...
   * `restore_path` 填写待加载的备份集或日志归档 Piece 的路径。可以是一个路径，也可以是多个路径。
      ...
      如果待加载的数据备份集或日志归档存放在对象存储上，则要求每个数据备份集或日志归档 Piece 路径中的 `host`、`access_id `以及 `access_key` 等参数信息均相同。
   ...
   * 该语句在同一个会话中可以连续多次执行。在同一会话中多次执行时，系统会增量添加到恢复源列表中。
   ...
   2. 加载待恢复的日志归档 Piece。
    ...
    语句执行成功后，如果发现输入了错误的路径，可以执行以下语句来撤销之前的输入后，再重新加载正确的恢复路径。
...
5. 所有路径加载成功后，执行以下语句，调用恢复命令。
   ...
   <li>执行恢复命令后，无论是否成功恢复出租户，之前加载的恢复源列表均会被清空。如果您需要重新调用恢复命令，则需要重新加载恢复所需的数据备份集和日志归档 Piece。</li>
   </ul>
   </main>
