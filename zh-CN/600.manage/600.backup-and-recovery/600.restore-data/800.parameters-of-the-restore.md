...
对于指定时间戳（timestamp）的恢复方式，OceanBase 数据库将根据备份集中最新记录的时区将时间戳转换成对应的 SCN，此处的时区来源于备份租户的系统变量 [system_time_zone](../../../700.reference/800.configuration-items-and-system-variables/200.system-variable/300.global-system-variable/12600.system_time_zone-global.md)，其默认值为 CST（即中国标准时间，+8:00），并保存在备份集的 `locality_info.obbak` 文件中。

例如，假设备份时租户的时区是 CDT（即美国中部夏令时区，-5:00），如果用户恢复时指定的时间戳为 `2025-08-01 10:00:00`，则实际恢复到的时间点是 `2025-08-01 15:00:00`（标准 UTC 时间）对应的 SCN 值。

更多参数的的详细说明，参见 [ALTER SYSTEM - RESTORE](../../../700.reference/500.sql-reference/100.sql-syntax/100.system-tenants/200.alter-system/3300.restore.md)。
