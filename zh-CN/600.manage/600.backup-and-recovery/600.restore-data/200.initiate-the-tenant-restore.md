...
4. 执行以下语句，开始执行恢复任务。
   ...
   * `uri`：指定备份时设置的数据备份路径 `backup_data_dest` 与日志归档路径 `log_archive_dest`。该值至少包含一条数据备份路径和一条日志归档路径，路径之间使用英文逗号（,）分隔。如果在备份过程中执行了归档日志备份（即配置了 `BACKUP_ARCHIVE_DEST`），则还需要在 `uri` 中补充归档备份目的端路径 `backup_archive_dest`，格式为 `backup_data_dest,log_archive_dest,backup_archive_dest`，多条路径必须按照该顺序排列。

     * 如果发起数据备份时通过 `TO` 语法指定了备份目标路径（如备份到指定目录 A），则 `uri` 中的数据备份路径需填写备份时指定的备份目标路径（A），而非系统参数 `data_backup_dest` 配置的默认路径，格式仍为 `A,log_archive_dest`。
     * 如果租户执行过带 `DELETE INPUT` 参数的归档备份，则格式为 `A,log_archive_dest,backup_archive_dest`。

      <main id="notice" type='explain'>
      <h4>说明</h4>
      <p>对于 V4.4.2 版本，从 V4.4.2 BP3 版本开始，支持在 <code>uri</code> 中指定归档备份目的端路径 <code>backup_archive_dest</code>。只要配置了 <code>BACKUP_ARCHIVE_DEST</code>，发起恢复时即可始终在 <code>uri</code> 中携带归档备份路径，恢复期间系统会按需读取对应的 Piece。当旧的 Piece 位于归档备份目的端、对应的日志归档目的端中的副本已被清理，新 Piece 仍在日志归档目的端时，恢复任务可同时从两个目的端读取日志。</p>
      </main>

      <main id="notice" type='notice'>
      <h4>注意</h4>
      <p>归档备份目的端仅保存已冻结（FROZEN）的 Piece，恢复到较新的位点时，通常仍需提供包含新 Piece 和 ACTIVE 尾部的日志归档目的端，建议恢复时同时提供日志归档路径和归档备份路径。恢复期间，如果任意必要的 Piece 在日志归档目的端和归档备份目的端均不存在，恢复将失败。</p>
      </main>

   * `pool_list`：填写用户的资源池。多个资源池之间用英文逗号（,）分隔。

   * `locality`、`primary_zone`：指定新租户副本分布的 Locality 信息以及Leader 副本的偏好位置。可选。有关这些参数的详细说明，参见 [ALTER SYSTEM - RESTORE](../../../700.reference/500.sql-reference/100.sql-syntax/100.system-tenants/200.alter-system/3300.restore.md)。
   ...
   * `concurrency`：指定数据恢复的并发度，可选。默认等于该租户被分配的 Unit 的 `MAX_CPU` 数。

   * `kms_encrypt`：可选，为 `true` 时表示在恢复时需要使用在恢复前指定的 `kms_encrypt_info`。
   ...
   * 从 OSS 备份路径、日志归档路径和归档备份路径恢复租户 `mysql` 到指定时间戳 `2020-06-01 00:00:00`，指定资源池为 `restore_pool`，以全量恢复方式进行恢复。
...
tab NFS
   ...
   * 从数据备份路径、日志归档路径与归档备份目的端路径恢复租户 `mysql` 恢复到当前最新的归档位点, 指定资源池为 `restore_pool`。
   ...
   tab AWS S3
      ...
      从 S3 数据备份路径、S3 日志归档路径与 S3 归档备份目的端路径恢复租户 `mysql` 到指定时间戳 `2024-01-15 00:00:00`, 指定资源池为 `restore_pool`，以全量恢复方式进行恢复。
   ...
   * 从 OBS 数据备份路径、OBS 日志归档路径与 OBS 归档备份目的端路径恢复租户 `mysql` 到指定时间戳 `2023-06-01 00:00:00`，指定资源池为 `restore_pool`，以全量恢复方式进行恢复。
   ...
   * 从 GCS 数据备份路径、GCS 日志归档路径与 GCS 归档备份目的端路径恢复租户 `mysql` 到指定时间戳 `2023-06-01 00:00:00`，指定资源池为 `restore_pool`，以全量恢复方式进行恢复。
   ...
   * 从 COS 数据备份路径、COS 日志归档路径与 COS 归档备份目的端路径恢复租户 `mysql` 到指定时间戳 `2023-06-01 00:00:00`，指定资源池为 `restore_pool`，以全量恢复方式进行恢复。
...
5. （可选）在待恢复租户对应的 Meta 租户创建成功后，如果需要加速恢复，可以通过配置项 `ha_high_thread_score` 配置恢复任务的并发线程数。
   ...
   1. 通过 `oceanbase.DBA_OB_TENANTS` 视图查看待恢复租户对应的 Meta 租户是否创建成功。
      ...
      查询结果中，`TENANT_NAME` 列中以 `META$` 开头的租户即为 Meta 租户，当 Meta 租户的状态为 `NORMAL` 时，则表示 Meta 租户创建成功。
...
* [发起归档日志备份](../300.log-archive/310.archival-data-backup.md)
