...
4. 执行以下语句，开始执行恢复任务。
   ...
   * 从 OBS 备份路径与日志归档路径恢复租户 `mysql` 到指定时间戳 `2020-06-01 00:00:00`，指定资源池为 `restore_pool`，以全量恢复方式进行恢复。   
   ...
   * 从 GCS 备份路径与日志归档路径恢复租户 `mysql` 到指定时间戳 `2020-06-01 00:00:00`，指定资源池为 `restore_pool`，以全量恢复方式进行恢复。   
      ...
      ```
   * 从 COS 备份路径与日志归档路径恢复租户 `mysql` 到指定时间戳 `2020-06-01 00:00:00`，指定资源池为 `restore_pool`，以全量恢复方式进行恢复。   
