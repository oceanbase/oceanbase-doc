...
* 恢复任务完成后，如果您是从低版本的备份数据恢复到高版本集群中的场景，还需要对租户进行升级，具体操作请参见 [恢复完成后升级租户](510.update-the-tenant-after-the-restore.md)。

* 恢复成功后，物理恢复流程与物理备库统一，物理恢复后的租户为备租户，后续该租户可作为备租户提供相关服务，也可转为主租户提供服务。作为备租户继续日志归档从源租户回放日志的详细操作参见 [备租户分段回放日志](../600.restore-data/700.recover-the-standby-tenant.md)；将备租户转为主租户的详细操作请参见 [备租户转为主租户](../600.restore-data/600.active-standby-tenant.md)。
