...
4. 执行以下语句，开始执行恢复任务。
   ...
   * `restore_option` 支持指定 `pool_list`、`locality`、`primary_zone`、`concurrency`、`kms_encrypt` 等参数, 不同参数之间通过 `&` 分隔。在指定 `locality` 和 `primary_zone` 时，建议尽量与源租户保持同构。如果不同构，待租户恢复后激活为主租户可能会产生负载均衡操作，影响性能。
      ...
      * `kms_encrypt`：为 `true` 时，表示在恢复时需要使用在恢复前指定的 `kms_encrypt_info`。
   ...
   * OSS
      ...
      从 OSS 备份路径与 OSS 日志归档路径恢复租户 `mysql` 到指定时间戳 `2020-06-01 00:00:00`, 指定资源池为 `restore_pool`。
