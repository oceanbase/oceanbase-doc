...
本节主要介绍如何通过命令的方式进行租户级物理恢复，目前 OceanBase 数据库支持租户级别的基于时间点的全量恢复和快速恢复。全量恢复与快速恢复的区别以及快速恢复的使用限制请参见 [物理备份与恢复概述](../100.overview-of-physical-backup-and-recovery.md)。
...
* 恢复出来的租户在使用期间，仍然要保证该租户的所有 Unit 所在的 OBServer 节点均能访问到源租户数据备份中的数据，若数据备份使用的是备份介质为 NFS，则要求其不能是 UMOUNT 状态。
