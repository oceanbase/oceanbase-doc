...
4. 执行以下语句，开始执行恢复任务。
   ...
   * COS
      ...
      从 COS 备份路径 `cos://oceanbase-test-appid/backup?host=cos.ap-nanjing.myqcloud.com&access_id=xxx&access_key=xxx&appid=xxx` 与 COS 日志归档路径 `cos://oceanbase-test-appid/backup/archive?host=cos.ap-nanjing.myqcloud.com&access_id=xxx&access_key=xxx&appid=xxx` 恢复租户 `mysql` 到指定时间戳 `2020-06-01 00:00:00`, 指定资源池为 `restore_pool`。

      ```sql
      ALTER SYSTEM RESTORE mysql FROM 'cos://oceanbase-test-appid/backup?host=cos.ap-nanjing.myqcloud.com&access_id=xxx&access_key=xxx&appid=xxx, cos://oceanbase-test-appid/backup/archive?host=cos.ap-nanjing.myqcloud.com&access_id=xxx&access_key=xxx&appid=xxx' UNTIL TIME='2020-06-01 00:00:00' WITH 'pool_list=restore_pool';
      ```
