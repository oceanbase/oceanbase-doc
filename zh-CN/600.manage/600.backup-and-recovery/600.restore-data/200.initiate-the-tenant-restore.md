...
3. 执行以下语句，开始执行恢复任务。
   ...
   * S3
      ...
      从 S3 备份路径 `s3://oceanbase-test-bucket/backup/data?host=s3.<region>.amazonaws.com&access_id=xxx&access_key=xxx&s3_region=xxx` 与 S3 日志归档路径 `s3://oceanbase-test-bucket/backup/archive?host=s3.<region>.amazonaws.com&access_id=xxx&access_key=xxx&s3_region=xxx` 恢复租户 `mysql` 到指定时间戳 `2024-01-15 00:00:00`, 指定资源池为 `restore_pool`。
