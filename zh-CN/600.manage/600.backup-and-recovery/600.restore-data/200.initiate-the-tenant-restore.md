...
对于 V4.2.1 版本，OceanBase 数据库从 V4.2.1 BP10 版本开始支持在数据备份的同时备份租户级配置项，但备份的租户级配置项仅用于为用户提供配置参考，物理恢复时不会直接恢复到租户中。有关备份配置项的详细介绍，参见 [备份配置项](../400.data-backup/320.backup-parameters.md)。
...
4. 执行以下语句，开始执行恢复任务。
   ...
   * `UNTIL [TIME|SCN]` 指用户指定的恢复终点，恢复到该位点为止，且包括该位点。当指定恢复到 `TIME` 或 `SCN` 时, 必须以 `=` 来连接指定的值。有关 `TIME` 或 `SCN` 的选取的详细说明，参见 [物理恢复相关参数介绍](../600.restore-data/800.parameters-of-the-restore.md)。
