...
按表恢复的整个过程包括辅助租户恢复和跨租户导入。任务的整体进度通过CDB_OB_RECOVER_TABLE_JOB视图查看。
我们可以通过CDB_OB_RECOVER_TABLE_JOB表中的 AUX_TENANT_NAME定位到 CDB_OB_RESTORE_PROGRESS中的临时租户恢复任务。
当临时租户恢复完成后，可以通过CDB_OB_IMPORT_TABLE_JOBS视图定位到导入任务的进度。

## 操作步骤

1. 使用 `root` 用户登录表待恢复的目标租户所在集群的 `sys` 租户。

2. 根据实际情况，分别执行以下语句，查看表恢复任务相关的进度。
   ...
   * 查看按表恢复任务的整体进度
     ...
     一条表级恢复命令对应两条任务记录: `sys` 租户的任务记录和目标租户的任务记录。`sys` 租户的任务记录主要记录表待恢复的目标租户的恢复进度，目标租户的任务记录主要记录自身的恢复进度。
