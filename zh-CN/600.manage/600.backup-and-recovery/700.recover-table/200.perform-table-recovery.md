...
4. 执行以下命令，恢复指定的表。
   ...
   * `remap_table_name_list`：重命名恢复后的表名。支持仅重命名表名，表所属的 Database 不变；也支持表名不变，仅重命名到其他 Database；还支持重命名表名，同时所属的 Database 重命名为其他 Database。源对象与重命名对象之间使用英文冒号（:）连接，具体格式的示例如下：
     ...
     * 将 `school` 下的所有表恢复到 `college` 中：<code>REMAP TABLE school.`*`:college.`*`</code>
     ...
     <h4>说明</h4>
     <p>对于 V4.2.1 BP3（不含该版本）之前版本，不支持重命名的 <code>database_name</code> 或 <code>table_name</code> 中含有特殊字符。对于 V4.2.1 BP3 及之后版本，如果重命名的 <code>database_name</code> 或 <code>table_name</code> 中含有特殊字符，则含特殊字符的 <code>database_name</code> 或 <code>table_name</code> 需要放在反引号（``）内。</p>
     </main>     
...
* 指定表的恢复结束后，您还需要在辅助租户所在的集群中执行以下命令，手动释放为辅助租户创建的资源池。
    ...
    ```shell
    obclient> DROP RESOURCE POOL restore_pool;
    ```
