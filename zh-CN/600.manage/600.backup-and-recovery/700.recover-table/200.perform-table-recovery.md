...
按表恢复有两种方式：全量恢复和快速恢复。由于物理恢复辅助租户采用全量恢复时，所有数据都要从备份介质恢复到辅助租户，这种恢复方式易产生以下两个问题：

1. 用户必须要在集群上预留充足的 CPU、内存和磁盘资源，以便支持临时的辅助租户的恢复。  
2. 将所有的数据从备份介质恢复到辅助租户的过程，需要消耗大量的时间和网络带宽。

为了解决上述两个问题，OceanBase 提供了快速恢复的恢复方式，适用于不同的恢复场景。快速恢复减少了表级恢复的临时辅助租户资源（主要指磁盘资源）、缩短了恢复需要时间。

当恢复命令被发起时，OceanBase 将根据 OceanBase 的部署模式（共享存储/无共享模式），自动选择合适的恢复辅助租户的方式：

* 如果是共享存储模式，则选择辅助租户的全量恢复方式。
* 如果是无共享模式，则选择辅助租户的快速恢复方式。
