...
<h4>注意</h4>
<p>在当前版本中，如果集恢复使用的备份集的数据版本号为 V4.3.5.0 及以上版本，则系统会自动使用快速恢复方式来执行表级恢复过程中辅助租户的恢复。如果恢复使用的备份集的数据版本号为 V4.3.5.0（不含）以下版本，则仍然使用全量恢复方式来执行辅助租户的恢复。恢复所使用的备份集的数据版本号可以通过查询视图 <code>CDB_OB_BACKUP_SET_FILES</code>（sys 租户）或 <code>DBA_OB_BACKUP_SET_FILES</code>（用户租户）中的 <code>TENANT_COMPATIBLE</code> 列来确认。</p>
</main>
