...
* 恢复表时，指定的表名需要与系统实际存储的表名一致。例如，Oracle 租户下创建表 `test`，而系统内部实际存储的表名为 `TEST`，故在恢复表时需要指定表名为 `TEST`，否则系统会报错，提示表不存在。
...
4. 执行以下命令，恢复指定的表。
   ...
   * `table_name_list`：需要恢复的表，格式为 `database_name.table_name1,database_name.table_name2,...`，多个表之间使用英文逗号（,）分隔。
     ...
     * `table_name` 需要与系统实际存储的表名一致。例如，Oracle 租户下创建表 `test`，而系统内部实际存储的表名为 `TEST`，故在恢复表时需要指定表名为 `TEST`，否则系统会报错，提示表不存在。
