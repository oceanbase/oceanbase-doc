...
<ul><li>如果集群使用的是无共享（Shared-Nothing，SN）模式，且待恢复的数据备份的版本为 V4.3.5 及以上版本，则系统会自动使用快速恢复方式来执行表级恢复过程中辅助租户的恢复。但是，如果集群使用的是 SN 模式，而待恢复的数据备份的版本为 V4.3.5（不含）以下版本，则仍然使用全量恢复方式来执行辅助租户的恢复。</li>
