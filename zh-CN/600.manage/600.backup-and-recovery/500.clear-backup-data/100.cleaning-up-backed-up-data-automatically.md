...
* 对于备份介质为 NFS 或兼容 S3 协议的对象存储（OBS、GCS 等） 的场景，在进行备份数据的清理时，系统会直接删除满足要求的备份文件；对于备份目的端为 S3/COS(以 S3 协议访问) 的场景，其备份文件的清理方式取决于 `data_backup_dest` 以及 `log_archive_dest` 中 `delete_mode` 参数的值，该参数的详细介绍，参见 [SET LOG_ARCHIVE_DEST](../../../700.reference/500.sql-reference/100.sql-syntax/100.system-tenants/200.alter-system/150.set-log-archive-dest.md) 和 [SET DATA_BACKUP_DEST](../../../700.reference/500.sql-reference/100.sql-syntax/100.system-tenants/200.alter-system/3500.set-data-backup-dest.md)。

* 对于路径属性中配置了 `enable_worm=true`（对于 V4.3.5 版本，该参数从 V4.3.5 BP2 版本开始支持，目前仅阿里云 OSS 支持该设置），且对应路径的 Bucket 设置了保留策略的场景，在进行备份数据的自动清理时，系统将按照 `RECOVERY_WINDOW` 指定的时间选择满足条件的备份集进行清理，但最终能否成功清理与该路径对应的清理模式（`delete_mode` 参数的值）以及对应 Bucket 的保留策略有关。例如，某一备份集在 `RECOVERY_WINDOW` 指定的时间之外，但仍然在 WORM（Write Once Read Many）保留期内，且清理模式为 `delete`，则清理该备份集将会失败，需要等到该 Object 的保留期到期后，才能清理该备份集。

  如果清理模式为 `tagging`，则 OceanBase 数据库会将 `RECOVERY_WINDOW` 指定时间外的备份集均打上 tag，最终由对象存储在生命周期结束并且过了保留期后执行删除。
