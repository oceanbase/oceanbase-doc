...
* 特别地，对于路径属性中配置了 `enable_worm=true` 参数（对于 V4.3.5 版本，该参数从 V4.3.5 BP2 版本开始支持，目前仅阿里云 OSS 支持该设置），且对应路径的 Bucket 设置并锁定了保留策略的场景，在进行备份数据的自动清理时，系统将按照 `RECOVERY_WINDOW` 指定的时间选择满足条件的备份集进行清理，但最终能否成功清理与该路径对应的清理模式（`delete_mode` 参数的值）以及对应 Bucket 的保留策略有关。
