...
* 对于当备份介质为 OSS 的场景，在进行备份数据的自动清理时，系统将按照 `RECOVERY_WINDOW` 指定的时间选择满足条件的备份集进行清理，但最终能否成功清理与该路径对应的清理模式（`delete_mode` 参数的值）以及对应 Bucket 的保留策略有关。例如，如果备份集在 `RECOVERY_WINDOW` 指定的时间之外，但仍然在 WORM（Write Once Read Many）保留期内，且清理模式为 `delete`，则清理该备份集将会失败，需要等到该 Object 的保留期到期后，才能清理该备份集；如果备份集在 `RECOVERY_WINDOW` 指定的时间之外，并且过了 WORM 保留期，但清理模式为 `tagging`，则清理该备份集也会失败，需要在该 Object 的生命周期结束后，才能成功清理该备份集。
