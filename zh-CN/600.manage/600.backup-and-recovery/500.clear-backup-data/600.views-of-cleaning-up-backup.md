...
| TYPE                      |   清理类型：<ul><li>`DELETE OBSOLETE BACKUP`：过期清理</li> <li>`DELETE BACKUP ALL`：清空路径中的所有备份或归档数据</li> <li>`DELETE BACKUPSET`：清理指定的数据备份集</li> <li>`DELETE ARCHIVELOG_PIECE`：清理指定的日志归档 Piece </li></ul> |
| PARAMETER                 |   清理参数，描述不同清理类型的具体参数信息 |
