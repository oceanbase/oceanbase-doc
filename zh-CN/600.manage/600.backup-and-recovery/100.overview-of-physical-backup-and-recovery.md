...
在当前版本中，无共享模式（SN 模式）和共享存储模式（SS 模式）默认均使用 obdal 来访问对象存储。obdal 为基于 OpenDAL 开发的对象存储统一框架。有关 OpenDAL 的详细介绍，参见 [OpenDAL](https://github.com/oceanbase/opendal)。

此外，如果需要使用对象存储的 SDK，对象存储的 SDK 版本与 observer 版本对应关系如下：
