...
OceanBase 数据库的备份恢复模块提供了备份、备份校验、恢复、清理等功能。
...
日志归档是指日志数据的自动归档功能，OBServer 节点会定期将日志数据归档到指定的备份路径。这个动作是全自动的，不需要外部定期触发。日志归档服务开启后，数据字典也会被归档。
...
备份校验从校验粒度，支持整个备份和归档路径的校验、指定备份集或日志归档 Piece 的校验以及跨集群指定路径下的校验。
...
OceanBase 数据库提供了备份数据的自动和手动清理能力，以便用户根据业务场景灵活处理。

* 自动清理：基于恢复窗口（recovery_window）的自动清理机制。用户设置备份数据的过期时间后，系统会定时触发清理任务，清理过期的备份集和数据归档 Piece。

* 手动清理：手动清理可以分为两类：第一类是手动清理指定的备份集或日志归档 Piece；第二类是手动清空整个路径（数据备份路径或日志归档路径）下所有的备份集或数据归档 Piece。

## 备份介质要求

OceanBase 数据库当前版本支持阿里云 OSS、NFS、Azure Blob、AWS S3 以及兼容 S3 协议的对象存储（例如：华为 OBS、Google GCS、腾讯云 COS） 等备份介质，部分备份介质需要满足一些基本要求才能使用。
...
* `backup_set_1_full`：该目录表示的是一个数据备份集，`backup_set_1` 表示该目录集的 `backup_set_id` 为 `1`；目录名结尾为 `full` 表示全量备份，为 `inc` 表示增量备份。每一次数据备份都会生成一个对应的备份集，数据备份结束后，该备份集就不会再修改了。
...
* `piece_d1002r1p1`：日志归档的 Piece 目录，目录命名格式为 `piece_DESTID_ROUNDID_PIECEID`。其中，`DESTID` 指的是 `log_archive_dest` 对应的 id；`ROUNDID` 指的是日志归档 Round 的 id，是一个单调递增的整数；`PIECEID` 指的是日志归档 Piece 的 `piece_id`，也是一个单调递增的整数。
...
已与 OceanBase 数据库完成认证的备份恢复生态伙伴，参见 [产业生态伙伴](https://www.oceanbase.com/partner/product-ecology)。

在 [产业生态伙伴](https://www.oceanbase.com/partner/product-ecology) 页面的 **方案类型选择** 中，选择 **数据集成与备份** > **备份恢复**，即可查看目前与 OceanBase 数据库完成认证的备份恢复生态伙伴列表，以及认证的版本情况和证书的有效期限。
