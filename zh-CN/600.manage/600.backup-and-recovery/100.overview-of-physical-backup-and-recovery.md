...
OceanBase 数据库当前版本除了支持 OSS、NFS 等备份介质以外，从 V4.2.1 BP7 版本开始，还支持 AWS S3 以及兼容 S3 协议的对象存储（例如：华为 OBS、Google GCS、腾讯云 COS），提供了备份、恢复、清理三大功能。
...
* 全部用户表数据
  ...
  <h4>说明</h4>
  <p>数据备份会备份系统变量，且对于 V4.2.1 BP10 及以上版本，还会备份租户级配置项，但不会备份集群级配置项以及私有系统表数据。</p>
  </main>
...
OceanBase 数据库当前版本除了支持阿里云 OSS 和 NFS 等备份介质以外，从 V4.2.1 BP7 版本开始，还支持 AWS S3 以及兼容 S3 协议的对象存储（例如：华为 OBS、Google GCS、腾讯云 COS），部分备份介质需要满足一些基本要求才能使用。
...
|-----------------------|------------|--------------|
| 4.2.1 BP6 及之前版本   | 3.9.2      | 不支持  |
| 4.2.1 BP7/8/9         | 3.9.2      | 1.11.156  |
| 4.2.1 BP10            | 3.11.2     | 1.11.156  |
