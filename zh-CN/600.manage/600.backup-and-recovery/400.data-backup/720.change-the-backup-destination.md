...
由于自动清理仅支持清理 `DATA_BACKUP_DEST` 和 `LOG_ARCHIVE_DEST` 中当前指定的路径下的备份和归档数据，如果需要更改备份目的端，则更改后，原备份路径下的数据只能通过手动方式进行清理。手动清理备份的详细操作，参见 [手动清理指定的备份或归档数据](../500.clear-backup-data/100.manual-backup-cleanup/200.cleaning-up-specified-backup-data.md) 和 [手动清空备份路径或归档路径](../500.clear-backup-data/100.manual-backup-cleanup/300.clear-the-backup_dest_or_archivelog_dest.md)。
