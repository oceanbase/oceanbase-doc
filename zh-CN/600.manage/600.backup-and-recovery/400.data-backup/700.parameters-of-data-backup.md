...
2. 分别执行以下语句，获取备份相关的参数配置信息。
   ...
   * 查看租户的备份参数信息。
      ...
      * `tagging` 模式：表示清理模式为对满足清理要求的备份文件设置 Tag，备份文件将仍然保留。
      ...
      此外，在该租户备份路径的参数中，`checksum_type=md5` 表示使用 MD5 算法校验备份文件的完整性；`enable_worm=true` 表示 OceanBase 数据库对该备份路径对应的写入及删除与该 Bucket 的 WORM 策略一致。
   ...
   * 查看租户下备份路径的相关信息。
      ...
      查询结果中，对于租户 ID 为 `1004` 的租户，其备份路径为 `oss://oceanbase-test-bucket/backup/data`；`checksum_type=md5` 表示使用 MD5 算法校验备份文件的完整性；`enable_worm=true` 表示 OceanBase 数据库对该备份路径的写入及删除与该 Bucket 的 WORM 策略一致。
