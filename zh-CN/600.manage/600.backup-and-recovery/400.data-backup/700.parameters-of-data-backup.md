...
2. 分别执行以下语句，获取备份相关的参数配置信息。
   ...
   * 查看租户下的备份参数信息（不含源端配置信息）。
      ...
      `sys` 租户通过视图 `oceanbase.CDB_OB_BACKUP_PARAMETER` 查看当前集群中所有租户的备份参数信息。用户租户可以通过 `oceanbase.DBA_OB_BACKUP_PARAMETER` 视图（MySQL 模式）或 `SYS.DBA_OB_BACKUP_PARAMETER` 视图（Oracle 模式）查看本租户的备份参数信息。
      ...
      * `tagging` 模式：表示清理模式为对满足清理要求的备份文件设置 Tag，备份文件将仍然保留。
          ...
          该模式下，当您清理备份文件时，对于满足清理要求的备份文件，系统会给这些文件设置标签，标签的 `key` 为 `"delete_mode"`，`value` 为 `"tagging"`，以便后续可以通过设置的标签在 OSS 或 S3 上对这些文件的生命周期进行管理。

   * 查看租户下备份路径的源端配置信息。

      `sys` 租户通过视图 `CDB_OB_BACKUP_STORAGE_INFO` 查看所有租户备份或归档路径的源端配置信息。用户租户可以通过 `oceanbase.DBA_OB_BACKUP_STORAGE_INFO` 视图（MySQL 模式）或 `SYS.DBA_OB_BACKUP_STORAGE_INFO` 视图（Oracle 模式）查看本租户备份或归档路径的源端配置信息。
      ...
      查询结果中，对于租户 ID 为 `1002` 的租户，其备份路径为 `file:///data/nfs/backup`，该备份径对应的源端配置为 `zone=zone1`。
