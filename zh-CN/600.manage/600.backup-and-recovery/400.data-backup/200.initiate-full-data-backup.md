...
2. （可选）设置租户的备份并发度。
   ...
   * 设置集群中所有租户的备份并发度
      ...
      ```sql
      obclient [(none)]>  ALTER SYSTEM SET ha_low_thread_score = 4 TENANT = all_user;
      ```

      或者

      ```sql
      obclient [(none)]>  ALTER SYSTEM SET ha_low_thread_score = 4 TENANT = all;
      ```
...
2. （可选）设置备份并发度。
   ...
   ```sql
   obclient [(none)]>  ALTER SYSTEM SET ha_low_thread_score = 4;
   ```
