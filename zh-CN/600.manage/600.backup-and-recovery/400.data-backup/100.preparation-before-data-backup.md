...
2. 配置备份目的端。
   ...
   * 该语句可以配置备份路径及源端配置。目前，OceanBase 数据库支持的备份介质为 NFS、阿里云 OSS、Azure Blob（在 V4.2.5 版本中，从 V4.2.5 BP6 版本开始支持）、AWS S3 以及兼容 S3 协议的对象存储，例如：华为 OBS、Google GCS、腾讯云 COS。部分备份介质可能需要满足一些基本要求才能使用，有关各备份介质的具体要求，参见 [物理备份与恢复概述](../100.overview-of-physical-backup-and-recovery.md) 中的 **备份介质要求**。

   * 指定备份路径的源端即是指定可以访问该备份路径的节点范围。有关备份路径源端配置的详细介绍及说明，参见 [SET DATA_BACKUP_DEST](../../../700.reference/500.sql-reference/100.sql-syntax/100.system-tenants/200.alter-system/3500.set-data-backup-dest.md)。

     备份路径的源端配置后，支持修改，具体修改操作，参见 [修改备份路径的源端配置](710.modify-the-configuration-of-backup-destination.md)。
   ...
   详细设置说明如下。
    ...
    当使用 OSS 作为备份介质时，除了设置备份路径，还可以通过 `delete_mode` 参数来配置备份文件的清理模式。`delete_mode` 参数当前支持 `delete` 模式和 `tagging` 模式。如果不配置 `delete_mode` 参数，则默认使用 `delete` 模式。

    此外，OSS 还支持通过 `checksum_type` 参数指定校验算法来校验备份数据的完整性，支持的取值为 `md5` 和 `no_checksum`。如果不显式配置 `checksum_type` 参数，则默认值为 `md5`。

    在 V4.2.5 版本中，从 V4.2.5 BP7 版本开始，OceanBase 数据库支持使用已配置了合规保留策略的阿里云 OSS Bucket 作为备份路径。在配置备份路径时，可以通过 `enable_worm` 参数来指定 OceanBase 数据库是否按照 Bucket 的保留策略（WORM 策略）对该路径执行写入及删除操作。该参数支持的取值为 `true` 和 `false`。如果不显式配置 `enable_worm` 参数，则默认为 `false`。设置 `enable_worm=true` 时要求同时显式配置 `checksum_type=md5`。该参数一旦设置后，不支持变更。
    ...
    * 使用 NFS 作为备份介质时，用户租户为本租户设置备份路径。
    ...
    示例中，`file://` 表示使用 NFS 作为备份介质，备份路径为 `file:///data/nfs/backup/data`。
    ...
    * 使用 AWS S3 作为备份介质时，用户租户为本租户设置备份路径，配置清理模式为 `delete`，指定校验算法为 `crc32`。
    ...
    示例中，`s3://` 表示使用 S3 作为备份介质，存储桶名 `oceanbase-test-bucket`，在存储桶中的路径是 `/backup/data`，同时使用 `?` 来分隔路径的其他参数，参数之间使用 `&` 分隔。`host` 用于设置 Amazon S3 服务的域名，`access_id` 和 `access_key` 用于设置 AWS 服务的访问密钥，`s3_region` 在使用 S3 作为备份介质时为必选参数，表示 S3 的存储桶所在的地域，清理模式为 `delete`，使用 CRC32 算法来校验备份数据的完整性。

    有关 AWS S3 路径格式的详细说明，参见 [附：AWS S3 路径格式说明](../1600.appendix-explanation-of-aws-s3-backupdest.md)。
    ...
    * 使用 OBS 作为备份介质时，用户租户为本租户设置备份路径。
    ...
    上述示例中，`s3://` 表示使用兼容 S3 协议的对象存储作为备份介质，存储桶名 `oceanbase-test-bucket`，在存储桶中的路径是 `/backup/data`，同时使用 `?` 来分隔路径的其他参数，参数之间使用 `&` 分隔。`host` 用于设置 OBS 服务的域名，`access_id` 和 `access_key` 用于设置 OBS 服务的访问密钥。
    ...
    * 使用 GCS 作为备份介质时，用户租户为本租户设置备份路径。
    ...
    上述示例中，`s3://` 表示使用兼容 S3 协议的对象存储作为备份介质，存储桶名 `oceanbase-test-bucket`，在存储桶中的路径是 `/backup/data`，同时使用 `?` 来分隔路径的其他参数，参数之间使用 `&` 分隔。`host` 用于设置 GCS 服务的域名，`access_id` 和 `access_key` 用于设置 GCS 服务的访问密钥。
    ...
    **使用 COS 作为备份介质的操作步骤及示例如下**：
    ...
    3. 设置备份路径，通过 `delete_mode` 参数来配置备份文件的清理模式。

        使用 COS 作为备份介质时，系统租户为指定租户 `mysql_tenant` 设置备份路径，配置为 `delete` 清理模式的示例如下：
        ...
        使用 COS 作为备份介质时，用户租户为本租户设置备份路径，配置为 `delete` 清理模式的示例如下：
        ...
        示例中，`s3://` 表示使用 S3 协议访问备份路径，存储桶名 `oceanbase-test`，在存储桶中的路径是 `/backup/archive`，同时使用 `?` 来分隔路径的其他参数，参数之间使用 `&` 分隔。`host` 用于设置 COS 的主机地址，即 endpoint（不带存储桶名的地址），`access_id` 和 `access_key` 用于设置 `COS` 的访问密钥。

    tab Azure Blob

    在 V4.2.5 版本中，从 V4.2.5 BP6 版本开始支持 Azure Blob 作为备份介质。
...
3. 配置成功后，可以通过视图确认详细的配置信息。
   ...
   `sys` 租户可以通过 `CDB_OB_BACKUP_PARAMETER` 视图来查看，用户租户可通过 `DBA_OB_BACKUP_PARAMETER` 来查看。
...
* 如果 `format` 文件已存在，则要求 `format` 文件的内容检验通过，配置项才能设置成功。否则，系统会报 `-9081` 的错误，提示 format 文件不匹配，format 文件的内容校验主要是检查集群、租户和备份目的端的介质类型等与当前操作的集群、租户及备份介质类型是否匹配。
...
3. 配置成功后，可以通过视图查看秘钥备份的路径。
   ...
   * 用户租户通过视图 `DBA_OB_BACKUP_STORAGE_INFO` 查看秘钥备份路径
     ...
     有关视图 `CDB_OB_BACKUP_STORAGE_INFO` 和 `DBA_OB_BACKUP_STORAGE_INFO` 中字段的详细说明，请参见 [CDB_OB_BACKUP_STORAGE_INFO](../../../700.reference/700.system-views/300.system-view-of-sys-tenant/200.dictionary-view-of-sys-tenant/6900.o-cdb_ob_backup_storage_info-of-sys-tenant.md) 和 [DBA_OB_BACKUP_STORAGE_INFO](../../../700.reference/700.system-views/400.system-view-of-mysql-mode/200.dictionary-view-of-mysql-mode/7000.o-dba_ob_backup_storage_info-of-mysql-mode.md)。
