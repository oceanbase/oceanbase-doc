...
2. 配置备份目的端。
   ...
   目前，OceanBase 数据库支持 NFS、阿里云 OSS、腾讯云 COS、S3（包括 AWS S3、华为 OBS 以及 Google GCS） 等作为备份目的端。

   <main id="notice" type='explain'>
   <h4>说明</h4>
   <p>由于大多数对象存储兼容了 S3 协议，对于兼容 S3 协议并满足 OceanBase 数据库要求的对象存储均可以作为 OceanBase 数据库的备份介质，并通过访问 S3 的方式来访问兼容的对象存储，例如：OBS 或 GCS 等都是通过 S3 协议访问。</p>
   </main>
   ...
   * 用户租户配置本租户的备份目的端
    ...
    * 使用 COS 作为备份介质时，用户租户为本租户设置备份目的端。
    ...
    与 OSS/COS 一样，兼容 S3 协议的对象存储也支持通过 `delete_mode` 参数来配置备份文件的清理模式，配置方法与 OSS/COS 一样。

    使用原生 S3 作为备份介质的配置示例如下：

    * 使用原生 S3 作为备份介质时，系统租户为指定租户 `mysql_tenant` 设置备份目的端并配置为 `tagging` 清理模式。
    ...
    * 使用原生 S3 作为备份介质时，用户租户为本租户设置备份目的端并配置为 `tagging` 清理模式。
    ...
    示例中，`s3://` 表示使用兼容 S3 协议的对象存储作为备份目的端介质类型，存储桶名 `oceanbase-test-bucket`，在存储桶中的路径是 `/backup/data`，同时使用 `?` 来分隔路径的其他参数，`host` 用于设置 Amazon S3 服务的域名，`access_id` 和 `access_key` 用于设置 AWS 服务的访问密钥，`s3_region` 在使用原生 S3 作为备份介质时为必选参数，表示 S3 的存储桶所在的地域，清理模式设置为 `tagging`。

    从上述示例可知，使用原生 S3 作为备份介质时，必须指定 `s3_region` 并且支持将 `delete_mode` 参数设置为 `tagging` 模式。与原生 S3 不同，使用 OBS 和 GCS 时，不需要指定 `s3_region` 并且不支持将 `delete_mode` 参数设置为 `tagging`，仅支持将 `delete_mode` 参数配置为 `delete` 清理模式。如果不显示配置 `delete_mode` 参数，默认就是 `delete` 清理模式。

    使用 OBS 和 GCS 作为备份介质的配置示例如下：
    ...
    * 使用 OBS 作为备份介质时，系统租户为指定租户 `mysql_tenant` 设置备份目的端并配置清理模式。
      ...
      示例中，`s3://` 表示使用兼容 S3 协议的对象存储作为备份目的端介质类型，存储桶名 `oceanbase-test-bucket`，在存储桶中的路径是 `/backup/data`，同时使用 `?` 来分隔路径的其他参数，`host` 用于设置 OBS 服务的域名，`access_id` 和 `access_key` 用于设置 OBS 服务的访问密钥。
    ...
    * 使用 GCS 作为备份介质时，系统租户为指定租户 `mysql_tenant` 设置备份目的端并配置清理模式。
      ...
      示例中，`s3://` 表示使用兼容 S3 协议的对象存储作为备份目的端介质类型，存储桶名 `oceanbase-test-bucket`，在存储桶中的路径是 `/backup/data`，同时使用 `?` 来分隔路径的其他参数，`host` 用于设置 GCS 服务的域名，`access_id` 和 `access_key` 用于设置 GCS 服务的访问密钥。
