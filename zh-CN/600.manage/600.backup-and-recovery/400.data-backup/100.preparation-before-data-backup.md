...
2. 配置备份目的端。
   ...
   * 用户租户配置本租户的备份目的端
   ...
   详细设置说明如下。
    ...
    示例中，`cos://` 表示使用 COS 作为备份目的端介质类型，存储桶名 `oceanbase-test-appid`，在存储桶中的路径是 `/backup`，同时使用 `?` 来分隔路径的其他参数，`host` 用于设置 COS 的主机地址，即 endpoint（不带存储桶名的地址），`access_id` 和 `access_key` 用于设置 `OSS` 的访问密钥，`appid` 为必选参数，用于腾讯云账号的 APPID。
