...
2. 配置备份目的端。
   ...
   * 该语句可以配置备份路径及源端配置。目前，OceanBase 数据库支持的备份介质为 NFS、阿里云 OSS、AWS S3 以及兼容 S3 协议的对象存储，例如：华为 OBS、Google GCS、腾讯云 COS。部分备份介质可能需要满足一些基本要求才能使用，有关各备份介质的具体要求，参见 [物理备份与恢复概述](../100.overview-of-physical-backup-and-recovery.md) 中的 **备份介质要求**。
   ...
   * 指定备份路径的源端即是指定可以访问该备份路径的节点范围。有关备份路径源端配置的详细介绍及说明，参见 [SET DATA_BACKUP_DEST](../../../700.reference/500.sql-reference/100.sql-syntax/100.system-tenants/200.alter-system/3500.set-data-backup-dest.md)。
    ...
    此外，从 V4.2.1 BP10 版本开始，OSS 还支持通过 `checksum_type` 参数指定校验算法来校验备份数据的完整性，支持的取值为 `md5` 和 `no_checksum`。如果不显式配置 `checksum_type` 参数，则默认值为 `md5`。
    ...
    * 使用 OSS 作为备份介质时，系统租户为指定租户 `mysql_tenant` 设置备份路径，配置备份文件的清理模式，同时指定校验算法为 `md5`。
      ...
      ```sql
      obclient> ALTER SYSTEM SET DATA_BACKUP_DEST='oss://oceanbase-test-bucket/backup/?host=***.aliyun-inc.com&access_id=***&access_key=***&delete_mode=delete&checksum_type=md5' TENANT = mysql_tenant;
      ```
    ...
    * 使用 OSS 作为备份介质时，用户租户为本租户设置备份路径，配置备份文件的清理模式，同时指定校验算法为 `md5`。
      ...
      ```sql
      obclient> ALTER SYSTEM SET DATA_BACKUP_DEST='oss://oceanbase-test-bucket/backup/?host=***.aliyun-inc.com&access_id=***&access_key=***&delete_mode=delete&checksum_type=md5';
      ```

    示例中，`oss://` 表示使用 OSS 作为备份介质，存储桶名 `oceanbase-test-bucket`，在存储桶中的路径是 `/backup`，同时使用 `?` 来分隔路径的其他参数，`host` 用于设置存储桶的主机地址，`access_id` 和 `access_key` 用于设置 `OSS` 的访问密钥，清理模式为 `delete`，使用 MD5 算法来校验备份数据的完整性。
    ...
    * 使用 NFS 作为备份介质时，用户租户为本租户设置备份路径的示例如下：
    ...
    与 OSS 一样，AWS S3 也支持通过 `delete_mode` 参数来配置备份文件的清理模式，配置方法与 OSS 一样。同样，从 V4.2.1 BP10 版本开始，AWS S3 也支持通过 `checksum_type` 参数指定校验算法来校验备份数据的完整性，支持的取值为 `md5` 和 `crc32`。如果不显式配置 `checksum_type` 参数，则默认为值为 `md5`。
    ...
    * 使用 S3 作为备份介质时，用户租户为本租户设置备份路径，配置清理模式为 `delete`，指定校验算法为 `crc32`。
    ...
    示例中，`s3://` 表示使用 S3 作为备份介质，存储桶名 `oceanbase-test-bucket`，在存储桶中的路径是 `/backup/data`，同时使用 `?` 来分隔路径的其他参数，`host` 用于设置 Amazon S3 服务的域名，`access_id` 和 `access_key` 用于设置 AWS 服务的访问密钥，`s3_region` 在使用 S3 作为备份介质时为必选参数，表示 S3 的存储桶所在的地域，清理模式为 `delete`，使用 CRC32 算法来校验备份数据的完整性。

    tab 兼容 S3 协议的对象存储

    由于大多数对象存储兼容了 S3 协议，对于兼容 S3 协议并满足 OceanBase 数据库要求（对象存储需要兼容实现了 OceanBase 系统调用的几个 S3 API 的行为）的对象存储均可以作为 OceanBase 数据库的备份介质，并通过访问 S3 的方式来访问兼容的对象存储，例如：OBS、GCS、OSS 等都是通过 S3 协议访问。

    从 V4.2.1 BP10 版本开始，对于通过 S3 协议接入的 OBS、GCS、COS 等对象存储，也支持通过 `checksum_type` 参数指定校验算法来校验备份数据的完整性，仅支持取值为 `md5`。如果不显式配置 `checksum_type` 参数，则默认值为 `md5`。有关 `checksum_type` 参数的详细说明，参见 [SET DATA_BACKUP_DEST](../../../700.reference/500.sql-reference/100.sql-syntax/100.system-tenants/200.alter-system/3500.set-data-backup-dest.md)。
    ...
    * 使用 OBS 作为备份介质时，用户租户为本租户设置备份路径。
    ...
    上述示例中，`s3://` 表示使用兼容 S3 协议的对象存储作为备份介质，存储桶名 `oceanbase-test-bucket`，在存储桶中的路径是 `/backup/data`，同时使用 `?` 来分隔路径的其他参数，`host` 用于设置 OBS 服务的域名，`access_id` 和 `access_key` 用于设置 OBS 服务的访问密钥。
    ...
    * 使用 GCS 作为备份介质时，用户租户为本租户设置备份路径。
    ...
    上述示例中，`s3://` 使用兼容 S3 协议的对象存储作为备份介质，存储桶名 `oceanbase-test-bucket`，在存储桶中的路径是 `/backup/data`，同时使用 `?` 来分隔路径的其他参数，`host` 用于设置 GCS 服务的域名，`access_id` 和 `access_key` 用于设置 GCS 服务的访问密钥。

    **使用 COS 作为备份介质时的操作步骤及示例如下**：
    ...
    3. 设置备份路径并通过 `delete_mode` 参数来配置备份文件的清理模式。
        ...
        示例中，`s3://` 使用兼容 S3 协议的对象存储作为备份介质，存储桶名 `oceanbase-test`，在存储桶中的路径是 `/backup/archive`，同时使用 `?` 来分隔路径的其他参数，`host` 用于设置 COS 的主机地址，即 endpoint（不带存储桶名的地址），`access_id` 和 `access_key` 用于设置 `COS` 的访问密钥，清理模式设置为 `delete`。
...
* 如果 `format` 文件已存在，则要求 `format` 文件的内容检验通过，配置项才能设置成功。否则，系统会报 `-9081` 的错误，提示 format 文件不匹配，format 文件的内容校验主要是检查集群、租户与备份目的端的介质类型等与当前操作的集群、租户及备份目的端介质类型是否匹配。
