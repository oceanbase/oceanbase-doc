...
2. 配置备份目的端。
   ...
   * 用户租户配置本租户的备份目的端
   ...
   <main id="notice" type='notice'>
   <h4>注意</h4>
   <p>使用对象存储作为备份介质时，其 <code>access_key</code> 参数中不能包含逗号等特殊符号。</p>
   </main>
   ...
   详细设置说明如下。
    ...
    当使用 OSS 作为备份介质时，除了设置备份路径，您还可以通过 `delete_mode` 参数来配置备份文件的清理模式。`delete_mode` 参数当前支持 `delete` 模式和 `tagging` 模式。如果不显式配置 `delete_mode` 参数，则默认使用 `delete` 模式。同时，OSS 还支持通过 `checksum_type` 参数来指定校验算法，如果不显式配置 `checksum_type` 参数，则默认值为 `md5`。
    ...
    * 使用 OSS 作为备份介质时，系统租户为指定租户 `mysql_tenant` 设置备份路径并配置备份文件的清理模式。
    ...
    * 使用 OSS 作为备份介质时，用户租户为本租户设置备份路径并配置备份文件的清理模式。
    ...
    示例中，`oss://` 表示使用 OSS 作为备份介质类型，存储桶名 `oceanbase-test-bucket`，在存储桶中的路径是 `/backup/data`，同时使用 `?` 来分隔路径的其他参数，`host` 用于设置存储桶的主机地址，`access_id` 和 `access_key` 用于设置 `OSS` 的访问密钥，清理模式设置为 `delete`。
    ...
    * 使用 NFS 作为备份介质时，用户租户为本租户设置备份路径。
    ...
    与 OSS 一样，AWS S3 也支持通过 `delete_mode` 参数来配置备份文件的清理模式，配置方法与 OSS 一样。同样，AWS S3 也支持通过 `checksum_type` 参数指定校验算法，支持指定的值为`md5` 和 `crc32`。如果不显式配置 `checksum_type` 参数，则默认值为 `md5`。

    有关 `delete_mode` 参数和 `checksum_type` 参数的详细说明，参见 [SET DATA_BACKUP_DEST](../../../700.reference/500.sql-reference/100.sql-syntax/100.system-tenants/200.alter-system/3500.set-data-backup-dest.md)。

    使用 S3 作为备份介质的配置示例如下：

    * 使用 S3 作为备份介质时，系统租户为指定租户 `mysql_tenant` 设置备份路径并配置为 `delete` 清理模式。
    ...
    * 使用 S3 作为备份介质时，用户租户为本租户设置备份路径并配置为 `delete` 清理模式。
    ...
    示例中，`s3://` 表示使用 S3 作为备份介质类型，存储桶名 `oceanbase-test-bucket`，在存储桶中的路径是 `/backup/data`，同时使用 `?` 来分隔路径的其他参数，`host` 用于设置 Amazon S3 服务的域名，`access_id` 和 `access_key` 用于设置 AWS 服务的访问密钥，`s3_region` 在使用 S3 作为备份介质时为必选参数，表示 S3 的存储桶所在的地域，清理模式设置为 `delete`。
    ...
    OBS、GCS、COS 等对象存储也支持通过 `checksum_type` 参数来指定校验算法，不同的对象存储其取值可能不同。有关  `checksum_type` 参数的详细说明，参见 [SET DATA_BACKUP_DEST](../../../700.reference/500.sql-reference/100.sql-syntax/100.system-tenants/200.alter-system/3500.set-data-backup-dest.md)。
    ...
    * 使用 OBS 作为备份介质时，用户租户为本租户设置备份路径并配置清理模式。
    ...
    上述示例中，`s3://` 表示使用 S3 协议访问备份目的端，存储桶名 `oceanbase-test-bucket`，在存储桶中的路径是 `/backup/data`，同时使用 `?` 来分隔路径的其他参数，`host` 用于设置 OBS 服务的域名，`access_id` 和 `access_key` 用于设置 OBS 服务的访问密钥。
    ...
    * 使用 GCS 作为备份介质时，用户租户为本租户设置备份路径。
    ...
    上述示例中，`s3://` 表示使用 S3 协议访问备份目的端，存储桶名 `oceanbase-test-bucket`，在存储桶中的路径是 `/backup/data`，同时使用 `?` 来分隔路径的其他参数，`host` 用于设置 GCS 服务的域名，`access_id` 和 `access_key` 用于设置 GCS 服务的访问密钥。
