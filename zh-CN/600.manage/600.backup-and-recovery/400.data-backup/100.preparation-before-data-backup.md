...
2. 配置备份目的端。
   ...
   * 用户租户配置本租户的备份目的端
   ...
   详细设置说明如下。
    ...
    在 V4.3.5 版本中，从 V4.3.5 BP2 版本开始，OceanBase 数据库支持使用已配置了合规保留策略的 OSS Bucket 作为备份路径。在配置备份路径时，可以通过 `enable_worm` 参数来指定 OceanBase 数据库对该路径的写入及删除是否与 OSS 中 Bucket 的保留策略一致。该参数支持的取值为 `true` 和 `false`。设置 `enable_worm=true` 时要求同时显式配置 `checksum_type=md5`。如果不显式配置 `enable_worm` 参数，则默认为 `false`。该参数一旦设置后，不支持变更。

    <main id="notice" type='notice'>
    <h4>注意</h4>
    <p>在设置 <code>enable_worm</code> 参数时，需要用户自行确认 Bucket 的 WORM 策略被正确配置。</p>
    </main>
