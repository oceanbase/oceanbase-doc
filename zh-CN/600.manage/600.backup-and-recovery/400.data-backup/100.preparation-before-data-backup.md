...
2. 配置备份目的端。
   ...
   目前，OceanBase 数据库支持 NFS、阿里云 OSS、腾讯云 COS、AWS S3、华为 OBS、Google GCS 等作为备份目的端。
   ...
   * 用户租户配置本租户的备份目的端
    ...
    * 使用 S3 作为备份介质时，用户租户为本租户设置备份目的端并配置为 `tagging` 清理模式。
    ...
    示例中，`s3://` 表示使用支持 S3 协议的对象存储作为备份目的端介质类型，存储桶名 `oceanbase-test-bucket`，在存储桶中的路径是 `/backup/data`，同时使用 `?` 来分隔路径的其他参数，`host` 用于设置 Amazon S3 服务的域名，`access_id` 和 `access_key` 用于设置 AWS 服务的访问密钥，`s3_region` 在使用 S3 作为备份介质时为必选参数，表示 S3 的存储桶所在的地域，清理模式设置为 `tagging`。
    ...
    * 使用 OBS 作为备份介质时，用户租户为本租户设置备份目的端并配置清理模式。
    ...
    示例中，`s3://` 表示使用支持 S3 协议的对象存储作为备份目的端介质类型，存储桶名 `oceanbase-test-bucket`，在存储桶中的路径是 `/backup/data`，同时使用 `?` 来分隔路径的其他参数，`host` 用于设置 OBS 服务的域名，`access_id` 和 `access_key` 用于设置 OBS 服务的访问密钥，`s3_region` 在使用 OBS 作为备份介质时为可选参数，表示 OBS 的存储桶所在的地域。
    ...
    * 使用 GCS 作为备份介质时，用户租户为本租户设置备份目的端并配置清理模式。
    ...
    示例中，`s3://` 表示使用支持 S3 协议的对象存储作为备份目的端介质类型，存储桶名 `oceanbase-test-bucket`，在存储桶中的路径是 `/backup/data`，同时使用 `?` 来分隔路径的其他参数，`host` 用于设置 GCS 服务的域名，`access_id` 和 `access_key` 用于设置 GCS 服务的访问密钥，`s3_region` 在使用 GCS 作为备份介质时为可选参数，表示 GCS 的存储桶所在的地域。
