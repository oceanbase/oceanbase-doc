...
2. 配置备份目的端。
   ...
   * 指定备份路径的源端即是指定可以访问该备份路径的节点范围。有关备份路径源端配置的详细介绍及说明，参见 [SET DATA_BACKUP_DEST](../../../700.reference/500.sql-reference/100.sql-syntax/100.system-tenants/200.alter-system/3500.set-data-backup-dest.md)。
    ...
    * 使用 OSS 作为备份介质时，用户租户为本租户设置备份路径，配置备份文件的清理模式，同时指定校验算法为 `md5`。
    ...
    示例中，`oss://` 表示使用 OSS 作为备份介质，存储桶名 `oceanbase-test-bucket`，在存储桶中的路径是 `/backup/data`，同时使用 `?` 来分隔路径的其他参数，`host` 用于设置存储桶的主机地址，`access_id` 和 `access_key` 用于设置 `OSS` 的访问密钥，清理模式为 `delete`，使用 MD5 算法来校验备份数据的完整性。
    ...
    * 使用 GCS 作为备份介质时，用户租户为本租户设置备份路径。
    ...
    上述示例中，`s3://` 表示使用兼容 S3 协议的对象存储作为备份介质，存储桶名 `oceanbase-test-bucket`，在存储桶中的路径是 `/backup/data`，同时使用 `?` 来分隔路径的其他参数，`host` 用于设置 GCS 服务的域名，`access_id` 和 `access_key` 用于设置 GCS 服务的访问密钥。
    ...
    **使用 COS 作为备份介质的操作步骤及示例如下**：
    ...
    3. 设置备份路径，通过 `delete_mode` 参数来配置备份文件的清理模式。
        ...
        示例中，`s3://` 表示使用 S3 协议访问备份路径，存储桶名 `oceanbase-test`，在存储桶中的路径是 `/backup/archive`，同时使用 `?` 来分隔路径的其他参数，`host` 用于设置 COS 的主机地址，即 endpoint（不带存储桶名的地址），`access_id` 和 `access_key` 用于设置 `COS` 的访问密钥。
