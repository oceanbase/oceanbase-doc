...
3. 调整配置。
   ...
   1. 将 `/etc/nfs.conf` 文件中 `vers3=y` 前的 `#` 去掉。
   ...
   2. 查看 `/etc/sysconfig/nfs` 文件的 `RPCNFSDARGS` 参数中是否指定了 `-N 3`， 如果指定了，去掉 `-N 3`。

      例如，修改前，`RPCNFSDARGS` 参数中的值如下：

      ```shell
      RPCNFSDARGS="-N 2 -N 3 -N 4"
      ```
      
      去掉 `-N 3` 后，`RPCNFSDARGS` 参数中的值如下：
