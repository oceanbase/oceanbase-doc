...
3. 设置 Exports。
   ...
   3. 执行以下命令，为 `nobody` 赋权，确保 `nobody` 有权限访问 `exports` 中指定的目录。

      <main id="notice" type='explain'>
      <h4>说明</h4>
      <p>对于 CentOS 7 版本，NFS 默认使用 <code>nfsnoboody</code> 作为默认的匿名用户；CentOS 8 及之后版本，NFS 默认使用 <code>noboody</code> 作为默认的匿名用户。本文档以 CentOS 8 版本为例，提供 NFS 的配置指导。</p>
      </main>
