...
使用 S3 或兼容 S3 协议的其他对象存储作为归档介质时，要求其归档延迟时间（由配置项 `archive_lag_targe` 控制）大于或等于 60s。在将归档介质从 OSS/NFS/COS 更改为 S3 或兼容 S3 协议的其他对象存储时，如果配置项 `archive_lag_target` 的值小于 60s，则执行更改归档目的端的命令时，系统会报错。
