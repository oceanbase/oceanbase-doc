...
OceanBase 数据库使用租户级配置项 `archive_lag_target` 来控制租户日志归档延迟的时间，以便尽量将较小的 IO 聚合为更大 IO 来提高归档速度。配置项 `archive_lag_target` 的默认值为 `120s`，取值范围为 \[0ms, 7200s\]，有关配置项 `archive_lag_target` 的更多说明，请参见 [archive_lag_target](../../../700.reference/800.configuration-items-and-system-variables/100.system-configuration-items/400.tenant-level-configuration-items/7200.archive_lag_target.md)。
...
* 对于归档介质为 OSS/NFS/COS 的场景，配置项 `archive_lag_target` 的值可以设置为取值范围内的任意值。

* 对于归档介质为 S3 及兼容 S3 协议的其他对象存储的场景，配置项 `archive_lag_target` 的值不能小于 `60s`，小于 `60s` 系统将会报错。
