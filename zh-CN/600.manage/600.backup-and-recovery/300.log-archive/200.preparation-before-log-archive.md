...
2. 根据实际情况，选择合适的语句，配置归档延迟。
   ...
   * `sys` 租户为指定租户配置归档延迟时间
     ...
     <main id="notice" type='notice'>
     <h4>注意</h4>
     <p>在配置 <code>archive_lag_target</code> 参数时，不支持 sys 租户通过 <code>TENANT = all</code> 或 <code>TENANT = all_user</code> 或 <code>TENANT = all_meta</code> 来分别指定所有用户或所有 Meta 租户。</p>
