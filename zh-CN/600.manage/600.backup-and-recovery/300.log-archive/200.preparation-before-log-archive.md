...
3. 归档目的端设置成功后，如果用户因安全或其他原因，更改了对象存储的密钥信息，需要为已设置的归档路径更新 `access_id` 和 `access_key` 信息，具体操作参见 [变更备份路径或归档路径的 `access_id` 和 `access_key` 信息](../450.change-sk-and-ak-for-the-backup-path-or-logarchive-path.md)。
