...
2. 配置归档目的端。
   ...
   * 配置 `LOCATION` (必选)
      ...
      * 使用 S3 作为归档介质时，用户租户为本租户设置归档目的端并配置为 `tagging` 清理模式。
      ...
      示例中，`s3://` 表示使用 S3 作为归档目的端介质类型，存储桶名 `oceanbase-test-bucket`，在存储桶中的路径是 `/backup/archive`，同时使用 `?` 来分隔路径的其他参数，`host` 用于设置 Amazon S3 服务的域名，`access_id` 和 `access_key` 用于设置 AWS 服务的访问密钥，`s3_region` 为必选参数，表示 S3 的存储桶所在的地域，清理模式设置为 `tagging`。
      ...
      * 使用 OBS 作为归档介质时，用户租户为本租户设置归档目的端并配置清理模式。
      ...
      示例中，`s3://` 表示使用 OBS 作为归档目的端介质类型，存储桶名 `oceanbase-test-bucket`，在存储桶中的路径是 `/backup/archive`，同时使用 `?` 来分隔路径的其他参数，`host` 用于设置 OBS 服务的域名，`access_id` 和 `access_key` 用于设置 OBS 服务的访问密钥，`s3_region` 为必选参数，表示 OBS 的存储桶所在的地域。
      ...
      * 使用 GCS 作为归档介质时，用户租户为本租户设置归档目的端并配置清理模式。
      ...
      示例中，`s3://` 表示使用 GCS 作为归档目的端介质类型，存储桶名 `oceanbase-test-bucket`，在存储桶中的路径是 `/backup/archive`，同时使用 `?` 来分隔路径的其他参数，`host` 用于设置 GCS 服务的域名，`access_id` 和 `access_key` 用于设置 GCS 服务的访问密钥，`s3_region` 为必选参数，表示 GCS 的存储桶所在的地域。
