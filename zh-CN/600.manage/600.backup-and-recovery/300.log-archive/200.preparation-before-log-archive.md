...
2. 配置归档目的端。
   ...
   * 配置 `LOCATION` (必选)
      ...
      * 使用 S3 作为归档介质时，用户租户为本租户设置归档目的端并配置为 `tagging` 清理模式。
      ...
      与 S3 不一样，OBS 不支持将 `delete_mode` 参数设置为 `tagging`，仅支持将 `delete_mode` 参数设置为 `delete` 清理模式。如果不配置 `delete_mode` 参数，则默认不清理归档文件。
      ...
      * 使用 OBS 作为归档介质时，用户租户为本租户设置归档目的端并配置清理模式。
      ...
      与 S3 不一样，GCS 不支持将 `delete_mode` 参数设置为 `tagging`，仅支持将 `delete_mode` 参数设置为 `delete` 清理模式。如果不配置 `delete_mode` 参数，则默认不清理归档文件。
