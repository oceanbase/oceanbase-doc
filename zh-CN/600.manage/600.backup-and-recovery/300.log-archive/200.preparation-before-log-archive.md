...
2. 配置归档目的端。
   ...
   * 配置 `LOCATION` (必选)
       ...
       * 腾讯云 COS
         ...
         当归档目的端的介质为 COS 时，系统租户设置归档目的端并配置为 `tagging` 清理模式的示例如下：
         ...
         其中，`appid` 为必选参数，表示腾讯云账号的 APPID。

       * AWS S3

         与 OSS/COS 一样，S3 也支持通过 `delete_mode` 参数来配置归档文件的清理模式，配置方法与 OSS/COS 一样。

         当归档目的端的介质为 S3 时，系统租户设置归档目的端并配置为 `tagging` 清理模式的示例如下：

         ```sql
         obclient> ALTER SYSTEM SET LOG_ARCHIVE_DEST='LOCATION=s3://oceanbase-test-bucket/backup/archive?host=s3.<region>.amazonaws.com&access_id=xxx&access_key=xxx&s3_region=xxx&delete_mode=tagging' TENANT = mysql_tenant;
         ```

         其中，`s3_region` 为必选参数，表示 S3 的存储桶所在的地域。
