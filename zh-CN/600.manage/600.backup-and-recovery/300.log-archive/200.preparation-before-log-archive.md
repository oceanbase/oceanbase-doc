...
2. 配置归档目的端。
   ...
   * 配置 `LOCATION` (必选)
       ...
       `LOCATION` 属性用于指定归档目的端，目前 OceanBase 数据库支持的归档目的端介质为 NFS 、阿里云 OSS、腾讯云 COS、AWS S3、华为 OBS 以及 Google GCS。

       <main id="notice" type='explain'>
       <h4>说明</h4>
       <p>由于大多数对象存储均兼容 S3 协议，故在 OceanBase 数据库中，使用 S3、GCS、OBS 等作为归档介质时，其目的端地址均以 <code>s3://</code> 开头。</p>
       </main>
      ...
      * 使用 COS 作为归档介质时，用户租户为本租户设置归档目的端并配置为 `tagging` 清理模式。
      ...
      与 OSS/COS 一样，S3 也支持通过 `delete_mode` 参数来配置归档文件的清理模式，配置方法与 OSS/COS 一样。

      * 使用 S3 作为归档介质时，系统租户为指定租户 `mysql_tenant` 设置归档目的端并配置为 `tagging` 清理模式。
      ...
      * 使用 S3 作为归档介质时，用户租户为本租户设置归档目的端并配置为 `tagging` 清理模式。
      ...
      示例中，`s3://` 表示使用支持 S3 协议的对象存储作为归档目的端介质类型，存储桶名 `oceanbase-test-bucket`，在存储桶中的路径是 `/backup/archive`，同时使用 `?` 来分隔路径的其他参数，`host` 用于设置 Amazon S3 服务的域名，`access_id` 和 `access_key` 用于设置 AWS 服务的访问密钥，`s3_region` 在使用 S3 作为归档介质时为必选参数，表示 S3 的存储桶所在的地域，清理模式设置为 `tagging`。

      tab 华为 OBS

      与 S3 不一样，OBS 不支持将 `delete_mode` 参数设置为 `tagging`，仅支持将 `delete_mode` 参数设置为 `delete` 清理模式。您也可以不配置 `delete_mode` 参数，不配置时，默认就是 `delete` 清理模式。
      ...
      * 使用 OBS 作为归档介质时，系统租户为指定租户 `mysql_tenant` 设置归档目的端并配置清理模式。
        ...
        &access_id=xxx
        &access_key=xxx
        &s3_region=xxx' TENANT = mysql_tenant;
        ```
      ...
      * 使用 OBS 作为归档介质时，用户租户为本租户设置归档目的端并配置清理模式。
      ...
      示例中，`s3://` 表示使用支持 S3 协议的对象存储作为归档目的端介质类型，存储桶名 `oceanbase-test-bucket`，在存储桶中的路径是 `/backup/archive`，同时使用 `?` 来分隔路径的其他参数，`host` 用于设置 OBS 服务的域名，`access_id` 和 `access_key` 用于设置 OBS 服务的访问密钥，`s3_region` 在使用 OBS 作为归档介质时为可选参数，表示 OBS 的存储桶所在的地域。

      tab Google GCS

      与 S3 不一样，GCS 不支持将 `delete_mode` 参数设置为 `tagging`，仅支持将 `delete_mode` 参数设置为 `delete` 清理模式。您也可以不配置 `delete_mode` 参数，不配置时，默认就是 `delete` 清理模式。
      ...
      * 使用 GCS 作为归档介质时，用户租户为本租户设置归档目的端并配置清理模式。
      ...
      示例中，`s3://` 表示使用支持 S3 协议的对象存储作为归档目的端介质类型，存储桶名 `oceanbase-test-bucket`，在存储桶中的路径是 `/backup/archive`，同时使用 `?` 来分隔路径的其他参数，`host` 用于设置 GCS 服务的域名，`access_id` 和 `access_key` 用于设置 GCS 服务的访问密钥，`s3_region` 在使用 GCS 作为归档介质时为可选参数，表示 GCS 的存储桶所在的地域。
