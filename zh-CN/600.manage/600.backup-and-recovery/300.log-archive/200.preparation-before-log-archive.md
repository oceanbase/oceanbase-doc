...
2. 配置归档目的端。
   ...
   * 配置 `LOCATION` (必选)
      ...
      * 使用 GCS 作为归档介质时，用户租户为本租户设置归档路径。
      ...
      上述示例中，`s3://` 表示使用兼容 S3 协议的对象存储作为归档介质类型，存储桶名 `oceanbase-test-bucket`，在存储桶中的路径是 `/backup/archive`，同时使用 `?` 来分隔路径的其他参数，`host` 用于设置 GCS 服务的域名，`access_id` 和 `access_key` 用于设置 GCS 服务的访问密钥。
      ...
      **使用 COS 作为归档介质的操作步骤及示例如下**：
      ...
      3. 设置归档路径并通过 `delete_mode` 参数来配置归档文件的清理模式。
          ...
          示例中，`s3://` 使用 S3 协议访问归档路径，存储桶名 `oceanbase-test`，在存储桶中的路径是 `/backup/archive`，同时使用 `?` 来分隔路径的其他参数，`host` 用于设置 COS 的主机地址，即 endpoint（不带存储桶名的地址），`access_id` 和 `access_key` 用于设置 `COS` 的访问密钥，清理模式为 `delete`。
