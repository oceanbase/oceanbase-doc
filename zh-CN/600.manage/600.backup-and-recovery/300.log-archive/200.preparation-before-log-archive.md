...
2. 配置归档目的端。
   ...
   * 配置 `LOCATION` (必选)
      ...
      在 V4.3.5 版本中，从 V4.3.5 BP2 版本开始，OceanBase 数据库支持使用已配置了合规保留策略的 OSS Bucket 作为归档路径。在配置归档路径时，可以指定 `enable_worm` 参数来确认 OceanBase 数据库是否以 WORM（Write Once Read Many）支持的方式去写 Bucket。该参数支持的取值为 `true` 和 `false`。设置 `enable_worm=true` 时要求同时显式配置 `checksum_type=md5`。如果不显式配置 `enable_worm` 参数，则默认为 `false`。该参数一旦设置后，不支持变更。

      <main id="notice" type='notice'>
      <h4>注意</h4>
      <p>在设置 <code>enable_worm</code> 参数时，用户应确保 Bucket 的 WORM 策略已被正确配置。</p>
      </main>

      有关 `delete_mode` 参数、`checksum_type` 参数以及 `enable_worm` 参数的详细说明，参见 [SET LOG_ARCHIVE_DEST](../../../700.reference/500.sql-reference/100.sql-syntax/100.system-tenants/200.alter-system/150.set-log-archive-dest.md)。
      ...
      * 使用 OSS 作为归档介质时，系统租户为指定租户 `mysql_tenant` 设置归档路径，配置清理模式为 `delete`，指定以 WORM 支持的方式写 Bucket，同时指定校验算法为 `md5`。
      ...
      * 使用 OSS 作为归档介质时，用户租户为本租户设置归档路径，配置清理模式为 `delete`，同时指定校验算法为 `md5`。
      ...
      示例中，`oss://` 表示使用 OSS 作为归档介质类型，存储桶名 `oceanbase-test-bucket`，在存储桶中的路径是 `/backup/archive`，同时使用 `?` 来分隔路径的其他参数，`host` 用于设置存储桶的主机地址，`access_id` 和 `access_key` 用于设置 `OSS` 的访问密钥，清理模式为 `delete`，`enable_worm=true` 表示以 WORM 支持的方式写 Bucket，并使用 MD5 算法校验归档文件的完整性。
      ...
      * 使用 S3 作为归档介质时，用户租户为本租户设置归档路径，配置清理模式为 `delete`，指定校验算法为 `crc32`。
      ...
      上述示例中，`s3://` 表示使用 AWS S3 作为归档介质类型，存储桶名 `oceanbase-test-bucket`，在存储桶中的路径是 `/backup/archive`，同时使用 `?` 来分隔路径的其他参数，`host` 用于设置 Amazon S3 服务的域名，`access_id` 和 `access_key` 用于设置 AWS 服务的访问密钥，`s3_region` 在使用 S3 作为归档介质时为必选参数，表示 S3 的存储桶所在的地域，清理模式为 `delete`，使用 CRC32 算法校验归档文件的完整性。
