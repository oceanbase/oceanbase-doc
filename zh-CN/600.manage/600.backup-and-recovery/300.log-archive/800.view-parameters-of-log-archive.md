...
2. 执行以下语句，查看租户的归档相关参数。
   ...
   * 查看所有租户日志归档的归档路径相关信息。
      ...
      ```shell
      obclient [oceanbase]> SELECT PATH, DEST_TYPE, EXTENSION FROM oceanbase.CDB_OB_BACKUP_STORAGE_INFO;
      ```
      ...
      +-----------+---------------------------------------------+-------------+------------------------------------+
      |      1002 | oss://oceanbase-test-bucket/backup/archive  | archive_log | checksum_type=md5&enable_worm=true |
      |      1002 | oss://oceanbase-test-bucket/backup/data     | backup_data | checksum_type=md5&enable_worm=true |
      +-----------+---------------------------------------------+-------------+------------------------------------+
      2 rows in set
      ```

      查询结果中，对于租户 ID 为 `1002` 的租户，其归档路径为 `oss://oceanbase-test-bucket/backup/archive`，`enable_worm=true` 表示 OceanBase 数据库对该归档路径的写入及删除与 OSS 中该 Bucket 的 WORM 策略一致; `checksum_type=md5` 表示使用 MD5 算法校验归档文件的完整性。
...
2. 执行以下语句，查看当前租户归档相关的参数。
   ...
   * 查看本租户日志归档的归档路径的相关信息。
      ...
      ```shell
      obclient [SYS]> SELECT PATH, DEST_TYPE, EXTENSION FROM SYS.DBA_OB_BACKUP_STORAGE_INFO;
      ```
      :::
      ...
      | oss://oceanbase-test-bucket/backup/data    | backup_data | checksum_type=md5 |
      +--------------------------------------------+-------------+-------------------+
      2 rows in set
      ```
