...
2. 执行以下语句，查看租户的归档相关参数。
   ...
   * 查看所有租户日志归档的归档路径相关信息（含源端配置信息）。
      ...
      ```shell
      obclient(root@sys)[oceanbase]> SELECT TENANT_ID, PATH, DEST_TYPE, EXTENSION FROM oceanbase.CDB_OB_BACKUP_STORAGE_INFO WHERE DEST_TYPE='archive_log';
      ```
      ...
      查询结果中，对于租户 ID 为 `1004` 的租户，其归档路径为 `oss://oceanbase-test-bucket/backup/archive`，`enable_worm=true` 表示 OceanBase 数据库按照 WORM 的规范对该路径执行写入与删除操作; `checksum_type=md5` 表示使用 MD5 算法校验归档文件的完整性；源端配置为 `zone=zone1`，表示仅 `zone1` 的节点可以访问该路径。
...
2. 执行以下语句，查看当前租户归档相关的参数。
   ...
   * 查看本租户日志归档的归档路径的相关信息（含源端配置信息）。
      ...
      ```shell
      obclient(root@mysql001)[oceanbase]> SELECT PATH, DEST_TYPE, EXTENSION FROM oceanbase.DBA_OB_BACKUP_STORAGE_INFO WHERE DEST_TYPE='archive_log';
      ```
      ...
      ```shell
      obclient(SYS@oracle001)[SYS]> SELECT PATH, DEST_TYPE, EXTENSION FROM SYS.DBA_OB_BACKUP_STORAGE_INFO WHERE DEST_TYPE='archive_log';
      ```
