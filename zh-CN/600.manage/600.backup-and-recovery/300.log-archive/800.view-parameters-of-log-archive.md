...
2. 执行以下语句，查看租户的归档相关参数。
   ...
   * 查看所有租户日志归档的归档参数信息（不含源端配置信息）。
      ...
      更多 `CDB_OB_ARCHIVE_DEST` 视图的说明，参见 [CDB_OB_ARCHIVE_DEST](../../../700.reference/700.system-views/300.system-view-of-sys-tenant/200.dictionary-view-of-sys-tenant/5400.o-cdb_ob_archive_dest-of-sys-tenant.md)。
...
2. 执行以下语句，查看当前租户归档相关的参数。
   ...
   * 查看本租户日志归档的归档参数信息（不含源端配置信息）。
      ...
      用户租户可以通过视图 `oceanbase.DBA_OB_ARCHIVE_DEST`（MySQL 模式）和视图 `SYS.DBA_OB_ARCHIVE_DEST` （Oracle 模式）查看本租户的归档参数信息。
