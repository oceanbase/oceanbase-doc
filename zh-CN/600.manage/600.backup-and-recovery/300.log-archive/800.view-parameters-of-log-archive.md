...
2. 执行以下语句，查看租户的归档相关参数。
   ...
   * 查看所有租户日志归档的归档路径相关信息。
      ...
      查询结果中，对于租户 ID 为 `1004` 的租户，其归档路径为 `oss://oceanbase-test-bucket/backup/archive`，`enable_worm=true` 表示 OceanBase 数据库对该归档路径的写入及删除该 Bucket 的 WORM 策略一致; `checksum_type=md5` 表示使用 MD5 算法校验归档文件的完整性。
