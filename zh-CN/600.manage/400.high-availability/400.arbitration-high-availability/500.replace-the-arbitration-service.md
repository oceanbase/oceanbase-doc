...
2. 执行以下命令，获取集群当前的仲裁服务地址。
   ...
   ```shell
   obclient(root@sys)[(none)]> SELECT * FROM oceanbase.DBA_OB_ARBITRATION_SERVICE;
   ```
...
3. 执行以下命令，为集群替换仲裁服务。
   ...
   * `new_arb_server_ip:new_arb_server_port`：表示新的仲裁服务的 IP 和 RPC 端口，端口号默认为 2882。
   ...
   ```shell
   obclient(root@sys)[(none)]> ALTER SYSTEM REPLACE ARBITRATION SERVICE 'xx.xx.xx.64:2882' WITH 'xx.xx.xx.192:2882';
   ```
   ...
   ```shell
   obclient(root@sys)[(none)]> SHOW WARNINGS;
   ```
...
4. （可选）仲裁服务替换成功后，可以在 `DBA_OB_ROOTSERVICE_EVENT_HISTORY` 视图中通过查询指定 `module` 的方式来查看仲裁服务的替换历史。
   ...
   ```shell
   obclient(root@sys)[(none)]> SELECT * FROM oceanbase.DBA_OB_ROOTSERVICE_EVENT_HISTORY WHERE module LIKE "%arb_service%";
   ```
