...
* 假设当前时间为 `T`，设置 `undo_retention` 后，如果在 `T - undo_retention` 时间段内有 DDL 操作，则：
  ...
  * 对于添加列的操作，闪回查询 DDL 操作发生前的数据时，所有新添加的列均为 Default 值。

  * 对于创建表、删除表或删除列的操作，闪回查询 DDL 操作发生前的数据时，系统会报错。
