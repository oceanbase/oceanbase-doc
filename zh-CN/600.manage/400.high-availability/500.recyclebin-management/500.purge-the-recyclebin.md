...
3. 执行以下语句，开启自动清理回收站的功能并指定清理多少时间前进入回收站的对象。
   ...
   开启自动清理回收站功能后，在当前版本中，回收站的自动清理动作将由后台定时任务 `SCHEDULED_PURGE_RECYCLEBIN` 控制执行，默认触发的时间间隔为 10 分钟。用户可根据业务需要，修改该定时任务触发的时间间隔，详细修改操作可参见 [SET_ATTRIBUTE](../../../700.reference/600.pl-reference/300.pl-oracle/1400.pl-system-package-oracle/14200.dbms-scheduler-oracle/1100.set-attribute-oracle.md)。
