...
由于当前仅 `sys` 租户可以删除租户，故 MySQL 模式和 Oracle 模式的租户被删除后，只能进入 `sys` 租户的回收站中。删除租户有强制和非强制两种方式，具体请参见 [DROP TENANT](../../../700.reference/500.sql-reference/100.sql-syntax/100.system-tenants/1100.drop-tenant.md)。对于 `sys` 租户，除了能查看本租户的数据库、表和索引等回收站对象外，还可以查看系统内所有的租户回收站对象（进入回收站的所有租户）；对于 MySQL 用户租户，只能查看本租户的数据库、表和索引等回收站对象；对于 Oracle 用户租户，只能查看本租户的表和索引等回收站对象。
## 背景信息
