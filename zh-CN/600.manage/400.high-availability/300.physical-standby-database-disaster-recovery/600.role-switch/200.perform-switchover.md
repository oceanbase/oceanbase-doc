...
* 在将备租户切换为主租户前，要求备租户所有日志流的副本均在线，否则需要等待对应的副本永久下线。备租户或备租户所在集群的`sys` 租户可以分别通过 `DBA_OB_LS` 视图或 `CDB_OB_LS` 视图查询所有日志流副本是否在线，有关日志流副本的详细说明信息，参见 [副本介绍](../../../300.replica-management/100.replica-introduction.md)。
...
3. 为原主租户设置日志恢复源。
   ...
   2. 执行以下命令，设置日志恢复源。
      ...
      * 原主租户所在集群的 `sys` 租户设置原主租户的日志恢复源
        ...
        ```shell
        obclient> ALTER SYSTEM SET LOG_RESTORE_SOURCE ='SERVICE=$ip_list USER=$user_name@$tenant_name PASSWORD=$password' TENANT = tenant_name;
        ```

      * 原主租户设置本租户的日志恢复源

        ```shell
        obclient> ALTER SYSTEM SET LOG_RESTORE_SOURCE ='SERVICE=$ip_list USER=$user_name@$tenant_name PASSWORD=$password';
        ```

      部分参数说明如下：

      * `$ip_list`：新主租户所在副本的 OBServer 节点的 IP 及 SQL 端口号。
