...
在基于网络的物理备库场景中，Switchover 主要包括以下三个阶段：将主租户切换为备租户 --> 为原主租户设置日志恢复源 --> 将备租户切换为主租户。
...
2. 为原主租户设置日志恢复源。
   ...
   通常，设置日志恢复源的操作可以在备切主操作之前执行，也可以在备切主操作之后执行。但是，如果希望执行 Switchover 后，租户的保护模式不变，则需要在备切主之前，保证主备租户均已设置日志恢复源且恢复源是指向对方。

   <main id="notice" type='explain'>
   <h4>说明</h4>
   <p>对于 V4.4.2 版本，OceanBase 数据库从 V4.4.2 BP1 版本开始支持最大保护模式和最大可用模式。</p>
   </main>
