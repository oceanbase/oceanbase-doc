...
1. 将主租户切换为备租户。
   ...
   2. 执行 `SWITCHOVER TO STANDBY VERIFY` 命令，验证 `SWITCHOVER` 命令是否可以执行成功。
      ...
      * 主租户执行 `SWITCHOVER TO STANDBY VERIFY` 命令
        ...
        如果出现报错，请根据提示的文本信息参考文档 [主切备验证失败的错误信息及处理方法](400.switchover-to-standby-verfify-failed.md) 进行处理后，再重新执行本命令。
...
2. 将备租户切换为主租户。
   ...
   2. 执行 `SWITCHOVER TO PRIMARY VERIFY` 命令，验证 `SWITCHOVER` 命令是否可以执行成功。
      ...
      * 备租户执行 `SWITCHOVER TO PRIMARY VERIFY` 命令
        ...
        如果出现报错，请根据提示的文本信息参考文档 [备切主验证失败的错误信息及处理方法](500.switchover-or-failover-to-primary-verfify-failed1.md) 进行处理后，再重新执行本命令。
