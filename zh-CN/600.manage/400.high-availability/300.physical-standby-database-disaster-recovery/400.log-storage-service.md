...
2. 执行以下命令，获取主租户的 `TENANT_ID`。
   ...
   * `sys` 租户
     ...
     ```shell
     obclient(root@sys)[(none)]> SELECT TENANT_ID,TENANT_NAME FROM oceanbase.DBA_OB_TENANTS;
     ```
   ...
   * 主租户
     ...
     * MySQL 模式
       ...
       ```shell
       obclient(root@mysql)[(none)]> SELECT TENANT_ID,TENANT_NAME FROM oceanbase.DBA_OB_TENANTS;
       ```
     ...
     * Oracle 模式
       ...
       ```shell
       obclient(sys@oracle)[SYS]> SELECT TENANT_ID,TENANT_NAME FROM SYS.DBA_OB_TENANTS;
       ```
...
3. 执行以下命令，查询主租户日志流的工作模式。
   ...
   * MySQL 模式
     ...
     ```shell
     obclient(root@mysql)[(none)]> SELECT TENANT_ID, LS_ID, SVR_IP, SVR_PORT, ROLE, ACCESS_MODE FROM oceanbase.GV$OB_LOG_STAT WHERE TENANT_ID = 1002;
     ```

   * Oracle 模式

     ```shell
     obclient(sys@oracle)[SYS]> SELECT TENANT_ID, LS_ID, SVR_IP, SVR_PORT, ROLE, ACCESS_MODE FROM SYS.GV$OB_LOG_STAT WHERE TENANT_ID = 1004;
     ```
...
3. 执行以下命令，查询备租户日志流的工作模式。
   ...
   * MySQL 模式
     ...
     ```shell
     obclient(root@mysql)[(none)]> SELECT TENANT_ID, LS_ID, SVR_IP, SVR_PORT, ROLE, ACCESS_MODE FROM oceanbase.GV$OB_LOG_STAT WHERE TENANT_ID = 1004;
     ```

   * Oracle 模式

     ```shell
     obclient(sys@oracle)[SYS]> SELECT TENANT_ID, LS_ID, SVR_IP, SVR_PORT, ROLE, ACCESS_MODE FROM SYS.GV$OB_LOG_STAT WHERE TENANT_ID = 1006;
     ```
...
3. 执行以下命令，查看主备租户允许使用的日志盘存储空间以及实际使用情况。
   ...
   * MySQL 模式
     ...
     ```shell
     obclient(root@mysql)[(none)]> SELECT SVR_IP, SVR_PORT, TENANT_ID, LOG_DISK_SIZE, LOG_DISK_IN_USE FROM oceanbase.GV$OB_UNITS WHERE TENANT_ID = 1004;
     ```
   ...
   * Oracle 模式
     ...
     ```shell
     obclient(sys@oracle)[SYS]> SELECT SVR_IP, SVR_PORT, TENANT_ID, LOG_DISK_SIZE, LOG_DISK_IN_USE FROM SYS.GV$OB_UNITS WHERE TENANT_ID = 1006;
     ```
