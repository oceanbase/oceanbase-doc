...
查看备租户日志同步状态的详细操作，请参见 [查看日志同步进度](4.view-the-log-synchronization-progress.md)。
...
2. 选择合适的语句，设置配置项 `log_restore_concurrency`。
   ...
   * `sys` 租户调整指定租户的日志同步性能
   ...
   有关配置项 `log_restore_concurrency` 的更多信息，请参见 [log_restore_concurrency](../../../../7.reference/8.configuration-items-and-system-variables/1.system-configuration-items/4.tenant-level-configuration-items/249.log_restore_concurrency.md)。
