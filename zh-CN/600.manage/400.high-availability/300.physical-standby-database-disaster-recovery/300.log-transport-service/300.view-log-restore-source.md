...
2. 执行以下命令，查看日志恢复源信息。
   ...
   * 系统租户（`sys` 租户）下查看日志恢复源信息
     ...
     ```shell
     obclient(root@sys)[(none)]> SELECT * FROM oceanbase.CDB_OB_LOG_RESTORE_SOURCE;
     ```
   ...
   * 用户租户下查看日志恢复源信息
     ...
     * MySQL 模式：
       ...
       ```shell
       obclient(root@mysql)[(none)]> SELECT * FROM oceanbase.DBA_OB_LOG_RESTORE_SOURCE;
       ```
...
3. 执行以下命令，查看日志链路相关信息。
   ...
   * 系统租户（`sys` 租户）下查看日志链路相关信息
     ...
     ```shell
     obclient(root@sys)[(none)]> SELECT * FROM oceanbase.GV$OB_LOG_TRANSPORT_DEST_STAT;
     ```
   ...
   * 用户租户下查看日志链路相关信息。
     ...
     * MySQL 模式：
       ...
       ```shell
       obclient(root@mysql)[(none)]> SELECT * FROM oceanbase.GV$OB_LOG_TRANSPORT_DEST_STAT;
       ```

     * Oracle 模式：

       ```shell
       obclient(sys@oracle001)[SYS]> SELECT * FROM SYS.GV$OB_LOG_TRANSPORT_DEST_STAT;
       ```
