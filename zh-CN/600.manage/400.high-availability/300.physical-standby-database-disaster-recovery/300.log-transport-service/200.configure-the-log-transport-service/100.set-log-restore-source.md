...
2. 执行以下命令，配置备租户的恢复源指向主租户的归档目的端。
   ...
   * `LOCATION`：必选，用于指定主租户的归档目的端。

   * `DELAY`：可选，指定备租户（备库）延迟拉取日志的时间，指定数值（不带单位）时，默认单位为秒。支持指定带单位的数值，单位可以是 us（微秒）、ms（毫秒）、m（分钟）、h（1小时）、d（天）。如果不指定该参数，则延迟时间默认为 0 秒，表示备库不延迟。

     <main id="notice" type='explain'>
     <h4>说明</h4>
     <p>对于 V4.4.2 版本，OceanBase 数据库从 V4.4.2 BP2 版本开始支持延迟备库功能。有关延迟备库的详细操作及注意事项，参见 <a href="300.configure-a-delayed-standby-tenant.md">配置备库延迟</a>。</p>
     </main>
...
2. 执行以下命令，设置日志恢复源。
   ...
   * `$delay_time`：可选，指定备租户（备库）延迟拉取日志的时间，指定数值（不带单位）时，默认单位为秒。支持指定带单位的数值，单位可以是 us（微秒）、ms（毫秒）、m（分钟）、h（1小时）、d（天）。如果不指定该参数，则延迟时间默认为 0 秒，表示备库不延迟。
