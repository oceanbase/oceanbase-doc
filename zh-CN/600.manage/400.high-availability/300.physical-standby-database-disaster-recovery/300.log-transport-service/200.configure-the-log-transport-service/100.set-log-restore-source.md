...
2. 执行以下命令，配置备租户的恢复源指向主租户的归档目的端。
   ...
   * 备租户所在集群的 `sys` 租户配置备租户的恢复源
     ...
     ```shell
     obclient(root@sys)[(none)]> ALTER SYSTEM SET LOG_RESTORE_SOURCE ='LOCATION=archive_path' TENANT = tenant_name;
     ```

   * 备租户配置本租户的恢复源

     ```shell
     obclient> ALTER SYSTEM SET LOG_RESTORE_SOURCE ='LOCATION=archive_path';
     ```
