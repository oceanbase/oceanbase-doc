...
对于使用其他方式（创建空备租户和使用 `BACKUP DATABASE PLUS ARCHIVELOG` 功能创建备租户）创建的备租户，如果需要将日志恢复源设置为使用主租户的日志归档，则需要参考以下操作手动设置日志恢复源。此外，当备份介质为对象存储时，如果主租户或源端的备租户更改了备份介质的密钥信息，也需要参考一下操作重新设置日志恢复源。
