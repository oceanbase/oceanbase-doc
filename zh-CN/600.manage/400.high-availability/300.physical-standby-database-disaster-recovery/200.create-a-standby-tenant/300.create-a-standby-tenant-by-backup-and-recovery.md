...
1. 主租户或源端的备租户开启归档模式，并保证归档状态一直为 `DOING`。
   ...
   3. 确认归档状态是否为 `DOING`。
      ...
      * Oracle 租户
         ...
         ```shell
         obclient(root@oracle001)[SYS]>SELECT DEST_ID, ROUND_ID, DEST_NO, STATUS, CHECKPOINT_SCN, CHECKPOINT_SCN_DISPLAY, PATH FROM SYS.DBA_OB_ARCHIVELOG;
         ```
...
2. 主租户或源端的备租户开启数据备份，并完成一次全量备份。
   ...
   3. 确认数据备份状态。
      ...
      * 系统租户
         ...
         ```shell
         obclient(root@sys)[(none)]> SELECT * FROM oceanbase.CDB_OB_BACKUP_JOB_HISTORY;
         ```

      * MySQL 租户

         ```shell
         obclient(root@mysql)[(none)]> SELECT * FROM oceanbase.DBA_OB_BACKUP_JOB_HISTORY;
         ```

      * Oracle 租户

         ```shell
         obclient(sys@oracle001)[SYS]> SELECT * FROM SYS.DBA_OB_BACKUP_JOB_HISTORY;
         ```
...
4. 执行 `RESTORE` 命令，使用租户的数据备份和日志归档功能，恢复出备租户。
   ...
   * `WITH KEY FROM 'backup_key_path' ENCRYPTED BY 'password'`：指定加密租户的秘钥备份信息。仅当源租户配置了透明加密，才需要在恢复时指定秘钥备份相关的信息。
   ...
   ```shell
   obclient(root@sys)[(none)]> ALTER SYSTEM RESTORE standby_tenant 
...
2. 根据实际使用场景，执行 `RECOVER` 命令，更改新备租户的恢复终点。
   ...
   * 新备租户所在集群的 `sys` 租户为新备租户开启日志持续同步
     ...
     ```shell
     obclient> ALTER SYSTEM RECOVER STANDBY TENANT = standby_tenant UNTIL UNLIMITED;
     ```
...
3. 再次查询 `DBA_OB_TENANTS` 视图，通过 `SYNC_SCN` 字段确认新备租户的同步位点。
   ...
   * 新备租户所在集群的 `sys` 租户查询视图
     ...
     ```shell
     obclient(root@sys)[oceanbase]> SELECT TENANT_NAME, TENANT_TYPE, CREATE_TIME, STATUS, TENANT_ROLE,SCN_TO_TIMESTAMP(SYNC_SCN)  FROM oceanbase.DBA_OB_TENANTS WHERE TENANT_NAME = 'standby_tenant';
     ```
   ...
   * 新备租户查询视图
     ...
     * MySQL 租户
       ...
       ```shell
       obclient(root@standby_tenant)[oceanbase]> SELECT TENANT_NAME, TENANT_TYPE, CREATE_TIME, STATUS, TENANT_ROLE,SCN_TO_TIMESTAMP(SYNC_SCN)  FROM oceanbase.DBA_OB_TENANTS WHERE TENANT_NAME = 'standby_tenant';
       ```
     ...
     * Oracle 租户
       ...
       ```shell
       obclient(sys@standby_tenant)[SYS]> SELECT TENANT_NAME, TENANT_TYPE, CREATE_TIME, STATUS, TENANT_ROLE,SCN_TO_TIMESTAMP(SYNC_SCN)  FROM SYS.DBA_OB_TENANTS WHERE TENANT_NAME = 'standby_tenant';
       ```
