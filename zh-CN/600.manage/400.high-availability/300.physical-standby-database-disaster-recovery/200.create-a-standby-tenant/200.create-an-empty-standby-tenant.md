...
* `DBA_OB_LOG_RESTORE_SOURCE`：用于备切主时查询原主租户的上游信息，以判断是否可以在执行 Switchover 时不修改保护模式。

  关于 `DBA_OB_LOG_RESTORE_SOURCE` 视图的更多介绍，参见 [DBA_OB_LOG_RESTORE_SOURCE](../../../../700.reference/700.system-views/400.system-view-of-mysql-mode/200.dictionary-view-of-mysql-mode/9500.o-dba_ob_log_restore_source-of-mysql-mode.md)。
...
3. 给用户授权。
   ...
   以下示例是将 `oceanbase` 数据库下所有表的 `SELECT` 权限授予该用户。您也可以仅授予 `oceanbase` 库下 `GV$OB_LOG_STAT`、`GV$OB_UNITS`、`GV$OB_PARAMETERS`、`DBA_OB_ACCESS_POINT`、`DBA_OB_TENANTS`、`DBA_OB_LS`、`DBA_OB_LOG_RESTORE_SOURCE` 等视图的 SELECT 权限。
