...
* 创建空备租户
  ...
  2. 执行以下命令，查看主租户上的日志流对比信息。
     ...
     * 主租户所在集群的 `sys` 租户查询主租户的日志流对比信息
       ...
       ```sql
       (SELECT LS_ID FROM oceanbase.CDB_OB_LS_HISTORY WHERE TENANT_ID = xxx) EXCEPT (SELECT LS_ID FROM oceanbase.CDB_OB_LS WHERE TENANT_ID = xxx);
       ```
  ...
  3. 执行以下命令，查看主租户上的日志流信息。
     ...
     * 主租户所在集群的 `sys` 租户查询主租户的日志流信息
       ...
       ```sql
       SELECT LS_ID, BEGIN_LSN FROM oceanbase.GV$OB_LOG_STAT WHERE TENANT_ID = 1002 AND ROLE = 'LEADER' ;
       ```
