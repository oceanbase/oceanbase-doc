...
此外，仲裁服务（arbserver）也内置了标准的 Prometheus `/metrics` 接口，用于暴露 arbserver 自身的 CPU、内存、磁盘使用率，以及集群、租户、LS 状态、选举和 RPC 延迟等指标。仲裁服务的 Prometheus 监控为独立指标体系（`arb_*` 指标族），与 observer 的 `GV$`/`V$` 视图体系互不影响。有关仲裁服务监控的详细配置和指标说明，请参见 [仲裁服务 Prometheus 监控](./500.arbitration-service-monitoring.md)。
