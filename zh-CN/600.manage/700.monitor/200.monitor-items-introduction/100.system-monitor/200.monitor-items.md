...
| rpc net frame delay | 在分发 RPC 时 RPC 框架的时延，即当前时间（now）减去接收时间 |
...
| cpu usage | 逻辑cpu使用率,例如148表示百分比148%,该租户用了一个多核 |
...
| clog trans log total size | MEMTable mutator size 和 `trans_header` 之和 |
