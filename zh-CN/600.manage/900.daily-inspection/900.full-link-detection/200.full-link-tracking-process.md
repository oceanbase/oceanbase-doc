...
* 级别 1
  ...
  * `ob_proxy` 为 ODP 对一条 SQL 处理的全部耗时，即 OceanBase 数据库除前端驱动层以外的从数据库请求开始到数据处理完成，结果反馈的整条链路耗时。

  * `ob_proxy_server_process_req` 为 ODP 对一条 SQL 请求的处理耗时和访问来回的网络开销。
...
* 级别 2
  ...
  * `ob_proxy_cluster_resource_create` 为 ODP 在请求转发阶段，准备集群资源的耗时。
...
* 级别 3
  ...
  * `ob_proxy_partition_location_lookup` 为 ODP 在请求转发阶段，获取 partition location 准备进行路由的耗时。

  * `ob_proxy_do_observer_open` 为 ODP 在请求转发阶段，选择 OBServer，connect 等流程的耗时。

  * `ob_proxy_client_response_write` 为 ODP 在收完 OBServer 的 response 之后，将 response 转发给客户端的耗时。

  * `ob_proxy_server_request_write` 为 ODP 将客户端请求，转发给 OBServer 的耗时。
