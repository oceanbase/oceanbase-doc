...
7. 分区 Transfer 成功后，再次查看对应的分区信息。
    ...
    根据结果可知，`SYS` 库下的表 `TBL1_LOG_R` 的 `M202005` 分区，从之前的 `1002` 号日志流迁移到了 `1003` 号日志流上，分区 Transfer 成功。
...
5. Transfer 分区的命令执行成功后，可以通过以下视图查看任务状态。
...
6. 分区 Transfer 成功后，再次查看对应的分区信息。
    ...
    根据结果可知，`test` 库下的表 `tbl1_l` 的 `p1` 分区，从之前的 `1001` 号日志流迁移到了 `1003` 号日志流上，分区 Transfer 成功。
