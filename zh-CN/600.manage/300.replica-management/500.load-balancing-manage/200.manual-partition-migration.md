...
* 关闭租户的 Transfer 功能（即租户下配置项 `enable_transfer` 的值为 `false`）后，对该租户执行 `TRANSFER PARTITION` 语句会报错，已经执行成功了的 `TRANSFER PARTITION` 语句，其对应的分区迁移任务可能会被取消。
...
6. 分区迁移过程中，可以通过以下视图查看分区迁移情况。
   ...
   1. 查询视图 `CDB_OB_TRANSFER_PARTITION_TASKS`，获取任务的 `TASK_ID`、`TRANSFER_TASK_ID` 和 `BALANCE_JOB_ID`。
      ...
      视图 `CDB_OB_TRANSFER_PARTITION_TASKS` 展示了所有租户当前正在处理的分区迁移任务。查询示例如下：
  ...
  4. 根据获取的 `TASK_ID` 查询视图 `CDB_OB_TRANSFER_PARTITION_TASK_HISTORY`，确认分区迁移任务的结果。

      视图 `CDB_OB_TRANSFER_PARTITION_TASK_HISTORY` 展示了所有租户已执行的分区迁移任务历史。
      ...
      查询结果中的 `STATUS` 列展示了分区迁移任务的结果：

      * `COMPLETED`：表示分区迁移任务执行成功。

      * `CANCELED`：表示分区迁移任务被取消。
...
6. 分区迁移过程中，可以通过以下视图查看分区迁移情况。
   ...
   1. 查询视图 `DBA_OB_TRANSFER_PARTITION_TASKS`，获取任务的 `TASK_ID`、`TRANSFER_TASK_ID` 和 `BALANCE_JOB_ID`。
      ...
      视图 `DBA_OB_TRANSFER_PARTITION_TASKS` 展示了本租户下当前正在处理的分区迁移任务。查询示例如下：
  ...
  4. 根据获取的 `TASK_ID` 查询视图 `DBA_OB_TRANSFER_PARTITION_TASK_HISTORY`，确认分区迁移任务的结果。

      视图 `DBA_OB_TRANSFER_PARTITION_TASK_HISTORY` 展示了本租户已执行的分区迁移任务历史。查询示例如下：
...
* [CDB_OB_TRANSFER_PARTITION_TASKS](暂时无法添加)
* [DBA_OB_TRANSFER_PARTITION_TASKS](暂时无法添加)
* [CDB_OB_TRANSFER_PARTITION_TASK_HISTORY](暂时无法添加)
* [DBA_OB_TRANSFER_PARTITION_TASK_HISTORY](暂时无法添加)
