...
由于调整定时分区均衡任务单次任务执行的超时时间实际上调整的是所有通过调用 `TRIGGER_PARTITION_BALANCE` 子程序而触发的后台分区均衡任务的执行超时时间，故调整该值同时也会影响通过手动方式触发的分区均衡任务。有关手动触发分区均衡的详细操作，参见 [动触发分区均衡任务](200.manually-trigger-a-partition-balancing-task.md)。
