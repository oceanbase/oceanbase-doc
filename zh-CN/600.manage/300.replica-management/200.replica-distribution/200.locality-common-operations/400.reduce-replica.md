...
当您通过修改 Locality 减少副本时，正常情况下，系统会自动删除多余的副本，但由于一些异常原因，导致系统未正常工作，您可以通过 `ALTER SYSTEM REMOVE REPLICA` 命令手动删除多余的日志流副本。
...
5. 执行删除副本的命令。
   ...
   * `svr_ip:svr_port`：指定待删除副本所在服务器的 IP 及 RPC 端口号。示例：`172.xx.xxx.212:2882`。
