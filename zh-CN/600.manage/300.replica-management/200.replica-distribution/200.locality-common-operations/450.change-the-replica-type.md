...
您可以根据业务需要，将租户下日志流的 F 副本转换为 R 副本或者 R 副本转换为 F 副本。
...
当您通过修改 Locality 的方式修改副本类型时，正常情况下，系统会自动转换副本类型，但由于一些异常原因，导致系统未正常工作，您可以通过 `ALTER SYSTEM MODIFY REPLICA` 命令手动进行副本类型转换。
...
5. 执行副本类型转换命令。
   ...
   * `svr_ip:svr_port`：指定待转换副本类型的目标服务器的 IP 及 RPC 端口号。示例：`172.xx.xxx.192:2882`。
