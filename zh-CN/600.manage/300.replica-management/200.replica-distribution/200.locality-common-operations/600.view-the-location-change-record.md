...
|dir-name-en||
|tenant-type||
|slug|check-locality-change-history|
...
3. 查看 Locality 变更任务。
   ...
   * 查看某一租户上的历史 Locality 变更记录。
     ...
     ```shell
     obclient(root@sys)[oceanbase]> SELECT * FROM oceanbase.DBA_OB_TENANT_JOBS WHERE JOB_TYPE = 'ALTER_TENANT_LOCALITY' AND tenant_id = 1002 ORDER BY START_TIME DESC LIMIT 10;
     ```
