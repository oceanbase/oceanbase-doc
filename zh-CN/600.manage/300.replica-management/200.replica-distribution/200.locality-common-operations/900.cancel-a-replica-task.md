...
由于当前容灾任务的执行有限制，即一个租户的一个日志流同时只允许执行一个副本任务，导致有一些紧急任务可能需要等待正在执行中的任务完成后才能执行该紧急任务。如果需要优先执行紧急任务，您需要使用 `ALTER SYSTEM CANCEL REPLICA TASK` 命令手动取消正在执行的副本任务后，再执行紧急任务。

## 使用限制

* 当前版本仅支持取消补副本和迁移副本等两种类型的副本任务。
...
* 在查询视图前，请确认当前用户拥有以下视图的 `SELECT` 权限，否则无法查询相关信息。
  ...
  * `DBA_OB_LS_REPLICA_TASKS_HISTORY`/ `CDB_OB_LS_REPLICA_TASKS_HISTORY`
...
2. 查看当前正在执行的容灾任务。
    ...
    * 系统租户
      ...
      有关视图 `CDB_OB_LS_REPLICA_TASKS` 中各字段的详细说明，请参见 [CDB_OB_LS_REPLICA_TASKS](../../../)。
    ...
    * 用户租户
      ...
      有关视图 `DBA_OB_LS_REPLICA_TASKS` 中各字段的详细说明，请参见 [DBA_OB_LS_REPLICA_TASKS](../../../)。
...
4. 查看副本任务的取消结果。
    ...
    * 系统租户
      ...
      ```shell
      obclient [oceanbase]> SELECT * FROM oceanbase.CDB_OB_LS_REPLICA_TASKS_HISTORY WHERE TASK_ID = 'Y13CE64586BD4-000610C5F3EDBBCB-0-0';
      ```

      有关视图 `CDB_OB_LS_REPLICA_TASKS_HISTORY` 中各字段的详细说明，请参见 [CDB_OB_LS_REPLICA_TASKS_HISTORY](../../../)。
    ...
    * 用户租户
      ...
      ```shell
      obclient [oceanbase]> SELECT * FROM oceanbase.DBA_OB_LS_REPLICA_TASKS_HISTORY WHERE TASK_ID = 'Y13CE64586BD4-000610C5F3EDBBCB-0-0';
      ```
      ...
      ```shell
      obclient [SYS]> SELECT * FROM SYS.DBA_OB_LS_REPLICA_TASKS_HISTORY WHERE TASK_ID = 'Y13CE64586BD4-000610C5F3EDBBCB-0-0';
      ```
      ...
      有关视图 `DBA_OB_LS_REPLICA_TASKS_HISTORY` 中各字段的详细说明，请参见 [DBA_OB_LS_REPLICA_TASKS_HISTORY](../../../)。
