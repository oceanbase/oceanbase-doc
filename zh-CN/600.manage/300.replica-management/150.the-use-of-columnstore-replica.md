...
在 OceanBase 数据库中，C 副本仅将 F 副本上为行存的用户表的用户分区转换为列存存储，但该描述仅表示 “最终” 状态为列存。在列存副本上，并非任意时刻分区均表现为列存形式，在以下场景中，列存副本上的用户表分区会暂时为行存，需要系统自发调度行转列任务，将最新的基线数据转换为列存。

| 场景             | 描述                                                          |
|-----------------|---------------------------------------------------------------|
| 补副本（增加副本）             | 以修改 Locality 使得 `F@z1, F@z2` 变更为 `F@z1, F@z2, C@z3` 为例：<ul><li>行存基线从源端（F 或者 R 副本）迁移到目的端（C 副本）时，会先拷贝行存到 C 副本上，并尝试为该 Tablet 发起行转列。</li> <li>由于行转列也是一类特殊的合并，如果多次尝试行转列失败（例如 dag scheduler 队列满或者 IO 压力大/磁盘空间不足等），系统就会将日志流的 `migration_status` 修改为 `None` 使其对外可见，并可以提供基于行存的弱读查询。</li></ul>|
...
| 全量旁路导入                   | 全量旁路导入当前仅支持先往 C 副本上导入行存，待后台调度行转列任务后转为列存。|
| 表级恢复                       | 由于当前列存表暂不支持表级恢复，因此 C 副本上的表级恢复也仅支持先恢复为行存，待后台调度行转列任务后再转为列存。|

上述场景下，在进行行存转列存的过程中，尽管此时优化器生成的是列存查询计划，但实际执行的仍然是对行存基线的查询。用户可以通过视图 `CDB_OB_CS_REPLICA_STATS`（系统租户）和 `DBA_OB_CS_REPLICA_STATS`（用户租户）来查询列存副本的可用信息及行转列的进度，待行转列完全结束后，再进行列存副本上的查询。
