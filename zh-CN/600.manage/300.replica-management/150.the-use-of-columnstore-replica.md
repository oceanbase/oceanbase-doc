...
| 场景             | 描述                                                          |
|-----------------|---------------------------------------------------------------|
| 补副本（增加副本）             | 以修改 Locality 使得 `F@z1, F@z2` 变更为 `F@z1, F@z2, C@z3` 为例：<ul><li>行存基线从源端（F 或者 R 副本）迁移到目的端（C 副本）时，会先拷贝行存到列存副本上，并尝试为该 Tablet 发起行转列。</li> <li>由于行转列也是一类特殊的合并，如果多次尝试行转列失败（例如 dag scheduler 队列满或者 IO 压力大/磁盘空间不足等），系统就会将日志流的 `migration_status` 修改为 `None` 使其对外可见，并可以提供基于行存的弱读查询。</li></ul>|
...
| 全量旁路导入                   | 全量旁路导入当前仅支持先往列存副本上导入行存，待后台调度行转列任务后转为列存。|
| 表级恢复                       | 由于当前列存表暂不支持表级恢复，因此列存副本上的表级恢复也仅支持先恢复为行存，待后台调度行转列任务后再转为列存。|
