...
在 OceanBase 数据库中，如果一张表在被创建时为行存表，则系统会在列存副本上会创建对应的纯列存表；如果一张表在被创建时为列存表，则在列存副本上的存储方式将与 F 副本保持一致。故，对于列存副本而言，仅负责将 F 副本上行存的用户表的用户分区转换为列存存储，且该描述仅表示 “最终” 状态为列存。在列存副本上，并非任意时刻分区均表现为列存形式，在以下场景中，列存副本上的用户表分区会暂时为行存，需要系统自发调度行转列任务，将最新的基线数据转换为列存。
