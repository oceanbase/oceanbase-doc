...
Nested Loop Join 可能会对内表进行多次全表扫描，因为每次扫描都需要从存储层重新迭代一次，这个代价相对比较高，所以 OceanBase 数据库支持对内表进行一次扫描并把结果物化在内存中，这样在下一次执行扫描时就可以直接在内存中扫描相关的数据，而不需要从存储层进行多次扫描。但是物化在内存中的方式是有代价的，所以 OceanBase 数据库的优化器基于代价去判断是否需要物化内表。
...
* 缓存块嵌套循环联接（Blocked Nested Loop Join）
  ...
  在 OceanBase 数据库中，Blocked Nested Loop Join 的实现方式是 Batch Nested Loop Join，通过从外表中批量读取数据行（默认是 1000 行），然后再去扫描内表找到满足条件的数据。这样将批量的数据与内层表的数据进行匹配，减少了内表的读取次数和内层循环的次数。
...
* 索引嵌套循环联接（Index Nested Loop Join）
  ...
  *************************** 1. row ***************************
  ...
  在 `outputs & filters` 的输出结果中 `nl_param` 出现参数 `[t2.c1]`，说明执行了条件下压优化。详细信息请参考 [JOIN](/zh-CN/9.sql-tuning-guide/2.sql-execution-plan/2.execution-plan-operators/3.join.md)。
