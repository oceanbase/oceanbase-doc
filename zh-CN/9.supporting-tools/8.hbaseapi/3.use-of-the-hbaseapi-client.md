...
在进行本文操作之前，您可以先了解下列 OceanBase 的基本知识：

+ [HBaseAPI方案介绍](1.introduction-to-hbaseapi-scheme.md)
+ [如何部署OB集群](https://open.oceanbase.com/docs/observer-cn/V3.1.1/10000000000013235)
+ [如何连接OB数据库](https://open.oceanbase.com/docs/observer-cn/V3.1.2/10000000000015365)

## HBaseAPI 部署

### 添加客户端依赖

添加 HBaseAPI 客户端jar包依赖到本地 java 工程的 pom.xml 文件（或参考 [HBaseAPI 使用 Demo](https://github.com/oceanbase/obkv-hbase-client-java/tree/main/example/simple-hbase-demo)）。
...
使用 OBClient 或 MySQL 连接 OBserver 集群，并建立 HBase 相关数据表，请参考 [HBaseAPI 存储模型](2.hbaseapi-storage-models.md)。
...
登录 OCP 网站后，选中集群选项，得到 ConfigURL 地址。该参数在客户端初始化中需要配置。
如下图，当 OCP 拉起集群后，ConfigUrl 一栏会显示IP地址。
![TUPIAN]()

#### 从 OBD 部署的集群获取

由于 HBaseAPI 的使用需要依赖 rslist 列表获取，目前暂不支持使用 OBD 部署的集群模式。

### 设置客户端对OBServer的登录用户&密码
...
```Plaintext
grant select on oceanbase.* to proxyro identified by 'your passwd'; 
```
...
相关 HBaseAPI 使用的 Demo 请参考：[HBaseAPI使用Demo](https://github.com/oceanbase/obkv-hbase-client-java/tree/main/example/simple-hbase-demo)。
