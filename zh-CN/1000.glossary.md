...
* 列存副本：与只读型副本类似，区别在于 Major SSTable 的存储格式为列存格式。

在 V4.x 版本中，OceanBase 数据库支持的副本类型如下表所示。

| **类型** |  **LOG**  |  **MemTable**  | **SSTable**  | **数据安全** | **恢复为 Leader 时间** | **资源成本** | **服务** | **名称(简写)**  |
|--------|------|----|-------|----------|-----------------|----------|------------------------------|-------------|
| 全能型    | 有，参与投票（SYNC_CLOG） | 有（WITH_MEMSTORE） | 有（WITH_SSSTORE），且 Major SSTable 为行存格式    | 高    | 快    | 高  | Leader 提供读写，Follower 可非一致性读 | FULL(F)     |
| 只读型    | 有，异步日志，但不属于 Paxos 组，只是 listener（ASYNC_CLOG） | 有（WITH_MEMSTORE）    | 有（WITH_SSSTORE） ，且 Major SSTable 为行存格式     | 中   | 不支持    | 高        | 可非一致性读                       | READONLY(R) |
| 列存副本 | 有，异步日志，但不属于 Paxos 组，只是 listener（ASYNC_CLOG） | 有（WITH_MEMSTORE）    | 有（WITH_SSSTORE），且 Major SSTable 为列存格式      | 中   | 不支持    | 高        | 可非一致性读                       | COLUMNSTORE(C) |
