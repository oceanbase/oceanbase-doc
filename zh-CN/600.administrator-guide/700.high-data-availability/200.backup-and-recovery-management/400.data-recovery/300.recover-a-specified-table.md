...
2. 执行以下语句，开始执行恢复任务。
   ...
   |         参数         |                描述                 |
   |--------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
   | table_name_list    | 指定待恢复的租户内的表，表名需要包含表所在的数据库名，多个表名之间中用英文逗号（,）连接。 单个表的表示方法：`database_name.table_name` 或 `user_name.table_name` 示例：`crm.sales,crm.products,hr.employees` </br>**说明**  </br>数据库名或表名可能含有特殊字符，对于含特殊字符的数据库名或表名需要放在反引号（\`\`）内。例如：```crm`.`sales`, `crm`.`products```。                 |
   ...
   | restore_options    | 支持 `backup_cluster_name`、`backup_cluster_id`、`pool_list`、`locality`、`kms_encrypt`： <ul><li>`backup_cluster_name` 为必选项，填写源集群的名称。</li>   <li> `backup_cluster_id` 为必选项，填写源集群的 `cluster_id`   </li>   <li>`pool_list` 为必选项，填写用户的资源池。</li>   <li> `locality` 为可选项，填写租户的 Locality 信息。   </li>   <li> `kms_encrypt` 为可选项，为 `true` 则表示在恢复时需要使用步骤 5 指定的 `kms_encrypt_info`。 </li></ul>   |
