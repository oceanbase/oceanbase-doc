...
* `PARTITION BY RANGE ( expr )` 里的 `expr` 表达式的结果必须为整型或 YEAR 类型。
...
Range 分区要求表分区键表达式的结果必须为整型或 YEAR 类型，如果要按其他时间类型列做 Range 分区，则必须使用 Timestamp 类型，并且使用函数 UNIX_TIMESTAMP 将时间类型转换为数值。这个需求也可以使用 Range Columns 分区实现，并且不需要表拆分键表达式的结果为整型。
