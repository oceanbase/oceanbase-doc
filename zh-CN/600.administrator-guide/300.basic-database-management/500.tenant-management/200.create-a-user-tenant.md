...
2. 查询系统资源占用情况，例如：
   ...
   +--------------+----------+-----------+--------------+---------------+-------+
   | xx.xx.xx.2   |    33332 |        30 | 236223201280 | 1434105937920 | zone1 |
   | xx.xx.xx.3   |    33332 |        30 | 236223201280 | 1434105937920 | zone2 |
   | xx.xx.xx.1   |    33332 |        30 | 236223201280 | 1434105937920 | zone3 |
   +--------------+----------+-----------+--------------+---------------+-------+
   ```
...
5. 创建资源池。
   ...
   3. 如果在某个 zone 内找不到足够剩余资源的机器来创建资源单元，资源池会创建失败。
      ...
      详细信息，参考 [资源管理概述](../400.resource-management/100.overview-of-resource-management.md)。
...
6. 创建租户。
   ...
   详细信息，参考 [租户管理概述](../500.tenant-management/100.overview-of-tenant-management.md)。
...
首次使用新租户连接 OceanBase 数据库时，您必须使用 `root` 用户。成功连接后，您可以创建新用户连接 OceanBase 数据库。详细信息，参考 [创建用户](../../700.manage-user-privileges/200.create-a-user.md) 文档。
