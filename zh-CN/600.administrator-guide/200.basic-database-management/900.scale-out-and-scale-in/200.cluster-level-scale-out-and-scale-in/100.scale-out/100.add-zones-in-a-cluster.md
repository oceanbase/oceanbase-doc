...
5. 通过修改租户的 Locality 来增加副本。
   ...
   根据 Locality 的变更规则，每次只能增加一个 Zone 内的 Locality，Locality 的变更规则相关信息请参见 [Locality 概述](../../../../500.distributed-storage-management/400.manage-locality/100.overview-of-locality.md)。
...
2. 向集群中增加 2 个 Zone 同时在 Zone 内各添加 1 台 OBServer。
   ...
   通过 OCP 向集群中增加 Zone 的相关操作请参见 [增加或删除 Zone](../../../100.manage-clusters/400.manage-zones-in-a-cluster/200.add-or-delete-zones.md)。
   

3. 添加成功后，在租户内，依次为新增的 2 个 Zone 新增副本。

   通过 OCP 新增副本的相关操作请参见 [新增副本](../../../../500.distributed-storage-management/300.manage-replicas/200.add-replicas.md)。
