...
| location | 表示位置。它是系统已知的一组枚举值。  `location` 的值为 Zone 的名称。集群和租户中的各 Zone 的名称的查看方法，可参见 [查看集群拓扑图](../../2.basic-database-management/1.manage-clusters/6.view-cluster-topology.md)。                                |
