...
将一级分区表加入表组后，您还可以执行将一级分区表从表组中移出、将一级分区表迁移到其他表组以及删除表组等操作，这些操作与普通表组相同，具体操作请参见 [表组管理命令](../../400.database-object-management/300.manage-table-groups/200.table-group-management-statements.md)。
