| Description   |                 |
|---------------|-----------------|
| keywords      |                 |
| dir-name      |                 |
| dir-name-en   |                 |
| tenant-type   | MySQL Mode      |

# 向量检索实践教程概述

本文档将介绍如何基于 OceanBase 构建向量检索的典型应用场景。

## 向量检索实践教程

### 基于 OceanBase 构建智能问答机器人

文档智能助手是一种基于 OceanBase 数据库的应用，它将文档以向量的形式批量存储。用户可以通过 UI 界面向它提问，它会使用 BGE-M3 模型将提问内容嵌入成为向量，并在数据库中检索相似向量。当找到相似向量对应的文档内容后，应用会将它们会同用户提问一起发送给 LLM，LLM 会根据提供的文档生成更加准确的回答。

如果你想了解更多关于如何构建文档智能助手的信息，请参见[基于 OceanBase 构建智能问答机器人](../../690.practical-tutorial/200.building-an-intelligent-question-answering-robot-based-on-OceanBase.md)。

### 基于 OceanBase 构建图搜图应用

图搜图应用是另一种基于 OceanBase 数据库的应用，它将图片库以向量形式存储在数据库内。用户可以在对应的 UI 界面上传需要查询的图片，图片会被应用转换为向量，并在数据库内查询相似向量。最终，它会以图片形式，在 UI 页面上展示相似图片。

如果你想了解更多关于如何构建图搜图应用的信息，请参见[基于 OceanBase 构建图搜图应用](../../690.practical-tutorial/100.building-a-graph-search-application-based-on-oceanBase.md)。
