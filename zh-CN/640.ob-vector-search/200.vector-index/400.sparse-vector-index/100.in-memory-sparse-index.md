...
本文档介绍 OceanBase 数据库中基于内存的稀疏向量索引的创建、搜索和使用方法。

## 概述

基于内存的稀疏向量索引是 OceanBase 数据库针对稀疏向量数据（大部分元素为零的向量）提供的高效索引类型。基于内存的稀疏向量索引需要将索引完整的载入内存，支持 DML 和实时搜索。

为了提升稀疏向量的查询性能，OceanBase 数据库集成了 VSAG 算法库的稀疏向量索引（SINDI），该索引在性能上优于基于磁盘的稀疏向量索引，适合在内存资源充足的情况下使用。
