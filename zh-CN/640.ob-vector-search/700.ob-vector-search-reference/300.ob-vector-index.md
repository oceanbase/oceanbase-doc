...
| type |  | hnsw | 是 | 指定索引类型。 | 目前支持 `hnsw` / `hnsw_sq` / `ivf` / `ivf_sq8` / `ivf_pq`。 |
| lib | vsag | vsag | 否 | 指定向量索引库类型。 | 目前仅支持 VSAG 向量库。 |
| m | 16 | [5,64] | 否 | 向量索引构建参数，每个节点的最大邻居数。 | 值越大，索引构建越慢，查询性能越好。 |
| ef_construction | 200 | [5,1000] | 否 | 向量索引构建参数，构建索引时的候选集大小。 | 值越大，索引构建越慢，索引质量越好。`ef_construction` 必须大于 `m`。|
| ef_search | 64 | [1,1000] | 否 | 向量索引查询参数，查询时的候选集大小。 | 值越大，查询越慢，召回率越高。 |
