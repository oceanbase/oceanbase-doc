...
* （实验特性）支持 IVF/IVF SQ/IVF PQ 索引：基于数据库表实现的 IVF 索引，可不占用较消耗性能的常驻内存，适用于对性能要求不高，但数据量较大，成本敏感的场景。IVF SQ 和 IVF PQ 索引则可以进一步降低存储成本。
...
* V4.3.5 BP1 版本针对带有过滤条件的向量查询调整了策略，解决了旧版本某些场景下查询结果少于预期的问题。
...
* HNSW SQ 索引/IVF 索引/IVF SQ 索引/IVF PQ 索引均建议在写入数据后再创建索引，并在写入较多增量数据后进行索引重建。每个索引具体的创建说明见下文具体示例。
...
<main id="notice" type='notice'>
  <h4>注意</h4>
  <p>IVF/IVF SQ/IVF PQ 索引由于其索引结构的限制，不支持增量刷新机制。</p>
</main>
