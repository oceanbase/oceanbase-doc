...
* 支持 HNSW SQ 索引：HNSW SQ 索引提供了和 HNSW 索引相近的构建速度，查询性能，召回率，但总的内存使用降低到原本的 1/2～1/3，适用于对性能和召回率有较高要求的场景。

* （实验特性）支持 IVF/IVF SQ/IVF PQ 索引：基于数据库表实现的 IVF 索引，可不占用较消耗性能的常驻内存，适用于对性能要求不高，但数据量较大，成本敏感的场景。IVF SQ 和 IVF PQ 索引则可以进一步降低存储成本，提高查询性能。
...
使用 HNSW/HNSW SQ 向量索引前，需要通过设置 [ob_vector_memory_limit_percentage](../../700.reference/800.configuration-items-and-system-variables/100.system-configuration-items/400.tenant-level-configuration-items/6150.ob_vector_memory_limit_percentage.md) 开启向量功能。IVF/IVF SQ/IVF PQ 索引不需要常驻内存，所以无需手动设置此参数来开启向量功能。
...
* HNSW SQ 索引/IVF 索引/IVF SQ 索引/IVF PQ 索引均建议在写入一定数据量后再创建索引，或在写入一定数据量后进行索引重建。每个索引具体的创建说明见下文具体示例。
...
<main id="notice" type='notice'>
  <h4>注意</h4>
  <p>创建 HNSW 索引时，建议索引名长度不超过 25 个字符，否则可能因为索引辅助表名长度超过 <code>index_name</code> 限制导致异常。</p>
</main>
...
<main id="notice" type='notice'>
  <h4>注意</h4>
  <p>创建 IVF 索引时，建议索引名长度不超过 33 个字符，否则可能因为索引辅助表名长度超过 <code>index_name</code> 限制导致异常。</p>
</main>
...
| m |  | [1,dim] | 是 | IVF_PQ 索引构建参数，表示 PQ 压缩之后的维度。 |  |
| nlist | 128 | [1,65535] | 否 | 聚类中心的个数。 |  |
| samples_per_nlist | 256 | [1,unit64_max] | 是 | 每个聚类中心的取样的数据量，后建索引中使用。 |  |
...
```sql
CREATE VECTOR INDEX vec_idx5 ON vec_table_ivf_pq(c2) WITH (distance=l2, type=ivf_pq, m=1);
```
...
<main id="notice" type='notice'>
  <h4>注意</h4>
  <p>IVF/IVF SQ/IVF PQ 索引暂不支持增量刷新。</p>
</main>
