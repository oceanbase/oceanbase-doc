...
<main id="notice" type='notice'>
  <h4>注意</h4>
  <p>使用向量索引前，需要通过设置 <a href="../../700.reference/800.configuration-items-and-system-variables/100.system-configuration-items/400.tenant-level-configuration-items/6150.ob_vector_memory_limit_percentage.md">ob_vector_memory_limit_percentage</a>开启向量功能。</p>
</main>
...
* 使用 `APPROXIMATE`/`APPROX` 关键字时，必须指定 `ORDER BY` 和 `LIMIT` 语句。

* 调用距离函数时：
  * 未指定 `APPROXIMATE`/`APPROX` 的情况下，调用 `inner_product` / `negative_inner_product` / `L2`，查询计划不会选择向量索引路径，但也不会报错。
  * 指定 `APPROXIMATE`，距离函数与向量索引算法不匹配，查询计划不会选择向量索引路径，但也不会报错。
  - 指定 `APPROXIMATE`，表达式是 `inner_product`，查询计划不会选择向量索引路径。
  - 指定 `APPROXIMATE`，表达式是 `negative_inner_product` / `L2`，且与向量索引算法匹配，查询计划选择向量索引路径。
