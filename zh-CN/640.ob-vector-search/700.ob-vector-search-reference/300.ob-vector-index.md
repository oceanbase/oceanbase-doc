...
* 支持 HNSW/HNSW SQ 索引，索引列最大维度为 4096。HNSW/HNSW SQ 索引是内存索引，需要完整载入内存，支持 DML 和实时查询。

* （实验特性）支持 IVF/IVF SQ/IVF PQ 索引，索引列最大维度为 4096：IVF/IVF SQ/IVF PQ 索引都是内存索引，需要完整载入内存，支持 DML 和实时查询。
