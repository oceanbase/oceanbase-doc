...
<h4>说明</h4>
<p>该功能从 V4.4.1 版本开始引入。</p>
</main>

本文档介绍 OceanBase 数据库中 AI Function 的相关功能。
...
| CREATE_AI_MODEL_ENDPOINT | 创建一个 AI 模型服务连接点对象。 |
| ALTER_AI_MODEL_ENDPOINT | 修改一个 AI 模型服务连接点对象。 |
| DROP_AI_MODEL_ENDPOINT | 删除一个 AI 模型服务连接点对象。 |

你可以通过调用该系统包，实现在 Oceanbase 内部直接管理 AI 模型和端点，而不需要依赖外部服务。详细参数和语法请参考 [DBMS_AI_SERVICE](../../700.reference/600.pl-reference/200.pl-mysql/1000.pl-system-package-mysql/1000.dbms-ai-service-mysql/100.dbms-ai-service-overview-mysql.md) 包的相关文档。

## AI 模型调用监控

OceanBase 数据库支持通过视图查看和监控 AI 模型信息及调用情况：
...
<main id="notice" type='explain'>
<h4>说明</h4>
<p>使用 AI 函数表达式时，需要确保数据库中已注册 AI 模型。</p>
</main>
...
| parameters | 用于支持配置 API 提供的可选项。 | JSON | Yes |

必须指定 `model_key`、`prompt`，并且其中一个为 `NULL` 时，函数提示报错。

返回值：

* text(longtext)，大模型根据提示词生成的文本。
...
* vector(varbinary)，大模型根据提示词生成的向量。
