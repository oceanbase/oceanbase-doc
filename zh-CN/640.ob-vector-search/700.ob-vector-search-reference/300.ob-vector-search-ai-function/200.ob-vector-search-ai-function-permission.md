...
3. 调用 `CREATE_AI_MODEL_ENDPOINT` 程序
    ...
    ```shell
    ERROR 1227 (42501): Access denied; you need (at least one of) the no privilege to create ai model endpoint privilege(s) for this operation
    ```
...
1. 调用 `AI_COMPLETE` 函数
    ...
    ```shell
    ERROR 1227 (42501): Access denied; you need (at least one of) the no privilege to access ai model privilege(s) for this operation
    ```
