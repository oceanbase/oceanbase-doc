...
本文档介绍 OceanBase 的混合搜索 SQL 接口。该接口通过 `HYBRID_SEARCH` 子句直接在一条 `SELECT` 查询语句中组合全文、向量搜索与过滤条件，并返回按相关性融合后的结果。
...
* **`doc_json_array`**：用于 JSON、ARRAY 过滤混合搜索的示例表
...
这部分示例需要配合搜索索引使用，完整概念、语法和示例说明请参见文末相关文档 `搜索索引（Search Index）`。

#### JSON 过滤

支持 `json_contains`、`json_overlaps`、`json_member_of`、`json_extract` 几种过滤方式，这里仅介绍 `json_contains` 示例。
...
支持 `array_contains`、`array_contains_all`、`array_overlaps` 几种过滤方式，这里仅介绍 `array_contains` 示例。
...
SELECT id, tags_array1 FROM HYBRID_SEARCH(
  ...
  '{
    "query": {
      "bool" : {
        "filter" : [
          { "array_contains": { "tags_array1" : "mysql" }}
        ]
      }
    }
  }'
