...
对于同一列在同一个 `bool.must` 或 `bool.filter` 路径下的多个标量过滤条件，这些条件仍以 `AND` 逻辑取交集，筛选同时满足所有条件的记录。查询优化器会在逻辑计划阶段将这些条件合并，避免执行阶段重复迭代。如果是 OR下例设置 `c1 >= 3` 与 `c1 <= 5`（相当于 `c1 >= 3 AND c1 <= 5`），因此只返回 `c1` 为 3、4、5 的行。
