...
本示例混合搜索 `doc_table` 表中 `content` 列包含 `oceanbase mysql` 的 5 条记录，并返回所有列。RRF 算法可以把多个查询的排序结果合并为单一排名结果。
...
本示例搜索 `products_multi_vector` 表中 `vec1`、`vec2`、`vec3` 与 `[0.5,0.1,0.6,0.9]`、`[0.2,0.3,0.4,0.5]`、`[0.1,0.2,0.3,0.4]` 最相似的 3 条记录，并返回所有列。
...
这部分示例需要配合搜索索引使用，具体请参见[搜索索引（Search Index）](../../700.reference/100.oceanbase-database-concepts/400.database-objects/200.database-objects-of-mysql-mode/300.index-of-mysql-mode/340.search-indexes-of-mysql-mode-in-concepts.md)。
