...
<main id="notice" type='notice'>
  <h4>注意</h4>
  <p>稀疏向量索引，HNSW_BQ 索引和索引分区重建功能从 V4.3.5 BP2 版本开始支持。</p>
</main>
...
<main id="notice" type='notice'>
  <h4>注意</h4>
  <p>从 V4.3.5 BP2 版本开始支持自动分区重建功能，推荐使用，详见下文。</p>
</main>
...
<main id="notice" type='notice'>
  <h4>注意</h4>
  <p>此功能从 V4.3.5 BP2 版本开始支持，如果你的向量集群从旧版本升级到 V4.3.5 BP2，那么你需要在升级后先手动全表重建所有向量索引，否则升级后可能无法自动执行分区重建任务。</p>
</main>
...
1. 配置执行周期
    ...
    在 `oceanbase` 数据库下，通过配置项 `vector_index_optimize_duty_time` 配置执行周期。示例如下：

    ```sql
    ALTER SYSTEM SET vector_index_optimize_duty_time='[23:00:00, 24:00:00]';
    ```

    如上配置完成后，分区重建任务只会在 23:00:00~24:00:00 时间段执行，其他时间段里不会发起。

2. 查看任务进展/历史任务

    可通过 `DBA_OB_VECTOR_INDEX_TASKS` 或 `CDB_OB_VECTOR_INDEX_TASKS` 视图查看任务进展和历史任务。

    ```sql
    -- 查询视图示例，需在 MySQL 租户下
    SELECT * FROM DBA_OB_VECTOR_INDEX_TASKS;
    SELECT * FROM CDB_OB_VECTOR_INDEX_TASKS;
    ```
    ...
    * 3（FINISHED），表示任务已完成。
    ...
    执行结束的任务，即状态 `status=3` 的任务，无论是否执行成功都会被转存到历史表。更多参数信息请查看文末相关文档。
