...
| refine_k | 4.0 | [1.0,1000.0] | 否 | <main id="notice" type='notice'><h4>注意</h4><p>该参数从 V4.4.1 版本开始支持，仅在创建 HNSW_BQ 索引时可指定。</p></main>该参数为浮点数类型，用于调整量化向量索引的重排比例。 | 该参数可以在创建索引时设置，也可以在搜索时指定：<ul><li>如果搜索时没有设置，则使用创建索引时的值；</li><li>如果搜索时设置了，则使用搜索时设置的值。</li></ul> |
...
| bq_bits_query | 32 | 0/4/32 | 否 | <main id="notice" type='notice'><h4>注意</h4><p>该参数从 V4.4.1 版本开始支持，仅在创建 HNSW_BQ 索引时可指定。</p></main>用于设置量化向量索引的搜索精度，单位为 bit。 | 该值通过降低索引构建时的内存开销和索引构建时间来提高效率，但可能会影响召回率。 |
| bq_use_fht | true <main id="notice" type='notice'><h4>注意</h4><p>如果集群是从旧版本升级到 V4.4.1 版本，则该参数默认值为 false。</p></main>  | true/false | 否 | <main id="notice" type='notice'><h4>注意</h4><p>该参数从 V4.4.1 版本开始支持，仅在创建 HNSW_BQ 索引时可指定。</p></main>用于设置是否使用 FHT 进行搜索。FHT（Fast Hadamard Transform）是一种用于加速向量内积计算的算法。 |  |
...
该功能可以降低向量索引的内存使用，目前支持索引分区并行重建，支持设置并行度。

<main id="notice" type='notice'>
  <h4>注意</h4>
  <p>此功能仅支持 HNSW/HNSW_SQ/HNSW_BQ 索引。</p>
</main>
...
1. 配置并行度（可选）
    ...
    为合理利用集群资源，支持通过配置项 `vector_index_optimization_concurrency` 在 `oceanbase` 数据库中配置并行度。默认并行度为 2，并行度规则如下：
      * 若租户 CPU 数量小于 4，并行度固定为 1（即不并行）。
      * 若租户 CPU 数量大于等于 4，并行度将采用用户配置的值。

    示例如下：

    ```sql
    ALTER SYSTEM SET vector_index_optimization_concurrency=8;
    ```

    可以通过 `SHOW PARAMETERS` 命令或 `GV$OB_PARAMETERS` 视图查看配置项的当前值。
...
3. 查看任务进展/历史任务
    ...
    * 2（PENDING），表示任务暂停执行。
    * 3（FINISHED），表示任务已完成。
    * 4（EXCHANGE），表示交换索引辅助表新旧数据 // TODO：需要补充说明
    * 5（CLEAN），表示清理索引辅助表和元信息表旧数据。

    执行结束的任务，即状态 `status=FINISHED` 的任务，无论是否执行成功，都会以 10s 的频率转存到历史表。详细使用示例请参见相应视图文档。
