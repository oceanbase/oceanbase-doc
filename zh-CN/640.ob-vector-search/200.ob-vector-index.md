...
* **V4.4.1 版本之前，使用 HNSW/HNSW_SQ/HNSW_BQ 向量索引前**，需要手动设置 `ob_vector_memory_limit_percentage` 开启向量功能，建议设置为 `30`，以获得最佳的检索性能。如保持默认值，则表示不为向量索引分配内存，创建索引会报错。IVF/IVF_SQ/IVF_PQ 索引不需要常驻内存，所以无需关注此参数。配置示例如下：
    ```sql
