...
OceanBase 向量搜索内存通过 [ob_vector_memory_limit_percentage](../../700.reference/800.configuration-items-and-system-variables/100.system-configuration-items/400.tenant-level-configuration-items/6150.ob_vector_memory_limit_percentage.md) 参数配置。
...
<main id="notice" type='notice'>
  <h4>注意</h4>
  <p>目前仅支持后建 IVF/IVF_PQ 索引，不支持随表创建。</p>
</main>
...
| nlist | 128 | [1,65536] | 否 | 聚类中心的个数。 | <ul><li>如果业务数据少于 100W，推荐值为 `数据量/1000`。</li><li>如果业务数据大于 100W，推荐值为 `sqrt(数据量)`。</li>多分区表场景，`数据量`按照最大分区的数据量来估算。</ul>  |
| sample_per_nlist | 256 | [1,int64_max] | 是 | 每个聚类中心的取样的数据量，后建索引中使用。 | 当前版本推荐保持默认值，不建议修改。 |
...
**针对 IVF/IVF_PQ 索引**，有以下额外说明：

* 索引限制：
  * 不支持在堆表和分区表上创建 IVF/IVF_PQ 索引。
  * 不建议将主表在创建表的时候设置 `ROW_FORMAT=COMPRESSED` 压缩格式。
  * 不建议弱读场景下使用 IVF 索引。
* 构建建议：
  * 强烈建议在数据导入完成后再创建 IVF/IVF_PQ 索引。因为在创建时若无数据，索引无法进行聚类，导致无法使用。此时需要手动执行 `REBUILD INDEX` 或 `DROP` 后重新 `CREATE INDEX`。
...
* 搜索语法要求：
  ...
  * `PARAMETERS` 子句用于指定搜索参数。对于 IVF/IVF_PQ 索引，支持 `nprobes` 参数，例如 `PARAMETERS(nprobes=200)`，用于指定搜索时的聚类中心数量。
...
* 重建索引时支持：
    ...
    * IVF/IVF_PQ 修改 `nlist`、`sample_per_nlist`、`distance`、`nbits`、`m` 值，其中 `m` 和 `nbits`  是 IVF_PQ 索引的独有参数。
    * 在线重建 `ef_search` 参数。
...
该功能可以降低向量索引的内存使用，目前支持索引分区并行重建，支持设置单个任务执行时的线程数。
...
1. 配置线程数（可选）
    ...
    为合理利用集群资源，支持通过配置项 `vector_index_optimization_concurrency` 在 `oceanbase` 数据库中配置单个任务执行时的线程数。默认线程数为 2，规则如下：
      * 若租户 CPU 数量小于 4，线程数固定为 1（即不并行）。
      * 若租户 CPU 数量大于等于 4，线程数将采用用户配置的值。
