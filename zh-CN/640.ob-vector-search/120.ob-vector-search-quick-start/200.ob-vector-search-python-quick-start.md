...
* 请确保你的环境已安装 Python 3.8-3.12。

## 快速上手

### 安装 pyobvector

首先，需要将 pyobvector 先安装到我们的本地环境中。pyobvector 是 OceanBase 向量存储的 Python SDK，基于 SQLAlchemy，基本兼容 Milvus API。可参考如下命令：
...
使用 ObVecClient 建立数据库连接，参数与 MilvusLikeClient 相同。
