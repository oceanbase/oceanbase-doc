...
<h4>说明</h4>
<p><code>AI_EMBED</code> 在处理多行数据时会自动并发发送请求，而非逐行串行调用。并发度可通过端点的 <code>parameters</code> 字段配置，详见 <a href="200.ob-vector-search-ai-function-register-model.md">配置并发批处理</a>。</p>
</main>
...
<main id="notice" type='explain'>
<h4>说明</h4>
<p><code>AI_COMPLETE</code> 在处理多行数据时会自动并发发送请求，而非逐行串行调用。并发度可通过端点的 <code>parameters</code> 字段配置，详见 <a href="200.ob-vector-search-ai-function-register-model.md">配置并发批处理</a>。</p>
</main>
