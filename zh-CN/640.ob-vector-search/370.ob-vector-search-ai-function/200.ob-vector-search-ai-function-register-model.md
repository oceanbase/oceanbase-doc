...
<main id="notice" type='notice'>
<h4>注意</h4>
<p>图片嵌入与图片理解从 V4.6.0 Hotfix1 版本开始支持。</p>
</main>

#### 注册 AI 供应商

内置 AI 供应商只需填写 `access_key`，`protocol` 与 `base_url` 可省略。若同名 AI 供应商已存在，请改用 `ALTER_PROVIDER` 更新配置，目前仅支持 `aliyun-dashscope`。

:::tab	
tab 阿里云 DashScope

当前多模态示例基于阿里云 DashScope API。
...
用于 `AI_COMPLETE` 和 `AI_PROMPT` 图片理解，可选择图片 URL 或二进制图片数据（BASE64）。
...
（可选）通过 `ALTER_MODEL_PROFILE` 配置模型参数：

```sql
CALL DBMS_AI_SERVICE.ALTER_MODEL_PROFILE('aliyun-dashscope/qwen3.5-plus', '{
  "model_config": {
    "max_tokens": 64
  }
}');
