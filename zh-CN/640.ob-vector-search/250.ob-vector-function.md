...
* 稠密向量索引支持 L2、内积（Inner Product）、余弦距离作为索引距离算法。具体请参见[HNSW 系列索引](200.ob-vector-index/300.ob-dense-vector-index/200.ob-hnsw-index.md)和[IVF 系列索引](200.ob-vector-index/300.ob-dense-vector-index/300.ob-ivf-index.md)；稀疏向量索引支持内积和负内积（Negative Inner Product）作为索引距离算法。
