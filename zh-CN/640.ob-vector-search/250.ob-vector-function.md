...
* 稠密向量索引支持 L2、内积（Inner Product）、余弦距离作为索引距离算法；稀疏向量索引支持内积和负内积（Negative Inner Product）作为索引距离算法。具体请参见[稠密向量索引](200.ob-vector-index/200.ob-vector-index.md)。稀疏向量索引距离算法从 V4.3.5 BP2 版本开始支持。
...
* `semantic_distance(column_name, 'query_text')`：使用原始文本进行向量检索，系统会自动调用嵌入模型将文本转换为向量。
* `semantic_vector_distance(column_name, query_vector)`：使用向量进行检索，支持索引检索（带 `APPROXIMATE`/`APPROX` 子句）和全表扫描（不带 `APPROXIMATE`/`APPROX` 子句）两种方式。

这两个函数专门用于语义索引场景，详细说明和示例请参见[语义索引](200.ob-vector-index/300.ob-semantic-index.md)。