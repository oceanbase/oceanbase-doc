...
OceanBase 数据库还支持通过 Hint 控制是否对 SQL 语句中的字面量做参数改写，详细信息参见 [CURSOR_SHARING_EXACT Hint](../../../11.sql-reference-oracle-mode/3.basic-elements-1/6.annotation-1/4.Hint/2.hint-list/7.other-hints.md)。
...
2. 首先查看 `cursor_sharing` 系统变量，具有不同 `cursor_sharing` 的 SQL 一定会命中到不同的计划。请参见 [cursor_sharing](../../../13.reference-oracle-mode/2.system-variable-6/20.cursor_sharing.md)。
