...
| OB_TENANT_MINI_CPU | 无 | 不涉及 |  设置 OceanBase 集群中 `OB_TENANT_NAME` 对应租户的最小 CPU 规格，未配置的情况下默认会占用 OceanBase 集群全部剩余资源。其作用等同于创建资源单元中的 `MINI_CPU`，创建资源单元的详细介绍可参见 [CREATE RESOURCE UNIT](../700.reference/500.sql-reference/100.sql-syntax/100.system-tenants/700.create-resource-unit.md)。 |
