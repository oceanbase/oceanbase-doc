...
* ODP
  ...
  OceanBase Database Proxy，OceanBase 数据库代理，是 OceanBase 数据库专用的代理服务器，简称为 ODP（又称为 OBProxy）。详细信息请参考官网文档 [OceanBase 数据库代理](https://www.oceanbase.com/docs/odp-cn)。
...
| 变量名  |  默认值  | 取值说明 | 描述   |
|--------|----------|--------|-------|
| MODE | mini | <ul><li><code>mini</code>：表示容器将使用最少的资源启动集群。</li><li><code>normal</code>：表示容器将尽可能使用容器的全部资源启动集群。</li><li><code>slim</code>：表示容器将只启动 OceanBase 数据库并使用快速启动模式。</li></ul> | 设置集群启动模式。 |
...
| OB_TENANT_NAME | test | 最长 63 个字节，只能为大小写英文字母，数字和下划线，并且不能是 OceanBase 数据库的关键字。 |  配置后将在 OceanBase 集群中创建一个同名的 MySQL 租户，未配置的情况下会默认创建名为 `test` 的 MySQL 租户。 |
| OB_MEMORY_LIMIT | 6G | [6G, +∞) |  设置 OceanBase 集群可用的总内存大小，作用等同于其他两种方案中的 [memory_limit](../700.reference/800.configuration-items-and-system-variables/100.system-configuration-items/300.cluster-level-configuration-items/13800.memory_limit.md) 配置项。 |
| OB_DATAFILE_SIZE | 5G | [5G, +∞) |  设置 OceanBase 集群中数据文件的大小，作用等同于其他两种方案中的 [datafile_size](../700.reference/800.configuration-items-and-system-variables/100.system-configuration-items/300.cluster-level-configuration-items/4900.datafile_size.md)。 |
| OB_LOG_DISK_SIZE | 5G | [5G, +∞) |  设置 OceanBase 集群中 Redo 日志磁盘的大小，作用等同于其他两种方案中的 [log_disk_size](../700.reference/800.configuration-items-and-system-variables/100.system-configuration-items/300.cluster-level-configuration-items/13000.log_disk_size.md)。 |
...
| OB_SYSTEM_MEMORY | 1G | [1G, +∞) |  设置 OceanBase 集群中预留给租户 ID 为 `500` 的租户的内存容量，作用等同于其他两种方案中的 [system_memory](../700.reference/800.configuration-items-and-system-variables/100.system-configuration-items/300.cluster-level-configuration-items/23000.system_memory.md)。 |
| OB_TENANT_MINI_CPU | 默认会占用全部剩余资源 | 不涉及 |  设置 OceanBase 集群中 `OB_TENANT_NAME` 对应租户的最小 CPU 规格。等同于创建资源单元中的 `MINI_CPU`，详细介绍可参见 [CREATE RESOURCE UNIT](../700.reference/500.sql-reference/100.sql-syntax/100.system-tenants/700.create-resource-unit.md)。 |
| OB_TENANT_MEMORY_SIZE | 默认会占用全部剩余资源 | 不涉及 |  设置 OceanBase 集群中 `OB_TENANT_NAME` 对应租户的内存规格。等同于创建资源单元中的 `MEMORY_SIZE`，详细介绍可参见 [CREATE RESOURCE UNIT](../700.reference/500.sql-reference/100.sql-syntax/100.system-tenants/700.create-resource-unit.md) |
| OB_TENANT_LOG_DISK_SIZE | 默认会占用全部剩余资源 | 不涉及 |  设置 OceanBase 集群中 `OB_TENANT_NAME` 对应租户日志盘空间的大小。等同于创建资源单元中的 `LOG_DISK_SIZE`，详细介绍可参见 [CREATE RESOURCE UNIT](../700.reference/500.sql-reference/100.sql-syntax/100.system-tenants/700.create-resource-unit.md) |
