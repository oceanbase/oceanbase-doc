...
| DUPLICATE_SCOPE             | 用来指定复制表属性，取值如下： * `none`：表示该表是一个普通表。   * `cluster`：表示该表是一个复制表，Leader 需要将事务复制到 Cluster 的所有 F 副本及 R 副本。    不指定 `DUPLICATE_SCOPE` 的情况下，默认值为 `none`。 目前，OceanBase 数据库仅支持 `cluster` 级别的复制表。                                                                                                                                                                                     |
| BLOCK_SIZE                  | 指定表的微块大小。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| COMPRESSION                 | 指定存储格式 Flat/Encoding 以及压缩方法，对应如下： * `nocompress`：使用 Flat 格式，不进行压缩。   * `compress [basic]`：使用 Flat 格式和 lz4_1.0 压缩方法。   * `compress for oltp`：使用 Flat 格式和 zstd_1.0 压缩方法。   * `query [low|high]`：使用 Encoding 格式和  lz4_1.0 压缩方法。   * `archive [low|high]`：使用 Encoding 格式和 zstd_1.0 压缩方法。    |
