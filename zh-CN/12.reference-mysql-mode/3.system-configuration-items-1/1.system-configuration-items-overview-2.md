...
| 修改方式    | * 支持通过 SQL 语句修改，示例如下： ```unknow obclient> Alter SYSTEM SET max_stale_time_for_weak_consistency = '5s'; ```   * 支持通过启动参数修改，示例如下： ```unknow cd /home/admin && ./bin/observer -o "schema_history_expire_time='1h'" ```    | 仅支持通过 SQL 语句修改，示例如下： * MySQL 模式 ```sql obclient> SET ob_read_consistency = STRONG; obclient> SET GLOBAL ob_read_consistency = STRONG; ```   * Oracle 模式 ```sql obclient> SET ob_read_consistency = STRONG; obclient> SET GLOBAL ob_read_consistency = STRONG; obclient> ALTER SESSION SET ob_read_consistency = 'STRONG'; obclient> ALTER SYSTEM SET  ob_read_consistency = 'STRONG'; ```    |
...
有关配置项的相关信息，请参见 [系统配置项总览](/zh-CN/12.reference-mysql-mode/3.system-configuration-items-1/2.system-configuration-items-overview-3.md)。

更多配置项的查看和设置操作，请参见《管理员指南》的 [设置参数](/zh-CN/5.administrator-guide/2.basic-database-management/2.configuration-management/2.set-parameters.md) 章节。
