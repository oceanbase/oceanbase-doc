...
聚合函数扫描一组记录，然后返回单行记录。这组记录可以是一个表或者视图、或者一个子查询的结果。OceanBase 数据库支持的聚合函数，详情请参见手册《OceanBase 数据库 SQL 参考（MySQL 模式）》。

聚合函数通常跟 `GROUP BY` 子句一起使用，按照一个或多个列的值分组，然后每组返回单笔记录。

**示例：分组统计每个仓库的销售额** 

MySQL 租户中，聚合函数跟 `GROUP BY` 子句一起使用的时候，对 `select_list` 里的列没有要求。如果要求 `select_list` 里的列跟 `GROUP BY` 子句中的列保持一致，需要设置 MySQL 命令行下的 `sql_mode` 为 `ONLY_FULL_GROUP_BY`。SQL 查询示例如下：
...
FROM ordl GROUP BY ol_w_id  ORDER BY ol_w_id ;
