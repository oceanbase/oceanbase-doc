...
OceanBase 数据库提供了过期数据删除的功能，用户可以为关系表定义过期策略，同时对于 Hbase 表，用户也可以为每一个 column family 指定数据的存活时间 (Time To Live) 或者最大版本数 (MaxVersions)。在定义好过期策略之后，用户可以通过命令或者设置周期性任务两种方式触发删除任务，从而减少存储空间占用。同时，对于 TableAPI/HbaseAPI 操作，OceanBase 数据库也会对过期数据进行过滤，已屏蔽过期数据的可见性。
...
OBKV 当前支持两种模型，关系表和 Hbase 表模型，每种模型的 TTL 定义不一致，下面分别介绍。
...
* 创建一张 Hbase 模型表，并且定义 HColumnDescriptor 为 `{"TimeToLive": 3600， "MaxVersions": 3}`，表示每行数据的过期时间为 3600s，每行数据的最大版本数量为 3。
...
* 修改 Hbase 表，移除 TimeToLive 属性，修改最大版本数量为 4。
...
2. 插入数据（可以使用 TableAPI/SQL，这里使用 SQL 举例）。
