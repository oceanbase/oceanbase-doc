...
OceanBase 数据库提供了过期数据删除的功能，用户可以为关系表定义过期策略，同时对于 Hbase 表，用户也可以为每一个 column family 指定数据的存活时间 (Time To Live) 或者最大版本数 (MaxVersions)。在定义好过期策略之后，用户可以通过命令或者设置周期性任务两种方式触发删除任务，从而减少存储空间占用。同时，对于 OBKV（Table/Hbase）操作，OceanBase 数据库也会对过期数据进行过滤，已屏蔽过期数据的可见性。
