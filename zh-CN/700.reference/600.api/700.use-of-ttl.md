...
OceanBase 数据库提供了过期数据删除的功能，用户可以为每一个 column family 指定数据的存活时间 (Time To Live) 或者最大版本数 (MaxVersions)，并通过命令或者设置周期性任务两种方式触发删除任务，从而减少存储空间占用。同时，对于 HBase 查询，OceanBase 数据库也会基于 TimeToLive 和 MaxVersions 对结果集进行过滤，避免给用户返回过期数据。
...
TTL 过期删除任务是一个后台低优先级任务，暂无法保证过期立即删除的能力，为了不对系统的性能造成较大影响，在任务的执行过程中如果遇到 memstore 内存 total_memstore_used 超过阈值 major_freeze_trigger（可以通过视图 gv$ob_memstore 查看）的情况，会暂停当前任务的执行直到内存降低到阈值以下。同时，为了不影响系统的恢复流程，对于正常进行物理恢复的租户，TTL 任务同样也会暂停该租户 TTL 任务的执行，直到租户完成恢复。
...
* 创建一个 KV 模型（兼容Redis）的表，每个 K 有自己的过期时间。

  ```sql
  create table t2(k varchar(1024) primary key,
                v varchar(16384),
                expire_ts timestamp default NULL)
                TTL (expire_ts + INTERVAL 0 SECOND);
  ```

* 创建一张 Hbase 模型表，并且 HColumnDescriptor 为 {"TimeToLive": 3600， "MaxVersions": 3}
...
<main id="notice" type='explain'>
  <h4>说明</h4>
  <p>在 OceanBase 数据库 V4.2.1 版本中修改了 <code>tenant=all</code> 语义，改为仅包括所有用户租户，排除 sys 和 meta 租户，并增加了 all_user 和 all_meta，但是对于 TTL 来说，<code>tenant=all</code> 语义没有变化，且不支持 <code>tenant=all_user</code> 和 <code>tenant=all_meta</code>。</p>
</main>
...
<main id="notice" type='notice'>
  <h4>注意</h4>
  <p>租户级任务的 TABLE_NAME 为 NULL，PARTITION_ID、TABLE_ID 均为 -1。</p>
</main>
