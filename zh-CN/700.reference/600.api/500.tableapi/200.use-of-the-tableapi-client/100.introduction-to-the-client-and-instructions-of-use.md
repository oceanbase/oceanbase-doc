...
您可以使用该接口进行覆盖写的操作，必须指定主键列，如果该行不存在，插入该行，如果存在，覆盖已有行。
...
<main id="notice" type='notice'>
  ...
  <p>put 接口存在以下约束：
  <ul>
    <li>在 <code>binlog_row_image</code> 被设置成 FULL 时，不允许使用 put row，会报错 <code>OB_NOT_SUPPORTED</code>。</li>
    <li>表中有索引时（本地索引或全局索引），不允许使用 put，会报错 <code>OB_NOT_SUPPORTED</code>。</li>
    <li>使用 put 时，如果没有填全列，缺省的列将会使用默认值填充，如果列中没有默认值，将会报错。</li>
  </ul></p>
</main>
