...
<main id="notice" type='explain'>
  <h4>说明</h4>
  <p>本文档以 OceanBase 数据库版本为 V4.3.0 且 ODP 版本为 V4.2.3 版本为例。</p>
</main>
...
2. 通过 `SHOW PROCESSLIST` 语句查询当前租户的会话数量及会话 ID。
   ...
   <main id="notice" type='explain'>
     <h4>说明</h4>
     <p><code>client_session_id_version</code> 配置项的详细介绍可参见 <a href='https://www.oceanbase.com/docs/common-odp-doc-cn-1000000000517691'>client_session_id_version</a>，推荐设置为 <code>2<code> 以使用新的计算逻辑。</p>
     <ul>
   </main>

   * 当 ODP 配置项 `client_session_id_version` 设置为 `2` 时，显示的是租户对应的 OBServer 节点上的会话信息，查询结果如下：
   ...
   * 当 ODP 配置项 `client_session_id_version` 设置为 `1` 时，显示的是对应的 ODP 节点上的会话信息，查询结果如下：
