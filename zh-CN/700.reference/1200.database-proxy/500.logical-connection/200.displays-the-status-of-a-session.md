...
2. 查看指定 Client Session 的详细内部状态。
    ...
    * `LIKE` 模糊匹配，支持 '%' 和 '_' 。
    ...
    obclient [(none)]> SHOW PROXYSESSION ATTRIBUTE 65160 LIKE '%id%';
    +------------------------+----------------------+----------------+
    | attribute_name         | value                | info           |
    +------------------------+----------------------+----------------+
    | proxy_sessid           | -6044239443189891054 | cs common      |
    ...
    obclient [SYS]> SHOW PROXYSESSION ATTRIBUTE;
    +----------------------------------+----------------------+----------------+
    | attribute_name                   | value                | info           |
    +----------------------------------+----------------------+----------------+
    ...
    obclient [SYS]> SHOW PROXYSESSION ATTRIBUTE 65141 like '%id%';
    +------------------------+----------------------+----------------+
    | attribute_name         | value                | info           |
    +------------------------+----------------------+----------------+
    | proxy_sessid           | -6044239443189891058 | cs common      |
    ...
    | sys_var_version                  | ?                   |
    | user_var_version                 | ?                    |
    | last_insert_id_version           | ?                    |
    | db_name_version                  | ?                    |
    | is_checksum_supported            | ?                    |
    | is_safe_read_weak_supported      | ?                    |
    | is_checksum_switch_supported     | ?                    |
    | checksum_switch                  | /                    |
