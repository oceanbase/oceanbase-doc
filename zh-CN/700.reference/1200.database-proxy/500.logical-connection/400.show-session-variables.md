...
2. 查看指定 Client Session 的 Session 变量。
   ...
   * `like` 模糊匹配，支持 '%' 和 '_' 。
    ...
    | ob_proxy_user_privilege           | 4160946174 | changed sys var | cold modified vars |  && invisible && session_scope && readonly |
    | ob_capability_flag                | 2639       | changed sys var | cold modified vars |  && invisible && session_scope && readonly |
    ...
    obclient [(none)]> SHOW PROXYSESSION VARIABLES ALL 3221593149 LIKE "%id%";
    +--------------------------------------+-----------+---------+------------------------------+--------------------------------------------+
    | variable_name                        | value     | info    | modified_type                | sys_variable_flag                          |
    +--------------------------------------+-----------+---------+------------------------------+--------------------------------------------+
    | last_insert_id                       | 0         | sys var | last insert id modified vars |  && session_scope                          |
    | identity                             | 0         | sys var | cold modified vars           |  && session_scope                          |
    | server_id                            | 0         | sys var | cold modified vars           |  && global_scope && session_scope          |
