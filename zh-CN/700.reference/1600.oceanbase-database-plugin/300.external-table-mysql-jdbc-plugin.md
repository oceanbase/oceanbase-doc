...
获取 JDBC 插件 JAR 包，然后把 JDBC 插件 JAR 包放到指定目录（配置项 `ob_java_connector_path` 目录下）。
...
3. 设置 OBServer 启动的 jvm 可加载的可执行依赖 jar 包路径。
    ...
    **示例如下：**
    ...
    <main id="notice" type='notice'>
      <h4>注意</h4>
      <p>该设置是手动部署（使用命令行部署）的配置。如果后续是 OCP 安装部署，此处依赖 jar 包路径需要改成对应的 <code>/home/admin/oceanbase/jni_packages/current</code>，为了避免升级依赖包版本后还要修改该配置，此处可以设为软链接：<code>/home/admin/oceanbase/jni_packages/v1.0.0 -> /home/admin/oceanbase/jni_packages/current</code>。</p>
    </main>
...
MySQL 外表插件中有丰富的数据类型，不过 OceanBase 数据库还未完全支持。当前版本 OceanBase 数据库支持的数据类型如下所示：
