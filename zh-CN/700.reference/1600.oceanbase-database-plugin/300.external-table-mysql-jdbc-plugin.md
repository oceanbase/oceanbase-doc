...
本文将介绍如何在 OceanBase 数据库  安装和使用外表 MySQL JDBC 插件。通过外表 JDBC 插件，可以访问 JDBC 支持的数据源（当前仅支持访问 MySQL 数据库数据源）。

<main id="notice" type='notice'>
  <h4>注意</h4>
  <p><ul><li>从 V4.4.1 版本开始支持外表 MySQL JDBC 插件。</li><li>当前该功能为实验特性，暂不建议在生产环境使用。</li></ul></p>
</main>

## 功能限制

* OceanBase 数据库 Oracle 模式：当前该功能不支持 Oracle 模式。
* 数据类型：`JSON`/`Array`/`GIS` 等类型暂未支持。
* 查询限制：
...
* 已安装 JDK（版本 ≥ 11）及 MySQL JDBC 插件 JAR 包。

    <main id="notice" type='notice'>
      <h4>注意</h4>
      <p>关于部署 JAVA SDK 环境的详细介绍信息，参见 <a href="../../400.deploy/300.deploy-oceanbase-enterprise-edition/500.deploy-the-ob-java-sdk-environment.md">部署 OceanBase 数据库 JAVA SDK 环境</a>。</p>
    </main>
...
MySQL 数据库中有丰富的数据类型，不过 OceanBase 数据库还未完全支持。当前版本 OceanBase 数据库支持的数据类型如下所示：
