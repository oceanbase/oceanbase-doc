...
1. 进入 RPM 包缓存目录。
    ...
    ```shell
    cd /var/cache/yum/x86_64/7/oceanbase.development-kit/packages/
    ```
...
2. 解压分词器插件 RPM 包。
    ...
    **示例如下：**
    ...
    ```shell
    rpm2cpio oceanbase-plugin-dev-kit-0.1.0-22025022417.el7.x86_64.rpm | cpio -idmv
    ```

3. OceanBase 分词器插件样例代码 `space_ftparser.cpp` 在 OceanBase 插件开发包中的 `usr/share/examples/ObPlugin/ftparser` 目录下。直接复制这个目录到自己的开发目录下，即可以修改 `space_ftparser.cpp` 文件开发自己的分词器插件。
...
5. 重启集群，就会安装这个分词器插件。
    ...
    使用 OceanBase 部署工具（OBD）重启集群，使插件生效。请将 `<cluster_name>` 替换为实际的集群名称。
