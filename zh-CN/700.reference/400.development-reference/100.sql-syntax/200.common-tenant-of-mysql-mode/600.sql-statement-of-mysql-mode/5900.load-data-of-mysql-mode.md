...
OceanBase 数据库中的 `LOAD DATA` 语句支持加载本地直连的 OceanBase 数据库的输入文件，用户需要在导入之前将文件拷贝当前 OBServer 节点上。此外也支持从 OSS 加载输入文件。
...
| APPEND | 使用 Hint 启用旁路导入功能，即支持直接在数据文件中分配空间并写入数据。`APPEND` Hint 默认等同于使用的 `direct(false, 0)`，同时可以实现 在线收集统计信息（`GATHER_OPTIMIZER_STATISTICS` Hint）的功能。  <main id="notice" type='notice'><h4>注意</h4><p>建议用户在旁路导入完成后，再执行手动收集统计信息的操作。</p></main>   |
