...
`TINYTEXT` 是最大长度为 256 个字节的 `TEXT` 类型。
...
`MEDIUMTEXT` 是最大长度为 16,777,216 个字节的 `TEXT` 类型。
...
`LONGTEXT` 是最大长度为 536,870,911 个字节的 `TEXT` 类型。`LONGTEXT` 列的有效最大长度还取决于客户端/服务器协议中配置的最大数据包大小和可用内存。
