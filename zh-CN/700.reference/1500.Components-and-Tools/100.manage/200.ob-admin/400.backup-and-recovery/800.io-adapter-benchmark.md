...
io_adapter_benchmark 命令用于验证备份介质的性能。本文主要介绍 io_adapter_benchmark 命令的几种用法。
...
上述命令会在 `oss_bucket/test_path` 目录下执行写普通文件的性能测试，并在真正写入前清空 test_path 目录（`-b 'true'` 指定）。清空目录后，将通过四个线程（`-t 4` 指定）并发向该目录写入对象，每个对象大小为 100 字节（`-o 100` 指定），知道每个线程写入 10 个对象（`-r 10` 指定）或执行的总时间达到上限（100s，`-j 100` 指定）后停止写入。测试完成后会清空目录（`-c 'true'` 指定）。

命令执行后输出如下：

```shell
succ to open, filename=/data/tbegg/db/42x/build_debug/tools/ob_admin/tmp_dir/ob_admin.log, fd=3, wf_fd=2
succ to open, filename=/data/tbegg/db/42x/build_debug/tools/ob_admin/tmp_dir/ob_admin_rs.log, fd=4, wf_fd=2
