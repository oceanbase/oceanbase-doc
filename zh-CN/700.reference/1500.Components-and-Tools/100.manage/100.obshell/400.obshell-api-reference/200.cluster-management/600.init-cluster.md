...
<main id="notice" type='explain'>
  <h4>说明</h4>
  <p>在 V4.3.5 BP2 及之后版本（V4.3.5.x 系列）中支持配置请求参数。</p>
</main>

| 参数 | 类型 | 必选 | 示例值 | 描述  |
|-------|------|------|------|------|
| import_script | bool | 否 | false | 控制是否将时区数据和 GIS meta 数据导入到系统租户，默认为 `false`。  |
| create_proxyro_user | bool | 否 | false | 控制是否在 `sys` 租户中创建 `proxyro` 用户并设置相应的权限，默认为 `false`。  |
| proxyro_password | string | 否 | ***** | 设置 `proxyro` 用户密码。  |
