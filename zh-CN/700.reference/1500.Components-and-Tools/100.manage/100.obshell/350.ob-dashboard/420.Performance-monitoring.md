...
性能监控是 obshell Dashboard 的核心模块之一，为您提供 OceanBase 集群的集群维度、主机维度、租户维度等多种资源的多维度性能指标实时监控能力，帮助您快速定位性能瓶颈、优化系统资源使用。

## OceanBase 集群

OceanBase 集群展示了数据库性能和主机性能两个维度的数据。默认使用数据库性能的集群维度进行展示，不同维度支持查看的监控指标有所不同。
...
2. 在左侧导航栏单击 **性能监控**。
	...
	<main id="notice" type='explain'>
 	  <h4>说明</h4>
  	  <p>仅首次登录需要输入 Prometheus 的地址、用户名、密码。若暂无地址，建议使用 obd 安装部署。</p>
	</main>
...
+ **入口一**：登录 obshell Dashboard，单击相应租户，进入该租户的管理页面，在左侧导航栏单击 **性能监控**，系统默认进入 **性能与 SQL** 页签。
