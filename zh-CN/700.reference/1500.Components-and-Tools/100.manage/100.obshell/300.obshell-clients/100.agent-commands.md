...
| --password | 否       | string  | 无      | 该选项仅用于接管 OceanBase 实例，接管时需使用该选项指定 OceanBase 实例中 sys 租户的 root 用户密码。您也可通过环境变量 `OB_ROOT_PASSWORD` 指定 root 用户密码。 |
<!--  需确认一下这个环境变量 还是`root_password`-->
...
在接管 OceanBase 实例时，需使用选项 `--password` 指定 OceanBase 实例中 sys 租户的 root 用户密码。您也可通过环境变量 `OB_ROOT_PASSWORD` 指定 root 用户密码。
