...
| --create_proxyro_user  | 否  | string   | false | 用于控制是否在 `sys` 租户中创建 `proxyro` 用户并设置相应的权限，默认为 `false`。 |
...
| -t/--terminate | 否 | 无 | 无 | 该选项无需传值，命令中配置即表示在重启 OceanBase 之前，触发 `MINOR FREEZR` 命令。 |
