...
| --import_script | 否 | bool | false | 控制是否将时区数据和 GIS meta 数据导入到系统租户，默认为 `false`。 |
...
| -D/--delete_policy | 否 | string | default | 设置清理策略，设置了清理策略的租户将会每小时触发一次自动清理。当前支持的取值如下：<ul><li>`default`：表示自动清理的范围仅为 `-d`/`--data_backup_uri` 和 `-a`/`--archive_log_uri` 值中的备份。</li><li>`log_only`：表示自动清理的范围仅为 `-a`/`--archive_log_uri` 值中的备份。适用于没有数据备份，仅开启了日志归档的场景。</li></ul>  |
...
```shell
${home_path}/bin/obshell tenant archivelog show <tenant-name> [flags] 
...
```shell
${home_path}/bin/obshell tenant noarchivelog show <tenant-name> [flags] 
