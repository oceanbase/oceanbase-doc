...
| direct_load_inc_thread_num | 读取和解析增量旁路导入日志的线程数 | 0 | 是 | 用于内存限速的自动配置项，可根据 OBCDC 设置的 `memory_limit` 自动调整，也可以手动修改配置来动态调整。<main id="notice" type='notice'><h4>注意</h4><p>使用 OBCDC 同步增量旁路导入数据时，请谨慎调整该配置项，否则可能影响内存限速，导致内存超限。</p></main> |
| direct_load_inc_queue_backlog_lowest_tolerance | 触发限速的任务挤压阈值 | 0 | 是 | 用于内存限速的自动配置项，可根据 OBCDC 设置的 `memory_limit` 自动调整，也可以手动修改配置来动态调整。<main id="notice" type='notice'><h4>注意</h4><p>使用 OBCDC 同步增量旁路导入数据时，请谨慎调整该配置项，否则可能影响内存限速，导致内存超限。</p></main> |
