...
<main id="notice" type='notice'>
  <h4>注意</h4>
  <p>目前 OBCDC 暂不支持同步全量旁路导入的数据，仅支持同步增量旁路导入的数据。</p>
</main>
...
* 您已部署 OBCDC，详细操作可参见 [obcdc 安装部署](100.deploy-and-use-obcdc/100.install-and-deploy-obcdc.md)。

* 您已部署 OceanBase 数据库，且 OceanBase 数据库为 V4.3.1 或之后版本。

## 操作步骤

假设 OceanBase 集群中存在表 `A` 和表 `B` 两张表为例，其中，表 `A` 中的数据如下，`col1` 列为主键：
...
此处以执行增量旁路导入，将表 `B` 的数据导入到表 `A` 中为例介绍 OBCDC 侧的操作。
...
2. 使配置生效
   ...
   * 将 OBCDC 接入自己的数据消费链路进行使用时，您可重新编译运行所写代码，相关操作可参见 [obcdc 开发说明](../200.obcdc/100.deploy-and-use-obcdc/200.obcdc-development-instructions.md)。

   * 通过 obcdc_tailf 使用 OBCDC 功能时，您可重新执行 obcdc_tailf 命令，示例如下：

     ```shell
     [admin@test oceanbase]$ ./bin/obcdc_tailf -f libobcdc.conf -D output.txt
     ```

     此处以配置文件相对路径为 `libobcdc.conf`，将输出数据保存到 `output.txt` 文件为例，您需根据实际情况进行替换。obcdc_tailf 命令的详细介绍可参见 [obcdc_tailf](../200.obcdc/100.deploy-and-use-obcdc/300.obcdc-tailf.md)。
