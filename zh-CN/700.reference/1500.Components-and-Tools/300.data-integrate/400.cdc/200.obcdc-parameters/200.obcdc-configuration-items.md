...
| enable_output_virtual_generated_column | 控制是否输出虚拟生成列  | 0 | 否  | 需与 `enable_output_by_table_def` 配合使用。<main id="notice" type='explain'><h4>说明</h4><p>对于 OceanBase 数据库 V4.4.x 版本，该配置项从 V4.4.2 版本开始引入，即 OceanBase 数据库为 V4.4.2 或之后的 V4.4.x 系列版本时，obcdc 支持输出虚拟生成列。</p></main>  |
|  enable_update_split_merge| 是否开启将 DELETE+INSERT 合并为 UPDATE 的写入操作 | 0 | 否 | 开启合并后，带相同拆分标记的 DELETE 与 INSERT 能合并为一条 UPDATE。  |
| update_split_merge_abort_on_data_loss | 事务拆分合并导致数据丢失时的处理策略 | 1 | 是 |需与 `enable_update_split_merge` 配合使用。<ul><li>0：紧急绕过模式，继续运行但可能丢失数据</li><li>1：中止模式，立即中止 CDC 进程保证数据完整性</li></ul> |

## 路由配置

| 配置项      | 含义      | 默认值 | 可否动态变更 |  备注       |
|------------|-----------|--------|-------------|------------|
| region                  | 指定 CDC 优先拉取日志的目标 Region | default_region | 是 | <ul><li>设为默认值时，不对 Region 做过滤，所有 Region 的 Server 优先级相同。</li><li>支持指定具体的 Region 名称，如："beijing"、"shanghai"。</li><li>多个 CDC 实例可配置不同 Region 以实现就近读取。</li></ul>
| fetch_log_zone_priority | Zone 优先级的描述字符串| default_zone | 是 | 设为默认值时优先使用 Region 维度进行排序。fetch_log_zone_priority 使用类似 primary_zone 的语法，分号<code>;</code>分隔不同层级，逗号<code>,</code>分隔同一层级内的多个 Zone。<ul><li><code>zone1;zone2</code>：zone1 优先级高于 zone2</li><li><code>zone1,zone2;zone3</code>：zone1 和 zone2 同属第一层级（最高优先级），zone3 属于第二层级</li><li>留空或设为 default_zone 时，仅根据 region 配置区分优先级</li></ul><main id="notice" type='explain'><h4>说明</h4><ul><li>Zone 在设置时会成为最高优先级。</li><li>CDC 基于配置优先拉取指定目标，但不保证仅拉取指定目标。</li></ul></main>|
...
| 配置项       | 含义      | 默认值 | 可否动态变更 |  备注       |
|------------|-----------|-----|--------|-------------------------|
| meta_data_refresh_mode                  | 用于指定 schema/日志流等元信息的获取方式               | online   | 否      | <ul><li>online 模式：通过 SchemaService 向 server 查询 schema 信息，需要能够访问 OBServer 节点。</li><li>data_dict 模式：通过日志中的数据字典解析 schema 信息。</li></ul><main id="notice" type='notice'><h4>注意</h4><p>obcdc 同步 OceanBase 数据库 V4.2 及之后版本数据时会忽略用户配置，强制使用 <code>data_dict</code> 模式，如需使用 <code>online</code> 模式，需额外修改 <code>skip_ob_version_compat_check</code> 配置项为 <code>1</code>。</p></main> |
...
|          配置项           |      含义      |  默认值  | 取值范围 | 可否动态变更 |
|------------------------|--------------|--------|--------|---------|
| redo_dispatched_memory_limit_exceed_ratio   | 控制 enable_output_trans_order_by_sql_operation=1 时，下发处理的 redo 的大小限制（即允许内存中处理中的 redo 大小为 redo_dispatched_memory_limit_exceed_ratio * redo_dispatcher_memory_limit）。 | 2    |  [1, +∞)   | 是      |
...
| enable_filte_mow_lob_col | 控制是否过滤 delete_insert 模式表的 OUTROW LOB 列 |  0 | 否  | 当 OceanBase 数据库版本处于 V4.3.5.0 ~ V4.3.5.5 之间时，obcdc 无法处理 OceanBase 数据库写入的 delete_insert 表的 OUTROW LOB 数据。可通过调整该配置项使 obcdc 继续同步数据，被过滤的 OUTROW LOB 列将被 obcdc 标记为不可信列。 |
