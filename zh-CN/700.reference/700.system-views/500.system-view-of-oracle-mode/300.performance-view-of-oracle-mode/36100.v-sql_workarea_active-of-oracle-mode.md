...
展示当前节点上当前活跃 Operator 的 workarea 信息。
...
| POLICY           | VARCHAR2(24)  | NO             | workarea 的策略：<ul><li>MANUAL </li><li>AUTO </li></ul> |
...
| ACTIVE_TIME      | NUMBER   | NO             | workarea 活跃时平均时间，单位为毫秒                      |
| WORK_AREA_SIZE   | NUMBER       | NO             | 操作符使用 workarea 最大大小，单位为字节            |
| EXPECT_SIZE      | NUMBER   | NO             | workarea 期望的大小，单位为字节                     |
| ACTUAL_MEM_USED  | NUMBER   | NO             | 申请 workarea 的内存量，单位为字节               |
| MAX_MEM_USED     | NUMBER   | NO             | 使用 workarea 最大的内存使用量，单位为字节              |
| NUMBER_PASSES    | NUMBER   | NO             | workarea 在哪个模式下下运行： <ul><li>0：optimal 模式   </li><li> 1：one pass 模式   </li><li> \>=2：multi pass 模式 </li></ul>    |
