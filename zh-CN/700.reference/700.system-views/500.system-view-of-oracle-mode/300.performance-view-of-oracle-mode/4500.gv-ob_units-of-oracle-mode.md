...
| DATA_DISK_SIZE   | NUMBER(38)   | YES        | Unit 能够使用的本地缓存大小，单位为字节，仅适用于共享存储（Shared-Storage，SS）模式。无共享（Shared-Nothing，SN） 模式下，该字段值为 `NULL`，表示未定义，允许竞争使用其他租户的磁盘空间。 <main id="notice" type='explain'><h4>说明</h4><p>该字段从 V4.3.4 版本开始引入。</p></main>     | 
| DATA_DISK_IN_USE | NUMBER(38)   | NO         | <ul><li>SS 模式下，表示 Unit 已使用的本地缓存大小，单位为字节。</li> <li></li></ul>SN 模式下，Unit 已使用的数据盘大小，单位为字节。                      |
