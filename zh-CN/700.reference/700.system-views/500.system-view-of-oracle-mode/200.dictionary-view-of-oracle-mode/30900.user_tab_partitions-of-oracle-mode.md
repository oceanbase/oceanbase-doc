...
| INDEXING              | VARCHAR2(4)      | NO         | 指示索引属性。取值：<ul><li>ON：该子分区的索引已开启</li><li>OFF：此子分区的索引已关闭 </li></ul>    |
| READ_ONLY             | VARCHAR2(4)      | NO         | 指示子分区是否为只读：<ul><li>YES：子分区的默认设置是只读的</li><li>NO：子分区的默认设置是读/写 </li></ul>       |
