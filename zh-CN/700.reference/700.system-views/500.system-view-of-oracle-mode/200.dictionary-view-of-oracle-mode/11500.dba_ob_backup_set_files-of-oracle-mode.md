...
| FILE_STATUS | varchar2(64) | NO | 备份集的文件状态：<ul><li>COPYING：表示文件正在备份中</li><li>AVAILABLE：表示该备份为有效的备份</li><li>DELETING：表示文件正在删除</li><li>EXPIRED：表示备份文件已失效</li><li>INCOMPLETE：表示备份文件有缺失</li><li>BROKEN：表示备份的文件不完整，不可使用</li><li>DELETED：表示备份文件已删除</li></ul> |
...
| ENCRYPTION_MODE | varchar2(64) | YES | 加密模式：<ul><li>None：表示不加密</li><li>Password：表示只使用密码保护</li><li>Password Encryption：表示加密，并且使用密码保护</li><li>Transparent Encryption：表示使用 TDE 加密</li><li>Dual mode Encryption：表示使用 TDE 加密以及密码保护</li></ul> </br>目前仅支持 None 和 Password 两种模式 |
