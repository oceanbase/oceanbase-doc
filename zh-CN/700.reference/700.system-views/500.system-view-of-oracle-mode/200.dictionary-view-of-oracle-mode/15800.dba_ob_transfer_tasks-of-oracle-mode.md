...
展示本租户下当前正在执行的 Transfer 任务。同一时间可能有多个负载均衡任务正在执行，这些任务（TRANSFER_TASK）都属于同一个负载均衡工作（BALANCE_JOB）。
...
* 查看本租户下 Tablet 级别的负载均衡任务（Task）历史：[DBA_OB_TRANSFER_TASK_HISTORY](15700.dba_ob_transfer_task_history-of-oracle-mode.md)
