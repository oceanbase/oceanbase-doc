...
<h4>说明</h4>
<p>从 V4.2.1 BP7 版本开始，该视图的查询结果中不再展示索引表相关信息。</p>
</main>
...
1. 创建表 `tbl2`。
   ...
   ```shell
   obclient [SYS]> CREATE TABLE tbl2(id NUMBER,name VARCHAR2(20) PRIMARY KEY);
   ```
2. 为表 `TBL2` 创建索引。

   ```shell
   obclient [SYS]> CREATE INDEX tbl2_idx ON tbl2(id);
   ```

3. 查询表 `TBL2` 的列信息。

   ```shell
   obclient [SYS]> SELECT TABLE_NAME, COLUMN_NAME, DATA_TYPE, DATA_DEFAULT FROM SYS.USER_TAB_COLUMNS WHERE TABLE_NAME LIKE '%TBL2%';
   ```
...
* [ALL_TAB_COLUMNS](3200.all_tab_columns-of-oracle-mode.md)

* [DBA_TAB_COLUMNS](10800.dba_tab_columns-of-oracle-mode.md)