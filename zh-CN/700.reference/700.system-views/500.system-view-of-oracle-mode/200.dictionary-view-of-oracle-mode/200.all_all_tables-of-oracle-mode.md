...
展示当前用户可访问的对象表和关系表，与 `ALL_TABLES` 不同，`ALL_ALL_TABLES` 能够查询到对象表。
...
查看用户可访问的对象表和关系表中，表 `T_SUBPART` 的信息。
...
* 查看当前租户下的所有对象表和关系表：[DBA_ALL_TABLES](6600.dba_all_tables-of-oracle-mode.md)

* 查看当前用户拥有的所有对象表和关系表：[USER_ALL_TABLES](24900.user_all_tables-of-oracle-mode.md)