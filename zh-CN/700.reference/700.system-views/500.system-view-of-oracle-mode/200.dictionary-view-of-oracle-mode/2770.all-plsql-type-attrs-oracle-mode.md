...
`ALL_PLSQL_TYPE_ATTRS` 用于显示所有 PL/SQL 记录类型和表类型的属性。只有在 package 成功编译的情况下，package 中的类型才能在该视图中展示。
...
查看所有 PL/SQL 记录类型和表类型的属性。

```shell
obclient [SYS]> SELECT * FROM ALL_PLSQL_TYPE_ATTRS;
```
