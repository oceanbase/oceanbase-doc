...
| SKIP_INDEX_LEVEL          | NUMBER         | NO         | 表示 `skip_index` 的创建级别。取值如下：<ul><li>0：表示表的列存基线都生成 Skip Index 聚合信息，行存基线根据 Schema 生成 Skip Index 聚合信息，增量 SSTable 不生成 Skip Index 聚合信息。</li><li>1：表示在 0 的基础上，表的增量 SSTable 会根据基线的行为生成 Skip Index 聚合信息。</li></ul> <main id="notice" type='explain'><h4>说明</h4><p><ul><li>该字段从 V4.6.0 版本开始引入。</li><li>该字段对于 <code>partial_update</code> 模式的表不生效。</li></ul></p></main>|
| TTL_DEFINITION            | VARCHAR2(262144)  | NO      | TTL 定义  <main id="notice" type='explain'><h4>说明</h4><p>该字段从 V4.6.0 版本开始引入。</p></main>  |
| DELTA_FORMAT              | VARCHAR2(1048576) | NO      | 表示表的增量数据存储格式。取值如下：<ul><li>`flat`：flat 数据格式。</li><li>`encoding`：encoding 数据格式。</li></ul> <main id="notice" type='explain'><h4>说明</h4><p>该字段从 V4.6.0 版本开始引入。</p></main>|
