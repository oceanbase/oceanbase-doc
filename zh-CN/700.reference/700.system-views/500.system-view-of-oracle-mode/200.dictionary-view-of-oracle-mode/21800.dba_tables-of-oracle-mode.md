...
| SKIP_INDEX_LEVEL          | NUMBER         | NO         | 表示 `skip_index` 的创建级别。取值如下：<ul><li>0：表示表的列存基线都生成 Skip Index 聚合信息，行存基线根据 Schema 生成 Skip Index 聚合信息，增量 SSTable 不生成 Skip Index 聚合信息。</li><li>1：表示在 0 的基础上，表的增量 SSTable 会根据基线的行为生成 Skip Index 聚合信息。</li></ul> <main id="notice" type='explain'><h4>说明</h4><p>该字段从 V4.6.0 版本开始引入。</p></main>|
