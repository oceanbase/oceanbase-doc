...
视图 `ALL_TAB_COLS` 与 [ALL_TAB_COLUMNS](4600.all_tab_columns-of-oracle-mode.md) 的区别在于：`ALL_TAB_COLS` 会展示快速删除的列，而 `ALL_TAB_COLUMNS` 不会展示快速删除的列。
...
| DATA_LENGTH          | NUMBER         | NO             | 列长度，单位为字节                               |
...
| CHARACTER_SET_NAME   | VARCHAR2(44)   | NO             | 字符集名称：<ul><li>`CHAR_CS`</li><li> `NCHAR_CS` </li></ul>   |
...
| HIDDEN_COLUMN        | VARCHAR2(3)    | NO             | 表示列是否是隐藏列：<ul><li>YES </li><li> NO </li></ul> |
| VIRTUAL_COLUMN       | VARCHAR2(3)    | NO             | 表示列是否是虚拟列：<ul><li>YES </li><li> NO </li></ul>   |
...
* 查看本租户所有表、视图的列信息：[DBA_TAB_COLS](20000.dba_tab_cols-of-oracle-mode.md)

* 查看当前用户所拥有的表、视图的列信息：[USER_TAB_COLS](28900.user_tab_cols-of-oracle-mode.md)
