...
| STATUS | VARCHAR2(64) | NO | 任务状态：<ul><li>`INIT`：任务创建  </li><li>`CREATE_LS`：创建日志流 </li><li>`ALTER_LS`：修改日志流属性 </li> <li>`TRANSFER`：日志流间的 Transfer</li> <li>`SET_LS_MERGING`：日志流合并 </li><li>`DROP_LS`：删除日志流 </li><li>`COMPLETED`：任务执行成功 </li><li>`CANCELED`：任务取消 </li></ul>|
...
* 查看本租户下日志流级别的负载均衡任务（Task）：[DBA_OB_BALANCE_TASK_HISTORY](12200.dba_ob_balance_task_history-of-oracle-mode.md)
