...
| SCHEDULE_TYPE | VARCHAR2(12) | YES | 默认值为 NULL |
...
| STATE | VARCHAR2(15) | YES | 工作现状：<ul><li>DISABLED</li><li>RETRY SCHEDULED</li><li>SCHEDULED</li><li>BLOCKED</li><li>RUNNING</li><li>COMPLETED</li><li>BROKEN</li><li>FAILED</li><li>REMOTE</li><li>RESOURCE_UNAVAILABLE</li><li>SUCCEEDED</li><li>CHAIN_STALLED </li></ul>|
...
| MAX_FAILURES | NUMBER | YES | 在被标记为损坏之前允许作业失败的次数 |
...
| DESTINATION | VARCHAR2(261) | YES | 作业的目标，即作业要执行的对象或位置。 |
| CREDENTIAL_OWNER | VARCHAR2(128) | NO | 凭证的所有者。 |
| CREDENTIAL_NAME | VARCHAR2(128) | YES | 凭证的名称。 |
...
| THIS_DATE                     | TIMESTAMP(6) WITH TIME ZONE  | YES  |  本次调度开始的时间。<main id="notice" type='explain'><h4>说明</h4><p>该字段从 V4.6.0 版本开始引入。</p></main>    |
| USER_ID                       | NUMBER   | YES  |  JOB 所属的 `user_id`。<main id="notice" type='explain'><h4>说明</h4><p>该字段从 V4.6.0 版本开始引入。</p></main>   |
| DATABASE_ID                   | NUMBER   | YES  |  JOB 所属的 `database_id`。<main id="notice" type='explain'><h4>说明</h4><p>该字段从 V4.6.0 版本开始引入。</p></main>    |
| THIS_EXEC_ADDR                | TIMESTAMP(6) WITH TIME ZONE | YES  | 本次执行所在的 OBServer 地址。 <main id="notice" type='explain'><h4>说明</h4><p>该字段从 V4.6.0 版本开始引入。</p></main>   |
