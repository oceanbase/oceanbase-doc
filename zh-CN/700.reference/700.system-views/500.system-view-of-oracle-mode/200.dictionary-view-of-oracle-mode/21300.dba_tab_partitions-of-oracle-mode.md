...
| INTERVAL               | VARCHAR2(3)     | NO         | 指示该分区是否为 Interval 分区自动创建的分区：<ul><li>YES：该分区为 Interval 分区自动创建的分区</li><li>NO：该分区不是 Interval 分区自动创建的分区；对于非 Interval 分区表，该字段恒为 `NO`</li></ul> |
