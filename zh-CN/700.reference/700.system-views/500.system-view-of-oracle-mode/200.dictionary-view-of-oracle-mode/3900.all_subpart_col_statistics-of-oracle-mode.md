...
展示当前用户可访问的分区表中二级分区的列统计信息和直方图信息。
...
| GLOBAL_STATS      | VARCHAR2(3)   | YES            | <ul><li>`YES`：表示统计信息已收集   </li><li> `NO`：表示统计信息未收集 </li></ul>   |
...
查看当前用户可访问的分区表中，表 `T_SUBPART` 的二级分区的列统计信息。
...
* 查看本租户下所有分区表的二级分区的列统计信息：[DBA_SUBPART_COL_STATISTICS](19300.dba_subpart_col_statistics-of-oracle-mode.md)

* 查看当前用户拥有的分区表的二级分区的列统计信息：[USER_SUBPART_COL_STATISTICS](28200.user_subpart_col_statistics-of-oracle-mode.md)
