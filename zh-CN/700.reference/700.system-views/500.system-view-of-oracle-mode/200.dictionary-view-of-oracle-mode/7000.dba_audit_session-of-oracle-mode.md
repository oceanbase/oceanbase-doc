...
视图 `DBA_AUDIT_SESSION` 显示所有关于 `CONNECT` 和 `DISCONNECT` 的审计跟踪记录。
...
查看当前租户中所有关于 `CONNECT` 和 `DISCONNECT` 的审计跟踪条目。
...
* 查看关于当前用户的 `CONNECT` 和 `DISCONNECT` 的审计跟踪条目：[USER_AUDIT_SESSION](25200.user_audit_session-of-oracle-mode.md)
