...
展示当前用户拥有的函数和过程的参数。其列与 `ALL_ARGUMENTS` 中的列相同，去除了 `OWNER` 字段。
...
| IN_OUT             | VARCHAR2(5)   | YES            | 参数的输入输出属性（NULL 表示是返回值）：<ul><li> IN   </li><li> OUT   </li><li> IN OUT </li></ul>   |
...
* 查看当前租户下所有可用的函数和过程的参数：[DBA_ARGUMENTS](6500.dba_arguments-of-oracle-mode.md)
