...
| PLUGIN                      | VARCHAR2(64)   | NO             | 记录密码 HASH 计算使用的 plugin 名称。目前仅允许以下取值：<ul><li>空串（与 `mysql_native_password` 等价）</li><li>`ob_native_password`：Oracle 模式下等价于 `mysql_native_password`</li><li>`mysql_native_password`</li><li>`caching_sha2_password`</li></ul><main id="notice" type='notice'><h4>注意</h4><p>该字段从 V4.4.2 BP2 版本开始支持。</p></main> |
