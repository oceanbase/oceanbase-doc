...
| REFRESH_DOP          | NUMBER(38) | NO | 表示物化视图的后台刷新并行度，默认值为 0。<main id="notice" type='explain'><h4>说明</h4><p><ul><li>在 V4.4.2 BP2 和 V5.0.1 版本之前创建物化视图时，通过 <code>PARALLEL</code> 属性可指定此值，后续也可通过 <code>ALTER MATERIALIZED VIEW ... PARALLEL n</code> 语法修改此值。</li><li>对于 V4.4.2 版本，从 V4.4.2 BP2 版本开始创建物化视图时，通过 <code>REFRESH ... PARALLEL n ...</code> 属性可指定此值，后续也可通过 <code>ALTER MATERIALIZED VIEW ... REFRESH PARALLEL n</code> 语法修改此值。</li><li>对于 V5.x 版本，从 V5.0.1 版本开始创建物化视图时，通过 <code>REFRESH ... PARALLEL n ...</code> 属性可指定此值，后续也可通过 <code>ALTER MATERIALIZED VIEW ... REFRESH PARALLEL n</code> 语法修改此值。</li><li>当用户未显式设置刷新并行度时（<code>REFRESH_DOP</code> 为默认值 0），后台刷新并行度会取决于全局系统变量 `mview_refresh_dop` 的值。</li><li>对于 OceanBase 数据库 V4.3.5 版本，该字段从 V4.3.5 BP1 版本开始引入。</li></ul></p></main>|
