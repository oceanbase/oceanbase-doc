...
展示当前用户可访问的表的二级分区的二级分区名、表的名称和它所属的分区以及它的存储属性。
...
查看当前用户可访问的分区表中，表 `TBL2_F_RL` 的二级分区信息。
...
* 查看本租户所有分区表的二级分区信息：[DBA_TAB_SUBPARTITIONS](20900.dba_tab_subpartitions-of-oracle-mode.md)

* 查看当前用户所拥有的分区表的二级分区信息：[USER_TAB_SUBPARTITIONS](29800.user_tab_subpartitions-of-oracle-mode.md)

* 查看当前用户可访问的分区表的一级分区详细信息：[ALL_TAB_PARTITIONS](5000.all_tab_partitions-of-oracle-mode.md)
