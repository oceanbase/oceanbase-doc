...
| LOG_DISK_ASSIGNED      | bigint(20)   | NO         | 日志盘已分配大小，它是 OBServer 节点上所有 Unit 的 `LOG_DISK_SIZE` 规格总和，单位为字节          |
| LOG_DISK_IN_USE        | bigint(20)   | NO         | 日志盘已使用大小，单位为字节              |
| DATA_DISK_CAPACITY      | bigint(20)   | NO         | 数据盘空间总大小，单位为字节        |
| DATA_DISK_ASSIGNED      | bigint(20)   | YES        | 暂不支持该字段，展示为 NULL <main id="notice" type='explain'><h4>说明</h4><p>该字段从 V4.3.3 版本开始引入。</p></main>    |
...
| DATA_DISK_ALLOCATED     | bigint(20)   | NO         | OBServer 节点已分配的数据盘大小，单位为字节<main id="notice" type='explain'><h4>说明</h4><p>该字段从 V4.2.0 版本开始引入。</p></main>    |
