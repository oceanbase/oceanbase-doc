...
| MDS_FILTER_INFO                 | varchar(1024) | NO  | 合并使用的 MDS 过滤信息 <main id="notice" type='explain'><h4>说明</h4><ul><li>对于 V4.4.x 版本，该字段从 V4.4.0 版本开始引入并生效。</li><li>对于 V4.3.5 版本，该字段从 V4.3.5 版本开始引入，从 V4.3.5 BP2 版本开始生效。</li></ul></main> |
| EXECUTE_TIME                    | bigint(20)    | NO | 合并任务消耗 CPU 的时间，单位为毫秒  |
| FILTER_ROW_COUNT                | bigint(20)    | NO | 通过 Compaction 进行数据回收的行数 <main id="notice" type='explain'><h4>说明</h4><ul><p>该字段从 V4.6.0 版本开始引入。</p></main>  |
...
```shell
obclient(root@sys)[oceanbase]> SELECT * FROM oceanbase.GV$OB_TABLET_COMPACTION_HISTORY LIMIT 1\G
```
...
* 查看当前节点上 Tablet 级的合并的详细历史信息：[V$OB_TABLET_COMPACTION_HISTORY](34700.v-ob_tablet_compaction_history-of-sys-tenant.md)

* 查看所有节点上 Tablet 级 Compaction 的进度信息：[GV$OB_TABLET_COMPACTION_PROGRESS](4400.gv-ob_tablet_compaction_progress-of-sys-tenant.md)
