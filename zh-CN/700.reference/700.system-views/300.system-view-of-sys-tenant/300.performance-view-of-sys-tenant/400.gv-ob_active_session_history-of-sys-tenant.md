...
| GROUP_ID         | bigint(20) | YES  | 所属资源组 ID。     |
| PLAN_HASH                   | bigint(20) unsigned | YES  | 当前执行 SQL 命令对应 Plan Hash。     |
| THREAD_ID                   | bigint(20)          | YES  | 当前活跃会话所在的线程 ID。    |
| STMT_TYPE                   | bigint(20)          | YES  | 当前活跃会话的 SQL 类型。     |
...
| ACTION | varchar(64) | YES | 会话在采样时记录的 ACTION 值，通过 `DBMS_APPLICATION_INFO.SET_ACTION` 包设置。 |
| CLIENT_ID | varchar(64) | YES | 会话在采样时记录的 CLIENT_ID 值，通过 `DBMS_SESSION.set_identifier` 包设置。 |
...
| TX_ID                       | bigint(20)          | YES  | 当前事务 ID     |
| BLOCKING_SESSION_ID         | bigint(20)          | YES  | 如果当前会话被阻塞，显示阻塞该会话的会话 ID，目前仅在锁冲突场景生效，展示持有锁的会话 ID<main id="notice" type='explain'><h4>说明</h4><p>对于 V4.4.x 版本，该字段为预留字段，默认值为 NULL。</p> </main>      |
| TABLET_ID                   | bigint(20)          | YES  | 当前 SQL 正在处理的 Tablet 的 ID     |
| PROXY_SID                   | bigint(20)          | YES  | 代理会话 ID     |
...
* 查看所有租户下当前 OBServer 节点的活跃会话历史记录：[V$OB_ACTIVE_SESSION_HISTORY](30500.v-ob_active_session_history-of-sys-tenant.md)

* 获取活跃会话历史记录后，可以按需生成 ASH 报告进行分析。有关 ASH 的详细介绍，参见 [ASH 报告](../../../1000.performance-tuning-guide/400.performance-diagnosis/500.ash-report-diagnosis/100.ash-introduction.md)。