...
| RET_CODE                | bigint(20)          | NO             | 执行结果返回码：<ul><li> 0：表示没有错误，语句成功执行 </li><li>其他返回码：表示 OceanBase 错误码。</li></ul>有关错误码的详细介绍，请参见 [错误码](../../../900.error-code/600.error-code-of-mysql-mode/100.use-error-information-of-mysql-mode.md)                             |
...
| TX_INTERNAL_ROUTING     | bigint(20)           | NO            | 表示当前事务是否启用了事务路由：<ul><li>0：未启用事务路由，语句只能路由到事务开始时的 OBServer 节点上。  </li><li>1：启用事务路由，事务内的语句可以由代理 Proxy 按照最优的方式路由。 </li></ul><main id="notice" type='explain'><h4>说明</h4><ul><li>该字段从 V4.1.0 版本开始引入。</li><li>OBProxy 从 V4.1.0 版本开始支持该功能。</li></ul></main>  |
| TX_STATE_VERSION        | bigint(20) unsigned  | NO            | 当事务路由开启后，执行语句后事务状态若发生变化，则版本号递增；若未发生变化，则版本号不变<main id="notice" type='explain'><h4>说明</h4><ul><li>该字段从 V4.1.0 版本开始引入。</li><li>OBProxy 从 V4.1.0 版本开始支持该功能。</li></ul></main>  |
...
| NETWORK_WAIT_TIME               | bigint(20) unsigned | YES  |  所有 Network 类事件的总时间，单位为微秒<main id="notice" type='explain'><h4>说明</h4><p>该字段从 V4.2.3 版本开始引入。</p></main>    |
...
| PROXY_USER                      | varchar(128)        | NO  |<ul><li>代理用户登录场景，返回代理用户的名称。</li><li>非代理用户登录场景，显示为空。</li></ul><main id="notice" type='explain'><h4>说明</h4><p>该字段从 V4.2.3 版本开始引入。</p></main> |
| SEQ_NUM |bigint(20)|NUMBER(38)|事务中的语句序号。</li></ul><main id="notice" type='explain'><h4>说明</h4><p>该字段从 V4.2.5 版本开始引入，字段默认为 NULL。</p></main>|
| PLSQL_COMPILE_TIME              | bigint(20)          |  NUMBER(38) |  PL 编译用时。</li></ul><main id="notice" type='explain'><h4>说明</h4><p>该字段从 V4.2.5 BP2 版本开始引入，字段默认为 NULL。</p></main>   |
| USER_CLIENT_PORT                | bigint(20)          | NUMBER(38)  |用于展示客户端端口号。</li></ul><main id="notice" type='explain'><h4>说明</h4><p>该字段从 V4.2.5 BP2 版本开始引入。</p></main>|
| TRANS_STATUS                    | varchar(256)        | VARCHAR2(256) |用于展示显示开启事务或隐式开启事务或不开启事务。</li></ul><main id="notice" type='explain'><h4>说明</h4><p>该字段从 V4.2.5 BP2 版本开始引入。</p></main>|
| TX_TABLE_READ_CNT                    | bigint(20)        | NO   | 查询读事务状态表的次数。</li></ul><main id="notice" type='explain'><h4>说明</h4><p>对于 V4.2.5 版本，该字段从 V4.2.5 BP8 版本开始引入。</p></main>  |
| OUTROW_LOB_CNT                   | bigint(20)        | NO   | 查询过程中，读取到的 outrow lob 列的数量。</li></ul><main id="notice" type='explain'><h4>说明</h4><p>对于 V4.2.5 版本，该字段从 V4.2.5 BP8 版本开始引入。</p></main>  |
