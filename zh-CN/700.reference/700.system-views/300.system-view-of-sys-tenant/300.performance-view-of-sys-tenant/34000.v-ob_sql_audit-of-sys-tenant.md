...
| MEMSTORE_READ_ROW_COUNT | bigint(20)          | NO             | 对于 V4.4.2 版本：<ul><li>V4.4.2 BP1 之前版本：MemStore 中的读行数。 </li><li>V4.4.2 BP1 及之后版本：做 Filter 前从 MemStore 读取的行数。</li></ul>|
| SSSTORE_READ_ROW_COUNT  | bigint(20)          | NO             | 对于 V4.4.2 版本：<ul><li>V4.4.2 BP1 之前版本：SSStore 中连读的行数。 </li><li>V4.4.2 BP1 及之后版本：下压/非下压路径下读取的 Major SSTable、Minor SSTable、Mini SSTable 行数。</li></ul>|
...
| BLOCKSCAN_BLOCK_CNT     | bigint(20)          | NO            | 对于 V4.4.2 版本：<ul><li>V4.4.2 BP1 之前版本：单边扫描的数据微块数量。 </li><li>V4.4.2 BP1 及之后版本：下压路径下打开的微块数。</li></ul>|
| BLOCKSCAN_ROW_CNT       | bigint(20)          | NO            | 对于 V4.4.2 版本：<ul><li>V4.4.2 BP1 之前版本：单边扫描的数据行数。 </li><li>V4.4.2 BP1 及之后版本：下压路径下打开的微块在 Range 范围内的总行数。</li></ul>|
| PUSHDOWN_STORAGE_FILTER_ROW_CNT | bigint(20)  | NO            | 对于 V4.4.2 版本：<ul><li>V4.4.2 BP1 之前版本：下压存储 Filter 过滤后行数。 </li><li>V4.4.2 BP1 及之后版本：下压/非下压 Filter 过滤的总行数。</li></ul>|
...
| NETWORK_WAIT_TIME               | bigint(20) unsigned | NO  | 所有 Network 类事件的总时间，单位为微秒。|
...
| INSERT_DUPLICATE_ROW_COUNT | bigint(20) | NO |记录 `insertup` 或者 `replace into` 时重复行数的数量。<main id="notice" type='explain'><h4>说明</h4><ul><li>该字段从 V4.3.5 BP2 版本开始引入。</li> |
...
| TX_TABLE_READ_CNT       | bigint(20)          | NO            | 查询读事务状态表的次数。 <main id="notice" type='explain'><h4>说明</h4><p>对于 V4.4.2 版本，该字段从 V4.4.2 BP1 版本开始引入。</p></main>|
| OUTROW_LOB_CNT          | bigint(20)          | NO            | 查询过程中，读取到的 outrow lob 列的数量。 <main id="notice" type='explain'><h4>说明</h4><p>对于 V4.4.2 版本，该字段从 V4.4.2 BP1 版本开始引入。</p></main>|
