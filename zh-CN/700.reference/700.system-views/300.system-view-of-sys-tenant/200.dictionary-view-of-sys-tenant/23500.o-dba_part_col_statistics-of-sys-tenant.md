...
|slug|o-dba_part_col_statistics-sys-tenant|
...
| NUM_NULLS | decimal(10,0) | NO | 列中 NULL 值的个数 |
| NUM_BUCKETS | decimal(10,0) | NO | 列直方图中桶的个数 |
| LAST_ANALYZED | datetime(6) | NO | 上次分析该列的日期 |
| SAMPLE_SIZE | decimal(10,0) | NO | 分析该列时的采样大小 |
| GLOBAL_STATS | varchar(3) | NO | 指示统计信息是否已收集：<ul><li>YES：已收集</li><li>NO：已从子分区汇总或者未收集</li></ul> |
| NOTES | varchar(80) | NO | 描述统计信息的一些附加属性：<ul><li>INCREMENTAL：表示该列有概要</li><li>INCREMENTAL(HLL)：表示该列具有概要，并且概要采用 hyperloglog 格式</li><li>INCREMENTAL(SAMPLING)：表示该列具有概要，并且概要采用自适应采样格式</li></ul><br>此列可用于确定自适应采样格式的概要是否已完全淘汰并正确清除 |
| AVG_COL_LEN | decimal(10,0) | NO | 列的平均长度（以字节为单位） |
| HISTOGRAM | varchar(15) | NO | 指示直方图的类型：<ul><li>NONE</li><li>FREQUENCY</li><li>HEIGHT BALANCED</li><li>HYBRID</li><li>TOP-FREQUENCY</li></ul> |
...
* 查看索引统计信息：[DBA_IND_STATISTICS](14400.o-dba_ind_statistics-of-sys-tenant.md)
