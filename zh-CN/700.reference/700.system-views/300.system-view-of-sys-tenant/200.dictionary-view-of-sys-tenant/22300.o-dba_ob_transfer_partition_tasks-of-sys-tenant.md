...
视图 `DBA_OB_TRANSFER_PARTITION_TASKS` 展示了本租户下当前正在处理的 Transfer Partition 任务。由于不支持对系统租户下的用户表执行 Transfer Partition 操作，该视图的查询结果为空。
...
* 查看所有租户当前正在处理的 Transfer Partition 任务：[CDB_OB_TRANSFER_PARTITION_TASKS](11300.o-cdb_ob_transfer_partition_tasks-of-sys-tenant.md)
