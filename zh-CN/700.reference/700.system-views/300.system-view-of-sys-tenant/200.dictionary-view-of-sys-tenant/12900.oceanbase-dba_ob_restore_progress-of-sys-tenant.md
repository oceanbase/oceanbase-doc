...
| RECOVER_SCN                      | bigint(20) unsigned |  NO   | 日志的恢复位点。对于系统租户，该字段的值为 `NULL`<main id="notice" type='explain'><h4>说明</h4><p>该字段从 V4.2.1 BP7 版本开始引入。</p></main> |
| RECOVER_SCN_DISPLAY              | datetime(6)         |  NO   | 日志的恢复位点的时间戳表示。对于系统租户，该字段的值为 `NULL`<main id="notice" type='explain'><h4>说明</h4><p>该字段从 V4.2.1 BP7 版本开始引入。</p></main> |
| RECOVER_PROGRESS                 | decimal(6,2)        |  NO   | 日志恢复的进度，结果保留两位小数，例如：显示 `87.65` 表示当前进度为 87.65%。对于系统租户，该字段的值为 `NULL` <main id="notice" type='explain'><h4>说明</h4><p>该字段从 V4.2.1 BP7 版本开始引入。</p></main>|
| TABLET_COUNT                     | bigint(20)          |  NO   | 待恢复的 Tablet 总数。对于系统租户，该字段的值为 `NULL` <main id="notice" type='explain'><h4>说明</h4><p>该字段从 V4.2.1 BP7 版本开始引入。</p></main> |
| FINISH_TABLET_COUNT              | bigint(20)          |  NO   | 已完成恢复的 Tablet 总数。对于系统租户，该字段的值为 `NULL` <main id="notice" type='explain'><h4>说明</h4><p>该字段从 V4.2.1 BP7 版本开始引入。</p></main> |
| RESTORE_PROGRESS                 | decimal(6,2)        |  NO   | 数据恢复进度，结果保留两位小数，例如：显示 `87.65` 表示当前进度为 87.65%。对于系统租户，该字段的值为 `NULL` <main id="notice" type='explain'><h4>说明</h4><p>该字段从 V4.2.1 BP7 版本开始引入。</p></main> |
