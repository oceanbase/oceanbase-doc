...
| COMPOSITE               | varchar(3)      | NO         | 指示分区是否属于 composited-partition 表的本地索引：<ul><li>YES</li><li>NO</li></ul>       |
...
| COMPRESSION             | varchar(13)     | NO         | 表示分区索引是否开启压缩：<ul><li> 开启（ENABLE）   </li><li> 未开启（DISABLE） </li></ul>   |
...
系统租户查看本租户的索引表 `tbl2_f_rl` 中索引的一级分区信息。
