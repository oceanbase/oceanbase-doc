...
以下主要展示了 `TENANT ROLE CHANGE`（主备租户切换）、`SERVICE NAME`（服务名）、`DBMS_BALANCE` 等模块的典型租户事件。
...
`DBMS_BALANCE` 模块下有 `TRIGGER PARTITION BALANCE` 、`SET BALANCE WEIGHT`、`CLEAR BALANCE WEIGHT`、`CLEAR EXPIRED BALANCE WEIGHT` 4 种操作。

* **TRIGGER PARTITION BALANCE** 操作

  当用户发起一次 `DBMS_BALANCE.TRIGGER_PARTITION_BALANCE` 操作时，租户事件记录表中就会记录 `TRIGGER PARTITION BALANCE` 事件。

  `TRIGGER PARTITION BALANCE` 事件记录了 `DBMS_BALANCE.TRIGGER_PARTITION_BALANCE` 命令发起的具体时刻及相关参数信息。

  | 属性                         |	属性值                              |
  |------------------------------|--------------------------------------|
  | NAME1：BALANCE_TIMEOUT_S	   | VALUE1：触发均衡时的超时参数，单位秒  |

* **SET BALANCE WEIGHT**

  当用户发起一次 `DBMS_BALANCE.SET_BALANCE_WEIGHT` 操作时，租户事件记录表中就会记录 `SET BALANCE WEIGHT` 事件。

  `SET BALANCE WEIGHT` 事件记录了 `DBMS_BALANCE.SET_BALANCE_WEIGHT` 命令发起的具体时刻及相关参数信息。

  | 属性                         |	属性值                              |
  |------------------------------|--------------------------------------|
  | NAME1：WEIGHT	               | VALUE1：分区权重                     |
  | NAME2：DATABASE_NAME	       | VALUE2：设置分区权重的表所属的数据库名  |
  | NAME3：TABLE_NAME            | VALUE3：设置分区权重的表的表名         |
  | NAME4：PARTITION_NAME        | VALUE4：设置分区权重的一级分区名       |
  | NAME5：SUBPARTITION_NAME     | VALUE5：设置分区权重的二级分区名       |

* **CLEAR BALANCE WEIGHT**

  当用户发起一次 `DBMS_BALANCE.CLEAR_BALANCE_WEIGHT` 操作时，租户事件记录表中就会记录 `CLEAR BALANCE WEIGHT` 事件。

  `CLEAR BALANCE WEIGHT` 事件记录了 `DBMS_BALANCE.CLEAR_BALANCE_WEIGHT` 命令发起的具体时刻及相关参数信息。

  | 属性                         |	属性值                              |
  |------------------------------|--------------------------------------|
  | NAME1：DATABASE_NAME	       | VALUE1：清除分区权重的表所属的数据库名  |
  | NAME2：TABLE_NAME            | VALUE2：清除分区权重的表的表名         |
  | NAME3：PARTITION_NAME        | VALUE3：清除分区权重的一级分区名       |
  | NAME4：SUBPARTITION_NAME     | VALUE4：清除分区权重的二级分区名       |

* **CLEAR EXPIRED BALANCE WEIGHT**

  当后台线程清理失效的分区权重时，租户事件记录表中就会记录 `CLEAR EXPIRED BALANCE WEIGHT` 事件。

  `CLEAR EXPIRED BALANCE WEIGHT` 事件记录了后台清理失效分区权重的具体时刻及相关参数信息。

  | 属性                         |	属性值                              |
  |------------------------------|--------------------------------------|
  | NAME1：TABLE_NAME            | VALUE1：清理的失效分区权重的表的表名         |
  | NAME2：PARTITION_NAME        | VALUE2：清理的失效分区权重的一级分区名       |
  | NAME3：SUBPARTITION_NAME     | VALUE3：清理的失效分区权重的二级分区名       |
