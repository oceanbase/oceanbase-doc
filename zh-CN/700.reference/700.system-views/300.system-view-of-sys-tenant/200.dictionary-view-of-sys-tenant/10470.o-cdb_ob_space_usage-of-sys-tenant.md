...
视图 `CDB_OB_SPACE_USAGE` 用于记录各个租户在所有节点合计占用存储空间的详细情况。
...
| ENDPOINT    | varchar(1024) | NO   | 该字段值为 `NULL`，表示未定义。      |
| PATH        | varchar(1024) | NO   | 该字段值为 `NULL`，表示未定义。      |
| SPACE_TYPE  | varchar(11)   | NO   | <ul><li>Local Data：本地部署模式下的所有宏块，包括租户所有副本节点的所有数据、所有元数据、所有临时文件。 </li><li>clog Data：本地部署模式下的 clog，包括租户所有副本节点的所有 clog。 </li><li>PRIVATE：多副本多版本总和、临时文件 </li><li>Shared Data：租户共享的多版本基线，该字段值恒为 `0`，表示未共享数据。 </li></ul>    |
| USAGE_BYTES | decimal(42,0) | NO   | 数据实际占用磁盘的大小 required_size，单位为字节。   |

## 查询示例

sys 租户查看各个租户在所有节点合计占用磁盘空间的详细情况。
...
* 查看所有租户下所有用户表（多副本环境下以 Leader 副本为准）的大小：[oceanbase.CDB_OB_TABLE_SPACE_USAGE](11330.o-cdb_ob_table_space_usage-of-sys-tenant.md)
* 查看各租户在各个节点存储空间使用的详细信息：[oceanbase.CDB_OB_SERVER_SPACE_USAGE](10450.o-cdb_ob_server_space_usage-of-sys-tenant.md)