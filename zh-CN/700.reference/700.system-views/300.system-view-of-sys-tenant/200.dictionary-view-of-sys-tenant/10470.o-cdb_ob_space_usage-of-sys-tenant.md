...
| SPACE_TYPE  | varchar(11)   | NO   | <ul><li>`Local Data`：本地部署模式下的所有宏块。包括租户所有副本节点的所有数据、所有元数据、所有临时文件。 </li><li>`clog Data`：本地部署模式下的 clog。包括租户所有副本节点的所有 clog。 </li><li>`PRIVATE`：多副本多版本总和、临时文件，该字段值恒为 `0`，表示未共享数据。 </li></ul>    |
