...
展示所有租户（不含 Meta 租户）的分区表的模板化二级分区信息。
...
| COMPRESSION           | varchar(4)      | NO         | 指示子分区模板中的每个新添加的复合分区，其子分区数据是否以压缩格式存储：<ul><li>YES</li><li>（默认）NO </li></ul> <br>该值可以在子分区模板中指定        |
| INDEXING              | varchar(4)      | NO         | 指示子分区模板中的每个新添加的复合分区，其子分区数据是否被视为部分索引：<ul><li>ON</li><li>（默认）OFF</li></ul><br>该值可以在子分区模板中指定          |
| READ_ONLY             | varchar(4)      | NO         | 指示子分区模板中的每个新添加的复合分区，其子分区数据是否为只读：<ul><li>YES</li><li>（默认）NO</li></ul><br>该值可以在子分区模板中指定          |
...
* 查看本租户下的分区表的模板化二级分区信息：[DBA_SUBPARTITION_TEMPLATES](25300.o-dba_subpartition_templates-of-sys-tenant.md)
