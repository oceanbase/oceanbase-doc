...
2. 查询表 `tbl1` 的列信息。
   ...
   ```shell
   obclient [test]> SELECT CON_ID, OWNER, TABLE_NAME, COLUMN_NAME FROM oceanbase.CDB_TAB_COL_STATISTICS WHERE TABLE_NAME LIKE '%tbl1%';
   ```
   ...
   ```shell
   +--------+-------+------------+-------------+
   | CON_ID | OWNER | TABLE_NAME | COLUMN_NAME |
   +--------+-------+------------+-------------+
   |      1 | test  | tbl1       | name        |
   |      1 | test  | tbl1       | id          |
   |      1 | test  | tbl1       | date        |
