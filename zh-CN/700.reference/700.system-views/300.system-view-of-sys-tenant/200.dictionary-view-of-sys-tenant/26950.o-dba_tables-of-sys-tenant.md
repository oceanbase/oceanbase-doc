...
该视图用于展示当前租户未被回收的普通表、索引及外表等对象的属性信息，例如是否支持自动分区分裂等信息。
...
| INMEMORY                  | varchar(8)   | NO   | 兼容 Oracle 字段，值为 NULL，不具备实际含义。|
| INMEMORY_PRIORITY         | varchar(8)   | NO   | 兼容 Oracle 字段，值为 NULL，不具备实际含义。|
| INMEMORY_DISTRIBUTE       | varchar(15)  | NO   | 兼容 Oracle 字段，值为 NULL，不具备实际含义。|
| INMEMORY_COMPRESSION      | varchar(17)  | NO   | 兼容 Oracle 字段，值为 NULL，不具备实际含义。|
| INMEMORY_DUPLICATE        | varchar(13)  | NO   | 兼容 Oracle 字段，值为 NULL，不具备实际含义。|
...
| CELLMEMORY                | varchar(24)  | NO   | 兼容 Oracle 字段，值为 NULL，不具备实际含义。|
...
| INMEMORY_SERVICE          | varchar(12)  | NO   | 兼容 Oracle 字段，值为 NULL，不具备实际含义。|
| INMEMORY_SERVICE_NAME     | text         | NO   | 兼容 Oracle 字段，值为 NULL，不具备实际含义。|
