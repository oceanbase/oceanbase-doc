...
| SPACE_TYPE  | varchar(9)    | NO   | 无共享下展示为 BULL；共享模式下只统计节点私有数据，可分为几类：<ul><li>DATA_TABLE：所有主表 Tablet 的数据，包含 Lob Tablet </li><li>INDEX_TABLE：所有索引表 Tablet 的数据 </li><li>META：所有元数据 </li><li>TMP_FILE：临时文件 </li><li>CLOG：节点上的 clog 总大小 </li><li>SLOG：节点上的 slog 总大小，共享存储没有改行 </li></ul>     |
| DATA_BYTES  | decimal(42,0) | NO   | 数据内容大小     |
| USAGE_BYTES | decimal(42,0) | NO   | 实际占用磁盘空间大小     |

## 查询示例

SN（Shared-Nothing） 模式下，sys 租户查询各个节点存储空间使用的详细信息。
