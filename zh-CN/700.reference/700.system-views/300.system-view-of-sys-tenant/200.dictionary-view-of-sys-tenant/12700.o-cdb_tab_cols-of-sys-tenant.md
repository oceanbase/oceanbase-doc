...
| NUM_NULLS            | bigint(20)      | NO         | 列中 `NULL` 值的个数                                 |
| NUM_BUCKETS          | bigint(20)      | NO         | 列直方图中桶的个数                                    |
| LAST_ANALYZED        | date            | NO         | 上次分析此列的时间                                    |
| SAMPLE_SIZE          | bigint(20)      | NO         | 分析此列时的采样大小                                   |
| CHARACTER_SET_NAME   | varchar(44)     | NO         | 字符集名称：<ul><li> `CHAR_CS`   </li><li> `NCHAR_CS`   </li></ul>    |
| CHAR_COL_DECL_LENGTH | bigint(0)       | NO         | 该字段默认为 NULL                                  |
| GLOBAL_STATS         | varchar(3)      | NO         | 如果统计信息被收集或增量维护，则是 `YES`，否则是`NO`                                                                                  |
| USER_STATS           | varchar(3)      | NO         | 指示统计信息是否由用户直接输入 ( `YES`) 或不是 ( `NO`)         |
| AVG_COL_LEN          | bigint(21)      | NO         | 列的平均长度，单位为字节                              |
| CHAR_LENGTH          | bigint(20)      | NO         | 以字符显示列的长度。此值仅适用于以下数据类型：<ul><li>`CHAR`   </li><li> `VARCHAR2`   </li><li> `NCHAR`   </li><li> `NVARCHAR2`   </li></ul>  |
| CHAR_USED            | varchar(1)      | NO         | 表示该列使用`BYTE`长度语义（`B`）或`CHAR`长度语义（`C`），或者数据类型是否不是以下任何一种（NULL）：<ul><li>`CHAR`   </li><li> `VARCHAR2`   </li><li> `NCHAR`   </li><li> `NVARCHAR2`   </li></ul>    |
