...
| CHAR_LENGTH          | bigint(20)      | NO         | 以字符显示列的长度。仅适用于以下数据类型： <li> `CHAR`   <li> `VARCHAR2`   <li> `NCHAR`   <li> `NVARCHAR2`                                                      |
| CHAR_USED            | varchar(1)      | NO         | 表示该列使用 `BYTE` 长度语义（`B`）或 `CHAR` 长度语义（`C`），如果数据类型不是以下任何一种，则展示为 `NULL`： <li> `CHAR`   <li> `VARCHAR2`   <li> `NCHAR`   <li> `NVARCHAR2`                |
...
| HISTOGRAM            | varchar(15)     | NO         | 指示直方图的类型： <li> `NONE`   <li> `FREQUENCY`   <li> `TOP-FREQUENCY`   <li> `HEIGHT BALANCED`   <li> `HYBRID`    |
