...
展示所有租户的表、视图对象的列信息。该视图与 `CDB_TAB_COLS` 一致，但比 `CDB_TAB_COLS` 多了 `NOTES` 列，`NOTES` 列当前未使用。
...
| DATA_TYPE_MOD        | varchar(3)      | NO         | 该字段默认为 NULL               |
| DATA_TYPE_OWNER      | varchar(128)    | NO         | 该字段默认为 NULL        |
| DATA_LENGTH          | bigint(21)      | NO         | 列的长度，单位为字节                                 |
| DATA_PRECISION       | bigint(20)      | NO         | <ul><li>展示 `NUMBER` 数据类型的小数精度</li><li>展示 `FLOAT` 数据类型的二进制精度</li> <li>所有其他数据类型为 `NULL`</li></ul>         |
| DATA_SCALE           | bigint(20)      | NO         | 数字中小数点右边的数字                                  |
| NULLABLE             | varchar(1)      | NO         | 指示列是否允许 NULL。该值是列是否存在 `NOT NULL` 约束或列是否属于 `PRIMARY KEY`                                                                    |
...
| NUM_NULLS            | bigint(20)      | NO         | 列中的 NULL 值的个数                                   |
| NUM_BUCKETS          | bigint(20)      | NO         | 列直方图中桶的个数                                    |
| LAST_ANALYZED        | date            | NO         | 最近分析此列的日期                                    |
| SAMPLE_SIZE          | bigint(20)      | NO         | 用于分析此列的样本量                                   |
| CHARACTER_SET_NAME   | varchar(44)     | NO         | 字符集名称：<ul><li>`CHAR_CS`</li><li>`NCHAR_CS`  </li></ul>    |
| CHAR_COL_DECL_LENGTH | bigint(0)       | NO         | 该字段默认为 NULL                                  |
| GLOBAL_STATS         | varchar(3)      | NO         | `GLOBAL_STATS`如果统计信息被收集或增量维护，将`YES`是，否则它将是`NO`                                                                                  |
| USER_STATS           | varchar(3)      | NO         | 指示统计信息是否由用户直接输入 ( `YES`) 或不是 ( `NO`)         |
| NOTES                | varchar(80)     | NO         | 该字段默认为 NULL |
| AVG_COL_LEN          | bigint(21)      | NO         | 列的平均长度（以字节为单位）                               |
| CHAR_LENGTH          | bigint(20)      | NO         | 以字符显示列的长度。仅适用于以下数据类型：<ul><li>`CHAR` </li><li> `VARCHAR2` </li><li>`NCHAR`</li><li>`NVARCHAR2`  </li></ul>    |
| CHAR_USED            | varchar(1)      | NO         | 表示该列使用 `BYTE` 长度语义（`B`）或 `CHAR` 长度语义（`C`），如果数据类型不是以下任何一种，则展示为 `NULL`：<ul><li>`CHAR`   </li><li> `VARCHAR2`   </li><li> `NCHAR`   </li><li> `NVARCHAR2`  </li></ul>    |
| V80_FMT_IMAGE        | varchar(3)      | NO         | 该字段默认为 NULL   |
| DATA_UPGRADED        | varchar(3)      | NO         | 该字段默认为 NULL        |
| HIDDEN_COLUMN        | varchar(3)      | NO         | 指示该列是否为隐藏列 ( `YES`) 或不是 ( `NO`)              |
| VIRTUAL_COLUMN       | varchar(3)      | NO         | 指示列是否为虚拟列 ( `YES`) 或不是 ( `NO`)               |
| SEGMENT_COLUMN_ID    | bigint(0)       | NO         | 该字段默认为 NULL                                       |
| INTERNAL_COLUMN_ID   | bigint(0)       | NO         | 该字段默认为 NULL                                      |
| HISTOGRAM            | varchar(15)     | NO         | 指示直方图的类型：<ul><li>`NONE`   </li><li> `FREQUENCY`   </li><li> `TOP-FREQUENCY`   </li><li> `HEIGHT BALANCED`   </li><li> `HYBRID` </li></ul>   |
| QUALIFIED_COL_NAME   | text            | NO         | 限定列名                                         |
| USER_GENERATED       | varchar(3)      | NO         | 指示该列是用户生成的列 ( `YES`) 还是系统生成的列 ( `NO`)        |
| DEFAULT_ON_NULL      | varchar(3)      | NO         | 该字段默认为 NULL                      |
| IDENTITY_COLUMN      | varchar(3)      | NO         | 该字段默认为 NULL           |
| EVALUATION_EDITION   | varchar(128)    | NO         | 该字段默认为 NULL               |
| UNUSABLE_BEFORE      | varchar(128)    | NO         | 该字段默认为 NULL                     |
| UNUSABLE_BEGINNING   | varchar(128)    | NO         | 该字段默认为 NULL            |
| COLLATION            | varchar(100)    | NO         | 该字段默认为 NULL                     |
| COLLATED_COLUMN_ID   | bigint(0)       | NO         | 该字段默认为 NULL                     |
