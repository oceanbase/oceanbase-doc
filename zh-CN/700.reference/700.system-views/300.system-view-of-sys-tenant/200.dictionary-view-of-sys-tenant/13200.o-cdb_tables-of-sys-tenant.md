...
| IOT_TYPE                  | varchar(12)   | NO         | 表示表组织模式。取值如下：<ul><li>`NULL`：表示堆表表组织模式（用于 OLAP 的无序数据存储）。</li><li>`IOT`：表示聚集索引表模式（用于 OLTP 的索引组织存储）。</li></ul> <main id="notice" type='explain'><h4>说明</h4><p>对于 OceanBase 数据库 V4.3.5 版本，该字段从 V4.3.5 BP1 版本开始支持。</p></main>|
