...
展示所有租户的分区对象（分区表、分区索引）的一级分区键包含的列。
...
系统租户下，查询租户 ID 为 1002 的租户的分区表 `tbl2_f_rl` 的一级分区键包含的列。
...
查看当前租户的分区对象的一级分区键包含的列，参见 [DBA_PART_KEY_COLUMNS](24800.o-dba_part_key_columns-of-sys-tenant.md)。