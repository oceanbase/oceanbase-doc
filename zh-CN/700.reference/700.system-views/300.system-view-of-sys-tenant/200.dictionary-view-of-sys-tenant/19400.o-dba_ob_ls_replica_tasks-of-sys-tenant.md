...
| DATA_SOURCE_SVR_IP     | varchar(46)    | YES                | 数据源 IP    <main id="notice" type='explain'><h4>说明</h4><p>该字段从 V4.2.3 版本开始引入。</p></main>   |
| DATA_SOURCE_SVR_PORT   | bigint(20)     | YES                | 数据源 PORT    <main id="notice" type='explain'><h4>说明</h4><p>该字段从 V4.2.3 版本开始引入。</p></main>   |
| IS_MANUAL              | varchar(6)     | NO                 | 容灾任务来源，可取值：<ul><li>`TRUE`：表示手动运维命令触发的容灾任务</li><li>`FALSE`：默认值，表示 RS 层容灾自动生成的容灾任务</li></ul>     <main id="notice" type='explain'><h4>说明</h4><p>该字段从 V4.2.3 版本开始引入。</p></main>  |
...
* 查看所有租户正在运行中的 Replica 级别的容灾任务：[CDB_OB_LS_REPLICA_TASKS](9700.o-cdb_ob_ls_replica_tasks-of-sys-tenant.md)

* 查看所有租户已执行的 Replica 级别容灾任务历史：[CDB_OB_LS_REPLICA_TASK_HISTORY](9600.o-cdb_ob_ls_replica_task_history-of-sys-tenant.md)

* 查看当前租户已执行的 Replica 级别容灾任务历史：[DBA_OB_LS_REPLICA_TASK_HISTORY](19300.o-dba_ob_ls_replica_task_history-of-sys-tenant.md)
