...
| UNIT_NUM | bigint(20) | NO | 租户各 Zone 的 `UNIT_NUM` 的最小值 <main id="notice" type='explain'><h4>说明</h4><ul><li>该字段从 V4.2.0 版本开始引入。</li> <li>在 V4.2.5 版本中，V4.2.5 BP5 （不含）之前版本，该字段表示租户各 Zone 的 `UNIT_NUM` 值；V4.2.5 BP5 及之后版本，该字段表示租户各 Zone 的 `UNIT_NUM` 的最小值。</li></ul></main> |
| ZONE_UNIT_NUM_LIST | varchar(65536) | NO | 展示租户下每个 Zone 内 Unit 的数量 <main id="notice" type='explain'><h4>说明</h4><p>该字段从 V4.2.5 BP5 版本开始引入。</p></main> |
