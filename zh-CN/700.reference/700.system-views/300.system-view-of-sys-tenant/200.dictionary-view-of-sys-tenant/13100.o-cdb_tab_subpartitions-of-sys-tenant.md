...
|dir-name-en||
|tenant-type||
|slug|o-cdb_tab_subpartitions-sys-tenant|
...
展示所有租户的分区表的二级分区信息，包含二级分区名称、表和所属分区的名称、其存储属性以及 `DBMS_STATS` 包生成的统计信息。
...
系统租户查询租户 ID 为 1002 的租户的分区表 `tbl2_f_rl` 的二级分区信息。
...
* 查看本租户的分区表的二级分区表信息：[DBA_TAB_SUBPARTITIONS](26900.o-dba_tab_subpartitions-of-sys-tenant.md)

* 查看所有租户的分区表的一级分区详细信息：[CDB_TAB_PARTITIONS](13000.o-cdb_tab_partitions-of-sys-tenant.md)