...
| COMPRESSION           | varchar(4)      | NO         | 指示子分区模板中的每个新添加的复合分区，其子分区数据是否以压缩格式存储：<ul><li>YES</li><li>（默认）NO <br>该值可以在子分区模板中指定 </li></ul>  |
| INDEXING              | varchar(4)      | NO         | 指示子分区模板中的每个新添加的复合分区，其子分区数据是否被视为部分索引：<ul><li>ON</li><li>（默认）OFF<br>该值可以在子分区模板中指定 </li></ul> |
| READ_ONLY             | varchar(4)      | NO         | 指示子分区模板中的每个新添加的复合分区，其子分区数据是否为只读：<ul><li>YES</li><li>（默认）NO<br>该值可以在子分区模板中指定 </li></ul>   |

## 查询示例

系统租户查询本租户的分区表的模板化二级分区信息。
...
* 查看所有租户下的分区表的模板化二级分区信息：[CDB_SUBPARTITION_TEMPLATES](12500.o-cdb_subpartition_templates-of-sys-tenant.md)
