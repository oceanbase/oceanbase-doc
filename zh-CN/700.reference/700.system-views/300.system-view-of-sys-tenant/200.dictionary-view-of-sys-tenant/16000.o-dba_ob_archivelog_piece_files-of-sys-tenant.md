...
| BACKUP_FILE_STATUS | varchar(64) | NO | 备份日志分片在备份目的端（`BACKUP_ARCHIVE_DEST`）上的文件状态，该状态取值和已有的 `FILE_STATUS` 列完全保持一致，有以下几种：<ul><li>AVAILABLE：表示有效的备份，可以用于恢复</li><li>COPYING：表示日志文件正在备份中</li><li>INCOMPLETE：表示未做归档备份，为初始状态</li><li>DELETING：日志文件正在删除</li><li>EXPIRED：表示备份文件已失效</li><li>BROKEN：备份的文件不完整，不可使用</li><li>DELETED：日志文件已删除</li></ul> |
