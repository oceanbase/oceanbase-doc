...
视图 `CDB_OB_TENANT_FLASHBACK_LOG_SCN` 用于展示本集群中所有租户下每一次日志裁剪的有关信息。
...
| OP_TYPE           | varchar(64)  | NO          | 操作类型，包括：<ul><li>`FAILOVER_TO_PRIMARY`：这是 Failover 操作的类型，当主租户不可用时，将备租户切换为主租户的操作</li><li>`FLASHBACK_STANDBY_LOG`：这是备租户日志裁剪操作的类型，用于将备租户的日志裁剪到指定的 SCN 位置</li></ul> |
