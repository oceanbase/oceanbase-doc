...
| SOURCE_REPLICA_SVR_IP  | varchar(46)    | YES                | 源端副本所在机器的 IP 地址。<main id="notice" type='explain'><h4>说明</h4><p>对于删除副本、修改法定副本数等任务，该字段无效，展示为 NULL。</p></main>|
| SOURCE_REPLICA_SVR_PORT| bigint(20)     | YES                | 源端副本所在机器的端口号。<main id="notice" type='explain'><h4>说明</h4><p>对于删除副本、修改法定副本数等任务，该字段无效，展示为 0。</p></main>|
| SOURCE_PAXOS_REPLICA_NUMBER | bigint(20)| YES                | 日志流原法定 Paxos 副本数量 |
| SOURCE_REPLICA_TYPE    | varchar(16)    | YES                | 源端副本类型。<main id="notice" type='explain'><h4>说明</h4><p>对于删除副本、修改法定副本数等任务，该字段无效，展示为 NULL。</p></main> |
| DATA_SOURCE_SVR_IP     | varchar(46)    | YES                | 数据源 IP <main id="notice" type='explain'><h4>说明</h4><p>对于 V4.2.1 版本，该字段从 V4.2.1 BP8 版本开始引入。</p></main>      |
| DATA_SOURCE_SVR_PORT   | bigint(20)     | YES                | 数据源 PORT <main id="notice" type='explain'><h4>说明</h4><p>对于 V4.2.1 版本，该字段从 V4.2.1 BP8 版本开始引入。</p></main>    |
| IS_MANUAL              | varchar(6)     | NO                 | 容灾任务来源，可取值：<ul><li>`TRUE`：表示手动运维命令触发的容灾任务</li><li>`FALSE`：默认值，表示 RS 层容灾自动生成的容灾任务</li></ul> <main id="notice" type='explain'><h4>说明</h4><p>对于 V4.2.1 版本，该字段从 V4.2.1 BP8 版本开始引入。</p></main>    |
...
+-----------+-------+-------------+------------------------------------+-------------+----------+-----------------------+-------------------------+-----------------------------+---------------------+-----------------------+-------------------------+-----------------------------+---------------------+--------------------+----------------------+-----------+------------------+--------------------+---------------------+---------------------+---------------------+-----------------------+
|         1 |     1 | ADD REPLICA | Y13CE64586BD4-000XXXXXXXXXXXXX-0-0 | INPROGRESS  | HIGH     |    xx.xx.xx.xx        |                   5078 |                           4 | FULL                |     xx.xx.xx.xx        |                    5070 |                           4 | FULL                | 0.0.0.0            |                    0 | TRUE      |    xx.xx.xx.xx   |               5078 | 2024-03-27 17:09:13 | 2024-03-27 17:09:13 | 2024-03-27 17:09:13 | add replica by manual |
+-----------+-------+-------------+------------------------------------+-------------+----------+-----------------------+-------------------------+-----------------------------+---------------------+-----------------------+-------------------------+-----------------------------+---------------------+--------------------+----------------------+-----------+------------------+--------------------+---------------------+---------------------+---------------------+-----------------------+
...
[CDB_OB_LS_REPLICA_TASK_HISTORY](18240.o-cdb_ob_ls_replica_task_history-of-sys-tenant.md)