...
| DATA_DISK_SIZE        | bigint(20)   | NO         |  Unit 能够使用的缓存云盘大小，单位为字节，仅适用于 SS（Shared-Storage） 模式。SN（Shared-Nothing） 模式下，该字段值为 `NULL`，表示未定义，允许竞争使用其他租户的磁盘空间。 <main id="notice" type='explain'><h4>说明</h4><p>该字段从 V4.3.4 版本开始引入。</p></main>   |
