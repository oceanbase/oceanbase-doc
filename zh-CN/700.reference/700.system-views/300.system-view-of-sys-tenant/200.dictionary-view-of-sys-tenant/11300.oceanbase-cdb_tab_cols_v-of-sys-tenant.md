...
<h4>说明</h4>
<ul><li>从 V4.2.1 BP7 版本开始，该视图的查询结果中不再展示索引表相关信息。</li>
<li>该视图从 V4.0.0 版本开始引入。</li>
</ul>
</main>
...
2. 查询表 `tbl1` 的列信息。
   ...
   ```shell
   obclient [test]> SELECT CON_ID, TABLE_NAME, COLUMN_NAME, DATA_TYPE, DATA_DEFAULT FROM oceanbase.CDB_TAB_COLS_V$ WHERE TABLE_NAME LIKE '%tbl1%';
   ```
   ...
   ```shell
   +--------+------------+-------------+-----------+--------------+
   | CON_ID | TABLE_NAME | COLUMN_NAME | DATA_TYPE | DATA_DEFAULT |
   +--------+------------+-------------+-----------+--------------+
   |      1 | tbl1       | name        | VARCHAR2  | NULL         |
   |      1 | tbl1       | id          | NUMBER    | NULL         |
   |      1 | tbl1       | date        | DATE      | NULL         |
   +--------+------------+-------------+-----------+--------------+
   3 rows in set
   ```
