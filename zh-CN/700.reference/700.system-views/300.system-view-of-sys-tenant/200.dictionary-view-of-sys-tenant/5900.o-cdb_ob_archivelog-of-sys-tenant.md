...
| UNIT_SIZE   | bigint(20)   | NO   | 日志归档数据单次压缩或加密的日志块大小    |
| COMPRESSION | varchar(128) | NO   | 压缩算法：<ul><li> none: 表示不压缩 </li><li> lz4_1.0：表示使用压缩算法 lz4_1.0 </li><li>zstd_1.3.8：表示使用压缩算法 zstd_1.3.8</li></ul>    |
| INPUT_BYTES | bigint(21) | NO | 读取的数据量，单位为字节|
| INPUT_BYTES_DISPLAY | varchar(28) | NO | 对 INPUT_BYTES 单位转换之后的展示，单位为 MB 、GB、TB 和 PB 中的一种|
| OUTPUT_BYTES | bigint(21) | NO | 输出的数据量，单位为字节 |
| OUTPUT_BYTES_DISPLAY | varchar(28) | NO | 对 OUTPUT_BYTES 单位转换之后的展示，单位为 MB 、GB、TB 和 PB 中的一种|
| COMPRESSION_RATIO | decimal(24,2) | NO | 压缩率 |
| DELETED_INPUT_BYTES | bigint(20) | NO | 删除的原始数据量，单位为字节 |
| DELETED_INPUT_BYTES_DISPLAY | varchar(27) | NO | 对 DELETED_INPUT_BYTES 单位转换之后的展示，单位为 MB 、GB、TB 和 PB 其中的一种 |
| DELETED_OUTPUT_BYTES | bigint(20) | NO | 删除的有效数据量，单位为字节 |
| DELETED_OUTPUT_BYTES_DISPLAY | varchar(27) | NO | 对 DELETED_OUTPUT_BYTES 单位转换之后的展示，单位为 MB 、GB、TB 和 PB 其中的一种 |
| COMMENT | varchar(262144) | NO | 放错误提示信息 |
| PATH | varchar(2048) | NO | 归档路径 |

## 查询示例

系统租户查看租户 ID 为 1002 的租户的日志归档状态。
