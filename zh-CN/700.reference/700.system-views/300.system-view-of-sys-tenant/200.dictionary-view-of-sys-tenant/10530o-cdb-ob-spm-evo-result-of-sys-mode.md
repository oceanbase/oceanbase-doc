...
|slug|oceanbase-cdb_ob_spm_evo_result-sys-tenant|
...
| TYPE                | varchar(32)  | NO   |    记录的类型：<ul><li>`OnlineEvolve`：表示在线演进模式记录的是演进任务的结果，下面所有的字段仅当类型为当前值时才有效。</li><li>`FirstBaseline`：表示记录的是生成首个基线的 SQL。</li><li>`UnReproducible`： 表示记录的是当前 SQL 所有的基线都无法复现。</li><li>`BaselineFirst`：表示基线优先模式产生了新的计划，但是还原并使用了基线计划。</li><li>`BestBaseline`：表示产生并并使用了最优基线计划。</li><li>`FixedBaseline`：表示使用了固定基线计划。</li></ul> |
