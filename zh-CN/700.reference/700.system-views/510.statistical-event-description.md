...
| backup io read delay | 备份读 I/O 接口调用时延 | 69012 | 32 |
| backup io write delay | 备份写 I/O 接口调用时延 | 69013 | 32 |
| cos io read delay | COS Get 接口调用时延 | 69014 | 32 |
| cos io write delay | COS Put 接口调用时延 | 69015 | 32 |
...
| xa start total count           | `xa_start` 语句总个数（包含成功的以及失败的）   | 30200| |
| xa start total used time       |`xa_start` 语句的总耗时（包含成功的以及失败的）  | 30201| |
| xa start with rpc total count  | 远程 `xa_start` 语句的总个数（仅包含成功的）    | 30202| |
| failed xa start total count    | `xa_start` 语句失败的总个数                    | 30203| |
| xa end total count             |`xa_end` 语句的总个数，包含成功的与失败的        | 30204| |
| xa end total used time         | `xa_end` 语句的总耗时，包含成功的与失败的       | 30205| |
| xa end with rpc total count    |  远程 `xa_end` 语句的总个数（仅包含成功的）     |30206| |
| failed xa end total count      | `xa_end` 语句失败的总个数                      | 30207| |
| xa prepare total count         | `xa_prepare` 语句的总个数（包含成功的与失败的） |30208| |
| xa prepare total used time     | `xa_prepare` 语句的总耗时（包含成功的与失败的） | 30209| |
| xa prepare with rpc total count| 远程 `xa_prepare` 语句的总个数（仅包含成功的）  | 30210 | |
| failed xa prepare total count  | `xa_prepare` 语句失败的总个数                  | 30211 | |
| xa commit total count          | `xa_commit` 语句的总个数（包含成功的与失败的）  | 30212 | |
| xa commit total used time      | `xa_commit` 语句的总耗时（包含成功的与失败的）  | 30213 | |
| xa commit with rpc total count |  远程 `xa_commit` 语句的总个数（仅包含成功的）  | 30214 | |
| failed xa commit total count   | `xa_commit` 语句失败的总个数                   | 30215 | |
| xa rollback total count        | `xa_rollback` 语句的总个数（包含成功的与失败的） | 30216 | |
| xa rollback total used time    | `xa_rollback` 语句的总耗时（包含成功的与失败的） | 30217 | |
| xa rollback with rpc total count | 远程 `xa_rollback` 语句的总个数（仅包含成功的）| 30218 | |
| failed xa rollback total count | `xa_rollback` 语句失败的总个数                  | 30219 | |
