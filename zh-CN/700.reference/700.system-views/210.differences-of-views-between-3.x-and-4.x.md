...
| DBA_OB_TENANT_FLASHBACK_LOG_SCN	| V4.2.5 BP4 | MySQL/Oracle 租户下新增，用于记录日志裁剪操作的相关记录。|
| CDB_OB_TABLE_SPACE_USAGE        | V4.2.5 BP3 | `sys` 租户下新增，展示所有用户表（多副本环境下以 Leader 副本为准）的大小。|
| DBA_OB_TABLE_SPACE_USAGE        | V4.2.5 BP3 | MySQL/Oracle 租户下新增，展示所有用户表（多副本环境下以 Leader 副本为准）的大小。|
| CDB_OB_OBJECT_BALANCE_WEIGHT    | V4.2.5 BP2 | `sys` 租户下新增，展示用户租户下所有设置过特殊权重的分区。 |
| DBA_OB_OBJECT_BALANCE_WEIGHT    | V4.2.5 BP2 | MySQL/Oracle 租户下新增，展示用户租户下所有设置过特殊权重的分区。 |
| CDB_OB_DATA_DICTIONARY_IN_LOG   | V4.2.0     | `sys` 租户下新增，记录数据字典在系统日志流中的位置范围，方便 CDC 消费数据字典。|
| DBA_OB_DATA_DICTIONARY_IN_LOG   | V4.2.0     | MySQL 租户下新增，记录数据字典在系统日志流中的位置范围，方便 CDC 消费数据字典。|
| DBA_OB_TASK_OPT_STAT_GATHER_HISTORY| V4.2.0 | MySQL/Oracle 租户下新增，查询历史收集任务执行情况。|
| DBA_OB_TABLE_OPT_STAT_GATHER_HISTORY| V4.2.0 | MySQL/Oracle 租户下新增，查询表的历史收集执行情。|
| CDB_OB_LOG_RESTORE_SOURCE| V4.2.0 | `sys` 租户下新增，展示物理恢复租户、备租户的日志恢复源。|
| DBA_OB_TABLE_STAT_STALE_INFO| V4.2.0 | MySQL/Oracle 租户下新增，展示自上次收集统计信息以来每个表发生的 DDL 操作次数，以及当前统计信息是否已过期。|
| DBA_OB_EXTERNAL_TABLE_FILES| V4.2.0 | MySQL/Oracle 租户下新增，展示当前租户下所有已创建的外表所对应的文件列表。|
| oceanbase.ALL_OB_EXTERNAL_TABLE_FILES| V4.2.0 | 展示当前用户所拥有访问权限的外部表的文件列表。|
| oceanbase.CDB_OB_EXTERNAL_TABLE_FILES| V4.2.0 | `sys` 租户下新增，展示所有租户创建的外部表的文件列表。|
| oceanbase.DBA_OB_BALANCE_JOBS| V4.2.0 | MySQL/Oracle 租户下新增，展示租户当前正在执行的负载均衡工作。|
| oceanbase.CDB_OB_BALANCE_JOBS| V4.2.0 | `sys` 租户下新增，展示所有租户当前正在执行的负载均衡工作 。|
| oceanbase.DBA_OB_BALANCE_JOB_HISTORY| V4.2.0 | MySQL/Oracle 租户下新增，展示租户下所有执行负载均衡工作的历史。|
| oceanbase.CDB_OB_BALANCE_JOB_HISTORY | V4.2.0 | `sys` 租户下新增，展示所有租户已执行的负载均衡工作历史。|
| oceanbase.DBA_OB_BALANCE_TASKS| V4.2.0 | MySQL/Oracle 租户下新增，展示租户当前正在执行的负载均衡任务。  |
|oceanbase.DBA_OB_BALANCE_TASK_HISTORY| V4.2.0 | MySQL/Oracle 租户下新增，展示租户下所有执行负载均衡任务的历史。|
| oceanbase.CDB_OB_BALANCE_TASKS| V4.2.0 | `sys` 租户下新增，展示所有租户当前正在执行的负载均衡任务。|
| oceanbase.CDB_OB_BALANCE_TASK_HISTORY| V4.2.0 | `sys` 租户下新增，展示所有租户已执行的负载均衡任务历史。|
| oceanbase.DBA_OB_TRANSFER_TASKS| V4.2.0 | MySQL/Oracle 租户下新增，展示租户下当前正在执行的 Transfer 任务。|
| oceanbase.DBA_OB_TRANSFER_TASK_HISTORY| V4.2.0 | MySQL/Oracle 租户下新增，展示租户下所有执行 Transfer 任务的历史。|
| oceanbase.CDB_OB_TRANSFER_TASKS| V4.2.0 | `sys` 租户下新增，展示所有租户当前正在执行的 Transfer 任务。|
| oceanbase.CDB_OB_TRANSFER_TASK_HISTORY| V4.2.0 | `sys` 租户下新增，展示所有租户已执行的 Transfer 任务历史。|
| oceanbase.CDB_OB_ACCESS_POINT | V4.2.0 | `sys` 租户下新增，展示租户入口的 Server 列表。|
| oceanbase.DBA_OB_ACCESS_POINT| V4.2.0 | MySQL/Oracle 租户下新增，展示租户入口的 Server 列表。|
| oceanbase.DBA_DB_LINKS| V4.2.0 | MySQL/Oracle 租户下新增，展示当前租户下所有已创建的 DBLINK。  |
| oceanbase.CDB_OB_LS_HISTORY| V4.2.0 | `sys` 租户下新增，展示所有租户历史上存在过的日志流。 |
| oceanbase.DBA_OB_LS_HISTORY| V4.2.0 | MySQL/Oracle 租户下新增，展示本租户历史上存在过的日志流。 |
| oceanbase.CDB_OB_TENANT_EVENT_HISTORY| V4.2.0 | `sys` 租户下新增，用于展示本集群中所有租户的历史操作记录。 |
| oceanbase.DBA_OB_TENANT_EVENT_HISTORY| V4.2.0 | MySQL/Oracle 租户下新增，用于展示本租户的历史操作记录。 |
| DBA_OB_DATA_DICTIONARY_IN_LOG | V4.2.0  | MySQL/Oracle 租户下新增，记录数据字典在系统日志流中的位置范围，方便 CDC 消费数据字典。 |
...
| information_schema.STATISTICS | V4.0.0 |  展示租户获取索引信息。  |
