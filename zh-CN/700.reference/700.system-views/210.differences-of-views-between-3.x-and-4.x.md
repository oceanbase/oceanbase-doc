...
| **视图名** | **引入版本**| **描述** |
| ------ | ------ | ------ |
| oceanbase.DBA_OB_DATA_DICTIONARY_IN_LOG| V4.2.0 | 记录数据字典在系统日志流中的位置范围，方便 CDC 消费数据字典|
| oceanbase.CDB_OB_DATA_DICTIONARY_IN_LOG| V4.2.0 | 记录数据字典在系统日志流中的位置范围，方便 CDC 消费数据字典|
...
| GV$OB_THREAD| V4.2.0  | 用于显示 observer 进程中的所有线程的状态信息 |
| V$OB_THREAD| V4.2.0 | 用于显示 observer 进程中的所有线程的状态信息  |
| GV$OB_ARBITRATION_SERVICE_STATUS| V4.2.0 | 用于展示集群和仲裁服务之间的通信状态  |
| V$OB_ARBITRATION_SERVICE_STATUS| V4.2.0 | 用于展示集群和仲裁服务之间的通信状态 |
| GV$OB_ARBITRATION_MEMBER_INFO| V4.2.0 | 用于展示本集群的仲裁成员信息 |
| V$OB_ARBITRATION_MEMBER_INFO| V4.2.0 | 用于展示本集群的仲裁成员信息 |
| GV$OB_OPT_STAT_GATHER_MONITOR | V4.2.0 | 查询统计信息收集任务的实时状态 |
| V$OB_OPT_STAT_GATHER_MONITOR| V4.2.0 | 查询统计信息收集任务的实时状态 |
| GV$OB_LOCKS | V4.2.0 | 用于显示当前用户各表持锁或请求锁的情况 |
| V$OB_LOCKS | V4.2.0 | 用于显示当前用户各表持锁或请求锁的情况  |
| V$OB_LS_LOG_RESTORE_STATUS| V4.2.0 | 展示日志流级别日志恢复状态 |
| V$OB_TIMESTAMP_SERVICE| V4.2.0 | 显示租户时钟服务的 GTS/STS 值、时钟服务所在的节点信息和时钟源类型  |
|GV$OB_PX_P2P_DATAHUB| V4.2.0 | 展示并行执行的数据传输操作的相关信息 |
| V$OB_PX_P2P_DATAHUB | V4.2.0 | 展示并行执行的数据传输操作的相关信息 |
| GV$SQL_JOIN_FILTER| V4.2.0 | 动态展示 Join Filter 执行相关信息  |
| V$SQL_JOIN_FILTER | V4.2.0 | 动态展示 Join Filter 执行相关信息  |
| GV$OB_SQL_PLAN| V4.2.0 | 显示用户通过执行 EXPLAIN PLAN 收集到的计划信息|
| V$OB_SQL_PLAN| V4.2.0 | 显示用户通过执行 EXPLAIN PLAN 收集到的计划信息|
...
|DBA_POLICIES| V4.1.0 | 展示了对某一个表进行访问控制时，一个表上可以创建多个策略|
| ALL_POLICIES| V4.1.0 | 展示了对某一个表进行访问控制时，一个表上可以创建多个策略|
| USER_POLICIES | V4.1.0 | 展示了对某一个表进行访问控制时，一个表上可以创建多个策略|
| DBA_POLICY_GROUPS | V4.1.0 | 展示策略组信息，配合策略上下文实现对不同的用户生效不同的策略 |
|ALL_POLICY_GROUPS | V4.1.0 | 展示策略组信息，配合策略上下文实现对不同的用户生效不同的策略 |
| USER_POLICY_GROUPS| V4.1.0 | 展示策略组信息，配合策略上下文实现对不同的用户生效不同的策略|
| DBA_POLICY_CONTEXTS| V4.1.0 | 展示访问视图时会根据上下文的取值选择对应的策略组生效|
| ALL_POLICY_CONTEXTS | V4.1.0 | 展示访问视图时会根据上下文的取值选择对应的策略组生效 |
| USER_POLICY_CONTEXTS| V4.1.0 | 展示访问视图时会根据上下文的取值选择对应的策略组生效 |
| DBA_SEC_RELEVANT_COLS | V4.1.0 |将某一列和列级策略关联起来，当访问这一列时对应策略生效  |
| ALL_SEC_RELEVANT_COLS| V4.1.0 | 将某一列和列级策略关联起来，当访问这一列时对应策略生效 |
| USER_SEC_RELEVANT_COLS |  V4.1.0| 将某一列和列级策略关联起来，当访问这一列时对应策略生效 |
...
| ALL_TRIGGER_ORDERING | V4.1.0 | 用于查看当前数据库下所有触发器的排序 |
| DBA_TRIGGER_ORDERING | V4.1.0 | 用于查看当前数据库下触发器的排序 |
| USER_TRIGGER_ORDERING |V4.1.0 | 用于查看当前用户下触发器的排序 |
