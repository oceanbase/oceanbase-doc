...
展示本租户当前正在执行的负载均衡工作。每个租户在同一时间只能有一个负载均衡工作（JOB）在运行。每个负载均衡工作会生成多个负载均衡任务（TASK），用于实现负载均衡的具体操作。
...
| MAX_END_TIME            | timestamp(6) | YES | 负载均衡任务的最大结束时间 <main id="notice" type='explain'><h4>说明</h4><ul><li>对于 V4.2.1 版本，该字段从 V4.2.1 BP9 版本开始引入；对于 V4.2.x 其他版本，该字段从 V4.2.4 版本开始引入。</li> <li>对于 V4.3.x 版本，该视图从 V4.3.4 版本开始引入。</li></ul></main>   |
