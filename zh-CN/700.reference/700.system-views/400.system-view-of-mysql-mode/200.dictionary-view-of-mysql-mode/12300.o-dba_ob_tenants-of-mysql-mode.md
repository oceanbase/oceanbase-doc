...
| TABLEGROUP_NAME | varchar(128) | NO   | 租户默认绑定的表组名称，当租户未绑定任何表组时，`TABLEGROUP_NAME` 列的值为 `NULL`。<main id="notice" type='explain'><h4>说明</h4><p>对于 V4.4.2 版本，该字段从 V4.4.2 BP2 版本开始引入。</p></main>|
...
| UNIT_NUM | bigint(20) | NO | 租户各 Zone 的 `UNIT_NUM` 的最小值 <main id="notice" type='explain'><h4>说明</h4><ul><li>该字段从 V4.2.0 版本开始引入。</li> <li>对于 V4.4.x 版本，该字段从 V4.4.2 版本开始，字段含义从 “租户各 Zone 的 <code>UNIT_NUM</code> 值” 变更为 “租户各 Zone 的 <code>UNIT_NUM</code> 的最小值”。</li> <li>在 V4.2.5 版本中，V4.2.5 BP5 （不含）之前版本，该字段表示租户各 Zone 的 <code>UNIT_NUM</code> 值；V4.2.5 BP5 及之后版本，该字段表示租户各 Zone 的 <code>UNIT_NUM</code> 的最小值。</li></ul></main> |
| ZONE_UNIT_NUM_LIST | varchar(65536) | NO | 展示租户下每个 Zone 内 Unit 的数量 <main id="notice" type='explain'><h4>说明</h4><ul><li>对于 V4.4.x 版本，该字段从 V4.4.2 版本开始引入。</li> <li>对于 V4.2.5 版本，该字段从 V4.2.5 BP5 版本开始引入。</li></ul></main> |
...
| FLASHBACK_LOG_SCN | bigint(20) | NO | 租户最近一次执行日志裁剪的位点。在 Failover 后，支持通过备租户日志裁剪到指定位点的运维命令，将原主租户的备租户接入新主租户。<main id="notice" type='explain'><h4>说明</h4><p>该字段从 V4.4.1 版本开始引入。</p></main> |
| COMMENT                    | varchar(4096)       | NO   | 注释信息     |
| PROTECTION_MODE            | varchar(128)        | NO   | 当前租户的保护模式，取值如下：<ul><li>`MAXIMUM PERFORMANCE`：最大性能</li> <li>`MAXIMUM PROTECTION`：最大保护</li> <li>`MAXIMUM AVAILABILITY`：最大可用</li></ul> <main id="notice" type='explain'><h4>说明</h4><p>对于 V4.4.2 版本，该字段从 V4.4.2 BP1 版本开始引入。</p></main> |
| PROTECTION_LEVEL           | varchar(128)        | NO   | 当前租户的保护级别，取值如下：<ul><li>`MAXIMUM PERFORMANCE`：最大性能</li> <li>`MAXIMUM PROTECTION`：最大保护</li> <li>`MAXIMUM AVAILABILITY`：最大可用</li> <li>`RESYNCHRONIZATION`：追赶日志状态</li></ul> <main id="notice" type='explain'><h4>说明</h4><p>对于 V4.4.2 版本，该字段从 V4.4.2 BP1 版本开始引入。</p></main>  |
