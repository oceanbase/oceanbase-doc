...
| TABLEGROUP_NAME | varchar(128) | NO   | 租户默认绑定的表组名称，当租户未绑定任何表组时，`TABLEGROUP_NAME` 列的值为 `NULL`。 <main id="notice" type='explain'><h4>说明</h4><p><ul><li>对于 V5.x 版本，该字段从 V5.0.1 版本开始引入。</li><li>对于 V4.4.2 版本，该字段从 V4.4.2 BP2 版本开始引入。</li></ul></p></main>|
...
| UNIT_NUM | bigint(20) | NO | 租户各 Zone 的 `UNIT_NUM` 的最小值 <main id="notice" type='explain'><h4>说明</h4><ul><li>该字段从 V4.2.0 版本开始引入。</li> <li>对于 V4.6.x 版本，该字段从 V4.6.0 版本开始，字段含义从 “租户各 Zone 的 <code>UNIT_NUM</code> 值” 变更为 “租户各 Zone 的 <code>UNIT_NUM</code> 的最小值”。</li> <li>对于 V4.4.x 版本，该字段从 V4.4.2 版本开始，字段含义从 “租户各 Zone 的 <code>UNIT_NUM</code> 值” 变更为 “租户各 Zone 的 <code>UNIT_NUM</code> 的最小值”。</li> <li>在 V4.2.5 版本中，V4.2.5 BP5 （不含）之前版本，该字段表示租户各 Zone 的 <code>UNIT_NUM</code> 值；V4.2.5 BP5 及之后版本，该字段表示租户各 Zone 的 <code>UNIT_NUM</code> 的最小值。</li></ul></main> |
| ZONE_UNIT_NUM_LIST | varchar(65536) | NO | 展示租户下每个 Zone 内 Unit 的数量 <main id="notice" type='explain'><h4>说明</h4><ul><li>对于 V4.6.x 版本，该字段从 V4.6.0 版本开始引入。</li> <li>对于 V4.4.x 版本，该字段从 V4.4.2 版本开始引入。</li> <li>对于 V4.2.5 版本，该字段从 V4.2.5 BP5 版本开始引入。</li></ul></main> |
...
| PROTECTION_MODE            | varchar(128)        | NO   | 当前租户的保护模式，当前仅支持 `MAXIMUM PERFORMANCE`（最大性能）保护模式。<main id="notice" type='explain'><h4>说明</h4><p>该字段从 V4.6.0 版本开始引入。</p></main> |
| PROTECTION_LEVEL           | varchar(128)        | NO   | 当前租户的保护级别，当前仅支持 `MAXIMUM PERFORMANCE`（最大性能）保护级别。<main id="notice" type='explain'><h4>说明</h4><p>该字段从 V4.6.0 版本开始引入。</p></main>  |
