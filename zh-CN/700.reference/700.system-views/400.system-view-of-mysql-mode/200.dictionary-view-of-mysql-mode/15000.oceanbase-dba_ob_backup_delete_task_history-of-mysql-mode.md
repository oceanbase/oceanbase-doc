...
展示当前租户下已完成的 backup_set/backup_piece 清理任务的历史信息。
...
| STATUS | varchar(64) | NO | 任务状态，包含：<ul><li> `INIT` ：任务初始化阶段 </li><li> `DOING` ：任务执行阶段 </li><li> `CANCELING` ：任务被取消阶段 </li><li> `COMPLETED`：任务执行成功 </li><li> `FAILED` ：任务执行失败 </li><li> `CANCELED` ：任务成功被取消 </li></ul> |
