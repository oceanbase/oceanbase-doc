...
|---------------------|---------------|------------|-----------------------------------------------------------------|
| SEQUENCE_ID         | bigint(20)    | NO         | 序列在本租户内的唯一 ID                                            |
| SCHEMA_VERSION      | bigint(20)    | NO         | 序列记录对应的 SCHEMA 版本                                        |
| DATABASE_ID         | bigint(20)    | NO         | 序列所属数据库的 ID                                      |
| SEQUENCE_NAME       | varchar(128)  | NO         | 序列名称                                                     |
| MIN_VALUE           | decimal(28,0) | NO         | 序列可取的最小值（含）                                              |
| MAX_VALUE           | decimal(28,0) | NO         | 序列可取的最大值（含最大值）                                           |
| INCREMENT_BY        | decimal(28,0) | NO         | 每次取值的步长，可为正整数或负整数，不可为 0                                         |
| START_WITH          | decimal(28,0) | NO         | 序列取值的起始值                                                 |
| CACHE_SIZE          | decimal(28,0) | NO         | 每个节点缓存值的数量                                                      |
| ORDER_FLAG          | tinyint(4)    | NO         | 取值是否保证全局有序                                                      |
| CYCLE_FLAG          | tinyint(4)    | NO         | 是否可以循环取值                                                        |
| IS_SYSTEM_GENERATED | tinyint(4)    | NO         | 是否是内部生成的序列，目前 IDENTITY COLUMN 生成的序列会将这个域设置为 True |
