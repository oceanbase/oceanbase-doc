...
| DATA_LENGTH                   | bigint(0) unsigned  | NO  | 存储在分区或子分区中的总字节数，即表分区或子分区占据的存储空间，按宏块大小计算，单位为 Byte。<main id="notice" type='explain'><h4>说明</h4><p>在 V4.3.x 版本，该字段从 V4.3.5 版本开始生效。</p></main> |
| MAX_DATA_LENGTH               | bigint(0) unsigned  | NO  | 目前暂不支持该字段，当前该字段默认为 0。 |
| INDEX_LENGTH                  | bigint(0) unsigned  | NO  | 索引总长度，即表分区或子分区的索引占据的存储空间，按宏块大小计算，单位为 Byte。<main id="notice" type='explain'><h4>说明</h4><p>在 V4.3.x 版本，该字段从 V4.3.5 版本开始生效。</p></main> |
