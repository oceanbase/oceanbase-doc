...
| PASSWD                    | varchar(128)  | NO   | 用户或角色密码（哈希）。认证串开头的插件标识可用于观测所用认证插件：<ul><li>`$S$`：表示 `ob_sm3_password`</li><li>`$A$`：表示 `caching_sha2_password`</li></ul> |
...
| PRIV_CREATE_SENSITIVE_RULE | varchar(3)   | NO   | 是否有创建敏感规则的权限     |
| PRIV_PLAINACCESS          | varchar(3)    | NO   | 是否有所有敏感规则的明文访问权限     |
| OLD_PASSWORD              | varchar(128)  | NO   | 旧密码的哈希值（密文），在双密码功能开启或密码轮换期间时有效。 <main id="notice" type='explain'><h4>说明</h4><p>该字段从 V4.4.2 BP1 版本开始引入。</p></main>            |
| OLD_PASSWORD_START_TIME   | bigint(20)    | NO   | 旧密码生效的时间点，值为 `-1` 时表示没有可用的旧密码。 <main id="notice" type='explain'><h4>说明</h4><p>该字段从 V4.4.2 BP1 版本开始引入。</p></main>                     |
| PLUGIN                    | varchar(64)   | NO   | 记录密码哈希计算使用的 plugin 名称。目前仅允许以下取值：<ul><li>空字符串（默认值，等同于 `mysql_native_password`）</li><li>`mysql_native_password`</li><li>`caching_sha2_password`</li><li>`ob_sm3_password`</li></ul>可通过本字段观测用户实际使用的认证插件。<main id="notice" type='explain'><h4>说明</h4><p><ul><li>该字段从 V4.4.2 BP2 版本开始引入。</li><li>对于 V4.4.2 版本，`caching_sha2_password` 取值从 V4.4.2 BP2 版本开始引入。</li><li>对于 V4.4.2 版本，`ob_sm3_password` 取值从 V4.4.2 BP3 版本开始引入。</li></ul></p></main>   |
