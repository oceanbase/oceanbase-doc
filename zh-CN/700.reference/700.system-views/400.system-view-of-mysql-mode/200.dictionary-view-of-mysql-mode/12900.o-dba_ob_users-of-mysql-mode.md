...
| PLUGIN                    | varchar(64)   | NO   |  记录密码 HASH 计算使用的 plugin 名称。目前仅允许以下取值：<ul><li>空串（与 <code>mysql_native_password</code> 等价）</li><li><code>ob_native_password</code>：Oracle 模式下等价于 <code>mysql_native_password</code></li><li><code>mysql_native_password</code></li><li><code>caching_sha2_password</code></li></ul><main id="notice" type='explain'><h4>说明</h4><p>对于 V5.x.x 来说，该字段从 V5.0.1 版本开始引入。</p></main>  |
...
| PRIV_CREATE_CATALOG        | varchar(3)    | NO   | 是否有创建 CATALOG 的权限  |
| PRIV_USE_CATALOG           | varchar(3)    | NO   | 是否有使用 CATALOG 的权限 |
| PRIV_CREATE_SENSITIVE_RULE | varchar(3)    | NO   | 是否有创建 SENSITIVE RULE 的权限  |
| PRIV_PLAINACCESS           | varchar(3)    | NO   | 是否有所有 SENSITIVE RULE 的明文访问权限 |
| OLD_PASSWORD               | varchar(128)  | NO   | 旧密码的哈希值（密文），在双密码功能开启或密码轮换期间时有效。 <main id="notice" type='explain'><h4>说明</h4><p>该字段从 V5.0.1 版本开始引入。</p></main>   |
| OLD_PASSWORD_START_TIME    | bigint(20)    | NO   | 旧密码生效的时间点，值为 `-1` 时表示没有可用的旧密码。 <main id="notice" type='explain'><h4>说明</h4><p>该字段从 V5.0.1 版本开始引入。</p></main>     |
