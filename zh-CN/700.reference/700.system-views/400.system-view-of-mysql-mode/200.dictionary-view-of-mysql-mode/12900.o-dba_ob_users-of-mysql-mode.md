...
| OLD_PASSWORD               | varchar(128)  | NO   | 存储旧密码的哈希值（密文）。当开启双密码或处于密码轮转期时，该字段不为空。  |
| OLD_PASSWORD_START_TIME    | bigint(20)    | NO   | 存储旧密码开始生效的时间点。（值为 -1 时代表无旧密码可用）Oracle 租户下该值还被用于计算旧密码是否处仍于轮转期内，过期的旧密码将不可用。     |
