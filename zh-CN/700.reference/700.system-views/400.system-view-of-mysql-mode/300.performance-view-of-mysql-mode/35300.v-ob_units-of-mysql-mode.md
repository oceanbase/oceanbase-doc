...
| DATA_DISK_SIZE   | bigint(20)   | YES        | 该字段值为 `NULL`，表示未定义，允许竞争使用其他租户的磁盘空间。<main id="notice" type='explain'><h4>说明</h4><p>该字段从 V4.3.4 版本开始引入。</p></main>     |
| DATA_DISK_IN_USE | bigint(20)   | NO         | 表示 Unit 已使用的数据盘大小，单位为字节。  |
...
| REPLICA_TYPE     | varchar(7)   | NO         | 副本类型，默认为 `FULL` <main id="notice" type='explain'><h4>说明</h4><ul><li>对于 V4.4.x 版本，字段 <code>REPLICA_TYPE</code> 从 V4.4.0 BP1 版本开始引入。</li> <li>对于 V4.5.x 版本，字段 <code>REPLICA_TYPE</code> 从 V4.5.0 版本开始引入。</li><li>对于 V4.2.5 版本，字段 <code>REPLICA_TYPE</code> 从 V4.2.5 BP7 版本开始引入。</li></ul></main>   |

## 查询示例

用户租户查看本 OBServer 节点的 Unit 信息。
...
* 查看所有节点的 Unit 信息：[GV$OB_UNITS](4600.gv-ob_units-of-mysql-mode.md)

* 有关租户资源相关的介绍，参见 [查看租户和资源信息](../../../../600.manage/200.tenant-management/600.common-tenant-operations/400.view-tenant-information.md)。