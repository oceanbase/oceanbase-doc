...
| DB_ID             | bigint(20) unsigned | NO   | 存储过程编译时使用的 database ID     |
| PL_CG_MEM_HOLD    | bigint(20)          | NO   |  表示 PL CG 阶段 hold 的内存。 <main id="notice" type='explain'><h4>说明</h4><p>该字段从 V4.2.5 BP2 版本开始引入。</p></main>    |
| SYS_VARS          | varchar(4096)       | NO   |  表示当前 Session 上影响 Plan 生成的系统变量。<main id="notice" type='explain'><h4>说明</h4><p>该字段从 V4.2.5 BP2 版本开始引入。</p></main>    |
| PARAM_INFOS       | longtext            | NO   |  用于对匿名块和 call proc info 展示被参数化的类型信息，用来判断是否匹配成功。<main id="notice" type='explain'><h4>说明</h4><p>该字段从 V4.2.5 BP2 版本开始引入。</p></main>   |

## 查询示例

查看本租户 PL 相关的缓存对象的基础信息。
