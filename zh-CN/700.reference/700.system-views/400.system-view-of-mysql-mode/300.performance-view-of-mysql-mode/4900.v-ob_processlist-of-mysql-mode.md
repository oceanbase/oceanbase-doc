...
| SERVICE_NAME      | varchar(64)         | YES   |  该 Session 由哪个服务名创建，值为 `NULL` 表示该 Session 不是由服务名创建 <main id="notice" type='explain'><h4>说明</h4><ul><li>对于 V4.2.1 版本，该字段从 V4.2.1 BP9 版本开始引入；对于 V4.2.x 其他版本，该字段从 V4.2.4 版本开始引入。</li> <li>对于 V4.3.x 版本，该字段从 V4.3.1 版本开始引入。</li></ul></main> |
| TOTAL_CPU_TIME    | bigint(21)       | NO          | 当前命令执行 CPU 使用时间，单位为秒 <main id="notice" type='explain'><h4>说明</h4><p>对于 V4.2.1 版本，该字段从 V4.2.1 BP5 版本开始引入。</p></main>   |
| TOP_INFO          | varchar(262143)  | NO         | 展示执行中 SQL 所在的顶层 PL 语句信息<main id="notice" type='explain'><h4>说明</h4><p>对于 V4.2.1 版本，该字段从 V4.2.1 BP9 版本开始引入。</p></main>     |
...
* [GV$OB_PROCESSLIST](700.gv-ob_processlist-of-mysql-mode.md)
