...
| PROFILE                 | varchar(65535)       | NO | 展示 SQL PLAN MONITOR 拓展监控指标 PROFILE。二进制的 Profile 字符串，直接查询为乱码，需要通过 `FORMAT_PROFILE(RAW_PROFILE)` 表达式解析成 Json 格式。 <main id="notice" type='explain'><h4>说明</h4><p>对于 V4.4.x 版本，从 V4.4.1 版本开始引入该字段。</p></main>|
