...
| TOTAL_SIZE     | bigint(20)   | NO             | 总共接收数据量，单位为字节               |
| NET_TIME       | bigint(20)   | NO             | RPC 从发送端发送出去到被目的端接收之间的时间差，单位为微秒                  |
| WAIT_TIME      | bigint(20)   | NO             | 接收时间到请求被放入队列的时间，单位为微秒       |
| QUEUE_TIME     | bigint(20)   | NO             | 在队列中等待的时间，单位为微秒             |
| PROCESS_TIME   | bigint(20)   | NO             | 实际处理消耗的时间，单位为微秒             |
| LAST_TIMESTAMP | timestamp(6) | NO             | 上次更新的时间               |
| DCOUNT         | bigint(20)   | NO             | 每种 Pcode RPC 的请求计数，30s 为一个统计周期  |
...
* 查看当前租户在所有节点收到的 RPC 的统计信息：[GV$OB_RPC_INCOMING](4000.gv-obrpc_incoming-of-mysql-mode.md)

* 查看当前租户在当前节点发送的 PRC 统计信息：[V$OB_RPC_OUTGOING](34600.v-obrpc_outgoing-of-mysql-mode.md)
