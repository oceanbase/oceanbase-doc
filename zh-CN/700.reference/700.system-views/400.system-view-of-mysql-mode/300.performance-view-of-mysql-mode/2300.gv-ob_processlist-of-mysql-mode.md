...
| TRANS_STATE    | varchar(32)         | YES        | 事务状态（如果会话上没有事务，则显示为空）：<ul><li>`IDLE`：空闲状态，表示事务未开始。</li> <li>`ACTIVE`：事务显式开启。通过 `START TRANSACTION`、`BEGIN` 等语句显式开启。</li> <li>`IMPLICIT_ACTIVE`：事务隐式开启，`autocommit = on` 模式下，执行 DML 等修改数据的语句后事务被开启。</li> <li>`ROLLBACK_SAVEPOINT`：事务正在回滚至保存点。</li> <li>`IN_TERMINATE`：事务正在结束，包括内部原因导致的结束和用户发起的结束事务（例如，执行 `COMMIT`、`ROLLBACK` 语句）。</li> <li>`ABORTED`：事务因发生异常而内部终止或回滚。</li> <li>`ROLLED_BACK`：事务已经被回滚。</li> <li>`COMMIT_TIMEOUT`：事务提交超时。</li> <li>`COMMIT_UNKNOWN`：事务提交结果未知。</li> <li>`COMMITTED`：事务已经成功提交。</li></ul>                                                          |
...
| SAMPLE_PERCENTAGE | bigint(20) | NO | 表示该 Session 的全链路追踪采样频率。 例如，50 表示以 50% 的概率进行诊断信息的采样<main id="notice" type='explain'><h4>说明</h4><p>该字段从 V4.2.1 版本开始引入。</p></main> |
...
| USER_CLIENT_PORT  | bigint(20)          | NO   | 客户端地址信息<main id="notice" type='explain'><h4>说明</h4><p>该字段从 V4.2.3 版本开始引入。</p></main>|
| PROXY_USER        | varchar(128)        | YES  |<ul><li>代理用户登录场景，返回代理用户的名称。</li><li>非代理用户登录场景，显示为空。</li></ul><main id="notice" type='explain'><h4>说明</h4><p>该字段从 V4.2.3 版本开始引入。</p></main> |
| SERVICE_NAME      | varchar(64)         | YES   |  该 Session 由哪个服务名创建，值为 `NULL` 表示该 Session 不是由服务名创建 <main id="notice" type='explain'><h4>说明</h4><p>该字段从 V4.2.4 版本开始引入。</p></main> |
| TOTAL_CPU_TIME    | bigint(21)          | NO   | 当前命令执行 CPU 使用时间，单位为秒 <main id="notice" type='explain'><h4>说明</h4><ul><li>在 V4.3.x 版本中，该字段从 V4.3.1 版本开始引入。</li> <li>在 V4.2.1 版本中，该字段从 V4.2.1 BP5 版本开始引入；在 V4.2.x 其他版本中，该字段从 V4.2.5 版本开始引入。</li></ul></main>      |
| TOP_INFO          | varchar(262143)     | YES  | 展示执行中 SQL 所在的顶层 PL 语句信息 <main id="notice" type='explain'><h4>说明</h4><ul><li>在 V4.3.x 版本中，该字段从 V4.3.3 版本开始引入。</li> <li>在 V4.2.1 版本中，该字段从 V4.2.1 BP9 版本开始引入；在 V4.2.x 其他版本中，该字段从 V4.2.5 版本开始引入。</li></ul></main>     |
| MEMORY_USAGE      | bigint(20)          | YES  | 单条 SQL 语句当前使用的内存大小，单位为字节 <main id="notice" type='explain'><h4>说明</h4><p>对于 V4.2.5 版本，该字段从 V4.2.5 BP2版本开始引入。</p></main>|
