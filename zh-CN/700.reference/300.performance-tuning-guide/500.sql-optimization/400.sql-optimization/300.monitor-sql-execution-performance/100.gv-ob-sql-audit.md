...
| REQUEST_TIME            | bigint(20)          | number(38)     |开始执行时间点（微秒）    |
| ELAPSED_TIME            | bigint(20)          | number(38)     |接收到请求到执行结束消耗总时间（微秒）       |
| NET_TIME                | bigint(20)          | number(38)     |发送 RPC 到接收到请求时间（微秒）       |
| NET_WAIT_TIME           | bigint(20)          | number(38)     |接收到请求到进入队列时间（微秒）        |
| QUEUE_TIME              | bigint(20)          | number(38)     |请求在队列等待时间（微秒）      |
| DECODE_TIME             | bigint(20)          | number(38)     |出队列后 Decode 时间（微秒）      |
| GET_PLAN_TIME           | bigint(20)          | number(38)     |开始执行到获得计划时间（微秒）      |
| EXECUTE_TIME            | bigint(20)          | number(38)     |Plan 执行消耗时间（微秒）       |
| APPLICATION_WAIT_TIME   | bigint(20) unsigned | number(38)     |所有 Application 类事件的总时间（微秒）      |
| CONCURRENCY_WAIT_TIME   | bigint(20) unsigned | number(38)     |所有 Concurrency 类事件的总时间（微秒）     |
| USER_IO_WAIT_TIME       | bigint(20) unsigned | number(38)     |所有 `user_io` 类事件的总时间（微秒）    |
| SCHEDULE_TIME           | bigint(20) unsigned | number(38)     |所有 Schedule 类事件的时间（微秒）      |
...
| LOCK_FOR_READ_TIME      | bigint(20)          | number(38)     |读取数据时等待锁的耗时（微秒）       |
