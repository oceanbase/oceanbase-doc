...
* 错误原因：没有对应的插件功能。通常是插件未安装或插件初始化失败了。

* 解决方式：检查插件是否正确安装并完成初始化。
...
* 错误原因：在并发事务中发生资源竞争，导致事务之间相互等待锁释放，形成死锁。

* 解决方法：

  * 应用层重试事务逻辑，通常可自动恢复。

  * 优化事务顺序或减少事务持有锁的时间，避免循环等待。
...
* OceanBase 错误码：5315
* 兼容 MySQL 错误码：1452
* 错误原因：尝试插入或更新子表（外键表）数据时，违反了外键约束。通常是因为提供的值在父表（被引用表）的主键或唯一键中不存在。
* 解决方法：

  * 确保在向子表（外键表）插入或更新数据前，父表（被引用表）中存在对应的主键值。
  * 检查外键关系两边的数据类型和值是否完全匹配。
  * 如果确实不需要此约束，可以考虑删除该外键约束（需谨慎评估对数据完整性的影响）。
...
* 错误原因：
  * 不支持输入的插件类型。
  * 支持输入的插件类型（目前 OceanBase 数据库支持 `mysql_native_password` 和 `caching_sha2_password` 类型），但是插件未被程序正确的加载。`caching_sha2_password` 从 V4.4.2 BP2 版本开始支持。
...
* 错误原因：尝试创建一个索引时，该索引已存在。

* 解决方式：检查指定的索引是否存在，删除或修改现有索引后，重新创建新的索引。
...
* [GIS 功能中空间数据类型字段及相关函数运行报错 ERROR 3548](https://www.oceanbase.com/knowledge-base/oceanbase-database-1000000000245401)
