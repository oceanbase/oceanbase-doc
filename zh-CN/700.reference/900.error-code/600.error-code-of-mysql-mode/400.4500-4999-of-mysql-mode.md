...
* 解决方法：请检查源租户是否存在以下操作：源租户的删除、源租户资源池变化、源租户的unit迁移、源租户的日志流副本各种容灾任务（补副本、删副本、迁移副本、副本类型转换、paxos_replica_number 变更）、源租户的 Switchover、源租户的升级、源租户的 Transfer 任务、源租户的日志流属性变化等。如果存在，需要保证以上操作与克隆操作没有同时进行。
