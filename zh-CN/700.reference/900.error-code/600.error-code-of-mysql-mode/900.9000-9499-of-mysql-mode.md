...
## ERROR 9008 (HY000) : Read all data fail
...
## ERROR 9009 (HY000) : Build MD5 fail
...
## ERROR 9015 (HY000) : agent initing backup count error
...
## ERROR 9034 (HY000) : log archive already stopped
...
## ERROR 9043 (HY000) : pg log archive status is still invalid
...
## ERROR 9093 (HY000) : Restore log to end
...
* 解决方式：请联系 OceanBase 的技术支持人员协助排查。

## ERROR 9103 (HY000) : too many open files 

* OceanBase 错误码：9103

* 错误原因：文件系统打开的 FD（File Descriptor）达到上限。
...
## ERROR 9121 (HY000) : reuse_version of mem_block or phy_block is not mismatched with micro_meta in s2 micro_cache
...
## ERROR 9122 (HY000) : entry not exist in s2 micro_cache
...
## ERROR 9123 (HY000) : cannot write to split src tablet
...
## ERROR 9124 (HY000) : fail to allocate a tmp file page
...
* 错误原因：微块缓存被设置为 `disable` 状态，上层模块访问微块缓存返回该错误码。
...
* 错误原因：微块缓存使用的内存达到阈值，导致无法为新加入缓存的微块分配 meta。

* 解决方法：正常情况，不影响系统运行，忽略即可。
...
* 解决方式：检查租户状态。如果是过期清理场景，需要确认其相关租户是否已正确配置清理策略。
