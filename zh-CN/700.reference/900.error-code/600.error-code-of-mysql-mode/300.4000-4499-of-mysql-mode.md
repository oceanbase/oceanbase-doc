...
* 兼容 MySQL 错误码：4012
  ...
  <main id="notice" type='explain'>
   <h4>说明</h4>
   <ul><li>对于 V4.3.x 版本，该字段从 V4.3.3 版本开始引入。</li>
   <li>对于 V4.2.x 版本，该字段从 V4.2.5 版本开始引入。</li></ul>
  </main>
...
* 错误原因：不支持的功能。
  ...
  * 数据源端 Server 为非 Alive 状态。

  * 数据源端 Server 为 Stop 状态。
  ...
  * 同构 Zone 模式下，不支持修改资源池的 `UNIT_NUM` 来进行扩缩容。

  * 同构 Zone 模式下，各资源池的 `UNIT_NUM` 不相同。要求各资源池的 `UNIT_NUM `必须相同。

  * 异构 Zone 模式下，租户下各资源池的 `UNIT_NUM` 超过两种。要求一个租户下的所有资源池的 `UNIT_NUM` 最多为两种。

  * `UNIT_NUM` 的扩缩容过程结束前，除非执行回滚操作，否则禁止修改 `UNIT_NUM` 为其他值。

  * `UNIT_NUM` 的扩缩容过程中，禁止修改租户的 Locality。

  * `UNIT_NUM` 的扩缩容过程中，禁止扩大租户资源池的 `ZONE_LIST` 范围。

  * `UNIT_NUM` 的扩缩容过程中，禁止为租户添加新的资源池。

  * `UNIT_NUM` 的扩缩容过程中，禁止合并资源池。

  * `UNIT_NUM` 的扩缩容过程中，禁止迁移 `DELETING` 状态的 Unit。

  * GTS 独占已开启，租户下各 Zone 的 `UNIT_NUM` 不能小于 2。GTS 独占开启后，要求租户下各 Zone 的 `UNIT_NUM` 大于或等于 2。

  * GTS 独占已开启，不允许指定删除 GTS 独占的 Unit 或 Unit Group。
...
* oceanbase 错误码：4412
* 兼容 MySQL 错误码：ER_QUERY_TIMEOUT（3024）
* 错误原因：`SELECT` 语句查询时超过 `max_execution_time` 所设置时间时的超时报错。
* 解决方式：调整超时时间或优化查询。
...
* 错误原因：添加 OBServer 时，Root Service 连接目标 OBServer 节点失败。

* 解决方法：

  * 检查 Root Service 与目标 OBServer 间的网络连通性。
  * 检查目标 OBServer 进程是否启动，并初始化完成。
  * 检查目标 OBServer 是否已被添加至其他集群，进程启动前数据目录、配置文件目录是否为空。

## ERROR 4785 (HY000) : Cannot drop index: needed in a foreign key constraint

* OceanBase 错误码：4785

* 错误原因：尝试删除的索引被一个或多个外键约束依赖导致删除失败。

* 解决方法： 需先移除依赖索引的外键约束，再执行索引删除操作。
