...
* 解决方法：
  ...
  * 查看 `obHdfsGetLastExceptionRootCause()`，获取 Java 侧异常。

## ERROR 11021 (HY000) : HDFS: error reading the file on hdfs

* OceanBase 错误码：11021

* 错误原因：在指定 offset 读取数据失败。

* 解决方法：

  * 检查读取的 offset 和 size 是否合法，是否超出文件实际大小。

  * 确认 HDFS datanode 节点是否健康，网络连接是否正常。

  * 查看 HDFS 日志确认是否有块损坏（corrupt block）或副本不可用的问题。
...
* 错误原因：
  ...
  * 传入的 `namenode`、`path`、`storage_info`、`hdfs_builder`、`hdfs_client` 等参数为空
  * HDFS 客户端未初始化或无效
  * Kerberos 配置缺失
  * HDFS 配置项设置失败
  * HDFS 连接建立失败
...
* 解决方法：
  ...
  * 检查 HDFS 的 URI 格式是否正确，`storage_info` 配置是否完整。
  - 确认 Kerberos 认证所需的文件（keytab、krb5.conf、ticket cache）存在且路径正确。
  - 检查 HDFS 集群的连通性，确保 namenode 可达。
  - 确认 HDFS 自定义配置项的 `key=value` 格式正确，没有空 key 或空 value。

## ERROR 11044 (HY000) : HDFS: not implementation

* OceanBase 错误码：11044

* 错误原因：调用了未实现的接口。例如，调用了 `ObStorageHdfsWriter::pwrite()`(按偏移写入)、`ObStorageHdfsAppendWriter::write()`（追加写入） 等未实现的接口。

* 解决方法：

  * 该报错为预期行为，当前 HDFS 存储后端不支持 pwrite 和 append write 操作。
  
  * 避免在业务逻辑中调用 `ObStorageHdfsWriter::pwrite()`(按偏移写入) 和 `ObStorageHdfsAppendWriter::write()`（追加写入）这两个接口。     
...
* 错误原因：
  ...
  * 重复 的 Open，可能是 HDFS util 或 reader 已经 open 过，又尝试再次 open。

  * HDFS util 未 Open，可能是在未调用 open 的情况下执行了 delete、mkdir、list 等操作。
...
* 解决方法：
  ...
  * 查看 `obHdfsGetLastExceptionRootCause()` 返回的 Java 异常信息，用于定位问题根因。

  * 检查 HDFS 文件或目录权限，确认当前用户是否有读写以及删除权限。

  * 确保 HDFS util 的 open/close 生命周期管理正确，避免重复 open 或未 open 就操作。  

  * 对于写入失败或 flush 失败的场景，检查 HDFS 集群的磁盘空间是否充足。
