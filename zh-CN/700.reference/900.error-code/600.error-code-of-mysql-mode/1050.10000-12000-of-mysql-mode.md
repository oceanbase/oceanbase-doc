...
* 错误原因：尝试使用 HDFS 尚未实现的功能。

* 解决方法：当前 HDFS 存储后端不支持 pwrite 和 append write 操作。
...
* 错误原因：
  ...
  * HDFS 访问组件重复初始化错误，可能是已经打开的文件连接被再次尝试打开。

  * 在 HDFS 相关工具尚未启动完成时，就尝试执行了 delete、mkdir、list 等操作。
...
* 解决方法：
  ...
  * 确认 HDFS 操作的生命周期管理正确，避免出现重复初始化或者未初始化就操作的情况。
