...
* 错误原因：执行 Transfer 操作时，源端与目的端的副本成员不一致。

* 解决方式：检查源端与目的端的副本状态，如果正在进行成员变更，需要等待变更完成。
...
* 解决方式：等待锁释放，系统通常会尝试重试操作。

## ERROR 7112 (HY000) : change member list compare ls transfer scn too small

* OceanBase 错误码：7112

* 错误原因：执行成员列表变更时，传输位点（SCN）太小。日志流（LS）成员变更时，需要等待所有数据传输完成，确保数据一致性。

* 解决方式：通常等待一段时间或重试可以解决。
...
## ERROR 7125(HY000) : new mv major version is not match

* OceanBase 错误码：7125

* 错误原因：物化视图的版本不匹配。

* 解决方法：通常通过刷新物化视图或重建物化视图可以解决。

## ERROR 7126(HY000) :compare tablet reorganization scn not match

* OceanBase 错误码：7126

* 错误原因：Tablet 的 reorganization scn 不匹配，可能触发的场景是 Tablet 由于 Transfer 或者分裂之后发生了变化。

* 解决方法：重试可以成功，如果连续失败，请联系 OceanBase 技术支持人员协助排查。
...
## ERROR 7127(HY000) : wait ls replace complete timeout

* OceanBase 错误码：7127

* 错误原因：REPLACE LS 任务执行超时。

* 解决方法：确认 REPLACE LS 任务的执行情况，增加超时时间或重试。
...
* 解决方法：如果需要进一步设置，需要修改配置项 `memstore_limit_percentage`，请联系技术支持人员协助排查。
