...
* 错误原因：全局临时表正被某一个会话占用，导致当前的DDL访问请求被阻塞。

* 解决方法：解决此问题的核心思路是结束会话对临时表的占用。对于事务级临时表可以通过提交事务结束占用，对于会话级临时表可以通过 `truncate` 或结束占用会话解决问题。
