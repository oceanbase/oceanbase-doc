...
* OceanBase 错误码：5951、5968、9813、9788
...
* 错误原因：文件的字符集模式与读取或写入操作的字符集不匹配。

* 解决方法：使用正确的 `UTL_FILE` 读写程序，其对应于该文件的字符集模式。
