...
* 解决方法：如果您发现物化视图已经存在，并且需要重新创建，请先删除重复的物化视图，然后再尝试创建新的物化视图。使用适当的命令或操作来删除物化视图。
...
* 错误原因：存在上次刷新时间的物化视图日志。

* 解决方法：在下一次快速刷新之前需要完全刷新。
...
* 错误原因：定义中缺少某些表的 ROWID，或者外连接的内表对连接列没有 UNIQUE 约束。

* 解决方法：指定 FORCE 或 COMPLETE 选项。如果在创建过程中出现此错误，则物化视图定义可能已更改。
