...
* 错误原因：指定的 service_name 不存在。

* 解决方式：检查连接串中指定的 service_name 是否存在问题，或者检查 Proxy 和 OCP 的连通性是否存在问题。
...
* 错误原因：在进行 service_name 相关运维操作（例如：创建或删除 service_name），通知每个 OBServer 节点生效时，存在部分 Server 失败了。

* 解决方式：等待一段时间即可，正常情况下后台线程会定期刷新，默认刷新间隔为 10s 左右。
...
* 错误原因：指定了 service_name 并且指定了 failover_mode 时，需要保证租户一定是主租户，如果当前租户已经不是主租户了，则属于 Proxy 路由错误。

* 解决方式：Proxy 的缓存不正确，或者 OCP 的缓存不正确。如果 Proxy 和 OCP 的缓存均正常，则系统会进行重试并刷新，用户重新执行 SQL 即可路由到正确的主租户上。
...
* 错误原因：指定的 service_name 被用户暂停服务了。

* 解决方式：需要使用运维命令启动该 service_name：`ALTER SYSTEM START SERVICE service_name TENANT = tenant_name;`。
