...
## OBE-00600: internal error code, arguments: -9008, Read all data fail
...
## OBE-00600: internal error code, arguments: -9009, Build MD5 fail
...
* 解决方式：检查源数据的完整性，确认是否有数据损坏或不可读的问题。

## OBE-00600: internal error code, arguments: -9015, agent initing backup count error

* OceanBase 错误码：9015

* SQLSTATE：HY000

* 错误原因：Agent 初始化备份计数错误。

* 解决方式：重新发起备份。如果仍然无法解决，请联系技术支持协助处理。

## OBE-00600: internal error code, arguments: -9034, log archive already stopped
...
* 解决方式：确认当前日志归档的状态，如果确实已停止，重新开启归档。

## OBE-00600: internal error code, arguments: -9043, pg log archive status is still invalid
...
## OBE-00600: internal error code, arguments: -9121, reuse_version of mem_block or phy_block is not mismatched with micro_meta in s2 micro_cache
...
## OBE-00600: internal error code, arguments: -9122, entry not exist in s2 micro_cache

* OceanBase 错误码：9122

* SQLSTATE：HY000

* 错误原因：存储层缓存未命中的内部错误。可能原因如下：
...
## OBE-00600: internal error code, arguments: -9123, cannot write to split src tablet

* OceanBase 错误码：9123

* SQLSTATE：HY000

* 错误原因：无法向正在分裂的源 Tablet 执行写入操作。

* 解决方法：通常不需要人工干预，系统会在 Tablet 分裂完成后自动恢复。

## OBE-00600: internal error code, arguments: -9124, fail to allocate a tmp file page

* OceanBase 错误码：9124

* SQLSTATE：HY000

* 错误原因：无法分配临时文件页。可能原因如下：

  * 磁盘空间不足，可能是临时文件所在的磁盘空间耗尽。
  * 文件系统达到最大文件数和 inode 限制。
  * 当前用户无临时文件的写入权限。

* 解决方法：

  * 检查临时文件所在磁盘空间的大小，增加临时文件所在的磁盘空间。
  * 调整文件系统的最大文件数和 inode 限制。
  * 检查当前用户权限，确认当前用户具备临时文件的写入权限。
...
* 解决方法：
  ...
  * 检查临时文件相关配置，确认临时文件大小是否达到磁盘配额上限。
  * 监控临时文件的使用情况，避免出现并发访问冲突。
...
## OBE-00600: internal error code, arguments: -9812, materialized view can not nested consistent refresh
...
## OBE-00600: internal error code, arguments: -11004, Optimizer estimate deviation is too large 'Operator: %d.%.*s EST:%.ld -> REAL:%ld'
