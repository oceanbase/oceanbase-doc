...
* 错误原因：此处不允许使用 `NULL` 快照表达式

* 解决方法：检查闪回查询语句中的语法和其表达式的有效性，确保它指向一个明确且存在于保留期内的历史快照。
...
* OceanBase 错误码：9841
* SQLSTATE：HY000
* 错误原因：PL/SQL DOM 句柄访问的节点已不再可用。
* 解决方法：确保在访问节点之前，DOM 文档及其节点仍然有效且未被释放。
...
* OceanBase 错误码：5522
* SQLSTATE：22000
* 错误原因：JSON_TABLE 中路径表达式使用了无效标识符。
* 解决方法：严格检查并修正 JSON_TABLE 函数中 PATH 子句的语法，重点关注路径开头、键名引用和数组索引的书写规范。
