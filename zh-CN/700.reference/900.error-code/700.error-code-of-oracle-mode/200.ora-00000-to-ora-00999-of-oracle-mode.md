...
* OceanBase 错误码：9784
* SQLSTATE：HY000
* 错误原因：尝试对一个非游标的变量或标识符使用了游标专属的属性。
* 解决方法：找到出错的标识符，并确保它是一个合法定义的游标。
...
* 解决方式：检查磁盘空间使用情况，排查慢 SQL 来分析产生大量临时文件的原因。若产生数据量符合预期，可调大 `temporary_file_max_disk_size` 配置项调整临时文件限额。

<main id="notice" type='explain'>
  <h4>说明</h4>
  <p>该错误码从 V4.2.5 BP4 版本开始引入。</p>
</main>
...
<main id="notice" type='explain'>
<h4>说明</h4>
<p>对于 V4.2.5 版本，该错误码从 V4.2.5 BP7 版本开始引入。</p>
</main>
