...
| topnsql | WR 落盘 SQLSTAT 时，需要记录多少条数据。取值范围为 [30，50000]。 |
| sqlstat_interval|  SQLSTAT 快照落盘的时间间隔，单位为分钟。|

例如设置每个快照的保留时间为 2265 分钟（约 1 天 12 小时），每个快照之间的间隔设置为 16 分钟，落盘时记录 30 条 SQLSTAT 数据，并将 SQLSTAT 快照落盘的时间间隔设置为 5 分钟。示例如下：
...
* 通过 `oceanbase.CDB_WR_SNAPSHOT` 视图查看所有租户的快照信息。
  ...
  | **字段**  | **描述** |
  | --- |--- |
  | CLUSTER_ID  | 集群 ID。<ul><li>-1：表示提前落盘</li><li>正常 ID：表示定时落盘 </li></ul>， |
  | TENANT_ID  | 租户 ID |
  | SNAP_ID  | 快照 ID  |
  | SVR_IP  | 节点 IP |
  | SVR_PORT  | 节点 PORT |
