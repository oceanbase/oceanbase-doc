...
ASH 报告中通过 ASH Report 部分记录关于 ASH 报告的统计数据，包括系统的版本、操作系统环境、信息采样时间段、分析时间段、ASH 和 WR 数据源、采样数量、事件数量和平均活跃会话数等这些信息，生成 Top Active Tenants、Top Active Databases、Top Node Load、Top Groups、Top Foreground DB Time、Top Background DB Time、Top Sessions、Top IO Bandwidth、Top Blocking Sessions、Top Latchs、Top DB Objects、Activity Over Time、Top Execution Phase、Top IO Events、Top SQL Statement Types、Top SQL with Top Events、Top SQL with Top Operator、Top PL/SQL Procedures、Complete List of SQL Text And Status 等 19 个模块的信息。
...
* Ash Analysis Begin Time：ASH 分析开始的时间，表示对 ASH 采样数据进行分析的起始时间点。
* Ash Analysis End Time：ASH 分析结束的时间，表示对 ASH 采样数据进行分析的结束时间点。
* Wr Analysis Begin Time：WR 分析开始的时间，表示对 WR 采样数据进行分析的起始时间点。
* Wr Analysis End Time：WR 分析结束的时间，表示对 WR 采样数据进行分析的结束时间点。
...
这部分显示了资源消耗排名靠前的数据库（Database）信息，列出了 ASH 报告分析时间段内各数据库的活动概况，按租户和数据库维度进行统计。

* Tenant Name：租户名，标识特定租户的信息。
* Database Name：数据库名，标识特定数据库的信息。如果会话未明确访问某个数据库（例如后台系统会话），则显示为 `UNDEFINED`。
* Session Type：会话类型，可能值包括 `BACKGROUND` 和 `FOREGROUND`，分别表示后台进程和前台进程。
* Total Samples：ASH 报告分析时间段内的总样本数。
* Wait Event Samples：会话在等待事件上的样本数。
* On CPU Samples：会话在 CPU 上执行的样本数。
* Avg Active Sessions：ASH 报告分析时间段内的平均活跃会话数。
* % Activity：给定数据库在 ASH 报告分析时间段内的活动（CPU + 等待）百分比。

通过 Top Active Databases 信息，可以在租户维度定位到资源消耗较高的具体数据库，便于进一步分析与特定数据库相关的 SQL 执行情况，从而在数据库维度定位性能瓶颈。
