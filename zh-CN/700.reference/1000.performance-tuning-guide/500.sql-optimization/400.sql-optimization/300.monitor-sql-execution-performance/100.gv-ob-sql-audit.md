...
| PLAN_TYPE               | bigint(20)          | number(38)     |执行计划类型： <ul><li>    0：未初始化，对应的 SQL 无执行计划（Uninitialized）</li> <ul><li>    1：本地执行计划（Local）</li>   <li> 2：远程执行计划（Remote） </li>  <li> 3：分布式执行计划（Distribute） </li></ul>       |
