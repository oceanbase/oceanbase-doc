...
| REQUEST_MEMORY_USED     | bigint(20)          | number(38)     |该请求执行过程中 SQL 引擎的内存使用峰值（peak），单位：字节。该值为执行过程中的瞬时最高内存使用量，不是累计内存分配总量。统计范围不包括 MemStore 和 Logger 内存     |
...
<main id="notice" type='explain'>
  <h4>说明</h4>
  <p><code>REQUEST_MEMORY_USED</code> 记录的是单次 SQL 请求执行过程中 SQL 引擎内存使用的最高水位（峰值），不是累计内存分配总量。例如，一条 SQL 执行过程中内存先增长到 100 MB 再下降到 20 MB，该字段值为 100 MB（峰值），而非 120 MB（累计）。统计范围仅包含 SQL 执行引擎内存（如算子工作内存、表达式计算内存等），不包括 MemStore（增量数据写入内存）和 Logger（日志内存）。</p>
</main>
