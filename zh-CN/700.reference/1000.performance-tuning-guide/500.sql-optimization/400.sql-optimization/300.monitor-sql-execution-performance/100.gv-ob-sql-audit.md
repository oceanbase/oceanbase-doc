...
| PLSQL_COMPILE_TIME            | bigint(20)            | NUMBER(38)     |  PL 编译用时。|
| INSERT_DUPLICATE_ROW_COUNT      | bigint(20) | NUMBER(38) | 记录 `insertup` 或者 `replace into` 时重复行数的数量。|
| USER_CLIENT_PORT        | bigint(20)          | NUMBER(38)    | 用于展示客户端端口号。|
| TRANS_STATUS            | varchar(256)        | VARCHAR2(256) | 用于展示显示开启事务或隐式开启事务或不开启事务。|
| TX_TABLE_READ_CNT       | bigint(20)          | NUMBER(38)    | 查询读事务状态表的次数。|
| OUTROW_LOB_CNT          | bigint(20)          | NUMBER(38)    | 查询过程中，读取到的 outrow lob 列的数量。|
| COMMIT_TIME             | bigint(20)          | NUMBER(38)    | 一个事务中触发提交的 SQL 会记录提交耗时。若 SQL 未触发提交，则默认值为 0。|
