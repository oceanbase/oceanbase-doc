...
* 当含有多个 `MIN` 或 `MAX` 函数时，可以将多个 `MIN` 或 `MAX` 改写成多个子查询，每个子查询都能通过索引直接得到最大或最小值，避免全表扫描和排序。

  ```sql
  obclient> SELECT MIN(c1), MAX(c1) FROM t1;
  <==>
  obclient> SELECT
            (SELECT c1 FROM t1 ORDER BY c1 LIMIT 1) AS 'MIN(c1)',
            (SELECT c1 FROM t1 ORDER BY c1 DESC LIMIT 1) AS 'MAX(c1)'
            FROM dual;

  obclient> EXPLAIN EXTENDED_NOADDR SELECT MIN(c1), MAX(c1) FROM t1;
  ...
  |3 |└─TABLE FULL SCAN|t1(Reverse)|1       |4             |                                                            
  ==========================================================
