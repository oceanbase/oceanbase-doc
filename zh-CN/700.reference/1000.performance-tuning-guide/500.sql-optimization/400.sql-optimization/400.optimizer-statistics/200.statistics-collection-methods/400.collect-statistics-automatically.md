...
OceanBase 数据库中表的统计信息过期判断标准为：自上一次收集统计信息以来，表中发生增、删、改操作的累计影响行数超过设定的阈值时，统计信息即判定为过期。该阈值的默认值为表总行数的 10%，可通过 Prefs 进行调整，详细信息请参见 [收集策略配置项（Prefs）管理功能](../300.statistics-management/700.collection-policy-configuration-items-prefs-management.md)。
