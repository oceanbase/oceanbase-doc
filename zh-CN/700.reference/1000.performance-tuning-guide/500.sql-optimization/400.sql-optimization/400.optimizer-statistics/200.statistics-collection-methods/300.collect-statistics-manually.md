...
此外，`DBMS_STATS` 包还提供了 `GATHER_DATABASE_STATS_JOB_PROC` 来收集整个数据库所有表的统计信息，所有收集策略都会采用默认配置的 Prefs，关于设置 Prefs 的详细信息，请参见 [收集策略配置项（Prefs）管理功能](../../../../../300.performance-tuning-guide/500.sql-optimization/400.sql-optimization/400.optimizer-statistics/300.statistics-management/700.collection-policy-configuration-items-prefs-management.md)。
