...
| METHOD_OPT        | 默认值： "FOR ALL COLUMNS SIZE AUTO"   | 设置列级别的统计信息收集方式。语法和 `method_opt` 相同。 <main id="notice" type='notice'><h4>注意</h4><p>列类型为 <code>TEXT</code> 的列默认情况下不会收集统计信息。对于 OceanBase 数据库 V4.2.1 版本，如果希望为 <code>TEXT</code> 列收集统计信息，从 V4.2.1 BP9 版本开始可以通过直接使用 <code>method_opt</code> 参数进行指定，例如：<code>FOR COLUMNS C1, C2 SIZE AUTO</code>。</p></main>|
...
|  BLOCK_SAMPLE   | 默认值："FALSE"   | 指定采样是否使用块采样。默认值为 `False` 时，表示不使用块采用。|
