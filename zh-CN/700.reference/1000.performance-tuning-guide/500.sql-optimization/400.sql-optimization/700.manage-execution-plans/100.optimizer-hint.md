...
| PUSH_SUBQ                   | PUSH_SUBQ[(@qb_name)]                                           | 用于指示优化器尽早执行未合并的子查询。<main id="notice" type='explain'><h4>说明</h4><p>对于 OceanBase 数据库 V4.3.5 版本，从 V4.3.5 BP2 版本开始支持 <code>PUSH_SUBQ</code> Hint。</p></main>|
| NO_PUSH_SUBQ                | NO_PUSH_SUBQ[(@qb_name)]                                        | 是 `PUSH_SUBQ` 的反向操作，用于优化器将未合并的子查询放在最后执行。<main id="notice" type='explain'><h4>说明</h4><p>对于 OceanBase 数据库 V4.3.5 版本，从 V4.3.5 BP2 版本开始支持 <code>NO_PUSH_SUBQ</code> Hint。</p></main>|
...
`PUSH_SUBQ` Hint 用于指示优化器尽早执行未合并的子查询，通常这些子查询如果不被合并的话，会被放在执行计划的最后一步。如果子查询成本低且能大幅减少行数，提前执行可能提升性能。但要注意，如果子查询涉及远程表或使用了合并连接（merge join），这个提示无效。适用场景如下：
...
* `@qb_name`：可选参数，指定子查询的别名（详细介绍信息，可参见上文 **QB_NAME 参数**），用于明确提示作用的对象。如果省略，则默认对整个查询中的子查询生效。
...
`NO_PUSH_SUBQ` Hint 是 `PUSH_SUBQ` 的反向操作，用于优化器将未合并的子查询放在最后执行。适用于子查询成本高或无法显著减少行数的情况。适用场景如下：
