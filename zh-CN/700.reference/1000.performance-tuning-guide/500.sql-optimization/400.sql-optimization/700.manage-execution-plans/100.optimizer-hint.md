...
| UNION_MERGE                 | UNION_MERGE(table_name index_name_list)                         | 设置对应的 Index Merge 计划。<main id="notice" type='explain'><h4>说明</h4><p>对于 OceanBase 数据库 V4.3.5 版本，从 V4.3.5 BP1 版本开始支持 <code>UNION_MERGE</code> Hint。</p></main>|
| PUSH_SUBQ                   | PUSH_SUBQ[(@qb_name)]                                           | 用于指示优化器尽早执行未改写成连接的子查询。<main id="notice" type='explain'><h4>说明</h4><p>对于 OceanBase 数据库 V4.3.5 版本，从 V4.3.5 BP2 版本开始支持 <code>PUSH_SUBQ</code> Hint。</p></main>|
| NO_PUSH_SUBQ                | NO_PUSH_SUBQ[(@qb_name)]                                        | 是 `PUSH_SUBQ` 的反向操作，用于优化器将未改写成连接的子查询放在最后执行。<main id="notice" type='explain'><h4>说明</h4><p>对于 OceanBase 数据库 V4.3.5 版本，从 V4.3.5 BP2 版本开始支持 <code>NO_PUSH_SUBQ</code> Hint。</p></main>|
| OPT_PARAM                   | OPT_PARAM('{orc_filter_pushdown_level \| parquet_filter_pushdown_level}', 'int_value') | 用于控制语句级别 ORC/Parquet 外表条件下压级别。<main id="notice" type='explain'><h4>说明</h4><p>该 Hint 从 V4.4.0 版本开始支持。</p></main>|
...
<main id="notice" type='explain'>
  <h4>说明</h4>
  <p>该 Hint 从 V4.4.0 版本开始支持。</p>
</main>

`OPT_PARAM` Hint 用于控制语句级别 ORC/Parquet 外表条件下压级别。

**语法如下：**

```sql
/*+ OPT_PARAM('{orc_filter_pushdown_level \| parquet_filter_pushdown_level}', 'int_value') */
```

参数解释：

* `orc_filter_pushdown_level`：表示控制语句级别 ORC 外表条件下压级别。`int_value` 取值如下：
...
* `parquet_filter_pushdown_level`：表示控制语句级别 Parquet 外表条件下压级别。`int_value` 取值如下：
