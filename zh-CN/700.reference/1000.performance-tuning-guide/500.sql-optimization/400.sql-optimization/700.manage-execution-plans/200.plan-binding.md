...
从 V4.2.2 版本开始，数据库的 `CREATE OUTLINE` 和 `ALTER OUTLINE` 命令扩展了功能。除了原有的绑定特定查询的执行计划外，它们现在还允许您为查询设置最大并发执行次数的限制。这项新增的限流功能能够帮助您更有效地管理数据库负载，防止因高并发查询而导致的性能问题。

例如：

- 使用 `CREATE OUTLINE` 创建了一个 `OUTLINE`，其中包含了 `USE_NL(tbl2)` 提示，这告诉优化器对于表 `tbl2` 使用嵌套循环连接，同时 `MAX_CONCURRENT(1)` 提示限制该查询至单线程执行：
