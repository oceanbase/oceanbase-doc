...
在 OceanBase 数据库 V4.1.0 版本，DAS Group Rescan 适用于单级 Subplan Filter 执行、单级或两级 Nested Loop Join 执行。该优化会对 SPF（Subplan Filter）和 NLJ（Nested Loop Join）左支进行攒批，以减少对其右支的扫描次数。该优化默认开启，可以通过内部系统变量 `_NLJ_BATCHING_ENABLED`（Oracle 模式下设置该变量需要用双引号包裹）进行关闭。语法如下：
...
Nested Loop Join 执行计划结果中，`batch_join=true` 表示采用了 DAS Group Rescan 优化。示例如下：
...
Subplan Filter 执行计划结果中，`batch_das=true` 代表采用了 DAS Group Rescan 优化。示例如下：
