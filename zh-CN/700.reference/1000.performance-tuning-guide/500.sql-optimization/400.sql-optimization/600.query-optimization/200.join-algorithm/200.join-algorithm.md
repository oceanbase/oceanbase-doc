...
在 OceanBase 数据库 V4.1.0 版本，DAS Group Rescan 适用于单级 Nested Loop Join 执行。该优化会 NLJ（Nested Loop Join）左支进行攒批，以减少对其右支的扫描次数。该优化默认开启，可以通过内部系统变量 `_NLJ_BATCHING_ENABLED`（Oracle 模式下设置该变量需要用双引号包裹）进行关闭。语法如下：
