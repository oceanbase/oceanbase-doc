...
* Q4 查询删除分区表 `t2` 中满足 `c2 > '100'` 的数据行。从执行计划中可以看到，`DELETE` 算子分配在 `EXCHANGE` 算子下面，因此 2 号和 3 号算子会作为一个任务以分区的粒度进行调度。在计划执行时，4 号算子扫描出 `t2` 表在一个分区中满足 `c2 > '100'` 的数据，3 号算子 `DELETE` 则只会删除相应分区下扫描出的数据。
