...
| INTERNAL_FUNCTION     | OceanBase 数据库内部函数。 |
| dop                   | Degree of Parallelism（并行度）。`dop=1`：表示查询不是并行执行的。          |
| force partition granule| 表示查询被强制在每个分区粒度上执行。                     |
| is_index_back         | 表示查询是否通过索引访问来回表获取数据。`is_index_back=false`：表示查询没有使用索引回表。               |
| is_global_index       | 是否使用全局索引。`is_global_index=false`：表示查询没有使用全局索引。                             |
| access                | 列出查询访问的列。`access([t15.c1], [t15.c2])`：表示查询访问了列 c1 和列 c2。                                           |
| partitions            | 表示查询将涉及的分区范围。`partitions(p[0-4])`：表示查询会访问分区 p0 到 p4。                                   |
| range_key             | 内部使用的范围键，可能用于执行全表扫描。                     |
| range                 | 范围条件，通常用于表示全表扫描。`range(MIN ; MAX)always true`：表示全表扫描会检查所有行。                             |
...
| rowset | 表示当前算子的向量化大小。`rowset=16` 表示该算子每次处理的数据行数为 16。|
| dop                   | Degree of Parallelism（并行度）。`dop=1`：表示查询不是并行执行的。          |
| force partition granule| 表示查询被强制在每个分区粒度上执行。                     |
| is_index_back         | 表示查询是否通过索引访问来回表获取数据。`is_index_back=false`：表示查询没有使用索引回表。               |
| is_global_index       | 是否使用全局索引。`is_global_index=false`：表示查询没有使用全局索引。                             |
| access                | 列出查询访问的列。`access([t13.c1], [t13.c2])`：表示查询访问了列 c1 和列 c2。                                           |
| partitions            | 表示查询将涉及的分区范围。`partitions(p[0-4])`：表示查询会访问分区 p0 到 p4。                                   |
| range_key             | 内部使用的范围键，可能用于执行全表扫描。                     |
| range                 | 范围条件，通常用于表示全表扫描。`range(MIN ; MAX)always true`：表示全表扫描会检查所有行。                             |
...
上述示例的执行计划展示中的 `outputs & filters` 详细列出了 `EXCH-IN/OUT (HASH)` 算子的信息，字段的含义与 `EXCH-IN/OUT (PKEY)` 算子相同。
