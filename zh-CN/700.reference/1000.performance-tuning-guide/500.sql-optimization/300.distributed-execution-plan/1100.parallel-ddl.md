...
OceanBase 数据库支持并行 DDL 操作，可以同时执行多个 DDL 操作，以提高数据库的并发性能。在 OceanBase 数据库中，可以通过使用配置项 `_enable_parallel_table_creation` 和 `_parallel_ddl_control` 灵活控制并行 DDL 功能，包括 `CREATE TABLE`、`TRUNCATE TABLE`、`COMMENT` `DROP TABLE` 和 `CREATE INDEX` 操作。
...
`_parallel_ddl_control` 用于控制是否在租户级别开启各类并行 DDL 的功能。OceanBase 数据库当前版本 `_parallel_ddl_control` 用于控制 `TRUNCATE TABLE`、`CREATE INDEX`、`DROP TABLE` 和 `COMMENT` 并行功能的开关。
