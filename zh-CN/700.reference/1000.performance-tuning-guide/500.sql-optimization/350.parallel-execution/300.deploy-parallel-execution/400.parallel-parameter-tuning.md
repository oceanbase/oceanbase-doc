...
OceanBase 数据库提供了一系列参数，用于控制并行执行（Parallel eXecution，PX）的初始化和调优。在 OceanBase 启动时，可以根据租户的 CPU 数量和租户配置项 `px_workers_per_cpu_quota` 计算出默认的并行执行控制参数。用户也可以选择不使用默认值，在启动时手动指定参数值，并在后期根据实际场景手动调整参数值。并行执行功能默认是开启的。

本文从以下几个方面说明并行执行参数控制技巧：
...
从 OceanBase 数据库 V4.6.0 版本起，系统支持使用节点实时的 PX 任务数作为判断是否有足够资源执行并行查询的依据，并引入以下配置项：

|        **参数名称**        | **默认值** | **级别** | **说明** |
| -------------------------- | --------- | -------- | -------- |
| `px_target_workers_per_cpu` | 8 | 组户级| 用于控制预期的并行执行线程数和租户 CPU 数的比率。|
| `px_target_low_watermark` | 60 | 组户级 | 用于设置并行线程数的低水位线百分比阈值，超过该阈值时开始对新调度 DFO 的并行度进行降级控制，避免 CPU 资源耗尽。|
| `px_target_high_watermark` | 80 | 组户级 | 用于控制并行线程数的高水位线百分比阈值。实时负载越接近该水位，并行度降级幅度越大，超过该水位时并行度降为 1。|

<main id="notice" type='notice'>
  <h4>注意</h4>
  <p>这三个配置项应该满足：<code>px_target_low_watermark</code> <= <code>px_target_high_watermark</code> <= <code>px_target_workers_per_cpu</code>。</p>
</main>

### 注意事项

* V4.6.0 及之后版本新建租户场景默认开启实时负载的并行控制，`parallel_servers_target` 流程的并行控制会失效。
* 如果是升级到 V4.6.0 版本的场景，并行控制默认使用的还是 `parallel_servers_target` 流程。如果要使用实时负载的并行控制能力，需要修改集群级配置项 `_enable_px_adaptive_dop` 为 `true`，示例如下：
