...
| sock_option_flag_out         | OBProxy 和 OBServer 之间 tcp 的参数，用二进制位表示：<ul><li>bit 0：二进制的第一位，为 1 表示启用 <code>no_delay</code>。</li><li>bit 1：二进制的第二位，为 1 表示启用 <code>keepalive</code>。</li><li>bit 2：二进制的第三位，为 1 表示启用 <code>linger_on</code>。</li></ul>不同软件版本下，该参数的默认值如下：<ul><li>对于 ODP V1.8.x 版本，默认值为 1，即 bit 0 为 1，表示启用 <code>no_delay</code>。 </li><li>对于 ODP V3.x 版本，默认值为 3，即 bit 0 和 bit 1 均为 1，表示同时启用 <code>no_delay</code> 和 <code>linger_on</code>。 </li></ul>| <ul><li>对于 ODP V1.8.x 版本，建议配置为 3。 </li><li>对于 ODP V3.x 版本，建议保持默认值。 </li></ul>|
