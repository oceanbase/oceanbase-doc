...
| QUERY_SQL               | longtext            | NO             | 实际的 SQL 语句。<main id="notice" type='explain'><h4>说明</h4><p>该字段从 V4.2.5 BP2 版本开始只支持 SYS 租户查看。    |
...
| TX_INTERNAL_ROUTING     | bigint(20)           | NO            | 表示当前事务是否启用了事务路由：<ul><li>0：未启用事务路由，语句只能路由到事务开始时的 OBServer 节点上。  </li><li>1：启用事务路由，事务内的语句可以由代理 Proxy 按照最优的方式路由。 </li></ul><main id="notice" type='explain'><h4>说明</h4><p>OBProxy 从 V4.1.0 版本开始支持该功能。</p></main>  |
| TX_STATE_VERSION        | bigint(20) unsigned  | NO            | 当事务路由开启后，执行语句后事务状态若发生变化，则版本号递增；若未发生变化，则版本号不变<main id="notice" type='explain'><h4>说明</h4><p>OBProxy 从 V4.1.0 版本开始支持该功能。</p></main>  |
| FLT_TRACE_ID | varchar(1024) | NO | 表示该条记录全链路追踪的 trace_id，如果为空，则说明没有被全链路追踪监控。该字段是Uuid，不同于 Trace。其表示形式类似于：`000600d6-a5de-038c-6c80-df07e4e79149` |
...
| STMT_TYPE                       | varchar(128)        | YES  |  DML 类型会按需返回：<ul><li>DDL 类型会返回 <code>DDL_STMT</code>。</li><li>DCL 类型会返回 <code>DCL_STMT</code>。</li><li>TCL 类型会返回 <code>TCL_STMT</code>。</li><li>其他则会返回 <code>OTHER_STMT</code>。</li></ul>    |
| TOTAL_MEMSTORE_READ_ROW_COUNT   | bigint(20)          | NO   | 整个工作过程中从 MemStore 读出的总行数（仅在展示 query_text 的线程中显示该变量）     |
| TOTAL_SSSTORE_READ_ROW_COUNT    | bigint(20)          | NO   | 整个工作过程中从 SSSTORE 读出的总行数（仅在展示 query_text 的线程中显示该变量）     |
| PROXY_USER                      | varchar(128)        | YES  |<ul><li>代理用户登录场景，返回代理用户的名称。</li><li>非代理用户登录场景，显示为空。</li></ul> |
| SEQ_NUM |bigint(20)| NO | 事务中的语句序号。</li></ul><main id="notice" type='explain'><h4>说明</h4><p>该字段从 V4.2.5 版本开始引入，字段默认为 NULL。</p></main>|
| PLSQL_COMPILE_TIME              | bigint(20)          | NO   |  PL 编译用时。 </li></ul><main id="notice" type='explain'><h4>说明</h4><p>该字段从 V4.2.5 BP2 版本开始引入，字段默认为 NULL。</p></main>   |
| USER_CLIENT_PORT                | bigint(20)          | NO   | 用于展示客户端端口号。</li></ul><main id="notice" type='explain'><h4>说明</h4><p>该字段从 V4.2.5 BP2 版本开始引入，字段默认为 NULL。</p></main>     |
| TRANS_STATUS                    | varchar(256)        | NO   | 用于展示显示开启事务或隐式开启事务或不开启事务。</li></ul><main id="notice" type='explain'><h4>说明</h4><p>该字段从 V4.2.5 BP2 版本开始引入，字段默认为 NULL。</p></main>  |
