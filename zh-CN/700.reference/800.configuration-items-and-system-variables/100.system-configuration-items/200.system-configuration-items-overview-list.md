...
| [ob_storage_s3_url_encode_type](300.cluster-level-configuration-items/16720.ob_storage_s3_url_encode_type.md)| 设置 S3 发送请求时的 URL 编码方式是否兼容 Rfc3986 标准。|
...
| [workers_per_cpu_quota](300.cluster-level-configuration-items/25700.workers_per_cpu_quota.md)                    | 用于设置分配给每个 CPU 配额的工作线程数量。                      |
...
| [balancer_timeout_check_interval](300.cluster-level-configuration-items/2300.balancer_timeout_check_interval.md) | 用于设置检查负载均衡等后台任务超时的时间间隔。 |
...
| [unit_gc_wait_time](300.cluster-level-configuration-items/24830.unit_gc_wait_time.md) | 用于指定 Unit GC 的最大等待时长，超过最大等待时长则强制 GC。|

### 副本相关

|                                             配置项名                                             |                  功能描述                  |
|----------------------------------------------------------------------------------------------|----------------------------------------|
| [enable_rereplication](300.cluster-level-configuration-items/8300.enable_rereplication.md)                          | 日志流副本自动迁移复制的总开关。            |
...
| [net_thread_count](300.cluster-level-configuration-items/15800.net_thread_count.md)            | 用于设置网络 I/O 线程数。                                    |
| [high_priority_net_thread_count](300.cluster-level-configuration-items/10500.high_priority_net_thread_count.md) | 用于设置高优先级网络线程数。 |
...
| [ob_esi_syslog_level](300.cluster-level-configuration-items/16400.ob_esi_syslog_level.md) | 用于设置 obesi 进程的当前日志记录级别。 |
...
| [obconfig_url](300.cluster-level-configuration-items/16800.obconfig_url.md) | 用于设置 OBConfig 服务的 URL 地址。 |
...
| [min_observer_version](300.cluster-level-configuration-items/15000.min_observer_version.md) | 用于设置本集群最低的 OBServer 节点应用程序版本号。  |
| [zone](300.cluster-level-configuration-items/25800.zone-cluster.md) | 用于设置节点所在的 Zone 的名字。  |

### 带宽相关

|                                  配置项名                                   |             功能描述              |
|-------------------------------------------------------------------------|-------------------------------|
| [enable_ob_ratelimit](300.cluster-level-configuration-items/7700.enable_ob_ratelimit.md)      | 用于指定是否在跨区域的 RPC 连接中启用速率限制。      |
...
| [sys_bkgd_net_percentage](300.cluster-level-configuration-items/22200.sys_bkgd_net_percentage.md) | 用于设置后台系统任务可占用网络带宽百分比。|
| [devname](300.cluster-level-configuration-items/5600.devname.md) | 用于查看服务进程绑定的网卡设备名。|
...
| [leak_mod_to_check](300.cluster-level-configuration-items/11600.leak_mod_to_check.md)                 | 用于指定需要检查内存泄漏的模块名称，属于内部调试功能。                                                    |
...
| [memory_reserved](300.cluster-level-configuration-items/14000.memory_reserved.md)                   | 用于设置系统底层模块预留的内存容量大小。  |
...
| [stack_size](300.cluster-level-configuration-items/21300.stack_size.md) | 用于设置程序函数调用栈的大小。 |
| [enable_asan_for_memory_context](300.cluster-level-configuration-items/6200.enable_asan_for_memory_context.md) | 用于在启用 `ob_asan` 时，指定是否使用 ObAsanAllocator 作为 MemoryContext 的分配器（默认为 ObAllocator）。 |
...
| [tablet_ls_cache_priority](300.cluster-level-configuration-items/23200.tablet_ls_cache_priority.md) | 用于设置 Tablet 映射缓存优先级。 |
| [user_block_cache_priority](300.cluster-level-configuration-items/25000.user_block_cache_priority.md)                | 用于设置数据块缓存在缓存系统中的优先级。                                          |
| [enable_perf_event](300.cluster-level-configuration-items/7900.enable_perf_event.md) |用于设置是否开启性能事件的信息收集功能。 |
| [recyclebin_object_expire_time](300.cluster-level-configuration-items/18000.recyclebin_object_expire_time.md) | 用于自动清理回收站中过期的 Schema 对象。   |
| [schema_history_expire_time](300.cluster-level-configuration-items/19600.schema_history_expire_time.md) | 用于设置需要保留的完整元数据历史数据的范围，也可用于设置延迟删除的租户的保留时间。 |
| [schema_history_recycle_interval](300.cluster-level-configuration-items/19700.schema_history_recycle_interval.md) | 设置系统内部执行 `schema` 多版本记录回收任务的时间间隔。|
...
| [enable_syslog_recycle](300.cluster-level-configuration-items/9200.enable_syslog_recycle.md)                | 用于设置是否打开记录 OBServer 节点启动前的旧日志的开关。与 `max_syslog_file_count` 配合使用，决定回收逻辑是否考虑旧的日志文件。    |
...
| [log_storage_warning_trigger_percentage](300.cluster-level-configuration-items/13050.log_storage_warning_trigger_percentage.md) | 用于设置触发日志盘故障的写入性能百分比阈值。 |
...
| 配置项名 | 功能描述 |
|---------|---------|
|[migration_disable_time](300.cluster-level-configuration-items/14900.migration_disable_time.md)| 用于控制节点因资源异常（如磁盘空间耗尽）导致数据迁入失败后，系统禁止新迁入操作的冷却时间。 |
...
| [ignore_replay_checksum_error](300.cluster-level-configuration-items/10600.ignore_replay_checksum_error.md)     | 用于控制是否在事务日志回放时忽略校验和错误。                         |
...
| [enable_syslog_recycle](300.cluster-level-configuration-items/9200.enable_syslog_recycle.md)                | 用于设置是否打开记录 OBServer 节点启动前的旧日志的开关。与 `max_syslog_file_count` 配合使用，决定回收逻辑是否考虑旧的日志文件。    |
| [enable_syslog_wf](300.cluster-level-configuration-items/9300.enable_syslog_wf.md)                     | 用于设置是否把 WARN 以上级别的系统日志打印到一个单独的日志文件中。                 |
| [max_string_print_length](300.cluster-level-configuration-items/13500.max_string_print_length.md)              | 用于设置打印系统日志时单行日志的最大长度。                                |
| [max_syslog_file_count](300.cluster-level-configuration-items/13600.max_syslog_file_count.md)                | 用于设置在回收日志文件之前可以容纳的日志文件数量。                            |
| [syslog_level](300.cluster-level-configuration-items/16400.ob_esi_syslog_level.md)                         | 用于设置系统日志级别。                                          |
| [trace_log_sampling_interval](300.cluster-level-configuration-items/24300.trace_log_sampling_interval.md)          | 用于设置定期打印跟踪日志信息的时间。                                   |
| [diag_syslog_per_error_limit](300.cluster-level-configuration-items/5700.diag_syslog_per_error_limit.md)          | 用于设置每个错误码每秒允许的 DIAG 系统日志数，超过该阈值后，日志将不再打印。                                   |
...
| [minor_freeze_times](300.cluster-level-configuration-items/15200.minor_freeze_times.md) | 用于设置多少次冻结触发一次合并。 |
...
| 配置项名 | 功能描述 |
|---------|---------|
|[arbitration_degradation_policy](300.cluster-level-configuration-items/320.arbitration_degradation_policy.md)      | 用于设置自动降级的策略。 |
...
| [sts_credential](400.tenant-level-configuration-items/8220.sts_credential.md) | 用于指定访问 OCP 提供的 STS 服务的凭证。|
...
| [query_response_time_range_base](400.tenant-level-configuration-items/7600.query_response_time_range_base.md) | 用于设置 `information_schema.QUERY_RESPONSE_TIME` 视图中统计查询响应时间的时间区间基数。 |
...
|  配置项名  | 功能描述  |
|-----------|----------|
| [replica_parallel_migration_mode](400.tenant-level-configuration-items/7820.replica_parallel_migration_mode.md) | 用于指定日志流副本并行迁移功能的模式。|

### 扩缩容相关

|  配置项名  | 功能描述  |
|-----------|----------|
| [ls_scale_out_factor](400.tenant-level-configuration-items/5530.ls_scale_out_factor.md)                        | 日志流的扩容系数，用于控制租户在每台服务节点上日志流的个数。|
...
|                                   配置项名                                    |                功能描述                 |
|---------------------------------------------------------------------------|-------------------------------------|
| [px_workers_per_cpu_quota](400.tenant-level-configuration-items/7320.px_workers_per_cpu_quota.md) | 用于设置并行执行工作线程数的比例。            |
| [ddl_thread_score](400.tenant-level-configuration-items/1950.ddl_thread_score.md) | 用于设置 DDL 任务使用的线程数。 |
| [compaction_low_thread_score](400.tenant-level-configuration-items/1200.compaction_low_thread_score.md)   | 用于设置低优先级 compaction 的工作线程数。 |
| [compaction_high_thread_score](400.tenant-level-configuration-items/1100.compaction_high_thread_score.md)    | 用于设置高优先级 compaction 的工作线程数。  |
| [compaction_mid_thread_score](400.tenant-level-configuration-items/1300.compaction_mid_thread_score.md)   | 用于设置中优先级 compaction 的工作线程数。 |
...
|   配置项名    |  功能描述   |
|--------------|-------------|
| [rpc_memory_limit_percentage](400.tenant-level-configuration-items/7900.rpc_memory_limit_percentage.md)                   |用于设置租户中 RPC 的最大内存占租户总内存的百分比。|
...
| [query_memory_limit_percentage](400.tenant-level-configuration-items/7490.query_memory_limit_percentage.md)| 用于指定单条 SQL 可使用的租户内存百分比。 |

### 磁盘空间相关

|   配置项名  |   功能描述  |
|------------|-------------|
| [temporary_file_max_disk_size](400.tenant-level-configuration-items/8330.temporary_file_max_disk_size.md)   | 用于设置租户的单个节点内临时文件最大可占用的磁盘空间大小。|
...
| [kv_group_commit_batch_size](400.tenant-level-configuration-items/3960.kv_group_commit_batch_size.md) | 用于指定 OBKV 组提交中执行 batch 操作的大小 |
| [kv_group_commit_rw_mode](400.tenant-level-configuration-items/3970.kv_group_commit_rw_mode.md) | 用于在控制走组提交的场景。 |
...
| [plsql_v2_compatibility](400.tenant-level-configuration-items/7300.plsql_v2_compatibility.md) | 用于控制存储过程 DDL 阶段编译落盘功能是否开启。|

### 其他

|   配置项名  |   功能描述  |
|------------|-------------|
| [zone_deploy_mode](400.tenant-level-configuration-items/13000.zone_deploy_mode.md) | 用于设置租户在各 Zone 的分布模式。 |
...
| [default_enable_extended_rowid](400.tenant-level-configuration-items/2100.default_enable_extended_rowid.md)     | 用于设置是否将表创建为 Extended ROWID 模式。                                                                                 |
...
| [enable_user_defined_rewrite_rules](400.tenant-level-configuration-items/3200.enable_user_defined_rewrite_rules.md)     | 用于控制是否开启改写绑定功能。                                                                                 |
|[enable_ps_parameterize](400.tenant-level-configuration-items/3250.enable_ps_parameterize.md)|用于控制开启或关闭 PS 语句在解析时是否参数化。|
...
| [writing_throttling_trigger_percentage](400.tenant-level-configuration-items/9000.writing_throttling_trigger_percentage.md) | 用于设置触发写入限速的 MemStore 已使用内存占 MemStore 内存上限的百分比阈值。  |
...
| [major_compact_trigger](400.tenant-level-configuration-items/5600.major_compact_trigger.md)         | 用于设置多少次冻结触发一次全局合并。  |
| [minor_compact_trigger](400.tenant-level-configuration-items/6100.minor_compact_trigger.md)         | 用于配置触发转储（Minor Compaction）的 SSTable 个数的阈值。 |
