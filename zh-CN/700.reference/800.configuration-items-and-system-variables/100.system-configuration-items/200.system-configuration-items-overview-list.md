...
|                                          配置项名                                           |                     功能描述                      |
|-----------------------------------------------------------------------------------------|-----------------------------------------------|
| [cpu_count](300.cluster-level-configuration-items/3700.cpu_count.md)                                | 用于设置系统 CPU 总数。如果设置为 0，系统将自动检测 CPU 数量。         |
...
| [memory_reserved](300.cluster-level-configuration-items/13700.memory_reserved.md)                   | 用于设置系统底层模块预留的内存容量大小。   |
...
|                                        配置项名                                        |                                                        功能描述                                                         |
|------------------------------------------------------------------------------------|---------------------------------------------------------------------------------------------------------------------|
| [schema_history_archive_expire_time](400.tenant-level-configuration-items/7410.schema_history_archive_expire_time.md)  | 用于指定 Schema 多版本历史归档数据的保留时间。 |
| [schema_history_expire_time](400.tenant-level-configuration-items/7420.schema_history_expire_time.md)  | 用于设置元数据历史数据过期时间。   |
...
| [ob_enable_utl_tcp](300.cluster-level-configuration-items/16631.ob_enable_utl_tcp.md)|用来控制 UTL_TCP 系统包功能是否开启。|
...
|[async_ps_cursor_max_count](300.cluster-level-configuration-items/650.async_ps_cursor_max_count.md)|用于控制单节点中异步 PS cursor 的最大个数。|

### 日志服务相关

|       配置项名         |             功能描述        |
|-----------------------|-----------------------------|
| [enable_logservice](300.cluster-level-configuration-items/6950.enable_logservice.md)   |  用于控制日志服务模式的开关。    |
| [logservice_access_point](300.cluster-level-configuration-items/12750.logservice_access_point.md)   | 用于持久化日志服务访问点。     |
...
| 配置项名 | 功能描述 |
|---------|----------|
| [external_table_disk_cache_max_percentage](300.cluster-level-configuration-items/9350.external_table_disk_cache_max_percentage.md) | 用于设置 SN 模式下外表磁盘缓存空间占 [datafile_size](300.cluster-level-configuration-items/4700.datafile_size.md) 的最大比例。|
...
| [default_table_merge_engine](400.tenant-level-configuration-items/2130.default_table_merge_engine.md) | 用于指定用户租户默认创建表的 `merge_engine`。|
| [default_delta_format](400.tenant-level-configuration-items/1950.default_delta_format.md) | 用于设置创建表时默认的增量数据存储格式。|
...
|  配置项名  | 功能描述  |
|-----------|----------|
| [zone_disk_balance_tolerance_percentage](400.tenant-level-configuration-items/13001.zone_disk_balance_tolerance_percentage.md) | 用于控制 Zone 间表组磁盘均衡算法对不均衡的容忍程度。  |
| [enable_ls_leader_balance](400.tenant-level-configuration-items/2760.enable_ls_leader_balance.md)                         | 用于控制是否开启日志流 Leader 的自动均衡。 |
| [enable_gts_standalone](400.tenant-level-configuration-items/2550.enable_gts_standalone.md)                               | 用于设置是否开启租户 GTS 服务独占一组 Unit 的功能。   |
...
| [enable_database_sharding_none](400.tenant-level-configuration-items/2450.enable_database_sharding_none.md) | 用于控制创建 DataBase 时用户表是否开启自动聚合的开关。|
...
|                                             配置项名                                          |                  功能描述              |
|----------------------------------------------------------------------------------------------|----------------------------------------|
| [ls_scale_out_factor](400.tenant-level-configuration-items/5160.ls_scale_out_factor.md) | 日志流的扩容系数，用于控制租户在每台服务节点上日志流的个数。 |
...
|                                      配置项名                                    |            功能描述               |
|---------------------------------------------------------------------------------|-----------------------------------|
| [enable_auto_split](400.tenant-level-configuration-items/2350.enable_auto_split.md) | 用于控制租户是否开启自动分区功能。|
| [auto_split_tablet_size](400.tenant-level-configuration-items/450.auto_split_tablet_size.md) | 用于控制这个租户开启自动分区功能之后触发分裂的阈值。|
...
|                                   配置项名                                    |                功能描述                 |
|---------------------------------------------------------------------------|-------------------------------------|
| [px_workers_per_cpu_quota](400.tenant-level-configuration-items/6890.px_workers_per_cpu_quota.md) | 用于设置并行执行工作线程数的比例。            |
| [attach_shared_sstable_thread_score](400.tenant-level-configuration-items/230.attach_shared_sstable_thread_score.md) | 用于配置租户可用来执行共享存储模式下计算节点共享数据引用的线程数。 |
| [inc_sstable_upload_thread_score](400.tenant-level-configuration-items/3650.inc_sstable_upload_thread_score.md) | 用于配置租户可用来执行共享存储模式下数据共享类任务的线程数。 |
...
| [ddl_high_thread_score](400.tenant-level-configuration-items/2250.ddl_high_thread_score.md)|用于修改执行 ddl kv merge 的 DAG 线程数。|

### 缓存相关

|                               配置项名                            |        功能描述      |
|-------------------------------------------------------------------|---------------------|
| [storage_cache_clean_macro_interval](400.tenant-level-configuration-items/7750.storage_cache_clean_macro_interval.md) | 用于控制共享存储（Shared-Storage，SS）模式下宏块级别的热点宏块缓存清理任务的执行频率。 |
...
|   配置项名  |   功能描述  |
|------------|-------------|
| [temporary_file_max_disk_size](400.tenant-level-configuration-items/8030.temporary_file_max_disk_size.md)   | 设置租户的单个节点内临时文件最大可占用的磁盘空间大小。|
...
| [ob_enable_java_udf](400.tenant-level-configuration-items/5860.ob_enable_java_udf.md)|用于开启或关闭 Java UDF 功能。|
|[enable_pl_rich_error_msg](400.tenant-level-configuration-items/2540.enable_pl_rich_error_msg.md)|用于开启或关闭向 PLSQL 内异常信息添加额外内容的功能。|
...
| [load_data_diagnosis_log_compression](400.tenant-level-configuration-items/4220.load_data_diagnosis_log_compression.md) | 用于设置诊断日志使用的压缩算法。|
| [load_data_diagnosis_log_max_size](400.tenant-level-configuration-items/4230.load_data_diagnosis_log_max_size.md) | 用于设置单个诊断日志文件的大小。|

### PX 相关

|   配置项名  |   功能描述  |
|------------|-------------|
| [px_target_high_watermark](400.tenant-level-configuration-items/6860.px_target_high_watermark.md) | 用于控制并行线程数的高水位线百分比阈值。|
| [px_target_low_watermark](400.tenant-level-configuration-items/6870.px_target_low_watermark.md) | 用于设置并行线程数的低水位线百分比阈值。|
| [px_target_workers_per_cpu](400.tenant-level-configuration-items/6880.px_target_workers_per_cpu.md) | 用于控制预期的并行执行线程数和租户 CPU 数的比率。|

### 其他

|   配置项名  |   功能描述  |
|------------|-------------|
| [zone_deploy_mode](400.tenant-level-configuration-items/13000.zone_deploy_mode.md)                     |  设置租户在各 Zone 的分布模式。   |
...
| [default_skip_index_level](400.tenant-level-configuration-items/2110.default_skip_index_level.md) | 用于设置租户中创建表时 `skip_index_level` 的默认值。|
| [sql_func_extension_mode](400.tenant-level-configuration-items/7470.sql_func_extension_mode.md) | 用于控制是否启用命名风格与 MySQL 不一致的主流兼容模式函数。|
...
| [writing_throttling_trigger_percentage](400.tenant-level-configuration-items/8500.writing_throttling_trigger_percentage.md) | 用于设置触发写入限速的 MemStore 已使用内存占 MemStore 内存上限的百分比阈值。  |
...
| 配置项名 | 功能描述 |
|---------|----------|
| [external_table_io_hole_size](400.tenant-level-configuration-items/3230.external_table_io_hole_size.md) | 用于设置两个连续 IO 范围之间的最大距离（以字节为单位）。|
| [external_table_io_range_size](400.tenant-level-configuration-items/3250.external_table_io_range_size.md) | 用于设置合并范围的最大大小（以字节为单位）。|
...
| 配置项名 | 功能描述 |
|---------|---------|
| [enable_mlog_auto_maintenance](400.tenant-level-configuration-items/2670.enable_mlog_auto_maintenance.md) | 用于控制是否启用物化视图日志自动管理功能。|
| [mlog_trim_interval](400.tenant-level-configuration-items/5650.mlog_trim_interval.md) | 用于控制物化视图日志（mlog）后台自动裁剪任务的调度周期。|
| [enable_mv_binlog_minimal_mode](400.tenant-level-configuration-items/2750.enable_mv_binlog_minimal_mode.md) | 用于控制物化视图增量刷新场景中 `binlog minimal` 模式的开关。|
| [mv_refresh_concurrency](400.tenant-level-configuration-items/5670.mv_refresh_concurrency.md) | 用于控制同一租户内可同时运行的物化视图刷新任务的数量，即物化视图刷新任务的最大并发数。|
...
|                                     配置项名                                     |         功能描述         |
|------------------------------------------------------------------------------|----------------------|
| [enable_window_compaction](400.tenant-level-configuration-items/3150.enable_window_compaction.md) | 用于指定当前租户是否使用窗口合并。 |
| [mds_minor_compact_trigger](400.tenant-level-configuration-items/5460.mds_minor_compact_trigger.md) | 用于控制触发 MDS Minor Compaction 的 SSTable 数量。 |
| [mds_compaction_mid_thread_score](400.tenant-level-configuration-items/5450.mds_compaction_mid_thread_score.md) | 用于设置中优先级 MDS Compaction 的 DAG（Direct Acyclic Graph）线程数。 |
| [mds_compaction_high_thread_score](400.tenant-level-configuration-items/5440.mds_compaction_high_thread_score.md) | 用于设置高优先级 MDS Compaction 的 DAG（Direct Acyclic Graph）线程数。 |
...
| 配置项名 | 功能描述 |
|---------|---------|
| [enable_ttl](400.tenant-level-configuration-items/3020.enable_ttl.md) | 用于控制 SQL 模式下是否开启 TTL 功能。|
| [ttl_duty_time](400.tenant-level-configuration-items/8095.ttl_duty_time.md) | 用于指定 TTL 后台定期触发 TTL 任务的执行时间点。|
