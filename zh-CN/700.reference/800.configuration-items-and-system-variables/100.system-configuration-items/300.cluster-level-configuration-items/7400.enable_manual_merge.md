...
`enable_manual_merge` 为 `True` 时，执行 `alter system major freeze` 命令并不会触发真正的合并，而需要手动对各个 Zone 进行逐个合并。
例如，通过以下命令对 `zone1` 和 `zone2` 进行逐个合并。
...
```shell
obclient> ALTER SYSTEM START MERGE ZONE ='zone2'; 
```
...
```shell
obclient> ALTER SYSTEM SET enable_manual_merge= 1;
```
