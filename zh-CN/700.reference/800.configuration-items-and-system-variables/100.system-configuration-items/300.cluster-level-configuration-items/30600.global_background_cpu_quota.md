...
`global_background_cpu_quota` 用于控制在开启全局前后台资源隔离时后台任务的 CPU 配额。仅当 `enable_global_background_resource_isolation` 取值为 True 时该配置项才有效。
...
| -------- | -------- |
| 参数类型   | INT |
| 默认值     | -1，表示使用的 CPU 配额不受 Cgroup 限制。 |
| 取值范围   |[-1, +∞)|
...
```shell
obclient> ALTER SYSTEM SET global_background_cpu_quota=;
```
