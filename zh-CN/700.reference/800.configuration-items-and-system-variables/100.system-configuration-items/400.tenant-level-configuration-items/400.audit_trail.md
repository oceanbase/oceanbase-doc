...
| 参数类型           | String     |
| 默认值             | NONE        |
| 取值范围           | <ul><li>NONE：关闭审计。</li><li>OS：审计记录写本地文件。</li><li>LOG：审计记录写入审计日志，支持日志归档、压缩、轮转、清理及加密等管理策略。</li><li>DB：审计记录写内部表。</li><li>DB,EXTENDED：审计记录写内部表且记录包含执行的 SQL 语句。</li><li>DB_EXTENDED：审计记录写内部表且记录包含执行的 SQL 语句。</li></ul><main id="notice" type='explain'><h4>说明</h4><p><code>LOG</code> 值从 V4.4.2 BP2 版本开始引入。</p></main>   |
