...
* 用户租户下用于控制是否进行租户内均衡。
  ...
  * false：租户内不再进行负载均衡操作，已经在进行中的均衡操作会取消。若此时发起扩缩容操作将会报错，例如进行以下操作时：
    ...
    * Primary Zone 为 RANDOM 的情况下，F 副本个数发生变化。
    ...
    <h4>注意</h4>
    <P>对于 V4.2.5 版本：</P>
    <ul><li>从 V4.2.5 BP5 版本开始，关闭配置项 <code>enable_rebalance</code>（值为 <code> false</code>）时，增大或缩小租户的 Unit Number 不会再报错。</li>
    <li>从 V4.2.5 BP1 版本开始，关闭 <code>enable_rebalance</code>（值为 <code> false</code>）时，第一优先级的 Primary Zone 发生变化或者 Primary Zone 为 RANDOM 的情况下，F 副本个数发生变化，系统均不会再报错。</li>
