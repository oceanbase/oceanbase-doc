...
| 参数类型             | Int         |
| 默认值               | 0，即关闭向量索引功能       |
| 取值范围             | [0, 100)。实际最大值在 85 ～ Memstore（即 `memstore_limit_percentage` 值）之间。当 `memstore_limit_percentage` 参数设置为 `0`（默认值）时，系统会自动计算 MemStore 内存占比。自动计算规则：<ul><li>如果租户内存 > 8GB：MemStore 占用 50% 的租户内存。</li><li>如果租户内存 ≤ 8GB：MemStore 占用 40% 的租户内存。</li></ul> |
