...
|--------------------|--------------|
| 参数类型             | Int         |
| 默认值               | 0，单位为百分比 <main id="notice" type='notice'><h4>说明</h4><p>V4.4.1 之前的版本，该配置项默认值含义为不为向量索引分配内存，即关闭向量检索功能。<br>从 V4.4.1 版本开始，该配置项默认值含义变更为自适应模式，即系统自动调整租户中的向量索引数据的内存占用比例：<ul><li>租户实际内存为 8GB 及以下时，该值自适应为 40。</li><li>租户实际内存为 8GB 以上时，该值自适应为 50。</li></ul></p></main>      |
