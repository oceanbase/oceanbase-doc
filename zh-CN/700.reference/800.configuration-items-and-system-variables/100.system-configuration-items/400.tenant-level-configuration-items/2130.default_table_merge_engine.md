...
| 参数类型  | String |
| 默认值    | `partial_update` |
| 取值范围   | <ul><li>`partial_update`：表示部分更新模式，每次更新仅记录修改的列。</li><li>`delete_insert`：表示全列更新模式，每次更新写入完整行。</li><li>`append_only`：表示仅追加写模式。</li></ul> <main id="notice" type='explain'><h4>说明</h4><p>对于 V4.6.x 版本，<code>default_table_merge_engine</code> 从 V4.6.0 版本开始增加取值 <code>append_only</code>。</p></main>|
...
* `partial_update`：若建表语句中没有指定 `merge_engine`，用户租户建表的 `merge_engine = partial_update`，该模式存储更省，适用于 OLTP 场景。
* `delete_insert`：若建表语句中没有指定 `merge_engine`，用户租户建表的 `merge_engine = delete_insert`，该模式对查询更优，适用于 OLAP 场景。
* `append_only`：若建表语句中没有指定 `merge_engine`，用户租户建表的 `merge_engine = append_only`，该模式下仅写入数据，不允许修改数据，适用于 IoT、监控、金融交易等场景，该模式下如果要做过期数据清理可以使用 Compaction TTL 功能。
