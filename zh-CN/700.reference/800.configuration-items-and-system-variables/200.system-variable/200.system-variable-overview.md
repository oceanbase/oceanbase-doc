...
| master_info_repository                                  | 指定副本是将元数据记录到系统数据库 MySQL 的一个 InnoDB 表，还是作为数据目录中的文件。其默认值为 `FILE`。  <main id="notice" type='explain'><h4>说明</h4><p>该变量仅用于兼容 MySQL 5.7，相关功能并不支持，可以查询及设置，设置行为不报错但不生效。</p></main>  |
...
| debug          | 用于设置调试选项，其默认值为 `d:t:i:o,/tmp/mysqld.trace`。 <main id="notice" type='explain'><h4>说明</h4><p>该变量仅用于兼容 MySQL 5.7，相关功能并不支持，可以查询及设置，设置行为不报错但不生效。</p></main>                         |
| debug_sync          | 用于在 Debug 时设置同步点，其默认值为 `空字符串`。 <main id="notice" type='explain'><h4>说明</h4><p>该变量仅用于兼容 MySQL 5.7，相关功能并不支持，可以查询及设置，设置行为不报错但不生效。</p></main>                          |
| [ob_global_debug_sync](400.session-system-variable/540.ob_global_debug_sync-session.md) | 用于 Debug 时设置同步点，与 `debug_sync` 不同，设置此变量会影响所有 session。 |
...
|--------------|----------------|
| [last_insert_id](400.session-system-variable/200.last_insert_id-session.md)   | 用于返回本 session 最后一次插入的自增字段值。   |
| [identity](400.session-system-variable/95.identity-session.md)    | `identity` 和变量 `last_insert_id` 是同义词，可以通过 `select @@identity` 查询。 |
