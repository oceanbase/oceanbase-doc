...
| [ob_compatibility_version](300.global-system-variable/17700.ob_compatibility_version-global.md) | 用于控制产品行为发生变更的功能兼容的 OceanBase 数据库版本。在创建租户时指定兼容版本，且在租户创建后无法修改。|
...
|   变量名   |     功能描述 |
|----------------|---------|
| [debug](300.global-system-variable/2150.debug-global.md)           | 用于设置调试选项。                         |
...
| [ob_default_replica_num](300.global-system-variable/6200.ob_default_replica_num-global.md) | 用于设置每个集群中表的默认副本数。 |
...
|          变量名           |             功能描述             |
|-------------------------------|------------------------------|
| [ob_enable_pl_cache](300.global-system-variable/9560.ob_enable_pl_cache.md)                | 用于控制 PL cache 模块是否开启。        |
...
| [stored_program_cache](300.global-system-variable/19200.stored_program_cache-global.md)          | 用于设置存储过程和函数的缓存数量。 |
...
| 变量名 |  功能描述 |
|-------|-----------|
| [innodb_change_buffering_debug](300.global-system-variable/3010.innodb_change_buffering_debug-global.md)   | 用于设置更改 InnoDB 缓冲区的调试级别。     |
| [innodb_compress_debug](300.global-system-variable/3020.innodb_compress_debug-global.md)   | 用于调整和控制压缩行为的调试级别。 |
| [innodb_disable_resize_buffer_pool_debug](300.global-system-variable/3030.innodb_disable_resize_buffer_pool_debug-global.md)   | 用于是否开启 InnoDB 缓冲池的大小调整。 |
| [innodb_fil_make_page_dirty_debug](300.global-system-variable/3040.innodb_fil_make_page_dirty_debug-global.md)   | 用于在调试 InnoDB 中弄脏指定的页面数。 |
| [innodb_limit_optimistic_insert_debug](300.global-system-variable/3050.innodb_limit_optimistic_insert_debug-global.md)   | 用于设置每个 B-tree 页面的记录数。 |
| [innodb_merge_threshold_set_all_debug](300.global-system-variable/3060.innodb_merge_threshold_set_all_debug-global.md)   | 用于设置索引页面的页满百分比。 |
| [innodb_saved_page_number_debug](300.global-system-variable/3070.innodb_saved_page_number_debug-global.md)   | 用于设置在调试 InnoDB 中保存的页码数。 |
...
| [innodb_trx_purge_view_update_only_debug](300.global-system-variable/3140.innodb_trx_purge_view_update_only_debug-global.md)   | 用于控制事务清理过程中仅更新清理视图的调试级别。 |
| [innodb_trx_rseg_n_slots_debug](300.global-system-variable/3150.innodb_trx_rseg_n_slots_debug-global.md)   | 用于指定事务回滚段（transaction rollback segment）中的槽数量。每个槽用于存储事务的回滚信息。 |
...
| [server_uuid](300.global-system-variable/11550.server_uuid-global.md)               | 用于查看 OBServer 节点 UUID。              |
...
|  变量名 | 功能描述 |
|--------|---------|
| [runtime_filter_type](300.global-system-variable/11460 .runtime_filter_type-global.md) | 用于设置租户级别的 Runtime Filter 类型。 |
| [runtime_filter_wait_time_ms](300.global-system-variable/11470.runtime_filter_wait_time_ms-gloabl.md) | 用于设置 Runtime Filter 的最大等待时间。 |
| [runtime_filter_max_in_num](300.global-system-variable/11450.runtime_filter_max_in_num-global.md) | 用于设置 Runtime In Filter 的 NDV （某个列中不同值的个数）个数。 |
| [runtime_bloom_filter_max_size](300.global-system-variable/11440.runtime_bloom_filter_max_size-global.md) | 用于设置 Runtime Bloom Filter 的最大使用内存，单位为字节。|

### 时区和日期相关

|    变量名        |     功能描述         |
|-----------------|----------------------------------------|
| [lc_time_names](400.session-system-variable/3400.lc_time_names-global.md)             |   用于控制显示日期和月份名称和缩写的语言。            |
...
| [profiling](300.global-system-variable/10850.profiling-global.md) | 用于启用或禁用语句分析功能。      |
| [profiling_history_size](300.global-system-variable/10860.profiling_history_size-global.md) | 用于设置保存查询性能信息的历史记录数量。      |
| [have_profiling](300.global-system-variable/2900.have_profiling-global.md) | 用于检测是否支持查询性能分析。      |
| [innodb_strict_mode](300.global-system-variable/3100.innodb_strict_mode-global.md)  | 用于设置特定语法的 SQL 检查模式。  |
...
| [ob_enable_rich_error_msg](300.global-system-variable/9550.ob_enable_rich_error_msg-global.md) | 用于控制 SQL 执行出错时是否显示 svr_ip、time、trace_id 等信息。    |
...
| [innodb_stats_persistent](300.global-system-variable/17500.innodb_stats_persistent-global.md)| 用于控制是否将 InnoDB 索引统计信息持久化到磁盘上。|
...
| 变量名 | 功能描述 |
|-------|---------|
| [block_encryption_mode](400.session-system-variable/800.block_encryption_mode-session.md)           | 用于指定在函数 aes_encrypt 和 aes_decrypt 中使用的加密算法。   |

### 版本信息相关

|         变量名          |                   功能描述                    |
|-----------------------------|-------------------------------------------|
| [ob_enable_transmission_checksum](400.session-system-variable/7600.ob_enable_transmission_checksum-session.md) | 用于设置 obproxy 和 OBServer 节点之间是否进行数据包校验和，设置后实时生效。 |
...
|   变量名   |     功能描述 |
|----------------|---------|
| [debug](400.session-system-variable/2200.debug-session.md)           | 用于设置调试选项。                         |
| [debug_sync](400.session-system-variable/2300.debug_sync-session.md)           | 用于在 Debug 时设置同步点，与 MySQL 兼容。                          |
...
|    变量名     |                      功能描述                       |
|-------------------|-------------------------------------------------|
| [cte_max_recursion_depth](400.session-system-variable/1950.cte_max_recursion_depth-session.md)  | 用于设置公共表表达式（CTE）的最大递归深度。服务器会终止执行任何超过此 CTE 值的操作。 |
...
| [net_buffer_length](400.session-system-variable/4100.net_buffer_length-session.md)       | 用于设置 OBServer 节点能接受的 SQL Query 的大小。               |
| [net_read_timeout](400.session-system-variable/4200.net_read_timeout-session.md)          | 用于中断读之前等待连接的其它数据的秒数。     |
| [net_write_timeout](400.session-system-variable/4300.net_write_timeout-session.md)         | 用于设置中断写之前等待块写入连接的秒数。     |
| [ob_enable_aggregation_pushdown](400.session-system-variable/6700.ob_enable_aggregation_pushdown-session.md)  | 用于设置是否允许聚合操作下压。 |
| [ob_enable_index_direct_select](400.session-system-variable/7000.ob_enable_index_direct_select-session.md)   | 用于设置是否允许用户直接查询索引表。|
| [ob_enable_jit](400.session-system-variable/7100.ob_enable_jit-session.md)                   | 用于设置 JIT 执行引擎模式。              |
| [ob_query_timeout](400.session-system-variable/9000.ob_query_timeout-session.md)        | 用于设置查询超时时间。 |
| [ob_read_consistency](400.session-system-variable/9100.ob_read_consistency-session.md) | 用于设置读一致性级别。    |
...
|  变量名  | 功能描述 |
|---------|----------|
| [ob_create_table_strict_mode](400.session-system-variable/6300.ob_create_table_strict_mode-session.md)   | 用于设置建表的严格模式。 |
| [ob_default_replica_num](400.session-system-variable/6450.ob_default_replica_num-session.md) | 用于设置每个集群中表的默认副本数。 |
| [recyclebin](400.session-system-variable/11300.recyclebin-session.md) | 用于设置是否开启回收站功能。 |

### 回收站相关

|     变量名     |      功能描述      |
|----------|----------------|
| [ob_enable_truncate_flashback](400.session-system-variable/7700.ob_enable_truncate_flashback-session.md)    | 用于设置是否启用表截断的闪回。 |

### 缓存相关

|          变量名           |             功能描述             |
|-------------------------------|------------------------------|
| [ob_enable_plan_cache](400.session-system-variable/7150.ob_enable_pl_cache-session.md)                | 用于控制 PL cache 模块是否开启。        |
| [ob_enable_plan_cache](400.session-system-variable/7200.ob_enable_plan_cache-session.md)                | 用于设置是否打开 Plan Cache。         |
| [ob_bnl_join_cache_size](400.session-system-variable/6100.ob_bnl_join_cache_size-session.md)          | 用于设置 Batch Nest Loop Join 一次 Cache 多少数据做一次 Batch。 |

### 函数相关

|    变量名     |   功能描述     |
|--------------|----------------|
| [group_concat_max_len](400.session-system-variable/2800.group_concat_max_len-session.md)    | 用于设置允许的 `GROUP_CONCAT()` 函数结果的最大长度。         |
| [interactive_timeout](400.session-system-variable/3200.interactive_timeout-session.md)       | 用于设置服务器关闭交互式连接前等待活动的秒数。  |
| [wait_timeout](400.session-system-variable/13700.wait_timeout-session.md)              | 用于设置服务器关闭非交互连接之前等待活动的秒数。 |

### InnoDB 存储引擎相关

| 变量名 |  功能描述 |
|-------|-----------|
| [innodb_strict_mode](400.session-system-variable/3100.innodb_strict_mode-session.md)   | 用于设置特定语法的 SQL 检查模式。     |

### 集群相关

| 变量名 |  功能描述 |
|-------|-----------|
| [default_storage_engine](400.session-system-variable/2150.default_storage_engine-session.md)               | 用于查看 OBServer 节点的默认存储引擎。  |
...
| [ob_compatibility_mode](400.session-system-variable/6400.ob_compatibility_mode-session.md)   | 用于指定租户的兼容模式。     |
...
| [ob_route_policy](400.session-system-variable/9300.ob_route_policy-session.md)  | 用于设置 obproxy 或 Java 客户端与 OBServer 节点内部重试的路由策略。    |

### 目录路径相关

| 变量名 |        功能描述         |
|------------|---------------------|
| [ob_enable_hash_group_by](400.session-system-variable/6600.ob_enable_hash_group_by-session.md)  | 用于设置是否打开 Hash Group by 的路径。 |

### 内存相关

|          变量名          |               功能描述               |
|------------------------------|----------------------------------|
| [ob_temp_tablespace_size_percentage](400.session-system-variable/9800.ob_temp_tablespace_size_percentage-session.md) | 用于租户磁盘中临时表空间大小的百分比限制。            |

### 系统日志相关

|   变量名   |                 功能描述                  |
|----------------|---------------------------------------|
| [binlog_row_image](400.session-system-variable/700.binlog_row_image-session.md)     | 用于控制是否记录全列日志。                         |
| [ob_enable_show_trace](400.session-system-variable/7400.ob_enable_show_trace-session.md)  | 用于设置是否使用 Trace 日志。                    |
| [tracefile_identifier](400.session-system-variable/12900.tracefile_identifier-session.md) | 用于在日志中增加指定内容，便于筛选行迭代跟踪日志。             |
| [ob_log_level](400.session-system-variable/8000.ob_log_level-session.md)         | 用于设置 session 级的日志级别，如果不指定则使用系统中的日志级别。 |

### Runtime Filter 相关

|  变量名 | 功能描述 |
|--------|---------|
| [runtime_bloom_filter_max_size](400.session-system-variable/11350.runtime_bloom_filter_max_size-session.md) | 用于设置 Runtime Bloom Filter 的最大使用内存，单位为字节。|
| [runtime_filter_max_in_num](400.session-system-variable/11400.runtime_filter_max_in_num-session.md) | 用于设置 Runtime In Filter 的 NDV （某个列中不同值的个数）个数。 |
| [runtime_filter_type](400.session-system-variable/114500.runtime_filter_type-session.md) | 用于设置租户级别的 Runtime Filter 类型。 |
| [runtime_filter_wait_time_ms](400.session-system-variable/11500.runtime_filter_wait_time_ms-gloabl.md) | 用于设置 Runtime Filter 的最大等待时间。 |

### 时区和日期相关

|    变量名        |     功能描述         |
|-----------------|----------------------------------------|
| [lc_time_names](400.session-system-variable/34500.lc_time_names-session.md)             |   用于控制显示日期和月份名称和缩写的语言。            |
| [timestamp](400.session-system-variable/12800.timestamp-session.md)             | `TIMESTAMP` 是时间戳，单位为秒，该变量影响 `select now(6)`的结果。               |
| [nls_calendar](400.session-system-variable/4400.nls_calendar-session.md)          | 用于控制数据库使用的日历系统。            |
...
| [error_on_overlap_time](400.session-system-variable/2500.error_on_overlap_time-session.md) | 用于设置夏令时重叠区时间发生歧义时是否作报错处理。              |
| [nls_territory](400.session-system-variable/5800.nls_territory-session.md)         | 表示数据库国际化支持中的当前地域，与 `nls_language` 结合使用。   |
| [time_zone](400.session-system-variable/12700.time_zone-session.md)             | 用于设置当前租户会话使用的时区，支持 `'+08:00'` 的 Offset 形式和 `Asia/Shanghai` 的地域形式取值。         |
...
|    变量名        |       功能描述        |
|----------------|-------------------------|
| [autocommit](400.session-system-variable/600.autocommit-session.md)                | 用于设置是否自动提交事务。   |
| [ob_proxy_set_trx_executed](400.session-system-variable/8900.ob_proxy_set_trx_executed-session.md) | 用于设置用户是否通过 obproxy 发送 `START TRANSACTION Syntax` 给 OBServer。 |
| [ob_trace_info](400.session-system-variable/10000.ob_trace_info-session.md)        | 用于透传 Trace 信息，该信息最终可透持久化到 REDO 日志中。   |
| [nls_date_language](400.session-system-variable/4900.nls_date_language-session.md)     | 表示数据库国际化支持中的日期默认语言。        |
| [ob_trx_idle_timeout](400.session-system-variable/10100.ob_trx_idle_timeout-session.md)       | 用于设置事务空闲超时时间，即事务中两条语句之间的执行间隔超过该值时超时。                   |
| [ob_trx_lock_timeout](400.session-system-variable/10200.ob_trx_lock_timeout-session.md)       | 用于设置事务的等锁超时时长。           |
| [ob_trx_timeout](400.session-system-variable/10300.ob_trx_timeout-session.md)  | 用于设置事务超时时间。|
| [transaction_isolation](400.session-system-variable/13000.transaction_isolation-session.md)     | 用于设置事务的隔离级别。    |
| [transaction_read_only](400.session-system-variable/13100.transaction_read_only-session.md)     | 用于控制是否只允许开启只读事务。|
| [tx_isolation](400.session-system-variable/13200.tx_isolation-session.md)              | 用于设置事务隔离级别。     |
| [tx_read_only](400.session-system-variable/13300.tx_read_only-session.md) | 用于设置是否为只读事务。|

### 数据类型相关

|    变量名     |             功能描述             |
|-------------------|-------------------------------------|
| [explicit_defaults_for_timestamp](400.session-system-variable/2600.explicit_defaults_for_timestamp-session.md) | 用于指定 timestamp 数据类型在处理默认值和空值时是否启用非标准行为。       |
| [log_row_value_options](400.session-system-variable/3500.log_row_value_options-session.md)  | 用于控制 Lob 是否使用部分更新。 |
| [nls_currency](400.session-system-variable/4700.nls_currency-session.md)            | 用于设置数字格式元素 `L` 的本地货币符号。       |
| [nls_date_format](400.session-system-variable/4800.nls_date_format-session.md)         | 用于控制 date 类型转 str 的格式，以及 str 隐式转 date 的格式。  |
| [nls_dual_currency](400.session-system-variable/5000.nls_dual_currency-session.md)       | 用于设置数字格式元素 `U` 的本地货币符号。       |
| [nls_iso_currency](400.session-system-variable/5100.nls_iso_currency-session.md)        | 用于设置数字格式元素 `C` 的本地货币符号。       |
| [nls_length_semantics](400.session-system-variable/5300.nls_length_semantics-session.md)    | 表示 CHAR、VARCHAR2 类型的 Length 语义。             |
| [nls_nchar_conv_excp](400.session-system-variable/5500.nls_nchar_conv_excp-session.md)     | 用于控制 NCHAR/NVARCHAR2 与 CHAR/VARCHAR2 之间转换丢失数据时是否报错。        |
| [nls_numeric_characters](400.session-system-variable/5600.nls_numeric_characters-session.md)  | 用于控制 Number 和字符串转换中字符串中小数分隔符和千分位分隔符的字符。     |
| [nls_timestamp_format](400.session-system-variable/5900.nls_timestamp_format-session.md)    | 用于控制 Timestamp 或 Timestamp 中 ltz 类型转 str 的格式，以及 str 隐式转 Timestamp 或 Timestamp 中 ltz 的格式。 |
| [nls_timestamp_tz_format](400.session-system-variable/6000.nls_timestamp_tz_format-session.md) | 用于控制 Timestamp tz 类型转 str 的格式，以及 str 隐式转 Timestamp tz 的格式。 |
| [ob_default_lob_inrow_threshold](400.session-system-variable/6440.ob_default_lob_inrow_threshold-session.md)  | 用于指定建表时，表中 LOB 字段的默认行内存储阈值。 |
...
| 变量名 | 功能描述 |
|-------|----------|
| [cursor_sharing](400.session-system-variable/2000.cursor_sharing-session.md) | 用于判断处理 SQL 时是否进行参数化。      |
| [have_profiling](400.session-system-variable/2850.have_profiling-session.md) | 用于检测是否支持查询性能分析。      |
| [max_allowed_packet](400.session-system-variable/3700.max_allowed_packet-session.md)      | 用于设置最大网络包大小。                             |
| [max_sp_recursion_depth](400.session-system-variable/3900.max_sp_recursion_depth-session.md)  | 用于设置可以递归调用任一指定存储过程的最大层次。                          |
| [max_user_connections](400.session-system-variable/4000.max_user_connections-session.md) | 用于指定单个用户可以向 OBServer 节点建立的连接数，设置为 `0` 时表示不限制连接数。 |
| [ob_enable_blk_nestedloop_join](400.session-system-variable/6500.ob_enable_blk_nestedloop_join-session.md)  | 用于设置是否允许打开 block nested loop join。 |
| [ob_enable_rich_error_msg](400.session-system-variable/7250.ob_enable_rich_error_msg-session.md) | 用于控制 SQL 执行出错时是否显示 svr_ip、time、trace_id 等信息。    |
| [ob_pl_block_timeout](400.session-system-variable/8300.ob_pl_block_timeout-session.md)  | 用于设置 PL 的最大超时时间。   |
| [profiling](400.session-system-variable/11000.profiling-session.md) | 用于启用或禁用语句分析功能。      |
| [profiling_history_size](400.session-system-variable/11200.profiling_history_size-session.md) | 用于设置保存查询性能信息的历史记录数量。      |
| [sql_mode](400.session-system-variable/11700.sql_mode-session.md)      | 用于设置 SQL 模式，不同的 SQL 模式对于插入等行为有很大影响。      |
| [sql_quote_show_create](400.session-system-variable/11800.sql_quote_show_create-session.md) |用于设置是否为 `SHOW CREATE TABLE` 和 `SHOW CREATE DATABASE` 语句启动引用标识符。|
| [sql_safe_updates](400.session-system-variable/11900.sql_safe_updates-session.md)                | 用于设置是否启用 MySQL 的 SQL 安全模式，默认为关闭。 |
| [sql_select_limit](400.session-system-variable/12000.sql_select_limit-session.md)        | 用于设置一个 Select 查询允许返回的最大行数。                        |
| [sql_throttle_current_priority](400.session-system-variable/12100.sql_throttle_current_priority-session.md) | 用于设置限流优先级，只有 `sql_throttle_current_priority` 小于 `sql_throttle_priority` 的那些 session 上的请求才会被限流。 |
...
|   变量名    |    功能描述  |
|--------------------|----------------------|
| [foreign_key_checks](400.session-system-variable/2700.foreign_key_checks-session.md) | 用于设置是否在 DML/DDL 中检查外建约束。 |

### 优化器相关

|           变量名           |              功能描述              |
|--------------------------------|--------------------------------|
| [ob_enable_transformation](400.session-system-variable/7500.ob_enable_transformation-session.md)        | 用于设置是否开启 SQL 优化器的改写功能。                        |
| [optimizer_capture_sql_plan_baselines](400.session-system-variable/10400.optimizer_capture_sql_plan_baselines-session.md) | 用于控制是否自动捕获新计划到 Plan Baseline。  |
| [optimizer_features_enable](400.session-system-variable/10550.optimizer_features_enable-session.md) | 用于启用不同 OceanBase 版本优化器功能。 |
| [optimizer_use_sql_plan_baselines](400.session-system-variable/10600.optimizer_use_sql_plan_baselines-session.md)     | 用于控制优化器是否会使用 Plan Baseline 计划。 |

### 自增列相关

|    变量名     |   功能描述     |
|--------------|----------------|
| [auto_increment_increment](400.session-system-variable/400.auto_increment_increment-session.md)| 用于设置自增步长，仅用于 MySQL 客户端登录。     |
| [auto_increment_offset](400.session-system-variable/500.auto_increment_offset-session.md)   | 用于确定 `AUTO_INCREMENT` 列值的起点。   |
...
| [div_precision_increment](400.session-system-variable/2400.div_precision_increment-session.md)        | 用于设置除法结果精度在被除数精度基础上的增量，是 MySQL 兼容功能。     |
| [sql_auto_is_null](400.session-system-variable/11600.sql_auto_is_null-session.md)    | 用于控制是否获取最后插入行的自增列值。  |

### 字符集和字符串相关

|     变量名     | 功能描述  |
|--------------------|--------------------------------------------|
| [character_set_client](400.session-system-variable/900.character_set_client-session.md)    | 用于设置客户端发送语句的字符集。             |
| [character_set_database](400.session-system-variable/1100.character_set_database-session.md)   | 用于设置默认数据库的字符集。       |
| [character_set_filesystem](400.session-system-variable/1200.character_set_filesystem-session.md) | 用于设置文件系统字符集类型。      |
| [character_set_results](400.session-system-variable/1300.character_set_results-session.md)    | 用于设置服务器在将结果集或错误消息发送回客户端之前应转换为的字符集。       |
| [character_set_server](400.session-system-variable/1400.character_set_server-session.md)     | 用于设置服务器字符集。  |
| [character_set_system](400.session-system-variable/1500.character_set_system-session.md)     | 用于查看服务器使用的字符集。            |
| [collation_connection](400.session-system-variable/1600.collation_connection-session.md)     | 用于设置连接使用的字符集和字符序。   |
| [collation_database](400.session-system-variable/1700.collation_database-session.md)       | 用于设置创建数据库默认字符集和字符序。 |
| [collation_server](400.session-system-variable/1800.collation_server-session.md)         | 用于设置服务器默认字符集和字符序。   |
| [nls_comp](400.session-system-variable/4600.nls_comp-session.md)                 | 表示字符串值的比较规则。   |
| [nls_sort](400.session-system-variable/5700.nls_sort-session.md)                 | 表示字符串值的排序规则。  |
| [sql_warnings](400.session-system-variable/12500.sql_warnings-session.md)             | 用于在出现告警时控制单行 INSERT 语句是否生成信息字符串。     |
