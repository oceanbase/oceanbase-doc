...
您可以在创建表之前在全局级别设置 `innodb_stats_persistent`，或者使用 `CREATE TABLE` 和 `ALTER TABLE` 语句的 `STATS_PERSISTENT` 子句来为单个表配置持久性统计信息。通过将 InnoDB 索引统计信息持久化到磁盘，避免因频繁重新计算统计信息，而导致查询执行计划的变化。
