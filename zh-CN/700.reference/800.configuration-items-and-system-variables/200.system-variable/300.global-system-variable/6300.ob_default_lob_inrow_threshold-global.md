...
`ob_default_lob_inrow_threshold` 是一个 Global 级别系统变量，实际影响的是新建表（已有表不受影响）的表级属性 `lob_inrow_threshold` 的取值。修改系统变量 `ob_default_lob_inrow_threshold` 会影响新建表的行内行外存储阈值，修改表级属性 `lob_inrow_threshold` 则会影响该表的行内行外存储。建表时若没有指定 `lob_inrow_threshold`，则会取该变量的值写入表 Schema。
