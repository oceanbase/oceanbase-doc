| description |  |
| --- | --- |
| keywords |  |
| dir-name |  |
| dir-name-en |  |
| tenant-type |  |
| machine-translation | en |
| slug | ob-enable-pl-async-commit |


# ob_enable_pl_async_commit
#### 说明
对于 V4.2.5 版本，该变量从 V4.2.5 BP8 版本开始引入。

对于 V4.4.x 版本，该变量从 V4.4.2 版本开始引入。
对于 V4.6.x 版本，该变量从 V4.6.0 版本开始引入。

## 功能描述
`ob_enable_pl_async_commit` 用于控制是否开启 PL 异步提交优化。

## 权限要求
+ 查询变量
    - Global 级别`sys` 租户和所有用户租户均可以使用 `SHOW VARIABLES` 语句或视图 `SYS.TENANT_VIRTUAL_GLOBAL_VARIABLE`（Oracle 模式）及视图 `information_schema.GLOBAL_VARIABLES`（MySQL 模式）查看 Global 系统变量的值。
    - Session 级别`sys` 租户和所有用户租户均可以使用 `SHOW VARIABLES` 语句或视图 `SYS.TENANT_VIRTUAL_SESSION_VARIABLE`（Oracle 模式）及视图 `information_schema.SESSION_VARIABLES`（MySQL 模式）查看 Session 系统变量的值。
+ 修改变量
    - Global 生效
        * `sys` 租户可以直接修改 Global 系统变量的值。
        * MySQL 用户租户需要拥有 `SUPER` 或 `ALTER SYSTEM` 权限才能修改 Global 系统变量的值。
        * Oracle 用户租户需要拥有 `ALTER SYSTEM` 权限才能修改 Global 系统变量的值。
    - Session 生效`sys` 租户和所有用户租户均可以直接修改本租户的 Session 系统变量的值。

## 属性说明
| **属性** | **描述** |
| --- | --- |
| 参数类型 | Bool |
| 默认值 | OFF |
| 取值范围 | + OFF<br/>+ ON |
| 生效范围 | + Global<br/>+ Session |
| 是否可修改 | 是，支持通过 `SET` 或 `ALTER SESSION SET` 语句修改。 |


## 配置示例
设置开启 PS 模式下匿名块和 `call` 语句在解析时的参数化功能。

+ Global 生效

```shell
obclient> SET GLOBAL ob_enable_pl_async_commit=ON;
```

+ Session 生效

```shell
obclient> SET ob_enable_pl_async_commit=ON;
```
