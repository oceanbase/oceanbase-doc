...
| group_replication_single_primary_mode                     | 指定组复制是否以单主模式运行，并且自动挑选一台服务器来处理读/写工作负载。如果设置为 `ON`，则表示可读写的主节点只有一个，其他组成员均为辅助节点；如果设置为 `OFF`，则表示关闭单主模式，即启用多主模式，多主模式中通常情况下所有的组成员都是可读写的主节点。其默认值为 `ON`。 <main id="notice" type='explain'><h4>说明</h4><p>该变量仅用于兼容 MySQL 5.7，相关功能并不支持，可以查询及设置，设置行为不报错但不生效。</p></main>  |
...
| group_replication_start_on_boot                           | 指定在服务器启动时是否启动组复制，默认值为 `ON`。<main id="notice" type='explain'><h4>说明</h4><p>该变量仅用于兼容 MySQL 5.7，相关功能并不支持，可以查询及设置，设置行为不报错但不生效。</p></main>  |
...
| slave_sql_verify_checksum                               | 控制复制 SQL 线程是否使用从回放日志中读取的校验和来验证数据，默认值为 `ON`。 <main id="notice" type='explain'><h4>说明</h4><p>该变量仅用于兼容 MySQL 5.7，相关功能并不支持，可以查询及设置，设置行为不报错但不生效。</p></main> |
...
| rpl_semi_sync_master_wait_no_slave                      | 控制即使副本数下降到少于变量 `rpl_semi_sync_master_wait_for_slave_count` 配置的数量时，源是否也会等待超时（变量 `rpl_semi_sync_master_timeout` 设置的时间）。默认值为 `ON`。 <main id="notice" type='explain'><h4>说明</h4><p>该变量仅用于兼容 MySQL 5.7，相关功能并不支持，可以查询及设置，设置行为不报错但不生效。</p></main> |
