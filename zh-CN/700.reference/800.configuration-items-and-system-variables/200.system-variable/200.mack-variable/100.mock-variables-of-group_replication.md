...
| 变量名                                                   | 功能描述                                                      |
|---------------------------------------------------------|---------------------------------------------------------------|
| group_replication_allow_local_disjoint_gtids_join       | 当服务器中具有组中不存在的本地事务时，是否允许其加入该组。其默认值为 `0`。<main id="notice" type='explain'><h4>说明</h4><p>该变量仅用于兼容 MySQL 5.7，相关功能并不支持，可以查询及设置，设置行为不报错但不生效。</p></main>    |
| group_replication_allow_local_lower_version_join        | 当服务器所使用的 MySQL Server 版本低于组中其他成员的 MySQL Server 版本时，是否允许其加入该组。其默认值为 `0`。<main id="notice" type='explain'><h4>说明</h4><p>该变量仅用于兼容 MySQL 5.7，相关功能并不支持，可以查询及设置，设置行为不报错但不生效。</p></main>           |
...
| group_replication_bootstrap_group                       | 指定使用当前服务器来 Bootstrap 组，该变量只能在一个服务器上设置，并且只能在首次时在 Bootstrap 组或重新 Bootstrap 组时在其中一个服务器上设置。其默认值为 `0`。<main id="notice" type='explain'><h4>说明</h4><p>该变量仅用于兼容 MySQL 5.7，相关功能并不支持，可以查询及设置，设置行为不报错但不生效。</p></main>    |
...
| group_replication_compression_threshold                  | 组成员之间发送消息的阈值。当超过该阈值后，将会对组成员之间发送的消息进行压缩。其默认值为 `0`，表示禁用压缩。 <main id="notice" type='explain'><h4>说明</h4><p>该变量仅用于兼容 MySQL 5.7，相关功能并不支持，可以查询及设置，设置行为不报错但不生效。</p></main>  |
| group_replication_enforce_update_everywhere_checks       | 在任何时候启用或禁用多主更新的严格一致性检查。其默认值为 `0`，表示禁用检查。 <main id="notice" type='explain'><h4>说明</h4><p>该变量仅用于兼容 MySQL 5.7，相关功能并不支持，可以查询及设置，设置行为不报错但不生效。</p></main>    |
...
| group_replication_recovery_ssl_verify_server_cert         | 指定在分布式恢复过程中，是否校验 Donor 证书中的服务器通用名称。其默认值为 `0`。  <main id="notice" type='explain'><h4>说明</h4><p>该变量仅用于兼容 MySQL 5.7，相关功能并不支持，可以查询及设置，设置行为不报错但不生效。</p></main>   |
| group_replication_recovery_use_ssl                        | 指定组复制成员在进行分布式恢复时，用于分布式恢复的连接通道是否需要使用SSL。其默认值为 `0`。  <main id="notice" type='explain'><h4>说明</h4><p>该变量仅用于兼容 MySQL 5.7，相关功能并不支持，可以查询及设置，设置行为不报错但不生效。</p></main>   |
| group_replication_single_primary_mode                     | 指定组复制是否以单主模式运行，并且自动挑选一台服务器来处理读/写工作负载。如果设置为 `1`，则表示可读写的主节点只有一个，其他组成员均为辅助节点；如果设置为 `0`，则表示关闭单主模式，即启用多主模式，多主模式中通常情况下所有的组成员都是可读写的主节点。其默认值为 `0`。 <main id="notice" type='explain'><h4>说明</h4><p>该变量仅用于兼容 MySQL 5.7，相关功能并不支持，可以查询及设置，设置行为不报错但不生效。</p></main>  |
...
| group_replication_start_on_boot                           | 指定在服务器启动时是否启动组复制，默认值为 `0`。<main id="notice" type='explain'><h4>说明</h4><p>该变量仅用于兼容 MySQL 5.7，相关功能并不支持，可以查询及设置，设置行为不报错但不生效。</p></main>  |
...
| replication_optimize_for_static_plugin_config           | 控制是否启用共享锁，以提高半同步复制的性能。其默认值为 `0`。 <main id="notice" type='explain'><h4>说明</h4><p>该变量仅用于兼容 MySQL 5.7，相关功能并不支持，可以查询及设置，设置行为不报错但不生效。</p></main> |
| replication_sender_observe_commit_only                  | 控制是否启用限制回调，以提高半同步复制的性能。其默认值为 `0`。<main id="notice" type='explain'><h4>说明</h4><p>该变量仅用于兼容 MySQL 5.7，相关功能并不支持，可以查询及设置，设置行为不报错但不生效。</p></main>    |
| rpl_semi_sync_slave_enabled                             | 控制是否在副本上启用半同步复制，默认值为 `0`。  <main id="notice" type='explain'><h4>说明</h4><p>该变量仅用于兼容 MySQL 5.7，相关功能并不支持，可以查询及设置，设置行为不报错但不生效。</p></main> |
...
| slave_compressed_protocol                               | 当源和副本都支持压缩时，控制是否使用源或副本协议的压缩。其默认值为 `0`。<main id="notice" type='explain'><h4>说明</h4><p>该变量仅用于兼容 MySQL 5.7，相关功能并不支持，可以查询及设置，设置行为不报错但不生效。</p></main> |
...
| slave_preserve_commit_order                             | 对于多线程复制，控制其是否按照回放日志中的事务顺序来提交事务。其默认值为 `0`。<main id="notice" type='explain'><h4>说明</h4><p>该变量仅用于兼容 MySQL 5.7，相关功能并不支持，可以查询及设置，设置行为不报错但不生效。</p></main> |
...
| slave_sql_verify_checksum                               | 控制复制 SQL 线程是否使用从回放日志中读取的校验和来验证数据，默认值为 `0`。 <main id="notice" type='explain'><h4>说明</h4><p>该变量仅用于兼容 MySQL 5.7，相关功能并不支持，可以查询及设置，设置行为不报错但不生效。</p></main> |
...
| 变量名                                                   | 功能描述                                                      |
|---------------------------------------------------------|---------------------------------------------------------------|
| rpl_semi_sync_master_enabled                            | 控制源上是否启用半同步复制。其默认值为 `0`。<main id="notice" type='explain'><h4>说明</h4><p>该变量仅用于兼容 MySQL 5.7，相关功能并不支持，可以查询及设置，设置行为不报错但不生效。</p></main>  |
...
| rpl_semi_sync_master_wait_no_slave                      | 控制即使副本数下降到少于变量 `rpl_semi_sync_master_wait_for_slave_count` 配置的数量时，源是否也会等待超时（变量 `rpl_semi_sync_master_timeout` 设置的时间）。默认值为 `0`。 <main id="notice" type='explain'><h4>说明</h4><p>该变量仅用于兼容 MySQL 5.7，相关功能并不支持，可以查询及设置，设置行为不报错但不生效。</p></main> |
...
| 变量名                                                   | 功能描述                                                      |
|---------------------------------------------------------|---------------------------------------------------------------|
| master_verify_checksum                                  | 控制是否启用 Checksum 校验，默认值为 `0`。<main id="notice" type='explain'><h4>说明</h4><p>该变量仅用于兼容 MySQL 5.7，相关功能并不支持，可以查询及设置，设置行为不报错但不生效。</p></main>    |

### 集群相关

| 变量名                                                   | 功能描述                                                      |
|---------------------------------------------------------|---------------------------------------------------------------|
| slave_allow_batching                                    | 控制是否开启复制线程的批量更新。其默认值为 `0`。<main id="notice" type='explain'><h4>说明</h4><p>该变量仅用于兼容 MySQL 5.7，相关功能并不支持，可以查询及设置，设置行为不报错但不生效。</p></main> |
...
| 变量名                                                   | 功能描述                                                      |
|---------------------------------------------------------|---------------------------------------------------------------|
| pseudo_slave_mode                                       | 设置服务器是否转换为模拟从库模式，默认值为 `0`。<main id="notice" type='explain'><h4>说明</h4><p>该变量仅用于兼容 MySQL 5.7，相关功能并不支持，可以查询及设置，设置行为不报错但不生效。</p></main> |
