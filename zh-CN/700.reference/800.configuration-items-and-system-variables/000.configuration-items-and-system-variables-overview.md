...
| | 配置项 | | 系统变量 | |
|---------|-----------|---------|---------|---------|
| 参数类型 | 集群级配置项 | 租户级配置项 | 全局系统变量 | 会话级系统变量 |
| 生效范围 | 集群所有 OBServer，或指定节点，或指定 Zone 生效 | 租户所有 Unit | 租户所有 Unit | 当前会话 |
| 生效方式 | 立即生效（DYNAMIC_EFFECTIVE）/重启生效（STATIC_EFFECTIVE） | 立即生效（DYNAMIC_EFFECTIVE）/重启生效（STATIC_EFFECTIVE） | 重新登录后新建会话生效 | 当前会话立即生效 |
| 租户类型 | sys 租户| MySQL 租户和 Oracle 租户| MySQL 租户和 Oracle 租户 | MySQL 租户和 Oracle 租户|
| 修改语句 | <ul><li>支持通过 SQL 语句修改，示例：</br><code>ALTER SYSTEM SET schema_history_expire_time='1h';</code></li><li>支持通过启动参数修改，示例：</br><code>cd /home/admin/oceanbase && ./bin/observer -o "schema_history_expire_time='1h'";</code> </li></ul> | <ul><li>支持通过 SQL 语句修改，示例：</br><code>ALTER SYSTEM SET schema_history_expire_time='1h';</code></li><li>支持通过启动参数修改，示例：</br><code>cd /home/admin/oceanbase && ./bin/observer -o "schema_history_expire_time='1h'";</code> </li></ul> | 仅支持通过 SQL 语句修改，示例如下：<ul><li>MySQL 模式</br><code>SET ob_query_timeout = 20000000;</code></br><code>SET GLOBAL ob_query_timeout = 20000000;</code></br></li><li> Oracle 模式</br><code>ALTER SESSION SET ob_query_timeout = 20000000;</code></br><code>ALTER SYSTEM SET ob_query_timeout = 20000000;</code></br>或者</br><code>SET ob_query_timeout = 20000000;</code></br><code>SET GLOBAL ob_query_timeout = 20000000;</code></li></ul> | 仅支持通过 SQL 语句修改，示例如下：<ul><li>MySQL 模式</br><code>SET ob_query_timeout = 20000000;</code></br><code>SET GLOBAL ob_query_timeout = 20000000;</code></br></li><li> Oracle 模式</br><code>ALTER SESSION SET ob_query_timeout = 20000000;</code></br><code>ALTER SYSTEM SET ob_query_timeout = 20000000;</code></br>或者</br><code>SET ob_query_timeout = 20000000;</code></br><code>SET GLOBAL ob_query_timeout = 20000000;</code></li></ul> |
| 查看语句 | 可以使用 `SHOW PARAMETERS` 语句查询。示例：`SHOW PARAMETERS LIKE 'schema_history_expire_time';` | 可以使用 `SHOW PARAMETERS` 语句查询。示例：`SHOW PARAMETERS LIKE 'schema_history_expire_time';` | 可以使用 `SHOW [GLOBAL] VARIABLES` 语句查询。示例如下：<ul><li>MySQL 模式</br>`SHOW VARIABLES LIKE 'ob_query_timeout';`</br>`SHOW GLOBAL VARIABLES LIKE 'ob_query_timeout';`</br></li><li> Oracle 模式</br>`SELECT * FROM SYS.TENANT_VIRTUAL_SESSION_VARIABLE WHERE VARIABLE_NAME = 'ob_query_timeout';`</br>`SELECT * FROM SYS.TENANT_VIRTUAL_GLOBAL_VARIABLE WHERE VARIABLE_NAME = 'ob_query_timeout';`</li></ul> | 可以使用 `SHOW [GLOBAL] VARIABLES` 语句查询。示例如下：<ul><li>MySQL 模式</br>`SHOW VARIABLES LIKE 'ob_query_timeout';`</br>`SHOW GLOBAL VARIABLES LIKE 'ob_query_timeout';`</br></li><li> Oracle 模式</br>`SELECT * FROM SYS.TENANT_VIRTUAL_SESSION_VARIABLE WHERE VARIABLE_NAME = 'ob_query_timeout';`</br>`SELECT * FROM SYS.TENANT_VIRTUAL_GLOBAL_VARIABLE WHERE VARIABLE_NAME = 'ob_query_timeout';`</li></ul> |
| 语法兼容 | OceanBase 特有 | Oracle 兼容 | MySQL 兼容 | MySQL 兼容 |
| 持久化机制 | observer.config.bin 文件。持久化到内部表与配置文件，可以在 <code>/home/admin/oceanbase/etc/observer.config.bin</code> 与 <code>/home/admin/oceanbase/etc/observer.config.bin.history</code> 文件中查询该配置项。 | observer.config.bin 文件。持久化到内部表与配置文件，可以在 <code>/home/admin/oceanbase/etc/observer.config.bin</code> 与 <code>/home/admin/oceanbase/etc/observer.config.bin.history</code> 文件中查询该配置项。 | 系统表 | 不持久化 |
| 生命周期 | 长，从进程启动到退出。 | 长，从进程启动到退出。 | 短，需要租户的 Schema 创建成功以后才生效。 |短，需要租户的 Schema 创建成功以后才生效。 |
