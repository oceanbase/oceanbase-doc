...
| 配置项名                                                                                                       | 引入版本    | 描述                             |
|---------------------------------------------------------------------------------------------------------------|-------------|---------------------------------|
| [external_table_io_hole_size](100.system-configuration-items/400.tenant-level-configuration-items/3230.external_table_io_hole_size.md) | 用于设置两个连续 IO 范围之间的最大距离（以字节为单位）。|
| [external_table_io_range_size](100.system-configuration-items/400.tenant-level-configuration-items/3250.external_table_io_range_size.md) | 用于设置合并范围的最大大小（以字节为单位）。如果合并两个连续范围会产生大于此大小的范围，则不会合并它们。|
| [async_ps_cursor_max_count](100.system-configuration-items/300.cluster-level-configuration-items/650.async_ps_cursor_max_count.md) | 用于控制单节点中异步 PS cursor 的最大个数。|
| [ob_enable_java_udf](100.system-configuration-items/400.tenant-level-configuration-items/5860.ob_enable_java_udf.md) | 用于开启或关闭 Java UDF 功能。|
| [load_data_diagnosis_log_compression](100.system-configuration-items/400.tenant-level-configuration-items/4220.load_data_diagnosis_log_compression.md) | 用于设置诊断日志使用的压缩算法。 |
| [load_data_diagnosis_log_max_size](100.system-configuration-items/400.tenant-level-configuration-items/4230.load_data_diagnosis_log_max_size.md) | 用于设置单个诊断日志文件的大小。|
| [result_cache_max_size](100.system-configuration-items/400.tenant-level-configuration-items/7340.result_cache_max_size.md) | 用于控制 `result cache` 模块能使用最大内存量。 |
| [result_cache_max_result](100.system-configuration-items/400.tenant-level-configuration-items/7350.result_cache_max_result.md) | 用于控制单个 result cache 缓存对象最多能使用 `result_cache_max_size` 的内存比例。|
| [ob_result_cache_evict_percentage](100.system-configuration-items/400.tenant-level-configuration-items/7360.ob_result_cache_evict_percentage.md) | 用于设置 `result cache` 内存淘汰水位线。|
| [ob_deterministic_udf_cache_max_size](100.system-configuration-items/400.tenant-level-configuration-items/7370.ob_deterministic_udf_cache_max_size.md) | 用于标记语句执行期，所有 `deterministic` 的 UDF 的缓存池总共可以使用的最大内存量(以字节为单位)，如果为 0，则表示关闭执行期缓存功能。 |
| [attach_shared_sstable_thread_score](100.system-configuration-items/400.tenant-level-configuration-items/230.attach_shared_sstable_thread_score.md) | 用于配置租户可用来执行共享存储模式下计算节点共享数据引用的线程数。 |
| [inc_sstable_upload_thread_score](100.system-configuration-items/400.tenant-level-configuration-items/3650.inc_sstable_upload_thread_score.md) | 用于配置租户可用来执行共享存储模式下数据共享类任务的线程数。 |
