...
| px_node_policy             | V4.3.5 BP1 | 用于决定 PX 计算节点的候选池。 |
| ob_enable_java_env         | V4.3.5 BP1 | 控制是否开启 HDFS/MaxCompute（ODPS）等外表能力依赖的 java 环境。 |
| ob_java_home               | V4.3.5 BP1 | 设置 jni 运行依赖的 java home 路径，需要在 ob_enable_java_env 开启情况下进行配置。 |
| ob_java_opts               | V4.3.5 BP1 | 设置 jvm 启动配置项，如内存大小及回收算法等，需要在 ob_enable_java_env 开启情况下进行配置。 |
| ob_java_connector_path     | V4.3.5 BP1 | 设置外表 connector 依赖的 jar 包路径，需要在 ob_enable_java_env 开启情况下进行配置。 |
| enable_adaptive_plan_cache | V4.3.5 BP1 | 控制是否开启 PLAN CACHE 自适应能力。 |
