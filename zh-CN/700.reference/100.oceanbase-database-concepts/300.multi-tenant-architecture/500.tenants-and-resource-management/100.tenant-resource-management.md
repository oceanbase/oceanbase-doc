...
在 V4.x 之前版本中，OceanBase 数据库仅支持租户的同构 Zone 模式，同构 Zone 模式下，限制租户下各 Zone 的 `UNIT_NUM` 必须相同。

在 V4.2.5 版本中，OceanBase 数据库从 V4.2.5 BP5 版本开始，放开限制，支持租户的异构 Zone 模式。异构 Zone 模式下，允许租户下各 Zone 的 `UNIT_NUM` 不同，但一个租户的所有 Zone 最多只能有两种 `UNIT_NUM`。
...
* 未被分配的资源池的 UNIT_NUM 变更
  ...
  同构 Zone 模式下，当资源池未分配给租户时，可以通过 `CREATE RESOURCE POOL` 语句来调整资源池的 `UNIT_NUM`，可以调整资源池下每个 Zone 内资源单元的数量。
...
* 租户资源池的 UNIT_NUM 变更
  ...
  同构 Zone 模式下，资源池在分配给租户后，也可以进行 UNIT_NUM 的变更。资源池变更要以租户为单位，所有资源池都增加或都减少才可以成功。调整租户资源池的 UNIT_NUM 的示例语句如下：
  ...
  obclient> ALTER RESOURCE TENANT tt UNIT_NUM = 1 DELETE UNIT_GROUP = (1006);//指定 UNIT_GROUP 调小 UNIT_NUM
  ```

#### 异构 Zone 模式下的变更

异构 Zone 模式下，变更租户资源池的 `UNIT_NUM` 的示例语句如下：
...
需要注意的是，异构 Zone 模式下，不支持通过指定 `UNIT_GROUP` 的方式来调小 `UNIT_NUM`。

此外，在异构 Zone 模式下，还支持单独调整其中一个资源池的 `UNIT_NUM`，语句如下：

```shell
obclient> ALTER RESOURCE POOL pool2 UNIT_NUM = 3;//调大 UNIT_NUM
```

```shell
obclient> ALTER RESOURCE POOL pool2 UNIT_NUM = 1;//调小 UNIT_NUM
```
