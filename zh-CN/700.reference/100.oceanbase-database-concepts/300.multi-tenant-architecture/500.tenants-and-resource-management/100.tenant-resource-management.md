...
* 同构 Zone 模式下，限制租户下各 Zone 的 `UNIT_NUM` 必须相同。

* 异构 Zone 模式下，允许租户下各 Zone 的 `UNIT_NUM` 不同，但一个租户的所有 Zone 最多只能有两种 `UNIT_NUM`。
...
* **租户资源池的 UNIT_NUM 变更**
  ...
  同构 Zone 模式下，资源池在分配给租户后，也可以进行 UNIT_NUM 的变更。资源池变更要以租户为单位，所有资源池都增加或都减少才可以成功。
