...
* 也通过以下的运维命令手动触发合并。
  ...
  * 租户级合并
    ...
    * 系统租户发起其他租户的租户级合并
      ...
      * 对租户 t1 和 t2 发起的租户级合并

        ```shell
        obclient(root@sys)[oceanbase]> ALTER SYSTEM MAJOR FREEZE TENANT = t1,t2; 
        ```
  ...
  * 表级合并
    ...
    * 系统租户发起指定租户 t1 的表级合并
      ...
      ```shell
      obclient(root@sys)[oceanbase]> ALTER SYSTEM MAJOR FREEZE TENANT = t1 TABLE_ID = 500002;
      ```
  ...
  * 分区级合并
    ...
    假设待合并的表分区对应的 `tablet_id` 为 `200001`：

    * 系统租户发起指定租户 t1 的分区级合并

      ```shell
      obclient(root@sys)[oceanbase]> ALTER SYSTEM MAJOR FREEZE TENANT = t1 TABLET_ID = 200001;
      ```
