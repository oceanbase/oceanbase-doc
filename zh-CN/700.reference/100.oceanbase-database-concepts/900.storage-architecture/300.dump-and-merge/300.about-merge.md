...
* 窗口合并：在指定时间窗口执行的增量式、按需式的合并。
  ...
  为了解决租户合并时长不可控且分区数多场景下合并慢的问题，对于 V4.6.x 版本，OceanBase 数据库从 V4.6.0 版本开始，引入了一种新的合并机制——窗口合并（Window Compaction）。窗口合并是在指定时间窗口执行的增量式、按需式的合并，其仅合并有增量数据且满足特定条件的分区，并按照优先级调度合并顺序；窗口合并接入了 `DBMS_SCHEDULER` 系统包的定时任务管理程序，支持基于时区的调度，可精确控制合并的任务时长；支持通过资源管理计划精细化控制合并任务的资源占用，并能自适应调整合并线程数，使得合并任务占用的资源可控。
...
* 也通过以下的运维命令手动触发合并。
  ...
  * 租户级合并
    ...
    * 系统租户发起其他租户的租户级合并
      ...
      * 对租户 tenant1 和 tenant2 发起的租户级合并
  ...
  * 表级合并
    ...
    假设待合并的表的 `table_id` 为 `500002`：

    * 系统租户发起指定租户 tenant1 的表级合并
  ...
  * 分区级合并
    ...
    假设待合并的表分区对应的 `tablet_id` 为 `200007`：

    * 系统租户发起指定租户 tenant1 的分区级合并
...
更多合并操作，参见 [合并管理](../../../200.system-management/500.manage-data-storage/200.merge-management/100.consolidation-management-overview.md)。
