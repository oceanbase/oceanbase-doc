...
* 窗口合并：在指定时间窗口执行的增量式、按需式的合并。
  ...
  对于 V4.6.x 版本，OceanBase 数据库从 V4.6.0 版本开始，引入了一种全新的合并形式——窗口合并（Window Compaction）。窗口合并的特点如下：

  * 仅合并有增量数据后满足特定条件的分区，并按照优先级调度合并顺序。

  * 调度统一，接入 `DBMS_SCHEDULER` 系统包的定时任务管理程序，支持基于时区的调度，可精确控制合并的任务时长。

  * 资源可控：支持通过资源管理计划精细化控制合并任务的资源占用，并能自适应调整合并线程数。
