...
* caching_sha2_password：基于 SHA-256 的密码认证插件，具有更高的安全防护等级。该插件从 V4.6.1 版本开始支持，具备以下特性：
...
OceanBase 数据库在 Oracle 模式下从 V4.6.1 版本开始新增支持密码认证插件机制。用户可以在创建或修改用户时显式指定使用 `caching_sha2_password`（基于 SHA-256）认证插件，具有更高的安全防护等级。
