...
OceanBase 数据库是分布式的数据库系统，通常由多个可用区（Zone）组成，每个可用区（Zone）内包含若干多台物理服务器。OceanBase 数据库的整体结构如下图所示。

![1](https://obbusiness-private.oss-cn-shanghai.aliyuncs.com/doc/img/observer-enterprise/V4.2.1/700.reference/100.oceanbase-database-concepts/500.distributed-database-objects/100.cluster-level-scale-out-and-scale-in/1.ob_architecture.png)
...
存在一个 OceanBase 集群，共包含 3N 个 OBServer（包含 3 个可用区 Zone1、Zone2、Zone3，每个 Zone 包含 N 个 OBServer）提供读写服务，当集群提供的服务能力不能完全满足读写请求时，需要对该 OceanBase 集群扩容以提高集群服务能力。为集群扩容三台新 OBServer 节点，每个可用区扩容一台，扩容后每个可用区包含 N+1 台 OBServer。通过动态扩容，集群中的 OBServer 节点数量相应增加，Root Service 会根据内部的负载均衡机制，将集群内原有的数据和负载依次均衡到新扩容的节点上，具体的负载均衡机制与策略，可参考 [资源单元的均衡](../../300.multi-tenant-architecture/500.tenants-and-resource-management/300.resource-unit-balancing.md)。

相反地，当集群中的节点服务能力对当前负载有较多冗余时，可考虑通过缩减节点来降低集群成本，称即将被缩减掉的节点为待删除节点。具体地，可执行集群管理指令，将待删除节点上的数据和负载迁移到其他可用节点上去，然后回收待删除节点，完成物理机缩容。
