...
| enable_ls_leader_balance                 | 租户级 | true  | 日志流 Leader 均衡的开关。在 `enable_rebalance` 开启时，关闭 `enable_ls_leader_balance` 可以单独控制关闭日志流 Leader 的自动均衡。| LS 均衡 |
| partition_balance_schedule_interval      | 租户级 | 0s | 分区均衡调度间隔，从租户创建/集群重启开始计时。<main id="notice" type='notice'><h4>注意</h4><p>对于 V4.4.x 版本，该配置项的默认值从 V4.4.1 版本开始更改为 0s，对于 V4.4.1 及之后版本，不建议再使用该配置项来控制分区均衡，用户可以通过调用 <code>DBMS_BALANCE.TRIGGER_PARTITION_BALANCE</code> 子程序来定时或手动触发分区均衡。</p></main>| 分区均衡 |
| zone_disk_balance_tolerance_percentage   | 租户级 | 10 | 控制 Zone 间表组磁盘均衡算法对不均衡的容忍程度。 | 表组磁盘均衡 |
