...
* `duplicate_scope` 对齐
  ...
  <h4>说明</h4>
  <p>当前版本暂不支持表 <code>duplicate_scope</code> 属性的变更。</p>
  </main>

  复制表只能存在于广播日志流上。当用户修改表的 `duplicate_scope` 属性后，需要等待分区均衡执行完成后才能按照预期属性来使用该表。可以临时调小租户级配置项 `partition_balance_schedule_interval` 的值以便快速对齐。
...
您可以在没有业务流量的间隙，临时调小租户级配置项 `partition_balance_schedule_interval` 到 `30s`，然后观察 `CDB_OB_BALANCE_JOBS_HISTORY`、`CDB_OB_BALANCE_JOBS` 两个视图，如果长时间没有新任务产生，且通过上述分区均衡的判断方法确认达到分区均衡后，再将配置项的值调回默认值 `2h` 或者关闭。

由于太频繁的均衡容易影响业务，租户级配置项 `partition_balance_schedule_interval` 的值建议不要长期保持在小于 `2h`。
