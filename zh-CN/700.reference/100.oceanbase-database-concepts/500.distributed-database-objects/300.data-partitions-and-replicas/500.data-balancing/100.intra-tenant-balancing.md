...
<h4>说明</h4>
<p><ul>
<li>LS Group 是 LS 的属性，副本分布相同的日志流拥有相同的 LS Group ID。</li>
<li>Unit Group 是集群私有概念，租户的 Unit Group 个数取决于 Unit Number。</li>
...
![示例 1](https://obbusiness-private.oss-cn-shanghai.aliyuncs.com/doc/img/observer-enterprise/V4.2.2/system-principle/example1.png)

示例 2：如下图所示，`PRIMARY_ZONE = 1`，`UNIT_NUM` 从 `1` 变更为 `2`，为保证每个 Unit 都有 LS，分裂出 LS2，带走 1/2 的 Tablet，迁移到新的 Unit 上。

![示例 2](https://obbusiness-private.oss-cn-shanghai.aliyuncs.com/doc/img/observer-enterprise/V4.2.2/system-principle/example2.png)

示例 3：如下图所示，`UNIT_NUM = 1`，`PRIMARY_ZONE` 的第一优先级从 `RANDOM` 变更为 `Z1,Z2`。为保证 Leader 仅在 `PRIMARY_ZONE` 第一优先级的 Zone 上，还需要保证均衡完后，日志流上的 Tablet 的数量均衡。由于 LS3 与其余 LS 在同一个 LS Group 中，可以直接从 `LS3` 上 Transfer 走 1/2 的 Tablet，然后再将 `LS3` 合并到 `LS1` 上，让 `LS1` 承载剩余 1/2 的 Tablet，就可以在减少 LS 数量的同时保持 Tablet 数量均衡。

![示例 3](https://obbusiness-private.oss-cn-shanghai.aliyuncs.com/doc/img/observer-enterprise/V4.2.2/system-principle/example3.png)

### LS Leader 均衡

用户变更租户的 `PRIMARY_ZONE` 后，负载均衡后台线程会根据 `PRIMARY_ZONE` 的第一优先级动态调整 LS 数量，之后再对 LS 进行切主，保证第一优先级的各 Zone 内均有且仅有一个 LS Leader。

如下图所示，`UNIT_NUM = 1`，`PRIMARY_ZONE` 的第一优先级从 `Z1` 变更为 `Z1,Z2`，为保证 `PRIMARY_ZONE` 第一优先级的所有 Zone 上都有 Leader，同一个 Unit 里分裂出新的 LS，然后将新 LS 的 Leader 切到 `Z2`。

![Primary Zone 变更示例](https://obbusiness-private.oss-cn-shanghai.aliyuncs.com/doc/img/observer-enterprise/V4.2.2/system-principle/change_first_level_primary_zone_num.png)
