...
用户对租户执行 `PRIMARY_ZONE` 变更操作后，负载均衡后台线程会根据 `PRIMARY_ZONE` 的第一优先级动态调整 LS 数量，之后再对 LS 进行切主，保证第一优先级的各 Zone 内均有且仅有一个 LS Leader。LS Leader 均衡会在 LS 数量均衡完毕后立即触发。

如下图所示，`UNIT_NUM = 1`，`PRIMARY_ZONE` 的第一优先级从 `Z1` 修改为 `Z1,Z2`，为保证 `PRIMARY_ZONE` 第一优先级的所有 Zone 上都有 Leader，同一个 Unit 里分裂出新的 LS，然后将新 LS 的 Leader 切到 `Z2`。
