...
* 一级分区表：一级分区按照 Round Robin 算法，平均分布到用户的所有 LS 上。

* 二级分区表：每个一级分区下所有二级分区按照 Round Robin 算法，平均分布到用户的所有 LS 上。
...
* 示例 3：创建一个二级分区表 `tt8`。
    ...
    | table_name | partition_name | subpartition_name | ls_id | zone |
    +------------+----------------+-------------------+-------+------+
    | tt8        | p0             | p0sp0             |  1003 | z1   |
    | tt8        | p0             | p0sp1             |  1003 | z2   |
    | tt8        | p0             | p0sp2             |  1001 | z3   |
    | tt8        | p0             | p0sp3             |  1001 | z1   |
    | tt8        | p0             | p0sp4             |  1002 | z2   |
    | tt8        | p0             | p0sp5             |  1002 | z3   |
    | tt8        | p1             | p1sp0             |  1001 | z2   |
    | tt8        | p1             | p1sp1             |  1001 | z3   |
    | tt8        | p1             | p1sp2             |  1002 | z1   |
    | tt8        | p1             | p1sp3             |  1002 | z2   |
    | tt8        | p1             | p1sp4             |  1003 | z3   |
    | tt8        | p1             | p1sp5             |  1003 | z1   |
    +------------+----------------+-------------------+-------+------+
    12 rows in set
    ```

**V4.2.0（含）~V4.2.2（含）之间的版本**

* 非分区表：新建分区分配到分区数最少的 LS 上。

* 一级分区表：一级分区按序切分平均分布到用户的所有 LS 上。

* 二级分区表：每个一级分区下所有二级分区按序切分平均分布用户的所有 LS 上。

下面通过几个示例来进行说明。

* 示例 4：在 MySQL 模式下分别创建了 `tt1`、`tt2`、`tt3`、`tt4` 等 4 张非分区表。
