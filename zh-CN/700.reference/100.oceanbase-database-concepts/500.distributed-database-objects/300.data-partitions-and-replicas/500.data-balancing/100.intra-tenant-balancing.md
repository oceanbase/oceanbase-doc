...
示例 2：如下图所示，`UNIT_NUM = 1`，`PRIMARY_ZONE` 的第一优先级从 `RANDOM` 修改为 `Z1,Z2`，为保证 LS 的 Leader 仅在 Primary Zone 第一优先级的 Zone 上，同时还需要保证均衡完后日志流上的 Tablet 的个数均衡，系统会先将 `LS3` 分裂出 `LS4` 并带走 1/2 的 Tablet，然后把 `LS3` 合并到 `LS1`，`LS4` 合并到 `LS2`。
...
用户对租户执行 `PRIMARY_ZONE` 变更操作后，负载均衡后台线程会根据 `PRIMARY_ZONE` 的第一优先级动态调整 LS 数量，之后再对 LS 进行切主，保证第一优先级的各 Zone 内均有且仅有一个 LS Leader。

如下图所示，`UNIT_NUM = 1`，`PRIMARY_ZONE` 的第一优先级从 `Z1;Z2;Z3` 修改为 `Z1,Z2,Z3`，为保证 `PRIMARY_ZONE` 第一优先级的所有 Zone 上都有 Leader，同一个 Unit 里分裂出新的 LS，然后将新 LS 的 Leader 切到 `Z2`。
