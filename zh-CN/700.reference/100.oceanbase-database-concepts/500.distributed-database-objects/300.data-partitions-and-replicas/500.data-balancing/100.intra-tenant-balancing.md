...
在之前的版本中，OceanBase 数据库仅支持租户的同构 Zone 模式，同构 Zone 模式下，要求租户下各 Zone 的 Unit 数量必须相同。从 V4.2.5 BP5 （仅针对 V4.2.5 版本）版本开始，OceanBase 数据库支持了租户的异构 Zone 模式，异构 Zone 模式下，租户下各 Zone 的 Unit 数量可以不同，但限制一个租户最多只有两种 `UNIT_NUM`。
