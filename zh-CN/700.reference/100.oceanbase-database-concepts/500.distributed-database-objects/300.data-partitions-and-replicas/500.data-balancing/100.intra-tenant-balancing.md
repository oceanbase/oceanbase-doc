...
<h4>注意</h4>
<p>分区均衡仅针对用户表主表，并且 LS 均衡的优先级大于分区均衡。</p>
</main>
...
当用户对租户执行 `UNIT_NUM` 变更、修改 `PRIMARY_ZONE` 的第一优先级、修改 Locality（影响 `PRIMARY_ZONE`）等操作时，负载均衡模块的后台线程会立即通过 LS 分裂、LS 合并、LS 副本迁移等动作，变更 LS 的数量，以满足用户修改后的租户状态，即每个租户的 Unit 都有 LS 副本，`PRIMARY_ZONE` 第一优先级中每个 Zone 都有且仅有一个 LS Leader。

LS 数量的计算公式如下：

`LS_num = unit_num * first_level_primary_zone_num`

下面看两个简单的示例。

示例 1：如下图所示，`PRIMARY_ZONE = Z1`，`UNIT_NUM` 从 1 修改为 2。为保证每个 Unit 都有 LS，分裂出 `LS2`，带走 1/2 的 Tablet，迁移到新的 Unit 上。

![Unit Number 变更示例](https://obbusiness-private.oss-cn-shanghai.aliyuncs.com/doc/img/observer-enterprise/V4.2.2/system-principle/unit_number_increase.png)

示例 2：如下图所示，`unit_num = 1`，`PRIMARY_ZONE` 的第一优先级从 `RANDOM` 修改为 `Z1,Z2`，为保证 LS 的 Leader 仅在 Primary Zone 第一优先级的 Zone 上，系统将 `LS3` 合并到 `LS1` 上，所有 `LS3` 上的 Tablet 均 Transfer 到 `LS1` 上。

![Primary Zone 变更示例](https://obbusiness-private.oss-cn-shanghai.aliyuncs.com/doc/img/observer-enterprise/V4.2.2/system-principle/primary_zone_reduced.png)

### LS Leader 均衡

用户对租户执行 `PRIMARY_ZONE` 变更后，负载均衡后台线程会根据 `PRIMARY_ZONE` 的第一优先级动态调整 LS 数量，之后再对 LS 进行切主，保证第一优先级的各 Zone 内均有且只有一个 LS Leader。LS Leader 均衡会在 LS 数量均衡完毕后立即触发。

如下图所示，`UNIT_NUM = 1`，`PRIMARY_ZONE` 的第一优先级从 `Z1` 修改为 `Z1,Z2`，为保证 `PRIMARY_ZONE` 第一优先级的 Zone 上都有 Leader，同一个 Unit 里分裂出新 LS，然后将新 LS 的 Leader 切到 `Z2`。

![Primary Zone 增加示例](https://obbusiness-private.oss-cn-shanghai.aliyuncs.com/doc/img/observer-enterprise/V4.2.2/system-principle/primary_zone_increase.png)

## 建表时分区的分配

当用户创建用户表时，OceanBase 数据库会采用一套均衡分配策略将分区打散或聚合到各个用户日志流上，以保证各个日志流上的分区相对均衡。

### 指定 Table Group 的用户表

用户可以通过指定 Table Group，灵活配置不同表之间的聚集、打散规则。在创建用户表时，如果指定了对应 Sharding 属性的 Table Group，新建用户表的分区会按照 Table Group 的规则分配到对应用户的 LS 上。Table Group 的规则如下表所示。
...
在 LS 均衡的基础上，负载均衡模块会通过 Transfer，将分区 Tablet 打散或聚合在不同的 LS 上，达到租户下的分区均衡。分区均衡时间间隔通过租户级配置项 `partition_balance_schedule_interval` 控制，默认 2h 执行一次，每次执行仅运行一种分区均衡策略。
...
* `duplicate_scope` 对齐
  ...
  <h4>说明</h4>
  <p>当前版本暂不支持 <code>duplicate_scope</code> 属性的变更。</p>
  </main>

  复制表只能存在于广播日志流上。当用户修改 `duplicate_scope` 属性后，需要等待分区均衡执行完成后才能按照预期属性使用。可以临时调小租户级配置项 `partition_balance_schedule_interval` 的值以便快速对齐。
...
|-----------|-------------------------------------------|---------------------------------------------------------|
| 非分区表   | 租户下所有非分区表是一个均衡组。             | 租户下所有非分区表，平均分布到所有用户 LS 上（数量偏差不大于 1）。|
| 一级分区表 | 每张表下所有一级分区是一个均衡组。           | 以每张表为单位，一级分区平均分布到所有用户 LS 上。|
| 二级分区表 | 单张表的每个一级分区下所有二级分区是一个均衡组。| 单张表每个一级分区下所有二级分区，平均分布到所有用户 LS 上。|

均衡阶段，系统先对所有均衡组进行组内均衡（组内分区平均分布），然后在组内均衡的基础上，从分区数最多的 LS 上 Transfer 走一部分分区，进行组间均衡，从而实现分区数量均衡。

例如，现有 4 张表的所有分区均在 `LS1` 上，分为 3 个均衡组：

* 均衡组 1：非分区表 `non_part_t1、non_part_t1`

* 均衡组 2：一级分区表的 2 个分区 `part_one_t3_p0`、`part_one_t3_p1`

* 均衡组 3：二级分区表的 4 个分区 `part_two_t4_p0s0`、`part_two_t4_p0s1`、`part_two_t4_p1s0`、`part_two_t4_p0s1`
...
例如，新增一个 `sharding = 'NONE'` 的 Table Group `tg1`，其中包含 `non_part_t5_in_tg1`、`non_part_t6_in_tg1`、`non_part_t7_in_tg1`、`non_part_t8_in_tg1` 等四个非分区表。由于 `tg1` 中的所有表必须绑定到一起，均衡后表的分区分布为 4-3-5。
...
分区均衡是用户表主表的均衡，查询时需要注意指定 `table_type='USER TABLE'`。分区磁盘均衡主要关注视图 `CDB_OB_TABLET_REPLICAS` 中的 `data_size` 字段。
...
您可以在没有业务流量的间隙，临时调小租户级配置项 `partition_balance_schedule_interval` 到 `30s`，然后观察 `CDB_OB_BALANCE_JOBS_HISTORY`、`CDB_OB_BALANCE_JOBS` 两个视图，如果长时间没有新任务产生，且通过上述分区均衡的判断方法确认分区均衡后，再将配置项的值调回默认值 2h 或者关闭。

租户级配置项 `partition_balance_schedule_interval` 的值建议不要长期保持在小于 2h，太频繁的均衡容易影响业务。
