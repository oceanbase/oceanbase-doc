...
<h4>说明</h4>
<p>尽管原生 S3 及兼容 S3 协议的对象存储（OBS、GCS 等）与 OSS、NFS、COS 等备份介质的日志归档目录结构不相同，但将原生 S3、OBS、GCS 上的备份文件通过跨云拷贝到 OSS、NFS、COS 等备份介质上后，仍然支持恢复。例如，将原生 S3 归档的数据拷贝到 OSS 上，使用该 OSS 路径，仍然可以成功恢复。</p>
</main>
