...
对于 OSS、NFS 等备份介质，日志归档功能在归档目的地创建的目录以及各目录下保存的文件类型如下所示。
...
<h4>说明</h4>
<p>尽管 AWS S3 及以兼容 S3 协议访问的备份介质与 OSS、NFS 等备份介质的日志归档目录结构不相同，但将这些备份介质上的备份文件通过跨云拷贝到 OSS、NFS 等备份介质上后，仍然支持恢复。例如，将 AWS S3 归档的数据拷贝到 OSS 上，使用该 OSS 路径，仍然可以成功恢复。</p>
</main>
