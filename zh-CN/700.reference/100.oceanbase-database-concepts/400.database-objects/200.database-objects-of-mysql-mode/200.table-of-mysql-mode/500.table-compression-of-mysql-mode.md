...
OceanBase 数据库允许在创建表或修改表的语句中使用 `KEY_BLOCK_SIZE` 参数来指示索引键块的大小（单位为字节），这在语法上是支持的。该参数的目的是用于控制表或索引的压缩水平。然而，尽管该参数可以被包括在 SQL 语句中，OceanBase 数据库实际上并不实施与 `KEY_BLOCK_SIZE` 相关的压缩特性，因此设置此参数不会影响数据库的行为或性能。
