...
* JSON 数据长度不能超过 `LONGTEXT` 的长度。每个 JSON 对象或数组的默认最大深度为 100，支持配置范围为 `[100,1024]`。
...
```sql
ALTER system SET json_document_max_depth = 200;
```

修改最大深度后同样支持包括解析 JSON、构建 JSON、JSON 计算、JSON 写入存储以及读取 JSON 数据等相关操作。不推荐使用嵌套层次过深的 JSON 数据结构，因为一方面深层嵌套的 JSON 数据可读性和可维护性降低，另外一方面解析和序列化深层嵌套的 JSON 数据通常会消耗更多时间和计算资源，性能效率低下。