...
设置 `ob_route_policy = column_store_only` 时，将按照以下策略进行路由：

<main id="notice" type='notice'>
<h4>注意</h4>
<p>该选项需要保证集群内有列存副本的情况下才能使用，否则所有的查询均会报错。</p>
</main>

* 如果 Session 上没有设置弱读，则报错 `4007`，提示列存副本不支持强读。

* 如果查询为 DML 查询，则报错 `4007`，提示列存副本不支持写查询。

* 如果当前找不到列存副本，则报错 `NO_REPLICA_VALID`，并且不重试。

### 读写分离部署注意事项

不能仅仅在异地 Region 设置只读 Zone，否则可能导致每次请求都与 OBServer 节点进行建链，耗时增加。

原因是 ODP 内部有个策略，如果 Session 上设置了弱读，并且这个租户有只读 Zone，会判断上一次访问的 Server 是不是处于只读 Zone。如果不是的话，会关掉这个 Server Session。同时，根据上面的路由策略，如果同 Region 下是读写 Zone，异地是只读 Zone，ODP 始终会路由到同 Region 下的读写 Zone。因此，这样就可能会导致不断地与同 Region 下的读写 Zone 的 Server 建连，然后关闭链接，下次请求又继续重复。
