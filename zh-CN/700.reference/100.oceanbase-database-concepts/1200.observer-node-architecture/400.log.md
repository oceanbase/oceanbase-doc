...
Crash 日志不同于其他日志，该日志只有在系统或应用程序崩溃时生成的日志文件，它记录了崩溃发生时的详细信息。
...
* Crash 日志参数信息
  ...
  | -----|-----|
  | IP   | 当前正在执行的指令的地址 |
  | RBP  | 标识栈帧的基址，用于定位堆栈的区域 |
  | sig  | 信号，表示触发崩溃的特定信号 |
  | sig_code   | 引发信号的额外信息，提供有关信号类型和原因的详细信息 |
  | sig_addr   | 引发信号的内存地址，指示崩溃发生的位置 |
  | RLIMIT_CORE| 程序能产生的核心转储（Core Dump）文件的最大大小限制 |
  | timestamp  | 记录 Crash 事件的时间戳，标识崩溃发生的确切时间 |
  | tid        | 线程 ID，标识触发 Crash 的线程的唯一标识符 |
  | tname      | 线程名，表示触发 Crash 的线程的名称 |
  | trace_id   | Crash 事件的跟踪 ID，例如 YXXXXXXXX-000XXXXXXXXXX-0-0 |
  | extra_info | 额外信息，可能包括与崩溃相关的其他调试信息或上下文信息 |
  | lbt        | Crash 线程的堆栈信息，记录了崩溃发生时的函数调用堆栈 |
  | SQL_ID     | Crash 线程正在执行的 SQL 语句的 SQL ID |
  | SQL_STRING | Crash 线程正在执行的 SQL 语句的字符串表示|
...
* **enable_syslog_recycle**
  ...
  开启后将会自动删除多余的日志，有关 `enable_syslog_recycle` 的详细信息请参见 [enable_syslog_recycle](../../800.configuration-items-and-system-variables/100.system-configuration-items/300.cluster-level-configuration-items/9200.enable_syslog_recycle.md)。
