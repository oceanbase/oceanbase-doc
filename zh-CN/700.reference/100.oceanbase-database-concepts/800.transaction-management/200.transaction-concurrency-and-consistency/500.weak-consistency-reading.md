...
* 隔离级别为 `Repeatable Read` 和 `Serializable` 时执行弱读请求也生效。
...
无论采用单调读还是非单调读，V3.x 及之前版本均可保证同一事务在单个分区内数据修改的原子性；V4.x 及以上版本均可保证同一事务在单个日志流内数据修改的原子性。

原子性保证遵循以下原则：当读快照小于单个分区或日志流维护的最大安全可读版本号时，读取到的数据一定具备原子性。
