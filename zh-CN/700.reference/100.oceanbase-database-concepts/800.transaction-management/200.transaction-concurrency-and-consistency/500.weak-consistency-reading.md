...
对于 4.x 版本，`max_stale_time_for_weak_consistency` 和 `weak_read_version_refresh_interval` 两个配置项的功能保持与之前的版本一致。解除了对 `enable_monotonic_weak_read` 的依赖，弱读默认就是租户级别的单调读，支持关闭。
