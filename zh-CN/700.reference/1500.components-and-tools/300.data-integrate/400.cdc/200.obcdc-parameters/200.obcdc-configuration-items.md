...
| 配置项       | 含义      | 默认值 | 可否动态变更 |  备注       |
|------------|-----------|-----|--------|-------------------------|
| enable_output_virtual_generated_column | 控制是否输出虚拟生成列  | 0 | 否  | 需与 `enable_output_by_table_def` 配合使用。<main id="notice" type='explain'><h4>说明</h4><p>该配置项从 V4.2.5 版本开始引入，即自 OceanBase 数据库 V4.2.5 版本起，cdc 支持输出虚拟生成列。</p></main>  |
