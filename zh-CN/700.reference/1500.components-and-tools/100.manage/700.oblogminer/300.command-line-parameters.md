...
使用 OceanBase LogMiner 时，可通过配置不同的命令行参数实现 OceanBase LogMiner 的不同功能。OceanBase LogMiner 支持的命令行参数具体如下表，详细的使用操作可参见 [使用示例](400.oblogminer-usage-examples.md)。
...
| -s    | --start-time     | 是      | 字符串   | NULL     | 指定 OceanBase LogMiner 分析的日志的开始时间。需配置为 datatime 格式或微秒时间戳格式，如 `2024-01-09 16:23:00` 或 `1706164790844000`。开始时间需满足 OBCDC 的要求，过早的开始时间会导致 OceanBase LogMiner 启动失败。具体限制可参见 [OBCDC 常见问题](../../300.data-integrate/400.cdc/200.obcdc/300.faq-of-obcdc.md)。 |
| -e    | --end-time       | 否      | 字符串   | 默认为当前时间 | 指定 OceanBase LogMiner 分析的日志的结束时间。需配置为 datatime 格式或微秒时间戳格式，如 `2024-01-09 16:23:00` 或 `1706164790844000`。 |
...
| -L    | --log_level      | 否      | 字符串   | ALL.\*:INFO;PALF.\*:WARN;SHARE.SCHEMA:WARN | OceanBase LogMiner 运行时日志级别，OceanBase 数据库支持的日志级别可参见 [日志级别](../../../../600.manage/800.logging/200.log-level.md)。 |
