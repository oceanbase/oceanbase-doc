...
<main id="notice" type='explain'>
  <h4>说明</h4>
  <p>本文介绍的是社区版 oblogminer 的安装方法，企业版 oblogminer 会在部署企业版 OceanBase 数据库时自动安装（<code>oceanbase/bin</code> 下）。</p>
</main>

## 通过 YUM 命令安装

您可直接通过 YUM 命令安装 OceanBase 的工具集成包（OceanBase Utils），从而完成 oblogminer 的安装。通过 YUM 命令安装时，oblogminer 会被安装到 `/usr/bin/` 目录下。
