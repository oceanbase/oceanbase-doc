...
1. 通过如下两种方法设置 `max_syslog_file_count` 系统配置项：
   ...
   * 启动 OBServer 时，在 `-o` 参数中指定 `max_syslog_file_count` 的值。
     ...
     详细操作请参见 [添加 OBServer](../../100.manage-clusters/500.manage-observer/100.add-observer.md) 中的 **启动 OBServer** 。
...
2. （可选）开启系统日志自动回收功能。
  ...
  <main id="notice" type='explain'>
    <h4>说明</h4>
    <p>仅当 <code>max_syslog_file_count</code> 配置项的值设置为非 0 正数时，系统日志自动回收功能才会生效。有关 <code>max_syslog_file_count</code> 和 <code>enable_syslog_recycle</code> 配置项的详细介绍，请参见 <a href="../../../../500.system-reference/100.system-configuration-items/100.system-configuration-items-overview.md">系统配置项</a> 。</p>
  </main>
