...
OceanBase 数据库的当前版本中，由于租户工作线程和大部分后台线程均根据租户来区分，网络线程为共享线程，用户可以通过配置 cgroups 来控制租户 CPU 的占用。在开始配置 cgroups，建议您先了解 cgroups 的概念。有关 cgroups 的介绍信息，参见 [资源隔离概述](../100.resource-isolation-overview.md)。
