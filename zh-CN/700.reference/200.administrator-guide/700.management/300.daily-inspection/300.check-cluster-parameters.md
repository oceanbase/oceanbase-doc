...
| [resource_hard_limit](../../../500.system-reference/100.system-configuration-items/300.cluster-level-configuration-items/16800.resource_hard_limit.md) | 用于分配 Unit 。在分配 CPU 和内存等资源时，资源总量是实际数量乘以该百分比的值。最终 Server 资源占比不能超过 resource-hard-limit。取值范围 \[100, 10000\]。   | 100        |
...
* 如果您正在使用 OceanBase 云平台（OceanBase Cloud Platform，OCP）管理 OceanBase 集群，那么您也可以通过 OCP 来进行参数的统一管理，包括查看参数、修改参数以及查看参数修改历史等操作，详细信息请参见 [查看参数列表](https://www.oceanbase.com/docs/enterprise-oceanbase-ocp-cn-10000000001542055)。
