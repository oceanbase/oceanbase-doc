...
通过 obd.sh 执行 mysqltest 测试时需使用通过 obd.sh 部署的 OceanBase 数据库，本文通过示例介绍如何从编译源码开始使用 obd.sh 部署 OceanBase 数据库并执行 mysqltest 测试。

## 背景介绍

为简化开发者的操作步骤，降低开发者的理解成本，我们将 OBD 的一些命令封装到 obd.sh 脚本中，并将该脚本存放在 OceanBase 源码 [oceanbase/tools/deploy](https://github.com/oceanbase/oceanbase/blob/master/tools/deploy/obd.sh) 目录下。
...
* proxy 模式：通过 ODP 连接进行 mysqltest 测试，如使用配置文件 distributed-with-proxy.yaml 部署集群后执行测试。
...
2. （可选）修改配置文件
   ...
   | mysql_port | 必选      | 设置 SQL 服务协议端口号，OBClient 直连该端口可以访问该节点。 |
...
3. 部署 OceanBase 数据库
   ...
   其中，`-c` 用于指定部署所需的配置文件路径，`-n` 用于指定部署集群名称。

### 步骤三：执行 mysqltest 测试

您可选择全量测试，也可指定 case 或 suite 进行测试。
...
执行 mysqltest 测试命令时可根据实际情况配置一些参数，各参数说明见下表：
