...
=======
| --rs, --rs_list | 否 | string | 每个zone 的 root server 是 zone 中的第一个server。 | rs_list 是 root server列表。 默认每个zone的 root server 是zone 中的第一个server。格式为 observer_ip:rpc_port:mysql_port;observer_ip:rpc_port:mysql_port，每个 rootServer 之间用 `;` 分隔， --rs 整体需要用单引号包裹。 例如 `--rs 'xx.xxx.xx.x1:2882:2881;xx.xxx.xx.x2:2882:2881'`。 |
>>>>>>> 10a62d6b25ec4acc948c04aa152039641cc31d89
...
=======
| -s, --observer | 否 | string | 无 | 启动指定的 OBShell 运维的 observer，格式为 obshell_ip:obshell_port，多个中间使用 `,` 分隔，例如 `-s xx.xxx.xx.x1:2886,xx.xxx.xx.x2:2886`。 |
>>>>>>> 10a62d6b25ec4acc948c04aa152039641cc31d89
