...
# example
${home_path}/bin/obshell cluster join -z zone1 -s 10.10.10.1
```
...
# example
${home_path}/bin/obshell cluster remove -s 10.10.10.1
```
...
`-d`/`--show_detail` 非必选项，用于控制是否展示集群的详细信息，无需传值。
...
使用该命令可以将 OceanBase 数据库或者 OBShell 升级到指定版本。

```shell
${home_path}/bin/obshell cluster upgrade -d [-m] [-V] [-t] [-y]

# example
${home_path}/bin/obshell cluster upgrade -d /home/admin/oceanbase/upgrade/ -V 4.2.2.0-20231224224959
```
...
使用该命令可以将当前节点扩容到指定的集群中。

```shell
${home_path}/bin/obshell cluster scale-out -z -s [--rp] [-d] [-r] [-p] [-P] [-l] [-o] [-y]

# example
${home_path}/bin/obshell cluster scale-out -z zone1 -s xxx.xxx.xxx.1:2886
```

其中，`home_path` 需指定为绝对路径。选项说明见下表：
