...
```shell
[admin@test001 ~]$ curl -H 'X-OCS-Header:${encrypted_header}' -X POST -F "file=@/data/upgrade/obshell-4.2.2.0-xxx.el7.x86_64.rpm" http://10.10.10.1:2886/api/v1/upgrade/package
```

### 步骤 2：调用升级检查接口（可选）

调用任一 OBShell 的 `/api/v1/agent/upgrade/check` 进行升级检查。包括目标版本的包是否上传、升级所需制品是否齐全等。接口详见 [OBShell 升级检查](../400.obshell-api-reference/1000.agent-upgrade-check.md)。该接口会创建一个异步任务，查看任务进度参考 [获取任务的详细信息](../400.obshell-api-reference/2000.get-dag-detail.md)。等待任务完成，则升级检查完成。

```shell
[admin@test001 ~]$ curl -H "Content-Type: application/json" -H 'X-OCS-Header:${encrypted_header}' -X POST -d '{"version":"4.2.2.0", "release": "xxx.el7"}' http://10.10.10.1:2886/api/v1/agent/upgrade/check
```

### 步骤 3：调用升级接口

调用任一 OBShell 的 `/api/v1/agent/upgrade` 进行升级。接口详见 [OBShell 升级](../400.obshell-api-reference/1200.upgrade-agent.md)。该接口会创建一个异步运维任务，查看任务进度参考 [获取任务的详细信息](../400.obshell-api-reference/2000.get-dag-detail.md)。等待任务完成，则升级完成。

```shell
[admin@test001 ~]$ curl -H "Content-Type: application/json" -H 'X-OCS-Header:${encrypted_header}' -X POST -d '{"version":"4.2.2.0", "release": "xxx.el7"}' http://10.10.10.1:2886/api/v1/agent/upgrade
```
