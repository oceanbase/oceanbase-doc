...
<main id="notice" type='explain'>
  ...
  <li>
  <p>OBShell 会对被调用的 API 进行安全校验。因此，您在每次调用 API 之前都需先参考 <a href='../400.obshell-api-reference/200.api-hybrid-encryption.md'>API 混合加密</a> 一文对请求进行加密，并在 curl 命令中配置加密后的请求头部（<code>${encrypted_header}</code>）和请求体（<code>${encrypted_body}</code>）。</p>
  </li>
  </ul>
</main>
...
```shell
[admin@test001 ~]$ curl -H "Content-Type: application/json" -H 'X-OCS-Header:${encrypted_header}' -X POST -d ${encrypted_body} http://10.10.10.1:2886/api/v1/agent/join
```
...
```shell
[admin@test002 ~]$ curl -H "Content-Type: application/json" -H 'X-OCS-Header:${encrypted_header}' -X POST -d ${encrypted_body} http://10.10.10.2:2886/api/v1/agent/join
```

在 `10.10.10.3` 节点上执行如下命令：

```shell
[admin@test003 ~]$ curl -H "Content-Type: application/json" -H 'X-OCS-Header:${encrypted_header}' -X POST -d ${encrypted_body} http://10.10.10.3:2886/api/v1/agent/join
```
...
```shell
[admin@test001 ~]$ curl -H "Content-Type: application/json" -H 'X-OCS-Header:${encrypted_header}' -X PUT -d ${encrypted_body} http://10.10.10.1:2886/api/v1/obcluster/config
```
...
```shell
[admin@test001 ~]$ curl -H "Content-Type: application/json" -H 'X-OCS-Header:${encrypted_header}' -X PUT -d ${encrypted_body} http://10.10.10.1:2886/api/v1/observer/config
```
