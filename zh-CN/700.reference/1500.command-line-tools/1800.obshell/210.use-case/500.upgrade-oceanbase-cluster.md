...
- OceanBase 集群的所有 OBServer 节点、OBShell 都正常运行中。
...
```shell
[admin@test001 ~]$ curl -H 'X-OCS-Header:${encrypted_header}' -X POST -F "file=@/data/upgrade/oceanbase-ce-4.2.2.0-xxx.el7.x86_64.rpm" http://10.10.10.1:2886/api/v1/upgrade/package
```

```bash
[admin@test001 ~]$ curl -H 'X-OCS-Header:${encrypted_header}' -X POST -F "file=@/data/upgrade/oceanbase-ce-libs-4.2.2.0-xxx.el7.x86_64.rpm" http://10.10.10.1:2886/api/v1/upgrade/package
```
...
```shell
[admin@test001 ~]$ curl -H "Content-Type: application/json" -H 'X-OCS-Header:${encrypted_header}' -X POST -d '{"version":"4.2.2.0", "release": "xxx.el7"}' http://10.10.10.1:2886/api/v1/ob/upgrade/check
```
...
```shell
[admin@test001 ~]$ curl -H "Content-Type: application/json" -H 'X-OCS-Header:${encrypted_header}' -X POST -d '{"version":"4.2.2.0", "release": "xxx.el7", "mode": "rolling"}' http://10.10.10.1:2886/api/v1/ob/upgrade
```
