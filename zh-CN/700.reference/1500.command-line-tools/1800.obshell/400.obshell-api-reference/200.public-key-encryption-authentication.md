...
OBShell 服务端在接受到您的请求后，会解析出 root@sys 的密码，并加以校验。如果校验通过，则继续处理请求，否则将报错。目前仅支持 HTTP Header 中包含认证字符串。
...
resp = req.get('http://100.88.144.177:2886/api/v1/secret').text
...
func genAuth() (err error) {
    publicKey, err := getPublicKey("http://11.158.55.88:2886/api/v1/secret")
    if err != nil {
        return
    }
...
public class Main {
    public static void main(String[] args) {
        try {
            // 获取公钥
            String pkcs1PublicKeyStr = getPublicKey("http://100.88.144.177:9001/api/v1/secret");
