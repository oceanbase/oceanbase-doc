...
| 参数 | 类型 | 必选 | 示例值 | 说明 |
| --- | --- | --- | --- | --- |
| target | []string | 否 | 示例值：["xxx.xxx.12886", "xxx.xxx.2:2886"] 或者 ["zone1","zone2"] | 当 type 为 SERVER 时，设置为 observer 的列表。当 type 为 ZONE 时，设置为 zone 的列表。当 type 为 GLOBAL 时，不填 。|
| type | String | 是 | "SERVER" | 配置的生效级别。<br>枚举值：GLOBAL，ZONE，SERVER。 |
