...
您可以使用 curl 工具调用开放 API 的接口。下面的例子中，访问 OBShell 应用的地址为 `10.10.10.1`，端口为 `2886`，访问的接口为删除指定节点。其中 `${encrypted_header}` 指加密后的 Header 内容，`${encrypted_body}` 指请求体内容，具体生成方式可参见 [API 混合加密](200.api-hybrid-encryption.md)。
