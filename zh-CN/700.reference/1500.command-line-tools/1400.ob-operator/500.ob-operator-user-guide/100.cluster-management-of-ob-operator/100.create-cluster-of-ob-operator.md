...
本文主要介绍通过 ob-operator 进行 OceanBase 数据库集群的创建。
...
其中 node_name 可以通过命令 `kubectl get node` 获取。
...
OceanBase 数据库集群通过 yaml 配置文件进行定义，可参考 ob-operator 提供的配置文件进行自定义的修改，通过如下命令获取 OceanBase 数据库的配置文件 obcluster.yaml。
...
通过以下命令查看 OceanBase 集群是否部署成功。

```shell
kubectl get obcluster ob-test -o yaml -n obcluster
```

当返回的结果中，status 为 ready 状态时表示部署成功。
...
推荐通过 ODP 来连接 OceanBase，在部署完成 OceanBase 数据库和 ODP 后，您可通过以下命令来获取 ODP 的 service 连接地址。
