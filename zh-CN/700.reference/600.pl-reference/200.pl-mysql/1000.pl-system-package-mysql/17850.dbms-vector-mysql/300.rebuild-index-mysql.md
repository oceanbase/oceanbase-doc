...
| table_name | 表名。 |
| idx_vector_col | 向量列名。 |
| delta_rate_threshold | 增量数据的比例。默认值为 `0.2`，取值范围为 `[0, 1]`。 |
| idx_organization | 索引类型。默认 `NULL`，不支持自定义参数。 |
| idx_distance_metrics | 距离类型。默认 `NULL`，不支持自定义参数。 |
...
| idx_parallel_creation | 并行构建索引的并行度。 |
