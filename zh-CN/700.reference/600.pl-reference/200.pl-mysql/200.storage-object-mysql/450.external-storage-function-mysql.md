...
您可以编译并打包下述代码为 `MyAdd.java` 后，然后可以在 OBServer 中创建 Java UDTF，示例如下：
...
* `symbol`：表示指定的入口类名。
* `type`：表示 Java UDF 的类型，目前仅支持 `odpsjar`。
...
2. 创建 Java UDF
    ...
    执行如下 SQL 语句创建名为 `my_add` 的 Java UDTF。
...
2. 创建 Java UDAF
    ...
    执行如下 SQL 语句创建名为 `my_add` 的 Java UDAF。
