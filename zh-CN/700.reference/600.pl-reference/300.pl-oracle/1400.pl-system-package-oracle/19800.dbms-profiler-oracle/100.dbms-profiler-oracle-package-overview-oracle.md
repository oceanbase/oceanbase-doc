...
6. 查看性能分析运行信息:
    ...
    |RELATED_RUN|NUMBER|相关运行的 Runid|
    |RUN_OWNER|VARCHAR2(256)|开始运行的用户|
    |RUN_DATE|DATE|运行开始时间|
    |RUN_COMMENT|VARCHAR2(2047)|注释|
    |RUN_TOTAL_TIME|NUMBER|本次运行已用时间（纳秒）|
    |RUN_SYSTEM_INFO|VARCHAR2(2047)|目前未使用|
...
7. 查看被分析单元的信息:
    ...
    |字段名称|类型|含义|
    |-----|-----|------|
    |RUNID|NUMBER|来自 `plsql_profiler_runnumber` 的唯一运行标识符|
    |UNIT_NUMBER|NUMBER|主键，内部生成的库单元|
    |UNIT_TYPE|VARCHAR2(256)|库单元类型|
    |UNIT_OWNER|VARCHAR2(256)|库单元所有者|
    |UNIT_NAME|VARCHAR2(256)|库单元名称|
    |UNIT_TIMESTAMP|DATE|单元时间戳|
    |TOTAL_TIME|NUMBER|单元的总执行时间（单位：纳秒）|
    |SPARE1|NUMBER|未使用|
    |SPARE2|NUMBER|未使用|
