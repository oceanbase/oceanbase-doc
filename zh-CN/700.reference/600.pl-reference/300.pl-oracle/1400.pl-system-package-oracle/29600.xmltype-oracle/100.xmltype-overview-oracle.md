...
| **子程序** | **描述** |
| --- | --- |
| [CREATEXML](200.getxml-oracle.md) | 用于创建和返回 XMLType 实例的静态函数。 |
| [GETCLOBVAL](300.getclobval-oracle.md) | 以 `CLOB` 形式返回 XMLType 实例的值。 |
| [GETSTRINGVAL](400.getstringval-oracle.md) | 以字符串形式返回 XMLType 实例的值。 |
| [TRANSFORM](500.transform-oracle.md) | 获取一个 XMLType 实例和一个关联的样式表（也是一个 XMLType 实例），应用该样式表并将结果作为 XML 返回。 |
| [XMLTYPE](600.getxmltype-oracle.md) | 构造 XMLType 数据类型的实例。构造函数可以将 XML 作为 `CLOB`、`VARCHAR2` 或相应的对象类型。 |
| [EXISTSNODE](700.existsnode-oracle.md) | 判断 XML 变量是否存在 Xpath 路径的节点。 |
| [EXTRACT](800.extract-oracle.md) | 与 `EXISTSNODE` 类似，若存在节点，则返回节点的集合。 |
| [TOOBJECT](900.toobject-oracle.md) | 将 XMLType 实例映射到 PL/SQL 对象实例（从 V4.4.2 BP3 开始支持）。 |