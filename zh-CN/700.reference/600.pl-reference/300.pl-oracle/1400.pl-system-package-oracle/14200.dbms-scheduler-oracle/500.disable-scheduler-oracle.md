...
|        参数       |      说明         |
|------------------|--------------------|
| job_name             | 要禁用的作业的名称。多个作业名称可以是逗号分隔的列表。如果指定了作业类名称，则该作业类中的所有作业都将被禁用，该作业类不会被禁用。如果指定了组名，则该组将被禁用，但组成员的启用状态不受影响。 <p>系统内部定义的定时任务如下：</p><ul><li>`SCHEDULED_TRIGGER_PARTITION_BALANCE`：分区均衡任务</li> <li>`SCHEDULED_RECYCLE_SCHEMA_HISTORY`：Schema 多版本历史数据回收任务（对于 V4.4.2 版本，从 V4.4.2 BP1 版本开始支持）</li> <li>`SCHEDULED_RUN_INSPECTION`：Root Service 自检任务（对于 V4.4.2 版本，从 V4.4.2 BP1 版本开始支持）</li> <li>`SCHEDULED_PURGE_RECYCLEBIN`：回收站清理任务（对于 V4.4.2 版本，从 V4.4.2 BP2 版本开始支持）</li></ul>     |
