...
| plsql_function_source | DETERMINISTIC | 表示该函数是确定性的，即对于相同的输入参数总是返回相同的结果。这对于优化查询性能很重要。 |
| plsql_function_source | parallel_enable_clause | 表示该函数可以在并行查询中安全执行。当指定此特性时，函数必须满足确定性、不修改数据库状态、不依赖会话状态等条件。 |
