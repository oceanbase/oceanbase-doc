...
如果该值在从 `from_tz` 转换为 UTC 时超出 `TIMESTAMP` 类型的支持范围，则不会发生转换。有关 `TIMESTAMP` 类型范围详细信息，请参见 [DATE、DATETIME 和 TIMESTAMP 类型](../../../100.basic-elements/100.data-type/300.date-and-time-types-1/200.date-datetime-and-timestamp-types.md)。
