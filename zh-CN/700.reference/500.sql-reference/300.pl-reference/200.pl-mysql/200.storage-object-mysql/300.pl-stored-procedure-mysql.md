...
创建带有 `IN OUT` 或 `OUT` 参数的存储过程时，需要注意下面的事项：

在 OceanBase 数据库 V3.x 版本中，可以直接使用 `CALL` 语句传递字面量（例如 `CALL p1(1);`）来调用存储过程，并不会出错。然而，在 OceanBase 数据库 V4.x 版本中，如果存储过程中包含 `IN OUT` 或 `OUT` 参数，那么必须使用一个变量来接收 `IN OUT` 或 `OUT` 参数的输出值。

假设存在存储过程 p1：

```sql
CREATE PROCEDURE p1(INOUT param INT) AS
BEGIN
  -- 存储过程内部逻辑
END;
```

在 4.x 版本中，若要调用该存储过程，则需要进行如下的优化操作：
...
BEGIN
  v := 1; -- 设置输入值
  CALL p1(v); -- 调用存储过程，并使用变量 v 作为参数
  -- v 现在包含了存储过程 p1 的输出结果
END;
```
