...
在下面的示例中，先创建 order_items 表，它包含了每个订单的条目、数量和价格。

```sql
obclient > CREATE TABLE IF NOT EXISTS order_items (order_id INT NOT NULL, item_id INT NOT NULL, quantity INT NOT NULL, price DECIMAL(10, 2) NOT NULL, PRIMARY KEY (order_id, item_id) );
Query OK, 0 rows affected

obclient > INSERT INTO order_items (order_id, item_id, quantity, price) VALUES (1, 101, 2, 49.99), (1, 102, 1, 19.99), (1, 103, 1, 29.99), (2, 101, 1, 49.99), (2, 104, 1, 9.99);
Query OK, 5 rows affected
```

然后，创建一个名为 CalculateOrderTotal 的存储过程，用于计算基于订单条目的总金额，包括应用税率和折扣。

```sql
obclient > DELIMITER //
obclient > CREATE PROCEDURE CalculateOrderTotal(IN orderID INT, OUT totalAmount DECIMAL(10,2))
    -> BEGIN
    ->     -- 声明变量来存储中间结果
    ...
    ->
    ->     -- 计算订单的小计
    ->     SELECT SUM(quantity * price) INTO subtotal FROM order_items WHERE order_id = orderID;
    ->
    ->     -- 获取适用的税率和折扣率，这里假设税率为 7%，折扣率为 5%
    ->     SET taxRate = 7.00;
    ->     SET discountRate = 5.00;
    ->
    ->     -- 计算总金额
    ->     SET totalAmount = subtotal + (subtotal * taxRate / 100) - (subtotal * discountRate / 100);
    -> END //
IMITER ;
Query OK, 0 rows affected
obclient > DELIMITER ;
...
现在，我们可以调用这个存储过程来计算特定订单的总额。先使用 `SET @TotalAmount = 0` 声明一个变量来存储计算出的总额，再使用 `CALL CalculateOrderTotal(1, @TotalAmount);` 计算订单 ID 为 1 的订单总金额，最后展示计算结果。

```sql
obclient > SET @TotalAmount = 0;
obclient > CALL CalculateOrderTotal(1, @TotalAmount);
obclient > SELECT @TotalAmount AS OrderTotal;
+------------+
| OrderTotal |
+------------+
|     152.96 |
...
CalculateOrderTotal 过程接收订单 ID 作为输入参数 orderID，并且有一个 OUT 参数 totalAmount，用于输出计算后的订单总金额。存储过程中首先计算了订单条目的小计，然后根据假设的税率和折扣率计算出最终的总金额。
