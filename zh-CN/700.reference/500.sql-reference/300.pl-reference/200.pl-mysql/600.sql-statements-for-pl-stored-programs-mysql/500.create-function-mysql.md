...
在下面的示例中，创建一个名为 GetCustomerLevel 的简单函数，该函数根据输入的信用额度（credit）返回客户的等级。

```sql
obclient > DELIMITER //
obclient > CREATE FUNCTION GetCustomerLevel(credit DOUBLE) RETURNS VARCHAR(20)
    ->     DETERMINISTIC
    -> BEGIN
...
- `(credit DOUBLE)` 定义了一个名为 credit 的参数，其数据类型是 `DOUBLE`。

- `RETURNS VARCHAR(20)` 声明了函数将返回一个 `VARCHAR(20)` 类型的值。

- `DETERMINISTIC` 关键字表明这个函数在给定相同的输入下总是返回相同的结果。

- `DECLARE level VARCHAR(20);` 在函数内部声明了一个变量 `level` 用于存储客户等级。

- `IF ... THEN ... ELSEIF ... ELSE ... END IF;` 语句块根据 credit 的值来设置 level 变量。

- `RETURN level;` 返回计算得出的客户等级。

如果你想要为信用额度为 25000 的客户获取等级，你可以使用下面的 SQL 语句调用函数。

```sql
obclient > SELECT GetCustomerLevel(25000);
```
