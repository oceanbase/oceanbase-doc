...
`JSON_ARRAY_T` 是 `JSON_ELEMENT_T` 的子类型。`JSON_ARRAY_T` 与 JSON 数据中的 Array 类型相对应。

### 创建 `JSON_ARRAY_T` 的实例

您可以使用以下构造函数创建一个空的 `JSON_ARRAY_T` 实例。

```sql
CONSTRUCTOR FUNCTION JSON_ARRAY_T RETURN SELF AS RESULT,
```

可以使用以下 `parse` 函数创建 `JSON_ARRAY_T` 实例。该函数将 JSON 字符串作为输入，如果提供的输入不是有效的 JSON，则会发生报错。
...
上述示例中，定义了一个包含多种类型元素的 JSON 数组。其中，`jo` 是 `JSON_ARRAY_T` 类型，用于存储 JSON 数组；`jo_val` 是 `JSON_ELEMENT_T` 类型，用于存储从数组中检索到的 JSON 元素。使用 `JSON_ARRAY_T.parse` 方法从 JSON 格式的字符串创建一个 `JSON_ARRAY_T` 实例，该 JSON 数组包含六个元素：一个数字（`123`），一个对象（`{"abc":456}`），一个数组（`[789]`），一个布尔值（`true`），一个 `null` 值和一个字符串（`"test"`）。通过 FOR 循环来遍历 JSON 数组的每个元素（从下标 0 到 5），循环输出了数组中的每个元素。

### 获取数组的属性

您可以使用以下函数获取 JSON 数组的属性。
...
以下使用示例中，使用了 `jo.count` 方法获取 JSON 数组中的元素总数量，并且由于数组的下标是从 0 开始，故将元素总数减去 1。
