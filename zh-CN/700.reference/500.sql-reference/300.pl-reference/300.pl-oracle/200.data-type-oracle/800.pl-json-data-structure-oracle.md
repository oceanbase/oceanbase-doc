...
上述示例中，声明了一个 `JSON_ARRAY_T` 类型的变量 `jo`，并使用 `JSON_ARRAY_T` 的构造函数创建了一个 `JSON_ARRAY_T` 实例。构造函数接收一个字符串参数，该字符串表示一个 JSON 数组，且该数组中仅包含一个元素即数组 `[123]`。
...
以下示例中，使用了 `jo.get` 方法从 JSON 数组中获取索引（下标）为 `i` 的元素。
...
以下示例中，使用了 `jo.count` 方法获取 JSON 数组中的元素总数量，并且由于数组的索引（下标）是从 0 开始，故将元素总数减去 1。
