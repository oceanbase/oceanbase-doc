...
以下示例中，创建了一个包含多种类型元素的 JSON 数组。其中，`jo` 是 `JSON_ARRAY_T` 类型，用于存储 JSON 数组；`jo_val` 是 `JSON_ELEMENT_T` 类型，用于存储从数组中检索到的 JSON 元素。再使用 `JSON_ARRAY_T.parse` 方法从 JSON 格式的字符串创建一个 `JSON_ARRAY_T` 实例，该 JSON 数组包含六个元素：一个数字（`123`），一个对象（`{"abc":456}`），一个数组（`[789]`），一个布尔值（`true`），一个 `null` 值和一个字符串（`"test"`）。然后通过 FOR 循环来遍历 JSON 数组的每个元素（从下标 0 到 5），循环输出了数组中的每个元素。
