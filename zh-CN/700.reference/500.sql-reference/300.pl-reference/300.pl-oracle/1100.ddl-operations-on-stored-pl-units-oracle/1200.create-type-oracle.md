...
`CREATE TYPE` 用于定义新的用户自定义数据类型（UDT）。您可以使用 `CREATE TYPE` 指定类型的名称及其属性、方法和其他属性。
...
* 抽象数据类型（ADT）
  ...
  + 例如，当您要在数据库中表示具有结构化属性且可能需要封装为方法的复杂数据时，可以创建一个抽象数据类型。例如，您可以创建一个表示地址或者员工的 ADT。

* 独立的可变数组（VARRAY）类型

  + 例如，如果您需要表示一个带有固定上限的数组，且这个数组需要整体存储和检索，就可以定义一个 VARRAY 类型。

* 独立嵌套表类型

  + 例如，当您想存储和访问一个不限数量的同类型数据集合，并且集合中的每一项都是无序的，可以使用嵌套表类型。

* 不完整对象类型

  不完整类型是由前向类型定义创建的类型。之所以称为不完整，是因为它具有名称，但没有属性或方法。它可以被其他类型引用，允许自定义相互引用的类型。 但是，必须先创建完成类型，然后才能用于创建表的列、对象列或嵌套表类型的列。例如，不完整类型通常用于当您预见到类型之间的互相引用时创建。先声明一个不完整类型作为占位符，然后在完成引用类型的定义后，进一步定义不完整类型。

<main id="notice" type='explain'>
  <h4>说明</h4>
  <p>对象类型的形参默认值支持非确定性默认值。</p>
</main>

`CREATE TYPE BODY` 声明自带实现该类型的代码。即如果您创建的类型包含方法，还需要使用 CREATE TYPE BODY 语句来提供这些方法的实现代码。
