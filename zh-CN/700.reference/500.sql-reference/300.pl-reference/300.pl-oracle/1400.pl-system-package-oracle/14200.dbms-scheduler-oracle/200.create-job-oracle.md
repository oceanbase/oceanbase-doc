...
| job_type           | 指定要创建的作业的类型。如果未指定，则会生成错误。当前版本支持 `STORED_PROCEDURE` 和 `PLSQL_BLOCK`。`STORED_PROCEDURE` 表示执行存储过程，仅支持过程，不支持带有返回值的函数；`PLSQL_BLOCK` 表示执行匿名 PL 块，此时 `job_action` 应为完整的 PL 匿名块代码。   |
| job_action         | 指定作业的操作。如果没有为内联程序指定 `job_action`，则在创建作业时会报错。 `job_action` 在独立的会话事务中执行，其提交和回滚与调用方事务无关。当 `job_type` 为 `STORED_PROCEDURE` 时，`job_action` 是存储过程的名称；如果过程位于作业之外的另一个 Schema 中，则必须指定 Schema；如果需要区分大小写，请将 Schema 名称和存储过程名称用双引号括起来，例如 `job_action_proc=>'"Schema"."myProc "'`；此时不支持带有 `INOUT` 或 `OUT` 参数的 PL 过程。当 `job_type` 为 `PLSQL_BLOCK` 时，`job_action` 应为完整的 PL 匿名块代码（以 `BEGIN` 开头、`END;` 结尾）。   |
