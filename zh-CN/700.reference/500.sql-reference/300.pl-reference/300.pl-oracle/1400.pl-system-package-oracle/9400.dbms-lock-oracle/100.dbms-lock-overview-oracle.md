...
* 用户锁和 Oracle 锁不会产生冲突，它们拥有“UL”前缀。
* Session 断开后锁会放开。
* 锁 ID 的取值范围为（0, 1073741823）。
* 用户锁和 Oracle 锁表现相同，拥有 Oracle 锁所有的功能，例如死锁检测。
...
|                       **子程序**                      |    **描述**     |
|------------------------------------------------------|---------------|
| [ALLOCATE_UNIQUE](110.allocate-unique.md) |为锁名字申请唯一的锁 ID，锁 ID 的取值范围为（1073741824, 1999999999）。|
| [ALLOCATE_UNIQUE_AUTONOMOUS](120.allocate-unique-autonomous.md) |为锁名字申请唯一的锁 ID。|
| [CONVERT]() | 修改锁模式。 |
| [RELEASE]() | 释放锁。 |
| [REQUEST](150.request.md) | 申请锁。 |
