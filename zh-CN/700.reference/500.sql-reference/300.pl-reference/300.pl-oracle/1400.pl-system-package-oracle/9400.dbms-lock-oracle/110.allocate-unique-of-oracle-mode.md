...
`ALLOCATE_UNIQUE` 存储过程用于为锁名字申请唯一的锁 ID，锁 ID 的取值范围为（1073741824, 1999999999）。锁 ID 用来协调应用间锁的使用，这比使用应用的名字更加方便。
...
| lockhandle | 返回 `ALLOCATE_UNIQUE` 生成的锁句柄，可以进行 `REQUEST` 操作。任何一个 Session 通过 `ALLOCATE_UNIQUE` 用相同锁名字获取到的 Lock Handle 都指向同一把锁。因此，不要在 Session 间传递 Lock Handle。`lockhandle` 最大长度为 `VARCHAR2 (128)`。 |
