...
`DIRECT` Hint 在 `LOAD DATA` 和 `INSERT` 语句中可以指定启用旁路导入功能，从而提高数据导入的效率和性能。
...
此示例中，`direct(true, 0)` 意味着启用旁路导入，不进行排序，并且允许最多 0 行错误。

**`INSERT INTO SELECT` 中的 `DIRECT` Hint 示例**

在 `INSERT INTO SELECT` 语句中使用 `DIRECT` Hint：

```sql
INSERT /*+ direct */ INTO new_table
...
- `need_sort`：表示是否需要数据库对导入数据进行排序。
- `max_errors_allowed`：指定最大允许的错误行数，超过此数目则导入过程将失败。
- `load_mode`：指定导入模式，可以是 `'full'` 或 `'inc'`，分别表示全量或增量导入。默认值为 `'full'`。

通常情况下，推荐使用 `direct(true, 0)`，即不需要排序，允许最多 0 行错误，并使用全量导入模式。

### 示例

以下示例展示了如何在 `LOAD DATA` 中使用 `DIRECT` Hint：
...
在这个示例中，`PARALLEL(1)` 指定并行度为 1，`DIRECT(FALSE, 0)` 明确指定不使用旁路导入，以便更加灵活地控制导入过程。
