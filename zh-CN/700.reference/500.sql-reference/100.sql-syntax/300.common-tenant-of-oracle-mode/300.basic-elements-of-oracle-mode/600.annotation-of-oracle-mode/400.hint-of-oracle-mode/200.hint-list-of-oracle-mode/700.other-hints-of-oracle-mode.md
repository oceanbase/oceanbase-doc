...
OceanBase 数据库还支持 CURSOR_SHARING_EXACT Hint、MONITOR Hint、PX_JOIN_FILTER Hint、NO_PX_JOIN_FILTER Hint、QB_NAME Hint、READ_CONSISTENCY Hint、QUERY_TIMEOUT Hint、LOG_LEVEL Hint、USE_PLAN_CACHE Hint、TRANS_PARAM Hint、TRACING Hint、STAT Hint、TOPK Hint、TRACE_LOG Hint 和 DIRECT Hint。
...
- `load_mode`：指定导入模式，参数选项包括：
   ...
   + `inc_replace`：表示增量导入，但不检查主键是否重复，相当于 `REPLACE` 语义的增量导入。`inc` 与 `inc_replace` 的区别仅在于 `inc_replace` 不检查主键是否重复，是一种追加模式。

通常情况下，推荐使用 `direct(true, 0)`，即进行排序，允许最多 0 行错误，并使用全量导入模式。

#### 在 `LOAD DATA` 和 `INSERT INTO SELECT` 中使用方式

**`LOAD DATA` 中的 `DIRECT` Hint 示例**

在 `LOAD DATA` 中使用 `DIRECT` Hint 可以如下指定：
...
此示例中，`direct(true, 0, inc)` 表示启用增量旁路导入，进行排序，并且允许最多 0 行错误。
...
```sql
INSERT /*+ direct(true, 0, inc) enable_parallel_dml PARALLEL(4)*/ INTO new_table
SELECT * FROM old_table;
```

更多信息，参见[使用 LOAD DATA 语句旁路导入数据](../../../../../../../../500.data-migration/1100.bypass-import/200.use-load-data-statement-to-bypass-import-data.md)和[使用 INSERT INTO SELECT 语句旁路导入数据](../../../../../../../../500.data-migration/1100.bypass-import/100.overview-of-bypass-import.md)。

### 示例

**示例 1**：在 `LOAD DATA` 中使用 `DIRECT` Hint。

```sql
LOAD DATA
/*+ PARALLEL(1) DIRECT(TRUE, 0) */
...
在这个示例中，`PARALLEL(1)` 指定并行度为 1，`DIRECT(TRUE, 0)` 明确指定使用全量旁路导入。

**示例 2**：在 `INSERT INTO SELECT` 中使用 `DIRECT` Hint。

```sql
-- 启用旁路导入和并行 DML，将 old_table 的数据插入到 new_table
INSERT /*+ direct(true, 0, 'full') enable_parallel_dml PARALLEL(4) */ INTO new_table (id, name, value)
SELECT id, name, value
FROM old_table;
```

在这个示例中，`direct(true, 0, 'full')` 启用全量旁路导入，并对数据进行排序，允许最多 0 行错误，使用全量导入模式；`enable_parallel_dml` 启用并行 DML；`PARALLEL(4)` 设置并行度为 4。

**示例 3**：在 `INSERT INTO SELECT` 中使用 `DIRECT` Hint 开启增量旁路导入

在 `LOAD DATA` 中使用 `DIRECT` Hint 开启增量旁路导入方式相似，此处仅以 `INSERT INTO SELECT` 中使用 `DIRECT` Hint 开启增量旁路导入为例：
...
在这个示例中，`direct(true, 0, 'full')` 启用旁路导入，并对数据进行排序，允许最多 0 行错误，并使用增量导入模式。；`enable_parallel_dml` 启用并行 DML；`PARALLEL(4)` 设置并行度为 4。
