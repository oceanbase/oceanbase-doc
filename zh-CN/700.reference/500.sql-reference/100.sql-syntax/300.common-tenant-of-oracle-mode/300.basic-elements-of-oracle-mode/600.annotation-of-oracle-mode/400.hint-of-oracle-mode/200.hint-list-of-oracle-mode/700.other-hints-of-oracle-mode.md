...
`PX_NODE_POLICY` Hint 用于影响纯计算 DFO 的候选节点池,也可以通过租户配置项 [PX_NODE_POLICY](../../../../../../../../700.reference/800.configuration-items-and-system-variables/100.system-configuration-items/400.tenant-level-configuration-items/6850.px_node_policy.md) 设置。

`PX_NODE_POLICY` Hint 的语法如下：

```sql
/*+ PX_NODE_POLICY (policy) */
```

`policy` 指用于指定加速任务所使用的机器数量。可以选择使用单个区域（zone）内的所有机器，或者整个集群（cluster）内的所有机器。理论上，使用的机器数量越多，任务的加速效果越好。然而，使用整个集群可能会增加其他机器的负载，因此在选择时需权衡性能需求与集群的整体负载情况。

具体有以下取值：

* `DATA`:选择当前 SQL 涉及的所有数据节点。
* `ZONE`:可以使用当前 SQL 涉及的 ZONE 的该租户的所有机器。
* `CLUSTER`:可以使用当前租户的所有机器。
...
`PX_NODE_ADDRS` Hint 用于强制指定纯计算 DFO 分配到的具体的机器。

<main id="notice" type='explain'>
  <h4>说明</h4>
  <p>其中 <code>PX_NODE_ADDRS</code> 优先级最高，设置该 hint 后，另外两个 PX hint 不再生效。</p>
</main>

`PX_NODE_ADDRS` Hint 的语法如下：
...
`PX_NODE_COUNT` Hint 用于强制指定纯计算 DFO 分配到的机器数。

`PX_NODE_COUNT` Hint 的语法如下：

```SQL
/*+ PX_NODE_COUNT (COUNT) */
```

其中 `COUNT` 指选择的节点数量，选择节点的数量不能超出候选节点池的大小。
用以下示例说明：
将 DFO 分配到 cluster 集群级别，并且用集群里的 5 台机器，如果集群只有 3 台机器，小于 5 台机器则使用集群的全部机器。
...
更多信息，参见 [旁路导入](../../../../../../../../500.data-migration/620.obap/300.obap-import-data/20.bypass-import/100.overview-of-bypass-import.md)。
...
更多信息，参见[使用 INSERT INTO SELECT 语句旁路导入数据](../../../../../../../../500.data-migration/620.obap/300.obap-import-data/20.bypass-import/100.overview-of-bypass-import.md)。
...
`OPT_PARAM` Hint 对如下参数有效：
  ...
  * `enable_constant_type_demotion`: 用于控制语句级别常量类型降级行为，数据类型是 `BOOL`，取值范围为 `'TRUE'` 和 `'FALSE'`，默认值为 `TRUE`，取值需要用单引号（''）包裹。
  * `non_standard_comparison_level`: 用于控制是否使用非标比较规则来优化条件过滤，将常量对齐到列类型比较，目前仅影响字符类型列和整形常量之间的比较，数据类型是 `STRING`，取值范围为 `[NONE, EQUAL, RANGE]`，默认值为 `NONE`，取值需要用单引号（''）包裹。
