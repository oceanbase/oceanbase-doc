...
`PARALLEL` Hint 是语句级别的提示，用于指示优化器在并行操作中使用的并行线程数量。该 Hint 将覆盖初始化参数 `PARALLEL_DEGREE_ POLICY` 的值。它可以作为 `ALTER TABLE`、`SELECT`、`INSERT`、`UPDATE` 和 `DELETE` 语句的一部分，以及表扫描的一部分。如果违反任何并行限制，则 `PARALLEL` Hint 将被忽略。
