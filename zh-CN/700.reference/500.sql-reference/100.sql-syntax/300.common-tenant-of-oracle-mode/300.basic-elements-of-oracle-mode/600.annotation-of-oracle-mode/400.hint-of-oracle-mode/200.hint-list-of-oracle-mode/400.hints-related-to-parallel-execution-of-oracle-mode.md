...
有关并行 DML 的更多信息，参见[并行 DML](700.reference/1000.performance-tuning-guide/500.sql-optimization/300.distributed-execution-plan/1000.parallel-dml.md)。
...
`ENABLE_PARALLEL_DAS_DML` Hint 用于开启 Distributed Active Storage (DAS) 的并行数据修改语言（DML）操作。当您想要优化这些操作的执行时间，希望借助并行处理来加速处理速度时，可以使用这个 Hint。其反向 Hint 为 `DISABLE_PARALLEL_DAS_DML`。
...
`DISABLE_PARALLEL_DAS_DML` Hint 用于关闭 Distributed Active Storage (DAS) 的并行数据修改语言（DML）操作。如果您需要保证操作的顺序性，或者并行操作并不提供性能优势时，关闭并行处理能够帮助更好地控制操作的执行。其反向 Hint 为 `ENABLE_PARALLEL_DAS_DML`。
