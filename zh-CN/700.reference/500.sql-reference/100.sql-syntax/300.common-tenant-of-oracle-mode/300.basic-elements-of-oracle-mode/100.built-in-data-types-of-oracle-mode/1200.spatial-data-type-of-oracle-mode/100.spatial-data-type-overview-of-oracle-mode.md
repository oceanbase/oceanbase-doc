...
支持定义表中字段为 `SDO_GEOMETRY` 类型，`SDO_GEOMETRY` 数据类型当作 `BLOB` 列进行存储，有效提高存储效率。
...
为测试表 test 创建一个二维点几何图形对象，其类型代码为 2001，坐标为 X=10，Y=20，Z 坐标为 `NULL`。没有指定空间参考标识符 SRID、几何元素信息 `SDO_ELEM_INFO` 和具体坐标值 `SDO_ORDINATE_ARRAY`。

```sql
-- DDL，创建 SDO_GEOMETRY 类型字段
CREATE TABLE test (
  id NUMBER PRIMARY KEY,
  name VARCHAR2(32),
  shape SDO_GEOMETRY);

-- 插入 SDO_GEOMETRY 类型数据：通过 SDO_GEOMETRY 构造器创建空间对象实例
...
-- 查询 SDO_GEOMETRY 字段，默认按 SDO_GEOMETRY 构造器格式输出
