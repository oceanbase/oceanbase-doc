...
| overwrite_table_insert  | 用于将查询结果替换表中的现有数据，即该语句会先清空表中的旧数据，再将新数据批量写入到表中。有关使用 `INSERT OVERWRITE` 语句的示例信息，参见 [插入数据](../../../../../../300.develop/200.application-development-of-oracle-mode/400.write-data-of-oracle-mode/100.insert-data-of-oracle-mode-in-develop.md) 中的 **使用 INSERT OVERWRITE 语句插入数据** 章节。<main id="notice" type='notice'><h4>注意</h4><p><ul><li>该语句无法在事务中执行。因此，为确保操作顺利进行，需先执行 <code>SET autocommit = on;</code> 命令开启自动提交事务。</li><li>该子句暂不支持 <code>hint_options</code> 参数。</li></ul></p></main>|
