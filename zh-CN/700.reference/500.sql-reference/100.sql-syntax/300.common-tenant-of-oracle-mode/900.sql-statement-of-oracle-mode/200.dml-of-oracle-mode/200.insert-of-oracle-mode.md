...
| INSERT OVERWRITE        | 用于将查询结果替换表中的现有数据，即该语句会先清空表中的旧数据，再将新数据批量写入到表中。 <main id="notice" type='notice'><h4>注意</h4><p>该语句无法在事务中执行。因此，为确保操作顺利进行，需先执行 <code>SET autocommit = on;</code> 命令开启自动提交事务。</p></main>|
