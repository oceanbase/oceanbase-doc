...
| SET STATEMENT_ID = 'string' | 表示当前查询使用字符串标记，以方便后续查询该 SQL 的计划信息。如果没有指定 `SET statement_id`，默认使用空字符串作为信息标记。 |
...
### 示例 3：使用 STATEMENT_ID 标识执行计划

```sql
-- 使用 statement_id 标识执行计划
EXPLAIN INTO test SET STATEMENT_ID='test2' SELECT COUNT(*) FROM t1;

-- 查看指定 statement_id 的执行计划
...
### 示例 6：省略 explain_type

```sql
-- 省略 explain_type 查看 SELECT 语句执行计划
