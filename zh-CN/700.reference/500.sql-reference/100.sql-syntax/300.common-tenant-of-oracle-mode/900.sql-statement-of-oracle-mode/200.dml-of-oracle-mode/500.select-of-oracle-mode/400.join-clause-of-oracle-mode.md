...
joined_table:
    ...
    | table_reference1 
      {INNER | CROSS | NATURAL} JOIN table_reference2
    | table_reference1 
      {LEFT | RIGHT | FULL} [OUTER] JOIN table_reference2 
      {ON join_condition | WHERE join_condition}
    | table_reference1 NATURAL {LEFT | RIGHT | FULL} [OUTER] JOIN table_reference2
...
* `table_subquery [AS] table_alias_name]`：一个子查询，可以给子查询的结果指定别名。更多有关子查询的介绍，参见 [子查询](../../../../../../300.develop/100.application-development-of-mysql-mode/500.read-data-of-mysql-mode/200.multi-table-join-query-of-mysql-mode/500.subquery-of-mysql-mode-develop.md)。
...
* `table_reference1 {INNER | CROSS | NATURAL} JOIN table_reference2`：两个表引用，使用 INNER JOIN（内连接）、CROSS JOIN（交叉连接）或 NATURAL JOIN（自然连接）。
...
* `table_reference1 NATURAL {LEFT | RIGHT | FULL} [OUTER] JOIN table_reference2`：两个表引用，使用 NATURAL LEFT OUTER JOIN、NATURAL RIGHT OUTER JOIN 或 NATURAL FULL OUTER JOIN，自然连接会自动基于同名列进行连接。
