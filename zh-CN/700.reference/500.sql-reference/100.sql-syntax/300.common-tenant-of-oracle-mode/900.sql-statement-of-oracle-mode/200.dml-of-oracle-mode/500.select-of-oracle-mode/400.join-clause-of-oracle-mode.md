...
joined_table:
    ...
    | table_reference1 NATURAL [INNER] JOIN table_reference2
...
* `table_reference1 NATURAL [INNER] JOIN table_reference2`：自然内连接，自动基于两表同名列进行等值连接。
* `table_reference1 NATURAL {LEFT | RIGHT | FULL} [OUTER] JOIN table_reference2`：自然外连接（左/右/全），自动基于两表同名列进行等值连接，无需指定 ON 或 USING 子句。
