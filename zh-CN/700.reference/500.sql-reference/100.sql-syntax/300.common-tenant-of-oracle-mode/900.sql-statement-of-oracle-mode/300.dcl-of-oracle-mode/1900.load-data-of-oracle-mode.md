...
* 为提高导入效率，建议先建基础表，导入完成后再创建索引；若有全局索引，请务必在导入完成后再创建，否则可能报 not support 错误。
...
<main id="notice" type='explain'>
  <h4>说明</h4>
  <p>执行 <code>LOAD DATA LOCAL INFILE</code> 时，系统会自动添加 <code>IGNORE</code> 选项。</p>
</main>

<main id="notice" type='notice'>
  <h4>注意</h4>
  <p>如果需要导入的文件在 OBServer 的本地磁盘，请直连 OBServer 执行本语句，请勿通过 Proxy 执行。</p>
</main>
...
为便于多文件导入，引入文件通配符功能，适用于服务器端和 OSS 文件导入，不适用于客户端文件导入。服务器端可匹配文件名、目录等；OSS 仅支持文件名通配符 `*` 和 `?`。详见旁路导入文档。
...
**支持格式**：CSV、PARQUET、ORC（通过 FILES）；ODPS（通过 SOURCE）。**诊断**：使用 `LOG ERRORS`、`REJECT LIMIT`、`BADFILE` 及 `READ_ERROR_LOG`，详见 [log_errors](#log_errors)。
...
以下将以外部文件的位置在服务器（OBServer 节点）和在 OceanBase 数据库 Oracle 模式中创建外表为例，步骤如下：
