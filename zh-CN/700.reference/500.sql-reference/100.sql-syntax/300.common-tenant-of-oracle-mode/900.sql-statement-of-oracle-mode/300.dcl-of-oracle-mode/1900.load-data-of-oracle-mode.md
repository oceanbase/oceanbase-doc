...
| REPLACE \| IGNORE                | 可选项，如果遇到唯一键冲突，`REPLACE` 表示覆盖冲突行，`IGNORE` 表示忽略冲突行。`LOAD DATA` 通过表的主键来判断数据是否重复，如果表不存在主键，则 `REPLACE` 与 `IGNORE` 选项没有区别。缺省时，遇到重复数据的时候，`LOAD DATA` 会将出现错误的数据记录到日志文件中。  |
...
* `LOG ERRORS`：表示开启导入过程中的错误诊断，允许将失败的行记录下来（当前版本记录于 `warning buffer` 中，最大不超过 64 行），而不是让整个操作因第一个错误而终止。配合 `REJECT LIMIT` 子句，可以控制允许的错误行数。
