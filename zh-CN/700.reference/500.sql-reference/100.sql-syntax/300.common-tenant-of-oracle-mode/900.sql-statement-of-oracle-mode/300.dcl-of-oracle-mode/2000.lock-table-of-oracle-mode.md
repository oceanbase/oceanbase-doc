| Description   | LOCK TABLE 语句用于根据指定模式锁定一个或多个表、表的分区或者二级分区。 |
|---------------|-----------------|
| keywords      | LOCK TABLE, 表锁, 分区锁, 二级分区锁, 锁定模式 |
| dir-name      | LOCK TABLE |
...
| WAIT | 如果发生锁冲突，那么语句应该等待冲突的锁释放，直到超时，即用户设置的 `integer` 时间（单位为秒）。`integer` 的取值没有限制。 |
