...
```sql
ALTER TABLE [EXTERNAL] TABLE table_name
...
column_definition:
    ...
    [VISIBLE|INVISIBLE]
    {
    [DEFAULT expression] [AUTO_INCREMENT]
    [NULL | NOT NULL] [[PRIMARY] KEY] [UNIQUE [KEY]] [COMMENT string]
    }
...
| VISIBLE                     | 修改列状态为列可见，是默认的列状态。|
| INVISIBLE                   | 修改列状态为列不可见，当将一个列设置为 `INVISIBLE` 后，在查询时该列将不会被默认显示。|
| SET COMMENT                                    | 设置注释信息。    |
