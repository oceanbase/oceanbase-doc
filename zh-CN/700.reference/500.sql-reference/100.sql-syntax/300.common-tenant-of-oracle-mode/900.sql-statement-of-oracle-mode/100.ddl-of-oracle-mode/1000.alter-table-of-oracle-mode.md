...
>>>>>>> 5f3a0ade62f1c3fc0bb6e9e8b2b6642ba5876778
...
| DROP COLUMN \| DROP (column_name_list)    | 删除列。<main id="notice" type='notice'><h4>注意</h4><p><ul><li>不允许删除主键列。</li><li>对于 OceanBase 数据库 V4.3.5 版本，<code>DROP COLUMN</code> 是 Online DDL 操作从 V4.3.5 BP1 版本开始引入。</li></ul></p></main>|
| ADD UNIQUE                          | 增加唯一索引。       |
| ADD INDEX                                  | 增加普通索引      |
| ALTER INDEX                              | 修改索引属性。      |
| VISIBLE \| INVISIBLE                     | 表示该索引是否可见。`VISIBLE` 表示可见，`INVISIBLE` 表示不可见。|
...
| MODIFY PARTITION partition_name ADD add_subpartition_list | 表示新增二级分区。<main id="notice" type='notice'><h4>注意</h4><p>不支持二级分区类型是 Hash 类型的场景新增二级分区。</p></main>|
...
| [MODIFY] PARTITION BY RANGE([column_name_list]) [SIZE('size_value')] [range_partition_list] | 用于修改表自动分区的属性。详细介绍可参见 [自动分区分裂](../../../../../300.database-object-management/200.manage-object-of-oracle-mode/200.manage-partitions-of-oracle-mode/250.partition-splitting-of-oracle-mode/300.automatic-partition-splitting-of-oracle-mode.md) 中的修改表自动分区属性的语法。|
| DUPLICATE_SCOPE | 用于更改复制表属性。<ul><li>`none`：表示将当前表转换为普通表。</li><li>`cluster`：表示将当前表转换为复制表。</li></ul> 更多有关更改复制表属性的信息，请参见 [更改复制表属性](../../../../../300.database-object-management/200.manage-object-of-oracle-mode/100.manage-tables-of-oracle-mode/650.change-duplicate-scope-of-oracle-mode.md)。|
| FORCE | 清除废弃列。<main id="notice" type='explain'><h4>说明</h4><p><ul><li>对于 OceanBase 数据库 V4.3.5 版本，从 V4.3.5 BP1 版本开始支持清除废弃列。</li><li>当某些列被删除，即使这些列不再被使用，它们仍然占据物理存储空间。如果要移除这些废弃列并回收相关空间，需要清除废弃列。</li></ul></p></main>|
>>>>>>> 5f3a0ade62f1c3fc0bb6e9e8b2b6642ba5876778
| 参数 | 描述 |
|----------|------|
