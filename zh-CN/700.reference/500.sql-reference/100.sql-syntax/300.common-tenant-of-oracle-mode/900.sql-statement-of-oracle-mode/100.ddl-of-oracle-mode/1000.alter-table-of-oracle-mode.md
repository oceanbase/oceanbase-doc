...
| `SET table_option_list_space_seperated` | 通过 SET 语句设置表选项。其中，`SET MERGE_ENGINE = {partial_update \| delete_insert \| append_only}` 用于设置表的表模式（更新模型），支持在不同表模式之间进行 Online 转换，不阻塞业务读写。<main id="notice" type='notice'><h4>注意</h4><p><ul><li>该功能从 OceanBase 数据库 V5.0.1 版本开始引入，仅当集群内所有节点都升级到 V5.0.1 及之后的版本后才能执行。</li><li>表模式为 <code>append_only</code> 的表不支持转换为其它表模式。</li></ul></p></main> |
| `ENABLE ROW MOVEMENT` | 启用行移动。当分区表中主键或唯一键被更新导致行可能跨分区移动时，允许该行移动至其他分区。 |
| `DISABLE ROW MOVEMENT` | 禁用行移动。当分区表中主键或唯一键被更新导致行可能跨分区移动时，禁止该行移动至其他分区，UPDATE 操作将失败。 |
...
| `REFRESH` | 刷新表 |
| `enable_macro_block_bloom_filter`|用于指定是否持久化宏块级别 bloom filter。取值如下：<ul><li><code>True</code>：开启持久化宏块级别 bloom filter。</li></ul><ul></li></li><li><code>False</code>：关闭持久化宏块级别 bloom filter。</li></ul>|
...
| SET INTERVAL(expr) | 用于 Range 分区表与 Interval 分区表之间的相互转换。详细介绍可参见 [修改分区规则](../../../../../300.database-object-management/200.manage-object-of-oracle-mode/200.manage-partitions-of-oracle-mode/300.modify-partition-rules-of-oracle-mode.md) 中的修改分区类型。|
...
| TTL [=] col_name + INTERVAL interval_num ttl_unit BY COMPACTION | 修改表的 TTL 属性。具体解释如下：<ul><li>`col_name`：指定表的 TTL 过滤列。当前版本支持指定内部隐藏列 `ora_rowscn`（记录最后一次更新的时间戳）或用户定义的时间列。指定为用户定义的时间列时，支持的列类型为 DATE、TIMESTAMP、TIMESTAMP WITH TIME ZONE 或 TIMESTAMP WITH LOCAL TIME ZONE。</li><li>`interval_num`：指定过期时间的整数数值。</li><li>`ttl_unit`：指定过期时间的单位。取值支持 `SECOND`、`MINUTE`、`HOUR`、`DAY`、`MONTH` 或 `YEAR`。</li><li>`BY COMPACTION`：表示在后续 Compaction 过程中删除过期数据。</li></ul>|
...
| PARTITION BY RANDOM SIZE('size_value') | 用于修改随机分布表的分区大小。更多有关修改随机分布表的信息，参见 [修改随机分布表](../../../../../300.database-object-management/200.manage-object-of-oracle-mode/200.manage-partitions-of-oracle-mode/270.random-partition-table-of-oracle-mode/400.modify-random-partition-table-of-oracle-mode.md)。 <main id="notice" type='explain'><h4>说明</h4><p>该参数从 V5.0.1 版本开始引入。</p></main>|
...
* 为分区表启用或禁用行移动。
  ...
  ```sql
  -- 启用行移动，允许 UPDATE 导致的分区键变更时行跨分区移动
...
* 修改表的 TTL 属性，包括新增 TTL 属性、变更 TTL 列，修改 TTL 策略以及删除 TTL 属性。
  ...
  3. 为 TTL 表变更新的 TTL 列。将表的 TTL 列从 `ORDER_TIME` 变更为 `PAYMENT_TIME`。
     ...
     ```shell
     obclient(SYS@oracle001)[SYS]> ALTER TABLE TBL3 TTL PAYMENT_TIME + INTERVAL 7 DAY BY COMPACTION;
     ```

  4. 修改表的 TTL 策略。例如：将数据过期时间修改为 1 小时。

     ```shell
     obclient(SYS@oracle001)[SYS]> ALTER TABLE TBL3 SET TTL PAYMENT_TIME + INTERVAL 1 HOUR BY COMPACTION;
     ```
