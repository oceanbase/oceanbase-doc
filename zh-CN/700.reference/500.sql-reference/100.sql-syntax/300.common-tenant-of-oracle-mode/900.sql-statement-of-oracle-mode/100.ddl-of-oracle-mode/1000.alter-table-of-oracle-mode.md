...
* `TIME_UNIT = {'hour' | 'day' | 'week' | 'month' | 'year'}`：表示分区的时间单位，即自动创建分区边界的间隔。该参数在创建动态分区表时指定，指定后不可修改。修改已启用动态分区管理的表时，不能指定该参数（指定会报错）；为尚未启用动态分区管理的表添加动态分区策略时，需要指定该参数。
