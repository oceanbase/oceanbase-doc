...
| WHERE condition WITH CHECK OPTION | `WITH CHECK OPTION` 是一种用于创建视图的语法，它用于确保插入或更新的数据满足视图的定义条件。 |
...
obclient> create view view2 as select * from tbl2 where b > 1 with check option;
