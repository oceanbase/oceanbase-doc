...
* 认证串格式校验：`IDENTIFIED WITH ... BY VALUES`，校验 `$S$` 格式，拒绝 `$A$`（`sha2`）等错误格式。
* 未适配的 ODP 无法完成 SM3 用户（使用认证插件 `ob_sm3_password` 的用户）的连接。
...
| IDENTIFIED WITH auth_plugin [BY password] | 指定认证插件及对应的认证信息。 <ul><li>`auth_plugin`：用于指定用户身份验证的方式（认证插件），详细介绍可参见下文 [auth_plugin](#auth_plugin)。</li><li>`BY password`：可选项，用于指定明文密码。</li></ul> <main id="notice" type='explain'><h4>说明</h4><p>对于 V4.4.2 版本，<code>IDENTIFIED WITH</code> 语法从 V4.4.2 BP2 版本开始支持。</p></main> |
...
* 创建用户 `test3`，显式指定使用 `caching_sha2_password` 认证插件
...
* 有关认证插件的介绍，参见 [身份鉴别](../../../../../../600.manage/500.security-and-permissions/200.identification-and-authentication/200.identification-and-authentication-of-oracle-mode/100.identification-of-oracle-mode.md)。