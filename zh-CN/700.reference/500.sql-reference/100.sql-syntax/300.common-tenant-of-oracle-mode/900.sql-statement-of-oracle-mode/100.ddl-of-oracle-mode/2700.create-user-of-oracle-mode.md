...
| IDENTIFIED WITH | 指定认证插件及对应的认证信息，支持的认证插件为 `caching_sha2_password`（基于 SHA-256 的密码认证插件）。<code>BY password</code>：指定明文密码。<main id="notice" type='explain'><h4>说明</h4><p><code>IDENTIFIED WITH</code> 语法从 V5.0.1 版本开始支持。</p></main> |
...
| WITH auth_plugin | 可选项，指定用户身份验证插件。目前支持 `mysql_native_password` 等身份验证插件。 |
| DEFAULT TABLESPACE | 指定用户的默认表空间。表空间是用于存储用户数据的逻辑结构。指定 `NULL` 表示不设置默认表空间。 |
