...
| EXPIRE PASSWORD ROLLOVER PERIOD | 在密码轮转场景下提前结束轮转期，使旧密码立即失效，仅保留当前密码。<main id="notice" type='explain'><h4>说明</h4><p>该参数从 V5.0.1 版本开始支持。</p></main> |
...
| EXPIRE PASSWORD ROLLOVER PERIOD | 主动结束当前用户的密码轮转（双密码）期，使旧密码立即失效，仅保留新密码。需与 Profile 的 `PASSWORD_ROLLOVER_TIME` 配合使用。|
...
| DEFAULT TABLESPACE tablespace_name | 修改 `USER` 的默认表空间（`DEFAULT TABLESPACE`），`tablespace_name` 表示表空间的名称。 <main id="notice" type='explain'><h4>说明</h4><p>对于 V5.x 版本，该参数从 V5.0.1 版本开始引入。</p></main>|
...
* 修改用户 `user1` 的默认表空间为 `ts1`。

    ```shell
    obclient> ALTER USER user1 DEFAULT TABLESPACE ts1;
    ```