...
| create_fulltext_index_stmt | 表示创建全文索引语句。 <main id="notice" type='explain'><h4>说明</h4><p>对于 OceanBase 数据库 V4.4.2 版本，Oracle 模式从 V4.4.2 BP2 版本开始支持创建全文索引。</p></main>|
| PARAMETERS('LEXER parser_name') | 可选项，指定全文索引的分词器。当不显示指定 `PARAMETERS('LEXER parser_name')` 时，分词器默认为 `SPACE`，适用于英文等用空格分隔语义的语言。`parser_name` 表示分词器名称，详细介绍可参见下文 [parser_name](#parser_name)。|
