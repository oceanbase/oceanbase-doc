...
* 当 `FORMAT_SQL_ID` 相同时，使用 `FORMAT_SQL_TEXT` 方式创建的 Outline 会覆盖 `FORMAT_SQL_ID` 方式创建的 Outline，`FORMAT_SQL_TEXT` 方式创建的优先级更高。此外，OceanBase 数据库通过 `FORMAT_SQL_ID` 区分不同的 SQL，而 `FORMAT_SQL_ID` 是通过 `FORMAT_SQL_TEXT` 取 `MD5` 加密得到的。在实际生产系统中，推荐通过 `FORMAT_SQL_ID` 进行 Outline 绑定。
  ...
  <main id="notice" type='explain'>
    <h4>说明</h4>
    <p><ul><li><code>FORMAT_SQL_TEXT</code> 的匹配规则忽略参数变化、忽略大小写差异以及空格和换行符等非语法性符号的差异，对于 `in` 表达式，将会做归一化，只比较经过这些处理后的文本内容。</li><li><code>FORMAT_SQL_ID</code> 是通过格式化 <code>FORMAT_SQL_TEXT</code>（移除多余的空格和换行符，将所有字符统一转换为大写或小写）之后，对格式化的结果应用 <code>MD5</code> 哈希加密来生成的。</li></ul></p>
  </main>

* 对于 V4.4.2 版本，从 V4.4.2 BP3 版本开始 Format Outline 同时支持文本协议和 PS 协议（Prepared Statement Protocol）场景。
