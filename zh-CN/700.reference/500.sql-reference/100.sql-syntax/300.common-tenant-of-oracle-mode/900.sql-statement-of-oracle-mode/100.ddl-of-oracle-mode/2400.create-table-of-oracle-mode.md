...
| table_column_group_option | 指定表列存选项。具体解释如下：<ul><li><code>WITH COLUMN GROUP(all columns, each column)</code>：指定创建行存列存冗余的表。</li><li><code>WITH COLUMN GROUP(all columns)</code>：指定添加行存索引。</li><li><code>WITH COLUMN GROUP(each column)</code>：指定创建列存表。</li></ul>|
...
* Oracle 兼容模式下的临时表在多个数据库升级场景下都有实际应用，具备基本的正确性/功能性保证。
