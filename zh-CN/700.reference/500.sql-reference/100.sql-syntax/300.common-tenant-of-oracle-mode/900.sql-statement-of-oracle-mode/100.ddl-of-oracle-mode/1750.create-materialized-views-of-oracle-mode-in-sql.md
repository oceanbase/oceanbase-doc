...
CREATE MATERIALIZED VIEW view_name
  [([column_list] [PRIMARY KEY(column_list)])]
  [table_option_list]
  [partition_option]
...
refresh_clause:
    REFRESH [COMPLETE | FAST | FORCE] [ON DEMAND | COMMIT | STATEMENT] [[START WITH expr] [NEXT expr]]
    | NEVER REFRESH
...
* `REFRESH [COMPLETE | FAST | FORCE] [ON DEMAND | COMMIT | STATEMENT] [[START WITH expr] [NEXT expr]]`：表示刷新物化视图的方式。
...
* `NEVER REFRESH`：指定物化视图不自动刷新，只能通过手动执行 `REFRESH` 语句来刷新。
