...
| DEFAULT TABLESPACE tablespace_name | 指定数据库的默认表空间（`DEFAULT TABLESPACE`），`tablespace_name` 表示表空间的名称。如果没有指定 `DEFAULT TABLESPACE`，会继承当前租户的 `DEFAULT TABLESPACE`。 <main id="notice" type='explain'><h4>说明</h4><p>对于 V5.x 版本，该参数从 V5.0.1 版本开始引入。</p></main>|
...
* 创建数据库 `test3`，并指定默认表空间为 `ts1`。

  ```shell
  obclient> CREATE DATABASE test3 DEFAULT TABLESPACE ts1;
  ```
