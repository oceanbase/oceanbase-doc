...
| ALTER INDEX                                                | 修改索引是否可见，当索引状态为 `INVISIBLE` 时，SQL 优化器将不会选择该索引。<main id="notice" type='explain'><h4>说明</h4><p>通过 <code>SHOW CREATE TABLE table_name;</code> 命令可以查看索引是否可见。当索引设置为不可见后，会在表结构中进行标记 <code>/*!80000 INVISIBLE */</code>。</p></main>|
| key_part        | 创建（函数）索引。 |
