...
alter_table_action:
    ...
    | ADD FULLTEXT [INDEX | KEY] [index_name](column_name, [column_name ...]) [WITH PARSER {SPACE | NGRAM | BENG}] [LOCAL]
...
| WITH PARSER {SPACE \| NGRAM \| BENG}                       | 可选项，指定全文索引的分词器。当前版本仅支持 `SPACE`（空格）、`NGRAM` 和 `BENG`（基础英文）三种内置分词器，不指定默认使用 `SPACE`（空格）分词器。可以使用 [TOKENIZE](../400.functions-of-mysql-mode/960.text-processing-functions-of-mysql-mode/200.tokenize-of-mysql-mode.md) 函数查看文本按照指定的分词器及 Json 形式参数的分词结果。|
