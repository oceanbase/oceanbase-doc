...
alter_table_action:
    ...
    | ALTER INDEX index_name
          [VISIBLE | INVISIBLE]
    | ALTER INDEX index_name STORAGE_CACHE_POLICY = {"hot" | "auto" | "none"}
    ...
    | {PARTITION | SUBPARTITION} partition_name STORAGE_CACHE_POLICY = {"hot" | "auto" | "none"}
    | SET STORAGE_CACHE_POLICY (storage_cache_policy_option)
    | FORCE
...
timeline_strategy_list:
    ...
    | BOUNDARY_COLUMN_UNIT = {"s" | "us" | "ms"}
    | HOT_RETENTON = intnum retention_time_unit
...
|STORAGE_CACHE_POLICY|可选项，表示共享存储模式下，用于指定表、分区、索引的热点缓存策略。详细介绍请参见下文 [storage_cache_policy](#storage_cache_policy) 。|
...
* `GLOBAL = {"hot" | "auto"}`：用于指定全表的数据是否为热点数据，取值如下：
  * `hot`：表示指定表为热存储，表下所有数据会缓存到本地云盘。
  * `auto`：表示指定表为自动冷热识别，由共享存储自行决定表下数据的存储方式，本地云盘空间不足时会优先淘汰这部分数据。
* `timeline_strategy_list`：表示时间轴策略参数列表，各参数间使用英文逗号分隔。
* `BOUNDARY_COLUMN = column_name`：用于判定热数据的列，基于该列的值经分区键表达式计算后和当前时间的间隔是否小于定义的 `HOT_RETENTION`。
* `BOUNDARY_COLUMN_UNIT = {"s" | "us" | "ms"}`：用于指定参数 `BOUNDARY_COLUMN` 的时间单位，具体如下：
  * 如果 `BOUNDARY_COLUMN` 为整型（`BIGINT` 或者 `INT` 类型）时，格式为 Unix 时间戳（如果格式非 Unix 时间戳，将不能正确识别 INT 类型表示的时间。），分区方式可以是 Range/Range Columns 两种。此时 `BOUNDARY_COLUMN_UNIT` 参数是必须指定，取值可以为：
    * 当分区列为 `INT` 类型时，`BOUNDARY_COLUMN_UNIT` 取值只能为 `s`。
    * 当分区列为 `BIGINT` 类型时，`BOUNDARY_COLUMN_UNIT` 取值可以是 `s`、`ms` 或者 `us`。
  * 如果 `BOUNDARY_COLUMN` 类型为时间类型（`TIMESTAMP`、`DATE`、`DATETIME` 或者 `YEAR` 类型）时，分区方式只能使用 Range Columns（Range 分区方式不支持时间类型列）。此时若指定 `BOUNDARY_COLUMN_UNIT` 参数，会报错。
* `HOT_RETENTION = intnum retention_time_unit`：用于指定距离当前时间多久范围内的数据是热数据。
* `GRANULARITY = "partition"`：可选项，用于指定热数据缓存级别，取值如下：
  * `partition`：默认值，表示分区级热数据缓存。

**使用示例**

* 修改表 `tbl1` 的热点数据为自动冷热识别。

  ```sql
  ALTER TABLE tbl1 SET STORAGE_CACHE_POLICY (GLOBAL = "auto");
  ```

* 修改表 `tbl2` 中距离当前 2 MONTH 的数据是热数据。
...
* 增加分区时指定 `STORAGE_CACHE_POLICY`：
  ...
  ```sql
  ALTER TABLE table_name ADD PARTITION (partition_list [STORAGE_CACHE_POLICY = {"hot" | "auto" | "none"}]);
  ```
...
* `hot`：表示指定分区为热存储，分区下所有数据会缓存到本地云盘。
* `auto`：表示指定分区为自动冷热识别，由共享存储自行决定分区下数据的存储方式，本地云盘空间不足时会优先淘汰这部分数据。
* `none`：默认值，表示该分区/索引的策略跟随上一级 `STORAGE_CACHE_POLICY` 取值。

**使用示例**

* 增加一级分区时指定 `STORAGE_CACHE_POLICY`。

  ```sql
  ALTER TABLE tbl3 ADD PARTITION (PARTITION p2 VALUES LESS THAN(300) STORAGE_CACHE_POLICY = "auto");
  ```

* 修改一级分区 `STORAGE_CACHE_POLICY`。

  ```sql
  ALTER TABLE tbl3 PARTITION p1 STORAGE_CACHE_POLICY = 'hot';
  ```

* 修改二级分区时指定 `STORAGE_CACHE_POLICY`。

  ```sql
  ALTER TABLE tbl4 SUBPARTITON sp3 STORAGE_CACHE_POLICY = 'auto';
  ```
