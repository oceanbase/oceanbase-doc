...
| ADD COLUMN GROUP([all columns, ]each column)  | 将表变更为列存表。具体解释如下：<ul><li><code>ADD COLUMN GROUP(all columns, each column)</code>：将表变更为行列混存的表。</li><li><code>ADD COLUMN GROUP(each column)</code>：将表变更为列存表。</li></ul>|
| DROP COLUMN GROUP([all columns, ]each column) | 移除表的列存方式。具体解释如下：<ul><li><code>DROP COLUMN GROUP(all columns, each column)</code>: 移除表的行列混存方式。</li><li><code>DROP COLUMN GROUP(each column)</code>: 移除表的列存方式。</li></ul>|
| index_column_group_option | 指定索引的列存选项。具体解释如下：<ul><li><code>WITH COLUMN GROUP(all columns, each column)</code>：指定添加行列混存的索引。</li><li><code>WITH COLUMN GROUP(each column)</code>：指定添加列存索引。</li></ul>|
...
* 为表创建列存索引。
  ...
  3. 查询 Column Group 信息。

      ```sql
      obclient [test_db]> SHOW CREATE TABLE tbl3;
      ```
      ...
      | tbl3  | CREATE TABLE `tbl3` (
        ...
        KEY `idx1_tbl3` (`col1`) BLOCK_SIZE 16384 LOCAL WITH COLUMN GROUP(each column)
      ) DEFAULT CHARSET = utf8mb4 ROW_FORMAT = DYNAMIC COMPRESSION = 'zstd_1.3.8' REPLICA_NUM = 1 BLOCK_SIZE = 16384 USE_BLOOM_FILTER = FALSE TABLET_SIZE = 134217728 PCTFREE = 0 |
      +-------+------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
      1 row in set
      ```
...
2. 将表 `tbl1` 的变更为行列混存表，然后再删除行列混存属性。
