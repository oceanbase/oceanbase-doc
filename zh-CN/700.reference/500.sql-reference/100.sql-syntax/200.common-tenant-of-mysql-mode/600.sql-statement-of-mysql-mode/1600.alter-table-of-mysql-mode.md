...
alter_table_action:
    ...
    | DROP COLUMN GROUP([all columns, ]each column) [DELAYED]
    | DROP CLUSTERING KEY
...
table_option:
    ...
    | MERGE_ENGINE [=] {partial_update | delete_insert | append_only}
...
| \[SET\] table_option                                       | 设置表级属性，可选以下参数： <ul><li> `REPLICA_NUM`：设置表的副本数（暂不支持）。</li>   <li> `tablegroup_name`：设置表所属的表组。</li>   <li> `BLOCK_SIZE`：设置表的微块大小，默认为 `16384`，即 16 KB，取值范围为 \[1024,1048576\]。</li>  <li> `lob_inrow_threshold`：用于配置 `INROW` 阈值，当 LOB 数据大小超过该阈值时，会转为 `OUTROW` 存储在 LOB Meta 表中，默认为 4KB。</li>   <li> `COMPRESSION`：设置表的压缩方式，默认为 `None`，表示不压缩。</li>   <li> `AUTO_INCREMENT`：设置表中自增列的下一个取值，可以增大或者减小。<main id="notice" type='notice'><h4>注意</h4><p>当减小一个表的自增字段的值时，需注意以下情况：</br>如果表中已存在数据并且自增列中的最大值不小于新指定的 <code>AUTO_INCREMENT</code> 值时，新的 <code>AUTO_INCREMENT</code> 值将自动调整为表中自增列现有最大值的下一个取值。例如，自增列当前的最大值为 5，当前 <code>AUTO_INCREMENT</code> 的值是 8，而 <code>AUTO_INCREMENT</code> 设置为介于 0 到 6 之间的任何值，语句执行成功后，实际的 <code>AUTO_INCREMENT</code> 值都会被调整为 6。</p></main></li>   <li> `comment`：设置表的注释信息。</li>   <li> `PROGRESSIVE_MERGE_NUM`：设置渐进合并步数，取值范围为 \[0,100\]。</li>   <li> `parallel_clause`：指定表级别的并行度。</li> <li> `NOPARALLEL`：并行度为 `1`，默认配置。</li>   <li> `PARALLEL integer`：指定并行度，`integer` 取值大于等于 `1`。</li>   <li> `MERGE_ENGINE`：设置表的表模式（更新模型），取值为 `partial_update`、`delete_insert` 或 `append_only`，支持在不同表模式之间进行 Online 转换，不阻塞业务读写。<main id="notice" type='notice'><h4>注意</h4><p><ul><li>该功能从 OceanBase 数据库 V5.0.1 版本开始引入，仅当集群内所有节点都升级到 V5.0.1 及之后的版本后才能执行。</li><li>表模式为 <code>append_only</code> 的表不支持转换为其它表模式。</li></ul></p></main></li> </ul>     |
...
| DROP COLUMN GROUP([all columns, ]each column) [DELAYED] | 移除表的存储格式。具体解释如下：<ul><li><code>DROP COLUMN GROUP(all columns, each column)</code>: 移除表的行存列存冗余格式。</li><li><code>DROP COLUMN GROUP(all columns)</code>: 移除表的行存格式。</li><li><code>DROP COLUMN GROUP(each column)</code>: 移除表的列存格式。</li><li>`DELAYED`：可选项，用于将列存表 Online（异步、不阻塞 DML）转换为行存表。指定 `DELAYED` 后为 Online DDL，真正执行列存转行存操作的时机是执行合并任务时；如果没有指定 `DELAYED`，默认为 Offline DDL，将同步执行列存转行存。<main id="notice" type='notice'><h4>注意</h4><p><ul><li>该功能从 OceanBase 数据库 V5.0.1 版本开始引入，仅当集群内所有节点都升级到 V5.0.1 及之后的版本后才能执行。</li><li>对于在 V5.0.1 之前创建的行存列存冗余表（<code>WITH COLUMN GROUP(all columns, each column)</code>），不支持 Online 列存转行存，相关 SQL 会返回 <code>NOT SUPPORTED</code> 错误。</li></ul></p></main></li></ul>|
...
1. 创建一个非 TTL 表。
   ...
   ```shell
   obclient(root@mysql001)[infotest]> CREATE TABLE tbl5(
        order_id INT,
...
2. 为非 TTL 表新增 TTL 属性。
   ...
   ```shell
   obclient(root@mysql001)[infotest]> ALTER TABLE tbl5 TTL order_time + INTERVAL 7 DAY BY COMPACTION;
   ```

3. 为 TTL 表变更新的 TTL 列。将表的 TTL 列从 `order_time` 变更为 `payment_time`。

   ```shell
   obclient(root@mysql001)[infotest]> ALTER TABLE tbl5 TTL payment_time + INTERVAL 7 DAY BY COMPACTION;
   ```

4. 修改表的 TTL 策略。例如：将数据过期时间修改为 1 小时。

   ```shell
   obclient(root@mysql001)[infotest]> ALTER TABLE tbl5 SET TTL payment_time + INTERVAL 1 HOUR BY COMPACTION;
   ```

5. 删除表的 TTL 属性。

   ```shell
   obclient(root@mysql001)[infotest]> ALTER TABLE tbl5 REMOVE TTL;
   ```
