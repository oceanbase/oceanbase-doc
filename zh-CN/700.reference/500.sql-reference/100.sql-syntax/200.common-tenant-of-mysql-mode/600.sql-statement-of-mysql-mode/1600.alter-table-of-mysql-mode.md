...
alter_table_action:
    ...
    | ADD FULLTEXT [INDEX | KEY] [index_name](column_name, [column_name ...]) [WITH PARSER {SPACE | NGRAM | BENG | IK}] [PARSER_PROPERTIES[=](key=value [,key=value])] [LOCAL]
...
| WITH PARSER {SPACE \| NGRAM \| BENG \| IK}                 | 可选项，指定全文索引的分词器。取值如下：<ul><li>`SPACE`：默认值，表示按空格进行分词。</li><li>`NGRAM`：表示基于 N-Gram（中文）的分词方式。</li><li>`BENG`：基于 Beng （基础英文）的分词方式。</li><li>`IK`：表示基于 IK（中文）的分词方式。当前仅支持 `utf-8` 字符集。<main id="notice" type='explain'><h4>说明</h4><p>对于 OceanBase 数据库 V4.3.5 版本，从 V4.3.5 BP1 版本开始支持 <code>IK</code> 分词器。</p></main></li></ul> 可以使用 [TOKENIZE](../400.functions-of-mysql-mode/960.text-processing-functions-of-mysql-mode/200.tokenize-of-mysql-mode.md) 函数查看文本按照指定的分词器及 Json 形式参数的分词结果。|
| PARSER_PROPERTIES[=](key=value [,key=value]) | 可选项，指定全文索引的分词器属性。`key` 取值的详细介绍可参见下文 [PARSER_PROPERTIES](#parser_properties)。 <main id="notice" type='explain'><h4>说明</h4><p>对于 OceanBase 数据库 V4.3.5 版本，从 V4.3.5 BP1 版本开始支持指定分词器属性（<code>PARSER_PROPERTIES</code>）。</p></main>|
...
* `ik_mode`: 表示 `IK` 分词器的分词模式。取值如下：
  ...
  * `smart`：默认值，表示将尽可能分出最精简的有意义的词，采用后向最大采用一些计算方法来确定最优值。
  * `max_word`：表示将会尽可能多的输出词汇。
