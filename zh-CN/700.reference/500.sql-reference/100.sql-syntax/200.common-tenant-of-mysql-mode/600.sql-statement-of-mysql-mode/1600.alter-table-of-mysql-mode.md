...
* `GRANULARITY = {"partition" | "block"}`：可选项，用于指定热数据缓存级别，取值如下：
  ...
  * `block`：表示宏块级别识别出对应数据并转为热数据。
      ...
      <main id="notice" type='explain'>
        <h4>说明</h4>
        <p>在没有分区表的情况下，需要手动指定一段时间内数据为热点数据时，可以指定 <code>block</code>。详细使用宏块冷热策略的介绍，参见 <a href="../../../../../660.ob-shared-storage/400.customize-hotspot-cache-of-mysql-mode/700.create-macro-block-level-storage-cache-policy-of-mysql-mode.md">创建宏块级冷热策略</a>。</p>
      </main>
