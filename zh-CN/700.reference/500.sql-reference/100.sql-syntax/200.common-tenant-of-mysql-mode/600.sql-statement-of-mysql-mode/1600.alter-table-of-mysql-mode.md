...
* `GLOBAL = {"hot" | "auto" | "cold" | "none"}`：用于指定全表数据的热点缓存策略，取值如下：
...
`STORAGE_CACHE_POLICY = {"hot" | "auto" | "cold" | "none"}`：用于指定分区的数据是否为热点数据，取值如下：
  
* `hot`：表示指定分区为热存储，在缓存盘空间足够的情况下，分区下所有数据将会缓存到本地云盘。
* `auto`：表示指定分区热数据为系统自动识别。
* `cold`：表示指定分区中已经持久化到缓存的数据会主动触发缓存淘汰，后续访问的表数据可以存储在宏块内存缓存，不会持久化到宏块缓存和微块缓存。
