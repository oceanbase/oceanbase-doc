...
alter_table_action:
    ...
    | FORCE
    | SET {{TTL col_name + INTERVAL interval_num ttl_unit BY COMPACTION} | {REMOVE TTL}}
...
| TTL col_name + INTERVAL interval_num ttl_unit BY COMPACTION  | 指定表的 TTL 策略。具体解释如下：<ul><li>`col_name`：指定表的 TTL 过滤列。当前版本仅支持 Compaction TTL 表的列为 ora_rowscn。</li><li>`interval_num`：指定过期时间的整数数值。</li><li>`ttl_unit`：指定过期时间的单位。取值支持 `SECOND`、`MINUTE`、`HOUR`、`DAY`、`MONTH` 或 `YEAR`。</li> <li>`BY COMPACTION`：表示使用 Compaction 删除过期数据。</li></ul>|
| REMOVE TTL              | 表示删除表的过期策略。|
