...
|STORAGE_CACHE_POLICY|可选项，表示共享存储模式下，用于指定表、索引的热点缓存策略。详细介绍请参见下文 [storage_cache_policy](#storage_cache_policy) 。|
|ALTER {PARTITION \| SUBPARTITION} partition_name STORAGE_CACHE_POLICY|表示共享存储模式下，用于指定分区的热点缓存策略。详细介绍请参见下文 [partition_storage_cache_policy](#partition_storage_cache_policy) 。|
...
* `GLOBAL = {"hot" | "auto"| "none"}`：用于指定全表数据的热点缓存策略，取值如下：
  * `hot`：表示指定表的所有数据都为热数据，在缓存盘空间足够的情况下，表下所有数据将会缓存到本地云盘。
  * `auto`：表示指定表的热数据为系统自动识别。
  * `none`：表示该索引的策略跟随主表 `STORAGE_CACHE_POLICY` 取值。<main id="notice" type='notice'><h4>注意</h4><p>`none`属性只能给索引使用。</p></main>
...
* `STORAGE_CACHE_POLICY = {"hot" | "auto" | "none"}`：用于指定分区的数据是否为热点数据，取值如下：
