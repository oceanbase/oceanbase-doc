...
* `timeline_strategy_list`：表示时间轴策略参数列表，各参数间使用英文逗号分隔。热点缓存时间轴策略支持通过时间来判断分区数据是否为热数据的机制，系统根据配置的策略，自动调整本地缓存盘上的分区数据。
  ...
  * `BOUNDARY_COLUMN_UNIT = {"s"| "ms"}`：用于指定参数 `BOUNDARY_COLUMN` 的时间单位，仅支持 `BOUNDARY_COLUMN` 为整型时设置此参数，取值可以为：
...
* `hot`：表示指定分区的数据都为热数据，在缓存盘空间足够的情况下，分区下所有数据将会缓存到本地云盘。
* `auto`：表示指定表的热数据为系统自动识别。
* `none`：表示指定分区的热数据为系统自动识别。
