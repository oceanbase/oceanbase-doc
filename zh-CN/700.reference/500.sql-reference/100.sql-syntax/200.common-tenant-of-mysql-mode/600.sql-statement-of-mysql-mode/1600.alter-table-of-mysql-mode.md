...
| SEMISTRUCT_PROPERTIES | 可选项，用于指定半结构化编码类型，以键值对形式填写。具体如下：<ul><li>`encoding_type`：表示半结构化编码类型，取值为 `encoding` 或 `none`，默认值为 `none`。</li><li>`freq_threshold`：表示频率阈值，取值范围 `(0, 100]`，默认值为 `90`。对于 JSON 对象中出现频率高于此阈值的路径，系统会将其作为独立的子列存储，这些子列被称为频繁列。较低的 `freq_threshold` 值会增加频繁列的数量，从而提高压缩比，但同时也会增加查询性能开销。</li></ul></br>功能使用说明可参见[使用半结构化编码](../100.basic-elements-of-mysql-mode/100.data-type-of-mysql-mode/800.json-formatted-data-type-of-mysql-mode/200.create-a-json-value-of-mysql-mode.md)。<main id="notice" type='notice'><h4>注意</h4><p>该参数从 V4.4.1 版本开始开始支持。</p></main>|
