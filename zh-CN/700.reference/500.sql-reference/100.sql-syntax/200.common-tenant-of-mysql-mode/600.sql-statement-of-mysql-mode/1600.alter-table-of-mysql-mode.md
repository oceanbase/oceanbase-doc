...
| TTL col_name + INTERVAL interval_num ttl_unit BY COMPACTION  | 修改表的 TTL 策略。具体解释如下：<ul><li>`col_name`：指定表的 TTL 过滤列。当前版本仅支持指定列名为 `ora_rowscn`（内部隐藏列，记录数据最后一次更新的时间戳）。</li><li>`interval_num`：指定过期时间的整数数值。</li><li>`ttl_unit`：指定过期时间的单位。取值支持 `SECOND`、`MINUTE`、`HOUR`、`DAY`、`MONTH` 或 `YEAR`。</li> <li>`BY COMPACTION`：表示使用 Compaction 删除过期数据。</li></ul> |
