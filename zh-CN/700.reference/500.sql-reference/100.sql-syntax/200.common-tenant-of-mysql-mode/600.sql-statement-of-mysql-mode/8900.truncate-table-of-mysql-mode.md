...
* `TRUNCATE TABLE` 操作删除并重新创建表，这比逐行删除（使用 `DELETE FROM` 语句）要快得多，特别是对于大表。
...
1. 创建表 `test_tbl1`。
    ...
    ```sql
    CREATE TABLE test_tbl1(col1 INT AUTO_INCREMENT PRIMARY KEY, col2 VARCHAR(20)) PARTITION BY HASH(col1) PARTITIONS 5;
    ```
