...
<main id="notice" type='notice'><h4>建议阅读顺序</h4><p>先看 <strong>DSL 骨架示例</strong>（整体由哪些顶层字段组成），再按需对照 <strong>语法定义</strong> 中的 BNF 分块；各字段含义与约束以 <strong>详细参数说明</strong> 表格为准。端到端建表与更多场景示例见文末相关文档索引混合搜索（SQL 接口）。</p></main>
