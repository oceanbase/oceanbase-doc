...
```sql
single_term = range_query | match_query | match_phrase_query | term_query | terms_query | query_string | multi_match
...
<table>
  ...
  <tbody>
    ...
    <tr>
      <td rowspan="15">single term（单个词条搜索）</td>
    </tr>
    <tr>
      <td>range</td>
    ...
    <tr>
      <td>match_phrase</td>
      <td>全文匹配，在单列上做短语匹配。对应列须建立 <code>FTS_INDEX_TYPE = PHRASE_MATCH</code> 的单列全文索引。支持完整对象形式 <code>{"field":{"query":"...","slop":n,"boost":...}}</code> 或简化形式 <code>{"field":"短语字符串"}</code>。短语按索引分词器分词；若短语中含停用词，对应位置可匹配任意词元（Token）。<code>slop</code> 为词元间最大允许偏移，<code>0</code> 表示精确匹配，正整数表示模糊匹配（可选，默认 <code>0</code>）。</td>
    </tr>
    <tr>
      <td>term</td>
