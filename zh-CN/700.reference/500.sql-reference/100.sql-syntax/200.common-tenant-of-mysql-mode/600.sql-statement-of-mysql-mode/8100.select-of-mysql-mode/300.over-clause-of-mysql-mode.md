...
`OVER` 子句用于定义 "窗口"。"窗口" 也称为 Frame，OceanBase 数据库同时支持 `ROWS` 与 `RANGE` 两种 Frame 语义，前者是基于物理行偏移的窗口，后者则是基于逻辑值偏移的窗口。

在 `SELECT` 查询中，`WINDOW` 子句用于用于定义具有特定窗口规范的命名窗口。使用分析函数（某些数据库下也叫做窗口函数），并在它后面加上 `OVER window_name` 来引用窗口规范。适用于 `OVER` 子句的函数称为分析函数，有关分析函数的详细信息，请参见 [函数概述](../../400.functions-of-mysql-mode/100.function-overview-of-mysql-mode.md) 中 **分析函数** 章节。
