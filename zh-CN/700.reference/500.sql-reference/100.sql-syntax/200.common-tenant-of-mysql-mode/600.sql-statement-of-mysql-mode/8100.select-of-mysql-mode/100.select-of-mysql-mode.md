...
order_by_condition:
    expression [ASC | DESC]
    | vector_distance_function(vector_column, query_vector) APPROXIMATE
...
1. 创建带向量索引的表。
   ...
   CREATE TABLE vector_table (
     ...
     name VARCHAR(50),
     vector_col VECTOR(3),
     vector index idx_vector(vector_col) with (distance=l2, type=hnsw)
   );
   ```
...
<li>支持的向量距离函数包括 <code>l2_distance</code>（L2 距离）和 <code>cosine_distance</code>（余弦距离）。</li>
</ul>
</main>
