...
| STRAIGHT_JOIN | 可选项，强制优化器按照 `FROM` 子句中所列出的顺序连接表。调优命令，不影响执行结果。 <main id="notice" type='notice'><h4>注意</h4><p>OceanBase 数据库并不完全兼容 MySQL 中的 <code>STRAIGHT_JOIN</code> 语法。当 <code>STRAIGHT_JOIN</code> 能够明确指定表连接的顺序时，OceanBase 数据库会按照这个指示执行。如果 <code>STRAIGHT_JOIN</code> 无法确定唯一的连接顺序，OceanBase 数据库将会忽略这个命令。</p></main>|
