...
* FORMAT 子句用于指定读取文件格式相关的属性，支持 CSV/PARQUET/ORC 三种文件格式。
  * **TYPE = 'CSV'** 时，包含以下字段：
    ...
    * `PARES_HEADER`:指定该配置后直接获取 CSV 文件第一列信息，将其作为每列的列名。<main id="notice" type='notice'><h4>注意</h4><p>`PARES_HEADER`不能和 `SKIP_HEADER` 同时使用，语意上会有冲突。</p></main>
...
* `data/files/`:指定外表文件存放的路径。通常外表的数据文件存放于单独一个目录中，文件夹中可以包含子目录，在创建表时，外表会自动收集该目录中的所有文件。
