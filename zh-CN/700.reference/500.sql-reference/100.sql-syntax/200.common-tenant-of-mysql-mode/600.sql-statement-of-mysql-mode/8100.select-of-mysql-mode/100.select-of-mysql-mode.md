...
select_stmt:
    SELECT [hint_options]
        ...
        [GROUP BY group_by_condition_list [WITH ROLLUP]
         | GROUP BY group_by_summary_option (column_name_list)]
...
| GROUP BY group_by_summary_option (column_name_list) | 可选项，用于高级分组汇总操作，能够从不同维度分析数据。详细介绍可参见下文 [group_by_summary_option](#group_by_summary_option)。<main id="notice" type='explain'><h4>说明</h4><p>对于 OceanBase 数据库 V4.3.5 版本，从 V4.3.5 BP1 版本开始支持高级分组汇总操作。</p></main>|
...
* `ROLLUP`：表示分层汇总方式，用于生成多层次的汇总结果。具有以下特点：

  * 按照分组列的顺序，依次减少分组列的数量，直到只保留总计行。
  * 生成的结果集包括：
...
* `CUBE`：表示多维汇总方式，用于生成所有可能的分组组合。它会为分组列的所有子集生成汇总数据。生成的结果集包括：
...
* `GROUPING SETS`：表示自定义分组方式，允许指定多个分组组合。它不会自动生成所有可能的分组组合，而是仅返回指定的分组结果。具有以下特点：
