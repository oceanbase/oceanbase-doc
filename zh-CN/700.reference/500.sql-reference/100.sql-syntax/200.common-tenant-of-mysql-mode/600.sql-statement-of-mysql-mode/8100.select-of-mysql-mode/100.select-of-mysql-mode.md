...
| [ table_function \| location_url ] |可选项，当前 URL 外表支持两种语法形式，详细介绍可参见下文：<ul><li> [table_function](#table_function) 形式。</li><li>[location_url](#location-url) 形式。</li></ul>|
|location_url||
...
* FORMAT 子句用于指定读取件格式相关的属性，支持 CSV/PARQUET/ORC 三种文件格式，使用 TYPE 指定导出文件格式。对于 ODPS 的数据，需要使用PROPERTIES 子句。
