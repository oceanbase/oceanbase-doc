...
| vector_distance_function      | 向量距离函数，支持 `l2_distance`（L2 距离）和 `cosine_distance`（余弦距离）。|
| vector_column                 | 向量列名，必须是 VECTOR 类型的列。|
| query_vector                  | 查询向量，格式为 `'[value1, value2, ...]'`，维度必须与向量列匹配。|
| APPROXIMATE                   | 必需关键字，用于启用向量索引的近似搜索，提高查询性能。|
| inner_product | 内积函数，表示两个向量之间的一种乘积。|
...
* LOCATION 子句用于指定外表文件存放的路径。通常外表的数据文件存放于单独一个目录中，文件夹中可以包含子目录，在创建表时，外表会自动收集该目录中的所有文件。取值如下：

  * `file_path`：表示具体的外表文件路径信息，详细介绍如下：
  
    * 本地 `LOCATION` 格式为：`LOCATION = '[file:// | sfile://] local_file_path'`
      ...
      * `sfile://`：该协议头表示在多个 OBServer 节点上遇到相同逻辑路径的文件时，系统会根据逻辑路径进行去重处理，确保只读取一份数据（支持多服务器负载均衡）。
        ...
        <main id="notice" type='notice'>
          <h4>注意</h4>
          <p><ul><li>在使用 <code>sfile://</code> 时，系统会对文件大小进行一致性校验。如果同一个逻辑路径下的文件在不同 OBServer 节点上的大小不一致，系统将报错 <code>OB_INVALID_EXTERNAL_FILE</code>，以避免因数据不一致导致结果错误。</li> <li>对于 V4.4.2 版本，从 V4.4.2 BP2 版本开始在 <code>SELECT ... FILES(LOCATION = ...);</code> 语法本地 Location 格式中加入了对 <code>sfile://</code> 协议头的支持。</li></ul></p>
        </main>
  ...
  * `@location_name['/path']`：表示引用 Location 对象创建外表。`['/path']` 为可选项，表示子目录。关于创建 Location 对象详细介绍信息，参见 [CREATE LOCATION](../2230.create-location-of-mysql-mode-in-sql.md)。

    <main id="notice" type='explain'>
      <h4>说明</h4>
      <p>对于 OceanBase 数据库 V4.4.x 版本，从 V4.4.1 版本开始支持 <code>@location_name['/path']</code> 参数。</p>
    </main>
...
* FORMAT 子句用于指定读取文件格式相关的属性，支持 CSV/PARQUET/ORC 三种文件格式。
  ...
  * **TYPE = 'CSV'** 时，包含以下字段：
    ...
    * `BYTEORDERMARK`：可选项，用于设置是否需要检测并跳过 BOM 字符，不指定该属性时不会进行 BOM 字符检测。取值如下：

      * `check`：表示检测并跳过 BOM 字符（推荐用于 UTF-8 with BOM 的文件）。
      * `nocheck`：表示不进行 BOM 字符检测（适用于无 BOM 字符的 UTF-8 文件）。

      <main id="notice" type='explain'>
        <h4>说明</h4>
        <p>对于 V4.4.2 版本，从 V4.4.2 BP3 版本开始支持 <code>BYTEORDERMARK</code>。</p>
      </main>
...
<p>向量距离查询需要满足以下条件：</p>
<ul>
<li>必须使用 <code>APPROXIMATE</code> 关键字启用向量索引的近似搜索。</li>
<li>查询向量 <code>query_vector</code> 的维度必须与向量列的维度匹配。</li>
<li>建议在向量列上创建相应的向量索引以提高查询性能。</li>
<li>支持的向量距离函数包括 <code>l2_distance</code>（L2 距离）、<code>negative_inner_product</code>（负内积）和 <code>cosine_distance</code>（余弦距离）。</li>
</ul>
</main>
