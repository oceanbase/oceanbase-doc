...
* `data/files/`:用于存储外部表的数据文件。建议将所有数据文件统一存放在此目录下，支持多层子目录结构（例如 /data/files/subdir/）。创建外部表时，系统会自动扫描并加载该路径下（包括子目录）的所有文件，无需手动指定单个文件。
* FORMAT 子句用于指定读取件格式相关的属性，支持 CSV/PARQUET/ORC/ODPS 四种文件格式：
