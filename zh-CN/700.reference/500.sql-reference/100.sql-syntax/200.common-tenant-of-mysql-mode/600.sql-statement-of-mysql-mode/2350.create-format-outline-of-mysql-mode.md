...
### 使用 `FORMAT_SQL_TEXT` 创建 Outline 的示例
...
在这个示例中，通过 `CREATE FORMAT OUTLINE` 使用 `FORMAT_SQL_TEXT` 创建了一个 Outline，然后在实际的 SQL 语句中通过注释 `/*+ my_outline */` 使用了这个 Outline。
...
### 使用 `FORMAT_SQL_ID` 创建 Outline 的示例
...
在这个示例中，首先通过 `CREATE FORMAT OUTLINE` 使用 `FORMAT_SQL_ID` 创建了一个 Outline，并在实际的 SQL 语句中通过注释 `/*+ my_sql_id_outline */` 使用了这个 Outline。
