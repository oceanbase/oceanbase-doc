...
* `[LOW_PRIORITY] WRITE`：
  ...
  * `LOW_PRIORITY WRITE`：与 `WRITE` 表现相同。
...
4. 在会话 1 中执行如下语句，为 `orders` 表设置读锁。

   ```sql
   obclient> LOCK TABLES orders READ;
   ```

5. 在会话 1 中执行如下语句，查看表 `orders` 数据。
...
6. 在会话 1 中执行如下语句，向表 `orders` 中插入一行数据。
...
8. 在会话 2 中执行如下语句，向表 `orders` 中插入一行数据。
...
10. 在会话 1 中执行如下语句，向表 `orders` 中插入一行数据。

    ```sql
    obclient> INSERT INTO orders VALUES(2, 'Bob', 2000.00);
    ```

11. 在会话 2 中执行如下语句，向表 `orders` 中插入一行数据。
