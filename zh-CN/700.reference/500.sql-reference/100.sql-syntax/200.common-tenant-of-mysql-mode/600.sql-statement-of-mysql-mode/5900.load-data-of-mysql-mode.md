...
| url_table_function_expr | 可选项，通过 FILES 和 SOURCE 两大部分来定义如何从文件系统或数据源读取数据。<ul><li>FILES 用于描述需要导入的数据的地址，格式等信息。有关使用 FILES 的详细介绍，可参见下文 [FILES](#FILES)。</li> <li>SOURCE 用于定义通过 ODPS 格式的数据源进行数据提取。有关使用 SOURCE 的详细介绍，可参见下文 [SOURCE](#SOURCE)。</li></ul>|
