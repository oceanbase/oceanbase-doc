...
1. 设置全局安全路径（由于安全原因，只能使用通过本地 Unix Socket 连接的 Client 执行修改）。

   ```shell
   obclient> SET GLOBAL secure_file_priv = "/usr/sqldump"
...
2. 重连数据库后，从外部文件导入数据。
   ...
   ```shell
   obclient> LOAD DATA INFILE '/usr/sqldump/test.sql' INTO TABLE t1;
   Query OK, 0 rows affected 
   ```
