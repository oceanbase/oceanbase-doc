...
* `INTO 'into_string'`：可选项，用于设置错误信息还会被写入目标目录中的文件。当不指定 `INTO 'into_string'` 时，错误信息只记录于 `warning buffer` 中，可以通过 `show warnings` 查看。`into_string` 表示存放错误信息的目录，格式如下：

  * 当错误信息存放在本地时，`into_string` 格式为 `[file://] local_file_path`，其中 `local_file_path` 可以为相对路径，也可以为绝对路径。如果填写的是相对路径，则当前目录必须为 OceanBase 数据库的安装目录；`secure_file_priv` 用于配置 OBServer 节点有权限访问的文件路径。`local_file_path` 只能是 `secure_file_priv` 路径的子路径。
  * 当错误信息存放在远端时（参考创建外表语法中 Location 的部分），`into_string` 格式如下：
...
* `BADFILE 'badfile_string'`：指定错误数据文件存放的路径。`badfile_string` 取值格式同 `into_string` 的格式。
