...
## 旁路导入相关限制（LOAD DATA 语法）

使用 `LOAD DATA` 配合 `APPEND`、`DIRECT(...)` 等 Hint 或租户级旁路配置执行导入时，除 [数据导入概述](../../../../../620.obap/300.obap-data-transfer/300.data-import/100.data-import-overview.md) 中说明的**旁路导入能力**适用范围与限制外，还须满足以下 **`LOAD DATA` 语句级**限制：

* 不支持单行超过 2M 的数据导入。
* `LOAD DATA` 语句支持在多行事务中运行，在执行时会主动提交之前的事务。
* 对于带唯一索引的**全量**旁路导入，当出现重复唯一索引键时，暂时不支持 `REPLACE` 或 `IGNORE` 关键字，也不支持设置容错行。
* 当 `DIRECT(...)` 的 `load_mode` 取值为 `inc_replace` 时，语句中不允许再写 `REPLACE` 或 `IGNORE` 关键字。

<main id="notice" type='explain'>
  <h4>说明</h4>
  <p>导入过程中若发生 <strong>CSV 文件解析错误</strong>，可能在 OBServer 安装路径下的 <code>log</code> 子目录生成 <code>obloaddata.log</code> 日志文件。<strong>类型转换失败、主键冲突</strong>等情况不会写入该日志文件。使用 <code>FROM FILES</code> 且指定 <code>LOG ERRORS</code> 子句时，失败行的诊断信息记录方式以该子句及 <code>SHOW WARNINGS</code> 为准，参见下文 <a href="#log_errors">log_errors</a>。</p>
</main>
...
| REPLACE \| IGNORE                | 如果遇到唯一键冲突，`REPLACE` 表示覆盖冲突行，`IGNORE` 表示忽略冲突行。`LOAD DATA` 通过表的主键来判断数据是否重复，如果表不存在主键，则 `REPLACE` 与 `IGNORE` 选项没有区别。缺省情况下，既不按 `REPLACE` 覆盖冲突行，也不按 `IGNORE` 忽略冲突行；遇到唯一键冲突时，导入可能报错终止。 <main id="notice" type='notice'><h4>注意</h4><p><ul><li>在 MySQL 模式中执行 <code>LOAD DATA LOCAL INFILE</code> 命令时，系统会自动添加 <code>IGNORE</code> 选项。这种行为提供了与 MySQL 数据库更好的兼容性。</li><li>使用 <code>REPLACE</code> 或者 <code>IGNORE</code> 子句的情况下，如果并行度大于 <code>1</code>，发生冲突的行最后插入的记录可能与串行执行的结果不同。如果需要严格保证冲突记录的插入结果，请不要指定该语句的并行度（或将并行度设置为 <code>1</code>）。</li></ul></p></main> |
  ...
  * 本地 LOCATION 格式为 `LOCATION = '[file:// | sfile://] local_file_path'`
    ...
    * `sfile://`：该协议头表示在多个 OBServer 节点上遇到相同逻辑路径的文件时，系统会根据逻辑路径进行去重处理，确保只读取一份数据（支持多服务器负载均衡）。
      ...
      <main id="notice" type='notice'>
        <h4>注意</h4>
        <p><ul><li>在使用 <code>sfile://</code> 时，系统会对文件大小进行一致性校验。如果同一个逻辑路径下的文件在不同 OBServer 节点上的大小不一致，系统将报错 <code>OB_INVALID_EXTERNAL_FILE</code>，以避免因数据不一致导致结果错误。</li> <li>对于 V4.4.2 版本，从 V4.4.2 BP2 版本开始在 <code>LOAD DATA</code> 语法本地 Location 路径格式中加入了对 <code>sfile://</code> 协议头的支持。</li></ul></p>
      </main>
