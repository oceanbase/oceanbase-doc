...
* `LOG ERRORS`：表示开启导入过程中的错误诊断，允许将失败的行记录下来（当前版本记录于 warning buffer 中），而不是让整个操作因第一个错误而终止。配合 `REJECT LIMIT` 子句，可以控制允许的错误行数。
