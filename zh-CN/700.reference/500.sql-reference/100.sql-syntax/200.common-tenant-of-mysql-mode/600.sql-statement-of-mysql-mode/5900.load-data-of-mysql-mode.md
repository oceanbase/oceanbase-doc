...
LOAD DATA
   ...
   |[/*+ PARALLEL(N) direct(bool, int, ['full' | 'inc_replace']) */]
...
| direct | 使用 Hint 启用旁路导入功能。参数解释如下：<ul><li><code>bool</code>：参数表示给定的 CSV 文件是否需要排序，<code>true</code> 表示需要排序，<code>false</code> 表示不需要排序。</li><li><code>int</code>：表示最大容忍的错误行数。</li><li><code>full \| inc_replace</code>：可选参数，须使用英文单引号包起来，含义如下：<ul><li><code>full</code>：默认值，表示全量旁路导入。</li><li><code>inc_replace</code>：表示增量旁路导入。</li></ul></li></ul> 更多使用 `LOAD DATA` 旁路导入的信息，参见 [使用 LOAD DATA 语句旁路导入数据/文件](../../../../../500.data-migration/1100.bypass-import/200.use-load-data-statement-to-bypass-import-data.md)。|
