...
2. 重新连接数据库后，使用 `SELECT INTO OUTFILE` 语句导出数据。指定逗号作为两个字段值之间的标志；对字符串类型的值使用 `"` 字符包裹；使用换行符作为结束标志。
   ...
   * 并行写多个文件，指定文件名的前缀为 `dd2024`，并且每个文件的大小不超过 4MB。
    ...
    <main id="notice" type='explain'>
      <h4>说明</h4>
      <p><ul><li>为了避免多个导出任务同时执行时可能出现的报错（如文件已存在）和仅部分数据导出的问题，建议将导出路径设置为不同的路径（例如，<code>test/data1</code> 和 <code>test/data2</code>）。</li><li>当 <code>SINGLE = FALS</code>，且导出因为 file already exist 等原因失败后，可以删除当前导出目录下所有以指定前缀（如 <code>data</code>）命名的文件，或者直接删除整个导出目录并重新创建，然后再次尝试执行导出操作。</li></ul></p>
    </main>
