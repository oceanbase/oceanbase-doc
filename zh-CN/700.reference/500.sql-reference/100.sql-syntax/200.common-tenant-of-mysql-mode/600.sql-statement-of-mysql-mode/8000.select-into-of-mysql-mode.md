...
* 将导出文件保存在 HDFS 上时，格式为：
  ...
  * SIMPLE 认证的导出格式为：`hdfs://\$NAMENODE:\$PORT/\$PATH/\$FILENAME`。
  * KERBEROS 认证的导出格式为：`hdfs://\$NAMENODE:\$PORT/\$PATH/\$FILENAME?principal=\$PRINCIPAL&keytab=\$KEYTAB&krb5conf=\$KRB5CONF&configs=\$CONFIGS`。
  ...
  * `\$FILENAME`：表示导出文件的名称。
  ...
  不同于 SIMPLE 认证的方式，KERBEROS 认证还需要提供以下参数：

  * `\$PRINCIPAL`：表示 Kerberos 主体名称。
  * `\$KEYTAB`：表示 Kerberos 密钥表文件的路径。
  * `\$KRB5CONF`：表示 Kerberos 配置文件路径。
  * `\$CONFIGS`：表示 HDFS 额外配置参数，格式为键值对列表。
