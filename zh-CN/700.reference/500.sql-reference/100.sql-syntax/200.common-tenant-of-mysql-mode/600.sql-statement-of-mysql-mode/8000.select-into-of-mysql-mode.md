...
| PARTITION BY part_expr | 可选项，用于控制导出数据的分区方式，`part_expr` 的值作为导出路径的一部分，对每行数据计算 `part_expr` 的值，`part_expr` 取值相同的行属于同一个分区，将导出到同一个目录中。<main id="notice" type='notice'><h4>注意</h4><p><ul><li>当数据按分区导出数据时，要求 <code>SINGLE = FALSE</code>，即允许导出多文件。</li><li>当前按分区导出数据仅支持导入到 OSS 上。</li></ul></p></main>|
...
* 将导出文件保存在 OSS 上时，格式为：`oss://\$PATH/\$FILENAME/?host=\$HOST&access_id=\$ACCESS_ID&access_key=\$ACCESSKEY`，其中各参数解释如下：
  ...
  * `\$ACCESSKEY`：指定了访问 OSS 服务所需的 Access Key Secret，用于身份验证。
  ...
  <main id="notice" type='explain'>
    <h4>说明</h4>
    <p><ul><li>OceanBase 数据库适配 S3 和基于 S3 协议的对象存储作为数据导出的目的端。</li><li>由于阿里云 OSS 有文件大小的限制，对于超过 5 GB 的文件，导出到 OSS 时会被拆分成多个文件，每个文件小于 5 GB。</li></ul></p>
  </main>

* 将导出文件保存在 HDFS 上时，格式为：

  <main id="notice" type='notice'>
    <h4>注意</h4>
    <p>使用 HDFS 外表写能力时候，需要 OceanBase 数据库在具有 JAVA SDK 的环境当中使用。关于部署 JAVA SDK 环境的详细介绍信息，参见 <a href="../../../../../400.deploy/300.deploy-oceanbase-enterprise-edition/500.deploy-the-ob-java-sdk-environment.md">部署 OceanBase 数据库 JAVA SDK 环境</a>。</p>
  </main>
  ...
  * SIMPLE 认证的导出格式：
    ...
    * 说明：
      ...
      * 当使用基础认证（SIMPLE）时，可通过 `username` 参数指定 HDFS 用户名。
      * 若未指定 `username` 参数时，默认使用 OceanBase 数据库配置中定义的 HDFS 用户名。
  ...
  * KERBEROS 认证的导出格式：
    ...
    * Kerberos 认证需提供 Kerberos 主体（`principal`）、密钥表（`keytab`）和 Kerberos 配置文件（`krb5conf`）。
    * `username` 参数在此场景下不适用，因为 Kerberos 认证通过 `principal` 标识身份。

  其中各参数解释如下：

  * 通用参数：

    * `\$NAMENODE`：表示 HDFS NameNode 的 IP 地址。
    * `\$PORT`：表示 HDFS NameNode 的端口。
    * `\$PATH`：表示文件在 HDFS 中的存储路径。
    * `\$FILENAME`：表示导出文件的名称。

  * SIMPLE 认证专用参数：

    * `?username=\$USERNAME`：可选项，表示 HDFS 用户名。未指定 `username` 时，默认使用 OceanBase 数据库配置中定义的 HDFS 用户名。

  * KERBEROS 认证专用参数：

    * `\$PRINCIPAL`：表示 Kerberos 主体名称。例如 `principal=hdfs/hadoop@EXAMPLE.COM`。
    * `\$KEYTAB`：表示 Kerberos 密钥表文件的路径。例如 `keytab=/path/to/hdfs.keytab`。
    * `\$KRB5CONF`：表示 Kerberos 配置文件路径。例如 `krb5conf=/path/to/krb5.conf`。
    * `\$CONFIGS`：表示 HDFS 额外配置参数，格式为键值对列表。例如 `configs=dfs.data.transfer.protection=authentication,privacy`。

  **导出文件保存在 HDFS 上 file_name 取值示例如下：**

  * SIMPLE 认证（基础认证）：

    * 适用于无 Kerberos 认证的 HDFS 集群。
  ...
  * KERBEROS 认证：
    ...
    * 适用于需要 Kerberos 认证的 HDFS 集群。

    * 查询参数需按需填写，支持多个配置项（如 configs）。
...
* `BUFFER_SIZE [=] {int | string}`：用于控制导出时每个线程为每个分区专门申请的内存大小（不分区可视为单个分区），默认取值为 1 MB。
  ...
  <main id="notice" type='explain'>
    <h4>说明</h4>
    <p><code>BUFFER_SIZE</code> 用于导出性能调优，当机器内存充足且希望提高导出效率时，可设置一个较大的值（例如 4 MB），当机器内存不足时，可设置一个较小的值（例如 4 KB。设置为 0 时，表示单个线程中所有分区都使用一块公共内存）。</p>
  </main>
...
将 OceanBase 数据库表 `lineitem_external` 的数据导出到 HDFS 上，并存成 CSV 文件格式。
...
不同于 SIMPLE 认证的模式方式，KERBEROS 认证还需要提供更多的配置文件和密钥文件。将 OceanBase 数据库表 `lineitem_external` 的数据导出到 HDFS 上，并存成 CSV 文件格式。
