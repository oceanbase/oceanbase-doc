...
| index_column_group_option | 指定索引的列存选项。详细介绍可参见下文 [index_column_group_option](#index_column_group_option)。|
...
3. 查询 Column Group 信息。
    ...
    ```sql
    obclient [test_db]> SHOW CREATE TABLE test_tbl1;
    ```
    ...
    | test_tbl1 | CREATE TABLE `test_tbl1` (
      ...
      KEY `idx1_test_tbl1` (`col1`) BLOCK_SIZE 16384 LOCAL WITH COLUMN GROUP(each column)
