...
<main id="notice" type='notice'>
  <h4>注意</h4>
  <p>只有在共享存储模式下，才可以在创建索引时指定热点缓存策略（STORAGE_CACHE_POLICY）。索引可以指定表级 <code>STORAGE_CACHE_POLICY</code>。索引有分区的话，可以指定分区级的 <code>STORAGE_CACHE_POLICY</code>。其中局部索引默认跟随数据表的分区规则，全局索引拥有自己的分区规则，和表的分区独立。</p>
</main>

* 局部索引如果未指定表级 `STORAGE_CACHE_POLICY`，则默认为 `none`（局部索引的表级可以使用 `none`），表示使用数据表的表级 `STORAGE_CACHE_POLICY`。局部索引的分区也同步数据表对应的分区的 `STORAGE_CACHE_POLICY`。
  * 如果指定了表级 `STORAGE_CACHE_POLICY`，则局部索引的分区同步索引的表级 `STORAGE_CACHE_POLICY`。
* 全局索引如果未指定表级 `STORAGE_CACHE_POLICY`，则默认为 `none`。
* `GLOBAL = {"hot" | "auto" | "none"}`：用于指定索引的数据是否为热点数据，取值如下：
  * `hot`：表示指定索引为热存储，索引下所有数据会缓存到本地云盘，在空间允许时全部缓存。
  * `auto`：表示指定索引为自动冷热识别，由共享存储自行决定索引下数据的存储方式，本地云盘空间不足时会优先淘汰这部分数据。
  * `none`：默认值，表示该索引的策略跟随上一级 `STORAGE_CACHE_POLICY` 取值。局部索引的表级可以使用 `none` 属性。
...
* 创建表时创建索引，指定索引的表级 `STORAGE_CACHE_POLICY`。
...
* 给已有表增加索引时，指定索引的表级 `STORAGE_CACHE_POLICY`。
...
* 修改已有索引的表级 STORAGE_CACHE_POLICY
