...
partition_option:
    PARTITION BY {HASH(expression) | KEY([column_name_list]) | RANGE {(expression) | COLUMNS (column_name_list)}}
...
| partition_option | 指定创建索引分区的选项，支持指定哈希分区、键分区、范围分区、列表分区以及外部表分区。全局索引可以指定分区级别的 STORAGE_CACHE_POLICY。|
...
* 在表 `tbl2` 上新增全局索引表 `idx3`，并配置索引表 `idx3` 的数据为热数据。

  ```sql
  ALTER TABLE tbl2 ADD INDEX idx3(col1, col2) GLOBAL 
    STORAGE_CACHE_POLICY (GLOBAL = "hot");
  ```

* 修修改表 `test_table2` 上索引表 `idx2` 的数据是否缓存为本地云盘由系统决定。

  ```sql
  ALTER TABLE test_table2 ALTER INDEX idx2
...
* 创建索引后，通过 ALTER TABLE 设置分区级别的 STORAGE_CACHE_POLICY

  ```sql
  ALTER TABLE tbl1 ALTER INDEX idx_global PARTITION p1 STORAGE_CACHE_POLICY = "hot";
  ALTER TABLE tbl1 ALTER INDEX idx_global PARTITION p2 STORAGE_CACHE_POLICY = "auto";
  ALTER TABLE tbl1 ALTER INDEX idx_global PARTITION p3 STORAGE_CACHE_POLICY = "none";
  ```
