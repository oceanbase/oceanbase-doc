...
1. **`WITH cte1 AS (SELECT col1, col2 FROM tbl1), cte2 AS (SELECT col3, col4 FROM tbl2)`：**
   - 在 `WITH` 子句中定义了两个 CTE，分别命名为 `cte1` 和 `cte2`。
   - `cte1` 包含了从表 `tbl1` 中选择的列 `col1` 和 `col2`。
   - `cte2` 包含了从表 `tbl2` 中选择的列 `col3` 和 `col4`。

2. **`SELECT col2, col4 FROM cte1 JOIN cte2 WHERE cte1.col1 = cte2.col3;`：**
   - 在顶层 `SELECT` 语句中引用了这两个 CTE。
   - 通过 `JOIN` 操作将 `cte1` 和 `cte2` 连接起来，然后通过 `WHERE` 子句进行条件过滤。
   - 结果集中包含了从 `cte1` 中选择的 `col2` 和从 `cte2` 中选择的 `col4` 列。

关联条件是通过 `WHERE cte1.col1 = cte2.col3` 指定的，它表示只选择那些满足条件的行。在 CTE 中定义的列名可以直接在顶层的 `SELECT` 语句中使用，使得整个查询更加清晰和易读。
