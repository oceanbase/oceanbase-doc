...
| DEFAULT TABLEGROUP table_group_name                   | 设置数据库默认表组信息，`NULL` 表示取消数据库默认表组。当您更改数据库的默认表组（DEFAULT TABLEGROUP）后，已经存在的表不会自动迁移到新设置的默认表组。
仅当新建表且在建表时没有指定表组的情况下，新的默认表组设置才会被应用。      |
