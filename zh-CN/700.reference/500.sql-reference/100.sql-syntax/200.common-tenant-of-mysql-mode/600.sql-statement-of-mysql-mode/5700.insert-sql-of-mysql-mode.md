  ...
  * `APPEND | DIRECT(bool, int, [load_mode])`：可选项，使用 Hint 启用旁路导入功能。
    ...
    * `DIRECT(bool, int, [load_mode])`：
      ...
      * `load_mode`：可选参数，表示旁路导入的模式。取值须使用英文单引号包起来，取值如下：
        ...
        * `inc`：用于增量旁路导入场景。主键冲突时按插入或忽略冲突的方式处理，具体行为与语句中的 `IGNORE` 等选项及 `inc` 模式下的执行语义有关。增量旁路导入的能力限制参见 [数据导入概述](../../../../../620.obap/300.obap-data-transfer/300.data-import/100.data-import-overview.md#全量旁路导入与增量旁路导入)。
        ...
        * `inc_replace`：表示增量导入，但不检查主键是否重复，相当于 `REPLACE` 语义的增量导入。
        ...
        更多使用 `INSERT INTO SELECT` 旁路导入的信息，参见 [数据导入概述](../../../../../620.obap/300.obap-data-transfer/300.data-import/100.data-import-overview.md)。
...
使用 `INSERT INTO ... SELECT ...` 配合 `APPEND`、`DIRECT(...)`、`enable_parallel_dml`、`parallel(N)` 等 Hint 执行旁路导入时，除 [数据导入概述](../../../../../620.obap/300.obap-data-transfer/300.data-import/100.data-import-overview.md) 中说明的**旁路导入能力**适用范围与限制外，还须满足以下 **`INSERT INTO SELECT` 语句级**限制：

### 全量旁路导入

* 只支持 PDML（Parallel Data Manipulation Language，并行数据操纵语言），非 PDML 不能用旁路导入。有关并行 DML 的更多信息，参见 [并行 DML](../../../../1000.performance-tuning-guide/500.sql-optimization/300.distributed-execution-plan/1000.parallel-dml.md)。
...
* 只支持 PDML，非 PDML 不能用旁路导入。有关并行 DML 的更多信息，参见 [并行 DML](../../../../1000.performance-tuning-guide/500.sql-optimization/300.distributed-execution-plan/1000.parallel-dml.md)。
* 有索引（不包括主键）的表不支持增量旁路导入。

<main id="notice" type='explain'>
  <h4>说明</h4>
  <p>触发器、生成列、Liboblog、闪回查询，以及增量旁路对目标表结构（外键、<code>GLOBAL</code> 索引、约束、唯一索引等）的限制，属于<strong>旁路导入能力</strong>层面的通用限制，请参见 <a href="../../../../../620.obap/300.obap-data-transfer/300.data-import/100.data-import-overview.md#全量旁路导入注意事项">数据导入概述 · 全量/增量旁路导入注意事项</a>。</p>
</main>
...
* [数据导入概述](../../../../../620.obap/300.obap-data-transfer/300.data-import/100.data-import-overview.md)
