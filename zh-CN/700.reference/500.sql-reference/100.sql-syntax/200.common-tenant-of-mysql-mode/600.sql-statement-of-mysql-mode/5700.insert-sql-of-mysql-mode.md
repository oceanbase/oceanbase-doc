...
| ON DUPLICATE KEY UPDATE update_asgn_list | 可选项，指定对重复主键或唯一键的处理操作。<ul><li>如果指定了 `ON DUPLICATE KEY UPDATE`，当要插入的主键或唯一键有重复时，会用配置值修改原有行的值。</li><li>如果不指定 `ON DUPLICATE KEY UPDATE`，当要插入的主键或唯一键有重复时，插入报错。</li></ul> `update_asgn_list` 表示赋值语句，详细介绍可参见下文 [update_asgn_list](#update_asgn_list)。|
