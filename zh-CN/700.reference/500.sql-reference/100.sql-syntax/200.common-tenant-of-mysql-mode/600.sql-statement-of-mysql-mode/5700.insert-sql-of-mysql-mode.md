...
| INSERT OVERWRITE        | 用于将查询结果替换表中的现有数据，即该语句会先清空表中的旧数据，再将新数据批量写入到表中。|
...
5. 使用 `INSERT OVERWRITE` 语句，基于 `col3` 大于 25 从 `source_tbl1` 中筛选数据，并将这些数据插入到 `target_tbl1` 中，替换其原有内容。
