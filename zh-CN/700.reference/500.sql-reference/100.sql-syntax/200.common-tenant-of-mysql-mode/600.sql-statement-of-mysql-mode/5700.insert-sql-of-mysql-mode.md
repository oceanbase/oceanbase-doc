  ...
  * `APPEND | DIRECT(bool, int, [load_mode])`：可选项，使用 Hint 启用旁路导入功能。
    ...
    * `DIRECT(bool, int, [load_mode])`：
      ...
      * `load_mode`：可选参数，表示旁路导入的模式。取值须使用英文单引号包起来，取值如下：
        ...
        * `inc`：用于增量旁路导入场景。主键冲突时按插入或忽略冲突的方式处理，具体行为与语句中的 `IGNORE` 等选项及 `inc` 模式下的执行语义有关。增量旁路导入的能力限制参见 [数据导入概述](../../../../../620.obap/300.obap-data-transfer/300.data-import/100.data-import-overview.md)。
...
<main id="notice" type='explain'>
  <h4>说明</h4>
  <p>触发器、生成列、Liboblog、闪回查询，以及增量旁路对目标表结构（外键、<code>GLOBAL</code> 索引、约束、唯一索引等）的限制，属于<strong>旁路导入能力</strong>层面的通用限制，请参见 <a href="../../../../../620.obap/300.obap-data-transfer/300.data-import/100.data-import-overview.md">数据导入概述 · 全量/增量旁路导入注意事项</a>。</p>
</main>
