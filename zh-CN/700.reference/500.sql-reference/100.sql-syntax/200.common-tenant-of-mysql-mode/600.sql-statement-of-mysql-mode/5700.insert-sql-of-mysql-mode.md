...
overwrite_table_insert:
    OVERWRITE table_name select_stmt;
```
...
| overwrite_table_insert  | 用于将查询结果替换表中的现有数据，即该语句会先清空表中的旧数据，再将新数据批量写入到表中。有关使用 `INSERT OVERWRITE` 语句的示例信息，参见 [插入数据](../../../../../300.develop/100.application-development-of-mysql-mode/400.write-data-of-mysql-mode/100.insert-data-of-mysql-mode-in-develop.md) 中的 **使用 INSERT OVERWRITE 语句插入数据** 章节。|
