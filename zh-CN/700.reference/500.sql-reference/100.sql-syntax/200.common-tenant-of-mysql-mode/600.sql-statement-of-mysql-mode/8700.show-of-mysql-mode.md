...
SHOW {
   ...
   | [FULL] PROCESSLIST
   | [EXTENDED] INDEX {FROM | IN} table_name [{FROM | IN} database_name]
   | [EXTENDED] [FULL] {COLUMNS | FIELDS} {FROM | IN} table_name [{FROM | IN} database_name]
...
| \[EXTENDED\] INDEX {FROM \| IN} table_name [{FROM \| IN} database_name]        | 展示指定表的列信息。 `EXTENDED` 关键字用于展示数据库内部使用的隐藏索引的信息。 |
| \[EXTENDED\]\[FULL\]  {COLUMNS \| FIELDS}  {FROM \| IN} table_name [{FROM \| IN} database_name]   | 展示指定表的列信息。<ul><li>`EXTENDED` 关键字用于展示数据库内部使用的隐藏列的信息。 </li><li>`FULL` 关键字用于展示列的字符序、权限与注释。</li></ul> |
| TABLEGROUPS                                                                     | 展示表组信息。  |
...
* 通过 `show columns` 和 `show extended columns` 分别查看列信息和隐藏的列信息。
...
* 通过 `show index` 和 `show extended index` 分别查看索引信息和隐藏的索引信息。

  ```sql
  obclient> create index tbl2_index on tbl2(a, b ASC);
  Query OK, 0 rows affected
