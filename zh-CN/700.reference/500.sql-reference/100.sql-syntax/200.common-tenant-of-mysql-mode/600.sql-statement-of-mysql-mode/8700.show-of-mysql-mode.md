...
| CREATE LOCATION location_name | 用于查看创建 Location 对象的语句。<main id="notice" type='explain'><h4>说明</h4><p>对于 OceanBase 数据库 V4.4.x 版本，从 V4.4.1 版本开始支持该语法。</p></main>|
| LOCATIONS | 用于查看当前租户的 Location 对象。<main id="notice" type='explain'><h4>说明</h4><p>对于 OceanBase 数据库 V4.4.x 版本，从 V4.4.1 版本开始支持该语法。</p></main>|
...
| SENSITIVE RULES show_sensitive_rule_option | 展示当前租户下的敏感列保护规则。可选：仅使用 `LIKE`/`WHERE` 过滤；或使用 `FROM` \| `IN` 指定 `relation_factor`（表），并可选再跟 `FROM` \| `IN` `database_name` 限定库；或使用 `FROM` \| `IN` `DATABASE` `database_name` 按库筛选。规则定义参见 [CREATE SENSITIVE RULE](2470.create-sensitive-rule-of-mysql-mode.md)。 |
...
* 查看 Location 下存在的文件。
  ...
  假设已创建 Location 对象 `test_location_local`，且其指向的目录下存在若干 CSV 文件和子目录。创建 Location 的示例如下：

  ```sql
  obclient> CREATE LOCATION test_location_local URL = 'file:///home/admin/test_csv';
  ```
  ...
  ```sql
  obclient> SHOW FILES IN LOCATION @test_location_local;
  ```

* 查看 Location 下指定子目录的文件。

  ```sql
  obclient> SHOW FILES IN LOCATION @test_location_local'/subdir';
  ```

* 使用正则表达式筛选 Location 下的文件。

  ```sql
  obclient> SHOW FILES IN LOCATION @test_location_local PATTERN = '.*\\.csv$';
  ```

* 查看 Location 下指定子目录中匹配正则表达式的文件。

  ```sql
  obclient> SHOW FILES IN LOCATION @test_location_local'/subdir' PATTERN = 'sales.*\\.csv$';
  ```
...
* 查看创建 Catalog 的语句。
    ...
    ```sql
    obclient> SHOW CREATE CATALOG iceberg_hms;
    ```
...
* 使用 `SHOW FULL TABLES` 查看表的类型。

  ```sql
  obclient> SHOW FULL TABLES;
  ```

* 查看 `test` 库中所有表的详细信息。

  ```sql
  obclient> SHOW TABLE STATUS;
  ```
...
* 查看创建数据库的语句。
  ...
  ```sql
  obclient> SHOW CREATE DATABASE test;
  ```
...
* 查看存储引擎信息。
  ...
  ```sql
  obclient> SHOW STORAGE ENGINES;
  ```

* 查看全局状态变量。

  ```sql
  obclient> SHOW GLOBAL STATUS LIKE 'Com_select';
  ```
