...
| FILES IN LOCATION @location_name['/path'] [PATTERN = 'regex_pattern'] | 用于查看 Location 下存在的文件。<ul><li>`['/path']` 为可选项，表示子目录。</li><li>`PATTERN = 'regex_pattern'` 为可选项，用于指定一个正则表达式，用于筛选要 Location 目录下的文件。</li></ul> <main id="notice" type='explain'><h4>说明</h4><p>对于 OceanBase 数据库 V4.4.x 版本，从 V4.4.1 版本开始支持该语法。</p></main>|
