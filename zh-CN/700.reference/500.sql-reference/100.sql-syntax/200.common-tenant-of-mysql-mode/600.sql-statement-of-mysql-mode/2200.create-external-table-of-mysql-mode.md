...
* 执行 `CREATE EXTERNAL TABLE` 语句，需要当前用户拥有 `CREATE` 权限。有关 OceanBase 数据库权限的详细介绍，参见 [MySQL 模式下的权限分类](../../../../../600.manage/500.security-and-permissions/300.access-control/200.user-and-permission/200.permission-of-mysql-mode/100.permission-classification-of-mysql.md)。
* 当引用 Location 对象创建外表时，需要拥有该对象的 `READ` 权限。

  **示例如下：**

  授予用于 `user001` 对 Location 的对象 `test_location_local` 的读取权限。
...
| AS metadata$filecol{N} | 用于手动指定列映射。当文件中的列顺序与外表的列所定义顺序不一致时，可以通过 `metadata$filecol{N}` 所表示的伪列来指定外表的列与文件中的第 N 列的对应关系。例如，`c2 INT AS (metadata$filecol4)` 表示外表的 `c2` 列对应文件中的第 4 列。需要注意的是，如果指定了手动列映射，那么自动映射关系将会失效，而且所有的列都需要手动定义映射关系。|
| LOCATION    | 用于指定外表文件存放的路径。通常外表的数据文件存放于单独一个目录中，文件夹中可以包含子目录，在创建表时，外表会自动收集该目录中的所有文件。取值如下：<ul><li>`file_path`：表示具体的外表文件路径信息，详细介绍可参见下文 [file_path](#file_path)。</li><li>`@location_name['/path']`：表示引用 Location 对象创建外表。`['/path']` 为可选项，表示子目录。关于创建 Location 对象详细介绍信息，参见 [CREATE LOCATION](2230.create-location-of-mysql-mode-in-sql.md)。<main id="notice" type='explain'><h4>说明</h4><p>对于 OceanBase 数据库 V4.4.x 版本，从 V4.4.1 版本开始支持 <code>@location_name['/path']</code> 参数。</p></main></li></ul>|
...
* 文件在远程时，远程 `LOCATION` 格式为：
  ...
  * 文件在 HDFS 上时，有以下格式：
    ...
    * 基于 Hadoop HA（高可用）的逻辑命名服务访问集群格式为：`LOCATION = hdfs://nameserviceID/PATH`，其中 `nameserviceID` 指的是 HDFS 的是 Hadoop HA 的逻辑命名服务 ID，`PATH` 指文件路径。
      ...
      * 带 kerberos 认证的格式为：`LOCATION = 'hdfs://nameserviceID/PATH?principal=xxx&keytab=xxx&krb5conf=xxx&configs=xxx'`，其中：

        * `principal`：指登录认证用户，设置为非主节点 NN 的 `pricipal`。
        * `keytab` 和 `krb5conf`：与单节点 NN 设置一样。
        * `configs`：指定额外的 HDFS 配置项，需要设置多配置项，仅和 HA 配置项和安全配置项相关，其他无关。
      ...
      1. 下载 jar 包：访问 [阿里云 OceanBase 镜像](https://mirrors.aliyun.com/oceanbase/development-kit/)  下载jar 包。
          ...
          <main id="notice" type='notice'>
            ...
            <p><ul>
              ...
              <li>OceanBase 数据库 V4.4.0 版本：请下载 1.0.1 版本 devdeps-java-extensions RPM 安装包。</li>
              <li>OceanBase 数据库 V4.4.1 版本：请下载 1.0.2 版本 devdeps-java-extensions RPM 安装包。</li></ul></p>
          </main>
...
2. 启动 Observer 相关 JNI 配置项。
    ...
    具体信息参考 **步骤二** 中 **目标 hdfs 环境若打开 kerberos 认证** 的 **环境准备**，或者参考 [部署 OceanBase 数据库 JAVA SDK 环境](../../../../../400.deploy/300.deploy-oceanbase-enterprise-edition/500.deploy-the-ob-java-sdk-environment.md)。
...
3. 创建 HDFS 外表
    ...
    obclient> CREATE EXTERNAL TABLE test_ha (
        ...
        LOCATION = 'hdfs://mycluster/hadoop_ha_test?principal=ha/obrd.80c.sqlvd-a2-n0@QA.COM&keytab=/data/1/xutengting.xtt/configs/hadoop-kerberos-96/ha.keytab&krb5conf=/data/1/xutengting.xtt/configs/hadoop-kerberos-96/krb5.conf&configs=dfs.data.transfer.protection=integrity#dfs.nameservices=mycluster#dfs.ha.namenodes.mycluster=nn1,nn2#dfs.namenode.rpc-address.mycluster.nn1=obrd.80c.sqlvd-a2-n0:18020#dfs.namenode.rpc-address.mycluster.nn2=obrd.80c.sqlvd-a1-n0:18020#dfs.ha.automatic-failover.enabled.mycluster=true#dfs.client.failover.proxy.provider.mycluster=org.apache.hadoop.hdfs.server.namenode.ha.ConfiguredFailoverProxyProvider'
        FORMAT = (
            TYPE = 'CSV'
            FIELD_DELIMITER = ','
            FIELD_OPTIONALLY_ENCLOSED_BY = '\''
    ...
    本示例中 `LOCATION` 相关参数说明如下：

    * `principal=ha/obrd.80c.sqlvd-a2-n0@QA.COM` 故意设置非主节点 namenode（obrd.80c.sqlvd-a1-n0）的 pricipal。
    * `keytab` 和 `krb5conf` 和单节点 NN 设置一样。
    * 以下 `configs` 均需要配置：

    * `dfs.data.transfer.protection=integrity`：对齐了集群的 `dfs.data.transfer.protection` 的配置。
    * `dfs.nameservices=mycluster`：指明当前 HA 集群的 `namesevice`，也指别名。
    * `dfs.ha.namenodes.mycluster=nn1,nn2`：指明 HA 集群的 namenode 后备 ID 列表。
    * `dfs.namenode.rpc-address.mycluster.nn1=obrd.80c.sqlvd-a2-n0:18020`：设置说明 `nn1` 具体 namenode，方便客户端路由。
    * `dfs.namenode.rpc-address.mycluster.nn2=obrd.80c.sqlvd-a1-n0:18020`：设置说明 `nn2` 具体 namenode，方便客户端路由。
    * `dfs.ha.automatic-failover.enabled.mycluster=true`：让 HA 集群获取相关请求之后，自动获取可用 namenode 进行响应服务。
    * `dfs.client.failover.proxy.provider.mycluster=org.apache.hadoop.hdfs.server.namenode.ha.ConfiguredFailoverProxyProvider`：指示 HA 集群进行主备切换的逻辑工具类，也可自定义打包上传 HA 集群自己需要的逻辑等。
