...
| LOCATION    | 用于指定外表文件存放的路径。通常外表的数据文件存放于单独一个目录中，文件夹中可以包含子目录，在创建表时，外表会自动收集该目录中的所有文件。取值如下：<ul><li>`file_name`：表示具体的外表文件路径信息，详细介绍可参见下文 [file_name](#file_name)。</li><li>`@location_name['/path']`：表示引用 Location 对象创建外表。`['/path']` 为可选项，表示子目录。关于创建 Location 对象详细介绍信息，参见 [CREATE LOCATION](2230.create-location-of-mysql-mode-in-sql.md)。<main id="notice" type='explain'><h4>说明</h4><p>对于 OceanBase 数据库 V4.4.x 版本，从 V4.4.1 版本开始支持 <code>@location_name['/path']</code> 参数。</p></main></li></ul>|
