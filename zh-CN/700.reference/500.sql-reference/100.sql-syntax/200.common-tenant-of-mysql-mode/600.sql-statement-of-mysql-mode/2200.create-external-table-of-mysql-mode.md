  ...
  * 文件在 HDFS 上时，有以下格式：
    ...
    * 基于 Hadoop HA（高可用）的逻辑命名服务访问集群格式为：`LOCATION = hdfs://nameserviceID/PATH`，其中 `nameserviceID` 指的是 HDFS 的是 Hadoop HA 的逻辑命名服务 ID，`PATH` 指文件路径。
      ...
      * 带 kerberos 认证的格式为：`LOCATION = 'hdfs://nameserviceID/PATH?principal=xxx&keytab=xxx&krb5conf=xxx&configs=xxx'`，其中：
        ...
        * `configs`：指定额外的 HDFS 配置项，需要设置多配置项，仅和 HA 配置项和安全配置项相关，其他无关。例如：
          ...
          * `dfs.namenode.rpc-address.mycluster.nn1=xxxx.xxx.xxx-xxx2-n0:18020`：设置说明 `nn1` 具体 namenode，方便客户端路由。
          * `dfs.namenode.rpc-address.mycluster.nn2=xxxx.xxx.xxx-xxx1-n0:18020`：设置说明 `nn2` 具体 namenode，方便客户端路由。
...
3. 创建 HDFS 外表
    ...
    obclient> CREATE EXTERNAL TABLE test_ha (
        ...
        LOCATION = 'hdfs://mycluster/hadoop_ha_test?principal=ha/xxxx.xxx.xxx-xxx-n0@xxxx.COM&keytab=/data/1/xxxx.xxx/configs/hadoop-kerberos-96/ha.keytab&krb5conf=/data/1/xxxx.xxx/configs/hadoop-kerberos-96/krb5.conf&configs=dfs.data.transfer.protection=integrity#dfs.nameservices=mycluster#dfs.ha.namenodes.mycluster=nn1,nn2#dfs.namenode.rpc-address.mycluster.nn1=xxxx.xxx.xxx-xxx2-n0:18020#dfs.namenode.rpc-address.mycluster.nn2=xxxx.xxx.xxx-xxx1-n0:18020#dfs.ha.automatic-failover.enabled.mycluster=true#dfs.client.failover.proxy.provider.mycluster=org.apache.hadoop.hdfs.server.namenode.ha.ConfiguredFailoverProxyProvider'
        FORMAT = (
            TYPE = 'CSV'
            FIELD_DELIMITER = ','
            FIELD_OPTIONALLY_ENCLOSED_BY = '\''
