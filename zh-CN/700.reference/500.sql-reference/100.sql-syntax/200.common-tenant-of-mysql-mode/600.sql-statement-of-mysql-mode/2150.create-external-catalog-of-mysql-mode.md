...
|-------------|---------------|
| OSS         | AK/SK 认证。|
| HDFS        | HDFS 有以下认证方式：<ul><li>无认证（无需 Kerberos，且无需指定 HDFS 用户）。</li><li>用户认证（无需 Kerberos，但需以特定 HDFS 用户身份访问）。</li><li>Kerberos 认证，HDFS 为单 NameNode（非 HA 模式）。</li><li>Kerberos 认证，HDFS 为高可用（HA）模式。</li></ul>|
