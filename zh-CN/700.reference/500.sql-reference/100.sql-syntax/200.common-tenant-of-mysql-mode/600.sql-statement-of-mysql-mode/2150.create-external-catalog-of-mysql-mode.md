...
|-------------|---------------|
| OSS         | AK/SK 认证。|
| HDFS        | 无认证（无需 Kerberos，且无需指定 HDFS 用户）。|
| HDFS        | 用户认证（无需 Kerberos，但需以特定 HDFS 用户身份访问）。|
| HDFS        | Kerberos 认证，HDFS 为单 NameNode（非 HA 模式）。 |
| HDFS        | Kerberos 认证，HDFS 为高可用（HA）模式。|
...
3. 关联 Catalog。
    ...
    创建 Catalog 时无需额外配置，系统会自动根据路径匹配 Location。
...
* 场景 2：用户认证模式。
    ...
    <main id="notice" type='explain'>
      <h4>说明</h4>
      <p>适用于启用了 HDFS 权限控制（<code>hadoop.security.authentication=simple</code>），但未开启 Kerberos 的集群。</p>
    </main>

    创建 Location：指定 HDFS 用户。

    ```shell
    CREATE LOCATION hdfs_kerberos
...
* 场景 4：启用 Kerberos，HDFS 为高可用（HA）模式
    ...
    CREATE LOCATION hdfs_kerberos_ha
        URL = 'hdfs://testdev:8020/'  -- 推荐使用逻辑服务名
        CREDENTIAL (
            PRINCIPAL = "hdfs/xxx@xxx.COM",
