...
| ORGANIZATION [=] {INDEX \| HEAP} | 指定表中数据行的存储顺序。取值如下：<ul><li>`INDEX`：表示表模型为聚集索引表模型，即索引组织表。</li><li>`HEAP`：表示表模型为非聚集索引表模型，即堆表。</li></ul> 如果不指定 `ORGANIZATION` 选项，则该选项取值根据部署场景决定：<ul><li>OLAP 场景：取值为 `HEAP`。</li><li>其他场景：均为 `INDEX`。</li></ul> <main id="notice" type='explain'><h4>说明</h4><p>对于 OceanBase 数据库 V4.3.5 版本，<code>CREATE TABLE</code> 语句从 V4.3.5 BP1 版本开始支持 <code>ORGANIZATION</code> 选项。</p></main>|
