...
| MERGE_ENGINE = {delete_insert \| partial_update} | 可选项，用于指定表的更新模型，取值如下：<ul><li>`partial_update`：默认值，表示采用现有的更新模式不变。</li><li>`delete_insert`: 表示采用 `merge on write` 的方式，将 `update` 转为 `delete` 和 `insert`，并且写全列。</li></ul> <main id="notice" type='explain'><h4>说明</h4><p>参数 <code>MERGE_ENGINE</code> 在创建表时指定之后，其配置值将不可修改。</p></main>|
...
|STORAGE_CACHE_POLICY|可选项，表示共享存储模式下，用于指定表、分区、索引的内容缓存策略，如果为 `hot`，表示将对应的数据从对象存储上缓存到本地云盘，否则不进行本地云盘的缓存。详细介绍请参见下文 [storage_cache_policy](#storage_cache_policy) 。|
...
* 创建表 `tbl1`，并指定半结构化编码类型为 `encoding`，频率阈值为 `90`。
  ...
  ```sql
  CREATE TABLE tbl1 (col1 INT PRIMARY KEY, col2 INT) SEMISTRUCT_PROPERTIES=(ENCODING_TYPE=ENCODING, FREQ_THRESHOLD=90);
  ```

* 创建表 `` 启用新的 Flat 行存储格式（版本 2）。
...
`storage_cache_policy` 有以下取值：

* `hot`：指定表/分区/索引为热存储，表/分区/索引下所有数据会缓存到本地云盘。
* auto：指定表/分区/索引为自动冷热识别，由共享存储自行决定表/分区/索引下数据的存储方式，本地云盘空间不足时会优先淘汰这部分数据。
* `none`：分区级可以使用该属性，表级不能使用。表示该分区/索引的策略跟随上一级 `storage_cache_policy`。
* 时间轴：按照 range 分区定义的分区范围取值进行热数据缓存，当满足一定时间条件时，该分区的数据被判定为热数据，否则为冷数据。

`storage_cache_policy` 中不同属性以 Key-Value 格式定义。各个属性的语意如下：

* `GLOBAL`：表示指定全表的数据是否为热点数据，可以取值为 `hot`或 `auto`。`GLOBAL` 参数和其他参数互斥。
* `BOUNDARY_COLUMN`：用于判定热数据的列，基于该列的值经分区键表达式计算后和当前时间的间隔是否小于定义的 `HOT_RETENTION`。提供给时间轴策略使用。
* `BOUNDARY_COLUMN_UNIT`: 当 `BOUNDARY_COLUMN` 为整型时，指定 `BOUNDARY_COLUMN` 的时间单位，取值可以为`s/ms/us`。如果`BOUNDARY_COLUMN` 类型为 `date` 时，指定 `BOUNDARY_COLUMN_UNIT` 报错。提供给时间轴策略使用。
  * `BOUNDARY_COLUMN` 为 `bigint` 类型或者 `int` 类型，格式为 Unix 时间戳，分区方式可以是 `range/range columns` 两种。此时 `BOUNDARY_COLUMN_UNIT` 是必须指定参数，`BOUNDARY_COLUMN_UNIT` 的取值可以为 `s/ms/us`。当分区列为 `int` 类型时，`BOUNDARY_COLUMN_UNIT` 只能为 `s`。当分区列为 `bigint` 类型时，`BOUNDARY_COLUMN_UNIT` 可以是`s/ms/us`。
  * `BOUNDARY_COLUMN` 为时间类型，具体的可以是 `timestamp` 类型、`Date` 类型、`DateTime` 类型和 `Year` 类型。此时只能使用 range columns 分区方式（range 分区方式不支持时间类型列）。此时若指定 `BOUNDARY_COLUMN_UNIT` 报错。
* `HOT_RETENTION`：表示距离当前时间多久范围内的数据是热数据。提供给时间轴策略使用。
* `GRANULARITY`： 用于区分分区级还是行级热数据缓存，未指定时默认为 `partition`，即分区级。提供给时间轴策略使用。
