...
* `NORMAL`：默认值，表示正常。在该模式下，数据转储后触发合并的概率极低，并且仅会进行中等级别的压缩（Medium Compaction）。
* `QUEUING`：表示排队。在该模式下，数据转储后触发合并的概率仍然低，且仅会进行中等级别的压缩（Medium Compaction）。
* `MODERATE`：表示适度。在该模式下，数据转储后触发合并的概率为中等，优先做中等级别的压缩（Medium Compaction），但如果这个级别正在冷却（即不可用），则会转而进行元数据压缩（Meta Compaction）。
* `SUPER`：表示超级。在该模式下，数据转储后触发合并的概率高，优先做中等级别的压缩（Medium Compaction），但如果这个级别正在冷却（即不可用），则会转而进行元数据压缩（Meta Compaction）。
* `EXTREME`：表示极端。在该模式下，转储后触发合并的概率较高，并且仅进行元数据压缩（Meta Compaction）。
