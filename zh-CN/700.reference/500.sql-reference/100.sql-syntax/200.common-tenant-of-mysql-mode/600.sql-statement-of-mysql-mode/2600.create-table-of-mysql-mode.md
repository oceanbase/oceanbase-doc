...
|                        **参数**                        |        **描述**        |
|------------------------------------------------------|-----------------------------------|
| hint_options | 指定 Hint 选项，为可选项。<main id="notice" type='notice'><h4>注意</h4><p>在当前版本中，<code>CREATE TABLE</code> 语句的 Hint 选项仅限于支持 <code>/*+ parallel(N) */</code>，其中 <code>N</code> 是并行度。这一选项主要应用于 <code>CREATE TABLE ... AS SELECT ...</code> 的场景，可以控制表创建过程中的数据写入和选择查询的并行度。</p></main>|
