...
table_definition:
    ...
    | [FULLTEXT] {INDEX | KEY} [index_name] [index_type] (key_part,...) [WITH PARSER {SPACE | NGRAM | BENG | IK}] [PARSER_PROPERTIES[=](key=value [,key=value])]
...
| WITH PARSER {SPACE \| NGRAM \| BENG \| IK} | 可选项，指定全文索引的分词器。取值如下：<ul><li>`SPACE`：默认值，表示按空格进行分词。</li><li>`NGRAM`：表示基于 N-Gram（中文）的分词方式。</li><li>`BENG`：基于 Beng （基础英文）的分词方式。</li><li>`IK`：表示基于 IK（中文）的分词方式。当前仅支持 `utf-8` 字符集。<main id="notice" type='explain'><h4>说明</h4><p>对于 OceanBase 数据库 V4.3.5 版本，从 V4.3.5 BP1 版本开始支持 <code>IK</code> 分词器。</p></main></li></ul>|
