...
| SEARCH INDEX | 可选项，指定创建搜索索引（Search Index）。可在 `SEARCH INDEX [index_name] (search_index_column [, search_index_column ...])` 中按列声明；`search_index_column` 支持 `column_name [WITH (...)]`。其中 `WITH` 为列级选项，仅对 JSON 列生效，支持 `INCLUDE_PATHS`、`EXCLUDE_PATHS`（二选一）和 `INCLUDE_TYPES`。 |
