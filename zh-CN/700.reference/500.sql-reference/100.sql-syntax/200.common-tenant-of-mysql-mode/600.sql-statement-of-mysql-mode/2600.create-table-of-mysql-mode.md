...
| MERGE_ENGINE = {delete_insert \| partial_update \| append_only} | 可选项，用于指定表的更新模型，取值如下：<ul><li>`partial_update`：默认值，表示采用现有的更新模式不变。</li><li>`delete_insert`: 表示采用 `merge on write` 的方式，将 `update` 转为 `delete` 和 `insert`，并且写全列。</li> <li>`append_only`：表示仅允许 Insert 操作，禁止 Update 和 Delete，禁止存在潜在 Delete/Update 的操作（例如：`REPLACE INTO`），即数据一旦写入，不可修改或删除，写入顺序不强制要求 rowkey 递增。</li></ul> <main id="notice" type='explain'><h4>说明</h4><p>参数 <code>MERGE_ENGINE</code> 在创建表时指定之后，其配置值将不可修改。</p></main>|
