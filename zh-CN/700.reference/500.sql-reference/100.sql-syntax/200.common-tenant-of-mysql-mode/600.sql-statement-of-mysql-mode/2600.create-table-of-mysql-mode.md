...
|                        **参数**                        |        **描述**        |
|------------------------------------------------------|-----------------------------------|
| hint_options | 指定 Hint 选项，为可选项。<main id="notice" type='notice'><h4>注意</h4><p>在当前版本中，<code>CREATE TABLE</code> 语句的 Hint 选项仅限于支持 <code>/*+ parallel(N) */</code> 类型，其中 <code>N</code> 是并行度。并且该 Hint 选项仅适用于 <code>CREATE TABLE ... AS SELECT ...</code> 的场景，可以控制在表创建时数据写入操作的并行程度，查询的并行度可以由后面 SELECT 里的 Hint 选项控制。</p></main>|
