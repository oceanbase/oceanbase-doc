...
| DELTA_FORMAT [=] 'flat \| encoding' | 用于指定增量数据（转储）的格式，取值如下：<ul><li>`flat`：表示和现有增量数据格式一致。</li><li>`encoding`：表示增量数据格式使用 encoding。</li></ul> 可以通过修改配置项 `default_delta_format` 来调整表的默认增量数据格式，无需建表时反复指定，详细信息参见 [default_delta_format](../../../../800.configuration-items-and-system-variables/100.system-configuration-items/400.tenant-level-configuration-items/1950.default_delta_format.md)。|
