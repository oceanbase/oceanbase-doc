...
* 表更新模式（`MERGE_ENGINE`）必须为 `append_only` 或 `delete_insert`。

* 当前版本仅支持 TTL 列为内部隐藏列 `ora_rowscn`（近似于 rowkey，记录最后一次更新的时间戳）。

* 不允许添加列名为 `ora_rowscn` 的列。
...
table_option:
    ...
    | MERGE_ENGINE = {delete_insert | partial_update | append_only} [TTL [=]col_name + INTERVAL interval_num ttl_unit BY COMPACTION]
