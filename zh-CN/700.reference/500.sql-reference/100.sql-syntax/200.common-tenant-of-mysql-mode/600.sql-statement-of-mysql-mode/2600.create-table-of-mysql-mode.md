...
| FOREIGN KEY                                          | 为创建的表指定外键。如果不指定外键名，则会使用表名 + `OBFK` + 创建时间命名。（例如，在 2021 年 8 月 1 日 00:00:00 为 `t1` 表创建的外键名称为 `t1_OBFK_1627747200000000`）。外键允许跨表交叉引用相关数据，当 `UPDATE` 或 `DELETE` 操作影响与子表相匹配行的父表中键值时，其结果取决于 `ON UPDATE` 和 `ON DELETE` 子句的引用操作：<ul><li>`CASCADE`：从父表中删除或更新行，并自动删除或更新子表中匹配的行。</li><li>SET NULL：从父表中删除或更新行，并将子表中的外键列设置为 NULL。</li><li>RESTRICT：拒绝对父表的删除或更新操作。</li><li>NO ACTION指定延迟检查； 使用此功能时，直到提交时才会执行约束检查。</li><li>SET DEFAULT：
● MySQL同时支持update和 delete的 set null 行为，不过对于自引用的外键列上定义update的set null操作，但是其实际表现为 restrict; 自引用的外键列上可以定义delete操作的set null行为；    |
