...
| table_column_group_option/index_column_group_option | 指定表或索引的列存选项。具体解释如下：<ul><li><code>WITH COLUMN GROUP(all columns, each column)</code>：指定创建行存列存冗余的表或索引。</li><li><code>WITH COLUMN GROUP(all columns)</code>：指定添加行存索引。</li><li><code>WITH COLUMN GROUP(each column)</code>：指定创建列存表或索引。</li></ul>|
