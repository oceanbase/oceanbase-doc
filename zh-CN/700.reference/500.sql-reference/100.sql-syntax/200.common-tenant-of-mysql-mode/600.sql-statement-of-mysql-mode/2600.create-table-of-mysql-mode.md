...
column_definition:
     column_name data_type
         ...
         [NULL | NOT NULL] [[PRIMARY] KEY] [UNIQUE [KEY]] [COMMENT string_value] [SKIP_INDEX(skip_index_option)]
   | column_name data_type
...
| table_column_group_option/index_column_group_option | 指定表或索引的列存选项。具体解释如下：<ul><li><code>WITH COLUMN GROUP(all columns, each column)</code>：指定创建行存列存冗余的表或索引。</li><li><code>WITH COLUMN GROUP(each column)</code>：指定创建列存表或索引。</li></ul>|
| SKIP_INDEX | 标识列的 Skip Index 属性。取值如下：<ul><li><code>MIN_MAX</code>：是 Skip Index 中最通用的聚合数据类型，它会在索引节点粒度下存储被索引列的最大值、最小值和 Null Count。这种类型的数据可以加速普通比较计算。</li><li> <code>SUM</code>：用于加速数值类型的 `SUM` 聚合的下推。</li><li> <code>MIN_MAX, SUM</code>：同时使用 `MIN_MAX` 和 `SUM` 聚合的 Skip Index 类型。</li></ul>  <main id="notice" type='notice'><h4>注意</h4><p><ul><li>JSON 和空间数据类型的列禁止创建 Skip Index。</li><li>生成列禁止创建 Skip Index。</li></ul></p></main>|
