...
* 当前版本仅支持 TTL 列为内部隐藏列 `ora_rowscn`（近似于 rowkey 最后一次更新的时间戳）。
...
CREATE [hint_options] [TEMPORARY] TABLE [IF NOT EXISTS] table_name
      (table_definition_list) [table_option_list] [partition_option] [MERGE_ENGINE = {delete_insert | partial_update | append_only} [TTL [=]col_name + INTERVAL interval_num ttl_unit BY COMPACTION]] [table_column_group_option] [IGNORE | REPLACE] [AS] select;
