...
| `MAX_EXECUTION_TIME`                  | 指定 `SELECT` 语句的最大执行时间（毫秒），用于与 MySQL 兼容。 |
...
| `DISABLE_TRIGGER`|用于在当前 DML 中临时禁用触发器；可在圆括号内指定一个或多个触发器名称，省略括号及名称列表时禁用本条 DML 相关的全部触发器。|

## `APPEND` Hint

`APPEND` Hint 指定 `INSERT INTO SELECT` 开启旁路导入功能。更多信息，参见[使用 `INSERT INTO SELECT` 语句旁路导入数据](../../../../../../../../../620.obap/300.obap-data-transfer/300.data-import/100.data-import-overview.md)。
...
更多信息，参见 [旁路导入](../../../../../../../../../620.obap/300.obap-data-transfer/300.data-import/100.data-import-overview.md)。
...
更多信息，参见[使用 INSERT INTO SELECT 语句旁路导入数据](../../../../../../../../../620.obap/300.obap-data-transfer/300.data-import/100.data-import-overview.md)。
...
`LOAD_BATCH_SIZE` Hint 中 `batch_size` 指定每次插入批量记录的大小。 `LOAD DATA` 相关介绍可以查看 [LOAD DATA 介绍](../../../../../600.sql-statement-of-mysql-mode/5900.load-data-of-mysql-mode.md)。
...
`MAX_EXECUTION_TIME` Hint 用于指定当前 `SELECT` 查询语句的最大执行时间，主要用于与 MySQL 保持兼容。查询在指定时间内未执行完成时会返回超时错误。

与以微秒为单位的 `QUERY_TIMEOUT` Hint 相比，`MAX_EXECUTION_TIME` 的时间单位为毫秒。

### 语法

```sql
/*+ MAX_EXECUTION_TIME ( time_msec ) */
```

#### 参数解释

- `time_msec`：查询超时时间，单位为毫秒，取值为正整数。

### 示例

```sql
--指定查询超时时间为 1000 毫秒（即 1 秒），查询在指定时间内未执行完成将返回超时错误。
...
* `orc_filter_pushdown_level`：表示控制语句级别 ORC 外表条件下压级别。`parameter_value` 取值如下：

    <main id="notice" type='explain'>
      <h4>说明</h4>
      <p>OceanBase 数据库 <code>OPT_PARAM</code> Hint 中 <code>parameter_name</code> 参数从 V4.4.0 版本开始支持 <code>orc_filter_pushdown_level</code> 取值。</p>
    </main>

  * `0`：表示禁用过滤条件下压。
  * `1`：表示下压至文件级别。
  * `2`：表示下压至 stripe 级别。
  * `3`：表示下压至 row index 级别。
  * `4`：表示下压到 Encoding 级别。

* `parquet_filter_pushdown_level`：表示控制语句级别 Parquet 外表条件下压级别。`parameter_value` 取值如下：

    <main id="notice" type='explain'>
      <h4>说明</h4>
      <p>OceanBase 数据库 <code>OPT_PARAM</code> Hint 中 <code>parameter_name</code> 参数从 V4.4.0 版本开始支持 <code>parquet_filter_pushdown_level</code> 取值。</p>
    </main>
...
`PARALLEL` Hint 除了作为全局 Hint 外，还可以作为查询块 Hint 指定表级别的并行度，详见 [Access Path Hints](../200.optimizer-hint-of-mysql-mode/100.the-hint-related-to-the-access-path-of-mysql-mode.md) 中 `Parallel` hint 相关内容。
...
添加 `STAT` Hint 后，查询计划中会分配 `MONITORING DUMP` 算子，该算子直接输出排序算子的所有数据，并在执行结束后将算子执行时间、输出行数等信息打印在 observer 日志中。
...
`DISABLE_TRIGGER` Hint 用于在当前 DML 语句的执行过程中临时禁用触发器。适用于 `INSERT`、`UPDATE`、`DELETE`，以及对基于视图的 DML（含 `INSTEAD OF` 触发器）。

### 语法

`DISABLE_TRIGGER` 可单独使用，也可后接一对**圆括号**，在括号内列出触发器名称（多个名称用逗号分隔）。须使用圆括号，**不要使用方括号**。
...
`trigger_name`： 可选参数，指定要临时禁用的触发器名称。可以指定一个或多个触发器名称，多个名称用`,`分割。如果省略该参数，则默认禁用查询中涉及的所有触发器。

### 示例

```sql
-- 仅禁用名为 test_trigger1 的触发器
