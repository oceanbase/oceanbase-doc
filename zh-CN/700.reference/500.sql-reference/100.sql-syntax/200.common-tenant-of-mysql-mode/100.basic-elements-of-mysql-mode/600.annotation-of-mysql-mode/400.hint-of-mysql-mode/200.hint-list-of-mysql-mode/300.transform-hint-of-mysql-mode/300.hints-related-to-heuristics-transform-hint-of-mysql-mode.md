...
需要注意的是，不推荐直接使用 `JOIN_FIRST_UNNEST`/`NO_JOIN_FIRST_UNNEST` Hint，干预子查询提升改写推荐使用 `UNNEST`/`NO_UNNEST` Hint。
...
`NO_SIMPLIFY_DISTINCT` Hint 显式禁止 Distinct 查询的简化处理，这可以在特定场景下保持查询原貌，防止优化器自动进行可能不希望的改写。它的反向操作是 `SIMPLIFY_DISTINCT` Hint。
