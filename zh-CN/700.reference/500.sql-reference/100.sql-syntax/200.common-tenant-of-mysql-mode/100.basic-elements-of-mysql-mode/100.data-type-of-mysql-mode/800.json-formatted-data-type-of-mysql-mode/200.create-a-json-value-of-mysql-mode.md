...
OceanBase 数据库支持 MySQL 租户下建表时开启半结构化编码，主要由表级别参数 `semistruct_encoding_type` 控制，同时也要设置表的 `row_format=COMPRESSED`，否则会报错。当 `semistruct_encoding_type='encoding'` 时，表被认为是半结构化表，即整张表中的 JSON 列都会启用半结构化编码；当 `semistruct_encoding_type=''` 时，表被认为是结构化表。使用半结构化编码可以提高 [JSON_VALUE() 函数](../../../400.functions-of-mysql-mode/700.json-functions-of-mysql-mode/200.search-json-text-function-of-mysql-mode/700.json-value-of-mysql-mode.md) 条件过滤查询的性能。
