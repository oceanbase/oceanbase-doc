...
* `IN MATCH PHRASE MODE`：用于支持精确短语匹配。

    <main id="notice" type='explain'>
        <h4>说明</h4>
        <p>OceanBase 数据库从 V4.4.0 版本开始支持 <code>IN MATCH PHRASE MODE</code>。</p>
        </main>

  **示例如下：**

  1. 创建表 test 并为指定列添加全文索引。

     ```sql
     CREATE TABLE test(c1 int, c2 VARCHAR(200), c3 TEXT, FULLTEXT INDEX fts_idx (c2, c3));
     ```

  2. 查询 c2，c3 列中精确匹配 `some words` 的记录。
...
* 在查询包含 `OR` 连接的 `MATCH AGAINST` 谓词时，会自动尝试生成 Index Merge 计划，最终是否选择 Index Merge 计划取决于代价竞争的结果。
    ...
    <main id="notice" type='explain'>
      <h4>说明</h4>
      <p><ul><li>从 V4.4.1 版本开始，Index Merge 计划支持 <code>AND</code> 连接的多个谓词，在查询包含 <code>MATCH AGAINST</code> 在内的多个谓词时，会自动生成 Index Merge 计划。</li><li>在 V4.4.1 之前的版本， Index Merge 计划仅支持 <code>OR</code> 连接的多个谓词。</li></ul></p>
    </main>
