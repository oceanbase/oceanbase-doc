...
* `type`：表示目标系统所提供的数据类型。支持如下类型：
  ...
  * `BIGINT`：用于将表达式 `expr` 的值显式转换为 `BIGINT` 类型。

    <main id="notice" type='explain'>
      <h4>说明</h4>
      <p>对于 V4.4.2 版本，从 V4.4.2 BP1 版本开始支持转换为 <code>BIGINT</code> 类型。</p>
    </main>
  ...
  * `ARRAY`：用于将 `JSON` 数组中的元素转换为特定类型的数组。支持 `ARRAY<元素类型>` 格式声明数组元素类型，同时支持嵌套数组，如 `ARRAY<ARRAY<INT>>`，可用于表示多维数组结构。
...
* 将 `JSON` 类型的嵌套数组 `[[1,2],[3,4]]` 转换为嵌套 `ARRAY<ARRAY<INT>>` 数据，语句如下：
