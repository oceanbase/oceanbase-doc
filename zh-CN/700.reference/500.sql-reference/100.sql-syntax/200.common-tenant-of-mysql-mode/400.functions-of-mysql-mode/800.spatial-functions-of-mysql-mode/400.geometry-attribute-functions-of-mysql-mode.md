...
几何属性函数用于访问几何对象的定性或定量属性。OceanBase 数据库当前仅支持二维数据的空间运算函数包括 `ST_SRID()` 和 `_ST_SetSRID()`、`ST_Longitude()` 和 `ST_Latitude()`、`ST_X()` 和 `ST_Y()`。

二维和三维数据都支持的包括 `ST_Area`。该函数会将三维空间数据先转换为二维空间数据再进行处理。三维空间数据的 DDL 和 DML 和二维数据基本一致，但三维数据无法写入到由空间数据[子类型](../../100.basic-elements-of-mysql-mode/100.data-type-of-mysql-mode/900.spatial-data-type-of-mysql-mode/100.spatial-data-type-overview-of-mysql-mode.md)定义的列上。
...
`_ST_SetSRID` 函数用于设置空间几何对象的坐标系 ID。语法如下：

```sql
geometry _ST_SetSRID(geometry geom, integer srid);
```

示例如下：

```sql
obclient [test]> SELECT ST_ASTEXT(_st_setsrid(point(90,90), 4326));
+--------------------------------------------+
