...
空间运算函数用于生成几何图形。OceanBase 数据库当前仅支持二维数据的空间运算函数包括 `ST_Buffer_Strategy()`、`ST_Difference()`、`ST_Union()`、`ST_SymDifference()` 和 `_ST_ClipByBox2D()`。

二维和三维数据都支持的包括 `ST_Buffer()`、`_ST_Buffer`、`ST_Transform()`、`_ST_Transform()`、`_ST_Dwithin()` 和 `_ST_Covers()`。这些函数会将三维空间数据先转换为二维空间数据再进行处理。三维空间数据的 DDL 和 DML 和二维数据基本一致，但三维数据无法写入到由空间数据[子类型](../../100.basic-elements-of-mysql-mode/100.data-type-of-mysql-mode/900.spatial-data-type-of-mysql-mode/100.spatial-data-type-overview-of-mysql-mode.md)定义的列上。
...
`_ST_Buffer()` 函数返回表示与几何值 `g` 的距离小于或等于距离 `d` 的所有点。结果与几何参数在相同的 SRS 中。

语法如下：

```sql
_ST_Buffer(g, d [, strategy]), _ST_Buffer(g, d, ST_Buffer_Strategy())
```

`d` 参数不能小于 0.1，否则报错。如果参数为空，则 `_ST_Buffer()` 返回一个空值。如果距离为 0，则 `_ST_Buffer()` 返回几何参数 `g`。

`_ST_Buffer()` 的功能和 `ST_Buffer()` 差不多，但支持规则有所差异，具体如下：

* `quad_segs`：表示用来近似四分之一圆的弧段数，默认为 `8`。其功能相当于同时设置了 `point_circle`，`join_round` 和 `end_round`。其中 `point_circle` = 4 * `quad_segs`。
* `endcap`：表示端盖样式，可选取值有 `round` 和 `flat`，默认为 `round`。`butt` 与 `flat` 同义。
* `join`：表示连接方式，可选取值有 `round` 和 `mitre`，默认为 `round`。`miter` 与 `mitre` 同义。
    * `mitre_limit`：表示切割率限制，仅影响 `mitre` 的连接方式。`miter_limit` 与  `mitre_limit` 同义，和 `join_miter` 参数也相同。
* `side`：表示缓冲位置，可选取值有 `both`、`left` 和 `right`，默认为 `both`。`left` 和 `right` 在 geometry 对象上执行单面缓冲，其缓冲面相对于直线方向。缓冲位置只对 LineString 对象有意义，不影响 Point 对象和 Polygon 对象。

示例如下：

```sql
obclient [test]> SELECT ST_AsText(_ST_Buffer(ST_GeomFromText('POINT(0 0)'), 1, 'quad_segs=2'));
+--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
| ST_AsText(_ST_Buffer(ST_GeomFromText('POINT(0 0)'), 1, 'quad_segs=2'))                                                                                                                                                                                       |
+--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
| POLYGON((1 0,0.7071067811865474 0.7071067811865477,-1.8369701987210297e-16 1,-0.7071067811865477 0.7071067811865475,-1 -1.2246467991473532e-16,-0.7071067811865475 -0.7071067811865476,6.123233995736766e-17 -1,0.7071067811865476 -0.7071067811865475,1 0)) |
+--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
1 row in set
```
...
`_ST_Transform()`  函数用于将空间对象从一个空间参考系统 (SRS) 转换为另一个空间参考系统。

函数语法如下：

```sql
_ST_Transform(g1, srid);
_ST_Transform(g1, to_proj);
_ST_Transform(g1, from_proj, to_proj);
_ST_Transform(g1, from_proj, srid);
```

参数说明：

* `g1` 表示输入的空间对象。
* `srid` 表示 `g1` 的空间参考系统的 SRID 值。
* `from_proj` 表示输入空间对象 `g1` 的源空间参考系统。
* `to_proj` 表示要转换的空间参考系统。

示例如下：

```sql
// _ST_Transform(g1, srid);
OceanBase> SELECT ST_ASTEXT(_ST_Transform(ST_GEOMFROMTEXT('LINESTRING(0 0, 1 1)', 4326), 3819));
+------------------------------------------------------------------------------------------------+
...
// _ST_Transform(g1, from_proj, to_proj);
OceanBase> SELECT ST_AsText(_ST_Transform(
    ->                      ST_GeomFromText('POINT(16 48)', 4326, 'axis-order=long-lat'),
    ->                      '+proj=longlat +ellps=WGS84 +datum=WGS84 +no_defs ',
    ->                      '+proj=utm +zone=33 +ellps=WGS84 +datum=WGS84 +units=m +no_defs ')) as geometry;
+--------------------------------------------+
| geometry                                   |
+--------------------------------------------+
| POINT(574595.1124860339 5316784.009235403) |
+--------------------------------------------+
...
`_ST_Dwithin` 函数功能用于计算 2 个空间几何图形是否在指定距离内。距离的单位根据空间几何图形所在空间坐标系来定义。

语法如下：

```sql
_ST_Dwithin(g1, g2, distance);
```

示例如下：

```shell
obclient> SELECT _st_dwithin(ST_GeomFromText('POINT(0 0)', 4326), ST_GeomFromText('POINT(1 1)', 4326), 10);
+-------------------------------------------------------------------------------------------+
| _st_dwithin(ST_GeomFromText('POINT(0 0)', 4326), ST_GeomFromText('POINT(1 1)', 4326), 10) |
+-------------------------------------------------------------------------------------------+
|                                                                                         0 |
+-------------------------------------------------------------------------------------------+
1 row in set
...
`_ST_Covers` 函数用于判断空间几何图形 g2 是否完全被 g1 覆盖。
...
```shell
obclient> SELECT _st_covers(ST_GeomFromText('POLYGON((0 0, 10 0, 10 10, 0 10, 0 0))'), ST_GeomFromText('LINESTRING(1 10, 9 10, 9 8)'));
+-----------------------------------------------------------------------------------------------------------------------+
| _st_covers(ST_GeomFromText('POLYGON((0 0, 10 0, 10 10, 0 10, 0 0))'), ST_GeomFromText('LINESTRING(1 10, 9 10, 9 8)')) |
+-----------------------------------------------------------------------------------------------------------------------+
|                                                                                                                     1 |
+-----------------------------------------------------------------------------------------------------------------------+
1 row in set
