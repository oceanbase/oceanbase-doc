...
|slug|spatial-relation-functions-of-mysql-mode|

# 空间关系函数

空间关系函数来测试两个几何值 `g1` 和 `g2` 之间的关系，使用精确的对象形状。返回值 1 和 0 分别表示真和假，除了距离函数返回距离值。

OceanBase 数据库当前仅支持二维数据的空间关系函数包括 `ST_Crosses()`、`ST_Overlaps()`、`_ST_Touches()` 和 `ST_Equals()`。

二维和三维数据都支持的包括 `ST_Intersects()`、`ST_Distance()`、`ST_Distance_Sphere()`、`ST_Contains()`、 `ST_Within()` 和 `ST_IsValid`。这些函数会将三维空间数据先转换为二维空间数据再进行处理。三维空间数据的 DDL 和 DML 和二维数据基本一致，但三维数据无法写入到由空间数据[子类型](../../100.basic-elements-of-mysql-mode/100.data-type-of-mysql-mode/900.spatial-data-type-of-mysql-mode/100.spatial-data-type-overview-of-mysql-mode.md)定义的列上。
...
```sql
obclient> SELECT ST_Intersects(ST_GeomFromText('POINT Z (0 0 0)'), ST_GeomFromText('POINT Z (0 0 1)'));
+---------------------------------------------------------------------------------------+
| ST_Intersects(ST_GeomFromText('POINT Z (0 0 0)'), ST_GeomFromText('POINT Z (0 0 1)')) |
+---------------------------------------------------------------------------------------+
|                                                                                     1 |
+---------------------------------------------------------------------------------------+
1 row in set
...
`POINT (0 0 0)` 和 `POINT (0 0 1)` 在三维空间下是不相交，转换为二维数据后相交。
