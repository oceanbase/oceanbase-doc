...
`ST_Overlaps(g1,g2)` 函数返回 1 或 0 以确定两个几何对象 `g1` 和 `g2` 是否在空间上部分重叠，但并不完全互相包含。如果 `g1` 和 `g2` 重叠，则返回 1；如果 g1 和 g2 不重叠或者有一个完全包含另一个，则返回 0。且 ` ST_Crosses` 函数支持空间索引。且 `ST_Overlaps(g1,g2)` 函数支持空间索引。
