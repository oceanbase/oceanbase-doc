...
`MATCH` 语句会作为 `WHERE` 后的字段，表示过滤语义，如需计算分数需要搭配 `SCORE()` 语句一起使用。

`MATCH` 语句语法格式如下：

```sql
MATCH('A_COLUNM^x, B_COLUNM^y ...', 'expr', ['parameters'])
...
parameters:
    ...
    boost = number_value;
    score_norm = min-max;
    type = {most_fileds | best_fileds}
```

**相关参数说明如下：**

* `A_COLUNM^x, B_COLUNM^y ...`：表示列名与权重。权重为正数，默认值为 1。
...
* `parameters`：表示键对值列表，目前提供可使用参数如下：
  ...
  * `minimum_should_match`：表示在参数 `operator` 为 `or` 的情况下，第二组 token 的包含多少才能输出。
  * `boost`：表示 `MATCH` 级别权重，会与相关性相乘。权重为正数，默认值为 1。
...
WHERE
    MATCH('col2, col3',
          'OceanBase Database',
          'operator = or;
           minimum_should_match = 2<25%, 9<-25%;
           boost = 1;
           score_norm = min-max;
           type = most_fileds'
   );
...
```shell
SELECT SCORE() FROM table_name WHERE MATCH(...) ...;
```
...
FROM table_name
WHERE
    MATCH('A_COLUNM^x, B_COLUNM^y',
          'token1^n1 token2 token3^n2',
          'operator = or;
           minimum_should_match = 2<25%, 9<-25%;
           boost = XX;
           score_norm = min-max;
