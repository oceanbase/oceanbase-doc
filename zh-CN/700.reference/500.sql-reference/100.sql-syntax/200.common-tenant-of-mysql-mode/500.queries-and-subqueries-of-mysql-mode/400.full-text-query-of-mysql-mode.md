  ...
  **示例如下：**
  ...
  4. 使用短语匹配模式进行查询。
     ...
     * `MATCH AGAINST` 作为 `WHERE` 字段进行查询。
       ...
       ```sql
       SELECT * FROM test WHERE MATCH (title, body) AGAINST ('some words' IN MATCH PHRASE MODE);
       ```
...
OceanBase 数据库从 V4.4.1 版本开始支持 ES 全文查询的 SQL 语法支持，包括 `MATCH()` 语句和 `SCORE()` 语句：
...
* `SCORE()` 语句表示为对 `WHERE` 子句后所有 `MATCH()` 语句的相关性进行计算，默认为累加。`SCORE()` 语句作为 `MATCH()` 语法的搭配使用，单独出现无意义。
...
* `parameters`：可选项，表示键对值列表。不指定额外参数时，也需要保留一对空引号 `''`。目前提供可使用参数如下：
  ...
  * `minimum_should_match`：取值为正整数，表示在参数 `operator` 为 `or` 的情况下，第二组 token 的包含多少才能输出。
...
3. 从表 `tbl1` 中筛选同时包含 "OceanBase" 和 "Database" 的行（至少匹配 2 个关键词，`minimum_should_match=2`），按字段 `col2`（权重 2）和 `col3`（权重 1）进行全文检索，并返回匹配评分（归一化到 0-1 范围）。
...
4. 从表 `tbl1` 中筛选包含 "OceanBase" 或 "Database" 的行（至少匹配 1 个关键词，`minimum_should_match=1`），按字段 `col2`（权重 2）和 `col3`（权重 1）进行全文检索，并返回匹配评分（归一化到 0-1 范围）。
