...
* 重命名列的同时修改列的数据类型
  ...
  * `data_type`：指定修改后列的数据类型，支持修改的数据类型可参见下文 [修改列类型](#修改列类型)。
...
* 修改 test03 表中 `c2` 列可存储数字位数为 5 位，其中小数部分为 2 位
...
* `[PRIMARY] KEY`：指定设置选定列为主键。`KEY` 表示该列为主键但可以为空；`PRIMARY KEY` 表示该列为主键且不能为空。

* `UNIQUE [KEY]`：指定设置选定列的唯一性。`UNIQUE` 表示该列的值必须唯一，可以为空；`UNIQUE KEY` 表示该列的值必须唯一且不能为空。
