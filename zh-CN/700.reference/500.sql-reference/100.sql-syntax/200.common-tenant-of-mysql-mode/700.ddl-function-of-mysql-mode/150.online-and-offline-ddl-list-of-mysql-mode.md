...
- **定义**：进行 DDL 操作时，会锁定相关表，可以查询数据，写入会被阻塞。
...
|  列操作 | 中间加列（`BEFORE`/`AFTER`/`FIRST`） | 与数据量有关，需要重整数据（即把原来的数据要重写一遍） | 暂无 | 支持 <main id="notice" type='notice'><h4>注意</h4><p>对于 OceanBase 数据库 V4.3.5 版本，从 V4.3.5 BP1 版本开始支持物化视图日志基表增加列。</p></main>|
...
|  列操作 | 增加列类型长度或精度  |只需要修改元数据|  例如 `INT` 长度增长、`VARCHAR` 变长。| 不支持 |
...
|-------------|--------------------------|--------------------------|--------------------|--------------------|--------------------|
| 列操作 | 重排列（`BEFORE`/`AFTER`/`FIRST`）   |  与数据量有关，需要重整数据 | 暂无| 不支持 | 支持 |
| 列操作 | 添加自增列    | 与数据量有关，需要重整数据 |暂无| 不支持 | 支持 |
| 列操作 | 修改为自增列  | 与数据量有关，需要查询数据  |暂无| 不支持 | 支持 |
| 列操作 | 修改列类型    | 与数据量有关，需要重整数据 | 暂无| 不支持 | 支持 |
...
| 列操作 | 修改 `decimal` 类型的精度|与数据量有关，需要重整数据|暂无| 不支持 | 支持 |
| 主键操作 | 添加/删除主键 | 与数据量有关，需要重整数据 |暂无| 不支持 | 支持 |
| 表操作   | `TRUNCATE` 表  | 与是否有活跃事务有关 |暂无| 不支持 | 支持 |
| 表操作   | 转换字符集 | 与数据量有关，需要重整数据 | 暂无| 不支持 | 支持 |
