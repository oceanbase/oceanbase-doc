...
| DATA_DISK_SIZE       | 修改租户每个 Unit 可使用的本地磁盘空间大小，仅适用于共享存储（Shared-Storage,SS）模式，无共享（Shared-Nothing，SN）模式下修改该规格会报错。修改时，要求 `0` 值与非 `0` 值之间不能互相切换，即创建 Unit 时指定了 `DATA_DISK_SIZE` 为 `0` 后，不能再修改为非 `0` 的值；或者创建 Unit 时指定了 `DATA_DISK_SIZE` 为非 `0` 的值后，不能再修改为 `0`。否则，系统将会报错。<main id="notice" type='explain'><h4>说明</h4><p>SS 模式下，创建租户、迁移 Unit 或扩容时，系统会考虑 Unit 的 <code>DATA_DISK_SIZE</code> 规格，如果 OBServer 本地的磁盘空闲空间不足，会导致创建租户失败或者 Unit 不能迁移。</p></main>  |
