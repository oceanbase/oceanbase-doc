...
| DATA_DISK_SIZE       | 修改租户每个 Unit 可使用的本地磁盘空间大小，仅适用于共享存储（Shared-Storage,SS）模式，无共享（Shared-Nothing，SN）模式下修改该规格会报错。SS 模式下，修改该规格时：<ul><li>如果当前值为 `0`，则不支持修改。</li><li>如果当前值为非 `0` 的值，则要求修改后的值必须大于或等于 2GB。</li></ul><main id="notice" type='explain'><h4>说明</h4><p>SS 模式下，创建租户、迁移 Unit 或扩容时，系统会考虑 Unit 的 <code>DATA_DISK_SIZE</code> 规格，如果 OBServer 本地的磁盘空闲空间不足，会导致创建租户失败或者 Unit 不能迁移。</p></main>  |
