...
`ALTER SYSTEM SET LOG_ARCHIVE_DEST` 语句用于为用户租户配置归档目的端。可以是系统租户（`sys`）为指定租户配置归档目的端，也可以是用户租户为本租户配置归档目的端。系统租户和 Meta 租户不支持备份恢复，不需要配置归档目的端。

归档目的端的配置主要包括 `LOCATION`、`BINDING` 以及 `PIECE_SWITCH_INTERVAL`等参数。
...
* 用户租户
  ...
  * 用户租户为本租户配置归档目的端，归档介质为 OSS，归档文件的清理模式为 `tagging`。
