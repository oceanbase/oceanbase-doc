...
3. 确认分区信息。
   ...
   可以通过查询视图 `CDB_OB_TABLE_LOCATIONS`，获取指定分区的 `TABLE_ID`、`OBJECT_ID` 以及 `LS_ID`。
...
有关 Transfer Partition 的详细操作及说明，参见 [Transfer Partition](../../../../../600.manage/300.replica-management/500.load-balancing-manage/200.transfer-partition-management.md)。