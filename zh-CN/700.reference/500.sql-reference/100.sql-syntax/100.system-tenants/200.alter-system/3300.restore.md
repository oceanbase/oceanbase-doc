...
| uri                  | 指定备份时设置的数据备份路径 `backup_data_dest` 与日志归档路径 `log_archive_dest`，该值至少包含一条数据备份路径和一条日志归档路径，路径之间使用英文逗号（,）分隔。 目前 OceanBase 数据库支持的归档目的端介质为 NFS 、阿里云 OSS、AWS S3 以及兼容 S3 协议的对象存储，例如：华为 OBS、Google GCS、腾讯云 COS。|
