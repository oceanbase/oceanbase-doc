...
| uri                  | 指定备份时设置的数据备份路径 `backup_data_dest` 与日志归档路径 `log_archive_dest`，该值至少包含一条数据备份路径和一条日志归档路径，路径之间使用英文逗号（,）分隔。如果在备份过程中执行了归档日志备份（即配置了 `BACKUP_ARCHIVE_DEST`），则还需要在 `uri` 中补充归档备份目的端路径 `backup_archive_dest`，格式为 `backup_data_dest,log_archive_dest,backup_archive_dest`，多条路径必须按照该顺序排列。只要配置了 `BACKUP_ARCHIVE_DEST`，发起恢复时即可始终在 `uri` 中携带归档备份路径，恢复期间系统会按需读取对应的 Piece。如果任意必要的 Piece 在日志归档目的端和归档备份目的端均不存在，恢复将失败。目前 OceanBase 数据库支持的归档目的端介质为 NFS、阿里云 OSS、Azure Blob、AWS S3 以及兼容 S3 协议的对象存储，例如：华为 OBS、Google GCS、腾讯云 COS。`uri` 路径示例可参考 [SET LOG_ARCHIVE_DEST](3800.set-log-archive-dest.md) 或 [SET DATA_BACKUP_DEST](3500.set-data-backup-dest.md)。<main id="notice" type='explain'><h4>说明</h4><p>对于 V4.4.2 版本，从 V4.4.2 BP3 版本开始，支持在 <code>uri</code> 中指定归档备份目的端路径 <code>backup_archive_dest</code>。</p></main>|
...
* 从 NFS 数据备份路径、NFS 日志归档路径与 NFS 归档备份目的端路径恢复租户 `mysql` 到指定时间戳 `2022-06-01 00:00:00`，指定资源池为 `restore_pool`，以全量恢复方式进行恢复。
