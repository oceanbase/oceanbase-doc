...
* 从 COS 数据备份路径与 COS 日志归档路径恢复租户 `mysql` 到指定时间戳 `2022-06-1 00:00:00`, 指定资源池为 `restore_pool`。
   ...
   ```sql
   obclient> ALTER SYSTEM RESTORE mysql FROM 'cos://oceanbase-test-bucket/backup/data/?host=xxx.cos.ap-xxx.myqcloud.com&access_id=xxx&access_key=xxx&appid=xxx,cos://oceanbase-test-bucket/backup/archive/?host=xxx.cos.ap-xxx.myqcloud.com&access_id=xxx&access_key=xxx&appid=xxx' UNTIL TIME='2022-06-1 00:00:00' WITH 'pool_list=restore_pool';
   ```
