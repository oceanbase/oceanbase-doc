...
| 参数            | 描述                                                                                                               |
|-----------------|--------------------------------------------------------------------------------------------------------------------|
| POLICY          | 指定清理策略名，支持以下取值：<ul><li>`default`：表示自动清理的范围仅为租户当前 `DATA_BACKUP_DEST` 以及 `LOG_ARCHIVE_DEST` 参数中的备份。</li> <li>`log_only`：表示自动清理的范围仅为租户当前 `LOG_ARCHIVE_DEST` 参数中设置的归档路径中的日志。适用于没有数据备份，仅开启了日志归档的场景。此外，清理策略设置为 `log_only` 时，要求该租户当前 `DATA_BACKUP_DEST` 参数的值必须为空，即未配置数据备份目的端。相应的，如果设置了清理策略为 `log_only`，则无法再设置 `DATA_BACKUP_DEST` 参数。</li></ul> |
...
| tenant_name     | 指定租户名称。可选，仅在系统租户下执行时需要指定。  |
