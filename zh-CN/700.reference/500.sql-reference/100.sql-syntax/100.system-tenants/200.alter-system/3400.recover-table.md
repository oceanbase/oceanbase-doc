...
| UNTIL | 指定恢复终点，即恢复到该位点为止，且包括该位点。当指定恢复到 TIME 或 SCN 时, 必须以 `=` 来连接指定的值。如果不指定 `UNTIL` 子句，则默认恢复到最新的位点。<ul><li>`TIME ='timestamp'`：指定恢复的时间戳。</li>  <li>`SCN = scn`：指定恢复的 SCN（System Change Number），包括该 SCN 位点。</li></ul> |
