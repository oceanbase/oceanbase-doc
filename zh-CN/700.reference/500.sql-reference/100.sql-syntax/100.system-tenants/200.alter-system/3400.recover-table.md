...
| restore_option     | 指定辅助租户的 `pool_list`、`locality`、`primary_zone`、`concurrency`、`method` 等参数，不同参数之间通过 `&` 分隔：<ul><li>`pool_list`：必选，填写用户的资源池。多个资源池之间用英文逗号（,）分隔。</li>  <li>`locality`：可选，填写辅助租户副本分布的 Locality 信息，需要满足与辅助租户所在集群的 `pool_list` 的 Zone 信息相匹配。不填写该值时，默认按照 `resource_pool` 的 `zone_list` 为每个 Zone 设一个 F 副本。</br>填写样例: `locality='F@z1,F@z2,F@z3'`。 </li>  <li> `primary_zone`：可选，填写辅助租户的 Leader 副本的偏好位置，需要与 `pool_list` 及 `locality` 的匹配，即需要满足 Zone 信息匹配以及 `primary_region` 内至少有两个 Paxos 成员等限制。不填写该值时，则系统按照 `locality` 的 `zone_list` 将 Leader 随机分布在这些 Zone 中。 </br>填写样例: `primary_zone='z1'`。</li> <li>`concurrency`：可选，指定恢复的并发度。如果不指定，则默认等于辅助租户被分配的 MAX_CPU 数。</br>填写样例：`concurrency=50`</li> <li>`method`：可选，指定辅助租户数据恢复的方式，不区分大小写，取值如下：<ul><li>`full`：指全量恢复，不显式指定时，默认为 `full`。</li><li>`quick`：快速恢复。</li></ul></br>填写样例: `method=quick`</li></ul> |
