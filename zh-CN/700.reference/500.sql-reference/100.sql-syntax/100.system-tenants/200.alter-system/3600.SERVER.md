...
该语句用于维护系统中 OBServer 状态，控制 OBServer 的增、删、启、停等操作。

## 使用限制及注意事项

* 仅 `sys` 租户可以维护系统中 OBServer 状态。

## 语法

```sql
ALTER SYSTEM server_action SERVER ip_port_list [zone_name];
...
| zone_name     | 如果指定 Zone，将会对待维护 Server 执行 Zone 校验。   |
...
下面的示例将展示如何向集群新增 OBServer `xxx.xx.xxx.xx1`。
