...
* `enable_worm`：指定 OceanBase 数据库对该路径的写入及删除是否与 OSS 中 Bucket 的保留策略一致，是 OSS 的可选参数。支持的取值如下：

  <main id="notice" type='notice'>
  <h4>注意</h4>
  <ul><li>在 V4.3.5 版本中，从 V4.3.5 BP2 版本开始支持指定 <code>enable_worm</code> 参数。</li>
  <li>在设置 <code>enable_worm</code> 参数时，需要用户自行确认 Bucket 的 WORM 策略是否被正确配置。</li>
  ...
  <li>设置备份路径后，如果用户为该路径对应的 Bucket 配置了保留策略，则备份任务可能会因无法修改或删除目标路径下的文件而报 <code>-9140</code> 的错误（错误码可通过视图 <code>CDB_OB_BACKUP_DELETE_JOB_HISTORY</code>（sys 租户）或 <code>DBA_OB_BACKUP_DELETE_JOB_HISTORY</code>（用户租户）进行查看），提示 <code>the object is locked by worm</code>。当发现备份任务因 WORM 策略导致执行失败后，需要用户重新配置新的备份路径，并且指定<code>enable_worm=true</code>。</li></ul>
  </main>

  * `true`：是，表示 OceanBase 数据库对该路径的写入及删除将与 OSS 中 Bucket 的保留策略一致。

    OSS 的保留策略具有 WORM 特性，满足用户以不可删除、不可篡改的方式保存和使用数据。当用户为 Bucket 配置保留策略后，在保留策略指定的 Object 的保留时间到期前，仅支持在该 Bucket 中上传和读取 Object。待 Object 的保留时间到期后，才可以修改或删除该 Object。

    值设置为 `true` 时，要求同时显式配置 `checksum_type=md5`。
...
* 系统租户
  ...
  * 系统租户为租户 `mysql_tenant` 配置备份路径，备份介质为 OSS，备份文件的清理模式为 `delete`，对该路径的写入及删除将与 OSS 中 Bucket 的保留策略一致，并使用 MD5 算法校验归档文件的完整性。
...
* 用户租户
  ...
  * 租户 `mysql_tenant` 为本租户配置备份路径，备份介质为 OSS，备份文件的清理模式为 `delete`，不校验备份数据的完整性。
    ...
    * 租户 `mysql_tenant` 为本租户配置备份路径，备份介质为 OSS，备份文件的清理模式为 `delete`，对该路径的写入及删除将与 OSS 中 Bucket 的保留策略一致，并使用 MD5 算法校验归档文件的完整性。
