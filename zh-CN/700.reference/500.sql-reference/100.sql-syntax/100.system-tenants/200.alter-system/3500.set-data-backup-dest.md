...
其中，`file://your-nfs-server/your-backup-path` 用于指定OceanBase 数据库可以访问的 NFS 服务器的路径，以便存储数据备份。

#### 对象存储

```sql
# 阿里云 OSS
ALTER SYSTEM SET DATA_BACKUP_DEST = 'oss://your-bucket-name/your-archive-path?host=oss-your-region.aliyuncs.com&access_id=your-access-id&access_key=your-access-key[&delete_mode={delete | tagging}]';

# AWS S3
ALTER SYSTEM SET DATA_BACKUP_DEST = 's3://your-bucket-name/your-archive-path?host=s3.your-region.amazonaws.com&access_id=your-access-id&access_key=your-access-key&s3_region=your-s3-region[&delete_mode={delete | tagging}]';

# 兼容 S3 协议的对象存储
ALTER SYSTEM SET DATA_BACKUP_DEST = 's3://your-bucket-name/your-archive-path?host=your-service.com&access_id=your-access-id&access_key=your-access-key[&addressing_model={virtual_hosted_style | path_style}]';
```
...
* 系统租户
  ...
  * 系统租户为租户 `mysql_tenant` 配置备份目的端，备份介质为 S3，并配置备份文件的清理模式 `delete`。
  ...
  * 系统租户为租户 `mysql_tenant` 配置备份目的端，备份介质为 OBS，通过 S3 协议访问。
  ...
  * 系统租户为租户 `mysql_tenant` 配置备份目的端，备份介质为 GCS，通过 S3 协议访问。
  ...
  * 系统租户为租户 `mysql_tenant` 配置备份目的端，备份介质为 COS，通过 S3 协议访问。
...
* 用户租户
  ...
  * 租户 `mysql_tenant` 为本租户配置备份目的端，备份介质为 S3，并配置备份文件的清理模式 `delete`。
  ...
  * 租户 `mysql_tenant` 为本租户配置备份目的端，备份介质为 OBS，通过 S3 协议访问。
  ...
  * 租户 `mysql_tenant` 为本租户配置备份目的端，备份介质为 GCS，通过 S3 协议访问。
  ...
  * 租户 `mysql_tenant` 为本租户配置备份目的端，备份介质为 COS，通过 S3 协议访问。
