...
* `delete_mode`：在备份路径后配置备份文件的清理模式，OSS、S3、COS（以 S3 协议访问） 的可选参数，与其他参数之间通过 `&` 符号连接。该参数当前支持以下两种取值：
...
* 用户租户
  ...
  * 租户 `mysql_tenant` 为本租户配置备份目的端，备份介质为 AWS S3，备份文件的清理模式为 `tagging`，使用 CRC32 算法校验备份数据的完整性。
