...
* `enable_worm`：指定 OceanBase 数据库对该路径的写入及删除是否与 OSS 的 WORM（Write Once Read Many） 策略一致，是 OSS 的可选参数。支持的取值如下：

  <main id="notice" type='notice'>
  <h4>注意</h4>
  <p>在 V4.3.5 版本中，从 V4.3.5 BP2 版本开始支持指定 <code>enable_worm</code> 参数。</p>
  </main>

  * `true`：是，表示 OceanBase 数据库对该路径的写入及删除将与 OSS 的 WORM 策略一致。
