...
1. 系统租户查询待合并的表 tbl2 的 `table_id`。
   ...
   ```shell
   obclient(root@sys)[oceanbase]> SELECT t1.tenant_id, t2.tenant_name, t1.database_name, t1.table_id, t1.table_name FROM oceanbase.CDB_OB_TABLE_LOCATIONS t1, oceanbase.DBA_OB_TENANTS t2 
   WHERE t1.tenant_id=t2.tenant_id 
   AND t1.table_name = 'tbl2'
   AND t2.tenant_name = 'mysql001';
   ```
