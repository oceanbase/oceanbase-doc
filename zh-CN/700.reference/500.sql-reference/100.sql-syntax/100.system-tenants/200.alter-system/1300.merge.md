...
该语句用于系统租户发起存储层合并或转储（冻结）。手动转储支持租户级别、Zone 级别、Server 级别、日志流级别和分区级别。
...
merge_action:
    ...
    | MINOR FREEZE [tenant_list | TABLET_ID = tablet_id] [server_list]
...
2. 对本租户内指定 Tablet 发起转储。

    **示例如下：**

    ```sql
    ALTER SYSTEM MAJOR FREEZE TABLET_ID = 200008;
    ```
