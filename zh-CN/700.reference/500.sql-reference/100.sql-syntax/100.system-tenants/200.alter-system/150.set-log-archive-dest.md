...
* 系统租户
  ...
  * 系统租户为租户 `mysql_tenant` 配置归档目的端，归档介质为 AWS S3，归档文件的清理模式为 `tagging`，使用 CRC32 算法校验归档文件的完整性。
...
* 用户租户
  ...
  * 租户 `mysql_tenant` 为本租户配置归档目的端，归档介质为 AWS S3，归档文件的清理模式为 `tagging`，使用 CRC32 算法校验归档文件的完整性。
