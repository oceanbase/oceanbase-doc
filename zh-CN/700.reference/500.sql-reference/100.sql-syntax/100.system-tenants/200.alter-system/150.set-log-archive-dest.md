...
* `enable_worm`：指定 OceanBase 数据库对该路径的写入及删除是否与 OSS 中 Bucket 的保留策略一致，是 OSS 的可选参数。支持的取值如下：
  ...
  <li>如果待设置的路径对应的 Bucket 设置了保留策略，而在配置归档目的端时未指定 <code>enable_worm=true</code>，则语句执行后，系统将会报错，提示需要设置<code>enable_worm=true</code>。</li>
  <li><code>enable_worm</code> 参数一旦设置后，后续不支持变更。</li>
  <li>设置归档路径后，如果用户为该路径对应的 Bucket 配置了保留策略，则归档可能会因无法向目标路径追加写而报 <code>-9140</code> 的错误，提示 <code>the object is locked by worm</code>。当发现归档任务因 WORM 策略导致执行失败后，需要用户重新配置新的归档路径，并且指定<code>enable_worm=true</code>。</li></ul>
  </main>
...
* 系统租户
  ...
  * 系统租户为租户 `mysql_tenant` 配置归档路径，归档介质为 OSS，归档文件的清理模式为 `delete`，对该路径的写入及删除将与 OSS 中 Bucket 的保留策略一致，并使用 MD5 算法校验归档文件的完整性。
...
* 用户租户
  ...
  * 租户 `mysql_tenant` 为本租户配置归档路径，归档介质为 OSS，归档文件的清理模式为 `tagging`，对该路径的写入及删除将与 OSS 中 Bucket 的保留策略一致，并使用 MD5 算法校验归档文件的完整性。
