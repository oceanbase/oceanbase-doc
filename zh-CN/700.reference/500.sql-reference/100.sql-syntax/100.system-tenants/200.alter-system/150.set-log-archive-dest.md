...
* 系统租户
  ...
  * 系统租户为租户 `mysql_tenant` 配置归档路径，归档介质为 OSS，归档文件的清理模式为 `delete`，使用 MD5 算法校验归档文件的完整性。同时，系统将按照 WORM 的规范对该路径执行写入和删除操作。
...
* 用户租户
  ...
  * 租户 `mysql_tenant` 为本租户配置归档路径，归档介质为 OSS，归档文件的清理模式为 `tagging`，使用 MD5 算法校验归档文件的完整性。同时，系统将按照 WORM 的规范对该路径执行写入和删除操作。
