...
```sql
# 阿里云 OSS
ALTER SYSTEM SET LOG_ARCHIVE_DEST='LOCATION=oss://your-bucket-name/your-archive-path?host=oss-your-region.aliyuncs.com&access_id=your-access-id&access_key=your-access-key[&delete_mode={delete | tagging}];

# AWS S3
ALTER SYSTEM SET LOG_ARCHIVE_DEST='LOCATION=s3://your-bucket-name/your-archive-path?host=s3.your-region.amazonaws.com&access_id=your-access-id&access_key=your-access-key&s3_region=your-s3-region[&delete_mode={delete | tagging}];

# 兼容 S3 协议的对象存储
ALTER SYSTEM SET LOG_ARCHIVE_DEST='LOCATION=s3://your-bucket-name/your-archive-path?host=host=your-service.com&access_id=your-access-id&access_key=your-access-key[&addressing_model={virtual_hosted_style | path_style}];
```
...
* 系统租户
  ...
  * 系统租户为租户 `mysql_tenant` 配置归档目的端，归档介质为 S3，归档文件的清理模式为 `delete`，采用用户业务优先模式，且每隔一天切分一个 Piece。
...
* 用户租户
  ...
  * 用户租户为本租户配置归档目的端，归档介质为 S3，归档文件的清理模式为 `delete`，采用用户业务优先模式，且每隔一天切分一个 Piece。
