...
* `delete_mode`：配置归档文件的清理模式，OSS、S3 的可选参数，与其他参数之间通过 `&` 符号连接。该参数当前支持以下两种取值：
...
* `checksum_type`：指定归档文件的校验算法，用于校验归档文件的完整性。如果未显式指定，所有对象存储默认均使用 MD5 校验算法。根据不同的对象存储，该参数支持的取值如下：
  ...
  * `crc32`：使用 CRC32 算法校验归档文件的完整性。仅 S3 支持该取值。
...
* 系统租户
  ...
  * 系统租户为租户 `mysql_tenant` 配置归档目的端，归档介质为 S3 时，归档文件的清理模式为 `tagging`，使用 CRC32 算法校验归档文件的完整性。
...
* 用户租户
  ...
  * 租户 `mysql_tenant` 为本租户配置归档目的端，归档介质为 S3 时，归档文件的清理模式为 `tagging`，使用 CRC32 算法校验归档文件的完整性。
