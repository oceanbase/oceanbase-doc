...
| VERIFY      | 可选项。当使用 `VERIFY` 命令时，表示验证 `ACTIVATE STANDBY` 命令是否可以执行成功。 <main id="notice" type='notice'><h4>注意</h4><p><code>VERIFY</code> 命令只是用来验证 <code>ACTIVATE STANDBY</code> 命令是否可以成功执行，而不会实际执行激活操作。</p></main> |
