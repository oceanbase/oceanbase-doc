...
<h4>说明</h4>
<p>设置 <code>SERVICE=$host_ip_list</code>、<code>USER=$user_name@$tenant_name</code>、<code>PASSWORD=$password</code>、<code>DELAY</code>等子参数项时，各子参数项之间使用空格作为分隔符，解析时系统也会根据空格区分不同的子参数项，故设置子各参数项值的等号（=）前后不能有空格。</p>
</main>
...
2. 修改日志恢复源。
   ...
   * 备租户所在集群的 `sys` 租户修改备租户的日志恢复源，同时指定备租户的延迟时间为 3600 秒
   ...
   * 备租户修改本租户的日志恢复源，同时指定备租户的延迟时间为 3600 秒
...
* 修改延迟即是增大或缩小备库的延迟时间。将备库的延迟时间修改为 1800 秒，示例如下：

  * 备租户（备库）所在集群的 `sys` 租户修改该备租户的延迟时间
...
* 取消备库延迟即是将备库的延迟时间修改为 0 秒，示例如下：

  * 备租户（备库）所在集群的 `sys` 租户取消该备租户的延迟
    ...
    或者，也可以通过不指定 `DELAY` 参数来取消备租户的延迟，语句如下。
