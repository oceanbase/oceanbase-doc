...
<h4>说明</h4>
<p>设置 <code>SERVICE=$host_ip_list</code>、<code>USER=$user_name@$tenant_name<code>、<code>PASSWORD=$password<code> 等参数项时，各参数项之间使用空格作为分隔符，解析时系统也会根据空格区分不同的参数项，故设置各参数值的等号（=）前后不能有空格。</p>
</main>
