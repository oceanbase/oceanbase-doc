...
| NOWAIT      | 可选项。表示在进行 VERIFY 时不等待，发现有条件不符合时则立即报错。<main id="notice" type='explain'><h4>说明</h4><p>对于 V4.4.2 版本，该参数从 V4.4.2 BP2 版本开始支持。</p></main> |
...
* 主租户下，进行主切备前，执行预检查确认备租户是否已具备执行 `SWITCHOVER` 命令所必须的前提条件，并指定发现有条件不符合时，立即返回错误。
