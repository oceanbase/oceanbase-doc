...
```sql
ALTER SYSTEM SWITCHOVER TO {STANDBY | PRIMARY} [TENANT tenant_name] [VERIFY];
```
...
| VERIFY      | <main id="notice" type='explain'><h4>说明</h4><p>对于 OceanBase 数据库 V4.2.1 版本，本功能从 V4.2.1 BP8 版本开始支持。</p></main> 可选项。当使用此参数时，它的作用是进行预先检验，以确认执行 `SWITCHOVER` 命令所必须的前提条件是否已经具备。 <main id="notice" type='notice'><h4>注意</h4><p><ul><li>主备租户角色转换执行中存在一定失败风险。<code>VERIFY</code> 命令只是进行前期验证，评估满足角色转换的前提条件，有助于减少该操作失败的可能性，并不实际执行 <code>SWITCHOVER</code> 操作。</li><li>即使 <code>VERIFY</code> 命令运行成功，也不能保证随后的 <code>SWITCHOVER</code> 操作一定会成功。</li></ul></p></main> |
