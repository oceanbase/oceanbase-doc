...
* 如果当前日志流已经有 7 个 F 副本，此时不允许进行迁移任务。
...
| svr_ip:svr_port   | 指定待迁移的副本所在服务器的 IP 及 RPC 端口号。例如，`xxx.xx.xxx.001:2882`。|
| destination_addr  | 指定待迁移的副本的目标服务器的 IP 及 RPC 端口号。格式为 `svr_ip:svr_port`，示例：`xxx.xx.xxx.002:2882`。|
