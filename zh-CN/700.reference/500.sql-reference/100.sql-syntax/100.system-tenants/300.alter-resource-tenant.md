...
| unit_group_id_list | 仅用于同构 Zone 模式下的缩容场景。减小 `unit_num` 时，使用 `DELETE UNIT_GROUP` 可以删除指定的 Unit GROUP。如果不指定 `DELETE UNIT_GROUP`，系统将随机删除相应个数的 Unit Group。 |
...
* 同构 Zone 模式的缩容场景下，删除租户 `tenant2` 下 `UNIT_GROUP_ID` 为 `1006` 的 Unit Group。
