...
| CHARACTER SET \| CHARSET | 修改租户的字符集。<main id="notice" type='explain'><h4>说明</h4><p> 当前版本暂不支持修改租户的字符集。</p></main>   |
| COLLATE                  | 修改租户的字符序。<main id="notice" type='explain'><h4>说明</h4><p> 当前版本暂不支持修改租户的字符序。</p></main>  |
| READ ONLY \| READ WRITE  | 修改租户的属性为只读还是读写。<main id="notice" type='explain'><h4>说明</h4><p> 当前版本暂不支持修改租户的读写属性。</p></main>|
| DEFAULT TABLEGROUP       | 修改租户的默认表组信息。---默认表组有哪些？                                                   |
...
* 部署了仲裁服务的场景下，为所有用户租户开启仲裁服务。

  ```sql
  obclient [oceanbase]> ALTER TENANT ENABLE_ARBITRATION_SERVICE=true;
  ```
