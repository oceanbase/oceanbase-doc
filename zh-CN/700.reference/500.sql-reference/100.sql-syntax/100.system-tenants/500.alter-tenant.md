...
| COMMENT                  | 修改注释。                                                                              |
| DEFAULT TABLESPACE tablespace_name | 用于修改组户的默认表空间（`DEFAULT TABLESPACE`），`tablespace_name` 表示表空间的名称。 <main id="notice" type='notice'><h4>注意</h4><p><ul><li>对于 V5.x 版本，该参数从 V5.0.1 版本开始引入。</li><li>租户的 <code>DEFAULT TABLESPACE</code> 只能在租户创建后修改。</li></ul></p></main>|
...
* 修改租户 `tenant1` 的默认表空间为 `ts1`。

  ```shell
  obclient> ALTER TENANT tenant1 DEFAULT TABLESPACE ts1;
  ```