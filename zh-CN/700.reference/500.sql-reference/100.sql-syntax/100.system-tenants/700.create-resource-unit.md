...
| DATA_DISK_SIZE       | 指定租户每个 Unit 可使用的本地磁盘空间大小，仅适用于共享存储（Shared-Storage,SS）模式，无共享（Shared-Nothing，SN）模式下指定该规格会报错。指定该规格时：<ul><li>值为 `0`时，表示不限制租户每个 Unit 可使用的本地磁盘空间大小，每台 OBServer 可自动调整租户 Unit 分配的本地缓存盘大小。</li> <li>指定为非 `0` 的值时，需要该值大于等于 2GB。</li><li>如果不指定，则值默认为内存规格的 2 倍。</li></ul><p>SS 模式下，创建租户、迁移 Unit 或扩容时，系统会考虑 Unit 的 `DATA_DISK_SIZE` 规格，如果 OBServer 本地的磁盘空闲空间不足，会导致创建租户失败或者 Unit 不能迁移。</p>  |
