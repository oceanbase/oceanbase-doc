...
* 创建名为 `unit1` 的 Unit，同时指定 CPU 为 4 核；内存为 `5G`；日志盘容量为 `10G`；IOPS 的上限为 `1280`，IOPS 下限为 `1024`，同时 IOPS 的权重为 `1`；每个 Unit 可使用的网络带宽上限为 `5M`，网络带宽权重为 `10`。

  ```shell
  obclient(root@sys)[oceanbase]> CREATE RESOURCE UNIT unit1 MAX_CPU 4, MEMORY_SIZE '5G', MAX_IOPS 1280, MIN_IOPS=1024, IOPS_WEIGHT=1, MAX_NET_BANDWIDTH '5M',NET_BANDWIDTH_WEIGHT 10, LOG_DISK_SIZE '10G';
  ```
