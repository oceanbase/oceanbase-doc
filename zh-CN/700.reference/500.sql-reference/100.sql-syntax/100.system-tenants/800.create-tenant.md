...
| system_var_name          | 指定租户系统变量值。其中，`ob_compatibility_mode` 用于指定租户的兼容模式（可选 MySQL 或 Oracle 模式），而且只能在创建时指定；如果不指定 `ob_compatibility_mode`，默认兼容模式为 MySQL；`lower_case_table_names` 用于控制大小写敏感（0 表示关闭；1 表示开启），仅支持使用 `CREATE TENANT` 语句创建租户时指定并只在 MySQL 模式下生效，租户创建后不能通过 SQL 语句修改。 |
...
* 创建租户时开启大小写敏感。
  ...
  ```shell
  obclient> CREATE TENANT oblog_tt replica_num = 1, resource_pool_list=('oblog_pool') set ob_tcp_invited_nodes='%', lower_case_table_names=1;
  ```
