...
| COLLATE                  | 创建 MySQL 兼容模式的租户时，指定租户级的字符序。有关 MySQL 模式下字符序的详细介绍，参见 [字符序（MySQL 模式）](../200.common-tenant-of-mysql-mode/100.basic-elements-of-mysql-mode/300.character-set-and-collation-of-mysql-mode/300.collation-of-mysql-mode.md)。<main id="notice" type='explain'><h4>说明</h4><p>创建 Oracle 兼容模式的租户时，不支持设置租户级字符序。如果设置了字符序，语法上不会报错，但设置并不会生效，系统会将其覆盖为字符集对应的默认字符序。</p></main>   |
