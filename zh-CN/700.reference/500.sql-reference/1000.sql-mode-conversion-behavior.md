...
- **错误（`ERROR`）vs 警告（`WARNING`）：** 客户端返回 `ERROR` 则 DML 语句因完全失败。返回结果集但有警告意味着是 `SELECT` 或 `DELETE` 的 `WHERE` 子句中发生了转换问题。

- **检查 `sql_mode`：** 使用 `SELECT @@sql_mode;` 确认严格模式是否启用。

- **查看执行计划与日志：** 对于复杂情况，OceanBase 的执行计划输出（`EXPLAIN`）和 SQL 审计日志/错误日志可提供更详细的线索。
