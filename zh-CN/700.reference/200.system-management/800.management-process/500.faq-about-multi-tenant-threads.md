...
当发现 OBServer 节点的 CPU 负载偏高已经达到了 80%- 90% 甚至以上，该 OBServer 节点已经发生了整体性能不佳甚至是 Hang 住的情况，那么可以通过 OBStack 工具（V2.2.3x 高配版本、V2.2.7x 以及 V3.x 版本均可独立安装）执行以下命令获取线程栈重定向到任意文件中，比如`obstack.trc`。 OBStack 工具请联系 OceanBase 数据库支持团队获取。
