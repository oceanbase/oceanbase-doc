...
系统租户是 OceanBase 数据库的集群管理员为安装 OceanBase 数据库时自动创建。从 V4.2.1 版本开始，系统租户实际可使用的内存上限为：`hidden_sys_memory + unit_sys_memory`。
...
* unit_sys_memory 的值由资源规格 `sys_unit_config` 中的 `memory_size` 决定。`sys_unit_config` 是系统租户默认的资源规格，其 `memory_size` 默认值的计算公式为 `min((memory_limit - system_memory)/4, 16G)`。由于系统租户也是一个正常的租户，故可以通过 `ALTER RESOURCE` 语句修改 `sys_unit_config` 中 `MEMORY_SIZE` 的值。
