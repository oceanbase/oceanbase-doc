|description||
|---|---|
|slug|memory-management-in-tenant|
|keywords||
|dir-name||
|dir-name-en||
|tenant-type||
...
OceanBase 数据库会根据 MemStore 的内存使用比例决定何时进行转储或者合并释放 MemStore 的内存，该比例由配置项 `freeze_trigger_percentage` 控制，表示当 MemStore 内存占用到达其上限的百分比后就进行冻结（转储的前置动作），默认值为租户 MemStore 内存上限的 20%，即租户 `memory_size` 的 10%。
