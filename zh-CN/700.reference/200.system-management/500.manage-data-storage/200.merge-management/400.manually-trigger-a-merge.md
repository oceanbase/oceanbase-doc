...
合并也可以通过手动触发来完成。手动合并包括租户级合并、表级合并和分区级合并。
...
表级合并是指对某个租户下的某个表的所有分区发起分区合并命令。表级合并有以下使用限制：

* 仅支持对数据存储在数据库内部的用户表发起表级合并。

* 以下特定的表类型不支持表级合并：

  * 虚拟表

    虚拟表无实际物理存储且数据动态生成，故不支持发起表级合并。

  * 视图

    系统视图、用户视图等均不支持发起表级合并。

  * 物化视图

    物化视图通过租户合并刷新，分区合并无法刷新。

  * LS inner table

    LS inner table 本身不支持合并。

  * 临时表

    临时表的生命周期短、数据量小，故不建议发起表级合并。

  * 外表

    外表的存储格式与 OceanBase 数据库不同，故不支持发起表级合并。

  * 向量索引表

* 与分区级合并一样，表级合并为异步任务，不保证一定能发起成功。
...
2. 执行以下命令，发起合并。
   ...
   * 发起表级合并
      ...
      系统租户对指定租户发起表级合并的 SQL 语句如下：

      ```sql
      ALTER SYSTEM MAJOR FREEZE TENANT [=] tenant_name TABLE_ID = table_id;
      ```

      其中，`table_id` 为表 ID，可以由系统租户通过查询视图 `CDB_OB_TABLE_LOCATIONS` 来获取。有关视图 `CDB_OB_TABLE_LOCATIONS` 中各字段的详细介绍，请参见 [oceanbase.CDB_OB_TABLE_LOCATIONS](../../../700.system-views/300.system-view-of-sys-tenant/200.dictionary-view-of-sys-tenant/10600.o-cdb_ob_table_locations-of-sys-tenant.md)。

      系统租户对租户 `tenant1` 发起表级合并的示例如下：

      1. 系统租户获取待合并的表 t1 的 `table_id`。
   ...
   * 发起分区级的合并
      ...
      1. 系统租户获取待合并的表 t1 的分区对应的 `tablet_id`。
...
2. 对本租户发起租户级合并。
   ...
   * 发起表级合并
      ...
      用户租户对指定表发起合并的 SQL 语句如下：

      ```sql
      ALTER SYSTEM MAJOR FREEZE TABLE_ID = table_id;
      ```

      其中，`table_id` 表示表 ID，可以通过查询视图 `DBA_OB_TABLE_LOCATIONS` 来获取。有关视图 `DBA_OB_TABLE_LOCATIONS` 中各字段的详细介绍，参见 [DBA_OB_TABLE_LOCATIONS](../../../700.system-views/400.system-view-of-mysql-mode/200.dictionary-view-of-mysql-mode/11200.o-dba_ob_table_locations-of-mysql-mode.md)。

      用户租户对指定表发起合并的示例如下：

      1. 获取待合并的表 t1 的 `table_id`。
