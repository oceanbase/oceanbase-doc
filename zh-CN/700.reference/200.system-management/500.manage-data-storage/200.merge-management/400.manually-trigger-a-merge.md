...
Tablet 与分区一一对应，在进行 Tablet 级别的合并时，需要注意以下事项：

* 当对应的分区正在执行租户级合并任务时，无法发起 Tablet 级合并。

* 当对应的分区正在执行自适应调度的合并任务时，无法发起 Tablet 级合并。

* 当对应分区的多副本处于不一致状态时，无法发起 Tablet 级合并。

* 当对应分区处于恢复中或 Transfer 状态时，无法发起 Tablet 级合并。
...
2. 执行以下命令，发起合并。
   ...
   * 发起 Tablet 级别的合并
      ...
      对指定租户发起 Tablet 级别的合并的 SQL 语句如下：
      ...
      系统租户对租户 `tenant1` 发起 Tablet 级合并的示例如下：
