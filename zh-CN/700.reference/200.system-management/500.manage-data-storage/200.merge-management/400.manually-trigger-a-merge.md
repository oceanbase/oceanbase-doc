...
* C 副本的合并比 F/R 副本的合并慢。如果 C 副本未合并完成，将无法发起新一轮的租户级合并。在手动触发合并前，建议通过查看 `CDB_OB_ZONE_MAJOR_COMPACTION` 或 `DBA_OB_ZONE_MAJOR_COMPACTION` 视图来确认上一轮的所有副本是否都已完成合并。有关如何查看合并是否完成的详细操作，参见 [查看合并信息](500.view-merge-process.md)。
