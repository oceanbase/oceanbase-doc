...
2. 执行以下命令，发起合并。
   ...
   * 发起表级合并
      ...
      1. 系统租户获取待合并的表 t1 的 `table_id`。
         ...
         ```shell
         obclient(root@sys)[oceanbase]> SELECT TENANT_ID, DATABASE_NAME, TABLE_NAME, TABLE_ID FROM oceanbase.CDB_OB_TABLE_LOCATIONS WHERE TABLE_NAME = 't1';
         ```
         ...
         ```shell
         +-----------+---------------+------------+----------+
         | TENANT_ID | DATABASE_NAME | TABLE_NAME | TABLE_ID |
         +-----------+---------------+------------+----------+
         |      1002 | test          | t1         |   500002 |
         +-----------+---------------+------------+----------+
         2 rows in set
   ...
   * 发起分区级的合并
      ...
      1. 系统租户获取待合并的表 t1 的分区对应的 `tablet_id`。
         ...
         ```shell
         obclient(root@sys)[oceanbase]> SELECT TENANT_ID, DATABASE_NAME, TABLE_NAME, TABLE_ID, TABLET_ID FROM oceanbase.CDB_OB_TABLE_LOCATIONS WHERE TABLE_NAME = 't1';
         ```
         ...
         ```shell
         +-----------+---------------+------------+----------+-----------+
         | TENANT_ID | DATABASE_NAME | TABLE_NAME | TABLE_ID | TABLET_ID |
         +-----------+---------------+------------+----------+-----------+
         |      1002 | test          | t1         |   500002 |    200001 |
         +-----------+---------------+------------+----------+-----------+
         2 rows in set
         ```
...
2. 对本租户发起租户级合并。
   ...
   * 发起分区级别的合并
      ...
      假设待合并的表的分区对应的 `tablet_id` 为 200001，对该分区发起合并的示例如下：
