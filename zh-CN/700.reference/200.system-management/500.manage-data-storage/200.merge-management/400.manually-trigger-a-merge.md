...
* 表级合并为异步任务，与分区级合并一样，不保证一定能发起成功。以下场景下，会导致表级合并发起失败：
...
2. 对本租户发起租户级合并。
   ...
   * 发起表级合并
      ...
      假设待合并的表的 `table_id` 为 500002，对该表发起合并的示例如下：
   ...
   * 发起分区级别的合并
      ...
      假设待合并的表的分区对应的 `tablet_id` 为 200007，对该分区发起合并的示例如下：
