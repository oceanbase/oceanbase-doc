...
2. 执行以下语句，查看合并信息。
   ...
   * 通过视图 `GV$OB_MERGE_INFO` 或 `GV$OB_TABLET_COMPACTION_HISTORY` 查看分区级合并信息
      ...
      * `MINI_MINOR`：L1 Compaction，多个 Mini SSTable 合成一个
      * `META_MAJOR`：一种特殊的 Compaction 类型，是将某个指定时间点之前的数据合成一个 Buffer Minor SSTable，其数据格式与 Major SSTable 一样，不包含多版本数据和未提交事务数据。
