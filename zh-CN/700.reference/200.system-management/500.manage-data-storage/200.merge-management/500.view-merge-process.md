...
2. 执行以下语句，查看合并信息。
    ...
    * 通过视图 `GV$OB_TABLET_COMPACTION_HISTORY` 查看本租户分区级合并信息。
      ...
      * `META_MAJOR_MERGE`：一种特殊的 Compaction 类型，是将某个指定时间点之前的数据合成一个 Meta Major SSTable，其数据格式与 Major SSTable 一样，不包含多版本数据和未提交事务数据。
      ...
      视图 `GV$OB_TABLET_COMPACTION_HISTORY` 中更多字段的详细说明，参见 [GV$OB_TABLET_COMPACTION_HISTORY](../../../700.system-views/400.system-view-of-mysql-mode/300.performance-view-of-mysql-mode/1000.gv-ob_tablet_compaction_history-of-mysql-mode.md)。
