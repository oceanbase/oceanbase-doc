...
2. 如果需要触发每日租户合并，需要确保租户级配置项 [enable_window_compaction](../../../800.configuration-items-and-system-variables/100.system-configuration-items/300.cluster-level-configuration-items/7000.enable_window_compaction.md) 为 `False`，即关闭窗口合并。
   ...
   2. 确认租户级配置项 `enable_window_compaction` 的值。
      ...
      ```shell
      obclient(root@sys)[(none)]> SHOW PARAMETERS LIKE 'enable_window_compaction';
      ```

   3. 如果值为 `True`，执行以下语句，关闭窗口合并。

      ```shell
      obclient(root@sys)[(none)]> ALTER SYSTEM SET enable_window_compaction = False;
      ```
