...
* 租户合并：指定一个特定的 Frozen SCN 对租户内所有的表做冻结，强制租户内所有表都合并到同一个版本。
...
* 表合并：指定某个租户下的某个表，对该表下的所有分区发起一次分区合并命令。

  表合并可以通过执行 `ALTER SYSTEM MAJOR FREEZE TABLE_ID = table_id` 语句手动触发。

* 分区合并：以一个相同的 Snapshot Version 对该分区的所有副本做一次读快照持久化，并生成一个新的 Major SSTable。
