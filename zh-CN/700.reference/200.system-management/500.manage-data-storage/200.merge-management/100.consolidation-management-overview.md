...
* 表级合并：指定一个特定的 Frozen SCN 对租户内的指定表做冻结。

   表级合并可以通过执行 `ALTER SYSTEM MAJOR FREEZE TABLE_ID = table_id` 语句手动触发。

* 分区级合并：以一个相同的 Snapshot Version 对该分区的所有副本做一次读快照持久化，并生成一个新的 Major SSTable。
  ...
  * 手动触发：通过执行 `ALTER SYSTEM MAJOR FREEZE TABLET_ID = tablet_id` 语句触发。
  ...
  除此之外，OceanBase 数据库还引入了一种全新的合并形式——窗口合并（Window Compaction）。窗口合并允许用户在指定时间窗口执行的增量式、按需式的合并。相较于传统的租户级合并，窗口合并的特点如下：
