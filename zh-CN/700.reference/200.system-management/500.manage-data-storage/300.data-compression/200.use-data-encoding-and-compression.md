...
在之前的版本中，OceanBase 数据库转储数据的存储格式仅支持 flat 格式。在面对增量数据较多的场景时，flat 格式成为了性能瓶颈。为了提高 OLAP 场景的查询性能，降低增量数据的存储空间，当前版本支持转储数据使用 encoding 格式。用户可以通过表属性 `delta_format` 来指定转储数据的存储格式。

<main id="notice" type='notice'>
<h4>注意</h4>
<p>更改转储数据存储格式的语句执行成功后，系统会有一定的延时（大约 2 分钟），待修改生效后，系统将会在之后的转储中使用 encoding 格式存储增量数据。</p>
</main>
