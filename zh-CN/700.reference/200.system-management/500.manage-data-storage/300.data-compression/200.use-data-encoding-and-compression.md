...
在之前的版本中，OceanBase 数据库转储数据的存储格式仅支持 flat 格式。在面对增量数据较多的场景时，flat 格式成为了性能瓶颈。为了提高 OLAP 场景的查询性能，降低增量数据的存储空间，OceanBase 数据库从 V4.6.0 版本开始支持转储数据使用 encoding 格式。用户可以通过表属性 `delta_format` 来指定转储数据的存储格式。
