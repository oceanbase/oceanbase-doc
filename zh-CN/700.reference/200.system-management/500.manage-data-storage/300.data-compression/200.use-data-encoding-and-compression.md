...
在之前的版本中，OceanBase 数据库转储数据的存储格式仅支持 flat 格式。在面对增量数据较多的场景时，flat 格式成为了性能瓶颈。在 V4.6.x 版本中，为了提高 OLAP 场景的查询性能，降低增量数据的存储空间，OceanBase 数据库从 V4.6.0 版本开始支持转储数据使用 encoding 格式。用户可以通过表属性 `delta_format` 来指定转储数据的存储格式。

<main id="notice" type='notice'>
<h4>注意</h4>
<p>更改转储数据的存储格式后，系统将会在之后的转储中使用 encoding 格式存储增量数据，并且不会重写已有的增量数据。</p>
</main>
...
* 创建表时，指定转储数据的存储格式
  ...
  <main id="notice" type='explain'>
  <h4>说明</h4>
  <p>或者，用户也可以在建表时不指定转储数据的存储格式，直接通过租户级配置项 <code>default_delta_format</code> 来指定所有新建表的转储格式。</p>
  </main>
...
* 修改表时，更改转储数据的存储格式
  ...
  语句中，`delta_format` 支持以下两种取值：

  * `flat`：表示转储数据使用 flat 格式。flat 格式是默认的转储格式，面向 OLTP 场景，写入速度快，压缩率低，但对 OLAP 场景的查询不友好。

  * `encoding`：表示转储数据使用 encoding 格式。面向 OLAP 场景，写入相对较慢，压缩率高，对 OLAP 场景的查询友好，在 delete_insert 表上会有更明显的效果。
...
* 创建表 t1 时，指定表 t1 的转储数据的存储格式为 encoding 格式。

  ```shell
  obclient(root@mysql001)[test]> CREATE TABLE t1(col1 int, col2 float) delta_format = 'encoding';
  ```

* 修改表 t2 时，更改表 t2 的转储数据的存储格式为 encoding 格式。

  ```shell
  obclient(root@mysql001)[test]> ALTER TABLE t2 SET delta_format = 'encoding';
  ```

  语句执行成功后，系统会有一定的延时（大约 2 分钟），待修改生效后，系统将会在之后的转储中使用 encoding 格式存储增量数据（不会重写已有的增量数据）。
