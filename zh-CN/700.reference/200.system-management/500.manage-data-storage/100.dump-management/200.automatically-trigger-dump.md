...
当一个租户 Active MemStore 内存的使用量达到 `freeze_trigger_percentage * memstore_limit`（其中，`memstore_limit = 租户内存 * memstore_limit_percentage`）时，系统就会自动触发冻结（转储的前置动作），然后再调度转储。当冻结（minor freeze）的次数达到一定的阈值时，就会触发自动合并。
