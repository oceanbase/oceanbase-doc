...
系统租户可以通过视图查看所有租户的磁盘空间占用情况。当前暂不支持用户租户查看本租户的磁盘空间占用情况。
...
2. 执行以下语句，查看租户磁盘空间的占用情况。
   ...
   * 查看租户在所有节点上合计所占用的磁盘空间大小

        系统租户下，查询租户 `mysql001` 在所有节点上合计占用的磁盘空间大小，语句如下：
        ...
        +-----------+-------------+----------------+-------------+------------+------------+-------------+
        | 1002      | mysql001    |  xx.xx.xx.xx   |        2882 | clog Data  | 9246953565 |  9246953565 |
        | 1002      | mysql001    |  xx.xx.xx.xx   |        2882 | Index Data |       6729 |       57344 |
        | 1002      | mysql001    |  xx.xx.xx.xx   |        2882 | Meta Data  |   53637120 |    53637120 |
        | 1002      | mysql001    |  xx.xx.xx.xx   |        2882 | slog Data  |   66043904 |    66043904 |
        | 1002      | mysql001    |  xx.xx.xx.xx   |        2882 | Table Data |    8081723‬ |    15175680‬ |
        ...
        * `SPACE_TYPE`：数据的类型，包括以下几种类型：
          ...
          * `Table Data`：所有主表 Tablet 的数据，包含 Lob Tablet
          * `Index Data`：所有索引表 Tablet 的数据
          * `Meta Data`：所有元数据
          * `Tmp Data`：临时文件
          * `clog Data`：所有节点上的 clog 总大小
          * `slog Data`：所有节点上的 slog 总大小
        
        * `DATA_BYTES`：数据内容的大小，单位为字节。
        * `USAGE_BYTES`：实际占用的磁盘空间大小，单位为字节。

   * 查看租户在各节点上分别占用的磁盘空间大小

        系统租户下，查询租户 `mysql001` 在各节点上分别占用的磁盘空间大小，语句如下：
