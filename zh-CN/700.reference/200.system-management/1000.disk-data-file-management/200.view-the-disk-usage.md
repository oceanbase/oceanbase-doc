...
2. 执行以下语句，查看租户磁盘空间的占用情况。
   ...
   * 查看租户在所有节点上合计所占用的磁盘空间大小
        ...
        +-----------+-------------+----------+------+-------------+-------------+
        | 1002      | mysql001    | NULL     | NULL | clog Data   |  9246953565 |
        ...
        * `SPACE_TYPE`：数据的类型，包括以下几种类型：
          ...
          * `clog Data`：本地部署模式下的 clog，包括租户所有副本节点的所有 clog。
          * `Shared Data`：租户共享的多版本基线，仅适用于 SS 模式；SN 模式下，该字段值恒为 0，表示未共享数据。
          * `Local Data`：本地部署模式下的所有宏块，包括租户所有副本节点的所有数据、所有元数据、所有临时文件等。
          * `PRIVATE`：多副本多版本总和、临时文件等。
        
        * `USAGE_BYTES`：实际占用的磁盘空间大小，单位为字节。

   * 查看租户在各节点上分别占用的磁盘空间大小

        系统租户下，查询租户 `mysql001` 在各节点上各类数据分别占用的磁盘空间大小，语句如下：
