...
对于 OceanBase 数据库社区版和企业版，如果您在部署 OceanBase 数据库并启动 OceanBase 集群时，`datafile_size`（或`datafile_disk_percentage`）、`datafile_next`、`datafile_maxsize` 等均未配置，您可以参考以下步骤，开启磁盘数据文件的自动扩容。
