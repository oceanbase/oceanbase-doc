...
如果您在部署 OceanBase 数据库并启动 OceanBase 集群时，已将 `datafile_size`（或 `datafile_disk_percentage`）、`datafile_disk_percentage`、`datafile_next` 及 `datafile_maxsize` 配置为非 0 的值，则：
