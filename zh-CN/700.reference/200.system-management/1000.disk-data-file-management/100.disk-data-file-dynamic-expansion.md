...
对于 `datafile_size` 和 `datafile_disk_percentage` 均未配置的其他所有场景，您只需要确保 `datafile_next` 为非 0 的值，同时 `datafile_maxsize` 的值大于当前磁盘已经预分配的空间的值即可。具体操作请参考本文档中 **集群启动时，未配置 datafile_size、datafile_disk_percentage、datafile_next、datafile_maxsize** 的内容进行操作。
