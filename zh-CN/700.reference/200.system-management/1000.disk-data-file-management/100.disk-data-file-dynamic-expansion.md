...
* OceanBase 数据库社区版
  ...
  对于 OceanBase 数据库社区版，如果您在部署 OceanBase 集群前修改配置文件时，已将 `datafile_size`（或 `datafile_disk_percentage`）、`datafile_disk_percentage`、`datafile_next` 及 `datafile_maxsize` 配置为非 0 的值，则：
...
对于 OceanBase 数据库社区版和企业版，如果您在部署 OceanBase 数据库并启动 OceanBase 集群时，如果仅设置了 `datafile_size` 或 `datafile_disk_percentage` 的值，您只需要手动设置 `datafile_maxsize` 和 `datafile_next` 的值，并且确保 `datafile_maxsize` 的值大于 `datafile_size` 的值，或者大于根据 `datafile_disk_percentage` 比例换算的磁盘空间大小即可。
