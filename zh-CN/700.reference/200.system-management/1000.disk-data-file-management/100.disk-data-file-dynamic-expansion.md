...
* `datafile_size` 与 `datafile_disk_percentage`

  * 当 `datafile_size` 的值为 `OM` 且 `datafile_disk_percentage` 的值也为 `0` 时，系统会根据日志和数据是否共用同一磁盘来自动计算数据文件占用其所在磁盘总空间的百分比，即：

    * 如果日志和数据共用同一磁盘，则数据文件占用其所在磁盘总空间的百分比为 60%。

    * 如果日志和数据独占磁盘时，则数据文件占用其所在磁盘总空间的百分比为 90%。

  * 当 `datafile_size` 的值为 `OM` 且 `datafile_disk_percentage` 的值为非 `0`，或者，当 `datafile_size` 的值为非 0 且 `datafile_disk_percentage` 的值为 `0` 时，以非 `0` 的值为准。

  * 当 `datafile_size` 与 `datafile_disk_percentage` 同时配置为非 0 的值时，以 `datafile_size` 设置的值为准。

* `datafile_next` 和 `datafile_maxsize`
