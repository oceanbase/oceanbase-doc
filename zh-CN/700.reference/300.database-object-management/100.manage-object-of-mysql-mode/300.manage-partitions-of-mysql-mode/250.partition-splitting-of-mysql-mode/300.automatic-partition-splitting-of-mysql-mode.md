...
| table_column_group_option | 可选项，指定表的列存选项。具体解释如下：<ul><li>`WITH COLUMN GROUP(all columns)`：指定添加行存表。</li><li>`WITH COLUMN GROUP(each column)`：指定创建列存表。</li><li>`WITH COLUMN GROUP(all columns, each column)`：指定创建行存列存冗余的表。</li></ul>|
...
1. 创建自动分区行存表 `test5_tbl1`，不指定预分区键，指定分裂阈值为 10GB。
    ...
    ```shell
    obclient> CREATE TABLE test5_tbl1 (col1 INT, col2 INT, col3 VARCHAR(30), PRIMARY KEY(col1))
        PARTITION BY RANGE() SIZE('10GB')
        WITH COLUMN GROUP (all columns);
    ```
...
2. 创建自动分区列存表 `test5_tbl2`，不指定预分区键，指定分裂阈值为 10GB。
    ...
    ```shell
    obclient> CREATE TABLE test5_tbl2 (col1 INT, col2 INT, col3 VARCHAR(30), PRIMARY KEY(col1))
        PARTITION BY RANGE() SIZE('10GB')
        WITH COLUMN GROUP (each column);
    ```
...
3. 创建自动分区列存行存列存冗余表 `test5_tbl3`，不指定预分区键，指定分裂阈值为 10GB。
    ...
    ```shell
    obclient> CREATE TABLE test5_tbl3 (col1 INT, col2 INT, col3 VARCHAR(30), PRIMARY KEY(col1))
        PARTITION BY RANGE() SIZE('10GB')
        WITH COLUMN GROUP (all columns, each column);
    ```
