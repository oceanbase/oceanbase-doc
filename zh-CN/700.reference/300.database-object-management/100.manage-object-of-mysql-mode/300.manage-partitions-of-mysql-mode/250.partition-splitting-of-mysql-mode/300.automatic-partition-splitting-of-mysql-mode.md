...
OceanBase 数据库的行存表和列存表都支持自动分区分裂。当前版本仅支持对 Range/Range Columns 分区的一级分区有主键表支持自动分区能力。
...
1. 创建自动分区列存表 `test1_cg1`，不指定预分区键，指定分裂阈值为 10GB。
...
2. 创建自动分区列存行存列存冗余表 `test1_cg2`，不指定预分区键，指定分裂阈值为 10GB。
