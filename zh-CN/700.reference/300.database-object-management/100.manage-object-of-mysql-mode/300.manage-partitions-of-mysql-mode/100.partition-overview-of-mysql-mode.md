...
|slug|partition-overview-of-mysql-mode|
...
数据表中每一行中用于计算这一行属于哪一个分区的列的集合叫做分区键，分区键必须是主键或唯一键的子集（即，分区键必须由唯一键列的一个或多个组成，但不需要包含唯一键中的所有列），只要有主键就必须是主键的子集。由分区键构成的用于计算这一行属于哪一个分区的表达式叫做分区表达式。
