...
OceanBase 数据库的分区交换功能主要用来实现快速将数据导入/导出分区表，达到数据高效加载的目的，以简化向现有分区表快速加载新增数据的过程（作为增量数据旁路导入的临时替换功能）。
...
* 不支持数据校验，用户需要自行确保交换的数据满足所有要求。即分区交换只支持不校验数据的分区交换，所以需要保证分区表交换分区和非分区表数据的正确性，每次使用前希望用户务必自己检查交换分区的分区键范围，查询非分区表的数据是否满足该分区键，如果不满足要求，不去执行分区交换操作。
* 当前的分区交换功能建议只作为增量旁入导入的临时替换功能。
...
分区交换功能的使用需极为小心，因为不当操作可引起数据不符合原表分区键的条件，进而触发连锁错误。因此，在执行分区交换时，该按照以下步骤来判断分区表与非分区表是否能够进行分区交换。
...
2. 通过查询结果，需要保证分区表和非分区表的表级和列级属性一一对应。

3. 判断分区表和非分区表的约束是否完全相同。检查分区表和非分区表是否存在外键约束，分区交换当前版本不支持两表存在外键约束。

4. 校验分区表局部存储的索引表是否和非分区表的索引表能够一一对应。

5. 使用分区交换的语法进行分区交换，交换之后，分区表指定分区的数据会迁移到非分区表上，非分区表的数据会迁移到分区表的指定分区上。分区表的全局索引状态设置为 Unusable。
