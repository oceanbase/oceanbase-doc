...
* 仅支持以下类型的分区表开启动态分区管理：
  ...
  * Range Columns 分区且分区键为 `date`、`datetime`、`timestamp`、`year`、`bigint`（指定 `BIGINT_PRECISION` 精度）类型的分区表。
  * Range 分区且分区键为 `year`、`bigint`（指定 `BIGINT_PRECISION` 精度）类型的分区表。
  * List Columns 分区且分区键为字符串类型时间列类型的分区表。字符串分区键支持 `VARCHAR`/`CHAR`，格式由 `TIME_UNIT` 取值决定：
    ...
    * `TIME_UNIT = 'hour'` 时：字符串分区键格式为 `YYYYMMDDHH`，例如 `2026081910`。
    ...
    <main id="notice" type='explain'>
      <h4>说明</h4>
      <p>对于 V4.4.2 版本，从 V4.4.2 BP3 版本开始支持 List Columns 分区且分区键为字符串类型时间列类型的分区表开启动态分区管理。</p>
    </main>
...
创建 Range Columns 分区的动态分区表 `test_tbl1`，设置动态分区属性为：
...
创建 List Columns 分区的动态分区表 `test_tbl2`，分区键为字符串时间列，格式：`YYYYMMDD`。设置动态分区属性为：
...
* 使用北京时间（UTC+8）判断时间，确保分区边界和过期逻辑基于该时区。
* 分区键 `col2` 是 `VARCHAR` 类型，无需配置 `bigint` 的时间戳精度。
