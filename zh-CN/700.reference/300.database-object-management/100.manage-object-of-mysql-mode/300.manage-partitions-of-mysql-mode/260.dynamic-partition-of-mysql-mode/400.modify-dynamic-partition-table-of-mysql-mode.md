...
5. 为尚未启用动态分区管理的普通 Range Columns 分区表添加动态分区策略。

   1. 创建一张普通 Range Columns 分区表 `test_tbl3`。
