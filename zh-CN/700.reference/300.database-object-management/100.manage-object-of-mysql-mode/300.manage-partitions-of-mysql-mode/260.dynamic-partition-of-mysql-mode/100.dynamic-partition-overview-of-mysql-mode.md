...
* 若现存最大分区边界大于当前时间 `2025-04-23 16:24:56`，调度一次动态分区管理后，分区为：
...
* 若现存最大分区边界小于当前时间，预创建分区会尝试补充历史分区。例如现存最大分区为 `PARTITION P0 VALUES LESS THAN ('2025-04-23 15:30:00')`，当前时间为 `2025-04-23 16:26:56`，调度一次动态分区管理后，分区为：
...
* 对于从 V4.3.5 BP2 之前版本升级上来的租户，执行 `DROP` 分区操作将导致全局索引失效。对于存在全局索引的表，谨慎使用删除过期分区功能。

    <main id="notice" type='explain'>
      <h4>说明</h4>
      <p>对于 V4.3.5 版本，OceanBase 数据库从 V4.3.5 BP2 版本开始引入租户级隐藏配置项 <code>_ob_enable_truncate_partition_preserve_global_index</code>，用于控制是否允许在 <code>TRUNCATE</code>/<code>DROP</code> 主表分区（以下简称分区 DDL）时，全局索引不失效。<ul><li>对于升级上来的租户，该隐藏配置项默认值为 <code>False</code>，表示 <code>TRUNCATE</code>/<code>DROP</code> 主表分区，全局索引失效。</li><li>对于新建租户默认值为 <code>True</code>，表示 <code>TRUNCATE</code>/<code>DROP</code> 主表分区，全局索引不失效。</li></ul></p>
    </main>
