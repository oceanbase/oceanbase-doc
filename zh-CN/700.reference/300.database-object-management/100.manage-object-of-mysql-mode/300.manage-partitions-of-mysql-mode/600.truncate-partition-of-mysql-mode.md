...
对于 V4.3.5 版本，OceanBase 数据库从 V4.3.5 BP2 版本开始引入租户级隐藏配置项 `_ob_enable_truncate_partition_preserve_global_index`，用于控制是否允许在 `TRUNCATE`/`DROP` 主表分区（以下简称分区 DDL）时，继续维持全局索引。对于升级上来的租户，该隐藏配置项默认值为 `False`，对于新建租户默认值为 `True`，取值行为对比如下：

| 配置项值 | 分区 DDL 执行期间的行为 | 分区 DDL 执行完成后的结果 |
|----------|-----------------------|-------------------------|
| True | 严格隔离：主表的所有分区以及所有相关的索引表均不允许执行任何 DML 操作。| 索引维持：主表和所有的索引表都可以执行 DML 操作，并且主表相关的全局索引会继续维持。|
| False | 局部锁定：只有 `TRUNCATE` 的主表分区不允许 DML，主表的其他分区可以正常 DML。| 索引失效：主表和所有的索引表都可以执行 DML 操作，主表相关的全局索引会失效或者重建。|
