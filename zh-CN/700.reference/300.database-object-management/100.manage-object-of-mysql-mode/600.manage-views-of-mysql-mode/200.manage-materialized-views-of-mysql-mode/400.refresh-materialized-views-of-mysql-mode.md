...
<main id="notice" type='notice'>
  <h4>注意</h4>
  <p><ul><li>由于 <code>REFRESH FAST</code> 方法利用物化视图日志中的记录信息来确定需要增量刷新的内容，因此在使用增量刷新刷新物化视图时，需要在创建物化视图之前就创建基表的物化视图日志（mlog）。 <ul><li>有关创建物化视图日志的信息，请参见 <a href="200.materialized-views-log-of-mysql-mode.md">物化视图日志</a>。</li><li>OceanBase 数据库支持物化视图日志自动管理功能。如果开启自动管理 mlog，那么在创建增量刷新物化视图前，用户无需创建基表的 mlog，OceanBase 数据库会自动创建对应的 mlog 或更新已存在的 mlog 表定义使其包含新建物化视图依赖的列。详细信息，参见 <a href="250.automatic-management-materialized-views-log-of-mysql-mode.md">物化视图日志自动管理</a>。</li></ul> </li><li>增量刷新物化视图中使用的列都必须在 mlog 里。</li></ul></p>
</main>
...
满足以上单表聚合增量刷新基本要求时，使用聚合函数 `MAX` 和 `MIN` 的要求：在物化视图基表上存在以 `GROUP BY` 列为前缀的索引。
...
3. 创建刷新方法是增量刷新的物化视图。
    ...
    * 在 `GROUP BY` 中使用非基本列。
      ...
      3. 在表 `m_tbl1` 上基于 `col3` 和 `(DATE(col2))` 列创建一个名为 `idx1_m_tbl1` 的索引。

          ```shell
          obclient> CREATE INDEX idx1_m_tbl1 ON m_tbl1(col3, (DATE(col2)));
          ```

      4. 创建单表聚合增量刷新的物化视图 `m_tbl1_mv1`。
...
2. `FROM` 表至少包含两张表。
    ...
    <main id="notice" type='notice'>
      <h4>注意</h4>
      <p><ul><li>OceanBase 数据库物化视图的增量刷新功能支持多表外连接（<code>LEFT JOIN</code>/<code>RIGHT JOIN</code>）。</li><li>外连接使用限制：连接树需要是等价于 <code>INNER JOIN</code> 在先，<code>LEFT JOIN</code> 在后的左深连接树。</li><li>若增量刷新物化视图的 <code>SELECT</code> 子句包含 <code>LEFT JOIN</code> 算子，则禁止在创建时指定主键（<code>PRIMARY KEY</code>）或唯一索引（<code>UNIQUE INDEX</code>），以避免因约束冲突引发的刷新异常。</li></ul></p>
    </main>
...
DBMS_MVIEW.REFRESH (
    ...
    IN method           VARCHAR(65535) DEFAULT NULL, -- 刷新选项
                                                     ...
                                                     --   C|c 完全刷新
                                                     --   A|a 始终刷新，等价于C
    IN refresh_parallel INT DEFAULT 0);              -- 刷新并行度
```
...
<main id="notice" type='explain'>
  <h4>说明</h4>
  <p>OceanBase 数据库支持物化视图日志自动管理功能。如果开启自动管理 mlog，那么在创建实时物化视图前，用户无需创建基表的 mlog，OceanBase 数据库会自动创建对应的 mlog 或更新已存在的 mlog 表定义使其包含新建物化视图依赖的列。详细信息，参见 <a href="250.automatic-management-materialized-views-log-of-mysql-mode.md">物化视图日志自动管理</a>。</p>
</main>
...
7. 基于物化视图 `mv_tbl3_2` 创建物化视图 `mv_tbl3_3`。
...
* 更多有关收集表和列的统计信息的信息，请参见 [GATHER_TABLE_STATS](../../../../600.pl-reference/200.pl-mysql/1000.pl-system-package-mysql/15900.dbms-stats-mysql/1700.gather-table-stats-mysql.md)。

* 更多有关管理物化视图刷新操作的统计信息的收集和保留的信息，请参见 [DBMS_MVIEW_STATS 概述](../../../../600.pl-reference/200.pl-mysql/1000.pl-system-package-mysql/10050.dbms-mview-stat-mysql/100.dbms-mview-stat-overview-mysql.md)。
