...
<main id="notice" type='explain'>
  <h4>说明</h4>
  <p>对于 V4.3.5 版本，从 V4.3.5 BP4 版本开始支持 <code>MAX</code> 和 <code>MIN</code> 聚合增量刷新。</p>
</main>
...
多表聚合增量刷新基本要求是上述 [单表聚合增量刷新](#单表聚合增量刷新) 和 [多表连接增量刷新](#多表连接增量刷新) 基本要求的并集。不过其中与多表连接要求不同的点：`FROM` 表均存在主键，但是主键字段不要求在 `SELECT` 中输出。
