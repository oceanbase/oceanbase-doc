...
* `refresh_clause`：可选项，指定物化视图的刷新方式、刷新并行度等。
...
有关创建物化视图语法的详细参数说明信息，参见 [CREATE MATERIALIZED VIEW](../../../../500.sql-reference/100.sql-syntax/200.common-tenant-of-mysql-mode/600.sql-statement-of-mysql-mode/2250.create-materialized-views-of-mysql-mode-in-sql.md)。
...
* 在创建实时物化视图前，物化视图依赖的基表都需要创建物化视图日志，OceanBase 数据库支持物化视图日志自动管理功能，默认系统自动创建物化视图日志，详细信息参见 [物化视图日志](200.materialized-views-log-of-mysql-mode.md)。
...
3. 创建实时物化视图后，可以通过视图 [DBA_MVIEWS](../../../../700.system-views/400.system-view-of-mysql-mode/200.dictionary-view-of-mysql-mode/5000.o-dba_mviews-of-mysql-mode.md) 查看物化视图的定位是否为实时物化视图。
...
* `REFRESH FAST` 方法利用物化视图日志中的记录信息来确定需要增量刷新的内容，OceanBase 数据库支持物化视图日志自动管理功能，默认系统自动创建物化视图日志，详细信息参见 [物化视图日志](200.materialized-views-log-of-mysql-mode.md)。
...
2. 基于表 `tbl5` 创建名为 `mv_tbl5` 的物化视图，指定物化视图的刷新策略为增量刷新，在查询部分指定从 `tbl5` 表中按照 `col2` 列进行分组，并计算每个分组中的记录数（`cnt`）、非空 `col3` 列的记录数（`cnt_col3`）和 `col3` 列的总和（`sum_col3`）作为物化视图的结果。
