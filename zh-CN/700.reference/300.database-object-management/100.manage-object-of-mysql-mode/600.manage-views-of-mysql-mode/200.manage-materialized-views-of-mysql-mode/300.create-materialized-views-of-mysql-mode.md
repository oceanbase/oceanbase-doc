...
基于表 `tbl1` 创建名为 `mv_pp_tbl1` 的物化视图，指定物化视图的查询并行度为 `5`，将物化视图按照 `col1` 列进行 Hash 分区，并分为 `8` 个分区，将 `tbl1` 表中满足条件 `col3 >= 20` 的记录作为基表进行查询，并将查询结果作为物化视图的数据。
...
2. 基于表 `tbl5` 创建名为 `mv_tbl5` 的物化视图，指定物化视图的刷新策略为增量刷新，在查询部分指定从 `tbl5` 表中按照 `col2` 列进行分组，并计算 `col3` 列的总和（`sum_col3`）作为物化视图的结果。
