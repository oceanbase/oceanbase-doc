...
* 您可以通过配置项 [default_table_store_format](../../../../../800.configuration-items-and-system-variables/100.system-configuration-items/400.tenant-level-configuration-items/2200.default_table_store_format.md) 来设置创建的物化视图的默认存储格式。此外，您也可以通过指定 `mv_column_group_option` 选项来明确指定创建的物化视图的格式。
  ...
  **示例如下：**
  ...
  3. 查看物化视图 `mv5_tbl1` 定义。
      ...
      +----------+-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+----------------------+----------------------+
      | mv5_tbl1 | CREATE MATERIALIZED VIEW `mv5_tbl1` DEFAULT CHARSET = utf8mb4 ROW_FORMAT = DYNAMIC COMPRESSION = 'zstd_1.3.8' REPLICA_NUM = 1 BLOCK_SIZE = 16384 USE_BLOOM_FILTER = FALSE TABLET_SIZE = 134217728 PCTFREE = 0 REFRESH FORCE ON DEMAND  WITH COLUMN GROUP(each column) AS select `db_test`.`tbl1`.`id` AS `id`,`db_test`.`tbl1`.`name` AS `name`,`db_test`.`tbl1`.`age` AS `age` from `db_test`.`tbl1` | utf8mb4              | utf8mb4_general_ci   |
      +----------+-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+----------------------+----------------------+
      1 row in set
      ```
