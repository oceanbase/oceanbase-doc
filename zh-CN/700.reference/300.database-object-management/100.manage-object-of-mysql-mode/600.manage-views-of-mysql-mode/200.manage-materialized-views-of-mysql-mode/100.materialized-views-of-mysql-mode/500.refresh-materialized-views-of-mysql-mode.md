...
1. 创建表 `tbl2`。
    ...
    ```sql
    CREATE TABLE tbl2(col1 INT PRIMARY KEY, col2 INT, col3 INT, col4 INT);
    ```

2. 在 `tbl2` 表上创建物化视图日志。

    ```sql
    CREATE MATERIALIZED VIEW LOG ON tbl2 WITH PRIMARY KEY, ROWID, SEQUENCE (col2, col3) INCLUDING NEW VALUES;
    ```

3. 创建多表增量刷新的物化视图 `mv1_tbl2_tbl2`。

    ```sql
    CREATE MATERIALIZED VIEW mv1_tbl2_tbl2
      REFRESH FAST ON DEMAND
      AS SELECT a.col1 AS a_c1, b.col1 AS b_c1 
        FROM tbl2 a, tbl2 b
        WHERE a.col2 = b.col3;
    ```
