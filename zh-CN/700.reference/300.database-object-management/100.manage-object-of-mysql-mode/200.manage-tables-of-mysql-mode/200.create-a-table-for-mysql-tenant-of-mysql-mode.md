...
创建表时指定聚集列的语法是在 `CREATE TABLE` 语句后增加表选项 `CLUSTER BY`，SQL 语句如下：
...
`column_name`：表示聚集列用来排序列名称。

<main id="notice" type='notice'>
  <h4>注意</h4>
  <p>在 OceanBase 数据库 MySQL 租户下，如果用户没有显式指定表模式（<code>ORGNIZATION = HEAP</code>），但定义了 <code>CLUSTER BY</code> 选项，OceanBase 数据库将默认创建为堆组织表。</p>
</main>

**示例如下：**

创建一个按 `col2` 和 `col4` 排序的表 `cb_tbl1`。
...
<main id="notice" type='explain'>
  <h4>说明</h4>
  <p>从 OceanBase 数据库 V5.0.1 版本开始，表创建后可以使用 <code>ALTER TABLE ... SET merge_engine</code> 语句在不同表模式之间进行 Online 转换，具体操作请参见 <a href="600.change-table-of-mysql-mode.md">更改表</a>；在 V5.0.1 之前的版本，参数 <code>MERGE_ENGINE</code> 在创建表时指定之后，其配置值将不可修改。</p>
</main>

其中，`MERGE_ENGINE` 参数用于指定表的更新模型，取值如下：

* `partial_update`：表示采用部分更新模式。每次更新仅记录修改的列（delta），存储空间更节省，但查询时需合并多份数据以获取最新值，适用于高频更新、查询需求较低的 OLTP 场景。
...
* `append_only`：表示表的更新模式只允许 `INSERT` 操作，禁止其他所有会产生存储数据修改的 DML 和 DDL 操作。该模式下如果要做过期数据清理可以使用 TTL 功能。使用限制详细介绍可参见 [更新模型使用限制](#更新模型使用限制)。
...
| 不可变性、高效追加、简化查询 | `append_only` | 适合高吞吐场景（如日志、IoT 数据监控）；范围查询和聚合计算效率高。|
...
* `INSERT [IGNORE] xxx ON DUPLICATE KEY UPDATE` 语法不禁止，但是当产生实际数据修改时会报错。
...
* 创建一个 `append_only` 模式的表，并执行 `INSERT`、`SELECT`、`UPDATE` 和 `DELETE` 操作。
