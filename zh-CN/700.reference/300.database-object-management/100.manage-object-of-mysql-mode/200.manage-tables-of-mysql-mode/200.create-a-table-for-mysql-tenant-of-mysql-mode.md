...
* `append_only`：表示表的更新模式只允许 `INSERT` 操作，禁止其他所有会产生存储数据修改的 DML 和 DDL 操作。该模式下如果要做过期数据清理可以使用 Compaction TTL 功能。使用限制详细介绍可参见 [更新模型使用限制](#更新模型使用限制)。
...
`column_name`：表示聚集列用来排序列名称。可以由表中的任意列组成，并支持 `NULL` 值。
