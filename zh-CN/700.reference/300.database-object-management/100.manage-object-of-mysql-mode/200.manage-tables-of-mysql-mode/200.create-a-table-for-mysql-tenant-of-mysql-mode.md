...
创建表时指定组织形式的语法是在 `CREATE TABLE` 语句后增加表选项 `ORGANIZATION`，SQL 语句如下：
...
* `HEAP`：表示表模型为非聚集索引表模型，即堆表。

如果不指定 `ORGANIZATION` 选项，则该选项取值根据部署场景决定：

* OLAP 场景：取值为 `HEAP`。
* 其他场景：均为 `INDEX`。

有关 OceanBase 数据库部署场景的介绍，参见 [配置最佳实践](../../../../400.deploy/700.best-practices.md)。
