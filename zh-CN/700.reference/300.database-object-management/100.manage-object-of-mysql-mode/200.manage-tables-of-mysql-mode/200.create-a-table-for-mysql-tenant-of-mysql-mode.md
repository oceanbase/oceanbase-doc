...
|slug|create-a-table-for-mysql-tenant-of-mysql-mode|
...
当配置项 [`default_table_store_format=‘row’`](../../../800.configuration-items-and-system-variables/100.system-configuration-items/400.tenant-level-configuration-items/27000.default_table_store_format.md)（默认值）时，默认创建的表为行存表，当 [default_table_store_format](../../../800.configuration-items-and-system-variables/100.system-configuration-items/400.tenant-level-configuration-items/27000.default_table_store_format.md) 不为 `row` 时，可通过设置 `WITH COLUMN GROUP(all columns)` 选项创建行存表。
