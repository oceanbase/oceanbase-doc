...
Hadoop 分布式文件系统（HDFS）是 Hadoop 生态系统中的一个核心组件，用于存储和处理大规模数据集。因此，OceanBase 数据库新增了 HDFS 外表读功能，支持直接访问 HDFS 存储的数据。
...
OceanBase 数据库在 V4.4.0 版本支持 HDFS 外表驱动写的功能。关于 HDFS 外表驱动写的详细介绍信息，参见 [SELECT INTO](../../../../500.sql-reference/100.sql-syntax/200.common-tenant-of-mysql-mode/600.sql-statement-of-mysql-mode/8000.select-into-of-mysql-mode.md)。
...
OceanBase 数据库通过 Catalog（数据目录） 功能，实现对外部数据源的统一管理和高效查询。该功能通过新增 Catalog-Database-Table 三层数据层级，支持直接访问外部数据源（如 ODPS、HMS 等）中的表数据，无需手动创建映射表。更多 Catalog 介绍信息，参见 [Catalog 概述](../../1000.manage-catalog-of-mysql-mode/100.catalog-overview-of-mysql-mode.md)。

OceanBase 数据库 Catalog 数据源支持情况如下：

|     类型     | 支持版本 | 数据源类型 | 表格式支持 | 说明 |
|--------------|---------|-----------|-----------|------|
| ODPS Catalog | OceanBase 数据库 V4.3.5 BP2 及以上版本 | ODPS | MaxCompute 表    | 适用于阿里云 MaxCompute 平台的数据查询。|
| HMS Catalog  | OceanBase 数据库 V4.4.1 及以上版本     | HMS  | Hive、Iceberg 表 | 通过 Hive Metastore 管理元数据，支持开源数据湖生态。|
