...
* `col_name col_type [AS (metadata$filecol{N} | external$filepos{N})]`：用于定义列。其中，`AS (metadata$filecol{N})` 用于手动定义列映射，`AS (external$filepos{N})` 用于按位置索引列。
  ...
  当外部文件中的列顺序与外表中定义的列顺序不一致时，您可以通过以下两种方式指定列映射：

  * 使用 `metadata$filecol{N}` 语法：适用于 CSV 格式，指定外表的列对应外部文件中的第 N 列。其中，文件中的列从 1 开始编号。

  * 使用 `external$filepos{N}` 语法：适用于 Parquet 和 ORC 格式，按位置索引列。需要配合 `COLUMN_INDEX_TYPE = 'POSITION'` 参数使用。
  ...
  对于 Parquet 和 ORC 格式，您可以使用 `external$filepos{N}` 语法：
  ...
  <main id="notice" type='notice'>
  <h4>注意</h4>
  <p>当使用 <code>external$filepos{N}</code> 语法时，必须设置 <code>COLUMN_INDEX_TYPE = 'POSITION'</code> 参数。</p>
  </main>
...
* `FORMAT = ( TYPE = 'PARQUET'...)`：用于定义外部文件格式为 PARQUET。
  ...
  * `TYPE`：指定外部文件的类型为 `PARQUET`。
  * `COLUMN_INDEX_TYPE`：指定列索引类型。当前支持 `'POSITION'` 和 `'NAME'`，表示按位置索引列和按名称索引列。当使用 `external$filepos{N}` 语法时，需要设置此参数。

* `FORMAT = ( TYPE = 'ORC'...)`：用于定义外部文件格式为 ORC。

  * `TYPE`：指定外部文件的类型为 `ORC`。
  * `COLUMN_INDEX_TYPE`：指定列索引类型。当前支持 `'POSITION'`，表示按位置索引列。当使用 `external$filepos{N}` 语法时，需要设置此参数。

<main id="notice" type='explain'>
  <h4>说明</h4>
  <p>OceanBase 数据库从 V4.6.0 版本开始：<ul><li>Parquet/ORC 文件格式外表支持读取复杂数据类型：<code>Array</code>、<code>Map</code>、<code>JSON</code>。外表读取需与 OceanBase 数据库支持的数据类型需要保持一致，详细介绍请参见 <a href="../../../../500.sql-reference/100.sql-syntax/200.common-tenant-of-mysql-mode/100.basic-elements-of-mysql-mode/100.data-type-of-mysql-mode/100.data-type-overview-of-mysql-mode.md">数据类型概述</a>。</li><li>OceanBase 数据库 Parquet/ORC 文件格式外表支持读取 Bloom Filter，根据查询的谓词来过滤数据，减少不必要的 IO，用于查询加速。支持读取外表的 Bloom Filter 功能默认是关闭，如果要使用该功能，需要修改租户级配置项 <code>_enable_external_table_bloom_filter</code> 为 <code>true</code>（<code>ALTER SYSTEM SET _enable_external_table_bloom_filter = true;</code>）。</li></ul></p>
</main>
