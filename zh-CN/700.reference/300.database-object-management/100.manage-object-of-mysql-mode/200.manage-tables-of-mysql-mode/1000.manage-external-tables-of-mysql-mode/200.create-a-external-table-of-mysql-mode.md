...
* `TYPE = 'ODPS'`：指定外部文件的格式为 `ODPS` 类型。还包含以下字段：
  ...
  * `API_MODE`：指定调用 ODPS 的 API 模式。取值如下：
    ...
    * `tunnel_api`：默认值，在 C++ connector 和 jni connector 下有效, 默认情况就是该选项，也是旧版本的默认选项。无需对 MaxCompute 进行操作。在未开通 MaxCompute Storage API，或者 OceanBase 数据库和 MaxCompute 不能在同一个 VPC 下使用。
    * `storage_api`：要求 MaxCompute 和 OceanBase 数据库部署在同一个 VPC 内。当一张表各个行数据字节数差异过大的时候SPLIT采用byte，而其他情况采用row。
