...
* `EXTENDED`：如果需要查看隐藏的表，可以添加该关键字。通过该方式查出来的隐藏表同样可以使用 `DROP TABLE` 语句将其删除，有关删除表的详细操作，请参见 [删除表](800.delete-a-table-of-mysql-mode.md)。
...
* `EXTENDED`：如果需要查看隐藏的列，可以添加该关键字。

* `FULL`：用于展示列的字符序、权限与注释。
