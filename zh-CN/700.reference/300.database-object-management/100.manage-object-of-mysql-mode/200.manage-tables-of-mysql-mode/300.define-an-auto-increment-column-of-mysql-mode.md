...
<main id="notice" type='notice'>
  <h4>注意</h4>
  <p>在尝试减小表的自增字段值时，请注意以下情况：
  <ol><li>如果表中已存在记录并且至少有一个记录的自增字段值高于您试图设置的新值，则减小自增值的操作虽然会执行成功，但是表的自增设置不会被实际修改。因此，后续插入的记录仍会按照原始的更高自增值递增。</li><li>只有在表中没有任何数据，或者所有现有记录的自增值都小于您想要设置的新值之后，减少自增值的操作才会成功生效。在这种情况下，之后新增的记录将从您设置的新自增值开始递增。</li></ol></p>
</main>
...
1. 增大自增列的下一个取值。
   ...
   5. 向表 `tbl1` 中插入一条数据。
      ...
      ```sql
      obclient [test]> INSERT INTO tbl1(name) VALUES('A1');
      ```
   ...
   6. 再次查看表 `tbl1` 当前自增字段的取值。
      ...
      `id` int(11) NOT NULL AUTO_INCREMENT,
      `name` varchar(50) DEFAULT NULL
      ) AUTO_INCREMENT = 11 AUTO_INCREMENT_MODE = 'ORDER' DEFAULT CHARSET = utf8mb4 ROW_FORMAT = DYNAMIC COMPRESSION = 'zstd_1.3.8' REPLICA_NUM = 3 BLOCK_SIZE = 16384 USE_BLOOM_FILTER = FALSE TABLET_SIZE = 134217728 PCTFREE = 0 |
      +-------+-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
      1 row in set
      ```
...
2. 减小自增列的下一个取值。
   ...
   1. 将表 `tbl1` 当前自增列的下一个取值改为 5。

      ```sql
      obclient [test]> ALTER TABLE tbl1 AUTO_INCREMENT = 5;
      ```
   ...
   2. 再次查看表 `tbl1` 当前自增字段的取值。
      ...
      返回结果可以看到 `AUTO_INCREMENT` 的值还是 11，这是因为表中有数据并且自增字段的值大于减小自增值的语句设置的值，即使执行减小自增值的语句成功了，实际上自增值不会改变。
   ...
   4. 将表 `tbl1` 中字段 `id` 的值改为 1。

      ```sql
      obclient [test]> UPDATE tbl1 SET id = 1 WHERE id = 10;
      ```
   ...
   5. 再次执行下面 SQL 语句，将表 `tbl1` 当前自增列的下一个取值改为 5。
   ...
   6. 再次查看表 `tbl1` 当前自增字段的取值。
      ...
      `id` int(11) NOT NULL AUTO_INCREMENT,
      `name` varchar(50) DEFAULT NULL
      ) AUTO_INCREMENT = 5 AUTO_INCREMENT_MODE = 'ORDER' DEFAULT CHARSET = utf8mb4 ROW_FORMAT = DYNAMIC COMPRESSION = 'zstd_1.3.8' REPLICA_NUM = 3 BLOCK_SIZE = 16384 USE_BLOOM_FILTER = FALSE TABLET_SIZE = 134217728 PCTFREE = 0 |
      +-------+------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
      1 row in set
      ```
