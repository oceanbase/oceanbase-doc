...
外键是一种用于维护表之间关联关系完整性的约束，它指向其他表（父表）的主键列。

* 在创建表并添加外键约束时，需要确保当前用户具备 `CREATE` 权限以及对父表的 `REFERENCES` 权限。
* 在为现有表添加外键约束时，需要确保当前用户具备 `ALTER` 权限以及对父表的 `REFERENCES` 权限。

更多有关 OceanBase 数据库权限的详细介绍，请参见 [MySQL 模式下的权限分类](../../../../600.manage/500.security-and-permissions/300.access-control/200.user-and-permission/200.permission-of-mysql-mode/100.permission-classification-of-mysql.md)。
