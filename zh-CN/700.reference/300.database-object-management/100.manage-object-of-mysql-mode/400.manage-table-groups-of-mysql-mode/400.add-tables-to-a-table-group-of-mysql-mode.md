...
带有分区权重的表可以被添加到表组中，但添加到表组后，可能会影响表组的整体分布。带分区权重的表加入表组后对原表组的具体影响如下表所示。

| 表组的 SHARDING 属性   |  原来的分区分布方式    |  带分区权重后的影响     |
|-----------------------|-----------------------|------------------------------|
| NONE                  | 全部聚合               | 无影响 |
| PARTITION             | 各表的一级分区内 Value 相同的分区聚合（同一分区组）；一级分区之间打散。  | 少数带权重的分区会影响表组的整体分布。 |
| ADAPTIVE              | <ul><li>表组内全部是一级分区表时，各表的一级分区内 Value 相同的分区聚合（同一分区组）；一级分区之间打散。</li> <li>表组内全部是二级分区表时，各表的二级分区内 Value 相同的分区聚合（同一分区组）；二级分区之间打散。</li></ul>  | 表组内全部是一级分区表时，少数带权重的分区会影响表组的整体分布。由于目前暂不支持对二级分区表设置分区权重，故表组内全部是二级分区表的场景暂不涉及。 |

<main id="notice" type='explain'>
  <h4>说明</h4>
  <p>如果表组添加了表组权重，则表组中的表级、分区级权重无效。有关添加表组权重的介绍信息，参见 <a href="../../../600.pl-reference/200.pl-mysql/1000.pl-system-package-mysql/20800.dbms-balance-mysql/300.set-tablegroup-balance-weight-of-mysql-mode.md">SET_TABLEGROUP_BALANCE_WEIGHT</a>。</p>
</main>
...
* 对于 `SHARDING` 属性为 `SUBPARTITION`的表组，要求加入表组的表必须为二级分区表，不同表一级分区方式必须相同，二级分区方式必须相同，包括分区键类型、分区函数、分区数、分区边界值等。
