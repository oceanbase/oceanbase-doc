...
<main id="notice" type='notice'>
  <h4>注意</h4>
  <p>
  <ul><li>在 OceanBase 数据库 V4.2.1 BP8 及之前版本升级至 V4.2.1 BP9 版本时：
    <ul><li>定时分区均衡任务 <code>SCHEDULED_TRIGGER_PARTITION_BALANCE</code> 默认关闭（<code>enabled = 0</code>）。
    </li><li>租户级配置项 <code>partition_balance_schedule_interval</code> 的值保持不变，分区均衡任务还是依赖租户级配置项 <code>partition_balance_scheduler_interval</code> 控制，维持 V4.2.1 BP9 之前版本的行为。
  ...
  </li><li>在 OceanBase 数据库 V4.2.1 BP9 版本新建的租户：
