  ...
  * 租户级配置项 `PARTITION_BALANCE_SCHEDULE_INTERVAL`
  ...
  2. 开启租户内的负载均衡。
     ...
     * 系统租户开启所有用户租户的租户内负载均衡
       ...
       ```shell
       o bclient(root@sys)[(none)]>ALTER SYSTEM SET enable_rebalance = true TENANT = all_user;
       ```
  ...
  3. 设置生成分区负载均衡任务的时间间隔。
     ...
     * 系统租户
       ...
       ```shell
       obclient(root@sys)[(none)]> ALTER SYSTEM SET partition_balance_schedule_interval='2h' TENANT = 'mysql';
       ```

     * 用户租户

       ```shell
       obclient(root@mysql001)[(none)]> ALTER SYSTEM SET partition_balance_schedule_interval='2h';
       ```
