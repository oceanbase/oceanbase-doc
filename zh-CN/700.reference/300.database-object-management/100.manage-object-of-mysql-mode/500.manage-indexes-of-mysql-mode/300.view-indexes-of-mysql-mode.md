...
* 可以使用视图 [information_schema.STATISTICS](../../../700.system-views/400.system-view-of-mysql-mode/200.dictionary-view-of-mysql-mode/2600.information_schema-statistics-of-mysql-mode.md) 查看表索引信息，包含表名、是否是唯一索引、索引名、索引编号、列名等。

* 可以使用视图 [oceanbase.DBA_OB_TABLE_LOCATIONS](../../../700.system-views/400.system-view-of-mysql-mode/200.dictionary-view-of-mysql-mode/10900.o-dba_ob_table_locations-of-mysql-mode.md) 查看索引是全局索引（`GLOBAL`）还是局部索引（`LOCAL`）。
...
2. 查看索引 `tbl1_idx1` 是全局索引（`GLOBAL`）还是局部索引（`LOCAL`）。
...
3. 从视图 `information_schema.STATISTICS` 和 `oceanbase.DBA_OB_TABLE_LOCATIONS` 中获取表 `tbl1` 的索引信息。
