...
OceanBase 数据库支持在非分区表和分区表上创建索引，索引可以是局部索引或全局索引，也可以是唯一索引或非唯一索引，还可以是空间索引或函数索引。如果是分区表的唯一索引，则局部的唯一索引必须包含表分区的分区键。关于索引的更多介绍，请参见 [索引简介](../../../100.oceanbase-database-concepts/400.database-objects/200.database-objects-of-mysql-mode/300.index-of-mysql-mode/100.index-overview-of-mysql-mode.md) 章节。
...
更多全文索引信息，参见 [全文索引](../../../100.oceanbase-database-concepts/400.database-objects/200.database-objects-of-mysql-mode/300.index-of-mysql-mode/320.full-text-indexes-of-mysql-mode-in-concepts.md)。
...
更多通过 `ALTER TABLE` 语句为已有的表添加全文索引的语法信息，参见 [ALTER TABLE](../../../500.sql-reference/100.sql-syntax/200.common-tenant-of-mysql-mode/600.sql-statement-of-mysql-mode/1600.alter-table-of-mysql-mode.md)。
...
<h4>注意</h4>
<p><ul>
<li>`WITH (...)` 仅对 JSON 列生效。</li>
<li>`INCLUDE_PATHS` 与 `EXCLUDE_PATHS` 不能同时指定。</li>
</ul></p>
</main>

### 使用 CREATE SEARCH INDEX 语句创建搜索索引

通过 `CREATE SEARCH INDEX` 语句在已有表上创建搜索索引，语法如下：
...
更多使用 `CREATE SEARCH INDEX` 创建搜索索引的语法信息、支持的数据类型等，参见 [CREATE INDEX](../../../500.sql-reference/100.sql-syntax/200.common-tenant-of-mysql-mode/600.sql-statement-of-mysql-mode/2200.create-index-of-mysql-mode.md)。

**相关参数说明如下：**

* `index_name`：指定待创建的搜索索引名称。
* `table_name`：指定待创建索引的表名。
* `search_index_column`：搜索索引列定义，可指定一个或多个列；每列可通过 `WITH (...)` 指定列级路径与类型选项。
* `INCLUDE_PATHS / EXCLUDE_PATHS`：列级路径白名单/黑名单，二者二选一，用于控制该列参与索引构建的 JSON 路径。
* `INCLUDE_TYPES`：列级类型过滤，仅索引指定 JSON 标量类型（当前支持 `JSON_STRING`、`JSON_NUMBER`）。
* `LOCAL`：可选项，用于在分区表上创建局部搜索索引。当前分区表仅支持局部搜索索引。

**示例如下：**

创建表 `tbl_search_1` 后，再创建搜索索引 `search_idx_1`。
...
2. 在表 `tbl_search_1` 上创建搜索索引 `search_idx_1`，并为 JSON 列配置路径白名单与类型过滤。
...
### 使用 CREATE TABLE 语句创建搜索索引

通过 `CREATE TABLE` 语句在创建表时同时创建搜索索引，语法如下：
...
更多使用 `CREATE TABLE` 创建搜索索引的语法信息、支持的数据类型等，参见 [CREATE TABLE](../../../500.sql-reference/100.sql-syntax/200.common-tenant-of-mysql-mode/600.sql-statement-of-mysql-mode/2600.create-table-of-mysql-mode.md)。
...
创建表 `tbl_search_3` 时，同时创建搜索索引 `search_idx_3`。
...
### 使用 ALTER TABLE 语句创建搜索索引

通过 `ALTER TABLE` 语句为已有表添加搜索索引，语法如下：
...
更多通过 `ALTER TABLE` 语句为已有的表添加搜索索引的语法信息，参见 [ALTER TABLE](../../../500.sql-reference/100.sql-syntax/200.common-tenant-of-mysql-mode/600.sql-statement-of-mysql-mode/1600.alter-table-of-mysql-mode.md)。

**相关参数说明如下：**

* `table_name`：指定待添加搜索索引的表名。
* `index_name`：可选项，指定搜索索引名称；若未指定，系统将按规则生成默认索引名。
* `search_index_column`：搜索索引列定义，可指定一个或多个列；每列可通过 `WITH (...)` 指定列级路径与类型选项。
* `INCLUDE_PATHS / EXCLUDE_PATHS`：列级路径白名单/黑名单，二者二选一，用于控制该列参与索引构建的 JSON 路径。
* `INCLUDE_TYPES`：列级类型过滤，仅索引指定 JSON 标量类型（当前支持 `JSON_STRING`、`JSON_NUMBER`）。
...
创建表 `tbl_search_2` 后，再通过 `ALTER TABLE` 添加搜索索引 `search_idx_2`。
...
2. 为表 `tbl_search_2` 添加搜索索引 `search_idx_2`。
