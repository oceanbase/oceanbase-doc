...
```sql
CREATE TABLE table_name(column_name column_definition,[column_name column_definition,...] FULLTEXT [INDEX | KEY] [index_name](column_name) [WITH PARSER {SPACE | NGRAM}] [LOCAL];
```
...
* `WITH PARSER {SPACE \| NGRAM}`：可选，指定全文索引的分词器。当前版本仅支持 `SPACE`（空格）和 `NGRAM` 两种内置分词器。
