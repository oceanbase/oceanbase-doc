...
CREATE TABLE table_name(column_name column_definition,[column_name column_definition,...]
    ...
    [WITH PARSER {SPACE | NGRAM | BENG | IK}]
    [PARSER_PROPERTIES[=](key=value [,key=value])]
    [LOCAL]);
```
...
* `WITH PARSER {SPACE | NGRAM | BENG | IK}`：可选项，指定全文索引的分词器。取值如下：

  * `SPACE`：默认值，表示按空格进行分词。
  * `NGRAM`：表示基于 N-Gram（中文）的分词方式。
  * `BENG`：基于 Beng （基础英文）的分词方式。
  * `IK`：表示基于 IK（中文）的分词方式。当前仅支持 `utf-8` 字符集。
...
* `PARSER_PROPERTIES[=](key=value [,key=value])`：可选项，指定全文索引的分词器属性。`key` 取值如下：
