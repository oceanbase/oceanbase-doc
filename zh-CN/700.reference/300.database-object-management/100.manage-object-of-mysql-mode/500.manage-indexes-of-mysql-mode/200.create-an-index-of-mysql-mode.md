...
```sql
CREATE TABLE table_name(column_name column_definition,[column_name column_definition,...] [UNIQUE] INDEX|KEY [index_name](column_name) WITH COLUMN GROUP([all columns, ]each column));
```
...
创建表 `tbl4`，同时创建索引 `idx1_tbl4_cg` 的示例如下：

```sql
CREATE TABLE tbl4(id INT, name VARCHAR(18), date DATE, PRIMARY KEY (id), INDEX idx1_tbl4_cg (date) WITH COLUMN GROUP(each column));
```
...
创建 `tbl5` 表后，再创建列存索引 `idx1_tbl5_cg` 的示例如下：

1. 创建表 `tbl5`。

   ```sql
   CREATE TABLE tbl5(id INT, name VARCHAR(20));
   ```

2. 在表 `tbl5` 上创建列存索引 `idx1_tbl5_cg`。

   ```sql
   CREATE INDEX idx1_tbl5_cg ON tbl5(id) WITH COLUMN GROUP(each column);
   ```
...
```sql
ALTER TABLE table_name ADD [UNIQUE] INDEX|KEY [index_name](column_name) WITH COLUMN GROUP([all columns, ]each column);
```
...
创建表 `tbl6` 后，在创建列存索引 `idx1_tbl6_cg` 的示例如下：

1. 创建表 `tbl6`。

   ```sql
   CREATE TABLE tbl6(id INT, name VARCHAR(20));
   ```

2. 为表 `tbl6` 创建列存索引 `idx1_tbl6_cg`。

   ```sql
   ALTER TABLE tbl6 ADD INDEX idx1_tbl6_cg(id) WITH COLUMN GROUP(each column);
   ```
