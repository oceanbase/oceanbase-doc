...
* `table_name` ：指定待创建索引的表的表名。
* `column_name` ：指定对哪些列进行索引，指定多列时，各列之间用英文逗号分隔。
* `WITH PARSER {SPACE | NGRAM | BENG}`：可选项，指定全文索引的分词器。当前版本仅支持 `SPACE`（空格）、`NGRAM` 和 `BENG`（基础英文）三种内置分词器，不指定默认使用 `SPACE`（空格）分词器。可以使用 [TOKENIZE](../../../500.sql-reference/100.sql-syntax/200.common-tenant-of-mysql-mode/400.functions-of-mysql-mode/960.text-processing-functions-of-mysql-mode/200.tokenize-of-mysql-mode.md) 函数查看文本按照指定的分词器及 Json 形式参数的分词结果。
...
2. 在表 `tbl3` 上创建全文索引 `ft_idx1_tbl3`，指定全文索引的分词器为 `NGRAM`。
...
2. 为表 `tbl4` 添加全文索引 `ft_idx1_tbl4`，指定全文索引的分词器为 `BENG`。
