  ...
  * 租户级配置项 `PARTITION_BALANCE_SCHEDULE_INTERVAL`
  ...
  2. 开启租户内的负载均衡。
     ...
     * 系统租户开启指定租户的租户内负载均衡
       ...
       ```shell
       obclient(root@sys)[(none)]> ALTER SYSTEM SET enable_rebalance = true TENANT = 'oracle';
       ```
     ...
     * 用户租户开启本租户的租户内负载均衡
       ...
       Oracle 租户开启本租户的租户内负载均衡的语句如下：

       ```shell
       obclient(SYS@oracle001)[SYS]> ALTER SYSTEM SET enable_rebalance = 'true';
       ```
  ...
  3. 设置分区负载均衡任务生成的时间间隔。
     ...
     * 系统租户
      ...
      示例：
       ...
       ```shell
       obclient(root@sys)[(none)]> ALTER SYSTEM SET partition_balance_schedule_interval='2h' TENANT = 'oracle';
       ```

     * 用户租户

       ```shell
       obclient(SYS@oracle001)[SYS]> ALTER SYSTEM SET partition_balance_schedule_interval='2h';
       ```

* 为表组内的表设置分区权重或清除分区权重，可能会影响表组的整体分布。有关分区权重对表组的影响的详细说明及示例，参见 [租户内均衡](../../../100.oceanbase-database-concepts/500.distributed-database-objects/300.data-partitions-and-replicas/500.data-balancing/100.intra-tenant-balancing.md) 中 **分区权重对表组的影响**。
