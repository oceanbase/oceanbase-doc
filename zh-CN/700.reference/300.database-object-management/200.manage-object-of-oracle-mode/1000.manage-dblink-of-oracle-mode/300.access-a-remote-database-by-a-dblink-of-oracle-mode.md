...
需要注意，在某些特殊场景下，OceanBase 数据库为了提升远端数据库对象查询的性能，可能会导致远端序列的 `CURRVAL` 无法获取到最新的值。例如，以下查询是通过数据库链接 `my_link` 将远端数据库中表 `tbl2` 的 `c1` 列数据与序列 `seq` 的 `NEXTVAL` 值插入到本地数据库的表 `tbl` 中：
...
下面以几个简单的示例进行说明。假设本地 OceanBase 数据库中创建了到远端 Oracle 数据库的 DBLink `orcl_dblink`。

  ```sql
  CREATE DATABASE LINK orcl_dblink CONNECT TO orcl_user@oracle IDENTIFIED BY ****** OCI HOST 'xx.xx.xx.xx:1521/ORCL';
  ```

* 调用一个简单的独立存储过程

  远端 Oracel 数据库中存在着一张客户信息表:
  ...
  还有一个存储过程，该存储过程用于接受一个 `customer_id` 参数，并返回该客户的 ID。
  ...
  您可以在本地数据库中，执行 `SET SERVEROUTPUT ON;` 设置服务器输出后，再通过以下语句调用存储过程。

  ```sql
  CALL get_customer_id@orcl_dblink(1)；
  ```
  ...
  ```sql
  SELECT get_customer_id@orcl_dblink(1) FROM DUAL;
  ```
  ...
  * 调用远端 Oracle 数据库上的同义词
    ...
    ```sql
    CALL syn_remote_customer_id@orcl_dblink(1)；
    ```
...
* 调用一个包类型为复合数据类型的出入参的包存储过程
  ...
  如下所示，在远端 Oracle 数据库中，存在一个程序包 `pkg1`，该包包含了多种复合数据类型，包括记录和嵌套表，同时还包含一个名为 `pro1` 的存储过程。
  ...
  ```sql
  DECLARE
    param1 pkg1.ty1@orcl_dblink;
    param2 pkg1.ty2@orcl_dblink;
    param3 pkg1.ty3@orcl_dblink;
    param4 pkg1.ty4@orcl_dblink;
    param5 pkg1.ty5@orcl_dblink;
  BEGIN
    pkg1.pro1@orcl_dblink(param1, param2, param3, param4, param5);
  END;
  /
  ```
