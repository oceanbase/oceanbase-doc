...
1. 在本地数据库中，创建到远端 OceanBase 数据库的 DBLink `my_link` 和一个数据库表 `tbl1`。
...
* 调用一个简单的独立存储过程
  ...
  同时在远端数据库中还创建了一个存储过程，该存储过程用于接受一个 `customer_id` 参数，并返回该客户的 ID。
  ...
  接着，在本地数据库中，执行 `SET SERVEROUTPUT ON;` 设置服务器输出后，再通过以下语句调用存储过程。
