...
* 调用一个简单的独立存储过程
  ...
  2. 在本地数据库中调用存储过程。
     ...
     ```sql
     obclient> CALL get_customer_id@orcl_dblink(2);
     ```
...
* 调用一个包含出入参的存储过程
  ...
  创建了一个存储过程，该存储过程用于接受一个 `p_employee_id` 作为输入参数，然后返回该员工的姓名和ID。如果找不到对应的员工，则返回 `Employee not found` 的信息，如果发生其他异常，则返回 `Error occurred`。
