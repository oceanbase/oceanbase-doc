...
* 请确认已创建 DBLink，创建 DBLink 的详细操作，参见 [创建 DBLink](../1000.manage-dblink-of-oracle-mode/100.create-a-dblink-of-oracle-mode.md)。

* 使用 DBLink 功能远端 Oracle 数据库时，如果远端数据库版本为 Oracle-19C 及以上版本，则需要在本地 OceanBase 数据库中安装并配置 OCI-12.2；如果远端数据库版本为 Oracle-12C 及以下版本，则需要在本地 OceanBase 数据库中安装并配置 OCI-11.2。
...
* 调用一个包类型为复合数据类型的出入参的包存储过程
  ...
  远程 Oracle 数据库中有一个包存储过程 `pkg1`:

  create table link_result_tbl(v varchar2(1000));
  ...
  SQL> CREATE OR REPLACE PACKAGE BODY pkg1 IS
      ...
      v varchar2(1024);
      BEGIN
        DELETE FROM link_result_tbl;
        for idx in param1.first .. param1.last LOOP
          v := 'param1(' || idx || ')=' || param1(idx) || ';';
          insert into link_result_tbl values(v);
        END LOOP;
      END;
  ...
  您可以在本地 OceanBase 数据库中，调用包存储过程：

  ```sql
  DECLARE
      y pkgl.ty2@orcl_dblink;
    BEGIN
      SELECT id bulk collect INTO y FROM link_table;
      pkgl.pro1@orcl_dblink(y);
    END;
    /
  ```
  ...
  此外，对于远端数据库为 oracle-12c 及以下版本的场景，该示例中由于 `ty6` 没有出现在包 `pkg1` 内部 Routine 的参数列表位置，故通过以下语句调用包存储过程就会报错：
