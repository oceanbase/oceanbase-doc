...
|slug|update-data-in-remote-database-by-dblinks-oracle-mode|
...
* 在 SQL 中通过 DBLink 向远端数据库写入数据的场景，需要显示使用 `COMMIT` 语句提交本地和远端的 XA 事务。暂不支持通过设置 `autocommit = True` 来开启事务的自动提交。在 SQL 中通过 DBLink 写入数据时，一旦开启 XA 事务，系统就会将本 Session 的 `autocommit` 值强制置为 `False`，直到 XA 事务提交或者回滚后，才会恢复 `autocommit` 原本的值。

* 在 匿名块 中通过 DBLink 向远端数据库写入数据的场景，事务提交方式和普通事务类似。支持通过设置 `autocommit = True` 来开启事务的自动提交。设置 `autocommit = False` 时，需要使用 `COMMIT` 语句显示提交。
