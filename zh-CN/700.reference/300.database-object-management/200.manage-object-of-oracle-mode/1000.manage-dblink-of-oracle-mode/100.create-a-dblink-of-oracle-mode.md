...
* 强烈建议 `MY_NAME` 后的 `local_ip:local_port` 设置为本地集群的代理（proxy）节点，proxy 节点会自动路由反向 Link 到正确的节点上获取本地数据。

  如果创建 DBLink 时，`MY_NAME` 后的 `local_ip:local_port` 不是本地集群的 proxy 节点，而是本地集群的某个 OBServer 节点（例如节点 A），则在使用该 DBLink 在执行把 OceanBase 数据库 Oracle 模式本地表的数据写入（相关 `INSERT`、`UPDATE`、`DELETE` 和 `MERGE INTO` 的 SQL 语句）到 OceanBase 数据库 Oracle 模式远端表时，将出现以下情况：
  
  * 如果这条 SQL 是在 A 节点上被执行的，那么 DBLink 写的结果是符合预期的。
  * 如果这条 SQL 是在本地集群的其他节点（例如节点 B）上执行的，那么 DBLink 写的节点将不符合预期。这是因为 `MY_NAME` 后的 `local_ip:local_port` 是节点 A，反向 Link 一定会到 OBServer 节点 A 上去读 A 节点的分布式 XA 事务分支数据。但是 A 节点没有执行相应的 DBLink 写的 SQL 语句，故 A 节点不存在相应的分布式 XA 事务分支的，相应的分布式 XA 事务分支在节点 B 上才有。

* 建议 `CONNECT TO` 后的 `ip:port` 也使用远端集群的 proxy 节点，如果使用的是远端集群的某个 OBServer 节点，OBServer 节点一旦发生故障，该 DBLink 会无法连接到远端集群，即无法保证高可用。
