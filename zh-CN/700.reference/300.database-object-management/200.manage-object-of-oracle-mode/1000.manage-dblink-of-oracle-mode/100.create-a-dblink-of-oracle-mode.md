...
创建 DBLink 需要指定 DBLink 名称并且提供远端 Oracle 数据库的用户名、密码、IP 地址、端口号、Oracle 服务名以及访问类型等信息。DBLink 写数据功能中有一个反向 Link 特性，反向 Link 特性主要用于远端数据库访问本地数据库的对象，对象包括表、视图和同义词等。OceanBase 数据库到 Oracle 数据库的 DBLink 暂不支持反向 Link 特性。
