...
动态分区是 OceanBase 数据库提供的一种自动管理分区的功能，允许表根据预设规则自动创建和删除分区，从而简化运维并适应数据增长或过期的需求。
...
* V4.6.0 之前版本执行 `DROP` 分区操作将导致全局索引失效。对于存在全局索引的表，谨慎使用删除过期分区功能。
* V4.6.0 版本新增组户级配置项 `_append_update_global_indexes_for_dynamic_partition`，用于控制动态分区在清理过期分区时是否自动维护全局索引，取值如下：

  * `AUTO`：新建租户默认值，跟随 `_ob_enable_truncate_partition_preserve_global_index` 取值。适合在希望避免重建全局索引引入额外开销时使用。

    * `_ob_enable_truncate_partition_preserve_global_index` 为 `true` 时，追加 `UPDATE GLOBAL INDEXES`，保留全局索引。
    * `_ob_enable_truncate_partition_preserve_global_index` 为 `false` 时，不追加 `UPDATE GLOBAL INDEXES`，使全局索引失效。

  * `ON`：始终追加 `UPDATE GLOBAL INDEXES`，删除过期分区时根据 `_ob_enable_truncate_partition_preserve_global_index` 的取值选择保留全局索引或重建全局索引。适合在希望避免全局索引失效时使用。
  * `OFF`：升级租户默认值，与低版本行为保持一致，始终不追加 `UPDATE GLOBAL INDEXES`。删除过期分区时会直接使全局索引失效。适合在希望避免重建全局索引或保留全局索引引入额外开销时使用。

      如果是升级到 V4.6.0 版本的场景，要 `DROP` 分区操作后全局索引不失效，需要修改 `_ob_enable_truncate_partition_preserve_global_index` 为 `true` 和 `_append_update_global_indexes_for_dynamic_partition` 为 `AUTO` 或 `ON`，示例如下：
