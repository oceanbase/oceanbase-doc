...
* V4.6.0 之前版本执行 `DROP` 分区操作将导致全局索引失效。对于存在全局索引的表，谨慎使用删除过期分区功能。
  ...
  * V4.6.0 版本新增组户级配置项 `_append_update_global_indexes_for_dynamic_partition`，用于控制动态分区在清理过期分区时是否自动维护全局索引。新建租户该配置项默认值为 `AUTO`，表示动态分区在清理过期分区时自动维护全局索引，即 `DROP` 分区操作后全局索引不失效。
  * 如果是升级到 V4.6.0 版本的场景，`_append_update_global_indexes_for_dynamic_partition` 默认值为 `OFF`，表示与低版本行为保持一致，即执行 `DROP` 分区操作将导致全局索引失效。如果要 `DROP` 分区操作后全局索引不失效，需要修改 `_append_update_global_indexes_for_dynamic_partition` 为 `AUTO`，示例如下：
