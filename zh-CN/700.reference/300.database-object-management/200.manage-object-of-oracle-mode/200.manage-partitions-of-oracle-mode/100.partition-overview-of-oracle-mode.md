...
Interval 分区是 Range 分区的扩展，当新插入的数据超过现有已存在分区的范围时，允许新分区被自动创建。

Interval 分区使用限制如下：

* Interval 分区只能用于表的一级分区，不能作为二级分区；在二级分区表中，支持一级分区为 Interval 分区，二级分区为其他分区类型。
* Interval 分区键类型，仅支持 `NUMBER`、`DATE`、`FLOAT`、`TIMESTAMP`。
...
* 通过 `UPDATE` 自动新增分区时，要设置 `ALTER TABLE table_name ENABLE ROW MOVEMENT;`（允许在不同分区间移动以进行分区键更新）。
* 自动增加的分区，一旦创建成功，不支持随事务回滚。
* 有 Interval 分区的表不支持分区分裂和分区交换。
* 在通过 `INSERT` 语句自动增加分区的同时，执行另外的 DDL 语句，有可能检测到锁冲突而报错。
* Interval 分区表最大分区数为 1048575。
* 暂时不支持改变建表时定义的 Interval 间隔。
...
随机分区（Random Partition） 是 OceanBase 数据库提供的一种新型分区策略，适用于无需按特定列值进行数据路由的场景。该策略通过系统自动将数据随机分散到多个分区中，实现数据的均匀分布和负载均衡。相比传统基于业务字段的分区方式，用户无需指定分区键或手动管理分区结构，降低了使用复杂度。
