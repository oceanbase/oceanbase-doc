...
6. 执行二级分区表 `tbl1_rh` 的分区 `p1` 与一级分区表 `tbl1_h` 进行分区交换。
...
6. 执行分区表 `tbl1_rr` 的分区 `sp1` 与非分区表 `tbl2` 进行分区交换。
