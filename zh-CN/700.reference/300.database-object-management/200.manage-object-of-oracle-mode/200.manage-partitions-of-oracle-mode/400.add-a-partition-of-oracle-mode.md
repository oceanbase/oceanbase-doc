...
<main id="notice" type='notice'>
  <h4>注意</h4>
  <p><ul><li>不支持二级分区类型是 Hash 类型的场景新增二级分区。</li><li>添加的二级分区不能与表内现有的一级或二级分区重名。</li><li>添加 Range 二级分区时，Range 上界要大于当前一级分区下最大分区的上界；添加多个 Range 二级分区时，上界要递增排列，即上界大的 Range 分区写在后面。</li><li>添加 List 二级分区时，不能与当前一级分区下现有的二级分区值重复。</li><li>添加多个二级分区的操作是原子的，要么都成功，要么都失败。</li></ul></p>
</main>
...
* 向模版化二级分区表添加二级分区。
  ...
  1. 创建 Range + Range 分区的模板化二级分区表 `tbl_m2_rr`，再向其中添加一级分区 `p2`。
...
* 向非模版化二级分区表添加二级分区。
  ...
  1. 创建 Range + Range 分区的非模版化二级分区表 `tbl_f2_rr`。
