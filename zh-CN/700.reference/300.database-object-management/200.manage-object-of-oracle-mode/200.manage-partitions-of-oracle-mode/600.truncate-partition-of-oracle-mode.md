...
|slug|truncate-partition-oracle-mode|
...
对于使用 Range、Interval 或 List 分区（包括一级分区和二级分区）且支持 `TRUNCATE` 分区操作的表，当指定 `UPDATE GLOBAL INDEXES` 时，系统不会重建全局索引，而是通过后台合并删除数据的方式，以 lazy（延迟）维护策略保持索引有效，从而避免因索引失效导致的查询中断或性能下降。需要注意的是，该优化仅适用于普通全局索引。以下场景无法触发该优化：
...
<main id="notice" type='explain'>
  <h4>说明</h4>
  <p>对于 V4.4.2 版本，从 V4.4.2 BP1 版本开始 <code>INTERVAL</code> 分区表支持 <code>UPDATE GLOBAL INDEXES</code> 语法。</p>
</main>
