...
* Oracle 模式下，对于有全局索引的一级或二级分区表，Truncate 分区时，需要通过在 `ALTER TABLE` 语句中添加 `UPDATE GLOBAL INDEXES` 关键字的方式来更新全局索引信息；如果未添加 `UPDATE GLOBAL INDEXES` 关键字，则 Truncate 分区后，该分区表上的全局索引会处于不可用状态。
  ...
  * 当指定 `UPDATE GLOBAL INDEXES` 时，系统不会重建全局索引，而是通过后台合并删除数据的方式，以 lazy（延迟）维护策略保持索引有效，从而避免因索引失效导致的查询中断或性能下降。需要注意的是，该优化仅适用于普通全局索引；对于全文索引、非结构化类型等特殊全局索引，即使使用 `UPDATE GLOBAL INDEXES`，索引仍会处于不可用状态。
  * Oracle 模式下，对于使用 Range 或 List 分区（包括一级分区和二级分区）且支持 `TRUNCATE` 操作的表，指定 `UPDATE GLOBAL INDEXES` 可确保全局索引在 Truncate 分区后保持有效。
