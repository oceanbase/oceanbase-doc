...
2. 创建自动分区的非分区表 `test4_tbl1`（分裂阈值为 1024MB，使用用户配置）。
...
4. 查看表的自动分区属性。
    ...
    | OWNER | TABLE_NAME | AUTO_SPLIT | AUTO_SPLIT_TABLET_SIZE |
    +-------+------------+------------+------------------------+
    | SYS   | TEST4_TBL1 | TRUE       | 1073741824             |
    | SYS   | TEST4_TBL2 | FALSE      | 0                      |
    +-------+------------+------------+------------------------+
    2 rows in set
    ```

5. 在表 `test4_tbl1` 上创建全局索引 `idx1_test4_tbl1`。

    ```sql
    CREATE INDEX idx1_test4_tbl1 ON test4_tbl1 (col2) GLOBAL;
    ```

6. 在表 `test4_tbl2` 上创建全局索引 `idx1_test4_tbl2`。
