...
OceanBase 数据库的行存表和列存表都支持自动分区分裂。当前版本仅支持对 Range 分区的一级分区有主键表支持自动分区能力。

<main id="notice" type='explain'>
  <h4>说明</h4>
  <p>OceanBase 数据库从 V4.6.0 开始支持列存表的自动分区分裂。</p>
</main>
...
| SIZE('size_value')   | 可选项，指定分区的分裂阈值，取值范围 [1MB, +∞)。`SIZE()` 子句缺省时，默认读租户级别配置项 `auto_split_tablet_size` 的值。`size_value` 可取值 `unlimited`，表示分裂阈值无限制，不会分裂，后续不再调度新的自动分区分裂。<main id="notice" type='notice'><h4>注意</h4><p>表数据量大的场景不建议将 <code>SIZE</code> 设置过小，避免产生大量的分区，影响后续分区操作及系统稳定性。</p></main>|
| range_partition_list | 可选项，预先创建分区位点（分区规则）。<main id="notice" type='notice'><h4>注意</h4><p>当预先创建分区位点时，必须指定预分区键，否则报错。</p></main>|
| table_column_group_option | 可选项，指定表的存储格式选项。具体解释如下：<ul><li>`WITH COLUMN GROUP(all columns)`：指定添加行存表。</li><li>`WITH COLUMN GROUP(each column)`：指定创建列存表。</li><li>`WITH COLUMN GROUP(all columns, each column)`：指定创建行存列存冗余的表。</li></ul>|

### 修改表自动分区属性的语法

<main id="notice" type='notice'>
  <h4>注意</h4>
  <p>OceanBase 数据库从 V4.6.0 开始支持列存表的自动分区分裂，在 V4.6.0 之前版本创建的列存表升级到 V4.6.0 或之后版本，可以使用 <code>ALTER TABLE</code> 语句更改旧表启用自动分区属性。</p>
</main>
...
| SIZE('size_value')   | 可选项，修改表的自动分区阈值，取值范围 [1MB, +∞)。当 `size_value` 修改为 `unlimited` 时，不影响已有分区，后续不再调度新的自动分区分裂。<main id="notice" type='notice'><h4>注意</h4><p>表数据量大的场景不建议将 <code>SIZE</code> 设置过小，避免产生大量的分区，影响后续分区操作及系统稳定性。</p></main>|
...
* 创建自动分区列存表 `test1_cg1`，不指定预分区键，指定分裂阈值为 10GB。
...
* 创建自动分区列存行存列存冗余表 `test1_cg2`，不指定预分区键，指定分裂阈值为 10GB。
