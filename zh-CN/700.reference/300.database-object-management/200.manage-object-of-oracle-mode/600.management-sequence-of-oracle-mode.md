...
为防止其他的 Sequence 缓存过期问题，OceanBase 数据库从 V4.4.1 版本开始引入 `ALTER SYSTEM FLUSH SEQUENCE CACHE` 语句，用于主动清除 Sequence 缓存，以便发生问题后能快速恢复。语法格式如下：
...
更多有关清除 Sequence 缓存语句的信息，参见 [ALTER SYSTEM FLUSH SEQUENCE CACHE](../../500.sql-reference/100.sql-syntax/300.common-tenant-of-oracle-mode/900.sql-statement-of-oracle-mode/100.ddl-of-oracle-mode/850.alter-system-flush-sequence-cache-of-oracle-mode.md)。

**示例如下：**

在当前租户及 DataBase 下，清除本节点上名为 `test_seq1` 的 Sequence 的缓存。
