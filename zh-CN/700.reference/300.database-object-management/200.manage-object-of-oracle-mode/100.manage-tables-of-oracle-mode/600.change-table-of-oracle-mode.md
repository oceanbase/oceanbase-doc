...
* `merge_engine`：指定表的更新模型，取值如下：
  ...
  * `append_only`：仅追加写模式，该模式下仅写入数据，不允许修改数据，适用于 IoT、监控、金融交易等场景。
  ...
  有关表模式的更多信息，请参见 [创建表](200.create-a-table-for-oracle-tenant-of-oracle-mode.md) 和配置项 [default_table_merge_engine](../../../800.configuration-items-and-system-variables/100.system-configuration-items/400.tenant-level-configuration-items/2130.default_table_merge_engine.md)。
