...
OceanBase 数据库默认建表时，构建的是行存表，通过设置 `WITH COLUMN GROUP` 选项可以显示指定为列存或者行列混存状态。表创建成功后，您可以使用 `ALTER TABLE` 语句对表进行行列存转换。
