...
<main id="notice" type='explain'>
  <h4>说明</h4>
  <p>OceanBase 数据库从 V4.6.0 版本开始：<ul><li>Parquet/ORC 文件格式外表支持读取复杂数据类型：<code>Array</code>、<code>Map</code>、<code>JSON</code>。外表读取需与 OceanBase 数据库支持的数据类型需要保持一致，详细介绍请参见 <a href="../../../../500.sql-reference/100.sql-syntax/300.common-tenant-of-oracle-mode/300.basic-elements-of-oracle-mode/100.built-in-data-types-of-oracle-mode/100.overview-of-built-in-data-types-of-oracle-mode.md">内建数据类型概述</a>。</li><li>OceanBase 数据库 Parquet/ORC 文件格式外表支持读取 Bloom Filter，根据查询的谓词来过滤数据，减少不必要的 IO，用于查询加速。支持读取外表的 Bloom Filter 功能默认是关闭，如果要使用该功能，需要修改租户级配置项 <code>_enable_external_table_bloom_filter</code> 为 <code>true</code>（<code>ALTER SYSTEM SET "_enable_external_table_bloom_filter" = true;</code>）。</li></ul></p>
</main>
