...
OceanBase 数据库支持创建列存表、行列存转换以及创建列存索引的功能。OceanBase 数据库默认建表时，构建的是行存表，通过设置 `WITH COLUMN GROUP` 选项可以显示指定为列存或者行列混存状态。

有关行列存转换的信息请参见 [更改表](600.change-table-of-oracle-mode.md)。有关创建列存索引的信息，请参见 [创建索引](../400.manage-indexes-of-oracle-mode/200.create-an-index-of-oracle-mode.md)。

通过指定 `WITH COLUMN GROUP(all columns, each column)` 创建行列混存表。
