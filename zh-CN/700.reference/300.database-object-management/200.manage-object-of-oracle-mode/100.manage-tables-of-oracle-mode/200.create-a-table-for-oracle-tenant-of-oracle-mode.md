...
复制表创建成功后，可以与普通表一样进行插入和读写操作。复制表创建成功后，可以与普通表一样进行插入和读写操作。不同的是，如果使用 Proxy 连接数据库，则读请求会路由到任意一个 OBServer 节点执行，而普通表只能将读请求路由到 Leader 节点执行。如果是通过直连方式连接数据库，则只要本地副本可读，系统就会在直连的 OBServer 节点上执行读请求。更多关于 Proxy 路由规则的介绍，请参见 [连强读分区表路由](https://www.oceanbase.com/docs/common-odp-doc-cn-1000000000517777)。
