...
复制表是 OceanBase 数据库中一种支持数据最新修改读取的特殊表结构。复制表适合于对写入频率要求不高，但需要低读延时和高读负载均衡的场景。

在创建复制表之后，该表的副本会分布在租户下的所有 OBServer 节点上。这些副本中，有一个被选为 Leader，负责处理所有写操作，而其他副本则处理读操作。

各副本周期性地向 Leader 报告自己的数据同步状态，即数据回放进度。通常情况下，Follower 副本的数据回放会稍微滞后于 Leader。如果这种滞后在可接受范围内，即未超过预定的同步阈值，则可以认为 Follower 副本保持了数据的最新性，能够及时地呈现 Leader 上的数据变更。

Leader 基于 Follower 的同步状态，会给予一个时间限定的 Lease，相当于在 Lease 期限内，Leader 对该 Follower 副本有信心它能够继续提供准确的读服务。在 Lease 有效期间，Leader 会在每次复制表事务提交之前，验证 Follower 的数据回放是否到位。仅当确认 Follower 已经同步了事务的修改内容后，才会向用户报告事务的成功提交。于是，用户可以在任意 Follower 副本上获取到最新提交的数据。
