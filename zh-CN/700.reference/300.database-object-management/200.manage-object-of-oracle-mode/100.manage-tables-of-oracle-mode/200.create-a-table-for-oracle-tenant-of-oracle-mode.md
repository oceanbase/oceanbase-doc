...
OceanBase 数据库支持创建行存表、行存列存转换的功能。通过设置 `WITH COLUMN GROUP(all columns)` 选项创建行存表。你也可以通过设置配置项 `default_table_store_format` 设置默认存储格式。
...
OceanBase 数据库支持创建列存表、行存列存转换以及创建列存索引的功能。通过设置 `WITH COLUMN GROUP` 选项可以显示指定为列存或者行存列存冗余状态。你也可以通过设置配置项 `default_table_store_format` 设置默认存储格式。
