...
<main id="notice" type='explain'>
<h4>说明</h4>
<p><ul><li>复制表属性不支持修改，复制表不支持分区迁移操作（Transfer）。</li></ul><ul><li>复制表不支持Leader均衡设置，即Primary Zone设置为RANDOM不生效。</li></ul><ul><li>同一租户下复制表的日志都将记录在同一个日志流上。</li></ul><ul><li>对于包含大型对象（LOB）数据类型的表，不建议使用复制表功能。</li></ul><ul><li>在进行嵌套循环连接（Nested Loop Join, NLJ）时，若您需要组合使用复制表和非复制表，为避免潜在的随机路由问题，建议将非复制表安排在查询计划的前置位置。</li></ul></p>
</main>
