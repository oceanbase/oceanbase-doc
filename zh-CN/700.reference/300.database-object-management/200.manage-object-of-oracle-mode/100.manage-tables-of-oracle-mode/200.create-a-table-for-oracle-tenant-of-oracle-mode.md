...
1. **合并以优化读性能：**：在批量导入数据之后，建议进行一次合并操作。这有助于改善读取性能，因为合并操作会整理片段化数据，使其在物理存储上更连续，从而减少读取时的磁盘I/O。

2. **如何执行合并**：

   - 导入数据后，在租户内触发一次合并操作，以确保所有数据都被合并到基线层。可以使用以下命令：

     ```sql
     ALTER SYSTEM MAJOR FREEZE;
     ```
   - 接下来，在系统租户中执行查询，以监控合并操作的状态：

     ```sql
     SELECT STATUS FROM CDB_0B_MAJOR_COMPACTION WHERE TENANT_ID = '租户ID';
     ```

     当`STATUS`字段的值变为`IDLE`时，表示合并操作已经完成。

**统计信息收集**

1. **收集统计信息以优化查询性能**：合并操作完成之后，推荐进行统计信息的收集。这对于优化器生成有效的查询计划和执行策略是非常重要的。

2. **如何收集统计信息**：

   - 在业务租户中，可以一键对所有表收集统计信息。建议启动多个线程来并发执行此操作，以加快过程：
   ...
   - 收集进度可以通过以下视图进行监控：
     ...
     ```sql
     SELECT * FROM GV$0B_OPT_STAT_GATHER_MONITOR;
     ```
...
* [ALTER SYSTEM MAJOR FREEZE](../../../500.sql-reference/100.sql-syntax/300.common-tenant-of-oracle-mode/900.sql-statement-of-oracle-mode/100.ddl-of-oracle-mode/4600.alter-system-major-freeze-of-oracle-mode.md)
