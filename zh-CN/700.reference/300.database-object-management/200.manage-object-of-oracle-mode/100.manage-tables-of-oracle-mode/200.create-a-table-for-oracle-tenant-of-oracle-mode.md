...
在创建复制表之后，该表的副本会分布在租户下的所有 OBServer 节点上。这些副本中，有一个被选为 Leader，可以接受读写请求，而其他副本只能接受读请求。
...
* 复制表：
  ...
  * 事务内，有复制表做过变更，后续的查询操作：

    * OceanBase 数据库 V4.2.5 BP1 之前版本：查询复制表时，生成的执行计划会选择本地副本，但是因为做过变更，会报副本不可读的错误，SQL 重试选择 Leader 副本，此时 plan cache 无法命中，导致查询性能差。
    * OceanBase 数据库 V4.2.5 BP1 及之后版本：调整复制表执行计划，选择 Leader 副本，而不是本地副本，本事务内后续的复制表查询都退化为普通表查询。

      * 潜在问题：事务中的复制表都路由到 Leader 节点，由于复制表都建在一个日志流，可能出现热点及负载不均衡问题。

      <main id="notice" type='explain'>
        <h4>说明</h4>
        <p>对于 OceanBase 数据库 V4.2.5 版本，从 V4.2.5 BP1 版本开始在事务内，如果对复制表进行了变更，系统在查询时始终优先选择复制表的 Leader 副本，而非直接选择本地副本，从而确保 OceanBase 数据库提高查询效率，避免错误选择副本。</p>
      </main>
...
<main id="notice" type='explain'>
  <h4>说明</h4>
  <p>在事务内，如果对复制表进行了变更，系统在查询时始终优先选择复制表的 Leader 副本，而非直接选择本地副本，从而确保 OceanBase 数据库提高查询效率，避免错误选择副本。</p>
</main>
...
复制表创建成功后，可以与普通表一样进行插入和读写操作。不同的是，如果使用 Proxy 连接数据库，则读请求会路由到任意一个 OBServer 节点执行，而普通表只能将读请求路由到 Leader 节点执行。如果是通过直连方式连接数据库，则只要本地副本可读，系统就会在直连的 OBServer 节点上执行读请求。更多关于 Proxy 路由规则的介绍，请参见 [连强读分区表路由](https://www.oceanbase.com/docs/common-odp-doc-cn-1000000000517777)。
