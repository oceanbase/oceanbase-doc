...
Skip Index 是列的一种属性。显式设置 Skip Index 属性的操作主要针对行存格式列生效，对列式存储格式暂时无效。此外，当通过 `DESC table_name` 或 `SHOW CREATE TABLE table_name` 命令查看表的列属性时，对于列存表，Skip Index 属性不会展示出来，只有用户显式设置的属性才会展示出来。
