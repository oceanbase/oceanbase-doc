...
因为索引监控在集群启动时采用默认的采样（`SAMPLED`）模式，这种模式下可能会丢失一些数据，视图 `DBA_INDEX_USAGE` 可能出现查询结果为空的情况。如果需要准确的信息，可以在当前租户下将监控索引信息收集模式设置为 `ALL`。
...
6. 查看视图 `DBA_INDEX_USAGE`。
    ...
    <main id="notice" type='explain'>
      <h4>说明</h4>
      <p>可能需要等待几分钟才能在视图中查询到索引的监控结果。如果等待时间过长仍然没有结果，可以尝试执行以下操作来解决问题。</p>
    </main>

   1. （可选）使用以下 SQL 语句来设置监控索引信息收集模式为 `ALL`。
   ...
   3. （可选）再次查看视图 `DBA_INDEX_USAGE`。

       <main id="notice" type='explain'>
         <h4>说明</h4>
         <p>因为索引监控需要进行一段时间的收集和分析工作，所以在使用索引后，可能需要等待几分钟，才能在视图中查询到索引的监控结果。</p>
       </main>
