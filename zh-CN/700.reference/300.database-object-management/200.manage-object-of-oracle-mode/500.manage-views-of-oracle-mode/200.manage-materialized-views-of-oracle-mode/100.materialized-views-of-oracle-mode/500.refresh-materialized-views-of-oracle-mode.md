...
当在连接聚合场景物化视图使用增连更新/实时物化视图时，物化视图除了上述条件外，还需要具备如下条件：

1. 视图定义中存在 `GROUP BY` 子句，即仅在 SPJ 查询中添加 `GROUP BY` 子句。
2. `SELECT` 子句中包含所有 `GROUP BY` 列、`COUNT(*)`。
3. 聚合函数不包含 `DISTINCT`，支持的聚合函数类型及其它要求如下表：
