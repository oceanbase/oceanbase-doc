...
嵌套物化视图就是在已有的物化视图上构建的物化视图，即嵌套物化视图就是基于物化视图构建的新物化视图。例如下图，物化视图 `mv1` 基于表 `tbl1` 和表 `tbl2` 构建，是典型的物化视图。物化视图 `mv2` 则基于物化视图 `mv1` 和表 `tbl3` 构建，属于是嵌套物化视图。同理，物化视图 `mv3` 基于物化视图 `mv1` 和物化视图 `mv2` 构建，也是嵌套物化视图。
