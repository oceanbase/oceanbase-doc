...
<main id="notice" type='notice'>
  <h4>注意</h4>
  <p>如果一个物化视图（嵌套物化视图）是实时物化视图，那么它的查询结果是通过模拟 mlog 的方式去查询下层物化视图和自身结果进行合并。因此，需要先对下层物化视图进行增量刷新来更新 mlog，以此保证实时物化视图的数据是最新的。</p>
</main>
...
创建嵌套物化视图的语法跟创建普通物化视图的语法相同。嵌套物化视图就是在已有的物化视图上构建的物化视图，即嵌套物化视图就是基于物化视图构建的新物化视图。例如下图，物化视图 `mv1` 基于表 `tbl1` 和表 `tbl2` 构建，是典型的物化视图。物化视图 `mv2` 则基于物化视图 `mv1` 和表 `tbl3` 构建，属于是嵌套物化视图。同理，物化视图 `mv3` 基于物化视图 `mv1` 和物化视图 `mv2` 构建，也是嵌套物化视图。
...
* 如果一个物化视图（嵌套物化视图）是实时物化视图，那么它的查询结果是通过模拟 mlog 的方式去查询下层物化视图和自身结果进行合并。因此，需要先对下层物化视图进行增量刷新来更新 mlog，以此保证实时物化视图的数据是最新的。
...
1. 创建表 `tbl5`。
    ...
    ```sql
    CREATE TABLE tbl5(id INT, name VARCHAR2(30), PRIMARY KEY(id));
    ```

2. 创建表 `tbl6`。

    ```sql
    CREATE TABLE tbl6(id INT, age INT, PRIMARY KEY(id));
    ```

3. 基于 `tbl5` 和 `tbl6` 创建物化视图 `mv1_tbl5_tbl6`。

    ```sql
    CREATE MATERIALIZED VIEW mv1_tbl5_tbl6 (PRIMARY KEY (id1, id2))
        REFRESH COMPLETE
        AS SELECT tbl5.id id1, tbl6.id id2, tbl5.NAME, tbl6.AGE
            FROM tbl5, tbl6
            WHERE tbl5.id = tbl6.id;
    ```

4. 基于物化视图 `mv1_tbl5_tbl6` 创建物化视图（嵌套物化视图） `mv_mv1_tbl5_tbl6`。

    ```sql
    CREATE MATERIALIZED VIEW mv_mv1_tbl5_tbl6
        REFRESH COMPLETE
