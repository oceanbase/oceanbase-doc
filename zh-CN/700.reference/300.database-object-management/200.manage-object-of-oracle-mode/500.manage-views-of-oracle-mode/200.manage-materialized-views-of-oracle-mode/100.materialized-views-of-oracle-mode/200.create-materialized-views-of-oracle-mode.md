...
对于大宽表的物化视图，通过选择列存格式可以提高某些查询的性能。OceanBase 数据库支持创建行存、列存和行存列存冗余格式的物化视图，可以通过以下方式来确定物化视图的存储格式：

* 通过配置项 [default_table_store_format](../../../../../800.configuration-items-and-system-variables/100.system-configuration-items/400.tenant-level-configuration-items/2200.default_table_store_format.md) 来设置创建物化视图时的默认存储格式。

  * 执行 `ALTER SYSTEM SET default_table_store_format = 'row';` 命令后，在创建物化视图语句中没有加 `mv_column_group_option` 选项时，创建物化视图的默认存储格式为行存。
  * 执行 `ALTER SYSTEM SET default_table_store_format = 'column';` 命令后，在创建物化视图语句中没有加 `mv_column_group_option` 选项时，创建物化视图的默认存储格式为列存。
  * 执行 `ALTER SYSTEM SET default_table_store_format = 'compound';` 命令后，在创建物化视图语句中没有加 `mv_column_group_option` 选项时，创建物化视图的默认存储格式为行存列存冗余格式。

  **示例如下：**

  1. 执行将 `default_table_store_format` 设置为 `column`。
  ...
  2. 创建物化视图 `mv5_tbl1`。
      ...
      ```sql
      CREATE MATERIALIZED VIEW mv5_tbl1
        AS SELECT *
          FROM tbl1;
      ```

  3. 查看物化视图 `mv5_tbl1` 定义。

      ```sql
      SHOW CREATE VIEW mv5_tbl1;
      ```
      ...
      +----------+--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+----------------------+----------------------+
      | MV5_TBL1 | CREATE MATERIALIZED VIEW "MV5_TBL1" ("ID", "NAME", "AGE") COMPRESS FOR ARCHIVE REPLICA_NUM = 1 BLOCK_SIZE = 16384 USE_BLOOM_FILTER = FALSE TABLET_SIZE = 134217728 PCTFREE = 0 REFRESH FORCE ON DEMAND  WITH COLUMN GROUP(each column) AS select "SYS"."TBL1"."ID" AS "ID","SYS"."TBL1"."NAME" AS "NAME","SYS"."TBL1"."AGE" AS "AGE" from "SYS"."TBL1" | utf8mb4              | utf8mb4_bin          |
      +----------+--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+----------------------+----------------------+
      1 row in set
      ```

* 通过指定 `mv_column_group_option` 选项来明确指定创建的物化视图的格式。
...
通过指定 `WITH COLUMN GROUP(each column)` 创建列存格式的物化视图。
