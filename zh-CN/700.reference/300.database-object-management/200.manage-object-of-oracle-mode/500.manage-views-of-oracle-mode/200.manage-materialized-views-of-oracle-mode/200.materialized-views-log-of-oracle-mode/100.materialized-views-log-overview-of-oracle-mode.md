...
|   **列名**  | **类型** | **说明** |
|-------------|----------|----------|
| m_row$$     | uint64_t | 如果基表是无主键表，则 rowid 是基表的隐藏主键；如果基表还是分区表，那么 mlog 的主键列也将包含基表的分区键。 <main id="notice" type='explain'><h4>说明</h4><p>mlog 的主键列由一个分区内自增列 <code>m_row$$</code> 以及基表的主键和所有分区键构成的复合主键组成。</p></main>|
