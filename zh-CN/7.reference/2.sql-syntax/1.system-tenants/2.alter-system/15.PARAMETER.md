...
| TENANT         | 修改指定租户的租户级配置项。租户级配置项清单，请参见 [租户级别配置项](../../../../7.reference/5.system-configuration-items/4.tenant-level-configuration-items-1/1.audit_sys_operations-1-2-3.md)。   |
...
* 登录 tenant 租户的 root 用户，修改 tenant 租户的租户级配置项`connection_control_max_connection_delay`。

  ```sql
  obclient> SHOW PARAMETERS LIKE 'connection_control_max_connection_delay';
  +-------+----------+----------------+----------+-----------------------------------------+-----------+------------+--------------------------------------------------------------------------------------------------------------------------------------------------------------+----------+--------+---------+-------------------+
  | zone  | svr_type | svr_ip         | svr_port | name                                    | data_type | value      | info                                                                                                                                                         | section  | scope  | source  | edit_level        |
  +-------+----------+----------------+----------+-----------------------------------------+-----------+------------+--------------------------------------------------------------------------------------------------------------------------------------------------------------+----------+--------+---------+-------------------+
  ...
  obclient> ALTER SYSTEM SET connection_control_max_connection_delay = 2147483646;
