...
本文主要介绍在 OceanBase 数据库 MySQL 模式下 [修改表名](#修改表名)、[修改列名](#修改列名)、[修改列类型](#修改列类型)、[同时修改列名和列类型](#同时修改列名和列类型)、[普通列修改为自增列](#普通列修改为自增列)、[增加列](#增加列)、[删除列](#删除列)、[添加主键](#添加主键)、[修改主键](#修改主键)、[删除主键](#删除主键)、[添加外键](#添加外键)、[删除外键](#删除外键)、[添加 CHECK 约束](#添加-check-约束)、[修改表分区](#修改表分区)。
