...
OceanBase 数据库的事务控制语句与 Oracle 数据库兼容，在 OceanBase 数据库 Oracle 模式下执行以下语句时，会开启一个事务。
...
* 配置 `autocommit = 0` 时（关闭自动提交），`INSERT`、`UPDATE`、`DELETE`、`SELECT FOR UPDATE` 系统会会开启一个新事务。
...
在实际使用中，多个并发的连接下，对同一个数据表进行操作时，可能会出现两个事务对同一行数据进行操作。对于查询的读操作，可以通过 `SELECT FOR UPDATE` 语句锁定查询结果，避免其他 DML 语句对该笔记录进行同时修改。语句的详细使用方法，请参见 [锁定查询结果 SELECT FOR UPDATE](../../4.Query-table-data/09.Use-operators-and-functions-in-queries/10.lock-query-result-select-for-update.md)。
