...
* `CONVERT(expr,type)` 用于将表达式 `expr` 转换为 `type` 指定的数据类型。
  ...
  在这种用法中，`CONVERT(expr,type)` 与 `CAST(expr AS type)` 是完全一致的。有关 `CAST` 函数的详细信息，请参见 [CAST](/zh-CN/10.sql-reference-mysql-mode/4.functions/2.functions-1/3.conversion-functions-1/1.CAST-1.md)。
