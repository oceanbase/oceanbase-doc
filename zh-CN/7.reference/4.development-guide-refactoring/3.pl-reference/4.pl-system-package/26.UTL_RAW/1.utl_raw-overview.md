...
| [CAST_TO_RAW](6.CAST_TO_RAW.md)           | 将 `VARCHAR2` 值转换为 `RAW` 值。              |
| [CAST_TO_VARCHAR2](7.CAST_TO_VARCHAR2.md) | 将 `RAW` 值转换为 `VARCHAR2` 值。              |
| [COPIES](18.COPIES.md)           | 返回串联在一起的 `r` 的 `n` 个副本。                      |
| [COMPARE](9.COMPARE.md)         | 比较 `RAW r1` 和 `RAW r2`。                             |
| [CONCAT](10.CONCAT-utl.md)        | 将最多 12 个 `RAW` 串连到一个 `RAW` 中。                  |
| [LENGTH](11.LENGTH-utl.md)        | 返回 `RAW r` 的字节长度。                                |
| [REVERSE](12.REVERSE.md)        | 从端到端反转 `RAW r` 中的字节序列。                       |
| [SUBSTR](13.SUBSTR-utl.md)    | 返回 `len` 字节数，从 `RAW r` 的 `pos` 位开始计算。       |
