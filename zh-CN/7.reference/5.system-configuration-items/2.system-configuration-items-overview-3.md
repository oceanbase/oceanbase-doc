...
| [default_row_format](3.cluster-level-configuration-items-1/59.default_row_format-1-2-3.md)                  | 用于设置建表时使用的默认行格式。                                                                                        |
...
| [writing_throttling_maximum_duration](4.tenant-level-configuration-items-1/36.writing_throttling_trigger_percentage-1-2-3.md)   | 通过控制内存分配进度，控制写入速度。即指定在触发写入限速后，剩余 MEMStore 内存分配完所需的时间。 |
| [writing_throttling_trigger_percentage](4.tenant-level-configuration-items-1/37.workarea_size_policy-1-2-3.md) | 用于设置写入速度的阈值。                                          |
