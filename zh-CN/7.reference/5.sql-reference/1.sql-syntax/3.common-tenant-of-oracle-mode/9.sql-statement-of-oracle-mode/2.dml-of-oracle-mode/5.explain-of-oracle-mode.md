...
{EXPLAIN [INTO table_name ] [SET statement_id = string]  
 | DESCRIBE 
 | DESC} [explain_type] dml_statement;
...
| SET statement_id | 表示当前查询使用字符串标记，以方便后续查询该 SQL 的计划信息。如果没有指定 `SET statement_id`，默认使用空字符串作为信息标记。 || BASIC                         | 指定输出计划的基础信息，如算子 ID、算子名称、所引用的表名。   |
| explain_type        | 指定解释类型。   |
