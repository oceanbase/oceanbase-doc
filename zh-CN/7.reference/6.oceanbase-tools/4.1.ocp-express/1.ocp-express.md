...
* 集群总览：查看集群拓扑结构、状态信息、资源消耗、慢 SQL 等核心信息，详情可参见 [集群总览](2.ocp-express-cluster-overview.md)。
* 租户管理：管理集群下的租户，包括创建租户、删除租户、管理数据库、管理参数等，详情可参见 [租户管理](3.ocp-express-tenants-management.md)。
* 监控中心：提供集群与租户维度的监控，覆盖性能监控、主机监控等数十种监控指标，详情可参见 [集群监控](4.ocp-express-monitoring-center.md)。
* 诊断中心：可以对租户会话及慢 SQL 进行有效的管理，如关闭会话、SQL 限流等，详情可参见 [诊断分析](5.ocp-express-diagnosis-center.md)。
* 日志服务：可以在平台分析集群 OBServer 节点、Agent、主机的相关日志，定位故障更加便捷简单，详情可参见 [日志服务](6.ocp-express-log-service.md)。
