...
|                    配置项                     |               含义               | 默认值 | 可否动态变更 |                                  备注                                   |
|--------------------------------------------|--------------------------------|-----|--------|-----------------------------------------------------------------------|
| enable_output_trans_order_by_sql_operation | 是否将事务内行数据按 SQL 执行顺序输出          | 否   | 否      | 该配置项的值为 0 时，将按事务参与者顺序输出数据，此时如果源端存在 row movement 场景（数据从一个事务参与者迁移到另一个事务参与者）且表存在全局唯一索引时，OBCDC 输出顺序不能保证 DELETE 旧事务参与者的数据在前，INSERT 新事务参与者的数据在后，下游消费可能存在数据正确性问题。如果源端可能存在 row movement，需要设置该配置项的值为 1。 关于 Row movement，请参考 [CREATE TABLE](../../../../4.development-reference/1.sql-syntax/3.common-tenant-of-oracle-mode/9.sql-statement-of-oracle-mode/1.ddl-of-oracle-mode/24.create-table-of-oracle-mode.md) 和 [索引设计](../../../../4.development-reference/4.database-design-specifications-and-practices/2.object-structure-design-specification/4.index-design.md)  <main id="notice" type='explain'><h4>说明</h4><p>在 OceanBase 数据库 V4.x 版本中，事务参与者单位为日志流，在 OceanBase 数据库 V3.x 版本，事务参与者单位为分区。</p></main>                 |
