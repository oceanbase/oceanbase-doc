...
oblogproxy 的配置文件默认放在 `conf/conf.json` 中。您只需要关心 [使用源码构建 oblogproxy](1.install-and-deploy-oblogproxy/2.use-source-code-to-build-an-oblogproxy.md) 中描述过的参数，对于其他参数，在不完全了解参数用途的情况下，我们不建议您进行修改。
