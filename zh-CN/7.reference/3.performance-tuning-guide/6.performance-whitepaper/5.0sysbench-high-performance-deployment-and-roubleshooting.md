...
1. 分析 Sysbench 参数。
    ...
    在运行命令中优先分析 Sysbench 参数。

    * 如果 `--rand-type=special` 未设置，则可以设置为 `--rand-type=uniform`，减少数据间的锁竞争。
...
2. 确认连接方式。
   ...
   2. 查询租户 Leader。
      ...
      ```sql
      select svr_ip, count(1) from __all_virtual_ls_meta_table where tenant_id =1 and role=1 group by svr_ip;
      ```

3. 观察 CPU 开销。

   若 Sysbench 相关没有问题，可通过查看 CPU 使用率继续排查。
   ...
   * 查看线程 CPU 占用。
      ...
      * 如果 CPU 总开销接近机器上限，那么机器性能已到极限，只有用更大的机器才能跑出更高的性能。
   ...
   * 查看租户规格。
     ...
     如果 CPU 总开销接近机器上限，那么说明机器的性能已经到达了极限，此时只有使用更大的机器才能跑出更高的性能。

      * 如果存在网络线程（如 RpcIO）打满一个核，可能是网络线程数量偏少，网络通信成为了瓶颈，需要增加线程数量 `net_thread_count`。
      * 如果线程 CPU 占用率不高，则可能是硬件方面成为瓶颈，需要继续分析。

4. 分析硬件原因。

   排除 CPU 因素之后，剩下的硬件原因主要是磁盘和网络，磁盘和网络的相关排查如下。
...
5. 其他。
   ...
   排除上述因素外，还可能存在一些特殊情况，比如业务运行时间段。

   曾经有用户反映测试中有两组数据明显低于预期，检查各种配置均无明显问题，最后得知该用户在凌晨 2 点进行的业务测试，而该时间段恰好是 OceanBase 数据库进行每日转储合并的默认时间，因此对于性能测试大受影响。
...
* 处理方法：
  ...
  3. 减少转储线程数量或降低线程优先级，会让转储的单位时间开销降低，但是需要付出时间上的代价，转储整体的开销是不变的。
...
* 处理方法：即使数据集非常大，使用 `--rand-type=uniform` 参数，也可能会发生死锁。因此，可以使用 `--mysql-ignore-errors=1062,1213` 参数来忽略死锁错误。
...
* 查看事务相关 SQL。
  ...
  如果搜不到相关事务，说明没有开启 SQL Audit 功能，通过查看 `enable_sql_audit` 配置项是否设置为 True：
...
* 查询 SQL 对应日志。
  ...
  可以看到该条 SQL 所有的日志，一般最后一条就是报给客户端的错误码：

  ```shell
  sending error packet(err=-4101
  ```

  用最后一条 commit 或 begin 的 Trace ID，也可以搜索到相关日志：

  ```shell
  sending error packet(err=-6002
  ```

  如果没有日志，那就是日志级别不对，或者日志限流了。需要将日志级别调成 `'INFO'`，日志带宽限制调大：
...
1. 首先需要开启 `sql_audit` 记录。

    ```sql
    alter system set enable_sql_audit = true;
    ```

2. 清空之前的 `sql_audit` 记录。
...
* 开源版安装部署流程
   ...
   使用 OBD 部署：[使用命令行部署 OceanBase 数据库生产环境](../../../4.deploy/5.deploy-oceanbase-database-community-edition/2.local-deployment/5.deploy-OceanBase-database-of-multi-node-cluster.md)

* 企业版安装部署流程

  * 企业版部署：[部署流程](../../../4.deploy/3.deploy-oceanbase-database-enterprise/1.deployment-process.md)
...
* **memory_limit**
  ...
  * 相关文档：[memory_limit](../../../7.reference/5.system-reference/1.system-configuration-items/3.cluster-level-configuration-items/136.memory_limit.md)
...
* **memory_limit_percentage**
  ...
  * 相关文档：[memory_limit_percentage](../../../7.reference/5.system-reference/1.system-configuration-items/3.cluster-level-configuration-items/137.memory_limit_percentage.md)
...
* **system_memory**
  ...
  * 相关文档：[system_memory](../../../7.reference/5.system-reference/1.system-configuration-items/3.cluster-level-configuration-items/207.system_memory.md)
...
* **cpu_count**
  ...
  * 相关文档：[cpu_count](../../../7.reference/5.system-reference/1.system-configuration-items/3.cluster-level-configuration-items/45.cpu_count.md)
...
* **net_thread_count**
  ...
  * 相关文档：[net_thread_count](../../../7.reference/5.system-reference/1.system-configuration-items/3.cluster-level-configuration-items/152.net_thread_count.md)

* **sql_net_thread_count**

  相关文档：[sql_net_thread_count](../../../7.reference/5.system-reference/1.system-configuration-items/3.cluster-level-configuration-items/251.sql_net_thread_count.md)
...
* **datafile_size**
  ...
  * 相关文档：[datafile_size](../../../7.reference/5.system-reference/1.system-configuration-items/3.cluster-level-configuration-items/54.datafile_size.md)
...
* 相关文档：[datafile_disk_percentage](../../../7.reference/5.system-reference/1.system-configuration-items/3.cluster-level-configuration-items/53.datafile_disk_percentage.md)

* **log_disk_size**

  相关文档：[log_disk_size](../../../7.reference/5.system-reference/1.system-configuration-items/3.cluster-level-configuration-items/237.log_disk_size.md)

* **log_disk_percentage**

  相关文档：[log_disk_percentage](../../../7.reference/5.system-reference/1.system-configuration-items/3.cluster-level-configuration-items/238.log_disk_percentage.md)
...
有关创建租户的详细操作，参见 [创建租户](../../../4.deploy/3.deploy-oceanbase-database-enterprise/4.command-line-deployment/5.create-oceanbase-tenant-command-line/2.create-a-tenant-command-line.md)。
...
* **min_cpu**
  ...
  * 推荐值：`cpu_count`/2
  * 相关文档：`min_cpu` 参数详细信息请参见 [创建租户](../../../4.deploy/3.deploy-oceanbase-database-enterprise/4.command-line-deployment/5.create-oceanbase-tenant-command-line/2.create-a-tenant-command-line.md) 中 **创建资源单元** 的 **参数解释**。
...
* **cpu_quota_concurrency**
  ...
  * 相关文档：[cpu_quota_concurrency](../../../7.reference/5.system-reference/1.system-configuration-items/4.tenant-level-configuration-items/55.cpu_quota_concurrency.md)
...
* 安装部署
  ...
  相关文档：详细信息请参见 [OceanBase 数据库 Sysbench 测试](3.use-sysbench-to-test-the-performance-of-oceanbase-database.md) 中的 **安装 Sysbench**。
...
* **skip_trx**
  ...
  * 参数说明：跳过事务，在只读测试中可以选择是否跳过事务， 默认为 OFF，即就是开启事务。
...
3. 生成现象：TPS 跌到 0，但偶尔也会有一写 TPS 和 QPS，不是完全阻塞。
...
4. 查看内部表。
   ...
   ![image1.png](https://obbusiness-private.oss-cn-shanghai.aliyuncs.com/doc/img/observer-enterprise/V4.1.0/7.reference/3.performance-tuning-guide/6.performance-whitepaper/sysbench/%E6%9F%A5%E7%9C%8B%E5%86%85%E9%83%A8%E8%A1%A8-1.png)
    ...
    可以看到最近的死锁记录，`role=victim` 表示被杀掉的事务，`role=witness` 表示见证冲突的事务。`cycle_size=2` 说明只有2两个事务相互冲突，`cycle_size>2` 说明多个事务和一个事务冲突。这里我们为了便于观察选择一个 `cycle_size=2` 的 Event，通过 event_id 可以判断是一组冲突的事务。

    ```sql
    select* from __all_virtual_deadlock_event_history where event_id = 6722858974631518041;
    ```
   ...
   ![image2.png](https://obbusiness-private.oss-cn-shanghai.aliyuncs.com/doc/img/observer-enterprise/V4.1.0/7.reference/3.performance-tuning-guide/6.performance-whitepaper/sysbench/%E6%9F%A5%E7%9C%8B%E5%86%85%E9%83%A8%E8%A1%A8-2.png)
    ...
    Visitor 可以看到事务 ID，通过事务 ID 可以找到该事务相关的所有 SQL 命令：
   ...
   查找被杀事务发生冲突的那条 SQL 日志，可以看到最后一条日志发送的是 4101 错误码。但最后 Sysbench 这边并未报死锁错误，而是报了 6002 回滚错误。
    ...
    查找事务最后一条 "BEGIN" 的日志，可以找到返回 6002 回滚错误。得到 Sysbench 没有处理死锁 SQL 的报错，而是将 6002 错误报了出来。
...
1. 在默认 `rand-type=special` 场景下，可能因为出现死锁，这种场景下 Sysbench 压测会退出。
...
4. 如果 `ob_query_timeout` 设置比较长，有碰巧出现死锁检测没有检测到，事务无法被杀掉，SQL 一直阻塞，这种情况 tps 会长期为 0，且不报错。

**解决方法：**

1. 首先考虑调数据随机类型：设置 `--rand-type=uniform`
2. 如果用户不考虑调数据随机类型，建议考虑调大数据集：比如 `tables=100，table_size=1000000`
3. 若数据随机类型和数据集都不考虑调节，则可以尝试忽略错误码，但这样会无法避免死锁： `--mysql-ignore-errors=1062,1213,6002`
