...
租户的后台任务每隔 1s 会根据 OBServer 节点和 SQL Audit 的内存使用情况来决定是否触发淘汰。SQL Audit 内存上限根据 `ob_sql_audit_percentage` 分配可用的最大内存。
