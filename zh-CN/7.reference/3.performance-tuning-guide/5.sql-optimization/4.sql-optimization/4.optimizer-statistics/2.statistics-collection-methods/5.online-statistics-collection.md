...
在线统计信息收集指在执行 `INSERT` 插入的时候就收集统计信息，不用手动调用系统包。在批量导入数据的过程中，可以实时收集统计信息，不需要额外的发起收集任务，减少了运维操作，提高统计信息收集性能。
...
- `CREATE TABLE...AS SELECT`（即 CTAS）
  ...
  在系统变量 `_optimizer_gather_stats_on_load` 开启的情况下，CTAS 默认启动在线收集统计信息功能。如果需要禁用该功能，可以使用 `NO_GATHER_OPTIMIZER_STATISTICS` Hint，示例如下：
...
- `INSERT INTO`
  ...
  在系统变量 `_optimizer_gather_stats_on_load` 开启的情况下，使用`GATHER_OPTIMIZER_STATISTICS` Hint 或者 `APPEND` Hint 启动在线收集统计信息功能。
  示例如下：
...
用户在线收集到的统计信息可以在相应的视图中查看，视图的详细信息请参见 [统计信息收集方式概述](1.overview-of-statistics-collection-methods.md)。
