...
OceanBase 数据库还支持通过 Hint 控制是否对 SQL 语句中的字面量做参数改写，详细信息参见 [CURSOR_SHARING_EXACT Hint](../../../4.development-guide-refactoring-1/6.sql-syntax/3.common-tenant-oracle-mode/3.basic-elements-1/6.annotation-1/4.Hint/2.hint-list/7.other-hints.md)。

对于一条 SQL 请求，使用如下策略区分此条 SQL 要命中参数化的计划还是要命中非参数化的计划：

1. 查看 `cursor_sharing`

   首先查看 `cursor_sharing` 系统变量，具有不同 `cursor_sharing` 的 SQL 一定会命中到不同的计划。请参见 [cursor_sharing](../../14.system-reference/3.system-variable-for-oracle/20.cursor_sharing.md)。

2. 查看 `query_sql`

   如果 `cusor_sharing` 变量无法帮助我们辨别出是否为参数化计划时，则说明 `cursor_sharing` 属于 `EXACT` 模式是由 `CURSOR_SHARING_EXACT` Hint 控制的，由于添加了 Hint 就会影响到 `query_sql` 的生成，因此，我们可以通过 `query_sql` 来区分参数化计划和非参数化计划。`query_sql` 可以通过 `gv$sql_audit` 或者 `gv$plan_cache_plan_stat` 视图查看。
