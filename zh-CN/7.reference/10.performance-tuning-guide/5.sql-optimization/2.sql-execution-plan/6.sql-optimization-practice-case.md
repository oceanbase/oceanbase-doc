...
1. 通过查询 `GV$OB_SQL_AUDIT` 视图定位 TOP SQL。
   ...
   查询得到 `SELECT ... FROM V_TT01 WHERE (COL001 IN (20017476))` 类别 SQL 的执行频率和 CPU 使用率较高，而且集中于同一台 OBServer 节点上。

   ```shell
   +----------------------------------+------------+--------------+--------------+---------+------------------------------------------------------------------------------------------------------------------------------------------------------+
   |       sql_id                     | executions | tot_cup_time | avg_cup_time | cup_cnt | query_sql      |
   +----------------------------------+------------+--------------+--------------+---------+------------------------------------------------------------------------------------------------------------------------------------------------------+
   | 2705182A6EAB699CEC8E59DA80710B64 |   329196 |       33818182526  |     1443679  |      56  | SELECT ... FROM V_TT01 WHERE (COL001 IN (20017476)) |
   | 32AB97A0126F566064F84DDDF4936F82 |   275345 |       276913 |       1114496|       13 | SELECT ... FROM V_TT01 WHERE (COL001 IN (20017476)) |
   | A5F514E873BE9D1F9A339D0DA7481D69 |   154236 |       154873 |        3478098 |       10 | SELECT ... FROM V_TT01 WHERE (COL001 IN (20017476)) |
   | 31FD78420DB07C11C8E3154F1658D237 |      688 |       188973 |       20321589  |       1| SELECT ... FROM V_TT01 WHERE (COL001 IN (20017476))  |
   | C48AEE941D985D8DEB66892228D5E845 |     1137 |       288516 |       2839940 |       1| SELECT ... |
   | 101B7B79DFA9AE801BEE4F1A234AD294 |       26 |       289585 |        1014516  |       1 | SELECT ... |
   | 1D0BA376E273B9D622641124D8C59264 |       45 |       294356 |       2887948  |       1 | SELECT ... |
   | 64CF75576816DB5614F3D5B1F35B1472 |      128 |       192843 |        811987 |       1 | SELECT ... |
   | 23D1C653347BA469396896AD9B20DCA1 |       55 |       201594 |         41313078 |       1| SELECT ... |
   | FA4F493FA5CE2DCC64F51CF3754F96C6 |      241 |       235720 |         1196261|       0 | SELECT ... |
   +------------+----------------------------+--------------+------------+--------------+------------------------------------------------------------------------------------------------------------------------------------------------------+
   10 rows in set 
...
3. 分析相关表和视图的 DDL 定义，确定出现性能问题的原因。
   ...
   综合 DDL 信息和 `EXPALIN` 可知，该 SQL 执行过程中发生的 To_number 强制转换导致索引性能下降，是造成 11 号和 14 号算子的代价估算较大的原因。
