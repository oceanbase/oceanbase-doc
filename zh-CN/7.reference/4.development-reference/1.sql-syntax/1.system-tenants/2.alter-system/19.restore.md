...
restore_action:
  ...
  /* 恢复到指定 SCN */
  RESTORE dest_tenant_name FROM uri UNTIL SCN=scn WITH restore_option [DESCRIPTION description];
...
* 从 NFS 数据备份路径与 NFS 日志归档路径恢复租户 `mysql` 到指定 SCN `1658285759724047000`, 指定资源池为 `restore_pool`。
...
* 在目标集群上执行 `RESTORE` 命令，使用租户的备份和归档，创建并恢复出备租户。
   ...
   ```sql
   obclient> ALTER SYSTEM RESTORE restore_oracle_tenant FROM 'file:///data/1/zbf271370/backup//ob_backup_oracle_tenant/archive,file:///data/1/zbf271370/backup//ob_backup_oracle_tenant/data' UNTIL TIME='2022-11-11 15:04:23.825558' with 'pool_list=small_pool_2&primary_zone=z1';
