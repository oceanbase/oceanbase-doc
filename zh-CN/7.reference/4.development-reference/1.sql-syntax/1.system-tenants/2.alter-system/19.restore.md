...
* 在目标集群上执行 `RESTORE` 命令，使用租户的备份和归档，创建并恢复出备租户。
   ...
   ```sql
   obclient> ALTER SYSTEM RESTORE restore_oracle_tenant FROM 'file:///data/1/xxx/backup//ob_backup_oracle_tenant/archive,file:///data/1/xxx/backup//ob_backup_oracle_tenant/data' UNTIL TIME='2022-11-11 15:04:23.825558' with 'pool_list=small_pool_2&primary_zone=z1';
