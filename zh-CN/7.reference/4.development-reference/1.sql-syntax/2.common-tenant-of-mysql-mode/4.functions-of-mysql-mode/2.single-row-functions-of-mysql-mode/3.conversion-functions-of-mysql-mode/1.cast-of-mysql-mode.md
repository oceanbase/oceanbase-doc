...
* `type`：表示目标系统所提供的数据类型。支持如下类型：
  ...
  * `JSON`：生成 `JSON` 值。有关 JSON 与其他类型的值转换规则，请参见 [JSON 数据类型转换](../../../1.basic-elements-of-mysql-mode/1.data-type-of-mysql-mode/8.json-formatted-data-type-of-mysql-mode/4.json-formatted-data-type-conversion-of-mysql-mode.md)。
