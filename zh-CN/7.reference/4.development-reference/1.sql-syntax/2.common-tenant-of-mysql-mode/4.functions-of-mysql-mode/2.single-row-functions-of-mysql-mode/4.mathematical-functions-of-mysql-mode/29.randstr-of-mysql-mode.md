...
`RANDSTR()` 配合 `gen` 参数使用，可以控制生成的随机字符串的频率。如下示例中，生成 1000 个长度为 1 的字符串，它们的分布符合 zipf 分布规律。
...
`RANDSTR()` 函数通过控制 `N` 参数的大小，可以生成长度不同的字符串。
...
当 `gen` 为常量时，反复执行 `RANDSTR()` 函数，会生成相同的随机字符串。
