...
示例 2：创建触发器 `tri2_t` 和 `trg3_t` 在触发器 `tri1_t` 之后执行，触发器 `trg4_t` 在触发器 `tri1_t` 之前执行。
...
* 查询 `INFORMATION_SCHEMA` 的 `TRIGGERS` 表。详细信息，请参见 [INFORMATION_SCHEMA TRIGGERS](../8.information-schema-dictionary-view-mysql/3.information-schema-triggers-mysql.md)。
