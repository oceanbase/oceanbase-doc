...
序列化是 `parse` 函数的逆过程，解析输入的 JSON 数据并返回 PL JSON 对象类型的实例。序列化方法的名称以前缀 `to_` 开头。大多数序列化方法都是成员函数。但是，对于 `CLOB` 或 `BLOB` 实例的序列化有成员函数和成员过程两种形式的方法。
...
get_Object(key VARCHAR2)    RETURN JSON_OBJECT_T // 查找到的 value, 转换成 TIMESTAMP
...
PROCEDURE get_Blob(key NUMBER, c IN OUT BLOB)   // 查找到的 value, 转换成 BLOB, 放在 c参数中
