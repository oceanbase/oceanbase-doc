# 查看恢复结果
#docslug#/oceanbase-database/oceanbase-database/V4.0.0/view-the-restore-history
...
3. 待恢复完成后，您可以执行以下语句，查看集群的恢复任务结果。
   ...
   ```
    ...
    如果恢复失败，您可以通过删除租户的方式来取消本次恢复，具体操作请参见 [取消恢复](2.cancel-a-restore.md)。

## 后续处理

恢复操作执行成功后, 恢复后的租户均为备租户，您需要执行备切主命令，将备租户转换为主租户，具体操作请参见 [备租户转为主租户](5.failover-standby-tenant.md)。
