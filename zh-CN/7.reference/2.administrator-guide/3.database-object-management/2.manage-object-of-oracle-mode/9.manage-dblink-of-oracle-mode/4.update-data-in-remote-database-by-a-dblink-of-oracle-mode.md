...
* 通过 DBLink 向远端数据库写入数据的操作完成后，需要手动使用 `COMMIT` 或者 `ROLLBACK` 语句提交回滚本地和远端的 XA 事务。
  ...
  更多事务控制语句的详细说明信息，参见 [事务控制语句概述](../../../../../3.developer-guide/3.Application-development-based-on-Oracle-mode/5.about-DML-statements-and-transactions/02.transactions/01.about-transaction-control-statement.md)。
...
* 通过 DBLink 向远端数据库写入数据时，暂不支持通过设置 `autocommit = True` 来开启事务的自动提交，并且通过 DBLink 写入数据的操作一旦开启 XA 事务，系统就会将本 Session 的 `autocommit` 值强制置为 `False`，直到 XA 事务提交或者回滚后，才会恢复 `autocommit` 原本的值。
  ...
  系统变量 `autocommit` 用于设置是否开启自动提交事务，默认值为 `True`，更多系统变量 `autocommit` 的说明信息，参见 [autocommit](../../../../14.system-reference/3.system-variable-for-oracle/6.autocommit-1-2-3-4.md)。

* 如果预期 XA 事务耗时较长，为了防止 XA 事务超时出现非预期的错误，可以使用 `set ob_trx_timeout = 1000000000;` 语句提前调大事务的超时时间。
  
  系统变量 `ob_trx_timeout` 用于设置事务的超时时间，默认值为 `86400000000`，单位为微秒。更多系统变量 `ob_trx_timeout` 相关的信息，参见 [ob_trx_timeout](../../../../14.system-reference/3.system-variable-for-oracle/102.ob_trx_timeout-1-2-3-4.md)。
...
更多 `INSERT` 语句的详细使用及说明，请参见 [INSERT](../../../../6.sql-syntax/3.common-tenant-oracle-mode/9.sql-statement-1/2.DML/2.INSERT-1.md)。
...
更多 `UPDATE` 语句的详细使用及说明，请参见 [UPDATE](../../../../6.sql-syntax/3.common-tenant-oracle-mode/9.sql-statement-1/2.DML/6.UPDATE-1.md)。
...
更多 `MERGE INTO` 语句的详细使用及说明，请参见 [MERGE](../../../../6.sql-syntax/3.common-tenant-oracle-mode/9.sql-statement-1/2.DML/3.MERGE.md)。
...
更多 `DELETE` 语句的详细使用及说明，请参见 [DELETE](../../../../6.sql-syntax/3.common-tenant-oracle-mode/9.sql-statement-1/2.DML/1.DELETE-1.md)。
