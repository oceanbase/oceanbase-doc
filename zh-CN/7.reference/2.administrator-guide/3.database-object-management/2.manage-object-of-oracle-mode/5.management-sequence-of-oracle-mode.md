...
* 更多 `DBA_SEQUENCES` 视图的字段及说明信息请参见 [DBA_SEQUENCES](../../../5.system-reference/5.system-view-of-oracle-mode/2.dictionary-view-of-oracle-mode/256.dba_sequences-of-oracle-mode.md)。
...
OceanBase 数据库支持修改序列的起始值、最小值、最大值、步长和循环属性。修改序列的语法格式如下：
...
执行该语句的用户需要具备 `ALTER` 对象权限或 `ALTER ANY SEQUENCE` 系统权限。
