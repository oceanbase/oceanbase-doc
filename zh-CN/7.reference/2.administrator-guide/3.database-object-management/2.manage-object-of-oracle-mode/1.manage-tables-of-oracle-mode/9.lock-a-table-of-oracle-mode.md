...
创建分区表后，根据模板化二级分区表的分区命名规则，创建的二级分区名分别为 `p0ssp0`，`p0ssp1` 和 `p0ssp2`。有关区命名规则详细说明，请参见 [分区概述](../../../4.replica-management/2.manage-partition-table/1.oracle-mode/1.partition-overview-of-oracle-mode.md)。
