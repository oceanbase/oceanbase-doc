...
* 外键约束（`FOREIGN KEY`）：要求约束的列的值取自于另外一个表的主键列。
  ...
  OceanBase 数据库默认开启了外键约束检查，外键约束检查开关由租户变量 `foreign_key_checks` 来控制。更多 `foreign_key_checks` 变量的说明，请参见 [foreign_key_checks](../../../../7.reference/14.system-reference/3.system-variable-for-oracle/27.foreign_key_checks-1-2-3-4.md)。
...
更多管理约束的语法及详细介绍请参见 [ALTER TABLE](../../../../7.reference/6.sql-syntax/3.common-tenant-oracle-mode/9.sql-statement-1/1.DDL-1/10.alter-table-1.md)。
