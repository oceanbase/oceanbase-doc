...
在 OceanBase 数据库的 MySQL 模式中，对函数索引的表达式进行了限制，禁止部分系统函数的表达式作为函数索引，具体的函数列表请参见 [函数索引支持的系统函数列表](5.function-index-list-of-supported-functions.md) 和 [函数索引不支持的系统函数列表](6.function-index-list-of-not-supported-functions.md)。
