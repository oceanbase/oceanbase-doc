...
删除表组前，请通过视图 `CDB_OB_TABLEGROUP_TABLES` 或 `DBA_OB_TABLEGROUP_TABLES` 确认表组中是否存在表，仅当表组内没有任何表时，表组才能删除成功。通过视图查看表组中是否存在表的详细操作可参见 [查看表组信息](3.view-the-information-of-a-table-group-of-mysql-mode.md)。
