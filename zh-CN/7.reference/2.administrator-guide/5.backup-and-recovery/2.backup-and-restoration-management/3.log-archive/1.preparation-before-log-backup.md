...
2. 设置归档目的端参数。
  ...
  <main id="notice" type='notice'>
    ...
    <ul>
    <li>
    <p><code>LOCATION</code> 的值不支持设置为带有问号（?）的字符串。</p>
    </li>
    <li>
    <p><code>LOCATION</code> 的值必须设置为绝对路径，保证 <code>OBServer</code> 对 <code>LOCATION</code> 具有读写权限 。</p>
    <p>必须保证所有 OBServer 都挂载了同一个服务器的 NFS。同时，为保证归档顺利进行，务必使用本文档中建议的参数挂载 NFS。挂载 NFS 的具体操作，请参见 <a href="../2.deploy-nfs.md">部署 NFS 客户端</a>。</p>
    </li>
    </ul>
  </main>
