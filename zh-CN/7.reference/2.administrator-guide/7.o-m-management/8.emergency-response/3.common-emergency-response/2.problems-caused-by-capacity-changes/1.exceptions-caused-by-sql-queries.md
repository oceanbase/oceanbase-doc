...
1. 通过 OCP 查找慢 SQL。
   ...
   可以在 OCP 的 TOP SQL 页面查看最近一段时间耗时、执行次数最高的查询，从高到低进行排序，对于计划走错的 SQL直接在线绑定，具体操作请参见 [可疑 SQL 诊断](../2.problems-caused-by-capacity-changes/1.exceptions-caused-by-sql-queries.md)；对于需要限流的 SQL，需要结合业务研发的确认信息，定位可以进行限流的SQL。
