...
2. 根据业务需求，选择合适的转储操作。
   ...
   * 日志流级别的转储
      ...
      其中，`tenant_name` 为指定租户的租户名；`ls_id` 为指定租户的日志流 ID；租户的日志流 ID 可以通过查询  `CDB_OB_TABLET_TO_LS` 视图获取。
   ...
   * Tablet 级别的转储
      ...
      其中，`tenant_name` 为指定租户的租户名；`ls_id` 为指定租户的日志流 ID；`tid` 为指定日志流中的 Tablet ID，租户的日志流 ID 和 Tablet ID 可以通过查询  `CDB_OB_TABLET_TO_LS` 视图获取。
...
* [自动触发转储](2.automatically-trigger-dump.md)

* [修改转储配置](5.modify-dump-configuration.md)
