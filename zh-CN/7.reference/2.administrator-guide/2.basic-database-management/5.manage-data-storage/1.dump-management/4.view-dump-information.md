...
2. 查看转储进度，可以查看转储未完成的数据量及预期完成时间等信息。
   ...
   `oceanbase.GV$OB_TABLET_COMPACTION_PROGRESS` 视图用于展示 Tablet 级别 Compaction 的进度信息。具体查询语句如下：
   ...
   * `TYPE`：表示 Compaction 任务的类型。
     ...
     * `MINI_MERGE`：表示转储/L0 Compaction 任务，Memtable 通过转储变成 SSTable。

     * `MAJOR_MERGE`：表示合并任务。

     * `MINI_MINOR_MERGE` ：表示 L1 Compaction 任务，多个 Mini SSTable 合成一个。

     * `BUF_MINOR_MERGE` ：表示生成特殊的 Buf Minor SSTable 的 Compaction 任务。
   
   * `STATUS`：表示任务状态。其中，`NODE_RUNNING` 表示正在进行状态。
   ...
   * `ESTIMATED_FINISH_TIME`：表示预计完成时间。
   ...
   有关 `oceanbase.GV$OB_TABLET_COMPACTION_PROGRESS` 视图中字段的详细介绍，请参见 [oceanbase.GV$OB_TABLET_COMPACTION_PROGRESS](../../../../5.system-reference/4.system-view-of-mysql-mode/3.performance-view-of-mysql-mode/11.gv-ob_tablet_compaction_progress-of-mysql-mode.md)。
   
   根据查询结果，对于未出现在该视图中的 Tablet 或长时间未完成的 Tablet，可以进一步查看诊断视图 `oceanbase.GV$OB_COMPACTION_DIAGNOSE_INFO`，确认是否有异常情况出现。

   有关 `oceanbase.GV$OB_COMPACTION_DIAGNOSE_INFO` 视图中字段的详细介绍，请参见 [oceanbase.GV$OB_COMPACTION_DIAGNOSE_INFO](../../../../5.system-reference/4.system-view-of-mysql-mode/3.performance-view-of-mysql-mode/2.gv-ob_compaction_diagnose_info-of-mysql-mode.md)。
...
2. 查看转储历史。
   ...
   `oceanbase.GV$OB_TABLET_COMPACTION_HISTORY` 视图用于展示 Tablet 级别 Compaction 的历史信息。具体查询语句如下：
   ...
   *************************** 5. row ***************************
   ...
   有关 `oceanbase.GV$OB_TABLET_COMPACTION_HISTORY` 视图中字段的详细介绍，请参见 [oceanbase.GV$OB_TABLET_COMPACTION_HISTORY](../../../../5.system-reference/4.system-view-of-mysql-mode/3.performance-view-of-mysql-mode/10.gv-ob_tablet_compaction_history-of-mysql-mode.md)。