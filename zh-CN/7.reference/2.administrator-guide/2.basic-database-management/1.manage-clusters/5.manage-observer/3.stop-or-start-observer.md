...
关于节点状态的更多说明请参见 [查看节点状态](../5.manage-observer/2.view-observer-status.md)。
...
* 如果分区数多，或者被 Stop Server 的节点分区的 Leader 数量多， Stop Server 操作时间会比较长，如果超时，可以加大 SQL 超时时间。
  ...
  更多变量 `ob_query_timeout` 的介绍信息请参见 [ob_query_timeout](../../../../5.system-reference/2.system-variable/3.global-system-variable/90.ob_query_timeout-global.md) 。
...
* 执行 Stop Server 操作后，`oceanbase.DBA_OB_SERVERS` 视图中该 Server 的 `status` 字段仍为 `Active` 不变，但 `stop_time` 字段的值由 `NULL` 变为 Stop Server 的时间点，同时节点处于 `stop` 状态。
  ...
  更多节点状态的介绍和查看相关操作请参见 [查看节点状态](../5.manage-observer/2.view-observer-status.md)。
