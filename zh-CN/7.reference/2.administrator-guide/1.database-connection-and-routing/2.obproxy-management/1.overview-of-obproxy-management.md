...
OBProxy 支持将请求正确发送至主副本，并且通过特定配置还支持读写分离和备优先读等场景。另外在 OBServer 节点发生宕机、升级或合并等状态时，可以通过 [黑名单机制](../7.route-management/4.odp-routing-policy/3.blacklist-mechanism.md) 确保用户请求可以被路由至状态正常的 OBServer 上。
