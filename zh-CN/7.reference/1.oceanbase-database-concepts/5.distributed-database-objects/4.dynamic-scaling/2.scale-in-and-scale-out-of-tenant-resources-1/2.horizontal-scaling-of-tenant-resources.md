...
obclient> ALTER RESOURCE TENANT tt UNIT_NUM = 1; // 同时调小两个 resource pool 的 unit num。4.0版本暂不支持调小。
obclient> ALTER RESOURCE TENANT tt UNIT_NUM = 3; // 同时调大两个 resource pool 的 unit num。
```
