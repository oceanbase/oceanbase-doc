...
租户资源的水平管理主要是通过调整资源池的 `UNIT_NUM` 来动态调整租户的可用资源。具体表现为，您可以通过调整资源池的 `UNIT_NUM` 来调整资源池下每个 Zone 内资源单元的数量，进而提高或降低该租户在对应 Zone 上的服务能力。OceanBase 数据库 V4.0 版本后，不再支持租户的多个 Resource Pool 的 Unit 数目异构。
...
obclient> ALTER RESOURCE TENANT tt UNIT_NUM = 1; // 同时调小两个resource pool的unit num。4.0版本暂不支持调小。
obclient> ALTER RESOURCE TENANT tt UNIT_NUM = 3; // 同时调大两个resource pool的unit num
```
