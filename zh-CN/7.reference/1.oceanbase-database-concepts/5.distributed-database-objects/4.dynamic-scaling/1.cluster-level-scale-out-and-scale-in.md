...
OceanBase 数据库是分布式的数据库系统，通常由多个可用区（Zone）组成，每个可用区（Zone）内包含若干多台物理服务器（OBServer）。OceanBase 数据库的整体结构如下图所示。
