...
全能型和加密投票型副本称为 Paxos 副本，对应副本可构成 Paxos 成员组；只读型副本称为非 Paxos 副本，对应副本不可构成 Paxos 成员组。当前，V4.0.0 版本仅支持全能型副本类型。
...
```sql
locality='F@z1,F@z2,F{1},R{ALL_SERVER}@z3'
```

在该语法中，允许在副本类型之后的花括弧内限定副本的其他属性。花括弧内的数字表示对应 zone 对应副本类型的副本个数，其中：

* 对于 F、E 型副本：该值只能为 1，或者不指定（不指定视作 1）。
...
```sql
locality='F@z1,F{memstore_percent:0}@z2,F{1,memstore_percent:100},R{ALL_SERVER}@z3'
```
