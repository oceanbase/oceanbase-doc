...
* `<clog_file_name>` 使用详情，请参见 [dump_log 命令](#dump_log%20命令)。
...
* 目前，`filter_name` 的取值如下：
  ...
  * `table_id` 为 123，clog 文件名为 `101` 且位于当前目录，则运行以下命令解析并打印该文件。
    ...
    ```Shell
    ob_admin log_tool dump_filter 'table_id=123' 101
    ```

  * `table_id` 为 `123`，`partition_id` 为 `2`，需要解析的 clog 文件名为 `101` 且位于当前目录，则运行以下命令解析并打印该文件。

    ```Shell
    ob_admin log_tool dump_filter 'table_id=123;partition_id=2' 101
    ```
