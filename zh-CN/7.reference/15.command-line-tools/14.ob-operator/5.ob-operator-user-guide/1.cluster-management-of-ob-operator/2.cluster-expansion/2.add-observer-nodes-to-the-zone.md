...
假设当前集群中共包含 3 个可用区 zone1、zone2、zone3，每个 Zone 内包含 1 个 OBServer 节点。现在希望向每个 Zone 内添加 1 台 OBServer 节点来扩容，则扩容后，3 个 Zone 内均包含 2 台 OBServer 节点。
...
3. 通过以下命令确认操作是否成功。
  ...
  通过上述结果可看到，`zone1、zone2、zone3` 都存在 2 个 OBServer 节点，并且 READY 都为 2/2，STATUS 为 Running，则表示扩容成功。
