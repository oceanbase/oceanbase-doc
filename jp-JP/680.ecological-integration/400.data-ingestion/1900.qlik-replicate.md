...
- Qlik Replicateがインストール済みであること。（[Qlik Replicate公式ドキュメント](https://help.qlik.com/en-US/replicate)を参照してインストールしてください）
...
- SQL Serverを所有しており、そのネットワークがQlik Replicateのネットワークと正常に接続されていること。
...
1. Qlik Replicateの管理画面で、新しいソースエンドポイントを作成し、タイプとして **MySQL** を選択します。
...
**c. DSN接続をテストします：**
   ...
   isql -v MySQL-Test
   ```
接続が成功したら、Qlik Replicateの画面でDSN `MySQL-Test` を選択し、**Test Connection** をクリックします。
