...
このドキュメントでは、OceanBase Spark Catalogの設定と使用方法を迅速に完了するための手順を説明します。このチュートリアルを通じて、Spark Catalogを使用してOceanBaseデータベースにアクセスし、操作する方法を習得できます。
...
3. Spark設定ファイルを編集します。デフォルトは `$SPARK_HOME/conf/spark-defaults.conf` です。
    ...
    **例：**
    ...
    <main id="notice" type='explain'>
      <h4>説明</h4>
      <p>例となっているIPアドレスとパスワードはマスキングされています。検証時にはご自身のデータベースの実環境に合わせて記入してください。</p>
    </main>
...
5. その後、Spark SQL CLIに `use ob;` を入力して、手順3で設定したOceanBase Catalogに切り替えます。
...
#### Spark-SQLを使用してOceanBaseデータベーステーブルを作成する
...
* 実行が成功すると、OBClientを使用してOceanBaseデータベースに接続すると、`orders` テーブルが作成されていることが確認できます。
...
#### Spark-SQLを使用してOceanBaseデータベーステーブルにデータを書き込む

Spark SQL CLIで、以下の書き込みステートメントを実行します：
...
#### Spark-SQLを使用してOceanBaseデータベーステーブルをクエリする

Spark SQL CLIで、上記で書き込んだデータをクエリします。
...
#### HiveテーブルのデータをOceanBaseデータベースに同期する
...
OBClientを使用してOceanBaseデータベースに接続し、`orders` テーブルのデータをクエリすると、データが正常に同期されていることが確認できます。
