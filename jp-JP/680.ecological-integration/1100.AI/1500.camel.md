...
CamelAIは、チーム内のデータ連携の仕方を革命的に変えます。自然言語で質問するだけで、即座に正確なSQLクエリ、インテリジェントな分析、そして可視化された結果を得ることができます。
...
* ベクトル検索機能を有効にするために、テナントで`ob_vector_memory_limit_percentage`パラメーターが設定されていることを確認してください。推奨設定値は`30`ですが、このパラメーターをより正確に設定する必要がある場合は、[ob_vector_memory_limit_percentage](../../700.reference/800.configuration-items-and-system-variables/100.system-configuration-items/400.tenant-level-configuration-items/6150.ob_vector_memory_limit_percentage.md)を参照して値を計算してください。
