...
<!-- この例でのディレクトリ名は`cline-generate`です。 -->
...
3. 設定ファイルに情報を入力します。
    ...
    前のステップで開いた`cline_mcp_settings.json`ファイルに、以下の設定情報を入力し、ファイルを保存します。`/path/to/your/mcp-oceanbase`を`mcp-oceanbase`フォルダの絶対パスに、`OB_HOST`、`OB_PORT`、`OB_USER`、`OB_PASSWORD`、`OB_DATABASE`をご自身のデータベースの対応する情報に置き換える必要があります。
