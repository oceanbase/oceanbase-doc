...
golang-migrateはGoエコシステムで広く使われているデータベースマイグレーションツールであり、SQLスクリプトに基づいてデータベースオブジェクトのバージョン移行とロールバックをサポートします。本ドキュメントでは、golang-migrateを使用してOceanBaseデータベースのMySQLモードテナントに接続し、マイグレーションの作成、実行、検証、およびロールバックを完了する方法について説明します。
...
このドキュメントでは、golang-migrate V4.15.2バージョンのインストールを例に説明します。操作は以下のとおりです：
