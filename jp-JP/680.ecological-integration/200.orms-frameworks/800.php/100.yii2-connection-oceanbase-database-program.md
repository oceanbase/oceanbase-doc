...
Yii 2.0 は、高性能な PHP フレームワークであり、強力な ORM 機能を提供します。Yii 2.0 の ActiveRecord クラスは、データベース操作に対して洗練されたソリューションを提供し、OceanBase データベースとの互換性を含む、さまざまなデータベースシステムをサポートしています。

Yii 2.0 ORM の主な特徴：

* モデル定義、CRUD 操作、関連関係を含む完全な ORM サポート
* MySQL、PostgreSQL、SQLite など、さまざまなデータベースシステムをサポート
    ...
    class UserController extends Controller
    {
        public function actionIndex()
        {
            ...
            try {
                ...
                Yii::info('ページネーションクエリ結果: ' . count($users) . '件');
...
3. `runtime/logs/app.log` ファイルを確認すると、以下のようなログ出力が見られるはずです：
...
4. 応答結果の例：
    ...
    ```json
    {
        "success":true,"message":"操作に成功しました","data":{"userCount":6,"sampleUser":{"id":1,"username":"example_user1749628918","email":"user1749628919@example.com","age":31,"created_at":1749628919}}
    }
    ```

## 関連ドキュメント

* [Yii 2.0公式ドキュメント](https://www.yiiframework.com/doc/guide/2.0/en)
* [Yii 2.0データベースアクセスオブジェクト](https://www.yiiframework.com/doc/guide/2.0/en/db-dao)
* [OceanBaseデータベースドキュメント](https://en.oceanbase.com/docs/oceanbase-database)
* [PHP公式ドキュメント](https://www.php.net/manual/en/index.php)