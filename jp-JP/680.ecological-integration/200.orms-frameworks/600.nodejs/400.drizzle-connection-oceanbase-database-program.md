|description|本ドキュメントでは、Node.jsのDrizzle ORMフレームワークを使用してOceanBaseデータベースに接続し、テーブルの作成、データの挿入、クエリなどの基本的な操作を実行する方法について説明します。|
|---|---|
...
Drizzle ORMは軽量で型安全なTypeScript ORMです。本ドキュメントでは、Drizzle ORMを使用してOceanBaseデータベースのMySQLモードに接続する方法を紹介します。
...
* `$host`：OceanBaseデータベースへの接続IPアドレスを提供します
* `$port`：OceanBaseデータベースへの接続ポートを提供します
* `$database_name`：アクセス対象のデータベース名
* `$user_name`：テナントの接続アカウントを提供します
