...
`IUserMapper.xml` ファイルは、ユーザーオブジェクトに関連付けられた SQL ステートメントを定義するマッピングファイルです。このファイルでは、ユーザーオブジェクトに関連付けられた SQL ステートメントを格納するための `IUserMapper` というネームスペースが定義されています。
...
2. `IUserMapper.java` とのマッピング関係を設定します。
    ...
    1. Java コード内の `Mapper` インターフェースと対応する `Mapper` インターフェースのネームスペースを定義します。
