...
# DataHubを使用してOceanBaseデータソースを管理する

## 概要

DataHubは、エンドツーエンドのデータ検出、データ可観測性、およびデータガバナンスを実現するための最新のデータカタログです。
本記事では、OceanBaseをバックエンドストレージとして使用してDataHubサービスをデプロイする方法について説明し、簡単な例を通じてOceanBaseのメタデータ管理を実演します。
...
1. DataHub UIページにアクセスします：`http://localhost:9002`,登录用户名和密码为：`datahub``。
