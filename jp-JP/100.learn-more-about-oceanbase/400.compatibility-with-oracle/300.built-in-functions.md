...
この記事では、OceanBaseデータベースのOracleモードとネイティブOracleデータベースにおける組み込み関数の詳細な互換性比較情報を紹介します。

Oracleデータベース(19c)では312個の組み込み関数がサポートされており、OceanBaseデータベースでは現在177個がサポートされています。詳細については、[関数](../../700.reference/500.sql-reference/100.sql-syntax/300.common-tenant-of-oracle-mode/500.functions-of-oracle-mode/100.function-overview-of-oracle-mode.md)を参照してください。

## 数値関数

| # | Oracleデータベースの関数 | OceanBaseデータベースでサポートされているか |
|----|----------------|-------------------|
| 1  | ABS            | サポート               |
| 2  | ACOS           | サポート               |
| 3  | ASIN           | サポート               |
...
| # |    Oracleデータベースの関数    | OceanBaseデータベースはサポートしているか |
|----|---------------------|-------------------|
| 1  | CHR                 | サポート               |
| 2  | CONCAT              | サポート               |
| 3  | INITCAP             | サポート               |
...
| # | Oracleデータベースの関数 | OceanBaseデータベースはサポートしていますか |
|----|---------------|-------------------|
| 1  | GREATEST      | サポート               |
| 2  | LEAST         | サポート               |
...
| # |    Oracleデータベースの関数     | OceanBaseデータベースはサポートするか |
|----|----------------------|-------------------|
| 1  | ASCIISTR             | サポート               |
| 2  | CAST                 | サポート               |
| 3  | CHARTOROWID          | サポート               |
...
| # | Oracleデータベースの関数 | OceanBaseデータベースがサポートするか |
|----|---------------|-------------------|
| 1  | DECODE        | サポート               |
| 2  | DUMP          | サポート               |
| 3  | ORA_HASH      | サポート               |
...
| 9   | XMLTABLE  | サポート |
| 10  | XMLSEQUENCE | サポート |
| 11  | INSERTCHILDXML | サポート |
| 12  | DELETEXML | サポート |

## 集計関数

| # |        Oracleデータベースの関数         | OceanBaseデータベースはサポートされていますか |
|----|------------------------------|-------------------|
...
<main id="notice" type='explain'>
  <h4>説明</h4>
  <p><code>*</code> は、この関数が集計関数でありながら分析関数でもあることを示します。</p>
</main>
...
<main id="notice" type='explain'>
  <h4>説明</h4>
  <p>集計関数の表に<code>*</code>とマークされている関数は、同時に分析関数にも属しているため、この表では重複して記載されていません。</p>
</main>