...
これにより、OracleデータベースからOceanBaseデータベースへの移行に際して、ユーザーは新しい知識を習得するための時間を大幅に節約でき、スムーズに移行できることを意味します。

ただし、OceanBaseデータベースとOracleデータベースは、基盤アーキテクチャの違いにより、一部の機能ではOceanBaseデータベースが一時的に互換性を提供しない場合があるか、Oracleデータベースとの動作に違いが見られる場合があります。

この記事では、次のポイントからOceanBaseデータベースとOracleデータベースの互換性を比較します。
...
OceanBaseデータベースはをヒント句をサポートしています。Hint に関する詳細な説明は、[Hint](https://en.oceanbase.com/docs/common-oceanbase-database-10000000001976301) を参照してください。
...
<!-- 以上内容的日文翻译，与oracle官网的英文和日文目录一一对应：https://docs.oracle.com/cd/G11854_01/lnpls/user-defined-pl-sql-subtypes.html 对应英文 https://www.oceanbase.com/docs/common-oceanbase-database-cn-1000000003378536 -->

OceanBaseデータベースは、`NOCOPY`パラメータの動作に関して、Oracleデータベースとの互換性にいくつかの違いがあります。

* プリミティブなデータ型および純粋な「OUT」パラメーターとして渡される複雑なデータ型は、Oracleデータベースにおける参照渡しの動作とは互換性がありません。

* 「IN OUT」パラメータとして渡される複雑なデータ型は、Oracleの参照渡し動作と互換性があります。

tab サポートなし

OceanBaseデータベースは、以下のPL機能は現在サポートしていません。
...
<!-- * CASCADE CONSTRAINTS：https://docs.oracle.com/cd/E16338_01/server.112/b56299/statements_9003.htm -->
