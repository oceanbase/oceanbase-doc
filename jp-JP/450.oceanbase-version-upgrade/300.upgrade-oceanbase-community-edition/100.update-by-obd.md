...
obdを使用してOceanBaseデータベースをアップグレードする方法は以下の2つあります。実際の状況に応じて選択してください：

* プラン1：お使いのマシンがインターネットに接続できる場合、または設定されたイメージリポジトリに更新用のOceanBaseデータベースRPMパッケージがある場合は、オンラインアップグレードプランを参照してOceanBaseデータベースをアップグレードできます。詳細な操作手順については、**プラン1：オンラインアップグレード**を参照してください。

* プラン2：お使いのマシンがインターネットに接続できない場合、または設定されたイメージリポジトリに更新用のOceanBaseデータベースRPMパッケージがない場合は、まず`obd mirror clone`を使用して更新用のOceanBaseデータベースRPMパッケージをローカルミラーリポジトリに追加するか、OceanBaseデータベースの対応するバージョンのall-in-oneインストールパッケージをダウンロードしてインストールしてから、OceanBaseデータベースのアップグレード操作を実行します。詳細な操作手順については、**プラン2：オフラインアップグレード**を参照してください。
...
<main id="notice" type='explain'>
  <h4>説明</h4>
  <ul>
  <li><p>アップグレード対象のクラスタテナントにスタンバイテナントが存在する場合、まずスタンバイテナントが配置されているクラスタをアップグレードするか、switchoverを実行してプライマリ/スタンバイ切り替えを行う必要があります。</p></li>
...
1.  以下のコマンドを実行して、リモートイメージレジストリを有効にします。
...
3.  以下のコマンドを実行して、リモートイメージレジストリ内のOceanBaseデータベースのバージョンを確認します。
...
4.  （オプション）クラスタ内のテナントのprimary_zone設定を確認します。

   <main id="notice" type='explain'>
     <h4>説明</h4>
     <p>この手順は、対象バージョンがV4.0.x、V4.1.x、V4.2.0 BETAの場合にのみ実行してください。</p>
   </main>

   rootユーザーでデータベースのsysテナントにログインし、以下のコマンドを実行してprimary_zoneの設定を確認します。
...
5.  OceanBaseデータベースのメジャーコンパクションを実行します。
   ...
   以下のコマンドを実行して、メジャーコンパクションが完了したかどうか確認します。
...
6.  メジャーコンパクションが完了したら、以下のコマンドを実行してOceanBaseデータベースをアップグレードします。
   ...
   ここでは、クラスタ名がobtest、目標アップグレードバージョンが4.2.0の場合を例に説明します。実際のクラスタ名と目標アップグレードバージョンに置き換えてください。<!-- この手順で使用されるコマンドの詳細については、[Cluster commands](https://www.oceanbase.com/docs/common-obd-cn-1000000003415263)の`obd cluster upgrade`コマンドを参照してください。 -->

7.  アップグレードが成功したかどうかを確認します。
...
tab プラン2：オフラインでのアップグレード

1. RPMパッケージの設定

   更新用のOceanBaseデータベースのRPMパッケージをローカルミラーリポジトリに追加するには、`obd mirror clone` を使用するか、OceanBaseデータベースの対応するバージョンのall-in-oneインストールパッケージをダウンロードしてインストールする2つの方法があります。

   * 方法1：GitHubリポジトリの[releases note](https://github.com/oceanbase/oceanbase/releases)またはOceanBaseソフトウェアダウンロードセンターから必要なOceanBaseデータベースと対応する依存ライブラリのRPMパッケージを取得し、RPMパッケージをobdをインストールしたマシンにコピーします。RPMパッケージがあるディレクトリで以下のコマンドを実行し、ダウンロードしたインストールパッケージをローカルミラーリポジトリに追加します。

     ```shell
     obd mirror clone oceanbase-ce-*.rpm
     ```

   * 方法2：OceanBaseソフトウェアダウンロードセンターから必要なOceanBaseデータベースの対応するバージョンのall-in-oneインストールパッケージをダウンロードし、インストールパッケージをobdをインストールしたマシンにコピーします。以下のコマンドを実行して解凍し、インストールします。

     <main id="notice" type='notice'>
       <h4>注意</h4>
       <p>OceanBaseデータベースをデプロイした際に使用したホストアカウント（例えばadmin）を使用してall-in-oneインストールパッケージをインストールする必要があります。</p>
     </main>
...
2. 以下のコマンドを実行してリモートミラーリポジトリを無効化します。

   ```shell
   obd mirror disable remote
   ```

3. 以下のコマンドを実行して、ローカルミラーリポジトリ内のOceanBaseデータベースのバージョンを確認します。

   ```shell
   obd mirror list local | grep oceanbase-ce
   ```

   出力は以下のようになります。最終列の文字列がそのバージョンのoceanbase-ceのハッシュ値です。
...
4. （オプション）クラスタ内のテナントのprimary_zone設定を確認する
   ...
   <main id="notice" type='explain'>
     <h4>説明</h4>
     <p>この手順は、対象のバージョンがV4.0.x、V4.1.x、またはV4.2.0 BETAの場合にのみ実行する必要があります。</p>
   </main>
   ...
   出力では、sysテナント以外のテナントのprimary_zone列の値がRANDOMかどうか確認する必要があります。RANDOMの場合は、以下のコマンドを実行して、テナント内のゾーンに異なる優先順位を設定する必要があります。
