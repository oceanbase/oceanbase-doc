...
obdを使用してOceanBaseデータベースをアップグレードする方法は以下の2つです。実際の状況に応じて選択してください：

* プラン1：お使いのマシンがパブリックネットワークに接続できる場合、または設定されたイメージリポジトリに更新用のOceanBaseデータベースRPMパッケージがある場合は、オンラインアップグレードプランを参照してOceanBaseデータベースをアップグレードできます。詳細な操作手順については、**プラン1：オンラインアップグレード**を参照してください。

* プラン2：お使いのマシンがパブリックネットワークに接続できない場合、または設定されたイメージリポジトリに更新用のOceanBaseデータベースRPMパッケージがない場合は、まず`obd mirror clone`を使用して更新用のOceanBaseデータベースRPMパッケージをローカルミラーリポジトリに追加するか、OceanBaseデータベースの対応するバージョンのall-in-oneインストールパッケージをダウンロードしてインストールしてから、OceanBaseデータベースのアップグレード操作を実行します。詳細な操作手順については、**プラン2：オフラインアップグレード**を参照してください。
...
<main id="notice" type='explain'>
  <h4>説明</h4>
  <ul>
  <li><p>アップグレード対象クラスタのテナントにスタンバイテナントが存在する場合、まずスタンバイテナントが配置されているクラスタをアップグレードするか、switchoverを実行してプライマリ/スタンバイ切り替えを行う必要があります。</p></li>
  <li><p>obdはEnterprise EditionとCommunity EditionのOceanBaseクラスタのアップグレードをサポートしています。このセクションでは、Community EditionのOceanBaseクラスタのアップグレードを例に、OceanBaseクラスタのアップグレード手順を説明します。</p></li>
  </ul>
</main>

:::tab

tab プラン1：オンラインアップグレード

1. 以下のコマンドを実行して、リモートイメージリポジトリを有効にします。
...
3. 以下のコマンドを実行して、リモートイメージリポジトリ内のOceanBaseデータベースのバージョンを確認します。

   ```shell
   obd mirror list oceanbase.community.stable | grep oceanbase-ce
   ```

   出力は以下のとおりです。最終列の文字列が、oceanbase-ceの対応するバージョンのハッシュ値です。
...
4. （オプション）クラスタ内のテナントのprimary_zone構成を確認する
   ...
   <main id="notice" type='explain'>
     <h4>説明</h4>
     <p>この手順は、対象バージョンがV4.0.x、V4.1.x、およびV4.2.0 BETAの場合にのみ実行してください。</p>
   </main>

   rootユーザーでデータベースのsysテナントにログインし、以下のコマンドを実行してprimary_zoneの構成を確認します。
   ...
   出力では、sysテナント以外のテナントのprimary_zone列の値がRANDOMかどうか確認する必要があります。RANDOMの場合は、以下のコマンドを実行して、テナント内のZoneに異なる優先順位を設定する必要があります。

   ```shell
   obclient [oceanbase]> ALTER TENANT ocp primary_zone='zone1';
   ```

   ALTER TENANTコマンドの詳細については、[ALTER TENANT](../../700.reference/500.sql-reference/100.sql-syntax/100.system-tenants/500.alter-tenant.md) を参照してください。

5. OceanBaseデータベースのメジャーコンパクションを実行する

   rootユーザーでデータベースのsysテナントにログインし、以下のコマンドを実行してメジャーコンパクションを実行します。

   ```shell
   obclient [oceanbase]> ALTER SYSTEM MAJOR FREEZE;
   ```

   以下のコマンドを実行して、メジャーコンパクションが完了したかどうかを確認します。
   ...
   `oceanbase.CDB_OB_MAJOR_COMPACTION` ビューの出力フィールドの説明については、[oceanbase.CDB_OB_MAJOR_COMPACTION](../../700.reference/700.system-views/300.system-view-of-sys-tenant/200.dictionary-view-of-sys-tenant/10000.o-cdb_ob_major_compaction-of-sys-tenant.md) を参照してください。

6. メジャーコンパクション完了後、以下のコマンドを実行してOceanBaseデータベースをアップグレードする
   ...
   ここでは、デプロイ済みクラスタ名をobtest、目標アップグレードバージョンを4.2.0として例示します。実際のデプロイ済みクラスタ名と目標アップグレードバージョンに置き換えてください。この手順で使用されるコマンドの詳細については、[クラスタコマンドグループ](https://www.oceanbase.com/docs/common-obd-cn-1000000003415263) の `obd cluster upgrade` コマンドを参照してください。

7. アップグレードが成功したかどうかを確認する
...
tab 方法2：オフラインアップグレード

1. RPMパッケージの設定

   `obd mirror clone` を使用して、更新用のOceanBaseデータベースのRPMパッケージをローカルミラーリポジトリに追加するか、OceanBaseデータベースの対応するバージョンのall-in-oneインストールパッケージをダウンロードしてインストールする2つの方法で、OceanBaseデータベースのRPMパッケージを設定できます。

   * 方法1：GitHubリポジトリの [releases note](https://github.com/oceanbase/oceanbase/releases) または [OceanBaseソフトウェアダウンロードセンター](https://en.oceanbase.com/softwarecenter) から必要なOceanBaseデータベースと対応する依存ライブラリのRPMパッケージを取得し、RPMパッケージをobdがインストールされているマシンにコピーします。RPMパッケージがあるディレクトリで以下のコマンドを実行して、ダウンロードしたインストールパッケージをローカルミラーリポジトリに追加します。

     ```shell
     obd mirror clone oceanbase-ce-*.rpm
     ```

   * 方法2：[OceanBaseソフトウェアダウンロードセンター](https://en.oceanbase.com/softwarecenter) から必要なOceanBaseデータベースの対応するバージョンのall-in-oneインストールパッケージをダウンロードし、インストールパッケージをobdがインストールされているマシンにコピーします。以下のコマンドを実行して解凍し、インストールします。

     <main id="notice" type='notice'>
       <h4>注意</h4>
       <p>All-in-oneインストールパッケージは、OceanBaseデータベースをデプロイした際に使用したホストアカウント（例：admin）を使用してインストールする必要があります。</p>
     </main>
...
2. 以下のコマンドを実行してリモートミラーリポジトリを無効化する

   ```shell
   obd mirror disable remote
   ```

3. 以下のコマンドを実行して、ローカルのイメージリポジトリ内のOceanBaseデータベースのバージョンを確認します。

   ```shell
   obd mirror list local | grep oceanbase-ce
   ```

   出力は以下のとおりです。最終列の文字列が、その oceanbase-ce バージョンに対応するハッシュ値です。
...
4. （オプション）クラスタ内のテナントのprimary_zone設定を確認する

   <main id="notice" type='explain'>
     <h4>説明</h4>
     <p>この手順は、対象バージョンがV4.0.x、V4.1.x、またはV4.2.0 BETAの場合にのみ実行してください。</p>
   </main>

   rootユーザーでデータベースのsysテナントにログインし、以下のコマンドを実行してprimary_zone設定を確認します。
   ...
   出力結果では、sysテナント以外のテナントのprimary_zone列の値がRANDOMかどうか確認する必要があります。RANDOMの場合は、以下のコマンドを実行して、テナント内のゾーンに異なる優先順位を設定する必要があります。
...
5. OceanBaseデータベースでメジャーコンパクションを実行する
   ...
   `oceanbase.CDB_OB_MAJOR_COMPACTION`ビューの出力フィールドの紹介については、[oceanbase.CDB_OB_MAJOR_COMPACTION](../../700.reference/700.system-views/300.system-view-of-sys-tenant/200.dictionary-view-of-sys-tenant/10000.o-cdb_ob_major_compaction-of-sys-tenant.md) を参照してください。

6. メジャーコンパクション完了後、以下のコマンドを実行してOceanBaseデータベースをアップグレードします。

   ```shell
   obd cluster upgrade obtest -c oceanbase-ce -V 4.2.0.0 --usable=bf178e82c99ca1324a3df9e1a21cbbb8f8c4d46c
   ```

   ここでは、デプロイ済みクラスタ名をobtest、目標アップグレードバージョンを4.2.0として例示します。実際のデプロイ済みクラスタ名および目標アップグレードバージョンに置き換えてください。この手順で使用されるコマンドの詳細については、[クラスタコマンドグループ](https://www.oceanbase.com/docs/common-obd-cn-1000000003415263) の `obd cluster upgrade` コマンドを参照してください。
