...
obvec_jdbc の JSON Table 機能は、OceanBase の JSON データ型の操作処理能力 (`JSON_VALUE` / `JSON_TABLE` / `JSON_REPLACE` など) に基づいて、仮想テーブルメカニズムを実装しています：複数のユーザー (ユーザー id で区別) は、同一の物理テーブル上で仮想テーブルに対する DDL または DML 操作を同時に実行でき、ユーザー間のデータ分離を保証します。管理者ユーザーはDDLを、一般ユーザーはDMLを操作できます。
