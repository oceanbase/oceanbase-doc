...
| 第2層 | Partitions | ≈Tablet | Milvusは、同一Shard内（Shardsは通常主キーでパーティション化される）で他の列に基づいてブロックを分割し、読み取り性能を向上させることをサポートします。<br>OceanBaseは、パーティション内で主キーに従ってソートすることで実現します。 |
...
| add_field() | シーケンスcreate_schema->add_field->...->add_fieldを使用して呼び出すこともできます。</br>または、FieldSchemaのリストを手動で作成し、CollectionSchemaのコンストラクターを使用してSchemaを作成することもできます。 |
...
| **インターフェース** | **説明** |
|---|---|
| search() | ANNクエリインターフェース：<ul><li>collection_name：テーブル名</li><li>data：クエリ対象のベクトル</li><li>filter：フィルタ操作。`WHERE`と同等です</li><li>limit：上位K件</li><li>output_fields：投影列。`SELECT`と同等です</li><li>partition_names：パーティション名（Milvus Liteはサポートしていません）</li><li>anns_field：インデックス列名</li><li>search_params：ベクトル距離関数名、インデックスアルゴリズム関連パラメータ</li></ul> |
| query() | filter句を含むクエリ、すなわち`SELECT ... WHERE ids IN (..., ...) AND <filters>`。 |
| get() | filter句を含まないクエリ、すなわち`SELECT ... WHERE ids IN (..., ...)`。 |
...
1. Milvus search()：
    ...
    * filter：str式。具体的な使用例については、https://milvus.io/docs/boolean.md#Usage のSQLの`WHERE`式を参照してください。これに類似します。
