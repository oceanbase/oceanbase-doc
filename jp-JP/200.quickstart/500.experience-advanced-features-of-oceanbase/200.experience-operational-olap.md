...
# Operational OLAPを体験する

OceanBaseデータベースは、混合ワークロードタイプのシナリオを処理できます。OceanBaseデータベースはピアノードに基づく分散アーキテクチャを採用しているため、高同時実行性と拡張性を備えたOLTPタスクを処理できるだけでなく、同一のデータエンジン内でMPPアーキテクチャに基づいてOLAPの並列計算も実行でき、二つのデータセットを管理する必要がありません。

OceanBaseデータベースでは、大量のオンライン業務データに対して直接並列分析を行うだけでなく、PDML（Parallel DML）機能を利用して、バッチ書き込みを行う大規模トランザクションを並列処理により迅速かつ安全に実行することもできます。これらはすべて、トランザクションの一貫性を厳密に保証した上で実現されます。

以下では、手動でTPC-Hベンチマークを実行し、Operational OLAPシナリオにおけるOceanBaseデータベースの特徴と使用方法を示します。TPC-Hは、意思決定支援業務に基づく業界で広く使用されているベンチマークであり、大量のデータセット上で実行される一連の複雑なクエリ要求を通じて、データベースシステムの分析および意思決定支援能力を検証します。詳細については、[TPC公式ウェブサイト](https://www.tpc.org/default5.asp)を参照してください。
...
以下では、TPC公式のTPC-Hツールを用いて、手動でTPC-Hテストを実行する手順を説明します。手動テストは、特にパラメータ設定など、OceanBaseデータベースをより深く理解するのに役立ちます。
...
2. ダウンロード完了後、ファイルを解凍し、TPC-Hの解凍ディレクトリに移動します。
...
6. ファイルをコンパイルします。
   ...
   後の手順でデータを生成するためのdbgenファイルと、SQLを生成するためのqgenおよびdists.dssファイルが生成されます。

### ステップ4：データの生成

実際の環境に応じて、TPC-H 10G、100G、または1Tのデータを生成できます。このドキュメントでは、100Gのデータを生成する例を説明します。
...
以下の例では、マルチスレッドで1Tのデータを生成します。OceanBaseはダイレクトロードをサポートしており、複数のファイルのデータを同時にテーブルにインポートできます：
...
<main id="notice" type='explain'>
  <h4>説明</h4>
  <p>このセクションの以下の手順を参考にクエリSQLを生成して調整するか、<a href="https://github.com/oceanbase/obdeploy/tree/master/plugins/tpch/4.3.0.0/queries">GitHub</a>で提供されているクエリSQLを直接使用できます。GitHubのクエリSQLを使用する場合は、SQL文内の<code>cpu_num</code>を実際の並列数に変更する必要があります。</p>
</main>
...
2. `dbgen` ディレクトリ内に `gen.sh` スクリプトを作成し、クエリSQLを生成します。
   ...
   for i in {1..22}
   do
   ./qgen ./queries/$i -s 100 > "$i".sql
   done
   ```

   `dbgen` ディレクトリ内に22件のクエリSQLを生成し、すべてのクエリSQLを `mysql_sql` フォルダにコピーします。
...
3. メジャーコンパクションが完了したかどうかを確認します。
   ...
   <main id="notice" type='explain'>
     <h4>説明</h4>
     <p>すべての <code>FROZEN_SCN</code> と <code>LAST_SCN</code> の値が等しい場合、メジャーコンパクションは完了です。</p>
   </main>
...
上記の手順で生成されたデータとSQLを使用して、ご自身でスクリプトを作成できます。テストのサンプル操作は以下のとおりです：

1. テストスクリプト `tpch.sh` を作成します。

   ```bash
   #!/bin/bash
   host='$host_ip'   # 注意！！任意のobserver、例えばobserver Aが配置されているサーバーのIPアドレスを入力してください。
   port='$host_port' # observer Aのポート番号
   user='$user'      # ユーザー名
   tenant='$tenant_name'  # テナント名
   password='$password'   # パスワード
...
* データインポート時にエラーが発生しました。エラーメッセージは以下のとおりです：
...
前の手順により、TPC-Hのテスト環境が準備できました。次に、手動で実行してOceanBaseデータベースのOLAPにおける能力と特性を確認してみましょう。
...
次に、TPC-HテストのQ1を使用してOceanBaseデータベースのクエリ能力を体験します。Q1クエリは、最大の`lineitem`テーブルに対して、指定された期間内の各種商品の価格、割引、出荷、数量などの情報を集計・分析します。このクエリは、テーブル全体のデータを読み取り、パーティショニング、ソート、集計などの計算を行います。
...
OceanBaseデータベースのOperational OLAP機能は、単一のデータセットと実行エンジンに基づいており、異種システム間でデータを同期・管理する必要はありません。次に、`parallel`ヒントを追加し、並列度8で同じステートメントを再度実行します：
...
並列処理を有効にしないデフォルトの実行時間と比較すると、パラレルクエリでは実行速度が約6倍に向上しています。`EXPLAIN`コマンドで実行計画を確認すると、並列度も表示されています（18行目、演算子1、dop=8）：
...
本記事の例は単一ノード環境にデプロイされていますが、OceanBaseデータベースのパラレル実行フレームワークの最大の特徴は、大量のデータに対する分析クエリを複数ノードでパラレル実行できる点です。例えば、1億行以上のデータを含むテーブルが複数のOceanBaseデータベースノードに分散している場合、分析クエリを実行すると、OceanBaseデータベースの分散実行フレームワークは分散パラレル実行計画を生成し、複数ノードのリソースを活用して分析を行います。そのため、優れた拡張性を備えており、並列処理の設定はSQL、セッション、テーブルなど複数のレベルで行うことが可能です。

## obdツールを使用したTPC-Hテストの自動実行

<main id="notice" >
    <h4>適用対象</h4>
    <p>この内容はOceanBaseデータベースCommunity版にのみ適用されます。OceanBaseデータベースEnterprise版は、現在obdの使用をサポートしていません。</p>
  </main>
...
上記コマンドを実行すると、obdの実行が開始され、実行プロセス中の各ステップを確認できます：
...
データインポートが完了すると、obdは自動的に22個のSQLを実行し、各SQLの実行時間と合計実行時間を出力します。