...
デフォルトの非並列実行時間と比較して、並列クエリにより速度が約6倍向上していることがわかります。また、`EXPLAIN` コマンドで実行計画を確認すると、パラレル度も表示されています（18行目、演算子1、dop=8）：
...
本記事の例では単一ノード環境でのデプロイを使用していますが、特筆すべき点として、OceanBaseデータベースのパラレル実行フレームワーク最大の特徴は、大量データの分析クエリを複数ノードで並列実行できる点です。例えば、1億行以上のデータを含むテーブルが複数のOceanBaseデータベースノードに分散している場合、分析クエリを実行する際に、OceanBaseデータベースの分散実行フレームワークは分散パラレル実行計画を生成し、複数ノードのリソースを活用して分析を行うことができます。そのため、優れた拡張性を備えており、並列処理の設定はSQL、セッション、テーブルなど複数の観点から行うことが可能です。
