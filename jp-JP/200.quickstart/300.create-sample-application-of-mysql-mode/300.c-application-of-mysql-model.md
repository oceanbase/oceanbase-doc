...
# C言語のサンプルアプリケーションを作成する

本記事では、MySQL Connector/C (libmysqlclient) ドライバーを使用してOceanBaseデータベースに接続し、操作する方法について説明します。
...
* **-h**：OceanBaseデータベースの接続IPアドレス。場合によってはODPのアドレスであることがあります。
...
### ステップ2：MySQL Connector/Cドライバをインストール

#### yumを使用してMariaDBクライアントをインストール
...
アプリケーションは、MySQL Connector/Cを使用してデータベースサーバーOBServerノードとやり取りする基本的な手順は以下のとおりです：
...
* **host_name**：`-h` パラメータから取得します。OceanBaseデータベースの接続アドレスで、場合によってはODPのアドレスであることがあります。
...
* **db_name**：`-D` パラメータから取得します。アクセスが必要なデータベース名です。

* **port_num**：`-P` パラメータから取得します。OceanBaseデータベースの接続ポート番号で、ODPのリスニングポートでもあります。
...
* `-I` オプション：コンパイラがヘッダーファイルを見つけられるように、コンパイラの検索パスを指定します。例えば、`mysql.h` ヘッダーファイルがあるディレクトリ `/usr/include/mysql` をコンパイラの検索パスに追加することで、コンパイラは `mysql.h` ヘッダーファイルを見つけることができます。`find / -name mysql.h 2>/dev/null` というコマンドを使用して `mysql.h` ファイルのパスを探すことができます。
* `-L` オプション：動的リンクライブラリの検索パスを指定します。
* `-l` オプション：リンクするライブラリファイルを指定します。`-l` オプションを使用する場合、ライブラリファイル名から `lib` プレフィックスと `.so` サフィックスを削除する必要があります。例えば、上記のコマンドではライブラリファイル名は `libmysqlclient.so` ですが、-l オプションを使用する場合は `mysqlclient` のみを指定する必要があります。
...
出力結果は以下のとおりです。このような結果が出力された場合、データベースへの接続が成功し、サンプルSQLが正しく実行されたことを意味します。
