...
* 技術サポートにお問い合わせいただき、OBCIおよびLibOBClientのRPMインストールパッケージを取得してください。

<main id="notice" type='notice'>
  <h4>注意</h4>
  <p>前提条件には、以下の点にもご注意ください：<ul><li>OBCI V2.0.4以降では、RPMパッケージの<code>include</code>フォルダには、<code>oci.h</code>や<code>ociap.h</code>などのヘッダーファイルが含まれなくなりました。業務上のコンパイルには、まずOracle clntsh関連のRPMをインストールし、そのヘッダーファイルを直接使用する必要があります。</li><li>OceanBase：V2.2.76以降のバージョン（現在のバージョンの全機能を体験するには、OceanBaseデータベースV4.0以降のバージョンを推奨します）。</li><li>ODP（OBProxy）：V4.0以降のバージョンを推奨します。</li><li>libobclient：V2.1.1以降のバージョン。</li><li>Oracle関連ドライバーファイル<code>oracle-instantclient</code>のインストール（OBCI V2.0.4以降はOracleヘッダーファイルが必要です）。</li><li>OBCIがインストール済みであり、事前にGCCコンパイラがデプロイされていること。</li></ul></p>
</main>
...
本記事では、具体的な例を通じて、OceanBaseデータベースのOracleモードにおいて、CアプリケーションがOBCIを介してデータベースサーバーOBServerノードとやり取りする基本的な方法を説明します。
...
4. SQLを使用してOceanBaseサーバーとデータをやり取りし、その後データ処理を行います。OBCIアプリケーションにおける1つのSQL文の実行手順は以下のとおりです：
   ...
   2. `OCIBindByPos()`や`OCIBindByName()`などの関数を1つまたは複数呼び出し、入力変数のアドレスをDML文内のプレースホルダーにバインドします。
   ...
   4. `OCIDefineByPos()`関数を呼び出して、SQL文内のデータ出力項目に対応する出力変数を定義します。
...
コード内のデータベース接続パラメータを修正します。以下のフィールドを参照し、対応する値は手順1で取得したデータベース接続文字列から取得します。
...
* **strServerName**：`-h` および `-P` パラメータから取得します。`IP:port` の形式です。OceanBaseデータベースの接続IPアドレスは、通常ODPアドレスと、アクセスに使用するポート番号です。
...
OBCIのインストールおよび使用方法の詳細については、公式ドキュメント「[OceanBase C言語呼び出しインターフェース](https://www.oceanbase.com/docs/enterprise/obci-cn/V2.0.2/10000000000380586)」を参照してください。