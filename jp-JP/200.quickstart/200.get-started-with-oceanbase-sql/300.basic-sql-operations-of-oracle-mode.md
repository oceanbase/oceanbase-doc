...
* テーブル `TEST` の列 `C2` のデータ型を変更します。
  ...
  2. テーブル `TEST` 内の列 `C2` のデータ型を `CHAR` に変更します。
...
* テーブル `TEST` に列を追加および削除します。
  ...
  3. 再度テーブル `TEST` の列定義を確認し、列 `C3` が正常に追加されたことを確認します。
  ...
  5. 再度テーブル `TEST` の列定義を確認し、列 `C3` が正常に削除されたことを確認します。
...
`SELECT` ステートメントの詳細な構文については、[SELECT](../../700.reference/500.sql-reference/100.sql-syntax/300.common-tenant-of-oracle-mode/900.sql-statement-of-oracle-mode/200.dml-of-oracle-mode/500.select-of-oracle-mode/100.simple-select-of-oracle-mode.md)を参照してください。
