...
`DROP TABLE` ステートメントの詳細な構文については、[DROP TABLE](../../700.reference/500.sql-reference/100.sql-syntax/200.common-tenant-of-mysql-mode/600.sql-statement-of-mysql-mode/3700.drop-table-of-mysql-mode.md)を参照してください。
...
その他の `DROP INDEX` ステートメントに関する構文説明については、[DROP INDEX](../../700.reference/500.sql-reference/100.sql-syntax/200.common-tenant-of-mysql-mode/600.sql-statement-of-mysql-mode/3500.drop-index-of-mysql-mode.md)を参照してください。
...
* テーブルから指定された行を削除します。
  ...
  2. テーブル `t2` から `c1=2` の行を削除します。ここで、`c1` 列はテーブル `t2` の `PRIMARY KEY` です。

     ```shell
     obclient(root@mysqltenant)[db1]> DELETE FROM t2 WHERE c1 = 2;
     ```

  3. 削除後のテーブル `t2` のデータを確認し、削除が成功したことを確認します。
...
* 条件に合致する行をテーブルから削除します。
  ...
  2. `c2` 列に従ってソートした後のテーブル `t2` の最初の行のデータを削除します。
...
* テーブルから指定されたパーティションのデータを削除します。
  ...
  1. 削除前のテーブル `t3` の `p2` パーティションのデータを確認します。
...
* 複数のテーブルから同時にデータを削除します。
  ...
  1. 削除前のテーブル `t2` および `t3` のデータを確認します。
...
* テーブル `t4` の `c2` 列でソートされた上位2行の `c2` 列の値を `100` に変更します。
  ...
  2. テーブル `t4` の `c2` 列でソートされた上位2行の `c2` 列の値を `100` に変更します。
...
* テーブル `t4` とテーブル `t5` の `t4.c2 = t5.c2` に対応する行のデータについて、テーブル `t4` の `c2` 列の値を `100` に、テーブル `t5` の `c2` 列の値を `200` に変更します。

  1. 更新前のテーブル `t4` と `t5` のデータを確認します。
  ...
  2. テーブル `t4` とテーブル `t5` の `t4.c2 = t5.c2` に対応する行のデータについて、テーブル `t4` の `c2` 列の値を `100` に、テーブル `t5` の `c2` 列の値を `200` に変更します。

     ```shell
     obclient(root@mysqltenant)[db1]> UPDATE t4,t5 SET t4.c2 = 100, t5.c2 = 200 WHERE t4.c2 = t5.c2;
     ```

  3. 更新後のテーブル `t4` と `t5` のデータを確認し、更新が成功したことを確認します。
...
9. 再度、テーブル `t_insert` のデータを確認します。テーブル `t_insert` のデータの変更が正常に永続化されたことが確認できます。
...
トランザクションをロールバックするとは、トランザクションによる変更をすべて取り消すことです。未コミットのトランザクション全体をロールバックすることも、トランザクション内の任意のセーブポイントまでロールバックすることもできます。特定のセーブポイントまでロールバックする場合は、`ROLLBACK` と `TO SAVEPOINT` のステートメントを組み合わせて使用する必要があります。
その中で：
...
6. ロールバック後、再度テーブル `t_insert` のデータを確認すると、挿入したデータがロールバックされていることが確認できます。
