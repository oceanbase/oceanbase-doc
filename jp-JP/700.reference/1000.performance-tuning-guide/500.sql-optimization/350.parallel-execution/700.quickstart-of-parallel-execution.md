...
```sql
-- open_dtは、オペレータがOpenからCloseまでの時間間隔を表します。
-- row_dtは、オペレータが最初の行を吐き出してからOB_ITER_ENDを吐き出すまでの時間間隔を表します。
...
-- Oracleテナント：
--説明：open_dtは、オペレータがOpenからCloseまでの時間間隔を表します。
--説明：row_dtは、オペレータが最初の行を吐き出してからOB_ITER_ENDを吐き出すまでの時間間隔を表します。
...
4. ビジネス側で一切変更できない場合、ビジネスSQLを並列化するにはどうすればよいですか？
   ...
   OBProxyでは、ユーザーがWebインターフェース上で接続設定を変更し、並列処理を有効にできます。例えば、読み書き分離接続において、すべてのSQLの並列度を2に設定します。