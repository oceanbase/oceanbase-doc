...
分散実行計画はHintを使用して管理でき、SQLクエリのパフォーマンスを向上させることができます。

分散実行フレームワークがサポートするHintには、`ORDERED`、`LEADING`、`USE_NL`、`USE_HASH`、`USE_MERGE`などが含まれます。

## PARALLEL ヒント

分散実行の並列度を指定します。次の例のように、3つのWorkerを使用してスキャンをパラレル実行します。
...
グローバルな並列度とテーブルレベルの並列度が同時に指定されている場合、テーブルレベルの並列度は有効になりません。

  <main id="notice" type='notice'>
    <h4>注意</h4>
    <p>複雑なクエリでは、スケジューラーは2つのDFOを並列的にスイムスルー実行することができます。この場合、有効になっているWorker数は並列度の2倍、つまり<code>PARALLEL * 2</code>になります。</p>
  </main>
...
手書きのSQLでは、ユーザーが `JOIN` の最適な順序を知っている場合、テーブルを `FROM` の後ろに順番に記述し、`ORDERED` ヒントを追加することで、`ORDERED` ヒントは非常に役立ちます。

## LEADING ヒント

`LEADING` ヒントは、パラレルクエリ計画で最初に `JOIN` するテーブルを指定します。`LEADING` 内のテーブルの順序は、左から右への順序であり、それが `JOIN` の順序でもあります。これは `ORDERED` よりも柔軟性が高いです。
  <main id="notice" type='notice'>
    <h4>注意</h4>
...
* `tablespec` は、対象となるテーブルを指定し、`JOIN` の右側のテーブルに注目します。
...
`USE_NL` ヒントは、`ORDERED` または `LEADING` ヒントと一緒に使用される場合、`USE_NL` で指定されたテーブルが右側のテーブルでない場合、無視されます。
...
`USE_HASH` ヒントは、`HASH JOIN` を使用することを指定し、かつ `USE_HASH` で指定されたテーブルが `JOIN` の右側のテーブルである必要があります。
  ...
  <main id="notice" type='notice'>
    <h4>注意</h4>
    <p><code>ORDERED</code> および <code>LEADING</code> ヒントを使用していない場合、かつオプティマイザーが生成した結合順序で指定されたテーブル間に直接 <code>JOIN</code> の関係がない場合、<code>USE_HASH</code> ヒントは無視されます。</p>
  </main>
...
`USE_MERGE` ヒントは、`MERGE JOIN` を使用することを指定し、かつ `USE_MERGE` で指定されたテーブルが `JOIN` の右側のテーブルである必要があります。
  ...
  <main id="notice" type='notice'>
    <h4>注意</h4>
    <p><code>ORDERED</code> および <code>LEADING</code> ヒントを使用していない場合、かつオプティマイザーが生成した結合順序で指定されたテーブル間に直接 <code>JOIN</code> の関係がない場合、<code>USE_MERGE</code> ヒントは無視されます。</p>
  </main>