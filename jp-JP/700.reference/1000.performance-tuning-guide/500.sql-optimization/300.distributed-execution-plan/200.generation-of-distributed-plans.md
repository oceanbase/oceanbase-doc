...
1. 第一段階：データの物理的配置を考慮せず、ローカル関係に基づいて最適化されたすべての実行計画を生成します。ローカル計画が生成された後、オプティマイザーはデータが複数のパーティションにアクセスしているか、またはローカルの単一パーティションテーブルにアクセスしているにもかかわらず、ユーザーがHintを使用して並列クエリの実行を強制的に採用していないかどうかをチェックします。

2. 第二段階：分散実行計画を生成します。実行計画木に基づき、データ再配置が必要な箇所に `EXCHANGE` ノードを挿入することで、元のローカル計画木を分散実行計画に変換します。

## 分散実行計画の演算子

分散実行計画を生成するプロセスは、元の計画木に適切な位置に `EXCHANGE` 演算子を挿入するプロセスです。計画木をトップダウンで走査する際、対応する演算子のデータ処理状況および入力演算子のデータパーティション状況に基づいて、`EXCHANGE` 演算子を挿入する必要があるかどうかを判断する必要があります。
...
`t1` がパーティションテーブルの場合、`TABLE SCAN` にペアの `EXCHANGE` 演算子を挿入することで、`TABLE SCAN` と `EXCHANGE OUT` を1つのJobに封じ込め、並列実行に利用できます。

### 単一入力プッシュダウン可能な演算子

単一入力プッシュダウン可能な演算子には主に `AGGREGATION`、`SORT`、`GROUP BY`、`LIMIT` 演算子などが含まれます。`LIMIT` 演算子を除き、上記に挙げた演算子には操作のキーが存在します。操作のキーと入力データのデータ分布が一致する場合、第一段階の集約操作、すなわちPartition Wise Aggregationを行うことができます。操作のキーと入力データのデータ分布が一致しない場合、第二段階の集約操作を行う必要があり、集約演算子はプッシュダウン操作を行う必要があります。
...
二項入力演算子では主に `JOIN` 演算子の場合を考慮します。`JOIN` 演算子については、主にルールに基づいて分散実行計画を生成し、データ再配置方法を選択します。`JOIN` 演算子には主に以下の3種類の結合方式があります：

* Partition-Wise Join

  左右のテーブルがどちらもパーティションテーブルで、パーティショニング方式が同じで物理的配置も同じであり、かつ `JOIN` の結合条件がパーティションキーである場合、パーティション単位の結合方法を使用できます。以下の例のようになります：
...
* Partial Partition-Wise Join
  ...
  左右のテーブルのうち一方がパーティションテーブル、もう一方が非パーティションテーブルである場合、または両方ともパーティションテーブルであるが結合キーがどちらか一方のパーティションテーブルのパーティションキーとのみ一致する場合、そのパーティションテーブルのパーティション分布を基準に、もう一方のテーブルのデータを再配置します。以下の例のようになります：
...
* データの再分散
  ...
  <main id="notice" type='notice'>
    <h4>注意</h4>
    <p>並列度が1より大きい場合にのみ、以下の例で示す2つのデータ再分散方式が選択される可能性があります。</p>
  </main>
