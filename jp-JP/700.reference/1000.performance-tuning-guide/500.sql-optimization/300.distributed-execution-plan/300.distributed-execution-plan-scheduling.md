...
実行計画の生成最終段階で、`EXCHANGE`ノードを境に複数のサブ計画に分割され、各サブ計画はDFOとしてカプセル化されます。並列度が1より大きい場合、2つのDFOが一度にスケジュールされ、DFOツリーの走査実行が順次完了します。並列度が1の場合、各DFOは生成されたデータを中間結果マネージャーに格納し、後続走査の形式でDFOツリー全体の走査実行を完了します。

## 単一DFOスケジューリング

例：並列度が1の場合、クエリ計画の実行における単一DFOスケジューリング。
...
図のように、DFOツリーはRoot DFOを除き、垂直方向に0、1、2号DFOにそれぞれ分割されます。したがって、後続走査のスケジューリング順序は0-\>1-\>2となり、計画ツリー全体の反復処理が完了します。
...
例：並列度が1より大きい計画について、クエリ計画の実行における二重DFOスケジューリング。
...
図のように、DFOツリーはRoot DFOを除き、3つのDFOに分割されます。スケジューリング時にはまず0号と1号に対応するDFOが呼び出され、0号DFOの実行完了後に1号と2号DFOが順次スケジュールされ、反復処理を経て実行が完了します。
