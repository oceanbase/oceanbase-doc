...
| 複数テーブルDML | `INSERT ALL`構文は、並列DMLをサポートしていません | `UPDATE` の並列処理をサポートします。<main id="notice" type='explain'><h4>説明</h4><p><ul><li>OceanBaseデータベースV4.3.x系では、V4.3.5以降のバージョンから <code>UPDATE</code> の並列処理がサポートされています。</li><li>OceanBaseデータベースV4.2.x系では、V4.2.5 BP3以降のバージョンから <code>UPDATE</code> の並列処理がサポートされています。</li></ul></p></main>| UPDATEと同じ | 構文はサポートされていません |
