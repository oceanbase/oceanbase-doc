...
|------|--------|--------|--------|------------|
| テーブルに外部キー/TRIGGER（トリガー）/PL UDF（ユーザー定義関数）/一意インデックスが存在する <main id="notice" type='explain'><h4>説明</h4><p>トリガーが実際に発生するかどうかは明確ではありません。トリガーがDML操作と無関係である場合、またはトリガーが発生しない場合、並列DMLが続行されます。例えば、<code>UPDATE</code>トリガーがあるテーブルに対して、<code>INSERT</code>や<code>DELETE</code>などの操作を実行する場合は並列DMLが適用されますが、<code>UPDATE</code>操作自体を実行する場合は並列DMLは適用されません。</p></main> | サポートされていません | サポートされていません | サポートされていません | サポートされていません |
| 複数テーブルDML | `INSERT ALL`構文は、並列DMLをサポートしません | 関連する`UPDATE`の並列処理はサポートします。| UPDATEと同じ | 構文はサポートしません |
| 自動インクリメント列 | 特定のシナリオでは並列DMLをサポートします。詳細は以下の例を参照してください。 | サポートされています | サポートされています | サポートされています（seq）|
| ArrayBindingバッチ最適化 | サポートされていません | サポートされていません | サポートされていません | サポートされていません |
| USER_VARIABLEの使用 | サポートされていません | サポートされていません | サポートされていません | サポートされていません |
| IGNORE | サポートされていません | サポートされていません | サポートされていません | 構文はサポートしません |
| DBLink | サポートされていません | サポートされていません | サポートされていません | サポートされていません |
