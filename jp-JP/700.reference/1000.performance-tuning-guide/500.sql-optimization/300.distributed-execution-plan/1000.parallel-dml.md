...
パラレルDMLは、パラレル実行メカニズムを利用して、大規模データベースのテーブルやインデックスに対する挿入、更新、削除などの操作の実行効率を向上させます。意思決定支援システム（DSS）向けのデータベースでは、パラレルDMLはクエリと更新機能を提供し、パラレルクエリ機能を補完します。OLTPデータベースでは、パラレルDML操作によりバッチ処理ジョブの実行が高速化されます。
...
一般的に、`ENABLE_PARALLEL_DML` ヒントと `PARALLEL` ヒントを組み合わせて使用する必要があります。ただし、ターゲットテーブルのスキーマでテーブルレベルの並列度が指定されている場合は、`ENABLE_PARALLEL_DML` ヒントのみを指定します。

次の例では、`ENABLE_PARALLEL_DML` ヒントと `PARALLEL(n)` パラメータを同時に使用して、並列度 `n` を指定しています。ここで、`n > 1` であり、並列度 `dop=2` です。
...
セッションでパラレルDMLが有効になっていても、指定されたSQLステートメント内で `DISABLE_PARALLEL_DML` ヒントを使用してパラレルDMLを無効にできます。

### セッション内でのパラレルDMLの有効化と無効化

デフォルトでは、SQL文で`PARALLEL`ヒントを使用しても、パラレルDMLは有効になりません。そのため、セッション単位でパラレルDMLを有効にする必要があります。

MySQLモードにおけるセッション内でのパラレルDMLの有効化構文は次のとおりです：
...
Oracleモードにおけるセッション内でのパラレルDMLの有効化構文は次のとおりです：

```sql
ALTER SESSION ENABLE PARALLEL DML;
```

Oracleモードでは、セッション内で強制的にパラレルDMLを有効にするには、次のSQLステートメントを実行します：
...
注意すべき点として、SQL文でパラレルDMLを有効にする場合、一般的にヒントで指定された並列度が優先され、セッション内で強制的に指定された並列度よりも優先されます。Oracleモードの例は以下のとおりです：
...
パラレルDMLを無効にすると、SQL文で`PARALLEL`ヒントを使用しても、パラレルDMLは実行されません。
セッション内でパラレルDMLを有効にすると、そのセッション内のすべてのDML文に対してパラレル実行が適用されます。SQL文が`ENABLE_PARALLEL_DML`ヒントによってパラレルDMLを有効にした場合、パラレル実行は指定された文にのみ適用されます。ただし、パラレル属性を持つテーブルがない場合や、パラレル操作の制限に違反した場合は、パラレルDMLが有効になっていても、DML操作は引き続き直列実行されます。

## 使用方法

OceanBaseデータベースは、以下のSQLステートメントのパラレル実行をサポートしています：

* `INSERT INTO SELECT`
* `UPDATE`
* `DELETE`

テーブルに以下のインデックスタイプが存在する場合、パラレル実行をサポートする必要があります：

* ローカルインデックス
* 単一パーティショングローバルインデックス
* 複数パーティショングローバルインデックス

### パラレルDMLがサポートされるシナリオ

| シナリオ | INSERT | UPDATE | DELETE | MERGE INTO |
|------|--------|--------|--------|------------|
| テーブルに外部キー/トリガー(Trigger)/PL UDF(ユーザー定義関数)/一意インデックスが存在する場合 <main id="notice" type='explain'><h4>説明</h4><p>トリガーが実際にトリガーされるかどうかは明確ではありません。トリガーがDML操作と無関係である場合、トリガーはトリガーされず、並列DMLが引き続き実行されます。例えば、<code>UPDATE</code> トリガーを持つテーブルに対して <code>INSERT</code>、<code>DELETE</code> などの操作を実行する場合、並列DMLが使用されますが、<code>UPDATE</code> 操作を実行する場合は並列DMLを使用できません。</p></main> | 不支持 | 不支持 | 不支持 | 不支持 |
| 複数テーブルDML | `INSERT ALL`構文は、並列DMLをサポートしていません | `UPDATE` のパラレル処理をサポートします。<main id="notice" type='explain'><h4>説明</h4><p><ul><li>OceanBaseデータベースV4.3.x系では、V4.3.5以降のバージョンから <code>UPDATE</code> のパラレル処理がサポートされています。</li><li>OceanBaseデータベースV4.2.x系では、V4.2.5 BP3以降のバージョンから <code>UPDATE</code> のパラレル処理がサポートされています。</li></ul></p></main>| UPDATEと同じ | 構文はサポートされていません |
| 自動インクリメント列 | 一部のシナリオでは並列DMLがサポートされており、詳細については以下の例を参照してください。 | サポート | サポート | サポート (seq) |
| ArrayBindingバッチ最適化 | 不支持 | 不支持 | 不支持 | 不支持 |
| USER_VARIABLEの使用 | 不支持 | 不支持 | 不支持 | 不支持 |
| IGNORE | 不支持 | 不支持 | 不支持 | 構文はサポートされていません |
| DBLink | 不支持 | 不支持 | 不支持 | 不支持 |
| その他 | 以下のステートメントは並列DMLをサポートしていません：<ul><li>`INSERT ... VALUES ...`</li><li>`INSERT INTO ON DUPLICATE KEY UPDATE`</li><li>`REPLACE`</li></ul> | `ON UPDATE CURRENT_TIMESTAMP`列を含むものはサポートされていません | | |

`INSERT` でテーブルに書き込む際、自動インクリメント列が存在する場合、その自動インクリメント列が主キーまたはパーティションキーであり、かつその列が指定されている場合、パラレルDMLはサポートされません。

**例：**

1. テーブル `tbl1` を作成します。このテーブルの列 `col1` は自動インクリメント列であり、主キーとして指定されています。

    ```sql
    CREATE TABLE tbl1(col1 INT AUTO_INCREMENT PRIMARY KEY, col2 INT, col3 INT);
    ```

2. テーブル `tbl2` を作成します。このテーブルの列 `col1` は自動インクリメント列です。
...
4. `INSERT` でデータを書き込む際、自動インクリメント列 `col1` が主キーであるため、パラレルDMLはサポートされません。

    ```sql
    INSERT /*+enable_parallel_dml parallel(3)*/ INTO tbl1 SELECT * FROM tbl3;
    ```

5. `INSERT` でデータを書き込む際、列 `col1` が指定されていないため、パラレルDMLはサポートされます。

    ```sql
    INSERT /*+enable_parallel_dml parallel(3)*/ INTO tbl1(col2, col3) SELECT col2, col3 FROM tbl3;
    ```

6. `INSERT` でデータを書き込む際、列 `col1` は単なる自動インクリメント列であるため、パラレルDMLはサポートされます。
...
<main id="notice" type='explain'>
  <h4>説明</h4>
  <p>OceanBaseデータベースV4.2.5では、パラレルDMLはV4.2.5 BP3バージョンから、複数テーブルの結合(<code>JOIN</code>)を使用して関連更新機能をサポートしています。</p>
</main>

関連更新とは、`UPDATE` ステートメントで複数テーブルの結合操作を使用し、関連テーブルのデータに基づいてターゲットテーブルを更新することを指します。パラレルDML機能を利用することで、大規模なデータ更新操作のパフォーマンスを大幅に向上させることができます。

**例：**

以下の手順は、パラレルDMLを使用して関連更新機能を実現する方法を示しています。

1. テスト用のテーブル `customers` と `orders` を作成します。
...
3. パラレルDMLによる関連更新を実行します。
    ...
    `EXPLAIN` を使用して、パラレルDMLによる関連更新の実行計画を確認します。`PARALLEL` ヒントを使用して並列度を4に指定し、`customers` テーブル内の顧客ランク(`customer_level`)に基づいて、`orders` テーブル内の割引フィールド(`discount`)を更新します。
