...
## 特定期間内でリクエスト数が多い上位N件のSQLを照会する

以下のステートメントを実行すると、特定期間内でリクエスト数が多い上位N件のSQLを照会できます。
...
## 特定期間内で平均RTが長い上位N件のSQLを照会する

以下のステートメントを実行すると、特定期間内で平均RTが長い上位N件のSQLを照会できます。
...
## すべてのSQLから平均実行時間が長い上位N件のSQLを照会する

以下のステートメントを実行すると、すべてのSQLから平均実行時間が長い上位N件のSQLを照会できます。
