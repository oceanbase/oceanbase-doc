...
実行計画とは、データベースにおけるSQLクエリ文の実行プロセスを記述したものです。ユーザーは `EXPLAIN` コマンドを使用して、オプティマイザーが指定されたSQLに対して生成した論理実行計画を確認できます。

特定のSQLのパフォーマンス問題を分析する場合、通常はまずそのSQLの実行計画を確認し、各実行ステップに問題がないか調査する必要があります。したがって、実行計画を理解することはSQL最適化の前提条件であり、実行計画の演算子を理解することが `EXPLAIN` コマンドを理解する鍵となります。

## EXPLAINコマンドの形式

OceanBaseデータベースの実行計画コマンドには3つのモードがあります：`EXPLAIN BASIC`、`EXPLAIN`、`EXPLAIN EXTENDED`。これら3つのモードは、実行計画に表示される詳細レベルが異なります：

* `EXPLAIN BASIC` コマンドは、最も基本的な計画表示に使用されます。

* `EXPLAIN EXTENDED` コマンドは、最も詳細な計画表示に使用されます（通常は問題の調査時にこの表示モードを使用します）。
...
dml_statement:
  ...
  | INSERT statement
  | UPDATE statement
/* Oracleモードでのみサポートされます */
  | MERGE statement
/* MySQLモードでのみサポートされます */
  | REPLACE statement
```

`EXPLAIN` コマンドは `SELECT`、`DELETE`、`INSERT`、`REPLACE`、`UPDATE` ステートメントに適用され、オプティマイザーが提供するステートメント実行計画に関する情報を表示します。これには、ステートメントの処理方法、テーブルの結合方法、結合の順序などの情報が含まれます。

一般的に、`EXPLAIN EXTENDED` コマンドを使用することで、テーブルスキャンの範囲区間を表示できます。`EXPLAIN OUTLINE` コマンドを使用すると、アウトライン情報を表示できます。

`FORMAT` オプションを使用して出力形式を選択できます。`TRADITIONAL` はテーブル形式で出力を表示することを意味し、これがデフォルト設定です。`JSON` は `JSON` 形式で情報を表示することを意味します。

`EXPLAIN PARTITIONS` を使用して、パーティションテーブルを含むクエリをチェックすることもできます。非パーティションテーブルに対するクエリをチェックする場合はエラーが発生しませんが、`PARTITIONS` 列の値は常に `NULL` になります。

特に、ユーザーは `INTO table_name` SQLステートメントを使用して、`EXPLAIN` の計画情報を指定したテーブルに保存できます。デフォルトでは `PLAN_TABLE` テーブルに保存されます。`SET statement_id` ステートメントを使用すると、現在のクエリに文字列タグを付加し、後からそのSQLの計画情報を検索しやすくすることができます。デフォルトでは空文字列が情報タグとして使用されます。詳細については、[EXPLAIN](../../../500.sql-reference/100.sql-syntax/200.common-tenant-of-mysql-mode/600.sql-statement-of-mysql-mode/5300.explain-of-mysql-mode.md)を参照してください。

複雑な実行計画については、`PRETTY` または `PRETTY_COLOR` オプションを使用して、計画ツリー内の親ノードと子ノードを線または色付きの線で結び、実行計画の表示をより読みやすくすることができます。例：
...
データベースシステムでは、実行計画は内部では通常ツリー形式で表現されますが、データベースによってユーザーに表示する方法は異なります。

以下の例は、それぞれPostgreSQLデータベース、Oracleデータベース、OceanBaseデータベースにおけるTPCDS Q3の計画表示です。
...
* Oracleデータベースの実行計画は以下のように表示されます：
...
この例から、OceanBaseデータベースの計画表示はOracleデータベースと類似していることがわかります。

OceanBaseデータベースの実行計画における各列の意味は以下のとおりです：

|     列名      |             含義             |
|-------------|----------------------------|
| `ID`        | 実行木を先順走査した際に得られる番号（0から始まる）。 |
| `OPERATOR`  | 操作演算子の名前。                   |
| `NAME`      | 対応するテーブル操作のテーブル名（インデックス名）。             |
| `EST. ROWS` | その操作演算子の出力行数の推定値。              |
| `COST`      | その操作演算子の実行コスト（マイクロ秒）。            |

  <main id="notice" type='explain'>
    <h4>説明</h4>
    <p>テーブル操作において、<code>NAME</code> フィールドには、その操作に関連するテーブル名（エイリアス）が表示されます。インデックスを使用してアクセスする場合は、名前の後の括弧内にそのインデックス名が表示されます。例えば、<code>t1(t1_c2)</code> は <code>t1_c2</code> インデックスが使用されていることを示します。スキャンの順序が逆順である場合は、後ろに <code>REVERSE</code> キーワードで識別されます。例：<code>t1(t1_c2,REVERSE)</code>。</p>
  </main>

OceanBaseデータベースの `EXPLAIN` コマンドの出力の最初の部分は、実行計画のツリー構造の表示です。ツリー内での各操作の階層は、`operator` 内のインデントによって示されます。階層が深いものが優先的に実行され、階層が同じ場合は特定の演算子の実行順序に従って実行されます。

上記TPCDS Q3の例の計画表示ツリーは以下のとおりです：![p181620](https://obbusiness-private.oss-cn-shanghai.aliyuncs.com/doc/img/observer/V4.0.0/sql-execution-plan-1.png)

OceanBaseデータベースの `EXPLAIN` コマンドの出力の2番目の部分は、各操作演算子の詳細情報です。これには出力式、フィルタ条件、パーティション情報、および各演算子固有の情報（ソートキー、結合キー、プッシュダウン条件など）が含まれます。例：
...
OceanBaseデータベースのEXPLAIN情報をよりよく理解するためには、[実行計画演算子](200.execution-plan-operator/100.table-scan.md)を参照してください。
