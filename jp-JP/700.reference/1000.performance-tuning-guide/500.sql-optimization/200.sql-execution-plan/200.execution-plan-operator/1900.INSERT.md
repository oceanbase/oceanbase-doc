...
`INSERT` 演算子は、指定されたデータをテーブルに挿入するために使用されます。データの出典として、直接指定された値とサブクエリの結果が含まれます。
...
以下の例のように、Q1 クエリは値 (1, '100') を非パーティションテーブル `t1` に挿入します。ここで、1番目の演算子 `EXPRESSION` は定数式の値を生成するために使用されます。
...
上記の例では、実行計画表示の `outputs & filters` に `INSERT` 演算子の出力情報が詳細に記載されています。

|  **情報名**  |                                **意味**                                |
|------------|----------------------------------------------------------------------|
| output     | この演算子が出力する式。                                                           |
| filter     | この演算子におけるフィルター条件。例では `INSERT` 演算子に `filter` が設定されていないため、`nil` です。 |
| columns    | 挿入操作に関連するデータテーブルの列。                                                        |
| partitions | 挿入操作に関連するデータテーブルのパーティション（非パーティションテーブルは、パーティションが1つしかないパーティションテーブルと見なすことができます）。                               |
...
* Q3 クエリはサブクエリ `SELECT * FROM t3` の結果をテーブル `t1` に挿入します。
...
以下の例のように、Q5 クエリは値（2, '200'）、（3, '300'）をパーティションテーブル `t2` に挿入します。`partitions` から、これらの値が `t2` の `p0` および `p6` パーティションに挿入されることがわかります。
...
上記の例の実行計画表示の `outputs & filters` には `DISTRIBUTED INSERT` 演算子の情報が詳細に記載されており、フィールドの意味は `INSERT` 演算子と同じです。

`DISTRIBUTED INSERT` 演算子のその他の例は以下のとおりです：

* Q6 クエリは、サブクエリ `SELECT * FROM t3` の結果をパーティションテーブル `t2` に挿入します。サブクエリの結果セットが不確定なため、データは `t2` の `p0` から `p9` までのいずれかのパーティションに挿入される可能性があります。3番目の演算子からわかるように、ここでは `SELECT * FROM t3` がサブクエリとして配置され、サブクエリに `VIEW1` という名前が付けられます。OceanBaseデータベースがSQLを内部リライトしてサブクエリを生成すると、自動的にサブクエリに名前を付け、サブクエリの生成順に `VIEW1`、`VIEW2`、`VIEW3` と名付けます。
...
* Q7 クエリは、値 (1, '100') を非パーティションテーブル `t3` に挿入します。`t3` は本来非パーティションテーブルですが、`t3` にグローバルインデックス `idx_t3_c2` が存在するため、この挿入操作では複数のパーティションが関与します。
