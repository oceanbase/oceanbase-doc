...
`EXCHANGE` 演算子は、スレッド間でデータをやり取りするための演算子です。

`EXCHANGE` 演算子は分散シナリオで使用され、通常はペアで現れます。データソース側に `OUT` 演算子があり、ターゲット側に `IN` 演算子が配置されます。

## EXCH-IN/OUT

`EXCH-IN/OUT`、すなわち `EXCHANGE IN/ EXCHANGE OUT` は、複数のパーティションに分散されたデータを集約し、クエリが実行されているマスターノードに送信するために使用されます。

以下の例のように、このクエリでは5つのパーティション（`p0-p4`）のデータにアクセスしています。ここで、演算子1は演算子2が生成した出力を受け取り、データを出力します。演算子0は複数のパーティション上の演算子1が生成した出力を受け取り、結果を集約して出力します。
...
| filter         | この演算子におけるフィルター条件です。例では `EXCH-IN/OUT` 演算子に `filter` が設定されていないため、`nil` となります。 |

## EXCH-IN/OUT (REMOTE)

`EXCH-IN/OUT (REMOTE)` 演算子は、リモートのデータ（単一パーティションのデータ）をローカルにプルするために使用されます。

以下の例のように、Aマシンで非パーティションテーブルを作成し、Bマシンでクエリを実行してそのテーブルのデータを読み取ります。この場合、読み取るデータがリモートにあるため、実行計画では0番演算子と1番演算子が割り当てられ、リモートのデータを取得します。1番演算子はAマシンで実行され、`t14` テーブルのデータを読み取ってデータを送信します。0番演算子はBマシンで実行され、1番演算子が生成した出力を受信します。
...
上記の例の実行計画表示では、`outputs & filters` に `EXCH-IN/OUT (REMOTE)` 演算子の出力情報が詳細に記載されています。フィールドの意味は `EXCH-IN/OUT` 演算子と同じです。

## EXCH-IN/OUT (PKEY)

`EXCH-IN/OUT (PKEY)` 演算子はデータの再パーティショニングに使用されます。通常、二項演算子内で使用され、一方の子ノードのデータをもう一方の子ノードのパーティショニング方式に従って再パーティション化します。

以下の例では、このクエリは2つのパーティションテーブルのデータを結合するものです。実行計画では、`s15` テーブルのデータを `t13` テーブルのパーティショニング方式に従って再パーティション化します。4番目の演算子の入力は `s15` テーブルのスキャン結果であり、`s15` テーブルの各行に対して、この演算子は `t13` テーブルのデータパーティションとクエリの結合条件に基づき、その行のデータをどのノードに送信するかを決定します。

また、3番目の演算子が `EXCHANGE IN DISTR` であることがわかります。これは特殊な `EXCHANGE IN` 演算子であり、複数のパーティションのデータを集計する際に一定のマージソートを行います。この実行計画では、3番目の演算子が受け取る各パーティションのデータは `c1` 列で順序付けられており、受け取った各データに対してマージソートを行うことで、結果出力も `c1` 列で順序付けられることを保証します。
...
| PX COORDINATOR | 特殊な `EXCHANGE IN` 演算子であり、リモートデータの取得に加えて、サブプランの実行スケジューリングも担当します。 |
|       Output   | この演算子が出力する式です。                                                            |
|     filter     | この演算子におけるフィルター条件です。例では `EXCH-IN/OUT(PKEY)` 演算子に `filter` が設定されていないため、`nil` となります。 |
|    pkey        | どの列に基づいて再パーティション化を行うかを指定します。例えば、`#keys=1, [s15.c1]` は `c1` 列に基づいて再パーティション化を行うことを意味します。                |

## EXCH-IN/OUT (HASH)

`EXCH-IN/OUT (HASH)` 演算子は、データに対して一連のハッシュ関数を適用して再パーティション化します。

以下の例に示す実行計画では、3-5番および7-8番がハッシュ再パーティション化を使用する `EXCHANGE` 演算子の2組です。これら2組の演算子の役割は、`t12` テーブルと `s14` テーブルのデータを新しいハッシュ関数に基づいて複数の部分に分散させることです。例では、ハッシュ列は `t12.c2` と `s14.c2` であり、これにより `c2` 列の値が同じ行が同じ部分に配置されることが保証されます。再パーティション化後のデータに基づき、2番目の演算子 `HASH JOIN` は、各データ部分に対して `t12.c2= s14.c2` の条件で結合を行います。

また、クエリで並列度2が実行されているため、計画には `dop = 2` が表示されています（DOPはDegree of Parallelismの略です）。
...
その中で、`PX PARTITION ITERATOR` 演算子はパーティション粒度でデータを反復処理するために使用されます。詳細については、[GI](../200.execution-plan-operator/2400.GI.md)を参照してください。

上記の例の実行計画表示における `outputs & filters` は、`EXCH-IN/OUT (HASH)` 演算子の出力情報を詳細に列挙しています：

|  情報名  |                                   意味                                        |
|-----------|----------------------------------------------------------------------------------|
| PX COORDINATOR | 特殊な `EXCHANGE IN` 演算子であり、リモートデータの取得に加えて、サブプランの実行スケジューリングも担当します。 |
| Output | この演算子が出力する式です。                                                                 |
| filter | この演算子におけるフィルター条件です。例では `EXCH-IN/OUT (HASH)` 演算子に `filter` が設定されていないため、`nil` となります。 |
| pkey   | ハッシュ再パーティショニングを行う列です。例えば、`#keys=1, [s14.c2]` は `c2` 列に基づいてハッシュ再パーティショニングを行うことを示します。           |

## EXCH-IN/OUT (BC2HOST)

`EXCH-IN/OUT (BC2HOST)` 演算子は、入力データに対して `BC2HOST` のメソッドを用いて再パーティション化を行い、データを他のスレッドにブロードキャストします。

以下の例の実行計画では、3-4番が `BC2HOST` 再パーティション化方式を使用する `EXCHANGE` 演算子の一組です。これは `t11` テーブルのデータを各スレッドにブロードキャストし、`s13` テーブルの各パーティションのデータは、ブロードキャストされた `t11` テーブルのデータと結合を試みます。
...
上記の例の実行計画表示では、`outputs & filters` に `EXCH-IN/OUT (BC2HOST)` 演算子の情報が詳細に記載されており、フィールドの意味は `EXCH-IN/OUT` 演算子と同じです。