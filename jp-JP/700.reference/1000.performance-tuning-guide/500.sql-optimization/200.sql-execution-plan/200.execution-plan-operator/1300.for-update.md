...
以下の例では、Q1クエリは`t1`テーブル内で`c1 = 1`を満たす行に対してロックをかけます。`t1`テーブルは単一パーティションのテーブルであるため、オペレーター1は`FOR UPDATE`オペレーターを生成します。
...
上記の例では、Q1クエリの実行計画表示における`outputs & filters`詳細列に、`FOR UPDATE`オペレーターの出力情報が次のように詳細に示されています：
