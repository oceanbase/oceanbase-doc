...
OceanBase データベースでサポートされている `FOR UPDATE` 演算子には、`FOR UPDATE` と `MULTI FOR UPDATE` があります。
...
`FOR UPDATE` は、単一のテーブル（またはパーティション）にロックをかけるために使用されます。

以下の例では、Q1 クエリは `t1` テーブルの `c1 = 1` を満たす行にロックをかけます。`t1` テーブルは単一パーティションのテーブルであるため、1番目の演算子が `FOR UPDATE` 演算子を生成します。
...
`MULTI FOR UPDATE`は、複数のテーブル（またはパーティション）に対するロック操作に使用されます。

以下の例では、Q2クエリは`t1`と`t2`の2つのテーブルのデータをロックします。ロック対象は`c1 = 1 AND c1 = d1`を満たす行です。複数のテーブルの行をロックする必要があるため、1番目の演算子は`MULTI FOR UPDATE`です。
...
上記の例で、Q2クエリの実行計画表示における`outputs & filters`は、`MULTI FOR UPDATE`演算子の情報を詳細に示しています。
