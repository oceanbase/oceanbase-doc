...
`DELETE` 演算子は、データテーブル内で指定された条件を満たすデータ行を削除するために使用されます。

OceanBaseデータベースがサポートする `DELETE` 演算子には、`DELETE` と `DISTRIBUTED DELETE` が含まれます。

## DELETE

`DELETE`演算子は、データテーブルの単一パーティション内のデータを削除するために使用されます。

以下の例のように、Q1クエリはテーブル`t1`から`c2 > '100'`を満たすすべての行を削除します。
...
上記の例では、実行計画表示の`outputs & filters`列に`DELETE`演算子の出力情報が詳細に示されています。

|   **情報名**    |                                                 **意味**                                                       |
|---------------|-------------------------------------------------------------------------------------------------------------------|
| output        | この演算子が出力する式。    |
| filter        | この演算子におけるフィルタ条件。例では`DELETE`演算子に`filter`が設定されていないため、`nil`となります。削除文の場合、`WHERE`の述語はベーステーブルにプッシュダウンされます。例えば、Q1クエリ内の`c2>'100'`は、1番目の演算子にプッシュダウンされました。 |
| table_columns | 削除操作に関連するデータテーブルの列。     |

その他の`DELETE`演算子の例は次のとおりです：

* Q2クエリは、テーブル`t1`からすべてのデータ行を削除します。

* Q3クエリは、パーティションテーブル`t2`から`c1 = 1`を満たすデータ行を削除します。

* Q4クエリは、パーティションテーブル`t2`から`c2 > '100'`を満たすデータ行を削除します。実行計画からわかるように、`DELETE`演算子は`EXCHANGE`演算子の下に配置されているため、2番目と3番目の演算子はパーティション単位でスケジュールされるタスクとして処理されます。計画の実行時、3番目の演算子は`t2`テーブルのあるパーティションから`c2 > '100'`を満たすデータをスキャンし、2番目の演算子`DELETE`は対応するパーティションでスキャンされたデータのみを削除します。
...
以下の例のように、Q5クエリはテーブル `t3` 内で `c2 > '100'` を満たすすべてのデータ行を削除します。`t3` は非パーティションテーブルですが、`t3` にはグローバルインデックス `idx_t3_c2` が存在するため、各データ行は複数のパーティションにまたがって存在します。
...
上記の例の実行計画表示では、`outputs & filters` に `DISTRIBUTED DELETE` 演算子の情報が詳細に示されており、フィールドの意味は `DELETE` 演算子と同じです。