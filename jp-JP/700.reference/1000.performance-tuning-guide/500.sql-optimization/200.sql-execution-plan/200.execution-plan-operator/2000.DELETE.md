...
`DELETE` 演算子は、データテーブルから指定された条件に合致するデータ行を削除するために使用されます。

OceanBase データベースでサポートされている `DELETE` 演算子には、`DELETE` と `DISTRIBUTED DELETE` があります。

## DELETE

`DELETE` 演算子は、データテーブルの単一パーティション内のデータを削除するために使用されます。

以下の例のように、Q1 クエリは `t1` テーブルから `c2 > '100'` を満たすすべての行を削除します。
...
上記の例では、実行計画表示の `outputs & filters` が `DELETE` 演算子の出力情報を詳細に示しています：

|   **情報名**    |                                                 **意味**                                                       |
|---------------|-------------------------------------------------------------------------------------------------------------------|
| output        | この演算子が出力する式です。    |
| filter        | この演算子のフィルター条件です。例では `DELETE` 演算子に `filter` が設定されていないため、`nil` となります。削除文では、`WHERE` 句の述語がベーステーブルにプッシュダウンされます。例えば、Q1 クエリの `c2>'100'` は、1 番目の演算子にプッシュダウンされます。 |
...
* Q4 クエリは、パーティションテーブル `t2` から `c2 > '100'` を満たすデータ行を削除します。実行計画からわかるように、`DELETE` 演算子は `EXCHANGE` 演算子の下に配置されているため、演算子番号2と3はパーティション単位でスケジュールされるタスクとして実行されます。計画の実行時、演算子番号4が `t2` テーブルのうち `c2 > '100'` を満たすデータを1つのパーティションでスキャンし、演算子番号3の `DELETE` 演算子は対応するパーティションでスキャンされたデータのみを削除します。
...
以下の例のように、Q5 クエリはテーブル `t3` 内の `c2 > '100'` を満たすすべてのデータ行を削除します。`t3` は本来非パーティションテーブルですが、`t3` にグローバルインデックス `idx_t3_c2` が存在するため、各行は複数のパーティションにまたがって存在します。
...
上記の例の実行計画表示における `outputs & filters` は、`DISTRIBUTED DELETE` 演算子の情報を詳細に示しています。フィールドの意味は `DELETE` 演算子と同じです。