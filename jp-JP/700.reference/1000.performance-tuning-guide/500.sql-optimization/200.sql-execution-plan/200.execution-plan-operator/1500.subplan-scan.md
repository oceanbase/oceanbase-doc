...
`SUBPLAN SCAN` 演算子は、オプティマイザーがどのビューからデータをアクセスしたかを示すために使用されます。
...
以下の例では、Q1 クエリの演算子1はビュー内のクエリによって生成され、演算子0である `SUBPLAN SCAN` が演算子1を読み取り、出力します。
...
|----------|-------------------------------------------------------------------|
| output   | この演算子が出力する式。                                                     |
| filter   | この演算子のフィルター条件。例えば `filter([v.c1 > 0])` の `v.c1 > 0` など。 |
| access   | この演算子がサブノードから読み取る必要がある列名。                                       |

`FROM TABLE` がビューであり、かつクエリが特定の条件を満たす場合、ビュー結合によるクエリのリライトが可能となり、その際には実行計画に `SUBPLAN SCAN` は現れません。以下の例のように、Q2 クエリは Q1 クエリに比べてフィルター条件が削減されているため、`SUBPLAN SCAN` 演算子を割り当てる必要がなくなります。
