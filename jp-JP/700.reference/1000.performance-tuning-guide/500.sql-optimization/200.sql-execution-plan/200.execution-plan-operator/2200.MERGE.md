...
以下の例のように、Q1 クエリは `src_tbl` テーブルのデータ行を `t1` テーブルにマージします。`src_tbl` の各行について、以下の方法でマージが行われます：
...
ここで、`OUTER JOIN` はマージ機能の実装において依存する1次結合操作です。`MERGE` 演算子を使用する場合、必ず `source_table` と `target_table` に対して1次外部結合を行います。これは、どの行が一致し、どの行が一致しないかを区別するためです。
...
|---------------|-------------------------------------------------------------------------------------------------------------------|
| output        | この演算子が出力する式。                                                                                                |
| filter        | この演算子のフィルター条件。例では `MERGE` 演算子に `filter` 句がないため、`nil` となります。 |
