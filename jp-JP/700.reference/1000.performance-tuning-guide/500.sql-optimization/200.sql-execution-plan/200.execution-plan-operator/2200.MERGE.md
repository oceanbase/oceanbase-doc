...
`MERGE` 演算子は、ソーステーブルのデータ行を更新または挿入する形でターゲットテーブルにマージします。

OceanBase データベースがサポートする `MERGE` 演算子には、`MERGE` と `MULTI PARTITION MERGE` があります。

  <main id="notice" type='explain'>
    <h4>説明</h4>
    <p><code>MERGE</code> 演算子は、OceanBase データベースのOracleモードでのみ利用できます。</p>
  </main>
...
以下の例のように、Q1 クエリは `src_tbl` テーブルのデータ行を `t1` テーブルにマージします。`src_tbl` の各行に対して、以下の方法でマージが実行されます：

* `t1` テーブルに `t1.c1=src_tbl.c1` の条件を満たすデータ行が存在する場合：

  * `src_tbl.c2 > '100'` を満たす場合、更新操作を実行し、`t1.c2` の値を `src_tbl.c2` の値に設定します（ターゲットテーブルの各行は1回だけ更新されます）。
...
* `t1` テーブルに `t1.c1 = src_tbl.c1` の条件を満たすデータ行が存在しない場合：
  ...
  * `src_tbl.c1 > 10` を満たさない場合、挿入操作は実行されません。
...
ここで、`OUTER JOIN` はマージ機能の実装において依存する1次結合操作です。`MERGE` 演算子を使用する際は、必ず `source_table` と `target_table` に対して1次外部結合を行います。これは、どの行が一致し、どの行が一致しないかを区別するためです。

実行計画表示の `outputs & filters` は、`MERGE` 演算子の出力情報を詳細に示しています。

|   **情報名**    |                                                **意味**                                                       |
|---------------|-------------------------------------------------------------------------------------------------------------------|
| output        | この演算子が出力する式。                                                                                                |
| filter        | この演算子にかけられるフィルタ条件。例では `MERGE` 演算子に `filter` がないため、`nil` となります。 |
...
`MULTI PARTITION MERGE` 演算子は、データテーブルの複数のパーティションにまたがるデータをマージするために使用されます。

以下の例のように、Q2 クエリは `src_tbl` テーブルのデータ行をパーティションテーブル `t2` にマージします。`src_tbl` テーブルの各行について、以下の方法でマージが実行されます：

* `t2` テーブルに `t2.c1 = src_tbl.c1` の条件を満たすデータ行が存在する場合：

  * 更新操作を実行し、`t2.c2` の値を `substr(src_tbl.c2, 1, 5)` の値に設定します（対象テーブルの各行は1回だけ更新されます）。
  * 更新後、`t2.c2 > '80000'` を満たす場合、該当するデータ行を削除します。

* `t2` テーブルに `t2.c1 = src_tbl.c1` の条件を満たすデータ行が存在しない場合、挿入操作を実行し、`(src_tbl.c1, src_tbl.c2)` を `t2` テーブルに挿入します。
