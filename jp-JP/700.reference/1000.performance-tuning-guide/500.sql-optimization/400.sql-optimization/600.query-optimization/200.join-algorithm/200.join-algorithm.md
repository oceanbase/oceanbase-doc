...
OceanBaseデータベースV4.1.0バージョンでは、DAS Group Rescanは単一レベルのNested Loop Joinの実行に適用されます。この最適化は、NLJ（Nested Loop Join）の左側の部分でバッチ処理を行い、右側の部分へのスキャン回数を削減します。この最適化はデフォルトで有効になっており、内部システム変数`_NLJ_BATCHING_ENABLED`（Oracleモードではこの変数を設定する際には二重引用符で囲む必要があります）を使用して無効にすることができます。構文は以下のとおりです：
