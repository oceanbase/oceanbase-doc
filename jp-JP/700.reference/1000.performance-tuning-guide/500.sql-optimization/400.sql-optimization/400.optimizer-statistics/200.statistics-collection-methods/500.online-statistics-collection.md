...
オンライン統計情報収集とは、`INSERT` ステートメントの実行時にデータベースオプティマイザーが統計情報を収集できるようにする機能であり、システムパッケージを手動で呼び出す必要がないことを指します。一括データインポートの過程でリアルタイムに統計情報を収集でき、追加的な収集タスクを開始する必要がなく、運用保守作業を削減し、統計情報収集のパフォーマンスを向上させます。

OceanBaseデータベースでは、`GATHER_OPTIMIZER_STATISTICS`/`NO_GATHER_OPTIMIZER_STATISTICS` ヒントおよびシステム変数 `_optimizer_gather_stats_on_load`（デフォルトで有効）を使用してオンライン統計情報収集を行います。また、ダイレクトロード機能の `APPEND` ヒントを使用してオンライン統計情報収集を実現することもできます。

<main id="notice" type='explain'>
<h4>説明</h4>
  <p> <code>_optimizer_gather_stats_on_load</code> は内部システム変数であり、デフォルトで有効です。</p>
</main>

オンライン統計情報収集の主なトリガーシナリオは以下のとおりです：

- `CREATE TABLE...AS SELECT` (CTAS)

  システム変数 `_optimizer_gather_stats_on_load` が True の場合、CTAS はデフォルトでオンライン統計情報収集機能を有効にします。この機能を無効にするには、`NO_GATHER_OPTIMIZER_STATISTICS` ヒントを使用します。例：
...
- `INSERT INTO SELECT`
  ...
  <main id="notice" type='notice'>
    <h4>注意</h4>
    <p>OceanBaseデータベースは、<code>INSERT /*+GATHER_OPTIMIZER_STATISTICS*/ INTO table_name VALUES()...</code> および <code>INSERT /*+GATHER_OPTIMIZER_STATISTICS*/ INTO table_name(column_name) VALUES() | SELECT...</code> ステートメントを使用したオンライン統計情報収集を現在サポートしていません。</p>
  </main>

- PDML

  `ENABLE_PARALLEL_DML` ヒントを使用するか、`ALTER SESSION ENABLE PARALLEL DML;` でシステム並列モードを設定することで、挿入操作をパラレル処理で実行します。
  ...
  上記の例では、`OPTIMIZER_STATS MERGE` 演算子が生成されており、オンライン統計情報収集が実行されたことを示しています。
