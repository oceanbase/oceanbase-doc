...
オンライン統計情報収集とは、`INSERT` 文の実行時にデータベースオプティマイザーが統計情報を収集できる機能であり、システムパッケージを手動で呼び出す必要はありません。データの一括インポートプロセス中にリアルタイムで統計情報を収集できるため、追加の収集タスクを開始する必要がなく、運用保守作業が削減され、統計情報収集のパフォーマンスが向上します。

OceanBase データベースでは、`GATHER_OPTIMIZER_STATISTICS`/`NO_GATHER_OPTIMIZER_STATISTICS` ヒントとシステム変数 `_optimizer_gather_stats_on_load`（デフォルトで有効）を使用してオンライン統計情報収集を行います。また、ダイレクトロード機能の `APPEND` ヒントを使用してオンライン統計情報収集を有効にすることもできます。
...
オンライン統計情報収集が主にトリガーされるシナリオは以下の通りです：
...
- `INSERT INTO SELECT`
  ...
  <main id="notice" type='notice'>
    <h4>注意</h4>
    <p>OceanBase データベースは、<code>INSERT /*+GATHER_OPTIMIZER_STATISTICS*/ INTO table_name VALUES()...</code> ステートメントによるオンライン統計情報収集を現在サポートしていません。</p>
  </main>
