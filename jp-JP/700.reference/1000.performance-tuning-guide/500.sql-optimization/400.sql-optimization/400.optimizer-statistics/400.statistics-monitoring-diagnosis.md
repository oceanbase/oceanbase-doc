...
OceanBaseデータベースのオプティマイザーによる統計情報収集プロセスは、特に自動統計情報収集操作では時間がかかります。収集は現在のデータベース内のすべてのテーブルに関わるため、何らかの理由で収集に失敗するテーブルが存在する可能性が高いです。収集に失敗したテーブルをタイムリーに診断し、フィードバックできない場合、実行計画の生成時に利用可能な統計情報がなくなり、不良な計画が生成され、データベースシステムの異常な動作を引き起こす可能性があります。

OceanBaseデータベースV4.2.0バージョンから、統計情報収集の監視と診断機能がサポートされています。主に統計情報の収集状態を監視し、統計情報の収集状況をフィードバックするために使用されます。

## 統計情報収集状態の監視

ユーザーが手動または自動で統計情報収集を開始した後、`(G)V$OB_OPT_STAT_GATHER_MONITOR` ビューを使用してリアルタイムの統計情報収集状態を照会できます。これには、現在の収集タスクの実行時間、収集されたテーブル数、収集戦略などのリアルタイム情報が含まれるため、統計情報の収集状況を監視することができます。

`GV$OB_OPT_STAT_GATHER_MONITOR` ビューは、SYSテナントからアクセスするとすべてのテナントのリアルタイム統計情報収集状態が表示され、通常のテナントからアクセスするとそのテナント自身のリアルタイム統計情報収集状態が表示されます。`V$OB_OPT_STAT_GATHER_MONITOR` ビューは、現在のテナントが現在のOBServerノード上でのリアルタイム統計情報収集状態を表示します。

ユーザーは `DESC` コマンドを使用して `(G)V$OB_OPT_STAT_GATHER_MONITOR` ビューのフィールド情報を確認できます。例：
...
`(G)V$OB_OPT_STAT_GATHER_MONITOR` ビューの各フィールドの意味は以下の通りです：

* `TENANT_ID` はテナントIDを表します。
* `SVR_IP` は収集タスクが存在するノードのアドレスを表します。
* `SVR_PORT` は収集タスクが存在するノードのポートを表します。
* `SESSION_ID` は収集タスクが存在するセッションIDを表します。
* `TRACE_ID` は収集タスクが存在するTrace IDを表します。
* `TASK_ID` は収集タスクのIDを表します。
* `TYPE` は収集方式を表します。現在、`MANUAL GATHER` と `AUTO GATHER` の2種類があります。
* `TASK_START_TIME` は収集タスクの開始時間を表します。
* `TASK_DURATION_TIME` は収集タスク全体の持続時間（単位：us）を表します。
* `TASK_TABLE_COUNT` は収集タスクで収集する必要があるテーブル数を表します。
* `COMPLETED_TABLE_COUNT` は収集タスクが完了したテーブル数を表します。
* `RUNNING_TABLE_OWNER` は収集タスクがテーブルを収集しているユーザー名を表します。
* `RUNNING_TABLE_NAME` は収集タスクがテーブルを収集している名前を表します。
* `RUNNING_TABLE_DURATION_TIME` は収集タスクがテーブルを収集している持続時間（単位：us）を表します。

以下の例では、デフォルトのポリシーを使用してテーブル `test.t1` からヒストグラム統計情報を手動で収集します。
...
OceanBaseデータベースでは、関連するビューを通じて、テナントレベルおよびテーブルレベルの統計情報収集状況を照会できます。

### テナントレベルの履歴統計情報収集状況の確認

ユーザーは `DBA_OB_TASK_OPT_STAT_GATHER_HISTORY` ビューを使用して、統計情報収集タスクの詳細な状況を照会できます。これには、特定のテーブルの履歴統計情報の収集状況や、自動収集タスクの失敗リストの詳細などの情報が含まれます。

ユーザーは `DESC` コマンドを使用して `DBA_OB_TASK_OPT_STAT_GATHER_HISTORY` ビューのフィールド情報を確認できます。例：
...
`DBA_OB_TASK_OPT_STAT_GATHER_HISTORY` ビューの各フィールドの意味は以下のとおりです：

* `TENANT_ID` はテナントIDを表します。
* `TASK_ID` は収集タスクのIDを表します。
* `TYPE` は収集方式を表します。現在、`MANUAL GATHER` と `AUTO GATHER` の2種類があります。
* `STATUS` は収集タスクの状態を表します。現在の2つの状態は `SUCCESS` と `FAILED` です。
* `TABLE_COUNT` は収集タスクで収集されたテーブルの数を表します。
* `FAILED_COUNT` は収集タスクで収集に失敗したテーブルの数を表します。
* `START_TIME` は収集タスクの開始時間を表します。
* `END_TIME` は収集タスクの終了時間を表します。

以下の例は、`DBA_OB_TASK_OPT_STAT_GATHER_HISTORY` ビューを使用して、指定したテナントの履歴統計情報収集状況を照会する方法です。
...
### テーブルレベルの履歴統計情報収集状況の確認

ユーザーは `DBA_OB_TABLE_OPT_STAT_GATHER_HISTORY` ビューを使用して、特定のテーブルの履歴統計情報収集状況を確認できます。

ユーザーは `DESC` コマンドを使用して `DBA_OB_TABLE_OPT_STAT_GATHER_HISTORY` ビューのフィールド情報を確認できます。例：
...
`DBA_OB_TABLE_OPT_STAT_GATHER_HISTORY` ビューの各フィールドの意味は以下のとおりです：

* `OWNER` は、そのテーブルが属するユーザーを表します。
* `TABLE_NAME` は、そのテーブルの名前を表します。
* `TASK_ID` は、そのテーブルの収集に関連する収集タスクIDを表します。
* `STATUS` は、そのテーブルの収集状態を表します。現在の状態は `SUCCESS` と `FAILED` の2種類です。
* `START_TIME` は、そのテーブルの収集開始時間を表します。
* `END_TIME` は、そのテーブルの収集終了時間を表します。
* `MEMORY_USED` は、そのテーブルの収集期間中のメモリ使用量を表します。
* `STAT_REFRESH_FAILED_LIST` は、統計情報を収集したもののキャッシュのリフレッシュに失敗したクラスタマシンのIPアドレスリストを表します。
* `PROPERTIES` は、そのテーブルの収集プロパティ設定を表します。

以下の例は、`DBA_OB_TABLE_OPT_STAT_GATHER_HISTORY` ビューを使用して、ユーザー `test` の下にあるすべてのテーブルの履歴統計情報収集状態を確認する方法です。
...
OceanBaseデータベースは、現在以下の2つの方法で統計情報収集履歴のクリーンアップをサポートしています：

* `DBMS_STATS.PURGE_STATS($timestamp)` を使用した手動クリーンアップ。
* 自動スケジュールタスクを設定し、保存期限（デフォルトで31日）を超えた履歴情報を自動的にクリーンアップする。

以下の例は、`DBMS_STATS.PURGE_STATS($timestamp)` を使用して、指定された期間の履歴統計情報を手動でクリーンアップする方法です。
...
[履歴管理機能](300.statistics-management/600.history-management-features.md)