...
OceanBaseデータベースのオプティマイザーによる統計情報収集プロセスは時間がかかります。特に自動統計情報収集操作はそうです。収集は現在のデータベース内のすべてのテーブルに関わるため、何らかの理由で収集に失敗したテーブルが存在する可能性が高いです。収集に失敗したテーブルがタイムリーに診断されフィードバックされなければ、実行計画の生成時に利用可能な統計情報がなくなり、結果として最適でない計画が生成され、データベースシステムの異常な動作を引き起こすことになります。

OceanBaseデータベースV4.2.0バージョンから、統計情報収集のモニタリングと診断機能がサポートされています。これは主に統計情報収集の状態を監視し、統計情報収集の状況をフィードバックするために使用されます。

## 統計情報収集状態の監視

ユーザーが手動または自動で統計情報収集を開始した後、`(G)V$OB_OPT_STAT_GATHER_MONITOR`ビューを使用して、現在の収集タスクの実行時間、収集されたテーブルの数、収集戦略などのリアルタイム情報を照会し、統計情報の収集状況を監視できます。

`GV$OB_OPT_STAT_GATHER_MONITOR`ビューは、SYSテナントでアクセスすると、すべてのテナントのリアルタイム統計情報収集状態が表示されます。通常のテナントでアクセスすると、そのテナント自身のリアルタイム統計情報収集状態が表示されます。`V$OB_OPT_STAT_GATHER_MONITOR`ビューは、現在のテナントが現在のOBServerノード上でのリアルタイム統計情報収集状態を表示します。

ユーザーは`DESC`コマンドを使用して`(G)V$OB_OPT_STAT_GATHER_MONITOR`ビューのフィールド情報を照会できます。例：
...
* `TYPE`は収集方式を表します。現在は`MANUAL GATHER`と`AUTO GATHER`の2種類があります。
* `TASK_START_TIME`は収集タスクの開始時間を表します。
* `TASK_DURATION_TIME`は収集タスク全体の持続時間（単位：us）を表します。
* `TASK_TABLE_COUNT`は収集タスクで収集が必要なテーブルの数を表します。
* `COMPLETED_TABLE_COUNT`は収集タスクで既に完了したテーブルの数を表します。
* `RUNNING_TABLE_OWNER`は収集タスクがテーブルを収集しているユーザー名を表します。
* `RUNNING_TABLE_NAME`は収集タスクがテーブルを収集しているテーブル名を表します。
* `RUNNING_TABLE_DURATION_TIME`は収集タスクがテーブルを収集している持続時間（単位：us）を表します。

以下の例は、テーブル`test.t1`に対してデフォルトの戦略を使用して手動でヒストグラム統計情報を収集する場合です。
...
収集プロセス中に、指定されたMySQLテナントの統計情報収集状態を照会した結果は次のとおりです：
...
OceanBaseデータベースは、関連するビューを使用して、テナントレベルおよびテーブルレベルの履歴統計情報収集状態を照会することをサポートしています。

### テナントレベルの履歴統計情報収集状態の照会
...
* `TENANT_ID` はテナントIDを表します。
* `TASK_ID` は収集タスクのIDを表します。
* `TYPE` は収集方法を表します。現在、`MANUAL GATHER` と `AUTO GATHER` の2種類があります。
* `STATUS` は収集タスクの状態を表します。現在の状態は `SUCCESS` と `FAILED` の2種類です。
...
以下の例は、`DBA_OB_TASK_OPT_STAT_GATHER_HISTORY` ビューを使用して、指定されたテナントの履歴統計情報収集状態を照会するものです。
...
### テーブルレベルの履歴統計情報収集状態の照会

ユーザーは `DBA_OB_TABLE_OPT_STAT_GATHER_HISTORY` ビューを使用して、特定のテーブルの履歴統計情報収集状況を照会できます。
...
* `OWNER` はそのテーブルが属するユーザーを表します。
* `TABLE_NAME` はそのテーブルの名前を表します。
* `TASK_ID` はそのテーブルの収集に関連する収集タスクIDを表します。
* `STATUS` はそのテーブルの収集状態を表します。現在の状態は `SUCCESS` と `FAILED` の2種類です。
* `START_TIME` はそのテーブルの収集開始時間を表します。
* `END_TIME` はそのテーブルの収集終了時間を表します。
* `MEMORY_USED` はそのテーブルの収集期間中のメモリ使用量を表します。
* `STAT_REFRESH_FAILED_LIST` はそのテーブルの統計情報収集は成功したものの、キャッシュのリフレッシュに失敗したクラスタマシンのIPアドレスリストを表します。
* `PROPERTIES` はそのテーブルの収集プロパティ設定を表します。

以下の例は、`DBA_OB_TABLE_OPT_STAT_GATHER_HISTORY` ビューを使用して、ユーザー `test` の下のすべてのテーブルの履歴統計情報収集状態を照会するものです。
...
* `DBMS_STATS.PURGE_STATS($timestamp)` を使用して手動でクリーンアップします。
* 自動スケジュールタスクを設定し、保存期限（デフォルトで31日）を超えた履歴情報を自動的にクリーンアップします。

以下の例は、`DBMS_STATS.PURGE_STATS($timestamp)` を使用して、指定された時間の履歴統計情報を手動でクリーンアップする方法です。
