...
1. `(G)V$OB_PLAN_CACHE_PLAN_STAT` ビューおよび `(G)V$OB_SQL_AUDIT` の実行計画タイプのレコードを確認し、分散実行計画であるかどうかを判断します。

2. 当該実行計画が正しいかどうかを分析します。

3. `trace_id` を使用してクエリ `(G)V$OB_SQL_AUDIT` と関連付け、実行されたすべてのサブ計画の時間消費状況を確認します。各サブ計画の RPC 実行には1件のSQL Auditレコードが対応しており、このSQL Auditレコードを分析して問題を特定します。
