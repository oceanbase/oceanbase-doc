...
| UNPACK_csv_BASELINE | CSVファイル内のベースラインを内部テーブルにインポートします。 |
...
PROCEDURE PACK_CSV_BASELINE (
  detail_file      IN VARCHAR2,
  overall_file     IN VARCHAR2,
...
```sql
PROCEDURE PACK_CSV_BASELINE (
  detail_file      VARCHAR(65535),
  overall_file     VARCHAR(65535),
...
| **パラメータ名** | **説明** |
| --- | --- |
| detail_file | `__all_plan_baseline_item` の内容をエクスポートするファイルを指定します。絶対パスを使用してください。CSVファイルのエクスポートでは既存のファイルを上書きできません。ここでは、存在しないファイルを指定する必要があります。 |
| overall_file | `__all_plan_baseline` の内容をエクスポートするファイルを指定します。絶対パスを使用してください。CSVファイルのエクスポートでは既存のファイルを上書きできません。ここでは、存在しないファイルを指定する必要があります。 |
| baseline_owner | エクスポートするベースラインの所有者を指定します。指定しない場合、または NULL を指定した場合は、現在のユーザーを表します。 |

#### 例

現在のユーザーのすべてのベースライン計画をCSVファイルにエクスポートします。

```sql
BEGIN
  DBMS_SPM.PACK_CSV_BASELINE (
    detail_file => '/tmp/spm_baseline_detail.csv',
    overall_file => '/tmp/spm_baseline_overall.csv'
  );
END;
/
```
...
`UNPACK_CSV_BASELINE` プロシージャは、CSVファイルからベースラインを内部テーブルにインポートするために使用されます。
...
| **パラメータ名** | **説明** |
| --- | --- |
| detail_file | `__all_plan_baseline_item` のデータをインポートするファイルを指定します。絶対パスを使用してください。 |
| overall_file | `__all_plan_baseline` のデータをインポートするファイルを指定します。絶対パスを使用してください。 |
| baseline_owner | ベースラインをインポートするユーザーを指定します。指定しない場合、または NULL を指定した場合は、現在のユーザーを表します。 |

#### 例

CSVファイルからベースライン計画をインポートします。
