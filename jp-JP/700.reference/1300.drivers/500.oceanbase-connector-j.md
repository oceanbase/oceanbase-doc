...
JDBC標準はSun Microsystemsによって定義され、標準の`java.sql`インターフェースを通じて実装されます。これにより、各プロバイダーが独自のJDBCドライバを使用して標準を実装・拡張できるようになっています。JDBCはX/Open SQLに基づく呼び出しレベルインターフェース（Call Level Interface、CLI）です。
