...
* `PARTITION BY` は、1つまたは複数のパーティション列を指定するために使用されます。
...
外部テーブルに自動的にパーティションの管理を任せるのではなく、手動で追加や削除を行う場合は、`PARTITION_TYPE = USER_SPECIFIED` フィールドを指定する必要があります。
...
* `metadata$partition_list_col`は、この列が外部テーブルのパーティションキーの何番目の列であるかを指定し、`partition by`内の順序と対応します。

以下のステートメントを実行して、手動でパーティションを追加する方法で、`2023-06-01`サブディレクトリ内のファイルをパーティションに追加します。また、このパーティションのパーティションキー値も`2023-06-01`に設定されます。この値は任意に指定できます。
