...
制約は列に定義され、列に格納される値を制限します。制約定義に違反する値をその列に書き込もうとしたり、更新しようとしたりすると、エラーが発生し、その操作がロールバックされます。既存のテーブルの列に、既存のデータと矛盾する制約を追加しようとすると、同様にエラーが発生し、その操作がロールバックされます。

## 制約タイプ

OceanBaseデータベースのOracleモードは、NOT NULL制約、UNIQUE制約、PRIMARY KEY制約、FOREIGN KEY制約、CHECK制約などの制約タイプをサポートしています。

### NOT NULL制約 (`NOT NULL`)

NOT NULL制約は、制約対象の列の値が `NULL` であることを許可しません。NOT NULL制約がある列では、`INSERT` ステートメントでその列の値を指定する必要があります。ただし、その列にデフォルト値が定義されている場合は除きます。
...
UNIQUE制約は、制約対象の列の値に重複があることを許可しませんが、複数の `NULL` 値は存在可能です。
...
FOREIGN KEY制約は、制約対象の列の値が、別のテーブル（親テーブル）の主キー列から取得されることを要求します。

FOREIGN KEY制約を作成する際には、親テーブルで外部キーの参照列に `DELETE` 操作を実行した場合に、子テーブルの対応する行の外部キー列に対して実行されるカスケード操作を指定することもできます：

* `CASCADE`：親テーブルの外部キーの参照列が削除されたとき、子テーブルの対応する行の外部キー列も自動的に削除します。

* `SET NULL`：親テーブルの外部キーの参照列が削除されたとき、子テーブルの対応する行の外部キー列の値を `NULL` に設定します。

  外部キー制約のカスケード操作を `SET NULL` に指定する場合、外部キー列は生成列であってはなりません。

外部キーを作成する際に `ON DELETE` 句を指定しなかった場合、親テーブルの外部キーの参照列が削除されると、外部キー制約違反により操作は拒否されます。

例えば、テーブル `tbl4` を作成し、`col2` 列をテーブル `tbl3` の主キー列 `col1` に関連付けます。
...
テーブルを作成した後、テーブルに主キーを追加したり、既存の主キーを変更したりできます：
...
* 既存のテーブルに外部キー制約を追加します。
  ...
  * `column_name1` は制約を指定する列名を表します。`column_name2` は関連テーブルの主キーカラムの名前を表します。
...
* `CHECK` 制約の有効化または無効化を実行します。
  ...
  テーブル作成時に `CHECK` 制約が既に追加されている場合、この制約を有効または無効にすることができます。
...
テーブル作成後、作成済みのテーブルに `NOT NULL` 制約を追加または削除することができます：
...
* テーブルの `NOT NULL` 制約を削除します。
  ...
  また、以下の構文を使用して `NOT NULL` 制約を削除することもできます：

  ```sql
  obclient> ALTER TABLE table_name MODIFY c1 NULL;
  ```

制約を管理するための構文と詳細については、[ALTER TABLE](../../../500.sql-reference/100.sql-syntax/300.common-tenant-of-oracle-mode/900.sql-statement-of-oracle-mode/100.ddl-of-oracle-mode/1000.alter-table-of-oracle-mode.md)を参照してください。

## 時刻列のデフォルト時刻設定

列に `NOT NULL` 制約がある場合、通常はデフォルト値を設定することを推奨します。列の型が日付または時刻型の場合、データベースの現在時刻をデフォルト値として設定できます。
...
3. データの挿入が成功した後、テーブル内のデータを確認します。
