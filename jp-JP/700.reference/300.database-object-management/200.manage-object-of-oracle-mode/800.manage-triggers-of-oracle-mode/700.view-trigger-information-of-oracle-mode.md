...
OceanBaseデータベースでは、以下のトリガー関連ディクショナリビューをサポートしています：
...
現在ユーザーが持つすべてのトリガーを、タイプ、ステータス、作用対象のテーブルおよびそのユーザーと共に一覧表示します。
...
OceanBaseデータベースのユーザーは、`DBA_SOURCE`、`USER_SOURCE`、`DBA_OBJECTS` ビューを使用してトリガー関連情報を確認することもできます。クエリ文の例は以下のとおりです：
