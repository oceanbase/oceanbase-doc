...
|slug|function-index-list-of-not-supported-functions-mysql-compatible-mode|

# 関数インデックスでサポートされていないシステム関数のリスト

OceanBaseデータベースのMySQLモードでは、関数インデックスの式に制限があります。このセクションでは、関数インデックスで使用することが禁止されているシステム関数を主に説明します。

## 非決定的なシステム関数

以下の非決定的なシステム関数の式は、関数インデックスでの使用が禁止されています。
...
以下の空間システム関数は、OceanBaseデータベース独自のシステム関数であり、現在関数インデックスでの使用は禁止されています。
...
以下の関数の式は、関数インデックスでの使用が禁止されています。
