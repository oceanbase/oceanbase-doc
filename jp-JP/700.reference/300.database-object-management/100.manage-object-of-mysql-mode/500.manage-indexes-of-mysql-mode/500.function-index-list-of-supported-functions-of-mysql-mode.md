...
## パラメータタイプに制限がないシステム関数

次の表の関数の式は、関数インデックスで使用できます：
...
以下のシステム関数は、パラメータがTIME型の場合、関数インデックスでの使用を禁止します。例えば、`DATE` 関数では、最初のパラメータが TIME の場合の関数インデックス使用を禁止します。`DATEDIFF` 関数では、いずれかのパラメータが TIME 型の場合の関数インデックス使用を禁止します。
...
以下のJSON関数の式は、関数インデックスでの使用が許可されています。
