...
テーブルグループの `SHARDING` プロパティを変更するには、まずテーブルグループ内にテーブルが存在するかどうか、およびテーブルグループ内のテーブルのパーティション情報を確認する必要があります。テーブルグループ内のテーブルの具体的な操作については、[テーブルグループ情報の表示](../400.manage-table-groups-of-mysql-mode/300.view-the-information-of-a-table-group-of-mysql-mode.md)を参照してください。
