...
OceanBaseデータベースのパーティションは、以下のパーティションタイプをサポートしています：RANGEパーティション、RANGE COLUMNSパーティション、LISTパーティション、LIST COLUMNSパーティション、HASHパーティション、およびKEYパーティション。

OceanBaseデータベースのサブパーティションは、任意の2種類のパーティションタイプの組み合わせが可能です。具体的なサポート状況は以下のとおりです：
...
## RANGE/RANGE COLUMNSパーティションのパーティションテーブルを作成する
...
* MySQLモードでは、`PARTITION BY RANGE ( expr )` の `expr` 式の結果は整数である必要があります。時間型の列に基づいてRANGEパーティションを作成する場合は、時間型を数値型に変換する必要があります。
...
* 最後のパーティションの上限のみが `MAXVALUE` として定義できます。この値には具体的な数値がなく、他のすべてのパーティションの上限よりも大きく、NULL値も含みます。最後のRANGEパーティションに `MAXVALUE` が指定されている場合は、新しいパーティションを追加できません。
...
<h4>注意</h4>
<ul><p>Listパーティションを使用する場合、以下のルールに従う必要があります：</p>
<li>パーティション式の結果は整数でなければなりません。</li>
...
| column_type    | 列のデータ型を指定します。                                        |
| partition_name | パーティション名を指定します。                                          |
| DEFAULT        | 最後のパーティションにのみこの値を指定できます。この値に具体的な数値はなく、他のすべてのパーティションの上限よりも大きく、NULL値も含みます。 |
...
| column_name_list | Keyパーティション列名リストを指定します。 |
| partition_count  | パーティション数を指定します。         |

<main id="notice" type='explain'>
<h4>説明</h4>
<p>Hash/Keyパーティションの場合、作成時にパーティション名を指定しないと、システムが命名規則に基づいて名前を付けます。パーティションテーブルの場合、各パーティションはそれぞれp0、p1、...、pnという名前になります。</p>
</main>
...
<main id="notice" type='explain'>
  ...
  <p>
    <ul>
      <li>テンプレート型サブパーティションテーブルでは、各パーティション内のサブパーティションはすべてテンプレートに定義されたサブパーティション定義に従います。つまり、各パーティションのサブパーティション定義はすべて同じです。
          <ul>
      ...
      <li>テンプレート型サブパーティションテーブルでは、サブパーティションを定義すると、各サブパーティションの命名規則は <code>($part_name)s($subpart_name)</code> となります。例えば、以下の <code>tbl_rr</code> テーブルでは、パーティション <code>p0</code> の下の3つのサブパーティションの名前はそれぞれ <code>p0smp1</code>、<code>p0smp2</code>、<code>p0smp3</code> となります。</li>
    </ul>
  </p>
</main>
...
### Range Columns + Hashサブパーティションのテンプレート型セカンダリパーティションテーブルを作成する
...
### List Columns + Keyサブパーティションを持つテンプレート型サブパーティションテーブルの作成
...
### Hash + Range Columnsサブパーティションを持つテンプレート型サブパーティションテーブルの作成
...
### Hash + List Columnsサブパーティションを持つテンプレート型サブパーティションテーブルの作成
...
### Hash + Hashサブパーティションを持つテンプレート型サブパーティションテーブルの作成
...
### Hash + Keyサブパーティションを持つテンプレート型サブパーティションテーブルの作成
...
### Key + Range Columnsサブパーティションを持つテンプレート型サブパーティションテーブルの作成
...
### Key + List Columnsサブパーティションを持つテンプレート型サブパーティションテーブルの作成
...
### Key + Keyサブパーティションを持つテンプレート型サブパーティションテーブルの作成
...
### Range + Rangeサブパーティションを持つテンプレート型およびカスタムサブパーティションテーブル
...
## 非テンプレートのサブパーティションテーブルを作成する

OceanBaseデータベースのMySQLモードは現在、Hash、Range、List、Key、Range Columns、List Columnsの6種類のパーティションタイプをサポートしており、サブパーティションは任意の2種類のパーティションタイプの組み合わせです。

非テンプレートのサブパーティションテーブルを作成するSQL構文は以下のとおりです：
...
### Range + Rangeパーティションの非テンプレートサブパーティションテーブルを作成する
...
### Range + List Columnsパーティションの非テンプレートサブパーティションテーブルを作成する
...
### Range Columns + Range Columnsパーティションの非テンプレートサブパーティションテーブルを作成する
...
### Range Columns + List Columnsパーティションの非テンプレートサブパーティションテーブルを作成する
...
### Range Columns + Hashパーティションの非テンプレートサブパーティションテーブルを作成する
...
### Range Columns + Keyパーティションの非テンプレートサブパーティションテーブルを作成する
...
### List + List Columnsサブパーティションを持つ非テンプレートのサブパーティションテーブルを作成する
...
### List Columns + Range Columnsサブパーティションを持つ非テンプレートのサブパーティションテーブルを作成する
...
### List Columns + List Columnsサブパーティションを持つ非テンプレートのサブパーティションテーブルを作成する
...
### List Columns + Hashサブパーティションを持つ非テンプレートのサブパーティションテーブルを作成する
...
### List Columns + Keyサブパーティションを持つ非テンプレートのサブパーティションテーブルを作成する
...
### Hash + Range Columnsサブパーティションを持つ非テンプレートのサブパーティションテーブルを作成する
...
### Hash + List Columnsサブパーティションを持つ非テンプレートのサブパーティションテーブルを作成する
...
### Hash + Keyサブパーティションの非テンプレート型セカンダリパーティションテーブルを作成する
