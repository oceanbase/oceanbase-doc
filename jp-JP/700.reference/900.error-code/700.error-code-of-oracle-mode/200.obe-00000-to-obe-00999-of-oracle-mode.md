...
* [データ挿入時に重複した値というエラーメッセージが表示される、エラーコード OBE-00001](https://www.oceanbase.com/knowledge-base/oceanbase-database-1000000000217892)
...
* エラーの原因：`UPDATE` または `INSERT` ステートメントが重複するキーの挿入を試みました。
...
* エラーの原因：通常はDeadインスタンスによって引き起こされます。
...
* SQLSTATE：HY000
* エラー原因：更新操作でのロック取得失敗。上位層にこのエラーコードを返し、再試行します。
...
* SQLSTATE：HY000
* エラー原因：OceanBaseデータベースで予期しない内部エラーが発生しました。プログラムの実行異常が原因の可能性があります。
* 解決策：テクニカルサポートにお問い合わせいただき、調査をお手伝いいたします。
...
* エラー原因：移行元のトランザクションデータテーブルが回収され、移行先でコピー時にトランザクションデータテーブルが連続していないことが検出され、失敗してエラーが発生したため、再試行されます。
...
* エラー原因：オブジェクト '%.*s' は関数または配列型でなければこのように使用できません。
...
* エラー原因：バッチSQLのプロパティには、単一のインデックスを使用する必要があります。
...
* 解決策：この問題を解決する鍵は、データ取得とデータ変更のロジックを正しく分離し、適切なバッチ操作構文を使用することです。
...
* 解決策：`USING` 句で `FORALL` バッチ処理の対象となる集合要素変数を直接参照することを避け、値を動的SQL文字列に直接埋め込むか、仕様に準拠したバインディング方法を採用してください。
...
* エラー原因：移行レプリケーション/Rebuildでソース側を選択する際、ターゲット側のログストリームレプリカがLearner Listに存在しない。ターゲット側のログストリームレプリカが削除された可能性があります。
...
* 解決策：内部エラーコードです。テクニカルサポートエンジニアにお問い合わせいただき、調査のご協力をお願いいたします。
...
* 解決策：内部エラーコードです。テクニカルサポートエンジニアにお問い合わせいただき、調査をお手伝いください。
...
* SQLSTATE：42000
* エラーの原因：述語の外部で JSON_EQUAL が使用されています。
* 解決策：述語内で JSON_EQUAL を使用してください。例えば、CASE 式に組み込むなどしてください。
...
* エラー原因：あるパーティションのバランス調整操作を実行しようとした際、パーティションは既にバランスが取れていました。
...
* SQLSTATE：HY000
* エラー原因：二進数幾何関数が異なるSRIDを2つ返しました：%u と %u。これらは同じである必要があります。空間関数にパラメータとして渡される幾何値は、同じSRID値を持たなければなりません。
...
* SQLSTATE：22023
* エラー原因：関数 %s に有効でない空間データが提供されました。有効な幾何値として認識されないパラメータを使用して空間関数を呼び出しました。
...
* SQLSTATE：HY000
* エラー原因：関数 %s 内のジオメトリが空のため、重心を計算できません。
...
* SQLSTATE：HY000
* エラー原因：ジオメトリオーバーレイ計算エラー。関数 %s 内のジオメトリデータが無効です。
...
* SQLSTATE：SR003
* エラー原因：SRID %u の空間参照系の定義で、必須の %s (EPSG %u) 投影パラメータが指定されていません。
...
* SQLSTATE：22S00
* エラー原因：関数 %s はデカルト空間参照系にのみ定義されていますが、そのパラメータの1つが SRID %u であり、これはデカルト空間参照系ではありません。
...
* SQLSTATE：SR001
* エラー原因：関数 %s はデカルト空間参照系にのみ定義されていますが、そのパラメータの1つが未定義の SRID %u です。
...
* SQLSTATE：22S00
* エラー原因：%s(%s) は地理空間参照システムで実装されていません。
...
* SQLSTATE：HY000
* エラー原因：ジオメトリの SRID と列 '%s' の SRID が一致しません。ジオメトリの SRID は %lu ですが、列の SRID は %lu です。ジオメトリまたは列の SRID プロパティを変更することを検討してください。
...
* SQLSTATE：HY000
* エラーの原因：列 '%s' にSPATIALインデックスがあるため、SRID仕様を変更できません。SRID仕様を変更する前に、SPATIALインデックスを削除してください。
...
* SQLSTATE：22S00
* エラーの原因：関数 %s はSRID 0とSRID 4326のみで定義されています。
...
* SQLSTATE：01S01
* エラーの原因：SRID範囲 [%u, %u] はシステム使用用に予約されています。アップグレード中、警告なくこの範囲内のSRSが追加、変更、または削除される可能性があります。
...
* SQLSTATE：22S00
* エラーの原因：関数 %s は地理空間参照システムでのみ定義されていますが、そのパラメータの1つがSRID %u に含まれており、これは地理的ではありません。
...
* SQLSTATE：22S02
* エラーの原因：関数 %s のパラメータには、経度が %f の幾何図形が含まれており、範囲を超えています。それは (%f, %f] の範囲内でなければなりません。
...
* SQLSTATE：22S03
* エラーの原因：関数 %s のパラメータには、緯度が %f の幾何図形が含まれており、範囲を超えています。それは [%f, %f] の範囲内でなければなりません。
...
* SQLSTATE：SR002
* エラー原因：SRID %u の空間参照系定義で、無効な地理軸「%s」と「%s」が指定されています。一方の軸は NORTH または SOUTH でなければならず、もう一方は EAST または WEST でなければなりません。
...
* SQLSTATE：SR002
* エラー原因：逆展平は1.0より大きくなければなりません。楕円体が球体の場合は0.0です。
...
* SQLSTATE：22S00
* エラー原因：SRID %uからの変換はサポートされていません。空間参照システムにTOWGS84句がありません。
...
* SQLSTATE：22S00
* エラー原因：SRID %uへの変換はサポートされていません。空間参照システムにTOWGS84句がありません。
...
* SQLSTATE：SU001
* エラー原因：関数 %s に渡されたジオメトリはSRID 0であり、長さの単位が指定されていないため、“%s”に変換できません。
...
* SQLSTATE：22S05
* エラー原因：関数 %s のパラメータには、異なるSRIDを持つジオメトリ %u と %u が含まれています。すべてのジオメトリは同じSRIDを持たなければなりません。
...
* SQLSTATE：22003
* エラーの原因：GEOMETRY フィールドに送信したデータからジオメトリオブジェクトを取得できません。
...
* SQLSTATE：HY000
* エラーの原因：Srid の使用が不正確です。Srid 列属性はジオメトリ型でのみサポートされます。
...
* SQLSTATE：HY000
* エラーの原因：空間/全文/ハッシュインデックスおよび明示的なインデックス順序の使用が不正確です。
...
* SQLSTATE：22023
* エラー原因：関数「%s」内のオプション「%s」の値「%s」が無効です。
...
* 解決策：オブジェクトストレージの接続性を確認し、接続性に問題がないことを確認した後、テクニカルサポートにお問い合わせいただき、調査をお手伝いいたします。
...
* エラーの原因：一時ファイルがディスクに保存されるデータ量が設定された上限を超えています。

* 解決策：現在エラーが発生しているSQLが予想以上のデータ量を生成していないか確認してください（例：一時ファイルがディスクに保存されるデータ量の上限は最大500GBと予想されている場合）。予想通りであれば、パラメータ`temporary_file_max_disk_size`を適切な値に調整してください。
...
* エラーの原因：バックアップテナントで透過的データ暗号化が有効になっている状態で、リストアテナント時に秘密鍵のアドレスを指定しなかった、または指定したアドレスが誤っていた場合。

* 解決策：復元コマンドで正しいキーアドレスを指定するか、復元前にSession変数 `SET @kms_encrypt_info` を設定して正しいキーアドレスを指定します。
...
* 解決策：バックアップまたはアーカイブパスに保持ポリシーが設定されているかどうか、およびバックアップまたはアーカイブパスを設定する際に `enable_worm` パラメータが正しく設定されているかどうかを確認します。`enable_worm` パラメータが正しく設定されている場合は、OceanBaseテクニカルサポートに連絡し、調査を依頼してください。
...
* 解決策：OceanBase テクニカルサポートにお問い合わせいただき、調査をご依頼ください。

<main id="notice" type='explain'>
<h4>説明</h4>
<p>V4.3.5 では、このエラーコードは V4.3.5 BP2 から導入されました。</p>
</main>
...
* エラー原因：物理復元の実行中に、OBServerノードが指定されたバックアップメディア内のバックアップセットファイルに正常にアクセスできない場合。

* 解決策：オブジェクトストレージの接続性を確認し、接続性に問題がないことを確認した後、テクニカルサポートにお問い合わせいただき、調査をご依頼ください。

<main id="notice" type='explain'>
<h4>説明</h4>
<p>V4.4.x では、このエラーコードは V4.4.1 から導入されました。</p>
</main>
...
* エラー原因：データバックアップおよびアーカイブのクリーンアップを開始できません。原因として、関連するテナントの状態が `NORMAL` 以外の状態（例：作成中の `CREATING` 状態、復元中の `RESTORE` 状態など）であるか、または関連するテナントにクリーンアップポリシーが設定されていない（期限切れクリーンアップの場合）可能性があります。

* 解決策：テナントの状態が正常か確認します。期限切れクリーンアップの場合、関連するテナントにクリーンアップポリシーが正しく設定されているか確認する必要があります。
...
* 解決策：バックアップメディアを正しくマウントし、アーカイブ/バックアップ/リストア操作を実行するテナントのすべてのOBServerノードが同一のバックアップメディアにアクセスできるようにします。NFSの詳細な操作手順については、[NFSのデプロイ](../../../600.manage/600.backup-and-recovery/200.deploy-nfs.md)を参照してください。
...
* エラー原因：指定された時間に復元するためのバックアップセットが見つかりません。

* 解決策：復元ポイントを再選択するか、他のバックアップセットパスを指定してください。

<main id="notice" type='explain'>
<h4>説明</h4>
<p>V4.4.2系では、このエラーコードはV4.4.2 BP1バージョンから導入されました。</p>
</main>
...
* 解決策：優先的にtrace_idを用いてターゲットノードのログを特定し、リソース監視およびパラメータ調整を組み合わせて解決してください。
...
* 解決策：OceanBaseデータベースに再度ログインすると、新しいサーバーセッションが自動的に作成されます。詳細については、セッショントレースファイルを確認してください。
...
* 解決策：リモートテーブルでFORALL INSERT/UPDATE/DELETEを使用しないでください。
...
* 解決策：お使いのOceanBaseデータベースの対応するバージョンの「SQLリファレンス（Oracleモード）」を参照して、正しいSQLステートメントを検索してください。
...
* エラーの原因：指定された文字列の長さがデータ型がサポートする長さを超えています。具体的には、CHARおよびRAWデータ型では指定された文字列の長さが2000を超え、その他のデータ型では4000を超えます。

* 解決策：文字列の長さを短縮するか、VARCHAR2、LONG CHAR、またはLONG RAWなど、より長い文字列長をサポートする他のデータ型に切り替えてください。
...
* SQLSTATE：42000
* エラーの原因：INSERTステートメント、または形式が((C,D),(E,F), ...)の列または値のリストで、必要なカンマが省略されています。
...
* エラー原因：INSERTステートメントの実行時にキーワードINTOが欠落しています。
...
* [OceanBaseデータベースのOracleモードで型演算を行った際にinconsistent datatypesが発生した場合](https://www.oceanbase.com/knowledge-base/oceanbase-database-1000000000271750)
* [OracleモードでUpdateを実行しClobを更新した際にret_code=-5083エラーが発生した場合](https://www.oceanbase.com/knowledge-base/oceanbase-database-1000000000322504)
* [order by clob列でinconsistent datatypesエラーが発生した場合](https://www.oceanbase.com/knowledge-base/oceanbase-database-1000000000245505)
* [データ挿入時にデータ型が一致せず、エラーコードOBE-00932が発生した場合](https://www.oceanbase.com/knowledge-base/oceanbase-database-1000000000217851)
...
* 解決策：SQLクエリを確認し、不整合なデータ型を特定し、CASTまたはCONVERT関数を使用してデータ型を同じ型に変換します。
...
* エラーの原因：ここでは `group` 関数をサポートしていません。
...
* エラーの原因：テーブルまたはビューが存在しません。OceanBase V2.2.70以降では、ロール権限が追加されたため、現在のロールが特定のテーブルやビューに対する`SELECT`権限を持っていない場合もこのエラーが返されます。

* 解決策：SQLのスペルを確認してください。特に、OceanBase V2.2.70以降では、現在のロールの権限を確認してください。
...
* [Oracleモードでオブジェクトを再作成したことにより、オブジェクト上の権限が失われる](https://www.oceanbase.com/knowledge-base/oceanbase-database-1000000000207740)
...
* 解決策：`LEVEL`、`PRIOR`、`ROWNUM`、`CONNECT_BY_ROOT`、`CONNECT_BY_ISLEAF` または `CONNECT_BY_ISCYCLE` を削除してください。
...
* エラー原因：シノニムは有効なターゲットオブジェクトに変換されませんでした。原因は以下のいずれかです：
...
* 解決策：シノニム定義を変更し、シノニムが有効なターゲットオブジェクトを指すようにします。
...
* エラーの原因：この式には列のエイリアスを指定する必要があります。