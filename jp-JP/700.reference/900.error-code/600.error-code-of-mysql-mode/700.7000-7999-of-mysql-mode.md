...
* エラーの原因：選挙のログモジュールのバッファがいっぱいです。選挙モジュールによるログ帯域幅の使用量が予想を上回っているか、内部ログの出力が過剰であるか、ログバッファ `buffer` の設定が小さすぎます。

* 解決策：内部エラーコードです。テクニカルサポートにお問い合わせいただき、調査をご支援ください。
...
* エラーの原因：選挙プロセスで受信した投票数が過半数に達していません。主不在選挙または主在連続当選が失敗しました。これは、ダウン、ネットワーク遅延、時刻ずれ、高CPU負荷など、さまざまな原因による可能性があります。
...
* エラーの原因：選挙リースが期限切れになりました。これはフォロワーまたはリーダーで発生する可能性があり、ダウン、ネットワーク遅延、時刻のずれ、高CPU負荷など、さまざまな原因によって引き起こされる可能性があります。
...
* エラーの原因：選挙メッセージがプロトコルで定められた受信ウィンドウ内にありません。ネットワーク遅延、時刻のずれ、高CPU負荷など、さまざまな原因によって引き起こされる可能性があります。
...
* エラーの原因：リーダー切り替え対象がローカルに記録されているメンバーリストに含まれていないか、選挙メッセージを送信した側がその対象が自身のメンバーリストに含まれていないと判断しました。
...
* エラーの原因：受信した投票メッセージを送信した側が、ローカルに記録されているメンバーリストに含まれていません。
...
* エラー原因：プロトコルエラー、ネットワーク遅延および時刻偏差が想定された上限を超えたことにより、デュアルリーダー状態となった。
...
* エラー原因：選挙タイマーの実行時刻が予想から大きく逸脱しています。通常はCPU負荷が高いことが原因です。
...
* エラー原因：1回のリーダー切り替えタスクが完了する前に、別のリーダー切り替えタスクがトリガーされた。
...
* エラー原因：リーダー切り替えタスクの実行対象が既にリーダーではない。
...
* エラー原因：選挙集計フェーズで、ローカルに有効な投票メッセージが検出されなかった。
...
* エラー原因：選挙仮想グループが1つの再任サイクル内にリーダーから複数の投票リクエストを受信した場合の内部エンコードエラー。
...
* エラー原因：リーダー切り替えプロセス中に、旧リーダーから新リーダーへの投票応答を受信できませんでした。
...
* エラー原因：仮想グループメッセージを受信しましたが、ローカルに対応する仮想グループが存在しません。
...
* エラー原因：古いバージョンの仮想グループメッセージを受信しました。不安定なネットワーク遅延が原因である可能性があります。
...
* エラー原因：`ObElectionGroupMgr` が複数回起動されました。
...
* エラー原因：`ObElectionGroupMgr` を起動プロセスを実行せずに使用しました。
...
* エラーの原因：OBServerノードが同一の選挙サイクル内に再選とリーダー切り替えのメッセージを同時に受信しました。
...
* エラーの原因：OBServerノードが同一の選挙サイクル内に重複する再選/投票リクエストを同時に受信しました。
...
* エラーの原因：異なるバージョン間でのアップグレード時に、選挙優先順位の比較が発生しました。

* 解決策：内部エラーコードです。アップグレード中にのみ報告されることが想定されています。テクニカルサポートにお問い合わせいただき、調査をお手伝いください。

## ERROR 7109 (HY000) : transfer system error

* OceanBaseエラーコード：7109

* エラー原因：Transferシステムエラー。

* 解決策：内部エラーコードです。テクニカルサポートにお問い合わせいただき、調査をお手伝いください。

## ERROR 7110 (HY000) : transfer src ls and dest ls member list not same

* OceanBaseエラーコード：7110

* エラー原因：Transfer操作を実行する際、ソース側とターゲット側のログストリームメンバーリストが一致しません。

* 解決策：ソース側とターゲット側のメンバーリストを確認してください。また、ディザスタリカバリモジュールが正常に動作しているか、実行中のログストリームレプリケーションタスクがないかを確認し、タスクの完了を待ってください。

## ERROR 7111 (HY000) : lock owner id is not expected

* OceanBaseエラーコード：7111

* エラー原因：ロック操作時に、ロックの実際の保持者が予期と一致しないことが検出されました。

* 解決策：テクニカルサポートにお問い合わせいただき、調査をお手伝いください。

## ERROR 7112 (HY000) : change member list compare ls transfer scn too small

* OceanBaseエラーコード：7112

* エラー原因：メンバーリスト変更を実行する際、転送シャープナル（SCN）が小さすぎます。
...
* エラー原因：基盤レベルでTABLETの移動を行う際、SEQUENCEが一致しないことが検出されました。
...
* エラー原因：ロードバランシングタスクの基盤となる TABLET の移行時に、アクティブトランザクションが検出されました。
* 解決策：ロードバランシング時にアクティブトランザクションの強制終了を有効にします。トランザクションの強制終了は、`_enable_balance_kill_transaction` および `_balance_kill_transaction_threshold` 設定パラメータで確認できます。問題が解決しない場合は、技術サポートスタッフに連絡し、調査の支援を依頼してください。

関連するケース：

* [OceanBase データベース V4.2 バージョンのロードバランシングタスクでエラー -7119 および -7114 が発生する](https://www.oceanbase.com/knowledge-base/oceanbase-database-1000000000366656)
...
* 解決策：技術サポートエンジニアに連絡し、調査の支援を依頼してください。
...
* エラー原因：データ転送時に、インデックスまたはメタデータの再構築が必要であることが検出されました。
...
* エラー原因：ガベージコレクションの実行時に、古いclogログが検出され、そのログをスキップする必要があります。
* 解決策：処理は不要です。
...
* エラー原因：転送待ちトランザクション終了操作中にタイムアウトが発生しました。
* 解決策：転送待ちトランザクション終了のタイムアウト時間設定を調整し、すべてのトランザクション終了を十分に待てるようにします。

関連問題事例：

* [OceanBaseデータベースV4.2バージョンでのバランシングタスクエラー -7119および-7114](https://www.oceanbase.com/knowledge-base/oceanbase-database-1000000000366656)
...
* エラー原因：単調増加シーケンス番号が一致しません。

* 解決方法：バックグラウンドスレッドは、単調増加のシーケンス番号が一致しないことを検出すると、新しいシーケンス番号で再試行します。
...
* エラー原因：単調増加するシーケンス番号が既存のシーケンス番号より小さい。

* 解決策：バックグラウンドスレッドのシーケンス番号が既存のシーケンス番号より小さい場合、他のスレッドによって取得されていることを意味します。バックグラウンドスレッドは再試行します。
...
* エラー原因：ロードバランシングが起動できません。

* 解決策：RSはTransferを再スケジュールして再試行します。継続してエラーが発生する場合は、技術サポートに連絡し、調査を依頼してください。
...
* エラー原因：特定のパーティションの均衡操作を実行しようとした際、パーティションはすでに均衡状態にありました。
...
* エラー原因：Tabletのreorganization SCNが一致しません。これは、Transferや分割後にTabletが変更された可能性があるために発生する可能性のあるシナリオです。

* 解決策：再試行で成功する場合もありますが、連続して失敗する場合は、OceanBaseの技術サポートに連絡し、調査を依頼してください。
...
* エラー原因：SQLステートメントでカンマが欠けているか、カンマの使用が正しくありません。
* 解決策：SQLステートメントを注意深く確認し、カンマが欠けているか、カンマの使用が誤っている箇所を探します。
...
* エラー原因：XMLフラグメントを必要なデータ型に変換する際、データ型がマッチしないか無効です。
* 解決策：データ型またはXMLデータが正しいか確認してください。
...
* 解決策：文字列の長さまたはデータサイズを確認し、すべてのデータを収容できる十分なサイズであることを確保してください。
...
* 解決策：使用しているXPath式が正しいかどうか、およびデータベースのバージョンがそのXPath式の使用をサポートしているかどうかを確認してください。
...
* エラー原因：EXTRACTVALUE関数を使用する際、非葉ノードから値を抽出しようとしました。
* 解決策：XPath式が正しいかどうか確認し、式が葉ノードを指していることを確認してください。
...
* エラーの原因：無効なXMLドキュメントの挿入を試みました。
* 解決策：
   1. XMLデータの確認：挿入しようとしているXMLデータが完全であるか確認し、データがXMLドキュメント仕様に準拠していることを保証します。
   2. XMLType データ型の使用：XMLデータを個別のXMLType列として格納するために、XMLType データ型を使用できます。
...
* 解決策：
   1. MAPまたはORDERメソッドの定義：オブジェクトの定義内でMAPまたはORDERメソッドを定義し、ソート時に使用できるようにします。MAPメソッドはオブジェクトをスカラー値にマッピングするために使用され、ORDERメソッドは2つのオブジェクトの大小を比較するために使用されます。
   2. ソート方法の変更：MAPまたはORDERメソッドを定義できない場合は、ソート方法を変更し、他の方法でソートすることを検討できます。例えば、他の列を使用してソートする方法です。
...
* エラーの原因：XMLELEMENT() 関数のn番目の属性パラメータにエイリアスが設定されていません。
* 解決策：
...
* エラーの原因：データ型の変更を試みた際、データ型の変更が許可されていないか、変更後のデータ型が無効でした。
* 解決策：
   1. データ型の確認：変更しようとしているデータ型が変更を許可しているか確認し、変更後のデータ型が有効であることを保証します。
   2. データサイズの確認：変更しようとしているデータサイズを確認し、変更後のデータ型がすべてのデータを収容できることを保証します。
...
* エラーの原因：XMLノードを更新する際に、無効なノードまたはノードタイプを使用して更新値を指定しました。
...
* エラー原因：LOB値にアクセスしようとした際、その値が存在しないためです。
...
* エラー原因：指定された列`%.*s`はベクトルインデックスの作成をサポートしていません。
...
* OceanBaseエラーコード：7603

* エラーの原因：ベクトルインデックスのメモリ設定が不足しています。

* 解決策：さらなる設定が必要な場合は、パラメータ `memstore_limit_percentage` を変更してください。技術サポートにお問い合わせいただき、調査のご協力をお願いいたします。

## ERROR 7604 (22000) : Using the vsag interface returns an error

* OceanBase エラーコード：7604

* エラー原因：vsagインターフェースからエラーが返されました。
