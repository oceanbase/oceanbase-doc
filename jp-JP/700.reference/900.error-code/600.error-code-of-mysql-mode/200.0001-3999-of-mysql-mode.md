...
このエラーコード範囲は、MySQLとの互換性を持つエラーコードを示します。
...
* [データ挿入時に重複した値とのエラー、エラーコード ERROR 1062](https://www.oceanbase.com/knowledge-base/oceanbase-database-1000000000217891)
...
* [Unit仕様変更時のエラー：Parse error](https://www.oceanbase.com/knowledge-base/oceanbase-database-1000000000207664)
* [MySQLテナントでの列単位の権限付与失敗](https://www.oceanbase.com/knowledge-base/oceanbase-database-1000000000207670)
* [テーブル作成時にグローバルパーティションインデックスの作成がサポートされない](https://www.oceanbase.com/knowledge-base/oceanbase-database-1000000000207733)
* [テーブル作成時のglobal partition index定義エラー](https://www.oceanbase.com/knowledge-base/oceanbase-database-1000000000426280?back=kb)
...
* エラーの原因：XPath 式を使用する際、式の形式が正しくないか無効です。
...
* 解決策：使用している XPath 式が正しいか、データベースのバージョンがその XPath 式の使用をサポートしているか確認してください。
...
* [mysqldumpツールでデータをエクスポートする際のエラー、エラーコード1109](https://www.oceanbase.com/knowledge-base/oceanbase-database-20000000193)
...
* OceanBase エラーコード：9203

* MySQL 互換エラーコード：1128

* エラーの原因：対応するプラグイン機能がありません。通常は、プラグインがインストールされていないか、プラグインの初期化に失敗した場合です。

* 解決策：プラグインが正しくインストールされ、初期化が完了しているか確認してください。
...
* エラー原因：ユーザー '%.*s' に対して、ホスト '%.*s' でこの権限が定義されていません。
* 解決策：ユーザー '%.*s' がホスト '%.*s' で正しい権限を持っており、その権限が正しく定義されていることを確認してください。
...
* [テーブルをクエリしたときにテーブルが存在せず、エラーコードERROR 1146が表示される](https://www.oceanbase.com/knowledge-base/oceanbase-database-1000000000217889)
* [OBServerの再起動失敗、エラーコード5019](https://www.oceanbase.com/knowledge-base/oceanbase-database-20000000042?back=kb)
* [CONSISTENCY_LEVELフィールドが-1に設定されている](https://www.oceanbase.com/knowledge-base/oceanbase-database-20000000089)
...
* [OceanBaseデータベースのBLOB列には直接インデックスを作成できない](https://www.oceanbase.com/knowledge-base/oceanbase-database-1000000000207744)
...
* エラー原因：並行トランザクション間でリソース競合が発生し、トランザクション同士がロックの解放を待ち続けることでデッドロックが形成された。
...
* [小規模テナントリソースプール(1C2G)作成失敗](https://www.oceanbase.com/knowledge-base/oceanbase-database-1000000000207672)
...
* [逆順インデックス指定でエラー、エラーコード1235](https://www.oceanbase.com/knowledge-base/oceanbase-database-20000000104?back=kb)
* [DDLステートメントで外部キー追加エラー、エラーコード1235](https://www.oceanbase.com/knowledge-base/oceanbase-database-20000000067?back=kb)
* [ノード間データアクセスエラー、エラーコード1235](https://www.oceanbase.com/knowledge-base/oceanbase-database-20000001060?back=kb)
* [データ復元でエラー1235 should not have backup_dest and restore_source at the same time not supported](https://www.oceanbase.com/knowledge-base/oceanbase-database-1000000000208053)
...
* [ALTER TABLEでテーブルに主キーを追加する際にエラーが発生する](https://www.oceanbase.com/knowledge-base/oceanbase-database-20000001005)
* [パーティションテーブルではRANGEインデックスのDROP操作をサポートしていません](https://www.oceanbase.com/knowledge-base/oceanbase-database-20000000111)
* [sysテナントでエラーERROR 1235(0A000): uncertain plan violates external consistency not supported](https://www.oceanbase.com/knowledge-base/oceanbase-database-1000000000008215)
...
* エラー原因：現在のMySQLバージョンでは、JSON_TABLE式内の列に非文字列型のデフォルト値を指定することはサポートされていません。
...
* エラーの原因：現在のバージョンでは、JSONモードで参照を使用することはサポートされていません。
...
* [データ挿入時にデータが切り捨てられ、エラーコードERROR 1265が発生する](https://www.oceanbase.com/knowledge-base/oceanbase-database-1000000000217850)
...
* エラーの原因：この構文は推奨されておらず、将来のバージョンで削除されます。
...
* [パーティションテーブルJOINでエラーが発生し、エラーコードは1317です](https://www.oceanbase.com/knowledge-base/oceanbase-database-20000000018?back=kb)
...
* [OceanBaseデータベースのMySQLモードにおけるLONGTEXTフィールドの互換性問題](https://www.oceanbase.com/knowledge-base/oceanbase-database-1000000000207745)
...
* [DMLステートメントが関数を呼び出す際にtable is mutatingというエラーが発生する](https://www.oceanbase.com/knowledge-base/oceanbase-database-1000000000210009)
...
* エラー原因：プリペアドステートメントに自己再帰呼び出しが含まれています。
...
## ERROR 1452 (23000): Cannot add or update a child row: a foreign key constraint (%.*s.%.*s) fails

* OceanBaseエラーコード：5315
* MySQL互換エラーコード：1452
* エラーの原因：子テーブル（外部キーテーブル）へのデータ挿入または更新時に外部キー制約に違反しました。通常、これは親テーブル（参照先テーブル）の主キーまたは一意キーに指定された値が存在しないために発生します。
* 解決策：

  * 子テーブル（外部キーテーブル）にデータを挿入または更新する前に、親テーブル（参照先テーブル）に対応する主キー値が存在することを確認してください。
  * 外部キー関係の両側のデータ型と値が完全に一致しているか確認してください。
  * この制約が本当に不要な場合は、その外部キー制約を削除することを検討できます（データの整合性への影響を慎重に評価する必要があります）。

関連する問題事例：

* [自己参照外部キースキーマでの一括INSERTデータエラー -1452](https://www.oceanbase.com/knowledge-base/oceanbase-database-1000000000409003)
...
* 解決策：テーブルのパーティション定義に応じて、正しい構文を使用してください。
...
* [パーティションテーブル作成時のエラー、エラーコード1499/OBE-00600](https://www.oceanbase.com/knowledge-base/oceanbase-database-20000000076?back=kb)
* [パーティションテーブル作成時のエラー、エラーコード1499](https://www.oceanbase.com/knowledge-base/oceanbase-database-20000010175?back=kb)
...
* 解決策：
  * パーティショニング方式の変更：RANGE/LIST パーティションと HASH/KEY パーティションを同時にサブパーティションで使用する場合は、互換性のあるパーティショニング方式に変更することを検討してください。パーティショニング方式の変更やパーティションテーブルの分割は、既存のデータやアプリケーションに影響を与える可能性があるため、慎重に操作してください。
...
* MySQL 互換エラーコード：1517
* エラー原因：テーブルのパーティション名を変更する際、その名前が既存のパーティション名と重複していました（MySQL自体にサブパーティションの概念がないため、パーティションとサブパーティションの名前がこのエラーコードを共有します）。
...
* エラー原因：
  ...
  * サポートされているプラグインタイプ（現在OceanBaseデータベースは`mysql_native_password`と`caching_sha2_password`タイプをサポート）ですが、プログラムによってプラグインが正しくロードされていません。`caching_sha2_password`はV4.4.2 BP2バージョンからサポートされています。
...
* エラーの原因：ドキュメントを解析しようとした際に、XML パーサーがエラーを返しました。

* 解決策：解析対象のドキュメントが有効かどうか確認してください。

<main id="notice" type='explain'>
  <h4>説明</h4>
  <p>このエラーコードは V4.2.2 以降で導入されました。</p>
</main>
...
* エラーの原因：作成しようとするイベント名が既存のイベント名と競合します。

* 解決策：新しいイベントに、他と重複しない一意の名前を選択してください。

<main id="notice" type='explain'>
  <h4>説明</h4>
  <ul><li>V4.3.x バージョンでは、このエラーコードは V4.3.2 以降で導入されました。</li><li>V4.2.x バージョンでは、このエラーコードは V4.2.4 以降で導入されました。</li></ul>
</main>
...
* エラーの原因：アクセスまたは操作しようとするイベントは存在しません。

* 解決策：イベント名が正しいかどうか確認し、対象のイベントが存在するかどうか確認してください。
...
* 解決策：`ALTER EVENT` ステートメントの構文が正しいことを確認してください。変更するプロパティと値を含めて正しいか確認してください。
...
* 解決策：INTERVALの値が正の数であることを確認してください。間隔値が大きすぎる場合は、要件とシステムの制限に応じて適切な範囲に調整してください。
...
* エラーの原因：イベントの終了時刻(ENDS)が無効であるか、開始時刻(STARTS)よりも早い場合。

* 解決策：ENDSタイムスタンプが正しく、STARTS時刻より後であることを確認します。
...
* エラーの原因：イベント作成時に設定した開始時刻が現在時刻よりも早い場合。

* 解決策：イベントの開始時刻を将来の特定の時点に更新し、有効であることを確認します。
...
* 解決策：現在のユーザーにイベントを削除する権限が付与されていることを確認します。
...
* エラー原因：イベントの名前を変更しようとした際、新しい名前が既存の名前と同じであるため、操作が無効になりました。

* 解決策：ALTER EVENT ステートメントで、現在のイベント名とは異なる新しい名前を指定します。

<main id="notice" type='explain'>
  <h4>説明</h4>
  <ul><li>V4.3.x バージョンでは、このエラーコードは V4.3.2 バージョンから導入されました。</li><li>V4.2.x バージョンでは、このエラーコードは V4.2.4 バージョンから導入されました。</li></ul>
</main>
...
* エラー原因：ある列に挿入するデータの長さが、その列で定義されている最大長を超えています。
...
* エラー原因：DDL操作が、直接的または間接的に自身を再度変更しようとしています。イベント（EVENT）定義に本体（body）が含まれる場合、イベントのDDLステートメントの再帰実行は禁止されています。

* 解決策：DDLステートメントを確認し、作成または変更中のイベントを直接的または間接的に参照していないことを確認してください。
...
* エラーの原因：イベントは作成後すぐに期限切れとなり、自動的に削除されました。

* 解決策：イベントの開始時刻を将来の特定の時点に設定し、将来実行されるようにします。
...
* 解決策：変更後のイベントの実行時間を未来に設定してください。これにより、イベントが正常にスケジュールされます。

<main id="notice" type='explain'>
  <h4>説明</h4>
  <p>V4.3.x系では、このエラーコードはV4.3.5バージョンから、エラーメッセージが<code>Event execution time is in the past and ON COMPLETION NOT PRESERVE is set. The event was not changed. Specify a time in the future.</code>から<code>Event execution time is in the past. The event was not changed. Specify a time in the future.</code>に調整されました。</p>
</main>
...
* エラーの原因：交換を試みたテーブルがパーティションテーブルである。
...
* OceanBaseエラーコード：4772

* MySQL互換エラーコード：1736

* エラーの原因：2つのテーブルの定義が異なります。
...
* [SQL実行時に-6226エラーが発生する](https://www.oceanbase.com/knowledge-base/oceanbase-database-1000000000209999)
...
* エラー原因：特定の列は、外部キー制約でSET NULLとして設定する必要があるため、NOT NULLに設定できません。
...
* OceanBase エラーコード：9137

* MySQL 互換エラーコード：1831

* エラーの原因：インデックスを作成しようとした際、そのインデックスが既に存在していたためです。

* 解決策：指定したインデックスが存在するか確認し、既存のインデックスを削除または変更した後、新しいインデックスを再作成します。
...
* エラー原因：role '%.*s' が付与されていないか、存在しません。
...
* [MySQLモードでユーザーがロックされた場合、エラーコードERROR 3118](https://www.oceanbase.com/knowledge-base/oceanbase-database-1000000000217856)
* [Oracleモードでユーザーがロックされた場合、エラーコードERROR 3118](https://www.oceanbase.com/knowledge-base/oceanbase-database-1000000000217857)
...
* 解決策：操作対象のユーザー %.*s が存在することを確認するか、他の有効なユーザーで操作してください。
...
* [GIS機能における空間データ型フィールドおよび関連関数の実行エラー ERROR 3548](https://www.oceanbase.com/knowledge-base/oceanbase-database-1000000000245401)
...
* エラーの原因：再帰共通テーブル式（CTE）には UNION ALL 操作が含まれている必要があります。

* 解決策：再帰 CTE 内で UNION ALL 操作を使用します。

<main id="notice" type='explain'>
  <h4>説明</h4>
  <p>このエラーコードは V4.2.1 以降のバージョンから、エラーメッセージが <code>recursive WITH clause must use a UNION ALL operation</code> から <code>Recursive Common Table Expression should contain a UNION ALL</code> に調整されました。</p>
</main>
...
* エラーの原因：共通テーブル式の再帰クエリブロック内では、ORDER BY、LIMIT、SELECT DISTINCT、HAVING、WINDOW FUNCTION、GROUP BY の使用はサポートされていません。

* 解決策：共通テーブル式の再帰クエリブロック内の共通式で ORDER BY、LIMIT、SELECT DISTINCT、HAVING、WINDOW FUNCTION、GROUP BY を使用しないでください。

<main id="notice" type='explain'>
  <h4>説明</h4>
  <p>このエラーコードは V4.2.1 以降のバージョンから、エラーメッセージが <code>Recursive Common Table Expression can contain neither aggregation nor window functions in recursive query block</code> から <code>ORDER BY / LIMIT / SELECT DISTINCT / HAVING / WINDOW FUNCTION / GROUP BY in recursive query block of Common Table Expression not supported</code> に調整されました。</p>
</main>
...
* エラーの原因：RANGEフレームを持つウィンドウに、日付時刻型に基づくORDER BY式が含まれています。INTERVAL境界値のみが許可されます。

* 解決策：RANGEフレームを持つウィンドウのORDER BY式で日付時刻型の境界値を使用せず、INTERVAL境界値のみを使用するようにします。
...
* エラーの原因：RANGEフレームを持つウィンドウに、数値型に基づくORDER BY式が含まれています。INTERVAL境界値の使用は許可されません。

* 解決策：数値型に基づくORDER BY式を含むウィンドウで、INTERVAL境界値を使用しないでください。
...
* エラーの原因：配列またはオブジェクトをスカラーカラムに格納できません。

* 解決策：配列またはオブジェクトをスカラー値に変換するか、配列やオブジェクトの格納に適したデータ型に列を変更します。
...
* MySQL 互換エラーコード：3757
* エラー原因：関数インデックスを作成する際に、BLOBまたはTEXT型を返す式を使用しました。BLOBまたはTEXT型の式は関数インデックスの作成に使用できません。
* 解決策：CAST関数を使用してください。CAST関数を使用すると、BLOBまたはTEXT型の式を他の型に変換でき、変換後の式を使用して関数インデックスを作成できます。
...
* MySQL 互換エラーコード：3758
* エラー原因：関数インデックスを作成する際に、サポートされていない関数を使用しました。インデックス関数でSYSDATEやユーザー環境が使用されています。
...
* エラー原因：パラメータに有効でないJSONタイプが使用されており、実際にはオブジェクトタイプです。
...
* MySQL 互換エラーコード：3855
* エラー原因：列にパーティション関数の依存関係があり、そのため列を削除または名前変更できません。
...
* エラー原因：VALUES句内の各行には少なくとも1つの列が必要です。INSERT文のデータソースとして使用される場合を除きます。
...
* エラー原因：VALUES句ではデフォルト値を使用できません。INSERT文のデータソースとして使用される場合を除きます。

* 解決策：INSERT文のデータソースとして使用される場合を除き、VALUES句でデフォルト値を使用しないでください。
