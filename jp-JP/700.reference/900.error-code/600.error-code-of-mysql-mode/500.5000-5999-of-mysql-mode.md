...
* 解決策：SQLステートメント内のCASTステートメントが正しいか確認します。例えば、暗黙的変換が存在するかどうかを確認します。
...
* 解決策：EXECUTEでバインドする変数の数とPREPAREで必要なパラメータ数が一致するようにします。
...
* [OracleモードでUpdateしてClobを更新する際にエラーが発生し、ret_code=-5083となる場合](https://www.oceanbase.com/knowledge-base/oceanbase-database-1000000000322504) -->
...
* エラーの原因：`TIME_TO_USEC` 関数のパラメータは `VARCHAR`、`TIME`、`DATATIME`、または `TIMESTAMP` 型でなければなりません。
...
* エラーの原因：`USEC_TO_TIME` 関数のパラメータは `TINYINT`、`MEDIUMINT`、`INT` または `BIGINT` 型でなければなりません。
...
* [テーブル作成時にINFORMATION_SCHEMA.STATISTICSビューを参照してエラー5114が発生した場合](https://www.oceanbase.com/knowledge-base/oceanbase-database-1000000000279883) -->
...
* エラー原因：実行計画を追加する際、計画キャッシュ内に同じ計画が既に存在します。

* 解決策：内部エラーコードです。サポートチームに連絡し、調査の協力を依頼してください。
...
* 解決策：再試行してください。問題が継続する場合は、サポートチームに連絡し、調査の協力を依頼してください。
...
* エラー原因：Table groupが存在しません。無効なTable group名が指定されています。
...
* エラー原因：作成したテナント名が重複しているか、内部エラーが発生しました。

* 解決策：テナント名が重複している場合は、テナント名を変更して再作成してください。その他の場合は、サポートチームに連絡し、調査の協力を依頼してください。
...
* OceanBase エラーコード：5226

* エラー原因：プランキャッシュの使用メモリが上限に達し、自動的にエージェントがトリガーされます。
...
* [Oracleモードの通常ユーザーが読み取り専用ビューを作成後、DROPまたはREPLACEできない](https://www.oceanbase.com/knowledge-base/oceanbase-database-1000000000207728) -->
...
* エラー原因：フィールド定義が `NULL` と `NOT NULL` の両方を同時に指定しています。
...
* OceanBase エラーコード：5260

* エラー原因：テナントがロックされています。

* 解決策：テナントのロックを解除します。

## ERROR 5264 (HY000) : Outline '%.\*s' already exists

* OceanBase エラーコード：5264

* エラー原因：`OUTLINE` は既に存在します。
...
* エラー原因：リサイクルバインドのオブジェクトには操作できません。

* 解決策：オブジェクトをリサイクルバインドから復元してから操作します。

## ERROR 5270 (HY000) : object not in RECYCLE BIN

* OceanBase エラーコード：5270

* エラー原因：オブジェクトはリサイクルバインドにありません。
...
* エラー原因：`OUTLINE` は既に作成されています。

* 解決策：以前に作成した `OUTLINE` が期待通りか確認します。期待通りであれば、再度 `OUTLINE` を作成する必要はありません。期待通りでない場合は、`DROP OUTLINE` してから再度作成できます。
...
* 解決策：レート制限を作成するステートメントに既存のレート制限ルールが含まれていないか確認します。
...
* エラーの原因：プラン生成時に利用可能なレプリカが選択されませんでした。
...
* エラーの原因：gapまたはoverlap時間が使用されています。
...
* [OBServerログにout of px worker resourceが表示される原因](https://www.oceanbase.com/knowledge-base/oceanbase-database-1000000000267156)
...
* [OceanBaseデータベースログob_px_admissionにout of px worker resourceエラーが表示される](https://www.oceanbase.com/knowledge-base/oceanbase-database-1000000000209995) -->
...
* エラーの原因：外部キーフィールドの長さは、参照先フィールドの長さより短くてはなりません。
...
* エラーの原因：公有シノニムの使用時に DATABASE を指定しました。

* 解決策：公有シノニムの使用時には、DATABASE を実行する必要はありません。

## ERROR 5328 (HY000) : Looping chain of synonyms

* OceanBase エラーコード：5328

* エラーの原因：循環依存のシノニムが作成されました。

* 解決策：循環依存のシノニムを確認し、循環依存を解消するために異なる名前のシノニムを作成します。

## ERROR 5329 (HY000) : Cannot create a synonym with same name as object

* OceanBase エラーコード：5329
...
* エラーの原因：SQL のある式にウィンドウ関数が含まれています。

* 解決策：SQL の該当する式からウィンドウ関数を削除します。
...
* エラー原因：`UPDATE` 操作により、行が属するパーティションが変更されます。

* 解決策：`ALTER TABLE` ステートメントを使用して、テーブルの `ENABLE ROW MOVEMENT` 属性を有効にします。
...
* エラーの原因：SEQUENCEオブジェクトに `SELECT`、`ALTER` 以外の権限が付与されています。
...
* エラーの原因：テーブルに `EXECUTE` 権限が付与されています。
...
* エラーの原因：VIEWオブジェクトに `ALTER`、`INDEX`、`EXECUTE` 権限が付与されています。
...
* エラーの原因：ユーザーのロールを取り消す際、ロールがユーザーに付与されていないことが判明しました。
...
* 解決策：ユーザーに再付与権限を追加してください。

## ERROR 5381 (HY000) : duplicate username in list

* OceanBase エラーコード：5381

* エラーの原因：`GRANT` ステートメント内に重複するユーザーがいます。

* 解決策：余分なユーザーを削除してください。
...
* 解決策：ロールに付与された `REFERENCES` または `INDEX` 権限を削除してください。
...
* 解決策：`CASCADE CONSTRAINTS` を指定してください。
...
* 解決策：OceanBase データベースは、ユーザーが自己自身に権限を付与または取り消すことをサポートしていません。自己自身の権限を変更する必要がある場合は、システム管理者に連絡し、関連する操作を実行してください。

## ERROR 5386 (HY000) : Unable to read data -* Table definition has changed

* OceanBase エラーコード：5386
* MySQL 互換エラーコード：1412
* エラーの原因：テーブルの定義が変更されており、読み取り対象のスナップショットポイントが古すぎて、正しく読み取るまたは解析することができません。
* 解決策：テーブルの定義に変更があったかどうか確認し、列の追加や削除、データ型や長さの変更などがないか確認してください。
...
* エラー原因：DDLスレッドの実行中に同名のオブジェクトが存在します。
...
* エラー原因：パラメータタイプが期待と異なります。TIMESTAMP_TO_SCN 関数に渡すパラメータはTimestamp型である必要があります。
...
* エラー原因：パラメータタイプが期待と異なります。SCN_TO_TIMESTAMP 関数に渡すパラメータは整数型である必要があります。
...
* エラーの原因：生成列または関数インデックスの値に対する正当性チェックで、格納されている値が、現在の環境で生成列またはインデックスの定義に基づいて再計算された値と異なることが検出されました。これは、システム変数の影響を受ける計算結果、または本来動作が不確定な組み込み関数を使用して生成列やインデックスを定義したために発生する可能性があります。`_enable_defensive_check` 設定パラメータが有効な場合、防御的にエラーが報告されます。
* 解決策：生成列と関数インデックスの定義を確認し、システム変数の変更が計算結果の変化を引き起こしたことが確定した場合は、生成列/インデックスを削除して再作成します。それ以外の場合や問題が解決しない場合は、技術サポートにお問い合わせいただき、調査のご協力をお願いいたします。
...
* エラーの原因：現在のセッションで設定されているシステム変数の値が、関数インデックスまたは生成列を作成時に固定された値と異なります。計算時には、関数インデックスや生成列は、作成当初に固定された値を使用し、セッション内の現在の変数値ではありません。
...
* エラーの原因：不正なウィンドウ関数の使用。例えば、不合理なパラメータ設定などです。

* 解決方法：ウィンドウを修正し、正しいウィンドウ関数にします。
...
* エラー原因：LISTパーティションに既にデフォルトパーティションが存在するため、新しいパーティションを追加できません。

* 解決策：新しいパーティションを追加せず、パーティションの分割によって新しいパーティションを追加します。
...
* エラー原因：使用中の一時テーブルに対してインデックスの作成、変更、または削除を試みました。

* 解決策：一時テーブルを使用しているすべてのセッション接続を切断してから、操作を繰り返してください。
...
* エラーの原因：ステートメントで指定された値が多すぎます。これは `INSERT` ステートメントでよく発生します。

* 解決策：値の数を調整するか、サポートチームに連絡して調査を依頼します。

## ERROR 5614 (HY000) : not enough values

* OceanBase エラーコード：5614

* エラーの原因：ステートメントで指定された値が不足しています。これは `INSERT` ステートメントでよく発生します。
...
* エラーの原因：スカラーサブクエリの式に誤りがあり、1行を返すことが期待されているにもかかわらず、実際には複数行が返されています。

* 解決策：スカラーサブクエリの式を確認し、実行ごとに1行の結果セットのみを返すようにします。

## ERROR 5616 (HY000) : UPDATE ... SET expression must be a subquery

* OceanBase エラーコード：5616

* エラーの原因：rowset assignment のシナリオでは、`UPDATE` ステートメントの `SET` 句は `subquery` として指定する必要があります。

* 解決策：`UPDATE` の `SET` 句の構文を調整するか、サポートチームに連絡して調査を依頼します。
...
* エラーの原因：クエリブロックでANSIスタイルの結合を使用する場合、外部結合（+）を指定するための古い表記法は使用できません。
...
* エラー原因：年のうちの日付（1 - 366）とユリウス日が競合します。
...
* エラーの原因：日付時刻形式として `AM`、`A.M.`、`PM` または `P.M.` を指定する必要があります。

## ERROR 5651 (HY000) : BC/B.C. or AD/A.D. required

* OceanBase エラーコード：5651

* エラーの原因：日付時刻形式として `BC`、`B.C.`、`AD` または `A.D.` を指定する必要があります。
...
* 解決策：number format modelが構文要件を満たしているか確認します。
...
* 解決策：SQL内の`LIKE`ステートメントの`ESCAPE`フィールドの後の文字が要件を満たしているか確認します。

## ERROR 5666 (HY000) : escape character must be character string of length 1

* OceanBase エラーコード：5666

* エラーの原因：`LIKE`計算時に`ESCAPE`フィールドの後の文字が1文字ではありません。

* 解決策：`ESCAPE`フィールドの後の文字を1文字に指定します。
...
* エラーの原因：`ORDER BY` 項目が `SELECT` 項目の正しい番号ではありません。
...
* [JSON SQLの実行でthe interval is invalidエラーが発生した場合](https://www.oceanbase.com/knowledge-base/oceanbase-database-20000045546) -->

## ERROR 5677 (HY000) : PL/SQL: numeric or value error: %.\*s

* OceanBase エラーコード：5677

* エラーの原因：PL実行中に無効な数値や他のデータ型に遭遇しました。
...
* MySQL互換エラーコード：4028
* エラーの原因：関数インデックスはテーブルに隠れた列を追加します。テーブルに隠れた列がある状態で、そのテーブルのすべての可視列(visible column)を削除するとこのエラーが返されます。
* 解決策：データテーブルの列定義とメタデータを確認し、削除操作後に少なくとも1つの列が可視列であることを確認してください。
...
* エラーの原因：内部エラーコード。PXにSQLの再試行を指示します。
...
* 解決策：DMLに関連する列データが既存の`CHECK`制約を満たしているか確認してください。

## ERROR 5694 (HY000) : group function is not allowed here

* OceanBase エラーコード：5694

* エラーの原因：不正な場所に集約関数が記述されています。

* 解決策：SQL内の集約関数を調整してください。
...
* エラーの原因：汎用的なラベルセキュリティエラーです。詳細は具体的なエラーメッセージを参照してください。

* 解決策：エラーメッセージに基づき、対応する操作を修正してください。
...
* 解決策：インデックスが有効かどうか確認してください。削除してから再度作成することもできます。
...
* エラー原因：Profile関連のDDLで指定された `FAILED_LOGIN_ATTEMPTS`、`PASSWORD_LOCK_TIME` が有効なINT型ではないか、値が `0` です。
...
* エラー原因：Profileがユーザーに割り当てられているため、CASCADEなしでは削除できません。
...
* エラー原因：`time_zone` システム変数を設定する際、時刻が有効範囲を超えています。
...
* エラー原因：`time_zone` システム変数を設定する際、分が有効範囲を超えています。
...
* エラー原因：`INSERT`、`UPDATE`、`DELETE` ステートメントを使用して、更新できないビュー（例：生成列、式を変更する列、複数のテーブルからデータを取得する場合）を変更しようとしました。

* 解決策：ベーステーブルを直接変更するか、ビューを再定義してください。
...
* エラー原因：削除対象のテーブルにkey-preserved tableが存在しない、または複数のkey-preserved tableが存在する、あるいはそのkey-preserved tableが読み取り専用ビューから派生している場合。

* 解決策：ビューを再定義するか、基となるテーブルに対して削除操作を実行します。

## ERROR 5722 (HY000) : cannot modify a column which maps to a non key-preserved table

* OceanBase エラーコード：5722

* エラー原因：JOINビュー内のnon-key-preserved tableに対する挿入または更新を試みた場合。

* 解決策：直接、基となるテーブルを変更します。

## ERROR 5723 (HY000) : cannot perform a DML operation on a read-only view

* OceanBase エラーコード：5723

* エラー原因：`INSERT`、`DELETE`、`UPDATE` ステートメントを使用して、読み取り専用に定義されたビューを変更しようとした場合。

* 解決策：直接、基となるテーブルを変更するか、ビューを更新可能なビューに変更します。
...
* エラー原因：名前変更対象のテーブルオブジェクトには、ユーザー名を含めることはできません。
...
* エラーの原因：内部エラーコード。PXが実行を終了する必要がある場合、このエラーコードをブロードキャストしてすべての実行ノードに通知し、現在の実行を終了します。
...
* 解決策：`SYS_CONNECT_BY_PATH` 関数に別の区切り文字を指定します。指定した区切り文字がどの列値にも含まれないようにしてください。
...
* 解決策：OceanBase データベースに再度ログインし、新しいサーバーセッションが自動的に作成されるようにします。詳細については、セッショントレースファイルを確認してください。
...
* OceanBase エラーコード：5787

* エラーの原因：batched multi-stmt の実行方式を満たしていません。

* 解決策：処理は不要です。Server は自動的に単一実行にロールバックします。

## ERROR 5788 (HY000) : cannot select FOR UPDATE from view with ISTINCT, GROUP BY, etc

* OceanBase エラーコード：5788

* エラーの原因：マージされていないビューから `FOR UPDATE` を選択しようとしました。ビューがマージされていないのは、ビューがマージを妨げる構文（例えば `DISTINCT` または `GROUP BY`）を使用しているか、初期化ファイルのパラメータ設定によるものです。

* 解決策：ビューがマージされていない原因を特定し、ビューをマージ可能にするように変更してください。
...
* エラー原因：既にポリシーで保護されているテーブルにポリシーを適用しようとしています。
...
* エラー原因：制約で参照されている親キー列を削除しようとしています。
...
* エラー原因：`:=` を使用してユーザー変数に代入する場合、`SELECT` 文内でのみ使用可能であり、式のルートノードである必要があります。
...
* エラーの原因：以下のいずれかの可能性があります：
  ...
  * 組み込み関数に渡された文字列データがデータベースの文字セット以外の文字セットを使用しており、その組み込み関数がその文字セットを受け付けていません。
...
* 解決方法：内部エラーコードです。サポートチームに連絡し、調査を依頼してください。
...
* エラー原因：正規表現の `pattern` に無効なマッチパラメータが指定されています。
...
* エラー原因：正規表現エンジンの状態遷移機械に異常が発生しました。
...
* エラー原因：インデックスがVARRAY配列の最大長を超えているか、ネストされたテーブルではインデックスが大きすぎます。
...
* 解決策：PLで参照されているオブジェクトは既に存在しており、重複して作成することはできません。
...
* エラー原因：左辺でオブジェクト、`LOB`、またはその他の複合パーティションを参照していますが、オブジェクト、`LOB`、またはその他の複合パーティションは初期化されていません。

* 解決策：適切なコンストラクタまたは完全オブジェクト代入を使用して複合パーティションを初期化します。
...
* 解決策：適切なコンストラクタまたはオブジェクト代入を使用してコレクションを初期化します。
...
* エラーの原因：これは内部エラーメッセージです。PL プログラムでエラーが検出されました。

* 解決策：内部エラーコードです。サポート技術者に連絡し、調査を依頼してください。
...
* エラーの原因：PL は他のストレージを割り当てることができません。このエラーは、制御不能なプログラムによって引き起こされる場合があります。

* 解決策：

  1. PL プログラム内で追加のメモリの問題やエラーが使用されていないことを確認します。
...
* エラーの原因：インデックスが `VARRAY` の制限値を超えているか、インデックスが `VARRAY` またはネストされたテーブルの非正の値です。

* 解決策：プログラムのロジックを確認し、必要に応じて `VARRAY` の制限値を増やします。
...
* エラーの原因：特定のタイプの `Member` メソッドが `NULL SELF` 引数によって呼び出されています。

* 解決策：メソッド呼び出しのタイプを変更し、有効な `SELF` 引数を渡します。

## ERROR 5848 (HY000) : timeout occurred while waiting for a resource

* OceanBase エラーコード：5848

* エラーの原因：通常は Dead インスタンスが原因です。

* 解決策：Dead または回復していないインスタンスがないか確認します。ある場合は、回復させてください。
...
* 解決策：主キーまたは `CHECK` 制約が `NOT NULL` 制約の実施を強制している場合、その制約を削除します。
...
* 解決策：OceanBaseデータベースの対応するバージョンの「リファレンスガイド」を参照し、そのパラメータの取り得る範囲を確認してください。
...
* エラーの原因：`CHECK` 制約を持つ列のデータ型を変更しようとしました。しかし、`CHECK` 制約を持つ列は、データ型を `CHAR` から `VARCHAR` にのみ変更できます。
...
* エラーの原因：`ALTER TABLE` または `ALTER INDEX` ステートメントが `RENAME` 操作を他の操作と結合しようとしたことが原因です。これは不正です。
...
* エラーの原因：デフォルトのパーティション記述を生成するリクエスト（`PARTITIONS number-of-partitions` による）と同時に、`<(sub)partition-description>` 句を指定することは、無効です。
...
* 解決策：サブパーティションの名前を変更し、テーブルまたはインデックス内で一意であることを確認します。

## ERROR 5868 (HY000) : Incorrect usage of UPDATE and ORDER BY

* OceanBase エラーコード：5868

* エラーの原因：複数テーブルを `UPDATE` する際に、`ORDER BY` を使用できません。

* 解決策：`ORDER BY` を削除します。

<!-- 関連する問題例：

* [MySQLモードでUpdate + Order byエラー5868が発生した場合](https://www.oceanbase.com/knowledge-base/oceanbase-database-1000000000209975) -->

## ERROR 5869 (HY000) : Incorrect usage of UPDATE and LIMIT

* OceanBase エラーコード：5869

* エラーの原因：複数テーブルを `UPDATE` する際に、`LIMIT` を使用できません。
...
* エラーの原因：入力日付にFXおよびFM形式コードを指定した場合、数字の桁数は形式コードで指定された桁数と完全に一致している必要があります。例えば、`9` は形式指示子DDにマッチしませんが、`09` はマッチします。
...
* エラーの原因：サブパーティションテーブル以外に対してサブパーティションメンテナンス操作を実行しました。
...
* エラーの原因：LISTサブパーティションに `VALUES (value list)` 句を指定していません。
...
* エラーの原因：`CREATE TABLE` または `ALTER TABLE` ステートメントの `REFERENCES` 句で指定された列リストに対応する、参照テーブルの一意キーまたは主キー制約が存在しません。
...
* エラーの原因：テーブル作成時に、同一の参照列と参照される列を持つ複数の外部キー制約が存在します。

* 解決策：`CREATE TABLE` ステートメントを修正し、同一の参照列と参照される列を持つ外部キー制約が1つのテーブルに存在しないようにします。

## ERROR 5891 (HY000) : such a referential constraint already exists in the table

* OceanBase エラーコード：5891

* エラーの原因：テーブル変更時に、同一の参照列と参照される列を持つ複数の外部キー制約が存在します。

* 解決策：`ALTER TABLE` ステートメントを修正し、同一の参照列と参照される列を持つ外部キー制約が1つのテーブルに存在しないようにします。

## ERROR 5893 (HY000) : specified data type is not supported for a virtual column

* OceanBase エラーコード：5893

* エラーの原因：生成列のデータ型は現在サポートされていない `LOB` 型です。
...
* エラーの原因：外部キーの参照列が指定されていない場合、外部キーの主表に主キーがありません。

* 解決策：外部キーを作成する前に、参照列に主キーまたは一意インデックスを作成する必要があります。
...
* エラーの原因：列の最大長の制限を小さくすることは現在サポートされていません。
...
* 解決策：PL 関数は純粋関数（`RNDS`、`RNPS`、`WNDS`、`WNPS`）である必要があります。
...
* 解決策：そのステートメントを再度実行しますが、仮想列の値は設定しません。
...
* 解決策：識別列から `NOT NULL` 制約を削除しないでください。制約を削除する唯一の方法は、`ALTER TABLE MODIFY` の `DROP IDENTITY` を使用して、列の `identity` 属性を削除することです。

## ERROR 5913 (HY000) : column to be modified is not an identity column

* OceanBase エラーコード：5913

* エラーの原因：識別列ではない列の識別属性を変更しようとしています。
...
* エラーの原因：識別列を持つ既製のテーブルにインスタンス化ビューを作成しようとしています。

* 解決策：`identity` 属性を削除するか、インスタンス化ビュー定義から識別列を削除してください。
...
* 解決策：`GENERATED ALWAYS` キーワードで生成される識別列には直接挿入できません。代わりに、関連するシーケンスジェネレーターからその値を取得します。

## ERROR 5920 (HY000) : cannot update a generated always identity column

* OceanBase エラーコード：5920

* エラーの原因：`GENERATED ALWAYS` キーワードで作成された識別列を更新しようとしています。
...
* エラーの原因：`EXCHANGE` で指定された2つのテーブルの識別列のシーケンスが増加も減少もしていません。

* 解決策：`INCREMENT BY` を持つ識別列が同じ符号のシーケンスを持っていることを確認します。

## ERROR 5922 (HY000) : cannot rename a system-generated sequence

* OceanBase エラーコード：5922

* エラーの原因：システム生成のシーケンスを名前変更しようとしています。
...
* エラーの原因：存在しないストアドプログラムを実行しようとしています。

* 解決策：正しい名前を指定していることを確認します。
...
* エラーの原因：新しいエンジンでは、再帰階層が深すぎる場合にエラーが発生します。深さの制限は2の30乗です。古いエンジンでは、ルートノードから現在のノードへの経路上で、すべてのノードとその子ノードの数が一定の値（2の30乗）を超える場合にもエラーが発生します。現在設定されているこの値は固定されており、通常はこの値を超える問題には遭遇しません。

* 解決策：現時点では解決策を見つけることができません。メモリサイズを増やしてもこの問題は解決しません。唯一の解決策は、再帰階層を縮小するか、データ量を減らすことです。
...
* 解決策：ホスト名またはIPアドレスの綴りを確認し、ホスト名またはIPアドレスが有効であることを確認します。
...
* 解決策：関連するウィンドウ関数が作成されていることを確認し、正しいウィンドウ関数名を使用します。
...
* [PL 積み重ね呼び出しで refcursor がエラー 5930 を返す場合](https://www.oceanbase.com/knowledge-base/oceanbase-database-1000000000422799?back=kb) -->

## ERROR 5931 (HY000) : argument is null, invalid, or out of range

* OceanBase エラーコード：5931

* エラー原因：渡されたパラメータ値が `NULL`、無効、または範囲外です。例えば、`LOB / FILE` の位置またはサイズパラメータの値が 1 から (4GB - 1) の範囲を超えている場合や、無効なオープンモードでファイルを開いた場合などです。

* 解決方法：プログラムを確認し、`NULL`、無効、または範囲外のパラメータ値を渡さないよう修正します。
...
* エラー原因：無効なカーソルからアクセスを試みました。このエラーが発生する原因は以下のとおりです：
  ...
  * 最後の行まで取得して `OBE-1403` エラーが返された後、カーソルからアクセスを試みた場合。

  * `FOR UPDATE` 句を使用してカーソルを開いた後、`COMMIT` を実行してからアクセスを試みた場合。

  * SQL ステートメント内のすべてのプレースホルダーを再バインドし、そのステートメントを再度実行する前にアクセスを試みた場合。

  <main id="notice" type='explain'>
    <h4>説明</h4>
    <p>PL カーソルのループでは暗黙的に取得が行われるため、このエラーが発生する可能性もあります。</p>
  </main>

* 解決方法：

  * 最後の行まで取得した後、`FETCH` ステートメントを実行しないでください。

  * `FOR UPDATE` 句を使用して開いたカーソルに対して、`COMMIT` を実行した後にアクセスを試みないでください。

  * SQL ステートメントを再バインドして実行した後、アクセスを試みてください。
...
* 解決方法：\[1，2048\] の範囲内の `SIZE` 値を指定してください。

## ERROR 5975 (HY000) : \ndblink remote error code: %d,\nremote error msg: %.*s"

* OceanBase エラーコード：5975

* エラー原因：リモートデータベース接続でエラーが発生し、そのエラーコードを OceanBase エラーコードにマッピングできません。

* 解決方法：エラーメッセージ内のエラーコードに基づいて、関連する原因を調査します。

## ERROR 5976 (HY000) : can not find the expected version of OCI LIB: %.*s

* OceanBase エラーコード：5976

* エラーの原因：期待されるバージョンのOCIライブラリが見つかりません。
...
<main id="notice" type='explain'>
  <h4>説明</h4>
  <p>このエラーコードはV4.3.0以降で、エラーメッセージが <code>oci lib not founded</code> から <code>can not find the expected version of OCI LIB: %.*s</code> に調整されました。</p>
</main>