...
## ERROR 5727 (HY000) : SQL request should be rerouted

* OceanBase エラーコード：5727

* エラーの原因：現在の SQL リクエストは、他の OBServer ノードにリダイレクトして実行する必要があります。

* 解決策：

  * OBProxy の正常な動作フロー。
  * アプリケーションログでこのエラーが頻繁に発生する場合、OBProxy のルーティング設定が不適切である、クラスタトポロジーが頻繁に変化している、ネットワークまたはノードの安定性に問題があることを示している可能性があります。
