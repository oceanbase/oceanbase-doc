...
## ERROR 5107 (42000) : IS演算子に対して無効なオペランドタイプ、lval=%s
...
## ERROR 5319 (HY000) : 子列\'%.\*s\'のデータ長は、親列\'%.\*s\'のデータ長より短くてはなりません
...
## ERROR 5320 (HY000) : \\'%.\*s\\' : 外部キー制約で使用されています
...
## ERROR 5325 (HY000) : ORDER SIBLINGS BY句はここでは許可されていません
...
## ERROR 5326 (HY000) : SYS_CONNECT_BY_PATH関数を使用する場合、列値の一部としてセパレータを含めることはできません
...
## ERROR 5327 (HY000) : パブリックシノニムではデータベースを指定できません
...
## ERROR 5329 (HY000) : オブジェクトと同じ名前のシノニムを作成できません
...
## ERROR 5330 (HY000) : シノニム%sの翻訳はもはや有効ではありません
...
## ERROR 5331 (HY000) : 名前は既に既存のオブジェクトによって使用されています
...
## ERROR 5337 (HY000) : rowidが間違っています
...
## ERROR 5338 (HY000) : ここではウィンドウ関数を使用できません
...
## ERROR 5772 (HY000) : ORDER SIBLINGS BY句がここでは許可されていません
...
## ERROR 5773 (HY000) : CONNECT_BY_ISCYCLE疑似列にはNOCYCLEキーワードが必要です
...
## ERROR 5774 (HY000) : 関数の引数が不足しています
...
## ERROR 5778 (HY000) : constraint (%.\*s) を有効にできません - そのような制約は存在しません

* OceanBaseエラーコード：5778

* エラーの原因：このテーブルには指定された制約が存在しません。

## ERROR 5779 (HY000) : constraint (%.\*s) を無効にできません - そのような制約は存在しません

* OceanBaseエラーコード：5779

* エラーの原因：このテーブルには指定された制約が存在しません。

## ERROR 5780 (HY000) : PX DOPを%ldから%ldにダウングレード

* OceanBaseエラーコード：5780

* エラーの原因：正常終了。

## ERROR 5781 (HY000) : parallel_max_serversがCPUリソース不足により%ldから%ldにダウングレードされました。非推奨
...
## ERROR 5785 (HY000) : (%.\*s.%.\*s) の検証に失敗しました - 親キーが見つかりません

* OceanBaseエラーコード：5785

* エラーの原因：このテーブルに孤立したサブレコードがあるため、テーブル変更操作の検証制約に失敗しました。

## ERROR 5786 (HY000) : 列チェック制約は他の列を参照できません
...
## ERROR 5787 (HY000) : バッチマルチステートメント実行にはロールバックが必要です
...
## ERROR 5788 (HY000) : ISTINCT、GROUP BYなどを使用したビューからFOR UPDATEを選択できません
...
## ERROR 5838 (HY000) : 参照されたコレクションが初期化されていません
...
## ERROR 5840 (HY000) : PL/SQL: プログラムエラー
...
## ERROR 5841 (HY000) : PL/SQL: 結果セット変数またはクエリの戻り値の型がマッチしません
...
## ERROR 5842 (HY000) : PL/SQL: ストレージエラー
...
## ERROR 5844 (HY000) : 無効なカーソル

* OceanBaseエラーコード：5844

* エラーの原因：無効なカーソルです。

## ERROR 5845 (HY000) : 無効なユーザー名/パスワード。ログオン拒否
...
## ERROR 5847 (HY000) : NULL SELF引数に対するメソッドディスパッチは許可されていません
...
## ERROR 5848 (HY000) : リソース待機中にタイムアウトが発生しました
...
## ERROR 5849 (HY000) : 変更対象の列がNOT NULLであり、すでにNOT NULLです

* OceanBaseエラーコード：5849

* エラーの原因：`NOT NULL`に変更する列はすでに`NOT NULL`です。

## ERROR 5850 (HY000) : NULLに変更する列はNULLに変更できません
...
## ERROR 5866 (HY000) : CREATE TABLEまたはCREATE INDEXで\<(sub)partition-description\>句を指定できません
...
## ERROR 5868 (HY000) : 更新とORDER BYの誤用
...
## ERROR 5875 (HY000) : テーブルは複合パーティションメソッドによってパーティション化されていません
...
## ERROR 5895 (HY000) : テーブルパーティショニング列のデータ型または長さを変更できません

* OceanBaseエラーコード：5895

* エラーの原因：パーティション列のデータ型と長さは変更できません。

## ERROR 5896 (HY000) : テーブルパーティショニング列のデータ型または長さを変更できません

* OceanBaseエラーコード：5896

* エラーの原因：サブパーティション列のデータ型と長さは変更できません。

## ERROR 5897 (HY000) : 一部の値が大きすぎるため、列の長さを短縮できません

* OceanBaseエラーコード：5897

* エラーの原因：現在、列の最大長制限を短くすることはサポートされていません。

## ERROR 5898 (HY000) : Datetime/Interval内部エラー
...
## ERROR 5899 (HY000) : パーティション拡張テーブル名はリモートオブジェクトを参照できません
...
## ERROR 5901 (HY000) : ビューはここでは適切ではありません
...
## ERROR 5902 (HY000) : キー保持テーブルなしでは、結合ビューからROWIDを選択またはサンプリングできません

* OceanBaseエラーコード：5902

* エラーの原因：キー保持テーブルがない場合、結合ビューまたはサンプルから `ROWID` を照会できません。

## ERROR 5903 (HY000) : DISTINCT、GROUP BYなどを含むビューからROWIDを選択またはサンプリングできません

* OceanBaseエラーコード：5903

* エラーの原因：`DISTINCT` または `GROUP BY` などを含むビューから `ROWID` を照会できません。

## ERROR 5904 (HY000) : テーブルには少なくとも1つの仮想でない列が必要です
...
## ERROR 5905 (HY000) : インデックス付けできるのは純粋関数のみです
...
## ERROR 5906 (HY000) : 仮想列式に指定できるのは純粋関数のみです
...
## ERROR 5907 (HY000) : 仮想列への更新操作は許可されていません
...
## ERROR 5908 (HY000) : 無効な列式が指定されました
...
## ERROR 5909 (HY000) : テーブルには同一のIDENTITY列を複数持つことができません
...
## ERROR 5910 (HY000) : 同一列に無効なNOT NULL制約が指定されました
...
## ERROR 5911 (HY000) : 同一列に対するNOT NULL制約を変更できません
...
## ERROR 5912 (HY000) : 同一列に対するNOT NULL制約を削除できません
...
## ERROR 5913 (HY000) : 変更対象の列は同一列ではありません
...
## ERROR 5914 (HY000) : 同一列にデフォルト値を設定できません
...
## ERROR 5915 (HY000) : 同一列は数値型である必要があります
...
## ERROR 5916 (HY000) : プリビルドテーブルの管理列は同一列にすることができません
...
## ERROR 5917 (HY000) : システム生成シーケンスを変更できません
...
## ERROR 5918 (HY000) : システム生成シーケンスを削除できません
