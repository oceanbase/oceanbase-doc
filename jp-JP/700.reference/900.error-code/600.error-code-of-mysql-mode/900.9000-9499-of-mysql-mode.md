...
* エラーの原因：バックアップ・リストア設定の宛先リンクが誤っている可能性があります。
...
* エラーの原因：OSSファイルのデータ検証に誤りがあります。これは、OSS上のデータに異常なエラーが発生した可能性があります。
...
* [バックアップパスの設定に失敗し、エラーメッセージとして OSS errorが表示された場合](https://www.oceanbase.com/knowledge-base/oceanbase-database-1000000000208059)
* [バックアップ・リストアに失敗し、テナントがリストア中の状態が続く場合](https://www.oceanbase.com/knowledge-base/oceanbase-database-1000000000340640)

## ERROR 9004 (HY000) : Init MD5 fail

* OceanBase エラーコード：9004

* エラーの原因：OpenSSLの初期化におけるMD5エラーです。
...
* エラー原因：OSSファイルの長さが無効です。誤ったOSSファイルを指定した可能性があります。
...
* [異地バックアップからの復元時に「バックアップオブジェクトを指定した時点に復元できません」というエラーが発生します](https://www.oceanbase.com/knowledge-base/oceanbase-database-1000000000208064)

## ERROR 9012 (HY000) : Can not get data version from timestamp

* OceanBaseエラーコード：9012

* エラー原因：論理復元のパラメータに`data version`が渡されていません。
...
* エラー原因：OSSファイルへの書き込み中に異常エラーが発生しました。OSSに未知の問題が発生した可能性があります。

* 解決策：OSSの技術サポートにお問い合わせいただき、具体的な原因を調査してください。

## ERROR 9014 (HY000) : Another restore is in progress

* OceanBaseエラーコード：9014

* エラー原因：復元するテナント名が既存のテナント名と重複しています。

* 解決策：復元するテナントのユーザー名に重複がないか確認します。重複がある場合は、重複しないテナント名を使用してください。
...
* 解決策：OceanBaseサポートチームにお問い合わせいただき、調査をお手伝いいたします。
...
* エラー原因：既に復元済みのパーティションに対して、復元タスクが重複してスケジュールされています。
...
* 解決策：特に処理は不要です。物理バックアップが完了すると、自動的に削除されます。
...
* エラー原因：Stop段階のログアーカイブは、再度無効にできません。

* 解決策：既に無効になっているログアーカイブを再度無効にすることを避けてください。
...
* 解決策：大量の書き込みによりログアーカイブが追いつかない状況がないか確認してください。該当しない場合は、OceanBaseテクニカルサポートにお問い合わせいただき、調査をご依頼ください。
...
* エラー原因：ログアーカイブがRunning状態ではありません。通常は、バックアップ停止時にOBServerノードが出力するエラーです。
...
* エラー原因：ログアーカイブのRound情報が正しくありません。
...
* エラー原因：該当レプリカは現在移行レプリケーションまたはログレプリカであり、データバックアップ機能を提供できません。

* 解決策：OceanBaseデータベースは内部で再試行します。人為的な介入は不要です。

## ERROR 9032 (HY000) : backup info not exists

* OceanBaseエラーコード：9032

* エラー原因：バックアップ情報が存在しません。

* 解決策：OceanBaseサポートチームにお問い合わせいただき、調査をお手伝いいたします。

関連する問題事例：

* [バックアッププロセスが長時間実行され、ログに9032が表示される場合](https://www.oceanbase.com/knowledge-base/oceanbase-database-1000000000208050)
...
* エラー原因：バックアップの状態が一致しません。

* 解決策：

  * OBServerノードのログアーカイブでエラーが発生した場合、通常はログアーカイブの起動・停止時に発生します。人為的な介入は不要です。

  * Root Serviceのログの場合は、OceanBaseサポートチームにお問い合わせください。具体的な原因を調査します。
...
* 解決策：復元されたテナント内で、内部テーブルのロックインデックスの状態が`error`のインデックスを確認し、そのインデックスを再作成してください。

## ERROR 9036 (HY000) : Backup is in progress

* OceanBaseエラーコード：9036

* エラー原因：一般的には、バックアップと排他的な操作が実行されたことが原因です。
...
* エラー原因：`set member list` フェーズではレプリケーションをサポートしていません。

* 解決策：復元の `set member list` ステップではレプリカの複製をサポートできません。この時点で多数派レプリカが存在する場合は、復元を続行できます。多数派レプリカが存在しなくなっている場合は、復元をキャンセルしてから復元タスクを再スケジュールする必要があります。
...
* 解決策：内部エラーコードです。リーダー切り替えを検出すると、現在のPGのログアーカイブプロセスを中断します。新しいリーダーが自動的にバックアップ作業を再開するため、運用側の介入は不要です。

## ERROR 9040 (HY000) : backup can not start

* OceanBase エラーコード：9040

* エラー原因：ログアーカイブが `DOING` 状態ではないため、データバックアップを開始できません。

* 解決策：ログアーカイブが `DOING` 状態になるのを待ちます。長時間 `BEGINNING` 状態が続く場合は、OceanBaseテクニカルサポートに連絡し、具体的な原因を調査してもらう必要があります。
...
* エラー原因：データバックアップが16回以上の `Major Merge` 中に完了しなかった場合、自動的に失敗します。

* 解決策：データバックアップが遅延している原因を調査するか、`Major Merge` が頻繁すぎないか確認してください。
...
* 解決策：入力された `backup_set_id` が正しいか確認し、再試行してください。
...
* エラーの原因：NFSがマウントされていないOBServerノードが存在します。
...
* 解決策：OceanBaseのテクニカルサポートにお問い合わせいただき、調査をお手伝いいたします。
...
* 解決策：クリーンアップタスクがない状態で`Cancel`を発生させないようにしてください。
...
* 解決策：エラーメッセージを記録し、OceanBaseテクニカルサポートにお問い合わせいただき、調査をお手伝いいたします。
...
* エラーの原因：内部エラーコードです。主に、バックアップの内部スケジューリング中に、Backup SetまたはBackup Pieceが最大Backup Times要件を満たしていない場合に使用されます。
...
* 解決策：通常はOceanBaseデータベースのCOSアダプタロジックに誤りがあるため、OceanBaseサポートチームにお問い合わせください。

## ERROR 9061 (HY000) : IO limit

* OceanBase エラーコード：9061

* エラー原因：I/O制限によるものです。現在、このエラーコードはテンセントクラウドのCOSモジュールのList操作でのみ返されます。

* 解決策：COSのカスタマーサポートにお問い合わせし、List操作のレート制限を緩和または解除してください。

## ERROR 9062 (HY000) : reach backup backup copy limit

* OceanBase エラーコード：9062

* エラー原因：指定されたBackup Timesにバックアップをスケジュールする際、すべてのbackup_setとbackup_pieceがBackup Timesの要件を満たしており、バックアップが必要なbackup_setまたはbackup_pieceが見つからないことが検出されました。
...
* 解決策：
  ...
  * この問題が偶発的に発生する場合、処理は不要です。

  * この問題が頻繁に発生する場合、バックアップのCOSカスタマーサポートにお問い合わせし、List操作の頻度制限を緩和または解除してください。
...
* エラー原因：バックアップI/O独立サブプロセスのセッション競合。
...
* エラー原因：アーカイブログを連続して読み取る際、ログが既にリサイクルされていることが検出されました。
...
* 解決方法：ターゲット位置にフォーマットファイルが存在するか確認してください。存在しない場合は、そのフォーマットファイルを作成する必要があります。

<main id="notice" type='explain'>
  <h4>説明</h4>
  <p>このエラーコードはV4.3.0バージョンから、エラーメッセージが<code>the format file does not exist under the destination</code>から<code>format file does not exist%s</code>に調整されました。</p>
</main>
...
* エラー原因：ターゲット位置のフォーマットファイルが期待と一致しません。
* 解決方法：ターゲット位置のフォーマットファイルが期待される内容と一致していることを確認してください。

<main id="notice" type='explain'>
  <h4>説明</h4>
  <p>このエラーコードはV4.3.0バージョンから、エラーメッセージが<code>the content of the format file at the destination does not match</code>から<code>format file does not match%s</code>に調整されました。</p>
</main>
...
* エラー原因：バックアップのログストリームの進捗チェックポイントがフラッシュタイムアウトしました。
...
* エラー原因：アーカイブ時にアーカイブファイルがすでに回収されていました。
...
* エラー原因：ログ復元のソーステナントとログストリームの状態が一致せず、現在のテナントをプライマリテナントに切り替えることは許可されません。
* 解決方法：
   1. バックアップ復元のソーステナントの状態とターゲットテナントの状態が一致しているか確認します。
   2. プライマリ/セカンダリレプリケーションの設定が正しいか確認します。
   3. ソーステナントのバックアップが完全であるか、ターゲットテナントの復元が正しく行われているか確認します。
...
* エラーの原因：ログ復元のソーステナントとログストリームの状態が一致せず、現在のテナントをプライマリテナントに切り替えることはできません。
* 解決策：
   1. バックアップから復元したソースログの状態とターゲットログの状態が一致しているか確認します。
   2. プライマリ/セカンダリレプリケーションの設定が正しいか確認します。
...
* エラーの原因：obesiプロセスのセッションが存在しません。

* 解決策：obesiプロセスは自動的に再起動するため、手動での処理は不要です。
...
* エラーの原因：既にARCHIVELOGモードにあります。

* 解決策：現在の状態を示すもので、処理は不要です。
...
* エラー原因：スタンバイデータベースのバージョンがプライマリデータベースのバージョンよりも古い。
...
* 解決方法：復元の設定とパラメータを確認し、復元ターゲットの要件と期待と一致していることを確認します。
...
* 解決策：お客様サポートにお問い合わせいただき、調査をご依頼ください。
...
* 解決策：内部エラーコードです。お客様サポートにお問い合わせいただき、調査をご依頼ください。
...
* 解決策：
  1. 入力されたBucketが存在するか確認してください。
  2. オブジェクトストレージのエンドポイント設定が正しいか確認してください。エンドポイント設定が正しく、オブジェクトストレージサービスに正常にアクセスできることを確認してください。
  3. Bucketエラーの可能性を除外した場合は、OceanBaseデータベースのテクニカルサポートに連絡し、調査を依頼してください。
...
* エラーの原因：指定されたテナントスナップショットが存在しません。
* 解決策：指定されたテナントスナップショットが正しいか確認してください。スナップショットが存在しない場合は、新しいスナップショットを作成するか、実際の状況に応じて後続の処理を行う必要がある可能性があります。
...
* エラーの原因：指定されたテナントスナップショットが既に存在します。
* 解決策：指定されたテナントスナップショットが既に存在するか確認してください。スナップショットが既に存在する場合は、他の操作を実行するか、実際の状況に応じて後続の処理を行う必要がある可能性があります。
...
* エラーの原因：テナントスナップショットタスクがタイムアウトしました。
...
* エラー原因：テナントのスナップショットロックで競合が発生しました。
* 解決策：スナップショットロックの競合の具体的な状況を確認し、競合するスナップショットロックを解放するか、ロックの使用方法を調整する必要がある場合があります。
...
* エラー原因：オブジェクトストレージ先が無効です。
* 解決策：オブジェクトストレージ先が正しいかどうか確認し、有効な先であることを確認してください。オブジェクトストレージ先の設定や構成を修正する必要がある場合があります。
...
* エラー原因：オブジェクトストレージ先に接続できません。
* 解決策：オブジェクトストレージ先への接続設定を確認し、ネットワークが正常に接続されていることを確認してください。接続設定の修正、オブジェクトストレージキーの確認、またはネットワーク接続問題の解決が必要な場合があります。
...
* エラー原因：オブジェクトストレージのエラーコードです。一般的な原因はオブジェクトストレージの接続性や互換性の問題です。
...
* エラーの原因：設定されたバックアップパスまたはアーカイブパスのソース側のZone/IDC/Regionが存在しません（例：クラスタにzone1、zone2しかないのに、ユーザーがbackup zoneを設定する際にzone=zone3、zone4と設定した場合、zone3、zone4が存在しないことをチェックしたときにこのエラーが発生します）。

* 解決策：`DBA_OB_ZONES` ビューを照会し、入力されたLocalityが存在するか確認し、修正後に再設定します。
...
* エラーの原因：一時ファイルのディスクへの書き込みデータ量が設定上限を超えています。

* 解決方法：現在エラーが発生しているSQLが予想以上のデータ量を生成していないか確認します（例：一時ファイルのディスクへの書き込みデータ量の最大予想値が500GBの場合）。予想通りであれば、構成パラメータ`temporary_file_max_disk_size`を適切な値に調整します。
...
* エラーの原因：バックアップ中にマルチバージョンのメジャーサブストレートがリサイクルされました。

* 解決方法：データバックアップを再開します。
...
* エラーの原因：バックアップテナントで透過的データ暗号化が有効な状態で、復元テナント時に秘密鍵のアドレスを指定しなかった、または指定したアドレスが誤っていました。

* 解決方法：復元コマンドで正しい秘密鍵アドレスを指定するか、復元前にセッション変数`SET @kms_encrypt_info`を設定して正しい秘密鍵アドレスを指定します。
...
* 解決策：バックアップまたはアーカイブパスに保持ポリシーが設定されているかどうか、およびバックアップまたはアーカイブパスを設定する際に`enable_worm`パラメータが正しく設定されているかどうかを確認してください。`enable_worm`パラメータが正しく設定されている場合は、OceanBaseテクニカルサポートにお問い合わせいただき、調査を依頼してください。
...
* 解決策：OceanBaseテクニカルサポートにお問い合わせいただき、調査を依頼してください。
...
* エラー原因：マクロブロックキャッシュディレクトリに予期しないファイルが出現しました。これはユーザーが自ら作成したファイルである可能性があります。

* 解決策：OceanBaseテクニカルサポートにお問い合わせください。

<main id="notice" type='explain'>
<h4>説明</h4>
<p>このエラーコードは、V4.4.0バージョンから導入されました。</p>
</main>
...
* エラーの原因：物理復元の実行中に、OBServerノードが指定されたバックアップメディア内のバックアップセットファイルに正常にアクセスできません。

* 解決策：オブジェクトストレージの接続性を確認し、接続性に問題がないことを確認した上で、テクニカルサポートにお問い合わせください。
...
* エラーの原因：`log_only` のクリーンアップポリシーは、ログアーカイブのみが有効でデータバックアップがない場合に適用されます。`log_only` クリーンアップポリシーが存在する場合、データバックアップのパスを設定することはできません。

* 解決策：まず `log_only` クリーンアップポリシーを削除し、その後データバックアップのパスを設定します。
...
* エラー原因：`log_only` のクリーンアップ戦略は、ログアーカイブのみが有効でデータバックアップが設定されていないシナリオに適用されます。データバックアップパスが既に設定されているシナリオでは、クリーンアップ戦略を `log_only` に設定することは許可されません。

* 解決策：`log_only` クリーンアップ戦略を使用する場合は、まず既に設定されているデータバックアップパスを削除する必要があります。
...
* エラー原因：データバックアップおよびアーカイブのクリーンアップを実行できません。原因として、関連テナントの状態が `NORMAL` 以外の状態（例：作成中の `CREATING` 状態、復元中の `RESTORE` 状態など）であるか、または関連テナントにクリーンアップ戦略が設定されていない（期限切れクリーンアップシナリオの場合）可能性があります。

* 解決策：テナントの状態を確認します。期限切れクリーンアップシナリオの場合、関連テナントにクリーンアップ戦略が正しく設定されているか確認する必要があります。
...
* 解決策：バックアップメディアを正しくマウントし、アーカイブ/バックアップ/復元操作を開始したテナントのすべてのOBServerノードが同一のバックアップメディアにアクセスできることを確認します。NFSの詳細な操作手順については、[NFSのデプロイ](../../../600.manage/600.backup-and-recovery/200.deploy-nfs.md)を参照してください。
...
* 解決策：バックアップメディアがNFSの場合、[NFSのデプロイ](../../../600.manage/600.backup-and-recovery/200.deploy-nfs.md)を参照し、指定された方法でマウントしてください。バックアップメディアがオブジェクトストレージの場合、対応するサービスプロバイダーにお問い合わせください。
...
* エラー原因：指定された時間に復元するためのバックアップセットを使用できません。

* 解決策：復元ポイントを再選択するか、他のバックアップセットパスを指定してください。

<main id="notice" type='explain'>
<h4>説明</h4>
<p>V4.4.2 バージョンでは、このエラーコードは V4.4.2 BP1 から導入されました。</p>
</main>
...
* エラーの原因：SSBlockファイルはサイズ縮小をサポートしていません。

* 解決策：`ALTER SYSTEM SET datafile_size` で指定されたサイズが現在のSSBlockファイルより大きいかどうか確認するか、テクニカルサポートにお問い合わせください。
...
* エラーの原因：設定などの問題により、自動拡張が開始されていないか、`datafile` の事前設定されたディスク使用リソースが上限に達しています。

* 解決策：`__all_virtual_disk_stat` テーブルの `free_size` フィールドが0かどうか確認します。0の場合、ディスク容量が使い果たされていることを意味します。`datafile` のしきい値を引き上げるか、ディスクを拡張する必要があります。