...
* [バックアップパスの設定に失敗し、エラーメッセージとして OSS errorが表示される](https://www.oceanbase.com/knowledge-base/oceanbase-database-1000000000208059)
* [バックアップ・リストアに失敗し、テナントがリストア中のままになる](https://www.oceanbase.com/knowledge-base/oceanbase-database-1000000000340640)
...
* [異地へのバックアップ・リストア時に「バックアップオブジェクトを指定したタイミングポイントに復元できません」というエラーが発生する場合](https://www.oceanbase.com/knowledge-base/oceanbase-database-1000000000208064)
...
* 解決策：復元するテナントのユーザー名に重複がないか確認し、重複する場合は、重複しないテナント名を使用してください。
...
* 解決策：ログアーカイブを再開するコマンドの重複スケジュールを避けてください。
...
* エラー原因：該当レプリカは現在移行レプリケーションまたはログレプリカとして使用されており、データバックアップを提供できません。
...
* 解決策：復元されたテナント内で、テーブルのロック状態が`error`のインデックスを確認し、そのインデックスを再作成してください。

## ERROR 9036 (HY000) : Backup is in progress

* OceanBaseエラーコード：9036

* エラー原因：通常はバックアップと排他的な操作が実行されたためです。
...
* 解決策：復元の `set member list` ステップではレプリカの複製をサポートできません。この時点で多数派レプリカが存在する場合、復元は続行できます。多数派レプリカが存在しなくなっている場合は、復元をキャンセルしてから復元タスクを再スケジュールする必要があります。
...
* 解決策：ログアーカイブが `DOING` 状態になるまで待機してください。長時間 `BEGINNING` 状態が続く場合は、OceanBaseテクニカルサポートに連絡し、具体的な原因を調査してもらう必要があります。
...
* エラー原因：データバックアップが16回以上の `Major Merge` の間に完了しなかった場合、自動的に失敗します。

* 解決策：データバックアップが遅延する原因を調査するか、`Major Merge` が頻繁すぎないか確認してください。
...
* 解決策：入力した `backup_set_id` が正しいか確認し、再試行してください。
...
* エラー原因：クリーンアップタスクが存在しない状態で`Cancel`が発生しました。

* 解決策：クリーンアップタスクが存在しない状態で`Cancel`を発生させないようにしてください。
...
* 解決策：エラーメッセージを記録し、OceanBaseのテクニカルサポートにお問い合わせいただき、調査をご依頼ください。
...
* エラーの原因：テンセントクラウドCOSのアダプタモジュールでエラーが発生しました。

* 解決策：通常、OceanBaseデータベースのCOSへのアダプタロジックに誤りがあるため、OceanBaseのサポートチームにお問い合わせください。

## ERROR 9061 (HY000) : IO limit

* OceanBaseエラーコード：9061

* エラーの原因：I/O制限が原因です。現在、このエラーコードはテンセントクラウドのCOSモジュールのList操作でのみ返されます。
...
* エラーの原因：指定されたBackup Timesでバックアップをスケジュールする際、すべてのbackup_setとbackup_pieceがBackup Timesの要件を満たしており、バックアップが必要なbackup_setまたはbackup_pieceが見つからないことが判明しました。
...
* エラーの原因：COS環境下でのバックアップ・リストア実行時のList操作がレート制限されました。

* 解決策：

  * この問題が偶発的に発生する場合は、処理は不要です。

  * この問題が頻繁に発生する場合は、バックアップ先のCOSカスタマーサポートにお問い合わせし、List操作の頻度制限を解除または緩和してください。
...
* 解決策：
  ...
  * V3.1.X以降：データバックアップを再開始してください。

## ERROR 9065 (HY000) : agent has suspended

* OceanBase エラーコード：9065

* エラーの原因：論理バックアップの復元で、バックアップ機能を一時停止した際に実行中のタスクがこのエラーコードを返します。
...
* エラーの原因：OBServerノードには、バックアップメディアに対する操作権限がありません。
...
* エラーの原因：バックアップI/O独立サブプロセスのセッション競合。
...
* 解決策：ユーザーはHuawei Cloud OBSの接続状態や書き込み状況を確認する必要があります。
...
* エラー原因：連続してアーカイブログを読み取り、最後に到達しました。
...
* エラー原因：バックアップのログストリームポイント押し進めがタイムアウトしました。
...
* OceanBase エラーコード：9094

* エラー原因：ログストリームの復元に失敗しました。復元対象のログストリームデータが破損している可能性があります。

* 解決策：テクニカルサポートにお問い合わせいただき、調査をご依頼ください。

## ERROR 9095 (HY000) : No tablet need backup

* OceanBase エラーコード：9095

* エラー原因：バックアップが必要なTabletはありません。
...
* エラー原因：スタンバイデータベースのバージョンがプライマリデータベースのバージョンよりも古いため、アップグレードが必要です。
* 解決策：スタンバイデータベースとプライマリデータベースのバージョンを確認し、スタンバイデータベースのアップグレード完了を待ちます。
...
* エラー原因：プライマリ/スタンバイ同期中にプライマリテナントが削除されました。
...
* エラーの原因：ストレージ層で対応するディレクトリまたはファイルが見つかりません。設定されたストレージディレクトリに問題がある可能性があります。
...
* OceanBaseエラーコード：9102

* エラーの原因：アクセスしたファイルまたはディレクトリに対する権限がありません。

* 解決策：OceanBaseのテクニカルサポートにお問い合わせいただき、調査をご依頼ください。

## ERROR 9103 (HY000) : too many open files

* OceanBaseエラーコード：9103

* エラーの原因：ファイルシステムで開いているFD（File Descriptor）が上限に達しました。

* 解決策：内部ロジックが再試行します。手動での介入は不要です。

## ERROR 9104 (HY000) : fail to commit direct load

* OceanBaseエラーコード：9104

* エラーの原因：ダイレクトロードの`COMMIT`に失敗しました。プライマリ切り替えが原因の可能性があります。

* 解決策：ダイレクトロードに失敗した場合は、再度インポートできます。

## ERROR 9105 (HY000) : S3 error

* OceanBaseエラーコード：9105
...
* 解決策：
  ...
  2. オブジェクトストレージのエンドポイント設定が正しいかご確認ください。エンドポイント設定が正しく、オブジェクトストレージサービスに正常にアクセスできることを確認してください。
...
* 解決策：指定されたテナントスナップショットが正しいか確認してください。スナップショットが存在しない場合は、新しいスナップショットを作成するか、実際の状況に応じて後続処理を行う必要があります。
...
* 解決策：指定されたテナントスナップショットが既に存在するか確認してください。スナップショットが既に存在する場合は、他の操作を実行するか、実際の状況に応じて後続処理を行う必要があります。
...
* 解決策：オブジェクトストレージ先が正しいかどうか確認し、有効な先であることを保証してください。オブジェクトストレージ先の設定を修正する必要がある場合があります。
...
* 解決策：オブジェクトストレージ先への接続設定を確認し、ネットワークが正常に接続されていることを確認してください。接続設定の修正、オブジェクトストレージキーのアクセス、またはネットワーク接続問題の解決が必要な場合があります。
...
* OceanBase エラーコード：9125

* エラー原因：マイクロブロックキャッシュが`disable`状態に設定されており、上位モジュールがマイクロブロックキャッシュにアクセスする際にこのエラーコードが返されます。

* 解決策：テクニカルサポートに連絡し、調査を依頼してください。

## ERROR 9126 (HY000) : ss_micro_cache has reached memory limit

* OceanBase エラーコード：9126

* エラーの原因：マイクロブロックキャッシュが使用するメモリがしきい値に達し、新しくキャッシュに追加されるマイクロブロックにメタを割り当てることができません。

* 解決策：通常はシステムの動作に影響しませんので、無視してください。

## ERROR 9129 (HY000) : object storage io error

* OceanBase エラーコード：9129
* エラーの原因：オブジェクトストレージのエラーコードです。一般的な原因はオブジェクトストレージの接続性や互換性の問題です。
* 解決方法：オブジェクトストレージの接続性を確認し、接続性に問題がないことを確認した後、技術サポートに連絡して調査を依頼してください。

## ERROR 9133 (HY000) : Backup zone or backup idc or backup region invalid

* OceanBase エラーコード：9133

* エラーの原因：設定されたバックアップパスまたはアーカイブパスのソース側のZone/IDC/Regionが存在しません（例えば、クラスタにzone1,zone2しかないのに、ユーザーがbackup zoneの設定でzone=zone3,zone4を設定した場合、zone3,zone4が存在しないとこのエラーが発生します）。

* 解決方法：`DBA_OB_ZONES` ビューを照会し、入力したLocalityが存在するか確認し、修正後に再設定してください。
...
* エラーの原因：一時ファイルがディスクに書き込まれるデータ量が設定上限を超えています。

* 解決方法：現在エラーが発生しているSQLが予想以上のデータ量を生成していないか確認してください（例えば、一時ファイルがディスクに書き込まれるデータ量の上限が最大500GBと予想されている場合）。予想通りであれば、構成パラメータ`temporary_file_max_disk_size`を適切な値に調整してください。

<main id="notice" type='explain'>
<h4>説明</h4>
<p>V4.3.5バージョンについては、このエラーコードはV4.3.5 BP1バージョンから導入されました。</p>
</main>
...
* エラーの原因：バックアップ中にマルチバージョンのメジャーサブストーブレットが回収されました。

* 解決方法：データバックアップを再開してください。
...
* エラー原因：バックアップテナントで透過的データ暗号化が有効な場合、リストアテナント時に秘密鍵のアドレスを指定しなかった、または指定したアドレスが誤っていた。
...
* エラー原因：保持期間中のオブジェクトストレージ上のオブジェクトを削除または変更しようとした。

* 解決策：バックアップまたはアーカイブパスに保持ポリシーが設定されているかどうか、およびバックアップまたはアーカイブパスを設定する際に `enable_worm` パラメータが正しく設定されているかどうかを確認します。`enable_worm` パラメータが正しく設定されている場合は、OceanBaseテクニカルサポートにお問い合わせください。
...
* OceanBaseエラーコード：9144

* エラーの原因：マクロブロックキャッシュディレクトリに予期しないファイルが存在します。ユーザーが自ら作成したファイルの可能性があります。
...
* 解決策：オブジェクトストレージの接続性を確認し、接続性に問題がないことを確認した上で、テクニカルサポートにお問い合わせいただき、調査をご依頼ください。
...
* 解決策：ファイルをクリーンアップするか、ディスククォータを調整してください。
...
* エラーの原因：`log_only` のクリーンアップポリシーは、ログアーカイブのみが有効でデータバックアップがない場合に適用されます。`log_only` クリーンアップポリシーが設定されている場合、データバックアップパスの設定は許可されません。
...
* エラーの原因：データバックアップおよびアーカイブのクリーンアップが開始できません。原因として、関連テナントの状態が `NORMAL` 以外の状態（例：作成中の `CREATING` 状態、復元中の `RESTORE` 状態など）であるか、関連テナントにクリーンアップポリシーが設定されていない（期限切れクリーンアップの場合）可能性があります。

* 解決策：テナントの状態を確認します。期限切れクリーンアップの場合、関連テナントにクリーンアップポリシーが正しく設定されているか確認する必要があります。
...
* 解決策：バックアップメディアを正しくマウントし、アーカイブ/バックアップ/復元操作を実行するテナントのすべてのOBServerノードが同一のバックアップメディアにアクセスできることを確認します。NFSのマウント手順の詳細については、[NFSのデプロイ](../../../600.manage/600.backup-and-recovery/200.deploy-nfs.md)を参照してください。
...
* エラーの原因：バックアップメディアの読み書きに強い一貫性がありません。

* 解決策：バックアップメディアがNFSの場合は、[NFSのデプロイ](../../../600.manage/600.backup-and-recovery/200.deploy-nfs.md)を参照し、指定された方法でマウントしてください。バックアップメディアがオブジェクトストレージの場合は、該当するサービスプロバイダーにお問い合わせください。
...
* エラー原因：指定されたバックアップセットID/アーカイブログピースがバックアップパス/アーカイブパスに存在しません。

* 解決策：指定されたパスが正しいか確認してください。

<main id="notice" type='explain'>
<h4>説明</h4>
<p>V4.4.2 バージョンでは、このエラーコードは V4.4.2 BP2 から導入されました。</p>
</main>
...
* 解決策：`__all_virtual_disk_stat` テーブルの `free_size` フィールドが0かどうか確認してください。0の場合、ディスク容量が使い果たされていることを意味します。`datafile` のしきい値を引き上げるか、ディスクを拡張する必要があります。