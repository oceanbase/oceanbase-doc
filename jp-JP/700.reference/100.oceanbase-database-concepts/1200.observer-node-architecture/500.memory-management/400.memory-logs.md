...
テナントの総メモリ量を集計します。各行は1つのテナントを表します。
...
テナント内の各ctxのメモリ統計です。ctxは粗粒度の分割方法であり、1つのテナントには複数のctxが存在します。
...
テナントctx下のより詳細なメモリ統計です。1つのctxには複数のmodが存在します。
...
PMメモリはすべてSQL実行中の一時メモリであり、SQL実行終了とともに解放されます。
