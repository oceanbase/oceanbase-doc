...
OceanBaseデータベースは、ログを通じてメモリの使用状況を確認できます。

## テナント統計

テナントの総メモリを統計し、各行が1つのテナントを表します。
...
テナント内の各ctxにおけるメモリ統計。ctxは粗粒度の分割方式であり、1つのテナントには複数のctxが存在します。
...
テナントctxにおけるより詳細なメモリ統計。1つのctxには複数のmodが存在します。
...
PMメモリはすべて、SQL実行中の一時的なメモリであり、SQL実行終了時に解放されます。
