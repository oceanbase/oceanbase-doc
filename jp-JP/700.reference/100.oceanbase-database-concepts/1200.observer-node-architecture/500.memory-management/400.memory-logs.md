...
OceanBaseデータベースでは、ログを通じてメモリ使用状況を確認できます。

## テナント統計

テナントの総メモリ使用量を集計します。各行は1つのテナントを表します。
...
テナント内の各ctxにおけるメモリ統計です。ctxは粗い分割方法であり、一つのテナントには複数のctxが存在します。
...
テナントのctx下におけるより詳細なメモリ統計です。一つのctxの下には複数のmodが存在します。
...
プランメモリはすべてSQL実行中の一時的なメモリであり、SQL実行終了とともに解放されます。
