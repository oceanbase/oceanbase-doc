...
本記事では、具体的な例を通じて、OceanBaseデータベースにおけるパーティションテーブルのパラレルクエリ、非パーティションテーブルのパラレルクエリ、複数テーブルのパラレルクエリの有効化と無効化、およびパラレル実行に関連するシステムビューについて説明します。

## パーティションテーブルのパラレルクエリの有効化

パーティションテーブルに対するクエリにおいて、クエリの対象パーティション数が1より大きい場合、システムは自動的にパラレルクエリを有効化し、並列度DOP値はシステムデフォルトで1に設定されます。
...
パーティションテーブルに対して、`PARALLEL`ヒントを追加してパラレルクエリを開始し、`dop`値を指定します。`EXPLAIN`コマンドで生成された実行計画を確認します。
...
実行計画から、パラレルクエリの`dop`値が8であることがわかります。クエリ対象のパーティションが配置されているOBServerの数が`dop`値以下の場合、ワーカースレッド（合計数は`dop`値に等しい）は一定の戦略に従って関連するOBServerに割り当てられます。クエリ対象のパーティションが配置されているOBServerの数が`dop`値を超える場合、各OBServerは少なくとも1つのワーカースレッドを起動し、合計で起動するワーカースレッドの数は`dop`値を超えます。
...
パーティションテーブルに対するクエリにおいて、クエリ対象のパーティション数が1以下の場合、システムはパラレルクエリを起動しません。以下の例のように、`ptable`に対するクエリにフィルタ条件`c1=1`を追加します。
...
実行計画から、クエリの対象パーティション数が1であり、システムがパラレルクエリを起動していないことがわかります。1つのパーティションに対するクエリでもパラレル実行を希望する場合は、`PARALLEL`ヒントを追加してパーティション内のパラレルクエリを実行するしかありません。`EXPLAIN`コマンドで生成された実行計画を確認します。
...
```
  ...
  <main id="notice" type='explain'>
    <h4>説明</h4>
    <p>クエリ対象のパーティション数が1の場合でも、ヒントを使用してパーティション内のパラレルクエリを実行したい場合は、対応するDOP値が2以上である必要があります。DOP値が空または2未満の場合、パラレルクエリは起動しません。</p>
  </main>

## 非パーティションテーブルのパラレルクエリの有効化

非パーティションテーブルは本質的には1つのパーティションしか持たないパーティションテーブルであるため、非パーティションテーブルに対するクエリでは、`PARALLEL`ヒントを追加する方法でのみパーティション内のパラレルクエリを開始できます。それ以外の場合は、パラレルクエリは起動しません。

以下の例のように、非パーティションテーブル `stable` を作成し、`stable` の全表データに対するスキャン操作を実行します。`EXPLAIN` コマンドで生成された実行計画を確認します。
...
実行計画から、非パーティションテーブルでヒントを使用しない場合、パラレルクエリは起動しないことがわかります。

非パーティションテーブルに対して、`PARALLEL` ヒントを追加してパーティション内の並列クエリを開始し、`dop` の値（2以上）を指定します。`EXPLAIN` コマンドを使用して生成された実行計画を確認します。
...
クエリでは、複数テーブルの `JOIN` クエリが最も一般的です。複数テーブルのシナリオでは、クエリのパーティション数がすべて1より大きい場合、各テーブルで並列クエリが採用されます。
...
`p1table` と `p2table` の `JOIN` 結果をクエリします。`JOIN` 条件は `p1table.c1=p2table.c2` です。得られた実行計画は以下のとおりです。
...
デフォルトでは、`p1table` と `p2table`（両方のテーブルのクエリ対象パーティション数が1より大きい）に対して並列クエリが採用され、デフォルトの `dop` 値は1です。同様に、`PARALLEL` ヒントを使用することで並列度を変更できます。

以下の例のように、`JOIN` 条件を `p1table.c1=p2table.c2` と `p2table.c1=1` に変更すると、`p2table` では単一のパーティションのみが選択されます。実行計画は以下のとおりです：
...
OceanBaseデータベースは、並列実行の実行状態および統計情報を確認するためのシステムビュー `GV$OB_SQL_AUDIT/V$OB_SQL_AUDIT` を提供しています。

`GV$OB_SQL_AUDIT/V$OB_SQL_AUDIT` には多数のフィールドが含まれていますが、並列実行に関連するフィールドは以下のとおりです：`qc_id`、`dfo_id`、`sqc_id`、`worker_id`。
