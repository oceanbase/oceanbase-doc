...
|slug|pl-stored-procedure-mysql-compatible-mode|

# ストアドプロシージャ

ストアドプロシージャは、直接戻り値を持たないサブルーチンです。パラメータタイプが `OUT` の場合、ストアドプロシージャは呼び出し元に値を返すこともできます。

## ストアドプロシージャの構造

PLストアドプロシージャの構造は次のとおりです：
...
ストアドプロシージャは `CREATE PROCEDURE` ステートメントを使用して作成します。MySQLモードでは、Oracleモードと比較して、以下のような顕著な違いがあります：
...
* MySQLモードのストアドプロシージャにはオーバーロードは存在しません。
...
**例2：`IN` パラメータと `OUT` パラメータを持つストアドプロシージャを作成します。**
