...
`ADD_TRUSTED_CERTIFICATE`は、CAルート証明書を追加するために使用されます。これにより、当該証明書で発行されたデジタル証明書を使用するOBserverクラスタやOMSなどのコンポーネントが、RPC認証を経てクラスタに安全に接続できるようになります。
...
| **パラメータ**     | **説明**                                  |
|------------|----------------------------------------|
| common_name | 証明書の一意の識別子であり、異なる証明書を区別するために使用されます。       |
| description | 証明書の用途や関連するクラスタ情報を記述します。          |
| content     | 証明書の詳細情報です。    |
