...
`REBUILD_INDEX` プロシージャは、ベクトルインデックスをフルリフレッシュ（つまり再構築）するために使用されます。
...
| idx_parameters | インデックスパラメータ。テーブル作成時に使用したベクトルインデックスのパラメータ文字列、例：`distance=l2, type=hnsw, lib=vsag`。詳細なパラメータについては、[ベクトルインデックスのパフォーマンス最適化](../../../../../640.ob-vector-search/200.ob-vector-index/600.vector-index-monitor-and-maintain.md)を参照してください。| 任意 |
| idx_parallel_creation | インデックスの並列構築度。| 任意 |

注意事項：

* インデックスの再構築時には、テーブル名、インデックス名、インデックス列を指定する必要があります。
* `delta_rate_threshold` を指定しない場合、実際にインデックスの再構築がトリガーされない可能性があります。強制的に再構築するには、`delta_rate_threshold=0` を指定する必要があります。
