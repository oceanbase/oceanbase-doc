...
| format | プランのフォーマット情報を指定します。オプションパラメータは以下のとおりです：<ul><li>`'BASIC'`：演算子ID、演算子名、演算子備考、式情報など、少量の情報を表示します。</li><li> `'TYPICAL'`：演算子ID、演算子名、演算子備考、オプティマイザー推定の出力行数、オプティマイザー推定の実行時間、式情報などの基本情報を表示します。このプランが実行されている場合、実際のフィードバック情報（実行行数、コストなど）も初回実行時のものを表示します。</li><li> `'ALL'`：演算子ID、演算子名、演算子備考、オプティマイザー推定の出力行数、オプティマイザー推定の実行時間、式情報、有効なHint、Query Block Name Trace情報、アウトライン情報、ベーステーブルの最適化情報、定数パラメータ化情報、制約情報、Plan Noteなど、豊富なプラン情報を表示します。</li><li> `'ADVANCED'`：豊富なプラン情報を表示します。プラン情報は'ALL'パラメータと同じですが、さらに複雑なプランについてはツリー構造情報も表示します。</li></ul>|
