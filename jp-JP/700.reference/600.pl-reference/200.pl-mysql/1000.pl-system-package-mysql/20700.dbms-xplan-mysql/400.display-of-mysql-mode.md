...
MySQLテナントはFunction Table機能を持たないため、`SELECT`ステートメント内で直接`DISPLAY`関数を使用して実行計画情報を出力できます。
