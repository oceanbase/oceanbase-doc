...
コレクションに少なくとも1つの要素が含まれている場合、`FIRST` と `LAST` はそれぞれ最初の要素と最後の要素のインデックスを返します（削除された要素は無視されます。`DELETE` 文でその要素のためにプレースホルダーが残されていた場合でもです）。
...
空でない可変配列に対して、`FIRST` は常に `1` を返します。任意の可変配列に対して、`LAST` は常に `COUNT` と等しくなります。
...
ネスト表において、`LAST` は `COUNT` と等しくなりますが、中央から要素を削除する場合を除き、`LAST` は `COUNT` より大きくなります。
...
上記の例では、`team.FIRST` と `team.LAST` を使用した `FOR LOOP` ステートメントを使用して、ネスト表 `name` を出力します。ネスト表は疎な構造になる可能性があるため、`FOR LOOP` ステートメントは `team.EXISTS(i)` が `TRUE` の場合にのみ `team(i)` を出力します。