...
ネスト表とは、PLプログラム内で配列を参照および模倣できる非永続的なテーブルです。
...
PLネスト表のテーブル型がサポートする演算子は以下の表のとおりです。

|    **演算子**     |                                                                                                     **説明**                                                                                                     |
|---------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| EXISTS(n)     | n番目の行が存在する場合、Trueを返します。                                                                                                                                                                                           |
| COUNT         | このPLテーブルの行数を返します。                                                                                                                                                                                                  |
| FIRST \| LAST | 最初の行と最後の行（最小値と最大値）の行番号を返します。PLテーブルにデータがない場合はNULLを返します。                                                                                                                                                                        |
| PRIOR(n)      | PLテーブルのn行目のレコードの前のレコードの行番号を返します。                                                                                                                                                                                      |
| NEXT(n)       | PLテーブルのn行目のレコードの次のレコードの行番号を返します。                                                                                                                                                                                      |
| DELETE        | <ul><li> DELETE：テーブル内のすべてのレコードを削除します。   </li><li> DELETE(n)：テーブルのn行目のレコードを削除します。   </li><li> DELETE(m,n)：行番号mからnまでのすべてのレコードを削除します。</li></ul>    |

ネスト表の例を以下に示します。PLテーブルのレコードを削除しても、他のレコードの行番号は移動しません。
...
BEGIN
    ...
    -- PRIORを使用して、末尾から順にレコードを出力する。このときCOUNTは4になる
...
概念的には、ネスト表は要素数が任意の一次元配列のようなものです。ネスト表と配列の主な違いは以下の通りです：

* 配列は要素数を宣言できますが、ネスト表はできません。ネスト表のサイズは動的に増やすことができます。

* 配列は常に密です。ネスト表は初期段階では密ですが、要素を削除することができるため、疎になることもあります。
