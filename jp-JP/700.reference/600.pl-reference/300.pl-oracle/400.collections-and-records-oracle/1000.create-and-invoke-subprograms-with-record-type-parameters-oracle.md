...
# レコード型パラメータを持つサブプログラムの作成と呼び出し

レコード型の変数は、他の一般的なデータ型の変数と同様に、サブプログラムへのパラメータ渡しが可能です。
...
**例：レコード型パラメータを持つサブプログラムの作成と呼び出し。**
...
その後、`SELECT INTO` ステートメントを使用して、レコード変数に値を代入する2つの方法を示しています。これら2つの変数の型の実際の構造は同じであるため、代入操作が可能です：`r_ware1 := r_ware2;`

最後に、レコード変数をパラメータとしてサブプログラムに渡し、変数の値を出力します。
