...
関数へのパラメータ渡し方は以下の3通りです：

* 位置表示法。

  パラメータ定義の順に値を渡します。構文は `argument_value1[,argument_value2 ...]` です。
...
* 名前表示法。
  ...
  パラメータ名と渡す値を明示的に指定します。パラメータ定義の順にこだわらず、構文は `argument1 => parameter1 [, argument2 => parameter2[, ...]]` です。
...
* 混合表示法。
  ...
  位置表示法と名前表示法を組み合わせて関数にパラメータを渡します。この方法を使用する場合、位置表示法で渡されるパラメータは名前表示法で渡されるパラメータの前に配置する必要があります。つまり、関数のパラメータ数に関係なく、名前表示法を使用するパラメータが1つある場合、その後のすべてのパラメータも名前表示法を使用しなければなりません。
...
`DETERMINISTIC` 機能は、関数が決定的であることを示します。つまり、同じ入力パラメータに対して常に同じ結果を返すことを意味します。これはクエリのパフォーマンス最適化において重要です。
...
デフォルト値を持つ関数を作成した後、関数呼び出し時にデフォルト値を持つパラメータに実際のパラメータ値を指定しない場合、関数はそのパラメータのデフォルト値を使用します。しかし、呼び出し元がデフォルトパラメータに実際のパラメータを提供した場合、関数は実際のパラメータ値を使用します。関数作成時には、入力パラメータにのみデフォルト値を設定でき、入力/出力パラメータには設定できません。
