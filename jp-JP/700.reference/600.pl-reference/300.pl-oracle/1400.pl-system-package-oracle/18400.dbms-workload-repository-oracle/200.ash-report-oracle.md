...
| BTIME | サンプリング開始時刻。 |
| ETIME | サンプリング終了時刻。 |
| SQL_ID | サンプリング対象のSQLの`SQL_ID`です。未入力または`NULL`を指定した場合、`SQL_ID`を制限しません。 |
| TRACE_ID | サンプリング対象のSQLの`TRACE_ID`です。未入力または`NULL`を指定した場合、`TRACE_ID`を制限しません。 |
| WAIT_CLASS | サンプリング対象のイベントタイプです。`WAIT_CLASS`の値は`GV$ACTIVE_SESSION_HISTORY.WAIT_CLASS`フィールドの値を参照してください。未入力または`NULL`を指定した場合、待機タイプを制限しません。 |
| REPORT_TYPE | レポートタイプです。現在は`TEXT`タイプのみをサポートしており、省略可能です。 |
| REPORT_TYPE | レポートタイプです。現在は`HTML`と`TEXT`タイプをサポートしています。省略した場合、デフォルトで`TEXT`タイプになります。 |
| SVR_IP | ASH Reportを生成するノードのIPアドレスを指定するために使用されます。未入力または`NULL`を指定した場合、IPを制限しません。 |
| SVR_PORT | ASH Reportを生成するノードのポートを指定するために使用されます。未入力または`NULL`を指定した場合、ポートを制限しません。|
| TENANT_ID | ASH Reportを生成するテナントIDを指定するために使用されます。未入力または`NULL`を指定した場合、`TENANT_ID`を制限しません。 |
