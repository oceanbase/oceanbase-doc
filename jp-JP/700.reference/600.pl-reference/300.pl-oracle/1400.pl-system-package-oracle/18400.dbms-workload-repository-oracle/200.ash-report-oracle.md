...
| BTIME | サンプリング開始時刻。 |
| ETIME | サンプリング終了時刻。 |
| SQL_ID | サンプリング対象のSQLの`SQL_ID`。このパラメータを指定しない場合、または`NULL`を指定した場合、`SQL_ID`に制限はありません。 |
| TRACE_ID | サンプリング対象のSQLの`TRACE_ID`。このパラメータを指定しない場合、または`NULL`を指定した場合、`TRACE_ID`に制限はありません。 |
| WAIT_CLASS | サンプリング対象のイベントタイプ。`WAIT_CLASS`の値は`GV$ACTIVE_SESSION_HISTORY.WAIT_CLASS`フィールドの値を参照してください。このパラメータを指定しない場合、または`NULL`を指定した場合、待機タイプに制限はありません。 |
| REPORT_TYPE | レポートタイプ。現在は`TEXT`タイプのみサポートされています。このパラメータは省略可能です。 |
| REPORT_TYPE | レポートタイプ。現在は`HTML`と`TEXT`タイプをサポートしています。このパラメータを指定しない場合、デフォルトで`TEXT`タイプが適用されます。 |
| SVR_IP | ASHレポートを生成するノードのIPアドレスを指定します。このパラメータを指定しない場合、または`NULL`を指定した場合、IPアドレスに制限はありません。 |
| SVR_PORT | ASHレポートを生成するノードのポートを指定します。このパラメータを指定しない場合、または`NULL`を指定した場合、ポートに制限はありません。|
| TENANT_ID | ASHレポートを生成するテナントIDを指定します。このパラメータを指定しない場合、または`NULL`を指定した場合、`TENANT_ID`に制限はありません。 |

## 使用方法

ASH関連情報を取得するには、`oceanbase.GV$ACTIVE_SESSION_HISTORY` ビューをクエリする必要があります。
...
`ASH_REPORT` を呼び出し、診断する開始時間と終了時間を入力します。実行後、ASHレポート情報が画面上に表示されます。
