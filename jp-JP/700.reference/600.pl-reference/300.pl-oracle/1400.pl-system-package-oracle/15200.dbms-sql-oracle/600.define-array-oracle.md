| Description   |                 |
|---------------|-----------------|
| keywords      |                 |
| dir-name      |                 |
| dir-name-en   |                 |
| tenant-type   | Oracle Mode     |
| machine-translation | en          |
|slug|define_array-oracle-compatible-mode|

# DEFINE_ARRAY

`DEFINE_ARRAY` ステートメントは、行を取得するための列の集合を定義するために使用されます（`FETCH_ROWS` コールで使用）。

`DEFINE_ARRAY` は、単一の `SELECT` ステートメントを使用して行を一括取得し、パラメータとして `COLUMN_VALUE` に渡して呼び出すことができます。

  <main id="notice" >
    <h4>機能の適用範囲</h4>
    <p>この内容はOceanBaseデータベースEnterprise Editionにのみ適用されます。OceanBaseデータベースCommunity EditionはMySQLモードのみを提供します。</p>
  </main>

## 集合のスカラー型とLOB型

ローカル変数は以下の型に宣言でき、`DBMS_SQL` を使用して任意の数の行を抽出して格納できます。（これらの型は `BIND_ARRAY` ステートメントで指定された型と同じである必要があります。）

```sql
TYPE binary_double_table
                    IS TABLE OF BINARY_DOUBLE  INDEX BY BINARY_INTEGER;
TYPE binary_float_table
                    IS TABLE OF BINARY_FLOAT   INDEX BY BINARY_INTEGER;
TYPE bfile_table     IS TABLE OF BFILE          INDEX BY BINARY_INTEGER;
TYPE blob_table     IS TABLE OF BLOB           INDEX BY BINARY_INTEGER;
TYPE clob_table     IS TABLE OF CLOB           INDEX BY BINARY_INTEGER;
TYPE date_table     IS TABLE OF DATE           INDEX BY BINARY_INTEGER;
TYPE interval_day_to_second_Table
                    IS TABLE OF dsinterval_unconstrained
                                               INDEX BY BINARY_INTEGER;
TYPE interval_year_to_MONTH_Table
                    IS TABLE OF yminterval_unconstrained
                                               INDEX BY BINARY_INTEGER;
TYPE number_table   IS TABLE OF NUMBER         INDEX BY BINARY_INTEGER;
TYPE time_table     IS TABLE OF time_unconstrained
                                               INDEX BY BINARY_INTEGER;
TYPE time_with_time_zone_table
                    IS TABLE OF time_tz_unconstrained
                                               INDEX BY BINARY_INTEGER;
TYPE timestamp_table
                    IS TABLE OF timestamp_unconstrained
                                               INDEX BY BINARY_INTEGER;
TYPE timestamp_with_ltz_Table
                    IS TABLE OF timestamp_ltz_unconstrained
                                               INDEX BY BINARY_INTEGER;
TYPE timestamp_with_time_zone_Table
                    IS TABLE OF timestamp_tz_unconstrained
                                               INDEX BY BINARY_INTEGER;
TYPE urowid_table   IS TABLE OF UROWID         INDEX BY BINARY_INTEGER;
TYPE varchar2_table IS TABLE OF VARCHAR2(2000) INDEX BY BINARY_INTEGER;
```

## 構文

```sql
DBMS_SQL.DEFINE_ARRAY (
c                   IN INTEGER,
position            IN INTEGER,
<table_variable>    IN <datatype>
cnt                 IN INTEGER,
lower_bnd           IN INTEGER);
```

ここで、`<table_variable>` と対応する `<datatype>` は以下のペアと一致することができ、`DEFINE_ARRAY` は異なるデータ型を受け入れるようオーバーロードされています：

```sql
<n_tab>      Number_Table
<c_tab>      Varchar2_Table
<d_tab>      Date_Table
<bl_tab>     Blob_Table
<cl_tab>     Clob_Table
```

## パラメータの説明

|       パラメータ       |                      説明                      |
|----------------|----------------------------------------------|
| c              | バインドする配列のカーソルID。                                 |
| position       | 定義された配列内の列の相対位置。 文の最初の列の位置は1です。 |
| table_variable | `<datatype>` として宣言されたローカル変数。                     |
| cnt            | 取得する行数。                                     |
| lower_bnd      | この下限インデックスから結果をコレクションにコピーします。                          |

## 使用方法

* カウント `(cnt)` は0より大きい整数である必要があります。そうでない場合、例外が発生します。

* `lower_bnd` は正の数、負の数、または0に設定できます。

* `DEFINE_ARRAY` を使用するクエリには、配列バインディングを含めることはできません。