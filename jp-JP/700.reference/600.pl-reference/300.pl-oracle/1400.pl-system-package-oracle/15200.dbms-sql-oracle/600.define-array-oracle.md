...
`DEFINE_ARRAY` ストアドプロシージャは、行を取得するための列の集合を定義するために使用されます（`FETCH_ROWS` コールで使用）。
...
ローカル変数は以下の型に宣言でき、`DBMS_SQL` を使用して任意の数の行を格納することができます。(これらの型は `BIND_ARRAY` ステートメントで指定された型と同じである必要があります。)
...
ここで、`<table_variable>` と対応する `<datatype>` は以下のマッチングペアになります。`DEFINE_ARRAY` は、異なるデータ型を受け入れるようオーバーロードされています。
...
|----------------|----------------------------------------------|
| c              | バインドする配列のカーソルID。                                 |
| position       | 定義された配列内の列の相対的位置。 ステートメント内の最初の列の位置は1です。 |
