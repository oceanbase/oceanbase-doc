...
|action_name |`SET_ACTION` または `SET_MODULE` プロシージャを呼び出して、操作名を最新の値に設定します。|

## 使用方法

`READ_MODULE` プロシージャを呼び出すことで、登録済みアプリケーションのモジュールと操作名を取得できます。`READ_CLIENT_INFO` プロシージャを呼び出してクライアント情報を取得します。
