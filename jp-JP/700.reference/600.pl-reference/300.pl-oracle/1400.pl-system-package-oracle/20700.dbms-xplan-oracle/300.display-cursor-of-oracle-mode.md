...
`DISPLAY_CURSOR` 関数は、実行されたクエリプランの詳細を表示するために使用します。
...
| sql_handle  | 実行計画がSQL文を識別するために使用するハンドル（Handle）です。特定のSQL文の実行計画を照会する場合、複数の類似したSQLが存在する場合、sql_handleを使用して対象文を正確に指定できます。|
| plan_name   | 実行計画の名前。|

## 例

OracleテナントはFunction Table機能と併用する必要があります。
