...
| level | 追跡レベルを設定します。<ul> <li> 0：デフォルト動作。</li> <li> 1：各モジュールの使用メモリと時間を追加で出力します。</li> <li> 2：リライトの有無にかかわらず、各リライトされたQuery Blockに対応するSQLを追加で出力します。</li></ul> <br> **注意**：`level` はデータベースのキーワードであるため、Oracleテナントでは二重引用符（「」）で囲み、MySQLテナントでは `` で囲む必要があります。 |
