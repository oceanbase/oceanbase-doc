...
`DISPLAY_SQL_PLAN_BASELINE` 関数は、SPMのベースラインプランを表示するために使用されます。
...
| パラメータ | 説明 |
| --- | --- |
| sql_handle | BaselineプランのSQLハンドル。SPM関連ビューから取得できます。 |
| plan_name | Baselineプランの名前。 |
| format | プラン形式情報を指定します。オプションパラメータは以下のとおりです：<ul><li>`'BASIC'`：演算子ID、演算子名、演算子備考情報、式情報など、少量の情報を表示します。</li><li> `'TYPICAL'`：演算子ID、演算子名、演算子備考情報、オプティマイザー推定の出力行数、オプティマイザー推定の実行時間、式情報などの基本情報を表示します。プランが実行されている場合は、実際のフィードバック情報（実行行数、コストなど）も表示されます。</li><li> `'ALL'`：演算子ID、演算子名、演算子備考情報、オプティマイザー推定の出力行数、オプティマイザー推定の実行時間、式情報、有効なHint、Query Block Name Trace情報、Outline情報、ベーステーブル最適化情報、定数パラメータ化情報、制約情報、Plan Noteなど、豊富なプラン情報を表示します。</li><li> `'ADVANCED'`：豊富なプラン情報を表示します。プラン情報は'ALL'パラメータと同じですが、複雑なプランについてはツリー構造情報も表示されます。</li></ul>|
| svr_ip | プランが存在するノードのIPアドレス。デフォルトはセッションが接続しているノードのIPアドレスです。|
| svr_port | プランが存在するノードのポート番号。デフォルトはセッションが接続しているノードのポート番号です。|
| tenant_id | プランが属するテナントID。デフォルトはセッションが現在接続しているテナントです。|
