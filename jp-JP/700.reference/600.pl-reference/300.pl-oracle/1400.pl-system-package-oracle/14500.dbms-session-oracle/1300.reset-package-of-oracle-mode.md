...
リモート呼び出しシナリオでは、現在パッケージ変数のリモート同期設計の制限により、`RESET_PACKAGE` はリモート側に同期できません。OBServerノードのログにエラーが表示され、変数 `##__OB_RESET_ALL_PACKAGE_BY_DBMS_SESSION_RESET_PACKAGE__` が解決できず、その結果、後続の実行結果がローカルに返されることになります。
