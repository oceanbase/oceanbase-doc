...
リモート呼び出しシナリオにおいて、現在のところパッケージ変数のリモート同期設計上の制限により、`RESET_PACKAGE` はリモート側に同期できず、OBServerノードのログにエラーが報告されます。また、変数 `##__OB_RESET_ALL_PACKAGE_BY_DBMS_SESSION_RESET_PACKAGE__` が解釈できないため、その後の実行結果がローカルに返されることになります。
