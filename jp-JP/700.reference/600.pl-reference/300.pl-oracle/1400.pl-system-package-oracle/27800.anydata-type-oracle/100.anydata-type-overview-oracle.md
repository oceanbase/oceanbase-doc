...
`ANYDATA TYPE`は、指定された型のインスタンスとその型の記述を含みます。`ANYDATA`は自己記述的です。
...
* `CONVERT *` を1回呼び出して `ANYDATA` を完全に作成します。
  ...
  /* Datetime型とNchar型を変換する呼び出し */
   ...
   /* BFloat、BDouble、URowidを変換する呼び出し */
...
* 段階的な方法を使用して `ANYDATA` を作成します。

  `BEGINCREATE` 静的ストアドプロシージャの呼び出しで構築プロセスを開始し、`ENDCREATE` メンバーストアドプロシージャの呼び出しで構築プロセスを完了します。この2つの呼び出しの間に、`SET*` を使用してオブジェクト型の単一の属性または集合要素を設定できます。集合のオブジェクトや要素属性に段階的にアクセスするには、`GET*` 呼び出しの前に `PIECEWISE` メンバーストアドプロシージャを呼び出す必要があります。

  <main id="notice" type='notice'>
    <h4>注意</h4>
    <p><code>ANYDATA</code> は、最初の属性（または集合要素）から実行順に従って構築またはアクセスする必要があります。<code>BEGINCREATE</code> 呼び出しは、段階的な構築を自動的に開始します。<code>BEGINCREATE</code> の後にすぐに <code>PIECEWISE</code> を呼び出す必要はありません。<code>ENDCREATE</code> を呼び出して構築プロセスを完了する必要があります（その前に任意のアクセス呼び出しが可能です）。</p>
  </main>

## アクセス方法

アクセス機能はSQLに基づいています。これらの関数は型が一致しない場合に例外を発生させることはありません。代わりに、`ANYDATA` の型がアクセス対象の型に対応しない場合は、`NULL` を返します。クエリで適切な型を返す `ANYDATA` 関数のみを使用したい場合は、`WHERE` 句を使用して `GETTYPENAME` を使用し、興味のある型（例：`SYS.NUMBER`）を選択する必要があります。各関数は `SYS.ANYDATA` ワーマーで指定されたデータ型の値を返します。
...
```sql
  ...
  /* Float、DoubleおよびRowidへのアクセス呼び出し*/
...
| [PIECEWISE メンバーストアドプロシージャ](../27800.anydata-type-oracle/700.gettypename-member-function-oracle.md)   | 現在のデータ値のアクセスモードを1回属性に設定します（データ値が`TYPECODE_OBJECT`に属する場合） |
