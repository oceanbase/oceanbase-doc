...
プログラムパッケージ `STANDARD` は PL 環境を定義します。プログラムパッケージヘッダーでは、PL プログラムで使用可能な公開型、変数、例外、およびサブルーチンを宣言します。
...
システムパッケージ `STANDARD` の内容はアプリケーションから直接参照でき、ユーザーはプログラムパッケージ名の前に接頭辞を付ける必要なく、データベーストリガー、格納されたサブルーチン、または OceanBase ツールから直接呼び出すことができます。

  <main id="notice" type='explain'>
    <h4>説明</h4>
    <p>OceanBase データベースのシステム関数は <code>STANDARD</code> パッケージに登録されていません。現在のバージョンの <code>STANDARD</code> パッケージは、事前定義済みのデータ型、定数、および例外のみをサポートしています。</p>
  </main>
...
以下のデータ型はジェネリックであり、`STANDARD` パッケージおよび一部の他のパッケージ専用であり、他の用途には使用できません。なお、ジェネリックデータ型はまだ言語の一部となっていない点に注意してください。
...
|   **ジェネリックインデックステーブルのデータ型**   |          **値の説明**           |
|-------------------|-----------------------------|
| SYS $ INT_V2TABLE | `binary_integer` 型で表される整数インデックステーブル |

PLコンパイラは、以下のレコードタイプと対応するジェネリックインデックステーブルデータ型を使用して、テーブル実行中に `FORALL` バッチバインディングステートメントの実行によって発生する例外を記録します。
