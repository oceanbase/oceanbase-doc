...
* `PRAGMA EXCEPTION_INIT`を使用して宣言し、`EXCEPTION HANDLE`でその名前をキャッチします。
...
ここで、`handle_name`は例外の名前であり、この名前を使用して`EXCEPTION`でキャッチできます。`sql_err_code`は対応するデータベースエラーコードです。

例：名前付きキャッチを使用してシステム内部例外をキャッチします。
...
OceanBaseデータベースのPLエンジンでプリコンパイル済みの例外も、PLプログラム実行時の一般的な問題です。プリコンパイル済み例外はOceanBaseデータベースによって自動的にスローされ、明確な例外名があり、プログラムで直接キャッチできます。例えば、`SELECT... INTO...`は`NO_DATA_FOUND`例外をトリガーする可能性があります。

このような例外状況を処理するには、PLブロックの例外処理部分で対応する例外名を直接参照し、適切な例外処理を行うだけです。
...
obclient>DECLARE
     ...
     EXCEPTION
         ...
         WHEN TOO_MANY_ROWS THEN
             ...
             -- 従業員番号 v_empid に対応するレコードが複数ある場合、TOO_MANY_ROWS例外を発生させます。
