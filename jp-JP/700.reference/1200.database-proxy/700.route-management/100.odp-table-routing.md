...
* Location Cacheに存在し、利用可能な場合、直接使用します。
...
パーティションIDはSQLステートメントから取得され、その出典は通常where句の式やINSERTステートメントのvalues値です。`a = xxx`を取得できることを保証する必要があります。ここで、`a`はパーティションキー、`xxx`は定数です。例：
...
ここでは`c1 = 5`を抽出し、この情報から影響を受けるパーティションIDを計算し、SQLを対応するパーティションが存在するOBServerノードに転送します。
