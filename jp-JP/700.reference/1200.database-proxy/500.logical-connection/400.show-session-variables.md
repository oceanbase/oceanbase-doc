...
`sys`テナントとユーザーテナントは`SHOW PROXYSESSION VARIABLES`ステートメントを使用して、指定されたクライアントセッションのセッション変数を確認できます。

1. ODP接続を使用してOceanBaseデータベースに接続します。
    ...
    ODP接続を使用してデータベースに接続する操作の詳細については、[OBClientを使用してOceanBaseテナントに接続する（MySQLモード）](../../../300.develop/100.application-development-of-mysql-mode/100.connect-to-oceanbase-database-of-mysql-mode/300.connect-to-an-oceanbase-tenant-by-using-obclient-of-mysql-mode.md)および[OBClientを使用してOceanBaseテナントに接続する（Oracleモード）](../../../300.develop/200.application-development-of-oracle-mode/100.connect-to-oceanbase-database-of-oracle-mode/200.connect-to-an-oceanbase-tenant-by-using-obclient-of-oracle-mode.md)を参照してください。
...
2. 指定されたクライアントセッションのセッション変数を確認します。
   ...
   * `ALL`パラメータを指定しない場合、指定されたクライアントセッションのローカルセッション変数（変更されたシステム変数とユーザー変数を含む）が表示されます。

   * `ALL`パラメータを指定した場合、指定されたクライアントセッションのすべてのセッション変数（すべてのシステム変数とユーザー変数を含む）が表示されます。
   ...
   * `like`はあいまい一致をサポートし、'%'と'_'を使用できます。
    ...
    1. `cs_id` または `connection_id` を取得します。
       ...
       クエリ結果は次のとおりです。戻り値の `Id` が `cs_id` です。
       ...
       次のような結果が返されます。戻り値の `id` が `connection_id` です。
