...
1. `root`ユーザーとしてODP接続を使用して、クラスタの`sys`テナントにログインします。
    ...
    ODP接続を使用してデータベースに接続する方法の詳細な操作手順については、[OBClientを使用してOceanBaseテナントに接続する（MySQLモード）](../../../300.develop/100.application-development-of-mysql-mode/100.connect-to-oceanbase-database-of-mysql-mode/300.connect-to-an-oceanbase-tenant-by-using-obclient-of-mysql-mode.md)および[OBClientを使用してOceanBaseテナントに接続する（Oracleモード）](../../../300.develop/200.application-development-of-oracle-mode/100.connect-to-oceanbase-database-of-oracle-mode/200.connect-to-an-oceanbase-tenant-by-using-obclient-of-oracle-mode.md)を参照してください。
...
2. 指定されたクライアントセッションを終了します。
   ...
   * 指定されたクライアントセッションを終了する
     ...
     * `id`は`cs_id`または`connection_id`のいずれかで構いません。表示結果は同じです。

       `cs_id`はODP内部で各クライアントに割り当てられる`id`番号であり、`connection_id`はOceanBaseデータベース全体で各クライアントに割り当てられる`id`番号です。`connection_id`は`SELECT CONNECTION_ID();`ステートメントで取得できます。
     ...
     * `KILL connection_id`と同じ効果があります。`KILL`ステートメントの詳細については、[KILL（MySQLモード）](../../500.sql-reference/100.sql-syntax/200.common-tenant-of-mysql-mode/600.sql-statement-of-mysql-mode/5800.kill-of-mysql-mode.md)または[KILL（Oracleモード）](../../500.sql-reference/100.sql-syntax/300.common-tenant-of-oracle-mode/900.sql-statement-of-oracle-mode/300.dcl-of-oracle-mode/1800.kill-of-oracle-mode.md)を参照してください。
     ...
     この例からわかるように、`cs_id`または`connection_id`を指定して現在のセッションをKILLする場合、コマンドの実行が成功すると、現在の接続が切断されます。`SHOW PROXYSESSION`で確認すると、OBClientは新しいセッション接続を確立し、SQLを実行して結果を表示します。
   ...
   * 指定されたクライアントセッション上のサーバーセッションを終了する
     ...
     * `ss_id`はODP内部で各サーバーサイドセッション（Server Session）に割り当てられる`id`番号を表し、`SHOW PROXYSESSION ATTRIBUTE id`から取得できます。詳細な取得操作については、[セッションの詳細な状態を表示する](../500.logical-connection/200.displays-the-status-of-a-session.md)を参照してください。
