...
現在のテナントにおける、すべてのOBServerノード上でのRPC送信統計情報を表示します。
...
| TOTAL_SIZE     | bigint(20)   | NO             | 総送信データサイズ、単位はバイト            |
...
| LAST_TIMESTAMP | timestamp(6) | NO             | 統計情報最終更新時刻            |

## クエリ例

ユーザーテナント内で、当該テナントがすべてのノードに送信したRPCリクエストを確認し、上位10件のレコードを表示します。
...
* 現在のテナントが現在のノードで送信したPRC統計情報を確認する：[V$OB_RPC_OUTGOING](32400.v-ob_obrpc_outgoing-of-mysql-mode.md)

* 現在のテナントがすべてのノードから受信したRPCの統計情報を確認する：[GV$OB_RPC_INCOMING](1900.gv-ob_obrpc_incoming-of-mysql-mode.md)