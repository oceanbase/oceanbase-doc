...
|slug|gv-ob_rpc_outgoing-mysql-compatible-mode|
...
現在のテナントがすべてのOBServerノードで送信したRPCの統計情報を表示します。
...
| COUNT          | bigint(20)   | NO             | 使用回数                  |
| TOTAL_TIME     | bigint(20)   | NO             | 合計時間、単位はマイクロ秒                |
| TOTAL_SIZE     | bigint(20)   | NO             | 合計送信データサイズ、単位はバイト            |
| FAILURE        | bigint(20)   | NO             | 送信失敗回数                |
| TIMEOUT        | bigint(20)   | NO             | 送信タイムアウト回数                |
| SYNC           | bigint(20)   | NO             | 同期待機中のRPCリクエスト数         |
| ASYNC          | bigint(20)   | NO             | 非同期コールバックRPCリクエスト数         |
| LAST_TIMESTAMP | timestamp(6) | NO             | 統計情報最終更新日時            |

## クエリ例

ユーザーテナント内で、自身のテナントがすべてのノードに送信したRPCリクエストを確認し、上位10件のレコードを表示します。
...
* 現在のノードで現在のテナントが送信したPRC統計情報を確認する：[V$OB_RPC_OUTGOING](32400.v-ob_obrpc_outgoing-of-mysql-mode.md)

* すべてのノードで現在のテナントが受信したRPCの統計情報を確認する：[GV$OB_RPC_INCOMING](1900.gv-ob_obrpc_incoming-of-mysql-mode.md)