...
ビュー `V$OB_SQLSTAT` は、現在のノード上で実行されたSQLの基本的なパフォーマンス統計データを表示するために使用されます。各SQL文は1行として表示されます（つまり、各 `SQL_ID` + `Plan_Hash` の一意の値が1行に対応します）。`_DELTA` を含む列は、前回のWRスナップショットから現在時点までの統計値の増分を示しています。
...
| BUFFER_GETS_TOTAL             | bigint(20)          | NO   |   キャッシュからの論理読み取り総回数       |
| BUFFER_GETS_DELTA             | bigint(20)          | NO   |   キャッシュからの論理読み取り回数の増分      |
| ELAPSED_TIME_TOTAL            | bigint(20)          | NO   |   このステートメントで消費された総時間。パラレル実行シナリオでは、この値は各実行スレッドの累積値です。          |
...
| CCWAIT_TOTAL                  | bigint(20)          | NO   |   並行待機イベントの総消費時間       |
| CCWAIT_DELTA                  | bigint(20)          | NO   |   並行待機イベント総消費時間の増分       |
| USERIO_WAIT_TOTAL             | bigint(20)          | NO   |   I/O待機の総消費時間      |
| USERIO_WAIT_DELTA             | bigint(20)          | NO   |   I/O待機の総消費時間の増分      |
| APWAIT_TOTAL                  | bigint(20)          | NO   |   アプリケーション待機イベントの総消費時間        |
| APWAIT_DELTA                  | bigint(20)          | NO   |   アプリケーション待機イベント総消費時間の増分      |
...
| MUTI_QUERY_BATCH_DELTA        | bigint(20)        | NO   | 前回のWRスナップショット取得後から現在までに、このSQLバッチが実行された回数<main id="notice" type='explain'><h4>説明</h4><p>V4.4.x系では、このフィールドはV4.4.2バージョンから導入されました。</p></main>|
| FULL_TABLE_SCAN_TOTAL         | bigint(20)        | NO   | 実行計画生成後、このSQLによる全表スキャンの回数<main id="notice" type='explain'><h4>説明</h4><p>V4.4.x系では、このフィールドはV4.4.2バージョンから導入されました。</p></main>|
| FULL_TABLE_SCAN_DELTA         | bigint(20)        | NO   | 前回のWRスナップショット取得後から現在までに、このSQLによる全表スキャンの回数<main id="notice" type='explain'><h4>説明</h4><p>V4.4.x系では、このフィールドはV4.4.2バージョンから導入されました。</p></main>|
| ERROR_COUNT_TOTAL             | bigint(20)        | NO   | 実行計画生成後、このSQLで発生したエラーの回数<main id="notice" type='explain'><h4>説明</h4><p>V4.4.x系では、このフィールドはV4.4.2バージョンから導入されました。</p></main>|
| ERROR_COUNT_DELTA             | bigint(20)        | NO   | 前回のWRスナップショット取得後から現在までに、このSQLで発生したエラーの回数<main id="notice" type='explain'><h4>説明</h4><p>V4.4.x系では、このフィールドはV4.4.2バージョンから導入されました。</p></main>|
