...
ビュー `V$OB_SQLSTAT` は、現在のノード上で実行されているSQLの基本的なパフォーマンス統計データを表示するために使用されます。各SQL文は1行となります（つまり、各 `SQL_ID` + `Plan_Hash` の一意の値が1行に対応します）。`_DELTA` を含む列は、前回のWRスナップショットから現在時点までの統計値の増分を示しています。
...
| PLAN_TYPE                     | bigint(20)          | NO   |    SQL計画のキャッシュにおける計画タイプ。<ul><li>`1`：ローカル計画を表す </li> <li>`2`：リモート計画を表す</li> <li>`3`：分散計画を表す </li></ul> </br>PLオブジェクトキャッシュにおいて、このフィールドはPLオブジェクトのタイプを示します。<ul><li>`1`：プロシージャを表す</li> <li>`2`：関数を表す</li> <li>`3`：パッケージを表す</li> <li>`4`：匿名ブロックを表す </li></ul> |
| QUERY_SQL                     | longtext            | NO   | 全量のSQLテキスト |
| SQL_TYPE                      | bigint(20)          | NO   | ステートメントタイプ |
| MODULE                        | varchar(64)         | NO   |   このステートメントが初めて解析されたときのアプリケーションモジュール    |
| ACTION                        | varchar(64)         | NO   |   このステートメントが初めて解析されたときのアプリケーションアクション      |
...
| EXECUTIONS_TOTAL              | bigint(20)          | NO   |   キャッシュ内の計画の総実行回数     |
| EXECUTIONS_DELTA              | bigint(20)          | NO   |   キャッシュ内の計画の総実行回数の増分      |
...
| ELAPSED_TIME_TOTAL            | bigint(20)          | NO   |   このステートメントで消費された総時間。並列実行シナリオでは、この値は各実行スレッドの累積値です。          |
...
| FETCHES_TOTAL                 | bigint(20)          | NO   |   結果セットの取得総回数   |
...
| ROUTE_MISS_DELTA              | bigint(20)          | NO   |   前回のWRスナップショット取得以降、リクエストパーティションにヒットしなかった回数<main id="notice" type='explain'><h4>説明</h4><ul><li>V4.3.x系では、このフィールドはV4.3.5バージョンから導入されました。</li> <li>V4.2.x系では、このフィールドはV4.2.2バージョンから導入されました。</li></ul></main>   |
| FIRST_LOAD_TIME               | timestamp(6)        | NO   |   プラン生成時間<main id="notice" type='explain'><h4>説明</h4><ul><li>V4.3.x系では、このフィールドはV4.3.5バージョンから導入されました。</li> <li>V4.2.x系では、このフィールドはV4.2.5バージョンから導入されました。</li></ul></main>   |
...
| PLAN_CACHE_HIT_DELTA          | bigint(20)          | NO   |   前回のWRスナップショット取得以降、Plan Cacheにヒットした回数<main id="notice" type='explain'><h4>説明</h4><ul><li>V4.3.x系では、このフィールドはV4.3.5バージョンから導入されました。</li> <li>V4.2.x系では、このフィールドはV4.2.5バージョンから導入されました。</li></ul></main>  |
...
| MUTI_QUERY_DELTA              | bigint(20)        | NO   |   前回のWRスナップショット取得後、このSQLがMuti Query内にあった回数<main id="notice" type='explain'><h4>説明</h4><p>V4.4.x系では、このフィールドはV4.4.2バージョンから導入されました。</p></main>|
...
| MUTI_QUERY_BATCH_DELTA        | bigint(20)        | NO   |   前回のWRスナップショット取得後、このSQL Batchが実行された回数<main id="notice" type='explain'><h4>説明</h4><p>V4.4.x系では、このフィールドはV4.4.2バージョンから導入されました。</p></main>|
| FULL_TABLE_SCAN_TOTAL         | bigint(20)        | NO   |   プラン生成後、このSQLが全表スキャンを行った回数<main id="notice" type='explain'><h4>説明</h4><p>V4.4.x系では、このフィールドはV4.4.2バージョンから導入されました。</p></main>|
| FULL_TABLE_SCAN_DELTA         | bigint(20)        | NO   |   前回のWRスナップショット取得後、このSQLが全表スキャンを行った回数<main id="notice" type='explain'><h4>説明</h4><p>V4.4.x系では、このフィールドはV4.4.2バージョンから導入されました。</p></main>|
...
| ERROR_COUNT_DELTA             | bigint(20)        | NO   |   前回のWRスナップショット取得後、このSQLがエラーを返した回数<main id="notice" type='explain'><h4>説明</h4><p>V4.4.x系では、このフィールドはV4.4.2バージョンから導入されました。</p></main>|
| LATEST_ACTIVE_TIME            | timestamp(6)      | NO   |   特定ノード上のSQL ID + Plan Hashに対応するプランの最終アクティブ時間<main id="notice" type='explain'><h4>説明</h4><p>V4.4.x系では、このフィールドはV4.4.2バージョンから導入されました。</p></main>     |
...
現在のノード内のSQLの基本的なパフォーマンス統計データを確認し、最初のレコードを表示します。
