...
| STATUS | varchar(32) | NO |  スレッドの状態：<ul><li> Run：スレッドが実行中であることを示します。  </li><li> Wait：スレッドが待機状態にあることを示します。 </li><li> Sleep：スレッドがスリープ状態にあることを示します。 </li><li> Join：スレッドが別のスレッドの終了を待っていることを示します。 </li></ul>|
| LATCH_WAIT | varchar(16) | NO | ラッチを待機しているアドレス |
| LATCH_HOLD | varchar(256) | NO | ラッチを保持しているアドレス（複数含まれる場合あり） |
| TRACE_ID | varchar(40) | NO | スレッド上で実行されているトレースのID |
| CGROUP_PATH | varchar(256) | NO  | スレッドが属するcgroupのパス。このフィールドの情報は、`/proc/observer-pid/task/tid/cgroup` パスのCPU行から取得します。 |

## クエリ例

ユーザーテナントで、各OBServerノード上のobserverプロセス内のすべてのスレッドの状態情報を確認し、最初の2件のレコードを表示します。
