...
* `STATISTIC#`：統計項目の番号を表します。同一の統計項目でも、異なるOBServerノードバージョンでは番号が変更される場合があります。
* `STAT_ID`：統計項目のIDを表します。同一の統計項目のIDは、異なるOBServerノードバージョンでも同じであることが保証されています。
...
ユーザーテナント内で、現在のノード上における統計イベント情報を確認し、上位10件のレコードを表示します。
...
* テナントのすべての統計イベントの定義を確認するには、[V$STATNAME](36700.v-statname-of-mysql-mode.md)を参照してください。
