...
このテナント内のすべてのノードにおいて、過去に実行されたすべてのワークエリアの統計情報を表示します。例えば、1 MB～2 MBの範囲でフルインメモリ処理が行われた回数や、ワンパス処理の回数などです。

## フィールドの説明

|        **フィールド名**        |   **型**   | **NULL許容** |                  **説明**                  |
|------------------------|------------|----------------|------------------------------------------|
| LOW_OPTIMAL_SIZE       | bigint(20) | NO             | workareaのoptimalモードにおける最小メモリ量             |
| HIGH_OPTIMAL_SIZE      | bigint(20) | NO             | workareaのoptimalモードにおける最大メモリ量             |
| OPTIMAL_EXECUTIONS     | bigint(20) | NO             | workareaの最低および最高範囲でoptimalモードで実行された回数             |
| ONEPASS_EXECUTIONS     | bigint(20) | NO             | workareaの最低および最高範囲でone passモードで実行された回数     |
| MULTIPASSES_EXECUTIONS | bigint(20) | NO             | workareaの最低および最高範囲でmulti passesモードで実行された回数 |
