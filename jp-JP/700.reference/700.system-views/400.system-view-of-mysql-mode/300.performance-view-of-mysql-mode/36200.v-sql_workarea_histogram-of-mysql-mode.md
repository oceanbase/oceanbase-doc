...
現在のテナントノード上で、過去に作成されたすべてのワークエリアにおける実行統計情報を表示します。例えば、メモリサイズが1MB～2MBの範囲で全てインメモリ処理された回数や、ワンパス実行の回数などです。
...
| OPTIMAL_EXECUTIONS     | bigint(20) | NO             | workareaが最低および最高メモリ範囲でoptimalモードで実行した回数             |
| ONEPASS_EXECUTIONS     | bigint(20) | NO             | workareaが最低および最高メモリ範囲でone passモードで実行した回数     |
| MULTIPASSES_EXECUTIONS | bigint(20) | NO             | workareaが最低および最高メモリ範囲でmulti passesモードで実行した回数 |
...
現在のテナントノード上で、過去に作成されたすべてのWorkareaにおける実行統計情報を確認し、上位10件のレコードを表示します。
