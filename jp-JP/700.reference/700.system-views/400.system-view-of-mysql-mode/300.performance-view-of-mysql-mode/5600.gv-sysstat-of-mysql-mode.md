...
* `STATISTIC#`：統計項目番号を示します。同一の統計項目の番号は、異なるOBServerノードのバージョン間で変更される場合があります。

* `STAT_ID`：統計項目IDを示します。同一の統計項目のIDは、異なるOBServerノードのバージョン間でも同じであることが保証されます。
...
ユーザーテナント内で、自身のテナントがすべてのノード上で発生した統計イベント情報を確認し、上位10件のレコードを表示します。
...
* テナント内のすべての統計イベントの定義を確認する：[V$STATNAME](36300.v-statname-of-mysql-mode.md)
