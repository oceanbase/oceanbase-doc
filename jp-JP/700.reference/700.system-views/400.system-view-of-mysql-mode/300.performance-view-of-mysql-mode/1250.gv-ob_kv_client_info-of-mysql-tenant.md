...
MySQLテナントでは、ビュー`GV$OB_KV_CLIENT_INFO`を使用して、現在のOBKVテナントのクライアント接続に関する構成パラメータ情報にアクセスできます。
...
`client_info` フィールドのJSON文字列形式は以下のとおりです：
...
| rpc.execute.timeout           | 3000          | RPCリクエスト実行のソケットタイムアウト時間。         |
| rpc.operation.timeout         | 10000         | OceanBase内部でのRPCリクエスト実行タイムアウト時間。    |
| runtime.continuous_failure.ceiling           | 100          | 連続実行失敗の上限回数。この回数に達すると、テーブルのアドレスがリフレッシュされます。 |
...
MySQLテナントでビュー`GV$OB_KV_CLIENT_INFO`をクエリするステートメントは次のとおりです：
