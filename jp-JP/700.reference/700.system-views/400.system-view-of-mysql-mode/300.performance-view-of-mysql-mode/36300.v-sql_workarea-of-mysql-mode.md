...
| OPERATION_TYPE         | varchar(40)  | NO             | workarea演算子のタイプ。例えばSort、Hash Join、Group byなど        |
| OPERATION_ID           | bigint(20)   | NO             | プランツリー内で演算子を識別するための一意の識別子                                     |
| POLICY                 | varchar(10)  | NO             | workareaのポリシー：<ul><li>MANUAL</li><li> AUTO </li></ul>   |
| ESTIMATED_OPTIMAL_SIZE | bigint(20)   | NO             | 最適な場合に演算子を実行するための推定メモリサイズ。単位：bytes                        |
| ESTIMATED_ONEPASS_SIZE | bigint(20)   | NO             | one passシナリオで演算子を実行するための推定メモリサイズ。単位：bytes                |
...
| LAST_TEMPSEG_SIZE      | bigint(20)   | NO             | workarea前回実行時に使用した一時ディスク容量；NULLの場合は一時容量未使用を示す         |
