...
|slug|gv-ob-kv-group-commit-status-of-mysql-mode|
...
MySQLテナントでは、ビュー`GV$OB_KV_GROUP_COMMIT_STATUS`を使用して、このOBKVテナント内のすべてのノードのグループコミット状態情報にアクセスできます。
...
すべてのノードにおけるOBKVテナントクライアント接続のグループコミット状態情報を照会し、1日目のレコードを表示します。
