...
現在のテナントが現在のOBServerノード上でどのようなキャッシュ状況にあるかを表示します。
...
| CACHE HIT_SIZE       | bigint(20)    | NO   |  このTabletのキャッシュヒットデータ量、単位はバイト。現在このフィールドは有効ではなく、値は `-1`   |
| CACHE MISS_SIZE      | bigint(20)    | NO   |  このTabletのキャッシュミスデータ量、単位はバイト。現在このフィールドは有効ではなく、値は `-1`   |
| INFO                 | varchar(4096) | NO   |  その他の情報    |

## クエリ例

SSモードで、ユーザーテナントが現在のOBServerノード上における自身のテナント全体のキャッシュ状況を確認します。
