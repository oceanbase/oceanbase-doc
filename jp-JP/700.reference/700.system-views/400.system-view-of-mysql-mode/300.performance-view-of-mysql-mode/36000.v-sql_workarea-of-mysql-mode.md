...
|        **フィールド名**        |    **型**    | **NULL許容** |                                                                **説明**                                                                 |
|------------------------|--------------|----------------|---------|
| ADDRESS                | varbinary(8) | NO             | SQLに対応するハンドルアドレス。現在は値が指定されていません                           |
...
| TOTAL_EXECUTIONS       | bigint(20)   | NO             | workareaを使用した実行総回数                                |
...
| LAST_TEMPSEG_SIZE      | bigint(20)   | NO             | workareaが前回実行時に使用した一時ディスク容量；NULLの場合は一時空間未使用を示す         |
...
このOBServerノード上でSQLの自動メモリ管理によって実行されたすべてのOperatorのworkarea統計情報を確認します。
