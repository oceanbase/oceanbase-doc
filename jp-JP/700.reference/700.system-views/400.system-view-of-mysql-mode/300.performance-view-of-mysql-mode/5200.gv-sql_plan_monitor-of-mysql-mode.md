...
| USER_IO_WAIT_TIME       | bigint(20)           | NO | V4.4.2バージョンの場合：<ul><li>V4.4.2 BP1以前のバージョン：すべてのuser_ioイベントの合計時間。 </li><li>V4.4.2 BP1以降のバージョン：ミドルウェアインデックスおよびデータマイクロブロックの取得時のIO待ち時間。</li></ul>|
...
| PLAN_DEPTH              | bigint(20)           | NO | プランツリー内での演算子の深さ。 |
| PLAN_POSITION           | bigint(0) unsigned   | NO | 演算子が親ノードの何番目の子であるかを示す。 |
| PLAN_COST               | bigint(0) unsigned   | NO | 最適化エンジンによって計算された演算子コスト。 |
| PLAN_CARDINALITY        | bigint(0) unsigned   | NO | 最適化エンジンによって計算された演算子の出力データ行数。 |
| PLAN_BYTES              | bigint(0) unsigned   | NO | 最適化エンジンによって見積もられた演算子の出力データバイト数。 |
| PLAN_TIME               | bigint(0) unsigned   | NO | 最適化エンジンによって計算された演算子の実行時間。 |
| PLAN_PARTITION_START    | bigint(0) unsigned   | NO | デフォルトはNULL。 |
| PLAN_PARTITION_STOP     | bigint(0) unsigned   | NO | デフォルトはNULL。 |
| PLAN_CPU_COST           | bigint(0) unsigned   | NO | 最適化エンジンによって見積もられた演算子のCPUコスト。 |
| PLAN_IO_COST            | bigint(0) unsigned   | NO | 最適化エンジンによって見積もられた演算子のI/Oコスト。 |
| PLAN_TEMP_SPACE         | bigint(0) unsigned   | NO | 最適化エンジンによって見積もられた演算子の空間使用量。 |
| STARTS                  | bigint(20)           | NO | 演算子が再スキャンされた回数。 |
| OUTPUT_ROWS             | bigint(20)           | NO | 演算子の出力総行数（その演算子のすべての実行インスタンスの行数の合計）。 |
...
| OTHERSTAT_1_VALUE       | bigint(20)           | NO | TABLE SCAN演算子において、I/O読み取りバイト数を示す `IO_READ_BYTES`。<main id="notice" type='explain'><h4>説明</h4><ul><li>V4.3.x系では、TABLE SCAN演算子はV4.3.3 BP1以降、リアルタイム統計をサポートしており、演算子の計算終了時に集計されるものではありません。</li><li>V4.2.x系では、TABLE SCAN演算子はV4.2.4以降、リアルタイム統計をサポートしており、演算子の計算終了時に集計されるものではありません。</li></ul></main> |
| OTHERSTAT_2_ID          | bigint(20)           | NO | フィールドIDによるフィールド詳細情報の照会。 |
| OTHERSTAT_2_TYPE        | bigint(0) unsigned   | NO | 予約済み。 |
| OTHERSTAT_2_VALUE       | bigint(20)           | NO | TABLE SCAN演算子において、SSStore読み取りバイト数を示す `SSSTORE_READ_BYTES`。<main id="notice" type='explain'><h4>説明</h4><ul><li>V4.3.x系では、TABLE SCAN演算子はV4.3.3 BP1以降、リアルタイム統計をサポートしており、演算子の計算終了時に集計されるものではありません。</li><li>V4.2.x系では、TABLE SCAN演算子はV4.2.4以降、リアルタイム統計をサポートしており、演算子の計算終了時に集計されるものではありません。</li></ul></main> |
| OTHERSTAT_3_ID          | bigint(20)           | NO | フィールドIDによるフィールド詳細情報の照会。 |
| OTHERSTAT_3_TYPE        | bigint(0) unsigned   | NO | 予約済み。 |
| OTHERSTAT_3_VALUE       | bigint(20)           | NO | V4.4.2系について：<ul><li>V4.4.2 BP1以前のバージョン：TABLE SCAN演算子において、SSStore読み取り行数を示す `SSSTORE_READ_ROW_COUNT`。リアルタイム統計をサポートします。 </li><li>V4.4.2 BP1以降のバージョン：プッシュダウン/非プッシュダウンパスで読み取ったMajor SSTableのRange範囲内の総行数。</li></ul> <main id="notice" type='explain'><h4>説明</h4><ul><li>V4.3.x系では、TABLE SCAN演算子はV4.3.3 BP1以降、リアルタイム統計をサポートしており、演算子の計算終了時に集計されるものではありません。</li><li>V4.2.x系では、TABLE SCAN演算子はV4.2.4以降、リアルタイム統計をサポートしており、演算子の計算終了時に集計されるものではありません。</li></ul></main> |
| OTHERSTAT_4_ID          | bigint(20)           | NO | フィールドIDによるフィールド詳細情報の照会。 |
| OTHERSTAT_4_TYPE        | bigint(0) unsigned   | NO | 予約済み。 |
| OTHERSTAT_4_VALUE       | bigint(20)           | NO | V4.4.2バージョンについて：<ul><li>V4.4.2 BP1 より前のバージョン：TABLE SCAN 演算子について、MEMStore から読み取った行数 `MEMSTORE_READ_ROW_COUNT` を表し、リアルタイム統計をサポートします。 </li><li>V4.4.2 BP1 以降のバージョン：プッシュダウン/非プッシュダウンパスで読み取った Minor SSTable、Mini SSTable、MemTable の行数。</li></ul>|
| OTHERSTAT_5_ID          | bigint(20)           | NO | 予約済み。 |
| OTHERSTAT_5_TYPE        | bigint(0) unsigned   | NO | 予約済み。 |
| OTHERSTAT_5_VALUE       | bigint(20)           | NO | プッシュダウンパスで開いたマイクロブロック数。<main id="notice" type='explain'><h4>説明</h4><p>V4.4.2バージョンでは、このフィールドは V4.4.2 BP1 バージョンから有効になります。</p></main>|
| OTHERSTAT_6_ID          | bigint(20)           | NO | 予約済み。 |
| OTHERSTAT_6_TYPE        | bigint(0) unsigned   | NO | 予約済み。 |
| OTHERSTAT_6_VALUE       | bigint(20)           | NO | プッシュダウンパスで開いたマイクロブロックの範囲内の総行数。<main id="notice" type='explain'><h4>説明</h4><p>V4.4.2バージョンでは、このフィールドは V4.4.2 BP1 バージョンから有効になります。</p></main>|
| OTHERSTAT_7_ID          | bigint(20)           | NO | 予約済み。 |
| OTHERSTAT_7_TYPE        | bigint(0) unsigned   | NO | 予約済み。 |
| OTHERSTAT_7_VALUE       | bigint(20)           | NO | プッシュダウン/非プッシュダウン Filter でフィルタリングされた総行数。<main id="notice" type='explain'><h4>説明</h4><p>V4.4.2バージョンでは、このフィールドは V4.4.2 BP1 バージョンから有効になります。</p></main>|
| OTHERSTAT_8_ID          | bigint(20)           | NO | 予約済み。 |
| OTHERSTAT_8_TYPE        | bigint(0) unsigned   | NO | 予約済み。 |
| OTHERSTAT_8_VALUE       | bigint(20)           | NO | スキップインデックス情報を利用してプリフェッチが不要なマイクロブロック数。<main id="notice" type='explain'><h4>説明</h4><p>V4.4.2バージョンでは、このフィールドは V4.4.2 BP1 バージョンから有効になります。</p></main>|
...
| OTHER_XML               | varchar(255)         | NO | その他、予約語に書き込めないが外部利用が必要な構造化データ。外部ツールが解析を担当する。 |
...
`OTHERSTAT_X_ID` フィールドの数値に関する詳細情報は、[V$SQL_MONITOR_STATNAME](35800.v-sql_monitor_statname-of-mysql-mode.md) ビューで確認できます。
