...
ビュー`V$OB_SS_OBJECT_TYPE_IO_STAT`は、テナントの現在のノード上で異なる種類のオブジェクトに対する読み取り、書き込み、削除の使用量を表示するために使用されます。
...
| MODE            | varchar(32)  | NO   | 統計データの出典：<ul><li>`local`：ローカル</li><li>`remote`：オブジェクトストレージ</li></ul>     |
...
ユーザーテナントが、現在のノード上で異なる種類のオブジェクトに対する読み取り、書き込み、削除の使用量を確認します。
