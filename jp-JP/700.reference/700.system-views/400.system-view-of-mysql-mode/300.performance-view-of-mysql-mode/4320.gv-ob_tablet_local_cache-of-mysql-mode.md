...
現在のテナントがすべてのOBServerノード上でどのようにキャッシュされているかを全体として表示します。

<main id="notice" type='explain'>
<h4>説明</h4>
<p>このビューは、共有ストレージ(Shared-Storage, SS)モードでのみ適用されます。共有なし(Shared-Nothing, SN)モードでは、このビューのクエリ結果は空になります。</p>
</main>
...
| CACHE_HIT_COUNT      | bigint(20)    | NO   |  このTabletのキャッシュヒット回数。現在このフィールドは有効ではなく、値は `-1`    |
| CACHE_MISS_COUNT     | bigint(20)    | NO   |  このTabletのキャッシュミス回数。現在このフィールドは有効ではなく、値は `-1`    |
| CACHE_HIT_SIZE       | bigint(20)    | NO   |  このTabletのキャッシュヒットデータ量、単位はバイト。現在このフィールドは有効ではなく、値は `-1`   |
| CACHE_MISS_SIZE      | bigint(20)    | NO   |  このTabletのキャッシュミスデータ量、単位はバイト。現在このフィールドは有効ではなく、値は `-1`   |
| INFO                 | varchar(4096) | NO   |  その他の情報    |

## クエリ例

SSモードで、ユーザーテナントが自身のテナントの全OBServerノードにおけるキャッシュ状況を確認します。
