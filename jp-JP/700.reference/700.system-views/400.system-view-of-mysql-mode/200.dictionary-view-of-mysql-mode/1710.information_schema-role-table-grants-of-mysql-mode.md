...
`role_table_grants` ビューは、現在のセッションでユーザーに付与された有効なロールのテーブルレベル権限情報、およびそのロールが他のロールに付与したテーブルレベル権限情報を表示します。

## フィールドの説明

| **フィールド名**   | **タイプ**     | **NULL許容**  | **説明**        |
|----------------|--------------|----------------------|-----------------|
| GRANTOR        | varchar(97)  |  YES    | このロールに権限を付与したアカウントのユーザー名部分です。      |
| GRANTOR_HOST   | varchar(256) |  YES    | このロールに権限を付与したアカウントのホスト部分です。      |
...
| PRIVILEGE_TYPE | varchar(90)  |  NO     | 付与された権限です。この値はテーブルレベルで付与可能な任意の権限です。  |
