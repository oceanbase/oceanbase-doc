...
| DENSITY | decimal(10,0) | NO | 列の密度。COLUMN_NAMEに対するヒストグラムが利用可能な場合、この列はヒストグラム上で2つのエンドポイントを超えない値の選択性を示します。2つ以上のエンドポイントを超える値の選択性を表すものではありません。COLUMN_NAMEに対するヒストグラムが利用不可な場合、この列の値は1/NUM_DISTINCTとなります |
...
| NOTES | varchar(80) | NO | 統計情報の追加の属性を説明します。例：値がINCREMENTALの場合、グローバル統計情報はサマリーから取得されたものです。つまり、グローバル統計情報は増分でメンテナンスされます |
