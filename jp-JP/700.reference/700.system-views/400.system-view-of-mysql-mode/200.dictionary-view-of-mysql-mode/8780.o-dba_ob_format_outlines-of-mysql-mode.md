...
| OUTLINE_ID        | bigint(20)    | NO   | 実行計画アウトラインIDを表します。|
| DATABASE_NAME     | varchar(128)  | NO   | データベース名を表します。|
| OUTLINE_NAME      | varchar(128)  | NO   | 実行計画アウトライン名を表します。|
| VISIBLE_SIGNATURE | longtext      | NO   | Signatureの逆シリアライズ結果を表します。|
| FORMAT_SQL_TEXT   | longtext      | YES  | 実行計画アウトライン作成時に、`ON` 句で指定されたSQLを生成した `FORMAT SQL TEXT` を表します。|
