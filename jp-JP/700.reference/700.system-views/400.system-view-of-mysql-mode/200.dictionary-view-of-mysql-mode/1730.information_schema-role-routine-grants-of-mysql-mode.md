|description||
|---|---|
|keywords||
|dir-name||
|dir-name-en||
|tenant-type| MySQL Mode |
|machine-translation| en |
|slug|information_schema_role_routine_grants-mysql-mode|

# information_schema.role_routine_grants

<main id="notice" type='explain'>
<h4>説明</h4>
<p>V4.3x系では、このビューはV4.3.3バージョンから導入されました。V4.2.x系では、このビューはV4.2.4バージョンから導入されました。</p>
</main>

## 機能の概要

`role_routine_grants` ビューは、現在のセッションでユーザーに付与されている有効なロールのルーチン権限情報、およびそれらのロールが他のロールに付与したルーチン権限情報を表示します。

## フィールド説明

| **フィールド名**   | **型**     | **NULL許容**  | **説明**        |
|----------------|--------------|----------------------|-----------------|
| GRANTOR        | varchar(97)  | YES  | このロールに権限を付与するアカウントのユーザー名部分。    |
| GRANTOR_HOST   | varchar(256) | YES  | このロールに権限を付与するアカウントのホスト部分。      |
| GRANTEE        | varchar(32)  | NO   | このロールに対応するアカウントのユーザー名部分。      |
| GRANTEE_HOST   | varchar(255) | NO   | このロールに対応するアカウントのホスト名部分。    |
| SPECIFIC_CATALOG | varchar(3) | NO   | このロールに付与されるルーチンが属するディレクトリ名。この値は常にdefです。   |
| SPECIFIC_SCHEMA  | varchar(64)| NO   | このロールに付与されるルーチンが属するデータベース名。   |
| SPECIFIC_NAME    | varchar(64)| NO   | このロールに付与されるルーチンの名前。         |
| ROUTINE_CATALOG  | varchar(3) | NO   | このロールに付与されるルーチンが属するディレクトリ名。この値は常にdefです。         |
| ROUTINE_SCHEMA   | varchar(64)| NO   | このロールに付与されるルーチンが属するデータベース名。          |
| ROUTINE_NAME     | varchar(64)| NO   | このロールに付与されるルーチンの名前。         |
| PRIVILEGE_TYPE   | varchar(90)| NO   | 付与される権限。この値は、ルーチンレベルで付与可能な任意の権限です。         |
| IS_GRANTABLE     | varchar(3) | NO   | このロールの権限が他のユーザーに付与可能かどうか。         |

## クエリ例

1. ロールと関数を作成します。

   ロール `employee` を作成します：

   ```shell
   obclient [test]> CREATE ROLE employee;
   ```

   関数 `add_numbers` を作成します：

   ```shell
   obclient [test]> DELIMITER //
   ```

   ```shell
   obclient [test]> CREATE FUNCTION test.add_numbers(a INT, b INT)
   RETURNS INT
   BEGIN
         RETURN a + b;
   END;//
   ```

   ```shell
   obclient [test]> DELIMITER ;
   ```

2. ロールにルーチン権限を付与します。

   ```shell
   obclient [test]> GRANT EXECUTE,ALTER ROUTINE ON test.add_numbers TO employee;
   ```

3. ロールをユーザーに付与します。

   ```shell
   obclient [test]> GRANT employee TO user1;
   ```

4. ユーザー `user1` でデータベースに接続します。

5. 現在のセッションで `employee` ロールを有効化します。

   ```shell
   obclient [test]> SET ROLE employee;
   ```

6. 現在のセッションでユーザーに付与されている有効なロールのルーチン権限情報を確認します。

   ```shell
   obclient [test]> USE information_schema;
   ```

   ```shell
   obclient [information_schema]> SELECT * FROM information_schema.role_routine_grants;
   ```

   クエリ結果は次のとおりです：

   ```shell
   +---------+--------------+-----------+--------------+------------------+-----------------+---------------+-----------------+----------------+--------------+-----------------------+--------------+
   | GRANTOR | GRANTOR_HOST | GRANTEE   | GRANTEE_HOST | SPECIFIC_CATALOG | SPECIFIC_SCHEMA | SPECIFIC_NAME | ROUTINE_CATALOG | ROUTINE_SCHEMA | ROUTINE_NAME | PRIVILEGE_TYPE        | IS_GRANTABLE |
   +---------+--------------+-----------+--------------+------------------+-----------------+---------------+-----------------+----------------+--------------+-----------------------+--------------+
   | root    | %            | employee  | %            | def              | test            | add_numbers   | def             | test           | add_numbers  | Execute,Alter Routine | NO           |
   +---------+--------------+-----------+--------------+------------------+-----------------+---------------+-----------------+----------------+--------------+-----------------------+--------------+
   1 row in set
   ```

## 関連ドキュメント

[ロールの確認](../../../../600.manage/500.security-and-permissions/300.access-control/200.user-and-permission/200.permission-of-mysql-mode/340.role-management-of-mysql-mode/600.view-roles-of-mysql-mode.md)