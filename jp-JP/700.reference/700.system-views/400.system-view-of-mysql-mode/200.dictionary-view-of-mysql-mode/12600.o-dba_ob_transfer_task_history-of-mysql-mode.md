...
| MODIFY_TIME | timestamp(6) | YES | タスク終了時間。成功およびキャンセル時の時間を含む |
...
| NOT_EXIST_PART_LIST | longtext | YES | このタスクで実行予定だったが、ロック時に存在しなかったパーティションのリスト |
| LOCK_CONFLICT_PART_LIST | longtext | YES | このタスクで実行予定だったが、ロック時にロック競合が発生したパーティションのリスト |
...
| TABLET_LIST | longtext | YES | このTransfer操作に関連する完全なTabletのリスト。PART_LISTにリストされているTabletだけでなく、ローカルインデックステーブルやLOB補助テーブルなどに関連付けられたTabletも含まれます。 |
...
| STATUS | varchar(64) | NO | タスクのステータス：<ul><li>`COMPLETED`：タスクが正常に実行された </li><li>`FAILED`：タスクの実行に失敗した </li><li>`CANCELED`：タスクがキャンセルされた </li></ul>|
...
* テナント内で実行中のTabletレベルのロードバランシングタスクを確認する：[DBA_OB_TRANSFER_TASKS](12700.o-dba_ob_transfer_tasks-of-mysql-mode.md)
