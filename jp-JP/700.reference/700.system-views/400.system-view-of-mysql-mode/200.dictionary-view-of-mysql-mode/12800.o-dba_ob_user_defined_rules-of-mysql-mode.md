...
| REPLACEMENT | longtext | NO | Pattern に一致するステートメントテンプレートの書き換え方を指示するもの |
| NORMALIZED_PATTERN | longtext | NO | パラメータ化後の Pattern |
| STATUS | varchar(10) | NO |<ul><li> ENABLE は現在のルールが有効であることを示します。</li><li> DISABLE は現在のルールが無効であることを示します。</li></ul>|
| VERSION | bigint(20) | NO | ルールのバージョン番号 |
| PATTERN_DIGEST | bigint(20) unsigned | NO | NORMALIZED_PATTERN の Hash 値 |

## クエリ例

テナント内のカスタム変換されたSQL文のルールを確認します。
