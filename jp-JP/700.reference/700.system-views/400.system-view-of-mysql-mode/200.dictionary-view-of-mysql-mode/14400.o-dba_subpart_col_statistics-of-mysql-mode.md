...
| DENSITY | decimal(10,0) | NO | 列の密度。COLUMN_NAMEにヒストグラムが利用可能な場合、この列はヒストグラム上で2つ未満の端点をまたぐ値の選択性を示します。2つ以上の端点をまたぐ値の選択性を表すものではありません。COLUMN_NAMEにヒストグラムが利用できない場合、この列の値は1 / NUM_DISTINCTとなります |
| NUM_NULLS | decimal(10,0) | NO | 列内のNULL数 |
| NUM_BUCKETS | decimal(10,0) | NO | 列ヒストグラムのバケット数 |
| LAST_ANALYZED | datetime(6) | NO | この列を最後に分析した時間 |
...
| NOTES | varchar(80) | NO | 統計情報のいくつかの追加プロパティを説明します。例えば、値がINCREMENTALの場合、グローバル統計情報はサマリーから取得されます。つまり、グローバル統計情報は増分メンテナンスされます |
