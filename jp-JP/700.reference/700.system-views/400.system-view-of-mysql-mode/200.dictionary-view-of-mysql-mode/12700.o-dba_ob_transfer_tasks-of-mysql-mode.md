...
| STATUS | varchar(64) | NO | タスクのステータス：<ul><li>`INIT`：タスク作成  </li><li>`START`：Transfer実行開始 </li><li>`DOING`：Transfer実行中 </li><li>`ABORTED`：Transferタスクの実行に失敗し、終了 </li><li>`COMPLETED`：タスクの実行に成功 </li><li>`FAILED`：タスクの実行に失敗 </li><li>`CANCELED`：タスクがキャンセル </li></ul>|
| TRACE_ID | varchar(64) | NO | タスクの全プロセスTrace ID |
| RESULT | bigint(20) | YES | タスク結果 |
| BALANCE_TASK_ID | bigint(20) | NO | 関連するロードバランシングタスクID。このTransferタスクは対応するロードバランシングタスクが生成したサブタスクです。 |
