...
| ROLE           | longtext            | NO   | そのノードがデッドロックイベント内で果たす役割：<li>witness<li>victim     |
| PRIORITY_LEVEL | longtext            | NO   | そのノードの優先度レベルの説明：<li>extreme-low<li>low<li>normal<li>high<li>extreme-high     |
| PRIORITY       | bigint(20) unsigned | NO   | そのノードが持つ優先度値。優先度が同じ場合にのみ優先度値を比較し、ループ内で優先度値が最も低いノードが除外される     |
| CREATE_TIME    | timestamp(6)        | NO   | そのノードの作成時間     |
| START_DELAY_US | bigint(20) unsigned | NO   | そのノードが作成されてからデッドロックの検出を開始できるまでの待機時間。単位はマイクロ秒     |
| MODULE         | longtext            | NO   | そのノードが存在するモジュール。値は固定で transaction     |
| VISITOR        | longtext            | NO   | そのノードが代表するリソースアクセス者。形式は `{session_id:$1}:{txid:$2}`。`$1` と `$2` はどちらも数字     |
| OBJECT         | longtext            | NO   | そのノードが取得したいリソース。行の主キー内容の説明。形式は `{addr:“$1:$2”}:{ls:$3}:{tablet:$4}:{row_key:{$5}}`。ここで<ul><li><code>\$1</code> はIPアドレス。</li><li><code>\$2</code> はPORT番号。<code>\$1</code> と共に競合が発生したマシンのプロセスアドレスを構成します。</li><li><code>\$3</code> は競合が発生した際のログストリーム。</li><li><code>\$4</code> は競合が発生した際のTablet。</li><li><code>\$5</code> は行ロックの主キーを文字列化したもの。</li></ul>     |
...
| EXTRA_VALUE2   | longtext            | YES  | ロック保持ステートメントの要求時間     |
