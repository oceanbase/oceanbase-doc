...
|slug|oceanbase-dba_part_histograms-mysql-compatible-mode|
...
現在のテナント内のすべてのパーティションテーブルに対するヒストグラムデータ（各ヒストグラムの端点）を表示します。
...
| ENDPOINT_ACTUAL_VALUE_RAW | text | NO | エンドポイント実際値の生形式 |
| ENDPOINT_REPEAT_COUNT | decimal(10,0) | NO | エンドポイント頻度（ハイブリッドヒストグラムのみ対象、その他のヒストグラムタイプは0に設定） |
