...
| PARTITION_NAME | varchar(128) | YES | パーティション名。パーティションテーブルでは、この列の値がNULLの場合、現在のレコードはテーブルレベルで集計されていることを示します |
| SUBPARTITION_NAME | varchar(128) | YES | サブパーティション名。サブパーティションテーブルでは、この列の値がNULLの場合、現在のレコードはテーブルレベルまたはパーティションレベルで集計されていることを示します |
| LAST_ANALYZED_ROWS | bigint(20) | YES | 前回統計情報を収集した時点での行数。値がNULLの場合、統計情報が収集されたことがないことを示します |
| LAST_ANALYZED_TIME | timestamp(6) | YES | 前回統計情報を収集した時刻。値がNULLの場合、統計情報が収集されたことがないことを示します |
| INSERTS | bigint(21) | NO | 前回統計情報を収集して以来に挿入された行数 |
| UPDATES | bigint(21) | NO | 前回統計情報を収集して以来に更新された行数 |
| DELETES | bigint(21) | NO | 前回統計情報を収集して以来に削除された行数 |
| STALE_PERCENT | bigint(21) | NO | 統計情報が期限切れと判断される割合 |
| IS_STALE | varchar(3) | NO | 統計情報が期限切れかどうか。<ul><li>`YES`は期限切れを示します。<ul><li>テーブルの統計情報が収集されている場合、期限切れの条件は`INSERTS + UPDATES + DELETES > LAST_ANALYZED_ROWS * STALE_PERCENT / 100`です。  </li><li>テーブルの統計情報が収集されていない場合、期限切れの条件は`INSERTS + UPDATES + DELETES > 0`です。  </li></ul>  </li><li>`NO`は期限切れではないことを示します。 </li></ul>|

## クエリ例

本テナントのテーブル`t_subpart`について、前回統計情報を収集して以来に発生したDML操作の回数と統計情報の有効期限を確認します。
