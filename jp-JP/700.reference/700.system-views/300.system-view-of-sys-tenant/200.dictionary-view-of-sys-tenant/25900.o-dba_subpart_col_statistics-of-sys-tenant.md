...
このテナント内のすべてのサブパーティションテーブルの列統計情報とヒストグラム情報を表示します。
...
| DENSITY | decimal(10,0) | NO | 列の密度。COLUMN_NAMEにヒストグラムが利用可能な場合、この列はヒストグラム上で2つのエンドポイントを超えない値の選択性を示します。2つ以上のエンドポイントを超える値の選択性を表すものではありません。COLUMN_NAMEにヒストグラムが利用できない場合、この列の値は1 / NUM_DISTINCTとなります |
...
| SAMPLE_SIZE | decimal(10,0) | NO | クロマトグラフィーカラムを分析するためのサンプルサイズ |
| GLOBAL_STATS | varchar(3) | NO | 統計情報が収集されているかどうかを示します。<ul><li>YES</li><li>NO</li></ul> |
| USER_STATS | varchar(3) | NO | 統計情報がユーザーによって直接入力されたかどうかを示します。<ul><li>YES</li><li>NO</li></ul> |
| NOTES | varchar(80) | NO | 統計情報の追加の属性を説明します。例：値がINCREMENTALの場合、グローバル統計情報はサマリから取得されたものであり、つまりグローバル統計情報は増分でメンテナンスされています |
...
このテナントのパーティションテーブル`t_subpart`のサブパーティションの列統計情報を確認します。
...
* テーブルレベルの列統計情報の確認：[DBA_TAB_STATISTICS](26700.o-dba_tab_statistics-of-sys-tenant.md)
