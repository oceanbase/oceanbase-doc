...
ビュー `DBA_WR_SQLSTAT` は、テナント内で実行されたSQLの基本的なパフォーマンス統計データを表示するために使用されます。各SQL文が1行となります（つまり、各 `SQL_ID` + `Plan_Hash` の一意の値が1行となります）。そのうち、`_DELTA` を含む列は、前回のWRスナップショットから現在時点までの統計値の増分を示しています。
...
| EXECUTIONS_TOTAL              | bigint(20)          | NO   |   計画キャッシュ内でのその計画の総実行回数     |
| EXECUTIONS_DELTA              | bigint(20)          | NO   |   計画キャッシュ内でのその計画の総実行回数の増分      |
...
| ELAPSED_TIME_DELTA            | bigint(20)          | NO   |   このステートメントの消費時間の増分   |
...
| FULL_TABLE_SCAN_DELTA         | bigint(20)        | NO   | 前回のWRスナップショット収集以降の、このSQLによる全表スキャン回数<main id="notice" type='explain'><h4>説明</h4><p>V4.4.x系では、このフィールドはV4.4.2バージョンから導入されました。</p></main>|
...
| ERROR_COUNT_DELTA             | bigint(20)        | NO   | 前回のWRスナップショット収集以降の、このSQLのエラー回数<main id="notice" type='explain'><h4>説明</h4><p>V4.4.x系では、このフィールドはV4.4.2バージョンから導入されました。</p></main>|
