...
| RULE_NAME         | varchar(256)    | NO         | 感染ルール名                                           |
| PROTECTION_POLICY      | varchar(16)    | NO         | 保護ポリシー。値は `ENCRYPTION`、`MASKING`、`INVALID` のいずれかです。`ENCRYPTION` は暗号化保護を表し、`MASKING` はマスキング保護を表し、`INVALID` は無効を表します。                                        |
