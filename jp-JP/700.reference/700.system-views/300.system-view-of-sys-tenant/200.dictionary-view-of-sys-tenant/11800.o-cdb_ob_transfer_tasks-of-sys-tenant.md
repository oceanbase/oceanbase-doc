...
| NOT_EXIST_PART_LIST | longtext | YES | このタスクの実行計画では存在したが、ロック時には存在しなかったパーティションのリスト |
| LOCK_CONFLICT_PART_LIST | longtext | YES | このタスクの実行計画では存在したが、ロック時にロック競合が発生したパーティションのリスト |
...
| TABLET_LIST | longtext | YES | このTransfer操作に関連する完全なTabletリスト。PART_LISTにリストされているTabletだけでなく、ローカルインデックステーブルやLOB補助テーブルなどに関連付けられたTabletも含まれます。 |
...
| STATUS | varchar(64) | NO | タスクの状態：<ul><li>`INIT`：タスク作成  </li><li>`START`：Transferの実行開始 </li><li>`DOING`：Transferの実行中 </li><li>`ABORTED`：Transferタスクの実行失敗、終了 </li><li>`COMPLETED`：タスクの実行成功 </li><li>`FAILED`：タスクの実行失敗 </li><li>`CANCELED`：タスクのキャンセル </li></ul>|
| TRACE_ID | varchar(64) | NO | タスクの全プロセスTrace ID |
| RESULT | bigint(20) | YES | タスクの結果 |
