...
| MODIFY_TIME | timestamp(6) | YES | タスク完了時間。成功およびキャンセルした時間を含む |
...
| TABLE_LOCK_TABLET_LIST | longtext | YES | START状態に入った際にロックに成功したPartitionレベルのテーブルロックを持つTabletのリスト。テーブルロックの移行に使用されます。このリストにはPART_LIST内のパーティションテーブルに対応するTabletのみが含まれます。 |
| TABLET_LIST | longtext | YES | 今回のTransfer操作に関連する完全なTabletのリスト。PART_LISTにリストされているTabletだけでなく、ローカルインデックステーブルやLOB補助テーブルなどに関連付けられたTabletも含まれます。 |
...
| TABLE_LOCK_OWNER_ID | bigint(20) | YES | テーブルロックを取得する際、システムは各テーブルロックに対してowner_idを生成し、そのテーブルロックの所有者を識別します。テーブルロックを解除する際には、対応するowner_idを提供する必要があります。これにより、システムはテーブルロックを正しく認識し、解除できます。 |
...
* すべてのテナントで実行中のTabletレベルのロードバランシングタスクを確認する：[CDB_OB_TRANSFER_TASKS](11800.o-cdb_ob_transfer_tasks-of-sys-tenant.md)
