...
| NOT_EXIST_PART_LIST | longtext | YES | 本タスクの実行計画では存在したが、ロック時には存在しなかったパーティションのリスト |
| LOCK_CONFLICT_PART_LIST | longtext | YES | 本タスクの実行計画では存在したが、ロック時にロック競合が発生したパーティションのリスト |
...
| BALANCE_TASK_ID | bigint(20) | NO | 関連するロードバランシングタスクID。このTransferタスクは対応するロードバランシングタスクによって生成されたサブタスクです。 |
| TABLE_LOCK_OWNER_ID | bigint(20) | YES | テーブルロックを取得する際、システムは各テーブルロックに対してowner_idを生成し、そのテーブルロックの所有者を識別します。テーブルロックを解除する際には、対応するowner_idを提供する必要があります。これにより、システムはテーブルロックを正しく識別し、解除できます。 |
...
* すべてのテナントログストリームレベルのロードバランシングジョブを確認する：
...
* すべてのテナントログストリームレベルのロードバランシングタスクを確認する：
