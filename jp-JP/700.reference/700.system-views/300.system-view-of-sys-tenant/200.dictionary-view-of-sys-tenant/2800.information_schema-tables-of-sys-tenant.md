...
| DATA_LENGTH     | bigint(21) unsigned | NO | データ長、すなわちテーブルが占有するストレージ容量。マクロブロックサイズで計算し、単位はByteです。 |
| MAX_DATA_LENGTH | bigint(0) unsigned  | NO | 最大データ長。 |
| INDEX_LENGTH    | bigint(0) unsigned  | NO | インデックス長、すなわち非主キーインデックスが占有するストレージ容量。マクロブロックサイズで計算し、単位はByteです。<main id="notice" type='explain'><h4>説明</h4><p>このフィールドはV4.2.4バージョン以降では`NULL`として表示されなくなりました。</p></main> |
...
テーブルに関する詳細な説明については、[テーブルの表示](../../../../700.reference/300.database-object-management/100.manage-object-of-mysql-mode/200.manage-tables-of-mysql-mode/500.view-the-definition-of-a-table-of-mysql-mode.md)を参照してください。