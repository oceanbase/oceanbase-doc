...
| name                 | varchar(128)     | NO             | ストアドプロシージャ名                                                                                |
| type                 | varchar(10)      | NO             | ストアドプロシージャのタイプ：<ul><li>PROCEDURE：ストアドプロシージャ   </li><li> FUNCTION：ストアドファンクション  </li></ul>      |
| specific_name        | varchar(128)      | NO             | nameフィールドと同じです。</li>
| language             | varchar(4)       | NO             | 使用されていません。現在は固定値SQLです。                                                                          |
| sql_data_access      | varchar(32)      | NO             | 使用されていません。現在は固定値CONTAINS_SQLです。                                                                 |
| is_deterministic     | varchar(4)       | NO             | 使用されていません。ストアドプロシージャが`DETERMINISTIC`特性を持つかどうかを示します。定義：<ul><li>YES</li><li>NO</li></ul>  |
...
| sql_mode             | text      | NO             | ストアドプロシージャの作成または変更時に有効なSQLモードで、そのモードでストアドプロシージャを実行します                                                       |
...
ストアドプロシージャ`my_proc`の詳細情報を確認します。
