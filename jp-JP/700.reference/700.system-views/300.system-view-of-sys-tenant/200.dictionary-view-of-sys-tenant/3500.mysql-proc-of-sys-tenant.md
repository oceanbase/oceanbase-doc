...
| language             | varchar(4)       | NO             | 未使用、現在は固定値SQL                                                                          |
| sql_data_access      | varchar(32)      | NO             | 未使用、現在は固定値CONTAINS_SQL                                                                 |
| is_deterministic     | varchar(4)       | NO             | 未使用。ストアドプロシージャが`DETERMINISTIC`特性を使用するかどうかを示します。定義：<ul><li>YES</li><li>NO</li></ul>  |
| security_type        | varchar(10)      | NO             | 未使用。SQLのアクセス属性を示します。<ul><li> DEFINER   </li><li> INVOKER   </li></ul>  |
...
| sql_mode             | text      | NO             | ストアドプロシージャの作成または変更時に有効なSQLモード。このモードでストアドプロシージャを実行します                                                       |
| comment              | varchar(4096) | NO             | コメントテキスト|
| character_set_client | varchar(128)     | NO             | ストアドプロシージャの作成時または変更時のcharacter_set_clientシステム変数のセッション値                                             |
| collation_connection | varchar(128)     | NO             | ストアドプロシージャの作成時または変更時のcollation_connectionシステム変数のセッション値                                               |
| collation_database   | varchar(128)     | NO             | ストアドプロシージャの作成時または変更時のcollation_databaseシステム変数のセッション値                                                 |
