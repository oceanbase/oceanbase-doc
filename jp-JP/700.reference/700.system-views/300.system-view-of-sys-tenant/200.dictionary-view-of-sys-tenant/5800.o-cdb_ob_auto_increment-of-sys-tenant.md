...
このクラスタ内のすべての自動インクリメント列の情報を表示します。システムテナントのみが閲覧できます。
...
| SYNC_VALUE           | bigint(20) unsigned | NO             | 同期値、つまり`Insert`ステートメントで明示的に指定された自動インクリメント列の値であり、その値は自動インクリメント列の現在値よりも大きい。 |
