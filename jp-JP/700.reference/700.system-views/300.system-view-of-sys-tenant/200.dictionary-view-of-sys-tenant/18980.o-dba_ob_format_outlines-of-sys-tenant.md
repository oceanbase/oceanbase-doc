...
| VISIBLE_SIGNATURE | longtext      | NO   | Signature逆シリアライズ結果を表します。|
| FORMAT_SQL_TEXT   | longtext      | YES  | 実行計画Outline作成時に`ON`句で指定されたSQLが生成した`FORMAT SQL TEXT`を表します。|
...
現在のテナントの次のFormat Outlineの情報を照会します。
