...
`CDB_WR_SYSTEM_EVENT` ビューは、すべてのテナントの永続化されたテナントレベルの待機イベント情報を表示するために使用されます。
...
| WAIT_CLASS_ID     | bigint(20)  | YES  |  待機イベント分類のID      |
| WAIT_CLASS        | varchar(64) | YES  |  待機イベント分類名       |
...
ビュー内の `WAIT_CLASS` 列の値には、待機イベントが属するタイプ名と対応する待機イベントIDが含まれており、その内容は次の表のとおりです。
...
| ADMINISTRATIVE | 103 | データベース管理者がコマンドを入力したことによるユーザーの待機待機イベント |
...
| SYSTEM_IO | 109 | バックグラウンドプロセスのI/Oを待機するイベント |
| CLUSTER | 111 | クラスタ関連のイベント|
