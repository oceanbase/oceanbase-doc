...
| DTD_IDENTIFIER | longtext | NO | 使用されていません。プロシージャがストアドファンクションの場合、戻り値はデータ型です。プロシージャがストアドプロシージャの場合、この値は空です。`DATA_TYPE` の値は型名のみで、他の情報は含まれません。一方、DTD_IDENTIFIER の値には型名と精度や長さなどのその他の情報が含まれます。 |
...
| SQL_MODE | text | NO | プロシージャの作成または変更時に有効だったSQLモードです。プロシージャはこのモードで実行されます。 |
| ROUTINEコメント | longtext | NO | コメントテキスト |
