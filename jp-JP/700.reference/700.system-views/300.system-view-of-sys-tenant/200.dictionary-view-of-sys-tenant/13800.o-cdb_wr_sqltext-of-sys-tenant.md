...
|slug|oceanbase-cdb_wr_sqltext-sys-tenant|
...
ビュー `CDB_WR_SQLTEXT` は、すべてのテナントのWR収集SQLテキストを表示するために使用されます。
...
| SQL_ID    | varchar(32) | NO   |  SQLステートメントのID      |
| QUERY_SQL | longtext    | NO   |  SQLテキスト      |
| SQL_TYPE  | bigint(20)  | NO   |  ステートメントタイプ      |

ビュー内の`SQL_TYPE`列の値と対応するステートメントタイプは、次の表のとおりです。
...
すべてのテナントのWR収集SQLテキストを照会し、最初のレコードを表示します。
