...
| PARTITION_NAME | varchar(128) | YES | パーティション名。パーティションテーブルのこの列の値がNULLの場合、現在のレコードはテーブルレベルで集計されていることを示します |
| SUBPARTITION_NAME | varchar(128) | YES | サブパーティション名。サブパーティションテーブルのこの列の値がNULLの場合、現在のレコードはテーブルレベルまたはパーティションレベルで集計されていることを示します |
| LAST_ANALYZED_ROWS | bigint(20) | YES | 前回統計情報を収集した際の行数。値がNULLの場合、統計情報が収集されたことがないことを示します |
| LAST_ANALYZED_TIME | timestamp(6) | YES | 前回統計情報を収集した際の時間。値がNULLの場合、統計情報が収集されたことがないことを示します |
| INSERTS | bigint(21) | NO | 前回統計情報を収集してから、何行が挿入されたか |
| UPDATES | bigint(21) | NO | 前回統計情報を収集してから、何行が更新されたか |
| DELETES | bigint(21) | NO | 前回統計情報を収集してから、何行が削除されたか |
| STALE_PERCENT | bigint(21) | NO | 統計情報が期限切れとなる割合 |
| IS_STALE | varchar(3) | NO | 統計情報が期限切れかどうか。<ul><li>`YES` 期限切れを示します <ul><li>テーブルに統計情報が収集された場合、期限切れの条件は `INSERTS + UPDATES + DELETES > LAST_ANALYZED_ROWS * STALE_PERCENT / 100`  </li><li>テーブルに統計情報が収集されていない場合、期限切れの条件は `INSERTS + UPDATES + DELETES > 0`  </li></ul>  </li><li>`NO` 期限切れではないことを示します </li></ul>
