...
現在のテナント配下にあるすべてのパーティションテーブルのパーティションごとのヒストグラムデータ（各ヒストグラムのエンドポイント）を表示します。
...
| ENDPOINT_ACTUAL_VALUE_RAW | text | NO | エンドポイントの実際の値（生形式） |
| ENDPOINT_REPEAT_COUNT | decimal(10,0) | NO | エンドポイントの頻度（ハイブリッドヒストグラムのみ。その他のヒストグラムタイプは0に設定されます） |

## クエリ例

システムテナントが自身のテナントにあるパーティションテーブル `test_part_hist` のパーティションごとのヒストグラム情報を確認します。
