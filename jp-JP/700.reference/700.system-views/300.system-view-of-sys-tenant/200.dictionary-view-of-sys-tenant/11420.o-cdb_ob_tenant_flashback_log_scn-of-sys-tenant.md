...
ビュー `CDB_OB_TENANTFLASHBACK_LOG_SCN` は、クラスタ内のすべてのテナントにおける各ログトリミングの関連情報を表示するために使用されます。
...
| SWITCHOVER_EPOCH  | bigint(20)   | NO         | テナントロールが `NORMAL` 状態に切り替わったときの `SWITCHOVER_EPOCH`。`SWITCHOVER_EPOCH` はテナントロール切り替え（Switchover/Failover）操作を識別する一意の識別子です |
| OP_TYPE           | varchar(64)  | NO          | 操作タイプ。次のものが含まれます：<ul><li>`FAILOVER_TO_PRIMARY`：これはFailover操作のタイプで、プライマリテナントが利用不能な場合にスタンバイテナントをプライマリテナントに切り替える操作です。</li><li>`FLASHBACK_STANDBY_LOG`：これはスタンバイテナントのログトリミング操作のタイプで、スタンバイテナントのログを指定されたSCN位置までトリミングするために使用されます。</li></ul> |
| FLASHBACK_LOG_SCN | bigint(20) unsigned | NO  | テナントログを後から前にトリミングする終点             |

## クエリ例

クラスタ内のすべてのテナントにおける、各ログトリミングの関連情報を照会します。
