...
ビュー `CDB_WR_SQLTEXT` は、すべてのテナントで収集されたWR SQLテキストを表示するために使用されます。
...
ビュー内の `SQL_TYPE` 列の値と対応するステートメントタイプは次の表のとおりです。
...
すべてのテナントが収集したSQLテキストを照会し、1件目のレコードを表示します。
...
[oceanbase.DBA_WR_SQLTEXT](26600.o-dba_wr_sqltext-of-sys-tenant.md)
