...
このテナント内のすべてのサブパーティションテーブルに対する列統計情報とヒストグラム情報を表示します。
...
| LAST_ANALYZED | datetime(6) | NO | この列が最後に分析された時間 |
...
| NOTES | varchar(80) | NO | 統計情報の追加プロパティを説明します。例えば、値がINCREMENTALの場合、グローバル統計情報はサマリーから取得されます。つまり、グローバル統計情報は増分メンテナンスされます |
