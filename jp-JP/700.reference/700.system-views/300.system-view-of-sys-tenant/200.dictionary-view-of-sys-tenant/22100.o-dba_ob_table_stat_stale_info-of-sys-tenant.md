...
| PARTITION_NAME | varchar(128) | YES | パーティション名。パーティションテーブルにおいて、この列の値がNULLの場合、現在のレコードはテーブルレベルで集計されたものを示します |
| SUBPARTITION_NAME | varchar(128) | YES | サブパーティション名。サブパーティションテーブルにおいて、この列の値がNULLの場合、現在のレコードはテーブルレベルまたはパーティションレベルで集計されたものを示します |
| LAST_ANALYZED_ROWS | bigint(20) | YES | 前回統計情報を収集した際の行数。値がNULLの場合、統計情報が収集されたことがないことを意味します |
| LAST_ANALYZED_TIME | timestamp(6) | YES | 前回統計情報を収集した時刻。値がNULLの場合、統計情報が収集されたことがないことを意味します |
...
| IS_STALE | varchar(3) | NO | 統計情報が期限切れかどうか。<ul><li>`YES` は期限切れを意味します。<ul><li>テーブルの統計情報が収集されている場合、期限切れの条件は `INSERTS + UPDATES + DELETES > LAST_ANALYZED_ROWS * STALE_PERCENT / 100` です。 </li><li>テーブルの統計情報が収集されていない場合、期限切れの条件は `INSERTS + UPDATES + DELETES > 0` です。 </li></ul>  </li><li>`NO` は期限切れではないことを意味します。 </li></ul>|

## クエリ例

このテナントのテーブル `t_subpart` で、前回統計情報を収集して以来に発生したDDL操作の回数と統計情報の有効期限を確認します。
