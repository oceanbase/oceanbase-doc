...
ビュー`GV$SQL_PLAN_MONITOR`は、すべてのテナントにおけるすべてのOBServerノードのスロークエリのOperationレベルの統計を表示するために使用されます。各スロークエリの各Operationについて統計情報が提供されます。
...
| STATUS                  | varchar(19)          | NO | オペレータの実行状態：<ul><li>EXECUTING：実行中</li><li>DONE(ERROR)：実行エラー</li><li>DONE(FIRST N ROWS)：実行成功</li><li>DONE(ALL ROWS)：実行成功</li><li>DONE：実行中断</li></ul> |
...
| FIRST_CHANGE_TIME       | timestamp(6)         | YES| オペレータが最初の行データを吐き出した時間 |
| LAST_CHANGE_TIME        | timestamp(6)         | YES| オペレータが最後の行データを吐き出した時間 |
...
すべてのテナントのすべてのOBServerノードにおけるスロークエリのOperation層面の統計を照会し、最初のレコードを表示します。
...
* [V$SQL_PLAN_MONITOR](36300.v-sql_plan_monitor-of-sys-tenant.md)
