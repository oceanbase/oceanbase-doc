...
ビュー`GV$SESSION_EVENT`は、クラスタ内のすべてのサーバーのセッションレベルでの待機イベントを表示するために使用されます。
...
クラスタ内のすべてのサーバーのセッションレベルでの待機イベントを照会します。
