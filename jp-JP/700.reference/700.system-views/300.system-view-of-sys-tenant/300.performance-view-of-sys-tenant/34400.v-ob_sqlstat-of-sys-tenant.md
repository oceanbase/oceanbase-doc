...
ビュー `V$OB_SQLSTAT` は、現在のノード上で実行されたSQL文の基本的なパフォーマンス統計データを表示します。各SQL文につき1行が割り当てられます（つまり、各 `SQL_ID` + `Plan_Hash` の一意の値に対して1行が割り当てられます）。`_DELTA` を含む列は、前回のWRスナップショットから現在時点までの統計値の増分を示しています。
...
| MUTI_QUERY_BATCH_DELTA        | bigint(20)        | NO   | 前回のWRスナップショット取得以降、このSQLバッチが実行された回数<main id="notice" type='explain'><h4>説明</h4><p>V4.4.x系では、このフィールドはV4.4.2バージョンから導入されました。</p></main>|
| FULL_TABLE_SCAN_TOTAL         | bigint(20)        | NO   | 実行計画が生成されて以来、このSQLによる全表スキャンの回数<main id="notice" type='explain'><h4>説明</h4><p>V4.4.x系では、このフィールドはV4.4.2バージョンから導入されました。</p></main>|
| FULL_TABLE_SCAN_DELTA         | bigint(20)        | NO   | 前回のWRスナップショット取得以降、このSQLによる全表スキャンの回数<main id="notice" type='explain'><h4>説明</h4><p>V4.4.x系では、このフィールドはV4.4.2バージョンから導入されました。</p></main>|
| ERROR_COUNT_TOTAL             | bigint(20)        | NO   | 実行計画が生成されて以来、このSQLで発生したエラーの回数<main id="notice" type='explain'><h4>説明</h4><p>V4.4.x系では、このフィールドはV4.4.2バージョンから導入されました。</p></main>|
| ERROR_COUNT_DELTA             | bigint(20)        | NO   | 前回のWRスナップショット取得以降、このSQLで発生したエラーの回数<main id="notice" type='explain'><h4>説明</h4><p>V4.4.x系では、このフィールドはV4.4.2バージョンから導入されました。</p></main>|
