...
ビュー `V$OB_SQLSTAT` は、現在のノード上で実行されているSQLの基本的なパフォーマンス統計データを表示します。各SQL文につき1行が割り当てられます（つまり、各 `SQL_ID` + `Plan_Hash` の一意の値に対して1行）。`_DELTA` を含む列は、前回のWRスナップショットから現在時点までの統計値の増分を示しています。
...
| MODULE                        | varchar(64)         | NO   |   この文が初めて解析されたときのアプリケーションモジュール    |
| ACTION                        | varchar(64)         | NO   |   この文が初めて解析されたときのアプリケーションアクション      |
...
| NESTED_SQL_DELTA              | bigint(20)          | NO   |   ネストSQL実行の増分値    |
...
| FULL_TABLE_SCAN_TOTAL         | bigint(20)        | NO   |   プラン生成後、このSQLが全表スキャンした回数<main id="notice" type='explain'><h4>説明</h4><p>V4.4.x系では、このフィールドはV4.4.2バージョンから導入されました。</p></main>|
| FULL_TABLE_SCAN_DELTA         | bigint(20)        | NO   |   前回のWRスナップショット収集以降、このSQLが全表スキャンした回数<main id="notice" type='explain'><h4>説明</h4><p>V4.4.x系では、このフィールドはV4.4.2バージョンから導入されました。</p></main>|
