...
`GV$OB_PLAN_CACHE_PLAN_EXPLAIN` ビューは、すべてのOBServerノードにキャッシュされているプランキャッシュ内の物理実行計画を表示するために使用されます。

  <main id="notice" type='explain'>
    <h4>説明</h4>
    <p>このビューはGET操作のみをサポートしており、クエリ時にはIP、PORT、TENANT_ID、PLAN_IDフィールドを指定する必要があります。</p>
  </main>
...
1. [GV$OB_PLAN_CACHE_PLAN_STAT](2300.gv-ob_plan_cache_plan_stat-of-sys-tenant.md) ビューを使用して、現在のOBServerノードの `PLAN_ID` を照会します。
...
2. `PLAN_ID=605` を例に、`GV$OB_PLAN_CACHE_PLAN_EXPLAIN` ビューを使用して、プランキャッシュ内の物理実行プランを照会します。
