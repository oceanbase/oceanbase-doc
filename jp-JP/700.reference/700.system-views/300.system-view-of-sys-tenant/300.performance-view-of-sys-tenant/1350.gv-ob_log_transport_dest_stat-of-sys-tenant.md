...
ビュー `GV$OB_LOG_TRANSPORT_DEST_STAT` は、すべてのテナント内のすべてのOBServerにおけるダウンストリームログ同期リンク（OBCDC/ネットワークスタンバイデータベース）の情報を表示します。
...
| CLIENT_IP                   | varchar(46)         | NO   | 要求元のプロセスが存在するマシンのIPアドレス     |
| CLIENT_PID                  | bigint(20)          | NO   | 要求元のプロセスID     |
| CLIENT_TENANT_ID            | bigint(20)          | NO   | 要求元のテナントID     |
| CLIENT_TYPE                 | varchar(7)          | NO   | 要求元のタイプ：<ul><li>STANDBY：ネットワークスタンバイデータベース</li><li>CDC：OBCDC</li><li>UNKNOWN：クライアントタイプ不明</li></ul>    |
...
| LAST_REQUEST_TYPE           | varchar(24)         | NO   | 前回のリクエストのRPCタイプ：<ul><li>0：SEQUENTIAL_READ_SERIAL、直列ログ転送プロトコルを使用してログを順次読むことを示します。</li><li>1：SEQUENTIAL_READ_PARALLEL、並列ログ転送プロトコルを使用してログを順次読むことを示します。</li><li>2：SCATTERED_READ、前回のリクエストが離散したログの読み取りであったことを示します。</li><li>その他の値：RPCタイプが不明であることを示します。</li></ul>     |
...
| AVG_REQUEST_QUEUE_TIME      | bigint(20)          | NO   | 平均RPC待機時間     |
...
すべてのテナント内のすべてのOBServerにおけるダウンストリームログ同期リンク（OBCDC/ネットワークスタンバイデータベース）の情報を照会します。
