...
| PROPERTY     | varchar(4096) | NO         | 対応Operatorの情報  |

## クエリ例

1. [V$OB_PLAN_CACHE_PLAN_STAT](32700.v-ob_plan_cache_plan_stat-of-sys-tenant.md)ビューで現在のOBServerノードの`PLAN_ID`を取得します。
...
2. `PLAN_ID=605` を例に、`V$OB_PLAN_CACHE_PLAN_EXPLAIN` ビューを使用してプランキャッシュ内の物理実行計画を照会します。
