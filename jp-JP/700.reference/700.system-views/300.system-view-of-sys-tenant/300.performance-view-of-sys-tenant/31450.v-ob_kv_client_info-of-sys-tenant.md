...
システムテナント内で、ビュー`V$OB_KV_CLIENT_INFO`は、現在のノード上のすべてのOBKVテナントクライアント接続の設定パラメータ情報にアクセスすることができます。
...
| runtime.continuous_failure.ceiling           | 100          | 連続運行失敗の上限。この値に達するとTABLEのアドレスがリフレッシュされます。 |
| runtime.max.wait              | 3000          | 単一実行のタイムアウト時間。タイムアウト時間内に再試行されます。         |
...
システムテナントでビュー`V$OB_KV_CLIENT_INFO`をクエリするステートメントは次のとおりです：

```shell
obclient [oceanbase]> SELECT * FROM oceanbase.V$OB_KV_CLIENT_INFO LIMIT 1;
```
...
* [GV$OB_KV_CLIENT_INFO](6050.gv-ob_kv_client_info-of-sys-tenant.md)
