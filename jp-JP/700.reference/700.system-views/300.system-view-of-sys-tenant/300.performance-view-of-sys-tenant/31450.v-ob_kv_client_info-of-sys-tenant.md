...
システムテナントでは、ビュー `V$OB_KV_CLIENT_INFO` を使用して、現在のノード上のすべてのOBKVテナントクライアント接続の設定パラメータ情報にアクセスできます。
...
| svr_ip         | varchar:MAX_IP_ADDR_LENGTH           | NO   | 所在サーバーIPアドレス              |
| svr_port       | bigint                               | NO   | 所在サーバーポート                  |
...
| runtime.continuous_failure.ceiling           | 100          | 連続運用失敗の上限回数。この回数に達すると、TABLEのアドレスがリフレッシュされます。 |
| runtime.max.wait              | 3000          | 単一実行のタイムアウト時間内におけるリトライ回数。         |
| runtime.retry.interval        | 1             | 実行時にエラーが発生した場合のリトライ時間間隔。                 |
...
| table.entry.acquire.connect.timeout           | 500          | TABLEアドレスリフレッシュ接続要求の作成タイムアウト時間。          |
...
| connection.max.expired.time           | 8          | クライアント接続のリフレッシュ再ログインまでの最大時間間隔          |
