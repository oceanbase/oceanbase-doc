...
|slug|gv-sql_workarea_histogram-of-sys-tenant|
...
以前にすべてのワークエリアで実行された統計情報を表示します。例えば、1MB～2MBの区間で全てのデータをインメモリ処理した回数やワンパス処理の回数などです。
...
システムテナントが、テナントIDが`1002`のテナントにおけるすべてのノード上で以前に実行されたすべてのワークエリアの統計情報を照会し、上位10件のレコードを表示します。
