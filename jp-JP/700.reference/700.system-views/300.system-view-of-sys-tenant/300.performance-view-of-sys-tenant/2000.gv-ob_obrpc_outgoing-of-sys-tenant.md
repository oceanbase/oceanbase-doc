...
すべてのテナントにおける、すべてのOBServerノード上でのRPC送信統計情報を表示します。
...
| PCODE_NAME     | varchar(256) | NO             | RPCパケットコードの対応する名前 |
| COUNT          | bigint(20)   | NO             | 使用回数                  |
| TOTAL_TIME     | bigint(20)   | NO             | 総合計測時間、単位はマイクロ秒                |
...
システムテナント内で、テナントIDが1004のテナントがすべてのノードに送信したRPCリクエストを確認し、上位10件のレコードを表示します。
...
* すべてのテナントが現在のノードで送信したPRC統計情報を確認する：[V$OB_RPC_OUTGOING](32400.v-ob_obrpc_outgoing-of-sys-tenant.md)

* すべてのテナントがすべてのノードで受信したRPCの統計情報を確認する：[GV$OB_RPC_INCOMING](1900.gv-ob_obrpc_incoming-of-sys-tenant.md)