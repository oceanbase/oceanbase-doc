...
各テナントの全OBServerノードにおける全体のキャッシュ状況を表示します。
...
| STORAGE_CACHE_POLICY | varchar(64)   | NO   |  このTabletが現在いるキャッシュ状態：<ul><li>`AUTO`：自動ホット/コールドデータ識別</li> <li>`HOT`：ホットデータ</li></ul>   |
| CACHED_DATA_SIZE     | bigint(20)    | NO   |  このTabletがローカルにキャッシュしたデータ量、単位はバイト   |
| CACHE_hit_COUNT      | bigint(20)    | NO   |  このTabletのキャッシュヒット回数。現在このフィールドは未使用で、値は `-1`    |
| CACHE_MISS_COUNT     | bigint(20)    | NO   |  このTabletのキャッシュミス回数。現在このフィールドは未使用で、値は `-1`    |
| CACHE_HIT_SIZE       | bigint(20)    | NO   |  このTabletのキャッシュヒットデータ量、単位はバイト。現在このフィールドは未使用で、値は `-1`   |
| CACHE_MISS_SIZE      | bigint(20)    | NO   |  このTabletのキャッシュミスデータ量、単位はバイト。現在このフィールドは未使用で、値は `-1`   |
| INFO                 | varchar(4096) | NO   |  その他の情報    |

## クエリ例

SSモードで、システムテナントがテナントID `1002` のテナントの全OBServerノードにおける全体のキャッシュ状況を確認します。
