...
各テナントのすべてのOBServerノード上のキャッシュ状況を全体として表示します。
...
| STORAGE_CACHE_POLICY | varchar(64)   | NO   |  このTabletが現在いるキャッシュ状態：<ul><li>`AUTO`：自動でホット/コールドデータを識別する</li> <li>`HOT`：ホットデータ</li></ul>   |
| CACHED_DATA_SIZE     | bigint(20)    | NO   |  このTabletにローカルにキャッシュされたデータ量、単位はバイト   |
...
| CACHE_HIT_SIZE       | bigint(20)    | NO   |  このTabletのキャッシュヒットデータ量、単位はバイト。現在このフィールドは使用されておらず、値は `-1`   |
| CACHE_MISS_SIZE      | bigint(20)    | NO   |  このTabletのキャッシュミスデータ量、単位はバイト。現在このフィールドは使用されておらず、値は `-1`   |
| INFO                 | varchar(4096) | NO   |  その他の情報    |

## クエリ例

SSモードで、システムテナントがテナントID `1002` のテナントの全OBServerノード上のキャッシュ状況を確認します。
