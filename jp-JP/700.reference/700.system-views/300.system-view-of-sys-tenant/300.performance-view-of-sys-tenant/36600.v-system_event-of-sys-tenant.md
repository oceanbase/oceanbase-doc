...
各テナントの現在のOBServerノード上で発生している待機イベントを表示し、各種待機イベントの発生回数、待機時間、タイムアウト回数などの統計情報を集計します。
...
システムテナントで、現在のノード上におけるすべてのテナントの待機イベントを確認し、上位10件のレコードを表示します。
...
* テナント内のすべての待機イベント定義を確認する：[V$EVENT_NAME](30300.v-event_name-of-sys-tenant.md)
