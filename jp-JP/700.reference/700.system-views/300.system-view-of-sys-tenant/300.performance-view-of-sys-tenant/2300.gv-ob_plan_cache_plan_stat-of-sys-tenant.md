...
ビュー `GV$OB_PLAN_CACHE_PLAN_STAT` は、すべてのテナントにおけるすべてのOBServerノード上のプランキャッシュ内にキャッシュされている各キャッシュオブジェクトの状態を表示します。
...
| STATEMENT             | longtext            | NO         | SQLプランの場合、このフィールドはパラメータ化されたSQL文です。匿名ブロックオブジェクトの場合、このフィールドはパラメータ化された匿名ブロック文です。その他のオブジェクトではこのフィールドは無効です                     |
| QUERY_SQL             | longtext            | NO         | SQL計画については、計画が初めて読み込まれた際のクエリの元のSQL文です。匿名ブロックの場合、このフィールドにはパラメータ化された匿名ブロック文が格納されます                        |
...
| PLAN_HASH             | bigint(20) unsigned | NO         | SQL計画のHash値                                                                   |
| FIRST_LOAD_TIME       | timestamp(6)        | NO         | 初回読み込み時間                                                                         |
| SCHEMA_VERSION        | bigint(20)          | NO         | スキーマバージョン番号                                                                       |
| LAST_ACTIVE_TIME      | timestamp(6)        | NO         | 最終実行時間                                                                         |
| AVG_EXE_USEC          | bigint(20)          | NO         | 平均実行時間（マイクロ秒単位）                                                                           |
| SLOWEST_EXE_TIME      | timestamp(6)        | NO         | 最遅延実行タイムスタンプ                                                                          |
| SLOWEST_EXE_USEC      | bigint(20)          | NO         | 最遅延実行の実行時間                                                                        |
...
| USER_IO_WAIT_TIME     | bigint(20) unsigned | NO         | 全てのuser_io系イベントの合計時間、単位はマイクロ秒                                                               |
| ROWS_PROCESSED        | bigint(20)          | NO         | 全てのSchedule系イベントの時間                                                               |
| ELAPSED_TIME          | bigint(20) unsigned | NO         | リクエスト受信から実行終了までにかかった時間、単位はマイクロ秒                                                                 |
| CPU_TIME              | bigint(20) unsigned | NO         | 全体の実行にかかったCPU時間。単位はマイクロ秒。                                                                   |
...
| OUTLINE_ID            | bigint(20)          | NO         | アウトラインのID。-1はアウトライン結合によって生成された計画ではないことを示す                                         |
...
| EVO_CPU_TIME          | bigint(20) unsigned | NO         | 進化過程での総実行CPU時間。単位はマイクロ秒。                                                                 |
| TIMEOUT_COUNT         | bigint(20)          | NO         | タイムアウト回数                                                                            |
| PS_STMT_ID            | bigint(20)          | NO         | リクエストに対応するPrepare ID：<ul><li>`-1`：SQLステートメントがPSプロトコルを使用していないことを示す   </li><li> `-1`以外の場合：SQLステートメントがPSプロトコルを使用しており、そのステートメントに対してPSプロトコルが返した一意のフラグであることを示す  </li></ul>      |
...
| TEMP_TABLES           | longtext            | NO         | SQL計画に含まれるテンポラリテーブルの名前です。テンポラリテーブルがない場合、このフィールドは空になります                                                     |
...
| PL_SCHEMA_ID          | bigint(20) unsigned | NO         | 匿名ブロックでないPLオブジェクトについては、キャッシュオブジェクトのSchema IDを表します。匿名ブロックの場合はps_stmt_idと同じです。SQL計画については、このフィールドに意味はありません       |
| IS_BATCHED_MULTI_STMT | tinyint(4)          | NO         | Batched Multistmt最適化計画であるかどうかを示します。                                                    |
