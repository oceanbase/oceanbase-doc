...
現在のOBServerノード上で、すべてのテナントによって送信されたRPCの統計情報を表示します。
...
| LAST_TIMESTAMP | timestamp(6) | NO             | 統計情報が最後に更新された時間            |

## クエリ例

システムテナント内で、テナントIDが1004のテナントが現在のノードで送信したRPCリクエストを確認し、上位10件のレコードを表示します。
...
* すべてのテナントがすべてのノードで送信したPRC統計情報を確認する：[GV$OB_RPC_OUTGOING](2000.gv-ob_obrpc_outgoing-of-sys-tenant.md)

* すべてのテナントが現在のノードで受信したRPCの統計情報を確認する：[V$OB_RPC_INCOMING](32300.v-ob_obrpc_incoming-of-sys-tenant.md)