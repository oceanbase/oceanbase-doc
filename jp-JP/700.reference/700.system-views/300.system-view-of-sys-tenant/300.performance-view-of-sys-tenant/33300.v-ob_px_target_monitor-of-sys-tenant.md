...
|slug|v-ob_px_target_monitor-sys-tenant|
...
各テナントの現在のOBServerノード上のPXスレッドの使用状況を表示します。
...
| PEER_IP                      | varchar(46)         | NO         | 相手側サーバーのIPアドレス          |
| PEER_PORT                    | bigint(20)          | NO         | 相手側サーバーのポート番号          |
| PEER_TARGET                  | bigint(20)          | NO         | 相手側サーバーのPXスレッド総数          |
| PEER_TARGET_USED             | bigint(20)          | NO         | 相手側サーバーのPXスレッド使用数          |
| LOCAL_TARGET_USED            | bigint(20)          | NO         | ローカルサーバーが使用する相手側PXスレッド数          |
| LOCAL_PARALLEL_SESSION_COUNT | bigint(20)          | NO         | ローカルサーバーで並列実行中のセッション数          |

## クエリ例

システムテナントで、すべてのテナントの現在のOBServerノード上のPXスレッド使用状況を照会します。
