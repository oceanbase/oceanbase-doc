...
現在のOBServerノード上のobserverプロセス内のすべてのスレッドの状態情報を表示します。システムテナントでは、すべてのテナントに関連するスレッドを表示します。
...
| STATUS | varchar(32) | NO |  スレッドの状態：<ul><li> Run スレッドが実行中であることを示します  </li><li> Wait スレッドが待機状態であることを示します </li><li> Sleep スレッドがスリープ状態であることを示します </li><li> Join 別のスレッドの終了を待っていることを示します </li></ul>|
...
システムテナントで、現在のOBServerノード上のobserverプロセス内のすべてのスレッドの状態情報を確認し、最初の2件のレコードを表示します。
