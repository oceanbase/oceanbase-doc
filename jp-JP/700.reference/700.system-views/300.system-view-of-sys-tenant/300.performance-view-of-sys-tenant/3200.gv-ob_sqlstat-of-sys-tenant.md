...
| PLAN_TYPE                     | bigint(20)          | NO   |    SQL計画キャッシュの場合、このフィールドは計画のタイプを示します：<ul><li>`1`：Local Planを表します。</li> <li>`2`：Remote Planを表します。</li> <li>`3`：Distributed Planを表します。</li></ul> </br>PLオブジェクトキャッシュの場合、このフィールドはPLオブジェクトのタイプを示します：<ul><li>`1`：Procedureを表します。</li> <li>`2`：Functionを表します。</li> <li>`3`：Packageを表します。</li> <li>`4`：Anonymous Blockを表します。</li></ul> |
...
| EXECUTIONS_TOTAL              | bigint(20)          | NO   |   プランキャッシュ内のこのプランの総実行回数     |
| EXECUTIONS_DELTA              | bigint(20)          | NO   |   プランキャッシュ内のこのプランの総実行回数の増分      |
| DISK_READS_TOTAL              | bigint(20)          | NO   |   ディスク読み込みの総回数 |
...
| CCWAIT_TOTAL                  | bigint(20)          | NO   |   コンカレンシー系待機イベントの総使用時間       |
| CCWAIT_DELTA                  | bigint(20)          | NO   |   コンカレンシー系待機イベント総使用時間の増分       |
| USERIO_WAIT_TOTAL             | bigint(20)          | NO   |   I/O待機の総使用時間      |
| USERIO_WAIT_DELTA             | bigint(20)          | NO   |   I/O待機の総使用時間の増分      |
| APWAIT_TOTAL                  | bigint(20)          | NO   |   アプリケーション系待機イベントの総使用時間        |
| APWAIT_DELTA                  | bigint(20)          | NO   |   アプリケーション系待機イベント総使用時間の増分      |
...
| ROWS_PROCESSED_DELTA          | bigint(20)          | NO   |   ステートメントが処理した行数の増分     |
...
| PARTITION_TOTAL               | bigint(20)          | NO   |   SQL実行スキャン対象のパーティション数の合計      |
| PARTITION_DELTA               | bigint(20)          | NO   |   SQL実行スキャン対象のパーティション数の増分    |
| NESTED_SQL_TOTAL              | bigint(20)          | NO   |   ネストSQL実行の合計、サブSQLが1回ごとに実行されるたびに1ずつ増加します。     |
| NESTED_SQL_DELTA              | bigint(20)          | NO   |   ネストSQL実行の増分    |
| SOURCE_IP                     | varchar(46)         | NO   |   要求元のIPアドレス     |
| SOURCE_PORT                   | bigint(20)          | NO   |   要求元のポート番号      |
| ROUTE_MISS_TOTAL              | bigint(20)          | NO   |   自己実行計画生成以降、要求パーティションに正しくヒットしなかった総回数   |
| ROUTE_MISS_DELTA              | bigint(20)          | NO   |   最後のWRスナップショット取得以降、リクエストパーティションに正しくヒットしなかった総回数   |
| FIRST_LOAD_TIME               | timestamp(6)      | YES  | 実行計画の生成時刻  |
| PLAN_CACHE_HIT_TOTAL          | bigint(20)        | NO   | 実行計画生成以降、Plan Cacheにヒットした総回数  |
| PLAN_CACHE_HIT_DELTA          | bigint(20)        | NO   | 最後のWRスナップショット取得以降、Plan Cacheにヒットした回数  |
...
| MUTI_QUERY_DELTA              | bigint(20)        | NO   | 最後のWRスナップショット取得以降、このSQLがMuti Queryに存在した回数<main id="notice" type='explain'><h4>説明</h4><p>V4.2.5バージョンでは、このフィールドはV4.2.5 BP7バージョンから導入されました。</p></main>|
...
| MUTI_QUERY_BATCH_DELTA        | bigint(20)        | NO   | 最後のWRスナップショット取得以降、このSQL Batchが実行された回数<main id="notice" type='explain'><h4>説明</h4><p>V4.2.5バージョンでは、このフィールドはV4.2.5 BP7バージョンから導入されました。</p></main>|
...
| FULL_TABLE_SCAN_DELTA         | bigint(20)        | NO   | 最後のWRスナップショット取得以降、このSQLがフルテーブルスキャンを実行した回数<main id="notice" type='explain'><h4>説明</h4><p>V4.2.5バージョンでは、このフィールドはV4.2.5 BP7バージョンから導入されました。</p></main>|
...
| ERROR_COUNT_DELTA             | bigint(20)        | NO   | 最後のWRスナップショット収集以降、このSQLがエラーになった回数<main id="notice" type='explain'><h4>説明</h4><p>V4.2.5バージョンでは、このフィールドはV4.2.5 BP7バージョンから導入されました。</p></main>|
...
*************************** 1. row ***************************
             ...
             EXECUTIONS_TOTAL: 178246
             EXECUTIONS_DELTA: 817
             DISK_READS_TOTAL: 16
             DISK_READS_DELTA: 0
            BUFFER_GETS_TOTAL: 1687936
            BUFFER_GETS_DELTA: 8987
           ELAPSED_TIME_TOTAL: 32154697
           ELAPSED_TIME_DELTA: 159419
               CPU_TIME_TOTAL: 0
               CPU_TIME_DELTA: 0
                 CCWAIT_TOTAL: 0
                 CCWAIT_DELTA: 0
            ...
            USERIO_WAIT_DELTA: 0
                 APWAIT_TOTAL: 0
                 APWAIT_DELTA: 0
 PHYSICAL_READ_REQUESTS_TOTAL: 16
 PHYSICAL_READ_REQUESTS_DELTA: 0
    PHYSICAL_READ_BYTES_TOTAL: 102537
    PHYSICAL_READ_BYTES_DELTA: 0
         WRITE_THROTTLE_TOTAL: 0
         WRITE_THROTTLE_DELTA: 0
         ROWS_PROCESSED_TOTAL: 178246
         ROWS_PROCESSED_DELTA: 817
     MEMSTORE_READ_ROWS_TOTAL: 178227
     MEMSTORE_READ_ROWS_DELTA: 817
MINOR_SSSTORE_READ_ROWS_TOTAL: 288552
MINOR_SSSTORE_READ_ROWS_DELTA: 1634
MAJOR_SSSTORE_READ_ROWS_TOTAL: 158471
MAJOR_SSSTORE_READ_ROWS_DELTA: 817
                    RPC_TOTAL: 0
                    RPC_DELTA: 0
                FETCHES_TOTAL: 0
                FETCHES_DELTA: 0
             ...
             ROUTE_MISS_TOTAL: 0
             ROUTE_MISS_DELTA: 0
              FIRST_LOAD_TIME: 2025-10-15 11:11:59.282837
         PLAN_CACHE_HIT_TOTAL: 178245
         PLAN_CACHE_HIT_DELTA: 817
             MUTI_QUERY_TOTAL: 0
             MUTI_QUERY_DELTA: 0
...
* [V$OB_SQLSTAT](33600.v-ob_sqlstat-of-sys-tenant.md)

* [oceanbase.CDB_WR_SQLSTAT](../200.dictionary-view-of-sys-tenant/13600.o-cdb_wr_sqlstat-of-sys-tenant.md)

* [oceanbase.DBA_WR_SQLSTAT](../200.dictionary-view-of-sys-tenant/26500.o-dba_wr_sqlstat-of-sys-tenant.md)