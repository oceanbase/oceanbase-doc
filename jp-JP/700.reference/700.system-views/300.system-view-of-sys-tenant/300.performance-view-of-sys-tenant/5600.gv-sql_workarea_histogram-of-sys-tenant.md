...
本テナントの現在のノード上で、過去にすべてのワークエリアで実行された統計情報を表示します。これには、1MB～2MBの範囲でフルインメモリ処理された回数や、ワンパス処理の回数などが含まれます。
...
| OPTIMAL_EXECUTIONS     | NUMBER | NO             | workareaが最低および最高範囲のoptimalモードで実行した回数             |
| ONEPASS_EXECUTIONS     | NUMBER | NO             | workareaが最低および最高範囲のone passモードで実行した回数     |
| MULTIPASSES_EXECUTIONS | NUMBER | NO             | workareaが最低および最高範囲のmulti passesモードで実行した回数 |
