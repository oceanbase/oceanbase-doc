...
|slug|v-ob_kv_client_info-of-sys-tenant|
...
システムテナントにおいて、ビュー `V$OB_KV_CLIENT_INFO` は現在のノード上のすべてのOBKVテナントクライアント接続の構成パラメータ情報を参照できます。
...
`client_info`フィールドは、JSON文字列形式で表されます。
...
oceanbaseでビュー`V$OB_KV_CLIENT_INFO`をクエリするステートメントは次のとおりです：
...
* [GV$OB_KV_CLIENT_INFO](1650.gv-ob_kv_client_info-of-sys-tenant.md)
