...
ビュー `V$OB_SS_OBJECT_TYPE_IO_STAT` は、テナントが現在のノード上で異なる種類のオブジェクトに対して実行した読み取り、書き込み、削除の使用量を表示するために使用されます。
...
システムテナントが、現在のノード上でテナントIDが1002のテナントにおける、さまざまな種類のオブジェクトに対する読み取り、書き込み、削除の使用量を確認します。
