...
クラスタ内の各テナントにおけるすべてのOBServerノード上の待機イベントを表示し、各種待機イベントの発生回数、待機時間、タイムアウト回数などを統計します。
...
| TIME_WAITED | double | NO | この待機イベントの合計待機時間、単位は10ミリ秒 |
| AVERAGE_WAIT | double | NO | この待機イベントの平均待機時間、単位は10ミリ秒 |
| TIME_WAITED_MICRO | bigint(20) | NO | この待機イベントの合計待機時間、単位はマイクロ秒 |

## クエリ例

システムテナントで、すべてのテナントのすべてのノードにおけるすべての待機イベントを確認し、上位10件のレコードを表示します。
...
* テナント内のすべての待機イベントの定義を確認する：[V$EVENT_NAME](30300.v-event_name-of-sys-tenant.md)
