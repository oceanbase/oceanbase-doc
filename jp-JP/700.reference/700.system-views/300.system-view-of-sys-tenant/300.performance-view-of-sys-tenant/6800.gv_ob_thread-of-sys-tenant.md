...
|slug|gv_ob_thread-of-sys-tenant|
...
| STATUS | varchar(32) | NO |  スレッド状態：<ul><li> Runはスレッドが実行中であることを示します </li><li> Waitはスレッドが待機中であることを示します </li><li> Sleepはスレッドがスリープ状態であることを示します </li><li> Joinはスレッドが別のスレッドの終了を待機していることを示します </li></ul>|
| LATCH_WAIT | varchar(16) | NO | Latchを待機中のアドレス |
| LATCH_HOLD | varchar(256) | NO | Latchを保持しているアドレス（複数含まれる場合あり） |
| TRACE_ID | varchar(40) | NO | スレッド上で実行されているTraceのID |
| CGROUP_PATH | varchar(256) | NO  | スレッドが属するcgroupのパス。このフィールドの情報は、`/proc/observer-pid/task/tid/cgroup`パスのCPU行から取得されます<main id="notice" type='explain'><h4>説明</h4><p>このフィールドはV4.2.2バージョンから導入されました。</p></main>  |
