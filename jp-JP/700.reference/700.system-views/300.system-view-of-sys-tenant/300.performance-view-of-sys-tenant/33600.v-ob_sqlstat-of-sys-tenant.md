...
|slug|v-ob_sqlstat-of-sys-tenant|

# V$OB_SQLSTAT

<main id="notice" type='explain'>
<h4>説明</h4>
<p>このビューはV4.2.2バージョンから導入されました。</p>
</main>

## 機能の概要

ビュー `V$OB_SQLSTAT` は、現在のノード上のSQLの基本的なパフォーマンス統計データを表示するために使用されます。各SQL文が1行（つまり、各 `SQL_ID` + `Plan_Hash` の一意な値が1行）となります。そのうち、 `_DELTA` を含む列は、前回のWRスナップショット取得から現在時刻までの間の統計値の増分を示します。
...
| PLAN_TYPE                     | bigint(20)          | NO   |    SQL計画キャッシュの場合、このフィールドは計画のタイプを表します：<ul><li>`1`：Local Plan を表します。</li> <li>`2`：Remote Plan を表します。</li> <li>`3`：Distributed Plan を表します。</li></ul> </br>PLオブジェクトキャッシュの場合、このフィールドはPLオブジェクトのタイプを表します：<ul><li>`1`：Procedure を表します。</li> <li>`2`：Function を表します。</li> <li>`3`：Package を表します。</li> <li>`4`：Anonymous Block を表します。</li></ul> |
| QUERY_SQL                     | longtext            | NO   | 完全なSQLテキスト |
| SQL_TYPE                      | bigint(20)          | NO   | ステートメントのタイプ |
| MODULE                        | varchar(64)         | NO   |   このステートメントが最初に解析された際のアプリケーションModule。`DBMS_APPLICATION_INFO.SET_MODULE`パッケージで設定されます。    |
| ACTION                        | varchar(64)         | NO   |   このステートメントが最初に解析された際のアプリケーションAction。`DBMS_APPLICATION_INFO.SET_ACTION`パッケージで設定されます。      |
...
| EXECUTIONS_TOTAL              | bigint(20)          | NO   |   プランキャッシュ内の該当プランの総実行回数     |
| EXECUTIONS_DELTA              | bigint(20)          | NO   |   プランキャッシュ内の該当プランの総実行回数の増分      |
...
| NESTED_SQL_TOTAL              | bigint(20)          | NO   |   套接SQL実行の合計。サブSQLを1回ずつ実行するたびに1ずつ増加します。     |
| NESTED_SQL_DELTA              | bigint(20)          | NO   |   套接SQL実行の増分    |
...
| ROUTE_MISS_DELTA              | bigint(20)          | NO   |   前回のWRスナップショット収集以降、リクエストパーティションに正しくヒットしなかった総回数   |
| FIRST_LOAD_TIME               | timestamp(6)      | NO   | 実行計画生成時間  |
...
| MUTI_QUERY_TOTAL              | bigint(20)        | NO   | 実行計画生成以降、このSQLがMuti Queryに含まれた回数<main id="notice" type='explain'><h4>説明</h4><p>V4.2.5バージョンでは、このフィールドはV4.2.5 BP7バージョンから導入されました。</p></main>|
...
| FULL_TABLE_SCAN_DELTA         | bigint(20)        | NO   | 前回のWRスナップショット収集以降、このSQLがフルテーブルスキャンを実行した回数<main id="notice" type='explain'><h4>説明</h4><p>V4.2.5バージョンでは、このフィールドはV4.2.5 BP7バージョンから導入されました。</p></main>|
...
ビュー内の `SQL_TYPE` 列の値と対応するステートメントは、以下の表のとおりです。
...
現在のノード上のSQLの基本的なパフォーマンス統計データを確認し、1件目のレコードを表示します。
...
*************************** 1. row ***************************
            ...
            BUFFER_GETS_TOTAL: 0
            BUFFER_GETS_DELTA: 0
           ELAPSED_TIME_TOTAL: 1599
           ELAPSED_TIME_DELTA: 1599
               CPU_TIME_TOTAL: 0
               CPU_TIME_DELTA: 0
                 CCWAIT_TOTAL: 0
                 CCWAIT_DELTA: 0
...
MAJOR_SSSTORE_READ_ROWS_DELTA: 0
             ...
             ROUTE_MISS_TOTAL: 0
             ROUTE_MISS_DELTA: 0
              FIRST_LOAD_TIME: 2025-10-15 11:10:21.815222
...
* [GV$OB_SQLSTAT](3200.gv-ob_sqlstat-of-sys-tenant.md)
