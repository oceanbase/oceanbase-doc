...
|slug|gv-ob_log_transport_dest_stat-of-sys-tenant|
...
ビュー`GV$OB_LOG_TRANSPORT_DEST_STAT`は、すべてのテナントにおけるすべてのOBServerの下流ログ同期リンク(OBCDC/ネットワークスタンバイデータベース)の情報を表示します。
...
すべてのテナントにおけるすべてのOBServerの下流ログ同期リンク（OBCDC/ネットワークスタンバイデータベース）の情報を照会します。
