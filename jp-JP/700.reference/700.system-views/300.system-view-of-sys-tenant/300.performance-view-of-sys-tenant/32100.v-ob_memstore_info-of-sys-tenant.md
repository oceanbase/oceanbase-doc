...
<h4>説明</h4>
<p>ビュー <code>V$MEMSTORE_INFO</code> は V4.0.0 以降、<code>V$OB_MEMSTORE_INFO</code> に名称が変更されました。V4.0.0 以降のバージョンでは、新しいビュー名 <code>V$OB_MEMSTORE_INFO</code> を使用してクエリを実行する必要があります。旧ビュー名 <code>V$MEMSTORE_INFO</code> でクエリを実行すると、エラーが発生します。</p>
</main>

## 機能の概要

現在のサーバー上のすべてのテナントのすべてのパーティションのMemTableの詳細情報を表示します。
...
テナントIDが`1002`のテナントが現在のサーバー上に持つすべてのパーティションのMemTableの詳細情報を照会します。
...
* すべてのノード上のすべてのパーティションのMemTableの詳細情報を表示する：[GV$OB_MEMSTORE_INFO](1700.gv-ob_memstore_info-of-sys-tenant.md)

* 現在のノード上のテナントのMemTableのメモリ使用状況を表示する：[V$OB_MEMSTORE](32000.v-ob_memstore-of-sys-tenant.md)