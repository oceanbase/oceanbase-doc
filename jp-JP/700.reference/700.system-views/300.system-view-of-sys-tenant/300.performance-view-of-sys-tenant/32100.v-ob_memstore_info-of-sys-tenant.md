...
<h4>説明</h4>
<p>ビュー <code>V$MEMSTORE_INFO</code> は V4.0.0 以降、<code>V$OB_MEMSTORE_INFO</code> に名前が変更されました。V4.0.0 以降のバージョンでは、新しいビュー名 <code>V$OB_MEMSTORE_INFO</code> を使用してクエリを実行する必要があります。古いビュー名 <code>V$MEMSTORE_INFO</code> でクエリを実行すると、エラーが発生します。</p>
</main>

## 機能の概要

現在のサーバー上のすべてのテナントのすべてのパーティションの MemTable の詳細情報を表示します。
...
テナントIDが `1002` のテナントの、現在のサーバー上のすべてのパーティションの MemTable の詳細情報を照会します。
