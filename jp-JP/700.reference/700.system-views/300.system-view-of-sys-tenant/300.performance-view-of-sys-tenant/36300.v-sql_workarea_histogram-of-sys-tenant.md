...
過去にすべてのワークエリアで実行された統計情報を表示します。例えば、1MB～2MBの範囲で完全にインメモリ処理された回数や、ワンパス処理の回数などです。
...
システムテナントが、現在のノード上でテナントID `1002` のテナントが過去に実行したすべてのワークエリアの統計情報を確認し、上位10件のレコードを表示します。
