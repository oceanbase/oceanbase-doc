...
各テナントの現在のOBServerノード上の統計イベント情報を表示します。

ここで：

* `STATISTIC#`：統計項目番号を表します。同一の統計項目の番号は、異なるOBServerノードのバージョン間で変更される場合があります。

* `STAT_ID`：統計項目IDを表します。同一の統計項目のIDは、異なるOBServerノードのバージョン間で同一であることが保証されます。
...
システムテナント内で、現在のノード上のすべてのテナントの統計イベント情報を確認し、上位10件のレコードを表示します。
...
* テナント内のすべての統計イベント定義を確認する：[V$STATNAME](36400.v-statname-of-sys-tenant.md)
