...
ビュー `GV$OB_SQLSTAT` は、クラスタ内のSQLの基本的なパフォーマンス統計データを表示するために使用されます。各SQL文は1行として表示されます（つまり、各 `SQL_ID` + `Plan_Hash` の一意の値が1行となります）。そのうち、`_DELTA` を含む列は、前回のWRスナップショットから現在時点までの統計値の増分を示しています。
...
| PLAN_TYPE                     | bigint(20)          | NO   |    SQL計画のキャッシュにおける計画タイプ。<ul><li>`1`：ローカル計画を示します。</li> <li>`2`：リモート計画を示します。</li> <li>`3`：分散計画を示します。</li></ul> </br>PLオブジェクトキャッシュにおいて、このフィールドはPLオブジェクトのタイプを示します。<ul><li>`1`：プロシージャを示します。</li> <li>`2`：関数を示します。</li> <li>`3`：パッケージを示します。</li> <li>`4`：匿名ブロックを示します。</li></ul> |
| QUERY_SQL                     | longtext            | NO   | 全量のSQLテキスト |
| SQL_TYPE                      | bigint(20)          | NO   | ステートメントタイプ |
| MODULE                        | varchar(64)         | YES  |   このステートメントが初めて解析されたときのアプリケーションモジュール    |
| ACTION                        | varchar(64)         | YES  |   このステートメントが初めて解析されたときのアプリケーションアクション      |
...
| CCWAIT_TOTAL                  | bigint(20)          | NO   |   並列待機イベントの合計時間       |
| CCWAIT_DELTA                  | bigint(20)          | NO   |   並列待機イベントの合計時間の増分       |
...
| NESTED_SQL_TOTAL              | bigint(20)          | NO   |   ネストSQLの実行総数。サブSQLを1回実行するごとに1増加     |
...
| FIRST_LOAD_TIME               | timestamp(6)        | YES  |   プラン生成時間<main id="notice" type='explain'><h4>説明</h4><ul><li>V4.3.x系では、このフィールドはV4.3.5バージョンから導入されました。</li> <li>V4.2.x系では、このフィールドはV4.2.5バージョンから導入されました。</li></ul></main>   |
...
| MUTI_QUERY_DELTA              | bigint(20)        | NO   | 前回のWRスナップショット収集以降、このSQLがMuti Query内にあった回数<main id="notice" type='explain'><h4>説明</h4><p>V4.4.x系では、このフィールドはV4.4.2バージョンから導入されました。</p></main>|
...
| MUTI_QUERY_BATCH_DELTA        | bigint(20)        | NO   | 前回のWRスナップショット収集以降、このSQL Batchが実行された回数<main id="notice" type='explain'><h4>説明</h4><p>V4.4.x系では、このフィールドはV4.4.2バージョンから導入されました。</p></main>|
...
| FULL_TABLE_SCAN_DELTA         | bigint(20)        | NO   | 前回のWRスナップショット収集以降、このSQLが全表スキャンを実行した回数<main id="notice" type='explain'><h4>説明</h4><p>V4.4.x系では、このフィールドはV4.4.2バージョンから導入されました。</p></main>|
...
| ERROR_COUNT_DELTA             | bigint(20)        | NO   | 前回のWRスナップショット収集以降、このSQLがエラーを返した回数<main id="notice" type='explain'><h4>説明</h4><p>V4.4.x系では、このフィールドはV4.4.2バージョンから導入されました。</p></main>|
| LATEST_ACTIVE_TIME            | timestamp(6)      | YES  | SQL ID + Plan Hashに対応するプランの最終アクティブ時間<main id="notice" type='explain'><h4>説明</h4><p>V4.4.x系では、このフィールドはV4.4.2バージョンから導入されました。</p></main>     |
...
クラスタ内のSQLの基本的なパフォーマンス統計データを確認し、最初のレコードを表示します。
