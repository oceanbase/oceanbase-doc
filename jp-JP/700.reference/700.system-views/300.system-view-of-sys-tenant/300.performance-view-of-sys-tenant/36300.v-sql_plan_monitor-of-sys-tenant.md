...
ビュー`V$SQL_PLAN_MONITOR`は、すべてのテナントにおける現在のOBServerノードのスロークエリのOperationレベルの統計を表示するために使用されます。各Operationについて1件の統計情報が記録されます。
...
すべてのテナントにおける現在のOBServerノードのスロークエリのOperation層面の統計を照会し、最初のレコードを表示します。
