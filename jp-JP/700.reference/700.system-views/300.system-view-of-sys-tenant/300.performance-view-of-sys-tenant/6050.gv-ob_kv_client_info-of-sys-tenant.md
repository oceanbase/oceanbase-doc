...
システムテナント内で、ビュー`GV$OB_KV_CLIENT_INFO`はすべてのOBKVテナントクライアント接続の設定パラメータ情報にアクセスできます。
...
システムテナントでビュー`GV$OB_KV_CLIENT_INFO`をクエリするステートメントは次のとおりです：

```shell
obclient [oceanbase]> SELECT * FROM oceanbase.GV$OB_KV_CLIENT_INFO LIMIT 1;
```
...
* [V$OB_KV_CLIENT_INFO](31450.v-ob_kv_client_info-of-sys-tenant.md)
