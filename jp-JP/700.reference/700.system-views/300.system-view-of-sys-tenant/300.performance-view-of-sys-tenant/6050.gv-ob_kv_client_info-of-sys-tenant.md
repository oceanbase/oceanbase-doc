...
システムテナントでは、ビュー `GV$OB_KV_CLIENT_INFO` により、すべてのOBKVテナントのクライアント接続に関する構成パラメータ情報にアクセスできます。
...
| runtime.continuous_failure.ceiling           | 100          | 連続運用失敗の上限回数。この回数に達するとテーブルのアドレスがリフレッシュされます。 |
| runtime.max.wait              | 3000          | 単一実行のタイムアウト時間内でのリトライ回数。         |
| runtime.retry.interval        | 1             | 実行時エラー発生時のリトライ時間間隔。                 |
