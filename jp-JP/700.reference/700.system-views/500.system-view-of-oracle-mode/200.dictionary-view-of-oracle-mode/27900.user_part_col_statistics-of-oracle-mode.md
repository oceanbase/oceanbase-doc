...
現在のユーザーが所有するパーティションテーブル内のパーティションに属する列の統計情報とヒストグラム情報を表示します。
...
| LAST_ANALYZED  | DATE          | YES            | 最終分析日                                                          |
| GLOBAL_STATS   | VARCHAR2(3)   | YES            | <ul><li>`YES`：統計情報が直接収集されたことを示す   </li><li> `NO`：統計情報がサブパーティションの統計情報から推定されたか、統計情報が収集されていないことを示す</li></ul>    |
| USER_STATS     | VARCHAR2(3)   | YES            | パーティション統計情報がユーザー設定かどうかを示す                                               |
| NOTES          | VARCHAR2(80)  | YES            | 追加の属性を記録します                                                      |
| AVG_COL_LEN    | NUMBER        | YES            | 列の平均長                                                         |
| HISTOGRAM      | VARCHAR2(15)  | YES            | ヒストグラムタイプ                                                          |

## クエリ例

現在のユーザーが所有するパーティションテーブルのうち、テーブル `T_SUBPART` のパーティションに属する列の統計情報とヒストグラム情報を確認します。
...
* 現在のユーザーがアクセス可能なパーティションテーブルのパーティションに属する列の統計情報を確認する：[ALL_PART_COL_STATISTICS](2500.all_part_col_statistics-of-oracle-mode.md)

* テナント内のすべてのパーティションテーブルのパーティションレベルの列統計情報を確認する：[DBA_PART_COL_STATISTICS](17100.dba_part_col_statistics-of-oracle-mode.md)
