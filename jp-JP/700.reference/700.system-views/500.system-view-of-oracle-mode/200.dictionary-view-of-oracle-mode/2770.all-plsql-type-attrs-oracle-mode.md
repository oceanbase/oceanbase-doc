...
`ALL_PLSQL_TYPE_ATTRS`ビューは、現在のユーザーがアクセスできるパッケージ内で定義されているレコード型の全体的な情報を表示するために使用されます。
...
現在のユーザーがアクセス可能なパッケージに定義されているレコードタイプの全体的な情報を確認します。
...
* 現在のテナント内のすべてのパッケージで定義されているレコード型情報を確認するには、次のビューを参照してください。[DBA_PLSQL_TYPE_ATTRS](16370.dba-plsql-type-attrs-of-oracle-mode.md)

* 現在のユーザーが所有するすべてのパッケージで定義されているレコード型情報を確認するには、次のビューを参照してください。[USER_PLSQL_TYPE_ATTRS](26870.user-plsql-type-attrs-oracle-mode.md)