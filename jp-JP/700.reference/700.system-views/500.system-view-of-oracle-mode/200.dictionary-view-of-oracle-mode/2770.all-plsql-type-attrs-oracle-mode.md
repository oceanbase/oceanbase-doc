...
`ALL_PLSQL_TYPE_ATTRS`ビューは、パッケージで定義されたレコード型の全体的な情報を表示します。
パッケージの型がこのビューに表示されるには、そのパッケージが正常にコンパイルされている必要があります。
...
| ATTR_TYPE_OWNER    | VARCHAR2(128) | NO   | 属性の型の所有者名です。 |
...
以下のコマンドを実行して、すべてのPL/SQLレコード型とテーブル型の属性を照会します。
