...
現在のテナントで利用可能な関数とプロシージャのパラメータを表示します。その列は`ALL_ARGUMENTS`の列と同じです。
...
| COLLATION          | VARCHAR2(18)  | YES            | 文字セットのソートルール         |
...
現在のテナントで利用可能なすべての関数とプロシージャのパラメータを確認し、上位2件のレコードを表示します。
...
* 現在のユーザーが所有するすべての関数とプロシージャのパラメータを確認するには：[USER_ARGUMENTS](24300.user_arguments-of-oracle-mode.md)