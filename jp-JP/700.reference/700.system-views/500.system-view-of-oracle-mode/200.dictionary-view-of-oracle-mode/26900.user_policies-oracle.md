...
現在のユーザーが保有するアクセス制御ポリシーを表示します。特定のテーブルに対してアクセス制御を行う場合、そのテーブル上に複数のポリシーを作成できます。
...
| OBJECT_NAME | VARCHAR2(128) | NO | ポリシーによって制御されるオブジェクトの名前 |
...
| COMMON | VARCHAR2(3) | NO | 互換性のためにのみ使用され、現在の値は固定されています。NOです。 |
| INHERITED | VARCHAR2(3) | NO | 互換性のためにのみ使用され、現在の値は固定されています。NOです。 |
...
* 現在のユーザーがアクセスできるアクセス制御ポリシーを確認するには、次のURLを参照してください。[ALL_POLICIES](2800.all_policies-oracle.md)
