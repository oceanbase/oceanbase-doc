...
ビュー`DBA_OBJECTS`は、データベース内のすべてのデータベースオブジェクトを確認するために使用します。
...
| DATA_OBJECT_ID    | NUMBER        | NO         | そのオブジェクトSegmentを含むディクショナリID値                        |
...
| TEMPORARY         | VARCHAR2(1)   | NO         | そのオブジェクトが一時オブジェクトであることを示す                                 |
...
このテナントのすべてのデータベースオブジェクトを照会し、最初の2件のレコードを表示します。
