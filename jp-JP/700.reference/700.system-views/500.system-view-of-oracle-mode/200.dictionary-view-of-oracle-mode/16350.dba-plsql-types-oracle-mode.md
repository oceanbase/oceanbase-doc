...
`DBA_PLSQL_TYPES` ビューは、テナント内のパッケージタイプの全体的な情報を表示するために使用されます。パッケージが正常にコンパイルされた場合にのみ、そのパッケージ内のタイプがこのビューに表示されます。
...
* 現在のユーザーがアクセス可能なPL/SQLデータ型の情報のみを確認する：[USER_PLSQL_TYPES](26850.user-plsql-type-of-oracle-mode.md)