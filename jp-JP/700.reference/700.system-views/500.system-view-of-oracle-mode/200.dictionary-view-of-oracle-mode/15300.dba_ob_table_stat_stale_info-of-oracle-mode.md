...
| IS_STALE | VARCHAR2(3) | NO | 統計情報が有効期限切れかどうか。<ul><li>`YES` は有効期限が切れていることを示します。</li><li>テーブルの統計情報が収集されている場合、有効期限が切れる条件は `INSERTS + UPDATES + DELETES > LAST_ANALYZED_ROWS * STALE_PERCENT / 100` です。</li><li>テーブルの統計情報が収集されていない場合、有効期限が切れる条件は `INSERTS + UPDATES + DELETES > 0` です。</li></ul></li><li>`NO` は有効期限が切れていないことを示します。</li></ul>|
