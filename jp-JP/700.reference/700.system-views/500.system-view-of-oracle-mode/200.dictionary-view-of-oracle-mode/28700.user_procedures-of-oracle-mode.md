...
現在のユーザーが所有するすべての関数とプロシージャ、およびそれらの関連属性を表示します。列は`ALL_PROCEDURES`と同じですが、`OWNER`フィールドは除外されています。
