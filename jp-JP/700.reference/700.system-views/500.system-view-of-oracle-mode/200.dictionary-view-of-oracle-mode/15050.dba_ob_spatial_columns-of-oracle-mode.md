...
ビュー`DBA_OB_SPATIAL_COLUMNS`は、空間インデックス列に対応するデータベース名、テーブル名、タイプ、およびSRS情報を表示するために使用されます。
...
| TABLE_NAME         | VARCHAR2(256) | NO   | その空間列を含むテーブル名。|
| COLUMN_NAME        | VARCHAR2(128) | NO   | 列名。|
| SRS_NAME           | VARCHAR2(128) | NO   | 空間座標系名。|
| SRS_ID             | NUMBER(10)    | NO   | 空間座標系ID。|
| GEOMETRY_TYPE_NAME | VARCHAR2(128) | NO   | 空間データ型。|
| DIMINFO            | VARCHAR2(512) | NO   | 各緯度の設定です。この列の設定は現在サポートされていないため、デフォルト値が表示されます。|
| INDEX_NAME         | VARCHAR2(128) | NO   | その列に対応するインデックス名です。インデックスがない場合はNULLです。|
