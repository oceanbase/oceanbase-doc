...
| NUM_NULLS     | NUMBER        | YES            | 列の`NULL`値の数                              |
| NUM_BUCKETS   | NUMBER        | YES            | 列のヒストグラムにおけるバケット数                                   |
| LAST_ANALYZED | DATE          | YES            | 最終分析日時                                     |
| SAMPLE_SIZE   | NUMBER        | YES            | 分析時のサンプリングサイズ                                    |
| GLOBAL_STATS  | VARCHAR2(3)   | YES            | <ul><li>`YES`：統計情報が直接収集されたか、増分でメンテナンスされたことを示す</li><li> `NO`：その他の場合 </li></ul>   |
...
現在のユーザーが所有するテーブルのうち、テーブル `T_SUBPART` の列統計情報とヒストグラム情報を確認します。
...
* このテナント内のすべてのテーブルの統計情報を確認する：[DBA_TAB_COL_STATISTICS](20700.dba_tab_col_statistics-of-oracle-mode.md)
