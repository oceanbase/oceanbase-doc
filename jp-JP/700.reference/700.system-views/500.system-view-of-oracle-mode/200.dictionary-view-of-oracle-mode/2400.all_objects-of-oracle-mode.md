...
`ALL_OBJECTS` ビューは、ユーザーがアクセス可能なすべてのデータベースオブジェクトを表示するために使用されます。
...
| SUBOBJECT_NAME | VARCHAR2(128) | NO             | サブオブジェクト名（例：partition）                          |
| OBJECT_ID      | NUMBER    | NO             | オブジェクトのディクショナリID値                                     |
| DATA_OBJECT_ID | NUMBER        | NO             | 当該オブジェクトSegmentを含むセグメントのディクショナリID値                       |
...
| SECONDARY      | VARCHAR2(1)   | NO             | 現在は使用されていない                                         |
