...
| DATA_OBJECT_ID | NUMBER        | NO             | そのオブジェクトのSegmentを含むディクショナリID値                       |
| OBJECT_TYPE    | VARCHAR2(23)  | NO             | オブジェクトタイプ。テーブル、インデックス、パーティション、パッケージ、ストアドプロシージャ、シノニム、シーケンスなどが含まれる                |
| CREATED        | DATE          | NO             | オブジェクト作成日時                                       |
| LAST_DDL_TIME  | DATE          | NO             | オブジェクトの最終DDL実行日時                                |
| TIMESTAMP      | VARCHAR2(19) | NO             | オブジェクトの指定されたタイムスタンプ。文字列として出力されます                             |
| STATUS         | VARCHAR2(7)   | NO             | オブジェクトの状態：<ul><li>VALID：有効</li><li> INVALID：無効</li></ul>   |
| TEMPORARY      | VARCHAR2(1)   | NO             | このオブジェクトが一時オブジェクトであることを示します                                 |
