...
テナント内のパーティションテーブルにおけるサブパーティションの列統計情報とヒストグラム情報を表示します。
...
| NUM_NULLS         | NUMBER        | NO            | 列の`NULL`値の数                          |
| NUM_BUCKETS       | NUMBER        | NO            | 列のヒストグラムのバケット数                               |
| SAMPLE_SIZE       | NUMBER        | NO            | 分析時のサンプリングサイズ                                |
| LAST_ANALYZED     | DATE          | NO            | 最終分析日時                                 |
| GLOBAL_STATS      | VARCHAR2(3)   | NO            | <ul><li>`YES`：統計情報が収集されていることを示す   </li><li> `NO`：統計情報が収集されていないことを示す </li></ul>   |
| USER_STATS        | VARCHAR2(3)   | NO            | パーティション統計情報がユーザー設定かどうかを示す                        |
...
* 現在のユーザーがアクセス可能なパーティションテーブルのサブパーティションの列統計情報を確認する：[ALL_SUBPART_COL_STATISTICS](4100.all_subpart_col_statistics-of-oracle-mode.md)

* 現在のユーザーが所有するパーティションテーブルのサブパーティションの列統計情報を確認する：[USER_SUBPART_COL_STATISTICS](29700.user_subpart_col_statistics-of-oracle-mode.md)
