...
現在のユーザーが所有するパーティションテーブル内のサブパーティションに関する列統計情報とヒストグラム情報を表示します。
...
| NUM_BUCKETS       | NUMBER        | YES            | 列のヒストグラムのバケット数                               |
| SAMPLE_SIZE       | NUMBER        | YES            | 分析時のサンプリングサイズ                                |
| LAST_ANALYZED     | DATE          | YES            | 最終分析日時                                 |
| GLOBAL_STATS      | VARCHAR2(3)   | YES            | <ul><li>`YES`：統計情報が収集されていることを示す   </li><li> `NO`：統計情報が収集されていないことを示す </li></ul>   |
...
現在のユーザーが所有するパーティションテーブルのうち、テーブル `T_SUBPART` のサブパーティションに関する列統計情報を確認します。
...
* このテナント内のすべてのパーティションテーブルのサブパーティションの列統計情報を確認する：[DBA_SUBPART_COL_STATISTICS](20100.dba_subpart_col_statistics-of-oracle-mode.md)
