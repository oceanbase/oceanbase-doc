...
データディクショナリがシステムログストリーム内に配置される位置範囲を記録し、CDCがデータディクショナリを利用する際の利便性を高めます。
...
| REPORT_TIME | TIMESTAMP(6) WITH LOCAL TIME ZONE | NO | データディクショナリ生成完了後、内部テーブルに報告された時間（単位：マイクロ秒） |
| START_LSN | NUMBER(38) | NO | テナントのログストリーム1号に永続化された最初のログのLSN |
| END_LSN | NUMBER(38) | NO | テナントのログストリーム1号に永続化された最後のログのLSN |
