...
ビュー`DBA_OB_OBJECT_BALANCE_WEIGHT`は、現在のユーザーテナント配下で特別な重みが設定されているすべてのパーティションを表示するために使用されます。
...
| SUBPARTITION_ID   | NUMBER       | NO             | サブパーティションID。サブパーティションの重みが設定されていない場合、値は`NULL`となります。現在、サブパーティションの重み設定はサポートされていません     |
...
ユーザーテナントが自身のテナント配下で特別な重みが設定されているパーティションを照会します。
