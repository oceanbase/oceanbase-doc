...
|slug|dba_plsql_coll_types-oracle-compatible-mode|
...
`DBA_PLSQL_COLL_TYPES` ビューは、現在のテナントのパッケージで定義されているコレクションタイプの全体的な情報を表示するために使用されます。パッケージが正常にコンパイルされている場合にのみ、そのパッケージ内のタイプがこのビューに表示されます。
...
以下のコマンドを実行して、現在のテナントのpackageで定義されているcollection typeの全体情報を確認します。
...
* パッケージで定義されたコレクションタイプの全体的な情報を確認する：[ALL_PLSQL_COLL_TYPES](2760.all-plsql-coll-type-of-oracle-mode.md)
* 現在のユーザーパッケージで定義されたコレクションタイプの全体的な情報を確認する：[USER_PLSQL_COLL_TYPES](26850.user-plsql-type-of-oracle-mode.md)。