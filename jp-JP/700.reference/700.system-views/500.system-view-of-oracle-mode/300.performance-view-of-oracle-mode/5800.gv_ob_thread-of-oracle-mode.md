...
| STATUS | VARCHAR2(32) | NO | スレッドの状態：<ul><li> Run：スレッドが実行中であることを示します。  </li><li> Wait：スレッドが待機状態にあることを示します。 </li><li> Sleep：スレッドがスリープ状態にあることを示します。 </li><li> Join：スレッドが別のスレッドの終了を待っていることを示します。 </li></ul>|
| LATCH_WAIT | VARCHAR2(16) | NO | 待機中のLatchのアドレス |
| LATCH_HOLD | VARCHAR2(256) | NO | Latchを保持しているアドレス（複数含まれる場合あり） |
| TRACE_ID | VARCHAR2(40) | NO | スレッド上で実行中のTraceのID |
| CGROUP_PATH | VARCHAR2(256) | NO  | スレッドが属するcgroupのパス。このフィールドの情報は、`/proc/observer-pid/task/tid/cgroup` パスのCPU行から取得します |

## クエリ例

ユーザーテナント内の各OBServerノード上のobserverプロセス内のすべてのスレッドの状態情報を確認し、最初の2件のレコードを表示します。
