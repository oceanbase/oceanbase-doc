...
| SVR_PORT                      | NUMBER          | NO   |    ノードのポート番号     |
| TENANT_ID                     | NUMBER          | NO   |    テナントID |
| SQL_ID                        | VARCHAR2(32)    | NO   |    SQLステートメントID   |
| PLAN_ID                       | NUMBER          | NO   |    実行計画ID   |
| PLAN_HASH                     | NUMBER unsigned | NO   |    実行計画のハッシュ値   |
| PLAN_TYPE                     | NUMBER          | NO   |    SQL計画キャッシュの場合、このフィールドは計画のタイプを表します：<ul><li>`1`：ローカル計画を表します。</li> <li>`2`：リモート計画を表します。</li> <li>`3`：分散計画を表します。</li></ul> </br>PLオブジェクトキャッシュの場合、このフィールドはPLオブジェクトのタイプを表します：<ul><li>`1`：プロシージャを表します。</li> <li>`2`：関数を表します。</li> <li>`3`：パッケージを表します。</li> <li>`4`：匿名ブロックを表します。</li></ul> |
| QUERY_SQL                     | CLOB            | NO   | 完全なSQLテキスト |
| SQL_TYPE                      | NUMBER          | NO   | ステートメントのタイプ |
| MODULE                        | VARCHAR2(64)    | YES  |   このステートメントが最初に解析された際のアプリケーションModule。`DBMS_APPLICATION_INFO.SET_MODULE`パッケージで設定されます。    |
| ACTION                        | VARCHAR2(64)    | YES  |   このステートメントが最初に解析された際のアプリケーションAction。`DBMS_APPLICATION_INFO.SET_ACTION`パッケージで設定されます。      |
| PARSING_DB_ID                 | NUMBER          | NO   |   ステートメント解析時のデータベースID    |
| PARSING_DB_NAME               | VARCHAR2(128)   | NO   |   ステートメント解析時のデータベース名     |
| PARSING_USER_ID               | NUMBER          | NO   |   ステートメント解析時のユーザーID     |
| EXECUTIONS_TOTAL              | NUMBER          | NO   |   プランキャッシュにおける該当プランの総実行回数     |
| EXECUTIONS_DELTA              | NUMBER          | NO   |   プランキャッシュにおける該当プランの総実行回数の増分      |
| DISK_READS_TOTAL              | NUMBER          | NO   |   ディスク読み込みの総回数 |
| DISK_READS_DELTA              | NUMBER          | NO   |   ディスク読み込み回数の増分 |
| BUFFER_GETS_TOTAL             | NUMBER          | NO   |   キャッシュから論理読み取りされた総回数       |
| BUFFER_GETS_DELTA             | NUMBER          | NO   |   キャッシュから論理読み取り回数の増分      |
| ELAPSED_TIME_TOTAL            | NUMBER          | NO   |   このステートメントが消費した総時間。パラレル実行シナリオでは、この値は各実行スレッドの累積値です。          |
| ELAPSED_TIME_DELTA            | NUMBER          | NO   |   このステートメントが消費した時間の増分   |
| CPU_TIME_TOTAL                | NUMBER          | NO   |   総CPU使用時間       |
| CPU_TIME_DELTA                | NUMBER          | NO   |   総CPU使用時間の増分     |
| CCWAIT_TOTAL                  | NUMBER          | NO   |   コンカレンシークラス待機イベントの総使用時間       |
| CCWAIT_DELTA                  | NUMBER          | NO   |   コンカレンシークラス待機イベント総使用時間の増分       |
| USERIO_WAIT_TOTAL             | NUMBER          | NO   |   I/O待機の総使用時間      |
| USERIO_WAIT_DELTA             | NUMBER          | NO   |   I/O待機の総使用時間の増分      |
| APWAIT_TOTAL                  | NUMBER          | NO   |   アプリケーションクラス待機イベントの総使用時間        |
| APWAIT_DELTA                  | NUMBER          | NO   |   アプリケーションクラス待機イベント総使用時間の増分      |
| PHYSICAL_READ_REQUESTS_TOTAL  | NUMBER          | NO   |   物理読み取りリクエストの総回数       |
| PHYSICAL_READ_REQUESTS_DELTA  | NUMBER          | NO   |   物理読み取りリクエスト回数の増分      |
| PHYSICAL_READ_BYTES_TOTAL     | NUMBER          | NO   |   物理読み取りの総バイト数     |
| PHYSICAL_READ_BYTES_DELTA     | NUMBER          | NO   |   物理読み取りバイト数の増分   |
| WRITE_THROTTLE_TOTAL          | NUMBER          | NO   |   MemStoreへの書き込み時に制限された総時間    |
| WRITE_THROTTLE_DELTA          | NUMBER          | NO   |   MemStoreへの書き込み時に制限された総時間の増分    |
| ROWS_PROCESSED_TOTAL          | NUMBER          | NO   |   ステートメントで処理された総行数    |
| ROWS_PROCESSED_DELTA          | NUMBER          | NO   |   ステートメントで処理された総行数の増分     |
| MEMSTORE_READ_ROWS_TOTAL      | NUMBER          | NO   |   MemStoreから読み取られた総行数    |
| MEMSTORE_READ_ROWS_DELTA      | NUMBER          | NO   |   MemStoreから読み取った行数の増分    |
| MINOR_SSSTORE_READ_ROWS_TOTAL | NUMBER          | NO   |   MINOR SSStoreから読み取った総行数   |
| MINOR_SSSTORE_READ_ROWS_DELTA | NUMBER          | NO   |   MINOR SSStoreから読み取った行数の増分  |
| MAJOR_SSSTORE_READ_ROWS_TOTAL | NUMBER          | NO   |   MAJOR SSStoreから読み取った総行数     |
| MAJOR_SSSTORE_READ_ROWS_DELTA | NUMBER          | NO   |   MAJOR SSStoreから読み取った行数の増分   |
| RPC_TOTAL                     | NUMBER          | NO   |   RPCの総回数    |
| RPC_DELTA                     | NUMBER          | NO   |   RPC回数の増分   |
| FETCHES_TOTAL                 | NUMBER          | NO   |   結果セット取得の総回数   |
| FETCHES_DELTA                 | NUMBER          | NO   |   結果セット取得回数の増分     |
| RETRY_TOTAL                   | NUMBER          | NO   |   SQLリトライの総回数   |
| RETRY_DELTA                   | NUMBER          | NO   |   SQLリトライ回数の増分      |
| PARTITION_TOTAL               | NUMBER          | NO   |   SQL実行スキャン対象パーティション数の総量      |
| PARTITION_DELTA               | NUMBER          | NO   |   SQL実行スキャン対象パーティション数の増分    |
| NESTED_SQL_TOTAL              | NUMBER          | NO   |   套接SQL実行の総量。サブSQLが1回ずつ実行されるたびに1ずつ増加します。     |
| NESTED_SQL_DELTA              | NUMBER          | NO   |   套接SQL実行の増分    |
| SOURCE_IP                     | CHAR(46)        | NO   |   リクエスト元のIPアドレス     |
| SOURCE_PORT                   | NUMBER          | NO   |   リクエスト元のポート番号      |
| ROUTE_MISS_TOTAL              | NUMBER          | NO   |   実行計画生成以降、リクエストパーティションに正しくヒットしなかった総回数   |
| ROUTE_MISS_DELTA              | NUMBER          | NO   |   最後のWRスナップショット収集以降、リクエストパーティションに正しくヒットしなかった総回数   |
| FIRST_LOAD_TIME               | TIMESTAMP(6)  | YES   | 実行計画生成時間  |
| PLAN_CACHE_HIT_TOTAL          | NUMBER        | NO   | 実行計画生成以降、Plan Cacheにヒットした総回数  |
| PLAN_CACHE_HIT_DELTA          | NUMBER        | NO   | 最後のWRスナップショットの取得以降、Plan Cacheにヒットした回数 |
| MUTI_QUERY_TOTAL              | NUMBER        | NO   | 実行計画の生成以降、このSQLがMuti Queryに存在した回数<main id="notice" type='explain'><h4>説明</h4><p>V4.2.5バージョンでは、このフィールドはV4.2.5 BP7バージョンから導入されました。</p></main>|
| MUTI_QUERY_DELTA              | NUMBER        | NO   | 最後のWRスナップショットの取得以降、このSQLがMuti Queryに存在した回数<main id="notice" type='explain'><h4>説明</h4><p>V4.2.5バージョンでは、このフィールドはV4.2.5 BP7バージョンから導入されました。</p></main>|
| MUTI_QUERY_BATCH_TOTAL        | NUMBER        | NO   | 実行計画の生成以降、このSQL Batchが実行された回数<main id="notice" type='explain'><h4>説明</h4><p>V4.2.5バージョンでは、このフィールドはV4.2.5 BP7バージョンから導入されました。</p></main>|
| MUTI_QUERY_BATCH_DELTA        | NUMBER        | NO   | 最後のWRスナップショットの取得以降、このSQL Batchが実行された回数<main id="notice" type='explain'><h4>説明</h4><p>V4.2.5バージョンでは、このフィールドはV4.2.5 BP7バージョンから導入されました。</p></main>|
| FULL_TABLE_SCAN_TOTAL         | NUMBER        | NO   | 実行計画の生成以降、このSQLがフルテーブルスキャンを実行した回数<main id="notice" type='explain'><h4>説明</h4><p>V4.2.5バージョンでは、このフィールドはV4.2.5 BP7バージョンから導入されました。</p></main>|
| FULL_TABLE_SCAN_DELTA         | NUMBER        | NO   | 最後のWRスナップショットの取得以降、このSQLがフルテーブルスキャンを実行した回数<main id="notice" type='explain'><h4>説明</h4><p>V4.2.5バージョンでは、このフィールドはV4.2.5 BP7バージョンから導入されました。</p></main>|
| ERROR_COUNT_TOTAL             | NUMBER        | NO   | 実行計画の生成以降、このSQLでエラーが発生した回数<main id="notice" type='explain'><h4>説明</h4><p>V4.2.5バージョンでは、このフィールドはV4.2.5 BP7バージョンから導入されました。</p></main>|
| ERROR_COUNT_DELTA             | NUMBER        | NO   | 最後のWRスナップショットの取得以降、このSQLでエラーが発生した回数<main id="notice" type='explain'><h4>説明</h4><p>V4.2.5バージョンでは、このフィールドはV4.2.5 BP7バージョンから導入されました。</p></main>|

ビュー内の `SQL_TYPE` 列の値と対応するステートメントは以下の表のとおりです。
