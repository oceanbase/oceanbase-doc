...
|slug|v-plan_cache_plan_stat-of-oracle-mode|
...
ビュー`V$OB_PLAN_CACHE_PLAN_STAT`は、現在のテナントが現在のOBServerノード上で計画キャッシュにキャッシュされている各キャッシュオブジェクトの状態を表示します。
  ...
  <main id="notice" type='notice'>
    <h4>注意</h4>
    <p>このビューはSQL計画オブジェクトだけでなく、匿名ブロック、PLパッケージ、PL関数などのPLオブジェクトもキャッシュしており、一部のフィールドは特定のオブジェクトに対してのみ有効です。</p>
  </main>
...
| ELAPSED_TIME          | NUMBER(38)                        | NO             | リクエスト受信から実行終了までに要した時間（単位：マイクロ秒）  |
| CPU_TIME              | NUMBER(38)                        | NO             | すべての実行に消費されたCPU時間（単位：マイクロ秒）      |
...
| TABLE_SCAN            | NUMBER(38)                        | NO             | このクエリが主キー スキャンであるかどうかを示します        |
...
| PS_STMT_ID            | NUMBER(38)                        | NO             | リクエストに対応するPrepare ID：<ul><li>`-1`：SQLステートメントがPSプロトコルを使用していないことを示します   </li><li> `-1`以外の値：SQLステートメントがPSプロトコルを使用しており、そのステートメントに対してPSプロトコルが返す一意の識別子を示します   </li></ul> |
| SESSID                | NUMBER(38)                        | NO             | キャッシュ オブジェクトが存在するセッションID<br>V4.2.x系では：<ul><li> V4.2.5 BP4以前のバージョン：<br>このフィールドは、すべての接続方式（直接接続モード/ODPモード）において、サーバー セッションIDを表します。</li><li> V4.2.5 BP4以降のバージョン：<ul><li>直接接続モードでは：<br>このフィールドはサーバー セッションIDを表します。</li> <li>ODPモードでは：</li><ul><li>ODPの構成パラメータ <code>client_session_id_version = 2</code> の場合、このフィールドはクライアント セッションIDを表します。 </li> <li><code>client_session_id_version = 1</code> の場合、このフィールドはサーバー セッションIDを表します。</li></ul></ul></li></ul>   |
...
| PL_SCHEMA_ID          | NUMBER(38)                        | NO             | 匿名ブロック以外のPLオブジェクトについては、このフィールドはキャッシュオブジェクトのスキーマIDです。匿名ブロックの場合、このフィールドはps_stmt_idと同じです。SQL計画については、このフィールドは意味を持ちません。                              |
