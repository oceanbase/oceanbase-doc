...
|slug|gv-system_event-oracle-compatible-mode|
...
現在のテナントがすべてのOBServerノード上で待機しているイベントを表示し、各種待機イベントの発生回数、待機時間、タイムアウト回数などを統計します。
...
| EVENT_ID | NUMBER | NO | 待機中のイベントのID |
| EVENT | VARCHAR2(64) | NO | 待機中のイベントの説明 |
| WAIT_CLASS_ID | NUMBER | NO | 待機中のイベントが属するクラスのID |
| WAIT_CLASS# | NUMBER | NO | 待機中のイベントが属するクラスのインデックス |
| WAIT_CLASS | VARCHAR2(64) | NO | 待機中のイベントが属するクラス |
| TOTAL_WAITS | NUMBER | NO | この待機イベントの合計待機回数 |
| TOTAL_TIMEOUTS | NUMBER | NO | この待機イベントの合計タイムアウト回数 |
| TIME_WAITED | NUMBER | NO | この待機イベントの合計待機時間、単位は10ms |
| AVERAGE_WAIT | NUMBER | NO | この待機イベントの平均待機時間、単位は10ms |
| TIME_WAITED_MICRO | NUMBER | NO | この待機イベントの合計待機時間、単位はマイクロ秒 |

## クエリ例

ユーザーテナント内で、当該テナントのすべてのノードにおける待機イベントを確認し、上位10件のレコードを表示します。
