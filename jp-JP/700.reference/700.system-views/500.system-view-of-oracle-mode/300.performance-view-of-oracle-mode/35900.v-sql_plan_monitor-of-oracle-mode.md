...
| STATUS                  | VARCHAR2(19)  | NO   | オペレータの実行状態：<ul><li>EXECUTING：実行中</li><li>DONE(ERROR)：実行エラー</li><li>DONE(FIRST N ROWS)：実行成功</li><li>DONE(ALL ROWS)：実行成功</li><li>DONE：実行中断</li></ul> |
| SVR_IP                  | VARCHAR2(46)  | NO   | オペレータが存在するマシンのIPアドレス|
| SVR_PORT                | NUMBER(38)    | NO   | オペレータが存在するマシンのポート番号|
| TRACE_ID                | VARCHAR2(64)  | NO   | オペレータのトレースID|
| FIRST_REFRESH_TIME      | TIMESTAMP(6)  | NO   | オペレータの監視開始時間|
| LAST_REFRESH_TIME       | TIMESTAMP(6)  | NO   | オペレータの監視終了時間|
| FIRST_CHANGE_TIME       | TIMESTAMP(6)  | NO   | オペレータが最初の行のデータを吐き出した時間|
| LAST_CHANGE_TIME        | TIMESTAMP(6)  | NO   | オペレータが最後の行のデータを吐き出した時間|
...
| PLAN_POSITION           | NUMBER        | NO   | オペレータが親ノードの何番目の子供か|
