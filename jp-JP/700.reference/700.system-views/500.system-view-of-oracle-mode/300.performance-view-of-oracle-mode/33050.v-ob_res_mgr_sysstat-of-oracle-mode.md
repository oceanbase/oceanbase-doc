...
ビュー `V$OB_RES_MGR_SYSSTAT` は、テナント内の現在のOBServerにおけるリソースグループの統計情報を表示するために使用されます。
...
| VALUE      | NUMBER       | NO   | 統計値 |
| VALUE_TYPE | VARCHAR2(16) | NO   | 値の種類：累積型、代入型 |
| STAT_ID    | NUMBER       | NO   | 統計項目ID |

## クエリ例

本テナント内の本OBServerにおけるリソースグループの統計項目情報を照会します。
