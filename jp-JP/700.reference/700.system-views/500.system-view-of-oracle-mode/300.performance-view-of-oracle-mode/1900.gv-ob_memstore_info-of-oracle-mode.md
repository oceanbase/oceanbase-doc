...
<h4>説明</h4>
<p>ビュー <code>GV$MEMSTORE_INFO</code> は V4.0.0 以降、<code>GV$OB_MEMSTORE_INFO</code> に名称が変更されました。V4.0.0 以降のバージョンでは、新しいビュー名 <code>GV$OB_MEMSTORE_INFO</code> を使用してクエリを実行する必要があります。旧ビュー名 <code>GV$MEMSTORE_INFO</code> でクエリを実行すると、エラーが発生します。</p>
</main>

## 機能の概要

現在のテナントがすべてのサーバー上のすべてのパーティションにおけるMemTableの詳細情報を表示します。
...
ユーザーテナントが、現在のテナントのすべてのサーバー上にあるすべてのパーティションのMemTableの詳細情報を照会します。
...
* 現在のノード上のすべてのパーティションのMemTableの詳細情報を表示する：[V$OB_MEMSTORE_INFO](32400.v-ob_memstore_info-of-oracle-mode.md)

* すべてのノード上のテナントのMemTableのメモリ使用状況を表示する：[GV$OB_MEMSTORE](1800.gv-ob_memstore-of-oracle-mode.md)