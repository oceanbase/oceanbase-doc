...
現在のテナントがすべてのOBServerノード上で待機しているイベントを表示し、各種待機イベントの発生回数、待機時間、タイムアウト回数などの統計情報を提供します。
...
| TOTAL_WAITS | NUMBER | NO | その待機イベントの総待機回数 |
| TOTAL_TIMEOUTS | NUMBER | NO | その待機イベントの総タイムアウト回数 |
| TIME_WAITED | NUMBER | NO | その待機イベントの総待機時間、単位は10ミリ秒 |
| AVERAGE_WAIT | NUMBER | NO | その待機イベントの平均待機時間、単位は10ミリ秒 |
| TIME_WAITED_MICRO | NUMBER | NO | その待機イベントの総待機時間、単位はマイクロ秒 |

## クエリ例

ユーザーテナント内で、自身のテナントがすべてのノード上で待機しているすべてのイベントを確認し、上位10件のレコードを表示します。
...
* テナント内のすべての待機イベント定義を確認する：[V$EVENT_NAME](30500.v-event_name-of-oracle-mode.md)
