...
現在のテナントが現在のOBServerノードで待機しているイベントの一部を表示し、各種類の待機イベントの発生回数、待機時間、タイムアウト回数などの統計情報を提供します。
...
| EVENT_ID | NUMBER | NO | 待機イベントのID |
| EVENT | VARCHAR2(64) | NO | 待機イベントの説明 |
| WAIT_CLASS_ID | NUMBER | NO | 待機イベントが属するクラスID |
| WAIT_CLASS# | NUMBER | NO | 待機イベントが属するクラスのインデックス |
| WAIT_CLASS | VARCHAR2(64) | NO | 待機イベントが属するクラス |
...
ユーザーテナント内で、現在のノードにおける自身のテナントのすべての待機イベントを確認し、上位10件のレコードを表示します。
