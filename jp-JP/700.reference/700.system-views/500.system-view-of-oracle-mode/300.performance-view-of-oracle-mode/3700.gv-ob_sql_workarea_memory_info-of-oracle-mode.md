...
すべてのOBServerノード上のSQLワークエリアの全体的な情報を表示します。これには、利用可能な最大メモリ、現在使用中のメモリ、現在保持中のメモリなどが含まれます。このビューを通じて、現在のテナント配下でのワークエリアの使用状況、例えばメモリ利用率などを確認できます。
...
| WORKAREA_HOLD_SIZE     | NUMBER(38) | NO             | ワークエリアが現在保持しているメモリサイズ、単位はバイト <main id="notice" type='explain'><h4>説明</h4><p>>具体的な使用量はメモリ管理モジュールから取得できないため、保持値のみを取得できます。</p></main>   |
| MAX_AUTO_WORKAREA_SIZE | NUMBER(38) | NO             | autoモード時の予想最大利用可能メモリサイズ、単位はバイト。最大利用可能メモリは `max_work_area_size - hold + total_mem_used` の公式で計算されると考えることができます。これは、現在のワークエリア状況下で、auto管理が可能な最大メモリサイズを表します |
| MEM_TARGET             | NUMBER(38) | NO             | 現在のワークエリアで利用可能なメモリの目標サイズ、単位はバイト。max_auto_work_areaとの違いは、この値がmax_auto_work_areaの一定の割合であることです                                                          |
...
| WORKAREA_COUNT         | NUMBER(38) | NO             | 登録されているoperator profileの数                           |
...
ユーザーテナント内で、自身のテナントがすべてのOBServerノード上に持つSQLワークエリア情報を照会します。
