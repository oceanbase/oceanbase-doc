...
ビュー`GV$OB_RES_MGR_SYSSTAT`は、このテナント内のすべてのOBServerにおけるリソースグループの統計情報を表示するために使用されます。
...
| NAME       | VARCHAR2(64) | NO   | 統計項目の名前 |
| CLASS      | NUMBER       | NO   | 統計項目のカテゴリ |
| VALUE      | NUMBER       | NO   | 統計項目の値 |
| VALUE_TYPE | VARCHAR2(16) | NO   | 値のタイプ：累積型、代入型 |
| STAT_ID    | NUMBER       | NO   | 統計項目ID |

## クエリ例

このテナントのすべてのOBServerにおけるリソースグループの統計情報を照会します。
