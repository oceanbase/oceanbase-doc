...
| PLAN_TYPE                     | NUMBER          | NO   |    SQL計画キャッシュの場合、このフィールドは計画のタイプを表します：<ul><li>`1`：Local Plan</li> <li>`2`：Remote Plan</li> <li>`3`：Distributed Plan</li></ul> </br>PLオブジェクトキャッシュの場合、このフィールドはPLオブジェクトのタイプを表します：<ul><li>`1`：Procedure</li> <li>`2`：Function</li> <li>`3`：Package</li> <li>`4`：Anonymous Block</li></ul> |
| QUERY_SQL                     | CLOB            | NO   | 全SQLテキスト |
| SQL_TYPE                      | NUMBER          | NO   | ステートメントのタイプ |
| MODULE                        | VARCHAR2(64)    | NO  |   このステートメントが最初に解析された際のアプリケーションModule。`DBMS_APPLICATION_INFO.SET_MODULE`パッケージで設定されます    |
| ACTION                        | VARCHAR2(64)    | NO  |   このステートメントが最初に解析された際のアプリケーションAction。`DBMS_APPLICATION_INFO.SET_ACTION`パッケージで設定されます      |
...
| CCWAIT_TOTAL                  | NUMBER          | NO   |   パラレルクラス待機イベントの総使用時間       |
| CCWAIT_DELTA                  | NUMBER          | NO   |   パラレルクラス待機イベント総使用時間の増分       |
...
| MEMSTORE_READ_ROWS_DELTA      | NUMBER          | NO   |   MemStoreから読み取られた行数の増分    |
| MINOR_SSSTORE_READ_ROWS_TOTAL | NUMBER          | NO   |   MINOR SSStoreから読み取られた総行数   |
| MINOR_SSSTORE_READ_ROWS_DELTA | NUMBER          | NO   |   MINOR SSStoreから読み取られた行数の増分  |
| MAJOR_SSSTORE_READ_ROWS_TOTAL | NUMBER          | NO   |   MAJOR SSStoreから読み取られた総行数     |
| MAJOR_SSSTORE_READ_ROWS_DELTA | NUMBER          | NO   |   MAJOR SSStoreから読み取られた行数の増分   |
| RPC_TOTAL                     | NUMBER          | NO   |   RPCの総回数    |
| RPC_DELTA                     | NUMBER          | NO   |   RPC回数の増分   |
| FETCHES_TOTAL                 | NUMBER          | NO   |   結果セットの取得回数の総数   |
| FETCHES_DELTA                 | NUMBER          | NO   |   結果セット取得回数の増分     |
| RETRY_TOTAL                   | NUMBER          | NO   |   SQLリトライの総回数   |
| RETRY_DELTA                   | NUMBER          | NO   |   SQLリトライ回数の増分      |
| PARTITION_TOTAL               | NUMBER          | NO   |   SQL実行スキャン対象のパーティション数の合計      |
| PARTITION_DELTA               | NUMBER          | NO   |   SQL実行スキャン対象のパーティション数の増分    |
| NESTED_SQL_TOTAL              | NUMBER          | NO   |   嵌套SQL実行の合計。サブSQLが1回ごとに実行されるたびに1ずつ増加します     |
| NESTED_SQL_DELTA              | NUMBER          | NO   |   嵌套SQL実行の増分    |
...
| PLAN_CACHE_HIT_DELTA          | NUMBER        | NO   | 最後のWRスナップショット取得以降、Plan Cacheにヒットした回数 |
| MUTI_QUERY_TOTAL              | NUMBER        | NO   | 実行計画の生成以降、このSQLがMuti Queryに含まれた回数<main id="notice" type='explain'><h4>説明</h4><p>V4.2.5バージョンでは、このフィールドはV4.2.5 BP7バージョンから導入されました。</p></main>|
| MUTI_QUERY_DELTA              | NUMBER        | NO   | 最後のWRスナップショット取得以降、このSQLがMuti Queryに含まれた回数<main id="notice" type='explain'><h4>説明</h4><p>V4.2.5バージョンでは、このフィールドはV4.2.5 BP7バージョンから導入されました。</p></main>|
...
| MUTI_QUERY_BATCH_DELTA        | NUMBER        | NO   | 最後のWRスナップショット取得以降、このSQL Batchが実行された回数<main id="notice" type='explain'><h4>説明</h4><p>V4.2.5バージョンでは、このフィールドはV4.2.5 BP7バージョンから導入されました。</p></main>|
| FULL_TABLE_SCAN_TOTAL         | NUMBER        | NO   | 実行計画の生成以降、このSQLでフルテーブルスキャンが実行された回数<main id="notice" type='explain'><h4>説明</h4><p>V4.2.5バージョンでは、このフィールドはV4.2.5 BP7バージョンから導入されました。</p></main>|
| FULL_TABLE_SCAN_DELTA         | NUMBER        | NO   | 最後のWRスナップショット取得以降、このSQLでフルテーブルスキャンが実行された回数<main id="notice" type='explain'><h4>説明</h4><p>V4.2.5バージョンでは、このフィールドはV4.2.5 BP7バージョンから導入されました。</p></main>|
...
| ERROR_COUNT_DELTA             | NUMBER        | NO   | 最後のWRスナップショット取得以降、このSQLでエラーが発生した回数<main id="notice" type='explain'><h4>説明</h4><p>V4.2.5バージョンでは、このフィールドはV4.2.5 BP7バージョンから導入されました。</p></main>|
