...
現在のテナントにおける、すべてのサーバー上のMemtableのメモリ使用状況を表示します。
...
| ACTIVE_SPAN    | NUMBER(38)   | NO             | アクティブなMemTableが占有するメモリの推定値（過大評価される可能性あり）、単位はバイト |
...
ユーザーテナントが、自身のテナントにおけるすべてのサーバー上のMemTableのメモリ使用状況を照会します。
