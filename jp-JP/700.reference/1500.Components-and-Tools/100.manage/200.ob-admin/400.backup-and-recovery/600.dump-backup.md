...
dump_backup機能はob_adminのサブ機能であり、物理バックアップに使用されるデータバックアップ内容を解析します。本記事では、dump_backupの主な使用方法について説明します。
...
| -s/--storage-info  | いいえ  | 文字列  | なし  | オブジェクトストレージのホストアドレスと認証情報を指定します。認証情報にはアクセスID (access_id) とアクセスキー (access_key) が含まれます。Amazon S3サービスの場合は、リージョン (s3_region) も明示的に指定する必要があります。    |
| -f/--file-path  | いいえ  | 文字列  | なし | データバックアップファイルの相対パスを指定します。<main id="notice" type='notice'><h4>注意</h4><p>このオプションは廃止されています。設定しないでください。</p></main> |
| -o/--offset    | いいえ  | 整数型  | 0  | 読み取りオフセットを設定します。指定されたデータバックアップファイルによって、以下の2つのケースがあります：<ul><li>指定されたデータバックアップがマクロブロックファイルの場合、読み取るマクロブロックのバイトオフセットを表します。</li><li>指定されたデータバックアップがtable_listファイルの場合、読み取るtable_listエントリのオフセットを表します。<code>-o</code>/<code>--offset</code> の設定値がtable_listの総数を超える場合、出力は空文字列になります。</li></ul>    |
| -l/--length  | いいえ  | 整数型  | 0  | 読み取り長さを設定します。指定されたデータバックアップファイルによって、以下の2つのケースがあります：<ul><li>指定されたデータバックアップがマクロブロックファイルの場合、読み取るマクロブロックのバイト数を表します。</li><li>指定されたデータバックアップがtable_listファイルの場合、読み取るtable_listエントリ数を表します。読み取りオフセット (offset) と読み取り長さ (length) の合計がtable_listの総数 (以下の例出力の <code>count</code>) を超える場合、出力範囲は [offset,count) となります。</li></ul>  |
| -c/--check-exist  | いいえ | ブール型 | false  | データバックアップファイルが存在するかどうかをチェックするかどうかを制御します。このオプションを指定すると、コマンドは入力されたデータバックアップファイルが存在するかどうかをチェックし、ファイルが存在する場合は `true` を、存在しない場合は `false` を出力します。 |

## 使用例

### 現在のテナントのフルバックアップ情報を確認する
...
### バックアップセットに含まれるユーザーテーブル名を確認する

1. table_listのレコード総数とbatch_sizeを取得する
...
2. 出力されたcount値に基づいて、`--offset`と`--length`を指定します。
   ...
   上記のコマンドは、フルテーブルリストから2番目のレコード（offset+1）から読み取り、読み取るレコード数を3件（length）に設定します。出力は次のとおりです：
...
このセクションでは、dump_backupコマンドを実行する際に一般的に使用されるデータバックアップファイルについて説明します。ここで、${backup_data_path} は指定されたバックアップパスを表し、実際の構成に応じて置き換える必要があります。詳細については、[バックアップターゲット側の設定](../../../../../600.manage/600.backup-and-recovery/400.data-backup/100.preparation-before-data-backup.md)を参照してください。

| ファイルディレクトリ | 説明 |
|---------|------|
| ${backup_data_path}/backup_set_1_full/ | フルバックアップセット |
| ${backup_data_path}/backup_set_1_full/backup_set_1_full_20230111T193330_20230111T193420.obbak | フルバックアップの開始時刻と終了時刻を示すプレースホルダー |
| ${backup_data_path}/backup_set_1_full/single_backup_set_info.obbak | 現在のバックアップセットのメタ情報 |
| ${backup_data_path}/backup_set_1_full/tenant_backup_set_infos.obbak  | 現在のテナントのフルバックアップセット情報  |
| ${backup_data_path}/backup_set_1_full/infos/table_list       | フルテーブル名ファイル              |
| ${backup_data_path}/backup_set_1_full/infos/table_list/table_list.1702352553000000000.1.obbak  | スナップショットポイント1のテーブル名ファイル 1   |
| ${backup_data_path}/backup_set_1_full/infos/table_list/table_list.1702352553000000000.2.obbak  | スナップショットポイント1のテーブル名ファイル 2   |
...
| ${backup_data_path}/backup_set_1_full/infos/major_data_info_turn_1/ | major turn 1下のテナントレベルバックアップファイル |
...
| ${backup_data_path}/backup_set_1_full/infos/minor_data_info_turn_1/  | minor turn 1下のテナントレベルバックアップファイル |
...
| ${backup_data_path}/backup_set_1_full/infos/tenant_parameter.obbak | 現在のテナントのテナントレベル構成パラメータ情報 |
| ${backup_data_path}/backup_set_1_full/infos/locality_info.obbak | 現在のバックアップセットが属するテナントのLocality情報。現在のテナントのリソース分散情報とレプリカ分散情報が含まれます |
| ${backup_data_path}/backup_set_1_full/infos/meta_info/ls_attr_info.1.obbak | バックアップ時のログストリームリストのスナップショット |
| ${backup_data_path}/backup_set_1_full/infos/meta_info/ls_meta_infos.obbak | すべてのログストリームのmeta集合 |
| ${backup_data_path}/backup_set_1_full/logstream_1/ | ログストリーム1号 |
| ${backup_data_path}/backup_set_1_full/logstream_1/major_data_turn_1_retry_0/ | turn 1、retry 0下のベースラインデータ |
...
| ${backup_data_path}/backup_set_1_full/logstream_1/meta_info_turn_1_retry_0/ | turn 1、retry 0下のログストリームメタ情報ファイル |
| ${backup_data_path}/backup_set_1_full/logstream_1/meta_info_turn_1_retry_0/ls_meta_info.obbak | ログストリームメタ情報 |
| ${backup_data_path}/backup_set_1_full/logstream_1/meta_info_turn_1_retry_0/tablet_info.1.obbak | ログストリームtablet metaリスト |
| ${backup_data_path}/backup_set_1_full/logstream_1/minor_data_turn_1_retry_0/ | turn 1、retry 0下のダンプデータ |
| ${backup_data_path}/backup_set_1_full/logstream_1/sys_data_turn_1_retry_0/ | turn 1、retry 0下のシステムtabletデータ |
| ${backup_data_path}/backup_set_1_full/logstream_1001/ | ログストリーム1001 |
| ${backup_data_path}/backup_sets/ | データバックアップリストの集約ディレクトリで、すべてのデータバックアップセットリストが記録されています |
| ${backup_data_path}/backup_sets/backup_set_1_full_end_success_20230111T193420.obbak | フルバックアップ終了プレースホルダー |
| ${backup_data_path}/backup_sets/backup_set_1_full_start.obbak | フルバックアップ開始プレースホルダー |
| ${backup_data_path}/backup_sets/backup_set_2_inc_start.obbak | 増分バックアップ開始プレースホルダー |
| ${backup_data_path}/backup_sets/backup_set_2_inc_end_success_20230111T194420.obbak | 増分バックアップ終了プレースホルダー |
...
| ${backup_data_path}/cluster_parameter.20241018T140609.obbak  | クラスタレベル構成パラメータ情報 |