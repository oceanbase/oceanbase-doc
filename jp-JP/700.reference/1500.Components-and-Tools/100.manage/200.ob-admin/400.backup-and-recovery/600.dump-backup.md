...
dump_backup機能はob_adminのサブ機能であり、物理バックアップに使用されるデータバックアップ内容を解析することができます。本記事では主にdump_backupのいくつかの使い方について説明します。
...
| -o/--offset    | いいえ  | 整数型  | 0  | 読み取りオフセットを設定します。指定されたデータバックアップファイルによって、以下の2つのケースがあります：<ul><li>指定されたデータバックアップがマクロブロックファイルの場合、読み取るマクロブロックのバイトオフセットを表します。</li><li>指定されたデータバックアップがtable_listファイルの場合、読み取るtable_listエントリのオフセットを表します。<code>-o</code>/<code>--offset</code> の設定値がtable_listの総数を超える場合、出力は空です。</li></ul>    |
| -l/--length  | いいえ  | 整数型  | 0  | 読み取り長さを設定します。指定されたデータバックアップファイルによって、以下の2つのケースがあります：<ul><li>指定されたデータバックアップがマクロブロックファイルの場合、読み取るマクロブロックのバイト数を表します。</li><li>指定されたデータバックアップがtable_listファイルの場合、読み取るtable_listエントリ数を表します。読み取りオフセット (offset) と読み取り長さ (length) の合計がtable_listの総数 (以下の例出力中の <code>count</code>) を超える場合、出力範囲は [offset,count) となります。</li></ul>  |
...
### バックアップセットに含まれるユーザーテーブル名の確認

1. table_listのレコード総数とbatch_sizeを取得します。
...
2. 出力されたcount値に基づいて、`--offset`と`--length`を指定します。
   ...
   上記のコマンドは、フルのtable_listから2番目のレコード（offset+1）から読み始め、読み取るレコード数を3件（length）に設定します。出力は次のとおりです。
...
このセクションでは、dump_backupコマンドを実行する際に一般的に使用されるデータバックアップファイルについて説明します。ここで、${backup_data_path} は指定されたバックアップパスを表し、実際の設定に応じて置き換える必要があります。詳細については、[バックアップ宛先の設定](../../../../../600.manage/600.backup-and-recovery/400.data-backup/100.preparation-before-data-backup.md)を参照してください。
...
| ${backup_data_path}/backup_set_1_full/infos/locality_info.obbak | 現在のバックアップセットが属するテナントのLocality情報。現在のテナントのリソース分布情報とレプリカ分布情報を含む |
| ${backup_data_path}/backup_set_1_full/infos/meta_info/ls_attr_info.1.obbak | バックアップ時のログストリームリストのスナップショット |
| ${backup_data_path}/backup_set_1_full/infos/meta_info/ls_metaInfos.obbak | 全ログストリームのmeta集合 |
| ${backup_data_path}/backup_set_1_full/logstream_1/ | ログストリーム1 |
| ${backup_data_path}/backup_set_1_full/logstream_1/major_data_turn_1_retry_0/ | turn 1、retry 0のベースラインデータ |
...
| ${backup_data_path}/backup_set_1_full/logstream_1/meta_info_turn_1_retry_0/ | turn 1、retry 0のログストリームメタ情報ファイル |
...
| ${backup_data_path}/backup_set_1_full/logstream_1/minor_data_turn_1_retry_0/ | turn 1、retry 0のダンプデータ |
| ${backup_data_path}/backup_set_1_full/logstream_1/sys_data_turn_1_retry_0/ | turn 1、retry 0のシステムtabletデータ |
