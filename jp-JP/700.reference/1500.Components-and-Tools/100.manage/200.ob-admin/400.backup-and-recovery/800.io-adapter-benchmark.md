|description||
|---|---|
|slug|io_adapter_benchmark|

# io_adapter_benchmark

io_adapter_benchmark コマンドは、OBServerノードからバックアップ媒体への読み書き性能を検証するために使用されます。本記事では主に、io_adapter_benchmark コマンドの使用方法について説明します。

<main id="notice" type='explain'>
  <h4>説明</h4>
  <p>ob_adminコマンドを実行する前に、<code>export OB_ADMIN_LOG_DIR=&lt;your_log_path&gt;</code> を実行して、ob_adminの実行ログ保存ディレクトリを指定できます。<code>your_log_path</code> は実際の状況に応じて置き換えてください。また、対応するディレクトリが存在していることを確認してください。</p>
</main>

## コマンドの紹介

io_adapter_benchmarkコマンドでは、`-h`オプションを使用してコマンドの使い方を出力できます。
...
| オプション  | 説明   |
|------|--------|
| -d/--file-path-prefix  | テストディレクトリを指定します。ファイルプレフィックス付きの絶対パスを指定する必要があります。 |
| -s/--storage-info  | オブジェクトストレージの検証情報を指定します。  |
| -p/--type  | テストタイプを指定します。`auto-run`、`write`、`append`、`multi`、`read`、`del`、`is_exist`、`read_user_provided`が有効な値です。各値の詳細については、後述の値の説明を参照してください。  |
| -t/--thread-num  | 并列スレッド数を設定します。 |
| -r/--max-task-run-times  | 単一スレッドのタスク実行回数を設定します。デフォルト値は10です。  |
| -l/--time-limit  | 総実行時間の制限を設定します。  |
| -o/--object-size  | オブジェクトサイズを設定します。デフォルトは2097152バイト、つまり2MBです。 |
| -n/--object-num  | オブジェクト数を設定します。デフォルトは10です。  |
| -f/--fragment-size  | read操作において、このオプションは1回の読み取りデータサイズを指定します。append/multipart write操作において、このオプションは1回の書き込みデータサイズを指定します。  |
| --write-size  | `-p`/`--type`が`auto-run`に指定された場合に使用され、putタスク内のオブジェクトサイズを指定します。デフォルトは2097152バイト、つまり2MBです。 |
| --append-size  | `-p`/`--type`が`auto-run`に指定された場合に使用され、追記書き込みタスク内のオブジェクトサイズを指定します。デフォルトは67108864バイト、つまり64MBです。 |
| --append-fragment-size  | `-p`/`--type`が`auto-run`に指定された場合に使用され、追記書き込みタスク内のフラグメントサイズを指定します。デフォルト値は`0`で、毎回ランダムを意味します。  |
| --multi-size  | `-p`/`--type`が`auto-run`に指定された場合に使用され、multipart書き込みファイルのオブジェクトサイズを指定します。デフォルトは67108864バイト、つまり64MBです。 |
| --multi-fragment-size  | `-p`/`--type`が`auto-run`に指定された場合に使用され、追記書き込みタスク内のフラグメントサイズを指定します。デフォルト値は`0`で、毎回ランダムを意味します。  |
| -j/--is-adaptive  | アダプティブインターフェースの使用を設定します。値を指定する必要はありません。コマンド実行時にこのオプションを指定すると、アダプティブインターフェースが使用されます。  |
| -c/--clean-after-execution  | タスク完了後にディレクトリをクリーンアップするかどうかを設定します。値を指定する必要はありません。コマンド実行時にこのオプションを指定すると、タスク完了後にディレクトリがクリーンアップされます。 |
| -e/--s3_url_encode_type  | S3プロトコルURLエンコードタイプを設定します。 |
| -i/--sts_credential  | STS（Security Token Service、一時的なセキュリティトークンサービス）資格情報を設定します。  |
| -a  | obdalの有効化を設定します。obdalはopendalをベースに開発されたオブジェクトストレージ統一フレームワークです。値を指定する必要はありません。コマンド実行時にこのオプションを指定すると、obdalが有効になります。  |

`-p`/`--type`オプションでサポートされる値の説明は以下のとおりです：

| 値  | 意味   |
|-------|--------|
| `auto-run` | ウィンクルワン実行モードで、以下のテストを順次実行します：<ul><li>通常のファイル書き込み。<code>--write-size</code>でファイルサイズを指定します。デフォルトは2MBです。</li><li>通常ファイルの読み取り。1回のデータ読み取りサイズは16KBです。</li><li>通常ファイルの読み取り。1回のデータ読み取りサイズは2MBです。</li><li>ファイルの追記書き込み。<code>--append-size</code>でサイズを指定します。デフォルトは64MBです。毎回<code>--append-fragment-size</code>サイズのファイルを追記します。<code>--append-fragment-size</code>が指定されていない場合、毎回ランダムサイズのフラグメントを追記します。</li><li>追記書き込みファイルの読み取り。1回のデータ書き込みサイズは2MBです。</li><li>multipartファイルの書き込み。<code>--multi-size</code>でサイズを指定します。デフォルトは64MBです。毎回<code>--multi-fragment-size</code>サイズのファイルを追記します。デフォルトは2MBです。</li><li>multipart書き込みファイルの読み取り。1回のデータ読み取りサイズは2MBです。</li></ul>  |
| `write` | 通常ファイルの書き込みを表します。各スレッドは個別のディレクトリに書き込みます。  |
| `append`  | ファイルの追記書き込みを表します。  |
| `multi`  | フラグメンツアップロードを表します。  |
| `read`  | ファイルの読み取りを表します。readタスクを実行する前に、データを書き込む必要があります。  |
| `del`  | データの削除を表します。各スレッドは個別のディレクトリ内のデータを削除します。設定したスレッド数は、書き込み時に設定したスレッド数と同じである必要があります。  |
| `is_exist` | 指定パスのメタ情報を並列読み取りすることを表します。  |
| `read_user_provided`  | 指定ファイルを並列読み取りすることを表します。  |
...
ここでは簡単な例を示しています。`-t` オプションで並列度を、`-r` オプションで各スレッドの実行回数を、`-n` オプションで読み取るファイル数を指定できます（`-r` オプションの値以下に設定する必要があります）。これらのオプションを設定すると、ウィンクル実行内のすべてのテストに適用されます。
...
./ob_admin io_adapter_benchmark -d 'oss://oss_bucket/test_path' \
                              ...
                              -t 4   \  # 4スレッド並列
                              -r 10  \  # 単一スレッドの最大書き込みオブジェクト数は10個
                              -p 'write' \
                              -c  \
```

上記のコマンドは、`oss_bucket/test_path` ディレクトリで通常ファイルへの書き込み性能テストを実行します。4つのスレッド（`-t 4` で指定）が並列してこのディレクトリにオブジェクトを書き込みます。各オブジェクトのサイズは100バイト（`-o 100` で指定）です。各スレッドが10個のオブジェクト（`-r 10` で指定）を書き込んだ後、書き込みを停止します。テスト終了後、ディレクトリを削除します（`-c` で指定）。
...
./ob_admin io_adapter_benchmark -d 'oss://home/admin/backup_info' \
                              ...
                              -r 10  \  # 単一スレッドの最大書き込みオブジェクト数は10個
                              -p 'append' \
                              -f 10  \ # 1つのオブジェクトを書き込む際、毎回10バイトを追加する。つまり、10回の追加で1つのオブジェクト（この例では100バイト）の書き込みが完了する
```
...
./ob_admin io_adapter_benchmark -d 'oss://home/admin/backup_info' \
                              ...
                              -o 100 \  # オブジェクトサイズを100バイトに設定
                              -t 4   \  # 4スレッド並列
                              -r 10  \  # 単一スレッドの書き込み可能なオブジェクト数の上限は10個
                              -p 'multi' \
                              -f 10  \  # 1回の書き込みサイズは10バイトで、これは1つのオブジェクト（この例では100バイト）を10回の書き込み操作で構成することを意味します
```

現在のオブジェクトストレージのシャーディングアップロード実装では、各partが8MBに集約された後に初めて1つのシャードとして実際に書き込まれます。つまり、書き込み回数はpart数と等しくありません。
...
ob_admin io_adapter_benchmark -d 'oss://home/admin/backup_info' \
                              ...
                              -o 100  \  # オブジェクトサイズを100バイトに設定
                              -t 4    \  # 4スレッド並列
                              -r 10   \  # 単一スレッドが実行するread操作の上限は10回です
                              -p 'read' \
                              -f 10   \  # 1回のread操作で読み取るデータサイズ。これはランダムに10バイトのデータを読み取ることを意味します。
                                      \   # 完全なオブジェクトを読み取る必要がある場合は、-f'obj_size'を設定できます。
                              -j 1    \  # バックアップディレクトリがS3/OBS形式の場合は、-j'1'を設定する必要があります。
                              -n 10  \  # データ書き込み時の単一スレッドのオブジェクト数は10です。
```

上記のコマンドを実行する際には、読み取りタスクを実行する前にデータを書き込む必要があることに注意してください。`-n`オプションを使用して、各スレッドが以前に書き込んだオブジェクト数（書き込みタスクコマンドの`-r`に対応する値）を指定する必要があります。並列数（`-t`）は、データ書き込み時に指定した並列数を超えてはなりません。

<main id="notice" type='explain'>
  <h4>説明</h4>
  <p>複数の書き込みコマンドを実行した場合、読み取りコマンドの<code>-d</code>で指定された対応するディレクトリ内の最新の書き込みコマンドを基準とします。つまり、<code>-n</code>と<code>-r</code>は、対応するコマンドを参照して設定する必要があります。</p>
</main>
...
ob_admin io_adapter_benchmark -d 'oss://home/admin/backup_info' \
                              ...
                              -r 10    \ # 書き込み時に指定した書き込み数と同じである必要があります。
```

上記コマンドを実行する際には、各スレッドが個別のディレクトリ内のデータを削除するため、設定するスレッド数（`-t`）は書き込み時に設定した並列数と同じである必要がある点に注意してください。

<main id="notice" type='explain'>
  <h4>説明</h4>
  <p>複数の書き込みコマンドが実行されている場合、読み取りコマンドの<code>-d</code>で指定された対応するディレクトリ内の最新の書き込みコマンドが優先されます。つまり、<code>-t</code>と<code>-r</code>は、対応するコマンドを参照して設定する必要があります。</p>
</main>
...
ob_admin io_adapter_benchmark -d 'oss://home/admin/backup_info' \
                              ...
                              -r 100   \  # シングルスレッドで100回読み取り
                              -o 100   \  # ファイルサイズのヒント。実際のファイルサイズ以下である必要があります。
                              -f 50    \  # 1回の読み取り操作サイズ。-oパラメータで指定されたサイズ以下である必要があります。
```
