...
io_adapter_benchmarkコマンドは、OBServerノードからバックアップメディアへの読み書き性能を検証するために使用されます。本記事では、主にio_adapter_benchmarkコマンドの使用方法について説明します。

<main id="notice" type='explain'>
  <h4>説明</h4>
  <p>ob_adminコマンドを実行する前に、<code>export OB_ADMIN_LOG_DIR=&lt;your_log_path&gt;</code> を実行して、ob_adminの実行ログ保存ディレクトリを指定できます。<code>your_log_path</code> は実際の状況に応じて置き換えてください。また、対応するディレクトリが存在することを確認してください。</p>
</main>
...
| -p/--type  | テストタイプを指定します。取り得る値は `auto-run`、`write`、`append`、`multi`、`read`、`del`、`is_exist`、`read_user_provided` です。各値の詳細については、後述の値の説明を参照してください。  |
| -t/--thread-num  | 同時実行スレッド数を設定します。 |
| -r/--max-task-run-times  | 単一スレッドのタスク実行回数を設定します。デフォルト値は 10 です。  |
| -l/--time-limit  | 総実行時間の制限を設定します。  |
| -o/--object-size  | オブジェクトサイズを設定します。デフォルトは 2097152 バイト、すなわち 2 MB です。 |
| -n/--object-num  | オブジェクト数を設定します。デフォルトは 10 です。  |
| -f/--fragment-size  | read 操作では、このオプションは1回の読み取りデータサイズを指定します。append/multipart write 操作では、このオプションは1回の書き込みデータサイズを指定します。  |
| --write-size  | `-p`/`--type` が `auto-run` に指定された場合に使用され、putタスク内のオブジェクトサイズを指定します。デフォルトは 2097152 バイト、すなわち 2 MB です。 |
| --append-size  | `-p`/`--type` が `auto-run` に指定された場合に使用され、追記書き込みタスク内のオブジェクトサイズを指定します。デフォルトは 67108864 バイト、すなわち 64 MB です。 |
| --append-fragment-size  | `-p`/`--type` が `auto-run` に指定された場合に使用され、追記書き込みタスク内のフラグメントサイズを指定します。デフォルト値は `0` で、毎回ランダムを意味します。  |
| --multi-size  | `-p`/`--type` が `auto-run` に指定された場合に使用され、multipart 書き込みファイルのオブジェクトサイズを指定します。デフォルトは 67108864 バイト、すなわち 64 MB です。 |
| --multi-fragment-size  | `-p`/`--type` が `auto-run` に指定された場合に使用され、追記書き込みタスク内のフラグメントサイズを指定します。デフォルト値は `0` で、毎回ランダムを意味します。  |
| -j/--is-adaptive  | アダプティブインターフェースの使用を設定します。値を指定する必要はありません。コマンド実行時にこのオプションを指定すると、アダプティブインターフェースを使用します。  |
| -c/--clean-after-execution  | タスク完了後にディレクトリをクリーンアップするかどうかを設定します。値を指定する必要はありません。コマンド実行時にこのオプションを指定すると、タスク完了後にディレクトリをクリーンアップします。 |
| -e/--s3_url_encode_type  | S3プロトコルURLエンコードタイプを設定します。 |
| -i/--sts_credential  | STS（Security Token Service、一時的なセキュリティトークンサービス）資格情報を設定します。  |
| -a  | obdalを有効にするかどうかを設定します。obdalはopendalに基づいて開発されたオブジェクトストレージ統一フレームワークです。値を指定する必要はありません。コマンド実行時にこのオプションを指定すると、obdalを有効にします。  |

`-p`/`--type`オプションがサポートする値の説明は以下のとおりです：

| 値  | 意味   |
|-------|--------|
| `auto-run` | ウィンドウショット実行モードです。以下のテストを順に実行します。<ul><li>通常書き込みファイル、ファイルサイズは<code>--write-size</code>で指定します。デフォルトは2MBです。</li><li>通常ファイルの読み取り、1回のデータ読み取りサイズは16KBです。</li><li>通常ファイルの読み取り、1回のデータ読み取りサイズは2MBです。</li><li>ファイルの追記、サイズは<code>--append-size</code>で指定します。デフォルトは64MBです。<code>--append-fragment-size</code>サイズのファイルを毎回追記します。指定されていない場合は、毎回ランダムなサイズの断片を追記します。</li><li>追記ファイルの読み取りと書き込み、1回のデータ書き込みサイズは2MBです。</li><li>multipartファイルの書き込み、サイズは<code>--multi-size</code>で指定します。デフォルトは64MBです。<code>--multi-fragment-size</code>サイズのファイルを毎回追記します。デフォルトは2MBです。</li><li>multipart形式で書き込まれたファイルの読み取り、1回のデータ読み取りサイズは2MBです。</li></ul>  |
| `write` | 通常ファイルの書き込みを表します。各スレッドは個別のディレクトリに書き込みます。  |
| `append`  | ファイルの追記を表します。  |
| `multi`  | ファイルの断片化アップロードを表します。  |
| `read`  | ファイルの読み取りを表します。readタスクを実行する前に、データを書き込む必要があります。  |
| `del`  | データの削除を表します。各スレッドは個別のディレクトリ内のデータを削除します。設定するスレッド数は、書き込み時に設定したスレッド数と同じである必要があります。  |
| `is_exist` | 指定されたパスのメタ情報を並列して読み取ることを表します。  |
| `read_user_provided`  | 指定されたファイルを並列して読み取ることを表します。  |
...
ここでは簡単な例を示していますが、`-t` オプションで並列度を、`-r` オプションで各スレッドの実行回数を、`-n` オプションで読み取るファイルの数を指定できます（この数は `-r` オプションの値以下である必要があります）。これらのオプションを設定すると、ワンクリック実行内のすべてのテストに適用されます。
...
./ob_admin io_adapter_benchmark -d 'oss://oss_bucket/test_path' \
                              ...
                              -t 4   \  # 4スレッド並列
                              -r 10  \  # 単一スレッドあたりの最大オブジェクト数は10個
                              -p 'write' \
                              -c  \
```

上記のコマンドは、`oss_bucket/test_path`ディレクトリで通常ファイルの書き込み性能テストを実行します。4つのスレッド（`-t 4`で指定）が同時にこのディレクトリにオブジェクトを書き込みます。各オブジェクトのサイズは100バイト（`-o 100`で指定）で、各スレッドが10個のオブジェクトを書き込んだ後（`-r 10`で指定）、書き込みを停止します。テスト終了後、ディレクトリは削除されます（`-c`で指定）。
...
./ob_admin io_adapter_benchmark -d 'oss://home/admin/backup_info' \
                              ...
                              -r 10  \  # 単一スレッドあたりの最大オブジェクト数は10個
                              -p 'append' \
                              -f 10  \ # 1つのオブジェクトを書き込む際、毎回10バイトをappendする。つまり、10回のappendで1つのオブジェクト（この例では100バイト）の書き込みが完了する
```
...
./ob_admin io_adapter_benchmark -d 'oss://home/admin/backup_info' \
                              ...
                              -p 'multi' \
                              -f 10  \  # 1回の書き込みサイズは10バイト、つまり1つのオブジェクト（この例では100バイト）は10回の書き込み操作で構成される
```

現在のオブジェクトストレージにおけるシャーディングアップロードの実装では、各partが8MBに集約された後に初めて1つのシャードとして実際に書き込まれます。したがって、書き込み回数はpart数と等しくありません。
...
ob_admin io_adapter_benchmark -d 'oss://home/admin/backup_info' \
                              ...
                              -o 100  \  # オブジェクトサイズを100バイトに設定
                              -t 4    \  # 4スレッド並列
                              -r 10   \  # 単一スレッドのread操作の上限は10回
                              -p 'read' \
                              -f 10   \  # 単一read操作で読み取るデータサイズ。ランダムに10バイトのデータを読むことを意味する
                                      \   # 完全なオブジェクトを読み取る必要がある場合、-f'obj_size'を設定できます。
                              -j 1    \  # バックアップディレクトリがS3/OBS形式の場合、-j'1'を設定する必要があります。
                              -n 10  \  # データ書き込み時、単一スレッドが書き込むオブジェクト数は10です。
```

上記コマンドを実行する際には、読み取りタスクを実行する前にデータを書き込む必要があることに注意してください。`-n`オプションを使用して、各スレッドが以前に書き込んだオブジェクト数（書き込みタスクコマンドの`-r`に対応する値）を指定します。並列数（`-t`）は、データ書き込み時に指定した並列数を超えてはなりません。

<main id="notice" type='explain'>
  <h4>説明</h4>
  <p>複数の書き込みコマンドが実行されている場合、読み取りコマンドの<code>-d</code>で指定されたディレクトリ内の最新の書き込みコマンドを基準とします。つまり、<code>-n</code>と<code>-r</code>は、対応するコマンドを参照して設定する必要があります。</p>
</main>
...
上記のコマンドを実行する際には、各スレッドが個別のディレクトリ内のデータを削除するため、設定するスレッド数（`-t`）は書き込み時に設定した並列数と一致させる必要がある点に注意してください。

<main id="notice" type='explain'>
  <h4>説明</h4>
  <p>複数の書き込みコマンドを実行している場合、読み取りコマンドの<code>-d</code>で指定された対応するディレクトリ内の最新の書き込みコマンドが優先されます。つまり、<code>-t</code>と<code>-r</code>は対応するコマンドに従って設定する必要があります。</p>
</main>
...
ob_admin io_adapter_benchmark -d 'oss://home/admin/backup_info' \
                              ...
                              -r 100   \  # 単一スレッドで100回読み取る
                              -o 100   \  # プロンプトファイルサイズ、実際のファイルサイズ以下である必要があります
