|description||
|---|---|
|slug|api-hybrid-encryption|

# APIの混合暗号化

obshellがHTTPリクエストを受信すると、そのリクエストに対してセキュリティ検証を実行します。obshellの検証内容は、現在のノードのアイデンティティに応じて異なり、パスワード、URI、リクエストボディ（Request Body）の検証を含むがこれらに限定されません。

## 使用方法

### リクエストの送信

obshellにHTTPリクエストを送信する必要がある場合は、以下の手順で操作を行います。

1. AES暗号アルゴリズムを使用して、リクエストボディ（Request Body）の内容を暗号化します。

   1. 16ビットのランダムなAESキー（key）と初期化ベクトル（Initialization Vector、IV）を生成します。

   2. シリアライズされたリクエストボディの内容をCBC暗号モードで暗号化します。

2. 安全性検証に使用されるデータ構造HttpHeaderを構築します。構造体は以下のとおりです。

   ```Go
   type HttpHeader struct {
       Auth         string
       Ts           string  // Authの有効タイムスタンプ
       Uri          string  // リクエストのURI
       Keys         []byte  // HTTPリクエストのBodyを暗号化する際に使用されるAES暗号アルゴリズムのkeyとIV
   }
   ```

   ここで、代入されるパスワードタイプ（`Auth`）によって、HTTPリクエストの検証通過状況も異なります。`Auth`がobshellノードのパスワードに設定されている場合、すべてのコンポーネント関連のHTTPリクエスト検証を通過できます。一方、`Auth`がroot@sysユーザーのパスワードに設定されている場合、OBProxyを除く他のコンポーネントのHTTPリクエスト検証（OceanBaseデータベースやAgentのデプロイ、クラスタ運用保守関連リクエストなど）のみを通過できます。obshellノードのパスワードを設定する `/api/v1/agent/password` インターフェースのリクエスト検証方法も特殊であり、詳細については [ノードパスワードの設定](../400.obshell-api-reference/100.obshell-management/1500.set-node-password.md) を参照してください。

3. RSAアルゴリズムを使用してHttpHeaderを暗号化し、HTTPリクエストヘッダー（Request Headers）に設定します。

   1. `/api/v1/secret` インターフェースを呼び出して、対象ノードの公開鍵を取得します（このAPIは安全性検証が不要です）。

   2. 公開鍵を組み合わせ、RSAアルゴリズムを使用してシリアライズされたHttpHeader文字列を暗号化し、暗号化後にBase64エンコードします。その後、UTF-8エンコードの文字列に復号して、暗号化後の文字列Sを得ます。

   3. HTTPリクエストヘッダーを設定します。

      * `Auth`がobshellノードのパスワードに設定されている場合、リクエストヘッダーを次のように設定します：{"X-OCS-Agent-Header": S}
      * `Auth`がroot@sysのパスワードに設定されている場合、リクエストヘッダーを次のように設定します：{"X-OCS-Header": S}

### obshellの検証プロセス

obshell Serverは、HTTPリクエストを受信すると、以下のプロセスを順次実行して安全性検証を行います。

1. リクエストヘッダーからキー値ペア `"X-OCS-Header": S` または `"X-OCS-Agent-Header": S` を取得します。

2. 自身の秘密鍵を組み合わせて、Sを復号し、復号後のHttpHeader構造体を取得します。

3. HttpHeaderに記録されたAuthが正しいかどうかを確認します。`Auth` がリクエストAPIが属するコンポーネントと一致しない場合、該当するエラーメッセージを直接返します。

4. HttpHeaderに記録されたUriが実際にリクエストされたUriと一致するかどうかを確認します。

5. HttpHeaderに記録されたKeysを組み合わせて、リクエストボディの内容を復号し、リクエストボディの平文を取得します。復号に失敗した場合、検証に失敗します。

上記の安全性検証手順をすべて通過した場合にのみ、obshell Serverはリクエストの処理を続行します。そうでない場合、リクエスト元に安全性検証失敗の応答を返します。

## Pythonによるリクエスト送信コード

以下はPythonを使用してリクエストを送信するコード例です。コード内で定義されている `ip`、`port`、`pwd`、`uri`、`body` を変更するだけで済みます（意味はコードのコメントを参照）。変更後、このコードをコンパイルして実行すると、対応するHTTPリクエストヘッダーとHTTPリクエストボディが得られます。
...
以下はGoを使用してリクエストを送信するコード例です。コード内で定義されている`ip`、`port`、`pwd`、`uri`、`body`を変更するだけで済みます（意味はコードのコメントを参照）。変更後、このコードをコンパイルして実行すると、対応するHTTPリクエストヘッダーとHTTPリクエストボディが得られます。
...
以下はJavaを使用してリクエストを送信するコード例です。コード内で定義されている`agent`、`pwd`、`uri`、`body`を変更するだけで済みます（意味はコードのコメントを参照）。変更後にこのコードをコンパイルして実行すると、対応するHTTPリクエストヘッダーとHTTPリクエストボディが得られます。
...
public class Main {
    ...
    // X-OCS-Agent-Header
    // private static final String agentPwd = "******"; // obshellノードパスワード
    private static final String uri = "/api/v1/obcluster/config"; // リクエストのURI
    private static final String agent = "10.10.10.1:2886";  // リクエストノードのIPとポート
    ...
    public static void main(String[] args) throws Exception {
        ...
        byte[] iv = new byte[16]; // イニシャライズベクトルを16バイトに初期化する
        SecureRandom random = new SecureRandom();
        random.nextBytes(iv);

        // RSAでデータをセグメントごとに暗号化する
        Cipher rsaCipher = Cipher.getInstance(RSA_TRANSFORMATION);
        rsaCipher.init(Cipher.ENCRYPT_MODE, publicKey);

        // Headerに格納するデータ（URI、タイムスタンプ、AESキー、IV）を準備する
