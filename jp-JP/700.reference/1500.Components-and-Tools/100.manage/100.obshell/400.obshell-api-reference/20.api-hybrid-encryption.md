...
obshellがHTTPリクエストを受信すると、そのリクエストに対してセキュリティ検証を実施します。obshellの検証内容は、現在のノードのアイデンティティに応じて異なり、パスワード、URI、リクエストボディ（Request Body）の検証を含むがこれらに限定されません。

## 使用方法

### リクエストの送信

obshellにHTTPリクエストを送信する場合は、以下の手順を実行してください。

1. AES暗号アルゴリズムを使用して、リクエストボディ（Request Body）の内容を暗号化します。

   1. 16ビットのランダムなAESキー（key）と初期化ベクトル（Initialization Vector、IV）を生成します。

   2. CBC暗号モードを使用して、シリアライズされたリクエストボディの内容を暗号化します。

2. 安全性検証に使用するデータ構造HttpHeaderを構築します。構造体は以下のとおりです。
   ...
   ここで、設定するパスワードタイプ（`Auth`）によって、HTTPリクエストの検証通過状況が異なります。`Auth`がobshellノードのパスワードに設定されている場合、すべてのコンポーネント関連のHTTPリクエスト検証を通過します。一方、`Auth`がroot@sysユーザーのパスワードに設定されている場合、OBProxyを除く他のコンポーネントのHTTPリクエスト検証（OceanBaseデータベースやAgentのデプロイ、クラスタ運用保守関連のリクエストなど）のみを通過します。obshellノードのパスワードを設定する `/api/v1/agent/password` インターフェースのリクエスト検証方式も特殊であり、詳細については [ノードパスワードの設定](../400.obshell-api-reference/100.obshell-management/1500.set-node-password.md) を参照してください。
...
3. RSAアルゴリズムを使用してHttpHeaderを暗号化し、HTTPリクエストヘッダー（Request Headers）に設定します。
   ...
   2. 公開鍵と組み合わせて、RSAアルゴリズムを使用してシリアライズされたHttpHeader文字列を暗号化し、暗号化後にBase64エンコードします。その後、UTF-8エンコードの文字列に復号して、暗号化後の文字列Sを得ます。
...
1. リクエストヘッダーから、キー値ペア `"X-OCS-Header": S` または `"X-OCS-Agent-Header": S` を取得します。

2. 自身の秘密鍵と組み合わせて、Sを復号し、復号後のHttpHeader構造体を取得します。

3. HttpHeaderに記録されたAuthが正しいかどうかを確認します。`Auth` がリクエストAPIが属するコンポーネントと一致しない場合、該当するエラーメッセージを直接返します。

4. HttpHeaderに記録されたUriが、実際にリクエストされたUriと一致するかどうかを確認します。

5. HttpHeaderに記録されたKeysを使用して、リクエストボディの内容を復号し、リクエストボディの平文を取得します。復号に失敗した場合、検証に失敗します。

上記のセキュリティ検証ステップを通過した場合にのみ、obshell Serverはリクエストの処理を続行します。そうでない場合は、リクエスト元に対してセキュリティ検証失敗と応答します。
...
<main id="notice" type='explain'>
  <h4>説明</h4>
  <p>以下のPythonスクリプトを実行するには、Python 3.5以降が必要です。</p>
</main>
...
func AESEncrypt(raw []byte, key []byte, iv []byte) (string, error) {
    ...
    // Create an AES CBC mode encryptor.
...
以下は、Javaを使用してリクエストを送信するコード例です。コード内で定義されている `agent`、`pwd`、`uri`、`body`（意味はコードのコメントを参照）を変更するだけで済みます。変更後にこのコードをコンパイルして実行すると、対応するHTTPリクエストヘッダーとHTTPリクエストボディが得られます。
...
public class Main {
    ...
    public static String serializeStruct(HTTPHeader header) {
        ...
        // 構造体をJSONに変換し、keysフィールドをBase64エンコードする
    ...
    public static void main(String[] args) throws Exception {
        ...
        // ヘッダーに格納するデータ（URI、タイムスタンプ、AESキー、IV）の準備
