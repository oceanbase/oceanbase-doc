...
| binding | string | いいえ | "optional" | アーカイブと業務の優先モードを設定します。現在、`optional`（ユーザー業務優先）と`mandatory`（アーカイブ優先）をサポートしています。設定しない場合、デフォルトはOptionalモードです。 |
...
* obshell-sdk-pythonを使用したAPIメソッドのリクエスト方法の詳細については、[クラスタレベルバックアップ設定の設定](../../500.obshell-sdk-reference/100.python/600.backup-management/200.backup-configuration-for-cluster-of-python.md)を参照してください。

* obshell-sdk-goを使用したAPIメソッドのリクエスト方法の詳細については、[クラスタレベルバックアップ設定の設定](../../500.obshell-sdk-reference/200.go/600.backup-management/200.backup-configuration-for-cluster-of-go.md)を参照してください。