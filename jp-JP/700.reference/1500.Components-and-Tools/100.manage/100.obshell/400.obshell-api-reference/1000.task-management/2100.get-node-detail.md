...
ここでの `id` は、ノードの共通IDとして設定する必要があります。
...
| data | NodeDetailDTO | 詳細は以下の **NodeDetailDTOデータ構造** を参照してください。 |
...
| sub_tasks | []TaskDetailDTO | Node内のすべてのsubTask情報。詳細については、後述の**TaskDetailDTO情報のデータ構造**を参照してください。 |
...
| execute_agent | AgentInfo | 実行ノードのAgentInfo。以下の情報を含みます：<ul><li>`ip`：obshellのIPアドレス。</li><li>`port`：obshellのポート。</li></ul> |
...
例では、`Integrate server config` という名前のノードの情報が示されています。実行操作は `RUN`、実行結果は `SUCCEED` です。
