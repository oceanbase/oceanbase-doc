...
このドキュメントでは、クラスタ内のすべての未完了タスクを取得する方法について説明します。
...
| パラメータ | 型 | 必須 | サンプル値 | 説明 |
| --- | --- | --- | --- | --- |
| show_detail | bool | いいえ | true | subTaskの詳細情報を表示するかどうかを制御します。デフォルトは`true`で、表示することを意味します。 |
...
| data | []DagDetailDTO | 一連のDagDetailDTO情報です。DagDetailDTOの内容については、以下の**DagDetailDTO情報のデータ構造**を参照してください。  |
...
この例では、拡張中のクラスタが示されており、クラスタ内に未完了の拡張タスクが1つしか存在せず、各ノードに未完了のローカルタスクはないことを意味します。
...
* obshell-sdk-pythonを使用したAPIメソッドのリクエスト方法については、[未完了のすべてのタスクを照会する](../../500.obshell-sdk-reference/100.python/1000.task-management/2300.get-all-unfinish-task-of-python.md)を参照してください。

* obshell-sdk-goを使用したAPIメソッドのリクエスト方法については、[未完了のすべてのタスクを照会する](../../500.obshell-sdk-reference/200.go/1000.task-management/2300.get-all-unfinish-task-of-go.md)を参照してください。