...
| force | bool | いいえ | true | trueの場合、observerプロセスをkillします。 |
| forcePassDag | []string | いいえ | なし | 前順の起動/停止タスクのIDを指定します。このパラメータは特別な状況でのみ使用されます(クラスタの運用保守が1つまたは複数の先行する運用保守タスクによってロックされている場合、このパラメータを使用して先行タスクをアンロックできます)。 |
