...
<main id="notice" type='explain'>
  <h4>説明</h4>
  <p>obshell V4.2.3以降、タスクコマンドグループは <code>obshell utilities</code> から <code>obshell task</code> に変更されました。<code>obshell utilities</code> コマンドも引き続き使用できます。<code>obshell utilities</code> コマンドの詳細については、<a href='https://en.oceanbase.com/docs/common-oceanbase-database-10000000001168955'>obshell タスクコマンドグループ</a> を参照してください。</p>
</main>
...
| -y/--yes  | いいえ  | なし  |  なし   | 2回目の確認をキャンセルするかどうかを制御します。値は不要です。  |
...
`home_path` はOceanBaseデータベースのインストールディレクトリです。obshellはデフォルトでOceanBaseデータベースのインストールディレクトリ内の `bin` ディレクトリに配置されています。`-i/--id` は、実行中のタスクのIDを指定するために使用されます。