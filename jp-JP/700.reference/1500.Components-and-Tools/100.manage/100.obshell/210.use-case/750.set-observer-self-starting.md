...
本記事では、OBServerノードがパブリックネットワーク環境に接続できるかどうかに基づき、**オンライン設定** と **オフライン設定** の2つの方法を紹介します。これらの方法により、OBServerノードの起動時自動起動を設定できます。

<main id="notice" type='explain'>
  <h4>説明</h4>
  <p>OBServerノードの起動時自動起動を設定する場合、OceanBaseクラスタの各OBServerノードは本記事の内容を参照して設定する必要があります。</p>
</main>

## 適用シナリオ

このドキュメントで説明する設定方法は、以下の条件を満たすシナリオにのみ適用されます：
...
ノードがパブリックネットワークに接続できない場合、OBServerノードにログインして、以下の手順に従ってOBServerノードの起動時自動起動を設定できます。

1. 以下の内容を参考にスクリプトファイルを作成します。

   ここでは、スクリプトファイル名を `self_start.sh` とします。ご自由に変更してください。
...
2. 以下のコマンドを実行してスクリプトを起動します。
   ...
   ここでは、OceanBaseクラスタのインストールディレクトリを `/home/admin/oceanbase` とします。実際の状況に応じて置き換えてください。
