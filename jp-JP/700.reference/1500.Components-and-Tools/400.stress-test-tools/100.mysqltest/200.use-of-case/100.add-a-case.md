...
本記事では、caseとcaseのサンプル説明を追加する方法について説明します。
...
versionテストセットを例に、test_suite/versionにバージョン関連のテストケースを追加する手順は以下のとおりです。
  ...
  <main id="notice" type='explain'>
    <h4>説明</h4>
    <p>create table like機能をテストする場合、ケースファイル名はテストポイントを反映させることが望ましいです。例えば、create_table_like-syntax.testは構文レベルのテストを示します。</p>
  </main>
...
3. プロダクション結果ファイルを作成します：

    ```bash
    ./obd.sh mysqltest -n <deploy name> --test-set version.sample --record
    ```

4. 結果ファイルを人の目で確認した後（このステップはMySQLとの比較実行でも完了可能）、次の操作を行います：
...
`sequence_sql_demo_mysql.test` を例にとると、このケースの主な目的はシーケンスの基本機能をテストすることです。その中で、ハッシュ記号（#）で始まる行の内容は備考説明です。
...
--error 1054
select * from a group by id,seq_test1.nextval;
--error 4315
...
--error 0,4298
drop sequence seq_test1;
--echo  ########シュートダウン、テストデータベース、テーブルなどを削除する##########
--disableWarnings
drop database  IF EXISTS  oblrp;
--enableWarnings
purge recyclebin;
