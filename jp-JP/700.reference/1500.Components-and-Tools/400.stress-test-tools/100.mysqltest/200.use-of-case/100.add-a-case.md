...
本記事では、caseとcaseのサンプル説明を新規に追加する方法について説明します。

## 手順

versionテストセットを例に、test_suite/version内にバージョン関連のテストcaseを新規追加する手順は以下の通りです：

  <main id="notice" type='explain'>
    <h4>説明</h4>
    <p>例えばcreate table like機能をテストする場合、caseファイル名はテストポイントを反映させるのが望ましいです。例えば、create_table_like-syntax.testは構文レベルのテストです。</p>
  </main>
...
4. 結果ファイルを人が検証した後（このステップはMySQLとの比較実行でも完了できます）：
...
6. コミット時には、git addでtestファイルとresultファイルを同時にコミットすることを忘れないでください。

## 例

`sequence_sql_demo_mysql.test` を例にします。このcaseの主な目的は、sequenceの基本機能をテストすることです。シャープ記号（#）で始まる行は、備考説明です。
...
'sequence_sql_demo_mysql.result' の内容は以下の通りです：
...
drop sequence seq_test1;
########ショッピングカートを空にして、テストデータベースやテーブルなどを削除する##########
drop database  IF EXISTS  oblrp;
purge recyclebin;
```