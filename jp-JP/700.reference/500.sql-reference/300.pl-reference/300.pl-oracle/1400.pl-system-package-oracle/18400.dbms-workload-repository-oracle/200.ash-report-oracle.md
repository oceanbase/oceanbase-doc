...
| BTIME | サンプリング開始時間。 |
| ETIME | サンプリング終了時間。 |
| SQL_ID | サンプリング対象のSQLの`SQL_ID`。このフィールドを指定しないか、または`NULL`と指定した場合、`SQL_ID`に制限はありません。 |
| TRACE_ID | サンプリング対象のSQLの`TRACE_ID`。このフィールドを指定しないか、または`NULL`と指定した場合、`TRACE_ID`に制限はありません。 |
| WAIT_CLASS | サンプリング対象のイベントタイプ。`WAIT_CLASS`の値は`GV$ACTIVE_SESSION_HISTORY.WAIT_CLASS`フィールドの値を参照します。このフィールドを指定しないか、または`NULL`と指定した場合、待機タイプに制限はありません。 |
| REPORT_TYPE | レポートタイプ。現在は`HTML`と`TEXT`の2種類がサポートされています。このフィールドを指定しない場合、デフォルトは`TEXT`タイプです。 |
| SVR_IP | ASHレポートを生成するノードのIPアドレスを指定します。このフィールドを指定しないか、または`NULL`と指定した場合、IPアドレスに制限はありません。 |
| SVR_PORT | ASHレポートを生成するノードのポート番号を指定します。このフィールドを指定しないか、または`NULL`と指定した場合、ポート番号に制限はありません。 |
| TENANT_ID | ASHレポートを生成するテナントIDを指定します。このフィールドを指定しないか、または`NULL`と指定した場合、`TENANT_ID`に制限はありません。 |

## 使用方法

ASH関連情報を取得するには、`oceanbase.GV$ACTIVE_SESSION_HISTORY`ビューを照会する必要があります。
...
`ASH_REPORT` を呼び出し、診断する開始時間と終了時間を入力します。実行後、ASHレポート情報が画面に表示されます。
...
次のコマンドを実行して、レポートを `HTML` タイプで表示します。
