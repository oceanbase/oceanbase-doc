...
| level | 追跡レベルを設定するために使用されます。<ul> <li> 0：デフォルト動作。</li> <li> 1：各モジュールの使用メモリと時間を追加で出力します。</li> <li> 2：リライトされたQuery Blockに対応するSQLを追加で出力します。リライトが発生したかどうかにかかわらず。</li></ul> <br> **注意**：`level` はデータベースのキーワードであり、Oracleテナントでは二重引用符（「」）で囲み、MySQLテナントでは「`」で囲む必要があります。 |
