...
`RESET_PACKAGE`プロシージャは、セッション上のすべてのパッケージ変数をクリアするために使用されます。
...
リモート呼び出しのシナリオにおいて、現在はパッケージ変数のリモート同期設計の制限により、`RESET_PACKAGE` をリモート側に同期することができず、OBServerノードのログにエラーが報告されます。また、変数 `##__OB_RESET_ALL_PACKAGE_BY_DBMS_SESSION_RESET_PACKAGE__` は解決できず、その結果、後続の実行結果がローカルに返されることになります。
