...
ASH関連情報を取得するには、`GV$ACTIVE_SESSION_HISTORY`ビューを照会する必要があります。

## 例

`ASH_REPORT` を呼び出し、診断対象の開始時間と終了時間、およびノードのIPアドレスとポートを入力します。実行後、ASHレポート情報が画面に表示されます。

まず `use oceanbase` ライブラリで、以下のコマンドを実行します：
