...
|slug|information_schema_triggers-mysql-mode|

# INFORMATION_SCHEMA.TRIGGERS

`INFORMATION_SCHEMA.TRIGGERS` ディクショナリビューは、トリガーに関する情報を提供します。テーブルのトリガーに関する情報を表示するには、そのテーブルに対する `TRIGGER` 権限が付与されている必要があります。
...
| COLLATION_CONNECTION                                         | トリガー作成時の`collation_connection`システム変数の現在セッションでの値。   |
