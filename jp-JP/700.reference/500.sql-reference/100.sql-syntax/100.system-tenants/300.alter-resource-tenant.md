...
リソースプールの他の属性を変更する場合は、`ALTER RESOURCE POOL` ステートメントを使用する必要があります。`ALTER RESOURCE POOL` ステートメントの詳細については、[ALTER RESOURCE POOL](300.alter-resource-pool.md)を参照してください。
...
* 同種ゾーンモードでのスケールインシナリオにおいて、テナント `tenant2` の `UNIT_GROUP_ID` が `1006` のユニットグループを削除します。
  ...
  5. 実行が成功した後、ビュー `DBA_OB_TENANT_JOBS` を使用して、ユニット数削減タスクの実行状態を確認できます。
