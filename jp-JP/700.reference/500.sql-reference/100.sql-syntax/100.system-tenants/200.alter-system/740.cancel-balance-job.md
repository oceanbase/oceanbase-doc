| Description   |                 |
|---------------|-----------------|
| keywords      |                 |
| dir-name      |                 |
| dir-name-en   |                 |
| tenant-type   |                 |
|machine-translation| en          |
|slug|sql-tenant-cancel-balance-job|

# BALANCE JOB

## 説明

このステートメントは、Balance Jobを一時停止(SUSPEND)、再開(RESUME)、またはキャンセル(CANCEL)するために使用します。

テナントで実行中のBalance Jobがある状態で、他の緊急な変更が必要な場合、そのBalance Jobの実行が緊急変更に影響を与えるのを防ぐために、現在実行中のBalance Jobを一時停止し、緊急変更の実行後に再開して継続することができます。

現在進行中のBalance Jobが要件に合致しない場合は、そのタスクをキャンセルすることができます。また、Transfer Partitionを開始したものの、実行中のBalance Jobがあるためタスクの開始が遅延している場合も、現在進行中のBalance Jobをキャンセルできます。

## 使用制限と注意事項

* Balance Jobのキャンセル、一時停止、または再開の操作は、マスターデータベース（すなわち、マスターテナント）でのみ実行できます。

* Balance Jobを一時停止または再開する際、そのBalance Jobの状態が`CANCELING`であってはなりません。

## 権限要件

現在のユーザーが`ALTER SYSTEM`権限を持っていること。

## 構文

```sql
ALTER SYSTEM {SUSPEND | RESUME | CANCEL} BALANCE JOB [TENANT = 'tenant_name'];
```

## パラメータ説明

| パラメータ         | 説明        |
|--------------|------------|
| tenant_name  | 一時停止、再開、またはキャンセルするタスクのテナント名を指定します。オプションですが、システムテナントで実行する場合は指定する必要があります。|

## 例

以下はシステムテナントでの操作例です。

1.  以下のステートメントを使用して、現在実行中のタスクを照会します。

   ```shell
   obclient(root@sys)[oceanbase]> SELECT T.TENANT_NAME, J.TENANT_ID, J.JOB_ID, J.CREATE_TIME, J.MODIFY_TIME, J.BALANCE_STRATEGY, J.JOB_TYPE, j.STATUS
   FROM oceanbase.CDB_OB_BALANCE_JOBS J, oceanbase.DBA_OB_TENANTS T
   WHERE J.TENANT_ID = T.TENANT_ID;
   ```

   実行結果は次のとおりです：

   ```shell
   +-------------+-----------+--------+----------------------------+----------------------------+---------------------------+--------------+--------+
   | TENANT_NAME | TENANT_ID | JOB_ID | CREATE_TIME                | MODIFY_TIME                | BALANCE_STRATEGY          | JOB_TYPE     | STATUS |
   +-------------+-----------+--------+----------------------------+----------------------------+---------------------------+--------------+--------+
   | oracle001   |      1004 | 300793 | 2024-02-29 16:45:12.864110 | 2024-02-29 16:45:12.864110 | LS balance by shrink      | LS_BALANCE   | DOING  |
   +-------------+-----------+--------+----------------------------+----------------------------+---------------------------+--------------+--------+
   1 row in set
   ```

2. ビジネス状況に応じて、適切な操作を選択します。

   * 実行中のBalance Jobを一時停止する

     ```shell
     obclient(root@sys)[oceanbase]> ALTER SYSTEM SUSPEND BALANCE JOB TENANT = 'oracle001';
     ```

   * 一時停止されたBalance Jobを再開する

     ```shell
     obclient(root@sys)[oceanbase]> ALTER SYSTEM RESUME BALANCE JOB TENANT = 'oracle001';
     ```

   * 予期しないBalance Jobをキャンセルする

     ```shell
     obclient(root@sys)[oceanbase]> ALTER SYSTEM CANCEL BALANCE JOB TENANT = 'oracle001';
     ```

## 関連ドキュメント

* [oceanbase.CDB_OB_BALANCE_JOBS](../../../../700.system-views/300.system-view-of-sys-tenant/200.dictionary-view-of-sys-tenant/7800.o-cdb_ob_balance_jobs-of-sys-tenant.md)

* [oceanbase.DBA_OB_TENANTS](../../../../700.system-views/300.system-view-of-sys-tenant/200.dictionary-view-of-sys-tenant/23100.o-dba_ob_tenants-of-sys-tenant.md)

* [Balance Jobのキャンセル](../../../../../600.manage/350.load-balancing-manage/400.cancel-balance-job-task.md)

* [Balance Jobの一時停止と再開](../../../../../600.manage/350.load-balancing-manage/420.suspend-or-resume-a-balance-job.md)