...
| パラメータ | 説明 |
|---------|------|
| REGION | データベースサーバーが配置される地理的リージョンを指定します。オプションです。 |
| ZONE | データベースサーバーが配置されるアベイラビリティゾーンを指定します。必須です。 |
| SERVER | データベースサーバーのIPアドレスとポート番号を指定します。必須です。形式は 'IP:PORT' です。|
| shared_storage_info | クラスタ起動時に各Zoneに指定する初期のオブジェクトストレージ情報を記述します。文字列型です。文字列にはオブジェクトストレージのバケット名と、オブジェクトストレージ情報の複数のサブパラメータが含まれ、両者は `?` で区切られます。サブパラメータ間は `&` で区切ります。オプションです。複数のserver_infoはカンマで区切ります。|
