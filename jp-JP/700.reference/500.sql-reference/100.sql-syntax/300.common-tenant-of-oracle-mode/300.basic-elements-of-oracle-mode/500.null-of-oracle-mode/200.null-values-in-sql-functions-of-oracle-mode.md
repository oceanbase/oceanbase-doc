...
NULL値が発生した場合、`NVL()` 関数の戻り値を使用してNULL値を特定できます。`NVL()` 関数の式は `NVL(expr1,expr2)` であり、`expr1` が `NULL` でない場合は `expr1` を返し、そうでない場合は `expr2` を返します。
...
NVL関数の詳細については、[NVL](../../500.functions-of-oracle-mode/200.single-row-functions-of-oracle-mode/800.null-value-related-function-of-oracle-mode/500.nvl-of-oracle-mode.md)を参照してください。
