...
集合演算に関連する動作を制御するHintです。

集合演算句にはHintを直接追加する場所がないため、本記事で紹介するHintは、qb nameを使用して対象となる集合演算オブジェクトを指定する必要があります。詳細な使い方は、各Hintの例を参照してください。

| ヒントタイプ                        | 説明                                                  |
|----------------------------------|-------------------------------------------------------|
|`PQ_SET`                          | 集合演算の分散実行方式を制御します。                             |
|`USE_HASH_SET`                    | 最適化エンジンが集合演算でHASHメソッドを使用するように有効にします。逆操作は`NO_USE_HASH_SET`です。   |
|`NO_USE_HASH_SET`                 | 最適化エンジンが集合演算でHASHメソッドの使用を禁止します。逆操作は`USE_HASH_SET`です。     |

## `PQ_SET` ヒント

`PQ_SET` ヒントは、分散実行時の集合演算におけるデータ配布方法を指定します。
...
- `left_branch_qb_name`：`UNION` または `INTERSECT` 集合演算の左側の部分を指定します。

- `method_list`：具体的な分散実行方式を制御します。

具体的な方法の説明と例は以下のとおりです：

| データ配分方式           |   outline data hintの説明      |    説明   |
|----------------------|------------------------------|-----------|
...
|DIST_HASH_HASH        | PQ_SET ( @SET$1 SEL$1 HASH HASH)|  2つのブランチのunion/intersectに限定し、SEL$1を左ブランチとする場合 |
|DIST_NONE_PARTITION   | PQ_SET ( @SET$1 SEL$2 NONE PARTITION )| 2つのブランチのunion/intersectに限定し、SEL$2を左ブランチとする場合|
|DIST_PARTITION_NONE   | PQ_SET ( @SET$1 SEL$1 PARTITION NONE )|2つのブランチのunion/intersectに限定し、SEL$1を左ブランチとする場合|

### 例

以下のクエリ例では、`pq_set(@set$1 local local)` を使用してデータをローカルに直接プルする分散実行方式を指定し、`pq_set(@set$1 sel$2 hash hash)` を使用して `HASH` 方式でデータを再分散して集合演算を行い、`sel$2` を集合演算の左辺として使用します。
...
```sql
-- USE_HASH_SETヒントを使用して、集合演算でHASHメソッドの使用を強制する
EXPLAIN BASIC
...
以下のクエリ例では、`NO_USE_HASH_SET` を使用してHASHアルゴリズムを禁止します。OceanBaseはHASHとMERGEの2種類の集合演算アルゴリズムをサポートしているため、`NO_USE_HASH_SET` ヒントを使用すると、実際には集合演算で `MERGE` アルゴリズムが使用されます。`UNION ALL` の集合演算は重複除去の意味を持たないため、このヒントの影響を受けません。

```sql
-- NO_USE_HASH_SETヒントを使用して、オプティマイザーに集合演算でHASHメソッドの使用を回避するよう指示します。
EXPLAIN BASIC
