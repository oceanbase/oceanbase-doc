...
| Hintタイプ                       | 説明                                                   |
|--------------------------------|---------------------------------------------------------|
| PQ_SUBQUERY                    |  パラレル処理におけるサブクエリの動作をきめ細かく制御します。   |
| PUSH_SUBQ                      | 最適化エンジンに対し、結合に書き換えられていないサブクエリを早期に実行するよう指示します。<main id="notice" type='explain'><h4>説明</h4><p>OceanBaseデータベースV4.3.5では、V4.3.5 BP2から <code>PUSH_SUBQ</code> Hintをサポートしています。</p></main>|
| NO_PUSH_SUBQ                   | `PUSH_SUBQ` の逆操作であり、最適化エンジンが結合に書き換えられていないサブクエリを最後に実行するよう指示します。<main id="notice" type='explain'><h4>説明</h4><p>OceanBaseデータベースV4.3.5では、V4.3.5 BP2から <code>NO_PUSH_SUBQ</code> Hintをサポートしています。</p></main>|

## PQ_SUBQUERY Hint

`PQ_SUBQUERY` Hintは、パラレル処理におけるサブクエリの動作をきめ細かく制御し、特に分散データ配信が関与する場合に、パラレルクエリ(Parallel Query, PQ)環境下でのサブクエリの処理方法を最適化エンジンに指示します。

`PQ_SUBQUERY` Hintには、主に2つの制御方式が含まれます：非厳密制御方式と厳密制御方式です。
...
- `table_list`: オプションです。代表リストを指し、結合で割り当てられたsubplan filterメソッドでのみ使用されます。
- `op_order_no`: 操作ノードの順序番号です。
- `sub_qb_name_list`: サブクエリブロックのエイリアスリストです。
- `method_list`: 分散データ配信方式です。具体的な配信方法は以下の通りです：
  - `DIST_BASIC_METHOD`: 具体の方法を指定しません。
  - `DIST_PARTITION_WISE`: パーティション単位でデータを配分します。`NONE NONE` メソッドを使用します。
...
```sql
PQ_SUBQUERY(@SEL$1 0 SEL$1 PARTITION NONE) -- PQ_SUBQUERY(SEL$1)に省略可能
...
- `win_group`: (インデックスリスト) ウィンドウ関数のグループ化と内部動作を宣言するタプル。
  ...
  - `PARTITION_SORT`: ハッシュ分散実行(`HASH`)または分散実行なし(`NONE`)でパーティションソートアルゴリズムを適用するかどうか。デフォルトでは使用されません。
  - `PUSHDOWN`: ハッシュ分散実行を使用する場合、ウィンドウ関数のプッシュダウンを適用するかどうか。
...
`PUSH_SUBQ` ヒントは、オプティマイザーに対して、結合に書き換えられていないサブクエリを早期に実行するよう指示します。通常、これらのサブクエリは結合に書き換えられない場合、実行計画のすべてのテーブル結合計算の後に配置されます。サブクエリの計算コストが低く、大量のデータをフィルタリングできる場合、事前実行により計画のパフォーマンスが向上する可能性があります。ただし、サブクエリが結合(`/+unnest/`)に書き換えられている場合、このヒントは無効になります。適用シナリオは以下のとおりです：

* サブクエリの実行コストは小さいが、大量のデータを迅速にフィルタリングできる場合。
...
* `@qb_name`：オプションパラメータで、サブクエリのエイリアスを指定します(詳細については、前述の **QB_NAME パラメータ** を参照)。ヒントの対象を明確にするために使用されます。省略した場合、デフォルトでヒントが存在するサブクエリに対して有効になります。

**例：**

クエリ内で `PUSH_SUBQ` ヒントを使用して、オプティマイザーに対してサブクエリを早期に実行するよう指示し、`tbl1` テーブルのデータを事前にフィルタリングします。
...
<main id="notice" type='explain'>
  <h4>説明</h4>
  <p>OceanBaseデータベースV4.3.5では、V4.3.5 BP2バージョンから <code>NO PUSH_SUBQ</code> ヒントがサポートされています。</p>
</main>

`NO PUSH_SUBQ` ヒントは `PUSH_SUBQ` の逆操作であり、オプティマイザーに対して、結合されていないサブクエリを最後に実行するよう指示します。サブクエリのコストが高い場合や、行数を大幅に削減できない場合に適用されます。適用シナリオは以下のとおりです：
...
クエリ内で `NO PUSH_SUBQ` ヒントを使用し、オプティマイザーにサブクエリの実行を最後まで遅らせるよう指示します。
