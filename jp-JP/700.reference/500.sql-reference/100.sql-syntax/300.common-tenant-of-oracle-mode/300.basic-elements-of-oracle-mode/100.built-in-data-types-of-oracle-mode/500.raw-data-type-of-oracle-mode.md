...
データベースが文字型データを `RAW` 型に暗黙的に変換する場合、文字列内の各文字が表す16進数を4ビットに変換します。例えば、文字列 'AB' は 0XAB (10101011) に変換されます。
  ...
  <main id="notice" type='notice'>
    <h4>注意</h4>
    <p>入力されたいずれかの文字が16進数（0～9 および A～F または a～f）でない場合、エラーが発生します。文字数が奇数の場合、<code>RAW</code> 型データの先頭に0を追加し、<code>RAW</code> 型の長さが整数バイトになるようにします。例えば、文字列 'ABC' を <code>RAW</code> 型に変換すると、0x0ABC に変換されます。ここで、0x0A が1バイトを占め、0xBC が1バイトを占めます。</p>
  </main>
