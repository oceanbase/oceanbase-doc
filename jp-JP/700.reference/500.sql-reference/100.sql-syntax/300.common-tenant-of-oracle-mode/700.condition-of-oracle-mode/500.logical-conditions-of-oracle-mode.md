...
|slug|logical-conditions-oracle-compatible-mode|

# 論理条件

論理条件（Logical Condition）は、2つの条件を組み合わせて単一の結果を生成するか、単一の条件の結果を反転します。

論理条件の種類には `NOT`、`AND`、`OR` があります。

## 論理条件 NOT

論理条件 `NOT` は「非」を表し、単一の条件の結果を反転します。

### 判断ルール

条件が `FALSE` の場合、`TRUE` を返します。条件が `TRUE` の場合、`FALSE` を返します。条件が `UNKNOWN` の場合、`UNKNOWN` を返します。
...
論理条件 `AND` は「と」を表し、2つの条件を結合するために使用されます。
...
論理条件 `OR` は「または」を表し、いずれか一方であればよいことを示します。
