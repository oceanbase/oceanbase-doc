...
`IN` 条件は、式が式のリストまたはサブクエリのメンバーであるかどうか、または複数の式が式のリストまたはサブクエリのメンバーであるかどうかをテストできます。各式のリスト内の式は、演算子 `IN` の左側の式と数とデータ型が一致している必要があります。`expression_list` に1000個を超える式を指定することもサポートされています。
...
* `NOT IN` は `!= ALL` と同じです。集合内のいずれかのメンバーが `NULL` の場合、計算結果は `FALSE` になります。例：
  ...
  `NOT IN` 演算子の後のリスト内のいずれかの項目の計算結果が `NULL` になった場合、すべての行の計算結果は `FALSE` または `UNKNOWN` になり、行は返されません。