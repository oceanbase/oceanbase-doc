...
* クエリプランを解釈し、その情報を指定されたテーブル `test` に保存します。
  ...
  /* 日付時刻のフォーマットエラーが発生した場合は、以下のコマンドでTIMESTAMPフォーマットを再設定できます*/
  obclient> ALTER SESSION SET NLS_TIMESTAMP_FORMAT = 'YYYY-MM-DD HH24:MI:SS.FF';
  Query OK, 0 rows affected

  /* クエリプランを解釈し、その情報を指定されたテーブルtestに保存します */
...
* `explain_type` を省略し、`SELECT` ステートメントの実行計画情報を返します。
...
* `TRADITIONAL` フォーマットの `INSERT` ステートメントの実行計画を表示します。
...
* `JSON` フォーマットの `SELECT` ステートメントの実行計画を表示します。
