...
| TRADITIONAL \| JSON | `EXPLAIN` の出力形式を指定します。 <ul><li> `TRADITIONAL`：テーブル形式の出力です。   </li><li> `JSON`：`KEY:VALUE` 形式で出力します。`JSON` は `JSON` 文字列として表示され、`EXTENDED` および `PARTITIONS` 情報が含まれます。</li></ul>     |
