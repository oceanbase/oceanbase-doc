...
このステートメントは、複数の`SELECT`クエリの結果を1つの結果に統合するために使用されます。各`SELECT`の列数とデータ型は同じでなければなりませんが、列の長さは異なってもかまいません。
...
|    パラメータ     |                    説明                    |
|-----------|------------------------------------------|
| UNION ALL | 2つのクエリの結果を統合します。                               |
| UNION     | 2つのクエリの結果を統合し、重複を除去します。                           |
| MINUS     | 左側のクエリ結果セットから、右側のクエリに含まれる重複を除去した結果を取得します。                |
| INTERSECT | 左側のクエリ結果と右側のクエリ結果の共通部分、すなわち交差部分を保持し、重複を除去します。 |
