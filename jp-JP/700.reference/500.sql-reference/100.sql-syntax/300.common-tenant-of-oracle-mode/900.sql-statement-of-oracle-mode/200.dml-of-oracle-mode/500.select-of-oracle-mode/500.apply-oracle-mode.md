...
`APPLY`は、`ANSI CROSS JOIN`または`ANSI LEFT OUTER JOIN`の変種であり、左関連(left correlation)クエリをサポートします。`APPLY`には2つの形式があります：`OUTER APPLY`と`CROSS APPLY`です。

* `CROSS APPLY`は`CROSS JOIN`に似ていますが、右側で左テーブルの列を参照することができます（つまり、左関連です）。`table_reference`または`collection_expression`によって生成された結果セットに含まれる行のみを返します。
...
* `table_reference`（テーブル参照）：通常のテーブル、インラインビュー（つまり、サブクエリビュー）、TABLE()が指定できます。
* `collection_expression`（コレクション式）：サブクエリ、列、関数、コレクション構築子が指定できます。
* `collection_expression`の要件：ネストされたテーブルまたは可変配列の型である、つまりコレクション値を返す必要があります。
