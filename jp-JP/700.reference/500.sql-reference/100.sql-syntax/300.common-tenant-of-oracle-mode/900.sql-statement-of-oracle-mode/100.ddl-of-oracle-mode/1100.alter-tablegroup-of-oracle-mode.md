...
| SHARDING   | テーブルグループの属性です。`SHARDING` 属性を指定しない場合、デフォルトで `ADAPTIVE` のテーブルグループが作成されます。`SHARDING` の取り得る値は以下のとおりです：<ul><li>`NONE`：テーブルグループに追加されるテーブルに制限はありません。</li><li>`PARTITION`：テーブルグループに追加されるテーブルに対応するパーティション（パーティション1）は、テーブルグループ内のすべてのテーブルのパーティション1のパーティショニング方法（パーティションタイプ、パーティション数、パーティションValueを含む）と一致している必要があります。</li><li>`ADAPTIVE`：テーブルグループに追加されるテーブルに対応するパーティション（パーティション1）およびサブパーティションは、テーブルグループ内のすべてのテーブルのパーティション（パーティション1）およびサブパーティションのパーティショニング方法（パーティションタイプ、パーティション数、パーティションValueを含む）と一致している必要があります。</li></ul>同じパーティショニング方法には以下が含まれます：<ul><li>パーティションタイプが同じであること（例えば、どちらもHash+Rangeパーティションである場合）。</li><li>Hashパーティションの場合、参照する列数とパーティション数が同じである必要があります。</li><li>Rangeパーティションの場合、参照する列数とパーティション数が同じで、Range分割ポイントが同じである必要があります。</li></ul>サブパーティションについても、そのパーティションタイプに応じて上記の要件が適用されます。    |
...
* `tblgroup4` という名前のテーブルグループを作成し、その `SHARDING` プロパティを `ADAPTIVE` に設定します。テーブルグループ内のテーブル `tbl1` とテーブル `tbl2` のパーティション数を変更すると、テーブルグループ内のすべてのテーブルのパーティショニング方法が一致しなくなります。この状態では、新しいテーブル `tbl3` をテーブルグループに追加することはできません。
