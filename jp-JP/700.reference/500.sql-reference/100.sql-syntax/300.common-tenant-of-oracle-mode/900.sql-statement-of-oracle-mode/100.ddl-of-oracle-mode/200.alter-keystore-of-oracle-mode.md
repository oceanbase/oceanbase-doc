...
このステートメントは、Keystoreのプロパティを変更するために使用されます。Keystoreの開閉、Keystoreパスワードの変更、Keystoreキーの生成が含まれます。
...
* Keystoreを開きます。Keystoreが`open`状態である場合にのみ、暗号化されたテーブルにアクセスし、`SET KEY`操作を実行できます。
