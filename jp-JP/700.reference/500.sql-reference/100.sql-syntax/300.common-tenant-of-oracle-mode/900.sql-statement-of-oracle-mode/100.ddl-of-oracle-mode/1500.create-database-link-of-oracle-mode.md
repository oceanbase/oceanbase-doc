...
例3：リモートのOracleデータベースに接続する`dblink`を作成します。名前は`orcl_dblink`、リモートデータベースのテナントは`oracle`です。
