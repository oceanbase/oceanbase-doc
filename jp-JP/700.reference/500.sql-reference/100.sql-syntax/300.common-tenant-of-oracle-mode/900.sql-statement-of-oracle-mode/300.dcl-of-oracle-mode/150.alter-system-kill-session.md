...
| パラメータ | 説明 |
|------|------|
| session_id | 終了するセッションIDを指定します。`SHOW PROCESSLIST` または `SHOW FULL PROCESSLIST` を実行して `Id` 列を確認するか、`GV$SESSION` ビューをクエリして取得できます。 |
| IMMEDIATE | オプションパラメータで、セッションを即座に終了するかどうかを指定します。指定されている場合、セッションは即座に終了しようと試みます。指定されていない場合、現在のトランザクションが完了するのを待ちます。 |
...
現在のデータベース内のセッション数とセッションIDの詳細については、[テナントセッションの確認](../../../../../1200.database-proxy/1500.view-tenant-sessions.md)を参照してください。