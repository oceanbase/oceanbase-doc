...
| RECYCLEBIN                         | シュートダウンを確認します。                                                                                                   |
| \[FULL\] PROCESSLIST                 | 現在のテナントプロセスリストを確認します。具体的には以下のとおりです：<ul><li><code>SHOW PROCESSLIST</code> 簡潔なプロセスリストを表示します。具体的には以下のとおりです：  <ul><li><code>ID</code>：プロセスのID、つまり現在のセッションのクライアントセッションIDであり、このIDはクライアント上のセッションを一意に識別するものです。</li><li><code>USER</code>：接続ユーザー名。</li><li><code>HOST</code>：クライアントのIPアドレスとポート番号（ODP経由で接続する場合はODPのIPアドレスとポート番号）。</li><li><code>DB</code>：アクセスするデータベースの名前。</li><li><code>COMMAND</code>：現在実行されているコマンドタイプ。値は <code>Query</code>、<code>Sleep</code> などがあります。</li><li><code>TIME</code>：現在のコマンド実行時間。単位は秒です。コマンドが再試行された場合、ゼロにリセットされて再計算されます。</li><li><code>STATE</code>：現在のセッション状態。値は <code>SLEEP</code>、<code>ACTIVE</code> などがあります。</li><li><code>INFO</code>：現在実行中のコマンドを表示します。長さ制限は100文字で、超過分は切り捨てられます。</li></ul>  </li><li><code>SHOW FULL PROCESSLIST</code>：各プロセスの詳細情報を含む完全なプロセスリストを表示します。具体的には以下のとおりです：  <ul><li><code>ID</code>：プロセスのID、つまり現在のセッションのクライアントセッションIDであり、このIDはクライアント上のセッションを一意に識別するものです。</li><li><code>USER</code>：接続ユーザー名。</li><li><code>TENANT</code>：接続したテナント。</li><li><code>HOST</code>：クライアントのIPアドレスとポート番号（ODP経由で接続する場合はODPのIPアドレスとポート番号）。</li><li><code>DB</code>：アクセスするデータベースの名前。</li><li><code>COMMAND</code>：現在実行されているコマンドタイプ。値は <code>Query</code>、<code>Sleep</code> などがあります。</li><li><code>TIME</code>：現在のコマンド実行時間。単位は秒です。コマンドが再試行された場合、ゼロにリセットされて再計算されます。</li><li><code>STATE</code>：現在のセッション状態。値は <code>SLEEP</code>、<code>ACTIVE</code> などがあります。</li><li><code>INFO</code>：現在実行中のコマンドを表示します。</li><li><code>IP</code>：サーバーのIPアドレス。</li><li><code>PORT</code>：SQLポート番号。</li></ul>  </li></ul>  <main id="notice" type='explain'><h4>説明</h4><p><code>SHOW PROCESSLIST</code> ステートメントを使用して、現在のデータベース内のセッション数とセッションのIDの詳細情報を照会するには、<a href="../../../../../1200.database-proxy/1500.view-tenant-sessions.md">テナントセッションの確認</a> を参照してください。</p></main>|
| TRACE [FORMAT='JSON']              | SQLステートメントの実行状況を表示します。JSON形式で出力することができます。|
