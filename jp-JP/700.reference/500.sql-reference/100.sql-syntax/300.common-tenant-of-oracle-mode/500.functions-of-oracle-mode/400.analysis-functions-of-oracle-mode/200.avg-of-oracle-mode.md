...
|            パラメータ             |                                                                                                                                                説明                                                                                                                                                |
|---------------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| DISTINCT \| UNIQUE \| ALL | クエリ時の重複値の取り扱いを指定します。オプションで、デフォルトは `ALL` です。 <ul><li> `ALL`：クエリ時にデータ中の重複値を除去せず、`NULL` 値も無視します。   </li><li> `DISTINCT`：クエリ時にデータ中の重複値を除去し、`NULL` 値も無視します。   </li><li> `UNIQUE`：クエリ時にデータ中の重複値を除去し、`NULL` 値も無視します。</li></ul>    |
...
| OVER                      | `OVER` 句を使用してウィンドウを定義し、計算を行います。詳細については、[分析関数の説明](../400.analysis-functions-of-oracle-mode/100.window-function-description-of-oracle-mode.md)を参照してください。                                                                                                                                                                                                          |
  ...
  <main id="notice" type='notice'>
    <h4>注意</h4>
    <p><code>DISTINCT</code> または <code>UNIQUE</code> キーワードを指定した場合、<code>analytic_clause</code> に <code>order_by_clause</code> および <code>windowing_clause</code> を記述することはできません。</p>
  </main>
