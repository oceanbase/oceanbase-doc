...
|slug|max-oracle-compatible-mode|
...
|            パラメータ             |                                                                                                                                           説明                                                                                                                                            |
|---------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| DISTINCT \| UNIQUE \| ALL | クエリ時の重複除去の有無。オプションで、デフォルト値は `ALL` です。 <ul><li> `ALL`：重複行を含むすべての値を返し、`NULL` 値の行は無視します。  </li><li> `DISTINCT`：返される行から重複行を除外し、`NULL` 値の行は無視します。   </li><li> `UNIQUE`：返される行から重複行を除外し、`NULL` 値の行は無視します。 </li></ul>   |
