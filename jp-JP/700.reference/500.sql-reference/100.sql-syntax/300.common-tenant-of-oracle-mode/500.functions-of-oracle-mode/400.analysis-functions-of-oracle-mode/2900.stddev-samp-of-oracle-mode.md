...
|slug|stddev_samp-oracle-compatible-mode|

# STDDEV_SAMP

## 説明

この関数は、数値データをパラメータとしてサンプル標準偏差を計算します。サンプル標準偏差とは、サンプル分散の算術的平方根です。`STDDEV_SAMP` と関数 `STDDEV` の違いは、`STDDEV` が入力データが1行のみの場合 `0` を返すのに対し、`STDDEV_SAMP` は `NULL` を返す点です。集計または分析関数として使用できます。
...
列 `department_id` でグループ化し、列 `hiredate` の昇順でソートした上で、列 `salary` の標準偏差を求めます。
