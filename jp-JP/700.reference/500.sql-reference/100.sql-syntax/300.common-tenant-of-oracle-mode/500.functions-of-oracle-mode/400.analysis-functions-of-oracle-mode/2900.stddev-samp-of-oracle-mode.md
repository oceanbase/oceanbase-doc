...
この関数は、数値データをパラメータとして受け取り、標本標準偏差を計算します。標本標準偏差とは、標本分散の算術平方根です。`STDDEV_SAMP` と関数 `STDDEV` の違いは、`STDDEV` が入力データが1行のみの場合に `0` を返すのに対し、`STDDEV_SAMP` は `NULL` を返す点です。集計や分析関数として使用できます。
...
| expr | 数値型（`NUMBER`、`FLOAT`、`BINARY_FLOAT` および `BINARY_DOUBLE`）または数値型へ変換可能な式。                 |
