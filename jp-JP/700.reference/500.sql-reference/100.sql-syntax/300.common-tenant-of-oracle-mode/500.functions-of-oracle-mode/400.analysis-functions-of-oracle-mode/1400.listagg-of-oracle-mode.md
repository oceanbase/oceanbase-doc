...
この関数は列を行に変換するために使用されます。`LISTAGG` は `ORDER BY` 句で指定された各グループ内のデータをソートし、測定列の値を結合します。集計や分析関数として使用できます。
...
測定列が `RAW` の場合、データ型は `RAW` を返します。それ以外の場合、戻り値は `VARCHAR2` です。
...
2019年10月10日以前に採用された従業員情報を照会します。その従業員の所属部門、採用日、およびその部門内の他の従業員を含みます。
