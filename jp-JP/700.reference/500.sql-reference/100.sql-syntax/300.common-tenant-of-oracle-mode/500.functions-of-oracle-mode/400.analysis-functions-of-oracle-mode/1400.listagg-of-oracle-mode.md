...
この関数は、列を行に変換するために使用されます。`LISTAGG`は`ORDER BY`句で指定された各グループ内のデータをソートし、次にメジャー列の値を集計します。集計または分析関数として使用できます。

  <main id="notice" type='explain'>
    <h4>説明</h4>
    <ul>
    <li>分析関数として使用する場合、<code>OVER</code>句を使用してウィンドウを定義して計算を実行する必要があります。この関数は、<code>query_partition_clause</code>の1つ以上の式に基づいてクエリ結果セットをいくつかのグループに分割します。</li>
...
2019年10月10日以前に雇用された従業員情報を照会します。これには、その従業員の所属部門、雇用日、およびその部門の他の従業員が含まれます。
