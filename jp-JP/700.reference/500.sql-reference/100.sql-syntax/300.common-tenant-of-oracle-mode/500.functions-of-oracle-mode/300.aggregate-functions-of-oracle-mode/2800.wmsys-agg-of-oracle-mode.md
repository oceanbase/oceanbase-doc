...
この関数は、列を行に変換し、測定列の値を結合するために使用されます。集計または分析関数として使用できます。
  ...
  <main id="notice" type='explain'>
    ...
    <li>グループごとの集計として、<code>WM_CONCAT</code> は <code>GROUP BY</code> 句で定義された各グループに対して操作を実行し、出力行を返します。</li>
    </ul>
    </li>
    </ul>
  </main>
...
|      パラメータ      |                                           説明                                            |
|--------------|-----------------------------------------------------------------------------------------|
| DISTINCT     | `DISTINCT` を指定すると、結果に重複する値が削除されます。オプションです。                                                      |
...
`department_id` 列でグループ化し、同一グループ内のすべての `last_name` を照会します。
