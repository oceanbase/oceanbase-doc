...
この関数は、列を行に変換し、測定列の値を結合するために使用されます。集計や分析関数として利用できます。
  ...
  <main id="notice" type='explain'>
    <h4>説明</h4>
    <ul>
    <li>分析関数として使用する場合は、<code>OVER</code> 句を使用してウィンドウを定義し、計算を行います。この関数は、<code>query_partition_clause</code> 内の1つまたは複数の式に基づいて、クエリ結果セットをいくつかのグループに分割します。</li>
    <li>集計関数として使用する場合は、<code>OVER</code> キーワードを追加する必要はありません。
    <ul>
    <li>単一の集合としての集計関数として、<code>WM_CONCAT</code> はすべての行に対して操作を行い、単一の出力行を返します。</li>
    <li>グループごとの集計として、<code>WM_CONCAT</code> は <code>GROUP BY</code> 句で定義された各グループに対して操作を行い、出力行を返します。</li>
    </ul>
    </li>
    </ul>
  </main>
