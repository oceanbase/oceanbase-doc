...
この関数は、列を行に変換するために使用されます。`LISTAGG` は `ORDER BY` 句で指定された各グループ内のデータをソートし、測定列の値を結合します。集計や分析関数として使用できます。

  <main id="notice" type='explain'>
    <h4>説明</h4>
    <ul>
    <li>分析関数として使用する場合は、<code>OVER</code> 句を使用してウィンドウを定義し、計算を行います。この関数は、<code>query_partition_clause</code> の1つまたは複数の式に基づいて、クエリ結果セットをいくつかのグループに分割します。</li>
    <li>集計関数として使用する場合は、<code>OVER</code> キーワードを追加する必要はありません。
    <ul>
    <li>単一の集合としての集計関数として、<code>LISTAGG</code> はすべての行に対して操作を実行し、単一の出力行を返します。</li>
    <li>グループ化された集計として、<code>LISTAGG</code> は <code>GROUP BY</code> 句で定義された各グループに対して操作を実行し、出力行を返します。</li>
    </ul>
    </li>
    </ul>
  </main>
...
| delimiter    | 測定値を区切る文字列を指定します。この句はオプションで、デフォルトは `NULL` です。                                                       |
...
測定列が `RAW` の場合、戻り値のデータ型は `RAW` となります。それ以外の場合、戻り値は `VARCHAR2` となります。
...
2019年10月10日以前に採用された従業員情報を照会します。これには、その従業員の所属部門、採用日、およびその部門内の他の従業員が含まれます。
