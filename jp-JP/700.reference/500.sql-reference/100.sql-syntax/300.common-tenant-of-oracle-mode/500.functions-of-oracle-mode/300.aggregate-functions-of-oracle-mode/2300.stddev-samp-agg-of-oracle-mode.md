...
この関数は、数値データをパラメータとしてサンプル標準偏差を計算します。サンプル標準偏差は、サンプル分散の算術的平方根です。`STDDEV_SAMP` と関数 `STDDEV` の違いは、`STDDEV` が入力データが1行のみの場合 `0` を返すのに対し、`STDDEV_SAMP` は `NULL` を返す点です。集計または分析関数として使用できます。
