...
|            パラメータ             |                                                                                                                                           説明                                                                                                                                            |
|---------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| DISTINCT \| UNIQUE \| ALL | クエリ時の重複除去を指定します。オプションで、デフォルト値は `ALL` です。 <ul><li> `ALL`：すべての値を返します。重複行を含み、`NULL` 値の行は無視されます。   </li><li> `DISTINCT`：返される行から重複行を除外し、`NULL` 値の行は無視されます。   </li><li> `UNIQUE`：返される行から重複行を除外し、`NULL` 値の行は無視されます。 </li></ul>   |
