...
この関数は、パラメータで指定された列の最大値を返します。集計または分析関数として使用できます。
...
|            パラメータ             |                                                                                                                                           説明                                                                                                                                            |
|---------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| DISTINCT \| UNIQUE \| ALL | クエリ時の重複除去の有無。オプションで、デフォルト値は `ALL` です。 <ul><li> `ALL`：重複行を含むすべての値を返し、`NULL` 値の行は無視します。   </li><li> `DISTINCT`：返される行から重複行を削除し、`NULL` 値の行は無視します。   </li><li> `UNIQUE`：返される行から重複行を削除し、`NULL` 値の行は無視します。 </li></ul>   |
| expr                      | 数値、文字、日付型、またはその他のデータ型の列または式です。                                                                                                                                                                                                                                                               |
