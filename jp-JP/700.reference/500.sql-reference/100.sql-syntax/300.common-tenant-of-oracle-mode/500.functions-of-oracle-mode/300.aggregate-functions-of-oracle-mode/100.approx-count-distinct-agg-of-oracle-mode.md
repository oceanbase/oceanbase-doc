...
この関数は、パラメータで指定された列の重複を除去した後の行数を計算し、近似値として返します。この関数は、参照される列の選択性をさらに計算するために使用できます。

関数 `COUNT(DISTINCT x)` と比較して、`APPROX_COUNT_DISTINCT` は近似値を返すため、計算速度が非常に速いです。大量のデータを処理する場合、`COUNT(DISTINCT x)` はしばしば長時間かかりますが、`APPROX_COUNT_DISTINCT` を使用することで、わずかな精度を犠牲にしても計算効率が大幅に向上します。
...
* 列 `col1` と列 `col2` でグループ化して重複を除去した後、列 `col1` の各数値の出現回数を集計します。
...
* 列 `col1`、列 `col2`、および列 `col3` でグループ化して重複を除去した後、列 `col1` と列 `col3` の組み合わせ値の出現回数を集計します。
