...
|slug|coalesce-oracle-compatible-mode|

# COALESCE

## 説明

この関数は、パラメータリストの最初のNULLではない式を返します。少なくとも2つのパラメータを指定する必要があります。
...
* 戻り値のタイプは、パラメータリストの最初のNULLではない式 `expr` のデータ型と同じです。

* すべてのパラメータが `NULL` の場合、`NULL` を返します。

## 例

テーブル `tbl1` を作成します。`product_id` は商品ID、`list_price` は商品の定価、`min_price` は商品の最低価格、`Sale` は商品の実際の販売価格です。商品の割引率を `8` 割に設定し、各商品の実際の販売価格を計算します。この場合、`COALESCE` 関数を使用できます。`list_price` がNULLの場合は最低価格 `min_price` で計算し、`min_price` もNULLの場合は `10` で計算します。
