...
この関数は、ルートノードから指定されたノードまでの列値のパスを返します。`CONNECT BY` 条件で返される各行の列値は、指定された区切り文字で区切られます。
...
|   パラメータ   |                             説明                              |
|--------|-------------------------------------------------------------|
| column | 戻り値の列名を指定します。`CHAR`、`VARCHAR2`、`NCHAR`、または `NVARCHAR2` データ型を指定できます。 |
...
テーブル `tbl1` を作成し、テストデータを挿入します。ここで、列 `name` の `ABC` はルート値であり、他の値はノードです。列 `name` 内の値から `ABC` までのすべてのパスを照会します。
