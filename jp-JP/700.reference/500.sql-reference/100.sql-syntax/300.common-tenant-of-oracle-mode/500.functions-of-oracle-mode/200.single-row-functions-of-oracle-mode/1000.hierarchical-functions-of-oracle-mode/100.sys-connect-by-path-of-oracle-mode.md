...
|   パラメータ   |                             説明                              |
|--------|-------------------------------------------------------------|
| column | 返されるデータの列名を指定します。`CHAR`、`VARCHAR2`、`NCHAR`、または`NVARCHAR2`データ型を指定できます。 |
| char   | 分隔文字を指定します。`CHAR`、`VARCHAR2`、`NCHAR`、または`NVARCHAR2`データ型を指定できます。    |
...
テーブル `tbl1` を作成し、テストデータを挿入します。このテーブルの列 `name` において、`ABC` はルート値であり、その他の値はノードです。列 `name` 内の値から `ABC` へのすべてのパスを照会します。
