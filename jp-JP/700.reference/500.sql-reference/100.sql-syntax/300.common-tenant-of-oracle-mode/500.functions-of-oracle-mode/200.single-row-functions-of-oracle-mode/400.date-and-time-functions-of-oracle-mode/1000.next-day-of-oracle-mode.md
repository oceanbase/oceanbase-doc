...
この関数は、日付 `date` の翌週の `week`（曜日の値）に該当する日付を返します。
...
|------|-------------------------------------------------------------------------------------------------------------------------|
| date | `DATE` データ型の値。                                                                                                          |
| week | 周の値。`SUNDAY`（日曜日）、`MONDAY`（月曜日）、`TUESDAY`（火曜日）、`WEDNESDAY`（水曜日）、`THURSDAY`（木曜日）、`FRIDAY`（金曜日）、`SATURDAY`（土曜日）が含まれます。 |
...
obclient> SELECT SYSDATE "現在の日付",
    ...
    FROM DUAL;
+--------------+-----------+-----------+-----------+-----------+-----------+-----------+-----------+
| 現在の日付     | 来週の月曜日    | 来週の火曜日    | 来週の水曜日    | 来週の木曜日    | 来週の金曜日    | 来週の土曜日    | 来週の日曜日    |
+--------------+-----------+-----------+-----------+-----------+-----------+-----------+-----------+
