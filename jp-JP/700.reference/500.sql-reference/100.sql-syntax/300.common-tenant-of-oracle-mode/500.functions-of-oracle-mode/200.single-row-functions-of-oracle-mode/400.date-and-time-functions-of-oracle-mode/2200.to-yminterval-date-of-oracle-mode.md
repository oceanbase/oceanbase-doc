...
|                                               パラメータ                                                |                                                                                                                      説明                                                                                                                      |
|-------------------------------------------------------------------------------------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| \[+\|-\] years-months                                                                           | このパラメータ形式に適合する `CHAR`、`VARCHAR2`、`NCHAR` または `NVARCHAR2` データ型の文字列。<ul><li> `years` は年を表し、整数値で、範囲は \[0,999999999\] です。   </li><li> `months` は月を表し、整数値で、範囲は \[0,11\] です。</li></ul>    |
| \[-\]P\[ years Y\]\[months M\]\[days D\]\[T\[hours H\]\[minutes M\]\[seconds\[.fracsecs\]S\]\] | このパラメータ形式に適合する `CHAR`、`VARCHAR2`、`NCHAR` または `NVARCHAR2` データ型の文字列。 `fracsecs` は秒の小数部を表し、整数値で、範囲は \[0,999999999\] です。<br> **注意**  値にスペースを含めることはできません。                                                                   |
...
時間間隔として 2 年 6 ヶ月 10 日 4 時間 30 分 15 秒を指定するには、以下の例を使用できます：
...
以下は負の時間間隔の例です。3 年 1 ヶ月 5 日を引く必要がある場合、以下の例を使用できます：
