...
OceanBaseデータベースのOracleモードでは、`SDO_GEOMETRY` 型の4つのメンバー関数を使用して、`SDO_GEOMETRY` 空間オブジェクトの次元、タイプ、有効性情報を照会できます。また、`SDO_GEOM` パッケージの関数 `SDO_DISTANCE()` およびその他の関数 `SDO_RELATE()` を使用して、空間オブジェクトの距離や関係情報を計算することもサポートされています。

## `SDO_GEOMETRY` 型

### GET_DIMS() と ST_COORDDIM()

`GET_DIMS()` と `ST_COORDDIM()` 関数は、空間オブジェクトの次元（つまり、幾何オブジェクトの座標数）を取得するために使用されます。例えば、二次元オブジェクトの場合、次元は2、三次元オブジェクトの場合は3です。
...
`ST_ISVALID()` は、空間オブジェクトが有効な幾何オブジェクトかどうかをチェックするために使用されます。オブジェクトが有効な場合、関数は1を返します。無効な場合は0を返します。
...
`SDO_DISTANCE()`は、2つの空間幾何オブジェクト間の最小距離を計算します。例えば、2次元パラメータ geom1 と 2次元パラメータ geom2 間の最小距離です。
...
`SDO_AREA()`は、幾何オブジェクトの平面積を計算します。戻り値の単位は、その幾何が存在するSRSの線形単位と一致します。球面/楕円体への変換は行いません。2D平面幾何のみをサポートし、3D入力はサポートされていません。
...
戻り値は `NUMBER` 型データで、幾何オブジェクトの平面積を表します。単位は、その幾何が存在するSRSの線形単位と一致します。
...
この関数は、ある幾何オブジェクトが別の幾何オブジェクトを厳密に含むかどうかを判断するために使用されます。例えば、`geomA` が `geomB` を厳密に含むかどうか（境界点は含まれないものと見なす）。`SDO_RELATE(geomA, geomB, 'mask=CONTAINS')` と同義であり、論理的には `Contains(A, B) ⇔ Within(B, A)` と等価です。
...
この関数は、ある幾何オブジェクトが別の幾何オブジェクトと、交差、接触、重なりなど、いかなる形態の空間的な交わりを持つかどうかを判断します。境界が接触する場合も相互作用と見なされます。これは `SDO_RELATE(..., 'mask=ANYINTERACT')` と同じです。
...
戻り値は `BOOLEAN` 型データです：`TRUE`/`FALSE`/`NULL`（空の入力幾何オブジェクトには `NULL` を返します）。
...
`SDO_RELATE()` は、オブジェクトが指定された空間関係に合致するかどうかを判断するために使用されます。

注意点は以下のとおりです：

* 空間関係演算は現在 `ANYINTERACT` のみをサポートしています。
* パラメータ `geom1` と `geom2` が異なる座標系に基づいている場合、`geom2` でエラーが発生します。
...
```sql
-- テストテーブル test_spatial_index を作成
...
-- インデックスクエリ
SELECT * FROM test_spatial_index WHERE SDORELATE(g, SDO_GEOMETRY(2001, 4326, SDO_POINT_TYPE(10.1234,50.567,null), null, null), 'querytype=WINDOW mask=anyinteract') = 'TRUE';
```
...
実行計画は以下のとおりで、idxインデックスを使用してアクセスしています：
...
* 3Dジオメトリでは実際の `Z` 値が返され、`W` は常に `NULL` となります。
...
ネストされた `VERTEX_SET_TYPE` テーブルが返され、各行が1つの頂点を表します。X/Y/Z/W および予約済み列が含まれます：

```shell
# X,Y,Z,W が頂点座標、V1..V11 が予約済み列で、今後の拡張に使用されます
# ID が頂点IDで、1から順に増加します
X,Y,Z,W,V1..V11,ID
```
...
SELECT v.id, v.x, v.y, v.z
    FROM TABLE(
        SDO_UTIL.GETVERTICES(
            SDO_GEOMETRY(
                3002,            -- 3002 = 三次元ライン (3D LineString)
                NULL,
                NULL,
