...
| CONTENT | 解釈する内容は有効なXMLフラグメントである必要がありますが、必ずしも構文が正しいとは限りません。 |
| AS datatype | 戻り値の型を指定します。`VARCHAR`、`VARCHAR2`、`BLOB`、または`CLOB`を定義でき、デフォルトは`CLOB`です。戻り値の型が`BLOB`の場合、`ENCODING`を指定できます。 |
