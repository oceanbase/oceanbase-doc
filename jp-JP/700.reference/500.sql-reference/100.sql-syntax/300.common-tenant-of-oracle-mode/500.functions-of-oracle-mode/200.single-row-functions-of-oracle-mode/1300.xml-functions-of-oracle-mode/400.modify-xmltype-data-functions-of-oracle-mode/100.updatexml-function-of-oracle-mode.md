...
この関数は、XMLTypeデータを更新するための式を変更するために使用されます。1つまたは複数のXPathとValueのペアを指定することで、入力された元のXMLTypeデータの式を、XPathが検索したノードを`value_expr`で指定された更新値に、指定された順序で更新します。
...
| XPath_string | XPathを使用して、更新が必要なXMLコンテンツを指定します。 |
| value_expr | 更新値を指定します。データ型は、XMLType型データ、スカラー型（例：`CHAR`/`VARCHAR2`）、`CLOB`、`NULL`が可能です。 |
...
- `value_expr` は `NULL`、スカラー、XMLType型の入力が可能です。
- `namespace_string` はオプションであり、XPathでノードを検索する名前空間を指定できます。
