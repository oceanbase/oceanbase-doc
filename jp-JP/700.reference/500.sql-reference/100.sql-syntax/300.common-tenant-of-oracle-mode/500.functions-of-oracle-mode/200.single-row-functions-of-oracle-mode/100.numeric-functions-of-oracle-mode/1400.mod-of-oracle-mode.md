...
この関数は剰余関数であり、2つの数値式を除算したときの余りを返します。戻り値の符号は被除数の符号と同じです。
  ...
  <main id="notice" type='notice'>
    <h4>注意</h4>
    <p><code>REMAINDER (x,y)</code> と <code>MOD (x,y)</code> 関数で計算する際、どちらも <code>result=x-y*(x/y)</code> という公式を使用しますが、<code>x/y</code> の計算方法が異なります。<code>REMAINDER (x,y)</code> 関数では <code>ROUND(x/y)</code> を採用し、<code>MOD (x,y)</code> 関数では <code>FLOOR(x/y)</code> を採用します。</p>
  </main>
