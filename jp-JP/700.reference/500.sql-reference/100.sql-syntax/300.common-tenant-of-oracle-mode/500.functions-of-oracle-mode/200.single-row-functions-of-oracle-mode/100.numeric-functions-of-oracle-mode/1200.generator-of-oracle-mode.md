...
`GENERATOR` 関数は、Table Functionフレームワーク内で使用するために設計された特殊な組み込み関数です。主な役割は、指定された数の行を持つ仮想テーブルを生成することです。このテーブルには実際のデータは含まれず、データを挿入できる構造を提供します。`GENERATOR` 関数は通常、一連の連続した行を生成し、バッチ処理やデータセットのシミュレーションに使用されます。

Table FunctionはSQLの高度な機能であり、関数が単一の値ではなく、テーブル全体を返すことを可能にします。これにより、Table Functionは通常のデータテーブルのように、複数の行と列を生成できます。
...
`TABLE(GENERATOR(N))` は並列実行をサポートしています。並列 `SELECT` クエリや並列データ操作言語（DML）操作で使用できます。ただし、これらの仮想行を生成するプロセス自体は並列ではありません。`TABLE(GENERATOR(N))` が生成する行を読み取るのは、単一のスレッドだけです。そのため、`TABLE(GENERATOR(N))` を含むデータフロー操作（Data Flow Operation, DFO）では、Degree of Parallelism (DOP) の設定は1にする必要があります。
...
上記のSQLステートメントは、10行の空の仮想テーブルを生成します。表示するデータを指定していないため、各行は空です。
...
このクエリは、10行の仮想テーブルを生成します。各行の `greeting` 列には文字列 `Hello, World!` が含まれます。
...
- 例2：`GENERATOR()` 関数を使用して、条件に合致するランダムな数値を生成します。
