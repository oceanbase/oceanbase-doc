...
Table Functionとは、SQL言語で使用される関数の一種で、結果として1つのデータテーブルを返すことができます。従来のSQL関数がスカラー値しか返せないのとは異なり、Table Functionは複数の行や列を含むデータセットを返すことができます。

`TABLE(GENERATOR(N))` は実際にはデータを生成するわけではなく、連続したN行の「行」を生成するだけです。データを生成する必要がある場合は、`SELECT` の後に定数、変数、関数、UDFなどの具体的な式を追加する必要があります。`TABLE(GENERATOR(N))` は並列実行に使用でき、例えば並列 `SELECT` や並列DMLなどが挙げられます。ただし、`TABLE(GENERATOR(N))` が生成した行を読み取るスレッドは1つだけであり、つまり `TABLE(GENERATOR(N))` を含むDFOの `DOP=1` となります。
...
`GENERATOR()`関数は他のテーブルとも結合（JOIN）できます。例：
