...
| interval | `INTERVAL DAY TO SECOND` および `INTERVAL YEAR TO MONTH` データ型の値。間隔の単位は以下のとおりです： <ul><li> `YEAR` は年を表します。   </li><li> `MONTH` は月を表します。   </li><li> `DAY` は日を表します。   </li><li> `HOUR` は時間を表します。   </li><li> `MINUTE` は分を表します。   </li><li> `SECOND` は秒を表します。 </li></ul>   |
| fmt      | 出力形式を指定するためのオプションパラメータです。`fmt` パラメータを指定しない場合、`datetime` パラメータの値は以下の形式で `VARCHAR2` データ型に変換されます： <ul><li> `DATE`、`TIMESTAMP`、`TIMESTAMP WITH TIME ZONE`、`TIMESTAMP WITH LOCAL TIME ZONE` の値は、データベース内の日付時刻値のデフォルト形式に変換されます。各日付時刻型のデフォルト形式については、**日付時刻と間隔データ型** セクションを参照してください。   </li><li> `INTERVAL DAY TO SECOND` および `INTERVAL YEAR TO MONTH` データ型の値は、数値形式の間隔値に変換されます。 </li></ul>               |
...
* `TO_NCHAR` 関数を使用して、現在の日付を `YYYY/MM/DD` 形式と `DSDL` 形式で返します。
