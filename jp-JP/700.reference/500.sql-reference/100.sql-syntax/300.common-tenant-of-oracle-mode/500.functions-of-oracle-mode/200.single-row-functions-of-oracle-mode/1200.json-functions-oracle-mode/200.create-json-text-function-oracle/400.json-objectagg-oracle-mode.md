...
`JSON_OBJECTAGG()` 関数は、入力パラメータをキーと値のペアとして処理します。ここで、`key` と `value` はどちらもリスト式であり、すべてのキー・バリュー・ペアを1つのJSONオブジェクトに組み合わせて返します。
...
- `STRICT`：出力結果が有効なJSON形式のデータであるか検証します。
