...
`JSON_TABLE()` 関数は、リレーショナルテーブルとJSON構造データを関連付ける方法を提供します。SQLでは、仮想リレーショナルテーブルのように `JSON_TABLE()` の出力結果を使用できます。`JSON_TABLE()` は各JSON値に対して列出力を提供し、JSON内の配列については複数行（列）の出力を提供します。
...
- `FOR ORDINALITY` 句は、現在の出力行に行番号を提供するために使用され、`INT` 型の列です。ここで出力される値のタイプは4種類あります。`ORDINALITY` 列を除く他の3つは、`JSON_VALUE()`、`JSON_QUERY()`、`JSON_EXISTS()` の機能および動作と比較的近いです。
