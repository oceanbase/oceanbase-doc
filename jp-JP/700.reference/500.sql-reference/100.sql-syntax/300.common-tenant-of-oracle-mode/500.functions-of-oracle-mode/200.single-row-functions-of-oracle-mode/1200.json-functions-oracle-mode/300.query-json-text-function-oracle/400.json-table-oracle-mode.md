...
|slug|json_table-oracle-compatible-mode|

# JSON_TABLE

## 説明

`JSON_TABLE()` 関数は、リレーショナルテーブルとJSON構造データを関連付ける方法を提供します。SQL内で仮想リレーショナルテーブルのように `JSON_TABLE()` の出力結果を使用できます。`JSON_TABLE()` は各JSON値に対して列出力を提供し、JSON内の配列については複数行（列）の出力を提供します。
...
- `FORMAT JSON`：オプションです。`expr` のデータ型が `BLOB` の場合に使用する必要があります。
...
- `json_query_wrapper_clause`：`JSON_QEURY()` の `WRAPPER` 句と同じです。
- `FOR ORDINALITY` 句は、現在の出力行に行番号を提供するために使用され、`INT` 型の列となります。ここで出力される値のタイプは4種類あり、`ORDINALIY` 列を除く他の3種類は `JSON_VALUE()`、`JSON_QUERY()`、`JSON_EXISTS()` の機能および動作と比較的近いです。
- `[NESTED PATH ...]` 句は、ネスト構造を繰り返すものです。
