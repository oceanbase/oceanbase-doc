...
|slug|decode-oracle-compatible-mode|
...
* 一つ以上の `search` が `condition` の値と等しい場合、最初に `condition` の値と等しい `search` の後に続く `result` の値を返します。
...
`condition`、`search`、`result`、`default` は、数値データ型（`NUMBER`、`FLOAT`、`BINARY_FLOAT` または `BINARY_DOUBLE`）または文字データ型（`CHAR`、`VARCHAR2`、`NCHAR` または `NVARCHAR2`）の値または式です。
  ...
  <main id="notice" type='notice'>
    <h4>注意</h4>
    <p>最初の <code>result</code> が数値データ型の場合、返される <code>result</code> または <code>default</code> も数値データ型でなければなりません。そうでない場合は、エラーが発生します。</p>
  </main>
...
* `DECODE` を使用して、数値 `5*3-2` と `3*4-1` の大小を比較します。
...
* `DECODE` 関数を使用して、データに文字 `S` が含まれているかどうかを確認します。
  ...
  ```sql
  obclient> SELECT
      DECODE(INSTR('CLARK','S'), 0, 'Sを含まない', 'Sを含む') "CLARK",
      DECODE(INSTR('KING','S'), 0, 'Sを含まない', 'Sを含む') "KING",
      DECODE(INSTR('MILLER','S'), 0, 'Sを含まない', 'Sを含む') "MILLER",
      DECODE(INSTR('ADAMS','S'), 0, 'Sを含まない', 'Sを含む') "ADAMS",
      DECODE(INSTR('FORD','S'), 0, 'Sを含まない', 'Sを含む') "FORD",
      DECODE(INSTR('JONES','S'), 0, 'Sを含まない', 'Sを含む') "JONES"
      FROM DUAL;
  +-------------+-------------+-------------+----------+-------------+----------+
  | CLARK       | KING        | MILLER      | ADAMS    | FORD        | JONES    |
  +-------------+-------------+-------------+----------+-------------+----------+
  | Sを含まない    | Sを含まない    | Sを含まない    | Sを含む   | Sを含まない    | Sを含む   |
  +-------------+-------------+-------------+----------+-------------+----------+
  1 row in set
  ```