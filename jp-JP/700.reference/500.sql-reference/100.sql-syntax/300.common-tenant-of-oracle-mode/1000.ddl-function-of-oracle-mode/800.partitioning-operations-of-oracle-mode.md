...
OceanBaseデータベースのOracleモードにおけるパーティション操作には、再パーティション化（非パーティションテーブルをパーティションテーブルとサブパーティションテーブルに変換する）、パーティションの追加、パーティションの削除、およびパーティションのクリアが含まれます。

## 再パーティション化

再パーティション化の構文は次のとおりです：
...
以下の例は、テンプレート化されたサブパーティションテーブル `tbl4` にパーティション `p3` を追加するものです。パーティションの定義のみを指定すれば、サブパーティションの定義はテンプレートに従って自動的に埋め込まれます。
...
パーティションをクリアする構文は以下のとおりです：

```sql
ALTER TABLE table_name TRUNCATE PARTITION partition_name;
```

パーティションをクリアする例は以下のとおりです：
