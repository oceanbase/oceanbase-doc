...
ネストされたカーソルは、以下のいずれかの場合にのみ閉じられます：

* ユーザーが明示的にネストされたカーソルを閉じる場合
...
* 包囲するステートメントが `SELECT` ステートメントの場合、ネストされたカーソルは、クエリで定義された最外層の選択リスト、または別のネストされたカーソルの最外層の選択リストにも現れることができます。
...
* 関数のパラメータとして `CURSOR` 式を使用します。`emp` テーブルの特定の部門のすべての従業員を削除します。その部門に従業員がいなくなった場合、`dept` テーブルからその部門を削除します。
