...
`one_or_all` パラメータのオプション値は次のとおりです：

* `'one'`：ドキュメントに少なくとも1つのパスが存在する場合は1を返し、そうでない場合は0を返します。

* `'all'`：ドキュメントにすべてのパスが存在する場合は1を返し、そうでない場合は0を返します。
...
* `one_or_all` パラメータが `'one'` または `'all'` ではありません。
