...
|slug|cume_dist-mysql-compatible-mode|
...
この関数は、グループ（パーティション）内の行の累積分布値を計算し、`0` より大きく `1` 以下の値を返します。ソート後の現在の行の位置（そのデータより小さいか等しい行数）とパーティション内のすべての行の合計数の比率を計算することで、行の累積分布値を決定します。
...
3. `job_id` でグループ化し、給与が昇順に並べられた場合、各行データのウィンドウ内のソート列に占める割合を計算します。
