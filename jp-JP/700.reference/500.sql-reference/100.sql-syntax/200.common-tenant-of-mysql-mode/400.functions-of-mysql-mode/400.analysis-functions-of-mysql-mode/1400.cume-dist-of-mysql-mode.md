...
この関数は、グループ（パーティション）内での行の累積分布値を計算し、`0` より大きく `1` 以下の値を返します。ソート後の現在の行の位置（そのデータ以下の行数）とパーティション内の全行数の比率を計算することで、行の累積分布値を決定します。
...
3. `job_id` でグループ化し、給与の昇順で並べ替えた場合、各行データがウィンドウ内のソート列で占める割合を示します。
