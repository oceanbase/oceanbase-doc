...
`expr` 式の `n` 番目の値を返します。方向は `[ FROM { FIRST | LAST } ]` で指定し、デフォルトは `FROM FIRST` です。`NULL` 値を無視するかどうかを示すフラグが含まれます。そのウィンドウは統一された `analytic_clause` です。

ここで、`n` は正の数でなければなりません。`n` が `NULL` の場合、関数はエラーを返します。`n` がウィンドウ内の行数より大きい場合、この関数は `NULL` を返します。
