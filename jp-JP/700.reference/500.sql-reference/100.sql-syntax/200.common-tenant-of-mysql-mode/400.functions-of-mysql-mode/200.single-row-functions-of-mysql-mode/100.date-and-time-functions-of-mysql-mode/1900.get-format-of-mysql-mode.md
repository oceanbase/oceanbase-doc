...
この関数は、指定された書式の文字列を返します。`DATE_FORMAT` または `STR_TO_DATE` 関数と併用することができます。最初のパラメータと2番目のパラメータの組み合わせによって、次の表に示すような文字列形式が生成されます（結果で使用されるフォーマット指定子の説明については、[DATE_FORMAT](../100.date-and-time-functions-of-mysql-mode/1000.date-format-of-mysql-mode.md) 関数を参照してください）。
