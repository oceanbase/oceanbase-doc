...
この関数は指定された形式の文字列を返します。`DATE_FORMAT`または`STR_TO_DATE`関数と一緒に使用できます。最初のパラメータと2番目のパラメータの異なる値の組み合わせにより、次の表に示す文字列形式が生成されます（結果で使用されるフォーマット指定子の説明については、[DATE_FORMAT](../100.date-and-time-functions-of-mysql-mode/1000.date-format-of-mysql-mode.md)関数を参照してください）。
