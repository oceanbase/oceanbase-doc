...
`TABLE(GENERATOR(N))` は実際にはデータを生成するわけではなく、連続したN個の「行」を生成します。データを生成する必要がある場合は、`SELECT` の後に定数、変数、関数、UDFなどの具体的な式を追加する必要があります。`TABLE(GENERATOR(N))` は並列実行に利用できます。例えば、並列 `SELECT` や並列DMLなどですが、`TABLE(GENERATOR(N))` が生成する行を読み取るスレッドは1つだけになります。つまり、`TABLE(GENERATOR(N))` を含むDFOの `DOP=1` となります。

## 例

以下の例は、`GENERATOR()` 関数を使用して条件に合致するランダム数を生成する方法です。
