...
`TABLE(GENERATOR(N))` は実際にはデータを生成するわけではなく、連続したN行の「行」を生成するだけです。データを生成する必要がある場合は、`SELECT` の後に定数、変数、関数、UDFなどの具体的な式を追加する必要があります。`TABLE(GENERATOR(N))` は並列実行に利用できます。例えば、並列 `SELECT` や並列DMLなどです。ただし、`TABLE(GENERATOR(N))` で生成された行を読み取るスレッドは1つだけになります。つまり、`TABLE(GENERATOR(N))` を含むDFOの `DOP=1` となります。
