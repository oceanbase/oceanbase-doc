...
このステートメントは、OceanBaseデータベースのログインユーザーのユーザー名を変更するために使用されます。
  ...
  <main id="notice" type='explain'>
    <h4>説明</h4>
    <p>このコマンドを使用してユーザーをリネームするには、グローバルな <code>CREATE USER</code> 権限が付与されている必要があります。</p>
  </main>
...
2. 次のコマンドを実行して、`sqluser01` ユーザーのユーザー名を `obsqluser01` に変更します。

   ```sql
   obclient> RENAME USER 'sqluser01' TO 'obsqluser01';
   ```

3. 変更後、次のコマンドを実行してユーザーを確認します。結果は次のとおりです。
