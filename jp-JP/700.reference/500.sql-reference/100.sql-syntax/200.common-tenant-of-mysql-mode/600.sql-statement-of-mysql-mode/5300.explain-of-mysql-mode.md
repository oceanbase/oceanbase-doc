...
| SET statement_id | 現在のクエリに文字列タグを設定し、後からそのSQLの実行計画情報を検索しやすくするために使用されます。`SET statement_id` を指定しない場合、デフォルトでは空文字列がタグとして使用されます。 |
| PRETTY \| PRETTY_COLOR | 実行計画ツリー内の親ノードと子ノードを線または色付きの線で結び、実行計画をより読みやすく表示します。|
| BASIC                         | 出力する計画の基本情報を指定します。例えば、演算子ID、演算子名、参照されるテーブル名などです。   |
