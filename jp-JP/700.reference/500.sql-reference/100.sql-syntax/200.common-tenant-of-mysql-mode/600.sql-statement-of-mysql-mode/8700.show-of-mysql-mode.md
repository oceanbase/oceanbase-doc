...
| \[FULL\] PROCESSLIST                                                           | 現在のテナントのプロセスリストを確認します。具体的には以下のとおりです： <ul><li><code>SHOW PROCESSLIST</code>は簡略化されたプロセスリストを表示します。具体的には以下のとおりです：  <ul><li><code>Id</code>：プロセスのID、つまり現在のセッションのクライアントセッションIDです。このIDはクライアント側でのセッションの一意の識別子です。</li><li><code>User</code>：接続されているユーザー名。</li><li><code>Host</code>：クライアント側のIPアドレスとポート番号（ODP経由で接続した場合はODPのIPアドレスとポート番号）。</li><li><code>db</code>：アクセスするデータベースの名前。</li><li><code>Command</code>：現在実行中のコマンドタイプ。<code>Query</code>、<code>Sleep</code>などが取り得る値です。</li><li><code>Time</code>：現在のコマンド実行時間。単位は秒です。コマンドが再試行された場合、ゼロリセットされてから再計算されます。</li><li><code>State</code>：現在のセッションの状態。<code>SLEEP</code>、<code>ACTIVE</code>などが取り得る値です。</li><li><code>Info</code>：現在実行中のコマンドを表示します。長さは100文字に制限され、それを超える部分は切り捨てられます。</li></ul>  </li><li><code>SHOW FULL PROCESSLIST</code>：完全なプロセスリストを表示します。各プロセスの詳細情報を含みます。具体的には以下のとおりです：  <ul><li><code>Id</code>：プロセスのID、つまり現在のセッションのクライアントセッションIDです。このIDはクライアント側でのセッションの一意の識別子です。</li><li><code>User</code>：接続されているユーザー名。</li><li><code>Tenant</code>：接続されているテナント。</li><li><code>Host</code>：クライアント側のIPアドレスとポート番号（ODP経由で接続した場合はODPのIPアドレスとポート番号）。</li><li><code>db</code>：アクセスするデータベースの名前。</li><li><code>Command</code>：現在実行中のコマンドタイプ。<code>Query</code>、<code>Sleep</code>などが取り得る値です。</li><li><code>Time</code>：現在のコマンド実行時間。単位は秒です。コマンドが再試行された場合、ゼロリセットされてから再計算されます。</li><li><code>State</code>：現在のセッションの状態。<code>SLEEP</code>、<code>ACTIVE</code>などが取り得る値です。</li><li><code>Info</code>：現在実行中のコマンドを表示します。</li><li><code>Ip</code>：サーバーのIPアドレス。</li><li><code>Port</code>：SQLポート番号。</li></ul>  </li></ul>  <main id="notice" type='explain'><h4>説明</h4><p><code>SHOW PROCESSLIST</code> ステートメントを使用して、現在のデータベース内のセッション数とセッションIDの詳細情報を確認するには、<a href="../../../../1200.database-proxy/1500.view-tenant-sessions.md">テナントセッションの確認</a>を参照してください。</p></main>   |
| TABLEGROUPS                                                                     | テーブルグループ情報を表示します。  |
| {GLOBAL \| SESSION \| LOCAL} STATUS                                             | セッションの状態を表示します。 |
| TENANT \[STATUS\]                                                               | 現在のテナント名を表示します。`STATUS` キーワードはテナントの読み書き状態を表示するために使用されます。  |
...
| GRANTS opt_for_grant_user                                                       | 指定ユーザーの権限を確認します。ユーザー名を指定しない場合、現在のユーザーの権限が表示されます。現在のユーザーは自身の権限を確認できます。他の指定ユーザーの権限を確認するには、`mysql.user` テーブルに対する `SELECT` 権限が必要です。 |
...
| PROCEDURE CODE proc_name | ストアドプロシージャ <code>proc_name</code> の内部実装を確認します。<main id="notice" type='notice'><h4>注意</h4><p>現在のバージョンではこのキーワード構文のみサポートされており、機能は有効ではありません。</p></main>|
| FUNCTION CODE func_name | ストアドファンクション <code>func_name</code> の内部実装を確認します。<main id="notice" type='notice'><h4>注意</h4><p>現在のバージョンではこのキーワード構文のみサポートされており、機能は有効ではありません。</p></main>|
| ENGIN engine_name | ストレージエンジンに関する操作情報を確認します。<main id="notice" type='notice'><h4>注意</h4><p>現在のバージョンではこのキーワード構文のみサポートされており、機能は有効ではありません。</p></main>|
| OPEN TABLES | サーバーが現在開いているテーブルとその状態情報を確認するために使用されます。<main id="notice" type='notice'><h4>注意</h4><p>現在のバージョンではこのキーワード構文のみサポートされており、機能は有効ではありません。</p></main> |
| CREATE CATALOG external_catalog_name | Catalog作成文を確認するために使用されます。|
| CATALOGS | 現在のテナントのCatalogを確認するために使用されます。|
| CREATE LOCATION location_name | Locationオブジェクト作成文を確認するために使用されます。<main id="notice" type='explain'><h4>説明</h4><p>OceanBaseデータベースV4.4.x系では、V4.4.1バージョンからこの構文がサポートされています。</p></main>|
| LOCATIONS | 現在のテナントのLocationオブジェクトを確認するために使用されます。<main id="notice" type='explain'><h4>説明</h4><p>OceanBaseデータベースV4.4.x系では、V4.4.1バージョンからこの構文がサポートされています。</p></main>|
| FILES IN LOCATION @location_name['/path'] [PATTERN = 'regex_pattern'] | Location配下に存在するファイルを確認するために使用されます。<ul><li>`['/path']`はオプションで、サブディレクトリを示します。</li><li>`PATTERN = 'regex_pattern'`はオプションで、Locationディレクトリ内のファイルをフィルタリングするための正規表現を指定するために使用されます。</li></ul> <main id="notice" type='explain'><h4>説明</h4><p><ul><li><code>SHOW FILES IN LOCATION</code> 文を実行するには、現在のユーザーがLocationオブジェクトに対する <code>READ</code> 権限を持っている必要があります。</li><li>OceanBaseデータベースV4.4.x系では、V4.4.1バージョンからこの構文がサポートされています。</li></ul></p></main>|
...
* `SHOW COLUMNS` を使用して、テーブル `tbl2` の列情報を確認します。
...
* `SHOW EXTENDED COLUMNS` を使用して、テーブル `tbl2` の隠れた列情報を確認します。
...
* `SHOW INDEX` を使用して、テーブル `tbl2` のインデックス情報を確認します。
...
* `SHOW EXTENDED INDEX` を使用して、テーブル `tbl2` の隠れたインデックス情報を確認します。
