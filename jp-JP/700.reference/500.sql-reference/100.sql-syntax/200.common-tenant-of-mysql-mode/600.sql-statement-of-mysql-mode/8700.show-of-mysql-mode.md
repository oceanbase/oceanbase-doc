...
|                                       パラメータ                           |                        説明         |
|--------------------------------------------------|-------------------------------------------------------------------|
|\[EXTENDED\] \[FULL\] TABLES \{FROM \| IN} database_name                                    | `database_name`データベース内のすべてのテーブルを表示します。 <ul><li>`EXTENDED`キーワードは隠れたテーブルを表示するために使用されます。OceanBaseデータベースの現在のバージョンでは隠れたテーブルが生成されないため、このパラメータには実際の効果はありません。</li><li>`FULL`キーワードはテーブルタイプを表示するために使用されます。</li></ul>|
| {DATABASES \| SCHEMAS} \[STATUS\]                                               | 現在のテナント内のすべてのデータベースを表示します。 `STATUS`キーワードはデータベースの読み書き属性を表示するために使用されます。|
|\[EXTENDED\] \[FULL\] {COLUMNS \| FIELDS} {FROM \| IN} rel_name                              | `rel_name`リレーションの列を表示します。 <ul><li>`EXTENDED`キーワードは、データベース内部で使用される隠れた列の情報を表示するために使用されます。</li><li>`FULL`キーワードは、列の照合順序、権限、コメントを表示するために使用されます。</li></ul>|
...
|\[EXTENDED\] {INDEX \| INDEXES \| KEYS} {FROM \| IN} rel_name \[{FROM \| IN} database_name\] | `rel_name`リレーション上のインデックスまたはキーを表示します。 `EXTENDED`キーワードは、データベース内部で使用される隠れたインデックスの情報を表示するために使用されます。 |
|\[FULL\] PROCESSLIST  | 現在のテナントのプロセスリストを表示します。具体的には以下のとおりです： <ul><li><code>SHOW PROCESSLIST</code> 簡易なプロセスリストを表示します。具体的には以下のとおりです：  <ul><li><code>Id</code>：プロセスのID、つまり現在のセッションのクライアントセッションIDです。このIDは、クライアント上でのセッションの一意の識別子です。</li><li><code>User</code>：接続ユーザー名。</li><li><code>Host</code>：クライアント側のIPアドレスとポート番号（ODP経由で接続する場合はODPのIPアドレスとポート番号）。</li><li><code>db</code>：アクセスするデータベース名。</li><li><code>Command</code>：現在実行されているコマンドタイプ。<code>Query</code>、<code>Sleep</code> などの値があります。</li><li><code>Time</code>：現在のコマンド実行時間。単位は秒です。コマンドが再試行された場合、ゼロにリセットされて再計算されます。</li><li><code>State</code>：現在のセッション状態。<code>SLEEP</code>、<code>ACTIVE</code> などの値があります。</li><li><code>Info</code>：現在実行中のコマンドを表示します。長さ制限は100文字で、超過分は切り捨てられます。</li></ul>  </li><li><code>SHOW FULL PROCESSLIST</code>：各プロセスの詳細情報を含む完全なプロセスリストを表示します。具体的には以下のとおりです：  <ul><li><code>Id</code>：プロセスのID、つまり現在のセッションのクライアントセッションIDです。このIDは、クライアント上でのセッションの一意の識別子です。</li><li><code>User</code>：接続ユーザー名。</li><li><code>Tenant</code>：接続したテナント。</li><li><code>Host</code>：クライアント側のIPアドレスとポート番号（ODP経由で接続する場合はODPのIPアドレスとポート番号）。</li><li><code>db</code>：アクセスするデータベース名。</li><li><code>Command</code>：現在実行されているコマンドタイプ。<code>Query</code>、<code>Sleep</code> などの値があります。</li><li><code>Time</code>：現在のコマンド実行時間。単位は秒です。コマンドが再試行された場合、ゼロにリセットされて再計算されます。</li><li><code>State</code>：現在のセッション状態。<code>SLEEP</code>、<code>ACTIVE</code> などの値があります。</li><li><code>Info</code>：現在実行中のコマンドを表示します。</li><li><code>Ip</code>：サーバーのIPアドレス。</li><li><code>Port</code>：SQLポート番号。</li></ul>  </li></ul>  <main id="notice" type='explain'><h4>説明</h4><p><code>SHOW PROCESSLIST</code> ステートメントを使用して、現在のデータベース内のセッション数とセッションのIDの詳細情報を照会するには、<a href="../../../../1200.database-proxy/1500.view-tenant-sessions.md">テナントセッションの表示</a> を参照してください。</p></main>|
...
| CREATE USER user_name                                                           | ユーザー作成ステートメントを確認します。|
| WARNINGS \[LIMIT N \| LIMIT N,M\]                                               | アラートを確認します。 |
| COUNT(\*) WARNINGS                                                              | アラート数を確認します。  |
| ERRORS \[LIMIT N \| LIMIT N,M\]                                                 | エラーを確認します。 |
| COUNT(\*) ERRORS                                                                | エラー数を確認します。            |
| GRANTS opt_for_grant_user                                                       | 指定ユーザーの権限を確認します。 ユーザー名が指定されていない場合、現在のユーザーの権限が表示されます。現在のユーザーは自身の権限を確認できます。 他の指定ユーザーの権限を確認するには、`mysql.user` に対する `SELECT` 権限が必要です。 |
| \[STORAGE\] ENGINES                                                             | ストレージエンジンを確認します。OceanBaseデータベースのストレージエンジンは `OceanBase` です。    |
| PRIVILEGES                                                                      | MySQLの各権限の説明を表示します。     |
| RECYCLEBIN                                                                      | ごみ箱を表示します。     |
| SEQUENCE |作成されたシーケンスを表示します。|
| PROFILE |前回実行されたクエリのパフォーマンス詳細を表示します。現在のバージョンではこのキーワード構文のみがサポートされており、機能は有効ではありません。|
| PROFILES |分析が有効になって以来のすべてのクエリの要約リストを表示します。現在のバージョンではこのキーワード構文のみがサポートされており、機能は有効ではありません。|
| PLUGINS |サーバープラグインに関する情報を表示します。現在のバージョンではこのキーワード構文のみがサポートされており、機能は有効ではありません。|
