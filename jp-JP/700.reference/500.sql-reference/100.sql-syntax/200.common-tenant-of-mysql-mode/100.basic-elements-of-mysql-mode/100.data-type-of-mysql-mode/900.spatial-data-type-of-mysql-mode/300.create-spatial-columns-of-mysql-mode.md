...
以下の例では、さまざまな幾何型を持つテーブルの作成方法を示しています：
...
空間列に関する制約事項は以下のとおりです：

* 空間列を定義する際に、SRIDを明示的に指定できます。列にSRIDが定義されていない場合、クエリ時にオプティマイザーは空間インデックスを選択しませんが、挿入/更新時にはインデックスレコードが生成されます。

* 空間列に`NOT NULL`制約を指定し、さらにSRIDを指定した後であれば、その空間列に空間インデックスを定義できます。つまり、SRIDが定義された列でなければ空間インデックスを使用できません。

* 空間列にSRIDを定義した後、他のSRID値を挿入するとエラーが発生します。

`SRID`に関する制約事項は以下のとおりです：
