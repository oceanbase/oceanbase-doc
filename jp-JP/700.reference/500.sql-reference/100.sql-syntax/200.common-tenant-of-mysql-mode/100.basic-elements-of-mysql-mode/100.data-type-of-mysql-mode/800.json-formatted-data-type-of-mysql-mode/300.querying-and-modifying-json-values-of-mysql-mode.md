...
* 「-\\>」記号を使用して、JSONデータ内の二重引用符付きキー値をインポートします。
...
* 「.」記号は、アクセスするキー名を表します。パス式では、引用符なしの名前（例えばスペース）は無効であるため、キー名は必ず二重引用符で指定する必要があります。
...
パスセグメントは、ピリオド（.）文字で区切られます。配列内の要素は \[N\] で表され、ここでNは非負の整数です。キー名は、二重引用符で囲まれた文字列または有効なECMAScript識別子でなければなりません。
