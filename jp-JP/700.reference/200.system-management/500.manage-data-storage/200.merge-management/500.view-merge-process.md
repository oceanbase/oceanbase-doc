...
メジャーコンパクションの進捗状況はビューで確認できますが、通常、メジャーコンパクションにかかる時間は、前回のメジャーコンパクションと今回のメジャーコンパクションの間で発生したデータ変更量によって決まります。データ変更量が大きい場合、メジャーコンパクションにかかる時間も長くなります。また、メジャーコンパクションを実行するスレッド数や、メジャーコンパクション時のクラスタ負荷なども、メジャーコンパクションの所要時間に影響します。

## システムテナントによるすべてのテナントのメジャーコンパクション情報の確認

システムテナントは、ビューを使用してすべてのテナントのテナントレベルおよびパーティションレベルのメジャーコンパクション情報を確認できます。
...
2. 以下のステートメントを実行して、メジャーコンパクション情報を確認します。
    ...
    * ビュー`CDB_OB_ZONE_MAJOR_COMPACTION`または`CDB_OB_MAJOR_COMPACTION`を使用して、テナントレベルのメジャーコンパクション情報を確認します。
      ...
      実行結果で`status`列を確認します。一部のフィールドの説明は以下の表のとおりです。
      ...
      実行結果で`status`列を確認します。一部のパラメータの説明は以下の表のとおりです。
      ...
      |LAST_FINISH_TIME|前回のメジャーコンパクション終了時間。|
      |START_TIME|メジャーコンパクション開始時間。|
      |STATUS|メジャーコンパクションステータス：<ul><li>`IDLE`：メジャーコンパクション中ではありません</li>  <li>`COMPACTING`：メジャーコンパクション中</li>  <li>`VERIFYING`：チェックサムの検証中</li></ul>|
    ...
    * ビュー`GV$OB_MERGE_INFO`または`GV$OB_TABLET_COMPACTION_HISTORY`を使用して、パーティションレベルのメジャーコンパクション情報を確認します。
      ...
      * `META_MAJOR_MERGE`：特殊なCompactionタイプで、指定された時点以前のデータをMeta Major SSTableに合成します。そのデータ形式はMajor SSTableと同じで、マルチバージョンデータや未コミットトランザクションデータは含まれません。
      ...
      ビュー`GV$OB_MERGE_INFO`の詳細なフィールド説明については、[GV$OB_MERGE_INFO](../../../700.system-views/300.system-view-of-sys-tenant/300.performance-view-of-sys-tenant/1800.gv-ob_merge_info-of-sys-tenant.md)を参照してください。
      ...
      * `META_MAJOR_MERGE`：特殊なCompactionタイプで、指定された時点以前のデータをMeta Major SSTableに合成します。そのデータ形式はMajor SSTableと同じで、マルチバージョンデータや未コミットトランザクションデータは含まれません。
      ...
      ビュー`GV$OB_TABLET_COMPACTION_HISTORY`の詳細なフィールド説明については、[GV$OB_TABLET_COMPACTION_HISTORY](../../../700.system-views/300.system-view-of-sys-tenant/300.performance-view-of-sys-tenant/4300.gv-ob_tablet_compaction_history-of-sys-tenant.md)を参照してください。

## ユーザーテナントによる自身のテナントのメジャーコンパクション情報の確認

ユーザーテナントはビューを通じて自身のテナントのメジャーコンパクション情報を確認することもできます。
...
2. 以下のステートメントを実行して、メジャーコンパクション情報を確認します。
    ...
    * ビュー `DBA_OB_ZONE_MAJOR_COMPACTION` または `DBA_OB_MAJOR_COMPACTION` を使用して、テナントのテナントレベルのメジャーコンパクション情報を確認します。
      ...
      ビュー `DBA_OB_ZONE_MAJOR_COMPACTION` は、テナント内の各ゾーンのメジャーコンパクション情報を表示します。
      ...
      実行結果で `status` 列を確認し、一部のパラメータについては以下の表のように説明されています。
      ...
      |フィールド|説明|
      |---|---|
      |FROZEN_SCN|直近のメジャーコンパクションのバージョン番号|
      |FROZEN_TIME|`FROZEN_SCN`の読み取り可能な時間型。|
      |GLOBAL_BROADCAST_SCN |グローバルブロードキャストのメジャーコンパクションバージョン。|
      |LAST_SCN|前回完了したメジャーコンパクションのバージョン。|
      |LAST_FINISH_TIME|前回のメジャーコンパクション終了時間。|
    ...
    * ビュー `GV$OB_TABLET_COMPACTION_HISTORY` を使用して、テナントのパーティションレベルのメジャーコンパクション情報を確認します。
      ...
      * `META_MAJOR_MERGE`：特殊なCompactionタイプで、指定された時点以前のデータをMeta Major SSTableに合成します。そのデータ形式はMajor SSTableと同じで、マルチバージョンデータやコミットされていないトランザクションデータは含まれません。
      ...
      ビュー `GV$OB_TABLET_COMPACTION_HISTORY` の他のフィールドの詳細については、[GV$OB_TABLET_COMPACTION_HISTORY](../../../700.system-views/400.system-view-of-mysql-mode/300.performance-view-of-mysql-mode/4200.gv-ob_tablet_compaction_history-of-mysql-mode.md)を参照してください。
    ...
    * ビュー `DBA_OB_ZONE_MAJOR_COMPACTION` または `DBA_OB_MAJOR_COMPACTION` を使用して、テナントのテナントレベルのメジャーコンパクション情報を確認します。
      ...
      実行結果で `status` 列を確認すると、一部のパラメータについては以下の表のように説明されています。
      ...
      |IS_SUSPENDED            |<ul><li>`YES`：メジャーコンパクションが一時停止しています</li><li>`NO`：メジャーコンパクションは一時停止されていません</li></ul>|
      |INFO|メジャーコンパクション情報を表示します。|
    ...
    * ビュー `GV$OB_TABLET_COMPACTION_HISTORY` を使用して、テナントのパーティションレベルのメジャーコンパクション情報を確認します。
      ...
      * `META_MAJOR_MERGE`：特殊なCompactionタイプで、指定された時点以前のデータをMeta Major SSTableに合成します。そのデータ形式はMajor SSTableと同じで、マルチバージョンデータやコミットされていないトランザクションデータは含まれません。
      ...
      ビュー `GV$OB_TABLET_COMPACTION_HISTORY` の他のフィールドの詳細については、[GV$OB_TABLET_COMPACTION_HISTORY](../../../700.system-views/500.system-view-of-oracle-mode/300.performance-view-of-oracle-mode/900.gv-ob_tablet_compaction_history-of-oracle-mode.md)を参照してください。
