...
`sys`テナントで、`oceanbase.gv$sysstat`ビューを照会することで、指定されたマシンのメモリ使用状況を確認できます。例：
...
`sys`テナントで、`oceanbase.GV$OB_TENANT_MEMORY`ビューを照会することで、すべてのテナントのメモリ使用量を確認できます。
...
`sys` テナント内で、`oceanbase.GV$OB_MEMORY` ビューを照会することで、指定したテナントのメモリ使用状況を確認できます。
...
### 特定のメモリモジュール内のメモリ使用状況を確認する

`sys`テナントで、`oceanbase.GV$OB_MEMORY`を照会することで、特定のメモリモジュール内のメモリ使用状況を確認できます。また、マシンやテナントなどの情報を指定して照会することも可能です。
...
2. 以下のコマンドを実行して、ログファイルが保存されているディレクトリに移動します。
...
3. 以下のコマンドを実行して、OBServerノードの総メモリ使用状況を確認します。
   ...
   ログ情報内の一部パラメータの説明は、以下の表のとおりです。

   |      パラメータ       |             説明              |
   |---------------|-----------------------------|
   | limit         | OBServerノードの総メモリ制限              |
   | hold          | OBServerノードがオペレーティングシステムから取得したすべてのメモリ     |
   | used          | OBServerノードが実際に使用しているメモリ（キャッシュを含む） |
...
3. 以下のコマンドを実行して、各テナントのメモリ使用量を確認します。
...
3. 以下の手順を実行して、`tenant_id` が `1002` のテナントのメモリ使用量を確認します。
...
3. 以下の手順を実行して、`tenant_id` が `1002` のテナントの `DEFAULT_CTX_ID` モジュールのメモリ使用状況を確認します。
