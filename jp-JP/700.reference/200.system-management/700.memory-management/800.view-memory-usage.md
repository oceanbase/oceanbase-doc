...
本記事では、仮想テーブルとログ内容を用いてメモリ使用状況を確認する方法について説明します。

## ビューによるクエリ

### OBServerノードの総メモリの確認

`sys`テナントで、`oceanbase.gv$sysstat`ビューをクエリすることで、指定したマシンのメモリ使用量を確認できます。例：
...
`sys`テナントで、`oceanbase.GV$OB_MEMORY`ビューをクエリすることで、指定したテナントのメモリ使用状況を確認できます。
...
### 特定メモリモジュール内のメモリ使用状況の確認

`sys`テナントで、`oceanbase.GV$OB_MEMORY`をクエリすることで、特定のメモリモジュール内のメモリ使用状況を確認できます。マシンやテナントなどの情報を指定してクエリすることも可能です。
...
2. 以下のコマンドを実行し、ログファイルがあるディレクトリに移動します。

   ```shell
   cd ~/oceanbase/log
   ```

3. 以下のコマンドを実行し、OBServerノードの総メモリ使用状況を確認します。
   ...
   ログ情報に含まれる一部のパラメータについては、次の表のとおり説明します。

   |      パラメータ       |             説明              |
   |---------------|-----------------------------|
   | limit         | OBServerノードの総メモリ制限              |
   | hold          | OBServerノードがオペレーティングシステムから取得したすべてのメモリ     |
   | used          | OBServerノードが実際に使用しているメモリ(Cacheを含む) |
...
3. 以下のコマンドを実行し、各テナントのメモリ使用量を確認します。
...
3. 以下の操作を実行し、`tenant_id` が `1002` のテナントのメモリ使用量を確認します。
