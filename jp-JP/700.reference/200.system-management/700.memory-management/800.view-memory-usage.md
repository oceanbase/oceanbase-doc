...
本記事では、仮想テーブルとログ内容を使用してメモリ使用状況を確認する方法について説明します。

## ビューのクエリ

### OBServerノードの総メモリの確認

`sys`テナントで、`oceanbase.gv$sysstat`ビューをクエリすることで、指定されたマシンのメモリ使用状況を確認できます。例：
...
`sys`テナントで、`oceanbase.GV$OB_TENANT_MEMORY`ビューをクエリすることで、すべてのテナントのメモリ使用量の合計を確認できます。
...
`sys` テナント内で、`oceanbase.GV$OB_MEMORY` ビューをクエリすることで、指定したテナントのメモリ使用状況を確認できます。
...
### 特定のメモリモジュールのメモリ使用状況の確認

`sys`テナントで、`oceanbase.GV$OB_MEMORY`をクエリすることで、特定のメモリモジュールのメモリ使用状況を確認できます。マシンやテナントなどの情報を指定してクエリすることも可能です。
...
2. 以下のコマンドを実行して、ログファイルがあるディレクトリに移動します。

   ```shell
   cd ~/oceanbase/log
   ```

3. 以下のコマンドを実行して、OBServerノードの総メモリ使用状況を確認します。
   ...
   ログ情報に含まれる一部のパラメータについては、次の表のように説明します。

   |      パラメータ       |             説明              |
   |---------------|-----------------------------|
   | limit         | OBServerノードの総メモリ制限              |
   | hold          | OBServerノードがオペレーティングシステムから取得したすべてのメモリ     |
   | used          | OBServerノードで実際に使用されているメモリ(Cacheを含む) |
   | freelist_hold | OBServerノードで使用されていないメモリ             |

### 各テナントのメモリ使用量の合計を確認する
...
3. 以下のコマンドを実行して、各テナントのメモリ使用量の合計を確認します。
...
3. 以下の操作を実行して、`tenant_id` が `1002` のテナントのメモリ使用量の合計を確認します。

   1. Vimを使用して `observer.log` を開きます。

   2. `ob_malloc_allocator.cpp.*1002` を検索します。
...
3. 以下の操作を実行して、`tenant_id` が `1002` のテナントの `DEFAULT_CTX_ID` モジュールのメモリ使用状況を確認します。

   1. Vimを使用して `observer.log` を開きます。

   2. `MEMORY.*1002.*DEFAULT_CTX_ID` を検索します。
