...
# 常態化Memleakによるメモリリーク診断メカニズム

OceanBaseデータベースは、Memleakツールを用いてメモリリークを診断します。その仕組みは、追跡モジュールのメモリ割り当てスタックを記録し、特定のスタックの累積回数が一定期間にわたって継続的に増加する場合、そのスタックに対応するメモリ割り当てコンテキストでメモリリークが発生している可能性があると判断するものです。しかし、Memleakを有効にするタイミングは通常、メモリリークが発生した後です。Memleakを有効にした後にメモリリークが止まった場合、リークが発生したモジュールを追跡することはできず、再現を待つしかありません。そのため、常態化されたMemleakを使用し、メモリ割り当て情報のサンプルをリアルタイムで保存し、サンプリング結果に基づいてメモリリークが発生する可能性のあるモジュールを分析する必要があります。

常態化Memleakは、パラメータ `_min_malloc_sample_interval` と `_max_malloc_sample_interval` によってメモリ割り当て情報のサンプリングレートを制御します。

|           パラメータ             |     説明                        | 値の範囲     | デフォルト値  |
|-----------------------------|--------------------------------|-------------|--------|
| _min_malloc_sample_interval | 連続する2回のサンプリング間でのob_mallocの最小回数 | [1,10000]  | 16     |
| _max_malloc_sample_interval | 連続する2回のサンプリング間でのob_mallocの最大回数 | [1,10000]  | 256   |

パラメータの設定によるメモリ割り当て情報のサンプリングレートの制御は、以下の3つのモードに分けられます：
...
<main id="notice" type='notice'>
     <h4>注意</h4>
     <ul>
     <li>OceanBaseデータベースでは、名前が「_」で始まるパラメータは隠れたパラメータと呼ばれ、開発者が障害のトラブルシューティングや緊急な運用保守時にのみ使用します。</li>
     <li>パラメータの値を設定する際には、以下の要件を満たす必要があります：<code>_min_malloc_sample_interval <= _max_malloc_sample_interval</code>。</li>
     <li>パラメータ <code>_min_malloc_sample_interval</code> および <code>_max_malloc_sample_interval</code> は、ユーザーが無闇に変更することは推奨されません。不適切な変更はクラスタのパフォーマンスを低下させる可能性があります。</li>
     </ul>
</main>

## ノーマライズドMemleakを使用したメモリリーク診断
...
2. 仮想テーブル`__all_virtual_malloc_sample_info`を使用して、メモリリークが疑われるモジュールのメモリ割り当て情報を照会します。
   ...
   以下では、モジュール`glibc_malloc`を例に、そのメモリ割り当て情報を照会します：
    ...
    仮想テーブルを照会し、分析の結果、`0x5f8bf23 0x5c5a5bd 0x5c52ec3 0x198ed1c6 0x5d788ba 0x1d8e28a5 0x1d8e2919 0x187f31a6 0x188a867a 0x188aa126 0x15c53d3f 0x16b06ae2 0x16b369ef 0x16b32d7e 0x15c8bcf8 0x15c8606a` のスタックの`alloc_bytes`が最大であることから、メモリリークが発生している可能性が高いと判断されます。
    ...
    |alloc_bytes  |   bigint(20)                  |  メモリ割り当て総サイズ  |

3. 仮想テーブルの照会結果に基づき、`alloc_bytes`が大きいスタック`back_trace`を分析します：
    ...
    `addr2line`コマンドを使用して`alloc_bytes`が最大のスタックを解析し、技術サポート担当者に連絡して、そのスタックでメモリリークが発生していないか協力して分析します。