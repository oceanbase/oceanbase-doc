...
**第三に**：全量ダイレクトロードロジックを使用してデータを一括インポートできます。この方法でデータをインポートしたテーブルは、マージを行わなくても最適なカラムストアのスキャン性能を発揮します。全量ダイレクトロードをサポートするツールには、obloaderやネイティブの load data コマンドがあります。
