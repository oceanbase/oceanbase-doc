...
- **標準的なシナリオ**
  + **リアルタイムレポート**：OceanBaseのリアルタイム取引データとオフラインワイドテーブルを組み合わせます。
...
- APシナリオにおいて、OceanBaseがCatalogを通じてETL不要の連携クエリを実現するコア機能を習得します
- HMS Catalogを熟知し、Hive / Icebergデータにアクセスします
...
- 権限の準備：
  + Hive：HMS読み取り権限 + HDFSファイル読み取り権限（Location設定を通じて）。詳細については、[HMS Catalog](../400.data-lake/200.data-lake-integration/100.hive/100.load-hive-via-catalog.md)を参照してください。
...
-- 新しいデータベースを作成する（オプション。既存のデータベースの使用も可能です）
...
<main id="notice" type='notice'>
  <h4>注意</h4>
  <p>Hiveテーブルのevent_dateはstring型なので、STR_TO_DATE()を使用してDATE型に変換し、型を一致させる必要があります。</p>
</main>
