...
OceanBaseデータベースは、ユーザーが実行したすべてのクエリの実行プラン(物理プランとロジックプランを含む)を保存し、ユーザーが問題解決に使用できるようにします。ユーザーが過去のクエリプランを解釈しやすくするため、OceanBaseデータベースは関連するDBMSパッケージ形式のプランを提供しています。explainのプランとは異なり、ユーザーが実行したプランは、ユーザーが接続を切断した後もデータベースに保持されます。ユーザーがクラスタを再起動した場合にのみ、データベースは保存されたクエリプランをクリーンアップします。
