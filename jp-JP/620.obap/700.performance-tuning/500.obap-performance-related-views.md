...
本記事では、AP性能問題の診断と分析において利用できる主要なビューについて説明します。

## GV$SQL_WORKAREA

**ビューの紹介：**

SQL自動メモリ管理が実行を担当するすべてのOperatorのworkarea統計情報を表示します。

このビューをクエリすることで、実行計画の各演算子のメモリ使用量やディスクに落ちたデータ量を把握できます。その後、ビューのデータに基づいてテナントのsql_work_areaサイズを適切に調整し、計画をメモリ内で完全に実行してクエリ性能を向上させることができます。

**よく使われるフィールドの解説：**

|**プロパティ名**|**説明**|
|---|---|
| `CHILD_NUMBER` | この演算子が属する計画ID |
| `OPERATION_TYPE` | 物理演算子名 |
| `OPERATION_ID` | 物理演算子番号 |
| `LAST_MEMORY_USED` | この演算子の前回の平均メモリ使用量 |
| `LAST_DEGREE` | この演算子の前回の実行並列度 |
| `LAST_TEMPSEG_SIZE` | この演算子の前回のディスクに落ちたデータの平均量 |
| `MAX_TEMPSEG_SIZE` | この演算子のすべての実行回数における最大ディスクデータ量 |
...
このビューは、すべてのOBServerノードのスロークエリの計画レベルの統計を表示します。各スロークエリには1件の統計情報があり、同時にその計画のTrace情報も記録されます。
...
2. HINTを追加したスローSQLを実行し、Trace_idを取得して、以下のYxxxxxxxxxに置き換えます。
3. 以下のSQLを実行し、実行結果を保存して分析に使用します。
...
以下のドキュメントを参照して、このビューの詳細をご確認いただけます。
...
このビューは、すべてのOBServerノード上で発生した各SQLリクエストの情報を表示するために使用されます。ソース、実行状態、リソース消費量、待機時間などの情報に加え、SQLテキストや実行計画などの重要な情報も記録されています。このビューはテナントごとに分割されており、システムテナントを除く他のテナントではテナント間でのクエリはできません。
...
テナントリソースを最も多く消費しているSQLを照会し、実行時間 × 実行回数の降順で並べ替えます。該当テナントで現在容量不足（テナントCPU利用率が飽和状態）が発生している場合、このステートメントを用いてSQLが問題の原因であるか、疑わしいSQLが存在するかを判断できます。
...
<main id="notice" type='explain'>
  <h4>説明</h4>
  <ul>
    <li>テナントのジッターが発生した場合、一般的に「テナントCPUが満杯」および「すべてのSQL RTが急上昇」という症状が見られます。この時、まず判断すべきはSQL RTの急上昇が第一現場であるかどうかです（SQL自体の問題がRTの急上昇を引き起こしたのか、それとも他の問題がSQLのRTを急上昇させたのか）。</li>
    <li>上記で紹介したこのSQLは非常に有効なツールです。SQL_IDに基づいて集計し、リソース消費量の降順で並べ替えます（リソース消費量はavg_exec_time × cntと見なすことができます）。上位のSQLに明らかな異常がないか確認し、それによって第一現場であるかどうかを判断します。</li>
  </ul>
</main>

**関連ドキュメント：**

以下のドキュメントを参照することで、このビューの詳細な情報をさらに理解できます。
