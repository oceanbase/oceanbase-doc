...
`LOAD DATA` ステートメントは `append/direct()` Hint を使用してダイレクトロードを実行します。Hint が指定されていない場合は、設定項目 [default_load_mode](https://en.oceanbase.com/docs/common-oceanbase-database-10000000001976661) に基づいてデータインポート動作を決定します。
...
`INSERT INTO SELECT` ステートメントは、Hint で `append/direct()` と `enable_parallel_dml` を使用することにより、ダイレクトロードを実行します。Hint が指定されていない場合は、設定項目 [default_load_mode](https://en.oceanbase.com/docs/common-oceanbase-database-10000000001976661) に基づいてデータのインポート動作を決定します。
...
`CREATE TABLE AS SELECT` ステートメントは `DIRECT()` Hint を設定してダイレクトロードのデータロード方式を指定します。Hint が指定されていない場合は、設定項目 [default_load_mode](https://en.oceanbase.com/docs/common-oceanbase-database-10000000001976661) に基づいてデータインポート動作を決定します。
