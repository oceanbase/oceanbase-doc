...
テーブルに既存データがある状態でインクリメンタルデータをインポートしたい場合、インクリメンタルバイパスインポート機能を利用できます。フルバイパスインポートでもインクリメンタルデータをインポートできますが、フルバイパスインポートでインクリメンタルデータをインポートする過程では、すべての元データをオーバーライドするため、インポートのパフォーマンスが低下します。インクリメンタルバイパスインポート機能は、フルバイパスインポートとは異なり、インポートのプロセスでインクリメンタルデータのみを操作するため、インポートのパフォーマンスを保証できます。

本記事では、`LOAD DATA` ステートメント、`INSERT INTO SELECT` ステートメント、`CREATE TABLE AS SELECT` ステートメントを使用して、インクリメンタルバイパスインポートを実現する方法について説明します。

## 注意点

バイパスインポートを使用する際の注意点は、以下のとおりです：

* インクリメンタルバイパスインポートのデータはダンプをトリガーするため、データ量が少なくて数分以内にインポートが完了する場合は、インクリメンタルバイパスインポートの使用は推奨されません。
* インクリメンタルバイパスインポートは、ターゲットテーブルに非一意のローカルインデックスがある場合をサポートします。
...
* V4.3.5 バージョンについて、V4.3.5 BP1 バージョン以降のインクリメンタルバイパスインポートでは、以下の点が変更されています：

  * ターゲットテーブルのパーティションが Hash / Key パーティションの場合、パーティションレベルのバイパスインポートをサポートします。
  * ヒープテーブルにローカル一意のインデックスが 1 つある場合のみをサポートします。ターゲットテーブルに複数のローカル一意のインデックスまたはグローバル一意のインデックスが存在する場合、インポート操作は失敗します。
...
`LOAD DATA` ステートメントは `DIRECT()` Hint を使用してインクリメンタルバイパスインポートを実行します。Hint が指定されていない場合は、設定項目 [default_load_mode](https://en.oceanbase.com/docs/common-oceanbase-database-10000000001976661) に基づいてデータインポート動作を決定します。
...
* 外部キーを持つテーブルは、インクリメンタルバイパスインポートをサポートしません。
...
|パラメータ|説明|
|------|------|
| DIRECT() | Hint を使用してバイパスインポート機能を有効にします。<code>DIRECT()</code> パラメータの説明は以下のとおりです：<ul><li><code>need_sort</code>：書き込まれるデータがソートを必要とするかどうかを表します。値は <code>bool</code> 型：<ul><li><code>true</code>：ソートが必要であることを表します。</li><li><code>false</code>：ソートが不要であることを表します。</li></ul></li><li><code>max_error</code>：許容される最大のエラー行数を表します。値は <code>INT</code> 型で、この値を超えるとインポートタスクの実行が失敗します。</li><li><code>inc\|inc_replace</code>：インクリメンタルバイパスインポートの 2 つのモードを表します。値は英語のシングルクォート (') で囲む必要があります。<ul><li><code>inc</code>：インクリメンタルインポートを表します。プライマリキーの重複をチェックし、<code>INSERT</code> と <code>IGNORE</code> セマンティクスをサポートします。</li><li><code>inc_replace</code>：インクリメンタルインポートを表しますが、プライマリキーの重複をチェックしません。<code>REPLACE</code> セマンティクスに相当するインクリメンタルインポートです。</li></ul></li></ul><main id="notice" type='notice'><h4>注意</h4><p><code>load_mode</code> が <code>inc_replace</code> の場合、<code>LOAD DATA</code> ステートメントで <code>REPLACE</code> または <code>IGNORE</code> キーワードは使用できません。</p></main>|
...
| COMPRESSION                      | 圧縮ファイル形式を指定します。値は以下のとおりです：<ul><li><code>AUTO</code>：ファイル名のサフィックスに基づいて圧縮アルゴリズムを自動検出します。<br><code>AUTO</code> パラメータを使用する場合、サフィックス名ごとに対応する圧縮形式があります。<ul><li><code>.gz</code>：GZIP 圧縮ファイルに対応。</li><li><code>.deflate</code>：DEFLATE 圧縮ファイルに対応。</li><li><code>.zst/.zstd</code>：ZSTD 圧縮ファイルに対応。</li></ul></li><li><code>NONE</code>：ファイルが圧縮されていないことを表します。</li><li><code>GZIP</code>：GZIP 圧縮ファイル。</li><li><code>DEFLATE</code>：メタデータなしの GZIP 圧縮ファイル。</li><li><code>ZSTD</code>：ZSTD 圧縮ファイル。</li></ul>ファイルの圧縮形式を明確に指定するか、プログラムによってファイル名のサフィックスに基づいて圧縮形式を検出させることができます。|
  ...
  <main id="notice" type='explain'>
   <h4>説明</h4>
   <p>LOAD DATA ステートメントのインクリメンタルバイパスインポートも、フルバイパスインポートと同様に、ワイルドカード方式での複数ファイルインポートをサポートします。</p>
   </main>

### 使用例

LOAD DATA ステートメントのインクリメンタルバイパスインポートの操作手順は、フルバイパスインポートの手順と同じで、<code>full</code> フィールドの値を <code>inc</code> または <code>inc_replace</code> に置き換えるだけです。
...
3. データベースに再接続した後、`LOAD /*+ DIRECT */ DATA` ステートメントを使用してデータをインポートします。
    ...
    3. バイパスインポートを使用して、`tbl1.csv` ファイルのデータをテーブル `tbl1` にインポートします。
...
`INSERT INTO SELECT` ステートメントは、`direct()` に加えて `enable_parallel_dml` Hint を使用することにより、バイパスインポートを実行します。Hint が指定されていない場合は、設定項目 [default_load_mode](https://en.oceanbase.com/docs/common-oceanbase-database-10000000001976661) に基づいてデータのインポート動作を決定します。
...
* 生成列を含むテーブルはサポートされていません (特定のインデックスは隠れた生成列を作成する場合があります。例：KEY `idx_c2` (`c2`(16)) GLOBAL)。
* Liboblog とフラッシュバッククエリ (Flashback Query) はサポートされていません。
* インデックス (プライマリキーを除く) を持つテーブルは、インクリメンタルバイパスインポートをサポートしません。
* 外部キーを持つテーブルは、インクリメンタルバイパスインポートをサポートしません。
...
|パラメータ|説明|
|------|------|
| DIRECT() | Hint を使用してバイパスインポート機能を有効にします。<code>DIRECT()</code> パラメータの説明は以下のとおりです：<ul><li><code>need_sort</code>：書き込まれるデータがソートを必要とするかどうかを表します。値は <code>bool</code> 型：<ul><li><code>true</code>：ソートが必要であることを表します。</li><li><code>false</code>：ソートが不要であることを表します。</li></ul></li><li><code>max_error</code>：許容される最大のエラー行数を表します。値は <code>INT</code> 型で、この値を超えるとインポートタスクの実行が失敗します。</li><li><code>inc\|inc_replace</code>：インクリメンタルバイパスインポートの 2 つのモードを表します。値は英語のシングルクォート (') で囲む必要があります。<ul><li><code>inc</code>：インクリメンタルインポートを表します。プライマリキーの重複をチェックし、<code>INSERT</code> と <code>IGNORE</code> セマンティクスをサポートします。</li><li><code>inc_replace</code>：インクリメンタルインポートを表しますが、プライマリキーの重複をチェックしません。<code>REPLACE</code> セマンティクスに相当するインクリメンタルインポートです。</li></ul></li></ul>|
...
`INSERT INTO SELECT`ステートメントのインクリメンタルバイパスインポートの操作手順は、フルバイパスインポートの手順と同じで、<code>full</code> フィールドの値を <code>inc</code> または <code>inc_replace</code> に置き換えるだけです。

:::tab
tab MySQL モード

**例 1：INSERT INTO SELECT を使用したインクリメンタルバイパスインポートの完全な例です。**
...
3. バイパスインポートを使用して、テーブル `tbl2` のデータをテーブル `tbl1` にインポートします。
   ...
   * `INSERT INTO SELECT` ステートメントの Hint を指定しません。
      ...
      設定項目 `default_load_mode` の値を `INC_DIRECT_WRITE` または `INC_REPLACE_DIRECT_WRITE` に設定します。
...
ターゲットテーブルはサブパーティションです。パーティションは Range、サブパーティションは Hash で、パーティションを指定してインクリメンタルバイパスインポートを行います。
...
`CREATE TABLE AS SELECT` ステートメントは `DIRECT()` および `PARALLE()` Hint を設定してバイパスインポートのデータロード方式を指定します。Hint が指定されていない場合は、設定項目 [default_load_mode](https://en.oceanbase.com/docs/common-oceanbase-database-10000000001976661) に基づいてデータインポート動作を決定します。
...
`CREATE TABLE AS SELECT` 構文の詳細については、[CREATE TABLE (MySQL モード)](https://en.oceanbase.com/docs/common-oceanbase-database-10000000001974140) および [CREATE TABLE (Oracle モード)](https://en.oceanbase.com/docs/common-oceanbase-database-10000000001975088) を参照してください。
