...
`LOAD DATA` ステートメントは `DIRECT()` Hint を使用してインクリメンタルダイレクトロードを実行します。Hint が指定されていない場合は、構成パラメータ [default_load_mode](https://en.oceanbase.com/docs/common-oceanbase-database-10000000001976661) に基づいてデータインポート動作を決定します。
...
`INSERT INTO SELECT` ステートメントは、`direct()` に加えて `enable_parallel_dml` Hint を使用することにより、ダイレクトロードを実行します。Hint が指定されていない場合は、構成パラメータ [default_load_mode](https://en.oceanbase.com/docs/common-oceanbase-database-10000000001976661) に基づいてデータのインポート動作を決定します。
...
**例 2：Range-Hash コンポジット・パーティション表に対して、パーティションを指定してデータをダイレクトロードします。**
...
`CREATE TABLE AS SELECT` ステートメントは `DIRECT()` および `PARALLE()` Hint を設定してダイレクトロードのデータロード方式を指定します。Hint が指定されていない場合は、構成パラメータ [default_load_mode](https://en.oceanbase.com/docs/common-oceanbase-database-10000000001976661) に基づいてデータインポート動作を決定します。
