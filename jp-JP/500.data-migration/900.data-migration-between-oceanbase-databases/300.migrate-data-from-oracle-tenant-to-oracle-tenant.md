...
OceanBaseデータ導入・エクスポートツールには、エクスポートツールであるobdumperとインポートツールであるobloaderが含まれます。

* obdumperは、Javaで開発されたクライアント側のエクスポートツールです。このツールを使用して、OceanBaseデータベースで定義されたオブジェクトとデータをファイルにエクスポートできます。obdumperの詳細については、[obdumperの紹介](https://en.oceanbase.com/docs/enterprise-oceanbase-dumper-loader-en-10000000000860074)を参照してください。

* obloaderは、Javaで開発されたクライアント側のインポートツールです。このツールは非常に柔軟なコマンドラインオプションを提供しており、多様な複雑なシナリオで定義とデータをOceanBaseにインポートすることができます。obloaderの詳細については、[obloaderの紹介](https://en.oceanbase.com/docs/enterprise-oceanbase-dumper-loader-en-10000000000860068)を参照してください。
...
| テナントoracle001内ユーザーobdumper_user01のパスワード  | ******     |
...
| テナントoracle002内ユーザーobloader_user01のパスワード  | ******  |
...
シナリオの説明：クラスタ `test4000` のテナント `oracle001` 内のスキーマ `OBDUMPER_USER01` から、サポートされているすべてのオブジェクト定義ステートメントを `/home/admin/test_migrate_data/ddl_data` ディレクトリにエクスポートします。
...
シナリオの説明：クラスタ `test4000` のテナント `oracle001` 内のスキーマ `OBDUMPER_USER01` から、サポートされているすべての定義情報を `/home/admin/test_migrate_data/ddl_data` ディレクトリにエクスポートし、それをクラスタ `test4000` のテナント `oracle002` 内のスキーマ `OBLOADER_USER01` にインポートします。
...
シナリオの説明：クラスタ `test4000` のテナント `oracle001` 内のスキーマ `OBDUMPER_USER01` から、サポートされているすべてのオブジェクトデータを `/home/admin/test_migrate_data/csv_data` ディレクトリにエクスポートします。データ形式はCSV形式です。
...
シナリオの概要：クラスタ `test4000` のテナント `oracle001` 内のスキーマ `OBDUMPER_USER01` から、`/home/admin/test_migrate_data/csv_data` ディレクトリ内にあるすべてのサポートされているCSVデータファイルをインポートし、クラスタ `test4000` のテナント `oracle002` 内のスキーマ `OBLOADER_USER01` にインポートします。
