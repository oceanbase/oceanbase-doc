...
# OceanBaseデータ移行ツールを使用した、OceanBaseクラスタのOracleテナントからMySQLテナントへのデータ移行
...
* obloaderはJavaで開発されたクライアントのインポートツールです。このツールは非常に柔軟なコマンドラインオプションを提供しており、多様な複雑なシナリオで定義とデータをOceanBaseにインポートすることができます。obloaderの詳細については、[obloaderの概要](https://www.oceanbase.com/docs/enterprise-oceanbase-dumper-loader-cn-10000000001142221)を参照してください。
...
シナリオの説明：クラスタ `test4000` のテナント `oracle001` 内のスキーマ `OBDUMPER_USER01` から、`/home/admin/test_migrate_data/ddl_data` ディレクトリにあるすべてのサポートされている定義情報をエクスポートし、クラスタ `test4000` のテナント `mysql001` 内のスキーマ `test_data` にインポートします。

  <main id="notice" type='notice'>
    <h4>注意</h4>
    <p>OracleテナントからエクスポートされたDDLファイルはMySQLテナントの構文要件に適合していないため、OracleテナントからエクスポートされたDDLファイルを、MySQLテナントの構文要件に適合するDDLファイルに変更した後、MySQLテナントにインポートしてください。</p>
...
シナリオの概要：クラスタ `test4000` のテナント `oracle001` 内のスキーマ `OBDUMPER_USER01` から、`/home/admin/test_migrate_data/csv_data` ディレクトリ内にあるすべてのサポートされているCSVデータファイルをエクスポートし、クラスタ `test4000` のテナント `mysql001` 内のスキーマ `test_data` にインポートします。

  <main id="notice" type='notice'>
    <h4>注意</h4>
    <ul>
    <li>テナント <code>oracle001</code> 内のスキーマ <code>OBDUMPER_USER01</code> をエクスポートするディレクトリ <code>/home/admin/test_migrate_data/csv_data/data/</code> 内の <code>OBDUMPER_USER01</code> 名を <code>test_data</code> に変更する必要があります。</li>
