...
| テナントoracle001内のユーザーobloader_user01のパスワード  | ******     |
| テナントoracle001内のSchemaデータベース名               | OBLOADER_USER01 |

### テーブル構造の移行

obdumperとobloaderを使用してテーブル構造を移行する場合は、`--sys-user` および `--sys-password` オプションを指定してテーブル構造のメタデータ情報を取得する必要があります。これらのオプションを指定しない場合、エクスポート/インポート機能とパフォーマンスに大きな影響を与える可能性があります。
...
シナリオの概要：クラスタ `test4000` のテナント `mysql001` 内の Schema `test_data` から、サポートされているすべてのオブジェクト定義ステートメントを `/home/admin/test_migrate_data/ddl_data` ディレクトリにエクスポートします。
...
シナリオの概要：クラスタ `test4000` のテナント `mysql001` 内の Schema `test_data` から `/home/admin/test_migrate_data/ddl_data` ディレクトリにエクスポートされたすべてのサポートされている定義情報を、クラスタ `test4000` のテナント `oracle001` 内の Schema `obloader_user01` にインポートします。

  <main id="notice" type='notice'>
    <h4>注意</h4>
    <p>MySQLテナントからエクスポートされたDDLファイルはOracleテナントの構文要件に適合していないため、MySQLテナントからエクスポートされたDDLファイルを、まずOracleテナントの構文要件に適合するDDLファイルに変更した後、Oracleテナントにインポートします。</p>
    <p>例えば、テナント<code>mysql001</code>からエクスポートされたDDLファイルの変更ルールは以下のとおりです：</p>
    <ul>
    <li><code>if not exists</code>キーワードとシングルクォートを削除します。</li>
    <li>データ型をOracleモードで共通して使用されるデータ型に変更します：
    <ul>
    ...
    <p>Oracleモードのデータ型の詳細については、<a href="https://en.oceanbase.com/docs/common-oceanbase-database-10000000001975141">組み込みデータ型の概要</a>を参照してください。</p>
  </main>
...
シナリオの説明：クラスタ`test4000`のテナント`mysql001`内のスキーマ`test_data`から`/home/admin/test_migrate_data/csv_data`ディレクトリにエクスポートされたすべてのサポートされているCSVデータファイルを、クラスタ`test4000`のテナント`oracle001`内のスキーマ`OBLOADER_USER01`にインポートします。

  <main id="notice" type='notice'>
    <h4>注意</h4>
    <ul>
    <li>テナント<code>mysql001</code>のスキーマ<code>test_data</code>をエクスポートするディレクトリ<code>/home/admin/test_migrate_data/csv_data/data/</code>内の<code>test_data</code>の名前を<code>OBLOADER_USER01</code>に変更してください。</li>
    <li>エクスポートされたCSVファイル名およびテーブル名の英字を大文字に変更してください。</li>
    </ul>
    <p>サンプルステートメントは以下のとおりです：</p>
