...
* obloaderはJavaで開発されたクライアントのインポートツールです。このツールは非常に柔軟なコマンドラインオプションを提供しており、複雑なシナリオでも定義とデータをOceanBaseにインポートできます。obloaderの詳細については、[obloaderの概要](https://www.oceanbase.com/docs/enterprise-oceanbase-dumper-loader-cn-10000000001142221)を参照してください。
...
シナリオの説明：クラスタ `test4000` のテナント `mysql001` 内のスキーマ `test_data` から、`/home/admin/test_migrate_data/ddl_data` ディレクトリにあるすべてのサポートされている定義情報をエクスポートし、クラスタ `test4000` のテナント `oracle001` 内のスキーマ `obloader_user01` にインポートします。

  <main id="notice" type='notice'>
    <h4>注意</h4>
    <p>MySQLテナントからエクスポートされたDDLファイルはOracleテナントの構文要件に適合していないため、MySQLテナントからエクスポートされたDDLファイルを、Oracleテナントの構文要件に適合するDDLファイルに変更した後、Oracleテナントにインポートします。</p>
...
シナリオの説明：クラスタ `test4000` のテナント `mysql001` 内のスキーマ `test_data` を、`/home/admin/test_migrate_data/csv_data` ディレクトリ内のすべてのサポートされているCSVデータファイルにエクスポートし、それらをクラスタ `test4000` のテナント `oracle001` 内のスキーマ `OBLOADER_USER01` にインポートします。

  <main id="notice" type='notice'>
    <h4>注意</h4>
    <ul>
    <li>テナント <code>mysql001</code> 内のスキーマ <code>test_data</code> をエクスポートするディレクトリ <code>/home/admin/test_migrate_data/csv_data/data/</code> 内の <code>test_data</code> の名前を <code>OBLOADER_USER01</code> に変更する必要があります。</li>
    <li>エクスポートされたCSVファイル名およびテーブル名の英字を大文字に変更します。</li>
    </ul>
    <p>例文は以下のとおりです：</p>
