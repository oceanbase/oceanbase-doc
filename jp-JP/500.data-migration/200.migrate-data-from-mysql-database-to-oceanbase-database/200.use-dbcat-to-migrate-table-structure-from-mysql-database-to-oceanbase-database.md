...
インストールファイルには、理解しておくべきディレクトリがいくつかあります：
...
DBCATにはオンライン変換機能があり、これはDBCATがデータ送信元データベースに直接接続し、データベース内のオブジェクトをエクスポートできることを指します。オブジェクトが非常に多い場合（例えば1万個を超える場合）、エクスポートプロセスが遅くなる可能性があります。
...
| --schema | Y | スキーマ名（ソースデータベース）。DB2以外では、スキーマ名とデータ名は同じ |
...
| --procedure | Y | エクスポートされたストアドプロシージャ |
| --dblink | Y | すべてのDBLinkをエクスポートします |
| --type | Y | エクスポートされたtype |
| --type-body | Y | エクスポートされたtype body |
| --package | Y | エクスポートされたpackage |
| --package-body | Y | エクスポートされたpackage body |
| --no-quote | N | 生成されるDDLには引用符を含めません |
...
* パラメータの --from と --to はデータ送信元とデータ送信先のデータベースタイプであり、バージョン番号まで指定する必要があります。現在dbcatがサポートするデータ送信元とデータ送信先のデータベースの詳細は以下のとおりです：
...
DBCATでエクスポートされたファイル形式はSQLファイルです。ここでは、ODCのインポート機能を使用して、テーブル構造をOceanBaseデータベースに一括インポートできます。詳細については、[一括エクスポートとインポート](https://www.oceanbase.com/docs/enterprise-odc-doc-cn-10000000001033324)を参照してください。
