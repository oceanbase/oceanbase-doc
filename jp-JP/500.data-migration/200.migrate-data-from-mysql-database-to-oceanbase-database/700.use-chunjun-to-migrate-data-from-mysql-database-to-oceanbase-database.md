...
|slug|use-chunjun-to-migrate-data-from-mysql-database-to-oceanbase-database|

# ChunJunを使用してMySQLデータベースからOceanBaseデータベースへデータを移行する

純鈞（ChunJun、旧称FlinkX）は、安定性が高く、使いやすく、効率的で、バッチとストリームを統合したデータ統合フレームワークです。リアルタイム計算エンジンFlinkに基づき、JSONテンプレートでのタスク設定をサポートし、Flink SQL構文に互換性があります。また、分散実行に対応しており、flink-standalone、yarn-session、yarn-per jobなど、さまざまなコミット方式をサポートしています。さらに、完全同期や増分同期などの機能も備えています。
...
1. ChunJun圧縮ファイルをダウンロードし、解凍します。
    ...
    圧縮ファイルをディレクトリ`/chunjun_Home/chunjun`に解凍します。
...
環境情報に基づいて、MySQLデータベースのテーブル`tbl1`からOceanBaseデータベースのテーブル`test_tbl1`へデータをマイグレーションするためのJSONファイルを設定します。
...
* ChunJunの詳細な特徴については、[Features of ChunJun](https://github.com/DTStack/chunjun#features-of-chunjun)を参照してください。
* ChunJunのクイックスタートの詳細については、[Quick Start](https://dtstack.github.io/chunjun/documents/en/Quick%20Start?locale=zh&slug=%E5%BF%AB%E9%80%9F%E5%BC%80%E5%A7%8B&lang=en)を参照してください。
* MySQLデータベースをソースデータベースとして設定するためのJSONファイルの詳細については、[MySQL Source](https://dtstack.github.io/chunjun/documents/ChunJun%E8%BF%9E%E6%8E%A5%E5%99%A8@mysql@mysql-source)を参照してください。
* スクリプトのその他の例については、プロジェクト内の`/chunjun-dist/chunjun-examples`フォルダのファイルを参照してください。