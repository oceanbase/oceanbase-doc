...
純鈞（ChunJun、旧称FlinkX）は、安定性、使いやすさ、効率性に優れた、バッチとストリームを統合したデータ統合フレームワークです。リアルタイム計算エンジンFlinkを基盤とし、JSONテンプレートでタスクを設定できます。Flink SQL構文に互換性があり、分散実行をサポートしています。flink-standalone、yarn-session、yarn-per-jobなど、多様なコミット方法に対応しており、全量同期や増分同期などの機能も備えています。

以下では、ChunJunのローカルモードを使用して、MySQLデータベースからOceanBaseデータベース（MySQLモード）へデータを移行する方法について説明します。

## シナリオの概要

MySQLデータベースのテーブル `tbl1` のデータをOceanBaseデータベース内のテーブル `test_tbl1` に移行します。データベース情報は以下のとおりです：
...
| ユーザー名      | root             |
| ユーザーのパスワード    | ******           |
| Schemaデータベース名 | test_mysql_to_ob |
| テーブル名          | tbl1             |
...
環境情報に基づいて、MySQLデータベースのテーブル `tbl1` のデータをOceanBaseデータベース内のテーブル `test_tbl1` に移行するためのJSONファイルを設定します。
...
* ChunJunの特徴に関する詳細は、[Features of ChunJun](https://github.com/DTStack/chunjun#features-of-chunjun)を参照してください。
* ChunJunのクイックスタートに関する詳細は、[Quick Start](https://dtstack.github.io/chunjun/documents/zh/%E5%BF%AB%E9%80%9F%E5%BC%80%E5%A7%8B?lang=zh)を参照してください。
* MySQLデータベースをソースデータベースとして設定するJSONファイルの詳細については、[MySQL Source](https://dtstack.github.io/chunjun/documents/zh/ChunJun%E8%BF%9E%E6%8E%A5%E5%99%A8@mysql@mysql-source?lang=zh)を参照してください。
* スクリプトの例については、プロジェクト内の`/chunjun-dist/chunjun-examples`フォルダ内のファイルを参照してください。