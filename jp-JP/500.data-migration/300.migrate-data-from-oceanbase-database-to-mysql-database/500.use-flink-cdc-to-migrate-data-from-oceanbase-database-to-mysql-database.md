...
[Binlogクラスタの作成](https://en.oceanbase.com/docs/common-ocp-10000000001899110)を参照して、OceanBase Binlogサービスのインストールとデプロイを完了してください。
...
Flink SQL CLIを使用して、OceanBaseデータベースに対応するテーブルを作成します。OceanBaseデータベースの`test_ob_to_mysql`テーブルの`tbl1`と`tbl2`に対応するテーブルをFlink SQL CLIで作成し、これらの基盤データベーステーブルのデータを同期するために使用します。
