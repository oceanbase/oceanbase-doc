...
| ~/output | SQLファイルとレポートファイル。実行時に生成されます。 |
...
ここでは、4.0.0バージョンのOceanBaseクラスタ内のMySQLテナントにおいて、データベースtestのすべてのオブジェクト構造をエクスポートし、それをMySQL 5.7バージョンに移行する例を説明します。
...
* パラメータの --from と --to は、ソース側とターゲット側のデータベースタイプを指定し、バージョン番号まで詳細に記述する必要があります。現在dbcatがサポートするソース側およびターゲット側のデータベースの詳細は以下のとおりです：
...
DBCATでエクスポートされたファイル形式はSQLファイルです。sourceコマンドを使用できます。例：
...
例：テーブル構造をMySQLとOceanBaseデータベースで確認する方法を見てみましょう。
