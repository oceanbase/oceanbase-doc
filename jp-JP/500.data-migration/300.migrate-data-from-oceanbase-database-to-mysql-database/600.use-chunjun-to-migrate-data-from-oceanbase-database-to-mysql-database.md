...
純鈞（ChunJun、旧称FlinkX）は、安定性、使いやすさ、効率性に優れた、バッチとストリームを統合したデータ統合フレームワークです。現在、リアルタイム計算エンジンFlinkを基盤とし、多様な異種データソース間のデータ同期と計算を実現しています。OceanBase Binlogサービス（OceanBase Binlog Service、略称Binlogサービス）は、OceanBaseデータベースのトランザクションログを収集し、MySQL Binlogに変換する機能を備えており、主にリアルタイムデータサブスクリプションなどのシナリオで利用されます。
chunjun-connector-oceanbasecdcは、内部でoblogclientを統合し、Binlogサービスに接続して関連ログデータを取得することができ、OceanBaseデータベースの増分データ移行を実行できます。

以下では、ChunJunのLocalモードを使用して、OceanBaseデータベース（MySQLモード）からMySQLデータベースへデータを移行する方法について説明します。

## シナリオの概要

OceanBaseデータベースのテーブル `test_tbl1` のデータをMySQLデータベースのテーブル `tbl1` に移行します。データベース情報は以下の通りです：
...
環境情報に基づいて、OceanBaseデータベースのテーブル `test_tbl1` のデータをMySQLデータベースのテーブル `tbl1` に移行するためのJSONファイルを設定します。
...
OceanBaseデータベース（MySQLモード）にログインし、テーブル `test_tbl1` にデータを1件挿入します。
...
* ChunJunのクイックスタートの詳細については、[クイックスタート](https://dtstack.github.io/chunjun/documents/zh/%E5%BF%AB%E9%80%9F%E5%BC%80%E5%A7%8B?lang=zh)を参照してください。
* ターゲットデータベースとしてMySQLデータベースを設定するJSONファイルの詳細については、[MySQL Sink](https://dtstack.github.io/chunjun/documents/zh/ChunJun%E8%BF%9E%E6%8E%A5%E5%99%A8@mysql@mysql-sink?lang=zh)を参照してください。
