...
|dir-name-en||
|tenant-type||
|slug|use-dbcat-migrate-schemas-from-oceanbase-to-oracle-compatible-mode|
...
DBCATにはオンライン変換機能があります。これは、DBCATがソースデータベースに直接接続し、データベース内のオブジェクトをエクスポートできることを指します。オブジェクトが非常に多い場合（例：1万個を超える場合）、エクスポートプロセスが遅くなる可能性があります。
...
| --no-schema | N | 生成されるDDLにはスキーマ名を含めない |
| --target-schema | Y | 生成されるDDLで指定されたスキーマ名を使用 |
| --exclude-type | Y | --allと組み合わせて使用、例：--all --exclude-type 'TABLE'はTABLEタイプを除外することを意味します |

ここでは、4.0.0バージョンのOceanBaseクラスタ内のOracleテナントのtestにあるすべてのテーブル構造をエクスポートし、それをOracle 12cバージョンに移行する例を説明します。
...
* パラメータの --from と --to は、ソース側とターゲット側のデータベースタイプを指定し、バージョン番号まで詳細に指定する必要があります。現在dbcatがサポートするソース側およびターゲット側のデータベースの詳細は以下のとおりです：
...
例：OceanBaseデータベースとORACLEデータベースにおけるテーブル構造を確認する方法です。
