...
SQL形式のデータファイルをOceanBaseデータベースにインポートするには、コマンドラインインターフェースによるインポートとツールによるインポートの2つの方法があります。OceanBaseデータベースは、MySQLDumper、obloader、OceanBase Developer Center（以下、ODCという）などのグラフィカルインターフェースツールを含む、さまざまなツールでSQL形式のデータファイルをインポートすることをサポートしています。
...
3. インポート設定パネルに進む
    ...
    データベース接続後、上部ナビゲーションバーの **ツール** ラベルをクリックし、表示されたドロップダウンメニューで **インポート** ラベルをクリックしてインポート設定パネルに進みます。
...
6. インポートタスクの確認
    ...
    タスクが生成されると、自動的に **タスクセンター** パネルがポップアップ表示されます。タスクセンターでタスク情報を確認できます。詳細については、[インポートタスク](https://www.oceanbase.com/docs/enterprise-odc-doc-cn-10000000001033258)を参照してください。
