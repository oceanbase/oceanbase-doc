...
2. 置き換えるノードが存在するZoneに新しいノードを追加します。
...
3. 古いノード上のUnitを新しいノードに移行します。
   ...
   2. ノードIPアドレスに基づいて、置き換えるノードのUnitリストを取得します。
      ...
      ここで、`svr_ip`には置き換えるノードのIPアドレスを実際の状況に応じて入力する必要があります。
   ...
   3. Unit移行タスクを送信し、古いノード上のUnitを同一Zone内の他のノードに移行します。
   ...
   4. `oceanbase.DBA_OB_UNIT_JOBS`ビューをクエリして、Unitの移行進捗状況を確認します。
      ...
      クエリ結果が空の場合、Unitの移行は完了したことを意味します。
...
4. 古いノード上のすべてのUnitの移行が完了したら、古いノードを削除します。
   ...
   1. 以下のコマンドを実行して、古いノードを削除します。
      ...
      * `svr_ip`：削除する古いノードのIPアドレス。

      * `port`：削除する古いノードのRPCポート。デフォルトは2882。

      * `zone_name`：削除する古いノードが属するZone。
   ...
   2. 操作が終了したら、`oceanbase.DBA_OB_SERVERS`ビューをクエリして、古いノードが正常に削除されたかを確認します。

      ```shell
      obclient [(none)]> SELECT * FROM oceanbase.DBA_OB_SERVERS;
      ```

      リストで古いノードの情報が見つからない場合、削除が完了したことを意味します。リストにそのノードが引き続き存在し、ノードの状態が`DELETING`である場合、ノードはまだ削除中であることを意味します。
