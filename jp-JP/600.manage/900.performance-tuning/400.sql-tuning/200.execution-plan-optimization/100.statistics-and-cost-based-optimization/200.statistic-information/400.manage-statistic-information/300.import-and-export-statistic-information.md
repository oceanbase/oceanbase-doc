...
統計情報のエクスポート機能は、主にテーブルの統計情報を指定されたユーザーテーブルにエクスポートして保存するために使用されます。一方、インポート機能は、指定されたユーザーテーブルの統計情報を現在のデータベースにインポートすると、再収集のプロセスを省略することができます。使用シナリオとしては、データベースデータ移行する場合が挙げられます。テーブルの統計情報を指定されたユーザーテーブルにエクスポートしてデータ移行と同時に行い、データ移行完了後に、保存しておいた統計情報のユーザーテーブルの統計情報を新しいデータベースに直接インポート可能です。
...
* `import_schema_stats`：指定された統計情報ユーザーテーブルにあるschema内のすべてのテーブルの統計情報をインポートします。
