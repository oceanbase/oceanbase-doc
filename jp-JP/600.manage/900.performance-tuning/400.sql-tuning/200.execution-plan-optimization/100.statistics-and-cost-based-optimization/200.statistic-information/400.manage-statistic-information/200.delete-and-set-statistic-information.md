...
* `DELETE_SCHEMA_STATS`：schema内のすべてのテーブルの統計情報を削除します。
...
現在、`DBMS_STATS` システムパッケージでサポートされている統計情報の設定は、テーブルレベルでは行数/平均行長/マクロブロック数/マイクロブロック数、列レベルではndv/null数/平均列長のみです。ヒストグラムなどの他の属性については、現在設定をサポートしていません。関連する PROCEDURE は以下の3つです。
