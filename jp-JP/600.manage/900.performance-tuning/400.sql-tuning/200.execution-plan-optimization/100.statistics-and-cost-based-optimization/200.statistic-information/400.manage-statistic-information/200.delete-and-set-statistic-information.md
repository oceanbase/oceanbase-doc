...
現在、`DBMS_STATS` システムパッケージでサポートされている統計情報の設定は、テーブルレベルでは行数/平均行長/マクロブロック数/マイクロブロック数、列レベルではndv/null数/平均列長のみです。ヒストグラムなどの他の属性については、現在設定をサポートしていません。関連するプロシージャは以下の3つです。
