...
実行計画の選択精度を向上させるために、オプティマイザーのカードナリティ推定は重要な役割を果たします。しかし、不正確なカードナリティ推定は実行計画の偏りを引き起こす可能性があります。カードナリティ推定は通常、述語独立性仮説や結合包含仮説など、一連の仮定に依存しています。

実際のアプリケーションでは、ユーザーのデータモデルはこれらの仮定と完全に一致することは少なく、その結果、オプティマイザーが大きな偏りを持つ推定結果を生成し、理想的でない実行計画を選択する可能性があります。したがって、オプティマイザーはシステム変数[`cardinality_estimation_model`](../../../../../700.reference/800.configuration-items-and-system-variables/200.system-variable/300.global-system-variable/850.cardinality_estimation_model-global.md)を設定することで一部の仮定を制御し、複数のカードナリティ推定結果を生成することで、実行計画の選択を最適化する必要があります。