...
* `Q2_1_2`では、`Q2_1_1`に対して`ORDER BY`を調整し、主キーの使用を指定しています。`MERGE GROUP BY`は、`ORDER BY c2, c1`で必要な順序をそのまま演算子の集約入力順序として使用し、`GROUP BY`の下にソート演算子を割り当てます。
* `Q2_1_3`では、`Q2_1_2`と比較して主キーの使用を指定していないため、コストに基づいて`MERGE GROUP BY`が直接使用できる`idx`によって生成された`c1, c2`の順序を生成し、集約後に`ORDER BY c2, c1`にソート演算子を割り当てます。
...
* `Q2_2_2`: `idx` インデックスは `(c1, c2, c3)` の順序を提供できますが、`c1 = 4` 述語が含まれているため、出力順序は `order by c2` と一致します。
