...
稼働中のデータベースシステムでは、一連のスロークエリがシステム全体のパフォーマンスを低下させることがあります。また、特定の種類のクエリが非常に長時間実行され、終わらないこともあります。本記事では、`GV$OB_PROCESSLIST`、`GV$OB_PLAN_CACHE_PLAN_STAT`、`GV$OB_PLAN_CACHE_PLAN_EXPLAIN` を使用して、スロークエリと対応する実行計画を検索する方法を紹介します。
...
2. スローSQLの計画形式を取得します。
    ...
    物理計画に加えて、論理計画の収集も試すことができます。論理計画を収集する方法は非常に簡単で、SQLを直接 explainするだけです。
