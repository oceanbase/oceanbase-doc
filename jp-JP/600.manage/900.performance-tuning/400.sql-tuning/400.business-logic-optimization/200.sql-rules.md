...
SQLの`where`フィルター条件でフィールドに関数や演算子を使用すると、対応するフィールドのインデックスを利用できなくなります。
...
* エラーコード例2：`WHERE`条件の列にNULL値が存在する場合
    ...
    `a!=1`でフィルタリングすると、`id`が3のデータのみが選択され、`id`が2のデータは除外されます。`NULL!=1`の判断はNULLとなり、ブール値のtrueまたはfalseではないためです。

    `a not in (1,3)`でフィルタリングしても、`id`が2の値は選択されません。`NULL not in (1，3)`の判断はNULLとなり、ブール値のtrueまたはfalseではないためです。
