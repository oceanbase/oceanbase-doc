...
復元が完了した後、ビューを使用して物理復元の結果を確認できます。
...
2. 復元が完了したら、以下のステートメントを実行して、クラスタの復元タスク結果を確認できます。
   ...
   *************************** 2. row ***************************
   ...
   ```
    ...
    `CDB_OB_RESTORE_HISTORY` ビューの各フィールドは、基本的に `CDB_OB_RESTORE_PROGRESS` と対応しています。その中で、`COMMENT` はタスク失敗時の関連エラー情報を記録するために使用されます。

    復元に失敗した場合は、テナントを削除することで、この復元を取り消すことができます。具体的な操作については、[復元のキャンセル](../600.restore-data/300.cancel-a-restore.md)を参照してください。

## 次のステップ

物理復元が正常に実行されると、復元されたテナントはすべてスタンバイテナントになります。以下の操作を実行できます：

* `recover` コマンドを実行し、スタンバイテナントとしてソーステナントからのログ再生を続けます。具体的な操作については、[スタンバイテナントのログ再生](510.recover-the-standby-tenant.md)を参照してください。

* スタンバイからプライマリへの切り替えコマンドを実行し、スタンバイテナントをプライマリテナントに変換します。具体的な操作については、[スタンバイテナントからプライマリテナントへの切り替え](600.active-standby-tenant.md)を参照してください。