...
|dir-name-en||
|tenant-type||
|slug|check-physical-restore-results|

# 物理リストア結果の確認

リストアが完了したら、ビューを使用して物理リストアの結果を確認できます。

## 操作手順

1. `root` ユーザーでクラスタの `sys` テナントにログインします。

2. リストアが完了したら、以下のステートメントを実行して、クラスタのリストアタスク結果を確認できます。
   ...
   *************************** 2. row ***************************
   ...
   ```
    ...
    `CDB_OB_RESTORE_HISTORY` ビューの各フィールドは、基本的に `CDB_OB_RESTORE_PROGRESS` と対応しています。その中で、`COMMENT`は フィールドはタスクが失敗した場合の関連エラー情報を記録するために使用されます。

    リストアが失敗した場合は、テナントを削除することで、このリストアをキャンセルできます。具体的な操作については、[リストアのキャンセル](../600.restore-data/300.cancel-a-restore.md)を参照してください。

## 次のステップ

物理リストアが正常に実行されると、リストアされたテナントはすべてスタンバイテナントとなります。その後、以下の操作を実行できます：

* `recover` コマンドを実行し、スタンバイテナントとしてソーステナントからのログリプレイを続けます。詳細な操作については、[スタンバイテナントのログリプレイ](510.recover-the-standby-tenant.md)を参照してください。

* スタンバイからプライマリへの切り替えコマンドを実行し、スタンバイテナントをプライマリテナントに変換します。詳細な操作については、[スタンバイテナントからプライマリテナントへの切り替え](600.active-standby-tenant.md)を参照してください。