...
復元が完了したら、ビューを使用して物理復元の結果を確認できます。
...
2. 復元が完了したら、以下のステートメントを実行して、クラスタの復元タスク結果を確認できます。
   ...
   *************************** 2. row ***************************
   ...
   ```
    ...
    `CDB_OB_RESTORE_HISTORY` ビューの各フィールドは、基本的に `CDB_OB_RESTORE_PROGRESS` と対応しています。その中で、`COMMENT` フィールドはタスクが失敗した場合の関連エラー情報を記録するために使用されます。

    復元が失敗した場合は、テナントを削除することで、この復元をキャンセルできます。具体的な操作については、[復元のキャンセル](../600.restore-data/300.cancel-a-restore.md)を参照してください。

## 次のステップ

物理復元が正常に実行されると、復元されたテナントはすべてスタンバイテナントとなります。その後、以下の操作を実行できます：
