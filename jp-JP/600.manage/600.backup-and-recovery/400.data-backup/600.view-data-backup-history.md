...
2. データバックアップ履歴を確認します。
   ...
   * Taskレベルタスク履歴の確認
     ...
     `oceanbase.CDB_OB_BACKUP_JOB_HISTORY`ビューの各フィールドは基本的に`oceanbase.CDB_OB_BACKUP_JOBS`ビューに対応し、`oceanbase.CDB_OB_BACKUP_TASK_HISTORY`の各フィールドは基本的に`oceanbase.CDB_OB_BACKUP_TASKS`に対応します。その中で、`COMMENT`はタスク失敗時の関連エラー情報を記録するために使用されます。
