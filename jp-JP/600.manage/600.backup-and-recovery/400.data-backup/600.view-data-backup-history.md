...
データバックアップが完了すると、ビューを使用してバックアップタスクの履歴を確認できます。

## システムテナントにおけるデータバックアップタスク履歴の確認
...
2. データバックアップ履歴を確認します。
   ...
   * JOBレベルタスク履歴の確認
      ...
      テナントが一度バックアップを開始するごとに、一つのJOBタスクレコードが対応します。
   ...
   * Taskレベルタスク履歴の確認
      ...
      一つのTaskは一つのバックアップセットのタスクに対応します。
     ...
     `oceanbase.CDB_OB_BACKUP_JOB_HISTORY`ビューの各フィールドは基本的に`oceanbase.CDB_OB_BACKUP_JOBS`ビューに対応し、`oceanbase.CDB_OB_BACKUP_TASK_HISTORY`の各フィールドは基本的に`oceanbase.CDB_OB_BACKUP_TASKS`に対応します。その中で、`COMMENT`はタスクが失敗した際の関連エラー情報を記録するために使用されます。

   `CDB_OB_BACKUP_JOB_HISTORY`および`CDB_OB_BACKUP_TASK_HISTORY`ビューの詳細については、[CDB_OB_BACKUP_JOB_HISTORY](../../../700.reference/700.system-views/300.system-view-of-sys-tenant/200.dictionary-view-of-sys-tenant/6900.o-cdb_ob_backup_job_history-of-sys-tenant.md)および[CDB_OB_BACKUP_TASK_HISTORY](../../../700.reference/700.system-views/300.system-view-of-sys-tenant/200.dictionary-view-of-sys-tenant/7500.o-cdb_ob_backup_task_history-of-sys-tenant.md)を参照してください。

3. 対応するバックアップセット情報を確認します。
   ...
   *************************** 1. row ***************************
   ...
   `CDB_OB_BACKUP_SET_FILES`ビューの詳細については、[CDB_OB_BACKUP_SET_FILES](../../../700.reference/700.system-views/300.system-view-of-sys-tenant/200.dictionary-view-of-sys-tenant/7200.o-cdb_ob_backup_set_files-of-sys-tenant.md)を参照してください。
...
2. データバックアップ履歴を確認します。
   ...
   * JOBレベルのタスク履歴の確認
      ...
      テナントが開始する各バックアップに対応して、1件のJOBタスクレコードが作成されます。
