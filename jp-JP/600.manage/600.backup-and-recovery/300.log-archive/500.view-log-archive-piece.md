...
このセクションでは、ログアーカイブにおけるピースの表示方法のみを説明します。ピースの詳細については、[ログアーカイブの概要](../300.log-archive/100.overview-of-log-archive.md)を参照してください。

## システムテナントからすべてのテナントのすべてのピース情報を確認する

`sys` テナントは、`CDB_OB_ARCHIVELOG_PIECE_FILES` ビューを使用して、現在のクラスタ内のすべてのテナントのすべてのピース情報を確認できます。

1. `root` ユーザーでクラスタの `sys` テナントにログインします。

2. 以下のステートメントを実行して、すべてのテナントのすべてのピース情報を確認します。
   ...
   *************************** 6. row ***************************
   ...
   上記の例では、テナントIDが `1002` のテナントには4つのピース情報があり、テナントIDが `1004` のテナントには2つのピース情報があります。

   `1002` テナントの4つのピースについて：

   * `piece_id` がそれぞれ `1～3` の3つのピースの `round_id` はすべて `1` であり、これら3つのピースはすべて第1ラウンドのアーカイブで生成されたことを示しています。`piece 4` は第2ラウンドのアーカイブで生成されました。`piece 1` のログの連続範囲は `'2022-06-01 06:00:00.000000'` から `'2022-06-02 05:59:00.000000'` で、元のログデータ量は `25GB`、アーカイブされた実際のデータ量も `25GB`、データ圧縮率は `1.00` です。また、`piece 1` の `FILE_STATUS` フィールドの値は `DELETED` であり、このピースのデータは期限切れによりクリーンアップされたことを示しています。`STATUS` フィールドの値は `FROZEN` であり、このピースのデータは完全であり、今後ログがアーカイブされることはないことを示しています。

   * `piece id` が `2～4` の3つのピースの `FILE_STATUS` フィールドの値は `AVAILABLE` であり、これら3つのピースのデータはまだアクセス可能であり、削除されていないことを示しています。`piece 3` の `CHECKPOINT_SCN` が `'2022-06-04 06:00:00.000000'` に達する前に `FROZEN` に変わった理由は、ユーザーがアーカイブを無効にしたため、今後 `piece 3` にデータがアーカイブされることはないからです。現在、`1002` テナントは第2ラウンドのアーカイブを進めており、`piece 4` はそのラウンドで生成された最初のピースで、`STATUS` は `ACTIVE` であり、アーカイブ中のピースであることを示しています。
   `1004` テナントもアーカイブ中であり、このテナントの第1ラウンドのアーカイブです。すでに2つのピースが生成されており、`piece id` はそれぞれ `1` と `2` です。`piece 1` は完了しており、状態は `FROZEN` です。`piece 2` はアーカイブ中であり、状態は `ACTIVE` です。

## ユーザーテナントが自身のテナント内のすべてのピース情報を確認する

ユーザーテナントは、`oceanbase.DBA_OB_ARCHIVELOG_PIECE_FILES`ビュー（MySQLモード）および`sys.DBA_OB_ARCHIVELOG_PIECE_FILES`ビュー（Oracleモード）を使用して、自身のテナント内のすべてのピース情報を確認できます。
...
2. 以下のステートメントを実行して、現在のテナントのすべてのピースを確認します。

   MySQLテナントが自身のテナント内のすべてのピースを確認する例は次のとおりです：
...
`CDB_OB_ARCHIVELOG_PIECE_FILES`ビューの詳細については、[CDB_OB_ARCHIVELOG_PIECE_FILES](../../../700.reference/700.system-views/300.system-view-of-sys-tenant/200.dictionary-view-of-sys-tenant/5600.o-cdb_ob_archivelog_piece_files-of-sys-tenant.md)を参照してください。