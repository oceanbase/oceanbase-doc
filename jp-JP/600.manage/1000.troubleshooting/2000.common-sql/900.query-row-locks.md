...
ロックを保持またはロックを要求しているトランザクションの`trans_id`を取得した後、ビュー`GV$OB_TRANSACTION_PARTICIPANTS`を照会することで、そのトランザクションに対応する`session_id`を取得できます。
...
* [OceanBaseデータベースV4.2バージョン、ロック競合問題のトラブルシューティングマニュアル](https://www.oceanbase.com/knowledge-base/oceanbase-database-1000000000664387?back=kb) -->