...
インデックス名が取得できた場合は、ビュー`CDB_INDEXES`（システムテナント）、`information_schema.statistics`（MySQLユーザーテナント）、または`DBA_INDEXES`/`ALL_INDEXES`/`USER_INDEXES`（Oracleユーザーテナント）に直接クエリを実行し、インデックス名から対応する主表情報を取得することもできます。方法は以下のとおりです：
...
* システムテナント
  ...
  例えば、`tenant_id`が1002のテナント内のあるインデックスの名前が`tbl2_f_rl_idx1`だと仮定した場合、システムテナントでそのインデックスに対応する主表を照会するステートメントは以下のとおりです。
