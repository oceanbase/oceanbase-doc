...
以下のエラーが発生した場合、トランザクションのロールバックが発生していることを示します。
...
トランザクションのロールバックは必ずしも異常ではありません。トランザクションのタイムアウトや参加者によるリーダー切り替えなど、いくつかの状況でトランザクションのロールバックが発生する可能性があります。
...
Root Serviceによってトリガーされたリーダー切り替えでは、システムは既存のアクティブトランザクションに対して一定の時間を確保し、その実行完了を待ちます。同時に、新しいトランザクションの開始を許可しません。時間が経過すると、依然としてアクティブなすべてのトランザクションを新しいLeaderに移行します。その後、リーダーを切り替え、新しいLeaderがサービスを継続します。

一部のシナリオでは、リーダー切り替えによるトランザクションのキルが発生します：

* リーダー切り替えがRootServiceによってトリガーされていない場合、例えば異常なリーダー切り替えの場合
* 大規模トランザクションのシナリオ
* リーダー切り替えによって新しいLeaderへのトランザクション移行がトリガーされた際に、ステートメントの実行がまだ終了していない場合

トランザクションのロールバックエラーが発生した場合、ビュー`DBA_OB_SERVER_EVENT_HISTORY`を使用してリーダー切り替えが発生したかどうかを確認できます。以下の例は、switch replica leaderコマンドによってトリガーされたリーダー切り替えプロセスを示しています。
