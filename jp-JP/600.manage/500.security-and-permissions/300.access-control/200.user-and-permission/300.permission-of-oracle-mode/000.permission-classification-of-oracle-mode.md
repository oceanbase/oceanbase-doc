...
* オブジェクト権限：特定のオブジェクトに対する操作権限。例：あるテーブルオブジェクトに対するAlter、Select、Updateなどの権限
* システム権限：ユーザーがあるスキーマまたは任意のスキーマ上で特定のデータベース操作を実行することを許可する権限。

  <main id="notice" type='explain'>
    <h4>説明</h4>
    <p>システム権限は、オブジェクト権限よりはるかに大きいです。</p>
  </main>
...
| システム権限 | GRANT ANY ROLE              | 任意のロールに付与する権限。        |
...
| システム権限 | PURGE DBA_RECYCLEBIN        | リサイクルビンを空にする権限。         |
...
| システム権限 | PLAINACCESS | 機密データを含むすべての平文データにアクセスできるかどうかを決定します。これは機密ルールの制限を受けません。`GRANT/REVOKE ALL PRIVILEGES` にはこの権限は含まれません。SYSユーザーはデフォルトでこの権限を持ち、`REVOKE` することはできません。新しいクラスタの作成時やアップグレード後にも自動的にこの権限が付与されます。|
