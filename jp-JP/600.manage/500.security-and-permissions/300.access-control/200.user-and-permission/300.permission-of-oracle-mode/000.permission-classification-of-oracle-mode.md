...
* システム権限：ユーザーがあるSchemaまたは任意のSchema上で特定のデータベース操作を実行することを許可する権限。
  ...
  <main id="notice" type='explain'>
    <h4>説明</h4>
    <p>システム権限が提供する権限は、オブジェクト権限よりもはるかに広範です。</p>
  </main>
...
| システム権限 | CREATE TYPE                 | 型を作成する権限。          |
| システム権限 | CREATE ANY TYPE             | 任意の型を作成する権限。        |
| システム権限 | ALTER ANY TYPE              | 任意の型を変更する権限。        |
| システム権限 | DROP ANY TYPE               | 任意の型を削除する権限。        |
| システム権限 | EXECUTE ANY TYPE            | 任意の型を実行する権限。        |
...
| システム権限 | CREATE TABLESPACE           | テーブル領域を作成する権限。         |
| システム権限 | ALTER TABLESPACE            | テーブル領域を変更する権限。         |
| システム権限 | DROP TABLESPACE             | テーブル領域を削除する権限。         |
| システム権限 | SHOW PROCESS                | 全ユーザースレッドを表示する権限。      |
...
| システム権限 | CREATE LOCATION <main id="notice" type='explain'><h4>説明</h4><p>OceanBase データベース V4.4.x では、V4.4.1 から <code>CREATE LOCATION</code> 権限が導入されました。</p></main>| ユーザーが `CREATE LOCATION`、`ALTER LOCATION`、`DROP LOCATION` ステートメントを実行できるかどうかを決定します。|
| システム権限 | CREATE SENSITIVE RULE | ユーザーが `CREATE/DROP SENSITIVE RULE` ステートメントを実行できるかどうかを決定します。SYS ユーザーはデフォルトでこの権限を持ち、`REVOKE` することはできません。新しいクラスタの作成時やアップグレード後にも自動的にこの権限が付与されます。|
| システム権限 | PLAINACCESS | ユーザーが機密データを含むすべての平文データにアクセスできるかどうかを決定します。これは機密ルールの制限を受けません。`GRANT/REVOKE ALL PRIVILEGES` にはこの権限は含まれません。SYS ユーザーはデフォルトでこの権限を持ち、`REVOKE` することはできません。新しいクラスタの作成時やアップグレード後にも自動的にこの権限が付与されます。|
| システム権限 | RESTRICTED SESSION | ユーザーが制限モードでログインできるかどうかを決定します。制限モードは [ALTER SYSTEM ENABLE/DISABLE RESTRICTED SESSION](../../../../../700.reference/500.sql-reference/100.sql-syntax/300.common-tenant-of-oracle-mode/900.sql-statement-of-oracle-mode/100.ddl-of-oracle-mode/790.alter-system-enable-restricted-session-of-oracle-mode.md) で有効または無効にできます。|
| ルールレベル権限 | PLAINACCESS | ユーザーが特定のルールに関連付けられた列の平文データにアクセスできるかどうかを決定します。ルールを作成したユーザーは、そのルールの PLAINACCESS 権限を自動的には持ちません。別途権限を付与する必要があります。|

## 関連ドキュメント

OceanBaseデータベースの権限管理に関するその他の操作については、次の記事を参照してください：
