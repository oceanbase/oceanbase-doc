...
2. 以下のコマンドを実行して、ユーザー情報を確認します。
   ...
   * 管理者ユーザーが現在のテナント内のすべてのユーザーの詳細情報を確認する
      ...
      * `LBACSYS`：システム組み込みのデータベースセキュリティ管理者。デフォルトでロック状態になっています。使用する前にこのユーザーのロックを解除する必要があります。

      * `ORAAUDITOR`：システム組み込みのデータベース監査員。デフォルトでロック状態になっています。使用する前にこのユーザーのロックを解除する必要があります。
