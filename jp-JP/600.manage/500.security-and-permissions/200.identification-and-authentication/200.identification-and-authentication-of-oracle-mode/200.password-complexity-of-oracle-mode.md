...
OceanBaseデータベースのOracleモードにおけるパスワードの複雑さ戦略は、Oracleデータベースのパスワード戦略と互換性があります。悪意のあるパスワード攻撃を防ぎ、データベースのセキュリティを向上させるため、OceanBaseデータベースではユーザーが必要に応じてパスワードの複雑さ関数を設定し、ユーザーのログイン時の身元を検証できます。
...
Oracleモードでは、ユーザーはProfileの`PASSWORD_VERIFY_FUNCTION`属性を使用してパスワードの複雑さが要件を満たしているかを検証できます。まず、パスワードの複雑さを検証するためのPL Functionを作成する必要があり、以下のインターフェースを満たす必要があります：
...
ユーザーの作成時やパスワードの変更時に、この検証関数が呼び出され、関数の実行結果に基づいて新しいパスワードが複雑さ要件を満たしているかを判断されます。

## パスワードの複雑さを設定し、パスワード複雑さ検証を有効にする

OceanBaseデータベースのOracleモードは、パスワードの複雑さが要件を満たしているかを検証するための`PASSWORD_VERIFY_FUNCTION`パラメータを提供します。

`PASSWORD_VERIFY_FUNCTION`パラメータにより、PLのパスワード複雑さ検証関数をパラメータとして`CREATE PROFILE`および`ALTER PROFIL`ステートメントに渡すことが可能になります。

<main id="notice" type='notice'>
    <h4>注意</h4>
    <p>本番環境では、パスワードの総文字数を20文字に設定し、数字、大文字、小文字、および特殊文字を含めることを推奨します。ユーザー名の含まれるパスワードや繰り返し文字を使用したパスワードなどパスワードの複雑さが低い場合、解読される可能性が高くなります。セキュリティ上の理由から、ユーザーのパスワードは高い複雑さを備えている必要があります。
    </li></p>
  </main>
...
2. `PASSWORD_VERIFY_FUNCTION`パラメータに対応する関数を作成し、権限を付与します。
   ...
   * パスワードの総文字数は、最小8文字、最大256文字です。

   * 英大文字、英小文字、特殊文字をそれぞれ１文字以上含む必要があります。
   ...
   * パスワードは、ユーザー名、ユーザー名の逆順、またはサーバー名であってはなりません。
   ...
   例：
    ...
    (username varchar2,
        ...
        -- この例では3つの関数パラメータをカスタマイズしています---ユーザー名username、新しいパスワードpassword、古いパスワードold_passwordです。
    RETURN boolean IS
        differ integer;
...
3. UserのProfileに`PASSWORD_VERIFY_FUNCTION`パラメータを指定し、パスワード複雑さ検証を有効にします。
   ...
   ここでの`profile_name`はカスタムのProfileを使用することも、`DEFAULT` Profileを使用することもできます。
...
パスワードの複雑さ設定が完了した後、次回具体的な複雑さルールを確認する必要がある場合は、関連する関数の内容をクエリすることで取得できます。
