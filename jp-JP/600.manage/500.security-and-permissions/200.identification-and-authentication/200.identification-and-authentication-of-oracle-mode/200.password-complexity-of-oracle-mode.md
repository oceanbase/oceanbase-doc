...
OceanBaseデータベースのOracleモードにおけるパスワードの複雑さポリシーは、Oracleデータベースのパスワードポリシーと互換性があります。悪意のあるパスワード攻撃を防ぎ、データベースのセキュリティを向上させるため、OceanBaseデータベースではユーザーが必要に応じてパスワードの複雑さ関数を設定し、ユーザーのログイン身元を検証できます。
...
ユーザーの作成時やパスワードの変更時に、この検証関数が呼び出され、関数の実行結果に基づいて新しいパスワードが複雑さ要件を満たしているかどうか判断されます。

## パスワードの複雑さを設定し、パスワード複雑さ検証を有効にする

OceanBaseデータベースのOracleモードは、パスワードの複雑さが要件を満たしているかどうかを検証するための`PASSWORD_VERIFY_FUNCTION`パラメータを提供します。

`PASSWORD_VERIFY_FUNCTION`パラメータにより、PLのパスワード複雑さ検証関数を`CREATE PROFILE`および`ALTER PROFIL`ステートメントのパラメータとして渡すことが可能になります。

<main id="notice" type='notice'>
    <h4>注意</h4>
    <p>本番環境では、パスワードの長さを20文字に設定し、数字、大文字、小文字、および特殊文字を含めることを推奨します。パスワードの複雑さが低い場合、ユーザー名の含まれるパスワードや繰り返し文字を使用したパスワードなど、解読される可能性が高くなります。セキュリティ上の理由から、ユーザーのパスワードは高い複雑さを備えている必要があります。
    </li></p>
  </main>
...
2. `PASSWORD_VERIFY_FUNCTION`パラメータに対応する関数を作成し、権限を付与します。
   ...
   * パスワードは、ユーザー名、ユーザー名の逆順、またはサーバー名であってはなりません。
   ...
   関数の作成に関する操作については、[関数の作成](../../../../700.reference/600.pl-reference/300.pl-oracle/700.stored-procedure-and-functions-oracle/300.create-a-function-oracle.md)を参照してください。

3. UserのProfileに`PASSWORD_VERIFY_FUNCTION`パラメータを指定し、パスワード複雑さ検証を有効にします。
   ...
   ここで、`profile_name`にはカスタムのProfileを使用することも、`DEFAULT` Profileを使用することもできます。

   `DEFAULT` Profileを使用して`PASSWORD_VERIFY_FUNCTION`パラメータを指定する例：
...
4. 完了後、以下のステートメントを実行して、パスワード複雑さ検証が有効になっているか確認できます。

   戻り値が`NULL`の場合、パスワード複雑さ検証は有効になっていません。先ほど指定した関数の場合、パスワード複雑さ検証が有効になっています。
...
2. 要件を満たすパスワードと満たさないパスワードを使用して、それぞれユーザー`sectest1`を作成します。
...
パスワードの複雑さ設定が完了した後、次回具体的な複雑さルールを確認する必要がある場合は、関連する関数の内容を照会することで取得できます。
...
3. `USER_SOURCE`ビューで、現在のユーザーが所有する関数`VERIFY_FUNCTION`の内容を確認します。
   ...
   `USER_SOURCE`ビューは、システム内の現在のユーザーが所有するすべてのストアドプロシージャと関数を記録しています。
...
関数管理に関するその他の操作については、[ストアドプロシージャと関数の管理](../../../../700.reference/600.pl-reference/300.pl-oracle/700.stored-procedure-and-functions-oracle/400.view-and-delete-procedures-and-functions-oracle.md)を参照してください。