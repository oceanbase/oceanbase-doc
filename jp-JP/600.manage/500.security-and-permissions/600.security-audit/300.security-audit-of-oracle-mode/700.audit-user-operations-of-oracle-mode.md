...
## 指定ユーザーによるすべてのテーブルに対する成功したDDL操作の監査
...
2. `DBA_AUDIT_TRAIL` テーブルで、ユーザー `user1` が実行した成功した監査レコードを確認します。
...
2. `DBA_AUDIT_TRAIL` テーブルで、ユーザー `user1` の監査レコードを確認します。
