...
ターゲットのSQLリクエストログを容易に検索できるようにするために、事前にログ出力レベルを設定して、SQLモジュール以外のログの出力を削減できます。ログ出力レベルの関連設定については、[ログ出力レベルの設定](../200.log-level.md)を参照してください。
...
1. Trace機能を有効にします。
   ...
   * Session変数`ob_enable_show_trace`を設定してTrace機能を有効にします。この方法は、このSessionの後続のすべてのステートメントに適用されます。
...
2. `SHOW TRACE`使用してSQLの実行情報を確認します。

   Trace機能を有効にしてSQLリクエストを実行した後、`SHOW TRACE`ステートメントを使用して、前回のSQLステートメントの詳細な実行追跡情報を取得できます。
   ...
   `ElapseTime`フィールドは、このステップで費やした時間をミリ秒単位で示します。例えば：`pc_get_plan`の`ElapseTime`フィールドが`0.025 ms`と表示されている場合、OceanBaseデータベースがPlan Cacheから実行計画を取得するのに0.025 msかかったことを意味します。
   主要な関連Operationの紹介は以下のとおりです。その他のOperationタイプについては、[全リンクトレースプロセス](../../900.daily-inspection/900.full-link-detection/200.full-link-tracking-process.md)の**span**を参照してください。
   ...
   * optimize：コストに基づく最適化を行い、実行計画ログを生成。
   ...
   * get_das_id：dasタスクIDを取得。

   * do_local_das_task：ローカルdasタスクを実行。

   * px_schedule：pxに基づいてタスクをスケジューリング。
...
3. `trace_id`使用して、ログファイル内の前回のSQLリクエストの完全なログを検索します。

   OceanBaseデータベースのログ出力には常に`trace_id`が含まれているため、ログファイル(`observer.log`、`election.log`、`rootservice.log`)内で対応する`trace_id`を検索することで、前回のSQLリクエストの完全なログを取得できます。

   1. `last_trace_id()`使用して`trace_id`を取得します。
