...
obdiagは、OceanBase専用に設計されたブラックボックス診断ツールです。その機能は、OceanBaseのログ、SQL監査、およびOceanBaseプロセススタックなどの情報のスキャン、収集、分析を網羅しています。OCPやobdでデプロイする場合でも、ドキュメントに従って手動でデプロイする場合でも、obdiagはワンクリックで診断情報の収集と分析を迅速に完了できます。
...
-- 現在のテナント内の時間情報を記録し、すべて記録してすべて出力する。
...
エンドツーエンド・トレースの結果には、最も時間がかかった上位5つのLeaf Spanと、各Spanの詳細情報およびタグが含まれます。