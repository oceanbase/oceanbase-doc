...
obdiagは、OceanBase専用に設計されたブラックボックス診断ツールです。その機能は、OceanBaseのログ、SQL監査、およびOceanBaseプロセススタックなどの情報のスキャン、収集、分析をカバーしています。OCPやobdでのデプロイ、あるいはドキュメントに従って手動でデプロイする場合でも、obdiagはワンクリックで診断情報の収集と分析を迅速に完了できます。

このドキュメントでは、obdiagを使用してOceanBaseデータベースのフルリンクトレースを迅速に実行する方法を説明します。

## obdiagのインストールとデプロイ

1. [obdiag](https://en.oceanbase.com/softwarecenter)をダウンロードし、ダウンロードしたバージョンがv.1.5.0以降であることを確認してください。

2. 公式ドキュメント[アジャイル診断ツールobdiag](https://www.oceanbase.com/docs/common-obdiag-cn-1000000000493923)を参照して、obdiagのインストール、デプロイ、設定を完了してください。

## 手順

### ステップ1（オプション）：テナントのTraceをリセットする

包括的な診断を行う際にすべての結果を出力したい場合は、**テナントのTraceをリセットする**操作を実行できます。

OBServerに接続した後、次のSQLコマンドを実行します：

```sql
-- Traceを無効にする
call dbms_monitor.ob_tenant_trace_disable();
-- 現在のテナント内の時間消費情報を記録し、すべてを記録してすべて出力する。
call dbms_monitor.ob_tenant_trace_enable(1, 1, 'ALL');
```

### ステップ2：SQLステートメントを照会してテストを行う

ここでは、SQLクエリステートメントを例に、フルリンク機能の診断方法を示します。

```sql
select * from tx where c1=1 and c1=2;

-- SQL Auditでは、`query_sql` を使用して疑わしい遅延SQLの `flt_trace_id` を検索します
...
### ステップ3：フルトレースコマンドを実行する

OBServerを終了し、次のコマンドを実行してフルトレースを実行します。

```shell
-- ステップ2で取得した`flt_trace_id`と設定済みの環境を使用してフルトレースコマンドを実行します：
...
### ステップ4：フルトレースの詳細を確認する

```bash
$ cat analyze_pack_20231008171201/result_details.txt
```

フルトレースの結果には、時間消費量が上位5つのLeaf Spanと、各Spanの詳細情報およびタグが含まれます。