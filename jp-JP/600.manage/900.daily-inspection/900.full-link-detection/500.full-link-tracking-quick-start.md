...
obdiagは、OceanBase専用に設計されたブラックボックス診断ツールです。その機能は、OceanBaseのログ、SQL監査、およびOceanBaseプロセススタックなどの情報のスキャン、収集、分析を網羅しています。OCPやobdでのデプロイであれ、ドキュメントに従って手動でデプロイする場合でも、obdiagはワンクリックで診断情報の収集と分析を迅速に完了できます。

このドキュメントでは、obdiagを使用してOceanBaseデータベースのエンドツーエンドトレースを迅速に実行する方法を説明します。
...
### ステップ1（オプション）：テナントTraceをリセットする

包括的な診断時にすべての結果を出力する場合は、**テナントTraceをリセットする**操作を実行できます。
...
-- 現在のテナント内の時間消費情報を記録し、すべて記録してすべて出力する。
call dbms_monitor.ob_tenant_trace_enable(1, 1, 'ALL');
```

### ステップ2：SQL文をクエリしてテストする

ここでは、SQLクエリ文を例に、エンドツーエンド機能診断の実行方法を示します。

```sql
select * from tx where c1=1 and c1=2;

-- SQL Auditで、`query_sql`から疑わしいスローSQLの`flt_trace_id`を検索する
...
### ステップ3：エンドツーエンド・トレースコマンドを実行する

OBServerを終了し、以下のコマンドを実行してエンドツーエンド・トレースを開始します。

```shell
-- ステップ2で取得した `flt_trace_id` と設定済みの環境変数を使用して、エンドツーエンド・トレースコマンドを実行します。
...
エンドツーエンド・トレースの結果には、時間消費量が最も多い上位5つのリーフスパン（Leaf Span）および各スパンの詳細情報とタグが含まれます。