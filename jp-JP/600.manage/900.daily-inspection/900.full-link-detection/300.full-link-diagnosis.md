...
運用保守担当者は、必要に応じてPL/SQLパッケージDBMS_MONITORの関連メソッドを使用して、アプリケーションが異なる識別情報の観点でフルリンクトレースを有効にするかどうか、およびそのトレースのイベント記録と関連情報をトレースログに出力するポリシーを制御できます。ログの出力はサンプリング頻度に基づいてサンプリングされ、メモリに記録された後、一定の確率でトレースログに出力されます。
...
- `record_policy`: ログ出力ポリシー、つまりtrace情報をログファイルに出力するポリシーです。以下の3種類のポリシーをサポートしています。
...
以下の例では、現在のテナント内の時間消費情報をすべて記録し、すべて出力します。
