...
|slug|use-datetime-functions-in-queries-oracle-compatible-mode|

# クエリでの時間関数の使用

Oracleテナントで一般的に使用される時間型には、date、timestamp、intervalなどがあります。

Oracleテナントでよく使われるデータベース時間関数はsysdateとsystimestampです。両者のデフォルト表示形式は、パラメータNLS_DATE_FORMATとNLS_TIMESTAMP_FORMATによって決定されます。interval型の時間関数にはnumtodsintervalやnumtoymintervalがあり、数値とinterval型との間で変換できます。

* 例：時間をフォーマットして表示する。

     Oracleテナントでは、time型の表示形式を調整するにはto_char関数を使用できます。SQLは以下のとおりです：
...
* 例：時間から年/月/日/時/分/秒を抽出する。

     Oracleテナントでは、時間から年/月/日/時/分/秒を抽出するにはto_char関数を使用できます。SQLは以下のとおりです：
...
* 例：時間型の加算と減算。
     ...
     Oracleテナントでは、時間の加算と減算は、直接（単位は日）行うことも、NUMTODSINTERVAL **、** add_months 関数を利用することもできます。
