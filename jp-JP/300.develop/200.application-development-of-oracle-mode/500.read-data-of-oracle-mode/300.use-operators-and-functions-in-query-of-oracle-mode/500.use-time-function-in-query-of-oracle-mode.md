...
Oracleテナントで一般的に使用されるデータベース時間関数はsysdateとsystimestampであり、両者のデフォルト表示形式はパラメータNLS_DATE_FORMATとNLS_TIMESTAMP_FORMATによって決定されます。intervalタイプの時間関数にはnumtodsinterval、numtoymintervalがあり、数値とintervalタイプの間で変換できます。
