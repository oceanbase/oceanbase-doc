...
Oracleテナントで一般的に使用される日付時刻型にはdate、timestamp、intervalなどがあります。その他の日付時刻型の使い方については、**「OceanBase SQLステートメント（Oracleモード）」**を参照してください。
...
* 例：時間表示のフォーマット。
     ...
     Oracleテナントでは、日付時刻型の表示形式を調整するためにto_char関数を使用できます。SQLは以下のとおりです：
...
* 例：時間から年/月/日/時/分/秒を抽出します。
     ...
     Oracleテナントでは、時間から年/月/日/時/分/秒を抽出するためにto_char関数を使用できます。SQLは以下のとおりです：
...
* 例：日付時刻型の加減算。
     ...
     Oracleテナントでは、時間の加減算を直接行うことも、関数NUMTODSINTERVALやadd_months関数を利用することもできます。
