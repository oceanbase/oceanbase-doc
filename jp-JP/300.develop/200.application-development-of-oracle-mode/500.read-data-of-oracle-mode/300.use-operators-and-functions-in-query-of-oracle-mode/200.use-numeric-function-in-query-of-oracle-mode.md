...
|slug|use-numerical-functions-in-queries-oracle-compatible-mode|

# クエリでの数値関数の使用

よく使われる数値関数には、合計関数SUM()、平均値関数AVG()、切り上げ関数CEIL()、切り捨て関数FLOOR()、数値切り捨て関数TRUNC()、丸め関数ROUND()などがあります。
...
3. 各おもちゃの総売上高と1注文あたりの平均売上高をクエリで求めます。
