...
|slug|use-string-concatenation-operator-in-queries-oracle-compatible-mode|

# クエリでの文字列連結演算子の使用

文字列連結演算子として、Oracleテナントでは通常「\`\|\|\`」が使用されます。
...
4. 「\|\|」を文字列連結演算子として使用し、`tif_ny` テーブルの `f_name` 列と `l_name` 列を連結して `name` 列を作成します。
