...
|slug|use-aggregate-functions-in-queries-oracle-compatible-mode|

# クエリにおける集約関数の使用

集約関数は一連のレコードをスキャンし、単一行の結果を返します。この一連のレコードは、テーブルまたはビュー、あるいはサブクエリの結果です。OceanBaseがサポートする集約関数の詳細については、「OceanBase SQLステートメント（Oracleモード）」を参照してください。

集約関数は通常GROUP BY句と共に使用され、1つまたは複数の列の値でグループ分けを行い、各グループごとに1件のレコードを返します。

## 例

おもちゃの注文における売上高を集計します。

1. サンプルテーブル `toys_order_ny` を作成し、注文データを挿入します。
...
3. 集約関数を使用しておもちゃの注文における売上高を集計します。
...
OceanBaseデータベースの現在のバージョンでサポートされている集約関数の詳細については、マニュアル **「OceanBaseデータベースSQLステートメント（Oracleモード）」** を参照してください。