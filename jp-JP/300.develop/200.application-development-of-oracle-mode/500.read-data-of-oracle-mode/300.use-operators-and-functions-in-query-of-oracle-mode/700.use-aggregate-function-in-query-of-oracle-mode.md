...
集約関数は、1組のレコードをスキャンし、1行のレコードを返します。このレコードセットは、テーブルまたはビュー、あるいはサブクエリの結果です。OceanBaseがサポートする集約関数の詳細については、『OceanBase SQLステートメント（Oracleモード）』を参照してください。

集約関数は通常、GROUP BY句と共に使用され、1つまたは複数の列の値に基づいてグループ化し、グループごとに1つのレコードを返します。
...
OceanBaseデータベースが現在のバージョンでサポートする集約関数の詳細については、マニュアル **『OceanBaseデータベースSQLステートメント（Oracleモード）』** を参照してください。
