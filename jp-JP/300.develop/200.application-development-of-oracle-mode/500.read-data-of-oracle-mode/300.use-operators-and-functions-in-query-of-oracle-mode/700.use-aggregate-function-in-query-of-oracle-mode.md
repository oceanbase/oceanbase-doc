...
集計関数は一連のレコードをスキャンし、その後単一行のレコードを返します。この一連のレコードは、テーブルまたはビュー、あるいはサブクエリの結果である可能性があります。OceanBaseがサポートする集計関数の詳細については、「OceanBase SQL文（Oracleモード）」を参照してください。

集計関数は通常GROUP BY句と一緒に使用され、1つまたは複数の列の値に基づいてグループ化し、各グループから単一のレコードを返します。
...
OceanBaseデータベースの現在のバージョンでサポートされている集計関数の詳細については、マニュアル「OceanBaseデータベースSQL文（Oracleモード）」を参照してください。