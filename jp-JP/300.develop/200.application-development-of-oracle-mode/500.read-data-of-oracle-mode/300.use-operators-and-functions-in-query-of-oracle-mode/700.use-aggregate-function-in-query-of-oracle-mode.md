|description||
|---|---|
|keywords||
|dir-name||
|dir-name-en||
|tenant-type|Oracle Mode|
|slug|use-aggregate-functions-in-queries-oracle-compatible-mode|

# クエリでの集約関数の使用

集約関数は一連のレコードをスキャンし、単一の行を返します。この一連のレコードは、テーブルまたはビュー、あるいはサブクエリの結果です。OceanBaseがサポートする集約関数の詳細については、「OceanBase SQLステートメント（Oracleモード）」を参照してください。

集約関数は通常GROUP BY句と共に使用され、1つ以上の列の値でグループ分けを行い、各グループごとに1件のレコードを返します。

## 例

おもちゃの注文の売上高を集計します。

1. サンプルテーブル `toys_order_ny` を作成し、注文データを挿入します。

   ```unknow
   obclient> CREATE TABLE toys_order_ny (
              order_id     NUMBER,
              toy_id       NUMBER,
              toy_dealer   VARCHAR2(20),
              toy_price    NUMBER,
              toy_amount   NUMBER
             );
   Query OK, 0 rows affected
   ```

2. `toys_order_ny` テーブルにデータを挿入します。

   ```unknow
   obclient> INSERT INTO toys_order_ny VALUES ( 1001,1, 'A',53.12,100 ),
             ( 1002,2, 'A',24.8,190 ),( 1003,3, 'A',19.9,330)
              ,( 1004,1, 'B',53.12,50 ),( 1005,2, 'B',24.8,200 ),
             ( 1006,3, 'B',19.9,350 ),( 1007,2, 'A',24.8,100 );
   7 rows in set
   ```

3. 集約関数を使用しておもちゃの注文の売上高を集計します。

   ```unknow
   obclient> SELECT toy_id
             , count(*)                         order_count
             , sum(toy_amount)                  sum_amount
             , round(avg(toy_amount),2)         avg_amount
             , min(toy_amount)                  min_amount
             ,max(toy_amount)                   max_amount
             FROM toys_order_ny GROUP BY toy_id  ORDER BY toy_id;
   +--------+-------------+------------+------------+------------+------------+
   | toy_id | order_count | sum_amount | avg_amount | min_amount | max_amount |
   +--------+-------------+------------+------------+------------+------------+
   |      1 |           2 |        150 |      75.00 |         50 |        100 |
   |      2 |           3 |        490 |     163.33 |        100 |        200 |
   |      3 |           2 |        680 |     340.00 |        330 |        350 |
   +--------+-------------+------------+------------+------------+------------+
   3 rows in set
   ```

OceanBaseデータベースの現在のバージョンでサポートされている集約関数の詳細については、マニュアル **「OceanBaseデータベースSQLステートメント（Oracleモード）」** を参照してください。