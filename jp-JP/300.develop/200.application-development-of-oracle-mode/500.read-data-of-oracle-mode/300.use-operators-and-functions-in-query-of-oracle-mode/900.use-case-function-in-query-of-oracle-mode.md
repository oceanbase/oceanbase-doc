...
CASE式は、サブルーチンを呼び出すことなく、`IF...THEN...ELSE` に似たロジックを実現できます。CASE式には、単純な形式と検索条件付きの2種類の使用方法があります。

## 例

### 例1

クエリ内で単純なCASE式を使用して、国コードの略称を正式名称に翻訳します。
...
3. `CASE` 式を使用して、国コードに対応する正式名称を照会します。
...
3. 検索条件付きのCASE式を使用して、`Duration`列のデータを照会します。
