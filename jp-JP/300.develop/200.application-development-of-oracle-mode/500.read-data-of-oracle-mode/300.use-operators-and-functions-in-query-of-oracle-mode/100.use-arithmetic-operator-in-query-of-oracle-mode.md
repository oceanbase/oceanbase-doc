...
クエリで一般的な算術演算子には、+（加）、-（減）、\*（乗）、/（除）、-（符号反転）、MOD（剰余）があります。これらの演算子は数値データ型の列に対して使用できます。この記事では、具体的な例を通じてクエリ内で算術演算子を使用する方法を説明します。

## 例

条件に合致する顧客が購入した各商品の数量と価格を取得します。数量に価格を掛けることで、各商品の支払総額が得られます。`select_list` に列 `t2.ol_quantity * t3.i_price` を追加し、`item_sum_price` という名前に変更します。
...
7. 条件に合致する顧客が購入した各商品の数量と価格を取得します。
