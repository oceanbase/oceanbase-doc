...
クエリでよく使われる算術演算子には、+（加）、-（減）、*（乗）、/（除）、-（反転）、MOD（剰余）があります。これらの演算子は数値データ型の列に適用できます。本記事では具体的な例を通して、クエリ内で算術演算子をどのように使用するかを紹介します。

## 例

条件に合致する顧客が購入した各商品の数量と価格を照会します。数量に価格を乗じることで、各商品カテゴリの支払い総額が求められます。このため、`select_list` に `t2.ol_quantity * t3.i_price` という列を追加し、`item_sum_price` という名前に変更します。
...
7. 条件に合致する顧客が購入した各商品の数量と価格を照会します。
