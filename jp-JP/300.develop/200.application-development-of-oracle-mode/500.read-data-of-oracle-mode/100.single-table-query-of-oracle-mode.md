...
| LIMIT limit_clause          | オプションのパラメーターで、クエリで返す行数を制限し、ページネーションクエリ機能を実装します。オフセットと、返す行数または行のパーセンテージを指定できます。ORDER BY句を組み合わせてソート順序を指定することで、一貫性のある結果を得ることができます。|
...
テーブル `emp` のデータをクエリし、テーブルエイリアス `t` を指定します。
...
OceanBaseデータベースのOracleモードでは、`SELECT * FROM FUNCTION(*);` の構文を使用して、テーブルタイプを返す関数を呼び出し、その関数の戻り値をテーブルデータとしてクエリすることができます。

`SELECT * FROM FUNCTION(*);` を使用する場合、以下の点に注意が必要です：

* 関数が返すデータタイプはテーブルタイプでなければなりません。

* テーブル関数が返す列の数およびタイプは、呼び出し元が要求する列の数およびタイプと完全に一致する必要があります。

* テーブル関数が返す行数は、呼び出し元が要求する行数以上である必要があります。

* 関数のパラメーターの数とタイプは、関数定義と一致する必要があります。
...
2. テーブルタイプを含むセット型 `emp_type_list` を定義します。
...
3. テーブルタイプを返す関数 `get_emp_info` を作成します。
...
指定した列の値がターゲットと一致するデータをクエリします。値が文字列タイプの場合は、シングルクォートまたはダブルクォートで囲む必要があります。
...
構文の意味は、指定されたプロパティの列の値が `pattern` とマッチするタプルをクエリすることです。`pattern` は完全な文字列である場合もあれば、ワイルドカード `%` と `_` を含む場合もあります。その中で：
...
以下の例は、従業員名の最初の4文字が `ALLE` で、最後の1文字が任意の文字であるデータをクエリするものです。
...
以下の例は、従業員名の最初の文字が `A` であるデータをクエリするものです。
...
`IS NULL` は、指定された列の値が `NULL` であるデータをクエリするために使用されます。一般的なフォーマットは次のとおりです：
...
`IS NOT NULL` は、指定された列の値が `NOT NULL` のデータをクエリするために使用されます。一般的なフォーマットは次のとおりです：
...
各顧客が毎年行った注文数をクエリし、顧客ID、注文年、注文数を出力します。
...
2020年の各顧客の注文数をクエリし、顧客IDと注文数を出力します。
...
2019年に1件以上の注文があった顧客をクエリし、顧客IDと注文数を出力します。
...
各顧客の注文の最大金額をクエリし、その顧客IDと最大金額を金額の降順で出力します。
...
### numが最も低いデータのうち、上位30%のデータを取得し、かつ前の例で取得した最後のデータと同じ値を持つその他すべてのデータもクエリする
...
<!-- * クエリステートメントの最適化に関する詳細については、[クエリ書き換えの概要](../../../700.reference/1000.performance-tuning-guide/500.sql-optimization/400.sql-optimization/500.query-rewrite/200.rule-based-query-rewriting.md) を参照してください。 -->

* クエリステートメントの最適化に関する詳細については、[クエリ書き換えの概要](https://en.oceanbase.com/docs/common-oceanbase-database-10000000001974686) を参照してください。