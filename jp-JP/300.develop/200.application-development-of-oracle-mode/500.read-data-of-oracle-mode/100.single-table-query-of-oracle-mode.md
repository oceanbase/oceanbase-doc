...
[SELECT](../../../700.reference/500.sql-reference/100.sql-syntax/300.common-tenant-of-oracle-mode/900.sql-statement-of-oracle-mode/200.dml-of-oracle-mode/500.select-of-oracle-mode/100.simple-select-of-oracle-mode.md) ステートメントを使用してデータのクエリを実行してください。
...
| GROUP BY group_by_condition | オプションのパラメータで、指定された列に基づいて結果をグループ化します。集約関数と併用することが一般的です。|
...
| LIMIT limit_clause          | オプションのパラメータで、クエリで返す行数を制限し、ページネーションクエリ機能を実現します。オフセットと、返す行数または行の割合を指定できます。ORDER BY句を組み合わせてソート順序を指定することで、一貫性のある結果を得ることができます。|
...
3. `GROUP BY` を実行して、取得した各行をグループ化します。`GROUP BY` がない場合は、全体が1つのグループとなります。
...
7. `ORDER BY` を実行して、条件に基づいて結果を昇順または降順にソートします。
...
* テーブル関数が返す列数と列型は、呼び出し元が必要とする列数と列型と一対一で対応している必要があります。

* テーブル関数が返す行数は、呼び出し元が必要とする行数以上である必要があります。

* 関数の引数の数と型は、関数定義と一致している必要があります。
...
不等号には `<>` と `!=` の2種類の書き方があります。一般的なフォーマットは次のとおりです：
...
大于（`>`）和小于（`<`）は、数値の大小を比較します。文字を比較する場合は、ASCIIコードに対応する値に基づいて比較し、文字の対応する位置を順に比較します。一般的なフォーマットは次のとおりです：
...
論理クエリ演算子である「かつ」（`AND`）と「または」（`OR`）は、複数の条件を持つクエリをサポートします。
...
いずれか一方の条件を満たすデータはすべて返されます。一般的な形式は次のとおりです：
...
構文の意味は、指定された属性列の値が `pattern` と一致するタプルを検索することです。`pattern` は完全な文字列でも、ワイルドカード `%` と `_` を含む文字列でもかまいません。ここで：
...
```
  ...
  <main id="notice" type='notice'>
    <h4>注意</h4>
    <p>データベースの文字セットが ASCII の場合、1つの全角文字には2つの <code>_</code> が必要です。文字セットが GBK の場合は、1つの <code>_</code> だけが必要です。</p>
  </main>

### 区間クエリ（BETWEEN AND）

演算子 `BETWEEN ... AND` は、2つの値の間にあるデータを選択します。これらの値は、数値、テキスト、または日付です。一般的な形式は次のとおりです：
...
```
  ...
  <main id="notice" type='notice'>
    <h4>注意</h4>
    <p>区間クエリの2つの境界値の順序を入れ替えないでください。必ず左側の値以上、かつ右側の値以下である必要があります。</p>
  </main>
...
演算子 `IN` は、`WHERE` 句で複数の値を指定し、それらを集合として扱うことを許可します。`IN` は、指定された列のデータが集合内のいずれかの値と一致する場合にそのデータを返すことを意味します。`NOT IN` は、集合内のどの値とも一致しないデータを返すことを意味します。一般的な形式は次のとおりです：
...
比較演算子、`LIKE`、`BETWEEN AND`、`IN`、`NOT IN` は `NULL` 値に対して正確ではないため、NULL値専用のクエリステートメントである `IS NULL` と `IS NOT NULL` の使用を推奨します。

#### IS NULL

`IS NULL` は、指定された列の値が `NULL` であるデータを検索するために使用されます。一般的な形式は次のとおりです：
...
`GROUP BY` は単一フィールドのグループ化と複数フィールドのグループ化をサポートしています。`WHERE` 句を使用するとグループ化の前にデータをフィルタリングでき、`HAVING` 句を使用するとグループ化後にデータをフィルタリングでき、`ORDER BY` 句を使用するとグループ化後にデータをソートできます。
...
各顧客の注文の最大金額を照会し、最大金額の降順で顧客IDと最大金額を出力します。

```shell
obclient [SYS]> SELECT user_id 顧客ID, MAX(fruit_price) 最大金額 FROM fruit_order t GROUP BY user_id ORDER BY 最大金额 DESC;
```
...
`ORDER BY` は `WHERE` 句の後に記述してソートを行うことができます。例：
...
`NLSSORT` 関数を使用して、中国語のピンインソートや筆順ソートなどを行うことができます。
...
Row_Limiting_Clauseは、`SELECT` クエリが返す行数を制限するために使用され、ページネーション操作でよく用いられます。

<main id="notice" type='explain'>
  <h4>説明</h4>
  <p>ページネーションクエリの詳細情報とベストプラクティスについては、<a href="600.pagination-query-of-oracle-mode.md">ページネーションクエリ</a>を参照してください。</p>
</main>
...
### numがデータの下位30%に含まれる最初の30件のデータ、および前の例で取得した最後のデータと同じ値を持つ他のすべてのデータを照会する
...
* ページネーションクエリの詳細については、[ページネーションクエリ](600.pagination-query-of-oracle-mode.md)を参照してください。
