...
* `INSERT` または `CREATE TABLE` 文で、挿入先のターゲットテーブルの行セットを定義します。

* `CREATE VIEW` 文で、ビューに含める行セットを定義します。

* `UPDATE` 文で、既存の行に割り当てる1つまたは複数の値を定義します。

* `SELECT`、`UPDATE`、`DELETE` 文の `WHERE` 句、`HAVING` 句、または `START WITH` 句の条件に値を提供します。

## サブクエリのキーワード

サブクエリで使用されるキーワードには、`IN`、`ANY`、`SOME`、`ALL` などがあります。

* `IN` は `WHERE` 式でよく使われ、特定の範囲内のデータを検索するために使用されます。

* `ANY` と `SOME` は `=`、`>`、`>=`、`<`、`<=`、`<>` 演算子と組み合わせて使用でき、それぞれいずれかのデータと等しい、より大きい、以上、より小さい、以下、等しくないことを意味します。

* `ALL` は `=`、`>`、`>=`、`<`、`<=`、`<>` 演算子と組み合わせて使用でき、それぞれすべてのデータと等しい、より大きい、以上、より小さい、以下、等しくないことを意味します。
...
* 列サブクエリは、1列の複数行の結果セットを返します。
   ...
   列サブクエリと集約関数 `min` を使用して、職位 `job` が `MANAGER` のすべての給与よりも低い給与の従業員の従業員番号、氏名、職位、給与を返します。
...
`EXISTS` サブクエリは、まずメインクエリを実行してその結果を取得し、その後サブクエリに基づいてフィルタリングを行います。サブクエリにはメインクエリで使用されるフィールドが含まれているため、相関サブクエリと呼ばれます。`EXISTS` クエリの結果は1または0であり、サブクエリの結果セットに値が存在するかどうかを判断するために使用されます。一般的に、`EXISTS` サブクエリは `IN` で置き換えることができます。
