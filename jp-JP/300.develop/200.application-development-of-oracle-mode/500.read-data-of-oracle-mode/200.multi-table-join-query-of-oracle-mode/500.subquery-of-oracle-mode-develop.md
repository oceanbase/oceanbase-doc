...
|-------|------|----------------|
| スカラーサブクエリ | 単一列の単一行 | <ul><li>`SELECT` 句</li><li>`WHERE` 句</li><li>`HAVING` 句</li></ul> |
| 列サブクエリ  | 単一列の複数行 | <ul><li>`WHERE` 句</li><li>`HAVING` 句</li></ul> |
| 行サブクエリ  | 複数列の複数行 | <ul><li>`WHERE` サブ句</li><li>`HAVING` 句</li></ul> |
| テーブルサブクエリ  | 複数行の複数列 | <ul><li>`FROM` 句</li><li>`EXISTS` 句</li></ul> |
...
* `IN` は `WHERE` 式でよく使用され、特定の範囲内のデータをクエリするために使われます。
