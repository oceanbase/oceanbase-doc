...
内部結合はデフォルトで `JOIN` が返す結果が `ON` の後にある結合条件を満たすことを保証し、通常はキーワード `INNER` を省略します。`JOIN` の前後のテーブルはそれぞれ左テーブルと右テーブルと呼ばれ、`ON` 内の条件は左テーブルと右テーブルの結合条件およびフィルタ条件を示しています。

内部結合のSQL構文フォーマットは以下のとおりです：
