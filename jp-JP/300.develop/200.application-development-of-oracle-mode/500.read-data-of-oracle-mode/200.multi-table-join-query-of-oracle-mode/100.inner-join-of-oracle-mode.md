...
内部結合(INNER JOIN)は、2つ以上のテーブルを結合し、結合条件を満たす複数のテーブルデータを返すために使用されます。内部結合は、単純結合とも呼ばれます。
...
サンプルテーブルを作成し、適切なデータを挿入した上で、結合条件を使用してデータをクエリします。

* `tbl1` テーブルと `tbl2` テーブルの `id` が同じ `id` 列と `name` 列のデータを取得します。
...
* 内部結合を使用して、`tbl1` テーブルと `tbl2` テーブルの `id` が同じ `id` 列と `name` 列のデータを取得します。
...
* `WHERE` 条件を使用して、`tbl1` テーブルと `tbl2` テーブルの `id` が同じ `id`、`name` 列のデータを取得します。
...
上記で作成したテーブルとデータに基づき、無条件結合を使用して、`tbl1` テーブルと `tbl2` テーブルの `id` 列と `name` 列のデータを取得します。
...
上記で作成したテーブルとデータに基づき、組み合わせ条件を使用してデータをクエリします。

* 内部結合 + AND結合条件を使用してデータをクエリします。
...
* WHERE結合条件 + AND結合条件を使用してデータをクエリします。
...
* 内部結合条件 + ORDER BYでソートしてデータをクエリします。
