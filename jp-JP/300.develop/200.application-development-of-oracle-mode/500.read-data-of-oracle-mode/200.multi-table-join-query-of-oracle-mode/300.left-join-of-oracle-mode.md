...
左結合（LEFT JOIN）とは左外部結合を示し、このステートメントは左テーブルのすべての行を返しますが、右テーブルでマッチしない行は含まれません。
...
本記事では主に左結合の例を説明します。完全結合に関連する例については、[完全結合](../200.multi-table-join-query-of-oracle-mode/200.full-join-of-oracle-mode.md) を、右結合に関連する例については [右結合](../200.multi-table-join-query-of-oracle-mode/400.right-join-of-oracle-mode.md) を参照してください。
...
`JOIN` が返すデータが結合条件やフィルタ条件にマッチするデータのほか、左テーブルのフィルタ条件にマッチするものの結合条件を満たさないデータも含む場合、左結合を使用できます。左結合で返される結果において、右テーブルに属するデータが存在しない場合、その列は `NULL` を返します。
