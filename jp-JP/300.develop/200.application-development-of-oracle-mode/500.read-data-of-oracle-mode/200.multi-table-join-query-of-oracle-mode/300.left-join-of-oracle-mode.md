...
左結合（LEFT JOIN）は左外部結合を示し、このステートメントは左テーブルのすべての行を返します。右テーブルでマッチしない行は含まれません。
...
`JOIN` が返すデータが結合条件やフィルタ条件に合致するデータのほか、左テーブルのフィルタ条件には合致するが結合条件には合致しないデータも含む場合、左結合を使用できます。左結合で返される結果において、右テーブルに属するデータが存在しない場合、その列は `NULL` を返します。
