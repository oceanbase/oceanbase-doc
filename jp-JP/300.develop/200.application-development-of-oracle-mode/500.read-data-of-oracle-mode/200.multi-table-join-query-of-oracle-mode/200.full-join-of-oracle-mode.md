...
完全結合（FULL JOIN）とは、完全外部結合を示します。このステートメントを使用すると、マッチするかどうかに関係なく、結合したすべてのテーブルのすべての行を返します。

## 背景

外部結合（OUTER JOIN）は、比較演算子を使用して2つのテーブルのデータを比較します。結合結果には結合条件に合致する行だけでなく、条件に合致しない行も含まれます。

外部結合には、完全結合（FULL JOIN）、左結合（LEFT JOIN）、右結合（RIGHT JOIN）が含まれます。外部結合は結合条件を満たすすべての行を返します。同時に、一方のテーブルから未使用の行を返し、もう一方のテーブルの対応する位置に`NULL`を入力します。
...
ご覧のように、テーブル `tbl_a` には値が `5` の行がないため、実行結果ではその行のレコードが `NULL` に埋め込まれています。同様に、テーブル `tbl_b` には値が `2、4` の行がないため、実行結果ではこれら2行が `NULL` に埋め込まれています。

### 完全結合とWHERE句の組み合わせ使用

完全結合で結合結果を取得した後、`WHERE` 句を使用して結合結果をフィルタリングできます。

例2：テーブル `tbl_a` とテーブル `tbl_b` に対して完全結合クエリを実行した後、テーブル `tbl_b` の `value=1003` のデータを返します。
