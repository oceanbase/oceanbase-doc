...
* `-u`：テナントの接続アカウントを提供します。ODP接続の一般的な形式：`ユーザー名@テナント名#クラスタ名` または `クラスタ名:テナント名:ユーザー名`。直接接続方式の形式：`ユーザー名@テナント名`。
* `-p`：アカウントのパスワードを提供します。
...
2. `odbc_print_error` 関数を定義します。
    ...
    1. 関数名を `odbc_print_error` として定義し、パラメータ `SQLSMALLINT HandleType` と `SQLHANDLE Handle` を指定します。これらはエラー情報に対応するハンドルタイプとハンドルを指定するために使用されます。
