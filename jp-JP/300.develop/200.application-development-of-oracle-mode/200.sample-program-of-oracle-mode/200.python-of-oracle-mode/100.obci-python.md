|description|本記事では、cx_Oracleドライバーを使用してOceanBaseデータベースのOracleモードに接続し、テーブルの作成、データの挿入、更新、クエリなどの基本的な操作を実行する方法について説明します。|
|---|---|
|keywords|cx_Oracle、Python、OceanBase、データベース接続、Oracleモード|
|dir-name|Pythonドライバー Oracleモード|
...
システムにPython 3.7以降のバージョンがインストールされていることを確認します。以下のコマンドでPythonのバージョンを確認できます：
...
libobclientとOBCIを含むOceanBaseクライアントコンポーネントをインストールします：

```shell
# 古いバージョンをアンインストールする（存在する場合）
rpm -e libobclient
rpm -e obci
...
<h4>注意</h4>
<p>libobclientのバージョンが2.2.11以上、obciのバージョンが2.1.1以上であることを確認してください。バージョンが低すぎると、接続失敗や機能異常が発生する可能性があります。</p>
</main>

### ステップ3：cx_Oracleドライバのインストール

cx_Oracleドライバを解凍してインストールします：

```shell
# ドライバのインストールディレクトリに移動する
cd /directory/of/cx_Oracle-8.3.0.tar.gz

# cx_Oracleドライバパッケージを解凍する
tar -xvf cx_Oracle-8.3.0.tar.gz

# 解凍後のディレクトリに入る
cd cx_Oracle-8.3.0

# cx_Oracleドライバのインストール
...
プログラムは、文字型、数値型、日付時刻型、LOB型のデータ操作を順次テストし、実行結果を出力します。
...
def test_simple_pool():
    ...
    try:
        ...
        print("\n接続プールテストが正常に完了しました！", end="")
        # 注：pool.close() を呼び出さないことで、ダンプエラーを回避します
