...
`MERGE INTO` ステートメントの構文フォーマットは以下のとおりです：
...
|           パラメーター                                                             | 必須かどうか |          説明                 |
|----------------------------------------------------------------------------|---------|-------------------------------|
| MERGE INTO table_name alias1                                               | はい | データを挿入する必要があるテーブルとエイリアスを指定します。        |
...
ソーステーブルのレコードがターゲットテーブルに存在しない場合、`REPLACE INTO` ステートメントを使用して、ソーステーブルのレコードをターゲットテーブルに挿入できます。
...
例1では、ターゲットテーブル `t_insert` の `id` 列はプライマリキー列であり、一意性制約を満たす必要があります。同時に、ソーステーブルのレコード `(4,'JP',10004),(5,'CN',10005)` はターゲットテーブルに存在しないため、ソーステーブルのレコードは直接ターゲットテーブルに挿入されます。
...
例2では、ターゲットテーブル `t_insert` の `id` 列はプライマリキー列であり、一意性制約を満たす必要があります。ソーステーブル `t_merge` のレコード `(3,'CN',10003)` は、ターゲットテーブル `t_insert` に同じレコードが存在するため、ターゲットテーブルのレコードが更新されます。
