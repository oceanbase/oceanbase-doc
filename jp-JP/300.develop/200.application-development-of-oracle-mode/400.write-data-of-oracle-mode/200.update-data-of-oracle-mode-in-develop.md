...
| \[ WHERE condition \]                         | いいえ      | WHERE句では、更新される行が満たす必要がある条件を指定します。指定しない場合、更新対象のテーブルの該当列のすべてのレコードを更新します。 |

### 一部のレコードを更新する

テーブルデータを更新する際には、一部のレコードのみを更新できます。
...
`UPDATE` ステートメントを実行する際は、トランザクションが大きくなりすぎないように注意してください。`WHERE` キーワードを使用して更新範囲を制御できます。これは、条件を指定せずにデータを更新する場合、レコード数が数十万または数百万に達すると大規模なトランザクションが発生し、実行が失敗する可能性があるためです。

例2：`t_insert` テーブル内で、`value` の値が `10003` であるすべての行について、その値を `value+1` に更新します。
...
現在、OceanBaseデータベースはDBLinkを介して、OceanBaseデータベース（Oracleモード）およびOracleデータベースへのデータ更新をサポートしています。

DBLinkを使用して、リモートデータベースのテーブル`t2`の列`C1`の値を`3`に変更する例を以下に示します：
