...
通常、テーブルデータの更新には [UPDATE](../../../700.reference/500.sql-reference/100.sql-syntax/300.common-tenant-of-oracle-mode/900.sql-statement-of-oracle-mode/200.dml-of-oracle-mode/600.update-of-oracle-mode.md) ステートメントを使用します。
...
`UPDATE` ステートメントを実行する際は、トランザクションが大きくなりすぎないように注意してください。`WHERE` キーワードを使用して更新範囲を制御できます。これは、条件を指定せずにデータを更新する場合、レコード数が数十万または数百万に達すると大規模トランザクションが発生し、実行が失敗する可能性があるためです。
...
DBLinkを使用して、リモートデータベーステーブル `t2` の `C1` 列の値を `3` に変更する例を以下に示します：
