...
`INSERT INTO` ステートメントの構文フォーマットは次のとおりです：
...
この中で、テーブルの `id` 列と `name` 列は空欄にすることができず、`id` 列はプライマリキーであるため、一意性制約の要件を満たす必要があり、重複する値を持つことはできません。`gmt_create` 列にはデフォルト値が設定されています。
...
|    パラメーター      |      説明       |
|-------------|-----------------|
| PARALLEL(N) | オプション、上書き書き込み操作の並列実行度を指定します。指定されていない場合、デフォルトの並列度は2です。|
...
* 書き込みテーブルにテーブルロックをかけると、同一テーブルに対する並列DDL操作は許可されません。並行して実行されたDML操作は、テーブルロックが解除され、タイムアウトになるまで待ちます。操作中にテーブルへのクエリは可能です。
* このステートメントが操作するソースデータとターゲットテーブルの上書き書き込みの列数は厳密に一致する必要があります。一致しない場合はエラーが発生します。
* このステートメントのデータ書き込み操作は、フルパススルーインポート方式を採用しているため、操作はフルパススルーインポート機能の制限を受けます。バイパスインポートに関する情報は、[INSERT INTO SELECTステートメントによるデータのバイパスインポート](https://en.oceanbase.com/docs/common-oceanbase-database-10000000001971205) の **使用上の制限** セクションを参照してください。
* このステートメントは、バイパスインポートHintを指定するとエラーが発生します。
...
* フルバイパスインポートに基づいており、`/*+ append */` Hintを追加する必要はありません。そうしないと、使用時にエラーが発生します。
* このステートメントは、複数行トランザクションでは操作できません。そのため、操作をスムーズに実行するために、まず `SET autocommit = on;` コマンドを実行して自動トランザクションコミットモードを有効にする必要があります。
