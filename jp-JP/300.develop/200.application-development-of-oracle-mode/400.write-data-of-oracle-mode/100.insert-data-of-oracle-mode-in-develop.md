...
* このステートメントのデータ書き込み操作は、フルパススルーインポート方式を採用しているため、操作はフルパススルーインポート機能の制限を受けます。バイパスインポートに関する情報は、[INSERT INTO SELECTステートメントによるデータのバイパスインポート](../../../620.obap/300.obap-data-collection/300.obap-import-data/20.bypass-import/200.full-bypass-import.md) の **使用上の制限** セクションを参照してください。
