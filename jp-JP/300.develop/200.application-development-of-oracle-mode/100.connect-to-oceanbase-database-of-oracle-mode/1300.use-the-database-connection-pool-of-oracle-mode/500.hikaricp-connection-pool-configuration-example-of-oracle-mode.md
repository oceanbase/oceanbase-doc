...
1. OceanBaseデータベースのデプロイ担当者または管理者から、該当するデータベース接続文字列を取得します。
    ...
    **例：**
    ...
    接続文字列の詳細については、[OBClientを使用したOceanBaseテナントへの接続](../200.connect-to-an-oceanbase-tenant-by-using-obclient-of-oracle-mode.md)を参照してください。
...
5. プロジェクトが依存するコンポーネントを設定します。
    ...
    1. データベースとのやり取りを行うために、`oceanbase-client` 依存ライブラリを追加します：
       ...
       1. `<groupId>` を使用して、依存関係の所属組織を `com.oceanbase` と指定します。
    ...
    2. 高パフォーマンスなJDBC接続プールを実現するために、`HikariCP` 依存ライブラリを追加します：
       ...
       1. `<groupId>` を使用して、依存関係の所属組織を `com.zaxxer` と指定します。
    ...
    3. ログの記録と管理を容易に行うために、`logback-classic` 依存ライブラリを追加します：
       ...
       1. `<groupId>` を使用して、依存関係の所属組織を `ch.qos.logback` と指定します。
...
<table>
   ...
   <tr>
       <td>idleTimeout</td>
       <td>600000</td>
       <td>プール内のアイドル状態の接続の最大時間を制御します。単位はミリ秒です。デフォルト値は600000（10分）です。この設定には以下の制限があります：<ul><li><code>minimumIdle</code> が <code>maximumPoolSize</code> より小さい場合にのみ有効になります。</li><li>接続プール内の接続数が <code>minimumIdle</code> に達した場合、アイドル接続は回収されません。接続数が <code>minimumIdle</code> を超えた場合にのみ、接続は回収される可能性があります。</li></ul></td>
   </tr>
   <tr>
       <td>keepaliveTime</td>
       <td>0</td>
   ...
   <tr>
       ...
       <td>N/A</td>
       <td>接続プールがデータベースに送信する接続テストクエリを実行するために使用されます。これは、接続プールから接続を取得する前に実行され、データベースへの接続が引き続き有効であるかどうかを確認します。</td>
   </tr>
   <tr>
       <td>minimumIdle</td>
       <td>N/A</td>
   ...
   <tr>
       ...
       <td>10</td>
       <td>アイドル状態および使用中の接続を含む、接続プールが許可する最大サイズを制御します。この値によって、データベースバックエンドへの実際の接続数の最大値が決定されます。</td>
   </tr>
   <tr>
       <td>poolName</td>
       <td>N/A</td>
       <td>ユーザー定義の接続プール名を表すために使用されます。ログ記録およびJMX管理コンソールでは、この名前は主に接続プールの識別と接続プール設定に使用されます。デフォルトでは、自動的に名前が生成されます。</td>
   </tr>
</table>

### Main.javaコードの紹介

`Main.java` ファイルはサンプルプログラムの一部であり、HikariCP接続プールを使用してデータベース接続を取得し、テーブルの作成、データの挿入、削除、更新、クエリといった一連のデータベース操作を実行し、その結果を出力する方法を示しています。
...
1. 必要なクラスとパッケージのインポート。
    ...
    2. `java.sql.Connection` クラスをインポートします。データベースとの接続の確立と管理に使用されます。
...
2. クラス名とメソッドの定義。
    ...
    Mainクラスを定義し、その中の `main` メソッドをプログラムのエントリポイントとします。`main` メソッド内では、`db.properties` ファイルを読み込んでHikariCP接続プールを設定し、データベース接続を取得します。その後、テーブルの作成、データの挿入、データのクエリ、データの更新、データの削除を順次実行するための一連のメソッドを呼び出します。操作中に `SQLException` 例外が発生した場合、例外情報のスタックトレースを出力します。具体的な手順は以下の通りです：
...
3. テーブルを作成するためのメソッドを定義します。
    ...
    2. SQL ステートメント文字列を定義し、`test_hikaricp` という名前のテーブルを作成します。テーブルには `id` 列（データ型は `NUMBER`）と `name` 列（データ型は `VARCHAR2(50)`）を含めます。
    ...
    5. コンソールにメッセージを出力し、テーブルの作成が成功したことを示します。
...
4. データを挿入するためのメソッドを定義します。
    ...
    7. コンソールにメッセージを出力し、データの挿入が成功したことを示します。
...
5. データをクエリするためのメソッドを定義します。
    ...
    プライベート静的メソッド `selectData` を定義し、データベースからテーブル `test_hikaricp` のデータをクエリします。具体的な手順は以下のとおりです：

    1. プライベート静的メソッド `selectData` を定義します。このメソッドは Connection オブジェクトをパラメータとして受け取り、`SQLException` 例外をスローする可能性があることを宣言します。
    2. SQL ステートメント文字列を定義し、`test_hikaricp` という名前のテーブル内のすべてのデータをクエリするために使用します。
    ...
    4. コンソールにメッセージを出力し、ユーザーデータの出力中であることを示します。
    5. クエリ結果セットを走査し、`next()` メソッドを使用して結果セットに次の行のデータがあるかどうかを確認します。ある場合は、ループに入ります。
...
6. データを更新するためのメソッドを定義します。
    ...
    プライベート静的メソッド `updateData` を定義し、データベース内のテーブル `test_hikaricp` のデータを更新します。具体的な手順は以下のとおりです：
    ...
    2. SQL ステートメント文字列を定義し、`test_hikaricp` という名前のテーブル内のデータを更新するために使用します。`name` 列の値を指定された `name` に更新します。条件は `id` 列の値が指定された `id` と等しいことです。
    3. 接続オブジェクト `conn` を使用して、プリコンパイルされた SQL ステートメントオブジェクト `pstmt` を作成し、`try-with-resources` ブロック内でこのオブジェクトを使用します。
...
7. データを削除するためのメソッドを定義します。
    ...
    プライベート静的メソッド `deleteData` を定義し、データベースから `test_hikaricp` テーブルの条件に合致するデータを削除します。具体的な手順は以下のとおりです：

    1. プライベート静的メソッド `deleteData` を定義します。このメソッドは Connection オブジェクトと整数型の `id` パラメータを受け取り、`SQLException` 例外をスローする可能性があることを宣言します。
    2. SQL ステートメント文字列を定義し、`test_hikaricp` という名前のテーブルから条件 `id = ?` に合致するデータを削除するために使用します。
    3. 接続オブジェクト `conn` を使用して、プリコンパイルされた SQL ステートメントオブジェクト `pstmt` を作成し、`try-with-resources` ブロック内でこのオブジェクトを使用します。
    4. SQL ステートメントの最初のパラメータ `?` の値を `id` に設定します。
    5. SQL ステートメントを実行し、条件に合致するデータをテーブルから削除します。
