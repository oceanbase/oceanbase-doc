...
2. OceanBaseデータベース接続文字列の情報に基づいて、以下のURLの対応する情報を入力します。
    ...
    * `$password`：アカウントのパスワードを提供します。
    ...
    その他のOceanBase Connector/J URLパラメータの詳細については、[データベースURL](https://en.oceanbase.com/docs/common-oceanbase-connector-j-en-10000000002244946) を参照してください。
...
2. `Main` クラスを作成し、`main` メソッドを定義します。
    ...
    2. 公共静的メソッド `main` を定義して、プログラムのエントリポイントとして、コマンドラインパラメータを受け取ります。
...
7. データを更新するためのメソッドを定義します。
    ...
    3. `setString()` メソッドを使用して、最初のパラメータの値を文字列 `test_update` と設定します。
