...
OceanBaseデータベースにおいて、テーブルグループは、Tableの集合を記述する論理概念です。テーブルグループを定義することで、1組のテーブルの物理ストレージ上での近接関係を制御できます。
...
* `tablegroup_option`：テーブルグループの `SHARDING` プロパティ（オプション）。`SHARDING` プロパティを指定しない場合、デフォルトで `ADAPTIVE` のテーブルグループが作成されます。テーブルグループの `SHARDING` プロパティの値は以下のとおりです：
  ...
  * `PARTITION`：テーブルグループに追加されるテーブルに対応するパーティションで、テーブルグループ内のすべてのテーブルのパーティションのパーティショニングと一致する必要があります。これには、パーティションタイプ、パーティションの数、パーティションのValueが含まれます。
  * `ADAPTIVE`：`SHARDING` を指定しない場合のデフォルト値です。テーブルグループに追加されるテーブルに対応するパーティションとサブパーティションで、テーブルグループ内のすべてのテーブルのパーティションとサブパーティションのパーティショニングと一致する必要があります。これには、パーティションタイプ、パーティションの数、パーティションValueが含まれます。
...
2. 次のSQLステートメントを使用して、Hash + Rangeパーティションのサブパーティションテーブル `test_tbl1_hr` を作成し、`TABLEGROUP` を `test_tbl_tg1` と指定します。
...
4. 次のSQLステートメントを使用して、Hash + Rangeパーティションのサブパーティションテーブル `test_tbl3_hr` を作成し、`TABLEGROUP` を `test_tbl_tg3` と指定します。
