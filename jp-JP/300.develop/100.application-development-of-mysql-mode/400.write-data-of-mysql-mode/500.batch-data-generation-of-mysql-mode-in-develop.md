...
2. ストアドプロシージャを作成します。
    ...
    **例：**
    ...
    以下のSQLステートメントを実行して、ストアドプロシージャ `pro_generate_data` を作成します。入力パラメータは `n` で、挿入するデータの件数を指定するために使用されます。ループステートメントと `INSERT` ステートメントを使って、データをビルドして挿入します。この中で、`test_pro_tbl1` はデータ挿入先のテーブル名、`name` と `enrollment_date` はデータ挿入先のフィールド名です。`i` はループカウンターを表し、`CONCAT` 関数は名前をビルドするために使用され、`DATE_ADD` 関数は日付をビルドするために使用されます。
...
`CALL` ステートメントを使用してストアドプロシージャを呼び出し、テストデータをビルドするロジックを実行します。ストアドプロシージャにパラメータを渡すことで、ビルドするデータの数を指定できます。
...
<main id="notice" type='explain'>
  <h4>説明</h4>
  <p>入力パラメータのサイズを増減することで、テストデータの数を制御できます。パラメータのサイズ調整時には、データベースのパフォーマンスとストレージ容量の制限を考慮する必要があります。過剰なデータビルドにより、データベースがクラッシュしたり、ストレージ容量が不足したりするのを防ぐためです。</p>
</main>
