...
`UPDATE` ステートメントの構文フォーマットは以下のとおりです：
...
| column_name = value [, column_name = value] | はい      | 更新対象の列を指定します。等号（=）の後ろのvalueは更新後のターゲット値です。         |
| [ WHERE condition ]                         | いいえ    | 条件句は、更新される行レコードが満たす必要がある条件を指定します。何も入力しない場合は、更新対象のテーブルの該当列すべてのレコードを更新することを意味します。 |
...
* `LIMIT` を使用して `t_insert` テーブル内の最初の 100000 行のデータの `value` 列の値を `value+1` に更新します。
...
例中の `ON DUPLICATE KEY UPDATE name = VALUES(name)` は、挿入するデータがテーブルのプライマリキー値と重複する場合、テーブル内で競合する行の元のデータ `(3,'EN', 10003)` の `name` 列の値を、現在挿入しようとしている `name` 列の値に更新することを意味します。
