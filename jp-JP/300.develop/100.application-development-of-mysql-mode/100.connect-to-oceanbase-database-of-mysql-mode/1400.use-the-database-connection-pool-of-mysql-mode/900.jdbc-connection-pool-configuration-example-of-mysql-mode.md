...
* `useCursorFetch`：`TRUE`に設定することが推奨されます。
  ...
  大量のデータを取得するクエリ文の場合、データベースサーバーはCursorを作成し、FetchSizeのサイズに応じてクライアントにデータを配信します。このプロパティを`TRUE`に設定すると、自動的に`useServerPrepStms=TRUE`も設定されます。

* `useServerPrepStms`：SQLをデータベースサーバーに送信するためにPSプロトコルを使用するかどうかを制御します。
