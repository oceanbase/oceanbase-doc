...
2.  その他の接続プールパラメータの設定
    ...
    4.  廃棄接続のログを出力するかどうかを `true` と指定します。これは、廃棄接続が回収される際にエラーログに情報が出力されることを意味します。テスト環境では `true` に設定し、本番環境ではパフォーマンスへの影響を避けるために `false` に設定できます。
    ...
    8.  廃棄接続を回収するかどうかを `true` と指定します。これは、接続が `removeAbandonedTimeout` で定義された時間を超えると回収されることを意味します。
    ...
    11.  接続取得時に接続の可用性を検証するかどうかを `false` と指定します。`false` に設定するとパフォーマンスが向上しますが、取得した接続が使用不能になる可能性もあります。
    12.  接続返却時に接続の可用性を検証するかどうかを `false` と指定します。`false` に設定するとパフォーマンスが向上しますが、返却される接続が使用不能になる可能性もあります。
    13.  アイドル状態の接続を検証するかどうかを `true` と指定します。`true` に設定すると、接続プールは定期的に `validationQuery` を実行して接続の可用性を検証します。
...
6.  データを挿入するためのメソッドを定義します。
    ...
    5.  5回繰り返す `for` ループステートメントを使用します。これは5行のデータを挿入することを意味します。
...
9.  データをクエリするためのメソッドを定義します。
    ...
    5.  結果セットの各行データを走査するために、`while` ループステートメントを使用します。
