...
DBeaverに搭載されているOceanBaseドライバーまたはMySQLドライバーのいずれかを使用して、OceanBaseデータベースのMySQLテナントに接続できます。本記事では、OceanBaseドライバーを例に、DBeaverを使用してOceanBaseデータベースのMySQLテナントに接続する方法を紹介します。
...
<main id="notice" type='notice'>
    <h4>説明</h4>
    <p>DBeaverのバージョンによって操作画面が異なる場合があります。この記事では、DBeaver 22.0.5を例にデータベース接続のステップを説明します。他のバージョンでの操作については、DBeaverの対応するバージョンのプロダクトドキュメントを参照してください。</p>
</main>

1. （オプション）ドライバーファイルのバージョンを変更します。

   DBeaver内のOceanBaseドライバーのデフォルトのドライバーファイルはmysql-connector-java-5.1.44です。この記事ではmysql-connector-java-5.1.47を例に説明します。ドライバーファイルの変更については次のステップを参照してください。
...
4. **接続テスト**のポップアップウィンドウに**接続済み**と表示されれば接続成功です。**OK**をクリックした後に**完了**をクリックすると、OceanBaseデータベースに接続できます。
...
5. **データベースナビゲーション**バーのデータベース名をダブルクリックし、情報が正しく表示されれば、OceanBaseデータベースへの接続は成功です。
