...
PDOの正式名称はPHP Data Objectsで、PHPアプリケーションに提供されているデーターベース接続抽象化レイヤーの軽量で一貫性のあるインターフェースです。
...
### 手順2：podtest.phpファイルのデータベース接続情報を修正する
...
### 手順3：podtest.phpファイルを実行して検証します。

`podtest.php` ファイルに故意に間違ったデータベースの名前を入力し、`php podtest.php` コマンドを実行すると、以下のエラーメッセージが返されます：
