...
# RustによりOceanBaseデータベースに接続するサンプルプログラム
...
5. `main.rs` ファイルを実行する
6. インタラクティブなコマンドラインインターフェースで対応する操作を実行する

### 手順1：RustとCargoのバージョンを確認する
...
### 手順2：プロジェクトを作成し、必要な依存関係をインストールする
...
2. `cargo` を使用して `mysql` crateをインストールします：

    ```shell
    cargo add mysql
    ```

3. （オプション）非同期操作を使用する必要がある場合は、`mysql_async` crateをインストールできます：
...
* `$user_name`：テナントの接続アカウントを提供します。ODP接続フォーマット：`ユーザー名@テナント名#クラスター名` または `クラスター名:テナント名:ユーザー名`。直接接続方式の形式：`ユーザー名@テナント名`。
* `$password`：アカウントのパスワードを提供します。

### 手順4：main.rsファイル内のデータベース接続情報を修正する

手順3の情報に基づいて、`main.rs` ファイル内のデータベース接続情報を修正します。
