|description|本記事では、Rustを使用してOceanBaseデータベースに接続し、テーブルの作成、データの挿入、クエリなどの基本的な操作を実行する方法について説明します。|
|---|---|
|keywords|Rust、RustMySQL、OceanBase、データベース接続、MySQLモード|
|dir-name|MySQL Crate|
|dir-name-en||
|tenant-type|MySQL Mode|
|slug|connect-oceanbase-using-rust-mysql-compatible-mode|

# RustによるOceanBaseデータベース接続サンプルプログラム

本記事では、RustとOceanBaseデータベースを使用して、テーブルの作成、データの挿入、クエリなどの基本的な操作を実現するアプリケーションの構築方法を紹介します。

## 前提条件

* Rustツールチェイン（rustup）とCargoがインストール済みであること。
* OceanBaseデータベースV4.2.4以降のバージョンをインストール済みで、MySQLテナントが作成されていること。
...
### ステップ３：OceanBaseデータベースの接続情報を取得する
...
* `$host`：OceanBaseデータベースへの接続IPアドレス。ODP接続方式ではODPアドレスを使用し、直接接続方式ではOBServerノードのIPを使用します。
* `$port`：OceanBaseデータベースへの接続ポート。ODP接続のデフォルトポートは2883、直接接続方式のデフォルトポートは2881です。
* `$database_name`：アクセス対象のデータベース名。
* `$user_name`：テナントの接続アカウント。ODP接続形式：`ユーザー名@テナント名#クラスタ名`または`クラスタ名:テナント名:ユーザー名`。直接接続方式の形式：`ユーザー名@テナント名`。
