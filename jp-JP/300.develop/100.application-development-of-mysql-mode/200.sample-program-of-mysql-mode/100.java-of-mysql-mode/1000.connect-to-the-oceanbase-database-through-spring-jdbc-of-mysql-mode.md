|description|Spring JDBCフレームワークを使用してOceanBaseデータベースに接続するサンプルプログラム|
|---|---|
...
本記事では、Spring JDBCフレームワーク、MySQL Connector/J、およびOceanBaseデータベースを使用してアプリケーションを構築し、テーブルの作成、データの挿入、クエリなどの基本的なデータベース操作を実行する方法を紹介します。
...
2. OceanBaseデータベースの接続文字列情報に基づいて、以下のURLの対応する情報を入力します。
    ...
    * `$database_name`：アクセスするデータベース名。
        ...
        <main id="notice" type='notice'>
          <h4>注意</h4>
          <p>テナントに接続するユーザーには、データベースに対する <code>CREATE</code>、<code>INSERT</code>、<code>UPDATE</code>、<code>DELETE</code>、<code>SELECT</code>、および <code>DROP</code> 権限が付与されていなければなりません。その他のユーザー権限の詳細については、<a href="../../../../600.manage/500.security-and-permissions/300.access-control/200.user-and-permission/200.permission-of-mysql-mode/100.permission-classification-of-mysql.md">MySQLモードの権限分類</a>を参照してください。</p>
        </main>
...
<main id="notice" type='notice'>
  <h4>注意</h4>
  <p>MySQL Connector/J 8.xを使用する場合は、接続エラーが発生する可能性がありますので、アカウントのパスワードにシャープ記号（#）を含まないようにしてください。</p>
</main>
...
`Main.java` ファイルはサンプルプログラムの一部であり、Spring JDBCフレームワークを使用したデータベースの操作方法を示すコードです。まずデータベース接続情報を設定し、次に `JdbcTemplate` オブジェクトを作成してデータベース操作を実行します。コードには、テーブルの作成、データの挿入、データのクエリのサンプルも含まれています。
...
3.  データベース接続情報を定義します。
    ...
    データベースへの接続に使用するURL、ユーザー名、パスワードを定義します。`$host`、`$port`、`$database_name`、`$user_name`、`$password` は実際のデータベース接続情報に置き換える必要があります。
    ...
    *   `$user_name`：`-u` パラメータで指定します。形式は **ユーザー@テナント#クラスタ名** または **ユーザー名@SERVICE:サービス名** です。**ユーザー@テナント#クラスタ名** の形式を使用する場合、デフォルトのテナントは `sys`、管理者ユーザーは `root` です。データベースに直接接続する場合、クラスタ名を省略できますが、ODP接続の場合は入力する必要があります。
    *   `$password`：アカウントのパスワード。
...
6.  テーブルを作成します。
    ...
    1.  テーブル作成用のSQLステートメントを定義します。SQLステートメントでテーブル名を `test_springjdbc` と指定し、`id` という名前の `INT` 型列と `name` という名前の `VARCHAR(20)` 型列の2つの列を定義します。
...
7.  データを挿入します。
    ...
    1.  データ挿入用のSQLステートメントを定義します。SQLステートメントで挿入するテーブルを `test_springjdbc`、列を `id` と `name` と指定し、`(1,'A1')`、`(2,'A2')`、`(3,'A3')` の2行のデータを挿入します。
...
8.  データを更新します。
    ...
    1.  SQLステートメントを定義します。このステートメントは、データベース内のテーブル `test_springjdbc` 内の `id` が `1` のレコードの `name` フィールドの値を `A11` に更新するために使用されます。
...
9.  データを削除します。
    ...
    1.  SQLステートメントを定義します。このステートメントは、データベース内のテーブル `test_springjdbc` から `id` が `2` のレコードを削除するために使用されます。
...
10. データをクエリします。
    ...
    1.  データクエリ用のSQLステートメントを定義します。SQLステートメントで `SELECT *` を使用してすべての列を選択し、`test_springjdbc` テーブルからデータをクエリします。
...
11. テーブルを削除します。
    ...
    1.  SQLステートメントを定義します。このステートメントは、データベース内の `test_springjdbc` テーブルを削除するために使用されます。
