...
8.  データを更新します。
    ...
    1.  SQLステートメントを定義します。このステートメントは、データベーステーブル `test_springjdbc` 内の `id` が `1` のレコードの `name` フィールドの値を `A11` に更新するために使用されます。
...
9.  データを削除します。
    ...
    1.  SQLステートメントを定義します。このステートメントは、データベーステーブル `test_springjdbc` から `id` が `2` のレコードを削除するために使用されます。
