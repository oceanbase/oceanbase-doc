...
セーブポイントを設定した後、特定のセーブポイントが不要になった場合は、そのセーブポイントを解放することもできます。
...
```
  ...
  <main id="notice" type='notice'>
    <h4>注意</h4>
    <p><code>RELEASE SAVEPOINT</code> ステートメントが正常に実行されると、解放されたセーブポイントまで、またはそのセーブポイントと指定されたセーブポイントの間の他の任意のセーブポイントまでロールバックすることはできなくなります。</p>
  </main>

以下の例では、セーブポイント `ru` を解放した後、そのセーブポイントおよび `RELEASE SAVEPOINT` ステートメントとそのセーブポイントの間の他の任意のセーブポイントまでロールバックできなくなることを示しています。
...
4. セーブポイント `ru` および `ca` までそれぞれロールバックしようとしますが、どちらのセーブポイントも存在しないことがわかります。
