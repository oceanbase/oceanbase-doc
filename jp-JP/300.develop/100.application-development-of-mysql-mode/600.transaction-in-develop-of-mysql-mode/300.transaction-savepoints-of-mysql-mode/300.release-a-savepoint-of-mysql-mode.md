...
セーブポイントを設定した後、もはや必要がなくなった場合には、そのセーブポイントを解放することもできます。
...
```
  ...
  <main id="notice" type='notice'>
    <h4>注意</h4>
    <p><code>RELEASE SAVEPOINT</code> ステートメントが正常に実行されると、解放されたセーブポイントは指定されたセーブポイントおよび <code>RELEASE SAVEPOINT</code> ステートメントと指定されたセーブポイントの間の他のいかなるセーブポイントにもロールバックできなくなります。</p>
  </main>

次の例では、ロールバックポイント `ru` を解放した後、解放されたセーブポイントおよび `RELEASE SAVEPOINT` ステートメントとそのセーブポイントの間の他のいかなるセーブポイントにもロールバックできなくなることを示しています。

1. トランザクションを開始し、複数のセーブポイント情報をマークします。
...
2. テーブル内のデータを確認します。
   ...
   トランザクションがコミットされる前に、トランザクション内でコミットされていないすべてのデータを確認できます。
...
4. セーブポイント `ru` と `ca` にそれぞれロールバックすると、どちらのセーブポイントも存在しないことがわかります。
