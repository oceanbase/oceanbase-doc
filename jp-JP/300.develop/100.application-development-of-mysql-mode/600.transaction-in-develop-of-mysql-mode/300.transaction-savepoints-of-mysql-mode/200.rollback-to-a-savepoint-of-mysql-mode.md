...
以下の例では、1つのトランザクション内に複数のDMLステートメントと複数のセーブポイントを含み、その後、そのうちの1つのセーブポイントまでロールバックすることで、セーブポイント以降の変更のみが破棄されます。
...
4. トランザクションをそのうちの1つのセーブポイント `ru` までロールバックします。
   ...
   クエリ結果からわかるように、`(8,'CA')` はセーブポイント `ru` の後に挿入されたデータであるため、ロールバック後にこの変更は破棄されます。
