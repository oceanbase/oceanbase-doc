...
|slug|inner-join-queries-mysql-mode|
...
INNER JOINはデフォルトで `JOIN` が返す結果が `ON` の後にある結合条件を満たすことを保証し、通常はキーワード `INNER` を省略します。`JOIN` の前後のテーブルはそれぞれ左テーブルと右テーブルと呼ばれ、`ON` 内の条件は左テーブルと右テーブルの結合条件およびフィルタ条件を示しています。

INNER JOINのSQL構文フォーマットは以下のとおりです：
...
`WHERE` と `ORDER BY` の両方の句を同時に使用する場合、SQLが返す結果は `JOIN` の結果を基に、`WHERE` の後のクエリ条件でフィルタリングされ、さらに `ORDER BY` の後の列によって結果がソートされます。
