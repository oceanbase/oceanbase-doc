...
|slug|inner-join-queries-mysql-compatible-mode|

# INNER JOIN

内部結合（INNER JOIN）とは、2つ以上のテーブルを結合し、結合条件を満たす複数のテーブルデータを返すために使用されるものです。INNER JOINは単純結合とも呼ばれます。

## INNER JOIN構文

INNER JOINでは、デフォルトで`JOIN`が返す結果は`ON`の後に続く結合条件を満たします。通常、キーワード`INNER`は省略されます。`JOIN`の前後のテーブルはそれぞれ左テーブルと右テーブルと呼ばれ、`ON`の条件は左テーブルと右テーブルの結合条件およびフィルタリング条件を記述しています。
