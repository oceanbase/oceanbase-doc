...
外部結合には、全結合（FULL JOIN）、左結合（LEFT JOIN）、右結合（RIGHT JOIN）が含まれます。外部結合は、結合条件を満たすすべての行を返すとともに、片方のテーブルから使用されなかった行も返し、もう一方のテーブルの対応する位置には `NULL` を埋め込みます。
...
外部結合ステートメントには一般的に左テーブルと右テーブルが含まれ、`JOIN` 句の最も左側に現れるものが左テーブル、`JOIN` 句の最も右側に現れるものが右テーブルです。

FULL JOINの構文フォーマットは以下のとおりです：
...
ご覧のように、テーブル `tbl_a` には値が `5` の行がないため、実行結果でその行のレコードは `NULL` が埋め込まれています。テーブル `tbl_b` には値が `2`、`4` の行がないため、結果でこれら2行は `NULL` が埋埋め込まれています。
