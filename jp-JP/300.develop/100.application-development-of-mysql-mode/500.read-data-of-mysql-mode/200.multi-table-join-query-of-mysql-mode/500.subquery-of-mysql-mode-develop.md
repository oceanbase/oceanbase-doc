...
サブクエリは、SQL文のさまざまな句に記述できます。例えば、`SELECT` 句、`FROM` 句、`WHERE` 句などです。`SELECT` 文の `FROM` 句内のサブクエリは、インラインビューとも呼ばれます。インラインビューでは、任意の数のサブクエリをネストできます。`SELECT` 文の `WHERE` 句内のサブクエリは、ネストされたサブクエリとも呼ばれます。
...
* `ALL` は `=`、`>`、`>=`、`<`、`<=`、`<>` 演算子と組み合わせて使用でき、すべてのデータと等しい、より大きい、以上、より小さい、以下、等しくないことを意味します。
...
```sql
obclient> SELECT a.*,(SELECT count(*) FROM emp b WHERE b.deptno = a.deptno) AS 従業員数 FROM dept a;
+--------+------------+-------------+--------------+
...
* スカラーサブクエリ
    ...
    ```sql
    /*最低給与が部門30の最低給与より高い部門の部門番号とその最低給与を照会する*/
...
* 列サブクエリが1列の複数行の結果セットを返す場合

    ```sql
    /*jobがMANAGERの他の職位のすべての給与よりも低い給与を持つ従業員の従業員番号、氏名、職位、給与を返す*/

    obclient> SELECT empname 従業員名, empno 従業員番号, job 役職, sal 給与 FROM emp WHERE sal < ALL (SELECT DISTINCT sal
    ...
    obclient> SELECT empname 従業員名, empno 従業員番号, job 役職, sal 給与 FROM emp WHERE sal < ALL (SELECT min(sal)
        FROM emp WHERE job = 'MANAGER') AND job!= 'MANAGER';
    +--------------+--------------+----------+---------+
    | 従業員名      | 従業員番号     | 役職      | 給与    |
    +--------------+--------------+----------+---------+
