...
本記事では主に右結合の例を説明します。完全結合に関連する例については、[完全結合](200.full-join-of-mysql-mode.md) を参照してください。左結合に関連する例については、[左結合](300.left-join-of-mysql-mode.md) を参照してください。
...
`JOIN` が返すデータが結合条件やフィルタ条件に合致するデータのほか、右テーブルのフィルタ条件には合致するものの結合条件を満たさないデータも含まれる場合、右外部結合（`RIGHT OUTER JOIN`）を使用できます。これは略して右結合（`RIGHT JOIN`）とも表記されます。右結合で返される結果において、左テーブルに属するデータが存在しない場合、その列は `NULL` を返します。
