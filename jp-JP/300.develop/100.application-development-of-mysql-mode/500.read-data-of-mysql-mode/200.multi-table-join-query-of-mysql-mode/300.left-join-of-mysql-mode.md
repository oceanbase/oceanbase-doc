...
左結合（LEFT JOIN）とは左外部結合を表し、このステートメントを使用することで、左テーブルのすべてのレコードと、左テーブルと右テーブルの結合フィールドが一致するレコードを返すことができます。

## 背景

外部結合（OUTER JOIN）は、比較演算子を使用して2つのテーブルのデータを比較します。結合結果には結合条件に合致する行だけでなく、条件に合致しない行も含まれます。

外部結合には、完全結合（FULL JOIN）、左結合（LEFT JOIN）、右結合（RIGHT JOIN）が含まれます。外部結合は結合条件を満たすすべての行を返し、一方のテーブルから使用されていない行を返すと、もう一方のテーブルの対応する位置に `NULL` を入力します。
...
外部結合ステートメントには通常左テーブルと右テーブルが含まれ、`JOIN` 句の最も左側に現れるものが左テーブル、`JOIN` 句の最も右側に現れるものが右テーブルです。

LEFT JOIN のSQL構文は以下のとおりです：
...
`JOIN` が返すデータが結合条件やフィルタ条件に合致するデータに加えて、左テーブルのフィルタ条件には合致するが結合条件を満たさないデータも含む場合、左外部結合（`LEFT OUTER JOIN`）を使用できます。これは、略して左結合（`LEFT JOIN`）とすることもできます。

左結合クエリを使用する場合、左テーブルの1行が右テーブルで見つからない場合、右テーブルのその行に自動的に `NULL` が入力されます。
