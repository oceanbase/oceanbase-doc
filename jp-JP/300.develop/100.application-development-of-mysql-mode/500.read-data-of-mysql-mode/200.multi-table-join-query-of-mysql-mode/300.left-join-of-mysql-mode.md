...
外部結合には、完全結合（FULL JOIN）、左結合（LEFT JOIN）、右結合（RIGHT JOIN）が含まれます。外部結合は、結合条件がマッチするすべての行を返すとともに、一方のテーブルから使用されなかった行も返し、もう一方のテーブルの対応する位置には `NULL` が入力されます。
...
LEFT JOINのSQL構文フォーマットは以下のとおりです：
