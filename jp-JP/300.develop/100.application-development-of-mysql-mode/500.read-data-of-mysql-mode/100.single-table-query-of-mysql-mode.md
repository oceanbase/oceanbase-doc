...
| table_name                  | データをクエリする対象のテーブル名。|
| WHERE query_condition       | オプションのパラメーターで、検索条件を指定するために使用します。条件に合致する行のみが返されます。|
| GROUP BY group_by_condition | オプションのパラメーターで、指定された列に基づいて結果をグループ化します。集約関数と併用するのが一般的です。|
| HAVING group_condition      | オプションのパラメーターで、グループ化後の結果セットをフィルタリングし、条件を満たすグループのみを返します。|
| ORDER BY column_list        | オプションのパラメーターで、結果セットをソートします。1列または複数の列を指定してソートすることができます。|
| ASC | DESC                  | オプションパラメータで、ソートの順序を指定します。ASCは昇順（デフォルト）、DESCは降順を表します。|
| LIMIT limit_clause          | オプションのパラメーターで、返される結果セットの行数を制限するために使用します。|
| column_list                 | 検索する列を指定するパラメーターで、単一の列または複数の列をカンマ（,）で区切って指定できます。|
| column_name                 | 検索する列名。|
...
例1：以下のSQLステートメントを使用して、テーブル `student` のすべての列をクエリします。
...
等しい（`=`）：指定した列内のターゲット値と等しいデータをクエリします。値が文字列タイプの場合は、シングルクォートまたはダブルクォートで囲む必要があります。

例4：次のSQLステートメントを使用して、テーブル `student` 内の列 `gender` が `1` に等しいすべての行をクエリし、これらの行の `id`、`name`、`gender` 列のデータを返します。
...
例5：次のSQLステートメントを使用して、テーブル `student` 内の列 `gender` が `1` に等しくないすべての行をクエリし、これらの行の `id`、`name`、`gender` 列のデータを返します。
...
例6：次のSQLステートメントを使用して、テーブル `student` 内の列 `score` が `90` より小さいすべての行をクエリし、これらの行の `id`、`name`、`score` 列のデータを返します。
...
例7：次のSQLステートメントを使用して、テーブル `student` 内の列 `gender` が `1` に等しく `score` が `90` 以下のすべての行をクエリし、これらの行の `id`、`name`、`gender`、`score` 列のデータを返します。
...
例8：次のSQLステートメントを使用して、テーブル `student` 内の列 `gender` が `1` に等しいか `score` が `90` より小さいすべての行をクエリし、これらの行の `id`、`name`、`gender`、`score` 列のデータを返します。
...
例9：次のSQLステートメントを使用して、テーブル `student` 内の `name` に `am` が含まれるすべての行をクエリし、これらの行の `id` と `name` 列のデータを返します。
...
<main id="notice" type='notice'>
  <h4>注意</h4>
  <p>区間クエリの2つの境界値は入れ替えることができません。左側の値以上、かつ右側の値以下である必要があります。</p>
</main>

例10：次のSQLステートメントを使用して、テーブル `student` 内の列 `score` が `85 ～ 90` の範囲に存在するすべての行をクエリし、これらの行の `id`、`name`、`score` 列のデータを返します。
...
例11：次のSQLステートメントを使用して、テーブル `student` 内の `id` 値が集合 `(1, 3, 5, 7)` を満たすすべての行をクエリし、これらの行の `id` と `name` 列のデータを返します。
...
比較演算子、`LIKE`、`BETWEEN AND`、`IN`、`NOT IN` は、`NULL` 値に対するクエリ結果が正確ではないため、`NULL` 値専用のクエリステートメント `IS NULL` および `IS NOT NULL` を使用することを推奨します。この他にも、通常の数値と `NULL` 値のどちらも判断することができる NULL セーフ演算子（`<=>`）を使用することもできます。

#### IS NULL

`IS NULL` 条件は、指定された列の値が `NULL` であるデータをクエリするために使用されます。

例12：次のSQLステートメントを使用して、テーブル `student` 内の列 `notes` が NULL のすべての行をクエリし、これらの行の `id`、`name`、`score`、`notes` 列のデータを返します。
...
例13：次のSQLステートメントを使用して、テーブル `student` 内の列 `notes` がNULLではないすべての行をクエリし、これらの行の `id`、`name`、`score`、`notes` 列のデータを返します。
...
例14：テーブル `fruit_order` 内の各顧客の注文数をクエリし、`user_id` と `COUNT(order_id)` を出力します。
...
例15：テーブル `fruit_order` 内の各顧客の毎年の注文数をクエリし、`user_id`、 `order_year` と `COUNT(order_id)` を出力します。
...
例17：2019年の注文数が1より大きい顧客をクエリし、`user_id` と `COUNT(order_id)` を出力します。
...
例18：各顧客の注文の最大金額をクエリし、`user_id` と `MAX(fruit_price)` を金額の降順で出力します。
...
例23：テーブル `student` 内の学生情報を `score` が `85`より大きいという条件でクエリし、`DAY(enrollment_date)` に基づいて昇順で表示します。
...
例24：テーブル `student` の `id`、`name` 列のデータのうち、最初の5件のレコードのみをクエリします。
...
* クエリステートメントの最適化に関する詳細については、[クエリ書き換えの概要](https://en.oceanbase.com/docs/common-oceanbase-database-10000000001974685) を参照してください。
