...
|slug|use-numerical-functions-in-queries-mysql-compatible-mode|
...
よく使われる数値関数には、合計関数SUM()、平均値関数AVG()、切り上げ関数CEIL()、切り捨て関数FLOOR()、丸め関数ROUND()などがあります。
...
`SUM()`、`ROUND()`、`AVG()` を使用して、各おもちゃの総売上高と1注文あたりの平均売上高を照会します。
