...
一般的に使用される文字列関数には、文字列の長さを求める関数LENGTH()とCHAR_LENGTH()、文字列を切り取る関数SUBSTR()、文字列を結合する関数CONCAT()とCONCAT_WS()、文字列の大文字と小文字を変換する関数UPPER()とLOWER()、文字列の前後接辞を削除する関数LTRIM()、RTRIM()とTRIM()などがあります。
