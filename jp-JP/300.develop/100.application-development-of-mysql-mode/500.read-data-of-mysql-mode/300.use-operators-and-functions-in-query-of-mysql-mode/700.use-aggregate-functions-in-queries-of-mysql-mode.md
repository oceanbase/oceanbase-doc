...
本記事では、具体的な例を通じてクエリ内で集約関数を使用する方法を説明します。

## 集約関数

集約関数は、一連のレコードをスキャンして、単一の行を返します。この一連のレコードは、テーブルまたはビュー、あるいはサブクエリの結果です。

グループ化クエリでは、集約関数は通常 `GROUP BY` 句と共に使用され、1つ以上の列の値でグループ分けを行い、各グループごとに1行の結果を返します。
...
OceanBaseデータベースのMySQLモードでは、集約関数が `GROUP BY` 句と共に使用される場合、`SELECT-list` 内の列に特別な要件はありません。`SELECT-list` 内の列が `GROUP BY` 句内の列と一致する必要がある場合は、MySQLコマンドラインの `sql_mode` を `ONLY_FULL_GROUP_BY` に設定する必要があります。例：
