...
グループ化クエリでは、集計関数は通常 `GROUP BY` 句と併せて使用され、1つ以上の列の値でグループ分けを行い、各グループごとに1行の結果を返します。
...
OceanBaseデータベースのMySQLモードでは、集約関数を `GROUP BY` 句と併せて使用する場合、`SELECT List` 内の列に特別な要件はありません。`SELECT List` 内の列が `GROUP BY` 句内の列と一致する必要がある場合は、MySQLコマンドラインの `sql_mode` を `ONLY_FULL_GROUP_BY` に設定する必要があります。例：
