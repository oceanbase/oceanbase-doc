...
|slug|use-arithmetic-operators-in-queries-mysql-compatible-mode|

# クエリにおける算術演算子の使用

本記事では、具体的な例を通してクエリ内で算術演算子を使用する方法を紹介します。

## 算術演算子

クエリでよく使われる算術演算子には、+（加）、-（減）、*（乗）、/（除）、-（反転）、MOD（剰余）が含まれます。これらの演算子は数値データ型の列に適用できます。
...
条件に合致する商品の数量と価格を照会し、数量に価格を掛けることで各商品の支払い総額を求めます。`select_list` に `t2.ol_quantity * t3.i_price` という列を追加し、`item_sum_price` に名前を変更します。
