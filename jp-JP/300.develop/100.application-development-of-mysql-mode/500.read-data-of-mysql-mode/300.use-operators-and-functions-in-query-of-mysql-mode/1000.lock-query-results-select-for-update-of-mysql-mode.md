...
OceanBaseデータベースはマルチバージョン同時実行制御（MVCC）をサポートしています。デフォルトでは、読み取りトランザクションは書き込みトランザクションの実行をブロックしませんが、`SELECT ... FOR UPDATE` を使用することで、読み取りトランザクションの対象にロックをかけることで、書き込みトランザクションをブロックすることができます。

本記事では、具体的な例を通じて、`SELECT ... FOR UPDATE` を使用してクエリ結果をロックする方法を紹介します。

## ロック動作

特定の行のデータが他のトランザクションによってロックされている場合、他のトランザクションが行ロックを解放するのを待たないために、`NOWAIT` および `SKIP LOCKED` オプションを `SELECT ... FOR UPDATE` のロック付き読み取りのステートメントと一緒に使用できます。具体的な紹介は以下のとおりです：

* `SELECT ... FOR UPDATE` 句を使用する際、以下の動作があります：
...
* `SELECT ... FOR UPDATE NOWAIT` 句を使用する場合、次の動作があります：

  トランザクションが行をロックしようとしたとき、その行が別のトランザクションによってロックされている場合、ロックが解放されるのを待つのではなく、即座にエラーが返されます。

* `SELECT ... FOR UPDATE SKIP LOCKED` 句を使用する場合、次の動作があります：
...
1. 以下のステートメントを実行して、自動コミット機能を無効にします。
    ...
    <!-- 自動コミット機能に関する情報は、[autocommit](../../../../700.reference/800.configuration-items-and-system-variables/200.system-variable/300.global-system-variable/400.autocommit-global.md) を参照してください。 -->

    自動コミット機能に関する情報は、[autocommit](https://en.oceanbase.com/docs/common-oceanbase-database-10000000001972174) を参照してください。

    <main id="notice" type='notice'>
      <h4>注意</h4>
      <p>Globalレベルの変数を設定しても、現在のSessionには無効であり、有効にするには、再ログインして新しいSessionを確立する必要があります。</p>
    </main>
