...
型変換関数は、あるデータ型を別のデータ型に変換することができます。例えば、数値型や日付時刻型と文字列型の相互変換などです。

## 例

### 日付時刻文字列を日付時刻型に変換

OceanBaseデータベースのMySQLモードでは、日付時刻文字列を直接`DATE`型に代入できます。OceanBaseデータベースはそれを自動的に日付時刻型に変換できます。また、`CONVERT()`または`CAST()`関数を使用してデータ型を変換することもできます。SQLステートメントの例は以下のとおりです：
...
<main id="notice" type='explain'>
  <h4>説明</h4>
  <p><code>DATE_FORMAT</code>関数を使用すると、日付時刻型を文字列型に変換できます。</p>
</main>
