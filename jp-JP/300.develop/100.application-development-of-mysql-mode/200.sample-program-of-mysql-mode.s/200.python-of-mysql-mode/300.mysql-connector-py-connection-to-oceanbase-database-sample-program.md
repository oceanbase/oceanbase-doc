...
# MySQL-connector-pythonによりOceanBaseデータベースに接続するサンプルプログラム
...
### 手順2：MySQL-connector-pythonドライバーをインストールする
...
### 手順3：test.pyファイルを作成し、データベース接続情報を入力する

**手順1：OceanBaseデータベースの接続文字列を取得する** に記載されている情報に基づいて、`test.py`ファイルを作成して、データベース接続情報を入力します。
...
MySQL-connector-pythonを使用してOceanBaseデータベースに接続する際、さまざまなエラーが発生する可能性があります。以下は、一般的なエラーとそのその処理方法です：

1. **接続エラー**：データベースに接続できない場合、ホスト名、ポート、ユーザー名、パスワード、データベース名を含む接続パラメーターが正しいかどうか確認してください。
...
* データベースの作成に関する詳細については、[CREATE DATABASE](../../../../700.reference/500.sql-reference/100.sql-syntax/200.common-tenant-of-mysql-mode/600.sql-statement-of-mysql-mode/2100.create-database-of-mysql-mode.md) を参照してください。
