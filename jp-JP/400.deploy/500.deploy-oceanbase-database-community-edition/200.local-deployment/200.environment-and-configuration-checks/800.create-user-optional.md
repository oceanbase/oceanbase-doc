...
本記事は、一般ユーザーを作成し、sudo 権限を付与する方法について説明します。

個人ユーザーの場合は、root ユーザーでクラスターを直接デプロイし、この手順をスキップすることができます。企業ユーザーの場合は、セキュリティ確保のため、マシンに標準ユーザーアカウントを作成することを推奨します。実際の必要に応じて、よく使用するアカウントを作成することができます。
...
以下の手順に従ってユーザーを作成します。ここでは、admin ユーザーの作成を例として説明します。

1. 以下のコマンドを実行して、アカウント admin を作成します。
...
2. 以下のコマンドを実行して、アカウント admin のパスワードを設定します。

   ```shell
   [root@test001 ~]# passwd admin
   ```

3. (オプション) アカウント admin に sudo 権限を設定します。
...
4. 以下のコマンドを実行して、クラスター関連ファイルのディレクトリのアクセス権限を確認します。
