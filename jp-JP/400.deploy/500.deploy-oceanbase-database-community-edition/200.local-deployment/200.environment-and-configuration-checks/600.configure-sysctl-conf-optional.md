...
* ARMおよびハイゴアーマークのアーキテクチャで、`/etc/sysctl.conf` ファイルを変更し、 `kernel.numa_balancing`、 `vm.zone_reclaim_mode` 、 `vm.swappiness` パラメータを `0` に設定することを推奨します。次のコマンドを使用して `/etc/sysctl.conf` ファイルを開きます。
