|description||
|---|---|
|slug|configure-clock-source-oceanbase-community-optional|

# （オプション）クロックソースの設定

本記事では、クラスタ内の複数のマシンにNTPクロックソースを設定する方法について説明します。OceanBaseデータベースを単一マシンでデプロイする場合、個人テストを行う場合、またはNTPクロック同期が既に設定されている場合は、この設定は不要です。

分散型OceanBaseクラスタをデプロイする場合は、クラスタ内の各マシンの時刻が同期していることを保証する必要があります。そうでない場合、クラスタは起動できず、サービスの実行中に異常が発生する可能性があります。OceanBaseクラスタで許容される時刻偏差は2秒を超えてはなりません。2秒を超えると、リーダー不在の状態が発生します。時刻同期を回復した後、OceanBaseクラスタを再起動すると正常に戻ります。

<main id="notice" type='explain'>
   <h4>説明</h4>
   <ul>
   <li>
   <p>本記事は、x86アーキテクチャのCentOS Linux 7.9イメージ環境に基づいた操作手順を提供しています。その他の環境では若干異なる場合がありますので、実際の環境に準じてください。</p>
   </li>
   <li>
   <p>OceanBaseはV4.0.0から動的な時刻変更をサポートしています。変更間隔が2秒以内であれば、データの正確性やクラスタの安定した動作に影響はありません。</p>
   </li>
   <li>
   <p>OCPがOceanBaseクラスタノード上にデプロイされていない場合でも、OCPノードとOceanBaseクラスタノードの時刻同期を設定する必要があります。これは、時刻偏差によってOCPの操作中に異常が発生するのを防ぐためです。</p>
   </li>
   </ul>
</main>

## 前提条件

NTPクロックソースを設定する前に、すべてのマシンでrootユーザー権限を持っていることを確認してください。

## 手順

以下の手順に従って、NTPクロック同期を設定します。

1. 各マシンで以下のコマンドを実行し、NTPをインストールします。

   ```shell
   [root@test002 ~]# yum install ntp ntpdate -y
   ```

2. 以下のコマンドを実行して、サーバーのNTP接続を確認します。

   ```shell
   [root@test002 ~]# ntpq -4p
   remote refid st t when poll reach delay offset jitter
   ==============================================================================
   *time6.aliyun.co xxx.xxx.xxx.xxx 2 u 712 1024 377 21.951 4.253 4.208
   ```

3. （オプション）NTPサーバーサービスを有効にします。

   NTPサーバーが既に存在する場合は、この手順をスキップしてください。本記事では、ターゲットマシン`10.10.10.1`をNTPサーバーとして使用します。

   1. 設定ファイルを編集します。

      rootユーザーでNTPサーバーにログインし、以下のコマンドを実行して設定ファイルを編集します：

      ```shell
      [root@test001 ~]# vi /etc/ntp.conf
      ```

      設定ファイルに以下の内容を追加します：

      ```shell
      server <your_ntp_server_ip>
      ```

      設定ファイルを保存して閉じます。

   2. 以下のコマンドを実行して、NTPサーバーサービスを再起動します：

      ```shell
      [root@test001 ~]# service ntpd restart
      ```

   3. 以下のコマンドを実行して、NTPサーバーサービスが正常に起動したかどうか確認します：

      ```shell
      [root@test001 ~]# ps -ef | grep -i ntpd
      ```

   4. 以下のコマンドを実行して、NTPサービスの起動時自動起動を設定します：

      ```shell
      [root@test001 ~]# chkconfig ntpd on
      ```

4. NTPクライアントサービスを有効にします。

   <main id="notice" type='notice'>
     <h4>注意</h4>
     <p>NTPサーバーサービスが起動した後、5分待ってからNTPクライアントサービスを有効にしてください。そうでない場合、システムは「no server suitable for synchronization found」というメッセージを表示します。</p>
   </main>

   1. rootユーザーでOBServerノードサーバーにログインします。

   2. 以下のコマンドを実行して、NTPサービスを停止します：

      ```shell
      [root@test002 ~]# service ntpd stop
      ```

   3. 以下のコマンドを実行して、NTPサーバーの時刻を同期します：

      ```shell
      [root@test002 ~]# ntpdate 10.10.10.1
      ```

   4. 設定ファイルを編集します。

      以下のコマンドを実行して設定ファイルを開きます：

      ```shell
      [root@test002 ~]# vi /etc/ntp.conf
      ```

      設定ファイルに以下の内容を追加します：

      ```shell
      server <your_ntp_server_ip>
      ```

   5. 以下のコマンドを実行して、NTPサービスを起動します：

      ```shell
      [root@test002 ~]# /etc/init.d/ntpd start
      ```

5. 以下のコマンドを実行して、設定が正常に反映されているか確認します。

   実行コマンド：

   ```shell
   [root@test002 ~]# ntpstat
   ```

   次の結果が返されます：

   ```shell
   synchronised to NTP server (xxx.xxx.xxx.xxx) at stratum 3
   time correct to within 2 ms
   polling server every 64 s
   ```

   実行コマンド：

   ```shell
   [root@test002 ~]# timedatectl
   ```

   次の結果が返されます：

   ```shell
         Local time: Thu 2021-04-22 11:02:32 CST
     Universal time: Thu 2021-04-22 03:02:32 UTC
           RTC time: Thu 2021-04-22 11:02:32
          Time zone: Asia/Shanghai (CST, +0800)
        NTP enabled: yes
   NTP synchronized: yes
    RTC in local TZ: yes
         DST active: n/a
   ```

   上記のコマンドを実行して対応する結果が返されれば、NTPサービスが有効であることを示します。