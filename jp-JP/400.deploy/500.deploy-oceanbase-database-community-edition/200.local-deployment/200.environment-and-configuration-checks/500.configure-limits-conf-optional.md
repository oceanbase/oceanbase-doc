...
<main id="notice" type='explain'>
  <h4>説明</h4>
  <ul>
  <li><p><code>/etc/security/limits.d/20-nproc.conf</code> ファイルに nproc の設定が存在するかどうか確認し、存在する場合は、ファイル内の nproc の値を同時に修正する必要があります。</p></li>
  <li><p><code>limits.conf</code> の設定は、新しくログインするセッション（例えば、新しいターミナル、SSH接続、または新しく起動したプロセス）にのみ適用されます。既に存在するセッションやプロセスは、新規設定を自動的に引き継ぐことはなく、再ログインするか、関連するサービスを再起動する必要があります。</p></li>
  </ul>
</main>
