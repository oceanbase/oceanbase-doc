...
OceanBaseデータベースは、Java SDKを介してファイルシステムに接続し、外部テーブルとしてのサードパーティデータソースとして接続できるようになりました。この機能はJNIフレームワークを使用して実現されているため、対応するOceanBaseデータベース環境にはJava SDK環境を構築する必要があります。

## 環境設定

<main id="notice" type='notice'>
  <h4>注意</h4>
  HDFS/ODPS 外表機能を使用する必要がある場合は、OceanBaseデータベースが分散デプロイされている場合、該当するノードすべてにJava環境の設定と必要な依存関係のインストールが必要であり、単一のノードのみに設定してはなりません。
</main>

すべての環境構成と依存関係のインストールは、adminユーザーとして実行する必要があります。ユーザーを切り替えるには、次の方法を使用してください。
...
2. 対応するインストールパッケージを正常に解凍します。
    ...
    <main id="notice" type='notice'>
      <h4>注意</h4>
      <p>このパスは、現在のOBServerノードでJavaのホームディレクトリを設定するためのものです。つまり、設定項目ob_java_homeの値を指定するためのものです。</p>
    </main>
...
* devdeps-java-extensions RPMパッケージ：
  ...
  このパッケージには、HDFS外部テーブルやODPSなどの他の外部データソースで使用するコア依存ライブラリ（JARファイル）が統合されています。この拡張パッケージには、外部テーブル機能が分散環境下で動作する際の互換性とパフォーマンスの最適化を確保するための完全なJava実行時依存関係チェーンが含まれています。

#### 部署配置に依存する HDFS.so ライブラリ

1. devdeps-hdfs-sdk RPM インストールパッケージを取得します。

    * Enterprise Editionのユーザーは、サポートチームまでお問い合わせください。devdeps-hdfs-sdk RPM インストールパッケージを取得できます。
    * Community Editionのユーザーは、[Alibaba Cloud OceanBase イメージ](https://mirrors.aliyun.com/oceanbase/)ページの`development-kit/`ディレクトリをクリックし、開発ツールリソースディレクトリに移動して、devdeps-hdfs-sdk RPM インストールパッケージをダウンロードできます。
...
1. devdeps-java-extensions RPMインストールパッケージを取得します。
    ...
    * Community Editionのユーザーは、[Alibaba Cloud OceanBase イメージ](https://mirrors.aliyun.com/oceanbase/)ページの`development-kit/`ディレクトリをクリックし、開発ツールリソースディレクトリに移動して、devdeps-java-extensions RPMインストールパッケージをダウンロードします。
...
3. インストールが期待どおりに実行されているかを確認します。
    ...
    <main id="notice" type='notice'>
      <h4>注意</h4>
      <p>このパスは、OBServerの起動時にJVMが読み込む実行可能依存jarパッケージのパスを設定するために使用されます。つまり、このパスは、設定項目ob_java_connector_pathの値に対応します。</p>
    </main>
...
<!-- ODPS 外部テーブルを作成する詳細な手順については、[外部テーブルの作成（MySQL モード）](../../700.reference/300.database-object-management/100.manage-object-of-mysql-mode/200.manage-tables-of-mysql-mode/1000.manage-external-tables-of-mysql-mode/200.create-a-external-table-of-mysql-mode.md) または [外部テーブルの作成（Oracle モード）](../../700.reference/300.database-object-management/200.manage-object-of-oracle-mode/100.manage-tables-of-oracle-mode/1000.manage-external-tables-of-oracle-mode/200.create-a-external-table-of-oracle-mode.md) 参照してください。 -->
