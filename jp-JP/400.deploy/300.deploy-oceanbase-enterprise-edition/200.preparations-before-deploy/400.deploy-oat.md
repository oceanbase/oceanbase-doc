...
* Dockerがインストールされ、起動していること。Docker Community Edition 17.03以降のバージョンを推奨します。

   <main id="notice" type='notice'>
   <h4>注意</h4>
   <p>oat-all-in-oneインストールパッケージを使用してOATをデプロイする場合、oat-all-in-oneの<code>install.sh</code>スクリプトが自動的にDockerをインストールするため、別途Dockerをインストールする必要はありません。</p>
   </main>
...
### プラン1：oat-all-in-oneインストールパッケージを使用してOATをデプロイ

<main id="notice" type='notice'>
  <h4>注意</h4>
  <p>oat-all-in-oneインストールパッケージを使用してデプロイする前に、ターミナルで<code>setenforce 0</code>コマンドを入力してSELinux（Security-Enhanced Linux）を無効にすることを推奨します。</p>
</main>

1. `scp`コマンドを使用して、oat-all-in-oneインストールパッケージをサーバーにアップロードします。

   ここで、`oat_server_ip`はOATをデプロイするサーバーのIPアドレス、`oat_directory`はインストールパッケージを格納するディレクトリ、`oat_xxx.tar`はインストールパッケージ名です。
...
3. インストールスクリプト `install.sh` を実行します。
   ...
   環境にDockerがインストールされていない場合、このコマンドを実行すると、Dockerが自動的にインストールされ、OATがデプロイされ、既存のイメージとツールソフトウェアパッケージがスキャンされます。

   プロンプトに従い、以下の情報を順次確認します：
...
<main id="notice" type='explain'>
  <h4>説明</h4>
  <p><ul><li><code>install.sh</code> スクリプトでOATコンテナを起動する場合、デフォルトで <code>--net host</code>（ホストネットワークモード）が使用されます。他のネットワークモードに設定する必要がある場合は、OATコンテナをダウンロードして手動で起動してください。</li><li>x86（aarch64）版のoat-all-in-oneには、デフォルトでx86（aarch64）およびnoarchのイメージとソフトウェアパッケージのみが含まれています。aarch64（x86）アーキテクチャが必要な場合は、ご自身でダウンロードし、OATコンテナのマウントディレクトリにコピーしてから、スキャンタスクを実行して追加してください。</li></ul> </p>
</main>

### 方法2：OAT Dockerイメージを使用したOATの独立デプロイ
...
4. `docker images` コマンドを使用して、OATイメージのタグを取得します。
   ...
   <main id="notice" type='explain'>
     <h4>説明</h4>
     <p>以下のコマンドは、ローカルにOATインストールパッケージが1つだけロードされている場合にのみ適用されます。複数のインストールパッケージがロードされている場合は、<code>docker images</code> を実行して表示内容を確認し、コロン（:）で最初の2列の内容を結合してください。</p>
   </main>
...
OATのインストールに関する詳細情報については、[OATのインストール](https://www.oceanbase.com/docs/enterprise-oat-doc-cn-1000000000304881)を参照してください。