...
この記事では、Dockerを使用してOATをデプロイする方法について説明します。

## 前提条件

* OATサーバーのOSが要件を満たしていること。詳細については、[サーバー構成](../200.preparations-before-deploy/200.configure-servers.md)を参照してください。
* Dockerがインストール済みで起動していること。推奨されるDockerバージョンはCommunity Edition 17.03以降です。

   <main id="notice" type='notice'>
   <h4>注意</h4>
   <p>OATをoat-all-in-oneインストールパッケージを使用してデプロイする場合、oat-all-in-oneの<code>install.sh</code>スクリプトが自動的にDockerをインストールするため、Dockerを別途インストールする必要はありません。</p>
   </main>
...
3. インストールスクリプト `install.sh` を実行します。
   ...
   Dockerがインストールされていない環境では、このコマンドを実行すると自動的にDockerがインストールされ、OATがデプロイされ、既存のイメージとツールソフトウェアパッケージがスキャンされます。

   プロンプトに従って、以下の情報を順番に確認してください：

   1. Dockerのルートディレクトリを指定してください。デフォルト値は `/docker` ですが、カスタマイズ可能です。Dockerがインストール済みの場合は、この手順がスキップされます。
   2. OATデータディレクトリのパスを指定してください。デフォルト値は `/oat_data` ですが、カスタマイズ可能です。
   3. OAT HTTPリスニングポートを指定してください。デフォルト値は `7000` ですが、カスタマイズ可能です。
   4. OATデータベースポートを指定してください。デフォルト値は `3306` ですが、カスタマイズ可能です。
   5. OAT管理者アカウントのパスワード（初期ログインパスワードの設定）を入力してください。カスタマイズ可能です。
   6. 返されたURLにアクセスしてOATにアクセスし、OATサービスの可用性を検証してください。
   ...
   プロンプトに従って、以下の構成を完了してください：

   1. Dockerのルートディレクトリはデフォルト値を使用し、Enterキーを押します。
   ...
   3. OAT HTTPリスニングポートはデフォルト値を使用し、Enterキーを押します。

      ```shell
      Input the OAT HTTP listen port: 7000
      ```

   4. OATデータベースポートはデフォルト値を使用し、Enterキーを押します。

      ```shell
      Input the OAT database port: 3306
      ```

   5. OAT管理者アカウントのパスワード（初期ログインパスワード）を設定して、Enterキーを押します。
...
<main id="notice" type='explain'>
  <h4>説明</h4>
  <p><ul><li><code>install.sh</code> スクリプトは、OATコンテナを起動する際にデフォルトで <code>--net host</code>（ホストネットワークモード）を使用します。他のネットワークモードに設定する必要がある場合は、OATコンテナをダウンロードして手動で起動してください。</li><li>x86（aarch64）バージョンのoat-all-in-oneには、デフォルトでx86（aarch64）およびnoarchのイメージとソフトウェアパッケージのみが含まれています。aarch64（x86）アーキテクチャが必要な場合は、ご自身でダウンロードしてOATコンテナのマウント先の対応するディレクトリにコピーし、スキャンタスクを開始して追加してください。</li></ul> </p>
</main>

### プラン2：OAT Dockerイメージを使用した独立デプロイ
...
2. OATディレクトリをマウントします。
   ...
   サーバー上に `/data_dir` ディレクトリを作成し、OATの永続化データを保存します。`/data_dir` ディレクトリがOATコンテナにマウントされると、OATは自動的に `/data_dir/logs`、`/data_dir/images`、`/data_dir/db` の各ディレクトリを作成します。これらはそれぞれ、OATのシステムログ、コンポーネントおよびプロダクトのDockerイメージ、そしてOATのデータベースファイルを格納するために使用されます。例：
...
3. OATインストールパッケージをイメージとしてロードします。
   ...
   OATインストールパッケージが保存されているディレクトリに移動し、以下のステートメントを実行します：
...
4. `docker images` コマンドを使用して、OATイメージのタグを取得します。
   ...
   <main id="notice" type='explain'>
     <h4>説明</h4>
     <p>以下のコマンドは、ローカル環境にOATインストールパッケージが1つだけ読み込まれている場合にのみ適用されます。複数のインストールパッケージが読み込まれている場合は、<code>docker images</code> を使用して表示内容を確認し、コロン（:）で最初の2列の内容を結合します。</p>
   </main>
...
5. `docker run` コマンドを実行して、OATを起動します。
   ...
   <main id="notice" type='explain'>
     <h4>説明</h4>
     <p><ul><li>OATのHTTPサービスはデフォルトで <code>7000</code> ポートをリスニングしていますが、<code>-e HTTP_PORT=7001</code> パラメータを指定して他のポートに変更できます。</li><li><code>-e OAT_INITIAL_ADMIN_PASSWORD=xxx</code> パラメータを指定して、OATのパスワードを変更できます。</li><li>OATにはMariaDBがデータストレージとして組み込まれており、デフォルトで <code>3306</code> ポートをリスニングしています。このポートは <code>-e DB_PORT=3307</code> パラメータを指定して他のポートに変更できます。</li><li><code>--net host</code> パラメータを使用して起動することを推奨します。bridgeネットワークモードで起動されたコンテナは、<code>docker0</code> ブリッジの障害やOSパラメータ <code>ip_forward</code> の影響を受ける可能性があるためです。</li></ul></p>
   </main>
...
OATのデプロイが成功したら、OATにログインしてデプロイが正常に完了したかどうかを確認する必要があります。
...
2. ブラウザのウィンドウにログイン画面が表示されれば、OATのインストールと起動に成功しています。
   ...
   <main id="notice" type='notice'>
     <h4>注意</h4>
     <p>OceanBaseアフターサービスサポートにお問い合わせいただき、OATのデフォルトユーザー名adminアカウントのデフォルトパスワードを取得してください。アカウントのセキュリティを確保するため、初回ログイン後は速やかにパスワードを変更してください。</p>
   </main>
...
3. パスワードを設定してアカウントを有効化します。
   ...
   `admin` アカウントのデフォルトパスワードでログインし、その後パスワードを設定してアカウントを有効化します。
...
OATのインストールに関する詳細は、[OATのインストール](https://www.oceanbase.com/docs/enterprise-oat-doc-cn-1000000000304881)を参照してください。