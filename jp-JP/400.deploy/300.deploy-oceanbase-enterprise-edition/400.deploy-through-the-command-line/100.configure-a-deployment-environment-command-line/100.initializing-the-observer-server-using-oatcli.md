...
3. （オプション）以下のコマンドを使用して、oatcliの使い方を確認できます。
   ...
   詳細については、[oatcli](https://en.oceanbase.com/docs/enterprise-oat-10000000000949539)を参照してください。
...
7. OBServerサーバーを追加します（サーバーの追加と初期化）。
   ...
   * `--add-only | --no-add-only`：カスタムオプション。サーバーの初期化設定を行うかどうか [デフォルト値：no-add-only]。
