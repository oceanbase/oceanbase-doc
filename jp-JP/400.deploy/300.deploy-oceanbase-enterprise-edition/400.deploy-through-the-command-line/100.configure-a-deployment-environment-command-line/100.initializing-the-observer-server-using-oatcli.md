...
# oatcliを使用してOBServerサーバーを初期化する

oatcliはOATリソースを管理するためのコマンドラインツールであり、コマンドラインを使用してサーバーの追加やOceanBase製品のインストールなどをサポートしています。

## 前提条件

* OBServerサーバーを準備し、関連する設定が完了していること。詳細については、[サーバーの準備](../../200.preparations-before-deploy/100.prepare-servers.md)および[サーバー設定](../../200.preparations-before-deploy/200.configure-servers.md)を参照してください。
* OATがインストール済みであること。詳細については、[OATのデプロイ](../../200.preparations-before-deploy/400.deploy-oat.md)を参照してください。
* OATサーバーに対する`root`ユーザー権限を持っていること。

## 手順

1. OATがインストールされたサーバーに接続します。

   例：

   `ssh` を使用して、OATがインストールされたサーバーに接続します。
...
2. 以下のコマンドを使用して、OATコンテナに移動します。

   <main id="notice" type='explain'>
   <h4>説明</h4>
   <p>以下のコマンドを実行するには、<code>root</code>権限が必要です。</p>
   </main>
...
3. (オプション) 以下のコマンドを使用して、oatcliの使い方を確認できます。

   ```shell
   oatcli --help
   ```

   詳細については、[oatcli](https://www.oceanbase.com/docs/enterprise-oat-doc-cn-10000000002024800)を参照してください。
...
4. OAT APIアドレス、ユーザー名、パスワードを設定して、OAT APIを呼び出します。
...
5. **データセンター**を作成します。
   ...
   サーバー用のデータセンター情報を作成する方法は次のとおりです：
...
6. **クレデンシャル**を作成します。
   ...
   サーバー接続用のSSHクレデンシャルを作成する方法は次のとおりです：
   ...
   **クレデンシャル**の名前を `root_148`、ユーザータイプを `root` ユーザー、認証タイプをパスワード認証として作成します。
...
7. OBServerサーバーを追加します(サーバーの追加と初期化)。
   ...
   * `-c, --credential-name TEXT`：クレデンシャル名。既に存在している必要があります[必須]。
   * `--add-only | --no-add-only`：カスタムオプション。サーバーの初期化構成を実行するかどうか[デフォルト値：no-add-only]。

     * `--add-only`：サーバーのみを追加し、OS構成操作は実行しません。
     * `--no-add-only`：OS構成操作を実行します。
   ...
   * `IPS...`：サーバーIPアドレス[必須]。
   * `INIT_TAG:{observer|obproduct|both}`：サーバーの用途[必須]。

      * `observer`：OBServerのインストール。
      * `obproduct`：OceanBaseプロダクトサービスのインストール。
      * `both`：OBServerとOceanBaseプロダクトサービスの両方のインストール。

   例：

   **OBServerサーバー**を追加します。使用する**クレデンシャル**は`root_148`、使用する**データセンター名:リージョン**は`hz0:hangzhou`、**サーバー用途**は`observer`として設定します。
...
* [コマンドラインを使用して2レプリカ+アービトレーションサービスのOceanBaseクラスタをデプロイする](../200.deploy-the-oceanbase-cluster-command-line/200.deploy-the-quorum-high-availability-service.md)
* [コマンドラインを使用して1レプリカのOceanBaseクラスタをデプロイする](../200.deploy-the-oceanbase-cluster-command-line/300.deploy-single-replica-oceanbase-cluster.md)
* [コマンドラインを使用して3レプリカのOceanBaseクラスタをデプロイする](../200.deploy-the-oceanbase-cluster-command-line/400.deploy-three-oceanbase-replica-clusters.md)
