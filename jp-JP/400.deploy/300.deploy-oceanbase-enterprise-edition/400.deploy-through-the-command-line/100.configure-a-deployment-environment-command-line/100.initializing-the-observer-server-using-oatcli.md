...
oatcliはOATリソースを管理するコマンドラインツールであり、コマンドラインを使用したサーバーの追加やOceanBase製品のインストールなどをサポートします。

## 前提条件

* OBServerサーバーの準備と関連設定が完了していること。詳細については、[サーバーの準備](../../200.preparations-before-deploy/100.prepare-servers.md)および[サーバー設定](../../200.preparations-before-deploy/200.configure-servers.md)を参照してください。
...
1. OATがインストールされているサーバーに接続します。
   ...
   `ssh` を使用して、OATがインストールされているサーバーに接続します。
...
4. OAT APIのアドレス、ユーザー名、パスワードを設定して、OAT APIを呼び出します。
...
5. **データセンター**を作成します。
   ...
   サーバーのデータセンター情報を作成する方法は以下のとおりです：
   ...
   **データセンター**の名前を`hz0`、リージョンを`hangzhou`に設定します。
...
6. **クレデンシャル**を作成します。
   ...
   サーバー接続用のSSHクレデンシャルを作成する方法は以下のとおりです：
   ...
   **クレデンシャル**の名前を`root_148`、ユーザータイプを`root`ユーザー、認証タイプをパスワード認証に設定します。
...
7. OBServerサーバーを追加します（サーバーの追加と初期化）。
   ...
   * `--add-only | --no-add-only`：カスタムオプション。サーバーの初期化設定を行うか [デフォルト値：no-add-only]。

     * `--add-only`：サーバーのみを追加し、OS設定操作は実行しません。
     * `--no-add-only`：OS設定操作を実行します。
   ...
   * `INIT_TAG:{observer|obproduct|both}`：サーバーの用途 [必須]。
      ...
      * `both`：OBServerとOceanBaseプロダクトサービスのインストール。

   例：

   **OBServerサーバー**を追加します。使用する**クレデンシャル**は`root_148`、使用する**データセンター名:リージョン**は`hz0:hangzhou`、**サーバー用途**を`observer`に設定します。
...
* [コマンドラインを使用した1レプリカOceanBaseクラスタのデプロイ](../200.deploy-the-oceanbase-cluster-command-line/300.deploy-single-replica-oceanbase-cluster.md)
* [コマンドラインを使用した3レプリカOceanBaseクラスタのデプロイ](../200.deploy-the-oceanbase-cluster-command-line/400.deploy-three-oceanbase-replica-clusters.md)
