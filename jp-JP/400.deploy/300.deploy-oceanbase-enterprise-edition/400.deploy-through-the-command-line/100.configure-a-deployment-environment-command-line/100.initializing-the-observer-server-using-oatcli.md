...
* OBServerサーバーを準備し、関連する設定を完了していること。詳細については、[サーバーの準備](../../200.preparations-before-deploy/100.prepare-servers.md)および[サーバー設定](../../200.preparations-before-deploy/200.configure-servers.md)を参照してください。
...
4. OAT APIのアドレス、ユーザー名、パスワードを設定してOAT APIを呼び出します。
...
5. **データセンター**を作成します。
   ...
   サーバーのためのデータセンター情報を作成する方法は以下のとおりです：
...
7. OBServerサーバーを追加します（サーバーの追加と初期化）。
   ...
   * `--add-only | --no-add-only`：カスタムオプション。サーバーの初期化設定を実行するかどうか [デフォルト値：no-add-only]。
   ...
   **OBServerサーバー**を追加し、使用する**認証情報**は`root_148`、使用する**データセンター名:リージョン**は`hz0:hangzhou`、**サーバー用途**を`observer`に設定します。
...
8. サーバーが正常に追加されたか確認します。

   サーバーの詳細情報および関連するDAGRun情報を確認します。
