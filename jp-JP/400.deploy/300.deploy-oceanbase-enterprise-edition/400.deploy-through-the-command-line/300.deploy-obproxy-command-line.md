...
OceanBase データベースプロキシ（OceanBase Database Proxy、略称：ODP、別名：OBProxy）は、OceanBaseが提供するデータベースプロキシミドルウェアであり、接続プール管理、ロードバランシング、フェイルオーバーなどの機能を提供します。OBProxyはオプションのコンポーネントであり、そのデプロイの有無はアプリケーションの要件とシナリオによって決まります。
...
このセクションでは、RPMパッケージを使用してOBProxyをインストールする方法について説明します。
  ...
  <main id="notice" type='notice'>
    ...
    <li>1台のマシンには1つのOBProxyサービスのみをデプロイし、規定の <code>2883</code> ポートを使用することを推奨します。同一マシンに複数のOBProxyサービスをデプロイする場合は、区別のために異なるポートと異なる設定ファイルのパスを指定する必要があります。ポートは <code>3306</code> または <code>1521</code> など、他のポートにカスタマイズ可能です。</li>
    </ul>
  </main>
...
1. （オプション）obproxyへのソフトリンクを作成します。
   ...
   obproxyのインストールディレクトリは `/opt/taobao/install` です。obproxyプロセスのホームディレクトリは `/opt/taobao/install/obproxy` で、これはソフトリンクであり、実際には対応するobproxyバージョンのソフトウェアディレクトリを指しています。
...
2. obproxyプロセスの実行ログディレクトリを作成します。
   ...
   obproxyプロセスの実行ログディレクトリは、ソフトリンクにより `/home/admin/logs/obproxy/log` を指します。
...
OBProxyはバックエンドのOBServerノードと通信を維持する必要があります。そのため、事前にOceanBaseクラスタのsysテナント内にOBProxy用の接続ユーザー（proxyro）とパスワードを作成しておく必要があります。後でOBProxyを起動する際には、proxy起動パラメータを設定することで、この情報をproxyに通知できます。
...
<main id="notice" type='notice'>
    <h4>注意</h4>
    <p>OBProxyを起動する際は、adminユーザーであり、かつOBProxyソフトウェアのホームディレクトリにいる必要があります。他のユーザーや他のディレクトリから起動すると問題が発生する可能性があります。</p>
  </main>
...
1. OBProxyを起動する際には、OceanBaseクラスタの場所を知る必要があります。これは、rootservice_listパラメータで指定します。
   ...
   <main id="notice" type='explain'>
    <h4>説明</h4>
    <ul>
    <li><code>$sha1_value</code> は、実際の設定に応じてパスワードに置き換えてください。proxyの起動パラメータで設定するパスワードは、sha1ハッシュ後の値であり、元の値ではありません。例えば、proxyroユーザーの設定パスワードが <code>123456</code> の場合、<code>observer_sys_password</code> の値は <code>7c4a8d09ca3762af61e59520943dc26494f8941b</code> に設定する必要があります。</li>
...
OBProxyを介して接続する場合、ユーザー名の形式にはユーザー名、テナント名、クラスタ名が含まれる必要があります。形式は、**ユーザー名@テナント名#クラスタ名**または**クラスタ名:テナント名:ユーザー名**です。
...
以下は、実行ログ量を削減したりCPU消費を抑えたりするためのOBProxyのパラメータ設定です。
...
* OBProxyのデプロイに関する詳細は、[デプロイ方法](https://en.oceanbase.com/docs/enterprise-odp-enterprise-en-10000000000841252)を参照してください。
* OBProxyパラメータの調整に関する詳細は、[パラメータ説明](https://en.oceanbase.com/docs/enterprise-odp-enterprise-en-10000000000841253)を参照してください。
* OBProxyを介してOceanBaseデータベースに接続する方法の詳細については、[接続管理](https://en.oceanbase.com/docs/enterprise-odp-enterprise-en-10000000000841237)を参照してください。