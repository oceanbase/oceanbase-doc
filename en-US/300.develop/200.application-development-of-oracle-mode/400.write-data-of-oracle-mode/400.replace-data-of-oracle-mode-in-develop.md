...
* You have the `SELECT` object privilege on the source table, and the `INSERT` and `UPDATE` privileges on the target table. For more information about how to view your privileges, see [View user privileges](../../../600.manage/500.security-and-permissions/300.access-control/200.user-and-permission/300.permission-of-oracle-mode/600.view-user-permissions-of-oracle-mode.md). If you do not have the required privileges, contact the administrator to obtain the privileges. For more information, see [Grant direct privileges](../../../600.manage/500.security-and-permissions/300.access-control/200.user-and-permission/300.permission-of-oracle-mode/200.authority-of-oracle-mode.md).
...
The syntax of the `MERGE INTO` statement is as follows:
...
### Records in the source table do not exist in the target table

If the records in the source table do not exist in the target table, you can use the `REPLACE INTO` statement to insert these records into the target table.
...
In this example, the `id` column in the target `t_insert` table is the primary key column and must meet the `UNIQUE` constraint. The records `(4,'JP',10004)` and `(5,'CN',10005)` in the source table do not exist in the target table, and therefore are inserted into the target table.

### Records in the source table exist in the target table

If the records in the source table exist in the target table, you can use the `MERGE INTO` statement to update these records in the target table.
...
In this example, the `id` column in the target `t_insert` table is the primary key column and must meet the `UNIQUE` constraint. The `(3,'CN',10003)` record in the source `t_merge` table already exists in the target `t_insert` table, and therefore is updated in the target table.

## Use the MERGE INTO statement to replace data through a DBLink

The current OceanBase Database version allows you to write data to OceanBase Database in Oracle mode and to an Oracle database through a DBLink.
