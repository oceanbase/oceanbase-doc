...
After a table is created, you can execute the `INSERT` statement or other statements to insert rows into the table. This topic describes how to use these statements and provides examples.
...
Partition-level `INSERT OVERWRITE PARTITION` example:
...
7. Use the `insert overwrite` statement to overwrite the data in the `p1` partition of the `tbl2` table with the data from the `p1` partition of the `tbl1` table.
...
OceanBase Database allows you to write data into OceanBase Database in Oracle-compatible mode and into Oracle Database through a DBLink.
