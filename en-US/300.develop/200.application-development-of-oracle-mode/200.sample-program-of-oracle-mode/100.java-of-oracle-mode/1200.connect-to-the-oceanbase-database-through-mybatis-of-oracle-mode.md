...
2. Fill in the URL below based on the deployed OceanBase database.
    ...
    * `useServerPrepStmts=true&rewriteBatchedStatements=true` specifies the additional connection properties.

        * `rewriteBatchedStatements` specifies whether to rewrite `batchedStatement` in an `INSERT` query for execution in a single `executeQuery`. If you set this parameter to `true`, `useServerPrepStmts` is set to `false`. The default value is `false`.
...
3. Configure global settings for MyBatis.
    ...
    * `jdbcTypeForNull`: the JDBC type for null values, which is set to `NULL`.
    ...
    <settings>
        ...
        <!-- Set the local cache range session: there will be data sharing statement: statement range (so there will be no data sharing) default: session -->
...
1. Define the `Mapper` package.
    ...
    * `org.apache.ibatis.annotations.*`: all annotation classes used in MyBatis.
    * `java.util.List`: the `List` class in the `java.util` package.
    Here is the sample code:
...
2. Define the `IUserMapper` interface.
   ...
   Define an interface named `IUserMapper` that contains the methods to insert, delete, update, and query user data and the method to query user data with pagination. Use `JavaDoc` comments and `@Param` annotations to improve code readability and maintainability.
   Here is the sample code:
...
The `User.java` file defines the `User` class from which `User` objects are created. Each object contains two attributes: `id` and `name`. The class also defines the methods to get and set attribute values, and the `toString` method to convert objects into strings.
