...
2. Fill in the URL below based on the deployed OceanBase database.
    ...
    * `useSSL=false&useUnicode=true&characterEncoding=utf-8` specifies the additional connection properties.
        ...
        * `socketTimeout` specifies the network socket timeout period (`SO_TIMEOUT`), in ms. The value `0` specifies to disable socket timeout.
