...
2. Fill in the URL below based on the deployed OceanBase database.
    ...
    * `characterEncoding=utf-8&useSSL=false&serverTimezone=GMT%2B8` specifies the additional connection properties.
        ...
        * `serverTimezone` specifies the server time zone. The default value is China Standard Time (CST).
...
6. Configure core dependencies.
   ...
   5. Define a dependency named `oceanbase-client` that belongs to the `com.oceanbase` group and whose version is `2.4.3`. With this dependency, you can use the features of OceanBase Command-Line Client (OBClient), such as connections, queries, and transactions.
