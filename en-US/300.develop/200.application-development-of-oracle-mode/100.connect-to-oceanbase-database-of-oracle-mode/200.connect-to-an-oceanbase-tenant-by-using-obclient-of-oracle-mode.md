   ...
   * To connect to OceanBase Database by using an ODP, run the following command:
      ...
      * `-u` specifies the tenant account. The following account formats are supported: `username@tenant name#cluster name`, `cluster name:tenant name:username`, `cluster name-tenant name-username`, and `cluster name.tenant name.username`. The default username of the administrator of an Oracle tenant is `SYS`.
         ...
         <main id="notice" type='explain'>
           <h4>Note</h4>
           <p>When connecting to an OceanBase cluster using ODP, you can obtain the cluster name as follows:</br>First, connect to OceanBase Database using direct connection. Then, use the <code>SHOW PARAMETERS LIKE 'cluster';</code> command to retrieve the cluster name. This command will return the value of <code>VALUE</code> corresponding to the OceanBase cluster name.</p>
         </main>
