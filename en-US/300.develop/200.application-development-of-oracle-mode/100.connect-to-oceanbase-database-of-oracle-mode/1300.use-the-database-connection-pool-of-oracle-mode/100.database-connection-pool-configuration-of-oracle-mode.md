...
|----------------|-----------------------------------------------------|
| socketTimeout  | The network socket timeout (`SO_TIMEOUT`) in milliseconds. If set to 0, the timeout will be disabled. You can also limit the query time by setting the system variable `max_statement_time`. Default value: 0 (standard configuration) | 
