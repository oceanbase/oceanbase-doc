...
<!-- <div role="videolist">
      <a role='video' href='https://obbusiness-private.oss-cn-shanghai.aliyuncs.com/video-center/video/video/06%20druid-oceanbase-client.mp4'>
          ...
          Connect to OceanBase Database by using a Druid connection pool (Oracle mode)
      </a>
</div> -->
...
Modify the database connection information in the `db.properties` file in the `druid-oceanbase-client/src/main/resources/` directory based on the information obtained in **Step 2: Obtain the URL of OceanBase Database**.
...
Click [here](https://obbusiness-private.oss-cn-shanghai.aliyuncs.com/doc/img/observer-enterprise/V4.2.0/3.develop/connection-pool/druid-oceanbase-client/druid-oceanbase-client.zip) to download the project code, which is a package named `druid-oceanbase-client.zip`.

Decompress the package to obtain a folder named `druid-oceanbase-client`. The directory structure is as follows:
...
2. Configure other connection pool parameters.
    ...
    5. Set the minimum number of idle connections in the connection pool to 5. When the number of idle connections is less than 5, the connection pool automatically creates connections.
    6. Set the maximum wait time for requesting a connection to 1,000 ms. When the application requests a connection from the connection pool, if all connections are occupied, a timeout exception is thrown when the specified wait time expires.
...
The following table describes the general parameters of a Druid connection pool.
...
| minIdle | The minimum number of idle connections retained in the connection pool. When the number of idle connections in the connection pool is less than the specified value, the connection pool will create connections. |
| maxWait | The maximum wait time for requesting a connection, in ms. If you specify a positive value, it indicates the amount of time to wait. An exception is thrown if no connection is obtained when the specified time expires. |
| poolPreparedStatements | Specifies whether to enable the PreparedStatement Cache (PSCache) mechanism. If you set the value to `true`, `PreparedStatement` objects are cached to improve performance. Pay attention that the memory usage of ODP may constantly increase. In such a scenario, you need to properly configure memory parameters and monitor the memory usage to avoid memory leak or overflow. |
...
| filters | A series of predefined filters in the connection pool. These filters can be used to preprocess and postprocess connections based on a specific order to provide extra features and enhance the performance of the connection pool. General filters are described as follows:<ol><li>`stat`: collects statistics about the performance metrics of the connection pool, such as the number of active connections, number of requests, and number of errors. </li><li>`wall`: used in the SQL firewall to block and disable insecure SQL statements, thereby improving the security of the database. </li><li>`log4j`: outputs logs of the connection pool to `log4j` to facilitate log recording and debugging. </li><li>`slf4j`: outputs logs of the connection pool to `slf4j` to facilitate log recording and debugging. </li><li>`config`: loads configuration information of the connection pool from an external configuration file. </li><li>`encoding`: sets the character encoding format used between the connection pool and the database. </li></ol>After you configure these filters in the `filters` parameter, the connection pool applies them based on the specified order. You can separate filter names with commas (,), for example, `filters=stat,wall,log4j`. |
...
2. Create a `Main` class and define a `main` method.
    ...
    Define a `Main` class and a `main` method. The `main` method demonstrates how to use the connection pool to execute a series of operations in the database. To do this, perform the following steps:
...
3. Define a method for obtaining and using configurations in the configuration file.
    ...
    Define a private static method `loadPropertiesFile()` for loading the configuration file and returning a `Properties` object. To do this, perform the following steps:
...
4. Define a method for obtaining a data source object.
    ...
    Define a private static method `createDataSource()` for creating a `DataSource` object based on the configuration file. The object is used to manage and obtain database connections. To do this, perform the following steps:
    ...
    2. Call the `createDataSource()` method of the `DruidDataSourceFactory` class, pass in the `Properties` object as parameters, and return a `DataSource` object.
...
5. Define a method for creating tables.
    ...
    Define a private static method `createTable()` for creating tables in the database. To do this, perform the following steps:
    ...
    2. Call the `createStatement()` method of the `Connection` object `conn` and the `try-with-resources` block to create a `Statement` object named `stmt`.
...
6. Define a method for inserting data.
    ...
    Define a private static method `insertData()` for inserting data into the database. To do this, perform the following steps:
...
7. Define a method for updating data.
    ...
    Define a private static method `updateData()` for updating data in the database. To do this, perform the following steps:
...
8. Define a method for deleting data.
    ...
    Define a private static method `deleteData()` for deleting data from the database. To do this, perform the following steps:
...
9. Define a method for querying data.
    ...
    Define a private static method `selectData()` for querying data from the database. To do this, perform the following steps:
...
10. Define a method for dropping tables.
    ...
    Define a private static method `dropTable()` for dropping a table from the database. To do this, perform the following steps:
