...
In the Oracle mode of OceanBase Database, error codes are in the format of "prefix + number", which is the same as in Oracle Database. If an error originates from an SQL statement, the error code starts with `OBE`. If an error originates from a stored procedure, the error code starts with `PLS`. 
