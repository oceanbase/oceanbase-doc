...
OceanBase Database V2.0 and later are fully compatible with Oracle Database and provide an Oracle-compatible tenant mode. OceanBase Database also strives to maintain consistency with Oracle Database in terms of error codes, SQLSTATE, and error messages. Therefore, according to our product design principles, application error handling can refer to Oracle Database's handling specifications. However, as a distributed database, OceanBase Database has fundamental differences from a standalone Oracle database, and some errors unique to distributed environments cannot be expressed using Oracle Database's error codes. Therefore, these internal error codes are mapped to `OBE-00600`, which indicates that they are unique to OceanBase Database.

In the Oracle-compatible mode of OceanBase Database, the error message format is compatible with Oracle Database and uses the format of "prefix + number". If an error originates from SQL, the error message starts with `ORA`. If an error originates from a stored procedure, the error message starts with `PLS`. For the complete list of error codes in the Oracle-compatible mode of OceanBase Database, see [Overview of error messages](../../../../700.reference/900.error-code/700.error-code-of-oracle-mode/100.use-error-information-of-oracle-mode.md).
...
Drivers in different languages provide different APIs to obtain error information. For more information, see **Drivers** in [Overview of connection methods](../../100.connect-to-oceanbase-database-of-oracle-mode/100.connection-methods-overview-of-oracle-mode.md).
...
Most errors in the Oracle-compatible mode of OceanBase Database do not need to be handled by applications and can be directly returned. For fault tolerance and application robustness, the following four different strategies can be used for error handling based on the scenario and error type:
...
<h4>Notice</h4>
<p>For these <code>OBE-00600</code> error codes, applications must extract the internal error code from the error message. In the following format string, the integer at the placeholder <code>%d</code> is the internal error code.</p>
</main>
...
For general error codes that are compatible with Oracle Database, applications follow the same handling methods as those for Oracle Database. The following table describes common errors in the Oracle-compatible mode of OceanBase Database. Applications can handle these errors based on the strategies suggested in the following table.
...
|-----------|-----------------------------|-------------------------------------------|
| OBE-03113 | Connection to the server is lost. | Reestablish the connection. |
| OBE-01403 | The expected number of rows is greater than the number of rows returned. | Report the error or check whether the application logic is correct. |
| OBE-01405 | The column value is NULL. | Handle the NULL value. |
| OBE-01406 | The returned result exceeds the buffer length. | Increase the buffer length for receiving data. |
