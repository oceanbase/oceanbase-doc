...
|slug|oceanbase-error-codes-intro-oracle-compatible-mode|
...
For compatibility design, different error codes may be returned for the same error in MySQL-compatible mode (the `sys` tenant is in MySQL-compatible mode) and Oracle-compatible mode. For example, the internal error code `-5217` of OceanBase Database indicates that a column name referenced in an SQL statement does not exist. The following table describes the error codes returned for this error in two tenant modes.
...
Error messages provide user-friendly prompt information for humans. However, the messages may change as the database version evolves, and the same error may display different messages in different language settings. Therefore, error handling in applications should generally be based on error codes or SQLSTATE rather than on error messages.
