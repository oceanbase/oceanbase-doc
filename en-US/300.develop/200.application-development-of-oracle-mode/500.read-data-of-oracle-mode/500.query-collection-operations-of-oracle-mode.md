...
* `MINUS` returns rows in the result set of the first `SELECT` statement that are not contained in the result set of the second `SELECT` statement.
    ...
    <h4>Note</h4>
    <p>The Oracle-compatible mode of OceanBase Database supports only the <code>MINUS</code> operator and does not support the <code>EXCEPT</code> operator, though they are semantically equivalent. </p>
    </main>
