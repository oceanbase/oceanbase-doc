...
1. Use the following SQL statement to create a DBLink for connecting to a remote OceanBase database in Oracle-compatible mode. The DBLink name is `my_link`, and the remote tenant is `oracle001`.

   ```sql
   CREATE DATABASE LINK my_link CONNECT TO sys@oracle001 IDENTIFIED BY "******" OB HOST 'xxx.xxx.xxx.xxx:2881';
   ```

2. Use the following SQL statement to query the data of the `test_tbl1` table in the remote database through the DBLink named `my_link`.
...
* For more information about the `SELECT` statement, see [SIMPLE SELECT](../../../700.reference/500.sql-reference/100.sql-syntax/300.common-tenant-of-oracle-mode/900.sql-statement-of-oracle-mode/200.dml-of-oracle-mode/500.select-of-oracle-mode/100.simple-select-of-oracle-mode.md).
