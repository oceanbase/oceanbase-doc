...
The `*` symbol indicates that all fields in the table are returned. Example:
...
In the Oracle-compatible mode of OceanBase Database, you can use the `SELECT * FROM FUNCTION(*);` syntax to call a table-valued function and use the results returned by the function as table data for queries.
...
* The percent sign (`%`) matches zero or multiple characters in the value. The pattern `%` cannot match `NULL`.
  ...
  <main id="notice" type='explain'>
    <h4>Note</h4>
    <p>In the Oracle-compatible mode of OceanBase Database, the <code>LIKE</code> operator is replaced with <code>=</code> when the following conditions are met:</p>
    <p>
     <ul>
      ...
      <li>The column type is not <code>LOB</code>. In Oracle-compatible mode, the <code>LOB</code> type does not support <code>=</code> comparison, so the conversion to <code>=</code> is not supported. </li>
...
Use the `GROUP BY CUBE` clause to group records in the `fruit_order` table by `user_id` and summarize the grouping results.
...
For more information about grouping summary, see [SIMPLE SELECT](../../../700.reference/500.sql-reference/100.sql-syntax/300.common-tenant-of-oracle-mode/900.sql-statement-of-oracle-mode/200.dml-of-oracle-mode/500.select-of-oracle-mode/100.simple-select-of-oracle-mode.md).
