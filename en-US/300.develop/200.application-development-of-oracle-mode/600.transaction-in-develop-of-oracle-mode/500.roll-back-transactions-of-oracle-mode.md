...
|slug|roll-back-transaction-oracle-compatible-mode|
...
In this example, a transaction is explicitly started with `BEGIN`. Before the transaction is explicitly committed with `COMMIT`, all modifications are visible only to the current session and are not persisted. You can use the `ROLLBACK` statement to roll back the modifications.
...
* Savepoints after the specific savepoint (excluding the savepoint itself) are cleared.
...
For detailed examples and descriptions of rolling back a transaction to a savepoint, see [Roll back a transaction to a savepoint](300.transaction-savepoints-of-oracle-mode/200.rollback-to-a-savepoint-of-oracle-mode.md).
