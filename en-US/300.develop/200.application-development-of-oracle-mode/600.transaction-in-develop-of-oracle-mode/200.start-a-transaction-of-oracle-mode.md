...
The transaction control statements in Oracle mode of OceanBase Database are compatible with those in Oracle. In Oracle mode of OceanBase Database, you can start a transaction in the following ways:
...
* Execute the `INSERT`, `UPDATE`, `DELETE`, or `SELECT ... FOR UPDATE` statement to start a new transaction when `autocommit` is set to `0` to disable the autocommit mode.
