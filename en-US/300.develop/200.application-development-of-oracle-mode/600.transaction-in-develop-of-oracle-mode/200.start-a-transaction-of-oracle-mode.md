...
This topic describes commonly used statements and examples for starting a transaction in OceanBase Database.

## Start a transaction

The transaction control statements in the Oracle-compatible mode of OceanBase Database are compatible with those in Oracle Database. In the Oracle-compatible mode of OceanBase Database, you can start a transaction in the following ways:
...
* Execute the `START TRANSACTION` statement.
  ...
  <main id="notice" type='notice'>
    <h4>Notice</h4>
    <p><code>BEGIN</code> serves as an alias for <code>START TRANSACTION</code> and is used to start a transaction.</p>
  </main>
...
Query the participant information of active transactions in the `V$OB_TRANSACTION_PARTICIPANTS` view.
