...
* `FORMAT = ( TYPE = 'CSV'... )` is used to specify the format of the external file as CSV. The following options are available:
...
* `FORMAT = ( TYPE = 'PARQUET'... )` is used to specify the format of the external file as `PARQUET`.
* `FORMAT = ( TYPE = 'ORC'... )` is used to specify the format of the external file as `ORC`.
...
1. Create an external file.
   ...
   Create a file named `test_tbl1.csv` in the `/home/admin/external_csv` directory of the server where the OBServer node you want to log in to resides.
