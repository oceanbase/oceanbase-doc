...
* `FORMAT = ( TYPE = 'CSV'... )` specifies the CSV format for external files. The parameters are as follows:
  ...
  * `COMPRESSION`: optional. Specifies the compression format for the file. Valid values:
    ...
    * `SNAPPY_BLOCK`: HADOOP-SNAPPY-compressed files.
      ...
      <main id="notice" type='explain'>
        <h4>Note</h4>
        <p>For V4.4.2, support for the <code>SNAPPY_BLOCK</code> compression format was introduced starting with V4.4.2 BP1. </p>
      </main>
  ...
  * `MAX_ROW_LENGTH`: specifies the maximum length of a single row of data. The unit is bytes. The default value is 2 MB. This parameter is used for the boundary determination stage of parallel parsing of a single CSV file and does not affect the accuracy of data reading.
  ...
  <main id="notice" type='explain'>
    <h4>Note</h4>
    <p>For V4.4.2, the <code>FORMAT</code> option supports the <code>PARALLEL_PARSE_ON_SINGLE_FILE</code>, <code>PARALLEL_PARSE_FILE_SIZE_THRESHOLD</code>, and <code>MAX_ROW_LENGTH</code> parameters starting with V4.4.2 BP1. </p>
  </main>
...
The following example describes how to create an external table when the external files are stored locally in the Oracle-compatible mode of OceanBase Database. The steps are as follows:
