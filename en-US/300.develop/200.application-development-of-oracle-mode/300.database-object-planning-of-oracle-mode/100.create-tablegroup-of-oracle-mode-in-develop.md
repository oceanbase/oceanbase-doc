...
This topic describes how to create a table group by using an SQL statement, including the prerequisites, overview, limitations, and recommendations for creating table groups, with some examples.
...
## Limitations for creating a table group
...
* The table group name cannot use reserved keywords in OceanBase Database. For more information about the reserved keywords in OceanBase Database's Oracle-compatible mode, see [Reserved keywords](../../../700.reference/500.sql-reference/900.reserved-keywords-of-oracle-mode.md).

## Recommendations for creating a table group
...
## Create a table group by using the CLI
...
5. Query tables in the `test_tbl_tg1` table group.
    ...
    <h4>Notice</h4>
    <p>In the Oracle-compatible mode of OceanBase Database, when you query table groups using the <code>sys.DBA_OB_TABLEGROUP_TABLES</code> view, write the English letters in the table group name in uppercase.</p>
    </main>
