...
* Unique indexes ensure no duplicate values between two rows in an indexed column. In OceanBase Database, NULL values are also stored in indexes.
...
* A global index has its independent partitioning rules, and an index value in the index table may point to multiple partitions in the primary table.
...
You can use the [CREATE INDEX](../../../700.reference/500.sql-reference/100.sql-syntax/300.common-tenant-of-oracle-mode/900.sql-statement-of-oracle-mode/100.ddl-of-oracle-mode/1600.create-index-of-oracle-mode.md) statement to create a non-unique index on an existing table based on the following requirements.
...
* If you do not specify the `LOCAL` or `GLOBAL` attribute for an index, OceanBase Database in Oracle mode defines this index as a global index by default, and the index table has only one partition.
...
Create a HASH-partitioned table named `tbl1_h`, and then create a local index named `tbl1_h_idx1` on the table.
...
Create a RANGE-LIST-subpartitioned table named `tbl2_f_rl` without using a template, and then create a local unique index named `tbl2_f_rl_idx1` on the table.
...
Create a HASH-partitioned table named `tbl1_h`, and then create a RANGE-partitioned global index named `tbl1_h_idx1` on the table.
...
Create a RANGE-LIST-subpartitioned table named `tbl2_f_rl` without using a template, and then create a global unique index named `tbl2_f_rl_idx1` on the table.
