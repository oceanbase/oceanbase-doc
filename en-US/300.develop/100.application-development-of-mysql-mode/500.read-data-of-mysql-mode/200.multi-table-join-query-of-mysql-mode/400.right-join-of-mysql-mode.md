...
A `RIGHT JOIN` query is a `RIGHT OUTER JOIN` query that returns all rows in the right-side table and the matching rows in the left-side table.
...
The syntax of a `RIGHT JOIN` query is as follows:
...
Example 1: Execute a `RIGHT JOIN` query on the `tbl_a` and `tbl_b` tables and obtain the returned result.
