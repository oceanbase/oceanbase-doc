...
The syntax of a `LEFT JOIN` query is as follows:
...
Create tables and insert proper data into the tables.
...
Example 1: Execute a `LEFT JOIN` query on the `tbl_a` and `tbl_b` tables and obtain the returned result.
