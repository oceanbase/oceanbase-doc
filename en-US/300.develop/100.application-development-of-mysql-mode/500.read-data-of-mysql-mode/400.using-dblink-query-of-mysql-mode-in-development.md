...
|slug|use-dblink-queries-mysql-mode|
...
* Currently, the DBLink read feature in the MySQL-compatible mode of OceanBase Database only supports reading data between different MySQL-compatible tenants.
* In MySQL-compatible mode, you can access data of the following types through DBLinks:
...
* `table_name@dblink_name` specifies the table to be queried in the remote database. Separate multiple tables with commas (,). By default, the tables in the database specified when you created the DBLink are accessed. If you want to access a table in another database, you can specify the table in the SQL statement in the format of `database_name.table_name@dblink_name`.
...
1. Create a DBLink for connecting to a remote OceanBase database in MySQL-compatible mode. The DBLink name is `my_link`, the remote tenant is `mysql001`, and the default database to be accessed is `test`.
