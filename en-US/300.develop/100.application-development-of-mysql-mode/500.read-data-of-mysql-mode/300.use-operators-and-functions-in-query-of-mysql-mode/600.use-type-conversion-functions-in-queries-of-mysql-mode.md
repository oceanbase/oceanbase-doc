...
|slug|use-type-conversion-functions-in-queries-of-mysql-mode|
...
In the MySQL mode of OceanBase Database, you can directly copy time strings to records of the `DATE` type. OceanBase Database can automatically convert the strings to the `DATE` type. You can also use the `CONVERT()` or `CAST()` function to convert the time strings to the `DATE` type. Here is an example:
