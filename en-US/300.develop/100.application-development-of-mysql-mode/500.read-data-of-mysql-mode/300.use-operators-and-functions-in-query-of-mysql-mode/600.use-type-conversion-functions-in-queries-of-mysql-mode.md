...
|slug|use-type-conversion-functions-in-queries-mysql-compatible-mode|
...
In the MySQL-compatible mode of OceanBase Database, you can directly assign time strings to records of the `DATE` type. OceanBase Database can automatically convert the strings to the `DATE` type. You can also use the `CONVERT()` or `CAST()` function to convert the time strings to the `DATE` type. Here is an example:
...
In a MySQL-compatible tenant of OceanBase Database, you can use the `CONVERT()` or `CAST()` function to implement conversion between numbers and strings.
