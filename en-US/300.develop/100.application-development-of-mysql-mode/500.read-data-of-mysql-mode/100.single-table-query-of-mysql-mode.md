...
* You have connected to a MySQL-compatible tenant of OceanBase Database.
...
Example 22: Display the student information in the `student` table sorted by `DAY(enrollment_date)` in descending order and `score` in ascending order.
...
The syntax for limiting rows with the `LIMIT` clause is as follows (Format 1):

```sql
LIMIT [offset,] row_count
```

The syntax for limiting rows with the `LIMIT` clause is as follows (Format 2):
...
* For more information about comparison operators, see [Comparison operators](../../../700.reference/500.sql-reference/100.sql-syntax/200.common-tenant-of-mysql-mode/200.operator-of-mysql-mode/500.comparison-operators-of-mysql-mode.md).
...
* For more information about query statement optimization, see [Rule-based query rewrite](../../../700.reference/1000.performance-tuning-guide/500.sql-optimization/400.sql-optimization/500.query-rewrite/200.rule-based-query-rewriting.md).
