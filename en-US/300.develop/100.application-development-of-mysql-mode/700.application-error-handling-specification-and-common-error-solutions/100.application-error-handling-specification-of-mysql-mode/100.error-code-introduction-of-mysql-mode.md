...
Whether you use a Java or C driver library to connect to OceanBase Database, when any SQL statement fails to execute, you will receive an error indicating the failure. An error usually consists of the following three parts:
...
Here, `1054` is the error code, `42S22` in the parentheses is the SQLSTATE value, and the text after the colon is the detailed error message.

For compatibility, different error codes may be returned for the same error in MySQL-compatible mode (the `sys` tenant is in MySQL-compatible mode) and Oracle-compatible mode. For example, the internal error code `-5217` of OceanBase Database indicates that a column name referenced in an SQL statement does not exist. The following table describes the error codes returned for this error in two tenant modes.
...
SQLSTATE is an error code specified by the SQL standard. It is a string that contains five characters that can be digits or uppercase Latin letters. The first two characters indicate the general error category, and the last three characters indicate the specific error cause. For example, `00` indicates success, `01` indicates a warning, `02` indicates that data is not found, `22` indicates a data error, and `42` indicates a syntax error. Therefore, for the same error, OceanBase Database returns the same SQLSTATE value in MySQL-compatible mode and Oracle-compatible mode.
...
Error messages provide human-friendly prompt information and are intended for human readers only. However, error messages may change as the database version evolves, and the same error may display different messages in different language settings. Therefore, application error handling should be based on error codes or SQLSTATE rather than error messages.
