...
This topic describes the composition and meanings of error codes in OceanBase Database.
...
Here, `1054` is the error code, `42S22` in parentheses is the SQLSTATE value, and the colon is followed by the specific error message.
...
You can use the error code parsing tool `ob_error 10` to query the meanings of error codes and the mappings between internal and external error codes. For more information about the `ob_error` tool, see [ob_error](../../../../700.reference/1500.Components-and-Tools/100.manage/300.ob-error.md).
...
Although the SQL standard specifies SQLSTATE, the development of the standard always lags behind the practice of database vendors. Database vendors usually need to define their own error codes (vendor codes) to identify more detailed error causes. In addition, error codes defined by a vendor are not compatible with databases provided by other vendors.
