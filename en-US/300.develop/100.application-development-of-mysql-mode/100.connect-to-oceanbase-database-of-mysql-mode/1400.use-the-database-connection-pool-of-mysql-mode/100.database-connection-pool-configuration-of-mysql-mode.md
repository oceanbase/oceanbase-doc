...
|----------------|-----------------------------------------------------|
| socketTimeout  | The network socket timeout period, in ms. The value `0` specifies to disable socket timeout. You can also set the system variable `max_statement_time` to limit the query time. The default value is `0` (in standard configuration). |
| connectTimeout | The connection timeout period, in ms. The value `0` specifies to disable connection timeout. The default value is `30000`. |
