...
* `socketTimeout`: the time that the socket waits for the response to an SQL statement when the SQL statement is executed.
...
* `prepStmtCacheSQLLimit`: the maximum SQL length allowed for the PS cache. SQL statements with a length exceeding the limit cannot be placed into the PS cache. prepStmtCacheSize: the maximum number of SQL statements that can be stored in the PS cache.
