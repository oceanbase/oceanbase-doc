...
You can use the mysql client to connect to a MySQL-compatible tenant of OceanBase Database. This topic will guide you through the necessary steps and provide an example to help you get started.
...
* `-u` specifies the tenant account for connecting to OceanBase Database. The format of `$user_name` is `username@tenant name`. The default username of the administrator of a MySQL-compatible tenant is `root`.
...
2. Run the following command to connect to a MySQL-compatible tenant of OceanBase Database by using the mysql client:
