...
This topic describes how to build an application by using the Spring Data JDBC framework, MySQL Connector/J, and OceanBase Database. It also covers the use of the application for fundamental database operations, including table creation, data insertion, and data query.
...
3. Define the database connection information.
   ...
   * `$user_name` is specified by the `-u` parameter, formatted as **username@tenant#cluster_name** or **username@SERVICE:service_name**. When using the format **username@tenant#cluster_name**, the default tenant is `sys` and the administrator user is `root`. The cluster name can be omitted when connecting directly to the database, but must be provided for ODP connections.
