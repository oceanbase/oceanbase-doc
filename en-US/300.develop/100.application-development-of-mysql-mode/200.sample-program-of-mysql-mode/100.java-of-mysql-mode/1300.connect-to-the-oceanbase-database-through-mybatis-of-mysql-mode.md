...
This topic describes how to build an application by using the MyBatis framework and OceanBase Database. The application can perform basic operations such as creating tables, inserting data, and querying data.
...
<main id="notice" type='explain'>
  <h4>Note</h4>
  <p>The code in this topic is run by using IntelliJ IDEA 2021.3.2 (Community Edition). You can also run the sample code by using a tool of your choice. </p>
</main>
...
2. Fill in the corresponding information in the URL below based on the deployed OceanBase Database.
    ...
    * `user_name`: specified by the `-u` parameter. The format is **user@tenant#cluster name** or **username@SERVICE:service name**. When the format **user@tenant#cluster name** is used, the default tenant is `sys` and the administrator user is `root`. When directly connecting to the database, the cluster name can be omitted. When connecting through ODP, the cluster name must be specified.
...
3. IntelliJ IDEA automatically identifies various files in the project and displays the project structure, file list, module list, and dependency relationships in the **Project** tool window. The **Project** tool window is usually located on the left side of the IntelliJ IDEA interface and is open by default. If the **Project** tool window is closed, you can click **View > Tool Windows > Project** in the menu bar or press the shortcut key **Alt + 1** to reopen it.

    <main id="notice" type='explain'>
    <h4>Note</h4>
    <p>When you import a project using IntelliJ IDEA, it automatically detects the pom.xml file in the project, downloads the required dependency libraries based on the described dependencies in the file, and adds them to the project. </p>
    </main>
...
* The tenant connection account is `root@mysql001`. `mysql001` is a MySQL user tenant created in OceanBase Database, and `root` is the username of the `mysql001` tenant.
* The password is `******`.
...
2. In the tool menu bar, select **Run(U) > Run > TestMybatis**, or click the green triangle in the upper right corner to run it.
...
### 4. Special characters in account or password
...
<main id="notice" type='notice'>
  <h4>Notice</h4>
  <p>When using MySQL Connector/J 8.x, ensure that the account password does not contain a #. Otherwise, you may encounter a connection error.</p>
</main>
...
* `TestMybatis.java`: A Java file for storing test code for MyBatis.
...
<main id="notice" type='explain'>
  <h4>Note</h4>
  <p>If you want to verify the example, you can use the default code without any modifications. You can also modify the <code>pom.xml</code> file as needed based on the following instructions. </p>
</main>
...
1. File declaration statement.
    ...
    Declares that the file is an XML file using XML version `1.0` and character encoding `UTF-8`.
...
3. Configure basic information.
   ...
   1. Use `<groupId>` to specify the project identifier as `com.oceanbase.example`.
   2. Use `<artifactId>` to specify the project dependency as `java-oceanbase-mybatis`.
...
4. Use `<build>` to define the project build process.
   ...
   6. Use `<source>` to specify the compiler source code version as 8.
   7. Use `<target>` to specify the compiler source code version as 8.
...
5. Use `<dependencies>` to define the components that the project depends on.
   ...
   1. Specify the organization of the dependency item as `com.oceanbase`, the name as `oceanbase-client`, and the version as `2.4.2`.
   ...
   2. Specify the test architecture of the dependency item as `junit`, the name as `junit`, and the version as `4.10`.
   ...
   3. Specify the architecture of the dependency item as `org.mybatis`, the name as `mybatis`, and the version as `3.5.9`.
   ...
   4. Specify the plugin of the dependency item as `com.github.pagehelper`, the name as `pagehelper`, and the version as `5.3.0`.
...
`jdbc.properties` is a property file that stores configuration information related to database connections, including the database URL, username, and password. The code contains the following content:
...
1. File declaration.
    ...
    This declaration indicates that the file is an XML file and specifies that the current document is a MyBatis configuration file, using XML version `1.0` and character encoding `UTF-8`. The version number is 3.0, the language is English, and the DTD (Document Type Definition) file provided by the official MyBatis website is used for validation.
    ...
    * `mybatis.org`: represents the official website of MyBatis.
    * `DTD Config 3.0`: represents the version number of the MyBatis configuration file.
    * `EN`: represents the language of the document type as English.
...
2. Specify the path and filename of the property file to be loaded.
...
6. Configure the mapper of MyBatis.
    ...
    * resource: used to specify the path and filename of the XML configuration file of the mapper.
...
<main id="notice" type='explain'>
  <h4>Note</h4>
  <p>If you only want to verify the example, use the default code without any modifications. If you need to make changes, adjust them according to your specific requirements. </p>
</main>
...
The `IAppMapper.java` file defines SQL mapping relationships.
...
2. Define the `IAppMapper` interface.
   ...
   * `@Results` method: Specifies the mapping of query results, mapping fields in the query results to properties of Java objects.
   **Database operations include:**
   1. Insert data
    Map the App object to a record in the `test_app` table. Use `#{property name}` to represent the property values of the App object. The `Integer` return value indicates the automatically generated ID returned after the SQL statement inserts data, which corresponds to the ID field in the `test_app` table.
   ...
   3. Update data
    Perform an update operation on the records in the `test_app` table.
...
The `App.java` file is used to represent the App application object. It includes two properties, `id` and `name`. The class also defines access methods for the properties, a constructor, and a `toString` method for converting the object to a string.

The `App.java` file contains the following code:
...
2. Define the `App` class.
   ...
   3. Define `getId`, `setId`, `getName`, and `setName` methods to get and set the application name. The `setName` method returns the name of the application.
...
The `User.java` file is used to represent the user object. It includes two properties, `id` and `name`. The class also defines access methods for the properties, a constructor, and a `toString` method for converting the object to a string.
The creation of this class is similar to the creation of the `App.java` file above.
...
The TestMyBatis.java file is used to demonstrate how to perform data operations using MyBatis. It tests the basic functionality of the MyBatis framework, the execution of SQL statements, the invocation of Mapper interfaces, and the parameters and return values of SQL statements.
...
1. Import other classes and interfaces.
    ...
    * `PageHelper` plugin: implements pagination query functionality.
    ...
    * `IOException` class: represents errors during input/output operations.
...
2. Define the `testUserMapper` method.
    ...
    3. Use the `getConnection()` method of the `SqlSession` instance to obtain a `Connection` instance. This instance can perform various database operations.
    ...
    6. Use the `execute()` method of the `Statement` object to create the `test_user` table. This table contains two fields: `id` (type `number(20)`) as the primary key, and `name` (type `varchar2(100)`).
...
3. Define the `testSqlSession` method.
    ...
    8. Use the `delete()` method of the `sqlSession` object to execute the delete operation. Set the deletion condition by passing in the parameter 1L. Store the result of the delete operation in the `deleteResult` variable.
    9. Use the `sqlSession` object to execute a database update operation. Create a User object, call the `update` method, and pass in the SQL statement identifier and parameter object to complete the update operation. The specific SQL statements and parameter mapping relationships can be found in the XML configuration file of the "com.oceanbase.mapper.IUserMapper" interface. Store the result of the update operation in the `updateResult` variable.
    10. Use the `openSession()` method of the `SqlSessionFactory` instance to create a `SqlSession` instance. Execute a query operation through this instance and store the query result in the `userList` variable. Finally, use the `forEach` method to traverse the `userList` and output it to the console.
    11. Close the `Statement` object in the database connection using `statement.close()`. Then, use `sqlSession.commit()` to commit the transaction and persist all modifications to the database. Finally, use `sqlSession.close()` to close the `SqlSession` object, release the related resources, and close the connection to the database.
...
4. Define the `testAppMapper` method.
    ...
    9. Use the `delete()` method of the `mapper` object to execute the delete operation. Set the deletion condition by passing in the parameter `1L`. Store the result of the delete operation in the `deleteResult` variable.
    10. Use the `mapper` object to execute a database update operation. Create an App object, call the `update` method, and pass in the SQL statement identifier and parameter object to complete the update operation.
    11. Create an `App` object named `updateApp` with `id` set to `2L` and `name` set to "update".
    12. Call the `updateApp` method of the `mapper` object and pass in the `updateApp` object to execute the update operation.
    ...
    15. Call the `startPage` method of the `PageHelper` object and pass in the page number and number of records per page to set the pagination parameters. Query all `App` objects and return a `List` object containing all `App` objects.
    16. Call the `getList` method of the `PageInfo` object to obtain the list of App objects after pagination. Use the `System.out.println` method to print the list of App objects after pagination.
    17. Close the `SqlSession` object using `sqlSession.close()` to release the related resources.
