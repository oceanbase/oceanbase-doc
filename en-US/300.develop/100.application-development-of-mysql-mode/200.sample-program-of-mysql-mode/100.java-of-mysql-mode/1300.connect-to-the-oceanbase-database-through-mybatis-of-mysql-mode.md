...
<main id="notice" type='explain'>
  <h4>Note</h4>
  <p>The code examples in this topic are run in IntelliJ IDEA 2021.3.2 (Community Edition). You can also use your preferred tool to run the code examples. </p>
</main>
...
<main id="notice" type='explain'>
  <h4>Note</h4>
  <p>The steps described in this topic are based on the Windows environment. If you are using a different operating system or compiler, the steps may vary slightly. </p>
</main>
...
2. Fill in the following URL with the information of the deployed OceanBase Database.
    ...
    * `host`: the IP address for connecting to OceanBase Database. If you connect to OceanBase Database through an ODP, the value is the IP address of the ODP. If you connect to OceanBase Database directly, the value is the IP address of an OBServer node.
    * `port`: the port for connecting to OceanBase Database. If you connect to OceanBase Database through an ODP, the value is `2883` by default, which can be customized when you deploy an ODP. If you connect to OceanBase Database directly, the value is `2881` by default, which can be customized when you deploy OceanBase Database.
    * `schema_name`: the name of the schema to be accessed.
    * `user_name`: the username in the format of **username@tenant#cluster name** or **username@SERVICE:service name**. If you use the format **username@tenant#cluster name**, the default tenant is `sys`, and the administrator user is `root`. The cluster name is required when you connect to OceanBase Database through an ODP, but can be omitted when you connect to OceanBase Database directly.
...
3. IntelliJ IDEA automatically identifies various files in the project and displays the project structure, file list, module list, and dependency relationships in the **Project** tool window. The **Project** tool window is usually located on the left side of the IntelliJ IDEA interface and is open by default. If it is closed, you can click **View > Tool Windows > Project** in the menu bar or press the shortcut key **Alt + 1** to reopen it.
    ...
    <main id="notice" type='explain'>
    <h4>Note</h4>
    <p>When you import a project into IntelliJ IDEA, it automatically detects the pom.xml file in the project, downloads the required dependency libraries based on the described dependencies in the file, and adds them to the project.</p>
    </main>
...
* The URL extra connection attributes are set to `useServerPrepStmts=true&rewriteBatchedStatements=true`.
* The tenant connection account is `root@mysql001`. `mysql001` is a MySQL tenant created in OceanBase Database, and `root` is the username of the `mysql001` tenant.
* The password is `******`.
...
<main id="notice" type='notice'>
  <h4>Notice</h4>
  <p>When using MySQL Connector/J 8.x, ensure that the username and password do not contain the # character. Otherwise, you may encounter a connection error.</p>
</main>

## Project code

Click [java-oceanbase-mybatis](https://obbusiness-private.oss-cn-shanghai.aliyuncs.com/doc/img/observer-enterprise/V4.2.0/3.develop/demo/java/mybatis/java-oceanbase-mybatis.zip) to download the project code, which is a compressed package named `java-oceanbase-mybatis`.
...
* `pom.xml`: the Maven project configuration file, which contains information about the project's dependencies, plugins, and build process.
...
<main id="notice" type='explain'>
  <h4>Note</h4>
  <p>If you only want to verify the example, you can use the default code without any modifications. Alternatively, you can modify the <code>pom.xml</code> file as needed based on the following explanations.</p>
</main>
...
5. Use `<dependencies>` to define the components on which the project depends.
   ...
   1. Specify the organization, name, and version of the dependency as `com.oceanbase`, `oceanbase-client`, and `2.4.2`, respectively.
   ...
   2. Specify the test framework, name, and version of the dependency as `junit`, `junit`, and `4.10`, respectively.
   ...
   3. Specify the framework, name, and version of the dependency as `org.mybatis`, `mybatis`, and `3.5.9`, respectively.
   ...
   4. Specify the plugin, name, and version of the dependency as `com.github.pagehelper`, `pagehelper`, and `5.3.0`, respectively.
...
The `jdbc.properties` file is an attribute file used to store configuration information related to database connections, including the database URL, username, and password. The code contains the following content:

<main id="notice" type='explain'>
  <h4>Note</h4>
  <p>The values of the fields in the code are directly obtained from <b>Step 1: Obtain the OceanBase database connection string</b>.</p>
</main>
...
1. File declaration.
    ...
    This declaration specifies that the file is an XML file. It indicates that the current document is a MyBatis configuration file, using XML version `1.0` and character encoding `UTF-8`. The version is 3.0, the language is English, and the DTD (Document Type Definition) file provided by the MyBatis official website is used for validation.
    ...
    * `DTD Config 3.0`: indicates the version of the MyBatis configuration file.
    * `EN`: indicates the language of the document type as English.
...
3. Configure global settings of MyBatis.
    ...
    * `jdbcTypeForNull`: used to configure the JDBC type to be used when handling null values, set to `NULL`.
    ...
    <main id="notice" type='explain'>
    <h4>Note</h4>
    <p>The subelements of the settings element are optional and can be added or removed as needed.</p>
    </main>
...
4. Configure plugins of MyBatis.
    ...
    * `params`: indicates the mapping relationship between parameter names and parameter values for passing pagination parameters.
...
5. Configure the environment of MyBatis.
    ...
    * `property`: specifies the class name of the database driver.
    ...
    <main id="notice" type='explain'>
     <h4>Note</h4>
     <p>In other configuration elements of MyBatis, you can use <code>${}</code> placeholders to reference the data source, transaction manager, and executor configured in the environment.</p>
    </main>
...
6. Configure the mapper of MyBatis.
    ...
    * class: used to specify the fully qualified name of the Java class of the mapper.
    ...
    <main id="notice" type='explain'>
     <h4>Note</h4>
     <p>If you use an XML configuration file, you need to specify the path and file name of the XML file in the mapper element. If you use a Java interface, you need to specify the fully qualified name of the Java class in the mapper element.</p>
    </main>
...
<main id="notice" type='explain'>
  <h4>Note</h4>
  <p>If you only want to verify the example, use the default code without any modifications. If you need to make changes, adjust them based on your specific requirements. </p>
</main>
...
2. Configure the mapping relationship with the `IUserMapper.java` file.
    ...
    1. Define the namespace for the `Mapper` interface to correspond with the `Mapper` interface in the Java code.
    ...
    4. Create an SQL statement named `updateUser` to update a record in the `test_user` table, changing the value of the `name` field to `#{name}` based on the value of the `id` field, which is `#{id}`.
    5. Create an SQL statement named `selectUsers` to query all user records from the `test_user` table.
    6. Create an SQL statement named `selectUserByPage` to query user objects from the `test_user` table for a specified page. This involves subqueries and the `rownum` field for pagination, where `#{pageNum}` and `#{pageSize}` represent the current page number and the number of records per page, respectively.
...
1. Define the `Mapper` package.
    ...
    Declare the package name of the current file as `com.oceanbase.mapper`. The `Mapper` package includes the following interfaces and classes:
...
2. Define the `IAppMapper` interface.
   ...
   * `@Results` method: Represents the mapping relationship of query results, used to map fields from the query result to properties of a Java object.
   **Database operations include:**
   1. Insert data
    Map the App object to a record in the `test_app` table. Use `#{attributeName}` to represent the property values of the App object. The `Integer` return value indicates the automatically generated ID returned after the SQL statement inserts data, which corresponds to the ID field in the `test_app` table.
   ...
   3. Update data
    Update records in the `test_app` table using the Update operation.
...
2. Define the `IUserMapper` interface.
   ...
   Define an interface named `IUserMapper` that includes methods for inserting, deleting, updating, and querying user data, as well as methods for paginated queries. Use `JavaDoc` comments and the `@Param` annotation to enhance code readability and maintainability.
   **Sample code:**
...
The `App.java` file contains the following main sections:
...
2. Define the `App` class.
   ...
   3. Define `getId`, `setId`, `getName`, and `setName` methods for retrieving and setting the application name, and returning the set application name through the `setName` method.
   4. Override the `toString` method to convert the application object to a string representation, facilitating output and debugging.
...
The `User.java` file is used to represent the user object. It includes two properties: `id` and `name`. The class also defines access methods for the properties, a constructor method, and a `toString` method for converting the object to a string.
The creation of this class is similar to that of the `App.java` file above.
...
The TestMyBatis.java file demonstrates how to use MyBatis for data operations. It tests the basic features of the MyBatis framework, the execution of SQL statements, and the calling methods of Mapper interfaces, including parameters and return values of SQL statements.
...
1. Import other classes and interfaces.
    ...
    * `App` class: An App object for testing the execution of SQL statements.
    * `User` class: A User object for testing the execution of SQL statements.
    ...
    * `org.junit.Test`: An annotation from the JUnit testing framework, used to mark test methods.
...
2. Define the `testUserMapper` method.
    ...
    3. Use the `getConnection()` method of the `SqlSession` instance to obtain a `Connection` instance. It can perform various database operations.
    ...
    6. Use the `execute()` method of the `Statement` object to create a `test_user` table. The table contains two fields: `id` of type `number(20)` as the primary key, and `name` of type `varchar2(100)`.
    ...
    9. Delete, update, and query data in the `test_user` table, and use the `forEach()` method to print the information of each user in the user list.
    10. Create a user named `insert`, use the `selectUserByPage()` method to query all user data with the name "insert" in the `test_user` table, and return the user list for the specified page. Query the second page, with 3 records per page. Print these user data to the console. Close the `Statement` and `SqlSession` instances, commit the transaction, and release resources.
...
3. Define the `testSqlSession` method.
    ...
    4. Use the `createStatement()` method of the `Connection` instance to create a `Statement` instance. Execute SQL statements in sequence.
    5. Use the `execute()` method of the `Statement` object to execute an SQL statement to delete the `test_user` table.
    6. Use the `execute()` method of the `Statement` object to create a `test_user` table. This table contains two fields: `id` of type `number(20)` as the primary key, and `name` of type `varchar2(100)`.
    7. Use a `for` loop to insert 10 records into the `test_user` table. In each iteration, create a new User object, use the `insertUser()` method of the `Mapper` interface to execute the insert operation, and store the result in the `insertResult` variable.
    8. Use the `delete()` method of the `sqlSession` object to execute a delete operation. Set the deletion condition using the passed parameter 1L. Store the result of the delete operation in the `deleteResult` variable.
    9. Use the `sqlSession` object to execute a database update operation. Create a User object, call the `update` method, and pass the SQL statement identifier and parameter object to complete the update operation. The specific SQL statements and parameter mappings can be found in the XML configuration file of the "com.oceanbase.mapper.IUserMapper" interface. Store the result of the update operation in the `updateResult` variable.
    10. Use the `openSession()` method of the `SqlSessionFactory` instance to create a `SqlSession` instance. Execute a query operation using this instance and store the query results in the `userList` variable. Finally, use the `forEach` method to iterate through `userList` and output the results to the console.
    11. Close the `Statement` object in the database connection using `statement.close()`. Then, commit the transaction using `sqlSession.commit()` to persist all modifications to the database. Finally, close the `SqlSession` object using `sqlSession.close()` to release related resources and close the connection to the database.
...
4. Define the `testAppMapper` method.
    ...
    6. Use the `execute()` method of the `Statement` object to create a `test_app` table. This table contains two fields: `id` of type `number(20)` as the primary key, and `name` of type `varchar2(100)`.
    ...
    8. Use a `for` loop to insert 10 records into the `test_app` table. In each iteration, create a new App object, use the `insertApp()` method of the `Mapper` interface to execute the insert operation, and store the result in the `insertResult` variable.
    9. Use the `delete()` method of the `mapper` object to execute a delete operation. Set the deletion condition using the passed parameter `1L`. Store the result of the delete operation in the `deleteResult` variable.
    ...
    16. Call the `getList` method of the `PageInfo` object to obtain the paginated App object list and use the `System.out.println` method to print the paginated `App` object list.
    17. Close the `SqlSession` object using `sqlSession.close()` to release related resources.
