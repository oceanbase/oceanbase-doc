...
* Run the `AddPeopleWriterTest.java` file.
    ...
    1. Find the `AddPeopleWriterTest.java` file under **src** > **test** > **java** in the project package.
...
If you encounter connection timeout issues, you can configure connection timeout parameters in the JDBC URL:
...
To ensure correct character encoding, set the correct character encoding parameters in the JDBC URL:
...
To enable SSL connection with OceanBase Database, add the following parameters to the JDBC URL:
...
If the username or password contains special characters (such as #), you need to perform URL encoding:
...
<main id="notice" type='notice'>
  <h4>Note</h4>
  <p>When using MySQL Connector/J 8.x, ensure that the username and password do not contain the hash symbol (#). Otherwise, you may encounter connection errors.</p>
</main>

## Project code

Click [java-oceanbase-springbatch](https://obbusiness-private.oss-cn-shanghai.aliyuncs.com/doc/img/observer-enterprise/V4.2.0/3.develop/demo/java/java-oceanbase-springbatch/java-oceanbase-springbatch.zip) to download the project code, which is a package named `java-oceanbase-springbatch`.
...
3. Configure Spring Batch.
    ...
    `spring.batch.job.enabled`: specifies whether to enable Spring Batch jobs. The value `false` here indicates that Spring Batch jobs are disabled.
...
The `AddPeopleWriterTest.java` file is a class that uses JUnit for testing and is used to test the write logic of `AddPeopleWriter`.
