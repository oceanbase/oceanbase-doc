...
|slug|connect-to-the-oceanbase-database-through-spring-batch-of-mysql-mode|
...
* OceanBase Database V3.x (MySQL mode or Oracle mode)
...
```xml
  ...
  <!--Other Spring Boot dependencies are omitted here.-->
```
...
Call the database write class method to write data. The specific steps are as follows:
...
1. Create required classes.
   ...
   Batch processing class: `AddPeopleDescProcessor`
   ...
   Class that writes PeopleDESC to the database: `AddDescPeopleWriter`

   ```java
   // Class that writes PeopleDESC to the database.
...
2. Start the `spring-data-jpa` project. Tables are automatically created based on the entity classes `People` and `PeopleDESC`. The sample result is as follows:
...
3. Add test data to `people` in batches.
...
4. Execute the batch processing method.
