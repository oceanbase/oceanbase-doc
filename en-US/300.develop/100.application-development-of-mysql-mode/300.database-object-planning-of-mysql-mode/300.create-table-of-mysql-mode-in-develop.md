...
* Select a proper data type for the data to be stored in columns.
  ...
  For more information about the data types supported in MySQL mode of OceanBase Database, see [Data types](../../../700.reference/500.sql-reference/100.sql-syntax/200.common-tenant-of-mysql-mode/100.basic-elements-of-mysql-mode/100.data-type-of-mysql-mode/100.data-type-overview-of-mysql-mode.md).
...
* Determine whether to define the `PRIMARY KEY` constraint on specific columns based on the requirements for the the **PRIMARY KEY constraint** section.
...
* When you use `COMMENT` to create a comment on a column, the comment cannot exceed 1,024 characters in length.
...
In addition to the `PRIMARY KEY` constraint, OceanBase Database also supports constraints `NOT NULL`, `UNIQUE`, `FOREIGN KEY`, and `CHECK`. The constraints can simplify table queries, improve query performance, and ensure semantic validity of data.
...
* `FOREIGN KEY`: the constrained column can contain only values from the primary key column of another table.
...
|---------------------------|----------------------------------------------------------|
| auto_increment_cache_size | The number of cached auto-increment values. Value range: \[1, 100000000\]. Default value: `1000000`. This variable takes effect globally.       |
...
For more information about partitions, see [Overview](../../../700.reference/100.oceanbase-database-concepts/400.database-objects/200.database-objects-of-mysql-mode/400.partition-of-mysql-mode/100.partitions-overview-of-mysql-mode.md).
...
In HASH partitioning, you must specify the partitioning keys and the number of partitions. The system obtains an integer based on the HASH partitioning expression. Then the system performs a modulo operation on this integer and the number of partitions, so as to determine the partition to which a specific row belongs.
...
For more information about replicated tables, see **Create a replicated table** in [Create a table](../../../700.reference/300.database-object-management/100.manage-object-of-mysql-mode/200.manage-tables-of-mysql-mode/200.create-a-table-for-mysql-tenant-of-mysql-mode.md).
...
* For more information about how to query the attributes of a table, see [Query the definition of a table](../../../700.reference/300.database-object-management/100.manage-object-of-mysql-mode/200.manage-tables-of-mysql-mode/500.view-the-definition-of-a-table-of-mysql-mode.md).
