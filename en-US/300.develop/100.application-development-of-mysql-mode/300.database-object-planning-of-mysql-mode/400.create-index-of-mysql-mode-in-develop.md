...
<main id="notice" type='explain'>
  <h4>Note</h4>
  <p>This topic mainly introduces how to create indexes by using the <code>CREATE INDEX</code> statement. </p> 
</main>
...
* Spatial index restrictions:
  ...
  * Columns used for creating spatial indexes must be defined with the `SRID` attribute; otherwise, spatial indexes added on such columns will not be effective for subsequent queries.

  * Spatial indexes can only be created on columns with spatial data types. 
