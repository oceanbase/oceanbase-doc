...
* The table group name can contain only letters, digits, and underscores (_), and must start with a letter or an underscore.
...
<main id="notice" type='explain'>
  <h4>Note</h4>
  <p>You can query the oceanbase.CDB_OB_TABLEGROUPS view for table groups in tenants. </p>
</main>
...
3. Create a non-partitioned table named `test_tbl1` and set `TABLEGROUP` to `test_tbl_tg1`. An error is returned.
...
4. Create a HASH-partitioned table named `test_tbl1_h` and set `TABLEGROUP` to `test_tbl_tg1`. An error is returned.
...
3. Create a non-partitioned table named `test_tbl3` and set `TABLEGROUP` to `test_tbl_tg3`. An error is returned.
