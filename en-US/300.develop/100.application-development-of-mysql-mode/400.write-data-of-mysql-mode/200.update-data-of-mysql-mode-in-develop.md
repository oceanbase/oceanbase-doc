...
Generally, the [UPDATE](../../../700.reference/500.sql-reference/100.sql-syntax/200.common-tenant-of-mysql-mode/600.sql-statement-of-mysql-mode/9000.update-of-mysql-mode.md) statement is used to update the data in a table.
...
For example, increase all values in the `value` column of the `t_insert` table by 1.
...
When you execute the `UPDATE` statement, make sure that the transaction is not large. You can use the `LIMIT` keyword to control the number of records to be updated or the `WHERE` clause to control the update scope. When you execute an `UPDATE` statement that involves more than one hundred thousand records without using the `WHERE` clause, a large transaction is generated, which may cause the update to fail.

### Update part of the records

You can update part of the records in a table. Assume that the `t_insert` table contains a large number of records. You can use the `LIMIT` keyword to control the number of records to be updated or the `WHERE` clause to control the update scope.

Here are some examples:

* Use the `LIMIT` keyword to increase the values in the first 100,000 rows of the `value` column in the `t_insert` table by 1.

  ```sql
  obclient [test]> UPDATE t_insert SET value = value+1 LIMIT 100000;
  ```

* Use the `WHERE` clause to increase all the values equal to `10005` in the `value` column in the `t_insert` table by 1.
...
When you update data in a table with `UNIQUE` constraints to identical values, the system returns an error and the date update fails. Here is an example:
