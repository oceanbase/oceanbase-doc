...
* You have connected to a MySQL-compatible tenant of the database. For more information, see [Connection methods](../100.connect-to-oceanbase-database-of-mysql-mode/100.connection-methods-overview-of-mysql-mode.md).
...
* This statement writes data using the full direct load method and is subject to its limitations. For more information, see the **Limitations** section in [Import data through direct load by using the INSERT INTO SELECT statement](../../../620.obap/300.obap-data-collection/300.obap-import-data/20.bypass-import/200.full-bypass-import.md).
...
* Partition-level `INSERT OVERWRITE` operations support target tables where the last-level partitions use Hash or Key partitioning.
