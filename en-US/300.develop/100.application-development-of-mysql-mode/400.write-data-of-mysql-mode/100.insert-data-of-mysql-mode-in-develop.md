...
|slug|insert-data-mysql-compatible-mode|
...
You can use the `INSERT INTO` statement to insert a single row of data. To insert multiple records, you can execute the single-row insertion statement multiple times. To perform batch insertion, see the **Insert multiple rows of data in batches** section.
...
To insert multiple records, you can also use an `INSERT INTO` statement that contains multiple `VALUES` clauses. A single multi-row insertion statement is executed faster than multiple single-row insertion statements.
...
To back up a table or copy all records in a table to another table, you can use the `INSERT INTO ... SELECT ... FROM` statement as the `VALUES` clause in the `INSERT INTO` statement for batch insertion.
