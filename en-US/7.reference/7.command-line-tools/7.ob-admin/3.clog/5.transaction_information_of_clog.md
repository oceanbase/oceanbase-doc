...
* Local transaction: The information about a local transaction is maintained on the same node (OBServer) as the one that stores data accessed by the transaction. Such a transaction accesses only data in the same partition.
...
Transactions on an OBServer are uniquely identified by transaction IDs. Therefore, transaction IDs must be unique on the same OBServer.
...
| Field | Description |
|--------|-----|
| `server_`    | The IP address of the OBServer where this transaction belongs.  |
| `inc_`       | The monotonically increasing value in the OBServer, which is recounted after the OBServer restarts. |
