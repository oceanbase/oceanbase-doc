...
If no matching row is found, `BIT_AND()` returns a neutral value whose all bits are set to 1. The neutral value has the same length as the argument value. `NULL` values have no effect on the result unless all values are `NULL`. In this case, the result is a neutral value that has the same length as the argument value.
