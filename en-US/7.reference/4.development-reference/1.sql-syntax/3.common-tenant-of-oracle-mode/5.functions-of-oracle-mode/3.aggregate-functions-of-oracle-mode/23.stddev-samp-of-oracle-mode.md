...
`STDDEV_SAMP()` returns the sample standard deviation of numeric data to a parameter of the numeric data type, which is the arithmetic square root of the sample variance. The difference between `STDDEV` and `STDDEV_SAMP` is that the `STDDEV` function returns `0` for only one row of input data, whereas `STDDEV_SAMP` returns `NULL`. You can use this function as an aggregate or analytic function.
