...
In the current OceanBase Database version, the statistics type specified by users in `SET_COLUMN_STATS` must be stored in the dictionary. If the statistics type is `NULL`, the statistics type associated with the index or column is stored.
