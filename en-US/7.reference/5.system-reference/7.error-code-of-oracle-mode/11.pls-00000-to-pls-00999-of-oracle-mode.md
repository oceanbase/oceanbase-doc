...
* Cause: The prefix in a qualified name was a subprogram or cursor beyond an enclosing scope. That is, a subprogram or cursor name is being used as a scope qualifier for a reference to an entity (within that subprogram or cursor) that is not in scope. Example: `declare x number; type t1 is record (a number); function f return t1 is a number; begin x := f.a;`:
   ...
   * Resolves to local variable "a" in function `"f". x := f().a;`

   * Legal reference to component "a" of returned record end

   * Illegal use of function "f" as a scope qualifier `x := f().a;` and legal reference to component "a" of returned record end
...
* Cause: A form construct `WHEN excep1 OR OTHERS =>` was found in the definition of an exception handler. The `OTHERS` handler must appear by itself as the last exception handler in a block.
