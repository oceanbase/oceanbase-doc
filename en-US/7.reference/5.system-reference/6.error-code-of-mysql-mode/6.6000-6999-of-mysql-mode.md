...
* Cause: An internal error occurred. The liboblog instance finds no logs for the partition on the corresponding OBServer.
...
* Cause: An internal error occurred. When a liboblog instance identifies the `start_log_id` value corresponding to the specified start timestamp and uses the value to pull logs from the relevant OBServer through remote procedure calls (RPCs), resumable requests are supported. If the start_log_id value cannot be identified, this error is returned to the liboblog instance. Then, the liboblog instance determines and implements the subsequent logic.
...
* Cause: An internal error occurred. The stream corresponding to the stream ID carried in the request initiated by the liboblog instance is not found in the memory of the OBServer.
