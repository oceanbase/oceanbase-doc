...
* Cause: The `ON` keyword is not used.
...
* Cause: The year conflicts with the Julian day. Example: `SELECT TO_DATE('2021-03-23 2459063', 'YYYY MM DD J') FROM DUAL;`.
...
* Cause: The month conflicts with the Julian day. Example: `SELECT TO_DATE('11 2459063','MM J') FROM DUAL;`.
...
* Cause: The day of the month (1 to 31) conflicts with the Julian day. Example: `SELECT TO_DATE('11 2459063','DD J') FROM DUAL;`.
...
* Cause: The day of the week (1 to 7) conflicts with the Julian day. Example: `SELECT TO_DATE('3 2459063','D J') FROM DUAL;`.
...
* Cause: The week can be specified only once.
...
* Cause: The table must contain at least one visible column.
...
* Solution: Troubleshooting is not required. The OBServer will automatically roll back the process to execute the statements separately.
...
* Cause: The `ROW LIMITING` expression of the SQL statement contains components other than text, bound variables, correlation variables, or subqueries.
...
* Cause: The percentile value specified for the `PERCENTILE_CONT` or `PERCENTILE_DISC` function is invalid.
...
* Cause: The virtual column expression contained a function whose evaluation is non-deterministic.
...
* Cause: The operation attempted to delete a system-generated sequence.
...
* Solution: Set `PGA_AGGREGATE_TARGET` to an appropriately large value in `WORKAREA_SIZE_POLICY = AUTO` mode, or  set `SORT_AREA_SIZE` to an appropriately large value in `WORKAREA_SIZE_POLICY = MANUAL` mode.
...
ERROR 5975 (HY000): dblink remote ora error code: %d
--------------------------------------------------------------------------

* Error code in OceanBase Database: 5975

* Cause: The source Oracle database reported an error, and the returned Oracle error code cannot be mapped to the compatible error code in OceanBase Database.

* Solution: Locate the cause based on the Oracle error code in the error message.
