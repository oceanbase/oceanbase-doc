...
|dir-name-en||
|tenant-type||
|slug|initialize-observer-nodes-using-oatcli|

# Initialize OBServer nodes by using oatcli
...
* You have prepared OBServer nodes and completed the related configuration. For more information, see [Prepare servers](../../200.preparations-before-deploy/100.prepare-servers.md) and [Configure servers](../../200.preparations-before-deploy/200.configure-servers.md).
...
1. Connect to the OAT server.
   ...
   You can run the following `ssh` command to connect to the OAT server:
...
3. (Optional) Run the following command to view how to use oatcli:
