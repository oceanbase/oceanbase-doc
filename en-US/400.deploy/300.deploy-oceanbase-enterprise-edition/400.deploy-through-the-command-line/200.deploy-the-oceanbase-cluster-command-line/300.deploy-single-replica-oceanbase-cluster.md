...
2. Install OceanBase Client (OBClient).
   ...
   OceanBase Database tenants can be in Oracle or MySQL mode. To connect an Oracle tenant to a Java program, the Java driver file (`oceanbase-client-\*.jar`) provided by OceanBase Database is required. To access an Oracle tenant by using the CLI, you must also install OBClient.
