...
1. Install the RPM package for OceanBase Database.
   ...
   <main id="notice" type='explain'>
      <h4>Note</h4>
      <p>OceanBase Database is installed by default in the <code>/home/admin/oceanbase</code> directory.</p>
   </main>
...
<main id="notice" type='explain'>
  <h4>Note</h4>
  <p><ul><li>The sample IP addresses are desensitized. This is not an installation requirement. During deployment, use the actual IP addresses of your servers.</li><li>OceanBase Database supports starting the observer process on a server with a specified IPv4 or IPv6 address. The examples in this topic show how to start the observer process using an IPv4 address. For information about starting the observer process using an IPv6 address, see <a href="300.deploy-single-replica-oceanbase-cluster.md">Deploy a single-replica OceanBase cluster by using the CLI</a>.</li></ul></p>
</main>
...
1. Start the observer process on each node.
   ...
   <main id="notice" type='notice'>
   <h4>Notice</h4>
   <p>In a three-replica deployment, the startup parameters of each node are not exactly the same. When starting the observer process, you only need to specify the three (or more) Root Service machines. You can add new machines after the cluster is created.</p>
   </main>
