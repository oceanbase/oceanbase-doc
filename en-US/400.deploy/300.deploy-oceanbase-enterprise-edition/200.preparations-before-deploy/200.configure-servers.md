...
* In an ARM and Hygon architecture, we recommend that you modify the `/etc/sysctl.conf` configuration file and set the parameters `kernel.numa_balancing`, `vm.zone_reclaim_mode`, and `vm.swappiness` to `0`. The steps are as follows:
