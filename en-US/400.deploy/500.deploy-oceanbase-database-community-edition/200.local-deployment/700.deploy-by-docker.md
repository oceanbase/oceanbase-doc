...
* `-e` sets environment variables. In the example, `MODE` sets the OceanBase Database startup mode, and `OB_TENANT_PASSWORD` sets the root@test user password in OceanBase Database. For more information about environment variables, see [Supported environment variables](#supported-environment-variables) below.
