...
* You have deployed OCP. For more information, see [OceanBase Cloud Platform Deployment Guide](https://en.oceanbase.com/docs/community-ocp-en-10000000000840029).

* You have logged in to the OCP console and have the permissions for **host management** and **cluster creation**.
...
* (Optional) An OceanBase Database Proxy (ODP) cluster is available in OCP if you want to create a multi-replica OceanBase cluster. For more information about how to add an ODP cluster, see [Create an OBProxy cluster](https://en.oceanbase.com/docs/community-ocp-en-10000000000840332).
...
3. In the upper-right corner of the **Hosts** page, click **Add Host**.
   ...
   ![Add Host](https://obbusiness-private.oss-cn-shanghai.aliyuncs.com/doc/img/observer-enterprise/V4.2.5/400.deploy/200.deploy-by-ocp-01.png)

4. Specify the host information as prompted
   ...
   | **SSH Port** | The SSH port number. The default value is `22`.               |
   | **Host Type** | The tag that you specify for hosts with the same configurations to facilitate host management. If the required host type is unavailable, select **Create Host Type** in the drop-down list to add the type. |
   | **IDC** | Select the IDC where the host is deployed. If the target IDC is unavailable, select **Create IDC** in the drop-down list to add it. IDC information includes **IDC** and **Region**. <ul><li>**IDC** is one of the host attributes that you must record for an OceanBase cluster. The IDC is referenced in load balancing and SQL statement routing strategies. Specify the actual IDC. </li><li>**Region** specifies the geographical area where the host is located. It is one of the host attributes that you must record for an OceanBase cluster. The region affects the load balancing and SQL statement routing strategies. Specify the actual region. </li></ul><main id="notice" type='explain'><h4>Note</h4><p>OCP V3.1.1 and later support multi-zone mode. When you add an IDC, it is created in the zone where OCP is deployed. </p></main> |
   | **Host Type** | You can select **Physical Machine** or **Container**.   |
   | **Credentials** | Select the credentials used to remotely log in to the physical server. You can select **Add Credential** from the drop-down list to create credentials. </br>When you add a credential, specify the following parameters:<ul><li>**Credential Name**: The credential name must start with a letter and end with a letter or a digit. It can contain letters, digits, and underscores (_). It must be 2 to 32 characters in length.</li><li>**Authorization Type**: The authorization type of the credential. Valid values are **Username/Password** and **SSH Private Key**. </li><li>**User Type**: SSH users with root permissions are supported. A regular user with root permissions must first be granted the permission to run sudo commands without a password. OCP allows you to grant the permission to run all sudo commands without a password by adding `username ALL=(ALL) NOPASSWD:ALL` to the `sudoers` file in the `/etc/` directory. </li></ul> |
   | **Host Alias (optional)** | A host alias is a tag that you specify for hosts with the same configurations. We recommend that you specify a meaningful alias to facilitate host management.   |
...
2. On the **Create Cluster** page, configure the basic information of the cluster.
   ...
   ![1](https://obbusiness-private.oss-cn-shanghai.aliyuncs.com/doc/img/observer-enterprise/V4.2.5/400.deploy/200.deploy-by-ocp-03.png)
   ...
   | **Cluster Name** | The name of the cluster to be created. The cluster name must be 2 to 48 characters in length and can contain uppercase and lowercase letters, digits, and underscores (_). The cluster name must start with a letter. |
...
3. Set the deployment mode of the cluster.
   ...
   | **CPU Architecture** | If multiple installation packages for different architectures are available for the selected OceanBase Database version, you must specify a host hardware architecture that matches the installation package to use. <main id="notice" type='notice'><h4>Notice</h4><ul><li><p>By default, the architecture of the first installation package is used. You can change the architecture. After you specify the architecture, the <b>Host</b> drop-down list displays only hosts of the specified architecture. </p></li><li><p>You can set different architectures for the zones. However, hybrid architecture deployment may have stability and performance issues. Proceed with caution. </p></li></ul></main> |
   ...
   | **Zone Priority Rankings** | The priorities of the zones. The zone with the highest priority is taken as the primary zone. Only one primary zone is allowed. Perform the following steps to specify zone priorities:<ol><li>Select one or more zones from the left-side list. The left-side list shows all zones available in the cluster. </li><li>Click the closing angle bracket (>) in the middle. The selected zones are moved to the <b>Priority Rankings</b> list. The zones that are selected at the same time have the same priority. </li><li>Repeat steps 1 and 2 to add zones of a lower priority. </li><li>To adjust the priority of a zone, drag and drop the zone in the <b>Priority Rankings</b> list. </li></ol>The priorities of zones in the list are in descending order.  |
...
4. Enable **Parameter Settings** and specify related cluster parameters.
   ...
   * You can also click **Select Parameter Template** shown in section ② of the following figure and select a parameter template. The system will automatically populate parameters in this section by using the template.
...
5. Enable **Custom Settings** to configure paths and ports.
   ...
   * Configure the paths:
     ...
     2. Click **Test** to check whether the paths can be used. If the path cannot be used, you can perform troubleshooting based on the test results, or use another path.
...
<main id="notice" type='explain'>
  <h4>Note</h4>
  <p>To view the progress of this task, choose <b>System Management > Tasks</b>. </p>
</main>
