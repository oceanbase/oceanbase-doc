...
* You are using an RPM-based operating system. The following systems are currently verified and supported:
...
4. View the startup status of OceanBase Database.
   ...
   When `Service is ready` is displayed, OceanBase Database has been started successfully.

### Step 3: Connect to OceanBase Database

The following uses OBClient as an example to connect to OceanBase Database. Run the following command:
...
* `-h`: The IP address for connecting to OceanBase Database. For OceanBase Database started by using systemd, the default IP is `127.0.0.1`.

* `-u`: The account for connecting to OceanBase Database, in the format of `username@tenant_name`. The default administrator username for a MySQL-compatible tenant is `root`.

* `-P`: The port for connecting to OceanBase Database, which is the value of `mysql_port` in `/etc/oceanbase.cnf`.

* `-p`: The password for connecting to OceanBase Database, which is the value of `root_pwd` in `/etc/oceanbase.cnf`. We recommend that you do not provide the password in the command; instead, enter it when prompted after running the command, so that the password is not displayed.

* `-A`: Specifies not to automatically retrieve statistics when connecting to OceanBase Database by using OBClient.

* `oceanbase`: The name of the database to access. You can change it to your business database name.
...
* If the value of the `Active` field is `failed`, a service error occurred. Check the logs for troubleshooting.
...
* ERROR: current user(uid=0) that starts observer is not the same with the original one(uid=500)
   ...
   A common cause is that the owner of the `etc` directory under the OceanBase installation directory (default: `/home/admin/oceanbase`) is inconsistent with the current user. In this case, run the following command:
...
* errcode=-4290, file="ob_server_log_block_mgr.cpp", line_no=1127, info="::fallocate failed"
   ...
   A common cause is insufficient disk space in the data directory of OceanBase Database (default: `/var/lib/oceanbase/`). In this case, increase the disk space for the data directory.
