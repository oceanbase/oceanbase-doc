...
* `-P` specifies the port for connecting to OceanBase Database, which is the value of `mysql_port` in `/etc/oceanbase.cnf`.

* `-p` specifies the password for connecting to OceanBase Database, which is the value of `root_pwd` in `/etc/oceanbase.cnf`. Instead of providing the password in the command, we recommend that you enter the password as prompted after you run the command. This way, the password is masked.
