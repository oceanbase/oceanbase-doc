...
5. Modify the configuration file.
   ...
   3. Configure the obproxy-ce component and modify the IP address and `home_path`.
      ...
      | observer_sys_password | No | Empty by default for OBD V2.0.0 and earlier</br>A random string same as `proxyro_password` by default for OBD V2.1.0 and later | The password of the account for connecting the OceanBase cluster (proxyro@sys) through ODP. Set the password same as `observer_sys_password`. If you do not specify this parameter in OBD V2.1.0 or later, a random string same as `proxyro_password` is automatically generated.  |
