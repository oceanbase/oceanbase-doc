...
* OceanBase Database Proxy (ODP)
  ...
  ODP is a dedicated proxy service designed for OceanBase Database. For more information, see [ODP documentation](https://en.oceanbase.com/docs/odp-en).

* OceanBase Cloud Platform (OCP)

  OCP is an OceanBase Database management platform. For more information, see [OCP documentation](https://en.oceanbase.com/docs/ocp-en).
...
* You have installed obd (latest version recommended) on the central control server. For more information, see [Install and configure obd](https://en.oceanbase.com/docs/community-obd-en-10000000000862271).
...
<main id="notice" type='explain'>
  ...
  <ul>
  <li>
  <p>In the production environment, we recommend that you deploy your applications on the same servers as ODP to minimize the latency of access from ODP to the applications. You can deploy an ODP service on each application server. In this example, ODP is separately deployed for ease of illustration. </p>
  </li>
  <li>
  ...
  <li>
  <p>For more information about how to deploy a standalone database, see <a href="https://en.oceanbase.com/docs/community-obd-en-10000000001736334">Deploy OceanBase Database on a single OBServer node</a> in obd documentation. </p>
  </li>
  </ul>
</main>
...
OceanBase Database V4.0.0 and later provide an all-in-one installation package. You can use this package to install obd, OceanBase Database, ODP, OBAgent, and other components at a time.
...
<main id="notice" type='explain'>
  <h4>Note</h4>
  <p>You can run the <code>which obd</code> and <code>which obclient</code> commands to check whether the installation is successful. If you can find the <code>obd</code> and <code>obclient</code> paths in the <code>.oceanbase-all-in-one</code> directory, the installation is successful. </p>
</main>
...
<main id="notice" type='explain'>
  <h4>Note</h4>
  <p>This topic takes obd V2.8.0 as an example. The configuration file may vary with the obd version. For more information about obd, see <a href="https://en.oceanbase.com/docs/obd-en">obd documentation</a>. </p>
</main>
...
4. Select a configuration file.
   ...
   <main id="notice" type='explain'>
     ...
     <ul>
     <li>
     <p>The small-scale development mode applies to individual devices with at least 8 GB of memory. The configuration file names contain <code>mini</code> or <code>min</code>. </p>
     </li>
     <li>
   ...
   * Sample configuration files for standalone deployment with ODP: `mini-single-with-obproxy-example.yaml` and `single-with-obproxy-example.yaml`
   ...
   If you choose distributed deployment, multiple target servers are required. In this case, you can use a configuration file for distributed deployment.
...
5. Modify the configuration file.
   ...
   2. Modify the IP address, port, and related directories of each server, and specify memory-related parameters and the password.
      ...
      If the servers have inconsistent parameters, move the relevant parameters to the sections of target servers and separately set the parameters for the servers. Configurations in server sections have higher priority than those in the `global` section. For example, you can modify the configuration file as follows to configure different ports for two servers:
      ...
      | datafile_next | No | 0 | The auto scaling step of disk space for data files. For information about how to enable auto scaling when this parameter is not specified, see [Configure automatic scale-out of disk space for data files](../../../700.reference/200.system-management/1000.disk-data-file-management/100.disk-data-file-dynamic-expansion.md). |
      | datafile_maxsize | No | 0 | The maximum size that the disk space for storing data files can be scaled out to. For information about how to enable auto scaling when this parameter is not specified, see [Configure automatic scale-out of disk space for data files](../../../700.reference/200.system-management/1000.disk-data-file-management/100.disk-data-file-dynamic-expansion.md). |
      ...
      | ocp_meta_tenant  | No      | tenant_name: ocp</br>max_cpu: 1</br>memory_size: 2147483648        | The meta tenant specifications for OCP Express. The configurations under `ocp_meta_tenant` are passed in as parameters when you create a tenant. </br>The example only shows a few important parameters. For more information about the parameters that can be configured, see the description of the **obd cluster tenant create** command in [Cluster commands](https://en.oceanbase.com/docs/common-oceanbase-database-10000000001715136) in obd documentation. |
      | mysql_port  | Yes      | 2881        | The port number for the SQL service protocol. |
      | rpc_port  | Yes      | 2882       | The port number for remote access, which is the remote procedure call (RPC) communication port between the observer process and other node processes. |
      | obshell_port | Yes | 2886 | The O&M port of OceanBase Database.      |
      | home_path  | Yes      | None       | The installation path of OceanBase Database. We recommend that you choose a path under the `admin` user. |
      | data_dir | No | $home_path/store | The directory for storing SSTables and other data. We recommend that you use a separate disk. |
      | redo_dir  | No      | Same as `data_dir` by default | The directory for storing clogs. We recommend that you use a separate disk. |
      | root_password | No | Empty in versions earlier than obd V2.1.0</br>Random string in obd V2.1.0 and later | The password of the super administrator (`root@sys`) of the OceanBase cluster. We recommend that you set a complex password. If you do not specify this parameter in obd V2.1.0 or later, a random string is automatically generated. |
      | proxyro_password | No | Empty in versions earlier than obd V2.1.0</br>Random string in obd V2.1.0 and later | The password of the account (`proxyro@sys`) used by ODP to connect to the OceanBase cluster. If you do not specify this parameter in obd V2.1.0 or later, a random string is automatically generated. |

   3. Configure the obproxy-ce component and modify the IP address and `home_path` setting.
      ...
      | home_path  | Yes      | None       | The installation path of ODP. We recommend that you choose a path under the `admin` user. |
      | obproxy_sys_password | No | Empty in versions earlier than obd V2.1.0</br>Random string in obd V2.1.0 and later | The password of the ODP administrator account (`root@proxysys`). If you do not specify this parameter in obd V2.1.0 or later, a random string is automatically generated. |
      | observer_sys_password  | No      | Empty in versions earlier than obd V2.1.0</br>Random string same as `proxyro_password` in obd V2.1.0 and later | The password of the account (`proxyro@sys`) used by ODP to connect to the OceanBase cluster. This password must be the same as the value of `proxyro_password`. If you do not specify this parameter in obd V2.1.0 or later, a random string same as `proxyro_password` is automatically generated. |

   4. Modify the IP addresses and `home_path` settings for the OBAgent and OCP Express components.
      ...
      |----------|-----------|----------|----------|
      | servers  | Yes      | None        | Specify each server in the format of `- name: server identifier (line feed) ip: server IP address`. You can specify multiple servers. The server identifiers must be unique. </br>You can specify unique server IP addresses in the format of `- <ip> (line feed) - <ip>`, where `- <ip>` is equivalent to `- name: server IP address (line feed) ip: server IP address`. |
      | home_path  | Yes      | None       | The installation path of the component. We recommend that you choose a path under the `admin` user. |
      | memory_size | Yes | None | The memory capacity of the OCP Express server. The recommended value is calculated as follows: `memory_size` = 512 MB + (Expected number of nodes × Expected number of tenants) × 60 MB. The `sys` tenant and OCP meta tenant must be counted in the expected number of tenants. |
      | logging_file_total_size_cap | Yes | 1GB | The total size of log files. <main id="notice" type='notice'><h4>Notice</h4><p>The unit of this parameter must be GB or MB. If you use the unit G or M, an error is reported and OCP Express cannot be deployed. </p></main> |
      | admin_passwd  | Yes      | No        | The password of the admin account for logging in to OCP Express. The password must be 8 to 32 characters in length and contain at least two digits, two uppercase letters, two lowercase letters, and two special characters. The supported special characters are `~ ! @ # % ^ & * _ - + = \| ( ) { } [ ] : ; , . ? /`. If you do not specify this parameter in obd V2.1.0 or later, a random string is automatically generated. |

### Step 3: Deploy the OceanBase cluster

<main id="notice" type='explain'>
  <h4>Note</h4>
  <p>For more information about the commands in this section, see <a href="https://en.oceanbase.com/docs/common-oceanbase-database-10000000001715136">Cluster commands</a> in obd documentation. </p>
</main>
...
1. Deploy the OceanBase cluster.
   ...
   After you run the `obd cluster deploy` command, if your server is connected to the Internet, obd checks whether the desired installation package exists on the target server. If no, obd automatically obtains the installation package from the YUM repository.
...
2. Start the OceanBase cluster.
   ...
   If OCP Express is installed, the access address of OCP Express is returned. In Alibaba Cloud or other cloud environments, the installation program may fail to obtain a public IP address and thereby return an intranet network address. Make sure that you use the correct public IP address.
...
4. (Optional) Modify the cluster configurations.
   ...
   After you modify and save the configurations and exit, obd will prompt you on how to validate the modifications. The output returned after you save the configurations is as follows:
...
* `-u` specifies the account for connecting to OceanBase Database. Supported account formats are `username@tenant name#cluster name`, `cluster name:tenant name:username`, `cluster name-tenant name-username`, and `cluster name.tenant name.username`. The default username of the administrator of a MySQL tenant is `root`.
...
* Method 1: Create a user tenant by using obd.
  ...
  By default, this command creates a tenant based on all remaining available resources of the cluster. You can configure the parameters to allocate specific amounts of resources to the tenant. For more information about this command, see **obd cluster tenant create** in [Cluster commands](https://en.oceanbase.com/docs/common-oceanbase-database-10000000001715136) in obd documentation.
...
You can run the following commands to manage OceanBase clusters deployed by using obd. For more information, see [Cluster commands](https://en.oceanbase.com/docs/common-oceanbase-database-10000000001715136) in obd documentation.
...
# Stop a running cluster named `obtest`.
obd cluster stop obtest

# Destroy a deployed cluster named `obtest`.
...
3. Make the modification take effect.
   ...
   After you delete the environment variables, you must log in to the CLI again or run the `source` command to make the modification take effect. Here is an example::
