...
* Check the Windows version
  ...
  Press [Win] + [Q] on your keyboard, search for **<UI-TERM key="ocp-v2.SQLDiagnosis.Component.SQLLimit.Set">Settings</UI-TERM>**, and open it. In the **Settings** window, choose **System** > **About** to view your Windows version and build number.
...
* Check memory and CPU
  ...
  Open your Finder, click **Applications**, then navigate to the **Utilities** folder. Locate **Activity Monitor** and double-click to open it. In **Activity Monitor**, you can view the CPU usage under the **CPU** tab, and check memory usage by switching to the **<UI-TERM key="ocp-v2.Detail.Tenant.Memory">Memory</UI-TERM>** tab.
...
3. Enable the WSL and virtual machine platform features.
   ...
   * Method 2: Enable the features using the graphical interface
     ...
     Press [Win] + [R] on your keyboard, type `optionalfeatures`, and click **<UI-TERM key="oms-cloud.components.ETL.ETLFormDrawer.Determine">OK</UI-TERM>** to open the **Windows Features** window. Alternatively, press [Win] + [Q], search for **Turn Windows features on or off**, and click to open the **Windows Features** window. Scroll down, check the boxes for **Virtual Machine Platform** and **Windows Subsystem for Linux**, then click **<UI-TERM key="ocp-v2.component.Compaction.DumpPolicyDrawer.Ok">OK</UI-TERM>**. Restart your computer to apply the changes.
...
On the macOS operating system, two installation methods are provided: installation via Homebrew and installation via a dmg package. This section will introduce both methods to you.

  :::tab

  tab Install OceanBase Desktop using Homebrew
  ...
  2. Add the OceanBase Desktop Tap.
     ...
     After adding the OceanBase Desktop Tap to Homebrew, you can run the `brew install` command to install the official package provided by OceanBase.
  ...
  3. Install OceanBase Desktop.
     ...
     After the command is executed, the latest version of OceanBase Desktop will be automatically downloaded and installed, and OrbStack will be installed as a dependency (if it is not already installed). After the installation is complete, the application will be placed in the Applications folder.

  4. Start the application.

     Find `OceanBase Desktop` in the Applications folder or use Spotlight to search for `OceanBase Desktop`, and double-click it to start the application. On the first start, please wait patiently as the application will complete initialization in the background. After entering the application, click the **Start** button on the page to start the OceanBase database.

  <main id="notice" type='explain'>
    <h4>Note</h4>
    <p>If you need to uninstall OceanBase Desktop, you can run the command <code>brew uninstall --cask oceanbase-desktop</code>. This command will remove the application files, application support data, saved application states, and the application’s virtual machine.</p>
  </main>

  tab Install OceanBase Desktop using the dmg installation package
  ...
  1. Download the installation package.
     ...
     Go to [OceanBase Download Center](https://www.oceanbase.com/softwarecenter), search for **OceanBase Desktop**, and click **Mac** to download the installation package.
