...
For more information about how to back up data for an OceanBase cluster, see [Preparations](../600.manage/600.backup-and-recovery/400.data-backup/100.preparation-before-data-backup.md).
...
<main id="notice" type='notice'>
  <h4>Notice</h4>
  <p>The following operations are irreversible. In other words, the deleted files and directories cannot be recovered. Therefore, before you run the following commands, make sure that the data in the directories is no longer needed. </p>
</main>
...
1. Query the cluster status.
   ...
   The output is as follows. For more information, see the **obd cluster list** section in [Cluster commands](https://en.oceanbase.com/docs/community-obd-en-10000000001181575).
...
2. Check whether a standby tenant exists.
   ...
   The output is as follows, indicating that no standby tenant exists and the cluster contains only the `sys` tenant. If a tenant in the cluster has a standby tenant, you can see the [Deploy primary and standby tenants by using obd](https://en.oceanbase.com/docs/community-obd-en-10000000001181619) topic for the output and field description.
...
3. (Optional) Decouple the standby tenant from the primary tenant.
   ...
   For more information, see [Switch the roles of tenants and decouple a standby tenant from the primary tenant](https://en.oceanbase.com/docs/community-obd-en-10000000001181620).
...
* If your OceanBase cluster is managed by using OceanBase Cloud Platform (OCP), you can delete the cluster as prompted. For more information, see [Delete a cluster](https://en.oceanbase.com/docs/common-ocp-10000000001187628).
