...
1. **Export phase**: In the source AP database, export data to structured files (such as Parquet or ORC) using writable external tables, and store the files in object storage (such as S3 or OSS) or HDFS.
2. **Staging phase**: Temporarily store the data in object storage or a distributed file system as an intermediate medium.
...
- Supports parallel and direct load, significantly enhancing throughput.
...
- **High compression ratio**: Significantly reduces the volume of intermediate data, saving storage and network bandwidth.
- **Strong type and nested structure support**: Avoids delimiter conflicts and escape issues found in text formats like CSV.
- **High-performance read/write**: Import/export performance is improved by approximately 20%–30% compared to CSV, especially suitable for wide tables and complex data types.
...
### Import using `INSERT INTO` URL external tables
...
### Import using `LOAD DATA` statements
...
Besides object storage, OceanBase also supports direct access to heterogeneous data sources such as HDFS and ODPS through external tables, making it suitable for users with existing data lake architectures.
...
**Optimization suggestion**: For tables with a high number of partitions, you can combine the **Partition Exchange** technique to avoid the aforementioned issues.
...
- **Supported scenarios**: OMS supports incremental synchronization from mainstream OLTP databases including Oracle (including OGG) and MySQL.
- For detailed operation instructions, see: [Use OMS for data migration](https://www.oceanbase.com/docs/enterprise-oms-doc-cn-1000000004023033)
