...
* **Load Data import with URL CSV external table error tolerance mode**
  ...
  When you import data by using the Load Data method, an error is returned if the data types are incompatible or the precision does not match. This version adds the `LOG ERRORS` instruction to support error-tolerant import. In MySQL-compatible mode, failed rows are recorded. You can view the error data by using the `show warnings` command and perform error diagnostics.
