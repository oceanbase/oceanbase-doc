...
|-------------|------------------|--------------------|
| SIMPLE | No authentication method is specified when creating a catalog (default is SIMPLE). | <ul><li>If the HDFS directory is readable by all users (e.g., with permission `drwxr-xr-x`), no location needs to be created.</li><li>If the directory is accessible only to specific users (e.g., owned by hive or impala), you must explicitly specify the HDFS user with `CREATE LOCATION ... CREDENTIAL (USER = 'xxx')`.</li></ul> |
...
620.obap/400.data-lake/100.data-catalog/100.catalog-overview.md)
