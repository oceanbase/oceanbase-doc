...
**Shared Storage mode** is an architecture that stores data centrally on shared storage devices, allowing multiple database nodes to access the same data. This architecture is primarily used in public cloud environments. The main advantages of this mode include simplified storage management, improved resource utilization, and enhanced high-availability switching in specific scenarios.

* **Reduce storage redundancy:**   In a traditional shared-nothing architecture, each node maintains its own copy of the data, leading to storage resource wastage. In contrast, shared storage allows multiple compute nodes to access the same data, thereby reducing the number of data replicas and lowering the overall storage cost.

* **Rapid failover:** If a compute node fails, other nodes can directly access data in the shared storage, without relying on complex replication or migration processes, enabling faster service takeover.

* **Simplified O&M Management: **A unified storage pool facilitates backup, snapshots, monitoring, and other operations, enhancing the maintainability of the database system.

The 2-replica architecture based on OceanBase DB separates storage from computing and ensures data reliability and availability. It can support applications with high requirements for data reliability and availability.

<!-- ![](https://obbusiness-private.oss-cn-shanghai.aliyuncs.com/doc/img/observer/ap/shared-storage-1.png) -->

* **Cost-effective:** Full data is stored in object storage and hot data is cached in local disks, lowering storage costs and ensuring P99 query performance. The system supports elastic scaling, with independent scaling capabilities for compute and storage. Performance loss is typically between 0.3% and 1.7% compared to a shared-nothing architecture.
* **High Availability:** The 2F deployment mode ensures high availability of the compute nodes. Together with the Paxos-based independent logging storage service, the system guarantees RPO = 0 and RTO < 8s.

The standalone architecture with a single replica and storage-compute separation in OceanBase Database provides the following benefits:

<!-- ![](https://obbusiness-private.oss-cn-shanghai.aliyuncs.com/doc/img/observer/ap/shared-storage_2.png) -->

* **Optimize costs:** Hot data is cached on local disk storage while bulk data resides on low-cost object storage. Single-replica configurations reduce computing costs by 2-3 times compared with 2F1A and 3F cluster configurations. Compute nodes can be quickly added or removed within a zone to accommodate changing performance needs.

* **High Availability**: With multi-data center redundancy across cities and built-in disaster recovery at the IDC level. In a single-replica deployment, it utilizes an independent Paxos-based log service and shared storage. If a computing node fails, a new one can be quickly launched to maintain high availability.
...
In the latest OceanBase Database V4.3.5 release, the Nested Materialized View (Nested MV) feature has been introduced. This feature allows you to easily build new materialized views on existing ones, which is particularly useful in data warehouse ETL processes. Nested materialized views can generate intermediate results during the data transformation and loading stages. These results can then be used as inputs for subsequent processing steps, further optimizing the efficiency of the entire data processing workflow.
...
The query optimizer of OceanBase Database is designed for HTAP hybrid workloads and real-time analytics. Transactional workloads typically have the following characteristics: small data access per query, high response time requirements, and high throughput. Analytical workloads typically have the following characteristics: large data access per query, and relatively low throughput. The execution plans and tuning methods required for different types of workloads vary significantly.

* For transactional workloads, it is usually necessary to create appropriate index structures for filtering predicates and join predicates on base tables. Each table should select the most suitable index path to significantly reduce the amount of data scanned.
* For analytical workloads, full-table scans are typically performed using columnar storage, and skipindex is used to quickly skip the scanning of some data blocks. The response time of a query can be reduced by increasing the degree of parallelism.

<!-- ![](https://obbusiness-private.oss-cn-shanghai.aliyuncs.com/doc/img/observer/ap/optimism-structure.png) -->

On the basis of fully inheriting the capabilities of the TP optimizer, the AP optimizer of OceanBase Database has been specifically enhanced for various complex query scenarios.

+ **More comprehensive query rewriting capabilities:** The query rewriting module supports a wide range of rewriting algorithms. Different rewriting algorithms are matched with different scenarios and perform corresponding equivalent transformations to improve the business SQL in a better direction. Currently, it supports rule-based rewriting algorithms such as view merging, subquery lifting, inner join elimination, outer join elimination, and elimination of always-true or always-false conditions. It also supports cost-based rewriting algorithms such as OR expansion, JA subquery lifting, Win Magic, and Group-By Placement.

+ **Distributed plan generation:** OceanBase Database adopts a one-phase distributed plan generation approach. This is different from the two-phase approach commonly used in other database systems. In the two-phase approach, an optimal single-machine plan is first generated, and then it is distributed. However, this approach faces more challenges in HTAP scenarios. For example, in the process of join enumeration, an optimal NEST-LOOP JOIN algorithm in a single-machine scenario may result in a large amount of cross-machine data access after distribution, significantly reducing the overall execution efficiency compared to the distributed HASH JOIN algorithm. The one-phase plan generation framework of OceanBase Database considers factors such as data distribution and parallelism during the enumeration of join orders and join algorithms, making comprehensive decisions to select the overall optimal distributed execution plan.

+ **Row and column path selection:** To effectively handle HTAP hybrid workloads, the query optimizer has a core intelligent optimization capability: automatic row and column path selection. This means that for a query, the optimizer will intelligently decide whether to read data from row store (Row Store) or column store (Column Store) based on its data access characteristics, aiming for optimal execution performance. OceanBase Database has designed a specific cost model for column store scanning and introduced a new statistical information mechanism to accurately evaluate the benefits of column store SkipIndex for column store scanning, ensuring accurate decisions between row store indexes and column store scanning.

+ **Easier Auto DOP:** Typically, databases use parallel execution to accelerate the execution of complex SQL statements. However, in real business scenarios, it is difficult to easily evaluate whether to enable parallel execution and the appropriate degree of parallelism. OceanBase Database provides the Auto DOP capability. When generating a plan, the optimizer evaluates the time required for query execution and automatically determines whether to enable parallel execution and the appropriate degree of parallelism, ensuring that SQL statements can achieve optimal performance by default.

+ **Execution plan management:** The SQL Plan Management (SPM) technology of OceanBase Database is a key technology for ensuring the long-term stable operation of real-time analytical business. It intelligently manages the evolution of execution plans to address scenarios where the optimizer may select a worse new execution plan after data volume changes, statistical information updates, or database version upgrades. This is achieved through gray-scale verification using real traffic to prevent execution plan rollback.
...
By employing differentiated data structure designs, these indexes provide targeted optimizations for different data types (text/JSON) and query patterns (fuzzy matching), collectively forming an acceleration layer for complex data queries.
...
OceanBase Database is a distributed, multi-model, integrated database that can process structured, semi-structured, and unstructured data simultaneously in a single database using the SQL engine in real-time analytics scenarios.

The storage foundation for multi-model data is LOB (Large Object). LOB plays a crucial role in AI storage and data processing. In AI scenarios, multi-modal data such as images, text, audio, and video are all large objects. The performance of LOB is essential for the computational efficiency in AI pre-training scenarios. OceanBase Database provides a highly efficient LOB implementation at the storage layer. It supports 512 MB of storage under SQL and can efficiently access and store large objects up to the terabyte (TB) level using the DBMS.Lob package.

In data analysis scenarios, OceanBase Database natively supports complex data types such as arrays, RoaringBitmap, and maps. These data types enable efficient storage and querying of structured data, as well as direct support for semi-structured data and complex aggregation analysis.

* RoaringBitmap and Map: These data types are suitable for advanced data mining scenarios such as large-scale tag analysis, audience targeting, and deduplication aggregation.
* Array: This data type provides great flexibility for complex business scenarios such as log retrieval, behavior tracking, and multi-dimensional tagging. It truly supports a wide range of capabilities in a single database.

For more information about the efficient compression bitmap data type (RoaringBitmap), see [Efficient compression bitmap data type (RoaringBitmap)](../700.reference/500.sql-reference/100.sql-syntax/200.common-tenant-of-mysql-mode/100.basic-elements-of-mysql-mode/100.data-type-of-mysql-mode/1000.roaring-bitmap-data-type-of-mysql-mode/100.roaring-bitmap-data-type-overview-of-mysql-mode.md).

For more information about the array data type, see [Array data type](../700.reference/500.sql-reference/100.sql-syntax/200.common-tenant-of-mysql-mode/100.basic-elements-of-mysql-mode/100.data-type-of-mysql-mode/1200.array-data-type-of-mysql-mode/100.array-data-type-overview-of-mysql-mode.md).

JSON is the most widely used semi-structured data type and has extensive applications in transaction, analytics, and even AI scenarios.

1. In transaction scenarios, JSON serves as a schemaless, flexible columnar storage and computing solution.
2. In analytics scenarios, JSON's multi-value indexing is ideal for flexible computing with multi-dimensional tags.
...
To support JSON across various workloads, OceanBase Database provides rich computational expressions and JSON-based multi-value indexing. At the storage level, it supports JSON Binary and optimizes random read/write operations for JSON data. It also supports structured encoding of JSON data, extracting structured information from similar JSON data to significantly improve storage compression ratios and query performance based on JSON paths. For more information about the JSON data type, see [JSON data type](../700.reference/500.sql-reference/100.sql-syntax/200.common-tenant-of-mysql-mode/100.basic-elements-of-mysql-mode/100.data-type-of-mysql-mode/800.json-formatted-data-type-of-mysql-mode/100.json-formatted-data-types-of-mysql-mode.md).

In the era of generative AI, the processing of multi-modal data has become increasingly important. OceanBase Database enhances vector and full-text search capabilities to better support hybrid search requirements in knowledge retrieval scenarios. Based on a fully self-developed path combining vector algorithms and databases, OceanBase Database's vector capabilities outperform open-source vector databases. In the VectorDBbench benchmark, OceanBase Database's vector performance exceeds typical open-source competitors.

<!-- ![](https://obbusiness-private.oss-cn-shanghai.aliyuncs.com/doc/img/observer/ap/vector-skill.png) -->

## Columnstore replicas

To meet the physical strong isolation requirements of TP and AP resources in HTAP hybrid workloads, OceanBase Database supports columnstore replicas (Column Store Replica, C replicas). A columnstore replica is a new type of replica with read-only characteristics. All baseline data of user tables on a columnstore replica is stored in columnar format. Columnstore replicas are deployed in independent zones. OLAP workloads access columnstore replicas through dedicated ODP endpoints using weak-consistency reads. This leverages the advantages of columnar batch processing to accelerate queries without affecting existing OLTP workloads. In a 2F1A deployment, deploying 2F1A1C not only achieves physical strong isolation of TP and AP resources but also reduces storage overhead compared to row-column hybrid storage.

Columnstore replicas follow the same distribution strategy and weak-consistency read release mechanism as regular read-only replicas. The main difference lies in the storage structure of the base data. Like regular read-only replicas, columnstore replicas do not participate in leader election or log synchronization voting. However, they still include core components such as static data tables, committed logs, and in-memory data tables.

For more information about columnstore replicas, see [Columnstore replicas](../700.reference/100.oceanbase-database-concepts/500.distributed-database-objects/300.data-partitions-and-replicas/300.partition-replica-type/400.columnstore-replica.md).
