...
<main id="notice" type='notice'>
  <h4>Notice</h4>
  <p>When you use the <code>LOAD DATA</code> statement to import data in full direct load mode and the destination table has a unique index, if a duplicate unique index key is encountered, the operation is temporarily not supported <code>REPLACE</code> or <code>IGNORE</code> keyword, nor can an error-tolerant row be set. </p>
</main>
