...
To support hybrid workloads such as TP and AP, OceanBase Database allows you to deploy **read-only columnstore replicas (C replicas)** on independent zones.

There are two deployment modes for columnstore replicas:

* Before V4.6.0, you need to deploy weakly consistent columnstore replicas. In this mode, you must configure an independent ODP for the columnstore replica and set the request routing strategy through the ODP configuration.

  ![](https://obbusiness-private.oss-cn-shanghai.aliyuncs.com/doc/img/observer-enterprise/V4.6.0/OBServer_Zone2.png)

* From V4.6.0 onwards, you can deploy strongly consistent columnstore replicas. In this mode, you do not need to configure an independent ODP for the columnstore replica. The columnstore replica can share the same ODP with the rowstore replica, and the database automatically selects the routing strategy.

  ![](https://obbusiness-private.oss-cn-shanghai.aliyuncs.com/doc/img/observer-enterprise/V4.6.0/ODP-OBServer.png)

For detailed steps, see [Deploy and use columnstore replicas](150.uesing-columnstore-replica.md). You can also refer to [Typical AP architecture deployment guide](50.ap-architecture-deployment-guide.md) for architecture selection.
