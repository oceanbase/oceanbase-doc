# Efficient data loading with full bypass import and partition exchange

The partition exchange feature in OceanBase Database, when combined with bypass import technology, provides an efficient solution for large-scale incremental data loading. By using full bypass import, data can be quickly loaded into a newly created non-partitioned temporary table. Then, through partition exchange, the data is atomically migrated to the target partitioned table, significantly improving the performance of incremental data import for existing partitioned tables.
...
- [Overview of bypass import](100.overview-of-bypass-import.md)