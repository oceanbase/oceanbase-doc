...
1. Log in to the sys tenant of the OceanBase cluster as the `root` user and execute the following statement to query `UNIT_CONFIG NAME` used by the current tenant.
   ...
   <main id="notice" type='explain'>
    <h4>Note</h4>
    <p> <ul><li><code>sys_unit_config</code> is the unit config of the <code>sys</code> tenant and generally does not need to be modified. </li><li>In this example, the name of the unit config  of the <code>test</code> tenant is <code>test_unit</code>. </li></ul> </p>
   </main>

2. Copy the name of the unit config and execute the following statement to scale up the memory.
...
* `freeze_trigger_percentage`: the percentage of the MemStore memory usage of a tenant. When the value specified by this parameter is reached, the system automatically triggers a minor compaction to release the occupied memory space. The value range of this parameter is [1,99]. The default value is `20`, indicating that a minor compaction is triggered when the MemStore memory usage reaches 20%.
* `memstore_limit_percentage`: the percentage of the writable MemStore memory out of the total tenant memory. The value range of this parameter is [0, 100), and the default value is `0`, which indicates that the percentage of tenant memory available for the MemStore is adaptively adjusted by the system.
...
* [ob_query_timeout](../../700.reference/800.configuration-items-and-system-variables/200.system-variable/300.global-system-variable/9000.ob_query_timeout-global.md): the query timeout period, in μs. The default value is `10000000`.


* [ob_trx_timeout](../../700.reference/800.configuration-items-and-system-variables/200.system-variable/300.global-system-variable/10300.ob_trx_timeout-global.md): the transaction timeout period, in μs. The default value is `86400000000`.


* [ob_trx_idle_timeout](../../700.reference/800.configuration-items-and-system-variables/200.system-variable/300.global-system-variable/10100.ob_trx_idle_timeout-global.md): the idle timeout period for transactions, in μs. The default value is `86400000000`.
