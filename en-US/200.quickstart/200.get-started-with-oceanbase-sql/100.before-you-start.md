...
* `memstore_limit_percentage`: the percentage of tenant memory available for MemStore writes, with a range of [0,100). The default value is `0`, indicating that the percentage of memory used for MemStore is adaptively adjusted by the system based on the tenant's total available memory.
