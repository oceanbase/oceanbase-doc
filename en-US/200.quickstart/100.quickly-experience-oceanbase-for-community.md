...
* Pull the latest image of OceanBase Database.
  ...
  <main id="notice" type='explain'>
    ...
    <ul>
    <li>
    <p>If you failed to pull the Docker image, you can also pull the image from the quay.io or ghcr.io repository by replacing <code>oceanbase/oceanbase-ce</code> in the above pull command with <code>quay.io/oceanbase/oceanbase-ce</code> or <code>ghcr.io/oceanbase/oceanbase-ce</code>. For example, execute <code>sudo docker pull quay.io/oceanbase/oceanbase-ce</code> to pull the image from quay.io.</p>
    </li>
    <li>
    <p>After replacing the repository address in the command, you need to replace the repository address in the command in Step 2 accordingly. The repositories used in both commands must remain consistent.</p>
    </li>
    <li>
    <p>By default, the above command pulls the latest version. You can select a version based on actual needs from <a href="https://hub.docker.com/r/oceanbase/oceanbase-ce/tags">Docker Hub</a>, <a href="https://quay.io/repository/oceanbase/oceanbase-ce?tab=tags">quay.io</a>, or <a href="https://github.com/oceanbase/docker-images/pkgs/container/oceanbase-ce">ghcr.io</a>.</p>
    </li>
    </ul>
  </main> 
...
* `-e` is used to set environment variables. `MODE` is used to set the deployment specifications of OceanBase Database, and `OB_TENANT_PASSWORD` is used to set the password for the root@test user in OceanBase Database.
