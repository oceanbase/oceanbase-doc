...
First, log on to your OceanBase database through the OBClient or a MySQL client.
...
Then, run the following command to start the TPC-H benchmark test, with a dataset size of 1 GB. The entire process includes dataset generation, schema import, and automatic test execution. In this example, it is assumed that your test environment is deployed in the same way as that in [Get started with OceanBase Database](../1.quickly-experience-oceanbase-for-community.md). You can modify the cluster name, password, installation directory, etc. to adapt to your situation.
