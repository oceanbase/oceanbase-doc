...
The following describes how to manually run the Transaction Processing Performance Council Benchmark H (TPC-H) test to show the characteristics of OceanBase Database in operational OLAP scenarios. TPC-H is a commonly used benchmark that measures the analysis and decision support capabilities of database systems by using a series of complex queries on massive amounts of data. For more information, visit the [official website of TPC](https://www.tpc.org/default5.asp).

<main id="notice" type='explain'>
  <h4>Note</h4>
  <p>On May 20, 2021, OceanBase Database set a new world record in running the TPC-H benchmark with a result of 15.26 million QphH. It is the only database that has set world records in both the TPC-C and TPC-H benchmarks, which demonstrates its ability to simultaneously handle online transactions and real-time analysis. For more information, see <a href="https://www.tpc.org/tpch/results/tpch_results5.asp?version=3">TPC-H Results</a>.</p>
</main>
...
2. After the download completes, unzip the package and go to the TPC-H directory.
...
Use the tools provided with TPC-H to generate query SQL statements. Follow these steps:

1. Copy `dbgen/qgen` and `dbgen/dists.dss` to the `mysql_sql` directory.

2. Create the `gen.sh` script in the `dbgen` directory to generate SQL query statements.
   ...
   Generate 22 SQL query statements in the `dbgen` directory, and then copy all the query statements to the `mysql_sql` folder.
...
1. Create the data loading script `load_data.sh`.
   ...
   ```bash
   #!/bin/bash
   host='$host_ip'   # Note: Please enter the IP address of a server where an observer, such as observer A, resides. It is recommended to place the data files on this server as well.
   port='$host_port' # The port number of observer A.
   user='$user'      # The username.
   tenant='$tenant_name'  # The name of the tenant.
   password='$password'   # The password.
   database='$db_name'    # The name of the database.
   data_path='$data_file' # Note: Please specify the path of the .tbl data files generated in the data generation step under an observer, such as observer A.

   function load_data
   {
      remote_user="$user"         # The username of the observer node that stores data.
      table_name=${1}
      if [[ ${password} == "" ]];then
      ...
      echo `date "+[%Y-%m-%d %H:%M:%S]"`"----------------------Importing data file for table ${table_name}----------------------"

      # You can import data using the direct load method, or modify it to use other methods.
...
1. Write the test script `tpch.sh`.
   ...
   ```bash
   #!/bin/bash
   host='$host_ip'   # Note: Please enter the IP address of a server hosting an observer, such as observer A.
   port='$host_port' # The port number of observer A.
   user='$user'      # The username.
   tenant='$tenant_name'  # The name of the tenant.
...
* Q: What should I do if an error occurs while importing data? Here is the error message:
...
* Q: What should I do if an error occurs while viewing data? Here is the error message:
...
## Manually try out operational OLAP

Through the preceding steps, you have obtained a TPC-H test environment. Now, try out the operational OLAP capability of OceanBase Database.
...
Run the Q1 query below to verify the query capability of OceanBase Database. The Q1 query summarizes and analyzes the prices, discounts, shipments, and quantities of various products within a specified period of time on the largest table named `lineitem`. This query will read all the data in the entire table and perform partitioning, sorting, and aggregation.

### Execute the query with parallel queries disabled (default)
...
### Execute the query with parallel queries enabled

The operational OLAP capability of OceanBase Database is based on a single set of data and the execution engine, without the need to synchronize or maintain heterogeneous data. Add a `parallel` hint to the query statement to set the DOP to 8 and execute the statement again:
...
As you can see, the query speed is increased by nearly 6 times with parallel queries. Run the `EXPLAIN` command to view the execution plan, which shows the DOP (line 18, operator 1, dop=8):
...
Apart from using the official TPC-H tools from TPC, you can also use obd to conveniently generate the dataset, create tables, import data, and automatically run the 22 SQL statements.
