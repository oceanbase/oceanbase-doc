...
2. The `t_f1.csv` file contains 8 columns of data, which correspond to different data types, generated by using a random algorithm. Therefore, before you can experience the data compression feature of OceanBase Database, you need to create a table in the tenant and import the data in the CSV file to the table.
...
3. Create a table named `t_f1` in the `test` database of the `test` tenant. For more information, see the topic on how to create a tenant.
...
```
  ...
  <main id="notice" type='notice'>
    <h4>Notice</h4>
    <p>For security reasons, you can execute the statement to modify the <code>secure_file_priv</code> parameter only through a local connection. For more information, see <a href="../../700.reference/800.configuration-items-and-system-variables/200.system-variable/300.global-system-variable/11500.secure_file_priv-global.md">secure_file_priv</a>. </p>
  </main>
...
2. Perform a major compaction.
   ...
   Log in to the `sys` tenant of the cluster as the `root` user and trigger a major compaction to compress the incremental data and baseline data together. You can also manually trigger a compaction by using the following method.
...
3. Query the state of the compaction. When the following query returns IDLE, the compaction is complete.
...
4. Query the following statement in the `sys` tenant to check the data storage usage after the data is imported to OceanBase Database.
