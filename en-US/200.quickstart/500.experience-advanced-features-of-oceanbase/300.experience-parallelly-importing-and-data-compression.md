...
In addition to analytical queries, another important part of operational online analytical processing (OLAP) lies in the parallel import of massive amounts of data, namely the batch data processing capability. The parallel execution framework of OceanBase Database allows you to execute DML statements in parallel, which is known as parallel DML (PDML) operations. It supports concurrent data writes to multiple servers in a multi-node database without compromising data consistency for large transactions. The asynchronous minor compaction mechanism enhances the support of the log-structured merge-tree (LSM-tree)-based storage engine for large transactions when memory is tight.

This topic describes how to try out the PDML feature of OceanBase Database. First, create an empty table named `lineitem2` with the same schema as that of the `lineitem` table used in running Transaction Processing Performance Council Benchmark H (TPC-H). Then, execute the `INSERT INTO ...SELECT` statement to insert 6 million rows of data of the `lineitem` table into the `lineitem2` table. In the following examples, the statement is executed with PDML disabled and enabled respectively for comparison.
...
PDML is disabled by default. After you create the `lineitem2` table, execute the statement with PDML disabled. This is a large transaction involving 6 million rows of data. Therefore, you must execute the following statements to increase the default query and transaction timeout periods (in μs) of OceanBase Database:
...
According to the return result, OceanBase Database takes 107s to insert 6 million rows of data in a single transaction when PDML is disabled.
...
1. Name the generated file and check the actual file size.
...
3. Create a table named `t_f1` in the `test` database of the `test` tenant. For more information about how to create a tenant, see topics in **Manage tenants**.
...
```
  ...
  <main id="notice" type='notice'>
    <h4>Notice</h4>
    <p>For security reasons, the SQL statement for modifying <code>secure_file_priv</code> can only be executed locally and cannot be executed remotely on OceanBase Client (OBClient). For more information, see <a href="../../700.reference/800.configuration-items-and-system-variables/200.system-variable/300.global-system-variable/11500.secure_file_priv-global.md">secure_file_priv</a></p>.
  </main>

After you complete the settings, reconnect to the session to make the settings take effect. Then, execute the following statements to set the query and transaction timeout periods for the session to ensure that the execution does not exit due to timeout:
...
OceanBase Database takes about 4 minutes to import 10 GB of data when parallel import is enabled. In this example, the tenant is configured with 16 CPU cores. You can set an appropriate degree of parallelism (DOP) based on your configurations. A higher DOP indicates faster data import.
...
3. Run the following command to query the status of the major compaction. If the result returns `IDLE`, the major compaction is completed.
...
The table is about 6.145 GB in size after the compression, achieving a compression ratio of 1.62, which is calculated by dividing the data size before compression by the data size after compression, namely 10/6.145.
