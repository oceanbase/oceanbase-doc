...
* **-u**: the username for connecting to a tenant, in the format of **username@tenant name#cluster name**. The default username of the administrator in Oracle mode is `sys`. The cluster name is not required when you directly connect to the database, but is required when you connect to the database through an ODP.
...
4. Exchange data with OceanBase Server through SQL statements, and then process the data. The execution steps of an SQL statement in an OBCI application are as follows:
   ...
   3. Call the `OCIStmtExecute()` function to execute the SQL statement.
...
/* Define parameters for binding the result set of a SELECT statement. */
...
int main(int argc, char *argv[])
{
  ...
  /* Allocate memory for the define handle. */

  OCIHandleAlloc(envhp, (dvoid **)&defhp[0], OCI_HTYPE_DEFINE, 0, 0);
  /* Allocate memory for the bind handle. */
  ...
  /* Initialize the indicator variables. */
...
* **strServerName**: the IP address and port number for connecting to OceanBase Database, which is usually the IP address of an ODP and the port number for accessing it. The value of this parameter is in the format of `IP:port`.

* **strUserName**: the username for connecting to a tenant, in the format of **username@tenant name#cluster name**. The default value for the username of the administrator in Oracle mode is `sys`. The cluster name is not required when you directly connect to the database, but is required when you connect to the database through an ODP.
...
For more information about OBCI, see the [OceanBase C API](https://en.oceanbase.com/docs/oceanbase-connector-c-en) chapter on the official website.