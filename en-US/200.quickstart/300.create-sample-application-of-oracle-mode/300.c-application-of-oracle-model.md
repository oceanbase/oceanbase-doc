...
* Contact the technical support team to obtain the RPM installation packages of OBCI and LibOBClient.

<main id="notice" type='notice'>
  <h4>Notice</h4>
  <p>Pay attention to the following points regarding the prerequisites: <ul><li>Starting from OBCI V2.0.4, the <code>include</code> folder in the RPM package no longer contains the <code>oci.h</code> and <code>ociap.h</code> header files. For business compilation, install the Oracle clntsh RPM first, and then use its header files. </li><li>OceanBase Database: V2.2.76 and later (V4.0 and later are recommended to experience all the features of the current version)</li><li>OBProxy (ODP): V4.0 and later (recommended)</li><li>libobclient: V2.1.1 and later</li><li>Install the Oracle driver package <code>oracle-instantclient</code>. Starting from OBCI V2.0.4, this package depends on the Oracle header files. </li><li>Install OBCI and deploy the GCC compiler in advance. </li></ul></p>
</main>
...
* **-u**: the username for connecting to a tenant, in the format of **username@tenant name#cluster name**. The default username for the administrator in Oracle mode is `sys`. The cluster name is not required when you directly connect to the database, but is required when you connect to the database through an ODP.

* **-p**: the user password.

* **-P**: the port for connecting to OceanBase Database, which is also the listening port of the ODP.

### Step 2: Install the C-related driver
...
* The header files are installed in the `/u01/obclient/include` path.

* The library files are installed in the `/u01/obclient/lib` path.

### Step 3: Write an application

This topic explains how a C application interacts with an OBServer node in the Oracle mode of OceanBase Database. It uses a specific example to illustrate the basic approach.

1. Initialize the OBCI environment and the thread.
...
4. Exchange data with OceanBase Server through SQL statements, and then process the data. The execution steps of an SQL statement in an OBCI application are as follows:
   ...
   2. Call the `OCIBindByPos()` or `OCIBindByName()` function to bind the addresses of input variables to the placeholders in the DML statement.
   ...
   4. Call the `OCIDefineByPos()` function to define output variables for the output items in the SQL statement.
   ...
   5. Call the `OCIStmtFetch()` function to retrieve the result set of a query.
...
6. Deallocate the handles that are allocated in the program.
...
The code in the `test.c` sample file is as follows:
...
OCIDescribe *dschp;  /* description handle*/
OCIError *errhp;     /* Error handle*/
OCIDefine *defhp[3]; /* Define the handles.*/
OCIBind *bidhp[4];   /* Binding handles*/
sb2 ind[3];          /* Indicator variables*/
/* Parameters bound to the result set of the select statement*/
int szpersonid; /* Stores the personid column*/
text szname[51];    /* store the name column*/
text szemail[51];   /*Store the mail column*/
...
* **strServerName**: the IP address and port number of the ODP that you specified using the `-h` and `-P` parameters in the format of `IP:port`.

* **strUserName**: the username for connecting to a tenant, which is in the format of **username@tenant name#cluster name**. The default username of the administrator in Oracle mode is `sys`. The cluster name is not required when you directly connect to a database, but is required when you connect to a database through ODP.

* **strPassword**: the user password, which is obtained from the `-p` parameter.
...
For more information about how to install and use OBCI, see the official documentation [OceanBase C API](https://en.oceanbase.com/docs/enterprise/obci-en).