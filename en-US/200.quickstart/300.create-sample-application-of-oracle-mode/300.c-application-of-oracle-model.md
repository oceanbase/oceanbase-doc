...
* Make sure that the basic database development environment is set up.

* Make sure that the following hardware environment is met:
...
* Contact technical support to obtain the RPM packages of OBCI and LibOBClient.

<main id="notice" type='notice'>
  <h4>Notice</h4>
  <p>Before you proceed, make sure that the following conditions are met: <ul><li>Starting from OBCI V2.0.4, the <code>include</code> folder of the RPM package no longer contains the header files such as <code>oci.h</code> and <code>ociap.h</code>. Therefore, you must install the RPM package of Oracle clntsh and use its header files for business compilation. </li><li>OceanBase Database: V2.2.76 or later (we recommend that you use OceanBase Database V4.0 or later to experience the complete features of the current version). </li><li>OBProxy: V4.0 or later. </li><li>libobclient: V2.1.1 or later. </li><li>Install the <code>oracle-instantclient</code> driver package. Starting from OBCI V2.0.4, the Oracle header files are required. </li><li>OBCI is installed and the GCC compiler is deployed. </li></ul></p>
</main>
...
The database connection string contains the parameters required to access the database. Before creating an application, you can use the database connection string to verify the login to the database and ensure that the parameters in the connection string are correct.

Parameter description:

* **-h**: the IP address for connecting to OceanBase Database. It is sometimes the IP address of an ODP.

* **-u**: the username for connecting to the tenant. The format is **username@tenant name#cluster name**. In Oracle mode, the default username of the administrator is `sys`. If you connect to the database directly, you do not need to specify the cluster name. If you connect to the database through an ODP, you must specify the cluster name.
...
After you obtain the RPM package, run the following command in the CLI as the root user to install the OBCI driver:
...
This topic provides an example of how to use OBCI to interact with an OBServer node in Oracle mode of OceanBase Database.

1. Initialize the OBCI environment and threads.

   ```c
   /*Initialize the OBCI program environment*/
...
3. Establish a connection with the database and create a user session.
...
4. Exchange data with OceanBase Database by using SQL statements, and then process the data. The execution steps of an SQL statement in an OBCI application are as follows:
...
The sample file `test.c` contains the following code:
...
/* Bind parameters for the select result set */
...
Modify the database connection parameters in the code. For more information, see the following fields, and set the values to the database connection string obtained in step 1.
...
* **strServerName**: the IP address and port number of the OceanBase Database server. The IP address is usually an ODP address, and the port number is the port used for access.

* **strUserName**: The username for connecting to the tenant, specified by the `-u` option. The format is **username@tenant name#cluster name**. In Oracle mode, the default username for the administrator is `sys`. When connecting directly to the database, do not specify the cluster name. When connecting through ODP, you must specify the cluster name.

* **strPassword**: The password for the user, specified by the `-p` option.
...
After the compilation is completed, run the application. If the following result is returned, the database connection is successful and the statement is executed normally:
