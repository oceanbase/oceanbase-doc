...
You must configure labels for Kubernetes nodes. The labels must match the value of `nodeSelector` in the `obcluster.yaml` file. ob-operator will schedule pods to nodes with corresponding labels.
...
Input the Grafana address in your browser and press **Enter**. On the page that appears, input the username `admin` and default password `admin` to log on. When you log on for the first time, you will be prompted to change the password.
