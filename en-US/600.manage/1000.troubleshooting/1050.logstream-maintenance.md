...
After you execute a manual operation on a log stream, such as `ALTER SYSTEM CREATE LS` or `ALTER SYSTEM MODIFY LS`, the system may return an error message. This error message describes the reason why the operation failed. This topic describes the causes of the error and the solutions.

## Error message 1: The tenant status is not NORMAL. CREATE LS is not allowed

### Cause

The tenant status is not `NORMAL`, and you cannot create a log stream.

### Solution

Query the `DBA_OB_TENANTS` view to confirm the tenant status. If the tenant status is `CREATING`, wait until the tenant is created and then create a log stream.

## Error message 2: The tenant's switchover status is not normal, CREATE LS is not allowed

### Cause

The `SWITCHOVER_STATUS` of the tenant is not `NORMAL`, and you cannot create a log stream.
...
Confirm whether the tenant exists.

## Error message 4: Access denied

### Cause

You cannot specify a tenant name that is inconsistent with your own name in the user tenant.
...
You can grant the `ALTER SYSTEM` privilege to the current user by using the GRANT statement. For more information about user authorization, see [Directly grant privileges (MySQL mode)](../500.security-and-permissions/300.access-control/200.user-and-permission/200.permission-of-mysql-mode/200.authority-of-mysql-mode.md) and [Directly grant privileges (Oracle mode)](../500.security-and-permissions/300.access-control/200.user-and-permission/300.permission-of-oracle-mode/200.authority-of-oracle-mode.md).
...
You cannot create a log stream on a non-existent `UNIT_GROUP` or a `UNIT_GROUP` with `UNIT_GROUP_ID` of 0.
