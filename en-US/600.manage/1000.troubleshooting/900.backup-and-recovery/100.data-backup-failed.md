...
2. Execute the following statement to identify the cause of the failure:
   ...
   ```shell
   obclient(root@sys)[oceanbase]> SELECT * FROM oceanbase.DBA_OB_ROOTSERVICE_EVENT_HISTORY WHERE module='backup_data' AND event ='start_backup_data';
   ```
...
2. Execute the following statement to confirm the state at which the data backup task is stuck:
   ...
   ```shell
   obclient(root@sys)[oceanbase]> SELECT * FROM oceanbase.CDB_OB_BACKUP_TASKS WHERE tenant_id = xxx;
   ```
...
3. Execute the following statement to obtain task execution information:
   ...
   ```shell
   obclient(root@sys)[oceanbase]> SELECT * FROM oceanbase.__all_virtual_backup_schedule_task WHERE tenant_id = xxx;
   ```
