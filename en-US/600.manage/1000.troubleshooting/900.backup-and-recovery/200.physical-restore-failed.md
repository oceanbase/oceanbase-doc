...
2. Execute the following statement to check the error code:
   ...
   ```shell
   obclient(root@sys)[oceanbase]> SELECT * FROM oceanbase.DBA_OB_ROOTSERVICE_EVENT_HISTORY WHERE module='physical_restore';
   ```
...
2. Execute the following statement to query the status of the restore job:
   ...
   ```shell
   obclient(root@sys)[oceanbase]> SELECT * FROM oceanbase.DBA_OB_ROOTSERVICE_EVENT_HISTORY WHERE event LIKE 'change_restore_status' AND value1 = $job_id;
   ```
...
3. Execute the following statement to check the restore status of the log stream replicas of the tenant to be restored:
   ...
   ```shell
   obclient(root@sys)[oceanbase]> SELECT ls_id,svr_ip,svr_port,role,restore_status,zone FROM oceanbase.__all_virtual_ls_meta_table  WHERE tenant_id = xxxx;
   ```
...
4. If the `restore_status` value of a log stream replica is `6` or `8`, the log stream is being used for log restore or for restore of minor-compacted data. In this case, check whether the issue is caused by log restore. 
   ...
   1. Execute the following statement to check whether logs have been restored from the archive directory to the target tenant:
      ...
      ```shell
      obclient(root@sys)[oceanbase]> SELECT count(1) FROM oceanbase.GV$OB_LOG_STAT WHERE tenant_id = xxxx AND end_scn < (SELECT recovery_until_scn FROM oceanbase.DBA_OB_TENANTS WHERE tenant_id = xxxx);
      ```
   ...
   2. Verify whether log replay is completed.
      ...
      ```shell
      obclient(root@sys)[oceanbase]> SELECT * FROM oceanbase.__all_virtual_replay_stat WHERE tenant_id = xxxx;
      ```
      ...
      * `unsubmitted_log_scn`: the SCN of the next log entry waiting to be submitted to the replay engine. If log replay is completed, the value is the SCN of the last log entry replayed plus 1.
      ...
      ```shell
      obclient(root@sys)[oceanbase]> SELECT recovery_until_scn FROM oceanbase.DBA_OB_TENANTS WHERE tenant_id = xxxx;
      ```
   ...
   3. Verify whether log stream 1 has been restored.
      ...
      ```shell
      obclient(root@sys)[oceanbase]> SELECT * FROM oceanbase.CDB_OB_LS WHERE tenant_id = xxxx;
      ```
...
2. Query the `GV$OB_SERVER_SCHEMA_INFO` view for the schema refresh progress of the tenant.
   ...
   ```shell
   obclient(root@sys)[oceanbase]> SELECT * FROM oceanbase.GV$OB_SERVER_SCHEMA_INFO WHERE tenant_id=xxxx;
   ```
...
2. Query the `CDB_OB_RESTORE_HISTORY` view for information in the `comment` column.
   ...
   ```shell
   obclient(root@sys)[oceanbase]> SELECT * FROM oceanbase.CDB_OB_RESTORE_HISTORY WHERE TENANT_ID=xxxx;
   ```
