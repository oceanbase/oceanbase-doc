...
## Issue 1: The restore statement fails to be executed
...
2. Execute the following statement to query the error code:
...
3. Log in to the server where RootService resides based on the `RS_SVR_IP` value obtained in the previous step. Search the `rootservice.log` file to find the related error information.
   ...
   3. Run the following command to search for log records based on the queried error code and find the related error information.
      ...
      The common error message for a command execution failure is `-4018 no enough log for restore`. This issue is usually caused by an incorrect restore end point specified when you execute the `ALTER SYSTEM RESTORE` statement. The specified restore end point may not be within the restorable window. You must confirm whether any backup set and archive logs are available before the restore end point. For more information about the restorable window, see the **Constraint for specifying the timestamp and SCN** section in the [Parameters related to physical restore](../../600.backup-and-recovery/600.restore-data/800.parameters-of-the-restore.md) topic.
...
3. Execute the following statement to query the restore status of the log stream replicas of the tenant to be restored:
...
4. If the `restore_status` value of a log stream replica is `6` or `8`, the log stream is being used for log restore or for restore of minor-compacted data. In this case, check whether the issue is caused by log restore.
   ...
   <main id="notice" type='notice'>
   <h4>Notice</h4>
   <p>In OceanBase Database V4.1.0 and later, all logs are restored in a synchronized manner across log streams. If any log stream is stuck in a state earlier than <code>6</code> or <code>8</code>, all other log streams will be stuck in the <code>6</code> state. </p>
   </main>
   ...
   1. Execute the following statement to check whether logs have been restored from the archive directory to the target tenant:
      ...
      * `end_scn`: the maximum SCN for log consumption.

      * `recovery_until_scn`: the end SCN for restore of the tenant.
...
2. Query the `GV$OB_SERVER_SCHEMA_INFO` view for the schema refresh progress of the tenant.
    ...
    * The value of `REFRESHED_SCHEMA_VERSION` equals that of `RECEIVED_SCHEMA_VERSION`.
