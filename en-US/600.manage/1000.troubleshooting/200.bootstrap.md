...
Correctly specify `log_disk_percentage` or `log_disk_size`. OceanBase Database V4.0 supports specifying these parameters only at startup (using the `-o` option) and does not support dynamic modification. For example, you can specify the following option when you start an OBServer node: `-o log_disk_percentage=10`.