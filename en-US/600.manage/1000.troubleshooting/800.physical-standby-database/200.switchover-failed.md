...
* Network-based Physical Standby Database scenario
   ...
   2. Log on to the `sys` tenant of the cluster where the standby tenant resides and check the restore status of the standby tenant.
...
1. Obtain information about the tenant of the log restore source.
   ...
   1. Log on to the current standby tenant or to the `sys` tenant of the cluster where the standby tenant resides.
