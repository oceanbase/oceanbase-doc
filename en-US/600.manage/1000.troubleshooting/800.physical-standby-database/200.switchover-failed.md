...
OceanBase Database may return an error message for the execution of a switchover or failover statement, such as `SWITCHOVER TO STANDBY VERIFY`, `SWITCHOVER TO STANDBY`, `SWITCHOVER TO PRIMARY VERIFY`, `SWITCHOVER TO PRIMARY`, `ACTIVATE STANDBY VERIFY`, or `ACTIVATE STANDBY`. The returned error message shows the reason why the switchover or failover statement cannot be successfully executed. This topic describes how to troubleshoot these switchover and failover issues.
...
1. Log in to the cluster where the archive source or restore source is located and query the status of the tenant of the archive source or restore source.
    ...
    You can execute the following statement from the `sys` tenant to query the status of the tenant of the archive source or restore source:
    ...
    You can execute the following statement from a MySQL tenant that serves as the tenant of the archive source or restore source to query its status:

    ```sql
    obclient [oceanbase]> SELECT TENANT_NAME, TENANT_ID, TENANT_ROLE, STATUS, SWITCHOVER_STATUS FROM oceanbase.DBA_OB_TENANTS;
    ```

    You can execute the following statement from an Oracle tenant that serves as the tenant of the archive source or restore source to query its status:
...
1. Query the status of the tenant.
    ...
    You can execute the following statement from the `sys` tenant to query the status of the specified tenant:

    ```sql
    obclient [oceanbase]> SELECT TENANT_NAME, STATUS FROM oceanbase.DBA_OB_TENANTS WHERE TENANT_NAME = 'tenant_name';
    ```

    Replace `tenant_name` with the name of the tenant on which the switchover statement is executed.

    tab User tenant

    You can execute the following statement from a MySQL tenant to query its status:

    ```sql
    obclient [oceanbase]> SELECT TENANT_NAME, STATUS FROM oceanbase.DBA_OB_TENANTS;
    ```

    You can execute the following statement from an Oracle tenant to query its status:
...
1. Query the `SWITCHOVER_STATUS` value of the current tenant.
    ...
    You can execute the following statement from the `sys` tenant to query the `SWITCHOVER_STATUS` value of the specified tenant:
    ...
    You can execute the following statement from a MySQL tenant to query its `SWITCHOVER_STATUS` value:

    ```sql
    obclient [oceanbase]> SELECT TENANT_NAME, SWITCHOVER_STATUS FROM oceanbase.DBA_OB_TENANTS;
    ```

    You can execute the following statement from an Oracle tenant to query its `SWITCHOVER_STATUS` value:
    ...
    For more information about the values of the `SWITCHOVER_STATUS` field, see [DBA_OB_TENANTS](../../../700.reference/700.system-views/300.system-view-of-sys-tenant/200.dictionary-view-of-sys-tenant/22200.o-dba_ob_tenants-of-sys-tenant.md).
...
1. Query the type of the current tenant.

    :::tab
    tab sys tenant

    You can execute the following statement from the `sys` tenant to query the type of the specified tenant:
    ...
    You can execute the following statement from a MySQL tenant to query its type:

    ```sql
    obclient [oceanbase]> SELECT TENANT_NAME, TENANT_TYPE FROM oceanbase.DBA_OB_TENANTS;
    ```

    You can execute the following statement from an Oracle tenant to query its type:
...
You can perform a switchover only on the current user tenant. You cannot specify a tenant name in the switchover statement. If you specify a tenant name in the switchover statement, this error is reported.

### Troubleshooting procedure

Remove `TENANT tenant_name` from the statement and retry the switchover.
...
1. Log in to the `sys` tenant of the cluster where the current tenant resides, execute the following statement to query the list of log streams that do not have a leader:
...
3. Query the `server_permanent_offline_time` value of each temporarily offline OBServer node, and determine the time when the OBServer node goes permanently offline based on the obtained value and the `LAST_OFFLINE_TIME` value.
    ...
    The `server_permanent_offline_time` parameter specifies the time threshold for heartbeat missing. When the heartbeat of an OBServer node is missing for the specified period of time, the OBServer node is considered permanently offline. Data replicas on a permanently offline OBServer node must be automatically supplemented.

    ```sql
    obclient [oceanbase]> SHOW PARAMETERS LIKE 'server_permanent_offline_time';
    ```

    In the query result, if the sum of `LAST_OFFLINE_TIME` + `server_permanent_offline_time` values is greater than the value returned by the `NOW()` function, the OBServer node is permanently offline. Otherwise, the OBServer node is temporarily offline.
...
2. Check the information of the permanently offline OBServer nodes in the units for the tenant.
    ...
    3. If the sum of the `LAST_OFFLINE_TIME` + `server_permanent_offline_time` values is greater than the value returned by the `NOW()` function, the OBServer node is permanently offline.
...
<h4>Notice</h4>
<p><code>-4012</code> in <code>original error code: -4012</code> indicates only one of the possible error codes. The error message may contain other error codes. </p>
</main>
...
2. Check the value of `ACCESS_MODE` for all log streams of the tenant.
    ...
    In the query result, if the `ACCESS_MODE` value of a log stream is not `RAW_WRITE`, the log stream may be waiting for garbage collection (GC). After GC is performed, retry the switchover to `PRIMARY` on the standby tenant. If the retry still fails, contact OceanBase Technical Support.
...
1. Query the information about the tenant of the log restore source.

   1. Log in to the current standby tenant or to the `sys` tenant of the cluster where the standby tenant resides.

   2. Execute the following statement to query the information about the tenant of the log restore source.
...
2. Check the network connectivity of the OBServer node where the tenant of the log restore source resides or manually start the observer process on it. Then, retry the switchover to `PRIMARY` on the standby tenant. If the retry still fails, contact OceanBase Technical Support.
...
1. Query the information about the tenant of the log restore source.
   ...
   2. Execute the following statement to query the information about the tenant of the log restore source.
    ...
    In the query result, the `USER=xxx` attribute of the `VALUE` field displays the information about the tenant of the log restore source. In this example, `mytest` is the dedicated username specified to connect to the log restore source, and `backup_mysql_tenant` is the name of the tenant of the log restore source.
...
4. After the corresponding privileges are granted, retry the switchover to `PRIMARY` on the standby tenant. If the retry still fails, contact OceanBase Technical Support.
...
If the preceding error message is returned for a switchover to `PRIMARY` on a standby tenant in a network-based Physical Standby Database scenario, with an error code other than any of the error codes mentioned in this topic, use the following troubleshooting procedure:
...
2. Check whether all OBServer nodes of the restore source tenant can provide services.
   ...
   If an OBServer node fails to provide services, wait a while until the OBServer node resumes services. Then, retry the switchover to `PRIMARY` on the standby tenant. If the retry still fails, contact OceanBase Technical Support.
...
For a switchover to `PRIMARY` on a standby tenant in a network-based Physical Standby Database scenario, the user account for connecting to the log restore source (the dedicated user specified for the log restore source) must not be locked. If the user account is locked, this error is reported.
...
5. After the user is unlocked, retry the switchover to `PRIMARY` on the standby tenant. If the retry still fails, contact OceanBase Technical Support.
...
For a switchover to `PRIMARY` on a standby tenant in a network-based Physical Standby Database scenario, the tenant of the log restore source must not be locked. If the tenant of the log restore source is locked, this error is reported.
...
5. After the tenant is unlocked, retry the switchover to `PRIMARY` on the standby tenant. If the retry still fails, contact OceanBase Technical Support.
