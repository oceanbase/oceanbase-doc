...
<main id="notice" type='notice'>
    <h4>Notice</h4>
    <p>The methods for viewing and setting the log level apply only to the <code>election.log</code>, <code>observer.log</code>, and <code>rootservice.log</code> files. Logs at or above the WARN level are recorded only in log files with the `.wf` extension. </p>
</main>
...
* The log generation level set for the current session takes effect for the OBServer node where the current tenant is located.
    ...
    * A global variable does not take effect for the current session and takes effect only for sessions established upon re-login.
