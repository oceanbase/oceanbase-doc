...
OceanBase Database allows you to search for logs related to specific SQL requests. Depending on the current syslog level and the specific SQL exception scenario, you may not always find the corresponding SQL request logs in the log files, but it is always worth trying.
