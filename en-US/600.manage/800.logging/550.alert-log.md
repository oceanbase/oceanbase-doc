...
A single alert log file cannot exceed 256 MB in size. When the size of a log file reaches 256 MB, the system performs log rotation. During log rotation, the system appends the generation timestamp of the last log entry in the file to the original file name in the `yyyyMMddHHmmss` format, for example, `alert.log.20220304102928236`.
