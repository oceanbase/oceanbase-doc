...
Alert logs are stored in CSV files. Multiple log fields are separated with vertical bars (`|`). Log content is enclosed in double quotation marks (`""`).
...
| level | The log level, such as `INFO`, `WARN`, and `ERROR`. | INFO |
| module | The module to which the log belongs, such as `STORAGE`, `SQL`, and `TRANS`. | STORAGE |
