...
OceanBase Database has five types of log files: `observer.log`, `election.log`, `rootservice.log`, `trace.log`, and `alert.log`. By default, these log files record logs at or above the INFO level.

A WARNING log file with the `.wf` file name extension is automatically generated for each type of log file other than the `trace.log` and `alert.log` files: `observer.log.wf`, `election.log.wf`, and `rootservice.log.wf`. These WARNING log files record logs at or above the WARN level. The cluster parameter `enable_syslog_wf` specifies whether to generate WARNING log files.
...
| End-to-end tracing logs: `trace.log` | `$work_dir/log` on the OBServer node |
| Alert logs: `alert.log` | `$work_dir/log/alert` on the OBServer node |
...
* Fields in the header are as follows:
  * time: the time when the log was recorded.
  * log_level: the level of the log. Supported log levels are ERROR, USER_ERR, WARN, INFO, TRACE, and DEBUG in descending order.
  ...
  * trace_id: the unique ID for tracing a task. The trace_id is passed from one OBServer node to another by using remote procedure calls (RPCs), so that all log data of a task can be obtained based on the trace_id. It corresponds to the `TRACE_ID` field in the `GV$OB_SQL_AUDIT` view.
  ...
  * dropped_msg_count: The value is in the `dc=xx` format, where `xx` indicates the number of logs discarded between the generation time of the current log and the previous log. If no logs are discarded, the value is `dc=0`.

* Fields in the message body are as follows:
