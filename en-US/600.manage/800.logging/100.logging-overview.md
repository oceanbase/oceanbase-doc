...
Except for the `trace.log` and `alert.log` files, each type of log file automatically generates a WARNING log file with a `.wf` suffix (`observer.log.wf`, `election.log.wf`, `rootservice.log.wf`), which only records logs of WARN level and above. The generation of WARNING log files is controlled by the cluster parameter `enable_syslog_wf`.  
