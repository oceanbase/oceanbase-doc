...
3. After the modification succeeds, run the following statements to verify the configuration.
...
| Full direct load                   | Full direct load currently only supports first importing as rowstore to the columnstore replica, then converting to columnstore after the background row-to-column conversion task is scheduled. |
...
In the sys tenant, the following example queries the row-to-column conversion progress of tablets in columnstore replica log streams across all tenants:
...
From the first row of the query result, the columnstore replica of tenant 1004 is on server `xx.xxx.xxx.212` with port `63000`; the columnstore replica log stream ID is `1001`; the total number of partitions to convert to columnstore is `1019`, and the available partition count is `1019`; the total number of baseline macro blocks is `10706`, and the available count is `10706`. You can use `AVAILABLE_TABLET_CNT / TOTAL_TABLET_CNT` or `AVAILABLE_MACRO_BLOCK_CNT / TOTAL_MACRO_BLOCK_CNT` to roughly estimate the row-to-column conversion progress. A log stream is fully available only when all tablets on it are available.
