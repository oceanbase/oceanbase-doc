...
4. Query the replica distribution of each log stream.
   ...
   The following example queries the replica distribution and the value of the `paxos_replica_num` parameter of log stream `1002`.
    ...
    * User tenant
      ...
      The query result shows that log stream `1002` has four full-featured replicas but its `PAXOS_REPLICA_NUMBER` value is `5`. You must change the value of the `PAXOS_REPLICA_NUMBER` parameter to `4`.
...
5. Execute the `ALTER SYSTEM MODIFY` statement to modify the `paxos_replica_num` parameter.
   ...
   * `ls_id`: the ID of the log stream for which you want to modify the parameter.

   * `paxos_replica_num`: the number of Paxos replicas of the log stream, which is the number of full-featured replicas specified in the locality of the tenant.

      When you specify the `paxos_replica_num` parameter, take note of the following considerations:

      * After you modify the value of the `paxos_replica_num` parameter, both the Paxos replicas and the members in the `MEMBER_LIST` field of the leader meet the majority principle.

      * The value of the `paxos_replica_num` parameter must be greater than or equal to the number of members in the `MEMBER_LIST` of the leader. You can query the `MEMBER_LIST` field from the `CDB_OB_LS_LOCATIONS` view in the `sys` tenant or from the `DBA_OB_LS_LOCATIONS` view in a user tenant.
      ...
      * You can only decrease the value of the `paxos_replica_num` parameter, and the decrease cannot exceed 1. In this example, you can only change the value to `4`.
      ...
      If any one of the preceding conditions is not met, the system returns an error.
...
6. Query the replica distribution of the log stream again.
    ...
    * User tenant
      ...
      The query result shows that the value of the `PAXOS_REPLICA_NUMBER` parameter of log stream `1002` is changed to `4`.
