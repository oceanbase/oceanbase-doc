...
You can convert a full-featured (F) replica of a log stream to a read-only (R) replica or a read-only replica to a full-featured replica based on business needs.

## Considerations

Currently, only read-only and full-featured replicas support mutual conversion of replica types. Columnstore replicas do not support replica type conversion.

## Change the replica type by modifying locality
