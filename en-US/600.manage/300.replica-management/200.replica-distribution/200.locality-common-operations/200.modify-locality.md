...
* Reduce the number of replicas for a tenant:
   ...
   You can reduce the number of replicas for a tenant in the cluster. Only one replica can be removed at a time. For example, if you want to reduce the number of replicas for a tenant from 3 to 1, you must change the locality first from `F@z1,F@z2,F@z3` to `F@z1,F@z2`, and then from `F@z1,F@z2` to `F@z1`. For more information, see [Remove replicas](../200.locality-common-operations/400.reduce-replica.md).

   <main id="notice" type='notice'>
     <h4>Notice</h4>
     <p>You can directly reduce the number of replicas for a tenant from 5 to 3, without the need to first change to 4 replicas and then to 3 replicas. </p>
   </main>
