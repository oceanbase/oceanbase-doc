...
This topic describes how to migrate replicas. After you migrate replicas within a zone, the locations of the replicas change, but the number of replicas and the types of replicas in the zone remain unchanged. This feature is applicable to OBServer node failure and load balancing.
...
### Adjust the maximum time to wait for triggering unit GC (optional)

In OceanBase Database, after all units of a tenant on an OBServer node are migrated to another OBServer node, the system automatically removes the tenant from the source OBServer node and kills all sessions in the tenant. This may affect ongoing query operations. To solve this issue, OceanBase Database allows you to enable smooth garbage collection (GC) for units. When a unit is migrated, the system starts countdown for GC, during which no new connections are received, and the system waits for active requests to complete before performing GC.

However, if long-running tasks prevent GC from being performed, OceanBase Database allows you to use the cluster-level parameter `unit_gc_wait_time` to configure the maximum wait time before GC is forced. You can adjust the value of the [unit_gc_wait_time](../../../../700.reference/800.configuration-items-and-system-variables/100.system-configuration-items/300.cluster-level-configuration-items/24830.unit_gc_wait_time.md) parameter based on your business needs. The default value is 1 minute.

You can perform the following steps to adjust the parameter:
...
2. Query the value of the `unit_gc_wait_time` parameter.
...
* If you need to migrate multiple replicas at the same time, you must set the value of the `replica_parallel_migration_mode` parameter to `on` in the primary tenant, or set the value of the `replica_parallel_migration_mode` parameter to `on` or `auto` in the standby tenant. For more information about the tenant-level parameter `replica_parallel_migration_mode`, see [replica_parallel_migration_mode](../../../../700.reference/800.configuration-items-and-system-variables/100.system-configuration-items/400.tenant-level-configuration-items/7820.replica_parallel_migration_mode.md).
...
2. Query the tenant ID of the tenant.
...
3. Query all log streams of the tenant.
    ...
    * User tenant
      ...
      The query result shows that the `tenant1` tenant has log streams whose IDs are respectively `1`, `1001`, and `1002`, as well as a broadcast log stream whose ID is `1003`.
...
6. Execute the `ALTER SYSTEM MIGRATE REPLICA` statement to migrate the replica.
   ...
   * `svr_ip:svr_port`: the IP address and port number of the OBServer node from which you want to migrate the replica, such as `100.xx.xxx.003:5073`.

   * `destination_ip:destination_port`: the IP address and port number of the OBServer node to which you want to migrate the replica, such as `100.xx.xxx.002:5072`.

   * `data_source`: the data source of the replica that you want to migrate. You can select any other replica of the log stream as the data source based on your business needs, such as `100.xx.xxx.004:5074`.

      If the specified data source is unavailable, the system returns an error. If this parameter is not specified, the system automatically selects an available data source.
