...
After a unit migration is completed in OceanBase Database, the system automatically deletes the tenant on the source server and kills all sessions on the tenant. This may affect ongoing query operations. To resolve this issue, OceanBase Database implements a smooth garbage collection (GC) feature for units. According to this feature, once a unit migration is completed, a countdown for the GC of the unit starts. During the countdown, the system does not accept new connection requests. If there are ongoing requests, the system waits for the completion of these requests before performing the GC.

However, if a long-executing task is encountered during a smooth unit GC, the unit may not be able to be GCed for a long time. To ensure smooth unit GC, OceanBase Database uses the cluster-level parameter `unit_gc_wait_time` to control the maximum wait time for a smooth unit GC. After the maximum wait time elapses, the system automatically performs the GC. The default value of the [unit_gc_wait_time](../../../../700.reference/800.configuration-items-and-system-variables/100.system-configuration-items/300.cluster-level-configuration-items/24320.unit_gc_wait_time.md) parameter is 1 minute. You can adjust it according to your business needs.
