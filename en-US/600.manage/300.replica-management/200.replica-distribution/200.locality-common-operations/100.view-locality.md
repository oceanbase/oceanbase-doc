...
3. Query the types and distribution of replicas for tenants from the `oceanbase.DBA_OB_TENANTS` view.
   ...
   <main id="notice" type='explain'>
     <h4>Note</h4>
     <p>The locality <code>FULL{1}@sa128_obv4_1, FULL{1}@sa128_obv4_2</code> is merely an example. In practice, the majority principle must be met after you add zones. </p>
   </main>
...
For example, the locality for the `mq_t1` tenant is `'FULL{1}@sa128_obv4_1, FULL{1}@sa128_obv4_2'`, which indicates that the tenant has a full-featured replica in both of zones `sa128_obv4_1` and `sa128_obv4_2`.
