...
* Columnar replicas
  ...
  The name of a columnstore replica is COLUMNSTORE, abbreviated as C. A columnstore replica is the same as a primary replica in a log stream. In a columnstore replica, all baseline data of user tables are stored in columnar format. This columnstore replica does not include replicated tables, indexes, internal tables, and system tables. For example, if a user creates a rowstore table in replica F, then the table is stored in columnar format in the machine that runs the C replica. Similar to the read-only replica R, the C replica does not participate in leader election or log voting, and it contains complete SSTables, clogs, and MemTables.
