...
The high service availability solutions include the following: intra-cluster multi-replica disaster recovery, arbitration-based disaster recover, and inter-cluster physical standby database disaster recovery.
...
This solution is designed for multiple clusters, enabling the transmission of transaction logs among multiple clusters and providing log-based physical hot backup services. In OceanBase Database V4.2.0 version, the physical standby database solution adopts an independent primary/standby architecture. The primary/standby relationship exists at the tenant level. The primary and standby communicate directly through the network or through a transmission channel established using third-party log services, transmitting only logs. Unlike the centralized architecture in earlier versions, under the independent primary/standby architecture, each cluster is independent from the others, and users can manage clusters more flexibly.
