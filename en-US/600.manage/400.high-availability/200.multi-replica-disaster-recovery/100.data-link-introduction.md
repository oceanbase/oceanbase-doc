...
![Data link diagram 1](https://obbusiness-private.oss-cn-shanghai.aliyuncs.com/doc/img/observer-enterprise/V4.2.1/manage/odp-data-link1.jpg)
...
As described above, the application layer sends user requests to the proxy service ODP. After ODP routes the requests, they are sent to the OBServer nodes that actually serve the data. The execution results are returned to the application layer through the reverse path. Different components on the entire data link achieve high availability in different ways.
