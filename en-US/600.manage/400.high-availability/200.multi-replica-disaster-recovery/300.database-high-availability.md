...
Parameters related to OBServer heartbeat status are as follows:

* `lease_time`

   The cluster-level parameter `lease_time` specifies the heartbeat lease duration. The default value is 10s. For more information about this parameter, see [lease_time](../../../700.reference/800.configuration-items-and-system-variables/100.system-configuration-items/300.cluster-level-configuration-items/11400.lease_time.md).
...
2. Obtain the zone, server, and log stream replica information of tenants.
...
3. Execute the following command to switch the leader.
   ...
   * `role`: Replica role. Valid values are `LEADER` and `FOLLOWER`.

   * `ls_id`: ID of the log stream replica to be modified.

   * `ip:port`: IP address and RPC port of the server where the log stream replica to be modified resides.

   * `tenant_name`: Tenant to which the log stream replica to be modified belongs.
...
4. After the operation is completed, you can query the view again to confirm whether the replica roles were modified successfully.
...
Manual leader switchover applies to IDC-level and city-wide disaster recovery scenarios. You can switch the leader to a specified zone or region to better align with the distribution of application traffic.
