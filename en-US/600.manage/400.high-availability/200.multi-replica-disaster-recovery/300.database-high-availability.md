...
* `lease_time`
  ...
  This parameter specifies the heartbeat lease duration. The default value is `10s`.
...
OceanBase Database Proxy (ODP) monitors abnormal states of OBServer nodes, for example, the `stopped` or `INACTIVE` state, to avoid routing application traffic to abnormal OBServer nodes. An OBServer node in the `stopped` state is still in the `ACTIVE` state, but the value of `stop_time` is greater than 0.

If an OBServer node is unstable, it may fail to provide services from time to time and may have various abnormal conditions, such as slow response. If a client connects to this OBServer node, the client may experience unstable database services. OceanBase Database provides the `STOP SERVER` statement for isolating OBServer nodes. An isolated OBServer node does not provide external services, and ODP does not route requests to the OBServer node. Therefore, the OBServer node can be diagnosed, replaced, or maintained without affecting the business. This mechanism facilitates O&M and emergency response. The `STOP SERVER` statement checks whether all the remaining replicas meet the majority requirement and whether logs are synchronized. If the conditions are not met, the `STOP SERVER` statement returns a failure. If the conditions are met, the `STOP SERVER` statement returns success after all the leaders on the failed OBServer node are switched to normal OBServer nodes. After the `STOP SERVER` statement returns success, you can perform any O&M operations, for example, kill observer processes. For more information about the `STOP SERVER` statement, see [Isolate a node](../../100.cluster-management/300.common-cluster-operations/600.isolation-a-node.md).
...
2. Query the zones, OBServer nodes, and log stream replicas of tenants.
   ...
   For more information about the `CDB_OB_LS_LOCATIONS` view, see [CDB_OB_LS_LOCATIONS](../../../700.reference/700.system-views/300.system-view-of-sys-tenant/200.dictionary-view-of-sys-tenant/9400.o-cdb_ob_ls_locations-of-sys-tenant.md).
...
3. Perform leader switchover.
   ...
   * `role` specifies the role of the replica. Valid values are `LEADER` and `FOLLOWER`.

   * `ls_id` specifies the ID of the log stream replica to be modified.

   * `ip:port` specifies the IP address and remote procedure call (RPC) port of the server where the log stream replica to be modified resides.

   * `tenant_name` specifies the tenant to which the log stream replica to be modified belongs.
