...
2. Enable data backup for the primary tenant or source standby tenant and perform a full backup.
   ...
   3. Check the data backup status.
      ...
      * Oracle-compatible tenant
      ...
      In the query result, when the value of `STATUS` is `COMPLETED`, the data backup is completed.
...
3. Create the resource pool required for the standby tenant.
   ...
   When creating a resource pool for the standby tenant, we recommend that you keep it homogeneous with the source tenant, that is, use the same number of `unit_num` as the source tenant.
...
4. Execute the `RESTORE` command to restore the standby tenant by using the data backups and archive logs of the primary tenant or source standby tenant.
   ...
   * `WITH KEY FROM 'backup_key_path' ENCRYPTED BY 'password'` indicates the backup information of the key for encrypting the tenant. You need to specify the key backup information for a tenant restore only when the tenant is configured with transparent encryption.
   ...
   The following example restores the standby tenant `standby_tenant` from the log archive path `file:///data/1/sh_archive` and data backup path `file:///data/1/sh_databackup`:
...
5. Verify that the restore is completed.
   ...
   For more information about how to view the restore progress, see [View the restore progress](../../../600.backup-and-recovery/600.restore-data/400.view-the-restore-progress.md).
