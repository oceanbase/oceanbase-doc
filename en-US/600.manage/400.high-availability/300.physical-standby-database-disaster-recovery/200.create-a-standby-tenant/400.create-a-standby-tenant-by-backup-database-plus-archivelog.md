...
2. Enable archivelog for the primary tenant.
   ...
   1. Specify the archiving destination.
      ...
      * Execute the following statement in the `sys` tenant of the cluster where the primary tenant resides to specify the archiving destination for the primary tenant:
        ...
        ```shell
        obclient(root@sys)[(none)]> ALTER SYSTEM SET LOG_ARCHIVE_DEST = 'LOCATION=file:///data/1/sh_archive' TENANT = mysql;
        ```
      ...
      * Execute the following statement in the primary tenant to specify the archiving destination for the primary tenant:
        ...
        ```shell
        obclient> ALTER SYSTEM SET LOG_ARCHIVE_DEST = 'LOCATION=file:///data/1/sh_archive';
        ```
   ...
   3. Verify that the archiving status is `DOING`.
      ...
      * MySQL tenant
        ...
        ```shell
        obclient(root@mysql)[(none)]> SELECT DEST_ID, ROUND_ID, DEST_NO, STATUS, CHECKPOINT_SCN, CHECKPOINT_SCN_DISPLAY, PATH FROM oceanbase.DBA_OB_ARCHIVELOG;
        ```

      * Oracle tenant

        ```sql
        obclient(sys@oracle001)[SYS]> SELECT DEST_ID, ROUND_ID, DEST_NO, STATUS, CHECKPOINT_SCN, CHECKPOINT_SCN_DISPLAY, PATH FROM SYS.DBA_OB_ARCHIVELOG;
        ```
...
3. Perform a data backup for the primary tenant by using the BACKUP DATABASE PLUS ARCHIVELOG feature.
   ...
   1. Specify the backup destination.
      ...
      * Execute the following statement in the `sys` tenant of the cluster where the primary tenant resides to specify the backup destination for the primary tenant:
        ...
        ```shell
        obclient(root@sys)[(none)]> ALTER SYSTEM SET DATA_BACKUP_DEST = 'file:///data/1/sh_databackup' TENANT = mysql;
        ```
      ...
      * Execute the following statement in the primary tenant to specify the backup destination for the primary tenant:
        ...
        ```shell
        obclient> ALTER SYSTEM SET DATA_BACKUP_DEST = 'file:///data/1/sh_databackup';
        ```
   ...
   2. Perform data backup.
      ...
      * Perform data backup for the primary tenant from the `sys` tenant of the cluster where the primary tenant resides.
        ...
        ```shell
        obclient(root@sys)[(none)]> ALTER SYSTEM BACKUP TENANT = mysql PLUS ARCHIVELOG;
        ```
   ...
   3. Check the data backup status.
      ...
      * `sys` tenant
        ...
        ```shell
        obclient(root@sys)[(none)]> SELECT * FROM oceanbase.CDB_OB_BACKUP_JOBS;
        ```

      * MySQL tenant

        ```shell
        obclient(root@mysql)[(none)]> SELECT * FROM oceanbase.DBA_OB_BACKUP_JOBS;
        ```

      * Oracle tenant

        ```shell
        obclient(sys@oracle001)[SYS]> SELECT * FROM SYS.DBA_OB_BACKUP_JOBS;
        ```
      ...
      * `sys` tenant
        ...
        CODE_PLACEHOLDER_30552300130d40c6a120a1781301042e

      * MySQL tenant

        CODE_PLACEHOLDER_05921033858d48489282250829577525

      * Oracle tenant

        CODE_PLACEHOLDER_65928685588e480d9192056950736577
...
2. Disable archivelog for the primary tenant.
   ...
   * Execute the following statement in the `sys` tenant of the cluster where the primary tenant resides to disable archivelog for the primary tenant:
     ...
     ```shell
     obclient(root@sys)[(none)]> ALTER SYSTEM NOARCHIVELOG TENANT = mysql;
     ```
...
2. Create a unit config for the standby tenant.
   ...
   ```shell
   obclient(root@sys)[(none)]> CREATE RESOURCE UNIT unit1 MAX_CPU 1, MEMORY_SIZE = '5G';
   ```
...
3. Create a resource pool for the standby tenant.
   ...
   ```shell
   obclient(root@sys)[(none)]> CREATE RESOURCE POOL pool_for_standby UNIT = 'unit1', UNIT_NUM = 1, ZONE_LIST = ('zone1','zone2','zone3');
   ```
...
4. Restore the standby tenant from the complete dataset that contains archive logs.
   ...
   ```shell
   obclient(root@sys)[(none)]> ALTER SYSTEM RESTORE standby_tenant FROM 'file:///data/1/sh_databackup' WITH 'pool_list=pool_for_standby';
   ```
...
5. Verify that the restore is completed.
   ...
   ```shell
   obclient(root@sys)[(none)]> SELECT TENANT_NAME, TENANT_TYPE, CREATE_TIME, STATUS, TENANT_ROLE,SCN_TO_TIMESTAMP(SYNC_SCN)  FROM oceanbase.DBA_OB_TENANTS WHERE TENANT_NAME = 'standby_tenant';
   ```
