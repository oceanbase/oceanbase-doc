...
3. Grant privileges to the user.
   ...
   The following example grants the user the `SELECT` privilege on all tables in the `oceanbase` database. You can also grant only the `SELECT` privilege on the `GV$OB_LOG_STAT`, `GV$OB_UNITS`, `GV$OB_PARAMETERS`, `DBA_OB_ACCESS_POINT`, `DBA_OB_TENANTS`, and `DBA_OB_LS` views in the `oceanbase` database.
