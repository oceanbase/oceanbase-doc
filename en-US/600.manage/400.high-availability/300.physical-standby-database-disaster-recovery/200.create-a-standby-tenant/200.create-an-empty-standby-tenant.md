...
3. Grant the `STANDBY_REPLICATION` role to the created user `rep_user`.
   ...
   OceanBase Database provides the default system role `STANDBY_REPLICATION` since V4.2.2. By default, this role has the `CREATE SESSION` system privilege and the privilege to query the following views:
