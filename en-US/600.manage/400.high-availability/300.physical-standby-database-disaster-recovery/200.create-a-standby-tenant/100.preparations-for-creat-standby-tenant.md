...
## Select the source for a standby tenant
...
<li>For more information about how to enable log archiving on the source tenant, see <a href="../../../600.backup-and-recovery/300.log-archive/300.open-the-log-archive-mode.md">Enable ARCHIVELOG</a>. </li>
...
* Create an empty standby tenant
  ...
  2. Query comparison information about log streams in the primary tenant.

     * Query the information from the `sys` tenant of the cluster where the primary tenant resides
     ...
     * Query the information from the primary tenant
     ...
     You can query the `DBA_OB_LS_HISTORY` or `CDB_OB_LS_HISTORY` view for the information about all log streams created in the tenant, and query the `DBA_OB_LS` or `CDB_OB_LS` view for the log stream that provides services in the tenant. If the query result is not empty, the query result shows the log streams that were once created in the primary tenant but were deleted later due to reasons such as load balancing or scaling.
  ...
  3. Query log streams of the primary tenant.

     * Query the log streams from the `sys` tenant of the cluster where the primary tenant resides
