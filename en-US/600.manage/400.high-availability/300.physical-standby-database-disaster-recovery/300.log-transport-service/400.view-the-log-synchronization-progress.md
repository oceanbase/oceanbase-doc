...
2. Execute the following statement to view the synchronization progress of the specified standby tenant.
   ...
   ```shell
   obclient(root@sys)[(none)]> SELECT TENANT_NAME, TENANT_ID, TENANT_ROLE, SCN_TO_TIMESTAMP(SYNC_SCN) 
...
3. (Optional) If the synchronization progress of the standby tenant is slow, query the slowest log stream.
   ...
   ```shell
   obclient(root@sys)[(none)]> SELECT LS_ID, SCN_TO_TIMESTAMP(END_SCN) FROM oceanbase.GV$OB_LOG_STAT WHERE TENANT_ID = 1004 AND ROLE = 'LEADER';
   ```
...
2. Execute the following statement to view the synchronization progress of the standby tenant.
   ...
   * MySQL mode
     ...
     ```shell
     obclient(root@standby_tenant)[(none)]> SELECT TENANT_NAME, TENANT_ROLE, SCN_TO_TIMESTAMP(SYNC_SCN) FROM oceanbase.DBA_OB_TENANTS;
     ```
   ...
   * Oracle mode
     ...
     ```shell
     obclient(sys@standby_tenant)[SYS]> SELECT TENANT_NAME, TENANT_ROLE, SCN_TO_TIMESTAMP(SYNC_SCN) FROM SYS.DBA_OB_TENANTS;
     ```
...
3. (Optional) If the synchronization progress of the standby tenant is slow, query the slowest log stream.
   ...
   * MySQL mode
     ...
     ```shell
     obclient(root@standby_tenant)[(none)]> SELECT LS_ID, SCN_TO_TIMESTAMP(END_SCN) FROM oceanbase.GV$OB_LOG_STAT WHERE ROLE = 'LEADER';
     ```
   ...
   * Oracle mode
     ...
     ```shell
     obclient(sys@standby_tenant)[SYS]> SELECT LS_ID, SCN_TO_TIMESTAMP(END_SCN) FROM SYS.GV$OB_LOG_STAT WHERE ROLE = 'LEADER';
     ```
