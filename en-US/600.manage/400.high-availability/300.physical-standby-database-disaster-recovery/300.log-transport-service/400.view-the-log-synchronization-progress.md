...
1. Log in as the administrator to the `sys` tenant of the cluster where the specified standby tenant resides.

2. Execute the following statement to query the synchronization progress of the standby tenant:
   ...
   In the query result, the `SCN_TO_TIMESTAMP(SYNC_SCN)` field displays the synchronization progress of the specified standby tenant. You can compare the displayed time with the current time. A large difference indicates that the synchronization progress of the standby tenant lags behind that of the primary tenant.

   For more information about the `DBA_OB_TENANTS` view, see [DBA_OB_TENANTS](../../../../700.reference/700.system-views/300.system-view-of-sys-tenant/200.dictionary-view-of-sys-tenant/22200.o-dba_ob_tenants-of-sys-tenant.md).
...
3. (Optional) If the synchronization progress of the standby tenant is slow, query the slowest log stream.
   ...
   The query result shows that the synchronization of log stream 1 is slow.
...
2. Execute the following statement to query the synchronization progress of the standby tenant:
   ...
   * Oracle mode
   ...
   For more information about the `DBA_OB_TENANTS` view, see [DBA_OB_TENANTS (MySQL mode)](../../../../700.reference/700.system-views/400.system-view-of-mysql-mode/200.dictionary-view-of-mysql-mode/12000.o-dba_ob_tenants-of-mysql-mode.md) and [DBA_OB_TENANTS (Oracle mode)](../../../../700.reference/700.system-views/500.system-view-of-oracle-mode/200.dictionary-view-of-oracle-mode/15400.dba_ob_tenants-oracle.md).
