...
3. Grant the `STANDBY_REPLICATION` role to the created user `rep_user`.
   ...
   In OceanBase Database V4.2.2 and later, the default system role `STANDBY_REPLICATION` is provided. By default, this role has the `CREATE SESSION` system privilege and the privilege to query the following views:
