...
<h4>Notice</h4>
<p>If the archive medium is object storage (such as Alibaba Cloud OSS, AWS S3, or object storage compatible with the S3 protocol), during the operation of the physical standby database, if the primary tenant or the backup tenant on the source side changes the key information of the backup medium, you need to refer to the following steps to reset the log recovery source.</p>
</main>
...
2. Set the restore source of the standby tenant to the archiving destination of the primary tenant.
   ...
   * Execute the following statement in the standby tenant to specify the restore source of the standby tenant:
   ...
   The following takes OSS or NFS as examples of archive media.

   :::tab
   tab Alibaba Cloud OSS

   When the archive medium for the primary tenant is OSS, the following are examples of how the `sys` tenant or standby tenant in the cluster where the standby tenant resides can set the log recovery source:
   ...
   When the archive medium for the primary tenant is NFS, the following are examples of how the `sys` tenant or standby tenant in the cluster where the standby tenant resides can set the log recovery source:
