...
### Step 2: Obtain the access portal information of the primary tenant or source standby tenant

For network-based physical standby database, you also need to obtain the access portal information of the primary tenant or source standby tenant when setting the log restore source, that is, the IP addresses and port numbers of the OBServer nodes where the tenant replicas reside.

1. Log in to the primary tenant or source standby tenant as the administrator.

2. Execute the following command to obtain the access portal information:
