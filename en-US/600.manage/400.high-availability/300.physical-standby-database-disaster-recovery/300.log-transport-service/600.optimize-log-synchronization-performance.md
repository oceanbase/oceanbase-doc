...
2. Select an appropriate statement and specify the `log_restore_concurrency` parameter.
   ...
   The tenant-level parameter `log_restore_concurrency` specifies the total number of worker threads for log restore. The value range of this parameter is [0, 100]. The default value is `0`, which specifies to use the adaptive degree of parallelism (DOP) for log restore in OceanBase Database. The modification of this parameter takes effect immediately without restarting the OBServer node. We recommend that you use the default value.
...
In the parallel log transport framework, a standby tenant in the network-based Physical Standby Database solution sends remote procedure call (RPC) packets frequently. To improve performance and stability and reduce the performance overhead, you can disable parallel log transport for the standby tenant.

1. Log in as the administrator to the standby tenant or the `sys` tenant of the cluster where the standby tenant resides.

2. Query the value of the `_ob_enable_standby_db_parallel_log_transport` parameter.
   ...
   Execute the following statement to query the value of the `_ob_enable_standby_db_parallel_log_transport` parameter:
...
3. Select an appropriate statement and specify the `_ob_enable_standby_db_parallel_log_transport` parameter.
...
For more information about how to view the log archiving progress of the primary tenant, see [View the archiving progress](../../../600.backup-and-recovery/300.log-archive/600.view-log-archive-progress.md).
...
2. Select an appropriate statement and specify the `log_archive_concurrency` parameter.
   ...
   The tenant-level parameter `log_archive_concurrency` specifies the total number of worker threads for log archiving. The value range of this parameter is [0, 100]. The default value is `0`, which specifies to use the adaptive DOP for log archiving in OceanBase Database. The modification of this parameter takes effect immediately without restarting the OBServer node. We recommend that you use the default value.
