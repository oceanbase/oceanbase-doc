...
2. Enable log compression.
   ...
   * `TENANT [=] tenant_name` indicates the name of the tenant. You use this parameter to specify the name of the tenant only when you execute the statement in the `sys` tenant.
   ...
   ```shell
   obclient(root@sys)[(none)]> ALTER SYSTEM SET log_transport_compress_all = True TENANT = standby_tenant;
   ```
   ...
   ```shell
   obclient(root@standby_tenant)[(none)]> ALTER SYSTEM SET log_transport_compress_all = True;
   ```
   ...
   ```shell
   obclient(sys@standby_tenant)[SYS]> ALTER SYSTEM SET log_transport_compress_all = 'True';
   ```
...
3. (Optional) Change the compression algorithm by modifying the `log_transport_compress_func` parameter.
   ...
   * `TENANT [=] tenant_name` indicates the name of the tenant. You use this parameter to specify the name of the tenant only when you execute the statement in the `sys` tenant.
   ...
   ```shell
   obclient(root@sys)[(none)]> ALTER SYSTEM SET log_transport_compress_func = 'zstd_1.0' TENANT = standby_tenant;
   ```

   Execute the following statement in a standby tenant to change the compression algorithm for the standby tenant:

   ```shell
   obclient> ALTER SYSTEM SET log_transport_compress_func = 'zstd_1.0';
   ```
