...
| Parameters | Parameters of the primary and standby tenants are independent and not physically synchronized. If a parameter of the primary tenant is modified, it needs to be evaluated whether the same parameter of the standby tenant needs to be modified as well.  |
...
| Optimizer statistics | The standby tenants do not collect statistics. Instead, the statistics are synchronized from the primary tenant to the standby tenants. |
