...
1. The arbitration service running on the log stream leader detects that some replicas return no log confirmation messages within the period specified by the `arbitration_timeout` parameter. The arbitration service then performs further checks and prepares to perform log stream downgrade on the replicas with out-of-sync logs.
...
2. The arbitration service periodically checks whether the replicas with out-of-sync logs encounter the exceptions listed in the downgrade strategy. If yes, the arbitration service downgrades the log stream.
   ...
   <h4>Notice</h4>
   <p>The arbitration service downgrades a log stream only when the total number of failed replicas, including downgraded replicas, reaches half of the total number of full-featured replicas. For example, in a 4F1A (4 full-featured replicas plus 1 arbitration service) deployment scenario:</p>
   <ul>
...
2. The arbitration service checks the downgraded replicas for any exception listed in the log stream downgrade strategy. If no exception is detected, the arbitration service upgrades the log stream.

   <main id="notice" type='notice'>
   <h4>Notice</h4>
   <p>When you upgrade a log stream, it is not required that all downgraded replicas be free of exceptions. For example, in a 4F1A (4 full-featured replicas plus 1 arbitration service) deployment scenario, two full-featured replicas are downgraded due to downtime of the OBServer nodes where they reside. If only one downgraded full-featured replica recovers and starts to synchronize logs, the arbitration service still performs log stream upgrade for that replica. </p>
   </main>

## References

For more information about arbitration service operations, see the following:
...
* [Arbitration service Prometheus monitoring](../../700.monitor/500.arbitration-service-monitoring.md)
