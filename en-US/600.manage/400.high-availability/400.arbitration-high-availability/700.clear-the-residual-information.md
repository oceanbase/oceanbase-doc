...
The ob_admin tool has been installed on the original OBServer node where the arbitration service resides. When you deploy the arbitration service on an OBServer node, the ob_admin tool is installed in the `/home/admin/oceanbase/bin` directory on the OBServer node by default. If the ob_admin tool has not been installed on the original OBServer node where the arbitration service resides, you must install the ob_admin tool before you clear the residual cluster information. For more information about how to install the ob_admin tool, see [Overview](../../../700.reference/1500.Components-and-Tools/100.manage/200.ob-admin/100.overview-of-ob-admin.md).
...
2. Log in to the original OBServer node where the arbitration service resides and clear the residual cluster information on the OBServer node.
   ...
   3. Execute the following statement to clear the residual cluster information:
      ...
      * `$ARB_SERVER_IP` specifies the IP address of the original OBServer node where the arbitration service resides.

      * `$ARB_SERVER_PORT` specifies the remote procedure call (RPC) port number of the original OBServer node where the arbitration service resides. By default, port 2882 is used.

      * `$CLUSTER_ID` specifies the ID of the cluster. To obtain the cluster ID, connect to the database and execute the `SHOW PARAMETERS LIKE 'cluster_id';` statement.
         ...
         The query result shows that the `cluster_id` value of the cluster is `10001`.

      The following example clears the residual information of the specified cluster:
