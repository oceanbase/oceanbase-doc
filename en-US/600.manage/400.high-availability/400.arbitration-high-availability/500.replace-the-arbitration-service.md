...
The OceanBase Database software has been installed on the new OBServer node and the arbitration service has been enabled. For more information, see [Deploy an OceanBase cluster with two replicas and the arbitration service](../../../400.deploy/300.deploy-oceanbase-enterprise-edition/400.deploy-through-the-command-line/200.deploy-the-oceanbase-cluster-command-line/200.deploy-the-quorum-high-availability-service.md).
...
2. Execute the following statement to query the current arbitration service address of the cluster:

   ```sql
   SELECT * FROM oceanbase.DBA_OB_ARBITRATION_SERVICE;
   ```

   The `ARBITRATION_SERVICE` field in the query result indicates the current arbitration service address of the cluster.
...
3. Execute the following statement to replace the arbitration service for the cluster:
   ...
   * `curr_arb_server_ip:curr_arb_server_port` specifies the IP address and remote procedure call (RPC) port of the faulty OBServer node on which the arbitration service is deployed. By default, port 2882 is used.

   * `new_arb_server_ip:new_arb_server_port` specifies the IP address and RPC port of the new OBServer node on which the arbitration service is deployed. By default, port 2882 is used.
   ...
   If the execution times out, you can query the `DBA_OB_ARBITRATION_SERVICE` view for the replacement status of the arbitration service.
   ...
   Execute the following statement to query the warning:
