...
2. Execute the following statement to query the log stream upgrade or downgrade result.
   ...
   * `sys` tenant
      ...
      ```shell
      obclient(root@sys)[(none)]> SELECT * FROM oceanbase.GV$OB_LOG_STAT WHERE tenant_id= xxxx;
      ```
   ...
   * User tenant
      ...
      ```shell
      obclient(root@mysql)[(none)]> SELECT * FROM oceanbase.GV$OB_LOG_STAT WHERE tenant_id= xxxx;
      ```
      ...
      ```shell
      obclient(sys@oracle)[SYS]> SELECT * FROM SYS.GV$OB_LOG_STAT WHERE tenant_id= xxxx;
      ```
