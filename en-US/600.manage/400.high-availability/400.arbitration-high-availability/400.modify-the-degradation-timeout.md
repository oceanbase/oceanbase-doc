...
2. Query the log stream downgrade control time of the arbitration service for a tenant.
   ...
   * Execute the following statement in the `sys` tenant to query the log stream downgrade control time for the specified tenant:
     ...
     The following example queries the log stream downgrade control time for a tenant named `mysql`:

     ```sql
     SHOW PARAMETERS LIKE 'arbitration_timeout' TENANT = 'mysql';
     ```

   * Execute the following statement in a user tenant to query the log stream downgrade control time for the tenant:
...
3. Select an appropriate statement to modify the log stream downgrade control time based on your business scenario.
   ...
   * Execute the following statement in a user tenant to modify the log stream downgrade control time for the tenant:
   ...
   * Execute the following statement in the `sys` tenant to modify the log stream downgrade control time for all user tenants (excluding meta tenants):
