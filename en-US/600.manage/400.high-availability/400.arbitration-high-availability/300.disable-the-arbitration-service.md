...
3. Disable the arbitration service for the tenant.
   ...
   The following example disables the arbitration service for a tenant named `oracle001`:
...
4. Query the `oceanbase.DBA_OB_TENANTS` view to check whether the arbitration service is disabled for the tenant.
   ...
   *************************** 1. row ***************************
   ...
   In the query result, the value of the `ARBITRATION_SERVICE_STATUS` field is `DISABLED`, which indicates that the arbitration service is disabled for the tenant.
