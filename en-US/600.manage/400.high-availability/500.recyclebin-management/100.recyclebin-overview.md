...
The recycle bin is equivalent to a data dictionary table for storing information about objects dropped by the user. Objects in the recycle bin still occupy physical space unless they are manually cleared by using the `PURGE` command or periodically cleared by the system.
