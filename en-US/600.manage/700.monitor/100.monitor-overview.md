...
* Data dictionary views
    ...
    A data dictionary view displays the basic information about database objects that are managed by the system tenant. Typically, the names of data dictionary views begin with `DBA_` or `CDB_`. Views whose names begin with `DBA_` display information about each tenant. For example, the `DBA_OB_LS` view of a tenant displays the log stream information about the tenant, and the `DBA_OB_LS` view of the system tenant displays only the log stream information about the system tenant. Views whose names begin with `CDB_` are dedicated to the system tenant. You can query these views for the status of all tenants in the cluster from the system tenant. For example, the `CDB_OB_LS` view provides information about all log streams of all tenants in the cluster. In general, each view whose name begins with `DBA_` has a corresponding view whose name begins with `CDB_` in the system tenant.

* Dynamic performance views

    A dynamic performance view provides information about the dynamic status changes of the system. Typically, the names of dynamic performance views begin with `GV$` or `V$`. Views whose names begin with `V$` display only information about the node to which you logged on. Views whose names begin with `GV$` display information about all nodes. Dynamic performance views of a user tenant display information about the tenant. For example, you can query the `GV$OB_UNITS` view of a user tenant for the resource unit allocation information about the tenant. Dynamic performance views of the system tenant display information about all tenants in the cluster.
...
![View architecture](https://obbusiness-private.oss-cn-shanghai.aliyuncs.com/doc/img/observer-enterprise/V4.2.1/manage/view-architecture1.jpg)
...
<main id="notice" type='explain'>
      ...
      <li><code>GV$</code> views and <code>V$</code> views are homogenous. Therefore, they are not distinguished in the following sections. Select the appropriate view based on your specific scenario. </li></ul>
</main>

You can view the metrics by logging on to the system tenant or a user tenant. The names, fields, and user experience of the views in the two types of tenants are the same. The difference is that in a user tenant, you can view only the monitoring information about the current user tenant, but you can view the monitoring information about all tenants in the current cluster from the system tenant.
...
Furthermore, the arbitration service (arbserver) provides a standard Prometheus `/metrics` endpoint, which exposes metrics such as its own CPU, memory, and disk usage, as well as cluster, tenant, LS status, election, and RPC latency. The Prometheus metrics of the arbitration service are part of an independent metric family (`arb_*`) and do not affect the `GV$` or `V$` view system of the observer. For detailed configuration and metric descriptions of arbitration service monitoring, see [Prometheus monitoring for the arbitration service](./500.arbitration-service-monitoring.md).
