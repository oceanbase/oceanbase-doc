...
* `INSERT_DUPLICATE_ROW_COUNT`: the number of duplicate rows when executing `INSERT ... ON DUPLICATE KEY UPDATE` or `REPLACE INTO` statements.
...
* `enable_sql_audit`: a cluster parameter that controls whether the SQL audit feature is enabled for all tenants. It takes effect immediately after it is modified.
* `ob_enable_sql_audit`: a tenant-level system variable that controls whether the SQL audit feature is enabled for the current tenant. It takes effect immediately after it is modified.
* `ob_sql_audit_percentage`: a tenant-level system variable that specifies the percentage of tenant memory that can be used by the SQL audit feature for the current tenant. It takes effect immediately after it is modified. To prevent the SQL audit feature from consuming excessive memory, the system sets its memory limit to 1 GB.
