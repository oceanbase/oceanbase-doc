...
* `LOG_DISK_CAPACITY`: the total log disk capacity on a single node.
  ...
  * If the value of `log_disk_size` is `0M` but the value of `log_disk_percentage` is not `0`, the system allocates log disk space based on the value of `log_disk_percentage`.
  ...
  * If the value of `log_disk_size` is `0M` and the value of `log_disk_percentage` is `0`, the system calculates the percentage of the disk space for storing redo logs based on whether logs and data are stored on the same disk.
...
You can modify the CPU, memory, IOPS, and log disk capacities of a unit config. If you do not specify a new value for an item, the original value of the item takes effect.
