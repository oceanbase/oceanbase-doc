...
When you create a tenant, you can allocate an unused resource pool to the tenant. A sample SQL statement is as follows:
...
* You can bind a resource pool only to one tenant, while a tenant can have multiple resource pools.
* The total amount of resources allocated to a tenant in a zone is calculated as follows: Unit config × Number of resource units.

For more information about how to create a tenant, [Create a tenant](../200.manage-create-tenant.md).
...
Assume that the current cluster has three zones, which are `z1`, `z2`, and `z3`. The three zones belong to the same region, and each zone has one OBServer node. The cluster has a user tenant named `tenant1`. Replica distribution of the tenant is as follows: `locality='F@z1,F@z2,F@z3', resource_pool_list=('pool1');`. Based on business needs, the number of replicas of `tenant1` must be increased from 3 to 5. To be specific, the locality of the tenant must be changed from `F@z1,F@z2,F@z3` to `F@z1,F@z2,F@z3,F@z4,F@z5`.

### Procedure

1. Add two new zones, `z4` and `z5`, to the cluster that includes the `z1`, `z2`, and `z3` zones.

   For more information about how to add zones to a cluster, see [Add a zone](../../../100.cluster-management/300.common-cluster-operations/800.add-a-zone.md).

2. Add an OBServer node to each of the `z4` and `z5` zones.

   For more information about how to add an OBServer node to a zone, see [Add a node](../../../100.cluster-management/300.common-cluster-operations/400.add-a-node.md).

3. Log in to the `sys` tenant as the `root` user, and add resources for the `tenant1` tenant in zones `z4` and `z5`.
   ...
   2. Add resources to zones `z4` and `z5`.
...
4. Modify the locality of the tenant to add replicas.
   ...
   Based on the locality modification rule, you can directly change the locality to `F@z1,F@z2,F@z3,F@z4,F@z5` when the number of replicas increases from 3 to 5. For more information about the locality modification rule, see [Modify locality](../../../300.replica-management/200.replica-distribution/200.locality-common-operations/200.modify-locality.md).
