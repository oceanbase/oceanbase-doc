...
* If a tenant has multiple resource pools, you can modify the resource pool configuration of the tenant to drop a resource pool from the tenant. For more information, see [Modify attributes of a tenant](../1000.modify-tenant-properties.md).

* If a tenant has only one resource pool, you cannot directly drop the resource pool. To drop the resource pool, perform either of the following steps:

  * Create a resource pool and allocate it to this tenant. Then, modify the resource pool configuration of the tenant to release the resource pool to be dropped. For more information about how to create a resource pool, see the "Create a resource pool" section in [Create a tenant](../200.manage-create-tenant.md).
  * If the tenant is no longer needed, you can drop the tenant first and then drop the resource pool. For more information, see [Drop a tenant](../1200.delete-tenant.md).
