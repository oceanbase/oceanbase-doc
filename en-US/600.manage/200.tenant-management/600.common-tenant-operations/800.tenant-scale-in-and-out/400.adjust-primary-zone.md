...
* Enable load balancing for a user tenant from the current tenant
    ...
    You can execute the following statements in a MySQL tenant to enable load balancing and the transfer feature for the current tenant:
    ...
    You can execute the following statements in an Oracle tenant to enable load balancing and the transfer feature for the current tenant:
...
3. Query the basic information about the `mysql001` tenant, such as the `LOCALITY` and `PRIMARY_ZONE` attributes.
...
4. Query the number of resource units of the `mysql001` tenant. In this example, the tenant has two resource units in each zone.
...
5. Modify the `PRIMARY_ZONE` attribute of the `mysql001` tenant to change the number of zones with the highest priority from 1 to 2.
...
6. View the execution status of the job for adding a primary zone.
   ...
   * `EXTRA_INFO`: the primary zone information before or after the modification.
   ...
   If the value of `JOB_STATUS` is `SUCCESS` in the corresponding job record, the job for adding a primary zone is successfully executed.

   For more information about the fields in the `DBA_OB_TENANT_JOBS` view, see [DBA_OB_TENANT_JOBS](../../../../700.reference/700.system-views/300.system-view-of-sys-tenant/200.dictionary-view-of-sys-tenant/22200.o-dba_ob_tenants-of-sys-tenant.md).

7. Query the basic information of the `mysql001` tenant to verify whether zones with the highest priority are changed from `zone1` to `zone1,zone2`.
...
8. Query the resource units of the `mysql001` tenant to verify whether the number of resource units of the tenant remains unchanged.
...
In the preceding example, the number of primary zones is changed from 1 to 2 for the `mysql001` tenant. The tenant has two resource units in each zone before the change. After the change, the tenant still has two resource units in each zone but the number of primary zones is changed from 1 to 2.
...
5. Modify the `PRIMARY_ZONE` attribute of the `mysql001` tenant to change the number of zones with the highest priority from 2 to 1.

   ```sql
   ALTER TENANT mysql001 PRIMARY_ZONE='zone1;zone2,zone3';
   ```

6. View the execution status of the job for removing a primary zone.
   ...
   * `EXTRA_INFO`: the primary zone information before or after the modification.
   ...
   If the value of `JOB_STATUS` is `SUCCESS` in the corresponding job record, the job for removing a primary zone is successfully executed.


7. Query the basic information of the `mysql001` tenant to verify whether zones with the highest priority are changed from `zone1,zone2` to `zone1`.
...
8. Query the resource units of the `mysql001` tenant to verify whether the number of resource units of the tenant remains unchanged before and after the modification.
...
In the preceding example, the number of primary zones is changed from 2 to 1 for the `mysql001` tenant. The tenant has two resource units in each zone before the change. After the change, the tenant still has two resource units in each zone but the number of primary zones is changed from 2 to 1.
