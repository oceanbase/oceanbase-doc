...
5. Change the value of `UNIT_NUM` to `2` for the `mysql001` tenant.
   ...
   * Decrease the `UNIT_NUM` by deleting a specified `UNIT_GROUP`.
     ...
     After this statement is executed, for a scenario where each zone has two units, the system redistributes the data on the specified unit group to other unit groups, and then deletes the unit group.
