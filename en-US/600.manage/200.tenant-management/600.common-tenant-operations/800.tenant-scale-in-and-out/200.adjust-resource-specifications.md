...
* If a tenant is deleted, drop the resource pool of the tenant to release resources. Resources in an idle resource pool are also considered occupied resources.
...
* `LOG_DISK_CAPACITY`: the total capacity of the log disk.
  ...
  * If the values of both `log_disk_size` and `log_disk_percentage` are `0`, the system automatically calculates the percentage of the disk space for storing redo logs. The percentage depends on whether the disk is shared by redo logs and data.

    * If the disk is shared, the percentage of the disk space for storing redo logs is 30%.

    * If the disk is exclusive for redo logs, the percentage of the disk space for storing redo logs is 90%.

  For more information about the `log_disk_size` parameter, see [log_disk_size](../../../../700.reference/800.configuration-items-and-system-variables/100.system-configuration-items/300.cluster-level-configuration-items/13000.log_disk_size.md).
...
Obtain the ID of the unit config used by the current tenant, and verify whether the tenant uses an independent unit config.
...
In the query result, if the `UNIT_CONFIG_ID` value of the current tenant is the same as those of other tenants, multiple tenants use the same unit config.
...
* [Introduction to scaling in/out](../800.tenant-scale-in-and-out/100.introduction-to-tenant-scale-in-and-out.md)
