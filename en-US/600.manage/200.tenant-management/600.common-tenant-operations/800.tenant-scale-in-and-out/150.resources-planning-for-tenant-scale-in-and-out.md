...
* In the homogeneous zone mode, the `UNIT_NUM` of each zone of the tenant must be the same.
* In the heterogeneous zone mode, the `UNIT_NUM` of each zone of the tenant can be the same or different, but the `UNIT_NUM` of all zones of the tenant can be at most two different values.

<main id="notice" type='notice'>
<h4>Notice</h4>
<p>By default, existing tenants and tenants upgraded to the current version are in the homogeneous zone mode. To enable the heterogeneous zone mode, you must set the tenant-level parameter <code>zone_deploy_mode</code> to <code>hetero</code>. After you change the value of <code>zone_deploy_mode</code>, you cannot change it back to <code>homo</code>.</p>
</main>

This topic provides operation guidance for resource planning when you scale out a tenant in the homogeneous zone mode. The operation is similar for a tenant in the heterogeneous zone mode.
...
For tenants in heterogeneous zone mode, if you want to scale in resources by reducing the number of resource units in a resource pool, and each unit group is idle, you can perform the following steps to delete a unit group:
...
3. Query result units of the tenant to be scaled in.
   ...
   The query result shows that the tenant to be scaled in has two resource units in each zone. The `UNIT_ID` values are respectively `1016`, `1017`, `1018`, `1019`, `1020`, and `1021`.

4. Calculate the minimum amount of physical resources required for the tenant in each resource unit.

    For example, calculate the minimum amount of physical resources required for the tenant in each resource unit. In this example, the node corresponding to the resource unit whose `UNIT_ID` value is `1018` is `xxx.xx.xx.192:2882`.
   ...
   ```
    ...
    After you calculate the minimum amount of physical resources required for the tenant in each resource unit within each unit group, select the unit group that requires the least resources and delete it for tenants in homogeneous zone mode. For tenants in heterogeneous zone mode, select the unit list that requires the least resources and delete it.
