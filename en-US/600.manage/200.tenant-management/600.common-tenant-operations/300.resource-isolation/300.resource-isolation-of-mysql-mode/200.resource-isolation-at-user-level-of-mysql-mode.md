...
5. Call the `SET_CONSUMER_GROUP_MAPPING` subprogram in the `DBMS_RESOURCE_MANAGER` package to create a matching rule for resource isolation based on your business scenario.
   ...
   * `VALUE` indicates the attribute value.
     ...
     * If the attribute type is `function`, you can specify one or more of the following background tasks: `compaction_high`, `ha_high`, `compaction_mid`, `ha_mid`, `compaction_low`, `ha_low`, `ddl`, `ddl_high`, `clog_high`, and `opt_stats`. You can specify only one task. For more information about each task, see [Overview](../100.resource-isolation-overview.md).
