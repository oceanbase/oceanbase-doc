...
2. Call the `CREATE_CONSUMER_GROUP` subprogram in the `DBMS_RESOURCE_MANAGER` package to create resource groups.
...
4. Call the `CREATE_PLAN_DIRECTIVE` subprogram in the `DBMS_RESOURCE_MANAGER` package to create a plan directive, which is used to limit the CPU and IOPS resources available for the resource group when the resource management plan is enabled.
   ...
   * Create a plan directive as follows: Set the resource plan to `daytime`, the resource group to `batch_group`, and the maximum CPU resources available to 40% of the total CPU resources of the tenant. Set the minimum IOPS resources available upon I/O contention to 40% of the total IOPS resources, the maximum IOPS resources available to 80% of the total IOPS resources, the weight of IOPS resources to 70, the maximum network bandwidth available to 30%, and the weight of network bandwidth to 30.
...
5. Call the `SET_CONSUMER_GROUP_MAPPING` subprogram in the `DBMS_RESOURCE_MANAGER` package to create a matching rule for resource isolation based on your business scenario.
   ...
   * `VALUE` indicates the attribute value.
     ...
     * If the attribute type is `user`, specify the username. At present, you can specify only one user.

     * If the attribute type is `function`, specify one of the following background tasks corresponding to the DAG thread: `compaction_high`, `ha_high`, `compaction_mid`, `ha_mid`, `compaction_low`, `ha_low`, `ddl`, `ddl_high`, `clog_high`, and `opt_stats`. At present, you can specify only one task. For more information about each task, see [Overview of resource isolation](../100.resource-isolation-overview.md).
