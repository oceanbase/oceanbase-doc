...
* `global_background_cpu_quota`
  ...
  This parameter specifies the CPU quota available to background tasks. It is a cluster-level parameter. The default value is `-1`, which specifies that the CPU resources available to background tasks are limited by the cgroup.
