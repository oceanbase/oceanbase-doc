...
This topic describes how to configure control groups (cgroups) in OceanBase Database.

## Background information

In the current version of OceanBase Database, worker threads and most background threads are tenant-specific, and network threads are shared. You can configure cgroups to control the CPU utilization of tenants. Before you configure cgroups, we recommend that you learn about the concept of cgroup first. For more information, see [Overview](../100.resource-isolation-overview.md).
...
* If the OBServer server is restarted, the created cgroup subdirectories (for example, all subdirectories under the `/sys/fs/cgroup/` directory in this topic) will be cleared, and you need to reconfigure cgroup.
...
4. Create a directory hierarchy named `cpu`, attach the `cpu` subsystem to this hierarchy, and mount this hierarchy to the `/sys/fs/cgroup/cpu` directory.
...
6. Allocate CPU and memory resources for the `oceanbase` directory.
   ...
   1. Execute the following statement to view the mounting status of the `cpu`, `cpuacct`, and `cpuset` subsystems on your machine:

      ```shell
      [admin@xxx /]$ ll /sys/fs/cgroup
      ```

   2. Perform the corresponding operations based on the mounting status of the subsystems:

      * The `cpuset` subsystem is mounted alongside the `cpu` and `cpuacct` subsystems

         In this scenario, the three subsystems are often mounted to the same directory, as shown in the following example:
         ...
         In this case, run the following commands to allocate CPU and memory resources for the `oceanbase` directory:
      ...
      * The `cpuset` subsystem is mounted separately from the `cpu` and `cpuacct` subsystems
         ...
         In this scenario, the `cpuset` subsystem is mounted separately from the `cpu` and `cpuacct` subsystems, which is common in Elastic Compute Service (ECS) environments, as shown in the following example:
         ...
         In this case, proceed to the next step without performing additional operations.

7. Run the following command to set the inheritance attribute for subdirectories in the `oceanbase` directory:

   ```shell
   [admin@xxx /]$ sudo sh -c "echo 1 > /sys/fs/cgroup/cpu/oceanbase/cgroup.clone_children"
   ```

   After the command succeeds, the `cgroup` subdirectory created under the `oceanbase` directory inherits attributes from the parent directory.

### Step 2: Deploy OceanBase Database

After the cgroup system directory is configured, you can deploy OceanBase Database Enterprise Edition. For more information about how to deploy OceanBase Database Enterprise Edition, see [Deploy OceanBase Database](../../../../../400.deploy/300.deploy-oceanbase-enterprise-edition/400.deploy-through-the-command-line/200.deploy-the-oceanbase-cluster-command-line/100.stand-alone-deployment-of-oceanbase-database.md).
...
4. Log in to the `sys` tenant of the cluster and query the `V$OB_CGROUP_CONFIG` or `GV$OB_CGROUP_CONFIG` view for the cgroup configurations on each OBServer node.
...
After the cgroup system directory is configured, you can deploy OceanBase Database Community Edition. For more information about how to deploy OceanBase Database Community Edition, see [Deploy OceanBase Database](../../../../../400.deploy/500.deploy-oceanbase-database-community-edition/200.local-deployment/500.deploy-OceanBase-database-of-multi-node-cluster.md).
