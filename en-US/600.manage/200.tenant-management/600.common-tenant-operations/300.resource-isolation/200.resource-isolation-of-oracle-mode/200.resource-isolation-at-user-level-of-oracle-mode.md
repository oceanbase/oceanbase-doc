...
## Configure CPU and IOPS resource isolation
...
* Before you implement IOPS resource isolation, you must calibrate the disk performance. For more information about how to perform disk performance calibration, see [Calibrate the disk performance](../200.resource-isolation-of-oracle-mode/150.disk-performance-calibration-of-oracle-mode.md).
...
When you use the `DBMS_RESOURCE_MANAGER` system package to configure function-level resource isolation within a tenant after enabling global CPU resource isolation for foreground and background tasks, the maximum CPU resources available to each background task of the tenant are calculated based on the maximum CPU resources available to all background tasks of the tenant and are calculated by using the following formula: `min(global_background_cpu_quota, MAX_CPU of the tenant) × UTILIZATION_LIMIT within the resource group`.
...
2. Call the `CREATE_CONSUMER_GROUP` subprogram in the `DBMS_RESOURCE_MANAGER` package to create two resource groups.
   ...
   * `COMMENT`: the comments on the resource group.
   ...
   You can query the `DBA_RSRC_CONSUMER_GROUPS` view to verify whether the resource groups are created.
...
5. Call the `SET_CONSUMER_GROUP_MAPPING` subprogram in the `DBMS_RESOURCE_MANAGER` package to create a matching rule for resource isolation based on your business scenario.
   ...
   * `VALUE`: the attribute value.
     ...
     * If the attribute type is `column`, specify the database name, table name, column name, constant value, and username.
       ...
       * When you specify the table name, column name, and username, the specified table, column, and user must exist.
       ...
       In Oracle mode of OceanBase Database, the database name, table name, column name, and username will be automatically converted to uppercase after a statement is executed. You can enclose them with double quotation marks to prevent them from being converted to uppercase. Here is an example:
     ...
     * If the attribute type is `function`, specify a background task corresponding to the directed acyclic graph (DAG) thread. The following types of background tasks are supported: `compaction_high`, `ha_high`, `compaction_mid`, `ha_mid`, `compaction_low`, `ha_low`, `ddl`, `ddl_high`, `clog_high`, and `opt_stats`. At present, you can specify only one task. For more information about the tasks, see [Overview](../100.resource-isolation-overview.md).
   ...
   * Create a matching rule for SQL statement-level resource isolation.
      ...
      * Specify to bind an SQL statement that is initiated by the `tp_user` user and that has a `WHERE` clause containing `sys.t.c3 = 3` to the `big_group` resource group for execution by using the CPU and IOPS resources available for the resource group.
        ...
        <h4>Notice</h4>
        <p>An SQL statement can be bound to the <code>big_group</code> resource group provided that <code>c3</code> is parsed into <code>sys.t.c3</code> but the statement does not necessarily need to contain <code>sys.t.</code>. For example, the statement <code>SELECT * FROM sys.t WHERE c3 = 1;</code> can be bound to the resource group. </p>
        </main>
   ...
   * Create a matching rule for function-level resource isolation.
   ...
   You can query the `DBA_RSRC_GROUP_MAPPINGS` view to verify whether the matching rule is created.
...
In the current version of OceanBase Database, DDL and DML requests contend for foreground worker threads. A surge of DDL requests in a short time can significantly disrupt normal user requests. To minimize performance and stability issues caused by thread resource contention, OceanBase Database separates DDL and DML worker thread resources. You can specify whether to enable resource isolation for DDL worker threads as needed.

The tenant-level hidden parameter `_enable_ddl_worker_isolation` in OceanBase Database is used to specify whether to enable resource isolation for DDL worker threads. Its valid values are as follows:

* `False`: specifies to disable resource isolation for DDL worker threads. In this case, DDL and DML requests share worker threads and affect each other.

* `True`: specifies to enable resource isolation for DDL worker threads. In this case, independent worker threads are used for DDL requests and the number of DDL requests initiated at a time in a tenant is limited to avoid affecting DML requests.

You can perform the following steps to enable resource isolation for DDL worker threads:
...
2. Query the value of the `_enable_ddl_worker_isolation` parameter.
   ...
   +----------------+----------+-------+--------+-----------+------------------------------+-----------+-------+------------------------------------------+--------------+-------------------+---------------+-----------+
   | 172.xx.xxx.xxx |     2882 | zone1 | TENANT |      1004 | _enable_ddl_worker_isolation | NULL      | False | a switch controlling ddl thread isolation | ROOT_SERVICE | DYNAMIC_EFFECTIVE | False         | YES       |
   +----------------+----------+-------+--------+-----------+------------------------------+-----------+-------+------------------------------------------+--------------+-------------------+---------------+-----------+
   1 row in set
   ```
