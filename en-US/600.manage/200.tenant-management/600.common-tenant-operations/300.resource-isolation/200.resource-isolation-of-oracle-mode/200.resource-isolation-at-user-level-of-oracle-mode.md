...
* Before you implement input/output operations per second (IOPS) resource isolation, you can calibrate the disk performance. For more information about how to perform disk performance calibration, see [Calibrate the disk performance](../200.resource-isolation-of-oracle-mode/150.disk-performance-calibration-of-oracle-mode.md).

  <main id="notice" type='explain'>
  <h4>Note</h4>
  <p>In the current version, IOPS resource isolation no longer strongly depends on disk performance calibration operations.</p>
  </main>
...
4. Execute the following statement to query the disk calibration value on each OBServer node where the tenant for resource isolation is deployed, and use the 16 KB read disk calibration value as the upper IOPS limit for the node.
...
4. Call the `CREATE_PLAN_DIRECTIVE` subprogram in the `DBMS_RESOURCE_MANAGER` package to create a plan directive for the resource management plan. When the resource management plan is enabled, the plan directive limits the CPU and IOPS resources available for the resource group.
   ...
   * `MAX_IOPS` indicates the maximum IOPS resources available for the resource group. The sum of `MAX_IOPS` values of all resource groups can exceed 100. The default value is `100`.
   ...
   * `MAX_NET_BANDWIDTH` indicates the maximum network bandwidth available for the resource group. The sum of `MAX_NET_BANDWIDTH` values of all resource groups can exceed 100. The default value is `100`.

   * `NET_BANDWIDTH_WEIGHT` indicates the weight for network bandwidth. The sum of `NET_BANDWIDTH_WEIGHT` values of all resource groups can exceed 100. The default value is `0`.

   Here are some examples:

   * Create a plan directive as follows: Set the resource plan to `plan_a`, the resource group to `big_group`, and the maximum CPU resources available to 60% of the total CPU resources of the tenant. Set the minimum IOPS resources available upon I/O contention to 20% of the total IOPS resources, the maximum IOPS resources available to 100% of the total IOPS resources, the weight of IOPS resources to 20, the maximum network bandwidth available to 50%, and the weight of network bandwidth to 50.

      ```shell
      obclient [SYS]> BEGIN DBMS_RESOURCE_MANAGER.CREATE_PLAN_DIRECTIVE(
      PLAN => 'plan_a',
      GROUP_OR_SUBPLAN => 'big_group',
      COMMENT  => 'Prioritize transaction processing',
   ...
   * Create a plan directive as follows: Set the resource plan to `plan_a`, the resource group to `small_group`, and the maximum CPU resources available to 40% of the total CPU resources of the tenant. Set the minimum IOPS resources available upon I/O contention to 10% of the total IOPS resources, the maximum IOPS resources available to 90% of the total IOPS resources, the weight of IOPS resources to 30, the maximum network bandwidth available to 60%, and the weight of network bandwidth to 60.
   ...
   You can query the `DBA_RSRC_PLAN_DIRECTIVES` and `DBA_OB_RSRC_IO_DIRECTIVES` views to verify whether the plan directives are created.
...
5. Call the `SET_CONSUMER_GROUP_MAPPING` subprogram in the `DBMS_RESOURCE_MANAGER` package to create a matching rule for resource isolation based on your business scenario.
   ...
   * `VALUE` indicates the attribute value.
     ...
     * If the attribute type is `column`, specify the database name, table name, column name, constant value, and username.
       ...
       * When you specify the table name, column name, and username, the specified table, column, and user must exist.
       ...
       In Oracle-compatible mode of OceanBase Database, the database name, table name, column name, and username will be automatically converted to uppercase after a statement is executed. You can enclose them with double quotation marks to prevent them from being converted to uppercase. Here is an example:
     ...
     * If the attribute type is `function`, you can specify one of the following background tasks corresponding to the DAG thread: `compaction_high`, `ha_high`, `compaction_mid`, `ha_mid`, `compaction_low`, `ha_low`, `ddl`, `ddl_high`, `clog_high`, and `opt_stats`. Currently, you can specify only one task. For more information about the tasks, see [Overview of resource isolation](../100.resource-isolation-overview.md).
   ...
   * Create a matching rule for SQL statement-level resource isolation.
      ...
      * Specify to bind an SQL statement that is initiated by the `tp_user` user and that has a `WHERE` clause containing `sys.t.c3 = 3` to the `big_group` resource group for execution by using the CPU and IOPS resources available for the resource group.

        <main id="notice" type='notice'>
        <h4>Notice</h4>
        <p>An SQL statement can be bound to the <code>big_group</code> resource group provided that <code>c3</code> is parsed into <code>sys.t.c3</code> but the statement does not necessarily need to contain <code>sys.t.</code>. Here is an example: <code>SELECT * FROM sys.t WHERE c3 = 1;</code>.</p>
        </main>
   ...
   * Create a matching rule for user-level resource isolation.
      ...
      * Specify to bind SQL statements initiated by the `ap_user` user to the `small_group` resource group for execution by using the CPU and IOPS resources available for the resource group.
   ...
   * Create a matching rule for function-level resource isolation.
   ...
   You can query the `DBA_RSRC_GROUP_MAPPINGS` view to verify whether the matching rule is created. For more information about the `DBA_RSRC_GROUP_MAPPINGS` view, see [DBA_RSRC_GROUP_MAPPINGS](../../../../../700.reference/700.system-views/500.system-view-of-oracle-mode/200.dictionary-view-of-oracle-mode/18500.dba_rsrc_group_mappings-of-oracle-mode.md).
