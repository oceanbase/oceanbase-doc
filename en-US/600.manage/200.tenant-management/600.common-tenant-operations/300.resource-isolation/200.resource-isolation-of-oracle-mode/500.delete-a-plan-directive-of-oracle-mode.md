...
2. Call the `DELETE_PLAN_DIRECTIVE` subprogram in the `DBMS_RESOURCE_MANAGER` package to delete the plan directive.
   ...
   In this example, after the limits on the resources available for the `small_group` resource group are deleted, the SQL statements that originally use the resources in the `small_group` resource group will use the resources in the default resource group `OTHER_GROUPS`.
