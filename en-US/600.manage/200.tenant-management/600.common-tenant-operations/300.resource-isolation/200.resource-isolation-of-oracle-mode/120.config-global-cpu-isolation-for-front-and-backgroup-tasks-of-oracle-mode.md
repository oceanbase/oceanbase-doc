...
* `global_background_cpu_quota`
  ...
  This parameter specifies the CPU quota available to backend tasks. It is a cluster-level parameter. The default value is `-1`, which specifies that the CPU resources available to backend tasks are not limited by the cgroup.
