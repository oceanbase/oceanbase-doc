...
# Query tenant and resource information
...
|--------------|----------------------------------|
| LS | Log stream. This is the basic unit for transaction submission. The number of log streams a tenant can create on each node is determined by a hidden parameter (`_max_ls_cnt_per_server`), the tenant’s memory allocation (`MAX(8, (MEMORY_SIZE - 4G) / 200MB + 8)`), and the size of the log disk. Specifically: <ul><li>The tenant-level hidden parameter `_max_ls_cnt_per_server` sets the maximum number of log stream entities allowed per node for a tenant. Valid values range from [0, 1024], with a default of 0, which means it is not set explicitly and the system will automatically calculate the supported number of log streams based on internal rules. Any changes to this parameter take effect immediately and do not require an OBServer node restart.</li><li>`MEMORY_SIZE` refers to the memory specification of the Unit in the tenant’s resource pool, representing the memory available to the tenant.</li><li>Each log stream entity requires 512 MB of log disk space.</li></ul> |
| TABLET	     | Indicates a tablet. Tablets are the basic unit for data migration. The maximum number of tablets that you can create for a tenant on each OBServer node is determined by the tenant-level hidden parameters and memory size of the tenant, and can be calculated by using the following formulas: `(MEMORY_SIZE/1 GB) × _max_tablet_cnt_per_gb` and `(MEMORY_SIZE × _storage_meta_memory_limit_percentage)/200MB × 20000`. <ul><li>The tenant-level hidden parameter `_max_tablet_cnt_per_gb` specifies the maximum number of tablets supported per 1 GB of memory in the resource unit of the tenant. The value range is [1000, 50000). and the default value is `20000`. The modification of this parameter takes effect immediately, without the need to restart the OBServer node. </li> <li>The tenant-level hidden parameter `_storage_meta_memory_limit_percentage` specifies the maximum percentage of memory of the tenant available for metadata storage. The value range is [0, 50), and the default value is `20`. The value `0` indicates not to specify the maximum percentage. The modification of this parameter takes effect immediately, without the need to restart the OBServer node. </li></ul>      |
| SESSION	     | Indicates a session. The maximum number of sessions that you can create for a tenant on each OBServer node is determined by the tenant-level hidden parameter `_resource_limit_max_session_num` or calculated by using the following formula: `(MEMORY_SIZE × 5%)/100 KB`. </br>The `_resource_limit_max_session_num` parameter specifies the maximum number of concurrent sessions supported in a user tenant. The value range is [0, 1000000]. The default value `0` indicates that the system calculates the maximum number of concurrent sessions supported by using the following formula: `(MEMORY_SIZE × 5%)/100 KB`. When the value of the `_resource_limit_max_session_num` parameter is greater than 0, the value of this parameter is used. The modification of this parameter takes effect immediately, without the need to restart the OBServer node.       |
| THREAD	     | Indicates a thread. The maximum number of threads supported on each OBServer node is determined by the cluster-level hidden parameter `_ob_max_thread_num`. </br>The `_ob_max_thread_num` parameter specifies the maximum number of threads supported on each OBServer node. The value range is [0, 10000). The default value `0` indicates not to specify the maximum number. You can modify this parameter only in the `sys` tenant. The modification takes effect immediately, without the need to restart the OBServer node.   |
...
| CLOG_DISK	   | The disk used to store commit logs (clogs) of the tenant.                        |
...
OceanBase Database provides views related to tenants and resources. You can query these views for related information in OceanBase Database in the sys tenant.
...
For detailed information about the parameters in the `DBA_OB_UNIT_CONFIGS` view, see [DBA_OB_UNIT_CONFIGS](../../../700.reference/700.system-views/300.system-view-of-sys-tenant/200.dictionary-view-of-sys-tenant/22700.o-dba_ob_unit_configs-of-sys-tenant.md).
...
For detailed information about the parameters in the `DBA_OB_RESOURCE_POOLS` view, see [DBA_OB_RESOURCE_POOLS](../../../700.reference/700.system-views/300.system-view-of-sys-tenant/200.dictionary-view-of-sys-tenant/20000.o-dba_ob_resource_pools-of-sys-tenant.md).

## Query information of all tenants

You can query the `DBA_OB_TENANTS` view for information about all tenants. Here is an example:

In the third row of the query result, the tenant name is `mysql001`, the tenant ID is `1002`, the tenant type is `USER`, the creation time is `2023-05-17 18:15:21`, the primary zone setting is `zone1` (with a higher priority) and `zone2`, the locality is `zone1`, `zone2`, and `zone3`, and the compatibility mode is `MySQL`.
...
For detailed information about the parameters in the `DBA_OB_TENANTS` view, see [DBA_OB_TENANTS](../../../700.reference/700.system-views/400.system-view-of-mysql-mode/200.dictionary-view-of-mysql-mode/12000.o-dba_ob_tenants-of-mysql-mode.md).

## Query the resource configurations of tenants
...
## Query the deployment locations of the resource units of tenants

Execute the following statement to view the IP addresses of the nodes where the resource units of tenants are located.
...
## Query information about resource units on nodes

You can query the `GV$OB_UNITS` view for the information about resource units on all nodes. Here is an example:

* In the third row of the query result, the node IP address is `xx.xx.xx.218`, the port number is `2882`, the resource unit ID is `1001`, the tenant ID is `1002`, the zone name is `zone3`, the maximum number of CPU cores is `1.5`, the memory size is 5 GB, the log disk size is 16 GB, the log disk space used is 1.07 GB, the data disk space used is 572 MB, and the status of the resource unit is `NORMAL`.
...
For detailed information about the parameters in the `GV$OB_UNITS` view, see [GV$OB_UNITS](../../../700.reference/700.system-views/400.system-view-of-mysql-mode/300.performance-view-of-mysql-mode/3900.gv-ob_units-of-mysql-mode.md).

## Query information about resource allocation on nodes

You can query the `GV$OB_SERVERS` view for information about resource allocation on OBServer nodes. Here is an example:

In the first row of the query result, the node IP address is `xx.xx.xx.238`, the port number is `2882`, the zone name is `zone2`, the SQL port is `2881`, the number of CPU cores for the observer process is `64`, the memory available for the observer process is 54 GB, the memory allocated is 27.5 GB, the total log disk space is 167.25 GB, the log disk space allocated is 55 GB, the log disk space used is 22 GB, the total data disk space is 167.25 GB, and the data disk space used is 18.89 GB.
...
<!-- For detailed information about the parameters in the `GV$OB_SERVERS` view, see GV$OB_SERVERS. -->

## Query the usage and limits of logical resources of a tenant
...
The query result shows that the maximum number of log streams that you can create for tenant `1002` on OBServer node `172.xx.xxx.xxx:2882` is subject to the following limits (`9223372036854775807` indicates no limit):
