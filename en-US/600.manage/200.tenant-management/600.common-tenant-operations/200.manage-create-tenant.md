...
3. Create a tenant: When you create a tenant, you can allocate a resource pool to the tenant by specifying `RESOURCE_POOL_LIST`. You can query the `DBA_OB_TENANTS` view for all tenants. Each zone can use a resource pool with a different unit config. Alternatively, all zones can use the same resource pool with the same unit config. Besides the resource pool list, you must also set other important attributes and system variables such as the compatibility mode, primary zone setting, locality, and connection allowlist. The resource pool list attribute is required when you create a tenant. The password of the root user for a newly created tenant is empty. We recommend that you set a password before you use the root user.
...
A unit config defines the resource items such as the CPU, memory, disk space, and IOPS.
...
<main id="notice" type='explain'>
    ...
    <ul>
        ...
        <li>Log disk space isolation is supported between tenants. The system reserves specific log disk space for each tenant. This prevents the issue in OceanBase Database V3.x where other tenants are affected when the log disk is exhausted by a large number of writes in a single tenant. </li>
...
4. Execute the `CREATE RESOURCE UNIT` statement to create a unit config.
    ...
    * When you specify a value for a parameter, you can use a number without quotation marks ('), a number and a unit enclosed in quotation marks, or a number enclosed in quotation marks, for example, `'1T'`, `'1G'`, `'1M'`, or `'1K'`.
      ...
      The values in different formats are described as follows:
...
4. Execute the `CREATE RESOURCE POOL` statement to create a resource pool.
    ...
    * `ZONE_LIST`: the distribution of the resource pool in zones. OceanBase Database creates resource units in each zone specified by `ZONE_LIST` based on the number specified by the `UNIT_NUM` parameter. The unit config of each resource unit is the one specified by the `UNIT` parameter.
    ...
    Create a resource pool named `mq_pool_01`, create a resource unit for each of `zone1` and `zone2`, and specify the unit config `S1_unit_config` for the resource units.
...
4. Execute the `CREATE TENANT` statement to create a tenant.
    ...
    * `PRIMARY_ZONE`: the primary zone setting of the tenant.
