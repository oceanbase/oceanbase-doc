...
=======
* The automatic cleanup of backups varies depending on the backup media:
>>>>>>> 3c0f1df264 (大模型自动增量翻译自 commit 39fb25bc)

  * If the backup media are NFS or object storage services that are compatible with the S3 protocol (such as Amazon S3, Google Cloud Storage, and Alibaba Cloud Object Storage Service), the system directly deletes the backup files that meet the conditions.
  * If the backup destination is object storage services accessed by using the S3 protocol, the backup files are cleared based on the value of the `delete_mode` parameter specified for `data_backup_dest` or `log_archive_dest`. For more information, see [SET LOG_ARCHIVE_DEST](../../../700.reference/500.sql-reference/100.sql-syntax/100.system-tenants/200.alter-system/150.set-log-archive-dest.md) and [SET DATA_BACKUP_DEST](../../../700.reference/500.sql-reference/100.sql-syntax/100.system-tenants/200.alter-system/3500.set-data-backup-dest.md).

* If the path attribute contains the `enable_worm=true` parameter, which applies to a bucket with a compliance retention (Write Once Read Many) policy enabled, the system selects backup sets for cleanup based on the time specified by `RECOVERY_WINDOW`. However, whether the cleanup is successful depends on the cleanup mode (`delete_mode` parameter value) and the retention policy of the bucket corresponding to the path.

  <main id="notice" type='notice'>
  <h4>Notice</h4>
  <p>The <code>enable_worm=true</code> parameter applies to buckets with a compliance retention (WORM) policy enabled. In OceanBase Database V4.3.5 and later, buckets of Alibaba Cloud Object Storage Service (OSS) with a WORM policy can be specified as the backup or archive destination.</p>
  </main>

  For example, a backup set exists beyond the time specified by `RECOVERY_WINDOW`, but it is still within the WORM retention period:

  * If the cleanup mode of this backup set is `delete`, OceanBase Database will fail to clean up this backup set and you will need to wait until the Object's retention period expires.

  * If the cleanup mode of this backup set is `tagging`, OceanBase Database will tag this backup set. The tagged backup set will be deleted by the object storage service at the end of the object's lifecycle, after the WORM retention period has expired.
