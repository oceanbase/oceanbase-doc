...
During the restore of a table, you can execute the `ALTER SYSTEM CANCEL RECOVER TABLE` statement to stop the restore. This statement cancels all table-level restore tasks in the specified tenant, but does not release the resource pool that was created for the restore.
...
After you cancel the table-level restore operation, you can execute the following statement in the cluster where the destination tenant resides to manually release the resource pool that was created for the auxiliary tenant before the restore.
