...
| Table group          |     No      | When performing table-level restore, the corresponding table group must exist in the target tenant; otherwise, the restore will fail. For example, if the backed-up table originally belonged to a table group named `TG`, and the `REMAP TABLE` command (which allows renaming the table to another table group in the target tenant) is not used, the system will still restore the table into a table group named `TG` in the target tenant. |
