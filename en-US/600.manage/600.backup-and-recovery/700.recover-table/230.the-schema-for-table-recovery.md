...
| Tablet        |    Yes      | N/A |
| Column            |    Yes      | N/A |
| Constraint          |    Yes      | Supported constraints for restore:<ul><li>`NOT NULL` constraint </li> <li>`UNIQUE KEY` constraint </li> <li>`PRIMARY KEY` constraint <main id="notice" type='notice'><h4>Notice</h4><p>In Oracle mode, if there are duplicate <code>PRIMARY KEY</code> constraint names, the entire table restore will fail.</p></main></li> <li>`CHECK` constraint</li></ul> <main id="notice" type='explain'><h4>Note</h4><p>If the constraint name is user-defined and already exists in the target tenant, it will not be restored.</p></main> |
...
| Statistics      |    Yes      | N/A |
| Trigger        |    Yes      | <ul><li> If the table is restored using its original name, the associated triggers are restored. Among these:<ul><li> In Oracle mode, if the user associated with the trigger does not exist, the trigger restore will fail.</li> <li> In MySQL mode, if the database associated with the trigger does not exist, the trigger restore will fail.</li></ul></li> <li> If the table is renamed and restored using the new name, the associated triggers are not restored.</li></ul> |
