...
4. (Optional) Set the table-level restore degree of parallelism (DOP).

  a. Single-table restore

  Before performing table-level restore, you can set the DOP using the parameter [recover_table_dop](../../../700.reference/800.configuration-items-and-system-variables/100.system-configuration-items/400.tenant-level-configuration-items/7325.recover_table_dop.md). Once configured, the system will apply this DOP in the following two phases:
  ...
  * Index restore phase: Based on the restored primary table data, the system rebuilds the table's indexes using parallel execution (PX).
  ...
  b. (Optional) Multi-table restore

  Before performing a multi-table restore, you can set the DOP using the parameter [recover_table_concurrency](../../../700.reference/800.configuration-items-and-system-variables/100.system-configuration-items/400.tenant-level-configuration-items/7320.recover_table_concurrency.md). Then, multiple tables can perform restore tasks in parallel, which improves the restore performance.
  ...
  ```
   ...
   c. Set the number of restore worker threads on each observer node for table-level restore.

  After you set the table-level parameters [recover_table_dop](../../../700.reference/800.configuration-items-and-system-variables/100.system-configuration-items/400.tenant-level-configuration-items/7325.recover_table_dop.md) and [recover_table_concurrency](../../../700.reference/800.configuration-items-and-system-variables/100.system-configuration-items/400.tenant-level-configuration-items/7320.recover_table_concurrency.md), you must also set the [ddl_thread_score](../../../700.reference/800.configuration-items-and-system-variables/100.system-configuration-items/400.tenant-level-configuration-items/1850.ddl_thread_score.md) parameter to specify the number of restore worker threads on each observer node for table-level restore. During cross-tenant data export for table-level restore, the primary table data restoration depends on specific DAG threads in the target tenant.
...
<main id="notice" type='notice'>
  ...
  <p>
    <ul>
        <li><b>During the physical restore in the auxiliary tenant</b>, you can dynamically adjust the restore DOP by modifying the <a href="../../../700.reference/800.configuration-items-and-system-variables/100.system-configuration-items/400.tenant-level-configuration-items/3400.ha_high_thread_score.md">ha_high_thread_score</a> parameter. The adjustment takes effect immediately. The name of the auxiliary tenant can be queried from the <a href="../../../700.reference/700.system-views/300.system-view-of-sys-tenant/200.dictionary-view-of-sys-tenant/10200.o-cdb_ob_recover_table_jobs-of-sys-tenant.md">CDB_OB_RECOVER_TABLE_JOBS</a> table. For more information, see <a href="../600.restore-data/200.initiate-the-tenant-restore.md">Physical restore</a>. </li>
        <li><b>During cross-tenant data export</b>, you can dynamically adjust the parallelism for multi-table restore or single-table restore by modifying the value of the <code>recover_table_concurrency</code> or <code>recover_table_dop</code> parameter for the target tenant. The adjustment takes effect immediately.</li>
      </ul>
 </p>
</main>
...
5. Restore the specified table.
  ...
  <main id="notice" type='notice'>
  <h4>Notice</h4>
  <p><ul><li>Table data restoration is successful upon completion of the restore of the table. However, the restore of the index, constraint, or other schema elements associated with the table may fail. </li><li>After the restore of a table is completed, we recommend that you restore the <code>ddl_thread_score</code>, <code>recover_table_concurrency</code>, and <code>recover_table_dop</code> parameters to their original values. If you do not restore these parameters, the subsequent table-level restores will be performed based on the restored parameters. </li></ul></p>
  </main>
