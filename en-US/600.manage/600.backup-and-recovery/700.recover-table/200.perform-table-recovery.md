...
* Similar to tenant restore, table restore currently supports only restoring tables from backup data of a lower version to the same version or a higher version. The reverse restore between minor versions within the same major version is not supported.
...
4. Restore the specified table.
   ...
   * `[UNTIL TIME|SCN]=['timestamp'|scn]` specifies the timestamp or SCN to which the table is restored. To restore to a specified timestamp or SCN, join the parameter name and value with an equal sign (`=`). By default, if you do not specify the `UNTIL` clause, the table is restored to the latest checkpoint.

   * `restore_option` specifies the restore options of the auxiliary tenant, including `pool_list`, `locality`, `primary_zone`, and `concurrency`. `pool_list` is required and the others are optional. Separate the options with ampersands (`&`).
