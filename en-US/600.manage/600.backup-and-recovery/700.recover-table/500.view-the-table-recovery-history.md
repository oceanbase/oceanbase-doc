...
2. Execute the following statement to check the table restore result:
   ...
   * View the result of the entire table restore job:
      ...
      *************************** 1. row ***************************
      ...
      The fields in the `CDB_OB_RECOVER_TABLE_JOB_HISTORY` view correspond to those in the `CDB_OB_RECOVER_TABLE_JOBS` view. The `COMMENT` field records the error information of a failed job.
   ...
   * View the result of data import to the destination tenant:
      ...
      *************************** 1. row ***************************
      ...
      The fields in the `CDB_OB_IMPORT_TABLE_JOB_HISTORY` view correspond to those in the `CDB_OB_IMPORT_TABLE_JOBS` view. The `COMMENT` field records the error information of a failed job.
   ...
   * View the import result of each table:
   ...
   The fields in the `CDB_OB_IMPORT_TABLE_TASK_HISTORY` view correspond to those in the `CDB_OB_IMPORT_TABLE_TASKS` view. The `COMMENT` field records the error information of a failed task.

   For more information about the fields in the `CDB_OB_RECOVER_TABLE_JOB_HISTORY`, `CDB_OB_IMPORT_TABLE_JOB_HISTORY`, and `CDB_OB_IMPORT_TABLE_TASK_HISTORY` views, see [Views related to physical restore](700.views-of-the-table-recovery.md).
