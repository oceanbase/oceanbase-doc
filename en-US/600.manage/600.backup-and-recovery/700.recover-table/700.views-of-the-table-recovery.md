...
`CDB_OB_RECOVER_TABLE_JOB` and `DBA_OB_RECOVER_TABLE_JOBS` record table-level restore jobs.

| Column | Description |
|-----------------------|------------------------------------------------|
| TENANT_ID | The ID of the tenant that performed the table-level restore job. |
| JOB_ID | The ID of the table-level restore job. |
...
`CDB_OB_RECOVER_TABLE_JOB_HISTORY` and `DBA_OB_RECOVER_TABLE_JOB_HISTORY` views record historical table-level restore jobs.
...
`CDB_OB_IMPORT_TABLE_JOBS` and `DBA_OB_IMPORT_TABLE_JOBS` views record table import jobs, which are sub jobs of table-level restore jobs.
