...
* Since the current automatic cleanup feature applies only to the backup destinations specified in `DATA_BACKUP_DEST` and the archive destinations specified in `LOG_ARCHIVE_DEST`, if you change the archive destination, the data in the original archive path cannot be automatically cleaned up by OceanBase Database.
