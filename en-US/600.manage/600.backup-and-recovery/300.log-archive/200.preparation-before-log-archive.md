...
2. Configure the archive destination.
   ...
   * `LOCATION` (required)
      ...
      For detailed descriptions of each parameter, refer to [SET LOG_ARCHIVE_DEST](../../../700.reference/500.sql-reference/100.sql-syntax/100.system-tenants/200.alter-system/3800.set-log-archive-dest.md).
      ...
      * When using OSS as the archive medium, the system tenant sets the archiving path for the specified tenant `mysql_tenant`, configures the cleanup mode as `tagging`, specifies writing and deleting objects in a WORM-compliant manner, and sets the checksum algorithm to `md5`.

        <main id="notice" type='notice'>
        <h4>Notice</h4>
        <ul><li>OceanBase Database will not replace or override your configured compliance retention policies. Ensure that the OSS Bucket’s retention policy (WORM policy) is correctly configured and locked before setting the archiving path.</li>
        <li>When using a bucket configured with a WORM policy as the archiving path, we recommend that you use the <code>tagging</code> mode for cleanup.</li></ul>
        </main>
      ...
      * When using OSS as the archive medium, the user tenant sets the archiving path for its own tenant, configures the cleanup mode as `delete`, and specifies the checksum algorithm as `md5`.
      ...
      For more information about the `delete_mode` parameter and the `checksum_type` parameter, see [SET LOG_ARCHIVE_DEST](../../../700.reference/500.sql-reference/100.sql-syntax/100.system-tenants/200.alter-system/3800.set-log-archive-dest.md).
    ...
    <main id="notice" type='notice'>
      <h4>Notice</h4>
      <ul><li>When using object storage as an archive medium, separate the parameters in the object storage path with the <code>&</code> symbol. Please ensure that your parameter values contain only English letters (uppercase and lowercase), numbers, <code>/-_$+=</code>, and wildcards. If you enter any other characters, the settings may fail.</li>
      <li>Some object storage services compatible with the S3 protocol may not support the default URL encoding used by the AWS S3 SDK. When accessing via the <code>s3://</code> protocol, if you encounter object storage-related error codes such as 9129 or 9026, it is recommended to set the cluster-level configuration item <a href="../../../700.reference/800.configuration-items-and-system-variables/100.system-configuration-items/300.cluster-level-configuration-items/16620.ob_storage_s3_url_encode_type.md">ob_storage_s3_url_encode_type</a> to specify the URL encoding method used by AWS S3 requests for compatibility with the RFC3986 standard. For detailed instructions, please refer to <b>Operation steps and examples for using COS as an archive medium</b> below.</li></ul>
    </main>
      ...
      * When using GCS as the archive media, set the archiving path for the current tenant.
      ...
      For more information about the parameters, see [SET LOG_ARCHIVE_DEST](../../../700.reference/500.sql-reference/100.sql-syntax/100.system-tenants/200.alter-system/3800.set-log-archive-dest.md).
...
* For scenarios where the archive media is OSS or NFS, you can set the `archive_lag_target` parameter to any value within its valid range. For scenarios where the archive media is S3 or other object storage services compatible with the S3 protocol, the value of the `archive_lag_target` parameter must be greater than or equal to `60s`. If you set the value to be less than `60s`, the system will return an error.
