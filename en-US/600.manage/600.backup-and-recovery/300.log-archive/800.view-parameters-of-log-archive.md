...
=======
      The following example shows how to view the setting of the `LOG_ARCHIVE_DEST` parameter in MySQL mode:
>>>>>>> 3c0f1df264 (大模型自动增量翻译自 commit 39fb25bc)
...
=======
      The following example shows how to view the setting of the `LOG_ARCHIVE_DEST` parameter in Oracle mode:
>>>>>>> 3c0f1df264 (大模型自动增量翻译自 commit 39fb25bc)
...
=======
   * View the information about the archive path for log archiving of the current tenant.

      You can query the `oceanbase.DBA_OB_BACKUP_STORAGE_INFO` view in MySQL mode or the `sys.DBA_OB_BACKUP_STORAGE_INFO` view in Oracle mode for the archive path information for log archiving of the current tenant.

      :::tab
      tab MySQL mode

      The following example shows how to view the archive path information in MySQL mode:
      ...
      The following example shows how to view the archive path information in Oracle mode:
