...
User tenants can query the `oceanbase.DBA_OB_ARCHIVELOG` view (MySQL-compatible mode) or the `sys.DBA_OB_ARCHIVELOG` view (Oracle-compatible mode) for the archiving progress of the tenant.
...
2. View the log archiving progress of the current tenant.
   ...
   The following is an example of viewing the log archiving progress of a MySQL-compatible tenant:
