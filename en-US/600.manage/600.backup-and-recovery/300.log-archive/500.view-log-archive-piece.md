...
This topic only describes the method for viewing Piece in log archiving. For more information about Piece, see [Overview of log archiving](../300.log-archive/100.overview-of-log-archive.md).
...
2. Execute the following statement to view all pieces of all tenants.
   ...
   * The value of the `FILE_STATUS` column is `AVAILABLE` for the three pieces with the piece IDs `2` to `4`. This indicates that the data in the three pieces is not deleted and can be accessed. For piece 3, the STATUS became `FROZEN` before `CHECKPOINT_SCN` reached `'2022-06-04 06:00:00.000000'`. This is because the user disabled archiving, so no more logs will be archived to piece 3. Currently, tenant 1002 is undergoing the second round of archiving, and piece 4 is the first piece generated during this round of archiving. The status of piece 4 is `ACTIVE`, indicating that logs are being archived to the piece.
...
User tenants can query the `oceanbase.DBA_OB_ARCHIVELOG_PIECE_FILES` view (MySQL-compatible mode) or the `sys.DBA_OB_ARCHIVELOG_PIECE_FILES` view (Oracle-compatible mode) for all Piece information of the tenant.
...
2. View all pieces of the current tenant.
   ...
   The following is an example of viewing all Pieces of a MySQL-compatible tenant:
