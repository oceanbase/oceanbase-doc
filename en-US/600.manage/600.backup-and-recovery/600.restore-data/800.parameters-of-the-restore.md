...
This topic describes the parameters in the `RESTORE` statement.

## Parameters in the RESTORE statement
...
## Parameters in the RECOVER statement
...
| timestamp          | The timestamp to which data is restored. Join the parameter name and value with an equal sign (=). The value of `timestamp` must be in the `YYYY-MM-DD HH24:MI:SS.FF` format. The `FF` supports 0 to 6 decimal places, accurate to microseconds. |
| scn                | The SCN to which data is restored. Join the parameter name and value with an equal sign (=). |
...
| Parameter | Description |
|--------------------|-------------------|
| tenant_name         | The name of the standby tenant restored. You can use this statement to change the role of only one tenant at a time. |
