...
3. Execute the following statement to start a restore job:
   ...
   * In `restore_option`, you can specify `pool_list`, `locality`, `primary_zone`, and `concurrency`, where `pool_list` is required and the others are optional. Use `&` to separate the parameters.

      If `concurrency` is not specified, the `MAX_CPU` value assigned to the tenant is used as the default value. In this example, the value of `MAX_CPU` is `16` for the standby tenant.
