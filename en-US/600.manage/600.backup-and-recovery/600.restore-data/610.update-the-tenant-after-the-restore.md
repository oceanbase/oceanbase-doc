...
The restored standby tenant has been converted into the primary tenant. For more information, see [Convert a standby tenant into primary tenant](600.active-standby-tenant.md).
...
3. After the upgrade, check whether the tenant is successfully upgraded. The query statement varies with the version of the current OceanBase cluster.
   ...
   * When the current OceanBase cluster is of a version earlier than V4.2.1.1:
      ...
      Here, `1004` is the `tenant_id` value of the upgraded tenant. You can query the `DBA_OB_TENANTS` view for the value of `tenant_id`.
   ...
   * When the current OceanBase cluster is of V4.2.1.1 or later:
      ...
      * `1004` is the `tenant_id` value of the upgraded tenant. You can query the `DBA_OB_TENANTS` view for the value of `tenant_id`.
