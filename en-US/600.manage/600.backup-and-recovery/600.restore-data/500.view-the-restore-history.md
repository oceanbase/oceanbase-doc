...
After a physical restore job is completed, you can query the restore results from views.

## Procedure

1. Log in to the `sys` tenant of the cluster as the `root` user.

2. After a restore job is completed, execute the following statement to query the restore result:
   ...
   *************************** 2. row ***************************
   ...
   ```
    ...
    If the restore fails, you can cancel the restore by deleting the tenant. For more information, see [Cancel physical restore](../600.restore-data/300.cancel-a-restore.md).
...
* Execute the `RECOVER` statement to replay logs from the source tenant. For more information, see [Replay logs to a standby tenant](510.recover-the-standby-tenant.md).
