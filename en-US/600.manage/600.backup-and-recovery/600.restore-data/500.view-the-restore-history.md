...
2. After a restore job is completed, execute the following statements to view the restore result:
   ...
   *************************** 2. row ***************************
   ...
   The columns in the `CDB_OB_RESTORE_HISTORY` view correspond to those in the `CDB_OB_RESTORE_PROGRESS` view. The `COMMENT` column records the error information of a failed job or task.
...
* Run the `recover` command to replay logs from the source tenant. For more information, see [Replay logs to a standby tenant](../600.restore-data/700.recover-the-standby-tenant.md).

* Execute the `ACTIVATE` statement to convert the standby tenant into primary tenant. For more information, see [Convert a standby tenant into primary tenant](../600.restore-data/600.active-standby-tenant.md).
