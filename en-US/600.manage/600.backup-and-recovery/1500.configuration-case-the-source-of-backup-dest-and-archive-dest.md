...
![Before configuring the source information](https://obbusiness-private.oss-cn-shanghai.aliyuncs.com/doc/img/observer-enterprise/V4.2.5/600.backup-and-recovery/before_config_fixed.png)
...
In this scenario, based on the rule that backup tasks cannot access NFS across cities, you can configure the source information of the backup path to `region=Region1`, `idc=IDC1,IDC2`, or `zone=Z1,Z2,Z3,Z4`.

When configuring the source information, the metadata file of the data backup is operated by the RS node. To avoid losing access permissions to the backup path due to the RS node being outside the configured source range, which can lead to backup task failures, we recommend that you set the `PRIMARY_ZONE` of the tenant to `ALTER TENANT tenant_name PRIMARY_ZONE='z1,z2'`.

Assume that the backup path is `file:///data/nfs/backup/data`. The following statement configures the source information of the backup path to `region=Region1`:
...
![Backup tasks are forwarded to IDC2.](https://obbusiness-private.oss-cn-shanghai.aliyuncs.com/doc/img/observer-enterprise/V4.2.5/600.backup-and-recovery/after_config_fixed.png)

For example, you can use the following statement to configure the source to `idc=IDC1;IDC2`:

```shell
obclient> ALTER SYSTEM CHANGE EXTERNAL_STORAGE_DEST PATH='file:///data/nfs/backup/data' SET ATTRIBUTE='idc=IDC1;IDC2';
```

After the modification, when the RS selects nodes to execute backup tasks, nodes in IDC1 have higher priority than nodes in IDC2. This allows backup tasks to primarily use intra-IDC network resources. IDC2 serves as a backup path, enhancing the tolerance of backup tasks to exceptions (such as node failures in IDC1). Additionally, if the RS is switched to IDC2, it will still have access to the backup media, preventing backup failures.

![Modify the source configuration of the backup path.](https://obbusiness-private.oss-cn-shanghai.aliyuncs.com/doc/img/observer-enterprise/V4.2.5/600.backup-and-recovery/modify_the_configuration.png)

### Configure the source information for the log archive path

Similarly, for log archiving, in this scenario, based on the rule that NFS cannot be accessed across regions, the source information for the archive path can be configured as `region=Region1`, `idc=IDC1;IDC2`, or `zone=Z1,Z2;Z3,Z4`. Since all log archiving traffic is handled by the leader replica, there is no difference in access priority for the log archive path. Therefore, when configuring, `zone=Z1,Z2;Z3,Z4` is equivalent to `zone=Z1,Z2,Z3,Z4`, and `idc=IDC1;IDC2` is equivalent to `idc=IDC1,IDC2`.
...
After the configuration is successful, it can prevent unexpected access requests (such as requests from OBServer nodes in Region2) that may occur if the leader replica is switched to Region2 due to maintenance or other reasons. If the system detects that the leader replica is accessing the archive path in Region2, it will generate an error log (error code `-9063`) to alert the administrator to check: (1) whether the node has lost access rights due to a change in the locality name; (2) whether the leader replica has been switched to a node without access rights, and to update the source configuration or manually switch the leader replica to an OBServer node with access rights.
...
<h4>Note</h4>
<p>The <code>PRIMARY_ZONE</code> of the tenant must be consistent with the source configuration to prevent the leader replica from losing access rights to the archive path if it is outside the source configuration range, which would prevent the archiving process from progressing. </p>
</main>
