...
In the current version, both the standalone node mode (SN mode) and the shared storage mode (SS mode) use obdal to access object storage. obdal is a unified framework for object storage developed based on OpenDAL. For more information about OpenDAL, see [OpenDAL](https://github.com/oceanbase/opendal).
