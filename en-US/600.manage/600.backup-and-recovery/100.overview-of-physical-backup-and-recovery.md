...
* Tenant information, including the tenant name, cluster name, time zone, locality, and tenant compatible mode (MySQL compatible or Oracle compatible)
...
3. You can specify a point in time for physical restore.
   ...
   * Partial restore to the specified system change number (SCN) or timestamp: An SCN is a precise data version number in OceanBase Database. A timestamp is accurate to microseconds in MySQL-compatible mode, and the precision after microseconds will be lost.
