...
3. Execute the following command to view the NIC speed.
...
After the modification, you can query the [V$OB_NIC_INFO](../../700.reference/700.system-views/400.system-view-of-mysql-mode/300.performance-view-of-mysql-mode/32250.v-ob_nic_info-of-mysql-mode.md) view to confirm whether the modified NIC speed takes effect.
...
| Parameter                   | Description                                 | Default value |  Remarks          |
|-------------------------|--------------------------------------|--------|-------------------|
| log_archive_concurrency | The tenant-level parameter specifies the concurrency of log archiving. | 0, which means the system calculates the number of archive worker threads for the tenant based on the value of `MAX_CPU` in the following adaptive rules:<ul><li>If `tenant's MAX_CPU <= 8`, `archive threads = MAX_CPU`.</li> <li>If `8 < tenant's MAX_CPU < 32`, `archive threads = max(8, MAX_CPU/2)`. If the result of `MAX_CPU/2` is less than 8, the system uses 8.</li> <li>If `tenant's MAX_CPU >= 32`, `archive threads = max(16, MAX_CPU/4)`. If the result of `MAX_CPU/4` is less than 16, the system uses 16.</li></ul>  | We recommend that you keep the default value for small and large tenants and let the system calculate the number of worker threads based on the adaptive rules.

## Physical restore

| Parameter | Description | Default value | Remarks |
|-------------------------|--------------------------------------|-------|--------------------------------------|
| log_restore_concurrency | The tenant-level parameter specifies the concurrency of log restore. | 0, which means the number of CPU cores of the `MAX_CPU` value for the tenant. | Increasing the value of this parameter increases the number of worker threads and the memory resource overhead. We recommend that you set this parameter to the default value of 0. If the restore speed is too slow, you can appropriately increase the value of this parameter based on the actual resources of the server. |
...
| _restore_idle_time      | The cluster-level hidden parameter controls the scheduling interval of RS restore. | 1m, which means 1 minute. | If you set this parameter to 10s, the data restore time can be reduced by several dozen seconds to two minutes. We recommend that you set this parameter for small-scale tenants with high performance requirements (CPU ≤ 4C). Otherwise, the effect is not significant. |

## Table-level restore

### Physical restore of auxiliary tenant

You can use the views introduced in [Views related to table-level restore](700.recover-table/700.views-of-the-table-recovery.md) to obtain the name of the temporary auxiliary tenant associated with the task. By default, the name of the auxiliary tenant starts with the `AUX` prefix.
...
| recover_table_concurrency | The tenant-level parameter specifies the concurrency for restoring multiple tables, that is, the number of tables that can have cross-tenant import performed simultaneously.  | 0, which indicates that one table can be imported at a time. | We recommend that you set this parameter to the default value of 0. In addition: <ul><li>If the restore speed is too slow in scenarios involving the restoration of multiple tables, you can double the value based on the actual resources of the tenant. </li> <li>In scenarios involving the restoration of a large number of small tables, we recommend that you set this parameter to 8 or 16. </li></ul>   <main id="notice" type='notice'><h4>Notice</h4><p>Cross-tenant import consumes a large amount of temporary storage space and memory resources. We recommend that you increase the values based on the resources of the tenant. The consumption of temporary space is particularly high in the index reconstruction phase. If a table contains a large amount of data, a large amount of temporary space may be consumed. High consumption of temporary space may cause query failures, transaction rollbacks, and service unavailability. </p></main> |
| recover_table_dop         | The tenant-level parameter specifies the concurrency for restoring a single table. | 0, which indicates the default concurrency of 1. | We recommend that you set this parameter to 8 or 16, which can meet the requirements of most scenarios. <p>If the table to be restored is a non-partitioned table with a large amount of data or a table with a partition containing a large amount of data, we recommend that you double the value of the <code>recover_table_dop</code> parameter based on the data volume of the partition.</p> |

## References

* [Check NIC speed](https://en.oceanbase.com/docs/common-ocp-10000000002168977)

* [Configure resource isolation within a tenant (MySQL-compatible mode)](../200.tenant-management/600.common-tenant-operations/300.resource-isolation/300.resource-isolation-of-mysql-mode/200.resource-isolation-at-user-level-of-mysql-mode.md)

* [Configure resource isolation within a tenant (Oracle-compatible mode)](../200.tenant-management/600.common-tenant-operations/300.resource-isolation/200.resource-isolation-of-oracle-mode/200.resource-isolation-at-user-level-of-oracle-mode.md)

* [Update resource management plan directives (MySQL-compatible mode)](../200.tenant-management/600.common-tenant-operations/300.resource-isolation/300.resource-isolation-of-mysql-mode/400.update-a-plan-directive-of-mysql-mode.md)

* [Update resource management plan directives (Oracle-compatible mode)](../200.tenant-management/600.common-tenant-operations/300.resource-isolation/200.resource-isolation-of-oracle-mode/400.update-a-plan-directive-of-oracle-mode.md)
