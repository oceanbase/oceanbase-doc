...
4. Wait for the data backup to be completed.
   ...
   You can query the `DBA_OB_BACKUP_TASKS` view to check whether the data backup job is completed. If the returned job list is empty, the data backup job is completed.
...
3. Initiate a physical restore job to restore data to the `oracle_backup` tenant.
