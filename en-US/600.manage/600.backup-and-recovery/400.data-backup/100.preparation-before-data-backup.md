...
=======
    Starting from V4.3.5 BP2, OceanBase Database allows you to use an Alibaba Cloud OSS bucket configured with a compliance-based retention policy as the backup destination. When you configure the backup destination, you can specify the `enable_worm` parameter to indicate whether to comply with the bucket's retention policy (WORM policy) for write and delete operations on the path. The supported values are `true` and `false`. If you do not specify the `enable_worm` parameter, the default value is `false`. If you set `enable_worm=true`, you must explicitly specify `checksum_type=md5`. This parameter cannot be modified after it is set.
>>>>>>> 3c0f1df264 (大模型自动增量翻译自 commit 39fb25bc)
...
=======
    * Assume that you want to use OSS as the backup media. The system tenant configures the backup destination for the `mysql_tenant` tenant, specifies the backup file cleanup mode, specifies to write and delete objects in WORM-supported ways, and sets the checksum algorithm to `md5`.
    ...
    * Assume that you want to use OSS as the backup media. The user tenant configures the backup destination for the current tenant and sets the checksum algorithm to `md5`.
>>>>>>> 3c0f1df264 (大模型自动增量翻译自 commit 39fb25bc)
...
=======
    In the preceding examples, `oss://` indicates that OSS is used as the backup media. The bucket name is `oceanbase-test-bucket`, and the path in the bucket is `/backup/data`. The `?` character is used to separate the path and parameters, and the `&` character is used to separate the parameters. The `host` parameter specifies the endpoint of the storage bucket. The `access_id` and `access_key` parameters specify the access key of `OSS`. The `delete_mode` parameter is set to `delete`, and the MD5 algorithm is used to verify the integrity of archived files. The `enable_worm=true` parameter specifies to comply with the write and delete operations in WORM mode. Note that OceanBase Database does not replace your settings of the compliance-based retention policy. Before you set the backup destination, make sure that the OSS bucket has a correctly configured and locked retention policy (WORM policy).
...
=======
    In the preceding examples, `s3://` indicates that an Amazon S3 bucket is used as the backup media. The bucket name is `oceanbase-test-bucket`, and the path in the bucket is `/backup/data`. The `?` character is used to separate the path and parameters, and the `&` character is used to separate the parameters. The `host` parameter specifies the domain name of the Amazon S3 service. The `access_id` and `access_key` parameters specify the access key of the Amazon S3 service. The `s3_region` parameter is required when an Amazon S3 bucket is used as the backup media. It specifies the region where the Amazon S3 bucket is located. The `delete_mode` parameter is set to `delete`, and the CRC32 algorithm is used to verify the integrity of backup data.
>>>>>>> 3c0f1df264 (大模型自动增量翻译自 commit 39fb25bc)
...
<<<<<<< HEAD
    In the example above, `s3://` indicates accessing the backup destination using the S3 protocol. The bucket name is `oceanbase-test-bucket`, and the path in the bucket is `/backup/data`. The `?` is used to separate the path from the parameters, and the parameters are separated by `&`. The `host` parameter is used to set the domain name of the OBS service, while `access_id` and `access_key` are used to configure the access credentials for the OBS service.
=======
...
<<<<<<< HEAD
   In the example above, `s3://` indicates accessing the backup destination using the S3 protocol. The bucket name is `oceanbase-test-bucket`, and the path in the bucket is `/backup/data`. The `?` is used to separate the path from the parameters, and the parameters are separated by `&`. The `host` parameter is used to set the domain name of the GCS service, while `access_id` and `access_key` are used to configure the access credentials for the GCS service.
=======
...
<<<<<<< HEAD
        In the example, `s3://` indicates accessing the backup destination using the S3 protocol. The bucket name is `oceanbase-test`, and the path in the bucket is `/backup/archive`. The `?` is used to separate the path from the parameters, and the parameters are separated by `&`. The `host` parameter is used to set the COS host address, which is the endpoint (the address without the bucket name). The `access_id` and `access_key` are used to configure the access credentials for COS, and the cleanup mode is set to `delete`.

    :::

3. After the configuration is successful, you can verify the detailed configuration information through the views.

   The `sys` tenant can view the information via the `CDB_OB_BACKUP_PARAMETER` view, while user tenants can view it via the `DBA_OB_BACKUP_PARAMETER` view.

   For more information about data backup-related parameters and how to view them, refer to [View parameters related to data backup](700.parameters-of-data-backup.md).
=======
