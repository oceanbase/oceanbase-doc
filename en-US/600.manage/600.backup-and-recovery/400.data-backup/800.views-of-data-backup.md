...
`oceanbase.CDB_OB_BACKUP_TASKS` displays the progress of data backup jobs for tenants.

| Column | Description |
|----------------------------------|--------------|
| TENANT_ID | The ID of the tenant that performed the backup job. |
| TASK_ID | The ID of the backup job for a backup set. |
| JOB_ID | The ID of the job to which the backup job belongs. |
| INCARNATION | The ID of the incarnation. |
| BACKUP_SET_ID | The ID of the backup set corresponding to the backup job. |
| START_TIMESTAMP | The time when the backup job was initiated. |
| END_TIMESTAMP | The time when the backup job ended. |
| STATUS | The status of the backup job. |
| START_SCN | The start SCN of the backup job. |
| END_SCN | The end SCN of the backup job. |
...
`oceanbase.CDB_OB_BACKUP_TASK_HISTORY` displays the backup job history of tenants.
