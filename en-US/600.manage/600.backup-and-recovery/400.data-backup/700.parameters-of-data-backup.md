...
2. Query the settings of backup parameters.
   ...
   * Query the backup paths of tenants.

      You can query the `oceanbase.CDB_OB_BACKUP_PARAMETER` view from the `sys` tenant for the backup paths of all tenants in the current cluster. You can query the `oceanbase.DBA_OB_BACKUP_PARAMETER` view from a user tenant in MySQL mode or the `SYS.DBA_OB_BACKUP_PARAMETER` view from a user tenant in Oracle mode for the backup path of the current tenant.
     ...
     In this example, the data backup path (specified by `data_backup_dest`) of the tenant with the `TENANT_ID` value `1004` is `oss://oceanbase-test-bucket/backup/?host=xxx.aliyun-inc.com&access_id=xxx&access_key=xxx`, and the backup file cleanup mode (specified by `delete_mode`) of the tenant is `delete`. You can set `delete_mode` to either of the following values:
