...
<<<<<<< HEAD
    * View the backup parameter information of a tenant.

        The `sys` tenant can view the backup parameter information configured for each tenant in the current cluster through the `oceanbase.CDB_OB_BACKUP_PARAMETER` view. User tenants can view the backup parameter information configured for their own tenant via the `oceanbase.DBA_OB_BACKUP_PARAMETER` view (in MySQL mode) or the `SYS.DBA_OB_BACKUP_PARAMETER` view (in Oracle mode).
=======
...
=======
          In this mode, when you clean up backup files, the system sets a tag for these files. The `key` of the tag is `"delete_mode"` and the `value` is `"tagging"`. This way, you can manage the lifecycle of these files on Alibaba Cloud OSS, Tencent Cloud COS, or Amazon S3 based on the tag.

      In the backup path parameters for this tenant, `checksum_type=md5` indicates that the MD5 algorithm is used to verify the integrity of backup files; `enable_worm=true` indicates that OceanBase Database performs write and delete operations on this path in accordance with the WORM specification.

   * Query the backup paths of a tenant.

      The `CDB_OB_BACKUP_STORAGE_INFO` view records the backup or archive paths of all tenants.
      ...
      In the result, for the tenant with the ID of `1004`, its backup path is `oss://oceanbase-test-bucket/backup/data`. The parameters `checksum_type=md5` and `enable_worm=true` indicate that the MD5 algorithm is used to verify the integrity of backup files and OceanBase Database performs write and delete operations on this path in accordance with the WORM specification.
>>>>>>> 3c0f1df264 (大模型自动增量翻译自 commit 39fb25bc)
