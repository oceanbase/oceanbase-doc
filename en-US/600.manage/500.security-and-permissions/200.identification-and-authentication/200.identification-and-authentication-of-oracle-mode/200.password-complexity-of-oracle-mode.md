...
|slug|password-complexity-oracle-mode|

# Password complexity

OceanBase Database has a password complexity policy for Oracle-compatible mode, which is compatible with Oracle Database's password policy. OceanBase Database allows you to set password complexity in Oracle-compatible mode, in order to verify the identity of login users, prevent malicious password attacks, thereby improving database security.

<main id="notice" >
    <h4>Applicability</h4>
    <p>This topic applies only to the Oracle-compatible mode of OceanBase Database. OceanBase Database Community Edition only supports MySQL-compatible mode. For more information about password complexity in MySQL-compatible mode, see <a href="../100.identification-and-authentication-of-mysql-mode/200.password-complexity-of-mysql-mode.md">Password complexity</a>. </p>
</main>

## Prerequisites

In Oracle-compatible mode, you can specify the `PASSWORD_VERIFY_FUNCTION` parameter in the profile to check whether the password complexity meets the requirements. To verify the password complexity, you must create a PL function that meets the following requirements:
...
The Oracle-compatible mode of OceanBase Database provides the `PASSWORD_VERIFY_FUNCTION` parameter to verify password complexity.
...
1. Log in to an Oracle-compatible tenant of a cluster as the `SYS` user.
...
4. After you complete the previous steps, execute the following statement to verify whether password complexity verification is enabled.
