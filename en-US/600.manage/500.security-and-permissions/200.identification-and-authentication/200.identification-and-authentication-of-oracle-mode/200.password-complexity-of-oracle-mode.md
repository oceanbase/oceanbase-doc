...
When you create a user or modify a password, the verification function will be called, and the new password will be judged whether it meets complexity requirements based on the execution result of the function.
...
3. Query the `USER_SOURCE` view for the content of the `VERIFY_FUNCTION` function owned by the current user.
