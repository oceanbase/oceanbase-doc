...
In Oracle mode of OceanBase Database, for users who have multiple failed login attempts, the system will lock the users to prevent malicious password attacks, thereby improving database security.

The Oracle mode of OceanBase Database uses a profile to lock a user.

<main id="notice" >
  <h4>Applicability</h4>
  <p>This topic applies only to the Oracle mode of OceanBase Database. OceanBase Database Community Edition supports only the MySQL mode. For more information about how to handle login failures in MySQL mode, see <a href="../100.identification-and-authentication-of-mysql-mode/300.login-failure-handling-of-mysql-mode.md">Login failure handling</a>. </p>
</main>

## Concepts

The profile limits the resources used by database users. In OceanBase Database in Oracle mode, the profile controls login verification strategies for users. You can use the profile to implement user lockout.
...
The user lock and unlock strategies are as follows:

* Lock strategy: When there are repeated failed login attempts due to an incorrect password, if the failure threshold is reached, the user will be locked out.
* Unlock strategy: If a user is locked out, the system checks the lockout duration when the user attempts to log in again. If the lockout duration has expired, the system unlocks the user, and the user can attempt to log in again. If the lockout duration has not expired, the system returns an error, and you can unlock the user by using an SQL statement.
...
2. Apply the profile to a user.
   ...
   Take note of the following two scenarios:

   * When you create a user, you can directly specify the profile file that has already been created.
...
2. Create a profile and set up a strategy for consecutive failed logins.

   Create a profile named `userprof1` and configure a strategy to lock the user for one day upon five consecutive login failures.
...
## Modify the login failure handling strategy
