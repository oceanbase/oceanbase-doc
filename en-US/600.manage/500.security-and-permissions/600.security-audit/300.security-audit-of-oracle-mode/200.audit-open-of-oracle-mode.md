...
4. Enable the auditing feature and set the storage location for auditing records.
   ...
   * `LOG`: writes auditing records to the audit log. This value is supported starting from V4.4.2 BP2.
   ...
   * `DB,EXTENDED` / `DB_EXTENDED`: writes auditing records to internal tables and includes executed SQL statements.
   ...
   <main id="notice" type='explain'>
      <h4>Note</h4>
      <p>Auditing records can be written to internal tables, local disk files, or audit logs:</p>
      <ul>
      ...
      <li>Audit logs: Auditing records are written to audit log files. Management strategies such as log archiving, compression, rotation, cleanup, and encryption are supported. This feature is available starting with V4.4.2 BP2.</li>
