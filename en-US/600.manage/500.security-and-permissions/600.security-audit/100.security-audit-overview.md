...
* Modification, overwriting, and deletion of auditing data are not supported.

### Procedure

The auditing procedure for Oracle tenants includes enabling security auditing, setting and viewing audit rules, and viewing audit records. To enable auditing, you must configure parameters such as `audit_trail` and enable `ORAAUDITOR`. Rules are set using `AUDIT` or `NOAUDIT`. If audit records are written to disk files, archived logs can be encrypted using the `audit_log_encryption` parameter and two audit functions (supported starting from V4.4.2 BP2). Auditing checks are performed after a user SQL statement has finished executing and before the response packet is prepared. The procedure is as follows:
