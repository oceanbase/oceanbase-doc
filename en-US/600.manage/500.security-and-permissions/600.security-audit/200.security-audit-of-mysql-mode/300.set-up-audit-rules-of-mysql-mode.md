...
You can set the auditing log strategy by using the [audit_log_strategy](../../../../700.reference/800.configuration-items-and-system-variables/100.system-configuration-items/400.tenant-level-configuration-items/295.audit_log_strategy.md) parameter. This parameter determines how auditing logs are written. Valid values are `ASYNCHRONOUS`, `PERFORMANCE`, and `SYNCHRONOUS`. Choose a log strategy based on your business needs to balance performance and log integrity.
...
When asynchronous log writing is enabled, you can control the buffer size with the [audit_log_buffer_size](../../../../700.reference/800.configuration-items-and-system-variables/100.system-configuration-items/400.tenant-level-configuration-items/210.audit_log_buffer_size.md) parameter.

## Set a log format

You can set the auditing log format using the [audit_log_format](../../../../700.reference/800.configuration-items-and-system-variables/100.system-configuration-items/400.tenant-level-configuration-items/240.audit_log_format.md) parameter. Currently, only the CSV format is supported.
...
You can specify whether to record SQL statements in auditing logs using the [audit_log_query_sql](../../../../700.reference/800.configuration-items-and-system-variables/100.system-configuration-items/400.tenant-level-configuration-items/280.audit_log_query_sql.md) parameter. Valid values are ALL and NONE.
...
The [audit_log_max_size](../../../../700.reference/800.configuration-items-and-system-variables/100.system-configuration-items/400.tenant-level-configuration-items/250.audit_log_max_size.md) and [audit_log_prune_seconds](../../../../700.reference/800.configuration-items-and-system-variables/100.system-configuration-items/400.tenant-level-configuration-items/270.audit_log_prune_seconds.md) parameters control the capacity and retention period of archived auditing logs. Both parameters can take effect simultaneously. When a new log file is generated during log archiving, the system checks whether log cleanup is needed. The total log size is calculated separately for each OBServer.
...
You can set the [audit_log_compression](../../../../700.reference/800.configuration-items-and-system-variables/100.system-configuration-items/400.tenant-level-configuration-items/220.audit_log_compression.md) parameter to enable log compression. If log compression is enabled, archived log files are generated in compressed format. Only the `ZSTD` compression algorithm is supported, and streaming compression is available. Compressed files can be directly decompressed using the external tool [zstd](https://github.com/facebook/zstd).
