...
```sql
-- Set the transparent tablespace encryption method to internal or obcloud.
ALTER SYSTEM SET tde_method = '<encryption_method>';
ALTER INSTANCE ROTATE INNODB MASTER KEY;	-- The first execution needs to wait about 20 seconds for the key to take effect.
```
...
Before you create a column encryption rule, you must understand the syntax. OceanBase Database provides complete DDL statements to manage column encryption rules, including creating, dropping, and modifying rules.
...
In actual use, you may need to adjust the column encryption rules based on business requirements. OceanBase provides flexible modification syntax to meet these needs:
...
-- If you do not specify the encryption algorithm, the default algorithm, AES-256-ECB, is used. This is equivalent to USING ENCRYPTION = 'aes-256'.
...
* [Manage column encryption privileges](300.column-encryption-permissions-of-mysql-mode.md)
