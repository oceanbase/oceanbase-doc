...
* If the user executing the `INSERT INTO ... SELECT ...` statement has the necessary permissions to access the plaintext data, the inserted data will be plaintext. When querying, the data will also be plaintext, unless there are additional encryption rules on the table.
* If the user does not have the necessary permissions to access the plaintext data, the operation is not allowed.
...
When setting values from an encrypted column to a non-encrypted column, such as in `UPDATE test_01 SET age = id + 2;`, if the user executing the statement has the necessary permissions to access the plaintext data, the non-encrypted column's value will be handled as plaintext. Otherwise, the user cannot execute this statement.
...
During tenant-level backup and restore, column encryption rules are restored. However, table-level restore, which recovers independent objects, does not inherit the source table's column encryption rules and permissions. Therefore, if the source table contains column encryption rules, it is recommended to assess whether to add separate access controls for the restored data.
