...
1. Log in to a MySQL-compatible tenant of a cluster as an administrator.
...
1. Log in to a MySQL-compatible tenant of the database as a regular user.

2. After entering the database where the table is located, move the `t1` table into the `sectest_ts1` tablespace.
...
2. Set the `progressive_merge_num` parameter to perform a full compaction or progressive compaction on the table.
   ...
   * Perform a progressive compaction on the table
      ...
      1. Set the `progressive_merge_num` parameter to a value greater than `1`, and run the `OPTIMIZE` command to prepare for a progressive compaction.
