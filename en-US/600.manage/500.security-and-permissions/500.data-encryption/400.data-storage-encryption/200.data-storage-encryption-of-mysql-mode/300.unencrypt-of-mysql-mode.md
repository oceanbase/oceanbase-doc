...
You can decrypt a table by removing it from the encrypted tablespace.
...
1. Log in to a MySQL-compatible tenant of the database as the administrator.
...
6. Query the `oceanbase.V$OB_ENCRYPTED_TABLES` view to check the encryption status of the table.
   ...
   If the `ENCRYPTED` field of the `t1` table is `NO`, the decryption was successful.
