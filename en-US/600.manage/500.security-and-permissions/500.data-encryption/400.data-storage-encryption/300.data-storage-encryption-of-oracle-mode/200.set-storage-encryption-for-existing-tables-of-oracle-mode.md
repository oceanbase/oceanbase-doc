...
## Configure transparent encryption in internal mode
...
1. Log in to an Oracle-compatible tenant of a cluster as an administrator.
...
5. Enable the Keystore.
   ...
   Replace `password` with the Keystore key set in the previous step.
...
1. Log in to an Oracle-compatible tenant of the database as a regular user.
...
3. Modify the index table and add the indexes on the table to the `sectest_ts1` tablespace.
...
## Perform transparent encryption on a table

1. Log in to an Oracle-compatible tenant of the database as a regular user.

2. Set the `progressive_merge_num` parameter to perform a full compaction or progressive compaction on the table.
   ...
   * Perform a full compaction on the table
      ...
      2. Manually initiate a major compaction.
         ...
         <h4>Note</h4>
         <p>After a full compaction is completed, all table data is encrypted. You can query the <code>V$OB_ENCRYPTED_TABLES</code> view to verify the encryption status. </p>
         </main>
   ...
   * Perform a progressive compaction on the table
      ...
      1. Set the `progressive_merge_num` parameter to a value greater than `1`, and perform the `SHRINK SPACE` operation to prepare for a progressive compaction.
      ...
      2. Manually initiate multiple rounds of progressive compactions to encrypt all existing macroblocks of the table and its indexes.
         ...
         <main id="notice" type='explain'>
         <h4>Note</h4>
         <p>During a progressive compaction, you can query the <code>V$OB_ENCRYPTED_TABLES</code> view to monitor the encryption progress in real time. </p>
         </main>

3. After the operation is completed, you can query the following view to verify whether all macroblocks are encrypted:
