...
1. Log in to an Oracle-compatible tenant of a cluster as the administrator.
...
2. Perform progressive compactions on the table.
   ...
   2. Manually initiate multiple rounds of progressive compactions.

      The statement to initiate a progressive compactions is as follows:
...
5. Query the `V$OB_ENCRYPTED_TABLES` view to check the encryption status of the table.
   ...
   If the `ENCRYPTED` field of table `t1` is `NO`, the decryption was successful.
