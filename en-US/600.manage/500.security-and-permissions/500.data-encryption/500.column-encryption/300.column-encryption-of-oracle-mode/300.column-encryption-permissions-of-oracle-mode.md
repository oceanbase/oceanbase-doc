...
|---------|------|-----------|----------|----------|----------|----------|
| CREATE SENSITIVE RULE | This privilege enables the user to create and drop sensitive rules. | SYS user. | Automatically granted during system upgrades. | The SYS user privilege cannot be revoked. | You can use the `CREATE/DROP SENSITIVE RULE` syntax. | This privilege is included in `ALL PRIVILEGES`. |
...
In Oracle mode, you cannot use the `SHOW GRANTS` statement to view privilege information. Instead, you can query the following views:

| View | Description |
|--------|------|
| [DBA_OB_SENSITIVE_RULE_PLAINACCESS_USERS](../../../../../700.reference/700.system-views/500.system-view-of-oracle-mode/200.dictionary-view-of-oracle-mode/14131.dba_ob_sensitive_rule_plainaccess_users-of-oracle-mode.md) | View all users with plaintext access privilege. |
| `ALL_OBJECTS`, `DBA_OBJECTS`, and `USER_OBJECTS` | View the protected objects of sensitive rules. |
| `DBA_TAB_PRIVS`, `DBA_SYS_PRIVS`, `ALL_TAB_PRIVS`, `USER_TAB_PRIVS`, and `ROLE_TAB_PRIVS` | View the granted rule-level plaintext access (PLAINACCESS) privilege. |
| `USER_SYS_PRIVS` and `ROLE_SYS_PRIVS` | View the granted system-level CREATE SENSITIVE RULE and PLAINACCESS privilege. |
...
* [Create column encryption rules](200.create-column-encryption-rule-of-oracle-mode.md)