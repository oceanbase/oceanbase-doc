...
* Data encryption
  ...
  OceanBase Database supports encrypting data for transmission and storage. OceanBase Database supports end-to-end data encryption at the transport layer. In addition, OceanBase Database supports transparent encryption of stored data to maximize data security even when the storage medium is lost. In MySQL-compatible mode, column encryption is supported, allowing users with the relevant privileges to create column-level sensitive data protection rules on tables. For more information, see [Data transmission encryption](500.data-encryption/200.data-transfer-encryption/100.data-transfer-encryption-overview.md), [Transparent data encryption](500.data-encryption/400.data-storage-encryption/100.datastore-encryption-overview.md), and [Column encryption](550.column-encryption/100.column-encryption-overview.md).
