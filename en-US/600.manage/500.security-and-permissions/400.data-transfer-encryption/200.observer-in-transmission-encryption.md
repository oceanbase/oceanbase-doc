...
# OBServer node transmission encryption

The communication between OBServer nodes, liboblog, and ob_admin depends on the libeasy library. Therefore, they all support loading private keys and certificates from local files. CA certificates, user certificates, and private keys are stored in the `wallet/` directory and are loaded based on the configuration.

## Enable OBServer node transmission encryption
...
3. Enable SSL communication on the MySQL port.
   ...
   1. You can specify the SSL protocol version by configuring the `sql_protocol_min_tls_version` parameter. Currently, only TLS 1.0, TLS 1.1, TLS 1.2, and TLS 1.3 are supported. After you specify a version, the specified version and later versions are supported.

      ```sql
      ALTER SYSTEM SET sql_protocol_min_tls_version = 'TLSv1.1';
      ```

   2. You can enable SSL communication by configuring the `ssl_client_authentication` parameter.
...
4. Configure an SSL allowlist for remote procedure call (RPC) communication. Since the TCP connections between OBServer nodes are persistent, SSL-encrypted RPC communication can be enabled only after the OBServer nodes are restarted.
...
## Check whether OBServer node transmission encryption is enabled
...
* Use the `sys` tenant account to log in to an OBServer node from OceanBase Client (OBClient) or a MySQL client, and query the `ssl_cert_expired_time` field in the `oceanbase.GV$OB_SERVERS` view to check whether SSL is enabled.
