...
|slug|create-user-mysql-mode-operations-guide|
...
You may need to create users and grant them privileges in OceanBase Database as needed. Generally, users who have been granted the `CREATE USER` privilege can create users.
...
* Username uniqueness in a tenant
  ...
  In MySQL-compatible mode, to distinguish users in the system tenant from those in a user tenant, it is recommended to use a specific prefix for the username in the system tenant.
...
After a user is created, it is more common in a production environment to rotate the password (dual password) while retaining business availability. Operations such as changing the password, retaining the old password, and discarding the old password are performed using the `ALTER USER` statement. For more information, see [Dual Password Management](../../../200.identification-and-authentication/100.identification-and-authentication-of-mysql-mode/400.dual-password-of-mysql-mode.md).