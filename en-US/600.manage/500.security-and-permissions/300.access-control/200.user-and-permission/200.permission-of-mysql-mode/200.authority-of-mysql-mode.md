...
You can grant only the privileges that you have. For example, `test1` attempts to grant the `SELECT` privilege on the `t1` table to `test2`. In this case, `test1` must have the `SELECT` privilege on the `t1` table and the `GRANT OPTION` privilege.
...
Before you grant a privilege, observe the following notes:
...
* `priv_level`: the level of the privilege to be granted. In MySQL mode, OceanBase Database provides privileges of the following levels:
  ...
  * Database level: Privileges at this level apply to all objects in a specified database. You can use the `GRANT ... ON db_name.*` statement to grant database privileges.

  * Table level: Privileges at this level apply to all columns in a specified table. You can use the `GRANT ... ON database_name.table_name` statement to grant table-level privileges.

    When you grant column-level privileges, you can also use the `GRANT ... ON database_name.table_name` statement to specify the table name.
...
* `user_name IDENTIFIED BY 'password'` and `user_name IDENTIFIED BY PASSWORD 'password'`: The passwords in the `user_name IDENTIFIED BY 'password'` and `user_name IDENTIFIED BY PASSWORD 'password'` clauses are in ciphertext.
...
* Grant database privileges
  ...
  Database privileges are management privileges on all objects in a specified database. You can use the `GRANT ... ON db_name.*` statement to grant database privileges.
...
* Grant table-level privileges
  ...
  Table-level privileges are management privileges on a specified table in a specified database. You can use the `GRANT ... ON db_name.tb1_name` statement to grant table-level privileges.
