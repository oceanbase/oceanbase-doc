...
* `priv_type`: the type of the privilege to be granted. When you grant multiple privileges to a user at a time, separate the privileges with commas (,).
   ...
   * To allow a user to execute other statements on a column, grant the user the `SELECT` privilege on the columns to be accessed.
   ...
   For more information about the privilege types supported in MySQL-compatible mode, see [Privilege types in MySQL-compatible mode](100.permission-classification-of-mysql.md).

* `priv_level`: the level of the privilege to be granted. In MySQL-compatible mode, OceanBase Database provides privileges of the following levels:
