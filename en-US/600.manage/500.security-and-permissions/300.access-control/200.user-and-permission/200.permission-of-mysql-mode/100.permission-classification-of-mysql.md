...
|  Global privilege | GRANT OPTION           | Allows users to grant the same privileges they have to other users. It is usually used in conjunction with other privileges, such as `SELECT`, `INSERT`, and `UPDATE`. |
|  Global privilege | ALL PRIVILEGES         | Indicates that the user has all privileges except `GRANT OPTION` in the database. |