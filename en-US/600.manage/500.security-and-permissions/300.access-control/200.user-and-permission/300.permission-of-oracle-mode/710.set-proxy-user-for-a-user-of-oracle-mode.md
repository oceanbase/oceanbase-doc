...
OceanBase Database supports the proxy user feature in Oracle-compatible mode. This feature authorizes a proxy user (such as user A) to connect to OceanBase Database on behalf of a target user (such as user B) and perform database operations under the privileges of the target user.
...
1. Log in to an Oracle-compatible tenant of the cluster as a user with the `ALTER USER` privilege.
