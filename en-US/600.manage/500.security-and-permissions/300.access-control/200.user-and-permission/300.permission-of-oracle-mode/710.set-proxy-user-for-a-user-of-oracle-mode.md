...
1. Log in to an Oracle tenant of a cluster as a user with the `ALTER USER` privilege.
...
4. Use the `USERA` user to log in to OceanBase Database on behalf of the `USERB` user.
   ...
   * `--proxy_user`: the name of the target user corresponding to the proxy user. In this example, it is `USERB`.
   ...
   Here is an example that uses the `USERA` user to connect to OceanBase Database on behalf of the `USERB` user through OBClient:
...
* [Connect to an OceanBase Database tenant by using OBClient](../../../../../300.develop/200.application-development-of-oracle-mode/100.connect-to-oceanbase-database-of-oracle-mode/200.connect-to-an-oceanbase-tenant-by-using-obclient-of-oracle-mode.md)
