...
1. Log in to an Oracle-compatible tenant of a cluster as the `SYS` user. Create users A, B, and C, and grant all the users the `CREATE SESSION` privilege, and grant user A the `GRANT ANY PRIVILEGE` privilege.
...
7. Switch to user C and query the `test` table. Privilege delegation is successful if querying the `test` table is possible.
