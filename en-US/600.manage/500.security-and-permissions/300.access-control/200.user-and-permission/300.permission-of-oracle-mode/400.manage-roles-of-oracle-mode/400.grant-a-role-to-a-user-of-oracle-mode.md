...
You can grant a role to a user in Oracle mode by using SQL statements.

## Prerequisites

To grant a role to a user, you must have the role and the `ADMIN OPTION` or `GRANT ANY ROLE` privilege.
...
* `WITH ADMIN OPTION` specifies that the granted role can be granted to other users, and role revocation does not extend to other users.
