...
In Oracle-compatible mode, you can grant a role to a user by using SQL statements or in the OceanBase Cloud Platform (OCP) console.

## Prerequisites

To grant a role to a user, you must have the role to be granted, and the `ADMIN OPTION` or `GRANT ANY ROLE` privilege.
