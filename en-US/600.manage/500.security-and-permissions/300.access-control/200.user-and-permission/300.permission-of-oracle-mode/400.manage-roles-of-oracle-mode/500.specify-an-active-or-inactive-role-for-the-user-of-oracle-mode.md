...
The Oracle-compatible mode of OceanBase Database supports using the `DEFAULT ROLE` clause in the `ALTER USER` statement to activate or deactivate roles upon user login.
...
The Oracle-compatible mode of OceanBase Database supports using the `SET ROLE` statement to activate or deactivate roles granted to the current logged-on user for the current session.
