...
You have the privileges to be granted and any of the following privileges: `GRANT OPTION`, `GRANT ANY OBJECT PRIVILEGE`, and `GRANT ANY PRIVILEGE`.
...
* `system_privilege` specifies the type of the system privilege to be granted. When you grant multiple privileges to a user, separate the privileges with commas (,).
...
* `obj_privilege` specifies the type of the object privilege to be revoked. When you revoke multiple privileges from a user at a time, separate the privileges with commas (,).

* `obj_clause` specifies the objects related to the object privileges to be revoked.

* `system_privilege` specifies the type of the system privilege to be revoked. When you revoke multiple privileges from a user at a time, separate the privileges with commas (,).

The following example shows how to revoke the system privilege `CREATE SESSION` from the `role1` role.
