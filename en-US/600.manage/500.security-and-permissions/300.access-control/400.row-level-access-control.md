...
8. Specify a user label.
   ...
   1. Log on to the `Oracle` tenant as the `SYS` user, create a user named `user1`, and then grant all privileges of the `SYS` user, except for the `GRANT OPTION` privilege, to `user1`.
   ...
   2. Log on as the security administrator `LBACSYS` and specify a label for `user1`.
