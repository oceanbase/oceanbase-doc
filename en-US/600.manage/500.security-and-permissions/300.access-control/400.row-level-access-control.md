...
Currently, only the Oracle-compatible mode of OceanBase Database supports label security.

## Set row-level access control

1. Log in to an Oracle-compatible tenant as the `SYS` user and unlock the `LBACSYS` user.
...
3. Grant privileges to the `LBACSYS` user.
   ...
   The `LBACSYS` user has no privileges by default. To perform row-level access control related operations, you must grant the `CREATE SESSION` privilege to the `LBACSYS` user using the `GRANT` statement.

   ```shell
   obclient> GRANT CREATE SESSION TO LBACSYS;
   ```

4. Log in to the Oracle-compatible tenant as the `LBACSYS` user.
...
7. Create data labels and view the result.

   For more information about how to create data labels, see [SA_LABEL_ADMIN.CREATE_LABEL](../../../700.reference/600.pl-reference/300.pl-oracle/1300.pl-label-security-package-oracle/400.sa-label-admin-tag-management-pack-oracle/300.sa-label-admin-create-label-oracle.md).
...
9. Apply the security policy to a table.
   ...
   1. Log in to the Oracle-compatible tenant as `user1` and create a table named `tbl1`.
