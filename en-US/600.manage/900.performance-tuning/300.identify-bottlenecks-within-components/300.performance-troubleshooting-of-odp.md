...
1. Determine the optimization objectives.
   ...
   2. Latency: Pay attention to the number of components on the access link and the time consumption on each phase. The network latency must also be considered.

   3. Experience: From a global perspective, performance tuning performed nearer to the business system achieves better results.
...
5. Confirm the optimization result: Observe whether the throughput and latency meet the customer requirements or test results.
...
#### Use tools to view performance data
