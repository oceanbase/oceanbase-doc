...
cmd_time_stats={
    client_transaction_idle_time_us=0, // The time interval, in μs, between the end of the last SQL and the start of the current SQL in the transaction (client transaction idle time)
...
`request_total_time_us` records the total time ODP spent processing the request. `server_process_request_time_us` records the time for the request from ODP to OBServer, including network time and OBServer processing time. With these logs, you can locate the time consumption at each stage of slow SQL and more easily troubleshoot slow SQL issues.
