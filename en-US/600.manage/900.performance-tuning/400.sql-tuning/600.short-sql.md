...
SQL queries in transaction processing (TP) business are all short queries, which consume quite short execution time, in milliseconds, but are executed at a very high frequency. It is not easy to determine whether a short query is a slow one by the execution time. To locate a slow short query, it is key to calculate the number of rows read in a query instead of observing its absolute execution time.
...
The query is not slow simply from the perspective of execution time. However, if the query is executed at a quite high frequency, it still consumes a lot of computing resources and may even use up the CPU resources.
...
To locate a slow short query, you need to collect aggregate statistics to calculate the overall resource consumption of a certain type of queries. In OceanBase Database, you can use the `GV$SQL_AUDIT` view for locating a slow short query. A typical diagnostic process for locating a slow short query is as follows:

The following statement obtains the top 10 queries of a certain type that read the most number of rows. If the numbers of rows read by the top queries are significantly higher by one to two orders of magnitude, their optimization will bring a great performance improvement. It is worthwhile to optimize the execution plans of the queries.
...
You can obtain information about the execution plan for a slow short SQL query based on the query information returned.

1. Execute the following statement to obtain information about the execution plan of a slow short SQL query.

    Use the actual tenant ID for `tenant_id` and specify `svr_ip` and `sql_id` based on the returned information about the slow short SQL query.
    ...
    `outline_data` is the hint description of the execution plan.
