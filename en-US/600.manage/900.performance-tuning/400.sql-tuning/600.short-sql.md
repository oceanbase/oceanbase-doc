...
The following query returns the top 10 SQL requests of a certain type sorted by the number of rows read. If the numbers of rows read by the top queries are significantly higher by one to two orders of magnitude, their optimization will bring a great performance improvement. That is, it is very worthwhile to optimize the execution plans of the queries.
...
1. Execute the following statement to obtain information about the execution plan of a slow short SQL query.
   ...
   Use the actual tenant ID for `tenant_id` and specify `svr_ip`, `svr_port`, and `sql_id` based on the previously returned information about the slow short SQL query.
...
2. Obtain the tree-shaped execution plan for the slow SQL query.
   ...
   You can also try to obtain the logical plan by running `EXPLAIN` on the SQL.
