...
SQL tuning is basically the process of optimizing execution plans of SQL queries. Designing proper indexes to reduce the number of rows to scan is the most effective tuning method. SQL tuning is inseparable from application and database optimization.
...
This topic provides typical scenarios and cases of SQL tuning in OceanBase Database to help you better understand common methods for SQL diagnostics and tuning. At the same time, we expect to gradually build a library of SQL tuning methods based on these cases, and implement GUI-based SQL diagnostics and tuning capabilities to facilitate SQL risk management and tuning.
...
* For pagination SQL queries on large datasets, as the number of pages increases, more data needs to be scanned, which also slows the execution of the queries.
...
Do not use `offset` for pagination queries. Otherwise, the performance of queries on large pages becomes poor. You can transfer the primary key `id` instead. For example, each time you query a page, you can transfer the primary key `id` or another identifier in the last row on the page returned for the previous query to the current query. This way, you can quickly obtain data based on the information transferred and the use of `ORDER BY LIMIT n`.
...
* Promotion campaigns
* Hotspot invalidation

**Optimization suggestions:**

* For business: Add traffic control at the business layer to prevent requests from instantly using up database resources in scenarios such as hotspot invalidation, promotion campaigns, and scheduled tasks.
* For database: Calculate the CPU capacity required by the database and add buffer CPU capacity. Alternatively, enable tenant-level CPU overprovisioning to allow the tenant to automatically use overprovisioned CPU capacity in emergency situations.
...
We recommend that you use the `NOWAIT` keyword with the `SELECT FOR UPDATE` statement and implement a retry mechanism at the business code layer to avoid lock contention at the database layer. During SQL emergency response, you can throttle requests from the account that leads to a hotspot to lower the lock concurrency.
...
When a database system holds businesses of different scales, business data is stored in a structure similar to an inverted pyramid, which means that a few businesses occupy the most storage space. As a result, SQL requests of businesses with more data have much worse performance.
...
* Use paging queries if possible. Query only one shard at a time with the condition of `id > id in the last row on the previous page`. Then, aggregate data at the business layer. This ensures constant SQL performance for both large and small accounts.
...
- Condition 1: Most data inserted into a table will be deleted soon, and only a small amount of data persists in the table. In general, SQL execution plans that access this table perform a full table scan. This is because OceanBase Database recycles data block "holes" (unused space) from the table, which makes the high-water mark of the table rather low and thus greatly reduces the table scan cost.

- Condition 2: A great deal of data is inserted and deleted in a short period of time, and the high-water mark of the table remains high due to inefficient "hole" recycling or data in the table piles up because the amount of inserted data is larger than that of deleted data. In this case, the database actually scans lots of data in the table, leading to SQL performance deterioration.
...
* Do not build an index on enum columns with few enum options, such as `gender`, `age`, `type`, and `status`, unless the index is highly selective. For example, assume that the `status` column in a table has 99% `Initializing` values and 1% `Pending` values. The business scenario is to insert values into rows where `status` is `Initializing`, and then update the value in the `status` column to `Completed` for the rows based on the primary key. After the operations, only 1% of rows meet the condition `status = Pending`. In this case, if a query needs to obtain data in the `Pending` state from the table, an index on the `status` column will provide high performance.
- For scenarios with serious data skew, you need to perform optimization at the business layer. For more information, see the **Inverted-pyramid data storage** section.
