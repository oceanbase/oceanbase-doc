...
Example 3: SQL query with a right or full fuzzy match
...
## [Recommended][Quality] Avoid using non-unique columns for order by sorting

Do not use non-unique columns for `order by` sorting in your code. This practice can result in unstable return results in OceanBase Database, which may impact business operations.

Failing to follow this guideline can lead to the following issues:

* After a primary switch in OceanBase Database, SQL executed on different servers with different execution plans may return inconsistent results.
* When multiple rows have the same value in the ordered column, each execution may return different results.

It is recommended to avoid using non-unique columns for `order by` sorting. If you must use them, add a column with unique properties after the sorting column.
