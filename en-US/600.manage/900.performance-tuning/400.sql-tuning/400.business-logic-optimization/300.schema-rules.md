...
* Table naming specifications for the MySQL mode of OceanBase Database
   ...
   * A table name cannot contain keywords or reserved words of OceanBase Database's MySQL mode.
...
* Table naming specifications for OceanBase Database's Oracle mode
   ...
   * A table name can contain at least three characters (based on R&D specifications) and no more than the maximum characters allowed.
   * A table name cannot contain keywords or reserved words of OceanBase Database's Oracle mode.

   <main id="notice" type='notice'>
    <h4>Notice</h4>
    <p><ul><li>The table naming specifications for OceanBase Database's Oracle mode are the same as those for OceanBase Database's MySQL mode except for the letter case requirements. </li><li>OceanBase Database's Oracle mode supports a maximum length of 64 characters for a table name, which may vary with the version. </li></ul></p>
    </main>
...
* Example 1: Violation to naming specifications for OceanBase Database's MySQL mode
...
* Example 2: Violation to naming specifications for OceanBase Database's Oracle mode
...
* Index naming specifications for OceanBase Database's MySQL mode
   ...
   * In sharding scenarios, the index names for all physical tables of the same logical table must be the same and not distinguished by the physical table key.
   ...
   <main id="notice" type='notice'>
    <h4>Notice</h4>
    <p>In mainstream versions of MySQL and OceanBase Database's MySQL mode, an index name can contain up to 64 characters, which may vary with the version. </p>
    </main>

* Index naming specifications for Oracle Database and OceanBase Database's Oracle mode
...
* Example 3: The name of an index contains meaningless characters in OceanBase Database's MySQL mode

   The name of `index idx_1(email,adress)` contains meaningless characters.

* Example 4: The index names are inconsistent for physical tables of the same logical table in OceanBase Database's MySQL mode
...
* Example 5: The index names conflict with each other in the same schema in OceanBase Database's Oracle mode
...
* Example 6: The index names for different tables in a single database conflict with each other in OceanBase Database's Oracle mode
...
* Example 1: Common index in OceanBase Database's MySQL mode
...
* Example 2: Unique index in OceanBase Database's MySQL mode
...
* Example 3: Column abbreviation contained in an index name in OceanBase Database's MySQL mode

   `index idx_email_add(email,address)` is an index whose name contains the abbreviation `add` of the `address` column.

* Example 4: Index names for different physical tables of the same logical table in OceanBase Database's Oracle mode when the number of databases is the same as that of tables in each database in sharding scenarios
...
* Example 5: Index names for different physical tables of the same logical table in OceanBase Database's Oracle mode when the number of databases is different from that of tables in each database in sharding scenarios
...
* Example 6: Index names for different tables in a single database in OceanBase Database's Oracle mode
