...
Before you go further into index-based SQL tuning, you need to understand a key metric: the number of rows to read. Transaction processing (TP) business usually features a relatively high throughput. Ideally, the number of rows to read per request should be relatively small. The amount of data to read in a single execution is an important metric for evaluating the result of SQL tuning. The significance of this metric is easy to understand. Since the overall I/O capacity of a cluster is limited, if the I/O resources consumed by a single request are small enough, which means that I/O resources consumed by the entire business will be relatively small, the pressure on the entire cluster will not be great. The associations between this metric and SQL execution are as follows:
...
In most cases, indexes are created mainly to make full use of predicates in queries and narrow down the query range, thereby reducing the number of rows to read. Therefore, for a slow query, we need to first identify its query range filter, determine whether the filter is selective enough, and calculate the number of rows to read in the index query range. We can obtain the number of rows to read for a given query in two ways.
...
In the execution plan, `table_rows:10002, physical_range_rows:99, logical_range_rows:99` shows the estimated number of rows to read on the `IDX_C2` index. `table_rows` is the total number of rows in the table. `physical_range_rows` and `logical_range_rows` are the number of rows in the `range(A,MIN ; A,MAX)` range. Generally, these two values are close. You can use either one. In some special scenarios, `physical_range_rows` may be much greater than `logical_range_rows`. This query uses `IDX_C2`, and the estimated number of rows to scan is 99.
...
The use of a composite index is the most common way of query optimization based on indexes. A composite index allows you to use as many predicates as query range filters as possible. Before you create a composite index, you can construct a query to estimate the number of rows to read on the composite index.
