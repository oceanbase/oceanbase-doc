...
In addition to the preceding two procedures that are commonly used for statistics collection, the `DMBS_STATS` package also provides the `GATHER_INDEX_STATS` procedure for collecting index statistics, and the `GATHER_DATABASE_STATS_JOB_PROC` procedure for collecting statistics on all tables of all databases in a tenant.
