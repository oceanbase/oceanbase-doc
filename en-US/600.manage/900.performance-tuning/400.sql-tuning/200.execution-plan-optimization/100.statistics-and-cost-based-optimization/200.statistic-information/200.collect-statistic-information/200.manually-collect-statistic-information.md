...
In addition to the preceding two procedures that are commonly used for statistics collection, the `DBMS_STATS` package also provides the `GATHER_INDEX_STATS` procedure for you to collect index statistics and the `GATHER_DATABASE_STATS_JOB_PROC` procedure to collect statistics on all tables of all databases in a tenant.
