...
The preceding examples describe how to choose the troubleshooting method based on the scenario. However, if you are an experienced troubleshooter, you can also locate the exact bottleneck component based on the error message.
...
* **OBServer node errors**
  ...
  * `ERROR 4012 (HY000): Timeout, query has reached the maximum query timeout`
    ...
    `ob_query_timeout` specifies the maximum execution duration allowed for an SQL query in microseconds. For more information about the variable, see [ob_query_timeout](../../700.reference/800.configuration-items-and-system-variables/200.system-variable/300.global-system-variable/9000.ob_query_timeout-global.md).

<main id="notice" type='notice'>
    <h4>Notice</h4>
    <p>The aforementioned error messages that allow experienced troubleshooters to locate the exact components may vary with the version of each component. We recommend that you build a specific troubleshooting methodology for each version. </p>
</main>
