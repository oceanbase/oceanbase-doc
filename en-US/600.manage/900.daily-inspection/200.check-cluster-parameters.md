...
| [server_permanent_offline_time](../../700.reference/800.configuration-items-and-system-variables/100.system-configuration-items/300.cluster-level-configuration-items/20300.server_permanent_offline_time.md) | The time threshold for node heartbeat interruption. A node is considered permanently offline after its heartbeat has been interrupted for this duration. Data replicas on a permanently offline node need to be automatically supplemented. Value range: \[20s, +∞).  | 7200s |
...
| [system_memory](../../700.reference/800.configuration-items-and-system-variables/100.system-configuration-items/300.cluster-level-configuration-items/22400.system_memory.md) | The memory size reserved by the system for the tenant with ID `500`. Value range: \[0 MB, +∞).  | 50 GB |
...
Cluster parameters are configuration parameters. The syntax for modifying them is as follows. When modifying multiple parameters at the same time, separate them with commas (,).
...
The following describes the parameter modification statement:

* `PARAM_OPTS`: Additional qualifiers that can be specified when modifying parameters, such as Zone or Server.

* `ALTER SYSTEM`: You cannot specify both Zone and Server in the same statement. When specifying a Zone, only one Zone is supported. When specifying a Server, only one Server is supported.

* Cluster-level parameters (`Scope`) cannot be set in a user tenant, nor can they be set by the `sys` tenant for a user tenant. For example, `ALTER SYSTEM SET memory_limit='100G' TENANT='test_tenant'` will return an error because `memory_limit` is a cluster-level parameter.
