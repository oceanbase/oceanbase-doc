...
O&M engineers can use related methods in the PL/SQL package `DBMS_MONITOR` to enable applications to determine whether to enable end-to-end diagnostics trace based on different identifiers, and the strategy of recording the trace information in trace logs. The sampling frequency determines whether logs are sampled and recorded in the memory. Logs may also be recorded in the trace log file.
...
- `level`: the log printing level. Three log printing levels are supported. Level 1 indicates log printing at the module level, and Level 3 indicates log printing at the most fine-grained level.
...
For more information about the definition of the DBMS_MONITOR package, see [DBMS_MONITOR](../../../700.reference/500.sql-reference/300.pl-reference/300.pl-oracle/1400.pl-system-package-oracle/10000.dbms-monitor-oracle/100.dbms-monitor-overview-oracle.md).
...
```
Example: Record the time consumption information of the current session for only slow queries and set the sampling frequency to 50%.
...
Example: Record the time consumption information for only slow queries whose `client_id` is `jdbc` and set the sampling frequency to 50%.
...
Example: Record the time consumption information for only slow queries whose `module_name` is `backup` and `action_name` is `insert`, and set the sampling frequency to 50%. You can set `module_name` and `action_name` by using the `SET_MODULE` method in the `DBMS_APPLICATION_INFO` package.
...
For more information, see [Trace query](https://en.oceanbase.com/docs/common-ocp-10000000001703646).
