...
This topic guides you on how to quickly perform end-to-end tracing of OceanBase Database using obdiag.
...
If you want to print all results during a comprehensive diagnosis, you can perform the **Reset Tenant Trace** operation.

After connecting to OBServer, execute the following SQL commands:

```sql
-- Disable Trace
call dbms_monitor.ob_tenant_trace_disable();
-- Record all time consumption information in the current tenant and print all.
call dbms_monitor.ob_tenant_trace_enable(1, 1, 'ALL');
```

### Step 2: Query SQL statements for testing

Here, a SQL query statement is used as an example to demonstrate how to perform end-to-end diagnosis.

```sql
select * from tx where c1=1 and c1=2;

-- In SQL Audit, find the `flt_trace_id` of the suspected slow SQL by using `query_sql`
...
### Step 3: Execute the end-to-end tracing command

Exit OBServer and execute the following command to perform end-to-end tracing:
...
The end-to-end tracing results include the TOP 5 time-consuming Leaf spans, as well as detailed information and tags for each span.