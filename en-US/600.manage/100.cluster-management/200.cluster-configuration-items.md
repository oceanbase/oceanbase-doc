...
|------|----------------------|---------------|
| Sys tenant | Cluster-level parameters and tenant-level parameters<main id="notice" type='explain'><h4>Note</h4><p>Use the <code>TENANT</code> keyword in the <code>SHOW PARAMETERS</code> statement to view the parameters of a specified tenant. </p></main> | Cluster-level parameters and tenant-level parameters<main id="notice" type='explain'><h4>Note</h4><p>In the sys tenant, you can specify the <code>TENANT</code> keyword to modify the tenant-level parameters of all tenants or a specified tenant. </p></main> |
...
The main data types of parameters in OceanBase Database and their descriptions are shown in the following table:
...
To query a parameter, use the following statement:
...
For more information about O&M operations related to cluster parameters, see:
