...
<main id="notice" type='explain'>
  <h4>Note</h4>
  <p>You can also add OBServer nodes to zones to switch from the standalone deployment mode to the distributed deployment mode, which is the single-replica multi-node deployment mode. For more information, see <a href="400.add-a-node.md">Add a node</a>. </p>
</main>
...
2. (Optional) Clear the directory.
...
4. Start the observer process.
   ...
   You must start the observer process as the `admin` user. Here is an example:
   ...
   | **Option** | **Description** |
   |--------|----------------------------|
   | `-I` \| `-i` | <ul><li>`-I`: the IP address of the node to be started. If OceanBase Database is deployed in distributed mode, you cannot specify 127.0.0.1 for this option. We recommend that you specify another IP address such as `-I 10.10.10.1`. </li><li>`-i`: the NIC name. You can use the `ifconfig` command to view the NIC name. </li></ul><main id="notice" type='explain'><h4>Note</h4><p>You can specify both the IP address and the NIC name to start a node, such as `-I 10.10.10.1 -i eth0`. However, this is not recommended. </p></main> |
   ...
   | `-d` | The primary directory of the cluster, which is created during initialization. Do not modify other parts of the directory except `$cluster_name`.  |
...
5. Verify that the observer process is started.
...
5. Verify that the OBServer nodes are successfully added.
