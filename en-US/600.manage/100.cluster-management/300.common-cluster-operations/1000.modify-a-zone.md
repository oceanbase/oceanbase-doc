...
At present, you cannot modify the type of a zone.
...
2. Execute the following statement to modify the attributes of a zone:
   ...
   * `region_name` specifies the name of the region where the zone is located.
   ...
   To modify the region of the current cluster to `shanghai` and the IDC to `sh1`, execute the following statement:

   ```shell
   obclient [(none)]> ALTER SYSTEM ALTER ZONE zone4 SET REGION 'shanghai',IDC 'sh1';
   ```

3. After the statement is executed, query the `oceanbase.DBA_OB_ZONES` view to verify whether the zone is modified.
