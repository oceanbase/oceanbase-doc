...
2. Execute the following statement to delete a zone from a cluster:
   ...
   In the statement, `zone_name` specifies the name of the zone to be deleted. This statement deletes only one zone at a time. To delete multiple zones, execute this statement multiple times.

   To delete a zone named `zone4` from the cluster, execute the following statement:

   ```shell
   obclient [(none)]> ALTER SYSTEM DELETE ZONE zone4;
   ```

3. After the statement is executed, query the `oceanbase.DBA_OB_ZONES` view to verify whether the zone is added.
