...
The statement is as follows:
   ...
   * `SCOPE` specifies the effective scope of the parameter. The default value is `BOTH`. Valid values:
   ...
   * `TENANT`: a keyword used in the `sys` tenant to specify the tenants whose tenant-level parameters are to be modified. Valid values:
   ...
   * `SERVER` specifies the server for which the parameter is to be modified in the cluster. `ZONE` specifies the zone for which the parameter is to be modified in the cluster. `ALTER SYSTEM` allows you to specify a zone or a server, but not both. You can specify only one zone for `ZONE` and one server for `SERVER`, if these parameters are to be specified. If you do not specify a zone or a server when you modify a cluster-level parameter, the modification takes effect in the entire cluster.
   ...
   For example, the `ALTER SYSTEM SET memory_limit='100G' TENANT='test_tenant'` statement will cause an error because `memory_limit` is a cluster-level parameter. The value in the `scope` column of the execution result of the `SHOW PARAMETERS LIKE 'parameter_name';` statement indicates whether the parameter is a cluster-level parameter or a tenant-level parameter.
...
4. Determine whether to restart the OBServer node based on the effective mode of the parameter, and then execute the `SHOW PARAMETERS` statement to verify whether the value of the parameter is successfully modified.
