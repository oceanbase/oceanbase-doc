...
1. Obtain the RPM package of the target OceanBase Database version.
   ...
   To obtain the RPM package, go to [OceanBase Download Center](https://en.oceanbase.com/softwarecenter). If no installation package of the target version is available on the website, contact OceanBase Technical Support for help.
...
<main id="notice" type='notice'>
  <h4>Notice</h4>
  <p>When a barrier version exists between the current version and the target version, upgrade the OceanBase cluster to the barrier version first, and then upgrade from the barrier version to the target version. </p>
</main>
...
3. Stop the observer processes on the OBServer nodes in the zone. Replace the binary files with those of the new database version and then restart the processes.
   ...
   3. Restart the observer process.
      ...
      ```
         ...
         <main id="notice" type='explain'>
          <h4>Note</h4>
          <p>You do not need to specify the startup parameters when you restart the observer process because the startup parameters of the earlier version are automatically written to the parameter file. </p>
        </main>
...
<main id="notice" type='notice'>
  <h4>Notice</h4>
  <p>If an error occurs while running the upgrade script <code>upgrade_post.py</code>, it is most likely due to an RS leader switch. We recommend troubleshooting and resolving the issue as follows:<ol><li>Run <code>SELECT * FROM oceanbase.DBA_OB_TENANT_JOBS WHERE job_type = 'upgrade_all' ORDER BY job_id DESC LIMIT 1;</code> to check the status of the upgrade task. If <code>JOB_STATUS</code> shows <code>inprogress</code> and <code>MODIFY_TIME</code> is recent (close to the current time), you can manually execute <code>ALTER SYSTEM RUN UPGRADE JOB 'UPGRADE_ALL';</code> to resolve the problem.</li><li>Rerun the upgrade script.</li></ol></p>
</main>
...
If the OceanBase cluster was configured with cgroup before the upgrade, you must reconfigure cgroup after the upgrade. For more information, see [Configure cgroups (MySQL-compatible mode)](../../../../200.tenant-management/600.common-tenant-operations/300.resource-isolation/300.resource-isolation-of-mysql-mode/100.config-cgroups-of-enterprise-of-mysql.md).
