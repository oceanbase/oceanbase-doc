...
* Adjust the parameters of the migration task to control the migration speed.
   ...
   |------------------------|-----------------------------------------------|
   | balancer_idle_time | A tenant-level parameter that controls the interval for initiating migration tasks. Default value: 10s. Value range: [10s, +∞). |
   | sys_bkgd_net_percentage | A cluster-level parameter that controls the percentage of NIC bandwidth occupied by migration tasks. Default value: 60. Value range: [0, 100]. |
