...
During automatic unit migration, the migration task will consume the network and I/O resources on the source and target nodes. As a result, the migration traffic overlaps with the business traffic, and the business traffic is affected by the migration traffic. To prevent the business systems from being affected, you can perform the following operations:
...
* Adjust the parameters of the migration task to control the migration speed.
  ...
  |------------------------|-----------------------------------------------|
  | balancer_idle_time     | The interval for initiating a migration task. It is a tenant-level parameter. The default value is `10s`. The value range is [10s, +∞). |
  | sys_bkgd_net_percentage| The percentage of NIC bandwidth occupied by the migration task. It is a cluster-level parameter. The default value is `60`. The value range is [0, 100].            |
...
2. (Optional) Manually migrate the resource units on the node to be deleted.
   ...
   1. View the distribution of resource units and obtain the IDs of the resource units to be migrated.
      ...
      The fields in the view are described as follows:

      * `UNIT_ID`: the ID of a resource unit on the node to be deleted.

      * `TENANT_ID`: the ID of the tenant to which the unit belongs. If the value is `NULL`, the current unit does not belong to any tenant.

      * `STATUS` the current status of the unit. Valid values:
   ...
   2. Execute the following statement to manually migrate the resource units:
       ...
       * `svr_ip` specifies the IP address of the target node of the resource unit.

       * `svr_port` specifies the RPC port of the target node of the resource unit.

      To migrate a resource unit with the ID `1012` to the server with the IP address `172.xx.xx.xx` in the same zone, execute the following statement:

      ```shell
      obclient [(none)]> ALTER SYSTEM MIGRATE UNIT = 1012 DESTINATION = '172.xx.xx.xx:2882';
      ```

      You can execute this statement multiple times. If the manual migration fails, you can resolve the issue based on the error information and then try again.
...
If you directly perform the `DELETE SERVER` operation without manually migrating the resource units from the node to be deleted, resources may become insufficient during unit load balancing. In other words, the remaining resources of other nodes in the zone are insufficient to accommodate the resource units on the node to be deleted. As a result, the unit migration will fail and this node will remain in the deleting state. You can check whether the unit migration fails based on the `rootservice.log` file in the `/home/admin/oceanbase/log` directory of the RootService server. If the unit migration fails, you must perform the `CANCEL DELETE SERVER` operation, add a node to the zone, and then delete the target node. For more information about how to add a node to a zone, see [Add a node](../300.common-cluster-operations/400.add-a-node.md).

Perform the `CANCEL DELETE SERVER` operation as follows:
...
2. Execute the following statement to cancel the deletion of the node:
   ...
   * `svr_port` specifies the RPC port of the target node. The default value is `2882`.
