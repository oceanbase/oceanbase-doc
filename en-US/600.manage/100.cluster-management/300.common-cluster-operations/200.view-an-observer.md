...
2. Query the `DBA_OB_SERVERS` view for information about all nodes in the cluster.
   ...
   * `WITH_ROOTSERVER`: indicates whether the node is the RootServer of the cluster. The RootServer is responsible for cluster management.

   * `STATUS`: the status of the node. Valid values include:

      * `ACTIVE`: The node is available.

      * `INACTIVE`: The node is unavailable.
