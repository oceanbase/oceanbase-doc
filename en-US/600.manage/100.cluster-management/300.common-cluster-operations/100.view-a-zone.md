...
2. Query the `oceanbase.DBA_OB_ZONES` view for information about zones in the cluster.
   ...
   * `STATUS`: the status of the zone. Valid values:
     ...
     * `INACTIVE`: The zone is unavailable. When the client detects this state, it routes the business workflow to another zone.

       For a new zone or a zone on which you have run an isolation command such as `STOP ZONE`, `FORCE STOP ZONE`, or `ISOLATE ZONE`, its state is `INACTIVE`. This state usually occurs during fault isolation or O&M changes.

   * `IDC`: the name of the IDC where the zone is located.

   * `REGION`: the region where the zone is located. The value is usually a city name such as Shanghai.
