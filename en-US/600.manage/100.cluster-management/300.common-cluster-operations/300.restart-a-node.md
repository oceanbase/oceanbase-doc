...
Restart is a general O&M action. You can perform a restart for provisional maintenance of a node or for system parameter modifications to take effect. During the restart, the offline time of the node must be within the time specified by the `server_permanent_offline_time` parameter. Otherwise, the node will be permanently offline. If the maintenance of a node takes a long time, replace it. For more information about how to replace a node, see [Replace a node](../300.common-cluster-operations/700.replace-a-node.md).

<main id="notice" type='explain'>
<h4>Note</h4>
<p>The cluster-level parameter <code>server_permanent_offline_time</code> specifies the time threshold for heartbeat missing. When the heartbeat of a node is missing for the specified period of time, the node is considered permanently offline. Data replicas on a permanently offline node must be automatically supplemented. The default value is <code>3600s</code>.  </p>
</main>

## Background information

OceanBase Database is a distributed database and is typically built based on a multi-replica architecture, such as three replicas in three IDCs of the same city or five replicas in five IDCs across three regions. A transaction is committed only when the transaction log is synchronized among the majority of replicas by using the Paxos protocol, thereby ensuring data consistency among replicas. When a minority of replicas are abnormal, a service level agreement (SLA) objective with a recovery point objective (RPO) of 0 is met.
...
2. Mark the node to be restarted (which is in the `ACTIVE` state, and the value of the `stop_time` field for the node is greater than 0) as stopped on RootService. After the client detects a stopped node, the client will no longer route business requests to this node.
...
2. Execute the following statement to isolate the target node:
   ...
   * `svr_port` specifies the RPC port of the node to be stopped. The default value is `2882`.
   ...
   After the statement is executed, you can query the `oceanbase.DBA_OB_SERVERS` view for the `STATUS` field of this node. In the view, the value of this field is still `ACTIVE` but the value of `STOP_TIME` changes from `NULL` to the point in time when the node is stopped.
...
4. Stop the observer process.
   ...
   2. Use a command-line tool to go to the `/home/admin/oceanbase` directory.
   ...
   4. Stop the observer process.
      ...
      <h4>Notice</h4>
      <p>You can stop only one observer process on a node in the deployment directory. To stop the observer processes on multiple nodes, log in to the corresponding servers one by one and repeat the preceding procedure. </p>
      </main>
...
6. Start the observer process.
   ...
   2. Start the observer process.
      ...
      <main id="notice" type='notice'>
      <h4>Notice</h4>
      <p>You can start only one observer process on a node in the deployment directory. To start the observer processes on multiple nodes, log in to the corresponding servers one by one and repeat the preceding procedure. </p>
      </main>
