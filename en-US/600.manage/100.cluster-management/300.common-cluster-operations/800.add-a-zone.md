...
2. Execute the following statement to add a zone to a cluster:
   ...
   * `zone_name` specifies the name of the zone to be added. This statement adds only one zone at a time. To add multiple zones, execute this statement multiple times.

   * `idc_name` specifies the name of the IDC where the zone is located. By default, this parameter is left empty.
...
4. To use a newly added zone, you must start it so that it changes to the `ACTIVE` state.
   ...
   To start a zone, execute the following statement:
...
* After you add a zone, you can modify its `idc_name` or `region_name` setting. For more information about how to modify a zone, see [Modify a zone](../300.common-cluster-operations/1000.modify-a-zone.md).
