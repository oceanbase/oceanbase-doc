...
2. Execute the following statement to add a zone to a cluster:
   ...
   * `idc_name` specifies the name of the IDC where the zone is located. If not specified, the default value is empty.

   * `region_name` specifies the name of the region where the zone is located. If not specified, the default value is `default_region`.
...
* After you add a zone, you can modify its `idc_name` or `region_name` if needed. For more information about how to modify a zone, see [Modify a zone](../300.common-cluster-operations/1000.modify-a-zone.md).

* To use a newly added zone, you must add nodes to the zone. For more information about how to add a node, see [Add a node](../300.common-cluster-operations/400.add-a-node.md).
