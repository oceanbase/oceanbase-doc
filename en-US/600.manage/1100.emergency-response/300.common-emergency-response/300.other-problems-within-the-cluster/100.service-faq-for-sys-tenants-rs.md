...
1. Execute the following statement to query the resources allocated to tenants:
...
2. Execute the following statement to scale up the CPU and memory resources for the `sys` tenant:
  ...
  <main id="notice" type='notice'>
    <h4>Notice</h4>
    <p>As of OceanBase Database V3.2, you can modify only the CPU and memory configurations of a resource unit. The modifications to other I/O configurations, such as the disk space and IOPS, do not take effect, and the default values are used. </p>
  </main>

### Execute the STOP SERVER statement

If requests are accumulated in the request queue of the `sys` tenant, you can also execute the `STOP SERVER` statement to stop the OBServer node that hosts RootService and the `sys` tenant.
...
Restart the OBServer node that hosts the `sys` tenant. For more information, see [Restart an OBServer node](https://en.oceanbase.com/docs/common-ocp-10000000001704453).