...
If the memory of a tenant is full, the data write fails, and one of the following error messages is returned: `No memory or reach tenant memory limit` (error code 4013) or `Over tenant memory limits` (error code 4030). The following content describes the emergency operations by priority from high to low.

1. Increase the configurations of tenant memory. We recommend that you scale up tenant memory in the OceanBase Cloud Platform (OCP) console. For more information, see [OCP documentation](https://en.oceanbase.com/docs/ocp-en). Perform the following steps to modify memory configurations by using statements:
