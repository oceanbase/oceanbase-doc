...
3. Reduce the network bandwidth of background tasks.
   ...
   You can use the following command to lower the network bandwidth allocated to background tasks:
...
4. Enable clog transmission compression.
   ...
   For high NIC load on the current node, you can consider enabling log transmission compression. You can enable it by setting the `log_transport_compress_all` parameter. The default value is `False`, meaning compression is disabled. The modification takes effect immediately without a restart.
