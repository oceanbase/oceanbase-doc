...
| extra_redo_dispatch_memory_size | The extra redo size allocated to skew participants to avoid deadlocks, which may cause OceanBase Change Data Capture (CDC) not to generate data in a data skew scenario. | 4 MB | Yes | The value range is [0 M, 512 M]. |
