...
As full direct load supported in OceanBase Database V4.3.0 does not deliver the optimal import efficiency, incremental direct load is introduced in OceanBase Database V4.3.1. Simply put, incremental direct load is a replacement semantics that directly replaces rows with duplicate primary keys without checking for primary key conflicts. For more information, see [Overview of direct load](../../../../500.data-migration/1100.bypass-import/100.overview-of-bypass-import.md). To synchronize the imported data to downstream services, OceanBase Change Data Capture (obcdc) needs to adapt to the semantics of incremental direct load.
...
3. Log in to OceanBase Database, and import data in incremental direct load mode.
   ...
   For more information, see [Import data in direct load mode by using the INSERT INTO SELECT statement](../../../../500.data-migration/1100.bypass-import/200.full-bypass-import.md).
