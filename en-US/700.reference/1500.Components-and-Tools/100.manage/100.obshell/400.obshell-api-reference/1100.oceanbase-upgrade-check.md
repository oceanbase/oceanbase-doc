...
This API operation is used to check whether the conditions for an OceanBase Database upgrade are met. For example, OceanBase Shell (OBShell) checks whether the required environment is prepared, whether the OceanBase Database upgrade package is uploaded, and whether all required products are ready.
