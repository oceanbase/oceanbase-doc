...
| mode  | String  | Yes  | rolling | The upgrade mode. Valid values: <ul><li>`rolling`: During the upgrade, a majority of replicas are available for services. </li><li>`stopService`: Services are suspended for an overall upgrade within a specified period of time. </li></ul> |
