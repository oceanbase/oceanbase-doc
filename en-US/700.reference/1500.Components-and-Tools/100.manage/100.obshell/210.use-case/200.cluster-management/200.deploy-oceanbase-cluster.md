...
- In the production environment, you have checked the environment and configurations. For more information, see [Environment and configuration checks](../../../../../../400.deploy/500.deploy-oceanbase-database-community-edition/200.local-deployment/200.environment-and-configuration-checks/150.create-user-optional.md).
...
dag, err := client.V1().JoinSyncWithRequest(joinRequest1)  // Call /api/v1/agent/join.
··· 
```
...
dag, err :=  client.V1().JoinSyncWithRequest(joinRequest2)  // Call /api/v1/agent/join.

joinRequest3 := client.V1().NewJoinRequest("10.10.10.3", 2886, "zone3")
dag, err =  client.V1().JoinSyncWithRequest(joinRequest3)  // Call /api/v1/agent/join.
···
```
...
- `-u`: the tenant account in the following format: `Username@Tenant name`. The default username of the administrator of a MySQL-compatible tenant is `root`.
