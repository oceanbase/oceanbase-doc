...
${home_path}/bin/obshell tenant create <tenant_name> [-z] [-u] [--unit_num] [--replica_type] [--primary_zone] [--charset] [--collation] [--info] [--read_only]
  [--parameters] [--variables] [--whitelist] [--scenario] [--root_password] [--import_script]
...
| --import_script | No | bool | false | Controls whether to import timezone data and GIS metadata into the sys tenant. No value needs to be specified. |
...
Use this command to enable log archiving for a specified tenant.
...
`home_path` is the installation directory of OceanBase Database. obshell is located in the `bin` directory of the OceanBase Database installation directory by default. `t1` is the name of the tenant for which to enable log archiving.

You can specify the `-y`/`--yes` option to skip the confirmation prompt. No value needs to be specified. Specifying this option skips the confirmation prompt.

## obshell tenant noarchivelog

Use this command to disable log archiving for a specified tenant.
...
`home_path` is the installation directory of OceanBase Database. obshell is located in the `bin` directory of the OceanBase Database installation directory by default. `t1` is the name of the tenant for which to disable log archiving.
