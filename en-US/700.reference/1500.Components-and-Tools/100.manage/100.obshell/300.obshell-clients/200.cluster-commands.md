# obshell commands for clusters

This topic describes the cluster commands of obshell, which can be used to deploy and manage OceanBase clusters.

You can configure the `-h`/`--help` option in a command to view the help information of the command. You can also configure the `-v`/`--verbose` option to view the detailed execution process of a command when an error occurs during task execution.

## obshell cluster join

You can run this command to add the current obshell to the specified cluster (before the OceanBase cluster is initialized).
...
| --- | --- | --- | --- | --- |
| -z/--zone | Yes | string | N/A | The zone to which the node belongs. |
| -s/--server | Yes | string | N/A | The target node to be joined. The format is `ip:port`. The `port` parameter is optional. The default value is `2886`. |
| -d/--data_dir | No | string | ${home_path}/store | The directory for storing data such as the SSTable. The value must be an absolute path. The default value is the `store` directory under the working directory (`home_path`). |
...
| -o/--opt_str | No | string | N/A | Other parameters of the OBServer node. Each parameter is in the `key=value` format. Multiple parameters are separated with commas (,). The `-o` option must be enclosed in single quotation marks, for example: `-o 'memory_limit=16G,system_memory=8G,log_disk_size=24G,datafile_size=24G'`. |
| -l/--log_level | No | string | N/A | The log output level of the OBServer node. |
