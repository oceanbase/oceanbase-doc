...
| --- | --- | --- | --- | --- |
| -t/--tenant_name | No | string | N/A | The name of the tenant to configure backup for. If not specified, configurations for all tenants in the cluster are set. |
...
| -a/--archive_log_uri  | No | string | N/A | Sets the destination directory for the tenant log archive. If you have specified a destination directory in the preceding step and do not specify a destination directory here, the previous directory is used. If you have not specified a destination directory in the preceding step, a default value is used here.<main id="notice" type='explain'><h4>Note</h4><p>This option takes effect only when <code>-t</code>/<code>--tenant_name</code> is specified.</p></main> |
| -u/--backup_base_uri | No | string | N/A | Sets the backup destination for all tenants. After you configure this option, the data backup and log archive paths for each tenant are appended to the specified directory according to certain rules.<main id="notice" type='explain'><h4>Note</h4><p>This option takes effect only when you do not specify <code>-t</code>/<code>--tenant_name</code>. If both options are specified, an error is returned.</p></main> |
| -c/--log_archive_concurrency | No | int | N/A | Sets the total number of worker threads for log archiving. |
| -b/--binding   | No | string | Optional | The value must be `Optional` (user business takes priority) or `Mandatory` (archiving takes priority). If you do not specify this parameter, the default is Optional. |
| -i/--piece_switch_interval | No | string | N/A | Configures the piece switching cycle. Value range: [1d, 7d]. |
| -l/--archive_lag_target   | No | string | N/A | Controls the lag in tenant log archiving. Make sure that the log archiving destination is properly configured before you modify this parameter. |
...
| -r/--delete_recovery_window | No | string | N/A | Controls the time window for restoring backed-up data. The value must contain a time unit. For more information, see [Parameters of backup cleanup](../../../../../600.manage/600.backup-and-recovery/500.clear-backup-data/500.parameters-of-cleaning-up-backup.md). |
| -s/--ha_low_thread_score    | No | int | N/A | Specifies the current number of worker threads for backup tasks and other tasks of medium and low priorities. The value range is [0, 100]. |
| -y/--yes  | No | N/A | N/A | No value needs to be specified. Specifying this option skips the confirmation prompt. |

## obshell cluster backup

Use this command to set backup-related configurations for all tenants in the cluster and perform a backup. For more information, see [Initiate cluster-level backup](../210.use-case/200.cluster-management/600.initiate-cluster-level-backup.md).
...
`home_path` is the installation directory of OceanBase Database. By default, obshell is located in the `bin` directory of the OceanBase Database installation directory.
...
| -i/--piece_switch_interval | No | string | N/A | Configures the switching cycle for pieces. Value range: [1d, 7d]. |
| -e/--encryption  | No | string | N/A | Sets a password for the backup set. If this option is set, the password must be provided when restoring from this backup set, and the password cannot be deleted. |
| -l/--archive_lag_target  | No | string | N/A | Controls the log archive delay time for tenants. To modify this, ensure the log archive destination has been configured. |
...
| -r/--delete_recovery_window | No | string | N/A | Controls the recovery window for backup data. A time unit must be specified. For usage, see [Parameters of backup cleanup](../../../../../600.manage/600.backup-and-recovery/500.clear-backup-data/500.parameters-of-cleaning-up-backup.md). |
| -s/--ha_low_thread_score    | No | int | N/A | Specifies the current number of worker threads for low-priority tasks such as backup and backup cleanup. Value range: [0, 100]. |
| -P/--plus_archive         | No | N/A | N/A | Controls whether to back up archived logs together during data backup. No value required. When this option is added, a complete dataset with archived logs will be generated in the final backup directory. |
| -y/--yes  | No  | N/A | N/A | No value needs to be specified. Specifying this option skips the confirmation prompt. |
