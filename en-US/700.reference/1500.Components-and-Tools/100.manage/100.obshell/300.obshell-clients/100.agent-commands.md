...
|------------|----------|---------|---------|------|
| --ip       | No       | String  | N/A      | The IP address to bind obshell to. |
| -P/--port  | No       | Integer  | 2886    | The port number to bind obshell to. |
...
| --password | No       | String     | N/A      | This option is used only when taking over an OceanBase cluster. Specify the root user password of the sys tenant in the OceanBase cluster. You can also use the `OB_ROOT_PASSWORD` environment variable to specify the root user password. |
...
When taking over an OceanBase cluster, use the `--password` option to specify the root user password of the sys tenant in the OceanBase cluster. You can also use the `OB_ROOT_PASSWORD` environment variable to specify the root user password.
