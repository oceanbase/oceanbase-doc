...
2. Compile ob_error in the following modes:
    ...
    * Release mode
      ...
      By default, after you compile `ob_error`, you can find the compiled files in the `RELEASE_BUILD_DIR/tools/ob_error/src/ob_error` directory. `RELEASE_BUILD_DIR` indicates the compilation directory, which is `build_release` in this example.
...
After you enter an error code, `ob_error` automatically returns an error message corresponding to the operating system, Oracle-compatible mode, MySQL-compatible mode, and OceanBase Database. Here is an example:
...
* If you add the `my` facility, `ob_error` returns an error message of OceanBase Database if the error code is not generated for an error in MySQL-compatible mode but exists in OceanBase Database. Otherwise, `ob_error` returns an error message in MySQL-compatible mode.
...
* If you add the `ora` or `pls` facility, `ob_error` returns an error message of the Oracle-compatible mode if the error code exists in this mode.
...
* When you use the `-a` option in some circumstances, if the error code you enter exists in the corresponding mode, `ob_error` returns error messages of both OceanBase Database and the Oracle-compatible mode.
