...
| -f/--file-path | No | String | N/A | The relative path of the backup file.  <main id="notice" type='notice'><h4>Notice</h4><p>This option is deprecated. Do not configure it.</p></main>  |
| -o/--offset | No | Integer | 0 | The offset for reading. This value varies based on the type of backup file.  <ul><li>If the backup file is a macro block file, this value indicates the macro block byte offset.</li><li>If the backup file is a table list file, this value indicates the table list entry offset. If the value of the <code>-o</code>/<code>--offset</code> option is greater than the total number of table list entries, the output is empty.</li></ul>  |
| -l/--length | No | Integer | 0 | The length for reading. This value varies based on the type of backup file.  <ul><li>If the backup file is a macro block file, this value indicates the number of macro block bytes.</li><li>If the backup file is a table list file, this value indicates the number of table list entries. If the sum of the offset and length is greater than the total number of table list entries (as shown in the following example output), the output range is [offset, count).</li></ul>  |
