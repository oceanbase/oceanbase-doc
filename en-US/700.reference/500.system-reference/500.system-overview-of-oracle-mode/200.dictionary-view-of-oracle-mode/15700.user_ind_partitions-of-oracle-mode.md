...
`USER_IND_PARTITIONS` describes index partitions accessible to the current user in the database. It does not have the `INDEX_OWNER` field compared with the `ALL_IND_PARTITIONS` and `DBA_IND_PARTITIONS` views.
...
| COMPOSITE | VARCHAR2(3) | NO | Indicates whether a partition belongs to the local index of the composite-partitioned table. Valid values: <li>YES<li>NO |
