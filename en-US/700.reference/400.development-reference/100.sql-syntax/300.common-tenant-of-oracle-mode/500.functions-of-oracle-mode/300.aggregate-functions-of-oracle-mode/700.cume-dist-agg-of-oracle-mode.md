...
| DESC \| ASC | Optional. The sorting method of the list.  `*ASC* specifies to sort in ascending order. It is the default value.`  `DESC` specifies to sort in descending order.  |
| NULLS { FIRST \| LAST } | Optional. The position of `NULL` in `expr_col` after sorting.  *`NULLS FIRST` indicates that`` nulls are sorted before non-null values.*  `NULLS LAST` indicates that`` nulls are sorted after non-null values.  |
