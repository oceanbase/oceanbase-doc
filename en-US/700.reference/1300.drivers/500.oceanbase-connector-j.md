...
OceanBase Connector/J is a JDBC Type 4 driver that can be connected to a database engine by using local protocols. OceanBase Database supports OceanBase Connector/J and is fully compatible with MySQL Connector Java, the native JDBC driver for MySQL. OceanBase Connector/J is fully compatible with MySQL JDBC. OceanBase Connector/J automatically determines whether OceanBase Database runs in MySQL or Oracle-compatible mode and supports both modes at the protocol layer.

OceanBase Connection/J is compatible with the OB2.0 protocol.
  
  <main id="notice" type='notice'>
    <h4>Notice</h4>
    <p>OceanBase Database determines whether to run in MySQL or Oracle-compatible mode based on the tenant name that is used when the JDBC driver is connected to the database service. Tenants in Oracle-compatible mode can use only Oracle-compatible SQL syntax. </p>
  </main>

OceanBase Connector/J is compatible with standard JDBC APIs and Oracle JDBC. OceanBase Database in Oracle-compatible mode supports most Oracle syntax.
