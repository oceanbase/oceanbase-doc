...
obagent:
  global:
    ...
    monitor_user: root
    # Monitor password for OceanBase Database. When a depends exists, OBD gets this value from the oceanbase-ce of the depends. The value is the same as the root_password in oceanbase-ce.
    monitor_password:
...
* Log on to Grafana and change the password. The default account is `admin` at initial logon.
