...
| Modification | <ul><li>Modification can be performed by using SQL statements. For example,<br> `obclient> Alter SYSTEM SET schema_history_expire_time='1h';` </li><li>You can set the startup parameters to modify a system parameter. For example,<br> `cd /home/admin && ./bin/observer -o "schema_history_expire_time='1h'"`   </li></ul> | Modification can only be performed by using SQL statements. For example: <ul><li>MySQL mode:<br> `obclient> SET ob_query_timeout = 20000000;`<br> `obclient> SET GLOBAL ob_query_timeout = 20000000;`</li><li>Oracle mode:<br>`obclient> SET ob_query_timeout = 20000000;` <br>`obclient> SET GLOBAL ob_query_timeout = 20000000; `<br> `obclient> ALTER SESSION SET ob_query_timeout = 20000000;`<br> `obclient> ALTER SYSTEM SET ob_query_timeout = 20000000;`</li></ul> |
