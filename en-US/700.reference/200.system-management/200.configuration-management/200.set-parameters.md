...
|------|---------------------------------------------------------------------------------------------------|--------------------------------------------------------------------------------------------------|
| sys tenant | Cluster-level parameters and tenant-level parameters<main id="notice" type='explain'><h4>Note</h4><p>You can specify the <code>TENANT</code> keyword in the <code>SHOW PARAMETERS</code> statement to query the parameter settings of a specified tenant.</p></main> | Cluster-level parameters and tenant-level parameters<main id="notice" type='explain'><h4>Note</h4><p>You can specify the <code>TENANT</code> keyword in the sys tenant to modify the tenant-level parameters of all tenants or a specified tenant.</p></main> |
...
The following example shows how to modify the `log_disk_utilization_threshold` parameter for a specified zone:

```shell
obclient> ALTER SYSTEM SET log_disk_utilization_threshold = 20 ZONE='z1';
```

The following example shows how to modify the `log_disk_utilization_threshold` parameter for a specified OBServer node:

```shell
obclient> ALTER SYSTEM SET log_disk_utilization_threshold = 20 SERVER='XXX.XXX.XXX.XXX:XXXXX';
```

The following example shows how to modify a tenant-level parameter for all tenants or a specified tenant in the sys tenant:
...
| isdefault  | Whether the current value is the default value.<ul><li>0: The current value is not the default value.</li><li>1: The current value is the default value.</li></ul> |
