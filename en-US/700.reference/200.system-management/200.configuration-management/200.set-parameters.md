...
OceanBase Database provides cluster-level and tenant-level parameters. You can set these parameters to control load balancing, merge time, merge mode, resource allocation, module switches, and other functions of the entire cluster.

* Cluster-level parameters apply to all OBServer nodes in the cluster.

* Tenant-level parameters apply to OBServer nodes where the current tenant resides in the cluster.
...
The SQL syntax for modifying parameters in MySQL-compatible mode is as follows:
...
The SQL syntax for modifying parameters in Oracle-compatible mode is as follows:
...
* `expression` specifies the value of the parameter after modification.
  ...
  <main id="notice" type='explain'>
    ...
    <ul>
    <li>When you modify multiple parameters at a time, separate the parameters with commas (,). </li>
    <li>In Oracle-compatible mode, you can set only tenant-level parameters. </li>
    <li>You can modify tenant-level parameters directly in the current tenant or in the <code>sys</code> tenant by specifying the <code>TENANT</code> keyword. </li>
    </ul>
  </main>
...
The SQL syntax for resetting parameters in MySQL-compatible mode is as follows:
...
The SQL syntax for resetting parameters in Oracle-compatible mode is as follows:
...
* `expression` specifies the new value of the parameter.
  ...
  <main id="notice" type='explain'>
    ...
    <ul>
    <li>Separate multiple parameters with commas (,). </li>
    <li>Only tenant-level parameters can be reset in Oracle-compatible mode. </li>
