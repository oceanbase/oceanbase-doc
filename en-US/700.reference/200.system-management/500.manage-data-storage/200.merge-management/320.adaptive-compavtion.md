...
|dir-name-en||
|tenant-type||
|slug|adaptive-major-compactions|
...
When the partition leader meets the specified conditions, the system adaptively schedules a major compaction for this partition and the followers. The following items describe the rules for triggering an adaptive major compaction:
...
The adaptive major compaction feature is specified by the tenant-level hidden parameter `_enable_adaptive_compaction`, and is enabled by default. You can perform the following steps to query whether the adaptive major compaction feature is enabled and enable the feature if it is disabled:
...
You can set the table option `table_mode` to one of the following five valid values when creating or modifying a table:
...
You can set this option to an appropriate value for each table based on your business needs. Here is an example of creating a `queuing` table:

```shell
obclient> CREATE TABLE tbl1 (c1 int, c2 double) table_mode = 'queuing';
```

In the preceding statement, if you do not change the value of the table option `table_mode`, the system creates a `normal` table by default.

If you detect read amplification and buffer table issues with the created table, you can change the value of the table option `table_mode` for the table to increase the trigger frequency of major compactions. In this case, the query time can be shortened. However, frequent major compactions consume a large amount of computing resources. Therefore, we recommend that you change the value of the table option `table_mode` based on the frequency level of triggering major compactions. For example, you can change the value from `normal` to `queuing` first, and then change it to `moderate` after a period of observation. Here is an example:
