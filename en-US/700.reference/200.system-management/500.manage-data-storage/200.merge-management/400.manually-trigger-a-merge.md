...
2. Initiate a major compaction.
   ...
   * Initiate a partition-level major compaction
      ...
      You can query the `CDB_OB_TABLE_LOCATIONS` view for `tablet_id` in the `sys` tenant. 
...
2. Initiate a tenant-level major compaction for the current tenant.
   ...
   * Initiate a partition-level major compaction
      ...
      In the syntax, `tablet_id` can be queried from the `DBA_OB_TABLE_LOCATIONS` view. 
