...
2. Select a proper minor compaction type as needed.
   ...
   * Initiate a minor compaction at the log stream level
      ...
      In the syntax, `tenant_name` specifies the name of a tenant and `ls_id` specifies the ID of a log stream in the tenant. You can query the `oceanbase.CDB_OB_TABLE_LOCATIONS` view for the log stream IDs of the tenant.
...
1. Log in to a MySQL or Oracle-compatible tenant of the cluster as the administrator of the tenant.
