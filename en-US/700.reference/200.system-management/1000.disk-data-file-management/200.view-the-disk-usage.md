...
2. Execute the following statement to view the disk space occupied by the tenant on each node.
        ...
        In the sys tenant, the following statement is used to view the disk space occupied by the `mysql001` tenant on each node. The disk space is broken down by component:
...
2. Execute the following statement to view the disk space occupied by table data.
   ...
   * In a user tenant
      ...
      * `REQUIRED_SIZE`: the size of disk space actually occupied by the data, in bytes.
      ...
      You can calculate the utilization of disk space from the returned results by using `OCCUPY_SIZE` / `REQUIRED_SIZE`.