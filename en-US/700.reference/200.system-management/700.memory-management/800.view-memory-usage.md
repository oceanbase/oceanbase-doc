...
From the `sys` tenant, you can query the `oceanbase.gv$sysstat` view to view the memory usage of a specified server.
...
### Query the total memory usage of each tenant

From the `sys` tenant, you can query the `oceanbase.GV$OB_TENANT_MEMORY` view to check the total memory usage of all tenants.
...
From the `sys` tenant, you can query the `oceanbase.GV$OB_MEMORY` view to check the memory usage details of a specified tenant.
...
From the `sys` tenant, you can query the `oceanbase.GV$OB_MEMORY` view to check the memory usage details of a memory module. You can also specify the server or tenant in the query.
...
2. Run the following command to go to the directory where log files are located:
...
3. Run the following command to query the total memory usage of the OBServer node:
   ...
   The following table describes some of the parameters in the log information.

   | Parameter | Description |
   |---------------|-----------------------------|
   | limit | Total memory limit of the OBServer node. |
   | hold | Total memory obtained by the OBServer node from the operating system. |
   | used | Memory actually used by the OBServer node (including Cache). |
   | freelist_hold | Unused memory of the OBServer node. |
...
3. Run the following command to view the total memory usage of each tenant:

   ```shell
   grep ob_malloc_allocator.cpp observer.log
   ```

   A partial query result is as follows:
...
3. Perform the following steps to view the total memory usage of the tenant with `tenant_id` 1002.
...
3. Perform the following steps to view the memory usage details of the `DEFAULT_CTX_ID` module for the tenant with `tenant_id` 1002.
