...
The `_min_malloc_sample_interval` and `_max_malloc_sample_interval` parameters are used to specify the sampling rate for memory allocation information.
...
<main id="notice" type='notice'>
     ...
     <li>Make sure that the value of <code>min_malloc_sample_interval</code> is smaller than or equal to the value of <code>_max_malloc_sample_interval</code>. </li>
...
2. Query the `__all_virtual_malloc_sample_info` virtual table for memory allocation information about the module that is suspected of memory leaks.
   ...
   The following example shows how to query the memory allocation information about the `glibc_malloc` module.
    ...
    |alloc_count  |   bigint(20)                  |  The number of times that memory allocation was performed.      |
