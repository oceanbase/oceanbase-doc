...
If `module_sum` of a memory module is greater than the difference between the `memory_size` of the tenant and the MemStore size of the tenant, it is considered an overrun has occurred on the memory module. If an overrun has occurred on a memory module, the memory for the individual memory modules needs to be adjusted, such as modifying the `ob_sql_work_area_percentage` parameter. If the memory size of the tenant is too small, increase the memory size.
...
If a memory overrun occurs in the memory module, you need to adjust the memory size for the module. If the memory of the tenant is too small, you need to increase the memory of the tenant. For more information, contact OceanBase Technical Support for assistance.
