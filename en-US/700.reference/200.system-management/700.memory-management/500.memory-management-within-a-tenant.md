...
Both the memory for a user tenant itself and the memory for its corresponding meta tenant consists of a MemStore for storing incremental data and a KVCache. For more information about MemStores and KVCaches, see [Manage non-auto-scaling memory](#manage-non-auto-scaling-memory) and [Manage auto-scaling memory](#manage-auto-scaling-memory).
