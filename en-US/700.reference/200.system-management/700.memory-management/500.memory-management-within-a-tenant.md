...
* When the overall tenant specification is greater than or equal to 1 G and smaller than 2 G, the memory specification of a meta tenant is fixed at 512 M, and the remaining resources are allocated to user tenants.
...
* Cache types in the `sys` tenant
  ...
  | fuse_row_cache              | Caches snapshot data of rows to improve the performance of point queries and avoid cache invalidation caused by dump (mini and minor compactions) and merge (major compactions). |
