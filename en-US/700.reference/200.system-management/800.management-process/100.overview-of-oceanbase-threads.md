...
OceanBase Database is single-process. In OceanBase Database, a part of threads are system threads that maintain the distributed database services, and other threads are worker threads that manage database loads.
...
* System threads:
   ...
   * dag thread: executes tasks such as dump (mini and minor compactions), merge (major compactions), and migration of partitions.
   ...
   * In addition, there are RootServer dedicated threads, which are not detailed here.
...
For more information about OceanBase Database threads, see [Thread introduction](../../100.oceanbase-database-concepts/1200.observer-node-architecture/300.observer-thread-model/100.thread-introduction.md).
