...
| HAGtsSource | Trx | 1 | Refreshes ha_gts. The refresh frequency is controlled by the `gts_refresh_interval` parameter.  |
...
| DAG | Storage | 16 | The worker thread of dag, which is used to run tasks such as dump (mini and minor compactions), merge (major compactions), and migration of partitions.  |
| DagScheduler | Storage | 1 | The scheduling thread of dag.  |
| STableChecksumUp | Storage | 1 | Batch commits SSTable checksum tasks.  |
| PartitionScheduler | Storage | 2 | Schedules dump (mini and minor compactions) and merge (major compactions) tasks.  |
| MemstoreGC | Storage | 1 | Recycles the frozen MemStore after dump (mini and minor compactions) warm-up is complete.  |
...
| KVCacheWash | Storage | 1 | Cleans the memory of KVCache. The detection interval is controlled by the `_cache_wash_interval` parameter. Default value: `200ms`. Value range: \[1ms,1m\].  |
| KVCacheRep | Storage | 1 | Manages memory fragments of the cache to recycle memory. The detection interval is controlled by the `_cache_wash_interval` parameter. Default value: `200ms`. Value range: \[1ms,1m\].  |
