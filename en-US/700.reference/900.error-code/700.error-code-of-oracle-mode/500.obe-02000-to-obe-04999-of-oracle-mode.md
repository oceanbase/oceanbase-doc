...
* [Error message "OBE-02051: another session or branch in same transaction failed or finalized" is returned in an Oracle-compatible tenant](https://www.oceanbase.com/knowledge-base/oceanbase-database-1000000000284436) -->
...
* SQLSTATE: 23000
* Cause: A foreign key constraint violation occurred because the parent key was not found. That is, the foreign key value in the child table record does not have a corresponding primary key value in the parent table.
* Solution:

  * Confirm and complete the missing primary key records in the parent table.
  * Check and correct the data in the child table so that its foreign key value references a valid primary key of the parent table.
  * If necessary, contact the database administrator to temporarily disable the constraint for data repair, or delete this foreign key constraint after confirming that the business logic permits it.

<!-- Related cases:

* [An error occurred while executing the INSERT ALL INTO statement in Oracle-compatible mode. Error code: OBE-02291](https://www.oceanbase.com/knowledge-base/oceanbase-database-1000000000429362?back=kb) -->
...
* [An error occurred while querying a view that is successfully created in an Oracle-compatible tenant in OceanBase Database. Error code: OBE-04063](https://www.oceanbase.com/knowledge-base/oceanbase-database-1000000000207732)
...
* Cause: The client session needs to be terminated.
