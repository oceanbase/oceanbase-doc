...
* Cause: The virtual column expression contains a function with an indeterminate value.

* Solution: Rewrite the expression so that it references only pure functions.
...
* Cause: Virtual columns support only scalar data types. Virtual columns do not support the `LONG`, `BLOB`, `REF`, and `BFILE` data types.
...
* Cause: The data type of the base expression is not supported for a virtual column. Virtual columns support only scalar data types. Virtual columns do not support the `LONG`, `BLOB`, `REF`, and `BFILE` data types.
...
* Solution: Re-execute the statement without providing a value for the virtual column.
...
* Solution: Re-execute the statement without setting a value for the virtual column.
...
* Cause: The table is partitioned on a virtual column that contains a PL/SQL expression.
...
* Solution: These columns are not supported. Modify the DDL.
...
* Cause: The source-side configuration for the backup path or archive path you specified does not exist.
...
* Cause: The amount of data written to disk for temporary files exceeds the configured limit.

* Solution: Check whether the SQL statement that triggered the error produced more data than expected (for example, if the expected maximum amount of temporary file data written to disk is 500 GB). If the data volume is expected, adjust the `temporary_file_max_disk_size` configuration item to an appropriate value.
...
* Cause: During physical recovery, the OBServer node cannot access the backup set file in the specified backup medium.

* Solution: Check connectivity to the object storage. If connectivity is normal, contact OceanBase Technical Support for assistance.
