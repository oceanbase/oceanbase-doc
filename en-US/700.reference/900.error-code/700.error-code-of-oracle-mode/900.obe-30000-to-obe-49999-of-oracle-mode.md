...
* Cause: A column value contains the string that the `SYS_CONNECT_BY_PATH` function uses to separate column values.

* Solution: Specify another separator for the `SYS_CONNECT_BY_PATH` function, ensuring this separator is not present in any column value, and then retry the operation.
...
* Solution: Check the syntax of the WAIT command used and ensure the wait time parameter is correct.
...
* Error cause: On new engines, an error is reported when the recursion depth is too deep, with a limit of 2 to the power of 30. On older engines, an error is also reported if the number of all nodes and their child nodes along the path from the root node to the current node exceeds a fixed value (2 to the power of 30). This value is currently set fixedly and generally will not be exceeded.
...
## OBE-30055: NULL snapshot expression not allowed here

* OceanBase error code: 4786

* SQLSTATE: HY000

* Error cause: Using a `NULL` snapshot expression is not allowed here.

* Solution: Check the syntax and validity of the expressions in the flashback query statement to ensure it points to a specific historical snapshot that exists within the retention period.
...
* Error cause: The data extraction source does not contain the specified extraction field.
...
* Cause: The `TIME` or `TIMESTAMP` column does not have a `TIME ZONE` specification.
...
* Cause: When a time zone modifier is specified, the `datetime` parameter must be a value of the `TIME`, `TIME WITH TIME ZONE`, `TIMESTAMP`, or `TIME STAMP WITH TIME ZONE` data type.
...
* Cause: In the parameter of the SQL function `UNISTR`, to include a backslash (\\\\) in the string itself, append four hexadecimal characters or another backslash after it.
...
* Cause: The parameter of a window function must be a constant for the partition.
...
* Cause: The parameter must be of a numeric type or a date/time type.
...
* Cause: Client login triggers cannot be of the `BEFORE` type.

* Solution: Review and modify the trigger definition to ensure it does not use the `BEFORE` type. LOGON triggers can only be of the `AFTER` type.
...
* Cause: Client logout triggers cannot be of the `AFTER` type.

* Solution: Review and modify the trigger definition to ensure it does not use the `AFTER` type. LOGOFF triggers can only be of the `BEFORE` type.
...
* Error cause: Attempt to use the `+` operator in the `select_list`, `CONNECT BY` clause, `START WITH` clause, or `ORDER BY` clause.

* Solution: Do not use the operator in the `select_list`, `CONNECT BY` clause, `START WITH` clause, or `ORDER BY` clause.
...
* Error cause: A `MEMBER` method of a certain type is called with a `NULL SELF` parameter.
...
* Error cause: The `NOT NULL` constraint is incorrectly specified on a `DEFAULT ON NULL` column.
...
* Error cause: Attempt to specify multiple identity columns for a table.
...
* Cause: The `NOT NULL` constraint is incorrectly specified on an identity column.
...
* Solution: Do not drop the `NOT NULL` constraint on an identity column. The only way to remove a constraint is to use `ALTER TABLE MODIFY` with `DROP IDENTITY` to drop the column's `identity` attribute.
...
* Cause: The maximum number of concurrent connections was exceeded when establishing an external TCP connection using the `UTL_TCP` system package.

* Solution: Ensure that the number of concurrent connections does not exceed the maximum number of concurrent connections.
...
* Cause: A stable set of rows could not be obtained due to heavy DML activity or a non-deterministic `WHERE` clause.

* Solution: Remove the non-deterministic `WHERE` clause and reissue the DML statement.
...
* Cause: A query with the specified `ORDER SIBLINGS BY` clause does not contain a `CONNECT BY` clause.
...
## OBE-31181: PL/SQL DOM handle accesses node that is no longer available

* OceanBase error code: 9841
* SQLSTATE: HY000
* Cause: The node accessed by the PL/SQL DOM handle is no longer available.
* Solution: Ensure that the DOM document and its nodes are still valid and not released before accessing them.
...
* Cause: The given node type does not support this operation. For example, adding a child node to an attribute node or passing a document node as a child node is not supported.
...
* Cause: The input parameter value is `NULL` or an invalid value.
...
* Cause: You attempt to change a parameter value, but the number of characters in the value exceeds the maximum allowed. For a list parameter, one of the values in the list exceeds the maximum allowed.
...
* Error cause: The usage of this `WITH` clause is not supported for the following reasons:
  ...
  * Nesting `WITH` clauses within `WITH` clauses is not supported.
...
* Error cause: A column alias or list of names was specified for the query name in the `WITH` clause, but the number of elements in the list does not match the number of elements in the Select-list that defines the query.
...
* Error message: The `WITH` clause query indirectly references itself (recursively), for example, through subqueries or views.
...
* Cause: The `SEARCH` clause of a recursive `WITH` query specifies a `sequence` column, but this `sequence` column is already listed in the column alias list for the query name in the `WITH` clause.

* Solution: Specify a different name for the `sequence` column in the `SEARCH` clause, or remove that name from the column alias list.
...
* Cause: The `cycle mark` or `non-cycle mark` value specified in the `CYCLE` clause of a recursive `WITH` query is not a one-byte string value.
...
* Cause: The `CYCLE` clause of a recursive `WITH` query specifies a `cycle mark` column, but this column already exists in the column alias list for the query name in the `WITH` clause.

* Solution: Specify a different name for the `cycle mark` column in the `CYCLE` clause, or remove that name from the column alias list.
...
* Cause: A name in the column alias list for an element of a `WITH` clause appears multiple times in the list.

* Solution: Remove duplicate names from the column alias list.
...
* Cause: A `SEARCH` or `CYCLE` clause was specified for a non-recursive element of a `WITH` clause.

* Solution: Remove the `SEARCH` or `CYCLE` clause, or add recursion to the `WITH` clause element.

## OBE-32481: The cycle value for the CYCLE clause must be different from the non-cycle values.

* OceanBase error code: 5753

* SQLSTATE: HY000

* Cause: A `CYCLE` clause is specified for a recursive `WITH` query, but the `cycle` value is the same as a non-cycle value or the `DEFAULT` value.

* Solution: Change the `cycle` value or a non-cycle value so that they are no longer identical.

## OBE-32482: The sequence column for the SEARCH clause must be different from the cycle mark column for the CYCLE clause.

* OceanBase error code: 5754

* SQLSTATE: HY000

* Cause: Both the `SEARCH` and `CYCLE` clauses are specified for a recursive `WITH` query, but the `sequence` column in the `SEARCH` clause matches the `cycle mark` column in the `CYCLE` clause.

* Solution: Change the `sequence` column or the `cycle mark` column so that they are no longer identical.
...
* Cause: A `CYCLE` clause is specified for a recursive query, but the `cycle` column list in the `CYCLE` clause contains duplicate names.

* Solution: Remove duplicate names from the `cycle` column list in the `CYCLE` clause.

## OBE-32485: An element in the cycle column list of the CYCLE clause must appear in the column alias list of the WITH clause element.

* OceanBase error code: 5757

* SQLSTATE: HY000

* Cause: The elements in the `cycle` column list of the `CYCLE` clause specified for a recursive `WITH` query are not included in the column alias list of the `WITH` clause element.
...
* Cause: The recursive component of a `UNION ALL` operation within elements of a recursive `WITH` clause uses an unsupported operation. The `GROUP BY`, `DISTINCT`, `MODEL`, grouping sets, `CONNECY BY`, window functions, `HAVING`, and aggregate functions are not allowed in the recursive branches of a `UNION ALL` operation.
...
* Cause: The recursive component of an element in a recursive `WITH` clause contains an unsupported join operation:

  * It contains a `LEFT OUTER` join where the right side is a recursive query name.

  * It contains a `RIGHT OUTER` join where the left side is a recursive query name.

  * It contains a `FULL OUTER` join that involves a recursive query name.
...
* Cause: A `WITH` clause element uses either the `SEARCH` or `CYCLE` clause, or both, but does not specify a list of column aliases for its query name.

* Solution: Add a list of column aliases for the query name in the `WITH` clause.
...
* Cause: An element in the `sort specification` list specified for a `SEARCH` clause in a recursive `WITH` query is not included in the column alias list of the corresponding `WITH` clause element.
...
* Error message: A recursive component of a `UNION ALL` in a recursive `WITH` clause element referenced the recursive query name multiple times. In the recursive branches of a recursive `WITH` clause element, the recursive query name can only be referenced once.

* Solution: Rewrite the query to ensure that the recursive query name is referenced only once in the recursive branches of the recursive `WITH` clause element.

## OBE-32575: Explicit column default is not supported for modifying views

* OceanBase error code: 5511
* SQLSTATE: 42000
* Error message: The DEFAULT keyword is used to modify a view.
...
* Error message: You attempt to audit a DDL statement with the specified `BY SESSION` clause. DDL statements can only be audited by using `BY ACCESS`.
...
* Error message: You attempt to create an instance view on a prebuilt table with an identity column.

* Solution: Remove the `identity` attribute or remove the identity column from the instance view definition.
...
* Error message: You attempt to alter a system-generated sequence.
...
* Cause: You are trying to insert a value into an identity column created with the `GENERATED ALWAYS` keyword.

* Solution: You cannot directly insert a value into an identity column generated by the `GENERATED ALWAYS` keyword. Instead, the value must be provided by the associated sequence generator.
...
* Cause: You are trying to update an identity column created with the `GENERATED ALWAYS` keyword.

* Solution: You cannot directly update an identity column generated by the `GENERATED ALWAYS` keyword.
...
* Cause: The sequences of the identity columns for the two tables specified in the `EXCHANGE` clause do not increase or decrease by the same amount.

* Solution: Ensure that the identity columns with `INCREMENT BY` have sequences with the same sign.
...
* Cause: You are trying to rename a system-generated sequence.
...
* Cause: The `LHS` in the `UPDATE SET` clause contains columns referenced in the `ON` clause.
...
* Cause: A required column in the error log table does not exist.
...
* SQLSTATE: 22032
* Cause: The provided JavaScript Object Notation (JSON) data syntax is invalid and cannot be parsed.
...
<main id="notice" type='explain'>
  <h4>Note</h4>
  <p>This error code was modified starting from V4.1.0. The error message was changed from <code>OBE-00600:Invalid JSON text in argument</code> to <code>OBE-40441:JSON syntax error</code>.</p>
</main>
...
* SQLSTATE: 42000
* Error message: The provided JavaScript Object Notation (JSON) data is invalid and cannot be parsed.
...
* Error message: References are not supported in the current version of JSON mode.
...
* SQLSTATE: 22032
* Error message: The provided JavaScript Object Notation (JSON) path expression is invalid and cannot be parsed.
* Solution: Provide a valid JSON path expression.

<main id="notice" type='explain'>
  <h4>Note</h4>
  <p>This error code was modified in V4.1.0. The error message was changed from <code>OBE-00600:Invalid JSON path expression </code> to <code>OBE-40442:JSON path expression syntax error ('INT') </code>. </p>
</main>
...
* SQLSTATE: 42000
* Cause: The default value does not match the type defined in the RETURNING statement.
...
<main id="notice" type='explain'>
  <h4>Note</h4>
  <p>This error code was modified starting with V4.1.0. The error message was changed from <code> OBE-00600:The JSON object contains a key name that is too long</code> to <code> OBE-40476:JSON key too long</code>.</p>
</main>
...
* SQLSTATE: 42000
* Error message: Attempting to print or escape non-ASCII characters for non-text output in JavaScript Object Notation (JSON) operations.
...
* Solution: Please enter correct information for column "%s".
...
* SQLSTATE: 42000
* Error message: An invalid keyword is used, causing a syntax error in the expression.
...
* SQLSTATE: 42000
* Error message: Path expressions under the same level in JSON_TABLE cannot be duplicate.
...
* SQLSTATE: 42000
* Cause: The key does not exist in the JSON data.
...
* Solution: Use PL/SQL JSON object types only within PL/SQL.
...
* Solution: Add the variable to the outer PASSING clause and ensure the name matches.
...
# OBE-40680: invalid identifier used for path expression in JSON_TABLE

* OceanBase error code: 5522
* SQLSTATE: 22000
* Cause: An invalid identifier is used in the path expression of the JSON_TABLE function.
* Solution: Strictly check and correct the syntax of the PATH clause in the JSON_TABLE function, paying special attention to the writing conventions at the beginning of the path, key name references, and array indices.
...
* Solution: Ensure that the data type of the sort column is not a large object, such as CLOB, BLOB, or JSON.
...
<main id="notice" type='explain'>
  <h4>Note</h4>
  <p>This error code was modified starting from V4.1.0. The error message was changed from <code> OBE-00600:The JSON binary value contains invalid data</code> to <code> OBE-40783:invalid binary JSON data\nstring</code>.</p>
</main>
...
* SQLSTATE: 42000
* Cause: An attempt was made to convert a Boolean value selected by a JavaScript Object Notation (JSON path) expression with the number() or numberOnly() function into a number.
...
* Cause: You attempted to insert, delete, or update rows in a read-only view. Performing DML operations on such views is explicitly prohibited.

* Solution: Perform the required DML operations on the base table of the view, or use the `ALTER VIEW ... READ WRITE` command to make the view operable.
