...
* Cause: Virtual columns support only scalar data types. Virtual columns do not support the `LONG`, `BLOB`, `REF`, or `BFILE` data types.
...
* Cause: The base expression's data type is not supported for a virtual column; only scalar data types are supported. Virtual columns do not support the `LONG`, `BLOB`, `REF`, or `BFILE` data types.
...
* Solution: Modify the virtual column expression.
...
* Cause: You cannot update the value of a virtual column in the body of a trigger.

* Solution: This operation is invalid. You must modify the trigger definition.
...
* Error cause: Attempting to combine modifications to a list expression with other column operations.
...
* Error cause: Attempting to change the visibility of a column in a table owned by the SYS user.
...
* Error cause: Attempting to use a non-constant expression for `pivot|unpivot` values.
...
* Error cause: Attempting to use a non-aggregate expression inside a `pivot` operation.
...
* Error cause: Insufficient privileges to operate on the resource manager.

* Solution: Execute the related operation as the root user.
...
* Cause: The SQL `ROW LIMITING` expression contains components other than text, bind variables, related variables, or subqueries.

* Solution: Modify the SQL `ROW LIMITING` expression and retry the operation.

## OBE-64500: invalid PL/SQL XML parser handle

* OceanBase error code: 9842
* SQLSTATE: HY000
* Cause: An invalid PL/SQL XML parser handle was used.
* Solution: Check the initialization and usage of the PL/SQL XML parser to ensure that the parser handle has been correctly created before the operation.
...
* Cause: The `ALTER SEQUENCE` statement contains duplicate `RESTART` directives.

* Solution: Remove duplicate `RESTART` directives.
