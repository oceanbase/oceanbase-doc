...
* Cause: The `SYSDATE` function or a user environment is used in the index function.
...
* Cause: Granting privileges to a role failed. You cannot grant `REFERENCES`, `INDEX`, or global privileges such as `SYSDBA`, `SYSOPER`, `SYSASM`, `SYSBACKUP`, and `SYSKM` to a role.
