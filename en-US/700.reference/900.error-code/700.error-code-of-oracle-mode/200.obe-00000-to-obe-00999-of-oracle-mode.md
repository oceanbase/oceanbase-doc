...
* SQLSTATE: 22023
* Error cause: Invalid spatial data was provided to function %s. A spatial function was called with a parameter that was not recognized as a valid geometric value.
...
* SQLSTATE: SR004
* Error description: A spatial reference system with SRID %u already exists.
...
* SQLSTATE: 01S01
* Error message: The SRID range [%u, %u] is reserved for system use. During the upgrade process, SRS within this range may be added, modified, or deleted without warning.
...
* SQLSTATE: 22S00
* Error cause: The function %s is defined only for the geospatial reference system, but one of its parameters has an SRID of %u, which is not geographic.
...
* SQLSTATE: 22S04
* Cause: The conversion from %s to %s is invalid. The polygon ring has the wrong orientation.
...
* SQLSTATE: 22S05
* Cause: Function %s's parameters contain geometries %u and %u with different SRIDs. All geometries must have the same SRID.
...
* SQLSTATE: HY000
* Error message: Runtime error, %s in function %s.
...
* SQLSTATE: HY000
* Error message: Unknown exception, %s in function %s.
...
* SQLSTATE: 22003
* Error message: Unable to obtain a geometry object from the data you sent to the GEOMETRY field.
...
* SQLSTATE: HY000
* Error cause: The Srid is used incorrectly. The Srid column attribute is only supported in geometry.
...
* SQLSTATE: HY000
* Error message: An object storage error code. Common causes include object storage connectivity and compatibility issues.
...
* Error message: The amount of data stored on disk by temporary files exceeds the set limit.

* Solution: Check whether the current erroneous SQL statement generates more data than expected (for example, the maximum amount of data expected to be written to disk as temporary files is 500 GB). If so, adjust the `temporary_file_max_disk_size` parameter to an appropriate value.
