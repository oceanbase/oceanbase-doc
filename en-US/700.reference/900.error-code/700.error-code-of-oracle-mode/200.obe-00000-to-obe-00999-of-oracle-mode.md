...
* Cause: Cursor attributes were applied to a non-cursor object.
...
* SQLSTATE: HY000
<<<<<<< llm_translation_of_commit_d3c8bcb1_260522161136
* Cause: The geometry function %s is called with unsupported parameter types.
=======
...
* SQLSTATE: HY000
<<<<<<< llm_translation_of_commit_d3c8bcb1_260522161136
* Error message: The query optimizer will not use the spatial index on column "%s" because the column lacks an SRID attribute. Consider adding an SRID attribute to the column.
=======
...
* SQLSTATE: 22S02
<<<<<<< llm_translation_of_commit_d3c8bcb1_260522161136
* Cause: Longitude %f is out of the range of the function %s. It must be within the range (%f, %f].
=======
...
* SQLSTATE: 22S03
<<<<<<< llm_translation_of_commit_d3c8bcb1_260522161136
* Cause: Latitude %f is out of the range of the function %s. It must be within the range [%f, %f].
=======
...
* SQLSTATE: HY000
<<<<<<< llm_translation_of_commit_d3c8bcb1_260522161136
* Cause: Index type %s is not supported for spatial indexes.
=======
...
* SQLSTATE: 22023
<<<<<<< llm_translation_of_commit_d3c8bcb1_260522161136
* Error cause: The option key "%s" in the function %s is invalid.
=======
...
<<<<<<< llm_translation_of_commit_d3c8bcb1_260522161136
* Solution: Check whether the current erroneous SQL statement generates more data than expected (for example, the maximum amount of data expected to be written to disk in temporary files is 500 GB). If so, adjust the `temporary_file_max_disk_size` parameter to an appropriate value.
=======
...
<<<<<<< llm_translation_of_commit_d3c8bcb1_260522161136
* Solution: Specify the correct key address in the restore command, or set the correct key address by using the `SET @kms_encrypt_info` session variable before restore.
=======
...
<<<<<<< llm_translation_of_commit_d3c8bcb1_260522161136
* Cause: The `log_only` cleanup strategy is applicable only to scenarios where log archiving is enabled but no data backup is performed. In scenarios where a data backup path is configured, the cleanup strategy cannot be set to `log_only`.
=======
...
<<<<<<< llm_translation_of_commit_d3c8bcb1_260522161136
* Cause: Data backup and archiving cleanup cannot be performed. Possible reasons include the tenant's status being non-`NORMAL` (for example, `CREATING`, `RESTORE`, etc.) or the tenant not having a cleanup strategy configured (for expiration cleanup scenarios).
=======
...
* Cause: In shared storage mode, the size of the microblock to be cached exceeds the upper limit allowed by the microcache (default is 128 KB), and the system refuses to write the microblock to the microcache.
...
* Cause: The user does not have the privilege to operate on the catalog.

* Solution: Check whether the user has the privilege to operate on the catalog. If not, grant the corresponding privilege to the user.
...
* Cause: A sensitive rule with the specified name does not exist. This typically occurs when deleting a sensitive rule via a DDL statement if the specified rule name does not match any existing rule.
...
* Error cause: The sensitive column with the specified name does not exist. This usually occurs when you try to remove a column that is not a sensitive column while modifying sensitive rules using DDL statements.
...
* Error cause: The user does not have the permission to execute the sensitive rule command `'%.*s'`. This usually occurs when a DML statement (such as `INSERT`, `UPDATE`, or `REPLACE`) accesses a column protected by a sensitive rule.
...
* SQLSTATE: HY000
* Error cause: A property that is not defined on a database object was referenced.
* Solution: Ensure that each referenced database object name is accurate and exists.
...
* SQLSTATE: HY000
* Error cause: OceanBase Database only allows widening (increasing the length) or constraining fields, not narrowing them.
* Solution: When encountering this error, do not forcibly narrow the field; instead, reassess the necessity of reducing its length.
...
* Error message: An invalid character was encountered.
...
<<<<<<< llm_translation_of_commit_d3c8bcb1_260522161136
* Error message: Too many values were provided.
=======
...
<<<<<<< llm_translation_of_commit_d3c8bcb1_260522161136
* Solution: Remove the `LEVEL`, `PRIOR`, `ROWNUM`, `CONNECT_BY_ROOT`, `CONNECT_BY_ISLEAF`, or `CONNECT_BY_ISCYCLE` keywords.
=======
...
* SQLSTATE: HY000
<<<<<<< llm_translation_of_commit_d3c8bcb1_260522161136
* Error message: The current version does not support nested cursor variables within records, objects, or collections.
