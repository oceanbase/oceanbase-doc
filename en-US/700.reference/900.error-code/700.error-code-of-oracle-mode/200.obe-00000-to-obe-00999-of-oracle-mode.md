...
=======
  <h4>Note</h4>
  <p>For OceanBase Database V4.3.5, this error code is introduced starting from V4.3.5 BP2.</p>
>>>>>>> V4.6.0
</main>
...
* Error cause: Cursor attributes were applied to a non-cursor object.
...
* Solution: Check whether the options specified in the SET command are correct, and ensure there are no spelling or syntax errors.
=======
...
* Cause: A system error occurred during the data transfer.
...
* Cause: The member lists of the log streams at the source and destination are inconsistent during a data transfer operation.
...
* SQLSTATE: HY000
<<<<<<< llm_translation_of_commit_50b9715a_260604175155
* Error message: An unknown exception was caught in the spatial function %s.
=======
...
* SQLSTATE: 22023
<<<<<<< llm_translation_of_commit_50b9715a_260604175155
* Error message: Invalid spatial data was provided to the function %s. A spatial function was called with a parameter that was not recognized as a valid geometric value.
=======
...
* SQLSTATE: HY000
<<<<<<< llm_translation_of_commit_50b9715a_260604175155
* Error message: The geometry has no data in the function %s.
=======
...
* SQLSTATE: HY000
<<<<<<< llm_translation_of_commit_50b9715a_260604175155
* Error message: Unable to calculate the centroid because the geometry in the function %s is empty.
...
* SQLSTATE: HY000
<<<<<<< llm_translation_of_commit_50b9715a_260604175155
* Error message: Geometry overlay calculation error: The geometry data in the function %s is invalid.
=======
...
* SQLSTATE: HY000
<<<<<<< llm_translation_of_commit_50b9715a_260604175155
* Error cause: Online operations are not supported for tables with spatial indexes.
=======
...
* SQLSTATE: 22S01
<<<<<<< llm_translation_of_commit_50b9715a_260604175155
* Error cause: The %s value is a geometry of unexpected type %s in %s.
=======
...
* SQLSTATE: HY000
<<<<<<< llm_translation_of_commit_50b9715a_260604175155
* Cause: The SRID of the geometry does not match the SRID of column '%s'. The SRID of the geometry is %lu, but the SRID of the column is %lu. Consider changing the SRID of the geometry or the column.
=======
...
<main id="notice" type='explain'>
  <h4>Note</h4>
<<<<<<< llm_translation_of_commit_50b9715a_260604175155
  <p>This error code is available starting with V4.1.0. </p>
=======
  <p>This error code was introduced in OceanBase Database V4.1.0. </p>
>>>>>>> V4.6.0
</main>
...
* SQLSTATE: HY000
<<<<<<< llm_translation_of_commit_50b9715a_260604175155
* Error cause: Srid is used incorrectly. The Srid column attribute is supported only in geometry.
=======
...
* SQLSTATE: HY000
<<<<<<< llm_translation_of_commit_50b9715a_260604175155
* Error cause: Spatial/full-text/hash indexes and explicit index orders are used incorrectly.
=======
...
* SQLSTATE: 22023
<<<<<<< llm_translation_of_commit_50b9715a_260604175155
* Error message: The value "%s" for the option "%s" is invalid in the "%s" function.
=======
...
* Cause: An object storage data checksum error occurred.
...
<<<<<<< llm_translation_of_commit_50b9715a_260604175155
* Solution: Check whether the current erroneous SQL statement generates an unexpected amount of data (for example, if the expected maximum amount of data written to disk as temporary files is 500 GB). If so, adjust the `temporary_file_max_disk_size` parameter to an appropriate value.
=======
...
<<<<<<< llm_translation_of_commit_50b9715a_260604175155
* Error cause: The temporary file macroblock was unexpectedly released during use.
=======
...
<<<<<<< llm_translation_of_commit_50b9715a_260604175155
* Error cause: Writing a non-idempotent file with the same name to object storage configured with a retention policy.
=======
...
<<<<<<< llm_translation_of_commit_50b9715a_260604175155
* Error cause: During physical restore execution, the OBServer node cannot access the backup set file in the specified backup media.
=======
...
<<<<<<< llm_translation_of_commit_50b9715a_260604175155
* Error cause: The disk quota has been exceeded.
=======
...
<<<<<<< llm_translation_of_commit_50b9715a_260604175155
* Solution: Correctly mount the backup medium to ensure that all OBServer nodes of the tenant initiating the archiving, backup, or restore operation can access the same backup medium. For detailed operations on mounting NFS, see [Deploy NFS](../../../600.manage/600.backup-and-recovery/200.deploy-nfs.md).
=======
...
<main id="notice" type='explain'>
<h4>Note</h4>
<p>This error code is introduced in V4.6.0 for V4.6.x.</p>
</main>
...
<<<<<<< llm_translation_of_commit_50b9715a_260604175155
* Solution: Check the object types referenced in the materialized view definition (only user tables or other materialized views can be referenced).
=======
...
* Solution: Check if there are any ongoing DDL operations on the base table of the materialized view.
...
* Cause:
  ...
  * The file may not exist or you do not have permission.
...
* Solution:
  ...
  * Check whether the HDFS datanode is running normally and the network connection is normal.

  * Check the HDFS logs (not the HDFS-related logs on OceanBase Database) to confirm whether there are any corrupt blocks or unavailable replicas.
...
* Cause:
  ...
  * Incorrect HDFS URI format.
  * Incorrect Kerberos authentication configuration.
  * HDFS client environment not initialized or invalid.
  * Invalid HDFS connection parameters.
  * Failed to set HDFS parameters.

* Solution:

  * Check whether the HDFS URI format is correct and whether the HDFS-related configurations are complete.
  * Confirm that the files required for Kerberos authentication (keytab, krb5.conf, ticket cache) exist and the paths are correct.
  * Check the connectivity of the HDFS cluster network to ensure the namenode is accessible.
  * Check whether the `key=value` format of the HDFS custom parameters is correct, ensuring no keys or values are empty.
...
* Error cause: Attempting to use an HDFS feature that is not yet implemented. The current HDFS storage backend does not support pwrite and append write operations.
...
* Solution:
  ...
  * Check the permissions on the HDFS file or directory to confirm whether the current user has read, write, and delete permissions.
...
* SQLSTATE: HY000
* Error cause: Query memory exceeds the limit.
...
* Error cause: When creating an external table, the file format was not specified, or no valid format configuration appeared in the DDL statement.
...
* Solution:
  ...
  * If using `CSV`, provide the necessary parameters such as delimiters and encoding to avoid query parsing errors despite successful table creation.
...
* Cause: The data access path was not specified when creating an external table. This is usually due to the `LOCATION` parameter being unconfigured, empty, or not correctly bound to a Location object.

* Solution:

  * Add the `LOCATION` clause in the DDL statement, pointing to the directory or URL where the external data is stored.

  * Ensure the path is accessible to the OceanBase cluster (network, authentication, mounting, object storage access keys/secrets, etc., configured according to the environment).
  ...
  * It is recommended to use a protocol prefix agreed upon with relevant personnel (e.g., `file://`, `oss://`, `hdfs://`, etc., subject to the version's support).
...
* Cause: `PARSE_HEADER` and `SKIP_HEADER` cannot be used simultaneously, as they are mutually exclusive.
...
* Cause: DDL operations on the Internal Catalog are not permitted.
...
* Cause: The user does not have the operation permission on the catalog.
...
* Cause: The catalog does not have the required permission.

* Solution: Check the permissions of the catalog.
...
* Cause: A CCL rule with the same name already exists when creating the new one.
...
* Cause: The rule does not exist when modifying or deleting it.
...
* Cause: The number of concurrent SQL sessions has reached the upper limit specified by the CCL rule.
...
* Solution: First, locate the target node logs by trace_id, and resolve the issue by checking resource monitoring data and adjusting parameters.
...
<<<<<<< llm_translation_of_commit_50b9715a_260604175155
* Solution: Check if the specified sensitive rule name already exists, and specify a different, non-existent name.
=======
...
* Solution: Check if the entered sensitive rule name is correct, and re-enter the correct name.
...
* Cause: The specified column is already a sensitive column. This typically occurs when adding/modifying a sensitive rule via a DDL statement, and attempting to apply a sensitive rule to a column that is already covered by another sensitive rule.
...
* Error cause: The sensitive column with the specified name does not exist. This usually occurs when you try to remove a column that is not a sensitive column while modifying a sensitive rule using a DDL statement.

* Solution: Check whether the specified sensitive column exists and enter the correct name of the sensitive column.
...
* Error cause: The user does not have the permission to execute the sensitive rule command `'%.*s'`. This usually occurs when a DML statement (for example, `INSERT`, `UPDATE`, or `REPLACE`) accesses a column protected by a sensitive rule.

* Solution: Check the user's permissions. Grant the appropriate explicit access permission to the user using the `GRANT PLAINACCESS ON SENSITIVE RULE '%.*s' TO '%.*s'@'%.*s'` command.
...
* Error cause: The sensitive rule name is invalid.

* Solution: Check whether the specified sensitive rule name is correct and enter the correct name.
...
* Error cause: The server session is in a non-recoverable state.
...
* SQLSTATE: HY000
* Error cause: An attribute that is not defined on a database object was referenced.
* Solution: Ensure that each name of the referenced database object is accurate and exists.
...
* SQLSTATE: HY000
* Error cause: In an `ALTER TYPE` statement, the same attribute name or method name can only appear once.
...
* SQLSTATE: HY000
* Cause: OceanBase Database only allows widening the length of a field or its constraints; narrowing is not supported.
* Solution: When this error occurs, you cannot perform the narrowing operation forcibly. You must reassess the necessity of shortening the length.
...
<<<<<<< llm_translation_of_commit_50b9715a_260604175155
* Solution: Refer to the SQL Reference (Oracle-compatible mode) for your version of OceanBase Database to find the correct SQL statement.
=======
...
* [Stored procedure error caused by the $$PLSQL_UNIT variable not being supported in an Oracle-compatible tenant](https://www.oceanbase.com/knowledge-base/oceanbase-database-1000000000297649)
...
<<<<<<< llm_translation_of_commit_50b9715a_260604175155
* Error cause: The specified string length exceeds the supported length for the data type. For CHAR and RAW data types, the specified length exceeds 2000; for other data types, the specified length exceeds 4000.
=======
...
* [Update of a CLOB column in Oracle-compatible mode reports error ret_code=-5083](https://www.oceanbase.com/knowledge-base/oceanbase-database-1000000000322504)
...
* [Insertion error OBE-00932 due to inconsistent data types](https://www.oceanbase.com/knowledge-base/oceanbase-database-1000000000217851)
=======
...
<<<<<<< llm_translation_of_commit_50b9715a_260604175155
* Error cause: The table or view does not exist. For OceanBase Database V2.2.70 and later, role-based privileges are introduced. If the current role does not have the `SELECT` privilege on a specific table or view, this error will also be returned.
=======
...
* [Regular users cannot obtain object DDL via dbms_metadata.get_ddl() in Oracle-compatible mode, error code OBE-00942](https://www.oceanbase.com/knowledge-base/oceanbase-database-20000000169?back=kb)
