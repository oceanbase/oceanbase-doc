...
* Solution: Set `PGA_AGGREGATE_TARGET` to an appropriately large value in `WORKAREA_SIZE_POLICY = AUTO` mode, or  set `SORT_AREA_SIZE` to an appropriately large value when `WORKAREA_SIZE_POLICY` is set to `MANUAL`.
