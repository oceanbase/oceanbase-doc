...
* [Cursor execution failure (error code 5053)](https://www.oceanbase.com/knowledge-base/oceanbase-database-20000001059?back=kb) -->
...
<main id="notice" type='explain'>
    <h4>Note</h4>
    <p>Since OceanBase Database V4.0.0, the error message is changed from <code>ERROR 5307 (HY000): Scheduler thread number is not enough</code> to <code>ERROR 5307 (HY000): Not enough thread resource to execute the query. Require setting parallel_servers_target to a value no less than %ld for this query</code>. </p>
  </main>
...
* OceanBase error code: 5540

* Cause: The values of the system variables in the current session are different from the fixed values used to create a function index or generated column. Instead of the current variable values in the session, the fixed value set for the function index or generated column when they were created is used in calculations.

* Solution: To use the values in the current session, delete the generated column or function index and re-create them.
...
[Index creation failure (error code 5703)](https://www.oceanbase.com/knowledge-base/oceanbase-database-20000001012?back=kb) -->
...
[Index creation failure (error code 5827)](https://www.oceanbase.com/knowledge-base/oceanbase-database-20000000182?back=kb) -->
...
* Cause: The `REFERENCES` clause in the `CREATE TABLE` or `ALTER TABLE` statement specifies a column list that does not match a unique constraint or primary key constraint in the referenced table.
...
* Cause: `ROWID` cannot be queried from a view by using an SQL statement with the `DISTINCT` or `GROUP BY` clause.
...
* Cause: `SYSDATE` or a user environment is used in the index function.
