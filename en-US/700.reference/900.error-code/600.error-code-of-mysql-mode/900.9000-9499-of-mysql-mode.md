...
The range of error codes indicates backup restore error codes and STORAGE 3.0 error codes.

## ERROR 9001 (HY000): URI error

* OceanBase error code: 9001

* Error cause: The destination connection for backup restore may be incorrect.

* Solution: Check the relevant backup restore configurations and make the necessary modifications.

## ERROR 9002 (HY000): OSS file MD5 error

* OceanBase error code: 9002

* Error cause: The data of the OSS file is invalid, possibly due to an error on the OSS.

* Solution: Contact Alibaba Cloud OSS technical support to troubleshoot the issue.

## ERROR 9003 (HY000): OSS error

* OceanBase error code: 9003

* Error cause: An unknown error occurred on the OSS.
...
* Error cause: An error occurred during the initialization of MD5 in OpenSSL.
...
* Error cause: The number of elements exceeds the maximum limit.
...
* Error cause: The MD5 update failed.
...
* Error cause: The length of the OSS file is invalid, possibly due to an error in the file.
...
* Error cause: The MD5 checksum of the OSS file does not match, possibly due to data anomalies on the OSS or a bug in the backup restore program.
...
* Error cause: The backup file could not be found, possibly due to incorrect backup restore configurations.
...
* Error cause: The `data version` parameter is missing in the logical restore parameters.
...
* Error cause: An error occurred while writing the OSS file, possibly due to an unknown issue on the OSS.
...
* Error cause: A tenant with the same name exists for the restore tenant.
...
* Error cause: The cluster name and URL for logical backup are inconsistent.
...
* Error cause: The RS list is invalid, which may indicate an error in the specified RS list.
...
* Error cause: Logical backup has failed tasks.
...
* Error cause: The restored partition is repeatedly scheduled for restoration.
...
* Error cause: Delayed deletion of tables during physical backup.
...
* Error cause: Log archiving cannot be repeatedly enabled during log archiving.
...
* Error cause: Log archiving cannot be repeatedly disabled during the stop phase.
...
* Error cause: The file of log archiving information does not exist.
...
* Error cause: The backup destination is invalid.
...
* Error cause: Log archiving is interrupted.
...
* Error cause: The log archiving status does not match.
...
* Error cause: Log archiving is not running. This is usually an error message printed by the OBServer node when backup is being stopped.
...
* Error cause: The log archiving round information is invalid.
...
* Error reason: The replica is in the process of migration or log replication, and therefore cannot provide data backup capabilities.
...
* Error reason: Index reconstruction failed after the restore was completed.
...
* Error reason: It is usually caused by executing an operation that is mutually exclusive with the backup.
...
* Error reason: The log archiving status is invalid.
...
* Error reason: Replication is not supported during the `set member list` phase.
...
* Error reason: The leader for log archiving has changed.
...
* Error reason: Log archiving is not in the `DOING` state, so data backup cannot be enabled.
...
* Error reason: Canceling the backup task is not allowed.
...
* Error reason: If data backup is not completed during more than 16 `Major Merge` operations, it will fail.
...
* Error reason: It is not allowed to schedule a backup cleanup task again.
...
* Error reason: It is not allowed to delete a backup set.
...
* Error reason: The backup set ID is invalid, possibly due to an invalid `backup_set_id`.
...
* Error reason: Data backup and log archiving are discontinuous.
...
* Error reason: The scheduled backup task does not have data to back up.
...
* Error reason: Some OBServer nodes are not mounted with NFS.
...
* Error reason: The backup cleanup metadata is inconsistent.
...
* Error reason: The `Cancel` command was issued without any cleanup tasks.
...
* Error reason: The cleanup metadata does not exist.
...
* Error reason: An internal error code. This error typically occurs during internal backup scheduling when the Backup Set or Backup Piece has reached the maximum number of backup times.
...
* Error cause: The module responsible for adapting Tencent Cloud COS reported an error.
...
* Error cause: This error is caused by IO limitations. Currently, only the List operation in the Tencent Cloud COS module returns this error code.
...
* Error cause: When scheduling backups for the specified number of backup times, all backup_sets and backup_pieces have met the requirements, and no backup_set or backup_piece needs to be backed up.
...
* Error cause: The List operation for backup and restore in the COS environment is throttled.
...
* Error cause: The archived log data and original backup data are not continuous.
...
* Error cause: This error is returned when a task is being performed during the suspension of the backup feature for logical backup and restore.
...
* Error cause: Conflicting values exist.
...
* Error cause: Deleting backup log shards is not allowed.
...
* Error cause: There is no connectivity between the OBServer node and the backup medium, such as the NFS is not mounted or the object storage has no access permissions.
...
* Error cause: The OBServer node does not have the operation permissions on the backup medium.
...
* Error cause: Session conflicts in the backup I/O independent sub-process.
...
* Error cause: When the backup medium is Huawei Cloud OBS, a storage medium-level error may occur.
...
* Error cause: After the tenant is deleted, the backup validation task for the tenant is skipped.
...
* Error cause: Backup and restore I/O failed due to an abnormal obesi process.
...
* Error cause: The metadata index of the backup data does not exist.
...
* Error cause: When continuously reading the archive logs, the logs of two consecutive rounds are discontinuous and cannot be continuously read.
...
* Error cause: Continuous reading of the archive logs has reached the end.
...
* Error cause: When continuously reading the archive logs, the logs have been recycled.
...
* Error reason: The content of the format file in the destination directory does not match the expected content.
...
* Error reason: The backup disk has insufficient storage space.
...
* Error reason: The offset of the backup restore append write file does not match the file length.
...
* Error reason: The content of the backup restore append write file does not match the existing content of the file.
...
* Error reason: The range of the major SSTable to be backed up does not cover the minor SSTable.
...
* Error reason: The backup of the log stream position timed out.
...
* Error reason: The archive file has been recycled during consumption.
...
* Error reason: The state of the source tenant or log stream does not match, and the current tenant cannot be switched to the primary tenant.
* Solution:
...
* Error reason: The state of the source log stream does not match, and the current tenant cannot be switched to the primary tenant.
* Solution:
...
* Error reason: Already in ARCHIVELOG mode.
...
* Error reason: Already in NOARCHIVELOG mode.
...
* Error reason: The standby database version is lower than the primary database version.
...
* Error reason: The primary tenant was dropped during primary-standby synchronization.
...
* Error reason: An error occurred while trying to append content to a normal object in an object storage service (such as Alibaba Cloud OSS or Tencent Cloud COS).
...
* Error reason: The corresponding directory or file could not be found in the storage layer. This may be due to an issue with the configured storage directory.
...
* Error reason: The tenant cloning process timed out.
...
* Error reason: The tenant cloning job failed.
...
* Error reason: A tenant snapshot lock conflict occurred.
...
* Error reason: The connection to the object storage destination failed.
...
* Error reason: No I/O operation permission of the object storage.
...
* Error reason: The `s3_region` is not specified, or the specified `s3_region` does not match the endpoint.
...
* Error reason: The endpoint of the object storage is invalid.
...
* Error reason: An error occurs during I/O operations on the object storage. Common causes include connectivity and compatibility issues with the object storage.
...
* Error reason: The specified backup zone, IDC, or region does not exist. For example, if a cluster has only zone1 and zone2, but you set zone3 or zone4 as the backup zone, this error will be reported because zone3 and zone4 do not exist.
...
* Error reason: The amount of data written to the temporary file exceeds the specified limit.
...
* Error reason: The multi-version major compaction SSTable is recycled during backup.
...
* Error reason: Transparent encryption is enabled for the backup tenant, but the key address is not specified or the specified address is invalid during the restore.
...
* Error reason: The object is in the retention period and is being deleted or modified.
...
* Error reason: The temporary file macroblock was unexpectedly released.
...
* Error reason: A file with the same name is written to the object storage configured with a retention policy, but the write operation is not idempotent.
...
* Error reason: An unexpected file exists in the macroblock cache directory, which may be a file created by the user.
...
* Error reason: SSBlock files cannot be reduced in size.
...
* Error reason: Macroblock marking failed after the SSBlock file was automatically expanded.
...
* Error reason: Auto-expansion has not been started, possibly due to configuration issues, or the `datafile` preset disk resource has reached its maximum limit.
