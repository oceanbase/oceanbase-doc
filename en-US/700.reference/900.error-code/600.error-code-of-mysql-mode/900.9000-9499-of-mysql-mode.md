...
* [Cross-region restore error: The backup object cannot be restored to the specified point in time](https://www.oceanbase.com/knowledge-base/oceanbase-database-1000000000208064) -->
...
* [Failed to set the backup path; error "backup destination is not valid"](https://www.oceanbase.com/knowledge-base/oceanbase-database-1000000000208058) -->
...
* [Backup process runs for a long time and the logs report error 9032](https://www.oceanbase.com/knowledge-base/oceanbase-database-1000000000208050) -->
...
* Cause: An unexpected file exists in the macroblock cache directory, which may be a file created by the user.
...
* Cause: During physical restore, an OBServer node cannot access backup set files in the specified backup medium.

* Solution: Check object storage connectivity. After you rule out connectivity issues, contact OceanBase Technical Support for further assistance.
...
* Cause: The disk quota exceeds the limit.

* Solution: Free disk space or adjust the disk quota.
...
* Cause: The `log_only` cleanup policy applies when only log archiving is enabled and there is no data backup. When a `log_only` cleanup policy exists, you cannot configure a data backup destination.

* Solution: Delete the `log_only` cleanup policy first, and then configure the data backup destination.
...
* Cause: The `log_only` cleanup policy applies when only log archiving is enabled and there is no data backup. When a data backup destination is already configured, you cannot set the cleanup policy to `log_only`.

* Solution: If you need the `log_only` cleanup policy, delete the configured data backup destination first.
...
* Cause: Data backup and log archiving cleanup cannot start. For example, the tenant may not be in the `NORMAL` state (such as `CREATING` or `RESTORE`), or no cleanup policy is configured for the tenant (for expired cleanup scenarios).

* Solution: Check the tenant state. For expired cleanup scenarios, verify that the related tenants have cleanup policies configured correctly.
...
* Cause: The backup medium may not be mounted correctly.

* Solution: Mount the backup medium correctly so that all OBServer nodes of tenants that perform archiving, backup, or restore can access the same backup medium. For details about mounting NFS, see [Deploy NFS](../../../600.manage/600.backup-and-recovery/200.deploy-nfs.md).
...
* Cause: Read and write access to the backup medium is not strongly consistent.

* Solution: If the backup medium is NFS, see [Deploy NFS](../../../600.manage/600.backup-and-recovery/200.deploy-nfs.md) and mount it as documented. If the backup medium is object storage, contact the corresponding service provider.
...
* Cause: No backup set can be used to restore to the specified time.

* Solution: Choose another restore point or specify another backup destination path.

<main id="notice" type='explain'>
<h4>Note</h4>
<p>For V4.4.2, this error code was introduced in V4.4.2 BP1. </p>
</main>
