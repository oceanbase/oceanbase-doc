...
* Error code in OceanBase Database: 9139

* Cause: When transparent encryption is enabled for the backup tenant, the key address was either not specified or incorrectly specified during tenant recovery.
...
* Error code in OceanBase Database: 9140

* Cause: Attempted to delete or modify an object in object storage that is within its retention period.
...
* Error code in OceanBase Database: 9141

* Cause: The temporary file macro block was unexpectedly released during use.

* Solution: Please contact OceanBase technical support for assistance with troubleshooting.
...
* Error code in OceanBase Database: 9142

* Cause: Temporary file macro blocks were unexpectedly released during use.
...
* Cause: SSBlock files do not support downsizing.

* Solution: Check whether the size specified by `ALTER SYSTEM SET datafile_size` is greater than the current SSBlock file size, or contact OceanBase technical Support for assistance with troubleshooting.
...
* Cause: Macroblock marking failed after SSBlock file auto-expansion.
...
* Cause: Automatic expansion may not have been triggered due to configuration issues, or the preset disk resource usage for `datafile` has reached its limit.

* Solution: Check whether the `free_size` field in the `__all_virtual_disk_stat` table is 0. If it is, it indicates that the disk space is exhausted. You need to increase the `datafile` threshold or expand the disk capacity.