...
## ERROR 6290 (HY000): Transaction rollbacked as no rows were affected

* OceanBase error code: 6290
* Cause: In scenarios involving hotspot row aggregation optimization, if a data modification operation is performed within a transaction but ultimately no data rows are affected, the entire transaction is automatically rolled back.
* Solution: Verify that the SQL statement and data state comply with business logic.

<main id="notice" type='explain'>
  <h4>Note</h4>
  <p>This error code is introduced since OceanBase Database V4.6.0. </p>
</main>
...
## ERROR 6333 (HY000): attempt to access a transactional temp table already in use

* Error code in OceanBase Database: 6333
* Cause: Both transactional temporary tables and session-level temporary tables may be in an "occupied" state, causing new access requests to be rejected.
* Solution: Release the occupation of the temporary table.
