...
* [Error ERROR 1062 when inserting duplicate data](https://www.oceanbase.com/knowledge-base/oceanbase-database-1000000000217891)
...
* [Error: Parse error when modifying unit specifications](https://www.oceanbase.com/knowledge-base/oceanbase-database-1000000000207664)
* [Failed to grant column-wise privileges to a MySQL-compatible tenant](https://www.oceanbase.com/knowledge-base/oceanbase-database-1000000000207670)
...
* [Error when creating an index: length exceeds 16 KB](https://www.oceanbase.com/knowledge-base/oceanbase-database-1000000000207734)
...
* [Too many columns error when creating a table in OceanBase](https://www.oceanbase.com/knowledge-base/oceanbase-database-1000000000207739)
...
* Cause: The grant is not defined for the specified user and host.
* Solution: Ensure the correct grant is defined for the specified user and host.
...
* [Granting all privileges to a user in MySQL mode still results in permission denied to view tables](https://www.oceanbase.com/knowledge-base/oceanbase-database-1000000000262232)
...
* [Some tables created by CTAS are accidentally deleted after successful CTAS creation](https://www.oceanbase.com/knowledge-base/oceanbase-database-20000045542)
...
* [MySQL tenant foreign key does not support non-unique keys](https://www.oceanbase.com/knowledge-base/oceanbase-database-1000000000207714)
...
* [Error adding foreign key in DDL statement, error code 1235](https://www.oceanbase.com/knowledge-base/oceanbase-database-20000000067?back=kb)
* [Data access error across nodes, error code 1235](https://www.oceanbase.com/knowledge-base/oceanbase-database-20000001060?back=kb)
...
* [Error executing DDL ERROR 1235, changing character set or collation is not supported](https://www.oceanbase.com/knowledge-base/oceanbase-database-1000000000207750)
* [Error adding primary key to table via ALTER TABLE](https://www.oceanbase.com/knowledge-base/oceanbase-database-20000001005)
* [Partitioned table does not support DROP operations on RANGE indexes](https://www.oceanbase.com/knowledge-base/oceanbase-database-20000000111)
* [sys tenant error ERROR 1235(0A000): uncertain plan voilating external consistency not supported](https://www.oceanbase.com/knowledge-base/oceanbase-database-1000000000008215)
...
* Solution: Ensure that the data type of the columns you are trying to create an FTS index on is TEXT, such as VARCHAR, TEXT, or CHAR.
...
* Cause: This syntax is deprecated and will be removed in a future version.
...
* Solution: Check whether the user has the required privilege and verify that there are no spelling errors in the stored procedure or function name.
...
* [Compatibility of LONGTEXT fields in OceanBase Database's MySQL-compatible mode](https://www.oceanbase.com/knowledge-base/oceanbase-database-1000000000207745)
...
<main id="notice" type='explain'>
  <h4>Note</h4>
  <p>This error code has been introduced since V4.0.0.</p>
</main>
...
* Cause: VALUES LESS THAN can only be used in partition definitions that employ RANGE PARTITIONING.

* Solution: Use the correct syntax for partition definition based on the table's partitioning scheme.
...
* Cause: When creating a partitioned table, incompatible partitioning methods were used, such as simultaneously using RANGE/LIST partitioning and HASH/KEY partitioning in subpartitions.
* Solution:
  * Modify the partitioning method: If you need to use both RANGE/LIST partitioning and HASH/KEY partitioning in subpartitions, consider modifying the partitioning method to use a compatible one. Note that modifying the partitioning method or splitting a partitioned table may affect existing data and applications; proceed with caution.
...
* Compatible MySQL error code: 1517
* Cause: When modifying a table partition name, the new name duplicates an existing partition name in the table. (Since MySQL does not have a concept of subpartitions, this error code is shared by both partition and subpartition names.)
* Solution: Modify the partition name or delete the partition with the duplicate name.
...
* Solution: Modify the partition name or delete the duplicate partition.
...
* Cause: The attempted creation of an event with a name that conflicts with an existing event name.

* Solution: Select a different, unique name for the new event.
...
* Cause: The attempted access or operation on an event that does not exist.

* Solution: Confirm whether the event name is correct and whether the target event exists.

<main id="notice" type='explain'>
  <h4>Note</h4>
  <ul><li>In V4.3.x, this error code was introduced starting with V4.3.2.</li><li>In V4.2.x, this error code was introduced starting with V4.2.4.</li></ul>
</main>
...
* Error cause: The specified time interval (INTERVAL) is not a positive number or exceeds the allowed maximum value when defining an event.
...
* Cause: When you try to rename an event, the new name is the same as the old one, making the operation invalid.
...
* Solution: Please check whether the partition name is entered correctly.
...
* Cause: A DDL operation attempts to directly or indirectly modify itself again. Recursive execution of an event's DDL statements is prohibited when the event definition contains a body.
...
* Solution: Compare the structural definitions of the two tables to ensure they match, including the table structure, field types, and field count.
...
* Cause: The query execution time exceeds the maximum allowed by the system. It exceeds the threshold set by the `max_execution_time` parameter.
...
<main id="notice" type='explain'>
  <h4>Note</h4>
  <p>This error code is deprecated in V4.0.0.</p>
</main>
...
* Solution: Please use a different authorization ID, or delete the existing authorization ID and try creating the authorization ID again.
...
* [Error when running GIS feature spatial data type fields and related functions: ERROR 3548](https://www.oceanbase.com/knowledge-base/oceanbase-database-1000000000245401)
...
* Cause: ORDER BY, LIMIT, SELECT DISTINCT, HAVING, WINDOW FUNCTION, or GROUP BY is not supported in the public expression of a recursive query block.

* Solution: Do not use ORDER BY, LIMIT, SELECT DISTINCT, HAVING, WINDOW FUNCTION, or GROUP BY in the public expression of a recursive query block.
...
* Cause: A window with a RANGE frame contains an ORDER BY expression based on a datetime type, and only INTERVAL boundary values are allowed.

* Solution: Ensure that the ORDER BY expression in a window with a RANGE frame uses only INTERVAL boundary values, not datetime boundary values.
...
* Cause: A window with a RANGE frame contains an ORDER BY expression of a numeric type, and INTERVAL boundary values are not allowed.

* Solution: Do not use INTERVAL boundary values in a window whose ORDER BY expression is of a numeric type.
...
* Cause: The window has a non-constant frame boundary.
...
* Cause: Arrays or objects cannot be stored in a scalar column.

* Solution: Convert the array or object to a scalar value, or modify the column's data type to accommodate arrays or objects.
...
* Error cause: When creating a function-based index, an expression that returns a BLOB or TEXT type was used, which is not allowed for creating a function-based index.
* Solution: Use the CAST function. You can use the CAST function to convert a BLOB or TEXT type expression to another type, and then create a function-based index with the converted expression.
...
* Error cause: When creating a function-based index, an unsupported function was used as the index function, such as SYSDATE or a user-defined function.
...
* Error cause: When creating a function-based index, the index function refers to a single column.
* Solution: Please use a regular index.
...
* Cause: The column to be dropped has a functional index dependency and cannot be directly dropped or renamed.
* Solution:
...
<main id="notice" type='explain'>
  <h4>Note</h4>
  <p>For this error code, the error message has been adjusted from <code>Column '%.*s' has a functional index dependency and cannot be dropped.</code> to <code>Column '%.*s' has a functional index dependency and cannot be dropped or renamed.</code> starting from V4.2.1.</p>
</main>
...
* Cause: An invalid JSON type is used as an argument, but it should be an object type.
...
* Cause: Each row in a VALUES clause must have at least one column, unless it is used as a data source in an INSERT statement.

* Solution: Ensure each row in the VALUES clause has at least one column.
