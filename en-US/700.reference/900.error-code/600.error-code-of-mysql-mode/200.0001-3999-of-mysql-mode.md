...
* Cause: OceanBase Database of the current version in MySQL mode does not support specifying a non-string default value for a column in a JSON_TABLE expression.

* Solution: Upgrade to the latest version that supports the feature or specify a string-type default value for the column.
...
* Cause: We recommend that you do not use this syntax, which will be removed in later versions.
...
* Cause: The routine command to the user was denied.
...
* MySQL error code: 1517
* Cause: When you change the name of a partition, the new name is already taken by an existing partition. The concept of subpartitions is no longer used in the MySQL mode of OceanBase Database since version 4.2.0. Therefore, the same error code is used for both partitions and subpartitions in MySQL tenants of OceanBase Database.
...
* Cause:
   * The entered plug-in type is not supported.
   * The entered plug-in type is supported, but was not correctly loaded by the program. OceanBase Database only supports plug-ins of the `mysql_native_password` type.

* Solution:
   * Check whether the plug-in type is supported.
   * Check whether the statement and configuration file that load the plug-in, and the plug-in version are correct.
   * Check whether you have the permissions to load and install the plug-in.
...
* Cause: The parameter contains invalid JSON type.

* Solution: Make sure that the parameter is a valid JSON object type, or is converted to a valid JSON object type.
