...
* [Error "Too many columns" when creating a table in OceanBase](https://www.oceanbase.com/knowledge-base/oceanbase-database-1000000000207739)
...
* Cause: The grant is not defined for the specified user on the specified host.
* Solution: Ensure that the grant is correctly defined for the specified user on the specified host.
...
* [Still prompted no permission to view a table after granting all privileges to a user in MySQL-compatible mode](https://www.oceanbase.com/knowledge-base/oceanbase-database-1000000000262232)
...
* [Some tables created by CTAS are accidentally deleted after successful CTAS table creation](https://www.oceanbase.com/knowledge-base/oceanbase-database-20000045542)
* [Table does not exist when querying, error code ERROR 1146](https://www.oceanbase.com/knowledge-base/oceanbase-database-1000000000217889)
...
* [MySQL tenant foreign key does not support non-unique key](https://www.oceanbase.com/knowledge-base/oceanbase-database-1000000000207714)
...
* [Creation failed for small-specification tenant resource pool (1C2G)](https://www.oceanbase.com/knowledge-base/oceanbase-database-1000000000207672)
* [Error 1235 when modifying tenant locality](https://www.oceanbase.com/knowledge-base/oceanbase-database-1000000000207661)
* [Partition cleanup failed, error code 4007](https://www.oceanbase.com/knowledge-base/oceanbase-database-20000000194?back=kb)
* [Partition deletion failed, error code 1235](https://www.oceanbase.com/knowledge-base/oceanbase-database-20000000146?back=kb)
...
* Cause: The current version of MySQL mode does not support specifying a non-string default value for a column in a JSON_TABLE expression.
...
* Solution: Check whether the user has the corresponding permission and whether there are spelling errors in the stored procedure or function name.
...
* [Error -1452 occurs when inserting data in batches in a self-referencing foreign key scenario](https://www.oceanbase.com/knowledge-base/oceanbase-database-1000000000409003)
...
* Error cause: An incompatible partitioning method was used when creating a partitioned table. For example, RANGE/LIST partitioning and HASH/KEY partitioning are used together in subpartitions.
* Solution:
  ...
  * Split the partitions: If you need to use incompatible partitioning methods, consider splitting the partitioned table into multiple tables, each using a different partitioning method.
...
* Error cause:
  ...
  * The input plugin type is supported (currently, OceanBase Database supports `mysql_native_password` and `caching_sha2_password` types), but the plugin has not been correctly loaded by the program. `caching_sha2_password` is supported starting from V4.6.1.
...
* Cause: The name of the event to be created conflicts with an existing event name.
...
* Cause: The operation is invalid because the new name is the same as the old name when you try to rename an event.
...
* Solution: Check and shorten the length of the inserted data to ensure it does not exceed the column's maximum length limit.
...
* Solution: Ensure the modified event execution time is in the future so that the event can be scheduled properly.

<main id="notice" type='explain'>
  <h4>Note</h4>
  <p>For V4.3.x, starting from V4.3.5, the error message for this code has been adjusted from <code>Event execution time is in the past and ON COMPLETION NOT PRESERVE is set. The event was not changed. Specify a time in the future.</code> to <code>Event execution time is in the past. The event was not changed. Specify a time in the future.</code>.</p>
</main>
...
* Solution: Compare the structure definitions of the two tables to ensure they match, including table structure, field types, and field counts.
...
* Cause: A foreign key constraint with an ON DELETE SET NULL clause cannot be defined on a virtual column.
...
* Solution: Check whether the specified index exists. If so, delete or modify the existing index and then recreate the new one.
...
* Solution: Ensure that the ORDER BY expression for a window with a RANGE frame uses only INTERVAL boundary values, not date-time boundary values.
...
* Error cause: An array or object cannot be stored in a scalar column.

* Solution: Convert the array or object to a scalar value, or change the column's data type to accommodate the storage of an array or object.
...
* Error cause: Every table function must have an alias.
...
* Solution: Use the CAST function to convert the BLOB or TEXT expression to another data type, and then create the functional index with the converted expression.
...
* Cause: An unsupported function is used in the expression of the functional index. The index function uses SYSDATE or the user environment.
...
* Solution: Create a regular index.
...
<main id="notice" type='explain'>
  <h4>Note</h4>
  <p>Starting from V4.2.1, the error message is adjusted from <code>Column '%.*s' has a functional index dependency and cannot be dropped.</code> to <code>Column '%.*s' has a functional index dependency and cannot be dropped or renamed.</code>.</p>
</main>
