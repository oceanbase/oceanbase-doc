...
* [An error indicating duplicate data occurred while inserting data. Error code: 1062](https://www.oceanbase.com/knowledge-base/oceanbase-database-1000000000217891) -->
...
* [An error occurred while creating a table. Error code: 1118](https://www.oceanbase.com/knowledge-base/oceanbase-database-20000000066?back=kb) -->
...
<!-- * Cause:
  ...
  * The format of the server configuration is incorrect.
  * The value of the `data_source` parameter is invalid.
  * The value of the `paxos_replica_num` parameter is smaller than or equal to 0.
  * The number of Paxos replicas specified by the `paxos_replica_num` parameter is not the majority.
  * The number of Paxos replicas specified by the `paxos_replica_num` parameter is smaller than the number of replicas on the member list.
  * The value of the `paxos_replica_num` parameter is increased or decreased by more than 1.
  * The `paxos_replica_num` parameter is modified.
  * The new value of the `paxos_replica_num` parameter is greater than the current value.
...
* Cause:
  ...
  * You are not the `SYS` user.
  * You do not have the `ALTER SYSTEM` privilege.

* Solution: Make sure that you have the required privilege to perform this operation.
...
* Cause: In MySQL mode of OceanBase Database of the current version, you cannot specify a default value of a non-string type for a column in the JSON_TABLE expression.
...
* [An error occurred while creating a partitioned table. Error code: 1499 or ORA-00600](https://www.oceanbase.com/knowledge-base/oceanbase-database-20000000076?back=kb)
...
* Solution:
  * Check that the plug-in type is supported.
  * Check that the plug-in loading statement, plug-in configuration file, and plug-in version are correct.
  * Check that you have sufficient privileges to load and install plug-ins.
...
* Solution: Make sure that the document to be parsed is valid.
...
* Solution: Make sure that the partition name is correct.
...
* MySQL error code: 1588

* Cause: The event is automatically dropped due to timeout immediately after being created.
...
* MySQL error code: 1589

* Cause: You are attempting to change the execution time of the event to a past point in time.

* Solution: Change the execution time of the event to a future point in time so that the event can be normally scheduled.
...
* MySQL error code: 3061

* Cause: The name of the user variable cannot exceed 1,024 characters in length.
