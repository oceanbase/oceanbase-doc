...
The `GET*` member function is used to retrieve the current data value (returning data that matches the type of the calling function).

The type of the current data value depends on the current access mode of `ANYDATA`. This includes whether `PIECEWICE` has been called.
...
If `PIECEWISE` has been called, then `ANYDATA` is accessed in segments. The data value's type should match the type of the current attribute (or collection element).
...
|------|---------------|
| self | An `ANYDATA`. |
| num  | The value to retrieve.       |

## Return value

The return value varies depending on the access mode. In the `PIECEWISE` mode (used for collections), `DBMS_TYPES.NO_DATA` indicates the end of collection access.

## Exceptions

* `DBMS_TYPES.TYPE_MISMATCH`: The expected type does not match the provided type.

* `DBMS_TYPES.INVALID_PARAMETERS`: Invalid parameters (if a value cannot be added at a certain point during the procedure's creation).
