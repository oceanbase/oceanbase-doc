...
The `GET_DDL` function returns the metadata of a single object as a DDL statement. It is used to extract DDL statements for database objects such as `TABLE`, `VIEW`, `INDEX`, `PROCEDURE`, `SEQUENCE`, `TRIGGER`, `SYNONYM`, `FUNCTION`, and `PACKAGE`.

The `GET_xxx` function is used to retrieve the metadata of an object in a single call. Currently, only the `GET_DDL` function is supported in OceanBase Database.
...
The following examples show how to use `GET_DDL` to retrieve DDL statements for different object types. Note that the object names in the examples, such as `'MY_TABLE_NAME'`, need to be replaced with actual object names in your database.
...
- **Retrieve the DDL statement for a materialized view (MATERIALIZED VIEW):**
...
The following two examples show how to use `GET_DDL`.
...
-- Set server output to allow printing messages to the console.
set serveroutput on;
...
`DBMS_METADATA.GET_DDL` returns the DDL statement for creating the object.
...
-- Set server output to allow printing messages to the SQL command-line interface.
set serveroutput on;
