...
| WAIT_CLASS | The event type to be sampled. The value of `WAIT_CLASS` is the same as that of the `GV$ACTIVE_SESSION_HISTORY.WAIT_CLASS` field. If this parameter is not specified or is set to `NULL`, the waiting type is not restricted. |
...
| SVR_PORT | The port number of the node that generates the ASH report. If this parameter is not specified or is set to `NULL`, the port number is not restricted. |
