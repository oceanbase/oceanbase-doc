...
The `CREATE_WRAPPED` procedure encrypts the PL/SQL object definition string into a `CREATE WRAPPED` statement string and executes the statement to create a `WRAPPED` object.
...
|------------------|-----------------------------------------------------|
|CREATE_WRAPPED (DDL VARCHAR2)|<ur><li>DDL: A VARCHAR2 string containing the PL/SQL source code to obfuscate.</li><li>RETURN: The return value, a VARCHAR2 string containing the obfuscated PL/SQL code.|
|CREATE_WRAPPED(DDL DBMS_SQL.VARCHAR2A， LB  PLS_INTEGER, UB  PLS_INTEGER)|<ur><li>DDL: A DBMS_SQL.VARCHAR2A array containing the PL/SQL source code to obfuscate.</li><li>LB: The lower bound index, a PLS_INTEGER.</li><li>UB: The upper bound index, a PLS_INTEGER.|
|CREATE_WRAPPED(DDL DBMS_SQL.VARCHAR2S,LB  PLS_INTEGER,UB  PLS_INTEGER)|<ur><li>DDL: A DBMS_SQL.VARCHAR2S array containing the PL/SQL source code to obfuscate.</li><li>LB: The lower bound index, a PLS_INTEGER.</li><li>UB: The upper bound index, a PLS_INTEGER.|
...
The wrapped code will be an unreadable string, similar to the following (the actual output may vary by version and environment):
