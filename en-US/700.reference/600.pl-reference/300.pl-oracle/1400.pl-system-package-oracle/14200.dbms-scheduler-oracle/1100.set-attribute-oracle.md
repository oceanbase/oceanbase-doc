...
|-----------|--------------------------------------------------|
| name      | The name of the object. <p>The system-defined scheduled tasks are as follows:</p><ul><li>`SCHEDULED_TRIGGER_PARTITION_BALANCE` : partition balance task</li> <li>`DAILY_MAINTENANCE_WINDOW` : window merge task</li></ul>      |
...
| next_date          | The next runtime of the job. |
| max_run_duration   | The maximum duration of the job. |
...
* Set the next scheduled time of a partition balancing task to 01:00 on June 12, 2024.
...
* Set the next scheduled time of a window compaction task to 01:00 on March 10, 2026.
