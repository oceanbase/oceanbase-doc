...
|slug|converttoblob-oracle-mode|
...
| NOT_SUPPORTED     | The `CONVERTTOBLOB` operation is not supported for temporary Delta-type `LOB`s; or the destination `LOB` is read-only and cannot be modified. |
...
* It is not mandatory to wrap `LOB` operations within an Open/Close interface. If the `LOB` is not opened before executing the operation, the function and domain indexes on the `LOB` column will be updated during the call. However, if the `LOB` is opened before executing the operation, it must be closed before the transaction is committed.
