...
The `UTL_SMTP` package enables you to send emails by using the Simple Mail Transfer Protocol (SMTP).

The `UTL_SMTP` package depends on the `UTL_TCP` package. Make sure that the `UTL_TCP` package is effective.
...
|[CLOSE_CONNECTION](300.close-connection-oracle.md)|Closes an SMTP connection, which terminates the session and cancels the email only in the middle of a data session. |
