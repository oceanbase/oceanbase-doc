...
To create a user-defined aggregate function, you need to implement the `ODCIAggregate` interface and define the user-defined aggregate function.
...
This topic uses `SecMax()` as an example to describe how to create a user-defined aggregate function. The `SecMax()` function returns the second largest value in a set of numbers.

1. Implement the `ODCIAggregate` interface

   The `ODCIAggregate` routines are implemented as methods of the `SecMaxImpl` object type.

   ```javascript
   /*Implement the SecMaxImpl type to contain the ODCIAggregate routines*/
...
2. Define the user-defined aggregate function
   ...
   This function definition creates the `SecMax()` aggregate function by specifying its signature and the object type that implements the `ODCIAggregate` interface:
