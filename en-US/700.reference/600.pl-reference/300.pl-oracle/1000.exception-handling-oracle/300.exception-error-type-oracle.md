...
* Declare them using `PRAGMA EXCEPTION_INIT` and then capture their names in `EXCEPTION HANDLE`.

* Use `EXCEPTION HANDLE` with `OTHERS` to capture them, and retrieve details using `SQLCODE` and `SQLERRM`.
