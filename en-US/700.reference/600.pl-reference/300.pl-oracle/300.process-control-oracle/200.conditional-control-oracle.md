...
Conditional control statements execute or ignore a set of program code based on different conditions. They can be implemented using `IF` or `CASE`.
...
Boolean variables can store `TRUE`, `FALSE`, or `NULL`, and can be directly used for conditional judgments. Therefore, avoid the following example:
...
If `condition_1` is `TRUE`, the `statements_1` are executed, and the program ends; otherwise, if `condition_2` is `TRUE`, the `statements_2` are executed, and the program ends; and so on, until all `condition_n` are not `TRUE`, in which case the `else_statements` are executed, and the program ends.
...
In the above example, `selector` is an expression, and each `selector_value` can be a literal value or an expression. The simple `CASE` statement compares the value of `selector` with each `selector_value` in sequence. If a match is found, the comparison stops. If no match is found, the `CASE` statement executes the `else_statements`. If `else_statements` does not exist, an error is reported.
...
In a searched `CASE` statement, there is no expression to evaluate after `CASE`, but different boolean expressions are specified after `WHEN`. The searched `CASE` statement evaluates multiple boolean expressions and selects the first one that is `TRUE`.
...
The searched `CASE` statement executes the first `statement` where `condition` is `TRUE` and does not execute any other `statements`. If none of the `condition` are `TRUE`, the `else_statements` are executed. If `else_statements` does not exist, an error is reported.

Here is an example of a searched `CASE` statement corresponding to a simple `CASE` statement:
