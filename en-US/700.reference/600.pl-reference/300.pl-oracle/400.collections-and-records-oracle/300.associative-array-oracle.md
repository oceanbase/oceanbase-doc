...
The data type of the key can be `PLS_INTEGER` or `VARCHAR2`. The following table describes the two data types.

* If the key is of the `PLS_INTEGER` type, the associative array is indexed by integer and is dense (elements are contiguous). In this case, all elements from the first to the last are defined and have values (which can be `NULL`).

* If the key is of the `VARCHAR2` type, the associative array is indexed by string and is sparse (elements may have gaps). In this case, the elements may have gaps.

  <main id="notice" type='explain'>
    <h4>Note</h4>
    <p>When traversing a dense associative array, you do not need to consider gaps between elements. When traversing a sparse associative array, you need to be aware of potential gaps. </p>
  </main>
...
In particular, associative arrays can be used for temporary data storage. They do not require the same amount of space or network resources as tables. However, since associative arrays are intended for temporary storage, you cannot perform DML operations on them.
...
```javascript
obclient> DECLARE
    TYPE TYPE_SALARY IS TABLE OF NUMBER   -- Declare the type of the associative array.
          INDEX BY VARCHAR2(64);           -- Declare the type of the index as a string.
    salary TYPE_SALARY;             -- Declare the variable of the associative array.
...
The most efficient way to fill a dense associative array is to use the `SELECT [FETCH] BULK COLLECT INTO` clause.

  <main id="notice" type='explain'>
    <h4>Note</h4>
    <p>If you want to fill a dense associative array with a large amount of data and the result set returned by the <code>SELECT</code> statement exceeds the memory limit, you can use a cursor and the <code>FETCH</code> statement in combination with the <code>BULK COLLECT INTO</code> and <code>LIMIT</code> clauses instead of the <code>SELECT</code> statement.</p>
  </main>

You cannot use the `SELECT` statement to fill a sparse associative array. However, you can use an assignment statement within a `FOR LOOP` statement.

The following example shows how to use the `SELECT` statement to fill an associative array indexed by an integer and how to use a `FOR LOOP` statement to fill a sparse associative array indexed by a string.
...
The following example uses the `FOR LOOP` statement to print the values of each element in the associative array. The upper limit of the `FOR LOOP` statement, `tab_wares.COUNT`, calls the `COUNT` method of the collection to return the number of elements in the array.
