...
If the `n`th element of the collection exists, `EXISTS(n)` returns `TRUE`. Otherwise, it returns `FALSE`. If `n` is out of bounds, `EXISTS` returns `FALSE` without raising the predefined exception `SUBSCRIPT_OUTSIDE_LIMIT`.
