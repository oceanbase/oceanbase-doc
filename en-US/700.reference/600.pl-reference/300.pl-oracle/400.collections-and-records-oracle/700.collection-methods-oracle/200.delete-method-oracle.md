...
|slug|delete-method-oracle-compatible-mode|
...
* The `DELETE` method can remove all elements (including any type) from a collection.
...
* For associative arrays or nested tables (excluding variable-length arrays):
  ...
  * DELETE(m,n): If both m and n exist and m is less than or equal to n, it removes all elements with indices between m and n. Otherwise, the DELETE operation has no effect.
...
2. A sequence of elements is deleted, and one of them is restored.

3. All elements are deleted.

The restored elements occupy the same memory as the deleted ones. The `print_nt` stored procedure outputs the nested table variable after initialization and after each `DELETE` operation.