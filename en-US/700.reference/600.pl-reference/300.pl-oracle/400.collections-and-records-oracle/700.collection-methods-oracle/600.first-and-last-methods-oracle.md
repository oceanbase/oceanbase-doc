...
If the collection has at least one element, `FIRST` and `LAST` return the indexes of the first and last elements, respectively, ignoring deleted elements (even if `DELETE` has reserved placeholders for deleted elements).
...
For associative arrays with an index type of `PLS_INTEGER`, the first and last elements are the ones with the smallest and largest indexes, respectively. For associative arrays with a string index type, the first and last elements are the ones with the lowest and highest key values, respectively.
...
The preceding example shows the `FIRST` and `LAST` values for an associative array with a string index type, then deletes the first and last elements, and shows the `FIRST` and `LAST` values again.

## Use in varrays

For non-empty varrays, `FIRST` always returns `1`. For any varray, `LAST` is always equal to `COUNT`.
...
For nested tables, `LAST` is equal to `COUNT`, except when elements are deleted from the middle, which causes `LAST` to be greater than `COUNT`.
