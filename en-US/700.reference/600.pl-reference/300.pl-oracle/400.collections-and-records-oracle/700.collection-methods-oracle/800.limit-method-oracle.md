...
The `LIMIT` method returns the maximum number of elements that a collection can have. If a collection does not have a maximum number of elements, `LIMIT` returns `NULL`.
...
This example declares an associative array with five elements, a varray with three elements, and a nested table with four elements, and then outputs the values of `LIMIT` and `COUNT`.