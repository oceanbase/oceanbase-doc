...
`LIMIT` returns the maximum number of elements that the collection can hold. If the collection does not have a maximum number of elements, `LIMIT` returns `NULL`.
...
This example creates an associative array with five elements, a varray with three elements, and a nested table with four elements, and then outputs the values of `LIMIT` and `COUNT`.