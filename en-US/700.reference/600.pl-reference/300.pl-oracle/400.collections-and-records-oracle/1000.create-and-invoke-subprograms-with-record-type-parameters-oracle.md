...
|slug|create-and-invoke-subprograms-with-record-type-parameters-oracle-mode|

# Create and invoke subprograms with record-type parameters

Variables of record types can be passed as parameters to subprograms, just like variables of other commonly used data types.
...
**Example: Create and invoke subprograms with record-type parameters.**

In the following example, two variables of record types are defined in two different ways:
...
Then, two methods are used to assign values to the record variables by using the `SELECT INTO` statement. Since the actual structures of the two variables are the same, the two variables can be assigned values: `r_ware1 := r_ware2;`

Finally, the record variables are passed as parameters to a subprogram to print the values of the variables.
