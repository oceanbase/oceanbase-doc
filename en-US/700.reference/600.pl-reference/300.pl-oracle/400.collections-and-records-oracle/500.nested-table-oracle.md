...
A nested table is a nonpersistent table that can be referenced and manipulated in a PL/SQL program like an array.
...
You can define a table type and declare a variable of this type. Then, you can add records to your PL/SQL table and reference them in a manner similar to how you would reference array elements.
...
The following table lists the supported methods for PL/SQL nested tables.

|    **Method**     |                                                                                                     **Description**                                                                                                     |
|---------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| EXISTS(n)     | Returns TRUE if the nth row exists.                                                                                                                                                                                           |
| COUNT         | Returns the number of rows in the PL/SQL table.                                                                                                                                                                                                  |
| FIRST \| LAST | Returns the row numbers of the first and last rows (minimum and maximum). If the PL/SQL table contains no data, it returns an empty value.                                                                                                                                                                        |
...
| DELETE        | <ul><li> DELETE: deletes all records in the table.   </li><li> DELETE(n): deletes the nth record in the table.   </li><li> DELETE(m,n): deletes all records between rows m and n. </li></ul>    |
...
BEGIN
    ...
    -- Use the PRIOR method to print records in reverse order. Note that the COUNT is 4 at this point.
