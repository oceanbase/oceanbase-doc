...
A nested table is a temporary table that can be referenced and used in PL/SQL programs to simulate arrays.
...
The following table lists the methods supported by PL/SQL nested tables.

|    **Method**     |                                                                                                     **Description**                                                                                                     |
|---------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| EXISTS(n)     | Returns TRUE if the nth row exists.                                                                                                                                                                                           |
| COUNT         | Returns the number of rows in the PL/SQL table.                                                                                                                                                                                                  |
| FIRST \| LAST | Returns the row numbers of the first and last rows (minimum and maximum). If the PL/SQL table contains no data, this method returns NULL.                                                                                                                                                                        |
...
BEGIN
    ...
    -- Check for the existence of a record using EXISTS
    ...
    -- Print records from first to last using NEXT
    ...
    -- Print records from last to first using PRIOR, note that COUNT is now 4
...
* An array is always dense. A nested table is initially dense, but it can become sparse if elements are deleted.

## When to use nested tables

Use a nested table in the following situations:

* You do not know the number of elements in advance.
