...
A record type stores logically related data as a unit and must contain at least one scalar or `RECORD` data type member. These members are referred to as fields (`FIELD`) in PL/SQL records. Fields store information that is logically related but distinct.
...
For a record variable declared with `%ROWTYPE` or `%TYPE`, the initial value of each field is `NULL`. This variable does not inherit the initial values of the referenced item.
...
In a PL block, a `RECORD` type is a local type.

In a package header, a `RECORD` type is a public `RECORD` type. You can reference it from outside the package by using `package_name.type_name`. It is stored in the database and is dropped when you execute the `DROP PACKAGE` statement.
...
* If the target variable is explicitly declared as a `RECORD` type and the source variable is declared using `％ROWTYPE` based on the row type of a table, the number and order of fields in both variables must be the same. Additionally, corresponding fields must have the same data types.
...
* Declare a variable, type, and associate it with the type of a cursor. Syntax:
  ...
  <main id="notice" type='explain'>
    <h4>Note</h4>
    <p>You can perform operations on a record variable in the same way as on a regular variable. You can use the <code>SELECT INTO</code> statement to assign a value to a record variable. </p>
  </main>