...
A collection constructor is a system-defined function with the same name as the collection type. It returns a collection of that type.
...
In this syntax, `collection_type` is the name of a previously declared nested table or varray type (not an associative array type), and `value` is a valid value of the `collection_type` collection element. If `collection_type` is a varray type, the number of values cannot exceed the maximum value. If `collection_type` is a nested table type, there is no maximum value.
...
**Example: Specify indexes and elements for an associative array by using a constructor.**
...
In this example, a `record` type is first defined, and then an associative array type `ty_rec` is defined. The `index` is of the `PLS_INTEGER` type, and the `element` is of the `rec_type` type. `v_rec` is an associative array variable of the `ty_rec` type. The constructor with the `=>` operator is used to specify the `index` and its corresponding `element`. The associative array is then enumerated to output all its elements.
Additionally, the constructor can be used without specifying the `index` (i.e., without the `=>` operator), but only for associative arrays where the `index` is of the `PLS_INTEGER` type. In this case, the `index` starts at `1` and increments by `1` by default. Here is an example:
...
In this example, an associative array type `ty1` is declared with an `index` of the `PLS_INTEGER` type and an `element` of the `int` type. Then, a variable `x` of type `ty1` is declared. The `element` values are directly assigned to `x` using the constructor of the `ty1` type. Finally, the `index` and `element` of `x` are traversed and output sequentially.