...
If the function is in the `SYS` schema, you must connect as `SYSDBA`. Otherwise, the function must be in the current user's schema, or you must have the `ALTER ANY PROCEDURE` system privilege.
...
where `function_compile_clause` is described as follows:
