...
If the trigger is in the `SYS` schema, you must connect as `SYSDBA`. Otherwise, the trigger must be in your current schema, or you must have the `ALTER ANY TRIGGER` system privilege.
...
The `SYS` schema creates a trigger named `del_new_region` on the `regions` table. Every time a `DELETE` operation is performed on the `regions` table, the trigger is triggered and moves records with a `region_id` greater than 3 to the `reg_his` table. After creating this trigger, the database automatically enables it. You can then use the following statement to disable the trigger:
