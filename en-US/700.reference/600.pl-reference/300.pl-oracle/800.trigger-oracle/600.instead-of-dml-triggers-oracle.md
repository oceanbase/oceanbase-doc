...
`INSTEAD OF` DML triggers are created on non-updatable views and can be used to update the view. `INSTEAD OF` triggers can determine the expected operation and execute the appropriate DML operation on the base table.
...
The following example creates a view named `order_list` to display information about customers and their orders. Because the primary key `order_id` of the orders table is not unique in the result set of the joined view, the view is inherently not updatable. This example creates an `INSTEAD OF` trigger to handle `INSERT` statements targeting the view. The trigger inserts row data into the base tables `customers` and `orders` of the view.
...
/* Query and display the rows to be inserted */
...
/* Query and display the row inserted into the view */
obclient> SELECT COUNT(*) FROM order_list WHERE customer_id = 111;
+----------+
| COUNT(*) |
+----------+
...
/* Query and display the row inserted into the customers table */
...
/* Query and display the row inserted into the orders table */
