...
|slug|instead-of-dml-triggers-oracle-compatible-mode|

# INSTEAD OF DML triggers

`INSTEAD OF` DML triggers are created on non-updatable views and can be used to update the view. `INSTEAD OF` triggers can determine the intended operation and perform the appropriate DML operation on the underlying table.

`INSTEAD OF` triggers can only be row-level triggers. `INSTEAD OF` triggers can read but cannot modify `OLD` and `NEW` values.
...
The following example creates a view named `order_list` to display information about customers and their orders. The primary key `order_id` of the orders table is not unique in the result set of the join view, so the view is inherently not updatable. This example creates an `INSTEAD OF` trigger to handle `INSERT` statements targeting the view. The trigger inserts row data into the base tables `customers` and `orders` of the view.
