...
The `SA_COMPONENTS.ALTER_LEVEL` procedure is used to change the short name and long name of a level.
...
|----------------|---------------------------------------------------------|
| policy_name    | Specifies the name of the policy. To find existing policies, query the `POLICY_NAME` column of the `ALL_SA_POLICIES` view.  |
| level_num      | Specifies the number of the level to be changed. To find existing levels, query the `LEVEL_NUM` column of the `ALL_SA_LEVELS` view. |
