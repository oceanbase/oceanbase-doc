...
Expressions can be classified into the following types based on their complexity:
...
An operand can be a variable, constant, literal, operator, function call, placeholder, or another expression. The data type of the operand determines the data type of the expression. Each time the expression is evaluated, it produces a single value of that data type. The result of the expression has the same data type as the expression.
...
An operation can be a unary operator with a single operand or a binary operator with two operands. Expressions are evaluated based on operator precedence.

Do not embed any other characters (including spaces) in operators.
...
You can use parentheses to control the order of operations in an expression. Here is an example:
...
* The `NOT` operator returns the opposite value of its operand, unless the operand is `NULL`. `NOT NULL` returns `NULL` because `NULL` is an unknown value.

## Short-circuit evaluation

When evaluating logical expressions, PL uses short-circuit evaluation, which means that PL can stop evaluating an expression immediately after determining the result.
...
A comparison operator compares one expression with another. The result is always `TRUE`, `FALSE`, or `NULL`.

If the value of one of the expressions is `NULL`, the comparison result is also `NULL`.
...
The `IS NULL` operator returns the `BOOLEAN` value `TRUE` if its operand is `NULL`; otherwise, it returns `FALSE`. The `IS NOT NULL` operator has the opposite effect. Comparisons involving `NULL` values always return `NULL`.
...
The pattern can include two wildcard characters: the underscore (_) and the percent sign (%). The underscore matches exactly one character, while the percent sign matches zero or more characters.
...
The `IN` operator tests for membership in a set. The `x IN(set)` expression returns `TRUE` only if `x` is equal to a member of the set.
...
Typically, `BOOLEAN` expressions are used as conditions in the `WHERE` clause of control statements and DML statements.
...
In a search `CASE` statement, there is no expression to evaluate after `CASE`, but different `BOOLEAN` expressions are specified after `WHEN`. A search `CASE` evaluates multiple `BOOLEAN` expressions and selects the first one that returns `TRUE`.
...
A search `CASE` statement evaluates the first `boolean_expression` that returns `TRUE` and does not evaluate the other `boolean_expression` statements. If none of the `boolean_expression` statements return `TRUE`, it returns `else_result` (if it exists). If `else_result` does not exist, it returns `NULL`.

The following example shows a search `CASE` expression corresponding to the simple `CASE` expression:
...
All SQL functions can be used in PL expressions, except for the following:
...
* SQL sorting operators and functions `COLLATE`, `COLLATION`, `NLS_COLLATION_ID`, and `NLS_COLLATION_NAME`

* Other functions `CUBE_TABLE`, `DATAOBJ_TO_PARTITION`, `LNNVL`, `NVL2`, `SYS_CONNECT_BY_PATH`, `SYS_TYPEID`, and `WIDTH_BUCKET`
...
A static expression is one whose value can be determined at compile time. In other words, it does not contain character comparisons, variables, or function calls. Static expressions are the only expressions that can appear in conditional compilation directives.

The following rules define a static expression:
...
* An expression that is an operator supported by static expressions is static if all of its operands are static and the operator does not raise an exception when evaluated against those operands.
...
* The length of a character type (`VARCHAR2`, `NCHAR`, `CHAR`, `NVARCHAR2`, `RAW`, and `ANSI`)

* The range and precision of the `NUMBER` type and its subtypes (such as `FLOAT`)

* The precision of interval types (years, months, and seconds)

* The precision of time and timestamp types

* The bounds of a `VARRAY`

* The range bounds in a type declaration

In each case, the result type of the static expression must match the subtype of the declared item, and it must be in the correct context.
...
* Character literals, up to 32767 bytes in length.
...
In this syntax, the type of `static_expression` must match the type of `data_type` (`BOOLEAN` or `PLS_INTEGER`).

Even in the body of the `package_name` package, you must always reference static constants as `package_name.constant_name`.

If you use `constant_name` in a conditional compilation directive of a PL unit with a `BOOLEAN` expression, the PL unit depends on the `package_name` package. If the package header is changed, the dependent PL units may become invalid and will need to be recompiled.

If you use a package with static constants to control conditional compilation in multiple PL units, we recommend that you create only the package header and use it exclusively for controlling conditional compilation. This approach avoids the risk of the package becoming invalid due to changes to the package header.