...
The following table lists the operators from the highest to the lowest precedence. Operators with the same precedence do not have a specified evaluation order.
...
The logical operators `AND`, `OR`, and `NOT` follow three-state logic. `AND` and `OR` are binary operators, and `NOT` is a unary operator.
...
The logical operators return the following values:
...
When evaluating logical expressions, PL uses short-circuit evaluation. That is, PL can stop evaluating an expression immediately after determining the result.
...
The `IS NULL` operator returns the `BOOLEAN` value `TRUE` if the operand is `NULL`; otherwise, it returns `FALSE`. The `IS NOT NULL` operator works the opposite way. Comparisons involving `NULL` values always return `NULL`.
...
* `BOOLEAN` comparisons
  ...
  By definition, `TRUE` is greater than `FALSE`. Any comparison with `NULL` will return `NULL`.
...
The `BETWEEN` operator is used to test whether a value falls within a specified range.

The expression `c BETWEEN a AND b` has the same value as the expression `(c> = a)AND(c<= b)`. The value of `x` is calculated only once.
...
The `IN` operator is used to test membership in a set. The `x IN(set)` returns `TRUE` only if `x` is equal to a member of the `set`.
...
A simple `CASE` expression returns the first `result` that matches `selector_value` with `selector`. The remaining expressions are not evaluated. If no `selector_value` matches `selector`, the `CASE` expression returns `else_result` (if specified), otherwise `NULL`.
...
A search `CASE` statement executes the first `boolean_expression` that returns `TRUE` and does not execute the other `boolean_expression` statements. If none of the `boolean_expression` statements return `TRUE`, it returns `else_result` (if specified). If `else_result` is not specified, it returns `NULL`.
...
A static expression is an expression whose value can be determined at compile time. In other words, it does not contain character comparisons, variables, or function calls. Static expressions are the only type of expression that can appear in conditional compilation directives.
...
* An expression that uses a static expression-supported operator, provided that all of its operands are static and the operator does not raise an exception when evaluating these operands.
...
In each case, the result type of the static expression must match the subtype of the declared item and must be within the correct scope of the context.

**PLS_INTEGER static expressions**

A `PLS_INTEGER` static expression is:

* A `PLS_INTEGER` literal

* A `PLS_INTEGER` static constant

* `NULL`

**BOOLEAN static expressions**

A `BOOLEAN` static expression is:

* A `BOOLEAN` literal (`TRUE`, `FALSE`, or `NULL`)

* A `BOOLEAN` static constant

* An expression in which `x` and `y` are `PLS_INTEGER` static expressions:
...
* An expression in which `x` and `y` are Boolean expressions:
...
* An expression in which `x` is a static expression:
...
A `VARCHAR2` static expression is:

* A string literal of up to 32,767 bytes.
...
If a conditional compilation directive in a PL unit uses `constant_name` in a `BOOLEAN` expression, the PL unit depends on the `package_name` package. If the package header changes, the dependent PL units may become invalid and need to be recompiled.

If you use a package with static constants to control conditional compilation in multiple PL units, we recommend that you create only the package header and use it specifically for controlling conditional compilation to avoid invalidation of the package due to changes in the package header.