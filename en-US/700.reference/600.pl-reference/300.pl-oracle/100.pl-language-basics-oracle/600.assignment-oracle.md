...
After declaring a variable, you can assign a value to it in various ways.
...
After declaring a variable, you can use an assignment statement to assign an expression to the variable.

In PL/SQL, use the `:=` operator to perform an assignment. The syntax is as follows:
...
You can use the `SELECT INTO` statement to directly assign a value to a variable. Each time the `SELECT INTO` statement is executed, the value is assigned to the variable. The variable type must be compatible with the column type in the `SELECT` clause.
...
For each `select_item`, you must specify a `variable_name` that is compatible with the type of the `select_item`. Because SQL does not have a `BOOLEAN` type, the `variable_name` cannot be a `BOOLEAN` variable.
...
## Assign a value to a variable passed as a parameter to a subprogram
...
Example: Assign a value to a variable passed as a parameter to a subprogram.
