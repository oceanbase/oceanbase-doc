...
Like a cursor, a cursor variable is a pointer to the current row in a multi-row query result set.
...
However, unlike a cursor, a cursor variable is dynamic, while a cursor is static. A cursor can only be associated with a specific query, meaning it always points to the memory processing area of that query. In contrast, a cursor variable can be associated with different query statements, pointing to their respective memory processing areas (but not multiple at the same time; it can only be associated with one query statement at a time), provided the return types of these queries are compatible.

## Declare a cursor variable

A cursor variable is a pointer of type `REF CURSOR`. Therefore, you must first define the cursor variable type before you can declare a cursor variable. In PL/SQL, you can declare a cursor variable reference type in the declaration section of a block, subprogram, or package.
...
Here, `ref_type_name` specifies the name of the new cursor variable type, and `retuen_type` specifies the return value type of the cursor variable, which must be a record variable.

When you define a cursor variable type, you can use either strong typing or weak typing. Strong typing requires you to specify the return value type of the cursor variable, whereas weak typing does not require you to specify the return value type.
...
A weakly typed cursor does not have a return value and is a general type. You can directly define a weakly typed cursor by using the system-defined type `SYS_REFCURSOR`. Therefore, the preceding example can be rewritten as follows:
...
```
  ...
  <main id="notice" type='notice'>
    <h4>Notice</h4>
    <ul>
    <li>For a weakly typed cursor variable, the PL engine does not know the type of the variable during compilation. Therefore, the variable must be bound during execution. This means that the program consumes more resources and time during execution.</li>
    <li><code>SYS_REFCURSOR</code> cursor variables can be reused.</li>
    </ul>
  </main>
...
2. In the execution section, perform the following steps:
   ...
   <li> Retrieve row data from the cursor recordset. Each row can be retrieved using the LOOP statement in the following format:
