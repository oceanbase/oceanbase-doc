...
However, unlike cursors, cursor variables are dynamic, while cursors are static. A cursor can only be associated with a specific query, meaning it is fixed to a single query's memory processing area. In contrast, a cursor variable can be associated with different query statements, allowing it to point to the memory processing areas of various queries (but not multiple areas at the same time; it can only be connected to one query statement at a time), as long as the return types of these queries are compatible.

## Declare a cursor variable

A cursor variable is a pointer of type `REF CURSOR`. Therefore, you must define the type of the cursor variable before you declare it. In PL/SQL, you can declare a cursor variable reference type in the declaration section of a block, subprogram, or package.
...
In this syntax, `ref_type_name` is the name of the new cursor variable type, and `retuen_type` is the return value type of the cursor variable, which must be a record variable.

When you define a cursor variable type, you can use strong typing or weak typing. In strong typing, you must specify the return value type of the cursor variable. In weak typing, you do not need to specify the return value type.
...
A weakly typed cursor does not return a value and is a generic type. You can directly define a weakly typed cursor by using the system-defined type `SYS_REFCURSOR`. Therefore, the preceding example can be rewritten as follows:
...
```
  ...
  <main id="notice" type='notice'>
    <h4>Notice</h4>
    <ul>
    <li>For a weakly typed cursor variable, the PL/SQL engine does not know the type of the variable during compilation. Therefore, the variable must be bound during execution. This means that the program consumes more resources and time during execution.</li>
...
## Loop through a recordset by using a cursor variable

To loop through a recordset by using a cursor variable, perform the following steps:
...
2. In the execution section, perform the following steps:
   ...
   <li> Retrieve row data from the cursor recordset. Each row is retrieved in a loop. The following statement can be used with a LOOP statement to retrieve multiple rows of data.
...
**Example: Loop through a recordset by using a cursor variable.**
