...
However, unlike cursors, cursor variables are dynamic, while cursors are static. A cursor can only be associated with a specific query, meaning it is fixed to the memory processing area of that query. In contrast, a cursor variable can be associated with different query statements, pointing to the memory processing areas of these queries (but not multiple areas at the same time; it can only be associated with one query statement at a time), as long as the return types of these queries are compatible.

## Declare a cursor variable

A cursor variable is a pointer of the `REF CURSOR` type. Therefore, you must define the type of a cursor variable before you declare it. In PL/SQL, you can declare a cursor variable reference type in the declaration section of a block, subprogram, or package.
...
Here, `ref_type_name` specifies the name of the newly defined cursor variable type, and `retuen_type` specifies the return value type of the cursor variable, which must be a record variable.

You can define a cursor variable type in a strong type or weak type. In a strong type, you must specify the return value type of the cursor variable. In a weak type, you do not need to specify the return value type.
...
obclient> DECLARE
       ...
       -- Here, emp_job_refcur_type does not have a return type.
...
A weak-type cursor does not return a value. It is a general-purpose type. You can directly define it by using the system-defined type `SYS_REFCURSOR`. Therefore, you can rewrite the preceding example as follows:
...
```
  ...
  <main id="notice" type='notice'>
    <h4>Notice</h4>
    <ul>
    <li>For a weak-type cursor variable, the PL engine does not know the type of the variable during compilation. Therefore, you must bind the variable during execution. This means that the program consumes more resources and time during execution.</li>
    <li>Cursor variables of the <code>SYS_REFCURSOR</code> type support reuse.</li>
    </ul>
  </main>

## Define a cursor variable by using dynamic SQL

Dynamic SQL is a high-level programming method that generates and executes SQL statements during the runtime of a PL/SQL program. A cursor variable also supports dynamic binding. You can use the `OPEN... FOR` clause to define a cursor variable by using a string. In the following example, the cursor `cv` is defined by using the string variable `query_2`.
...
## Use a cursor variable to loop through each record in a recordset

To use a cursor variable to loop through each record in a recordset, perform the following steps:

1. In the declaration section, perform the following steps:

   <ol><li> Define a dynamic cursor type and execute the statement:

   ```sql
   TYPE cursor_type IS REF CURSOR [ RETURN return_type ];
   ```

   <li> Declare a variable of the dynamic cursor type and execute the statement:

   ```sql
   cursor_variable cursor_type;
   ```

   <li> Declare a record to store the records returned by the cursor and execute the statement:
...
2. In the execution section, perform the following steps:
   ...
   <ol><li> Open the cursor variable bound to a specific query and execute the statement:
   ...
   ```
    ...
    <li> Close the cursor and execute the statement:
...
**Example: Use a cursor variable to loop through each record in a recordset.**
