...
|slug|show-cursor-oracle-mode|

# Explicit cursor

Explicit cursors are mainly used for processing query statements, especially when the query result contains multiple records.
...
To process an explicit cursor, you need to perform the following four steps:
...
```
  ...
  <main id="notice" type='notice'>
    <h4>Notice</h4>
    <p>When you specify data types, you cannot use length constraints. For example, <code>NUMBER(4)</code> and <code>CHAR(10)</code> are invalid. </p>
  </main>

#### Open the cursor

The `OPEN` statement is used to open the cursor, execute the corresponding `SELECT` statement, store the query result in the workspace, and point the pointer to the beginning of the workspace to mark the cursor result set. If the cursor query statement contains the `FOR UPDATE` option, the `OPEN` statement will also lock the data rows in the database table corresponding to the cursor result set.
...
After fetching and processing the data from the cursor result set, you should promptly close the cursor to release the system resources it occupies and make the cursor's workspace invalid. At this point, you cannot use the `FETCH` statement to retrieve data from the cursor. A closed cursor can be reopened using the `OPEN` statement.

## Use an explicit cursor to loop through and retrieve records from a result set

The following steps show how to use an explicit cursor to loop through and retrieve records from a result set.

1. Declare a cursor in the declaration section. You can use the following two methods.
...
2. Declare a record in the declaration section to store the row records returned by the cursor.

   ```sql
   record_name cursor_name%ROWTYPE;
   ```

3. Open the cursor in the execution section. You can use the following two methods.
...
4. Fetch row data from the cursor's record set in the execution section. Each time you fetch one row, you can use the following `LOOP` statement to fetch multiple rows.
...
**Example: Use an explicit cursor to loop through and retrieve records from a result set.**
