...
An autonomous routine starts a transaction when it executes its first SQL statement. When a transaction ends, the next SQL statement starts another transaction.
...
When in the main transaction, rolling back to a savepoint marked before the autonomous transaction was started does not roll back the autonomous transaction. Remember that autonomous transactions are completely independent of the main transaction.

## Avoid errors in autonomous transactions

To avoid some common errors, keep the following in mind:
