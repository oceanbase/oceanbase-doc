...
|slug|create-package-oracle-usage|

# Create a package

A package consists of a package specification (`PACKAGE SPECIFICATION`) and a package body (`PACKAGE BODY`). The definition of a package is divided into the package specification and the package body.
...
The package specification (`PACKAGE SPECIFICATION`) declares the data types, variables, constants, cursors, subprograms, and exception handling in the package. These elements are public elements of the package. The package body (`PACKAGE BODY`) is the specific implementation of the package specification. It defines the cursors and subprograms declared in the package specification and can also declare private elements of the package.

The package specification and the package body are compiled separately and stored as two separate objects in the database dictionary views `USER_SOURCE`, `ALL_SOURCE`, and `DBA_SOURCE`.
...
The `AUTHID CURRENT_USER` and `AUTHID DEFINER` options specify the privilege mode used when an application calls a function.
...
The following example creates a package named `obdemo_pack`. The package contains a record variable `obDeptRec`, two functions, and a stored procedure.
...
The following example creates a package body based on the declared package specification:
...
obclient> CREATE OR REPLACE PACKAGE BODY obdemo_pack
    ...
    EXCEPTION
           WHEN NO_DATA_FOUND THEN
              DBMS_OUTPUT.PUT_LINE('No department with the code '||dept_no||' is found in the database.');
...
To call variables, constants, functions, and methods in a package, you must include the package name in the object name and separate it from the object name with a period (.). That is, the calling format for a public element of a package is: **package name.element name**.

The following example calls the functions in the `obdemo_pack` package to insert, query, and modify data in the `obdept` table. It also displays the queried database information by using the record variable `obDeptRec` in the `obdemo_pack` package.
