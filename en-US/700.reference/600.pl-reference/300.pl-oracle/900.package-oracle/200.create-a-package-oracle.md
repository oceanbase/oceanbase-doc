...
To call variables, constants, functions, and methods in a package, you must specify the package name in the object name, and separate the package name and object name with a period (.). That is, the syntax for calling a public element in a package is: **package_name.element_name**.

The following example calls functions in the `obdemo_pack` package to insert, query, and modify data in the `obdept` table, and displays the queried database information by using the record variable `obDeptRec` in the `obdemo_pack` package:
...
obclient> DECLARE
     ...
     BEGIN
         ...
         ELSE
             DBMS_OUTPUT.PUT_LINE('The record is added!');
             ...
             ELSE
                 DBMS_OUTPUT.PUT_LINE('The record is deleted!');
             END IF;
         END IF;
     END;
     /
