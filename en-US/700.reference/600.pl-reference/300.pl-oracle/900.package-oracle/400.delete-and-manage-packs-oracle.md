...
You can use the `DROP PACKAGE` statement to delete an unnecessary package. The syntax is as follows:
...
You can query the following views to view package information:

* `USER_OBJECTS`: View database objects. You can use this view to list the names, current status, and creation time of packages.
...
* `USER_SOURCE`: View the content of a package and a package body.

* `USER_ERRORS`: View the error messages that occurred during the last compilation of a package or a package body.
...
**Example 3: View the error messages and location of the last compilation error of the package** **`PKG`** **.**
