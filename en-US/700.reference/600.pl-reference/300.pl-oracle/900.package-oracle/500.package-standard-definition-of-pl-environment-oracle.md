...
# Package STANDARD defines the PL environment

The `STANDARD` package defines the PL environment. The package header declares public types, variables, exceptions, and subprograms available for use in PL programs.
...
The contents of the system package `STANDARD` are directly visible to applications. Users can directly call them from database triggers, stored subprograms, or OceanBase Database tools without prefixing the package name.
...
The following data types are generic and are used exclusively by the `STANDARD` package and some other packages. Note that generic data types are not part of the language.
...
The PL compiler uses the following generic indexed table data type to instantiate array attributes.
