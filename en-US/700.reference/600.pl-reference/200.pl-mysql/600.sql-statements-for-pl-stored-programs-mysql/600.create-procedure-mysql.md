...
To execute the `CREATE PROCEDURE` statement, you must have the `CREATE PROCEDURE` privilege.
...
|------|------|
| DEFINER | The user who created the stored procedure. If this parameter is omitted, the default value is the user who executed the `CREATE PROCEDURE` statement. You can specify this parameter as `'user_name'@'host_name'` or `CURRENT_USER`. |
| IF NOT EXISTS | If a stored procedure with the specified name already exists, no error is returned. |
| sp_name | The name of the stored procedure to be created. You can specify the database name by using `database_name.sp_name`. |
| proc_parameter | The list of parameters of the stored procedure. The format of a parameter is `[ IN \| OUT \| INOUT ] param_name type`. By default, each parameter is an `IN` parameter. |
| characteristic | The characteristics of the stored procedure, which can be specified as follows:<br>- `COMMENT 'string'`: the comment of the stored procedure<br>- `LANGUAGE SQL`: the language of the stored procedure (SQL only; this parameter can be omitted)<br>- `[NOT] DETERMINISTIC`: whether the stored procedure is deterministic<br>- `CONTAINS SQL \| NO SQL \| READS SQL DATA \| MODIFIES SQL DATA`: the data access characteristics of the stored procedure<br>- `SQL SECURITY { DEFINER \| INVOKER }`: the execution privilege mode |
| routine_body | The body of the stored procedure, which contains the implementation logic of the stored procedure. |
