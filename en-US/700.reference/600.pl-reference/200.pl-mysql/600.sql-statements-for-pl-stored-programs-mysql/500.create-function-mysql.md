...
* Scalar values can only be specified. For example, the `SET val=(SELECT 1, 2)` statement is invalid.
...
* For character data types, if the declaration includes `CHARACTER SET`, the specified character set and its default collation are used. If the `COLLATE` attribute is used, the specified collation is used instead of the default collation.
  ...
  If neither `CHARACTER SET` nor `COLLATE` is specified, the database character set and collation applied at the time of routine creation are used. To prevent the server from using the database character set and collation, explicitly specify the `CHARACTER SET` and `COLLATE` characteristics for the parameter.

  If the database default character set or collation is changed, the stored routine must be dropped and recreated to apply the new database defaults.
...
For detailed examples of creating functions, see [Stored functions](../200.storage-object-mysql/400.pl-storage-function-mysql.md).