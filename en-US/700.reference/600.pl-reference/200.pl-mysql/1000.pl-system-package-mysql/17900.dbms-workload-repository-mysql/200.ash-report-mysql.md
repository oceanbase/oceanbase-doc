...
| SQL_ID | The `SQL_ID` of the SQL statement to be sampled. If this parameter is not specified or set to `NULL`, no restriction is imposed on `SQL_ID`. |
| TRACE_ID | The `TRACE_ID` of the SQL statement to be sampled. If this parameter is not specified or set to `NULL`, no restriction is imposed on `TRACE_ID`. |
| WAIT_CLASS | The type of the event to be sampled. The value of `WAIT_CLASS` is referenced from the `GV$ACTIVE_SESSION_HISTORY.WAIT_CLASS` field. If this parameter is not specified or set to `NULL`, no restriction is imposed on the wait type. |
| REPORT_TYPE | The report type. The supported values are `HTML` and `TEXT`. If this parameter is not specified, the default value is `TEXT`. |
| SVR_IP | The IP address of the node that generates the ASH report. If this parameter is not specified or set to `NULL`, no restriction is imposed on the IP address. |
| SVR_PORT | The port of the node that generates the ASH report. If this parameter is not specified or set to `NULL`, no restriction is imposed on the port. |
| TENANT_ID | The ID of the tenant that generates the ASH report. If this parameter is not specified or set to `NULL`, no restriction is imposed on `TENANT_ID`. |
