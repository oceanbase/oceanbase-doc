...
The `REBUILD_INDEX` procedure is used to fully refresh the vector index.
...
| idx_parameters | The index parameters. The string of vector index parameters used when the table is created, for example, `distance=l2, type=hnsw, lib=vsag`. For more information, see [Create a vector index](../../../../../640.ob-vector-search/200.ob-vector-index.md). |
| idx_parallel_creation | The parallelism for parallel index construction. |
