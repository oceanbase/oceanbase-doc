| Description   |                 |
|---------------|-----------------|
| keywords      |                 |
| dir-name      |                 |
| dir-name-en   |                 |
| tenant-type   | MySQL Mode      |
|machine-translation| en          |
|slug|add_trusted_certificate-mysql-compatible-mode|

# ADD_TRUSTED_CERTIFICATE

The `ADD_TRUSTED_CERTIFICATE` procedure adds a CA root certificate so that components such as an OBServer cluster or OMS can securely authenticate and connect to the cluster through RPC using digital certificates issued by the certificate.

<main id="notice" >
  <h4>Applicability</h4>
  <p>This topic applies only to OceanBase Database Enterprise Edition. OceanBase Database Community Edition provides only MySQL-compatible mode. </p>
</main>

## Syntax

```sql
PROCEDURE ADD_TRUSTED_CERTIFICATE(
  common_name VARCHAR(256),
  description VARCHAR(256),
  content     LONGTEXT);
```

## Parameters

| **Parameter**     | **Description**                                  |
|------------|----------------------------------------|
| common_name | The unique identifier of the certificate.       |
| description | The description of the certificate's purpose or associated cluster.          |
| content     | The detailed information of the certificate.    |

## Examples

```shell
obclient>CALL DBMS_TRUSTED_CERTIFICATE_MANAGER.ADD_TRUSTED_CERTIFICATE
('MySQL_Server_5.7.2_Auto_Geanrateed_C_Certificate',
'cluster B CA',
'-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----'
);
Query OK, 0 rows affected
```