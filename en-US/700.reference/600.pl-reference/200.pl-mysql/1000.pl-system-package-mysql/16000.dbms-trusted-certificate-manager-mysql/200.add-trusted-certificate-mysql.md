| Description   |                 |
|---------------|-----------------|
| machine-translation ||
| keywords      |                 |
| dir-name      |                 |
| dir-name-en   |                 |
| tenant-type   | MySQL Mode      |

# ADD_TRUSTED_CERTIFICATE

The `ADD_TRUSTED_CERTIFICATE` procedure adds a CA root certificate so that components such as an OBServer cluster or an OMS can securely authenticate and connect to the cluster by using RPC with the digital certificate issued based on the certificate.

<main id="notice" >
    <h4>Applicability</h4>
    <p>This topic applies only to OceanBase Database Enterprise Edition. OceanBase Database Community Edition does not support this feature. </p>
  </main>

## Syntax

```sql
PROCEDURE ADD_TRUSTED_CERTIFICATE(
  common_name VARCHAR(256),
  description VARCHAR(256),
  content     LONGTEXT);
```

## Parameters

| **Parameter**     | **Description**                                  |
|------------|----------------------------------------|
| common_name | The unique identifier of the certificate, used to distinguish between different certificates.       |
| description | The description of the certificate's purpose or associated cluster information.          |
| content     | The detailed information of the certificate.    |

## Examples

```shell
obclient>CALL DBMS_TRUSTED_CERTIFICATE_MANAGER.ADD_TRUSTED_CERTIFICATE
('MySQL_Server_5.7.2_Auto_Geanrateed_C_Certificate',
'cluster B CA',
'-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----'
);
Query OK, 0 rows affected
```