...
```
  ...
  <main id="notice" type='explain'>
    <h4>Note</h4>
    <p>In the Oracle-compatible mode of OceanBase Database, you do not need to specify <code>tabname</code> because indexes are unique in the current user. However, in the MySQL-compatible mode, you must explicitly specify the table name because indexes are not unique in the current user.</p>
  </main>