...
```
  ...
  <main id="notice" type='explain'>
    <h4>Note</h4>
    <p>In Oracle mode of OceanBase Database, you do not need to specify <code>tabname</code>, because the index is unique under the current user. In MySQL mode, however, you must explicitly specify the table name because the index is not unique under the current user.</p>
  </main>
