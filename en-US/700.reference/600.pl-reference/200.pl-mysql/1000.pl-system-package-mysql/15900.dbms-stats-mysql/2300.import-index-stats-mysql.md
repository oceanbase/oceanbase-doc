...
<main id="notice" type='explain'>
    <h4>Note</h4>
    <p>In Oracle-compatible mode of OceanBase Database, you do not need to specify <code>tabname</code> because the index is unique in the current user. However, in MySQL-compatible mode, you must explicitly specify the table name, because the index is not unique in the current user.</p>
</main>