...
The variable declaration must be prior to the cursor or exception handler declaration. Names of local variables are case-insensitive. The allowed characters and referencing rules are the same as those of other identifiers.<!--For more information, see **Schema object names (MySQL-compatible mode)** in the Reference Guide.-->
