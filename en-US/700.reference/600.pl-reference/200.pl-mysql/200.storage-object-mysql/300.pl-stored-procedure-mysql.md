...
You can use the `CREATE PROCEDURE` statement to create a stored procedure. The following table describes the differences between MySQL mode and Oracle mode.

* In MySQL mode, you must place the `DECLARE` module in the `BEGIN ... END;` module. You can define other statements only after you complete all definitions and declarations.

* In MySQL mode, you can define `DETERMINISTIC / LANGUAGE SQL` and other `sp_create_chistic` information to enrich the usage of stored procedures.

* In MySQL mode, stored procedures cannot be overloaded.
