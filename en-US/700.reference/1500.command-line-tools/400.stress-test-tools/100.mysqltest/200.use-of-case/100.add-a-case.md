...
Perform the following steps to add a version-related test case to `test_suite/version`:
  ...
  <main id="notice" type='explain'>
    <h4>Note</h4>
    <p>To test the <code>create table like</code> feature, make sure that the test case name reflects the test point. For example, the name <code>create_table_like_syntax.test</code> indicates a syntax-level test case. </p>
  </main>
...
4. Verify the result file.

   This step can be completed by comparing the execution results of an OceanBase database and those of a MySQL database.
