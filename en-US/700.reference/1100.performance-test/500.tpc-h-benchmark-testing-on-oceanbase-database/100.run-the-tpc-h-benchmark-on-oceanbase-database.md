...
* If the system tenant's password has been changed through the terminal and is no longer the default empty value, you need to first reset the password to its default value in the terminal. Then, use the [obd cluster edit-config](https://en.oceanbase.com/docs/community-obd-en-10000000002136397) command to set the system tenant's password in the configuration file. The parameter is `# root_password: # root user password`. After executing the `obd cluster edit-config` command, you also need to run the `obd cluster reload` command to apply the changes.
