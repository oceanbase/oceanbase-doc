...
# Analyze performance issues in OceanBase Sysbench tests

This topic analyzes performance issues encountered by OceanBase Database in Sysbench tests. It offers solutions through bottleneck identification, configuration optimization, and performance tuning to improve the stability and efficiency of the database in high-concurrency scenarios. It also provides reference examples of performance issues to help users resolve potential performance issues.

## Quick troubleshooting of Sysbench issues

If Sysbench experiences a low and fluctuating TPS, a sudden drop to 0 TPS, or an unexpected termination, you should prioritize using the quick troubleshooting methods to identify the cause.

### Phenomenon 1: Sysbench TPS is lower than expected

To troubleshoot this issue, follow these steps:

1. Analyze the Sysbench parameters.

    Prioritarily, analyze the parameters in the Sysbench running command.

    * If `--rand-type=special` is set or the `--rand-type` parameter is not set, set it to `--rand-type=uniform` to reduce lock competition between data.
    * If the number or size of tables is small, increase the size of the dataset: for example, `tables=100,table_size=1000000`.
    * If the scenario is `read_write` or `write_only`, check whether there is a deadlock to verify the setup:
...
### Phenomenon 3: Sysbench exits unexpectedly or returns errors
...
* Solution: Increase the timeout period (in μs):
...
1. Increase the timeout period of the tenant.
...
2. Set the `Tables` parameter to 1 to quickly reproduce the issue. Other parameters remain unchanged.
...
3. The TPS drops to 0, but it occasionally exceeds 0. 
...
4. Query the deadlock view for more information.
