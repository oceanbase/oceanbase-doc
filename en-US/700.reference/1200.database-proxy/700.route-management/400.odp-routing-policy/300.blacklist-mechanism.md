...
3. To modify a parameter, use the`ALTER proxyconfig SET key=value` statement. The modification takes effect immediately.
   ...
   | min_congested_connect_timeout | The timeout period of client connections. When a connection times out, the OBServer node is added to the alive-but-unavailable blocklist. Default value: 10. Unit: s.  |
