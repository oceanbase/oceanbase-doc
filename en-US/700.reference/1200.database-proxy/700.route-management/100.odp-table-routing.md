...
ODP saves the mappings between partitions and IP addresses of OBServer nodes. Given a partition, ODP obtains the corresponding table name from the SQL parser. For a non-partitioned table, ODP obtains the partition information of the table from the location cache based on the table name. For a partitioned table, in addition to the corresponding table name, ODP also obtains the partitioning key value if it is provided in the SQL statement, and then obtains partition metadata from the location cache based on the table name. The obtained partitioning key value and metadata are used to calculate the partition ID for obtaining the IP address of the OBServer node to which the partition is mapped.
