...
* `_resource_limit_max_session_num`
  ...
  Maximum number of concurrent connections allowed for a user tenant = MAX(100, Actual memory of a user tenant × 5%/(100 KB))

  The actual memory for a user tenant is calculated as follows:
  
  * Actual memory of a user tenant: The memory size of a tenant resource unit (`memory_size`) allocated to the user tenant minus the memory size of the corresponding meta tenant. For more information about memory allocation for user tenants and their corresponding meta tenants, see [Memory Management Within a Tenant](../200.system-management/700.memory-management/500.memory-management-within-a-tenant.md).

  * `100`: The minimum number of connections.
  
  * `100 KB`: The memory that may be occupied by a single session, calculated based on experience.
  
  According to this algorithm, if the memory size (`memory_size`) of the user tenant is 3 GB and the memory size of the corresponding meta tenant is 1 GB, the actual memory is `3 GB - 1 GB = 2 GB`. The maximum number of connections for the user tenant is `MAX(100, 2 GB × 5%/(100 KB)) = 1000`.
