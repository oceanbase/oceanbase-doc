...
* `_resource_limit_max_session_num`
  ...
  Maximum number of concurrent connections allowed for a user tenant = MAX(100, Actual tenant memory × 5%/(100 KB))

  Where:

  * Actual tenant memory: the memory in the resource units of the resource pool allocated to the tenant when it was created (`memory_size`) minus the memory of the corresponding Meta tenant. For more information on how memory is allocated to user and Meta tenants, see [Memory management within a tenant](../200.system-management/700.memory-management/500.memory-management-within-a-tenant.md).

  * `100`: the smallest number of connections allowed.

  * `100 KB`: the size of memory that may be occupied by a single session and is calculated based on experience.

  Based on this algorithm, if the memory size (`memory_size`) of the user tenant is 3 GB and that of the Meta tenant is 1 GB, the maximum number of connections for the user tenant is `MAX(100, (3 GB - 1 GB) × 5%/(100 KB)) = 1000`.
...
* You can use the `max_connections` or `max_user_connections` system variable to adjust the maximum number of connections only for a MySQL-compatible tenant or a single user in a MySQL-compatible tenant.

* The `_resource_limit_max_session_num` parameter does not take effect on the `sys` tenant and the administrator user of a user tenant. This is to avoid login failures of the administrators due to a large number of persistent connections. The default administrator user is `root` in MySQL-compatible mode and `SYS` in Oracle-compatible mode.

## View the maximum number of connections for a MySQL-compatible tenant

1. Log in to a MySQL-compatible tenant of a cluster as the tenant administrator.
...
## View the maximum number of concurrent connections of an Oracle-compatible tenant from the tenant

1. Log in to an Oracle-compatible tenant of a cluster as the tenant administrator.
