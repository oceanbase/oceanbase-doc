...
In Oracle mode, if the keyword of an index is not specified, GLOBAL is used as the keyword by default. In this case, the index is created as a global index, and the index table has only one partition. Sample code:
