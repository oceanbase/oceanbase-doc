...
| --- | --- | --- | --- |
| rpc packet in | The cumulative number of remote procedure call (RPC) packets sent to the current OBServer node. | 10000 | 1 |
...
| request dequeue count | The cumulative number of requests that successfully pop out of the tenant waiting queue. | 20001 | 2 |
| request queue time | The cumulative request queuing time. | 20002 | 2 |
| trans commit log sync time | The total duration for synchronizing commit logs (clogs) to the majority of replicas. | 30000 | 4 |
...
| sql local count | The number of times that local statements are executed. | 40010 | 8 |
| sql remote count | The number of times that remote statements are executed. | 40011 | 8 |
| sql distributed count | The number of times that distributed statements are executed. | 40012 | 8 |
...
| memstore write lock succ count | The number of row locks successfully added to the MemStore. | 60021 | 32 |
| memstore write lock fail count | The number of row locks failed to be added to the MemStore. | 60022 | 32 |
...
| DB time                                |  The total active time of the database, which is an accumulative value of the CPU time and non-idle wait time. |  200001 |  3072 |
| DB CPU                                 |  The CPU time consumed by the database in the active state. | 200002 | 3072 |
| background elapsed time                | The total time spent on background process execution. | 200005 | 3072 |
| background cpu time                    |  The CPU time consumed by background processes. | 200006 | 3072 |
| non idle wait time                     | The non-idle wait time, namely, the total time elapsed when the session is in the wait but not idle state. | 200010 | 3072 |
| idle wait time                         |  The idle wait time, namely, the total time elapsed when the session is in the idle wait state. | 200011 | 3072 |
| background database time               | The total time spent in executing database operations by background processes. | 200012 | 3072 |
| background database non-idle wait time | The time spent in executing database operations by background processes in the non-idle wait state. | 200013 | 3072 |
| background database idle wait time     | The time spent in executing database operations by background processes in the idle wait state. | 200014 | 3072 |
| concurrency wait total time            | The total wait time caused by concurrency issues, such as the wait time caused by resource locking. |220001 | 3072 |
| user io wait total time                | The total time spent by user processes in waiting for the I/O operations, such as data reads from or data writes to the disk, to complete. | 220002 | 3072 |
| application wait total time            | The total wait time caused by the application code, such as lock waits caused by row-level locking or explicit lock commands. | 2200013 | 3072 |
