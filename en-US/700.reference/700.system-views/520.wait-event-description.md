...
Each wait event belongs to a specific wait event class. Wait events are classified into the following classes:
...
You can execute SQL statements in the `sys` tenant to display all wait events and their classes in alphabetical order. Here is an example:
...
| --- | --- | --- | --- | --- | --- |
| wait remove partition | The event of waiting for the garbage collection (GC) of partitions to complete. | ADMINISTRATIVE | tenant_id | table_id | partition_id |
...
| async rpc proxy condition wait | The asynchronous remote procedure call (RPC) wait event in top-level encapsulation. | CONCURRENCY | address | - | - |
...
| latch: server maintaince lock wait | The lock wait event when the RootServer performs an operation on an OBServer node. | CONCURRENCY | address | number | tries |
| latch: server status lock wait | The read-write lock wait event when an operation is performed on `server_statuses` recorded by the RootServer. | CONCURRENCY | address | number | tries |
...
| latch: group migrate lock wait | The lock wait event in batch migration. | CONCURRENCY | address | number | tries |
...
| oblog part mgr schema version wait | The event of waiting for liboblog to refresh the schema version logic. | CONCURRENCY | - | - | - |
...
| sync rpc                      | The event where OceanBase Database is waiting for a response after sending a synchronous RPC request. | NETWORK | pcode | size | - |
...
| das wait remote response          | The event where OceanBase Database is waiting for the remote execution result of a data access service (DAS) task during distributed SQL execution. | NETWORK      | - | - | - |
| wait for network request in queue | The event where a session request received by OceanBase Database is waiting for processing by a worker thread. | NETWORK      | pcode | level | priority |
| retry: insufficient px worker wait | The event where a retry occurs due to insufficient PX thread resources. | CONFIGURATION | degree of parallel | number of px workers required | the total number of idle PX workers currently |
| retry: wait GTS ready             | The event where a retry occurs due to waiting for Global Timestamp Service (GTS) to be ready. | CLUSTER | error code   | ls leader addr   | ls leader port  |
| retry: wait replica readable      | The event where a retry occurs because the replica is not readable | CLUSTER       |ls_id                | tablet_id                     |   -  |
| retry: wait schema cache load     | The event where a retry occurs due to the loading of the schema cache. |  CLUSTER       | error code           | table id    | table schema version    |
| retry: wait location cache load   | The event where a retry occurs due to the loading of the location cache. | CLUSTER | error      | ls_id   |  -  |
| retry: other retry wait           | The event where a retry occurs due to other causes. | CLUSTER  | error       |     -     |       -       |
| row lock wait                     | The wait event caused by a row lock. | APPLICATION | lock holder tx id | data seq number | holder lock time |
