...
| OTHERSTAT_3_VALUE       | NUMBER        | NO   | For V4.4.2: <ul><li>Before V4.4.2 BP1: For the TABLE SCAN operator, this column indicates the number of rows read from the SSStore `SSSTORE_READ_ROW_COUNT`. Real-time statistics are supported. </li><li>From V4.4.2 BP1 onwards: The total number of rows read from major SSTables in the specified range in the push-down or non-push-down path. </li></ul> <main id="notice" type='explain'><h4>Note</h4><ul><li>Starting from V4.3.3 BP1 of V4.3.x, the TABLE SCAN operator supports real-time statistics, instead of collecting statistics after the operator finishes computation. </li><li>Starting from V4.2.4 of V4.2.x, the TABLE SCAN operator supports real-time statistics, instead of collecting statistics after the operator finishes computation. </li></ul></main>|
| OTHERSTAT_4_ID          | NUMBER        | NO   | Query the field details by using the field ID.|
| OTHERSTAT_4_TYPE        | NUMBER        | NO   | Reserved.|
| OTHERSTAT_4_VALUE       | NUMBER        | NO   | For V4.4.2: <ul><li>Before V4.4.2 BP1: For the TABLE SCAN operator, this column indicates the number of rows read from the MEMStore `MEMSTORE_READ_ROW_COUNT`. Real-time statistics are supported. </li><li>From V4.4.2 BP1 onwards: The number of rows read from minor SSTables, mini SSTables, and MemTables in the push-down or non-push-down path. </li></ul>|
...
| OTHERSTAT_6_VALUE       | NUMBER        | NO   | The total number of rows read from microblocks opened in the push-down path in the specified range. <main id="notice" type='explain'><h4>Note</h4><p>For V4.4.2, this field is available starting from V4.4.2 BP1. </p></main>|
| OTHERSTAT_7_ID          | NUMBER        | NO   | Reserved.|
| OTHERSTAT_7_TYPE        | NUMBER        | NO   | Reserved.|
| OTHERSTAT_7_VALUE       | NUMBER        | NO   | The total number of rows filtered by push-down or non-push-down filters. <main id="notice" type='explain'><h4>Note</h4><p>For V4.4.2, this field is available starting from V4.4.2 BP1. </p></main>|
| OTHERSTAT_8_ID          | NUMBER        | NO   | Reserved.|
| OTHERSTAT_8_TYPE        | NUMBER        | NO   | Reserved.|
| OTHERSTAT_8_VALUE       | NUMBER        | NO   | The number of microblocks accessed by using skip index information without pre-fetching. <main id="notice" type='explain'><h4>Note</h4><p>For V4.4.2, this field is available starting from V4.4.2 BP1. </p></main>|
...
| USER_IO_WAIT_TIME       | NUMBER(38)    | NO   | For V4.4.2: <ul><li>For versions earlier than V4.4.2 BP1: the total time of all user_io events. </li><li>For V4.4.2 BP1 and later: the IO wait time when accessing the index and data microblocks in the middle layer. </li></ul>|
