...
| KEY                     | NUMBER        | NO   | The foreign key used for joining with the SQL_MONITOR virtual table.|
...
| PROCESS_NAME            | VARCHAR2(10)  | NO   | The execution thread ID.|
...
| PLAN_TEMP_SPACE         | NUMBER        | NO   | The amount of space occupied by the operator, estimated by the optimizer.|
...
| WORKAREA_MAX_MEM        | NUMBER        | NO   | The maximum size of the work area that can be occupied by the operator.|
| WORKAREA_TEMPSEG        | NUMBER        | NO   | The size of the disk dump space occupied by the operator.|
| WORKAREA_MAX_TEMPSEG    | NUMBER        | NO   | The maximum size of the disk dump space that can be occupied by the operator.|
| OTHERSTAT_GROUP_ID      | NUMBER        | NO   | NULL by default.|
| OTHERSTAT_1_ID          | NUMBER        | NO   | Query the details of the field by using the field ID.|
| OTHERSTAT_1_TYPE        | NUMBER        | NO   | Reserved.|
| OTHERSTAT_1_VALUE       | NUMBER        | NO   | For the TABLE SCAN operator, it indicates the number of bytes read from the storage `IO_READ_BYTES`.<main id="notice" type='explain'><h4>Note</h4><ul><li>Starting from V4.3.3 BP1 of V4.3.x, the TABLE SCAN operator supports real-time statistics, which are no longer collected only after the operator computation is completed. </li><li>Starting from V4.2.4 of V4.2.x, the TABLE SCAN operator supports real-time statistics, which are no longer collected only after the operator computation is completed. </li></ul></main>|
| OTHERSTAT_2_ID          | NUMBER        | NO   | Query the details of the field by using the field ID.|
...
| OTHERSTAT_3_ID          | NUMBER        | NO   | Query the details of the field by using the field ID.|
| OTHERSTAT_3_TYPE        | NUMBER        | NO   | Reserved.|
| OTHERSTAT_3_VALUE       | NUMBER        | NO   | The total number of rows in the major SSTables read in the push-down and non-push-down paths within the range.|
| OTHERSTAT_4_ID          | NUMBER        | NO   | Query the details of the field by using the field ID.|
| OTHERSTAT_4_TYPE        | NUMBER        | NO   | Reserved.|
| OTHERSTAT_4_VALUE       | NUMBER        | NO   | The total number of rows in the minor SSTables, mini SSTables, and MemTables read in the push-down and non-push-down paths.|
| OTHERSTAT_5_ID          | NUMBER        | NO   | Reserved.|
| OTHERSTAT_5_TYPE        | NUMBER        | NO   | Reserved.|
| OTHERSTAT_5_VALUE       | NUMBER        | NO   | The number of microblocks opened in the push-down path.<main id="notice" type='explain'><h4>Note</h4><p>Starting from V4.5.0 of V4.5.x, this field is available. </p></main>|
| OTHERSTAT_6_ID          | NUMBER        | NO   | Reserved.|
| OTHERSTAT_6_TYPE        | NUMBER        | NO   | Reserved.|
| OTHERSTAT_6_VALUE       | NUMBER        | NO   | The total number of rows in the microblocks opened in the push-down path within the range.<main id="notice" type='explain'><h4>Note</h4><p>Starting from V4.5.0 of V4.5.x, this field is available. </p></main>|
| OTHERSTAT_7_ID          | NUMBER        | NO   | Reserved.|
| OTHERSTAT_7_TYPE        | NUMBER        | NO   | Reserved.|
| OTHERSTAT_7_VALUE       | NUMBER        | NO   | The total number of rows filtered in the push-down and non-push-down paths.<main id="notice" type='explain'><h4>Note</h4><p>Starting from V4.5.0 of V4.5.x, this field is available. </p></main>|
| OTHERSTAT_8_ID          | NUMBER        | NO   | Reserved.|
| OTHERSTAT_8_TYPE        | NUMBER        | NO   | Reserved.|
| OTHERSTAT_8_VALUE       | NUMBER        | NO   | The number of microblocks that do not require pre-fetching based on the Skip Index information.<main id="notice" type='explain'><h4>Note</h4><p>Starting from V4.5.0 of V4.5.x, this field is available. </p></main>|
...
| OTHER_XML               | VARCHAR2(1)   | NO   | The structured data that cannot be written to the reserved fields but needs to be provided to external applications. This data is parsed by external tools.|
...
| SKIPPED_ROWS_COUNT      | NUMBER(38)    | NO   | The total number of rows that do not need to be computed in the operator (the total number of filtered rows) in vectorized mode.|
| DB_TIME                 | NUMBER(38)    | NO   | The CPU time consumed by this operator.|
| USER_IO_WAIT_TIME       | NUMBER(38)    | NO   | The time spent waiting for I/O when reading the intermediate index and data microblocks.|
| OTHER_WAIT_TIME         | NULL          | NO   | The wait time for other events.|
| PROFILE                 | VARCHAR2(65535 ) | NO | Displays the PROFILE extended monitoring metrics of the SQL PLAN MONITOR. <main id="notice" type='explain'><h4>Note</h4><p>For V4.4.x, this field is available starting from V4.4.1. </p></main>|
For the value of the `OTHERSTAT_X_ID` field, you can query the [V$SQL_MONITOR_STATNAME](35500.v-sql_monitor_statname-of-oracle-mode.md) view for detailed information.
...
* [View the execution plan in real time](../../../../700.reference/1000.performance-tuning-guide/500.sql-optimization/200.sql-execution-plan/500.real-time-execution-plan-display.md)