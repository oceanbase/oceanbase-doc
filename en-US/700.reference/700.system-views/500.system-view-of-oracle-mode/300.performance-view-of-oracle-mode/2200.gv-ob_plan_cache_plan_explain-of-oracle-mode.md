...
1. Query the [`GV$OB_PLAN_CACHE_PLAN_STAT`](2300.gv-ob_plan_cache_plan_stat-of-oracle-mode.md) view for the `PLAN_ID` value of the current OBServer node.
...
2. Query the `GV$OB_PLAN_CACHE_PLAN_EXPLAIN` view for the physical execution plan with the `PLAN_ID` value of `800` in the plan cache.
