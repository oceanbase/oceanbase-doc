...
This view displays the MemTable and SSTable information for each partition of the current tenant on the current OBServer node.
...
| SIZE                    | NUMBER(38)    | NO         | The disk space occupied, in bytes.   |
...
| CONTAIN_UNCOMMITTED_ROW | VARCHAR2(3)   | NO         | Indicates whether the table contains uncommitted transaction rows:<ul><li>`NO`: Indicates that the table does not contain uncommitted transaction rows.   </li><li> `YES`: Indicates that the table contains uncommitted transaction rows. </li></ul>   |

## Sample query

Query the MemTable and SSTable information for each partition of the current tenant on the current OBServer node in the user tenant for log stream 1001.
