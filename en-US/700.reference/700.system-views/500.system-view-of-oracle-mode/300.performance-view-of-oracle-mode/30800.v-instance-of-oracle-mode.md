...
| LOGINS           | VARCHAR2(10) | NO            | Indicates the login mode of the instance. Valid values: `ALLOWED` indicates that all users are allowed to log in. `RESTRICTED` indicates that only users with administrator privileges are allowed to log in. <main id="notice" type='explain'><h4>Note</h4><p>This column is available starting with V4.4.2 BP1. </p></main>                        |
