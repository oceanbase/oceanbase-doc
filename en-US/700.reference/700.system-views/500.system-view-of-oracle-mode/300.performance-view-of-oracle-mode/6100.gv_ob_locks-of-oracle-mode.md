...
The `GV$OB_LOCKS` view displays the status of the current user for holding or requesting locks on tables.
...
| LMODE | VARCHAR2(4) | NO | The lock holding mode. Valid values:<ul><li>`NONE`: No lock is held.</li><li>`RS`: ROW SHARE</li><li>`RX`: ROW EXCLUSIVE</li><li>`S`: SHARE</li><li>`SRX`: SHARE ROW EXCLUSIVE</li><li>`X`: EXCLUSIVE</li></ul>      |
| REQUEST | VARCHAR2(8) | NO | The lock requesting mode. Valid values:<ul><li>`NONE`: No lock is requested.</li><li>`RS`: ROW SHARE</li><li>`RX`: ROW EXCLUSIVE</li><li>`S`: SHARE</li><li>`SRX`: SHARE ROW EXCLUSIVE</li><li>`X`: EXCLUSIVE</li></ul>     |
| CTIME | NUMBER(38) | NO | The amount of time to hold or wait for a lock, in microseconds. |
