...
<main id="notice" type='explain'>
  <h4>Note</h4>
  <p>The view name has been changed from <code>GV$PLAN_CACHE_PLAN_STAT</code> to <code>GV$OB_PLAN_CACHE_PLAN_STAT</code> since OceanBase Database V4.0.0. </p>
</main>
...
| SQL_ID                | VARCHAR2(32)                      | NO             | The SQL ID corresponding to the cached object. If it is a PL object, the value of this column is `NULL`. |
...
| STATEMENT             | CLOB                              | NO             | If the cached object is an SQL plan, the value of this column is a parameterized SQL statement. If the cached object is an anonymous block, the value of this column is a parameterized anonymous block statement. This column is invalid for other types of objects. |
| QUERY_SQL             | CLOB                              | NO             | If the cached object is an SQL plan, the value of this column is the original SQL statement queried when the plan was loaded for the first time. If the cached object is an anonymous block, the value of this column is a parameterized anonymous block statement. |
...
| SLOWEST_EXE_USEC      | NUMBER(38)                        | NO             | The amount of time consumed by the slowest execution. |
...
| APPLICATION_WAIT_TIME | NUMBER(38)                        | NO             | The total amount of wait time of Application wait events during the request execution process. |
| CONCURRENCY_WAIT_TIME | NUMBER(38)                        | NO             | The total amount of wait time of Concurrency wait events during the request execution process. |
| USER_IO_WAIT_TIME     | NUMBER(38)                        | NO             | The total amount of wait time of User I/O wait events during the request execution process. |
...
| ELAPSED_TIME          | NUMBER(38)                        | NO             | The total amount of time consumed by all executions, that is, the amount of time elapsed from when the execution requests were received to when the execution of all the requests was completed. |
...
| PL_SCHEMA_ID          | NUMBER(38)                        | NO             | For PL objects that are not anonymous blocks, this column indicates the schema ID of the cached object. For anonymous blocks, this column is equivalent to `PS_STMT_ID`. For SQL plans, this column is meaningless. |
