...
| TYPE                  | NUMBER(38)                        | NO             | For a SQL plan, this field indicates the type of the plan:  <ul><li> 1: indicates a local plan.    </li><li> 2: indicates a remote plan. </li><li>3: indicates a distributed plan.  </li></ul>For a PL object, this field indicates the type of the PL object:<ul><li> 1: indicates a procedure.    </li><li> 2: indicates a function. </li><li>3: indicates a package.  </li><li> 4: indicates an anonymous block.   </li></ul>    |
...
| STATEMENT             | CLOB                              | NO             | For a SQL plan, this field contains the parameterized SQL statement. For an anonymous block, this field contains the parameterized anonymous block statement. This field is invalid for other types of objects.                                            |
| QUERY_SQL             | CLOB                              | NO             | For a SQL plan, this field contains the original SQL statement queried when the plan was first loaded. For an anonymous block, this field contains the parameterized anonymous block statement.                                               |
...
| PLAN_SIZE             | NUMBER(38)                        | NO             | The size of the memory occupied by the cached object.         |
...
| ROWS_PROCESSED        | NUMBER(38)                        | NO             | The number of result rows processed or the number of rows modified in the table.                                                                                  |
| ELAPSED_TIME          | NUMBER(38)                        | NO             | The time consumed from receiving the request to the end of the execution, in microseconds.  |
...
| PS_STMT_ID            | NUMBER(38)                        | NO             | The Prepare ID corresponding to the request:<ul><li>`-1`: Indicates that the SQL statement does not use the PS protocol.</li><li>Other values: Indicates that the SQL statement uses the PS protocol and is the unique identifier returned by the PS protocol for the statement.  </li></ul>     |
| SESSID                | NUMBER(38)                        | NO             | The session ID of the cache object.<br>For V4.3.x: <ul><li>For versions earlier than V4.3.5 BP2:<br>This field indicates the server session ID in all connection modes (direct mode and ODP mode). </li><li>For V4.3.5 BP2 and later: <ul><li>In direct mode:<br>This field indicates the server session ID. </li> <li>In ODP mode:</li><ul><li>When the <code>client_session_id_version</code> parameter in ODP is set to `2`, this field indicates the client session ID. </li> <li>When <code>client_session_id_version</code> is set to `1`, this field indicates the server session ID. </li></ul></ul></li></ul>   |
| TEMP_TABLES           | CLOB                              | NO             | The name of the temporary table in the SQL plan. If no temporary table is present, the field is empty.                                                                           |
| IS_USE_JIT            | NUMBER(38)                        | NO             | Indicates whether expression compilation is enabled in the SQL plan.  |
...
| PLAN_STATUS           | VARCHAR2(8)                       | NO             | The current status of the plan, either `ACTIVE` or `INACTIVE`.|
...
| CREATE_REASON           | VARCHAR2(16384)                 | NO             | The reason for adding the current plan.<main id="notice" type='explain'><h4>Note</h4><p>This field is available starting with V4.6.0. </p></main>|
