...
The `V$OB_TENANT_RESOURCE_LIMIT_DETAIL` view displays the logical resource limits of the current tenant on the current OBServer node and the maximum values of each limit.
...
Query the logical resource limits of the current tenant on the current OBServer node and the maximum values of each limit.
