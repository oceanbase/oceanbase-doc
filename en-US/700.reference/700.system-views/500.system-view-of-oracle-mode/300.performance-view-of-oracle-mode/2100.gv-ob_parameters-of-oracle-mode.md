...
| TENANT_ID  | NUMBER(38)      | YES        | The tenant ID: <ul><li>`NULL` indicates that the tenant does not exist.  </li><li>Valid value: the tenant ID. </li></ul>    |
| NAME       | VARCHAR2(128)   | NO         | The name of the parameter.                                                             |
| DATA_TYPE  | VARCHAR2(128)   | YES        | The type of the parameter value.   <main id="notice" type='explain'><h4>Note</h4><p>Starting from V4.3.0, the value of this field changes from `NULL` to the specific type of the parameter. </p></main>    |
...
| EDIT_LEVEL | VARCHAR2(128)   | NO         | The way in which the parameter takes effect after modification: <ul><li> `READONLY`: read-only. The parameter cannot be modified. </li><li>`STATIC_EFFECTIVE`: the parameter takes effect only after a restart. </li><li> `DYNAMIC_EFFECTIVE`: the parameter takes effect dynamically after modification. </li></ul>   |
