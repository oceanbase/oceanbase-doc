...
The `V$OB_THREAD` view displays the status information about all threads in the observer process. In a user tenant, it shows the threads related to the current tenant.
...
| CGROUP_PATH | VARCHAR2(256) | NO  | The path of the cgroup to which the thread belongs. This field is obtained from the CPU row in the `/proc/observer-pid/task/tid/cgroup` file. |

## Sample query

View the status information of all threads in the observer process on the current OBServer node in the user tenant, and display the first two records.
