...
Displays the information about the MemTables and SSTables under each partition of the current tenant on the current OBServer node.
...
| TABLE_TYPE | VARCHAR2(20) | NO | Table type |
| LS_ID | NUMBER(38) | NO | Log Stream ID |
| TABLET_ID | NUMBER(38) | NO | Data Shard ID |
| START_LOG_SCN | NUMBER(38) | NO | The left boundary of the log timestamp range for the data in this table. |
| END_LOG_SCN | NUMBER(38) | NO | Right Boundary of Log Timestamp Range for Data in This Table |
| SIZE | NUMBER(38) | NO | Disk usage, in bytes |
| REF | NUMBER(38) | NO | The count of references to this table, in units of number |
| UPPER_TRANS_VERSION | NUMBER(38) | NO | Maximum Transaction Version Number of a Committed Transaction |
| IS_ACTIVE | VARCHAR2(3) | NO | Whether it is an active MemTable |
| CONTAIN_UNCOMMITTED_ROW | VARCHAR2(3) | NO | Whether the table contains uncommitted transaction rows:<ul><li>`NO`: Indicates that the table does not contain uncommitted transaction rows.</li><li>`YES`: Indicates that the table contains uncommitted transaction rows. </li></ul> |

## Sample query

In the user tenant, view the MemTable and SSTable information of each partition for log stream 1001 in the current OBServer node.
