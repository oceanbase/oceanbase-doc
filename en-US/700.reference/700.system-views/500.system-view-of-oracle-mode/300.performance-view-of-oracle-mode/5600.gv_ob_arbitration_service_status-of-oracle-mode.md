...
| STATUS | VARCHAR2(64) | NO | The communication status between the arbitration service and the current node. Valid values:<ul><li>  `ACTIVE`: indicates that the arbitration service communicates with the current node. </li><li> `INACTIVE`: indicates that the arbitration service cannot communicate with the current node. </li></ul>|
