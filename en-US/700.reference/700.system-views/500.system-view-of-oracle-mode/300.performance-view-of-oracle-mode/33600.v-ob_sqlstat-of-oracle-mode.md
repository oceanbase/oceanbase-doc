...
The `V$OB_SQLSTAT` view displays basic performance statistics for SQL statements on the current node. Each SQL statement is represented by a row (i.e., each unique combination of `SQL_ID` and `Plan_Hash`). The columns with `_DELTA` in their names indicate the incremental statistics from the last WR snapshot collection to the current time.
...
| PLAN_TYPE                     | NUMBER          | NO   |    The type of the execution plan. <ul><li>`1`: Local Plan</li> <li>`2`: Remote Plan</li> <li>`3`: Distributed Plan</li></ul> </br>For PL object caching, this column indicates the type of the PL object. <ul><li>`1`: Procedure</li> <li>`2`: Function</li> <li>`3`: Package</li> <li>`4`: Anonymous Block</li></ul> |
| QUERY_SQL                     | CLOB            | NO   | The complete SQL text. |
...
| EXECUTIONS_TOTAL              | NUMBER          | NO   |   The total number of executions of the execution plan in the plan cache.     |
| EXECUTIONS_DELTA              | NUMBER          | NO   |   The incremental number of executions of the execution plan in the plan cache.      |
...
| ELAPSED_TIME_TOTAL            | NUMBER          | NO   |   The total time consumed by the statement. For parallel execution scenarios, this value is the sum of the elapsed times of each execution thread.          |
| ELAPSED_TIME_DELTA            | NUMBER          | NO   |   The incremental time consumed by the statement.   |
| CPU_TIME_TOTAL                | NUMBER          | NO   |   The total CPU time.       |
| CPU_TIME_DELTA                | NUMBER          | NO   |   The incremental CPU time.     |
...
| USERIO_WAIT_TOTAL             | NUMBER          | NO   |   The total time consumed by I/O waits.      |
| USERIO_WAIT_DELTA             | NUMBER          | NO   |   The incremental time consumed by I/O waits.      |
...
| WRITE_THROTTLE_TOTAL          | NUMBER          | NO   |   The total time when write throttling occurs during MemStore writes.    |
| WRITE_THROTTLE_DELTA          | NUMBER          | NO   |   The incremental time when write throttling occurs during MemStore writes.    |
...
| RPC_DELTA                     | NUMBER          | NO   | The incremental number of RPC calls.   |
| FETCHES_TOTAL                 | NUMBER          | NO   | The total number of times the result set is fetched.   |
| FETCHES_DELTA                 | NUMBER          | NO   | The incremental number of times the result set is fetched.     |
| RETRY_TOTAL                   | NUMBER          | NO   | The total number of SQL retries.   |
| RETRY_DELTA                   | NUMBER          | NO   | The incremental number of SQL retries.      |
| PARTITION_TOTAL               | NUMBER          | NO   | The total number of partitions scanned by the SQL execution.      |
| PARTITION_DELTA               | NUMBER          | NO   | The incremental number of partitions scanned by the SQL execution.    |
| NESTED_SQL_TOTAL              | NUMBER          | NO   | The total number of nested SQL executions. Each time a sub-SQL is executed, the value increases by 1.     |
| NESTED_SQL_DELTA              | NUMBER          | NO   | The incremental number of nested SQL executions.    |
| SOURCE_IP                     | CHAR(46)        | NO   | The IP address of the request source.     |
| SOURCE_PORT                   | NUMBER          | NO   | The port number of the request source.      |
| ROUTE_MISS_TOTAL              | NUMBER          | NO   | The total number of times the request partition is not hit since the OBServer was started<main id="notice" type='explain'><h4>Note</h4><ul><li>For V4.3.x, this field was introduced in V4.3.5. </li> <li>For V4.2.x, this field was introduced in V4.2.2. </li></ul></main>   |
| ROUTE_MISS_DELTA              | NUMBER          | NO   | The number of times the request partition is not hit since the last WR snapshot collection started<main id="notice" type='explain'><h4>Note</h4><ul><li>For V4.3.x, this field was introduced in V4.3.5. </li> <li>For V4.2.x, this field was introduced in V4.2.2. </li></ul></main>   |
...
| MUTI_QUERY_TOTAL              | NUMBER        | NO   | The number of times the SQL statement is in a Multi Query since the execution plan was generated<main id="notice" type='explain'><h4>Note</h4><p>For V4.6.x, this field was introduced in V4.6.0. </p></main>|
| MUTI_QUERY_DELTA              | NUMBER        | NO   | The number of times the SQL statement is in a Multi Query since the last WR snapshot collection<main id="notice" type='explain'><h4>Note</h4><p>For V4.6.x, this field was introduced in V4.6.0. </p></main>|
| MUTI_QUERY_BATCH_TOTAL        | NUMBER        | NO   | The number of times the SQL batch is executed since the execution plan was generated<main id="notice" type='explain'><h4>Note</h4><p>For V4.6.x, this field was introduced in V4.6.0. </p></main>|
| MUTI_QUERY_BATCH_DELTA        | NUMBER        | NO   | The number of times the SQL batch is executed since the last WR snapshot collection<main id="notice" type='explain'><h4>Note</h4><p>For V4.6.x, this field was introduced in V4.6.0. </p></main>|
| FULL_TABLE_SCAN_TOTAL         | NUMBER        | NO   | The number of times that the SQL statement has performed a full table scan since the execution plan was generated. <main id="notice" type='explain'><h4>Note</h4><p>This column is available starting with V4.6.0 for V4.6.x. </p></main>|
| FULL_TABLE_SCAN_DELTA         | NUMBER        | NO   | The number of times that the SQL statement has performed a full table scan since the last WR snapshot was collected. <main id="notice" type='explain'><h4>Note</h4><p>This column is available starting with V4.6.0 for V4.6.x. </p></main>|
| ERROR_COUNT_TOTAL             | NUMBER        | NO   | The number of errors that occurred during the execution of the SQL statement since the execution plan was generated. <main id="notice" type='explain'><h4>Note</h4><p>This column is available starting with V4.6.0 for V4.6.x. </p></main>|
| ERROR_COUNT_DELTA             | NUMBER        | NO   | The number of errors that occurred during the execution of the SQL statement since the last WR snapshot was collected. <main id="notice" type='explain'><h4>Note</h4><p>This column is available starting with V4.6.0 for V4.6.x. </p></main>|
| LATEST_ACTIVE_TIME            | TIMESTAMP(6)  | NO   | The last active time of the execution plan corresponding to the SQL ID and plan hash on a node. <main id="notice" type='explain'><h4>Note</h4><p>This column is available starting with V4.6.0 for V4.6.x. </p></main> |
...
Query the basic performance statistics of SQL on the current node and display the first record.
