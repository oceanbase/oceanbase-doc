...
The `V$OB_SQLSTAT` view displays the basic performance statistics of SQL statements on the current node. Each SQL statement is represented as a row (i.e., each unique combination of `SQL_ID` and `Plan_Hash` is a row). The lists with `_DELTA` in their names show the incremental statistics from the last WR snapshot collection to the current time.
...
| PLAN_TYPE                     | NUMBER          | NO   |    For SQL plan cache, this column specifies the type of the plan:<ul><li>`1` indicates a local plan. </li> <li>`2` indicates a remote plan. </li> <li>`3` indicates a distributed plan. </li></ul> </br>For PL/SQL object cache, this column specifies the type of the PL/SQL object:<ul><li>`1` indicates a procedure. </li> <li>`2` indicates a function. </li> <li>`3` indicates a package. </li> <li>`4` indicates an anonymous block. </li></ul> |
| QUERY_SQL                     | CLOB            | NO   | Full SQL text |
| SQL_TYPE                      | NUMBER          | NO   | Statement type |
| MODULE                        | VARCHAR2(64)    | NO  |   The module of the application when the statement was first parsed.    |
| ACTION                        | VARCHAR2(64)    | NO  |   The action of the application when the statement was first parsed.      |
...
| ELAPSED_TIME_TOTAL            | NUMBER          | NO   |   The total time consumed by the statement. For parallel execution, this value is the sum of the values for each execution thread.          |
...
| PHYSICAL_READ_BYTES_TOTAL     | NUMBER          | NO   |   The total number of bytes read by physical reads.     |
| PHYSICAL_READ_BYTES_DELTA     | NUMBER          | NO   |   The incremental number of bytes read by physical reads.   |
...
| NESTED_SQL_TOTAL              | NUMBER          | NO   |   The total number of nested SQL executions. Each time a sub-SQL statement is executed, the value increases by 1.     |
...
| ROUTE_MISS_TOTAL              | NUMBER          | NO   |   The total number of times that the request partition was not hit since the OBServer was started<main id="notice" type='explain'><h4>Note</h4><ul><li>For V4.3.x, this column is available starting with V4.3.5. </li> <li>For V4.2.x, this column is available starting with V4.2.2. </li></ul></main>   |
| ROUTE_MISS_DELTA              | NUMBER          | NO   |   The number of times that the request partition was not hit since the last WR snapshot was collected<main id="notice" type='explain'><h4>Note</h4><ul><li>For V4.3.x, this column is available starting with V4.3.5. </li> <li>For V4.2.x, this column is available starting with V4.2.2. </li></ul></main>   |
| FIRST_LOAD_TIME               | TIMESTAMP(6)    | NO   |   The time when the plan was generated<main id="notice" type='explain'><h4>Note</h4><ul><li>For V4.3.x, this column is available starting with V4.3.5. </li> <li>For V4.2.x, this column is available starting with V4.2.5. </li></ul></main>   |
| PLAN_CACHE_HIT_TOTAL          | NUMBER          | NO   |   The total number of times that the Plan Cache was hit since the OBServer was started<main id="notice" type='explain'><h4>Note</h4><ul><li>For V4.3.x, this column is available starting with V4.3.5. </li> <li>For V4.2.x, this column is available starting with V4.2.5. </li></ul></main>   |
| PLAN_CACHE_HIT_DELTA          | NUMBER          | NO   |   The number of times that the Plan Cache was hit since the last WR snapshot was collected<main id="notice" type='explain'><h4>Note</h4><ul><li>For V4.3.x, this column is available starting with V4.3.5. </li> <li>For V4.2.x, this column is available starting with V4.2.5. </li></ul></main>  |
| MUTI_QUERY_TOTAL              | NUMBER        | NO   | The number of times the SQL statement has been in a Multi Query since the execution plan was generated<main id="notice" type='explain'><h4>Note</h4><p>For V4.6.x, this column is available starting with V4.6.0. </p></main>|
| MUTI_QUERY_DELTA              | NUMBER        | NO   | The number of times the SQL statement has been in a Multi Query since the last WR snapshot was collected<main id="notice" type='explain'><h4>Note</h4><p>For V4.6.x, this column is available starting with V4.6.0. </p></main>|
| MUTI_QUERY_BATCH_TOTAL        | NUMBER        | NO   | The number of times the SQL batch has been executed since the execution plan was generated<main id="notice" type='explain'><h4>Note</h4><p>For V4.6.x, this column is available starting with V4.6.0. </p></main>|
| MUTI_QUERY_BATCH_DELTA        | NUMBER        | NO   | The number of times the SQL batch has been executed since the last WR snapshot was collected<main id="notice" type='explain'><h4>Note</h4><p>For V4.6.x, this column is available starting with V4.6.0. </p></main>|
| FULL_TABLE_SCAN_TOTAL         | NUMBER        | NO   | The number of full table scans for the SQL statement since the execution plan was generated. <main id="notice" type='explain'><h4>Note</h4><p>This column is available starting with V4.6.0 for V4.6.x. </p></main>|
| FULL_TABLE_SCAN_DELTA         | NUMBER        | NO   | The number of full table scans for the SQL statement since the last WR snapshot was collected. <main id="notice" type='explain'><h4>Note</h4><p>This column is available starting with V4.6.0 for V4.6.x. </p></main>|
| ERROR_COUNT_TOTAL             | NUMBER        | NO   | The number of errors for the SQL statement since the execution plan was generated. <main id="notice" type='explain'><h4>Note</h4><p>This column is available starting with V4.6.0 for V4.6.x. </p></main>|
| ERROR_COUNT_DELTA             | NUMBER        | NO   | The number of errors for the SQL statement since the last WR snapshot was collected. <main id="notice" type='explain'><h4>Note</h4><p>This column is available starting with V4.6.0 for V4.6.x. </p></main>|
| LATEST_ACTIVE_TIME            | TIMESTAMP(6)  | NO   | The last active time of the plan corresponding to the SQL ID and plan hash on a node. <main id="notice" type='explain'><h4>Note</h4><p>This column is available starting with V4.6.0 for V4.6.x. </p></main> |
...
Query the basic performance statistics of SQL statements on the current node and display the first record.
