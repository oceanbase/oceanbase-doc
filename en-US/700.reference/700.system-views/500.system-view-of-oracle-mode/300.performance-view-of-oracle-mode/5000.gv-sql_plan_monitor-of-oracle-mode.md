...
The `GV$SQL_PLAN_MONITOR` view displays the statistics of slow queries at the operation level in all OBServer nodes of the tenant. For each slow query, statistics are provided for each operation, including the time consumed by operators and the number of rows returned. The view also displays the TRACE_ID and other identifiers.
