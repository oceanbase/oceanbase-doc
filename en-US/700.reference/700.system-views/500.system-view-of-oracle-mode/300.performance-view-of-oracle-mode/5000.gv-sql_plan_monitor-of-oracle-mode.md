...
| OTHERSTAT_3_VALUE       | NUMBER        | NO   | The number of rows read from the SSStore by the TABLE SCAN operator. This value is `SSSTORE_READ_ROW_COUNT`. Real-time statistics are supported. <main id="notice" type='explain'><h4>Note</h4><ul><li>Starting from V4.3.3 BP1 of V4.3.x, the TABLE SCAN operator supports real-time statistics, which are no longer collected only after the operator finishes execution. </li><li>Starting from V4.2.4 of V4.2.x, the TABLE SCAN operator supports real-time statistics, which are no longer collected only after the operator finishes execution. </li></ul></main>|
...
| OTHERSTAT_4_VALUE       | NUMBER        | NO   | The number of rows read from the MEMStore by the TABLE SCAN operator. This value is `MEMSTORE_READ_ROW_COUNT`. Real-time statistics are supported.|
