...
The `GV$SQL_PLAN_MONITOR` view displays the statistics of operations for slow queries on all OBServer nodes in the current tenant. Each operation of a slow query provides statistical information.
...
| STATUS                  | VARCHAR2(19)  | NO   | The execution status of the operator:<li>EXECUTING: executing<li>DONE(ERROR): failed<li>DONE(FIRST N ROWS): successful<li>DONE(ALL ROWS): successful<li>DONE: interrupted|
...
| PLAN_TEMP_SPACE         | NUMBER        | NO   | The size of the space occupied by the operator estimated by the optimizer.|
...
| OTHERSTAT_3_ID          | NUMBER        | NO   | Query the detailed information of a field by using the field ID.|
| OTHERSTAT_3_TYPE        | NUMBER        | NO   | Reserved.|
| OTHERSTAT_3_VALUE       | NUMBER        | NO   | The total number of rows read from the major SSTables in the specified range in the push-down or non-push-down path.|
| OTHERSTAT_4_ID          | NUMBER        | NO   | Query the detailed information of a field by using the field ID.|
| OTHERSTAT_4_TYPE        | NUMBER        | NO   | Reserved.|
| OTHERSTAT_4_VALUE       | NUMBER        | NO   | The number of rows read from the minor SSTables, mini SSTables, and MemTables in the push-down or non-push-down path.|
...
| OTHERSTAT_6_VALUE       | NUMBER        | NO   | The total number of rows read from the microblocks in the specified range in the push-down path. <main id="notice" type='explain'><h4>Note</h4><p>This field is available starting from V4.5.0 in V4.5.x. </p></main>|
| OTHERSTAT_7_ID          | NUMBER        | NO   | Reserved.|
| OTHERSTAT_7_TYPE        | NUMBER        | NO   | Reserved.|
| OTHERSTAT_7_VALUE       | NUMBER        | NO   | The total number of rows filtered in the push-down or non-push-down path. <main id="notice" type='explain'><h4>Note</h4><p>This field is available starting from V4.5.0 in V4.5.x. </p></main>|
| OTHERSTAT_8_ID          | NUMBER        | NO   | Reserved.|
| OTHERSTAT_8_TYPE        | NUMBER        | NO   | Reserved.|
| OTHERSTAT_8_VALUE       | NUMBER        | NO   | The number of microblocks that do not require pre-fetching based on the Skip Index information. <main id="notice" type='explain'><h4>Note</h4><p>This field is available starting from V4.5.0 in V4.5.x. </p></main>|
...
| OTHER_XML               | VARCHAR2(1)   | NO   | The structured data that cannot be written to the reserved fields but needs to be provided for external use. This data is parsed by external tools.|
...
| SKIPPED_ROWS_COUNT      | NUMBER(38)    | NO   | The total number of rows to be skipped (filtered out) in the operator calculation in the vectorized mode.|
| DB_TIME                 | NUMBER(38)    | NO   | The CPU time consumed by this operator.|
| USER_IO_WAIT_TIME       | NUMBER(38)    | NO   | The time spent on I/O wait when the intermediate index and data microblocks are accessed.|
| OTHER_WAIT_TIME         | NULL          | NO   | The waiting time for other events.|
| PROFILE                 | VARCHAR2(65535 ) | NO | This column displays the PROFILE extended monitoring metric of the SQL PLAN MONITOR. <main id="notice" type='explain'><h4>Note</h4><p>This field is available starting from V4.4.1 in V4.4.x. </p></main>|
