...
The `V$OB_PLAN_CACHE_PLAN_EXPLAIN` view displays the physical execution plans cached on the current OBServer node.
  ...
  <main id="notice" type='explain'>
    <h4>Note</h4>
    <p>This view supports only the Get operation. You must specify the <code>PLAN_ID</code> field when querying. </p>
  </main>
...
| PLAN_DEPTH   | NUMBER(38)     | NO             | The depth of the operator in the display. |
...
| PROPERTY     | VARCHAR2(4096) | NO             | The information of the corresponding operator.  |

## Sample query

1. Query the `PLAN_ID` of the current OBServer node from the [V$OB_PLAN_CACHE_PLAN_STAT](34500.v-ob_plan_cache_plan_stat-of-oracle-mode.md) view.
...
2. Query the physical execution plan in the plan cache from the `V$OB_PLAN_CACHE_PLAN_EXPLAIN` view, for example, where `PLAN_ID=800`.
