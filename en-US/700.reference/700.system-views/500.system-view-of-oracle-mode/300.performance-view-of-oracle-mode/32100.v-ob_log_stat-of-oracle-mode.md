...
| SYNC_MODE         | VARCHAR2(128)  | NO | The synchronization mode of the log stream:<ul><li>`ASYNC`: asynchronous synchronization mode</li> <li>`SYNC`: strong synchronization mode</li> <li>`PRE_ASYNC`: the synchronization mode is being switched from strong synchronization mode to asynchronous synchronization mode</li></ul> <main id="notice" type='explain'><h4>Note</h4><p>This column is available starting with V4.4.2 BP1. </p></main> |
