...
The `GV$OB_TABLET_COMPACTION_HISTORY` view displays the historical information about minor and major compactions at the tablet level.
...
| COMPRESSION_RATIO                    | NUMBER(38)    | NO         | The compression ratio of new data: the size of the compressed data after compression/the size of the data before compression.                                                                 |
...
| PARALLEL_INFO                        | VARCHAR2(512) | NO         | Information about parallel tasks, which displays the statistics (minimum value, maximum value, and average value) of the data scanned, runtime, and data output for each parallel task.                                       |
|MACRO_ID_LIST    |varchar2(256)    |NO   |The list of macroblocks output. If the list is too long, it is not displayed.    |
|COMMENTS    |varchar2(256)    |NO   |Records the history of failed compactions and the time required for this compaction.    |
...
| KEPT_SNAPSHOT                   | VARCHAR2(128)| NO   |  The retained multi-version sites for this compaction<main id="notice" type='explain'><h4>Note</h4><p>This field is introduced in V4.3.0. </p></main>    |
