|description||
|---|---|
| machine-translation ||
|keywords||
|dir-name||
|dir-name-en||
|tenant-type|Oracle Mode|
|slug|gv-ob_arbitration_member_info-oracle-mode|

# GV$OB_ARBITRATION_MEMBER_INFO

<main id="notice" type='explain'>
<h4>Note</h4>
<p>This view is available starting with V4.2.0. </p>
</main>

## Purpose

This view displays the arbitration member information of the current tenant.

## Columns

| **Column** | **Type** | **Nullable** | **Description** |
| --- | --- | --- | --- |
| TENANT_ID | NUMBER(38) | NO | The tenant ID. |
| LS_ID | NUMBER(38) | NO | The log stream ID. |
| SVR_IP | VARCHAR2(46) | NO | The IP address of the arbitration server. |
| SVR_PORT | NUMBER(38) | NO | The port number of the arbitration server. |
| PROPOSAL_ID | NUMBER(38) | NO | The Paxos proposal ID. |
| CONFIG_VERSION | VARCHAR2(128) | NO | The version number of the configuration information. |
| ACCESS_MODE | VARCHAR2(32) | NO | The access mode. |
| PAXOS_MEMBER_LIST | VARCHAR2(1024) | NO | The list of members. |
| PAXOS_REPLICA_NUM | NUMBER(38) | NO | The number of required replicas. |
| ARBITRATION_MEMBER | VARCHAR2(128) | NO | The arbitration member. |
| DEGRADED_LIST | VARCHAR2(1024) | NO | The list of degraded replicas. |

## Sample query

Query the arbitration member information of the current tenant.

```shell
obclient[SYS]> SELECT * FROM SYS.GV$OB_ARBITRATION_MEMBER_INFO;
```

The query result is as follows:

```shell
+-----------+-------+----------------+----------+-------------+-------------------------------+-------------+---------------------------------------------+-------------------+---------------------+---------------+
| TENANT_ID | LS_ID | SVR_IP         | SVR_PORT | PROPOSAL_ID | CONFIG_VERSION                | ACCESS_MODE | PAXOS_MEMBER_LIST                           | PAXOS_REPLICA_NUM | ARBITRATION_MEMBER  | DEGRADED_LIST |
+-----------+-------+----------------+----------+-------------+-------------------------------+-------------+---------------------------------------------+-------------------+---------------------+---------------+
|      1004 |     1 | xxx.xx.xxx.224 |     2882 |           1 | {proposal_id:1, config_seq:2} | APPEND      | xxx.xx.xxx.207:2882:1,xxx.xx.xxx.218:2882:1 |                 2 | xxx.xx.xxx.224:2882 | NULL          |
|      1004 |  1001 | xxx.xx.xxx.224 |     2882 |           1 | {proposal_id:1, config_seq:2} | APPEND      | xxx.xx.xxx.207:2882:1,xxx.xx.xxx.218:2882:1 |                 2 | xxx.xx.xxx.224:2882 | NULL          |
+-----------+-------+----------------+----------+-------------+-------------------------------+-------------+---------------------------------------------+-------------------+---------------------+---------------+
2 rows in set
```

## References

* Query the communication status between the arbitration service and each node of the current tenant: [GV$OB_ARBITRATION_SERVICE_STATUS](5600.gv_ob_arbitration_service_status-of-oracle-mode.md)

* For more information about the arbitration service, see the following topics:

  * [Enable the arbitration service for a tenant](../../../../600.manage/400.high-availability/400.arbitration-high-availability/200.enable-the-arbitration-service.md)
  * [Disable the arbitration service for a tenant](../../../../600.manage/400.high-availability/400.arbitration-high-availability/300.disable-the-arbitration-service.md)
  * [Modify the degradation timeout period](../../../../600.manage/400.high-availability/400.arbitration-high-availability/400.modify-the-degradation-timeout.md)