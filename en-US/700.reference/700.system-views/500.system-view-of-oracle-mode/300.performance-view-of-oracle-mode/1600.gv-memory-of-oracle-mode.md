...
## Purpose
<h4>Note</h4>
<ul><li>The name of this view is changed from <code>GV$MEMORY</code> to <code>GV$OB_MEMORY</code> since OceanBase Database V4.0.0. </li>
<li> This view is supported only in MySQL mode. The query result is an empty set in Oracle mode. </li></ul>
<h4>Note</h4>

<p>The view name is changed from <code>GV$MEMORY</code> to <code>GV$OB_MEMORY</code> since OceanBase Database V4.0.0. </p>

The `GV$OB_MEMORY` view displays displays the memory statistics of the current tenant on all nodes.
