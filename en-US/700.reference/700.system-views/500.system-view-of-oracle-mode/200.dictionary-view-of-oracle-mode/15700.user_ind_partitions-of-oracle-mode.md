...
The `USER_IND_PARTITIONS` view displays index partitions accessible to the current user in the database. It does not have the `INDEX_OWNER` column compared with the `ALL_IND_PARTITIONS` and `DBA_IND_PARTITIONS` views.
