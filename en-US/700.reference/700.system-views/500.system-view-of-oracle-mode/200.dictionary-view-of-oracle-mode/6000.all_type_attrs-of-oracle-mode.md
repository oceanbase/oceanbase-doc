...
This view lists the attributes of user-defined types that the current user can access.
...
| ATTR_TYPE_MOD      | VARCHAR2(7)   | NO             | This column is not used. |
...
| SCALE              | NUMBER(38)    | NO             | The length of the data to the right of the decimal point for numeric types.          |
| CHARACTER_SET_NAME | CHAR(7)       | NO             | The character set of the attribute type.  <ul><li>`CHAR_CS`</li> <li> `NCHAR_CS` </li></ul>      |
...
| CHAR_USED          | CHAR(1)       | YES            | Indicates whether the attribute is defined based on the byte length (B) or character length (C).  |

## Sample query

Query the attributes of all user-defined types that the current user can access, and display the first 10 rows.
