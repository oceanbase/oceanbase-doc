...
|machine-translation||
|slug|all_triggers-oracle-mode|

# ALL_TRIGGERS

## Purpose

The `ALL_TRIGGERS` view displays the triggers on tables accessible to the current user. If the user has the `CREATE ANY TRIGGER` privilege, this view displays all triggers in the current tenant.
...
* View all triggers in the current tenant: [DBA_TRIGGERS](21300.dba_triggers-of-oracle-mode.md)

* View all triggers owned by the current user: [USER_TRIGGERS](30200.user_triggers-of-oracle-mode.md)
