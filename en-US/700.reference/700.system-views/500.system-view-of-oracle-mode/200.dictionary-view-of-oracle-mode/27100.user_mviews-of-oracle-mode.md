...
| REWRITE_ENABLED | VARCHAR2(1) | NO | Indicates whether rewrite is enabled. Valid values:<ul><li>`Y`</li><li>`N`</li></ul><main id="notice" type='explain'> <h4>Note</h4><p>This column is used only for compatibility and its value is `N` by default. </p></main> |
...
| ON_QUERY_COMPUTATION | VARCHAR2(1) | NO | Indicates whether the materialized view is a real-time view. Valid values:<ul><li>`Y`</li><li>`N`</li></ul><main id="notice" type='explain'> <h4>Note</h4><p>This column is used only for compatibility and its value is `N` by default. </p></main> |
| REFRESH_DOP          | NUMBER(38) | NO | The degree of parallelism for background refreshes of the materialized view. The default value is 0. <main id="notice" type='explain'><h4>Note</h4><p><ul><li>When you create a materialized view, you can specify this parameter by using the <code>PARALLEL</code> attribute. You can also modify it by using the <code>ALTER MATERIALIZED VIEW</code> statement later. When you do not explicitly set the <code>PARALLEL</code> attribute (when <code>REFRESH_DOP</code> is the default value 0), the degree of parallelism for background refreshes depends on the global system variable `mview_refresh_dop`. </li><li>For OceanBase Database V4.3.5, this column was introduced in V4.3.5 BP1. </li></ul></p></main>|
