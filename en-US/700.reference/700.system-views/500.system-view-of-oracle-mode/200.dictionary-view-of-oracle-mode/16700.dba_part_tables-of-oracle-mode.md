...
The `DBA_PART_TABLES` view displays the partitioning information of all partitioned tables in the current tenant.
...
| INTERVAL                  | VARCHAR2(1000) | YES        | At present, this column is not supported and its value is `NULL` by default. |
...
Query the partition information of the partitioned table `TBL2_F_RL` in the current user tenant.
