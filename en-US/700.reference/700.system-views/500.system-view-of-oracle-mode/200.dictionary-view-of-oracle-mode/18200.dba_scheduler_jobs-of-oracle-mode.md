...
| JOB_TYPE | VARCHAR2(16) | YES | The operation type of the job. Valid values: <ul><li>PLSQL_BLOCK</li><li>STORED_PROCEDURE </li><li>EXECUTABLE </li><li>CHAIN </li></ul> |
...
| SCHEDULE_TYPE | VARCHAR2(12) | NO | The type of the schedule used by the job. The default value is `NULL`. |
...
| SCHEDULE_LIMIT | INTERVAL DAY (3) TO SECOND (0) | NO | The time after which the job that has not run yet will be rescheduled. |
| MAX_RUN_DURATION | INTERVAL DAY (3) TO SECOND (0) | NO | The maximum duration that the job is allowed to run. |
| LOGGING_LEVEL | VARCHAR2(11) | NO | The level of logs to be recorded for the job. Valid values: <ul><li>OFF</li><li>RUNS</li><li>FAILED RUNS</li><li>FULL</li></ul> |
| STORE_OUTPUT | VARCHAR2(5) | NO | Indicates whether the output of the job is stored in the database. Valid values: <ul><li>TRUE</li><li>FALSE</li></ul> |
...
| RAISE_EVENTS | VARCHAR2(4000) | NO | The list of job events to raise for the job. The default value is `NULL`. |
...
| NUMBER_OF_DESTINATIONS | NUMBER | NO | The number of destinations associated with the job. |
| DESTINATION_OWNER | VARCHAR2(261) | NO | The owner of the destination. |
| DESTINATION | VARCHAR2(261) | NO | The destination on which the job will run. |
| CREDENTIAL_OWNER | VARCHAR2(128) | NO | The owner of the credential to be used for the job. |
| CREDENTIAL_NAME | VARCHAR2(128) | NO | The name of the credential to be used for the job. |
| INSTANCE_ID | NUMBER | YES | The instance on which the user requests the job to run. |
| DEFERRED_DROP | VARCHAR2(5) | NO | Indicates whether the job is dropped with a delay after it is completed. |
