|description||
|---|---|
| machine-translation ||
|keywords||
|dir-name||
|dir-name-en||
|tenant-type|Oracle Mode|
|slug|user_sys_privs-oracle-mode|

# USER_SYS_PRIVS

<main id="notice" type='explain'>
<h4>Note</h4>
<p>This view is available starting with V2.2.77. </p>
</main>

## Purpose

This view lists the system privileges granted to the current user.

  <main id="notice" >
    <h4>Applicability</h4>
    <p>This view is applicable only to OceanBase Database in Oracle mode. </p>
  </main>

## Columns

|   **Column**   |    **Type**     | **Nullable** |      **Description**      |
|--------------|---------------|----------------|------------------|
| GRANTEE      | VARCHAR2(128) | NO             | The name of the grantee, user, or role that received the grant. |
| PRIVILEGE    | VARCHAR2(40)  | NO             | The system privilege granted.             |
| ADMIN_OPTION | VARCHAR2(3)   | NO             | Indicates whether the management option was granted.           |

## Sample query

Query the system privileges granted to the `SYS` user and display the first 5 records.

```shell
obclient [SYS]> SELECT * FROM USER_SYS_PRIVS WHERE USERNAME='SYS' AND ROWNUM <=5;
```

The query result is as follows:

```shell
+----------+------------------+--------------+
| USERNAME | PRIVILEGE        | ADMIN_OPTION |
+----------+------------------+--------------+
| SYS      | CREATE SESSION   | YES          |
| SYS      | CREATE TABLE     | YES          |
| SYS      | CREATE ANY TABLE | YES          |
| SYS      | ALTER ANY TABLE  | YES          |
| SYS      | BACKUP ANY TABLE | YES          |
+----------+------------------+--------------+
5 rows in set
```

## References

[DBA_SYS_PRIVS](20600.dba_sys_privs-of-oracle-mode.md)