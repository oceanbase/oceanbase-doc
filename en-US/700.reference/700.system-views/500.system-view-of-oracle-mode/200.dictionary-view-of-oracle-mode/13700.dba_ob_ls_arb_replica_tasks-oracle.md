...
This view displays the arbitration service tasks that are being executed in the current tenant.
...
| TASK_TYPE | VARCHAR2(32) | NO | The task type. Valid values: <ul><li> `ADD REPLICA`: specifies an add replica task. </li><li> `REMOVE REPLICA`: specifies a remove replica task. </li></ul>|
...
Query the arbitration service tasks that are being executed in the current tenant.
