...
| ROW_MOVEMENT              | VARCHAR2(8)   | NO             | Records the enable or disable status of Row Movement. The possible values are as follows: <ul><li>YES: indicates the enable status.</li><li>NO: indicates the disable status.</li></ul> <main id="notice" type='explain'><h4>Note</h4><p>This column is effective in OceanBase Database V4.2.5 and later, starting from V4.2.5 BP2. </p></main>|
