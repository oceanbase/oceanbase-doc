...
The `USER_OBJECTS` view displays all database objects of the current user. The only difference between the `USER_OBJECTS` and `ALL_OBJECTS` views is that the former does not have the `OWNER` column.
