...
| SESSION_STATE | VARCHAR2(7) | NO | The status of the session when being sampled. Valid values: <ul><li>`EVENT_NO == 0`: "ON CPU" indicates that no wait event occurs and that the SQL logic is being executed.  </li><li>`EVENT_NO ≠ 0`: "WAITTING" indicates that a wait event occurs. The wait details are provided in the `EVENT` column. </li></ul> |
