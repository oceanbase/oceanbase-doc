|description||
|---|---|
|keywords||
|dir-name||
|dir-name-en||
|tenant-type|Oracle Mode|

# DBA_OB_LS_HISTORY

<main id="notice" type='explain'>
<h4>Note</h4>
<p>This view is introduced in V4.2.0. </p>
</main>

## Description

Displays the log streams that existed in the tenant.

## Fields

| Field | Type | Nullable | Description |
|-------------|---------------|-------------|-------------------------------------------|
| LS_ID       | NUMBER        | NO          |  The ID of the log stream.                                 |
| LS_GROUP_ID | NUMBER        | NO          |  The ID of the log stream group.                               |
| STATUS      | VARCHAR2(100) | NO          |  The current status of the log stream.  <ul><li>CREATING: The log stream is being created. </li><li>NORMAL: The normal status of a log stream. </li><li>DROPPING: The log stream is being dropped. </li><li>PRE_TENANT_DROPPING: A special status of a system log stream, indicating the start of dropping all log streams in the tenant. </li><li>TENANT_DROPPING: The log stream is being dropped when the tenant is being deleted. </li><li>DROPPED: The log stream has been recycled or is being recycled. </li><li>WAIT_OFFLINE: The log stream is waiting to be recycled. </li><li>CREATE_ABORT: The log stream creation is stopped. </li></ul>        |
| FLAG        | VARCHAR2(2048)| NO          |  The log stream flag.  <ul><li>NULL: A normal log stream. </li><li> DUPLICATE: A broadcast log stream. A replicated table is created on this log stream. </li> <li>BLOCK_TABLET_IN: Tablet creation is not allowed on this log stream. </li> <li>DUPLICATE\|BLOCK_TABLET_IN: Tablet creation is not allowed on the broadcast log stream. </li></ul>                        |
| CREATE_SCN  | NUMBER        | NO          |  The GTS at which the log stream was created, in nanoseconds.              |

## Examples

The following example shows how to query the log streams that existed in the user tenant.

```sshell
obclient [SYS]> SELECT * FROM SYS.DBA_OB_LS_HISTORY;
```

The query result is as follows:

```shell
+-------+-------------+--------+------+---------------------+
| LS_ID | LS_GROUP_ID | STATUS | FLAG | CREATE_SCN          |
+-------+-------------+--------+------+---------------------+
|     1 |           0 | NORMAL | NULL |                   0 |
|  1001 |        1001 | NORMAL | NULL | 1736152507462028000 |
+-------+-------------+--------+------+---------------------+
2 rows in set
```