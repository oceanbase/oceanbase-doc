...
| USER_STATS           | VARCHAR2(3)   | NO             | `YES` if statistics are directly entered by the user, otherwise `NO`.             |
...
Query all columns in the `T_SUBPART` table that is owned by the current user.
...
* Query columns in all tables or views that are accessible to the current user: [ALL_TAB_COLUMNS](4800.all_tab_columns-of-oracle-mode.md)

* Query columns in all tables or views in the current tenant: [DBA_TAB_COLUMNS](20900.dba_tab_columns-of-oracle-mode.md)