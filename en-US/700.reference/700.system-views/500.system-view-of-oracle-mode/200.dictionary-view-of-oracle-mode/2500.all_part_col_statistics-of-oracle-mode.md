...
| GLOBAL_STATS   | VARCHAR2(3)   | YES            | <ul><li>`YES`: indicates that the statistics are directly collected.   </li><li> `NO`: indicates that the statistics are derived from the statistics of subpartitions or that no statistics are collected.</li></ul>    |
...
* To query the column statistics of partitions in all partitioned tables of the current tenant, see [DBA_PART_COL_STATISTICS](17100.dba_part_col_statistics-of-oracle-mode.md).

* To query the column statistics of partitions in all partitioned tables of the current user, see [USER_PART_COL_STATISTICS](27900.user_part_col_statistics-of-oracle-mode.md).
...
* To query the column statistics of global partitions, see the following views:
