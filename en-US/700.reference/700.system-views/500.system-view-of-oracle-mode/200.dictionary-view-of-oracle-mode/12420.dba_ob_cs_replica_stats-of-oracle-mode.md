...
This view displays the progress of converting tablets to columnar storage in the log stream of C replicas on all OBServer nodes of the current tenant.
...
| AVAILABLE                 | VARCHAR2(5)   | NO   | Whether the log stream is fully available:<ul><li>`TRUE`: available. A log stream is fully available only when all tablets on it are available. </li> <li>`FALSE`: not available. </li></ul>  |

## Sample query

Query the progress of converting tablets to columnar storage in the log stream of C replicas on all OBServer nodes of the current tenant.
...
You can calculate the value of `AVAILABLE_TABLET_CNT / TOTAL_TABLET_CNT` or `AVAILABLE_MACRO_BLOCK_CNT / TOTAL_MACRO_BLOCK_CNT` to estimate the progress of converting rowstore to columnstore.
