...
The `ALL_SUBPART_HISTOGRAMS` view displays the histograms on all subpartitions that the current user can access.
...
Query the histogram information of the subpartitions of the `T_SUBPART` table in the partitioned table that the current user has access to.
...
* To view the histogram information of the subpartitions of the partitioned tables that the current user owns, see [USER_SUBPART_HISTOGRAMS](29800.user_subpart_histograms-of-oracle-mode.md).
...
* To view global column statistics, query the following views:
