...
This view displays the text source of the stored objects owned by the current user. The columns are the same as those in `ALL_SOURCE`, except for the `OWNER` column.
...
Query the text source of the stored objects owned by the current user and display the first two records.
...
* Query the text source of all stored objects accessible to the current user: [ALL_SOURCE](4000.all_source-of-oracle-mode.md)