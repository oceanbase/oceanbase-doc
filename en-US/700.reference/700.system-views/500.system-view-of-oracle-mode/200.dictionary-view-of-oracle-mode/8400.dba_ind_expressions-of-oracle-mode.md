|description||
|---|---|
| machine-translation ||
|keywords||
|dir-name||
|dir-name-en||
|tenant-type|Oracle Mode|
|slug|dba_ind_expressions-oracle-mode|

# DBA_IND_EXPRESSIONS

<main id="notice" type='explain'>
<h4>Note</h4>
<p>This view is available starting with V2.2.30. </p>
</main>

## Purpose

This view displays information about all function-based indexes in the current tenant.

  <main id="notice" >
    <h4>Applicability</h4>
    <p>This view applies only to OceanBase Database in Oracle mode. </p>
  </main>

## Columns

|     **Column**      |     **Type**     | **Nullable** |        **Description**        |
|-------------------|----------------|----------------|----------------------|
| INDEX_OWNER       | VARCHAR2(128)  | NO             | The owner of the index.             |
| INDEX_NAME        | VARCHAR2(128)  | NO             | The name of the index.                 |
| TABLE_OWNER       | VARCHAR2(128)  | NO             | The owner of the table to which the index belongs.            |
| TABLE_NAME        | VARCHAR2(128)  | NO             | The name of the table to which the index belongs.             |
| COLUMN_EXPRESSION | VARCHAR2(1000) | NO             | The expression of the function-based index. |
| COLUMN_POSITION   | NUMBER(38)     | NO             | The position of the column in the index.             |

## Sample query

Query information about all function-based indexes in the current tenant.

```shell
obclient [SYS]> SELECT * FROM SYS.DBA_IND_EXPRESSIONS;
```

The query result is as follows:

```shell
+-------------+----------------+-------------+------------+-------------------+-----------------+
| INDEX_OWNER | INDEX_NAME     | TABLE_OWNER | TABLE_NAME | COLUMN_EXPRESSION | COLUMN_POSITION |
+-------------+----------------+-------------+------------+-------------------+-----------------+
| SYS         | TBL2_FUNC_IDX1 | SYS         | TBL2_FUNC  | ("C1" + 1)        |               1 |
+-------------+----------------+-------------+------------+-------------------+-----------------+
1 row in set
```

## References

* Query information about all function-based indexes accessible to the current user: [ALL_IND_EXPRESSIONS](1500.all_ind_expressions-of-oracle-mode.md)

* Query information about all function-based indexes owned by the current user: [USER_IND_EXPRESSIONS](26400.user_ind_expressions-of-oracle-mode.md)

* For information about how to create and use function-based indexes, see [Create an index](../../../300.database-object-management/200.manage-object-of-oracle-mode/400.manage-indexes-of-oracle-mode/200.create-an-index-of-oracle-mode.md).