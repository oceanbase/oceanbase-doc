...
This view displays triggers on tables accessible to the current user. If the user has the `CREATE ANY TRIGGER` privilege, this view describes all triggers in the current tenant.
...
| ACTION_TYPE       | VARCHAR2(11)    | NO             | The type of the action. OceanBase Database supports only the PL/SQL type.                 |
