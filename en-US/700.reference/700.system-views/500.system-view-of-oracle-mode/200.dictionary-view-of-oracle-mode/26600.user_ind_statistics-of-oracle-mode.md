...
This view displays the optimizer statistics of indexes on tables owned by the current user.
...
| USER_STATS | VARCHAR2(3) | NO | Indicates whether statistics are directly entered by users. Valid values: <ul><li>YES</li><li>NO</li></ul> |
...
Query the optimizer statistics of the `TBL1_H_IDX1` index on tables owned by the current user.
...
* View the index statistics of all tables in the current tenant: [DBA_IND_STATISTICS](8600.dba_ind_statistics-of-oracle-mode.md)

* View the index statistics of tables accessible to the current user: [ALL_IND_STATISTICS](1700.all_ind_statistics-of-oracle-mode.md)
