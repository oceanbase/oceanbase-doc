...
This view displays the parameters of functions and procedures owned by the current user. The columns in this view are the same as those in the `ALL_ARGUMENTS` view, except that the `OWNER` column is omitted.
...
| OVERLOAD           | NUMBER        | YES            | The number of times the procedure or function is overloaded in the package source file.  |
...
| ORIGIN_CON_ID      | NUMBER(38)    | YES            | The current tenant ID.    |

## Sample query

Query the parameters of functions and procedures owned by the current user and display the first two records.
...
* Query the parameters of all functions and procedures available in the current tenant: [DBA_ARGUMENTS](6500.dba_arguments-of-oracle-mode.md)

* Query the parameters of all functions and procedures accessible by the current user: [ALL_ARGUMENTS](300.all_arguments-of-oracle-mode.md)