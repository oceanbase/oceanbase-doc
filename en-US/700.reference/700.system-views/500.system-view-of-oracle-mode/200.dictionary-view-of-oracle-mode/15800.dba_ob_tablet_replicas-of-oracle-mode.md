...
This view displays all tablet replicas of the current tenant. Each tablet replica is uniquely identified by the joint key: `<TABLET_ID, SVR_IP, SVR_PORT, LS_ID>`.
...
| COMPACTION_SCN  | NUMBER       | NO         | The snapshot version number of the data.                                                      |
| DATA_SIZE     | NUMBER       | NO         | The actual size of the data, in bytes.                                                     |
| REQUIRED_SIZE | NUMBER       | NO         | The size of the allocated storage space for the data, in bytes. Data is stored in macroblocks of 2 MB, so `REQUIRED_SIZE >= DATA_SIZE`. |

## Sample query

Query the tablet replicas of the current tenant and display the first 10 records.
