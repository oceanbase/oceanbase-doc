...
The DBA_TAB_COLUMNS view contains a row for each column in each table and view in the database.
...
| DATA_PRECISION       | NUMBER        | NO             | The decimal precision of the column if it is a numeric data type; the binary precision of the column if it is a floating-point data type; otherwise, NULL. |
...
| GLOBAL_STATS         | VARCHAR2(3)   | NO             | `YES` if the statistics are collected or incrementally maintained, otherwise `NO`.               |
...
* [ALL_TAB_COLUMNS](4800.all_tab_columns-of-oracle-mode.md): view all columns of all tables and views accessible to the current user.

* [USER_TAB_COLUMNS](30500.user_tab_columns-of-oracle-mode.md): view all columns of tables and views owned by the current user.