...
| ACCOUNT_STATUS              | VARCHAR2(32)   | NO   | The account status. Valid values: <ul><li>OPEN: the account is enabled. </li><li>EXPIRED: the account password has expired. </li><li>EXPIRED(GRACE): the account password has expired, but the password change grace period has not expired. </li><li>LOCKED(TIMED): the account is locked because the number of consecutive login failures exceeds the threshold. </li><li>LOCKED: the account is locked. </li><li>EXPIRED & LOCKED(TIMED): the account password has expired and the account is locked because the number of consecutive login failures exceeds the threshold. </li><li>EXPIRED(GRACE) & LOCKED(TIMED): the account is locked because the password has expired but the password change grace period has not expired, and the number of consecutive login failures exceeds the threshold. </li><li>EXPIRED & LOCKED: the account is in the password update period. </li><li>EXPIRED(GRACE) & LOCKED: the account is locked because the password has expired but the password change grace period has not expired. </li></ul>|
