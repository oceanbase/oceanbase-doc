...
| OPTIMIZER | VARCHAR2(4000) | NO | Index information (for example, physical rows, logical rows, number of rows accessed, type of statistics used, and version number of statistics) |
...
| POSITION | NUMBER(38) | NO | The position in the order among all operators with the same parent operator |
...
| ACCESS_PREDICATES | VARCHAR2(4000) | NO | Information about the expression that the operator needs to access |
...
| SPECIAL_PREDICATES | VARCHAR2(4000) | NO | Unique expression information of the current operator, such as Join Condition, GROUP EXPRS, and SORT EXPRS. |
