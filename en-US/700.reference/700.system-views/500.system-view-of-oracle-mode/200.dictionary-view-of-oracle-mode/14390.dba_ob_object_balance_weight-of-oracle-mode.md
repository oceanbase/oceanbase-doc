...
| SUBPARTITION_ID   | NUMBER       | NO             | The ID of the subpartition. If the partition weight is not set, the value is `NULL`. This view does not support subpartition weights.     |
...
Query all partitions with special weights in the current user tenant.
