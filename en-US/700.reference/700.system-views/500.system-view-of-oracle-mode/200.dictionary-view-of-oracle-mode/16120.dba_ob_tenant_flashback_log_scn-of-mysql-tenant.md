...
| SWITCHOVER_EPOCH    | NUMBER(38)                        | NO       | The `SWITCHOVER_EPOCH` value when the tenant role is switched to `NORMAL`. `SWITCHOVER_EPOCH` is a unique identifier for tenant role switching (Switchover/Failover) operations. |
| OP_TYPE              | VARCHAR2(64)                      | NO       | The operation type. Valid values: <ul><li>`FAILOVER_TO_PRIMARY`: This is the operation type for a failover. It is used to switch a standby tenant to a primary tenant when the primary tenant is unavailable. </li><li>`FLASHBACK_STANDBY_LOG`: This is the operation type for truncating standby tenant logs to a specified SCN. </li></ul> |
| FLASHBACK_LOG_SCN   | NUMBER(38)                        | NO       | The endpoint of the log truncation in the tenant. |
