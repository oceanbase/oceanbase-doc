|description||
|---|---|
| machine-translation ||
|keywords||
|dir-name||
|dir-name-en||
|tenant-type|Oracle Mode|
|slug|dba-ob-transfer-partition-tasks-history-oracle-mode|

# DBA_OB_TRANSFER_PARTITION_TASK_HISTORY

<main id="notice" type='explain'>
  <h4>Note</h4>
  <p>This view is available starting with V4.2.1 BP2. </p>
</main>

## Purpose

This view `DBA_OB_TRANSFER_PARTITION_TASK_HISTORY` displays the history of executed Transfer Partition tasks in the current tenant.

## Columns

| **Column** | **Type** | **Nullable** | **Description** |
| --- | --- | --- | --- |
| TASK_ID          | NUMBER(38)   | NO   | The unique identifier of the Transfer Partition task.   |
| CREATE_TIME      | TIMESTAMP(6) WITH LOCAL TIME ZONE | NO   | The creation time.   |
| FINISH_TIME      | TIMESTAMP(6) WITH LOCAL TIME ZONE | NO   | The completion time of the task, including successful and canceled tasks.   |
| TABLE_ID         | NUMBER(38)   | NO   | The table ID where the partition to be transferred resides.   |
| OBJECT_ID        | NUMBER(38)   | NO   | The `OBJECT_ID` of the partition to be transferred.   |
| DEST_LS          | NUMBER(38)   | NO   | The log stream on the destination.   |
| BALANCE_JOB_ID   | NUMBER(38)   | NO   | The ID of the load balancing job associated with the task.   |
| TRANSFER_TASK_ID | NUMBER(38)   | NO   | The ID of the Transfer task associated with the task.   |
| STATUS           | VARCHAR2(64) | NO   | The task status:<ul><li>COMPLETED: The task is completed. </li><li>FAILED: The task failed. </li></ul>   |
| COMMENT          | CLOB         | YES  | The comment.   |

## Sample query

```shell
obclient> SELECT * FROM SYS.DBA_OB_TRANSFER_PARTITION_TASK_HISTORY;
```

The query result is as follows:

```shell
+---------+----------------------------+----------------------------+----------+-----------+---------+----------------+------------------+-----------+---------------------------------+
| TASK_ID | CREATE_TIME                | FINISH_TIME                | TABLE_ID | OBJECT_ID | DEST_LS | BALANCE_JOB_ID | TRANSFER_TASK_ID | STATUS    | COMMENT                         |
+---------+----------------------------+----------------------------+----------+-----------+---------+----------------+------------------+-----------+---------------------------------+
|       1 | 2023-11-29 16:50:55.155765 | 2023-11-29 16:51:34.000000 |   500012 |    500014 |    1003 |          26506 |                2 | COMPLETED | Partition transfer to dest LS   |
|       2 | 2023-11-29 16:55:25.391784 | 2023-11-29 16:55:28.000000 |   500012 |    500014 |    1003 |             -1 |               -1 | FAILED    | Partition is already in dest LS |
|       3 | 2023-11-30 14:28:20.647541 | 2023-11-30 14:28:57.000000 |   500012 |    500014 |    1003 |         416470 |                6 | COMPLETED | Partition transfer to dest LS   |
|       4 | 2023-12-01 11:21:11.357272 | 2023-12-01 11:21:55.000000 |   500012 |    500014 |    1003 |         737367 |               10 | COMPLETED | Partition transfer to dest LS   |
+---------+----------------------------+----------------------------+----------+-----------+---------+----------------+------------------+-----------+---------------------------------+
4 rows in set (0.002 sec)
```

## References

* View the Transfer Partition tasks being processed in the current tenant: [DBA_OB_TRANSFER_PARTITION_TASKS](16300.dba_ob_transfer_partition_tasks-of-oracle-mode.md)

* For more information about how to transfer a partition, see [Transfer a partition](../../../../600.manage/350.load-balancing-manage/200.transfer-partition-management.md).

* For more information about data load balancing, see [Data load balancing](../../../../600.manage/350.load-balancing-manage/100.load-balancing-overview.md).