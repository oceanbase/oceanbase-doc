|description||
|---|---|
| machine-translation ||
|keywords||
|dir-name||
|dir-name-en||
|tenant-type|Oracle Mode|
|slug|user_segments-oracle-mode|

# USER_SEGMENTS

<main id="notice" type='explain'>
<h4>Note</h4>
<p>This view is available starting with V2.2.30. </p>
</main>

## Purpose

This view displays the physical storage allocation of segments owned by the current user.

  <main id="notice" >
    <h4>Applicability</h4>
    <p>This view is applicable only to OceanBase Database in Oracle mode. </p>
  </main>

## Columns

|     **Column**     |    **Type**     | **Nullable** |                    **Description**                     |
|------------------|---------------|----------------|-----------------------------------------------|
| SEGMENT_NAME     | VARCHAR2(128) | NO             | The name of the segment.                                      |
| PARTITION_NAME   | VARCHAR2(128) | NO             | The name of the partition. If the object is not partitioned, this column is NULL.                       |
| SEGMENT_TYPE     | VARCHAR2(18)  | NO             | The type of the segment.                                           |
| SEGMENT_SUBTYPE  | VARCHAR2(10)  | NO             | This column is not supported. It is NULL by default.         |
| TABLESPACE_NAME  | VARCHAR2(30)  | NO             | This column is not supported. It is NULL by default.              |
| BYTES            | NUMBER(38)    | NO             | The size of the segment, in bytes.                                  |
| BLOCKS           | NUMBER(38)    | NO             | The number of blocks in the segment.                                          |
| EXTENTS          | NUMBER        | NO             | This column is not supported. It is NULL by default.                |
| INITIAL_EXTENT   | NUMBER        | NO             | This column is not supported. It is NULL by default. |
| NEXT_EXTENT      | NUMBER        | NO             | This column is not supported. It is NULL by default.   |
| MIN_EXTENTS      | NUMBER        | NO             | This column is not supported. It is NULL by default.           |
| MAX_EXTENTS      | NUMBER        | NO             | This column is not supported. It is NULL by default.           |
| MAX_SIZE         | NUMBER        | NO             | This column is not supported. It is NULL by default.           |
| RETENTION        | VARCHAR2(7)   | NO             | This column is not supported. It is NULL by default.      |
| MINRETENTION     | NUMBER        | NO             | This column is not supported. It is NULL by default.    |
| PCT_INCREASE     | NUMBER        | NO             | This column is not supported. It is NULL by default.    |
| FREELISTS        | NUMBER        | NO             | This column is not supported. It is NULL by default.            |
| FREELIST_GROUPS  | NUMBER        | NO             | This column is not supported. It is NULL by default.             |
| BUFFER_POOL      | VARCHAR2(7)   | NO             | The buffer pool used for the segment blocks.                                      |
| FLASH_CACHE      | VARCHAR2(7)   | NO             | The database smart flash cache hint used for the segment blocks.                              |
| CELL_FLASH_CACHE | VARCHAR2(7)   | NO             | The cell flash cache hint used for the segment blocks.                                 |
| CELLMEMORY              | varchar(8)      | NO          | This column is not supported. It is NULL by default.                   |
| INMEMORY_PRIORITY     | varchar(8)      | NO          | This column is not supported. It is NULL by default.                     |
| INMEMORY_DISTRIBUTE   | varchar(15)     | NO          | This column is not supported. It is NULL by default.                        |
| INMEMORY_DUPLICATE    | varchar(13)     | NO          | This column is not supported. It is NULL by default.         |
| INMEMORY_COMPRESSION  | varchar(17)     | NO          | This column is not supported. It is NULL by default.        |
| CELLMEMORY            | varchar(24)     | NO          | This column is not supported. It is NULL by default.                         |

## Sample query

Query the physical storage allocation of the `T_SUBPART` table owned by the current user.

```shell
obclient [SYS]> SELECT SEGMENT_NAME, PARTITION_NAME, SEGMENT_TYPE, BYTES, BLOCKS FROM SYS.DBA_SEGMENTS WHERE SEGMENT_NAME='T_SUBPART';
```

The query result is as follows:

```shell
+--------------+----------------+--------------------+---------+--------+
| SEGMENT_NAME | PARTITION_NAME | SEGMENT_TYPE       | BYTES   | BLOCKS |
+--------------+----------------+--------------------+---------+--------+
| T_SUBPART    | P1SP0          | TABLE SUBPARTITION | 2097152 |  16384 |
| T_SUBPART    | P1SP1          | TABLE SUBPARTITION | 2097152 |  16384 |
| T_SUBPART    | P1SP2          | TABLE SUBPARTITION | 2097152 |  16384 |
| T_SUBPART    | P1SP3          | TABLE SUBPARTITION | 2097152 |  16384 |
| T_SUBPART    | P2SP0          | TABLE SUBPARTITION | 2097152 |  16384 |
| T_SUBPART    | P2SP1          | TABLE SUBPARTITION | 2097152 |  16384 |
| T_SUBPART    | P2SP2          | TABLE SUBPARTITION | 2097152 |  16384 |
| T_SUBPART    | P2SP3          | TABLE SUBPARTITION | 2097152 |  16384 |
| T_SUBPART    | P3SP0          | TABLE SUBPARTITION | 2097152 |  16384 |
| T_SUBPART    | P3SP1          | TABLE SUBPARTITION | 2097152 |  16384 |
| T_SUBPART    | P3SP2          | TABLE SUBPARTITION | 2097152 |  16384 |
| T_SUBPART    | P3SP3          | TABLE SUBPARTITION | 2097152 |  16384 |
+--------------+----------------+--------------------+---------+--------+
12 rows in set
```

## References

Query [DBA_SEGMENTS](19500.dba_segments-of-oracle-mode.md) to view the physical storage allocation of all segments in the current tenant.