...
This view displays the partition information of the partition indexes that belong to the current user.
...
| DEF_PARAMETERS            | VARCHAR2(1000) | NO             | This column is not supported and its default value is NULL.                  |
| INTERVAL                  | VARCHAR2(1000) | NO             | This column is reserved for future use. The default value is `NO`. |
| AUTOLIST                  | VARCHAR2(3)    | NO         | Indicates whether the local index is automatically partitioned: <ul><li>YES</li><li>NO</li></ul>    |
| INTERVAL_SUBPARTITION     | VARCHAR2(1000) | NO         | This column is not supported at present.  The column is set to NULL by default.     |
| AUTOLIST_SUBPARTITION     | VARCHAR2(1000) | NO         | This column is not supported. It is set to NULL by default.     |

## Sample query

Query the partition information of the partitioned index `TBL1_H_IDX1` owned by the current user.
...
* Query the partition information of all partitioned indexes accessible by the current user: [ALL_PART_INDEXES](2700.all_part_indexes-of-oracle-mode.md)