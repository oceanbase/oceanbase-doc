...
The `USER_PART_COL_STATISTICS` view displays column statistics and histogram information for primary partitions of tables that belong to the current user.
...
| GLOBAL_STATS   | VARCHAR2(3)   | YES            | <li> `YES`: indicates that statistics are directly collected.   <li> `NO`: indicates that statistics are derived from statistics of subpartitions or that statistics are not collected.    |
