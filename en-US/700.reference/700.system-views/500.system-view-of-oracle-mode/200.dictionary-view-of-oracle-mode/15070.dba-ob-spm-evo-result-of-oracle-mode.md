...
| TYPE | VARCHAR2(32) | NO | Record type:<ul><li>`OnlineEvolve`: Indicates that the online evolution mode records the results of evolution tasks. The following fields are valid only when their type is the current value.</li><li>`FirstBaseline`: Indicates that the SQL statement generates the first baseline.</li><li>`UnReproducible`: Indicates that all baselines for the current SQL cannot be reproduced. </li><li>`BaselineFirst`: Indicates that a new plan was generated in baseline-first mode, but the baseline plan was restored and used.</li><li>`BestBaseline`: Indicates that the optimal baseline plan was generated and used.</li><li>`FixedBaseline`: Indicates that a fixed baseline plan is used.</li></ul> |
| START_TIME | TIMESTAMP(6) WITH LOCAL TIME ZONE | YES | The time when the evolution task starts. |
| END_TIME | TIMESTAMP(6) WITH LOCAL TIME ZONE | YES | The end time of the evolution task. |
| STATUS | VARCHAR2(7) | YES | Status of the evolution task: <ul><li>`success`: The evolution task has ended normally. </li><li>`timeout`: The evolution task timed out. In this case, the plan that executed better during the evolution is chosen. |
| NEW_PLAN_BETTER | NUMBER(38) | YES | Whether the evolution plan is better. |
| EVO_PLAN_EXEC_COUNT | NUMBER(38) | YES | The number of times the evolution plan is executed during evolution. |
| EVO_PLAN_CPU_TIME | NUMBER(38) | YES | The average CPU overhead of the evolution plan during evolution. |
| BASELINE_EXEC_COUNT | NUMBER(38) | YES | Number of times the baseline plan is executed during the evolution. |
| BASELINE_CPU_TIME | NUMBER(38) | YES | The average CPU cost of the baseline plan during the evolution period. |
