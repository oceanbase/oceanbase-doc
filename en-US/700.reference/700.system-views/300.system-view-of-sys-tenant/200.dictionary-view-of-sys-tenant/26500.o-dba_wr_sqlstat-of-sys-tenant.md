...
| MODULE | varchar(64) | YES | The application module when this statement was first parsed. You can pass in the`DBMS_APPLICATION_INFO.SET_MODULE`Package settings |
| ACTION | varchar(64) | YES | The application action when this statement is parsed for the first time. The value of this parameter is set by the`DBMS_APPLICATION_INFO.SET_ACTION`Package settings |
...
View the basic performance statistics of SQL statements executed by the current tenant, displaying the first record.
