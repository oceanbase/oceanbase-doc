...
| STATUS | varchar(64) | NO | The status of the task. Valid values: <ul><li>`INIT`: The table-level import task is initialized. </li> <li>`DOING`: The table-level import task is being executed. </li> <li>`FINISH`: The import task is completed. </li></ul>  |
| START_TIMESTAMP | timestamp(6) | NO | The current time of the tenant when the task started. |
| COMPLETION_TIMESTAMP | timestamp(6) | NO | The current time of the tenant when the task was completed. |
...
| RESULT | varchar(64) | YES | The result of the task. Valid values: <ul><li>`SUCCESS`: The import task is executed successfully. </li><li>`FAIL`: The import task failed. </li></ul>  |
| COMMENT | longtext | YES | The remarks. |

## Sample query

After a table restore task is completed, you can query this view in the sys tenant to view the history of table imports.
