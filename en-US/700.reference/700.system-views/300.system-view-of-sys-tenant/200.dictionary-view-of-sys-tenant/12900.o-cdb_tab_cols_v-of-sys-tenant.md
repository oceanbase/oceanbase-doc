...
This view displays column information for tables and views of all tenants. It is consistent with the `CDB_TAB_COLS` view, but also includes the `NOTES` column, which is not currently used.
...
| CHARACTER_SET_NAME   | varchar(44)     | NO         | The character set name. Valid values: <ul><li> `CHAR_CS`   </li><li> `NCHAR_CS` </li></ul>      |
...
| CHAR_LENGTH          | bigint(20)      | NO         | The length of the column displayed as characters. This value applies only to the following data types:<ul><li> `CHAR`   </li><li> `VARCHAR2`   </li><li> `NCHAR`   </li><li> `NVARCHAR2`  </li></ul> |
| CHAR_USED            | varchar(1)      | NO         | Indicates whether the column uses `BYTE` length semantics (`B`) or `CHAR` length semantics (`C`) or if the data type is not one of the following (NULL):<ul><li> `CHAR`   </li><li> `VARCHAR2`   </li><li> `NCHAR`   </li><li> `NVARCHAR2` </li></ul> |
