...
|tenant-type||
|machine-translation||
|slug|oceanbase.cdb_ob_recover_table_job_history-o-cdb_ob_recover_table_job_history-of-sys-tenant|
...
This view displays the history of table-level restore tasks.
...
| STATUS | varchar(64) | NO | The task status. Valid values: <ul><li>COMPLETED: The task completed. </li><li>FAILED: The task failed. </li></ul> |
...
| IMPORT_ALL | tinyint(4) | NO | Indicates whether to import all tables under the tenant. This value is 1 only when the user specifies `RECOVER TABLE *.*`. In other cases, this value is 0. |
...
| EXTERNAL_KMS_INFO | varchar(4096) | YES | If the source tenant has key management configured, specify this parameter in the session. |
...
After a table-level restore task is completed, the system tenant can query the history of the table-level restore task.
