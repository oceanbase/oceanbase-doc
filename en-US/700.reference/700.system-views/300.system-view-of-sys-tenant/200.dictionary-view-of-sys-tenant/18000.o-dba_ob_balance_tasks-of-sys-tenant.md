...
| CREATE_TIME | timestamp(6) | YES | The time when the event was created. |
...
| BALANCE_STRATEGY         | varchar(64)   | YES | The balancing strategy corresponding to the Balance Task. Valid values: <ul><li>`LS balance by migrate`: migration balancing. </li><li>`LS balance by alter`: log stream attribute modification balancing. </li><li>`LS balance by expand`: scaling-up balancing. </li><li>`LS balance by shrink`: scaling-down balancing. </li><li>`partition balance`: partition balancing. </li></ul> <main id="notice" type='explain'><h4>Note</h4><p>This field was introduced in V4.3.2 of V4.3.x and V4.2.4 of V4.2.x. </p></main>
