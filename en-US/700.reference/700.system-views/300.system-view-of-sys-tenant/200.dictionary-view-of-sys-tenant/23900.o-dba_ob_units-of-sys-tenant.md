...
| TENANT_ID             | bigint(20)   | NO         | The ID of the tenant to which the unit belongs. Valid values:<li> `NULL`: indicates that the unit does not belong to any tenant.<li>Other valid values: indicates the ID of the tenant to which the unit belongs. |
...
| CREATE_TIME           | timestamp(6) | YES        | The time when the time was created. |
...
| MIGRATE_FROM_SVR_IP   | varchar(46)  | NO         | The IP address of the source OBServer node for migration. Valid values:<li> `NULL`: The unit is not being migrated.<li> Other valid values: The unit is being migrated. The value is the IP address of the OBServer node. |
| MIGRATE_FROM_SVR_PORT | bigint(20)   | NO         | The port number of the source OBServer node for migration. Valid values:<li> `NULL`: The unit is not being migrated.<li> Other valid values: The unit is being migrated. The value is the port number of the OBServer node. |
| MANUAL_MIGRATE        | varchar(3)   | NO         | The migration type of the unit. Valid values:<li> `NULL`: The unit is not being migrated.<li> `YES`: The unit migration is manually initiated.<li> `NO`: The unit migration is automatically triggered. |
