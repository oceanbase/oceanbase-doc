...
| TYPE | varchar(64) | NO | The cleanup type. Valid value: <ul><li>`DELETE OBSOLETE BACKUP`: indicates to clean expired backup data.</li><li>`DELETE BACKUP ALL`: indicates to clean all backup data in the specified path.</li><li>`DELETE BACKUPSET`: indicates to clean the specified backup set.</li><li>`DELETE ARCHIVELOG_PIECE`: indicates to clean the specified archive log piece.</li><li>`DELETE BACKED UP ARCHIVELOG PIECE`: indicates to clean the source archive log piece initiated by the `DELETE INPUT` option of the archive log backup.</li></ul> |
...
| STATUS | varchar(64) | NO | The status of the job. Valid values: <li>`COMPLETED`<li>`FAILED`<li>`CANCELED` |
