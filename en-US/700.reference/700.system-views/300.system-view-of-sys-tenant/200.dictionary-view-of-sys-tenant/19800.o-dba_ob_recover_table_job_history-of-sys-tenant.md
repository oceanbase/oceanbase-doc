...
|tenant-type||
|machine-translation||
|slug|oceanbase.dba_ob_recover_table_job_history|
...
| IMPORT_ALL | tinyint(4) | NO | Indicates whether to import all tables under the tenant. The value is 1 only when the `RECOVER TABLE *.*` statement is specified. In other cases, the value is 0. |
...
Query the results of table-level restore tasks as the sys tenant.
