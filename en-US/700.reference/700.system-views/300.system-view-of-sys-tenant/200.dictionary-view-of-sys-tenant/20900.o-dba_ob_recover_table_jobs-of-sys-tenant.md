...
| STATUS | varchar(64) | NO | The task status. Valid values: <ul><li>PREPARE: The task is initializing.  </li><li>RECOVERING: The task is being restored. </li><li>RESTORE_AUX_TENANT: The auxiliary tenant is being restored. </li><li>PRECHECK_IMPORT: The task is performing a pre-import check. </li><li>GEN_IMPORT_JOB: The task is initializing the import job. </li><li>IMPORTING: The task is importing tables. </li><li> CANCELING: The task is being canceled.</li><li>COMPLETED: The task is completed. </li><li>FAILED: The task failed. </li></ul> |
...
| IMPORT_ALL | tinyint(4) | NO | Indicates whether to import all tables under the tenant. The value is 1 only when the user specifies `RECOVER TABLE *.*`. In other cases, the value is 0. |
...
| REMAP_TABLEGROUP_LIST | longtext | YES | The list of remapped table groups. You can remap tables from the original table group to the target table group. Format: tg1:new_tg1, tg2:new_tg2 |
| REMAP_TABLESPACE_LIST | longtext | YES | The list of remapped tablespaces. You can remap tables from the original tablespace to the target tablespace. Format: ts1:new_ts1, ts2:new_ts2 |
...
Query the information about table-level restore tasks in the sys tenant during a table-level restore.
