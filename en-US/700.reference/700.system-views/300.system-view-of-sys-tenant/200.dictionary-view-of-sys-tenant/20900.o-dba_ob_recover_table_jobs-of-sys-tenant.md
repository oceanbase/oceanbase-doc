...
This view displays information about table-level recovery tasks.
...
| STATUS | varchar(64) | NO | The task status. Valid values: <ul><li>PREPARE: The task is initializing.  </li><li>RECOVERING: The task is recovering. </li><li>RESTORE_AUX_TENANT: The auxiliary tenant is being restored. </li><li>PRECHECK_IMPORT: Precheck is being performed before the import. </li><li>GEN_IMPORT_JOB: The import task is being initialized. </li><li>IMPORTING: The table is being imported. </li><li> CANCELING: The task is being canceled.</li><li>COMPLETED: The task is completed. </li><li>FAILED: The task failed. </li></ul> |
...
| EXTERNAL_KMS_INFO | varchar(4096) | YES | If the source tenant has key management configured, set this parameter in the session. |
...
| REMAP_TABLEGROUP_LIST | longtext | YES | The list of table groups to be remapped. This parameter specifies the original and target table groups. Format: tg1:new_tg1, tg2:new_tg2 |
| REMAP_TABLESPACE_LIST | longtext | YES | The list of tablespaces to be remapped. This parameter specifies the original and target tablespaces. Format: ts1:new_ts1, ts2:new_ts2 |
...
In the sys tenant, query the information about table-level recovery tasks during table-level recovery.
...
For more information about how to view the progress of a table-level recovery task, see [View the table-level recovery progress](../../../../600.manage/600.backup-and-recovery/700.recover-table/400.view-the-table-recovery-progress.md).