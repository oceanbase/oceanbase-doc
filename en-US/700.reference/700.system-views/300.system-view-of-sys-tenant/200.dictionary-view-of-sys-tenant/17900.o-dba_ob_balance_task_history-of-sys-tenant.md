...
This view displays the history of load balancing tasks executed in a tenant.
...
* View all load balancing tasks (tasks) at the log stream level that are being executed by all tenants: [CDB_OB_BALANCE_TASKS](8000.o-cdb_ob_balance_tasks-of-sys-tenant.md)

* View all load balancing jobs (jobs) at the log stream level for all tenants:
...
* View all load balancing tasks (tasks) at the tablet level for all tenants:
...
* View all Transfer Partition tasks (tasks) for all tenants:
