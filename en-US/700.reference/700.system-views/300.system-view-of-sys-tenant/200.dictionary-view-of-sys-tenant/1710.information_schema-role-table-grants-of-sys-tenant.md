...
|slug|information_schema-role-table-grants-sys-tenant|
...
The `role_table_grants` view displays the table-level privileges granted to activated roles for the current session and the table-level privileges granted to other roles by these roles.
...
| TABLE_CATALOG  | varchar(3)   |  NO     | The name of the catalog to which the role is granted. The value is always def.      |
...
| PRIVILEGE_TYPE | varchar(90)  |  NO     | The granted privilege. The value can be any privilege granted at the table level.  |
| IS_GRANTABLE   | varchar(3)   |  NO     | Whether the privileges of the role can be granted to other users.     |
...
6. View the table-level privileges granted to activated roles for the current session.
