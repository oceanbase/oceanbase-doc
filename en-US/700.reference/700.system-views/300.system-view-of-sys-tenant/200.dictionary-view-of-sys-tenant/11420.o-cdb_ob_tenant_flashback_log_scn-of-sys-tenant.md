...
| TIMESTAMP         | timestamp(6) | NO          | The time when the log truncation operation was performed, in microseconds.                   |
...
| OP_TYPE           | varchar(64)  | NO          | The operation type. Valid values: <ul><li>`FAILOVER_TO_PRIMARY`: This is a failover operation. When the primary tenant is unavailable, the standby tenant is switched to the primary tenant. </li><li>`FLASHBACK_STANDBY_LOG`: This is a standby log truncation operation. It truncates the standby tenant's logs to the specified SCN. </li></ul> |
| FLASHBACK_LOG_SCN | bigint(20) unsigned | NO         | The SCN at which the tenant's logs are truncated from the end.             |
