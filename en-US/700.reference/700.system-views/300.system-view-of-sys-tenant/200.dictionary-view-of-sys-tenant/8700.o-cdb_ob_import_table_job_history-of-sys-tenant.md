...
| FINISH_TIMESTAMP | timestamp(6) | NO | The current time of the tenant when the task completes. |
...
| IMPORT_ALL | tinyint(4) | NO | Indicates whether to import all tables under the tenant. This value is 1 only when the `RECOVER TABLE *.*` clause is specified. Otherwise, this value is 0. |
...
After a table-level restore task is completed, view the history records of cross-tenant import tasks in the sys tenant.
