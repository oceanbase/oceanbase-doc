...
* The task records of the `sys` tenant mainly record the progress of table-level restore for the target user tenant. A task is considered complete when the entire table-level restore task for the target tenant is finished.

* The task records of the target user tenant itself, including the recovery progress of auxiliary tenants and the progress of table import tasks. The task is considered complete when both the recovery task and the table import task for the auxiliary tenant are finished.
...
| IMPORT_ALL | tinyint(4) | NO | Whether to import all tables in the entire tenant. This parameter is valid only when you specify`RECOVER TABLE *.*`1, and 0 in other scenarios. |
...
For detailed operations and instructions on viewing the progress of a table-level restore task, see [View the progress of a table-level restore](../../../../600.manage/600.backup-and-recovery/700.recover-table/400.view-the-table-recovery-progress.md).
