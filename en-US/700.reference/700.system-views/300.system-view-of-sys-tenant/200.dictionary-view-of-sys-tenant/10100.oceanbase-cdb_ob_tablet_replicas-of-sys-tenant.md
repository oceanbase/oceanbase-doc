...
|dir-name-en||
|tenant-type||
|slug|oceanbase-cdb_ob_tablet_replicas-of-sys-tenant|

# oceanbase.CDB_OB_TABLET_REPLICAS

## Purpose

The `oceanbase.CDB_OB_TABLET_REPLICAS` view provides information about all tablet replicas in the current tenant. Each tablet replica is uniquely identified by the joint key string of `<TENANT_ID, TABLET_ID, SVR_IP, SVR_PORT, LS_ID>`.
