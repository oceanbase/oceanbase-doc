...
| STATUS | varchar(64) | NO | The status of the task. Valid values: <ul><li> INIT: the task is being initialized. </li><li>IMPORT_TABLE: importing tables. </li><li>RECONSTRUCT_REF_CONSTRAINT: reconstructing reference constraints. </li><li>CANCELING: the task is being canceled. </li><li>IMPORT_FINISH: the task has completed. </li></ul> |
| IMPORT_ALL | tinyint(4) | NO | Indicates whether to import all tables in the tenant. This value is 1 only when the `RECOVER TABLE *.*` clause is specified. Otherwise, the value is 0. |
