...
|tenant-type||
|machine-translation||
|slug|oceanbase.cdb_ind_subpartitions|
...
The `oceanbase.CDB_IND_SUBPARTITIONS` view displays the information about all index subpartitions in the database.
...
In the sys tenant, query the subpartitions (partitioned partitions) of the index in the `tbl2_f_rl` table of the tenant with the tenant ID of 1002.
