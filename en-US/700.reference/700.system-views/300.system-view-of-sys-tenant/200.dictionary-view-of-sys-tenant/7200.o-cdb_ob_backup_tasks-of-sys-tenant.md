...
|tenant-type||
|machine-translation||
|slug|oceanbase.cdb_ob_backup_tasks-o-cdb_ob_backup_tasks-of-sys-tenant|
...
The `oceanbase.CDB_OB_BACKUP_TASKS` view displays the backup tasks in all tenants in the cluster. A task record is generated for each backup set. The task records and job records are in one-to-one mapping.
...
| DATA_PROGRESS                 | decimal(26,2)  | NO              | The data backup progress, which is calculated by using the following formula: Number of macroblocks that have been backed up/Total number of macroblocks. The value is rounded off to the second decimal place. For example, the value `87.65` indicates that the current backup progress is 87.65%.  <main id="notice" type='explain'><h4>Note</h4><p>This column was introduced in OceanBase Database V4.2.1 BP3. </p></main> |

## Sample query

Query the backup progress of backup tasks of all tenants in the cluster.
