...
| JOB_STYLE               | varchar(17)   | YES  |This column is always set to `REGULAR`.|
...
| THIS_DATE               | datetime(6)   | YES  |The start time of the job execution (calculated from the time when the RPC was sent).|
...
Execute the following statement to query all running jobs in the current tenant.
...
RESOURCE_CONSUMER_GROUP:NULL
    ...
    NULL
    CREDENTIAL_NAME:NULL
    THIS_DATE:2026-02-09 14:51:41.032907
