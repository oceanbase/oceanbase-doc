...
| SEQ_IN_INDEX  | bigint(20) unsigned | NO | The serial number of the index. |
...
| COMMENT       | varchar(16)  | NO | The comments or description of the index.<main id="notice" type='explain'><h4>Note</h4><p>The value of this column has been changed from `NULL` to the actual index status since OceanBase Database V4.3.1. </p></main> |
| INDEX_COMMENT | text         | NO | The index comment. |
| IS_VISIBLE    | varchar(3)   | NO | Indicates whether the index is visible. |
| EXPRESSION    | varbinary(262144) | NO | This column is used only for compatibility with MySQL. The value of this column is `NULL`.<main id="notice" type='explain'><h4>Note</h4><p>This column is available in OceanBase Database V4.2.1 and later. </p></main> |
