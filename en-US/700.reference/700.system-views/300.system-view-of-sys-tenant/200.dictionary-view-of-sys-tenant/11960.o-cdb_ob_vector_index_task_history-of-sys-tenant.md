|description||
|---|---|
| machine-translation ||
|keywords||
|dir-name||
|dir-name-en||
|tenant-type||

# oceanbase.CDB_OB_VECTOR_INDEX_TASK_HISTORY

<main id="notice" type='explain'>
<h4>Note</h4>
<p>This view is available starting with V4.4.1. </p>
</main>

## Purpose

<<<<<<< HEAD
The `CDB_OB_VECTOR_INDEX_TASK_HISTORY` view displays the historical information of all vector index partition rebuild tasks for all tenants in the cluster.

## Columns

| Column                   | Data type      | Nullable | Description |
=======
The `CDB_OB_VECTOR_INDEX_TASK_HISTORY` view displays the historical information of vector index partition rebuild tasks for all tenants in the cluster.

## Columns

| Column | Type | Nullable | Description |
>>>>>>> V4.6.0
|---------------------------|---------------|------|----|
| TENANT_ID	                | bigint(20)	| YES   | The tenant ID. |
| TABLE_ID	                | bigint(20)	| YES   | The table ID. |
| TABLET_ID	                | bigint(20)	| YES   | The partition ID. |
| TASK_ID	                | bigint(20)	| YES   | The task ID. |
<<<<<<< HEAD
| START_TIME	                | timestamp(6)	| YES   | The task start time. |
| MODIFY_TIME	                | timestamp(6)	| YES   | The task end time. |
| TRIGGER_TYPE	                | varchar(16)	| NO   | The task trigger type. <ul><li><code>0</code>: automatically triggered</li><li><code>1</code>: manually triggered</li></ul> |
| STATUS	                | varchar(16)	| NO   | The task status. <ul><li><code>0</code>: task preparation in progress</li><li><code>1</code>: task execution in progress</li><li><code>2</code>: task paused</li><li><code>3</code>: task completed</li></ul> |
| TASK_TYPE	                | bigint(20)	| NO   | The task type. <ul><li><code>0</code>: asynchronous build</li><li><code>1</code>: index partition rebuild</li></ul>      | |
| TASK_SCN	                | bigint(20)	| NO   | The task version number. |
| RET_CODE	                | bigint(20)	| NO   | The task status when the task was completed. <ul><li><code>0</code>: succeeded</li><li><code>Non-zero</code>: failed</li></ul> |
=======
| START_TIME	                | timestamp(6)	| YES   | The start time of the task. |
| MODIFY_TIME	                | timestamp(6)	| YES   | The end time of the task. |
| TRIGGER_TYPE	                | varchar(16)	| NO   | The type of the task trigger. Valid values: <ul><li><code>0</code>: automatic trigger</li><li><code>1</code>: manual trigger</li></ul> |
| STATUS	                | varchar(16)	| NO   | The status of the task. Valid values: <ul><li><code>0</code>: preparing</li><li><code>1</code>: in progress</li><li><code>2</code>: paused</li><li><code>3</code>: completed</li></ul> |
| TASK_TYPE	                | bigint(20)	| NO   | The type of the task. Valid values: <ul><li><code>0</code>: asynchronous construction</li><li><code>1</code>: index partition rebuild</li></ul>      | |
| TASK_SCN	                | bigint(20)	| NO   | The version number of the task. |
| RET_CODE	                | bigint(20)	| NO   | The status of the task when it is completed. Valid values: <ul><li><code>0</code>: successful</li><li><code>Non-zero</code>: failed</li></ul> |
>>>>>>> V4.6.0
| TRACE_ID	                | varchar(512)	| NO   | The task execution trace ID. |

## Sample query

In the sys tenant, query the historical information of vector index performance optimization tasks for all tenants.

```sql
SELECT * FROM oceanbase.CDB_OB_VECTOR_INDEX_TASK_HISTORY ORDER BY MODIFY_TIME DESC\G;
```

The query result is as follows:

```shell
*************************** 1. row ***************************
    TABLE_ID: 500180
   TABLET_ID: 1152921504606847128
     TASK_ID: 10091
 CREATE_TIME: 2026-03-25 23:20:31.475323
 MODIFY_TIME: 2026-03-25 23:20:32.484323
TRIGGER_TYPE: AUTO
      STATUS: FINISHED
   TASK_TYPE: 6
    TASK_SCN: 1774452031481903000
    RET_CODE: 0
    TRACE_ID: Y2C277F000001-00064DDA40B0F2AE-0-1
   TASK_INFO: {"merge_segments_cnt":2,"merge_segments":[{"scn":1774451941331810002,"start_key":"1152921504606847129_1774451941331810002_hnsw_data_part00000","end_key":"1152921504606847129_1774451941331810002_hnsw_data_part00000","index_type":"HNSW","vector_cnt":"1005","mem_used":"252512","mem_hold":"252512","min_vid":"11","max_vid":"10005"},{"scn":1774452021463248000,"start_key":"1152921504606847129_1774452021463248000_hgraph_data_part00000","end_key":"1152921504606847129_1774452021463248000_hgraph_data_part00002","index_type":"HGRAPH","vector_cnt":"1000","mem_used":"8484032","mem_hold":"8484032","min_vid":"1011","max_vid":"2010"}],"result_segments":[{"scn":1774452031481903000,"start_key":"1152921504606847129_1774452031481903000_hnsw_data_part00000","end_key":"1152921504606847129_1774452031481903000_hnsw_data_part00000","index_type":"HNSW","vector_cnt":"2005","mem_used":"803354","mem_hold":"0","min_vid":"11","max_vid":"10005"}]}
   EXEC_ADDR: NULL
    PRIORITY: 0
  START_TIME: NULL
    END_TIME: NULL
     ERR_MSG: NULL
1 row in set
```

## References

* [DBA_OB_VECTOR_INDEX_TASK_HISTORY](24160.o-dba_ob_vector_index_task_history-of-sys-tenant.md)
<<<<<<< HEAD
* For information about the relationship between HNSW vector index dump, major compaction, and task types, see [Vector index principles](../../../100.oceanbase-database-concepts/400.database-objects/200.database-objects-of-mysql-mode/300.index-of-mysql-mode/330.vector-indexes-of-mysql-mode-in-concepts.md).
* For information about O&M and manual triggering, see [Monitor and maintain vector indexes](../../../../640.ob-vector-search/200.ob-vector-index/600.vector-index-monitor-and-maintain.md).
=======
* For the relationship between HNSW vector index dump, major compaction, and task types, see [Vector index principles](../../../../100.oceanbase-database-concepts/400.database-objects/200.database-objects-of-mysql-mode/300.index-of-mysql-mode/330.vector-indexes-of-mysql-mode-in-concepts.md).
* For O&M and manual trigger information, see [Monitor and maintain vector indexes](../../../../../640.ob-vector-search/200.ob-vector-index/600.vector-index-monitor-and-maintain.md).
>>>>>>> V4.6.0
