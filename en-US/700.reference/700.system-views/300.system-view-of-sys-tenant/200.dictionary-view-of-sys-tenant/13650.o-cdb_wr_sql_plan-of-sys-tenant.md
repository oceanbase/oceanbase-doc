...
The `CDB_WR_SQL_PLAN` view displays the [GV$OB_SQL_PLAN](../300.performance-view-of-sys-tenant/3700.gv-ob_sql_plan-of-sys-tenant.md) data collected by WR.
...
| OPTIMIZER | varchar(4000) | NO | Index information (such as physical rows, logical rows, number of rows accessed, type of statistics used, and statistics version number) |
...
| POSITION | bigint(20) | NO | The ordinal position among all operators that have the same parent operator |
...
| ACCESS_PREDICATES | varchar(4000) | NO | Information about the expression accessed by the current operator |
...
| SPECIAL_PREDICATES | varchar(4000) | NO | Information on the specific expressions of the current operator, such as Join Condition, GROUP EXPRS, and SORT EXPRS. |
