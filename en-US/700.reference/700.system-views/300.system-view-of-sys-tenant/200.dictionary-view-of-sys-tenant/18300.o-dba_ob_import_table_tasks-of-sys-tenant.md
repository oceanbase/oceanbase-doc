...
|tenant-type||
|machine-translation||
|slug|oceanbase.dba_ob_import_table_tasks|
...
| STATUS | varchar(64) | NO | The status of the task. Valid values: <ul><li>INIT: The table-level import task is being initialized. </li><li>DOING: The table-level import task is in progress. </li><li>FINISH: The import task is completed. </li></ul> |
| START_TIMESTAMP | timestamp(6) | NO | The current time in the tenant when the task starts. |
| COMPLETION_TIMESTAMP | timestamp(6) | NO | The current time in the tenant when the task is completed. |
