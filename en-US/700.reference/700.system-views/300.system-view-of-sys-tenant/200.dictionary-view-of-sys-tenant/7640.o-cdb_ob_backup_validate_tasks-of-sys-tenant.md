...
The `oceanbase.CDB_OB_BACKUP_VALIDATE_TASKS` view displays information about all backup validation tasks initiated by tenants. Each task corresponds to a validation job for a backup set or log archive piece.
...
| STATUS | varchar(64) | NO | Task status: <ul><li>`INIT`: The validation task is being initialized.</li> <li>`DOING`: The validation task is in progress.</li> <li>`COMPLETED`: The validation task has been completed.</li> <li>`FAILED`: Validation task execution failed.</li> <li>`CANCELED`: The validation task has been canceled.</li></ul> |
...
Query information about all tenant-initiated task-level backup validation tasks in the sys tenant.
...
* View the execution history of all tenant task-level backup validation tasks: [CDB_OB_BACKUP_VALIDATE_TASK_HISTORY](7650.o-cdb_ob_backup_validate_task_history-of-sys-tenant.md)
