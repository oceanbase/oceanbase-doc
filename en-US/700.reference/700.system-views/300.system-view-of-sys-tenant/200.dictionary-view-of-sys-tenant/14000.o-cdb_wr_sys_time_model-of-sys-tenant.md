...
|slug|o-cdb_wr_sys_time_model-of-sys-tenant|
...
The `CDB_WR_SYS_TIME_MODEL` view displays WR data of Time Model statistics for all tenants.
...
The values of the `STAT_NAME` column in this view indicate the names and IDs of statistics items, as shown in the following table.

| Statistics item name | Description | Statistics item ID | Statistics item type |
| ------------|------|------------|----------------|
...
| non idle wait time                     |  The total time of non-idle waiting, that is, the time when a session is in a waiting state but not in an idle waiting state.  |  200010 |  3072 |
| idle wait time                         |  The total time of idle waiting, that is, the time when a session is in an idle waiting state.    |  200011 |  3072 |
| background database time               |  The total time consumed by background processes in database operations.                   |  200012 |  3072 |
| background database non-idle wait time |  The time consumed by background processes in non-idle waiting states for database operations.    |  200013 |  3072 |
| background database idle wait time     |  The time consumed by background processes in idle waiting states for database operations.      |  200014 |  3072 |
...
| user io wait total time                | The total time spent by user processes waiting for I/O operations (such as reading data from or writing data to a disk) to complete. | 220002 |  3072 |
...
Query the WR data of Time Model statistics for all tenants and display the first 10 records.
