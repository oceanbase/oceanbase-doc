|description||
|---|---|
| machine-translation ||
|keywords||
|dir-name||
|dir-name-en||
|tenant-type|MySQL Mode|
|slug|information_schema-schema_privileges-sys-tenant|

# information_schema.SCHEMA_PRIVILEGES

<main id="notice" type='explain'>
<h4>Note</h4>
<p>This view is available starting with V1.4. </p>
</main>

## Purpose

This view displays database privileges.

## Columns

|    **Column**    |    **Type**     | **Nullable** |  **Description**  |
|----------------|---------------|----------------|----------|
| GRANTEE        | varchar(81)   | NO             | The account name that is granted privileges.     |
| TABLE_CATALOG  | varchar(512) | NO             | Fixed value (def). |
| TABLE_SCHEMA   | varchar(128)  | NO             | The database name.     |
| PRIVILEGE_TYPE | varchar(64)   | NO             | The privilege that is granted.     |
| IS_GRANTABLE   | varchar(3)    | NO             | Indicates whether the privilege can be granted to other users.    |

## Sample query

Query database privileges and display the first five records.

```shell
obclient [oceanbase]> SELECT * FROM information_schema.SCHEMA_PRIVILEGES LIMIT 5;
```

The query result is as follows:

```shell
+------------+---------------+--------------------+----------------+--------------+
| GRANTEE    | TABLE_CATALOG | TABLE_SCHEMA       | PRIVILEGE_TYPE | IS_GRANTABLE |
+------------+---------------+--------------------+----------------+--------------+
| 'root'@'%' | def           | information_schema | ALTER          | NO           |
| 'root'@'%' | def           | information_schema | CREATE         | NO           |
| 'root'@'%' | def           | information_schema | DELETE         | NO           |
| 'root'@'%' | def           | information_schema | DROP           | NO           |
| 'root'@'%' | def           | information_schema | INSERT         | NO           |
+------------+---------------+--------------------+----------------+--------------+
5 rows in set
```