...
| REWRITE_ENABLED | varchar(1) | NO | Indicates whether rewrite is enabled. Valid values:<ul><li>`Y`</li><li>`N`</li></ul><main id="notice" type='explain'> <h4>Note</h4><p>This column is used only for compatibility and its value is `N` by default. </p></main> |
...
| ON_QUERY_COMPUTATION | varchar(1) | NO | Indicates whether the materialized view is a real-time view. Valid values:<ul><li>`Y`</li><li>`N`</li></ul><main id="notice" type='explain'> <h4>Note</h4><p>This column is used only for compatibility and its value is `N` by default. </p></main> |
| REFRESH_DOP          | bigint(20) | NO | The degree of parallelism of background refreshes. The default value is 0. <main id="notice" type='explain'><h4>Note</h4><p><ul><li>When you create a materialized view, you can use the <code>PARALLEL</code> attribute to specify this value, and you can use the <code>ALTER MATERIALIZED VIEW</code> statement to modify it later. When you do not explicitly set the <code>PARALLEL</code> attribute (when the default value 0 is used for <code>REFRESH_DOP</code>), the degree of parallelism of background refreshes depends on the value of the global system variable `mview_refresh_dop`. </li><li>For OceanBase Database V4.3.5, this column was introduced in V4.3.5 BP1. </li></ul></p></main>|
