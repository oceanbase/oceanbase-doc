|description||
|---|---|
| machine-translation ||
|keywords||
|dir-name||
|dir-name-en||
|tenant-type|MySQL Mode|
|slug|mysql-db-sys-tenant|

# mysql.db

<main id="notice" type='explain'>
<h4>Note</h4>
<p>This view is available starting with V1.4. </p>
</main>

## Purpose

This view displays database-level privileges.

## Columns

|       **Column**        |    **Type**    | **Nullable** |                                 **Description**       |
|-----------------------|--------------|----------------|------------------------------------------------------------------------|
| host                  | varchar(128) | NO             | The host name.                                                                    |
| db                    | varchar(128) | NO             | The database name.                                                                   |
| user                  | varchar(128) | NO             | The username.                                                                    |
| select_priv           | varchar(1)   | NO             | Indicates whether the user can execute the SELECT statement.                                                        |
| insert_priv           | varchar(1)   | NO             | Indicates whether the user can execute the INSERT statement.                                                        |
| update_priv           | varchar(1)   | NO             | Indicates whether the user can execute the UPDATE statement.                                                        |
| delete_priv           | varchar(1)   | NO             | Indicates whether the user can execute the DELETE statement.                                                        |
| create_priv           | varchar(1)   | NO             | Indicates whether the user can execute the CREATE statement.                                                        |
| drop_priv             | varchar(1)   | NO             | Indicates whether the user can execute the DROP statement.                                                          |
| grant_priv            | varchar(1)   | NO             | Indicates whether the user can perform grant operations.                                                              |
| reference_priv        | varchar(1)   | NO             | Reserved.                                                                   |
| index_priv            | varchar(1)   | NO             | Indicates whether the user can create an index.                                                            |
| alter_priv            | varchar(1)   | NO             | Indicates whether the user can execute the `ALTER TABLE` statement.                                                 |
| create_tmp_table_priv | varchar(1)   | NO             | Indicates whether the user can create a temporary table. This column is not currently used.                |
| lock_tables_priv      | varchar(1)   | NO             | Indicates whether the user can execute the `LOCK TABLES` statement. The user also needs SELECT privileges on the table. This column is not currently used. |
| create_view_priv      | varchar(1)   | NO             | Indicates whether the user can execute the `CREATE VIEW` statement.                                                 |
| show_view_priv        | varchar(1)   | NO             | Indicates whether the user can execute the `SHOW CREATE VIEW` statement.                                            |
| create_routine_priv   | varchar(1)   | NO             | Indicates whether the user can create stored procedures. This column is not currently used.   |
| alter_routine_priv    | varchar(1)   | NO             | Indicates whether the user can execute the `ALTER FUNCTION` statement. This column is not currently used.                         |
| execute_priv          | varchar(1)   | NO             | Indicates whether the user can execute stored procedures. This column is not currently used.      |
| event_priv            | varchar(1)   | NO             | Indicates whether the user can create, delete, or modify events. This column is not currently used.    |
| trigger_priv          | varchar(1)   | NO             | Indicates whether the user can execute the `CREATE TRIGGER` or `DROP TRIGGER` statement. This column is not currently used.   |

## Sample query

Query the database-level privileges of the `test2` user.

```shell
obclient [oceanbase]> SELECT * FROM mysql.db WHERE user='test2' \G
```

The query result is as follows:

```shell
************************* 1. row ***************************
                 host: %
                   db: infotest
                 user: test2
          select_priv: N
          insert_priv: N
          update_priv: N
          delete_priv: Y
          create_priv: N
            drop_priv: N
           grant_priv: N
      references_priv: N
           index_priv: N
           alter_priv: N
create_tmp_table_priv: N
     lock_tables_priv: N
     create_view_priv: N
       show_view_priv: N
  create_routine_priv: N
   alter_routine_priv: N
         execute_priv: N
           event_priv: N
         trigger_priv: N
1 row in set
```