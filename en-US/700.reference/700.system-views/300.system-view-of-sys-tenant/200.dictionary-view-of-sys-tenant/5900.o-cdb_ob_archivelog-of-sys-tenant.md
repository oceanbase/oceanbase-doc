...
This view displays the log archiving status on each path for each tenant.
...
| INPUT_BYTES_DISPLAY | varchar(28) | NO | The display of INPUT_BYTES after the unit is converted. Valid values: MB, GB, TB, and PB. |
| OUTPUT_BYTES | bigint(21) | NO | The size of the data output, in bytes. |
| OUTPUT_BYTES_DISPLAY | varchar(28) | NO | The display of OUTPUT_BYTES after the unit is converted. Valid values: MB, GB, TB, and PB. |
...
Query the log archiving status of the tenant with the tenant ID 1002 in the sys tenant.
