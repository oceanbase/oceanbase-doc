...
| DOMIDX_OPSTATUS         | varchar(6)      | NO         | This column is not supported. The default value is NULL.      |
| PARAMETERS              | text            | NO         | This column is not supported. The default value is NULL.   |

## Sample query

In the sys tenant, query the subpartitions (secondary partitions) of the `tbl2_f_rl` index table in the current tenant.
