...
| SYNC_VALUE          | bigint(20) unsigned | NO       | The value specified explicitly for the auto-increment column by using the `INSERT` statement, which is greater than the current value of the auto-increment column. |

## Sample query

Query the information about all auto-increment columns in the current tenant.
