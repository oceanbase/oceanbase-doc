...
|dir-name-en||
|tenant-type||
|slug|o-cdb_ob_restore_history-sys-tenant|
...
This view displays the status of a physical restore task when the task is completed. It is a snapshot of the corresponding record in the `oceanbase.CDB_OB_RESTORE_PROGRESS` view when the task is finished.
...
| RESTORE_SCN_DISPLAY              | varchar(26)         |  NO   |The timestamp of the restore SCN specified by the user.  |
...
| STATUS                           | varchar(64)         |  NO   |The restore result. <ul> <li> `SUCCESS`: the restore task is completed. </li> <li> `FAILED`: the restore task fails. </li></ul> |
...
Query the status of a physical restore task when the task is completed in the sys tenant.
...
* Query the current execution progress of all physical restore tasks: [CDB_OB_RESTORE_PROGRESS](10400.o-cdb_ob_restore_progress-of-sys-tenant.md)

* For more information about the physical restore result, see [View the restore result](../../../../600.manage/600.backup-and-recovery/600.restore-data/500.view-the-restore-history.md).