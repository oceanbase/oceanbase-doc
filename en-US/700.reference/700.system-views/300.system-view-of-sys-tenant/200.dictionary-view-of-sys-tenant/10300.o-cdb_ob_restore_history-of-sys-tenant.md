...
The `oceanbase.CDB_OB_RESTORE_HISTORY` view displays the states of physical restore tasks upon completion. It is a snapshot of the records of the corresponding tasks in the `oceanbase.CDB_OB_RESTORE_PROGRESS` view.
...
| RESTORE_TENANT_NAME              | varchar(128)        |  NO   | The name of the tenant to restore. |
| RESTORE_TENANT_ID                | bigint(20)  |NO        | The ID of the tenant to restore. |
...
| RESTORE_OPTION                   | varchar(4096)       |  NO   | The restore option specified when the restore job was initiated. |
...
| STATUS                           | varchar(64)         |  NO   | The restore result. Valid values: <ul> <li>`SUCCESS`: The restore job succeeded.</li> <li>`FAILED`: The restore job failed.</li></ul> |
