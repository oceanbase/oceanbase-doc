|description||
|---|---|
|keywords||
|dir-name||
|dir-name-en||
|tenant-type||
|machine-translation| en |
|slug|oceanbase-cdb_ob_transfer_partition_task_history-sys-tenant|

# oceanbase.CDB_OB_TRANSFER_PARTITION_TASK_HISTORY

<main id="notice" type='explain'>
  <h4>Note</h4>
  <p>This view is available starting with V4.2.1 BP2. </p>
</main>

## Purpose

The `CDB_OB_TRANSFER_PARTITION_TASK_HISTORY` view displays the history of executed Transfer Partition tasks for all tenants.

## Columns

| **Column** | **Type** | **Nullable** | **Description** |
| --- | --- | --- | --- |
| TENANT_ID        | bigint(20)   | NO   |  The ID of the user tenant.   |
| TASK_ID          | bigint(20)   | NO   |  The unique identifier of the Transfer Partition task.   |
| CREATE_TIME      | timestamp(6) | NO   |  The time when the task was created.   |
| FINISH_TIME      | timestamp(6) | NO   |  The time when the task was completed, including successful and canceled tasks.   |
| TABLE_ID         | bigint(20)   | NO   |  The ID of the table where the partition to be transferred is located.   |
| OBJECT_ID        | bigint(20)   | NO   |  The `OBJECT_ID` of the partition to be transferred.   |
| DEST_LS          | bigint(20)   | NO   |  The log stream of the destination.   |
| BALANCE_JOB_ID   | bigint(20)   | NO   |  The ID of the load balancing job associated with the task.   |
| TRANSFER_TASK_ID | bigint(20)   | NO   |  The ID of the Transfer task associated with the task.   |
| STATUS           | varchar(64)  | NO   |  The status of the task. Valid values: <ul><li>COMPLETED: The task was successfully executed. </li><li>FAILED: The task failed. </li></ul>   |
| COMMENT          | longtext     | YES  |  The comment.   |

## Sample query

Query the history of executed Transfer Partition tasks for the tenant whose ID is 1006.

```shell
obclient> SELECT * FROM oceanbase.CDB_OB_TRANSFER_PARTITION_TASK_HISTORY  WHERE TENANT_ID = 1006;
```

The query result is as follows:

```shell
+-----------+---------+----------------------------+----------------------------+----------+-----------+---------+----------------+------------------+-----------+-------------------------------+
| TENANT_ID | TASK_ID | CREATE_TIME                | FINISH_TIME                | TABLE_ID | OBJECT_ID | DEST_LS | BALANCE_JOB_ID | TRANSFER_TASK_ID | STATUS    | COMMENT                       |
+-----------+---------+----------------------------+----------------------------+----------+-----------+---------+----------------+------------------+-----------+-------------------------------+
|      1006 |       1 | 2023-11-29 16:02:17.501500 | 2023-11-29 16:03:18.000000 |   500003 |    500009 |    1003 |          17304 |                2 | COMPLETED | Partition transfer to dest LS |
|      1006 |       2 | 2023-11-30 11:04:20.805526 | 2023-11-30 11:59:39.000000 |   500026 |    500026 |    1001 |         377170 |             2952 | COMPLETED | Partition transfer to dest LS |
|      1006 |       3 | 2023-12-01 11:26:43.757677 | 2023-12-01 11:27:26.000000 |   500003 |    500009 |    1001 |         760894 |             2954 | COMPLETED | Partition transfer to dest LS |
+-----------+---------+----------------------------+----------------------------+----------+-----------+---------+----------------+------------------+-----------+-------------------------------+
3 rows in set
```

## References

* View the Transfer Partition tasks being processed for all tenants: [oceanbase.CDB_OB_TRANSFER_PARTITION_TASKS](11500.o-cdb_ob_transfer_partition_tasks-of-sys-tenant.md)

* View the history of executed Transfer Partition tasks for the current tenant: [oceanbase.DBA_OB_TRANSFER_PARTITION_TASK_HISTORY](23300.o-dba_ob_transfer_partition_tasks_history-of-sys-tenant.md)

* For more information about the Transfer Partition operation, see [Transfer Partition](../../../../600.manage/350.load-balancing-manage/200.transfer-partition-management.md).

* For more information about data load balancing, see [Data load balancing](../../../../600.manage/350.load-balancing-manage/100.load-balancing-overview.md).