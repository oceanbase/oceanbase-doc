...
|slug|o-dba_ob_import_table_jobs-of-sys-tenant|
...
| STATUS | varchar(64) | NO | The status of the task. Valid values: <ul><li> INIT: the task is initializing. </li><li>IMPORT_TABLE: importing tables. </li><li>RECONSTRUCT_REF_CONSTRAINT: reconstructing reference constraints.  </li><li>CANCELING: canceling. </li><li>IMPORT_FINISH: the task is completed.  </li></ul> |
| IMPORT_ALL | tinyint(4) | NO | Indicates whether to import all tables in the source tenant. The value is 1 only when the `RECOVER TABLE *.*` option is specified. Otherwise, the value is 0. |
