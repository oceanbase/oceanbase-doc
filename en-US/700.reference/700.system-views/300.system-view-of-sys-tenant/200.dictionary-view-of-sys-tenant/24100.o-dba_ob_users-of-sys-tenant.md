...
| OLD_PASSWORD_START_TIME    | bigint(20)    | NO   | The point in time when the old password became effective. (-1 indicates no old password available.) In Oracle tenants, this value is also used to determine whether the old password is still within the rotation period; expired old passwords are unavailable.     |
