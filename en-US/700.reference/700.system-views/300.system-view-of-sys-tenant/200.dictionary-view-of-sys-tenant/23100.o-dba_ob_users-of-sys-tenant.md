...
|tenant-type||
|machine-translation||
|slug|oceanbase.dba_ob_users-o-dba_ob_users-of-sys-tenant|

# oceanbase.DBA_OB_USERS

<main id="notice" type='explain'>
<h4>Note</h4>
<p>This view is available only in MySQL mode. </p>
</main>
...
| PRIV_ALTER                | varchar(3)    | NO   |  Privilege to modify databases or tables   |
| PRIV_CREATE               | varchar(3)    | NO   |  Privilege to create databases or tables   |
| PRIV_DELETE               | varchar(3)    | NO   |  Privilege to delete records in databases or tables   |
| PRIV_DROP                 | varchar(3)    | NO   |  Privilege to drop databases or tables   |
...
| SSL_TYPE                  | bigint(20)    | NO   |  Standard SSL encryption and security types supported   |
| SSL_CIPHER                | varchar(1024) | NO   |  Standard SSL encryption and security passwords supported   |
...
| PRIV_REPL_CLIENT          | varchar(3)    | NO   |  Privilege to manage the primary server. A tenant can read binary log files used for maintaining the replication database environment. This privilege is located in the primary server to facilitate communication between the host and client.   |
| PRIV_DROP_DATABASE_LINK   | varchar(3)    | NO   | Whether has the privilege to drop database links.     |
| PRIV_CREATE_DATABASE_LINK | varchar(3)    | NO   | Whether has the privilege to create database links.     |
| PRIV_EXECUTE              | varchar(3)    | NO   | Whether has the privilege to execute procedures and functions.    |
| PRIV_ALTER_ROUTINE        | varchar(3)    | NO   | Whether has the privilege to modify and delete procedures and functions.      |
| PRIV_CREATE_ROUTINE       | varchar(3)    | NO   | Whether has the privilege to create procedures and functions.     |
| PRIV_CREATE_TABLESPACE    | varchar(3)    | NO   | Whether has the privilege to create, modify, and delete tablespaces.     |
| PRIV_SHUTDOWN             | varchar(3)    | NO   | Whether has the privilege to execute the mysqladmin shutdown command.     |
| PRIV_RELOAD               | varchar(3)    | NO   | Whether has the privilege to execute the Flush operation.     |
| PRIV_REFERENCES           | varchar(3)    | NO   | Whether has the privilege to create foreign keys.     |
| PRIV_CREATE_ROLE          | varchar(3)    | NO   | Whether has the privilege to create roles.     |
| PRIV_DROP_ROLE            | varchar(3)    | NO   | Whether has the privilege to drop roles.     |
| PRIV_TRIGGER              | varchar(3)    | NO   | Whether has the privilege to activate trigger operations.     |
| PRIV_LOCK_TABLE           | varchar(3)    | NO   | Whether has the privilege to lock tables.     |
| PRIV_ENCRYPT              | varchar(3)    | NO   | Whether can call the ENHANCED_AES_ENCRYPT function.     |
| PRIV_DECRYPT              | varchar(3)    | NO   | Whether can call the ENHANCED_AES_DECRYPT function.     |
| PRIV_EVENT                | varchar(3)    | NO   | Whether has the privilege to create and manage events.     |

## Query examples

In the sys tenant, query the information about users and roles in the current tenant.
