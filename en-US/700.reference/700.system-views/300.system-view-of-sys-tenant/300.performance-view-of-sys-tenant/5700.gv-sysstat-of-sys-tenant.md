|description||
|---|---|
|keywords||
|dir-name||
|dir-name-en||
|tenant-type||
|machine-translation| en |
|slug|gv-sysstat-of-sys-tenant|

# GV$SYSSTAT

<main id="notice" type='explain'>
<h4>Note</h4>
<p>This view is introduced since OceanBase Database V1.4. </p>
</main>

## Purpose

This view displays statistics information on all OBServer nodes for each tenant.

Where:

* `STATISTIC#`: the statistics item number, which may vary in different OBServer node versions.

* `STAT_ID`: the statistics item ID, which remains consistent in different OBServer node versions.

<main id="notice" type='explain'>
  <h4>Note</h4>
  <p>To query this view, you must enable monitoring by setting the <code>enable_perf_event</code> parameter to True. </p>
</main>

## Columns

| **Column** | **Type** | **Nullable?** | **Description** |
| --- | --- | --- | --- |
| CON_ID | bigint(20) | NO | Tenant ID |
| SVR_IP | varchar(46) | NO | IP address of the server where the information resides |
| SVR_PORT | bigint(20) | NO | Port number of the server where the information resides |
| STATISTIC# | bigint(20) | NO | Index of the statistics |
| NAME | varchar(64) | NO | Name of the statistics |
| CLASS | bigint(20) | NO | Type of the statistics |
| VALUE | bigint(20) | NO | Result value of the statistics |
| VALUE_TYPE | varchar(16) | NO | Statistics value type:<li>ADD: accumulated<li>SET: updated |
| STAT_ID | bigint(20) | NO | ID of the statistics |

## Sample query

In the sys tenant, query the statistics of all tenants on all nodes and display the first 10 records.

```shell
obclient [oceanbase]> SELECT * FROM oceanbase.GV$SYSSTAT LIMIT 10;
```

The query result is as follows:

```shell
+--------+----------------+----------+------------+-----------------------+-------+-------------+------------+---------+
| CON_ID | SVR_IP         | SVR_PORT | STATISTIC# | NAME                  | CLASS | VALUE       | VALUE_TYPE | STAT_ID |
+--------+----------------+----------+------------+-----------------------+-------+-------------+------------+---------+
|      1 | 11.xxx.xxx.xxx |    28825 |          0 | rpc packet in         |     1 |    60000651 | ADD_VALUE  |   10000 |
|      1 | 11.xxx.xxx.xxx |    28825 |          1 | rpc packet in bytes   |     1 | 14857436223 | ADD_VALUE  |   10001 |
|      1 | 11.xxx.xxx.xxx |    28825 |          2 | rpc packet out        |     1 |    13331823 | ADD_VALUE  |   10002 |
|      1 | 11.xxx.xxx.xxx |    28825 |          3 | rpc packet out bytes  |     1 |  4602803985 | ADD_VALUE  |   10003 |
|      1 | 11.xxx.xxx.xxx |    28825 |          4 | rpc deliver fail      |     1 |           0 | ADD_VALUE  |   10004 |
|      1 | 11.xxx.xxx.xxx |    28825 |          5 | rpc net delay         |     1 |   465112463 | ADD_VALUE  |   10005 |
|      1 | 11.xxx.xxx.xxx |    28825 |          6 | rpc net frame delay   |     1 |     1564592 | ADD_VALUE  |   10006 |
|      1 | 11.xxx.xxx.xxx |    28825 |          7 | mysql packet in       |     1 |         530 | ADD_VALUE  |   10007 |
|      1 | 11.xxx.xxx.xxx |    28825 |          8 | mysql packet in bytes |     1 |       63960 | ADD_VALUE  |   10008 |
|      1 | 11.xxx.xxx.xxx |    28825 |          9 | mysql packet out      |     1 |     1223369 | ADD_VALUE  |   10009 |
+--------+----------------+----------+------------+-----------------------+-------+-------------+------------+---------+
10 rows in set
```

## References

* View session-level statistics:

  * [GV$SESSTAT](5300.gv-sesstat-of-sys-tenant.md)
  * [V$SESSTAT](35800.v-sesstat-of-sys-tenant.md)

* View the definitions of all statistics of a tenant: [V$STATNAME](36400.v-statname-of-sys-tenant.md)

* For more information about statistics, see [Common statistical events](../../../700.system-views/510.statistical-event-description.md).