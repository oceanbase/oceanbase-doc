...
In the sys tenant, the `GV$OB_KV_GROUP_COMMIT_STATUS` view provides access to the group commit status information for all OBKV tenant client connections.
...
Query the group commit status information for all client connections of this OBKV tenant on all nodes, and display the records from the first day.
