...
The `V$OB_TENANT_RUNTIME_INFO` view displays information about queues and quotas of tenant threads on the current OBServer node.
...
| WORKERS | bigint(20) | NO | The number of worker threads held by the tenant, which equals the size of the workers_list.  |
...
In the system tenant, view the queue and thread limits of tenant threads.
