...
<main id="notice" type='explain'>
  <h4>Note</h4>
  <p>Introduced in version 4.0.0. </p>
</main>

## Purpose

The `V$OB_SQL_AUDIT` view displays statistics about each SQL request, including the source and execution status, of the current OBServer node. This view is tenant-scoped. Tenants other than the system tenant cannot query across tenants.
...
| SQL_ID                  | varchar(32)         | NO             | ID of the SQL statement <main id="notice" type='explain'><h4>Note</h4><ul><li>For V4.3.x, starting from V4.3.2, for PL requests that are executed by using the CALL statement and anonymous blocks, the value of this field is the actual generated MD5 value.  <li>For V4.2.x, starting from V4.2.3, for PL requests that are executed by using the CALL statement and anonymous blocks, the value of this field is the actual generated MD5 value.  </li></ul></main>  |
...
| WAIT_CLASS_ID           | bigint(20)          | NO             | Class ID of the waiting event       |
| WAIT_CLASS#             | bigint(20)          | NO             | Index of the waiting event in the class      |
...
| IS_HIT_PLAN             | tinyint(4)          | NO             | Whether the plan is hit in the plan cache        |
| REQUEST_TIME            | bigint(20)          | NO             | Start time of the execution, in microseconds                |
| ELAPSED_TIME            | bigint(20)          | NO             | Total time from receiving the request to the end of execution, in microseconds      |
...
| GET_PLAN_TIME           | bigint(20)          | NO             | Time from starting the process to obtaining the plan, in microseconds    |
| EXECUTE_TIME            | bigint(20)          | NO             | Time consumed by the execution of the plan, in microseconds             |
...
| CONSISTENCY_LEVEL       | bigint(20)          | NO             | Consistency level, which can take the following values:<ul><li>`-1`: invalid</li> <li>`1`: specifies to read data from SSTable</li><li>`2`: weakly consistent read</li><li>`3`: strongly consistent read</li></ul>                  |
...
| BLOCKSCAN_BLOCK_CNT  | bigint(20)          | NO             | Number of data microblocks scanned in a single scan         |
| BLOCKSCAN_ROW_CNT  | bigint(20)          | NO             | Number of data rows scanned in a single scan         |
| PUSHDOWN_STORAGE_FILTER_ROW_CNT  | bigint(20)          | NO             | Number of rows filtered by the storage filter after pushdown         |
...
| PS_CLIENT_STMT_ID      | bigint(20)          | NO             | Prepare ID of the request: <ul><li> `-1`: indicates that the SQL statement does not use the PS protocol.  </li><li> A non-negative value: indicates that the SQL statement uses the PS protocol. This value is the unique identifier returned by the PS protocol to the client for the statement. </li></ul>     |
| PS_INNER_STMT_ID      | bigint(20)          | NO             | Prepare ID of the request: <ul><li> `-1`: indicates that the SQL statement does not use the PS protocol   </li><li> A non-negative value: indicates that the SQL statement uses the PS protocol. This value is the unique identifier returned by the PS protocol to the internal server for the statement. </li></ul>      |
...
| REQUEST_TYPE            | bigint(20)          | NO             | Type of the request: <li> 0: illegal   <li> 1: internal request   <li> 2: local request, such as a local plan   <li> 3: remote request   <li> 4: distributed request   <li> 5: prepare request for an SQL statement   <li> 6: execute request for an SQL statement    |
| IS_BATCHED_MULTI_STMT   | tinyint(4)          | NO             | Whether to optimize batch multi-statement execution                                                        |
...
| LOCK_FOR_READ_TIME      | bigint(20)          | NO             | Time spent waiting for a lock when data is read, in microseconds      |
| PARAMS_VALUE            | longtext            | NO             | Values of the parameters bound by PS              |
| RULE_NAME | varchar(256) | NO   | Name of the rule   <main id="notice" type='explain'><h4>Note</h4><p> This field was introduced in V4.1.0. </p></main> |
| PARTITION_HIT | tinyint(4) | NO | <ul><li> If a DAS execution plan is obtained, this value is 1 if the local partition is hit and 0 if not.  </li><li> If a DAS execution plan is not obtained, this value is 1 by default. </li></ul>  <main id="notice" type='explain'><h4>Note</h4><p> This field was introduced in V4.1.0. </p></main> |
| TX_INTERNAL_ROUTING | bigint(20) | NO | Indicates whether the SQL statement is internally routed:<ul><li>0: indicates that the SQL statement is directly executed on the current node without internal routing.  </li><li>1: indicates that the SQL statement is internally routed to another node for execution on the current node. </li></ul> <main id="notice" type='explain'><h4>Note</h4><p> This field was introduced in V4.1.0. </p></main> |
...
| FLT_TRACE_ID | varchar(1024) | NO | Trace ID of the full-link tracing of the record. If this field is empty, full-link tracing is not enabled. This field is a UUID, which is different from Trace. The format is as follows: `000600d6-a5de-038c-6c80-df07e4e79149`<main id="notice" type='explain'><h4>Note</h4><p> This field was introduced in V4.2.1. </p></main> |
| PL_TRACE_ID             | varchar(128)        | NO   | Trace ID of the outermost PL of the current SQL statement (NULL if there is no outermost PL) <main id="notice" type='explain'><h4>Note</h4><ul><li> For V4.3.x, this field was introduced in V4.3.0.  <li> For V4.2.x, this field was introduced in V4.2.2.  </li></ul></main>  |
...
| TOTAL_MEMSTORE_READ_ROW_COUNT   | bigint(20)          | NO   | Total number of rows read from the MemStore during the entire process (displayed only in the thread that displays query_text) <main id="notice" type='explain'><h4>Note</h4><ul><li> For V4.3.x, this field was introduced in V4.3.0.  <li> For V4.2.x, this field was introduced in V4.2.2.  </li></ul></main>   |
| TOTAL_SSSTORE_READ_ROW_COUNT    | bigint(20)          | NO   | Total number of rows read from the SSSTORE during the entire process (displayed only in the thread that displays query_text) <main id="notice" type='explain'><h4>Note</h4><ul><li> For V4.3.x, this field was introduced in V4.3.0.  <li> For V4.2.x, this field was introduced in V4.2.2.  </li></ul></main>  |
| PROXY_USER                      | varchar(128)        | NO  |<ul><li>The name of the proxy user if the user logs in as a proxy user.  </li><li>NULL if the user logs in as a non-proxy user. </li></ul> <main id="notice" type='explain'><h4>Note</h4><ul><li> For V4.3.x, this field was introduced in V4.3.2.  <li> For V4.2.x, this field was introduced in V4.2.3.  </li></ul> </main> |
...
| NETWORK_WAIT_TIME               | bigint(20) unsigned | NO  | Total waiting time of all network events, in microseconds<main id="notice" type='explain'><h4>Note</h4><ul><li> For V4.3.x, this field was introduced in V4.3.5.  <li> For V4.2.x, this field was introduced in V4.2.3.  </li></ul></main>     |
