...
Displays the RPC sending statistics for all tenants across all OBServer nodes.
...
| TIMEOUT | bigint(20) | NO | Number of Sending Timeout |
...
In the sys tenant, view the RPC requests sent by the tenant with ID 1004 across all nodes, and display the first 10 records.
...
* View PRC statistics sent by all tenants on the current node: [V$OB_RPC_OUTGOING](34700.v-obrpc_outgoing-of-sys-tenant.md)

* View statistics of RPCs received by all tenants on all nodes: [GV$OB_RPC_INCOMING](4100.gv-obrpc_incoming-of-sys-tenant.md)
