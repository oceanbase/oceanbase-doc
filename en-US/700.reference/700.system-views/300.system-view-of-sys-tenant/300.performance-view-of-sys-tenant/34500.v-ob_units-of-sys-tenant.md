...
| REPLICA_TYPE     | varchar(7)   | NO         | The type of replica. The default value is `FULL`. <main id="notice" type='explain'><h4>Note</h4><p>The <code>REPLICA_TYPE</code> field was introduced in V4.2.5 BP7. </p></main>    |

## Sample query

Query the information about the units of the tenant with ID 1002 on the current OBServer node from the `sys` tenant.
