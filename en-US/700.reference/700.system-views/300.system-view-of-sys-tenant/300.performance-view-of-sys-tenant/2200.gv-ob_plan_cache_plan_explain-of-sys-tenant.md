...
1. Query the `PLAN_ID` column from the [GV$OB_PLAN_CACHE_PLAN_STAT](2300.gv-ob_plan_cache_plan_stat-of-sys-tenant.md) view.
...
2. Query the physical execution plan in the plan cache by using the `GV$OB_PLAN_CACHE_PLAN_EXPLAIN` view. For example, set `PLAN_ID=605`.
