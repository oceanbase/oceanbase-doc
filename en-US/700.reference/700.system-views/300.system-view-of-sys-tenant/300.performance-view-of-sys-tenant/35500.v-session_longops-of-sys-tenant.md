|description||
|---|---|
|keywords||
|dir-name||
|dir-name-en||
|tenant-type||

# V$SESSION_LONGOPS

## Purpose

Displays the execution status and progress of DDL operations on the current OBServer node.

<main id="notice" type='explain'>
  <h4>Note</h4>
  <p>This view is introduced since OceanBase Database V1.4. </p>
</main>

## Related tables and views

GV$SESSION_LONGOPS

## Columns

|     **Column**     |    **Type**     | **Nullable?** |  **Description**   |
|------------------|---------------|----------------|-----------|
| SID              | bigint(20)    | NO             | Session ID     |
| TRACE_ID         | varchar(64)  | NO   | Trace ID of the statement    |
| OPNAME           | varchar(64)  | NO             | Operation name      |
| TARGET           | varchar(64)  | NO             | Operation object      |
| SVR_IP           | varchar(46)   | NO             | IP address of the server where the operation is performed |
| SVR_PORT         | bigint(20)    | NO             | Port number of the server where the operation is performed |
| START_TIME       | date   | NO             | Start time      |
| ELAPSED_SECONDS  | bigint(21) | NO             | Time already consumed    |
| TIME_REMAINING   | bigint(20)    | NO             | Estimated remaining time  |
| LAST_UPDATE_TIME | date   | NO             | Time when statistics were last updated  |
| MESSAGE          | varchar(512)  | NO             | Remarks      |