...
|------------------------|--------------|----------------|---------|
| ADDRESS                | varbinary(8) | NO             | The handle address corresponding to the SQL statement. Currently, no value is specified.                           |
| HASH_VALUE             | bigint(0)    | NO             | The hash value of the statement.                                  |
| DB_ID                  | bigint(20)   | NO             | The database ID of the database to which the connection for the SQL request belongs. <main id="notice" type='explain'><h4>Note</h4><ul><li>For V4.3.x versions, this column was introduced starting with V4.3.1. </li> <li>For V4.2.x versions, this column was introduced starting with V4.2.3. </li></ul></main>  |
...
| POLICY                 | varchar(10)  | NO             | The workarea strategy: <ul><li>MANUAL</li><li> AUTO</li></ul>    |
...
Query the workarea statistics of all operators executed by SQL automatic memory management on the current OBServer node.
