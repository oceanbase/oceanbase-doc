...
This view displays the statistics of RPC requests received by all tenants on all OBServer nodes.
...
| PROCESS_TIME   | bigint(20)   | NO             | The actual time spent processing the request.             |
...
In the sys tenant, query the RPC requests received by all nodes where the waiting time in the queue exceeds 1 second.
...
* View the RPC statistics received by all tenants on the current node: [V$OB_RPC_INCOMING](32300.v-ob_obrpc_incoming-of-sys-tenant.md)

* View the PRC statistics sent by all tenants on all nodes: [GV$OB_RPC_OUTGOING](2000.gv-ob_obrpc_outgoing-of-sys-tenant.md)