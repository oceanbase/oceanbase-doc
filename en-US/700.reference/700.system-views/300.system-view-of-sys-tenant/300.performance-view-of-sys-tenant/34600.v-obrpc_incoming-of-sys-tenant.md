...
Displays the statistics of RPC requests received by all tenants on the current OBServer node.
...
In the sys tenant, view all RPC requests received by the current node and identify those whose queue wait time exceeds 1 second.
...
* View statistics on RPCs received by all tenants across all nodes: [GV$OB_RPC_INCOMING](4100.gv-obrpc_incoming-of-sys-tenant.md)

* View the PRC statistics sent by all tenants on the current node: [V$OB_RPC_OUTGOING](34700.v-obrpc_outgoing-of-sys-tenant.md)
