...
Displays the RPC sending statistics for all tenants on the current OBServer node.
...
In the sys tenant, view the RPC requests sent by the tenant with ID 1004 on the current node and display the first 10 records.
...
* View statistics of RPCs received by all tenants on the current node: [V$OB_RPC_INCOMING](34600.v-obrpc_incoming-of-sys-tenant.md)
