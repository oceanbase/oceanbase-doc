...
|dir-name-en||
|tenant-type||
|slug|v-ob-obrpc-outgoing-sys-tenant|
...
This view displays the statistics of RPCs sent by all tenants on the current OBServer node.
...
| COUNT          | bigint(20)   | NO             | The number of times the RPC is used.                  |
...
In the sys tenant, query the statistics of RPCs sent by tenant 1004 on the current node, and display the first 10 records.
...
* View the statistics of RPCs sent by all tenants on all nodes: [GV$OB_RPC_OUTGOING](2000.gv-ob_obrpc_outgoing-of-sys-tenant.md)

* View the statistics of RPCs received by all tenants on the current node: [V$OB_RPC_INCOMING](32300.v-ob_obrpc_incoming-of-sys-tenant.md)