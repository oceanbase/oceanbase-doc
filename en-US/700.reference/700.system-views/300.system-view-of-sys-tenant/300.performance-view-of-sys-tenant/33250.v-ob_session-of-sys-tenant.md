...
| --- | --- | --- | --- |
| ID | bigint(20) unsigned | NO | For V4.2.x: <ul><li> V4.2.5 BP4 and earlier: <br>In all connection modes (direct mode or ODP mode), this field indicates the server session ID. </li><li> V4.2.5 BP4 and later: <ul><li>Direct mode: <br>This field indicates the server session ID. </li> <li>ODP mode: </li><ul><li>If the parameter <code>client_session_id_version = 2</code> is configured in ODP, this field indicates the client session ID. </li> <li>If <code>client_session_id_version = 1</code>, this field indicates the server session ID. </li></ul></ul></li></ul> |
