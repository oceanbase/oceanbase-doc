...
| --- | --- | --- | --- |
| ID | bigint(20) unsigned | NO | For V4.2.x: <ul><li>For versions earlier than V4.2.5 BP3 Hotfix5: <br>In all connection modes (direct mode and ODP mode), this field indicates the server session ID. </li><li>For V4.2.5 BP3 Hotfix5 and later: <ul><li>In direct mode: <br>This field indicates the server session ID. </li> <li>In ODP mode: </li><ul><li>When the ODP configuration item <code>client_session_id_version = 2</code> is set, this field indicates the client session ID.  </li> <li>When <code>client_session_id_version = 1</code> is set, this field indicates the server session ID.</li></ul></ul></li></ul>  |
