...
This view displays the statistics of events for each tenant on the current OBServer node.
...
* `STAT_ID`: the ID of the statistics item. The ID of the same statistics item is guaranteed to be the same across different OBServer node versions.
...
In the sys tenant, query the statistics of events for all tenants on the current node and display the first 10 records.
...
* To query the statistics of events at the session level:

  * [GV$SESSTAT](5300.gv-sesstat-of-sys-tenant.md)
  * [V$SESSTAT](35800.v-sesstat-of-sys-tenant.md)

* To query the definitions of all statistics events: [V$STATNAME](36400.v-statname-of-sys-tenant.md)
