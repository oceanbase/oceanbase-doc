...
|TYPE    |varchar(64)    |NO   |The type of compaction: <ul><li>`MDS_TABLE_MERGE`: the system metadata is persisted to disk in the SSTable format. </li> <li>`MAJOR_MERGE`: a tenant-level major compaction. </li> <li>`MEDIUM_MERGE`: a partition-level major compaction. </li> <li>`MINI_MERGE`: a mini compaction that converts MemTables into a Mini SSTable. </li> <li>`MINOR_MERGE`: a minor compaction that combines multiple Mini SSTables into a new one or combines a Minor SSTable with several Mini SSTables into a new one. </li> <li>`META_MAJOR_MERGE`: a special type of compaction that synthesizes data from the start of the specified time point into a Meta Major SSTable. The data format of this SSTable is the same as that of a Major SSTable, but it does not contain multi-version data or uncommitted transactions. </li></ul>    |
...
| MERGE_LEVEL                     | varchar(64)  | NO   |  Specifies whether to perform a major compaction or reuse microblocks during the compaction<main id="notice" type='explain'><h4>Note</h4><p>This field is introduced in V4.3.0. </p></main>    |

## Example

The sys tenant queries the history of tablet-level major compactions on this OBServer node.
