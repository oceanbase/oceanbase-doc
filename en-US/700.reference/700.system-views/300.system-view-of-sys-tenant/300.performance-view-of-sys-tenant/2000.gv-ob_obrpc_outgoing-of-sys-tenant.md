...
This view displays the statistics of RPC requests sent by all tenants on all OBServer nodes.
...
| COUNT          | bigint(20)   | NO             | The number of times the RPC request was used.                  |
...
In the sys tenant, query the statistics of RPC requests sent by tenant 1004 on all nodes and display the first 10 records.
...
* Query the statistics of RPC requests sent by all tenants on the current node: [V$OB_RPC_OUTGOING](32400.v-ob_obrpc_outgoing-of-sys-tenant.md)

* Query the statistics of RPC requests received by all tenants on all nodes: [GV$OB_RPC_INCOMING](1900.gv-ob_obrpc_incoming-of-sys-tenant.md)