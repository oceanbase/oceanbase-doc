...
The `GV$OB_THREAD` view displays the status information about all threads in the observer process of each OBServer node. In the sys tenant, it shows the status information about all threads of all tenants.
...
| CGROUP_PATH | varchar(256) | NO  | The path of the cgroup to which the thread belongs. This field is obtained from the CPU row in the file located at `/proc/observer-pid/task/tid/cgroup`. |

## Sample query

In the sys tenant, view the status of all threads in the observer process on each OBServer node and display the first two records.
