...
The `V$OB_SQLSTAT` view displays basic performance statistics of SQL statements on the current node. Each SQL statement occupies one row. In other words, each unique value of `SQL_ID` + `Plan_Hash` occupies one row. Columns that contain `_DELTA` indicate the increments of the statistics from the last WR snapshot collection to the current time.
...
| MODULE | varchar(64) | NO | The application Module when the statement was parsed for the first time, through`DBMS_APPLICATION_INFO.SET_MODULE`Package settings |
| ACTION | varchar(64) | NO | The application Action when this statement was first parsed, through`DBMS_APPLICATION_INFO.SET_ACTION`Package settings |
...
| PLAN_CACHE_HIT_DELTA | bigint(20) | NO | Number of Plan Cache hits since the last WR snapshot collection |
...
The values of the `SQL_TYPE` column in the view and the corresponding statements are shown in the following table.
