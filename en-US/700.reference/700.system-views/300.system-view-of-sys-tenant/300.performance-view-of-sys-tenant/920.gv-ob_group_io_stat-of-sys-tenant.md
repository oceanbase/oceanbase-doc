...
This view displays the real-time status of each resource group on all OBServer nodes for all tenants.
...
In the sys tenant, query the real-time status of each resource group on all OBServer nodes for all tenants.
...
* Query the real-time status of each resource group on the current node for all tenants: [V$OB_GROUP_IO_STAT](31120.v-ob_group_io_stat-of-sys-tenant.md)
