...
| STORAGE_CACHE_POLICY | varchar(64)   | NO   |  The cache status of the tablet:<ul><li>`AUTO`: Automatic cold/hot data identification</li> <li>`HOT`: Hot data</li></ul>   |
| CACHED_DATA_SIZE     | bigint(20)    | NO   |  The amount of data cached locally for the tablet, in bytes.   |
| CACHE_HIT_COUNT      | bigint(20)    | NO   |  The number of cache hits for the tablet. This column is currently not enabled, and its value is `-1`.    |
| CACHE_MISS_COUNT     | bigint(20)    | NO   |  The number of cache misses for the tablet. This column is currently not enabled, and its value is `-1`.    |
| CACHE_HIT_SIZE       | bigint(20)    | NO   |  The amount of data cached for the tablet, in bytes. This column is currently not enabled, and its value is `-1`.   |
| CACHE_MISS_SIZE      | bigint(20)    | NO   |  The amount of data cached for the tablet, in bytes. This column is currently not enabled, and its value is `-1`.   |
| INFO                 | varchar(4096) | NO   |  Other information.    |

## Sample query

In SS mode, the sys tenant queries the overall cache status of tenant `1002` on all OBServer nodes.
