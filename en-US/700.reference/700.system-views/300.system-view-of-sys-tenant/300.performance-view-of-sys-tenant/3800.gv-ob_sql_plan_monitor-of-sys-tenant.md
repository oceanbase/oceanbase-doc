...
The `GV$SQL_PLAN_MONITOR` view displays the operation-level statistics of slow queries on all OBServer nodes in all tenants. The statistics of each operation of each slow query include the amount of time consumed by the operation and the number of rows returned.
