...
The `GV$SQL_PLAN_MONITOR` view displays the operation-level statistics of slow queries across all OBServer nodes in all tenants. Statistics are provided for each operation of each slow query.
...
| SQL_EXEC_START          | bigint(0) unsigned   | NO | The start time of SQL execution. |
...
| PLAN_POSITION           | bigint(0) unsigned   | NO | The index of the operator among its parent's children. |
...
| PHYSICAL_WRITE_BYTES    | bigint(0) unsigned   | NO | The number of bytes written by the I/O write requests issued by the operator. |
...
| OTHERSTAT_1_VALUE       | bigint(20)           | NO | For the TABLE SCAN operator, the value indicates the number of bytes read from the storage engine `IO_READ_BYTES`. <main id="notice" type='explain'><h4>Note</h4><ul><li>Starting from V4.3.3 BP1 of V4.3.x, the TABLE SCAN operator supports real-time statistics, instead of statistics after the operator finishes execution. </li><li>Starting from V4.2.4 of V4.2.x, the TABLE SCAN operator supports real-time statistics, instead of statistics after the operator finishes execution. </li></ul></main> |
| OTHERSTAT_2_ID          | bigint(20)           | NO | The ID of the field. |
| OTHERSTAT_2_TYPE        | bigint(0) unsigned   | NO | Reserved. |
| OTHERSTAT_2_VALUE       | bigint(20)           | NO | For the TABLE SCAN operator, the value indicates the number of bytes read from the SSStore `SSSTORE_READ_BYTES`. <main id="notice" type='explain'><h4>Note</h4><ul><li>Starting from V4.3.3 BP1 of V4.3.x, the TABLE SCAN operator supports real-time statistics, instead of statistics after the operator finishes execution. </li><li>Starting from V4.2.4 of V4.2.x, the TABLE SCAN operator supports real-time statistics, instead of statistics after the operator finishes execution. </li></ul></main> |
| OTHERSTAT_3_ID          | bigint(20)           | NO | The ID of the field. |
| OTHERSTAT_3_TYPE        | bigint(0) unsigned   | NO | Reserved. |
| OTHERSTAT_3_VALUE       | bigint(20)           | NO | The total number of rows in major SSTables read in the push-down and non-push-down paths within the specified range. |
| OTHERSTAT_4_ID          | bigint(20)           | NO | The ID of the field. |
| OTHERSTAT_4_TYPE        | bigint(0) unsigned   | NO | Reserved. |
| OTHERSTAT_4_VALUE       | bigint(20)           | NO | The total number of rows in minor SSTables, mini SSTables, and MemTables read in the push-down and non-push-down paths. |
...
| OTHERSTAT_6_VALUE       | bigint(20)           | NO | The total number of rows in microblocks opened in the push-down path within the specified range. <main id="notice" type='explain'><h4>Note</h4><p>This field is available starting from V4.5.0 of V4.5.x. </p></main>|
...
| OTHERSTAT_8_VALUE       | bigint(20)           | NO | The number of microblocks that do not require pre-fetching and use Skip Index information. <main id="notice" type='explain'><h4>Note</h4><p>This field is available starting from V4.5.0 of V4.5.x. </p></main>|
...
| SKIPPED_ROWS_COUNT      | bigint(20)           | NO | The total number of rows not processed during the calculation in vectorized mode (the total number of filtered rows). |
...
For the value of the `OTHERSTAT_X_ID` field, you can query the [V$SQL_MONITOR_STATNAME](35900.v-sql_monitor_statname-of-sys-tenant.md) view for more information.
