...
The `V$OB_THREAD` view displays the status information about all threads in the observer process. In the sys tenant, it shows the threads related to all tenants.
...
| CGROUP_PATH | varchar(256) | NO  | The path of the cgroup to which the thread belongs. This field is obtained from the CPU row in the `/proc/observer-pid/task/tid/cgroup` file. |

## Sample query

In the sys tenant, query the status of all threads in the observer process of the current OBServer node and display the first two records.
