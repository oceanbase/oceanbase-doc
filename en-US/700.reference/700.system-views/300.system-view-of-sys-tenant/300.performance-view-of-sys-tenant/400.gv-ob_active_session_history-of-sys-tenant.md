...
| SESSION_STATE | varchar(7) | NO | The status of the session. Valid values:<ul><li>`ON CPU`: The SQL logic is being executed without waiting.  </li><li>`WAITING`: The session is waiting. For more information, see the `EVENT` column. </li></ul> |
