...
Displays the information about the MemTable and SSTable under each partition of all tenants on the current OBServer node.
...
| TABLE_TYPE | varchar(20) | NO | Table type |
| TENANT_ID | bigint(20) | NO | Tenant ID |
| LS_ID | bigint(20) | NO | Log Stream ID |
| TABLET_ID | bigint(20) | NO | Data Shard ID |
| START_LOG_SCN | bigint(20) unsigned | NO | The left boundary of the log timestamp range for the data in this table. |
| END_LOG_SCN | bigint(20) unsigned | NO | Right Boundary of Log Timestamp Range for Data in This Table |
| SIZE | bigint(20) | NO | Disk usage, in bytes |
| REF | bigint(20) | NO | The count of references to this table, in units of number |
| UPPER_TRANS_VERSION | bigint(20) unsigned | NO | Maximum Transaction Version Number of a Committed Transaction |
| IS_ACTIVE | varchar(3) | NO | Whether it is an active MemTable |
| CONTAIN_UNCOMMITTED_ROW | varchar(3) | NO | Whether the table contains uncommitted transaction rows:<ul><li>`NO`: Indicates that the table does not contain uncommitted transaction rows.   </li><li>`YES`: Indicates that the table contains uncommitted transaction rows.</li></ul> |

## Sample query

In the sys tenant, view the MemTable and SSTable information of partition 1001 in the log stream with ID 1001 for the tenant with ID 1002 on the current OBServer node.

```shell
obclient [oceanbase]> SELECT * FROM oceanbase.V$OB_SSTABLES WHERE TENANT_ID=1002 AND LS_ID=1001;
```
