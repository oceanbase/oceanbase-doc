...
This view displays the information about MemTables and SSTables in each partition of the current tenant on the current OBServer node.
...
| SIZE                    | bigint(20)          | NO         | The disk space occupied by the table, in bytes.        |
...
| CONTAIN_UNCOMMITTED_ROW | varchar(3)          | NO         | Indicates whether the table contains uncommitted transaction rows:<ul><li>`NO`: Indicates that the table does not contain uncommitted transaction rows. </li><li>`YES`: Indicates that the table contains uncommitted transaction rows. </li></ul>   |

## Sample query

Query the MemTable and SSTable information in each partition of the 1001 log stream of the current tenant on the current OBServer node.
