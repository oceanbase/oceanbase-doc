...
| DATA_DISK_SIZE   | bigint(20)   | YES        | <ul><li>The size of the local cache that the unit can use, in bytes, in shared storage (Shared-Storage, SS) mode. </li> <li>In shared-nothing (Shared-Nothing, SN) mode, this field is `NULL`, indicating that the value is undefined. In this case, the unit can compete with other tenants for disk space. </li></ul> <main id="notice" type='explain'><h4>Note</h4><p>This field was introduced in V4.3.4. </p></main>     |
