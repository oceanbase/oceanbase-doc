...
This view displays the I/O statistics of all tenants at the function level on the current OBServer node. If the resource group corresponding to a background task at the function level does not have a resource management plan enabled, the displayed I/O statistics are only for reference and the corresponding resources cannot be limited.
...
The following example queries the I/O statistics of all tenants at the function level on the current OBServer node in the sys tenant.
