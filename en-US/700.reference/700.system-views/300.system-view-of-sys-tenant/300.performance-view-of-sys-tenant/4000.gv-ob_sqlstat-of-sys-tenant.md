...
|slug|gv-ob_sqlstat-of-sys-tenant|
...
The `GV$OB_SQLSTAT` view displays basic performance statistics of SQL statements in the cluster. Each SQL statement is represented as a row (i.e., each unique combination of `SQL_ID` and `Plan_Hash` is a row). The lists with `_DELTA` in their names indicate the incremental values from the last WR snapshot collection to the current time.
...
| PLAN_TYPE                     | bigint(20)          | NO   |    The type of the execution plan. <ul><li>`1`: Local Plan </li> <li>`2`: Remote Plan</li> <li>`3`: Distributed Plan </li></ul> </br>For PL object cache, this column indicates the type of the PL object. <ul><li>`1`: Procedure</li> <li>`2`: Function</li> <li>`3`: Package</li> <li>`4`: Anonymous Block </li></ul> |
| QUERY_SQL                     | longtext            | NO   | The full text of the SQL statement. |
| SQL_TYPE                      | bigint(20)          | NO   | The type of the statement. |
| MODULE                        | varchar(64)         | YES  |   The Module of the application when the statement is first parsed.    |
| ACTION                        | varchar(64)         | YES  |   The Action of the application when the statement is first parsed.      |
| PARSING_DB_ID                 | bigint(20)          | NO   |   The database ID when the statement is parsed.    |
| PARSING_DB_NAME               | varchar(128)        | NO   |   The database name when the statement is parsed.     |
| PARSING_USER_ID               | bigint(20)          | NO   |   The user ID when the statement is parsed.     |
| EXECUTIONS_TOTAL              | bigint(20)          | NO   |   The total number of executions of the execution plan in the plan cache.     |
| EXECUTIONS_DELTA              | bigint(20)          | NO   |   The incremental number of executions of the execution plan in the plan cache.      |
...
| ELAPSED_TIME_TOTAL            | bigint(20)          | NO   |   The total time consumed by the statement. For parallel execution, this value is the sum of the values of each execution thread.          |
...
| USERIO_WAIT_TOTAL             | bigint(20)          | NO   |   The total I/O wait time.      |
| USERIO_WAIT_DELTA             | bigint(20)          | NO   |   The incremental I/O wait time.      |
...
| WRITE_THROTTLE_TOTAL          | bigint(20)          | NO   |   The total time when the write to MemStore is throttled.    |
| WRITE_THROTTLE_DELTA          | bigint(20)          | NO   |   The incremental time when the write to MemStore is throttled.    |
...
| ROUTE_MISS_TOTAL              | bigint(20)          | NO   |   The total number of times no partition of the request was hit since the OBServer started<main id="notice" type='explain'><h4>Note</h4><ul><li>For V4.3.x, this column is available starting from V4.3.5. </li> <li>For V4.2.x, this column is available starting from V4.2.2. </li></ul></main>   |
| ROUTE_MISS_DELTA              | bigint(20)          | NO   |   The number of times no partition of the request was hit since the last WR snapshot was collected<main id="notice" type='explain'><h4>Note</h4><ul><li>For V4.3.x, this column is available starting from V4.3.5. </li> <li>For V4.2.x, this column is available starting from V4.2.2. </li></ul></main>   |
| FIRST_LOAD_TIME               | timestamp(6)        | YES  |   The plan generation time<main id="notice" type='explain'><h4>Note</h4><ul><li>For V4.3.x, this column is available starting from V4.3.5. </li> <li>For V4.2.x, this column is available starting from V4.2.5. </li></ul></main>   |
| PLAN_CACHE_HIT_TOTAL          | bigint(20)          | NO   |   The total number of times the Plan Cache was hit since the OBServer started<main id="notice" type='explain'><h4>Note</h4><ul><li>For V4.3.x, this column is available starting from V4.3.5. </li> <li>For V4.2.x, this column is available starting from V4.2.5. </li></ul></main>   |
| PLAN_CACHE_HIT_DELTA          | bigint(20)          | NO   |   The number of times the Plan Cache was hit since the last WR snapshot was collected<main id="notice" type='explain'><h4>Note</h4><ul><li>For V4.3.x, this column is available starting from V4.3.5. </li> <li>For V4.2.x, this column is available starting from V4.2.5. </li></ul></main>  |
| MUTI_QUERY_TOTAL              | bigint(20)        | NO   | The number of times the SQL statement was in a Multi Query since the execution plan was generated<main id="notice" type='explain'><h4>Note</h4><p>For V4.4.x, this column is available starting from V4.4.2. </p></main>|
| MUTI_QUERY_DELTA              | bigint(20)        | NO   | The number of times the SQL statement was in a Multi Query since the last WR snapshot was collected<main id="notice" type='explain'><h4>Note</h4><p>For V4.4.x, this column is available starting from V4.4.2. </p></main>|
| MUTI_QUERY_BATCH_TOTAL        | bigint(20)        | NO   | The number of times the SQL batch was executed since the execution plan was generated<main id="notice" type='explain'><h4>Note</h4><p>For V4.4.x, this column is available starting from V4.4.2. </p></main>|
| MUTI_QUERY_BATCH_DELTA        | bigint(20)        | NO   | The number of times the SQL batch has been executed since the last WR snapshot was collected.<main id="notice" type='explain'><h4>Note</h4><p>This column is available starting with V4.4.2 for V4.4.x versions. </p></main>|
| FULL_TABLE_SCAN_TOTAL         | bigint(20)        | NO   | The number of times the SQL has performed a full table scan since the execution plan was generated.<main id="notice" type='explain'><h4>Note</h4><p>This column is available starting with V4.4.2 for V4.4.x versions. </p></main>|
| FULL_TABLE_SCAN_DELTA         | bigint(20)        | NO   | The number of times the SQL has performed a full table scan since the last WR snapshot was collected.<main id="notice" type='explain'><h4>Note</h4><p>This column is available starting with V4.4.2 for V4.4.x versions. </p></main>|
| ERROR_COUNT_TOTAL             | bigint(20)        | NO   | The number of times the SQL has encountered errors since the execution plan was generated.<main id="notice" type='explain'><h4>Note</h4><p>This column is available starting with V4.4.2 for V4.4.x versions. </p></main>|
| ERROR_COUNT_DELTA             | bigint(20)        | NO   | The number of times the SQL has encountered errors since the last WR snapshot was collected.<main id="notice" type='explain'><h4>Note</h4><p>This column is available starting with V4.4.2 for V4.4.x versions. </p></main>|
| LATEST_ACTIVE_TIME            | timestamp(6)      | YES  | The last active time of the plan corresponding to the SQL ID + Plan Hash on a node.<main id="notice" type='explain'><h4>Note</h4><p>This column is available starting with V4.4.2 for V4.4.x versions. </p></main>     |

The following table describes the values of the `SQL_TYPE` column and the corresponding statements.
...
Query the basic performance statistics of SQL statements in the cluster and display the first record.
...
*************************** 1. row ***************************
             ...
             EXECUTIONS_TOTAL: 1305770
             EXECUTIONS_DELTA: 1082
             DISK_READS_TOTAL: 148
             DISK_READS_DELTA: 0
            BUFFER_GETS_TOTAL: 943
            BUFFER_GETS_DELTA: 0
           ELAPSED_TIME_TOTAL: 141890527
           ELAPSED_TIME_DELTA: 117708
               CPU_TIME_TOTAL: 0
               CPU_TIME_DELTA: 0
                 CCWAIT_TOTAL: 0
                 CCWAIT_DELTA: 0
 ...
 PHYSICAL_READ_REQUESTS_DELTA: 0
    ...
    PHYSICAL_READ_BYTES_DELTA: 0
         ...
         ROWS_PROCESSED_TOTAL: 1305770
         ROWS_PROCESSED_DELTA: 1082
     MEMSTORE_READ_ROWS_TOTAL: 63308
     MEMSTORE_READ_ROWS_DELTA: 0
MINOR_SSSTORE_READ_ROWS_TOTAL: 306
MINOR_SSSTORE_READ_ROWS_DELTA: 0
MAJOR_SSSTORE_READ_ROWS_TOTAL: 1294344
MAJOR_SSSTORE_READ_ROWS_DELTA: 1082
                    RPC_TOTAL: 0
                    RPC_DELTA: 0
                FETCHES_TOTAL: 0
                FETCHES_DELTA: 0
                  RETRY_TOTAL: 0
                  RETRY_DELTA: 0
              PARTITION_TOTAL: 1305770
              PARTITION_DELTA: 1082
             NESTED_SQL_TOTAL: 0
             NESTED_SQL_DELTA: 0
         ...
         PLAN_CACHE_HIT_TOTAL: 1305769
         PLAN_CACHE_HIT_DELTA: 1082
             MUTI_QUERY_TOTAL: 0
             MUTI_QUERY_DELTA: 0
