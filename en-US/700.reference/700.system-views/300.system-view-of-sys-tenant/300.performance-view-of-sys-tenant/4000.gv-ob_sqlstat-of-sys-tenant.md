...
The `GV$OB_SQLSTAT` view displays basic performance statistics of SQL statements in the cluster. Each SQL statement is represented as a row (i.e., each unique combination of `SQL_ID` and `Plan_Hash` is a row). The columns with `_DELTA` in their names indicate the incremental statistics from the last WR snapshot collection to the current time.
...
| PLAN_TYPE                     | bigint(20)          | NO   |    For SQL plan caching, this field indicates the type of the plan:<ul><li>`1` indicates a local plan. </li> <li>`2` indicates a remote plan. </li> <li>`3` indicates a distributed plan. </li></ul> </br>For PL/SQL object caching, this field indicates the type of the PL/SQL object:<ul><li>`1` indicates a procedure. </li> <li>`2` indicates a function. </li> <li>`3` indicates a package. </li> <li>`4` indicates an anonymous block. </li></ul> |
...
| WRITE_THROTTLE_TOTAL          | bigint(20)          | NO   |   The total time during which write throttling occurred when data was written to the MemStore.    |
| WRITE_THROTTLE_DELTA          | bigint(20)          | NO   |   The incremental time during which write throttling occurred when data was written to the MemStore.    |
...
| ROUTE_MISS_TOTAL              | bigint(20)          | NO   |   The total number of times that the request partition was not hit since the OBServer was started.<main id="notice" type='explain'><h4>Note</h4><ul><li>For V4.3.x, this column was introduced in V4.3.5. </li> <li>For V4.2.x, this column was introduced in V4.2.2. </li></ul></main>   |
| ROUTE_MISS_DELTA              | bigint(20)          | NO   |   The number of times that the request partition was not hit since the last WR snapshot collection.<main id="notice" type='explain'><h4>Note</h4><ul><li>For V4.3.x, this column was introduced in V4.3.5. </li> <li>For V4.2.x, this column was introduced in V4.2.2. </li></ul></main>   |
| FIRST_LOAD_TIME               | timestamp(6)        | YES  |   The plan generation time.<main id="notice" type='explain'><h4>Note</h4><ul><li>For V4.3.x, this column was introduced in V4.3.5. </li> <li>For V4.2.x, this column was introduced in V4.2.5. </li></ul></main>   |
| PLAN_CACHE_HIT_TOTAL          | bigint(20)          | NO   |   The total number of times that the Plan Cache was hit since the OBServer was started.<main id="notice" type='explain'><h4>Note</h4><ul><li>For V4.3.x, this column was introduced in V4.3.5. </li> <li>For V4.2.x, this column was introduced in V4.2.5. </li></ul></main>   |
| PLAN_CACHE_HIT_DELTA          | bigint(20)          | NO   |   The number of times that the Plan Cache was hit since the last WR snapshot collection.<main id="notice" type='explain'><h4>Note</h4><ul><li>For V4.3.x, this column was introduced in V4.3.5. </li> <li>For V4.2.x, this column was introduced in V4.2.5. </li></ul></main>  |
| MUTI_QUERY_TOTAL              | bigint(20)        | NO   | The number of times that the SQL statement was in a Multi Query since the execution plan was generated.<main id="notice" type='explain'><h4>Note</h4><p>For V4.6.x, this column was introduced in V4.6.0. </p></main>|
| MUTI_QUERY_DELTA              | bigint(20)        | NO   | The number of times that the SQL statement was in a Multi Query since the last WR snapshot collection.<main id="notice" type='explain'><h4>Note</h4><p>For V4.6.x, this column was introduced in V4.6.0. </p></main>|
| MUTI_QUERY_BATCH_TOTAL        | bigint(20)        | NO   | The number of times that the SQL batch was executed since the execution plan was generated.<main id="notice" type='explain'><h4>Note</h4><p>For V4.6.x, this column was introduced in V4.6.0. </p></main>|
...
The values of the `SQL_TYPE` column and the corresponding statements are shown in the following table.
...
*************************** 1. row ***************************
                     ...
                     SVR_PORT: 2882
                    TENANT_ID: 1
                       SQL_ID: 8B75F7F4622EB81B6D0FAFF31632EDCB
                      PLAN_ID: 0
                    PLAN_HASH: 5626833931290989975
                    PLAN_TYPE: 0
                    QUERY_SQL:
                     SQL_TYPE: 0
                       MODULE: NULL
...
MAJOR_SSSTORE_READ_ROWS_DELTA: 0
             ...
             ROUTE_MISS_TOTAL: 0
             ROUTE_MISS_DELTA: 0
              FIRST_LOAD_TIME: 2026-02-25 12:42:04.468165
