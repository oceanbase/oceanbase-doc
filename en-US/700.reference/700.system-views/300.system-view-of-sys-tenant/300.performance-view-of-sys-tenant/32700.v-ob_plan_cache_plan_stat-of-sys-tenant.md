...
| TYPE                  | bigint(20)          | NO         | For a SQL plan, this field indicates the type of the plan:  <ul><li> 1: a local plan.  </li><li> 2: a remote plan. </li><li>3: a distributed plan.  </li></ul>For a PL object, this field indicates the type of the PL object:<ul><li> 1: a procedure.    </li><li> 2: a function. </li><li>3: a package.  </li><li> 4: an anonymous block.   </li></ul>  |
...
| STATEMENT             | longtext            | NO         | For a SQL plan, this field contains the parameterized SQL statement. For an anonymous block, this field contains the parameterized anonymous block statement. This field is invalid for other objects.                     |
| QUERY_SQL             | longtext            | NO         | For a SQL plan, this field contains the original SQL statement queried when the plan was first loaded. For an anonymous block, this field contains the parameterized anonymous block statement.                        |
...
| EVO_CPU_TIME          | bigint(20) unsigned | NO         | The total CPU time consumed during evolutions, in microseconds.                                                                 |
| TIMEOUT_COUNT         | bigint(20)          | NO         | The number of times the query timed out.                                                                            |
| PS_STMT_ID            | bigint(20)          | NO         | The Prepare ID corresponding to the request:<ul><li>`-1`: indicates that the SQL statement did not use the PS protocol. </li><li>Non-`-1`: indicates that the SQL statement used the PS protocol. The value uniquely identifies the statement.   </li></ul>   |
...
| IS_USE_JIT            | tinyint(4)          | NO         | Indicates whether the SQL plan is compiled and executed.                                                               |
...
Query the status of each cached object in the plan cache of all tenants on the current OBServer node.
