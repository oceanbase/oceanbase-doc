...
<main id="notice" type='explain'>
  <h4>Note</h4>
  <p>The view name has been changed from <code>V$PLAN_CACHE_PLAN_STAT</code> to <code>V$OB_PLAN_CACHE_PLAN_STAT</code> since OceanBase Database V4.0.0. </p>
</main>
...
The `V$OB_PLAN_CACHE_PLAN_STAT` view displays the status of each object cached by the current tenant in the plan cache on the current OBServer node.
  ...
  <main id="notice" type='notice'>
    <h4>Notice</h4>
    <p>This view caches both SQL plan objects and PL objects (such as anonymous blocks, PL packages, and PL functions). Some columns take effect only for specific objects. </p>
  </main>
...
| SQL_ID                | varchar(32)         | NO         | The SQL ID corresponding to the cached object. If it is a PL object, the value of this column is `NULL`. |
...
| STATEMENT             | longtext            | NO         | If the cached object is an SQL plan, the value of this column is a parameterized SQL statement. If the cached object is an anonymous block, the value of this column is a parameterized anonymous block statement. This column is invalid for other types of objects. |
| QUERY_SQL             | longtext            | NO         | If the cached object is an SQL plan, the value of this column is the original SQL statement queried when the plan was loaded for the first time. If the cached object is an anonymous block, the value of this column is a parameterized anonymous block statement. |
...
| APPLICATION_WAIT_TIME | bigint(20) unsigned | NO         | The total amount of wait time of Application wait events. |
| CONCURRENCY_WAIT_TIME | bigint(20) unsigned | NO         | The total amount of wait time of Concurrency wait events. |
| USER_IO_WAIT_TIME     | bigint(20) unsigned | NO         | The total amount of wait time of User I/O wait events. |
| ROWS_PROCESSED        | bigint(20)          | NO         | The total amount of wait time of Schedule wait events. |
...
| PL_SCHEMA_ID          | bigint(20) unsigned | NO         | For PL objects that are not anonymous blocks, this column indicates the schema ID of the cached object. For anonymous blocks, this column is equivalent to `PS_STMT_ID`. For SQL plans, this column is meaningless. |
