...
The `V$OB_TENANT_RESOURCE_LIMIT` view displays the logical resource usage, upper limits, effective limit conditions, and maximum resource usage after a server restart for all tenants on the current OBServer node.
...
| EFFECTIVE_LIMIT_TYPE | varchar(128) | NO   | The type of the effective limit, such as configuration items, memory, CPU, and disk.    |

## Sample query

Query the logical resource usage, upper limits, effective limit conditions, and maximum resource usage after a server restart for all tenants on the current OBServer node.
