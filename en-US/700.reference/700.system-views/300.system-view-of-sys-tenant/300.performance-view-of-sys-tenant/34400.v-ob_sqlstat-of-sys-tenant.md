...
The `V$OB_SQLSTAT` view displays basic performance statistics of SQL statements on the current node. Each SQL statement is represented by a row (that is, each unique combination of `SQL_ID` and `Plan_Hash` values). The statistics with `_DELTA` in their names indicate the incremental statistics from the last WR snapshot to the current time.
...
| MODULE                      | varchar(64)         | NO   |    Application module when the statement was first parsed.    |
| ACTION                      | varchar(64)         | NO   |    Application action when the statement was first parsed.      |
...
| EXECUTIONS_TOTAL            | bigint(20)          | NO   |    Total number of times the plan was executed in the plan cache.     |
| EXECUTIONS_DELTA            | bigint(20)          | NO   |    Incremental number of times the plan was executed in the plan cache.      |
...
| ELAPSED_TIME_TOTAL          | bigint(20)          | NO   |    Total time taken by the statement. For parallel execution scenarios, this value is the sum of the times taken by each execution thread.          |
| ELAPSED_TIME_DELTA          | bigint(20)          | NO   |    Incremental time taken by the statement.   |
| CPU_TIME_TOTAL              | bigint(20)          | NO   |    Total CPU time.       |
| CPU_TIME_DELTA              | bigint(20)          | NO   |    Incremental CPU time.     |
| CCWAIT_TOTAL                | bigint(20)          | NO   |    Total time taken by concurrent wait events.       |
| CCWAIT_DELTA                | bigint(20)          | NO   |    Incremental time taken by concurrent wait events.       |
| USERIO_WAIT_TOTAL           | bigint(20)          | NO   |    Total time taken by I/O wait events.      |
| USERIO_WAIT_DELTA           | bigint(20)          | NO   |    Incremental time taken by I/O wait events.      |
| APWAIT_TOTAL                | bigint(20)          | NO   |    Total time taken by application wait events.        |
| APWAIT_DELTA                | bigint(20)          | NO   |    Incremental time taken by application wait events.      |
...
| WRITE_THROTTLE_TOTAL        | bigint(20)          | NO   |    Total time throttled when writing to the MemStore.    |
| WRITE_THROTTLE_DELTA        | bigint(20)          | NO   |    Incremental time throttled when writing to the MemStore.    |
...
| MEMSTORE_READ_ROWS_DELTA    | bigint(20)          | NO   |    Incremental number of rows read from the MemStore.    |
| MINOR_SSSTORE_READ_ROWS_TOTAL| bigint(20)          | NO   |    Total number of rows read from the MINOR SSStore.   |
| MINOR_SSSTORE_READ_ROWS_DELTA| bigint(20)          | NO   |    Incremental number of rows read from the MINOR SSStore.  |
| MAJOR_SSSTORE_READ_ROWS_TOTAL| bigint(20)          | NO   |    Total number of rows read from the MAJOR SSStore.     |
| MAJOR_SSSTORE_READ_ROWS_DELTA| bigint(20)          | NO   |    Incremental number of rows read from the MAJOR SSStore.   |
| RPC_TOTAL                   | bigint(20)          | NO   |    Total number of RPCs.    |
| RPC_DELTA                   | bigint(20)          | NO   |    Incremental number of RPCs.   |
| FETCHES_TOTAL               | bigint(20)          | NO   |    Total number of times the result set was fetched.   |
| FETCHES_DELTA               | bigint(20)          | NO   |    Incremental number of times the result set was fetched.     |
...
| ROUTE_MISS_TOTAL            | bigint(20)          | NO   |    Total number of times the request has not hit the request partition since the OBServer node started. <main id="notice" type='explain'><h4>Note</h4><ul><li>In V4.3.x, this field was introduced in V4.3.5. </li> <li>In V4.2.x, this field was introduced in V4.2.2. </li></ul></main>   |
| ROUTE_MISS_DELTA            | bigint(20)          | NO   |    Number of times the request has not hit the request partition since the last WR snapshot was collected. <main id="notice" type='explain'><h4>Note</h4><ul><li>In V4.3.x, this field was introduced in V4.3.5. </li> <li>In V4.2.x, this field was introduced in V4.2.2. </li></ul></main>   |
| FIRST_LOAD_TIME             | timestamp(6)        | NO   |    Plan generation time. <main id="notice" type='explain'><h4>Note</h4><ul><li>In V4.3.x, this field was introduced in V4.3.5. </li> <li>In V4.2.x, this field was introduced in V4.2.5. </li></ul></main>   |
| PLAN_CACHE_HIT_TOTAL        | bigint(20)          | NO   |    Total number of times the plan cache hit since the OBServer node started. <main id="notice" type='explain'><h4>Note</h4><ul><li>In V4.3.x, this field was introduced in V4.3.5. </li> <li>In V4.2.x, this field was introduced in V4.2.5. </li></ul></main>   |
| PLAN_CACHE_HIT_DELTA        | bigint(20)          | NO   |    Number of times the plan cache hit since the last WR snapshot was collected. <main id="notice" type='explain'><h4>Note</h4><ul><li>In V4.3.x, this field was introduced in V4.3.5. </li> <li>In V4.2.x, this field was introduced in V4.2.5. </li></ul></main>  |

The following table describes the statement types and corresponding values in the `SQL_TYPE` column of the view.
