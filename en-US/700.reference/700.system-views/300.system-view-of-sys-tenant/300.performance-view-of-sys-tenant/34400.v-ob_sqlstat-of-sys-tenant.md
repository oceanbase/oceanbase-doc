...
The `V$OB_SQLSTAT` view displays basic performance statistics for SQL statements on the current node. Each SQL statement is represented as a row (i.e., each unique combination of `SQL_ID` and `Plan_Hash` is a row). Columns with `_DELTA` in their names indicate the incremental values from the last WR snapshot collection to the current time.
...
| PLAN_TYPE                     | bigint(20)          | NO   |    For SQL plan caching, this column indicates the type of the plan:<ul><li>`1`: Local Plan </li> <li>`2`: Remote Plan</li> <li>`3`: Distributed Plan </li></ul> </br>For PL object caching, this column indicates the type of the PL object:<ul><li>`1`: Procedure</li> <li>`2`: Function</li> <li>`3`: Package</li> <li>`4`: Anonymous Block </li></ul> |
| QUERY_SQL                     | longtext            | NO   | Full SQL text |
| SQL_TYPE                      | bigint(20)          | NO   | Statement type |
| MODULE                        | varchar(64)         | NO   |   Module of the application when the statement was first parsed    |
| ACTION                        | varchar(64)         | NO   |   Action of the application when the statement was first parsed      |
...
| CPU_TIME_DELTA                | bigint(20)          | NO   |   Incremental total CPU time consumed     |
...
| WRITE_THROTTLE_TOTAL          | bigint(20)          | NO   |   Total time consumed by throttling during write to the MemStore    |
| WRITE_THROTTLE_DELTA          | bigint(20)          | NO   |   Incremental time consumed by throttling during write to the MemStore    |
...
| FIRST_LOAD_TIME               | timestamp(6)        | NO   |   The plan generation time<main id="notice" type='explain'><h4>Note</h4><ul><li>For V4.3.x, this column is available starting with V4.3.5. </li> <li>For V4.2.x, this column is available starting with V4.2.5. </li></ul></main>   |
...
| PLAN_CACHE_HIT_DELTA          | bigint(20)          | NO   |   The number of times that the Plan Cache is hit since the last WR snapshot was collected.<main id="notice" type='explain'><h4>Note</h4><ul><li>For V4.3.x, this column is available starting with V4.3.5. </li> <li>For V4.2.x, this column is available starting with V4.2.5. </li></ul></main>  |
| MUTI_QUERY_TOTAL              | bigint(20)        | NO   | The number of times that the SQL statement is in a multi-query since the execution plan was generated.<main id="notice" type='explain'><h4>Note</h4><p>For V4.4.x, this column is available starting with V4.4.2. </p></main>|
| MUTI_QUERY_DELTA              | bigint(20)        | NO   | The number of times that the SQL statement is in a multi-query since the last WR snapshot was collected.<main id="notice" type='explain'><h4>Note</h4><p>For V4.4.x, this column is available starting with V4.4.2. </p></main>|
| MUTI_QUERY_BATCH_TOTAL        | bigint(20)        | NO   | The number of times that the SQL batch is executed since the execution plan was generated.<main id="notice" type='explain'><h4>Note</h4><p>For V4.4.x, this column is available starting with V4.4.2. </p></main>|
| MUTI_QUERY_BATCH_DELTA        | bigint(20)        | NO   | The number of times the SQL batch has been executed since the last WR snapshot collection. <main id="notice" type='explain'><h4>Note</h4><p>For V4.4.x, this field was introduced in V4.4.2. </p></main>|
| FULL_TABLE_SCAN_TOTAL         | bigint(20)        | NO   | The number of times the SQL has performed a full table scan since the execution plan was generated. <main id="notice" type='explain'><h4>Note</h4><p>For V4.4.x, this field was introduced in V4.4.2. </p></main>|
| FULL_TABLE_SCAN_DELTA         | bigint(20)        | NO   | The number of times the SQL has performed a full table scan since the last WR snapshot collection. <main id="notice" type='explain'><h4>Note</h4><p>For V4.4.x, this field was introduced in V4.4.2. </p></main>|
| ERROR_COUNT_TOTAL             | bigint(20)        | NO   | The number of times the SQL has reported an error since the execution plan was generated. <main id="notice" type='explain'><h4>Note</h4><p>For V4.4.x, this field was introduced in V4.4.2. </p></main>|
| ERROR_COUNT_DELTA             | bigint(20)        | NO   | The number of times the SQL has reported an error since the last WR snapshot collection. <main id="notice" type='explain'><h4>Note</h4><p>For V4.4.x, this field was introduced in V4.4.2. </p></main>|
| LATEST_ACTIVE_TIME            | timestamp(6)      | NO   | The last active time of the SQL ID + Plan Hash pair on a node. <main id="notice" type='explain'><h4>Note</h4><p>For V4.4.x, this field was introduced in V4.4.2. </p></main>     |
...
The following example shows how to query the basic performance statistics of SQL statements on the current node and display the first record.
