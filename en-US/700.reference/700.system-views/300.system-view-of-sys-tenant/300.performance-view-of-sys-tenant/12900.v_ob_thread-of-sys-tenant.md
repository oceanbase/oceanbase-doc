...
| CGROUP_PATH | varchar(256) | NO  | The path of the cgroup to which the thread belongs. The information for this column comes from the CPU line under the `/proc/observer-pid/task/tid/cgroup` path.<main id="notice" type='explain'><h4>Note</h4><p>This column is introduced in V4.2.2.</p></main>  |
