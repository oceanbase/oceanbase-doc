...
| USER_IO_WAIT_TIME       | bigint(20)           | NO | The total time consumed by all user I/O events. |
...
| OTHERSTAT_3_VALUE       | bigint(20)           | NO | For the TABLE SCAN operator, it indicates the number of rows read from the storage `SSSTORE_READ_ROW_COUNT`, and supports real-time statistics.<main id="notice" type='explain'><h4>Note</h4><ul><li>Starting from V4.3.3 BP1 in V4.3.x, the TABLE SCAN operator supports real-time statistics, which are no longer collected only after the operator finishes calculation. </li><li>Starting from V4.2.4 in V4.2.x, the TABLE SCAN operator supports real-time statistics, which are no longer collected only after the operator finishes calculation. </li></ul></main> |
...
| OTHERSTAT_4_VALUE       | bigint(20)           | NO | For the TABLE SCAN operator, it indicates the number of rows read from the storage `MEMSTORE_READ_ROW_COUNT`, and supports real-time statistics. |
