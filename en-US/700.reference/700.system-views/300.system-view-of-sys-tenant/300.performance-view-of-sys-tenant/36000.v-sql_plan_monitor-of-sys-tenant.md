...
The `V$SQL_PLAN_MONITOR` view displays the operation-level statistics of slow queries on the current OBServer node for all tenants. Each operation has one statistics record.
...
| PHYSICAL_WRITE_BYTES    | bigint(0) unsigned   | NO | The number of I/O write requests issued by the operator. |
...
| OTHERSTAT_3_VALUE       | bigint(20)           | NO | The total number of major SSTables in the range read in the push-down or non-push-down path. |
...
| OTHERSTAT_4_VALUE       | bigint(20)           | NO | The number of rows in the minor SSTables, mini SSTables, and MemTables read in the push-down or non-push-down path. |
...
| OTHERSTAT_6_VALUE       | bigint(20)           | NO | The total number of rows in the microblocks opened in the push-down path within the range.<main id="notice" type='explain'><h4>Note</h4><p>This field is available starting from V4.5.0 in V4.5.x. </p></main>|
...
| OTHERSTAT_8_VALUE       | bigint(20)           | NO | The number of microblocks accessed without pre-fetching by using the Skip Index information.<main id="notice" type='explain'><h4>Note</h4><p>This field is available starting from V4.5.0 in V4.5.x. </p></main>|
...
| OTHER_XML               | varchar(255)         | NO | This column stores structured data that cannot be written to the reserved fields but needs to be provided for external use. It is parsed by external tools. |
...
| SKIPPED_ROWS_COUNT      | bigint(20)           | NO | The total number of rows not processed in the operator during vectorized mode (total number of filtered rows). |
| PROFILE                 | varchar(65535)       | NO | This column displays the PROFILE of the SQL PLAN MONITOR extended monitoring metrics. <main id="notice" type='explain'><h4>Note</h4><p>This column is available starting with V4.4.1. </p></main>|
...
Query the operation-level statistics of slow queries on the current OBServer node for all tenants.
