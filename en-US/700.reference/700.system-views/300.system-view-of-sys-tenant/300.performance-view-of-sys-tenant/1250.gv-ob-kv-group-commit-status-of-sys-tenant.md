...
In the sys tenant, the `GV$OB_KV_GROUP_COMMIT_STATUS` view provides information about the group commit status of all client connections for OBKV tenants.
...
| TENANT_ID | bigint(20) | NO | The tenant ID: <li> 1: the sys tenant ID <li>Other values: the ID of a user tenant or a meta tenant. |
...
Query the group commit status information of all client connections for the OBKV tenant on all nodes, and display the records from the first day.
