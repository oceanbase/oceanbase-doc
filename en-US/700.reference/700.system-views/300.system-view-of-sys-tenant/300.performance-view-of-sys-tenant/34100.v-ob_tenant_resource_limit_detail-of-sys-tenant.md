...
The `V$OB_TENANT_RESOURCE_LIMIT_DETAIL` view displays the limits on the logical resources on the current OBServer node in all tenants and the maximum value of each limit.
...
Query the limits on the logical resources on the current OBServer node in all tenants and the maximum value of each limit.
