...
|dir-name-en||
|tenant-type||
|slug|v-ob-obrpc-incoming-sys-tenant|
...
This view displays the statistics of RPC requests received by all tenants on the current OBServer node.
...
| COUNT          | bigint(20)   | NO             | The number of RPC requests of this type received.   |
...
| LAST_TIMESTAMP | timestamp(6) | NO             | The time when the data was last updated.               |
| DCOUNT         | bigint(20)   | NO             | The request count for each Pcode RPC. The statistics are collected every 30 seconds.  |

## Sample query

In the sys tenant, query the statistics of all RPC requests received on the current node and find out which requests have a waiting time in the queue greater than 1 second.
...
* View the statistics of RPC requests received by all tenants on all nodes: [GV$OB_RPC_INCOMING](1900.gv-ob_obrpc_incoming-of-sys-tenant.md)

* View the statistics of RPC requests sent by all tenants on the current node: [V$OB_RPC_OUTGOING](32400.v-ob_obrpc_outgoing-of-sys-tenant.md)