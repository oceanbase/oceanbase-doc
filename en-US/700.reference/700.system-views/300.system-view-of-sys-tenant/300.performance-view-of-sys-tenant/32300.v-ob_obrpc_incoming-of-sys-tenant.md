...
|slug|v-ob_obrpc_incoming-of-sys-tenant|
...
The `V$OB_RPC_INCOMING` view displays the statistics of all RPC requests received by all tenants on the current OBServer node.
...
In the system tenant, query which RPC requests received by the current node have a queue wait time greater than 1s.
...
* [GV$OB_RPC_INCOMING](1900.gv-ob_obrpc_incoming-of-sys-tenant.md): statistics about all RPC requests received by all tenants on all nodes

* [V$OB_RPC_OUTGOING](32400.v-ob_obrpc_outgoing-of-sys-tenant.md): statistics about all RPC requests sent by all tenants on the current node