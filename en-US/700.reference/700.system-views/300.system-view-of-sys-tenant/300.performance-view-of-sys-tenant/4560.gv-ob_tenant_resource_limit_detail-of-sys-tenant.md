...
The `GV$OB_TENANT_RESOURCE_LIMIT_DETAIL` view displays the limits on logical resources for all tenants across all OBServer nodes and the maximum values for each limit.
...
Query the limits on logical resources for all tenants across all OBServer nodes and the maximum values for each limit, and display the first 5 records.
