...
This view displays the PX thread usage for all tenants on all OBServer nodes.
...
| LOCAL_PARALLEL_SESSION_COUNT | bigint(20)          | NO         | The number of sessions being executed in parallel on the local server.          |

## Sample query

In the sys tenant, query the PX thread usage for all tenants on all OBServer nodes.
