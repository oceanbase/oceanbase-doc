...
| SESSION_ID | bigint(20) | NO | For V4.3.x: <ul><li>For versions earlier than V4.3.5 BP2: <br> This field indicates the server session ID in all connection modes (direct mode and ODP mode). </li><li>For V4.3.5 BP2 and later: <ul><li>In ODP mode: <ul><li>If the value of the <code>client_session_id_version</code> parameter in ODP is 2, this field indicates the client session ID. </li> <li>If the value of the <code>client_session_id_version</code> parameter in ODP is 1, this field indicates the server session ID. </li></ul></ul></li></ul> |
