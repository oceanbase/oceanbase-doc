...
The `GV$OB_SQLSTAT` view displays the basic performance statistics of SQL statements in the cluster, with each SQL statement displayed as one row (that is, each unique value of `SQL_ID` + `Plan_Hash` is displayed as one row). Columns that contain `_DELTA` indicate the increments of statistical values from the last WR snapshot collection to the current time.
...
| PLAN_TYPE | bigint(20) | NO | For SQL plan caching, this field indicates the type of the plan: <ul><li>`1`: indicates Local Plan </li> <li>`2`: indicates Remote Plan</li> <li>`3`: indicates Distributed Plan </li></ul> </br>For the PL object cache, this field indicates the type of the PL object:<ul><li>`1`:indicates Procedure</li> <li>`2`: indicates Function</li> <li>`3`: indicates Package</li> <li>`4`: Indicates Anonymous Block </li></ul> |
| QUERY_SQL | longtext | NO | Full SQL text |
| SQL_TYPE | bigint(20) | NO | Statement type |
| MODULE | varchar(64) | YES | The application Module when the statement was parsed for the first time, by`DBMS_APPLICATION_INFO.SET_MODULE`Package settings |
| ACTION | varchar(64) | YES | Application Action when the statement was parsed for the first time, through`DBMS_APPLICATION_INFO.SET_ACTION`Package settings |
