...
|slug|gv-ob_sql_workarea_memory_info-of-sys-tenant|
...
| WORKAREA_HOLD_SIZE     | bigint(20) | NO             | The current hold memory size of the workarea (in bytes). <main id="notice" type='explain'><h4>Note</h4><p>Since the actual usage amount cannot be obtained from the memory management module, only the hold value can be retrieved.</p></main>     |
...
| MEM_TARGET             | bigint(20) | NO             | The target size of available memory for the current workarea (in bytes). The difference from max_auto_work_area is that this value is a certain proportion of the max_auto_work_area value.     |
...
| DRIFT_SIZE             | bigint(20) | NO             | The current change in required memory size (in bytes). Mainly used to track fluctuation values; when the fluctuation reaches a certain range, it will automatically trigger a recalculation of the global bound size.    |
