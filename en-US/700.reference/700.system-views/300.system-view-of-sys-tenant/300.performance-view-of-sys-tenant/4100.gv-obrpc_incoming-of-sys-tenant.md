...
|slug|gv-ob_rpc_incoming-sys-tenant|
...
Displays the statistics of RPC requests received by all tenants on all OBServer nodes.
...
| PCODE | bigint(20) | NO | Packet Code of RPC |
...
In the sys tenant, view all RPC requests received on all nodes and identify those with a queue wait time greater than 1 second.
...
* View RPC statistics received by all tenants on the current node: [V$OB_RPC_INCOMING](34600.v-obrpc_incoming-of-sys-tenant.md)

* View PRC statistics sent by all tenants on all nodes: [GV$OB_RPC_OUTGOING](4200.gv-obrpc_outgoing-of-sys-tenant.md)
