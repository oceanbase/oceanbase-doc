...
| SID                     | bigint(20) unsigned | NO             | For V4.3.x: <ul><li> Versions earlier than V4.3.5 BP2: <br> This field indicates the server session ID in all connection modes (direct mode and ODP mode). </li><li> V4.3.5 BP2 and later: <ul><li> In direct mode: <br> This field indicates the server session ID. </li> <li> In ODP mode: </li><ul><li> When the value of the <code>client_session_id_version</code> parameter in ODP is 2, this field indicates the client session ID. </li> <li> When the value of the <code>client_session_id_version</code> parameter in ODP is 1, this field indicates the server session ID. </li></ul></ul></li></ul>																	 |
| CLIENT_IP               | varchar(46)         | NO             | <li> When the client connects to the OBServer node through ODP, this field indicates the IP address of the ODP that forwards the request.  <li> When the client directly connects to the OBServer node, this field indicates the IP address of the client that sends the request.  |
...
| SQL_ID                  | varchar(32)         | NO             | The ID of the SQL statement. <main id="notice" type='explain'><h4>Note</h4><ul><li>For V4.3.x, starting from V4.3.2, the field displays the actual MD5 value for PL requests executed by using the CALL statement or anonymous blocks. </li> <li>For V4.2.x, starting from V4.2.3, the field displays the actual MD5 value for PL requests executed by using the CALL statement or anonymous blocks. </li></ul></main>    |
...
| PLAN_TYPE               | bigint(20)          | NO             | The type of the execution plan: <li> local   <li> remote   <li> distribute																		 |
| IS_INNER_SQL            | tinyint(4)          | NO             | Indicates whether the request contains an internal SQL statement.																		 |
...
| CONSISTENCY_LEVEL       | bigint(20)          | NO             | The consistency level. Valid values: <ul><li>`-1`: invalid. </li> <li>`1`: specifies to read data stored in the SSTable. </li><li>`2`: weak consistency read. </li><li>`3`: strong consistency read. </li></ul>  |
...
| REQUEST_MEMORY_USED     | bigint(20)          | NO             | The memory consumed by the request, in bytes.																		 |
...
| PS_CLIENT_STMT_ID       | bigint(20)          | NO   | The field records the client Prepare ID of the request: <ul><li> `-1`: indicates that the PS protocol is not used for the current SQL statement. That is, the statement is directly executed without preprocessing.  </li><li> A value other than `-1`: indicates that the PS protocol is used for the current SQL statement, and this value is the unique identifier returned by the client for the statement. This value is used to reference the previously preprocessed SQL statement in subsequent operations. </li></ul>    |
| PS_INNER_STMT_ID        | bigint(20)          | NO   | The field records the internal (database-level) Prepare ID of the request: <ul><li> `-1`: indicates that the PS protocol is not used for the current SQL statement. That is, the statement is directly executed without preprocessing.  </li><li> A value other than `-1`: indicates that the PS protocol is used for the current SQL statement, and this value is the unique identifier returned by the client for the statement. This value is used to reference the previously preprocessed SQL statement in subsequent operations. </li></ul>    |
...
| REQUEST_TYPE            | bigint(20)          | NO             | The type of the request: <li> 0: invalid.   <li> 1: internal request.   <li> 2: local request. For example, a local plan.   <li> 3: remote request.   <li> 4: distributed request.   <li> 5: SQL prepare request.   <li> 6: SQL execute statement request.    |
...
| PARAMS_VALUE            | longtext            | NO             | The values of parameters bound to the PS statement.																		 |
| RULE_NAME | varchar(256) | NO   | The name of the rule.   <main id="notice" type='explain'><h4>Note</h4><p>This field is introduced in V4.1.0. </p></main> |
| PARTITION_HIT | tinyint(4) | NO | <ul><li> If a DAS execution plan is obtained, the value is 1 if the local partition is hit, and 0 otherwise.  </li><li> If no DAS plan is obtained, the value is 1 by default.  </li></ul>  <main id="notice" type='explain'><h4>Note</h4><p>This field is introduced in V4.1.0. </p></main> |
...
| TX_STATE_VERSION | bigint(20) unsigned | NO | The transaction state version of the SQL statement <main id="notice" type='explain'><h4>Note</h4><p>This field is introduced in V4.1.0. </p></main> |
| FLT_TRACE_ID | varchar(1024) | NO | The trace_id of the full-link trace. If this field is empty, the statement is not monitored by full-link trace. This field is a UUID, which is different from a trace. Its format is similar to: `000600d6-a5de-038c-6c80-df07e4e79149`<main id="notice" type='explain'><h4>Note</h4><p>This field is introduced in V4.2.1. </p></main> |
| PL_TRACE_ID             | varchar(128)        | NO   | The trace ID of the outer PL statement. If no outer PL statement exists, this field is set to NULL. <main id="notice" type='explain'><h4>Note</h4><ul><li>This field is introduced in V4.3.x in V4.3.0. </li> <li>This field is introduced in V4.2.x in V4.2.2. </li></ul></main>  |
| PLSQL_EXEC_TIME         | bigint(20)          | NO   | The time spent executing PL statements (excluding SQL execution time), in microseconds. <main id="notice" type='explain'><h4>Note</h4><ul><li>This field is introduced in V4.3.x in V4.3.0. </li> <li>This field is introduced in V4.2.x in V4.2.2. </li></ul></main>   |
| FORMAT_SQL_ID                   | varchar(32)         | NO   | The MD5 value generated by formatting the SQL text of the statement. <main id="notice" type='explain'><h4>Note</h4><ul><li>This field is supported in V4.3.x from V4.3.4. </li><li>This field is introduced in V4.2.x from V4.2.3. </li></ul></main> |
| STMT_TYPE                       | varchar(128)        | YES  | The DML type is returned as needed: <ul><li>The value `DDL_STMT` is returned for DDL statements. </li><li>The value `DCL_STMT` is returned for DCL statements. </li><li>The value `TCL_STMT` is returned for TCL statements. </li><li>The value `OTHER_STMT` is returned for other types. </li></ul> <main id="notice" type='explain'><h4>Note</h4><ul><li>This field is supported in V4.3.x from V4.3.4. </li><li>This field is introduced in V4.2.x from V4.2.3. </li></ul></main> |
| TOTAL_MEMSTORE_READ_ROW_COUNT   | bigint(20)          | NO   | The total number of rows read from MemStore during the entire process. This variable is displayed only in the thread that displays the query text. <main id="notice" type='explain'><h4>Note</h4><ul><li>This field is introduced in V4.3.x in V4.3.0. </li> <li>This field is introduced in V4.2.x in V4.2.2. </li></ul></main>   |
| TOTAL_SSSTORE_READ_ROW_COUNT    | bigint(20)          | NO   | The total number of rows read from SSStore during the entire process. This variable is displayed only in the thread that displays the query text. <main id="notice" type='explain'><h4>Note</h4><ul><li>This field is introduced in V4.3.x in V4.3.0. </li> <li>This field is introduced in V4.2.x in V4.2.2. </li></ul></main>  |
| PROXY_USER                      | varchar(128)        | NO  |<ul><li>If the user logs in as a proxy user, the name of the proxy user is returned. </li><li>If the user does not log in as a proxy user, this field is empty. </li></ul> <main id="notice" type='explain'><h4>Note</h4><ul><li>This field is introduced in V4.3.x in V4.3.2. </li> <li>This field is introduced in V4.2.x in V4.2.3. </li></ul> </main> |
| SEQ_NUM                         | bigint(20)          | NO   | The statement sequence number in the transaction. <main id="notice" type='explain'><h4>Note</h4><ul><li>This field is introduced in V4.3.x in V4.3.5. </li> <li>This field is introduced in V4.2.x in V4.2.5. </li></ul> </main>    |
| USER_CLIENT_PORT                | bigint(20)          | NO   | The port number of the client. </li></ul><main id="notice" type='explain'><h4>Note</h4><p>This field is introduced in V4.3.5 BP4. The field is set to NULL by default. </p></main>     |
| TRANS_STATUS                    | varchar(256)        | NO   | The transaction status. </li></ul><main id="notice" type='explain'><h4>Note</h4><p>This field is introduced in V4.3.5 BP4. The field is set to NULL by default. </p></main>  |
| NETWORK_WAIT_TIME               | bigint(20) unsigned | NO   | The total time of all network events, in microseconds. <main id="notice" type='explain'><h4>Note</h4><ul><li>This field is supported in V4.3.x from V4.3.5. </li> <li>This field is introduced in V4.2.x from V4.2.3. </li></ul></main>     |
| PLSQL_COMPILE_TIME              | bigint(20)          | NO   | The time spent compiling PL statements. <main id="notice" type='explain'><h4>Note</h4><p><ul><li>This field is introduced in V4.3.x in V4.3.5. </li><li>This field is introduced in V4.2.x in V4.2.5 BP2. </li></ul></p></main>|
| INSERT_DUPLICATE_ROW_COUNT | bigint(20) | YES | The number of duplicate rows in `insertup` or `replace into` statements. <main id="notice" type='explain'><ul><h4>Note</h4><li>This field is introduced in V4.3.5 BP2. </li>|
