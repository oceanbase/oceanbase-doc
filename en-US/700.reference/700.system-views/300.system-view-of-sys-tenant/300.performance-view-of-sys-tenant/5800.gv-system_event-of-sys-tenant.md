...
This view displays the wait events of all tenants on all OBServer nodes in the cluster, including the number of occurrences, wait time, timeout count, and other statistics for each type of wait event.
...
In the sys tenant, query all wait events of all tenants on all nodes, and display the first 10 records.
...
* To query the definitions of all wait events of a tenant: [V$EVENT_NAME](30300.v-event_name-of-sys-tenant.md)
