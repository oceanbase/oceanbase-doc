...
|slug|oceanbase-dba_ob_recover_table_jobs-mysql-compatible-mode|
...
| STATUS | varchar(64) | NO | The task status. Valid values: <ul><li>PREPARE: The task is initializing.  </li><li>RECOVERING: The task is recovering. </li><li>RESTORE_AUX_TENANT: The auxiliary tenant is being restored. </li><li>PRECHECK_IMPORT: The import is being prechecked. </li><li>GEN_IMPORT_JOB: The import task is initializing. </li><li>IMPORTING: The table is being imported. </li><li> CANCELING: The task is being canceled.</li><li>COMPLETED: The task is completed. </li><li>FAILED: The task failed. </li></ul> |
...
| IMPORT_ALL | tinyint(4) | NO | Indicates whether to import all tables in the tenant. The value is 1 only when the user specifies `RECOVER TABLE *.*`. In other cases, the value is 0. |
...
During table-level recovery, a user tenant views the information about the table-level recovery task.
...
For more information about how to view the progress of a table-level recovery task, see [View the progress of a table-level recovery task](../../../../600.manage/600.backup-and-recovery/700.recover-table/400.view-the-table-recovery-progress.md).