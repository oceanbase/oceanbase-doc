...
|slug|o-dba_ob_recover_table_jobs-mysql-mode|
...
This view displays information about table-level recovery tasks in the current tenant.
...
| STATUS | varchar(64) | NO | The status of the task. Valid values: <ul><li>PREPARE: The task is being initialized.  </li><li>RECOVERING: The task is being recovered. </li><li>RESTORE_AUX_TENANT: The auxiliary tenant is being restored. </li><li>PRECHECK_IMPORT: The import is being checked. </li><li>GEN_IMPORT_JOB: The import task is being initialized. </li><li>IMPORTING: The table is being imported. </li><li> CANCELING: The task is being canceled.</li><li>COMPLETED: The task is completed. </li><li>FAILED: The task failed. </li></ul> |
...
| IMPORT_ALL | tinyint(4) | NO | Indicates whether to import all tables under the tenant. This value is 1 only when the `RECOVER TABLE *.*` statement is specified. In other cases, the value is 0. |
...
| RESULT | varchar(64) | YES | The result of the table-level recovery task:<ul><li>SUCCESS  </li><li>FAIL </li></ul> |
| COMMENT | longtext | YES | The comment. |
| DESCRIPTION | longtext | YES | The DESCRIPTION parameter specified in the restore command. |

## Sample query

In the process of table-level recovery, a user tenant queries information about the table-level recovery task.
...
For more information about how to view the progress of a table-level recovery task, see [View the progress of table-level recovery](../../../../600.manage/600.backup-and-recovery/700.recover-table/400.view-the-table-recovery-progress.md).