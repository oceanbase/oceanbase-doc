...
The `DBA_OB_VECTOR_INDEX_TASK_HISTORY` view displays the historical information of vector index partition rebuild tasks, segment merge tasks, and major compactions in the current tenant. You can use the `TASK_TYPE` column to distinguish the task types.
...
| TASK_TYPE	                | bigint(20)	| NO   | The type of the task. Valid values: <ul><li><code>0</code>: asynchronous construction</li><li><code>1</code>: index partition rebuild</li><li><code>5</code>: freeze major compaction</li><li><code>6</code>: segment merge</li></ul>     | |
...
| TASK_INFO                  | long text    | NO   | The detailed progress information of the task. The information is in JSON format and contains details about each segment in a segment merge task, such as the SCN, key range, index, and memory usage of the segment. This helps you understand the task execution process. |
...
| merge_segments | array | The information of the segments to be merged. The number of elements in the array is the same as that of merge_segments_cnt. |
