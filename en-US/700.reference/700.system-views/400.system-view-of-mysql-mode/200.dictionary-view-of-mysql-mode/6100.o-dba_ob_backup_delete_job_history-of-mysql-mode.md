...
|machine-translation||
|slug|oceanbase.dba_ob_backup_delete_job_history-o-dba_ob_backup_delete_job_history-of-mysql-mode|
...
This view displays the history of backup cleanup tasks executed in the current tenant.
...
| TYPE | varchar(64) | NO | The cleanup type. Valid values: <ul><li>DELETE OBSOLETE BACKUP: obsolete cleanup. </li></ul> |
| PARAMETER | timestamp(6) | NO | The content of the parameter. The content is as follows: <li>For automatic cleanup or obsolete cleanup: policy_name = xxx; expired_time = xxx; redundancy:2; backup_copies:1; <li>For manual cleanup of a backup set: backup_set_id = 1; dest_id = 1; dest = xxxxx; <li>For manual cleanup of a backup piece: dest_round_id = 1; backup_piece_id = 1; dest_id = 1; dest = xxxx; <li>For manual cleanup of a backup round: dest_round_id = 1; dest_id = 1; dest = xxxx; <li>For cleanup of all backups in a destination: dest_id = 1; dest = xxx; |
...
Query the history of backup cleanup tasks executed in the current user tenant, and display the first 5 records.
