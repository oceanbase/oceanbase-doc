...
| STATUS | varchar(64) | NO | The status of the import task. Value: `IMPORT_FINISH`. |
| IMPORT_ALL | tinyint(4) | NO | Indicates whether to import all tables in the source tenant. This value is 1 only when the `RECOVER TABLE *.*` option is specified. Otherwise, this value is 0. |
...
Query the result of an import task in the user tenant.
