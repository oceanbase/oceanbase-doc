...
|slug|oceanbase-dba_wr_sqltext-mysql-compatible-mode|
...
The `DBA_WR_SQLTEXT` view displays the SQL text collected by the WR module for the current tenant.
...
Query the SQL text collected by the current user in the WR mode and display the first record.
