...
| ACTION_ORDER | bigint(20) | NO | In instances with the same`EVENT_MANIPULATION`and`ACTION_TIMING`The order and position of trigger operations in the trigger list for a value on the same table. |
...
| CHARACTER_SET_CLIENT | varchar(32) | NO | When creating a trigger`character_set_client`The value of the system variable in the current session. |
