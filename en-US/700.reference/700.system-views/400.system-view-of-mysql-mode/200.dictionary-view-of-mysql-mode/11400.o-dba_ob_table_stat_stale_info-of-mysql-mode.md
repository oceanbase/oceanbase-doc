...
| PARTITION_NAME | varchar(128) | YES | The name of the partition. If this column is NULL for a partitioned table, it indicates that the record is aggregated at the table level. |
| SUBPARTITION_NAME | varchar(128) | YES | The name of the subpartition. If this column is NULL for a subpartitioned table, it indicates that the record is aggregated at the table or partition level. |
| LAST_ANALYZED_ROWS | bigint(20) | YES | The number of rows when the statistics were last collected. NULL indicates that the statistics have never been collected. |
| LAST_ANALYZED_TIME | timestamp(6) | YES | The time when the statistics were last collected. NULL indicates that the statistics have never been collected. |
...
| IS_STALE | varchar(3) | NO | Indicates whether the statistics are stale. <ul><li>`YES` indicates that the statistics are stale. <ul><li>For a table that has had statistics collected, the condition for staleness is `INSERTS + UPDATES + DELETES > LAST_ANALYZED_ROWS * STALE_PERCENT / 100`. </li><li>For a table that has not had statistics collected, the condition for staleness is `INSERTS + UPDATES + DELETES > 0`. </li></ul>  </li><li>`NO` indicates that the statistics are not stale. </li></ul>|

## Sample query

Query the number of DML operations performed on the `t_subpart` table since the last statistics collection and the staleness of the statistics.
