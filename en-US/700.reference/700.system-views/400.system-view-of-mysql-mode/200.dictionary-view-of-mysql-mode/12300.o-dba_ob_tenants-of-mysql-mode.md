...
| FLASHBACK_LOG_SCN | bigint(20) | NO | The point in time when the tenant last performed log pruning. After a failover, you can execute an operation to prune logs of the original primary tenant to the specified point in time and then add the standby tenant to the new primary tenant. <main id="notice" type='explain'><h4>Note</h4><p>Introduced in V4.4.1. </p></main> |
