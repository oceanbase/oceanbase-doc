...
| PLUGIN | varchar(64) | NO | The name of the plugin used for password hash calculation. Valid values are as follows:<ul><li>An empty string, which is equivalent to <code>mysql_native_password</code>.</li><li><code>ob_native_password</code>, which is equivalent to <code>mysql_native_password</code> in Oracle-compatible mode.</li><li><code>mysql_native_password</code>.</li><li><code>caching_sha2_password</code>.</li></ul><main id="notice" type='explain'><h4>Note</h4><p>For V4.6.x, this field is available starting with V4.6.1.</p></main> |
...
| TYPE | varchar(4) | NO | Type. Valid values:<ul><li>`USER`: user.</li> <li>`ROLE`: role.</li></ul> |
...
| OLD_PASSWORD | varchar(128) | NO | The hash value (ciphertext) of the old password. This field is valid when the dual-password feature is enabled or during a password rotation period. <main id="notice" type='explain'><h4>Note</h4><p>This field is available starting with V4.6.1.</p></main> |
| OLD_PASSWORD_START_TIME | bigint(20) | NO | The time when the old password became effective. The value `-1` indicates that no old password is available. In Oracle tenants, this value is also used to determine whether the old password is still within the rotation period. <main id="notice" type='explain'><h4>Note</h4><p>This field is available starting with V4.6.1.</p></main> |
