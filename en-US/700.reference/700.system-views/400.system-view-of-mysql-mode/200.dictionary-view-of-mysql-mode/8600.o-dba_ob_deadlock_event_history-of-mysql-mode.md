...
This view `DBA_OB_DEADLOCK_EVENT_HISTORY` displays the history of deadlock events of the current user tenant.
...
| EXTRA_NAME1    | longtext            | YES  | The name of additional information 1 that the upper layer wishes to add to the deadlock report event. Valid value: `wait_sql`.     |
| EXTRA_VALUE1   | longtext            | YES  | The content of additional information 1 that the upper layer wishes to add to the deadlock report event. It indicates the SQL statement that causes the deadlock in the transaction.     |
| EXTRA_NAME2    | longtext            | YES  | The name of additional information 2 that the upper layer wishes to add to the deadlock report event. Valid value: `hold_sql_request_time`.     |
| EXTRA_VALUE2   | longtext            | YES  | The content of additional information 2 that the upper layer wishes to add to the deadlock report event. It indicates the request time of the SQL statement that holds the lock.     |
| EXTRA_NAME3    | longtext            | YES  | The name of additional information 3 that the upper layer wishes to add to the deadlock report event. Valid value: `hold_sql`.     |
| EXTRA_VALUE3   | longtext            | YES  | The content of additional information 3 that the upper layer wishes to add to the deadlock report event. It indicates the SQL statement that holds `cycle_idx - 1` row records corresponding to the Object.     |
