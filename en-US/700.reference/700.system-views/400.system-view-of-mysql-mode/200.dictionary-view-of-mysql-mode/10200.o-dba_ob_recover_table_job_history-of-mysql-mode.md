...
|machine-translation||
|slug|oceanbase.dba_ob_recover_table_job_history-o-dba_ob_recover_table_job_history-of-mysql-mode|
...
This view displays the results of table-level restore tasks in the current tenant.
...
| STATUS | varchar(64) | NO | The status of the task. Valid values: <ul><li>COMPLETED: The task is completed. </li><li>FAILED: The task failed. </li></ul> |
...
| IMPORT_ALL | tinyint(4) | NO | Indicates whether to import all tables in the tenant. The value is 1 only when the `RECOVER TABLE *.*` clause is specified. Otherwise, the value is 0. |
...
| REMAP_TABLEGROUP_LIST | longtext | YES | The list of remapped table groups. You can remap tables from the original table group to the target table group. The format is as follows: tg1:new_tg1, tg2:new_tg2. |
| REMAP_TABLESPACE_LIST | longtext | YES | The list of remapped tablespaces. You can remap tables from the original tablespace to the target tablespace. The format is as follows: ts1:new_ts1, ts2:new_ts2. |
...
Query the results of table-level restore tasks in the current tenant.
