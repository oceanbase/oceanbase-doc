...
Displays the results of the current tenant table restore task.
...
| STATUS | varchar(64) | NO | Task status. Valid values: <ul><li>COMPLETED: The task is completed. </li><li>FAILED: The task failed. </li></ul> |
| AUX_TENANT_NAME | varchar(64) | NO | Secondary Tenant Name |
| TARGET_TENANT_NAME | varchar(64) | NO | Target Tenant Name Identifier |
| IMPORT_ALL | tinyint(4) | NO | Specifies whether to import all tables in the entire tenant. This parameter is valid only when the user specifies`RECOVER TABLE *.*`When the time is up, this value is 1; in other scenarios, the value is 0. |
...
| EXTERNAL_KMS_INFO | varchar(4096) | YES | If the source tenant has key management enabled, you must configure this via the session. |
...
Users can query the results of table-level restore tasks for their own tenant.
...
For detailed operations and instructions on viewing the results of a table-level restore task, see [View table-level restore results](../../../../600.manage/600.backup-and-recovery/700.recover-table/500.view-the-table-recovery-history.md).
