...
The `DBA_OB_TABLE_SPACE_USAGE` view is used to display the sizes of all user tables (leader replicas in a multi-replica environment) in the current tenant.
...
The following example shows how to query the sizes of tables in a user tenant in a multi-replica environment in SN mode.
