...
| INITIATOR_TENANT_ID | bigint(20) | NO | The ID of the tenant that initiated the job, which is the ID of the tenant that executed the backup SQL statement. |
...
| JOB_LEVEL | varchar(4096) | NO | The level of the job. Valid values:<li>`CLUSTER`: The `sys` tenant initiates backup jobs for all tenants.<li>`SYS_TENANT`: The `sys` tenant initiates backup jobs for specified tenants.<li>`USER_TENANT`: A user tenant initiates its own backup jobs. |
