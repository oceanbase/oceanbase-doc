...
This view displays the history of load balancing tasks executed under the current tenant.
...
Query the history of load balancing tasks executed under the current tenant.
...
* Query log stream-level load balancing tasks (Task) under the current tenant: [DBA_OB_BALANCE_TASKS](8000.o-dba_ob_balance_tasks-of-mysql-mode.md)

* Query log stream-level load balancing jobs (Job) under the current tenant:
...
* Query tablet-level load balancing tasks (Task) under the current tenant:
...
* Query transfer partition tasks (Task) under the current tenant:
