|description||
|---|---|
| machine-translation ||
|keywords||
|dir-name||
|dir-name-en||
|tenant-type|MySQL Mode|
|slug|o-dba_ind_partitions-mysql-mode|

# oceanbase.DBA_IND_PARTITIONS

<main id="notice" type='explain'>
<h4>Note</h4>
<p>This view is available starting with V4.0.0. </p>
</main>

## Purpose

This view displays the primary and secondary partition information of all indexes in the current tenant.

## Columns

|          Column           |       Type        | Nullable |                                                                        Description                                                                         |
|-------------------------|-----------------|------------|---------|
| INDEX_OWNER             | varchar(128)    | NO         | The owner of the index.|
| INDEX_NAME              | varchar(128)    | NO         | The name of the index.  |
| TABLE_NAME              | varchar(128)    | NO         | The name of the table to which the index belongs.|
| COMPOSITE               | varchar(3)      | NO         | Indicates whether the partition belongs to a local index of a composite-partitioned table:<ul><li>YES</li><li>NO</li></ul>   |
| PARTITION_NAME          | varchar(128)    | NO         | The name of the partition.   |
| SUBPARTITION_COUNT      | bigint(20)      | NO         | The number of subpartitions.   |
| HIGH_VALUE              | mediumtext      | NO         | The expression of the value bound to the partition.   |
| HIGH_VALUE_LENGTH       | bigint(20)      | NO         | The length of the value bound to the partition.      |
| PARTITION_POSITION      | bigint(20)      | NO         | The position of the partition in the index.             |
| STATUS                  | varchar(8)      | NO         | Indicates whether the index partition is available.              |
| TABLESPACE_NAME         | varchar(30)     | NO         | The name of the tablespace that contains the partition.                 |
| PCT_FREE                | bigint(0)       | NO         | This column is not supported. The value of this column is NULL by default.               |
| INI_TRANS               | bigint(0)       | NO         | This column is not supported. The value of this column is NULL by default.             |
| MAX_TRANS               | bigint(0)       | NO         | This column is not supported. The value of this column is NULL by default.                     |
| INITIAL_EXTENT          | bigint(0)       | NO         | This column is not supported. The value of this column is NULL by default.      |
| NEXT_EXTENT             | bigint(0)       | NO         | This column is not supported. The value of this column is NULL by default.      |
| MIN_EXTENT              | bigint(0)       | NO         | This column is not supported. The value of this column is NULL by default.    |
| MAX_EXTENT              | bigint(0)       | NO         | This column is not supported. The value of this column is NULL by default.           |
| MAX_SIZE                | bigint(0)       | NO         | This column is not supported. The value of this column is NULL by default.                  |
| PCT_INCREASE            | bigint(0)       | NO         | This column is not supported. The value of this column is NULL by default.                 |
| FREELISTS               | bigint(0)       | NO         | This column is not supported. The value of this column is NULL by default.             |
| FREELIST_GROUPS         | bigint(0)       | NO         | This column is not supported. The value of this column is NULL by default.            |
| LOGGING                 | varchar(7)      | NO         | This column is not supported. The value of this column is NULL by default.               |
| COMPRESSION             | varchar(13)     | NO         | Indicates whether the partition index is compressed:<ul><li> Enabled (ENABLE)   </li><li> Disabled (DISABLE)</li></ul>    |
| BLEVEL                  | bigint(0)       | NO         | This column is not supported. The value of this column is NULL by default. |
| LEAF_BLOCKS             | bigint(0)       | NO         | This column is not supported. The value of this column is NULL by default.                  |
| DISTINCT_KEYS           | bigint(0)       | NO         | This column is not supported. The value of this column is NULL by default.                |
| AVG_LEAF_BLOCKS_PER_KEY | bigint(0)       | NO         | This column is not supported. The value of this column is NULL by default.                      |
| AVG_DATA_BLOCKS_PER_KEY | bigint(0)       | NO         | This column is not supported. The value of this column is NULL by default.                      |
| CLUSTERING_FACTOR       | bigint(0)       | NO         | This column is not supported. The value of this column is NULL by default.             |
| NUM_ROWS                | bigint(0)       | NO         | This column is not supported. The value of this column is NULL by default.            |
| SAMPLE_SIZE             | bigint(0)       | NO         | This column is not supported. The value of this column is NULL by default.               |
| LAST_ANALYZED           | date            | NO         | This column is not supported. The value of this column is NULL by default.              |
| BUFFER_POOL             | varchar(7)      | NO         | This column is not supported. The value of this column is NULL by default.                   |
| FLASH_CACHE             | varchar(7)      | NO         | This column is currently not supported. This column is set to NULL by default. |
| CELL_FLASH_CACHE        | varchar(7)      | NO         | This column is currently not supported. This column is set to NULL by default.              |
| USER_STATS              | varchar(3)      | NO         | This column is currently not supported. This column is set to NULL by default.            |
| PCT_DIRECT_ACCESS       | bigint(0)       | NO         | This column is currently not supported. This column is set to  NULL by default.       |
| GLOBAL_STATS            | varchar(3)      | NO         | This column is currently not supported. This column is set to  NULL by default. |
| DOMIDX_OPSTATUS         | varchar(6)      | NO         | This column is currently not supported. This column is set to NULL by default.                  |
| PARAMETERS              | text            | NO         | This column is currently not supported. This column is set to NULL by default.                |
| INTERVAL                | varchar(3)      | NO         | This column is currently not supported. This column is set to `NO` by default.          |
| SEGMENT_CREATED         | varchar(3)      | NO         | This column is currently not supported. This column is set to NULL by default.                       |
| ORPHANED_ENTRIES        | varchar(3)      | NO         | This column is currently not supported. This column is set to NULL by default.  |

## Sample query

The user tenant queries the primary partition information of the index `tbl2_f_rl` in the user tenant.

```shell
obclient [infotest]> SELECT * FROM oceanbase.DBA_IND_PARTITIONS WHERE TABLE_NAME='tbl2_f_rl'\G
```

The query result is as follows:

```shell
*************************** 1. row ***************************
            INDEX_OWNER: infotest
             INDEX_NAME: tbl2_f_rl_idx1
             TABLE_NAME: tbl2_f_rl
              COMPOSITE: YES
         PARTITION_NAME: p0
     SUBPARTITION_COUNT: 3
             HIGH_VALUE: 100
      HIGH_VALUE_LENGTH: 3
     PARTITION_POSITION: 1
                 STATUS: NULL
        TABLESPACE_NAME: NULL
               PCT_FREE: NULL
              INI_TRANS: NULL
              MAX_TRANS: NULL
         INITIAL_EXTENT: NULL
            NEXT_EXTENT: NULL
             MIN_EXTENT: NULL
             MAX_EXTENT: NULL
               MAX_SIZE: NULL
           PCT_INCREASE: NULL
              FREELISTS: NULL
        FREELIST_GROUPS: NULL
                LOGGING: NULL
            COMPRESSION: DISABLED
                 BLEVEL: NULL
            LEAF_BLOCKS: NULL
          DISTINCT_KEYS: NULL
AVG_LEAF_BLOCKS_PER_KEY: NULL
AVG_DATA_BLOCKS_PER_KEY: NULL
      CLUSTERING_FACTOR: NULL
               NUM_ROWS: NULL
            SAMPLE_SIZE: NULL
          LAST_ANALYZED: NULL
            BUFFER_POOL: NULL
            FLASH_CACHE: NULL
       CELL_FLASH_CACHE: NULL
             USER_STATS: NULL
      PCT_DIRECT_ACCESS: NULL
           GLOBAL_STATS: NULL
        DOMIDX_OPSTATUS: NULL
             PARAMETERS: NULL
               INTERVAL: NO
        SEGMENT_CREATED: NULL
       ORPHANED_ENTRIES: NULL
*************************** 2. row ***************************
            INDEX_OWNER: infotest
             INDEX_NAME: tbl2_f_rl_idx1
             TABLE_NAME: tbl2_f_rl
              COMPOSITE: YES
         PARTITION_NAME: p1
     SUBPARTITION_COUNT: 3
             HIGH_VALUE: 200
      HIGH_VALUE_LENGTH: 3
     PARTITION_POSITION: 2
                 STATUS: NULL
        TABLESPACE_NAME: NULL
               PCT_FREE: NULL
              INI_TRANS: NULL
              MAX_TRANS: NULL
         INITIAL_EXTENT: NULL
            NEXT_EXTENT: NULL
             MIN_EXTENT: NULL
             MAX_EXTENT: NULL
               MAX_SIZE: NULL
           PCT_INCREASE: NULL
              FREELISTS: NULL
        FREELIST_GROUPS: NULL
                LOGGING: NULL
            COMPRESSION: DISABLED
                 BLEVEL: NULL
            LEAF_BLOCKS: NULL
          DISTINCT_KEYS: NULL
AVG_LEAF_BLOCKS_PER_KEY: NULL
AVG_DATA_BLOCKS_PER_KEY: NULL
      CLUSTERING_FACTOR: NULL
               NUM_ROWS: NULL
            SAMPLE_SIZE: NULL
          LAST_ANALYZED: NULL
            BUFFER_POOL: NULL
            FLASH_CACHE: NULL
       CELL_FLASH_CACHE: NULL
             USER_STATS: NULL
      PCT_DIRECT_ACCESS: NULL
           GLOBAL_STATS: NULL
        DOMIDX_OPSTATUS: NULL
             PARAMETERS: NULL
               INTERVAL: NO
        SEGMENT_CREATED: NULL
       ORPHANED_ENTRIES: NULL
2 rows in set
```