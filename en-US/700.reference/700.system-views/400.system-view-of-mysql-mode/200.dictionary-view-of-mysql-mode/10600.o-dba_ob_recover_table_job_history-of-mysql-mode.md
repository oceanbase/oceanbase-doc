...
| IMPORT_ALL | tinyint(4) | NO | Indicates whether to import all tables under the tenant. This value is 1 only when the `RECOVER TABLE *.*` statement is specified. In other scenarios, this value is 0. |
...
| EXTERNAL_KMS_INFO | varchar(4096) | YES | If the source tenant has configured key management, configure this parameter in the session. |
...
For more information about how to query the results of table-level restore tasks, see [View the table restore history](../../../../600.manage/600.backup-and-recovery/700.recover-table/500.view-the-table-recovery-history.md).