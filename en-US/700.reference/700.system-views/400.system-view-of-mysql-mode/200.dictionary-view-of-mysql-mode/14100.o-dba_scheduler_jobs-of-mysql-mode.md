...
This view displays information about all scheduled jobs in the database.
...
| JOB_TYPE | varchar(16) | YES | The type of job operation. Valid values include:<ul><li>PLSQL_BLOCK  </li><li>STORED_PROCEDURE </li><li>EXECUTABLE </li><li>CHAIN </li></ul> |
...
| SCHEDULE_TYPE | varchar(12) | NO | The type of the schedule. The default value is NULL. |
...
| FILE_WATCHER_OWNER | varchar(261) | NO | The owner of the file watcher based on which the job is created. |
| FILE_WATCHER_NAME | varchar(261) | NO | The name of the file watcher based on which the job is created. |
| END_DATE | datetime(6) | YES | The date when the job will no longer run. |
| JOB_CLASS | varchar(128) | YES | The name of the job class to which the job belongs. |
| ENABLED | varchar(5) | YES | Indicates whether the job is enabled. Valid values include:<ul><li>TRUE  </li><li>FALSE </li></ul> |
| AUTO_DROP | varchar(5) | YES | Indicates whether the job is deleted after it is completed. Valid values include:<ul><li>TRUE  </li><li>FALSE </li></ul>  |
| RESTART_ON_RECOVERY | varchar(5) | NO | The default value is NULL. |
| RESTART_ON_FAILURE | varchar(5) | NO | The default value is NULL. |
| STATE | varchar(15) | YES | The job status. Valid values include:<li>DISABLED<li>RETRY SCHEDULED<li>SCHEDULED<li>BLOCKED<li>RUNNING<li>COMPLETED<li>BROKEN<li>FAILED<li>REMOTE<li>RESOURCE_UNAVAILABLE<li>SUCCEEDED<li>CHAIN_STALLED |
...
| MAX_FAILURES | bigint(0) | NO | The number of times the job is allowed to fail before it is marked as damaged. |
...
| LAST_RUN_DURATION | bigint(20) | YES | The duration of the last run in seconds. |
| NEXT_RUN_DATE | datetime(6) | NO | The date when the job is scheduled to run next. |
| SCHEDULE_LIMIT | bigint(0) | NO | The time when the job will be rescheduled if it has not been run. |
| MAX_RUN_DURATION | bigint(20) | YES | The maximum duration allowed for the job to run. |
| LOGGING_LEVEL | varchar(11) | NO | The default value is NULL. |
| STORE_OUTPUT | varchar(5) | NO | The default value is NULL. |
| STOP_ON_WINDOW_CLOSE | varchar(5) | NO | The default value is NULL. |
| INSTANCE_STICKINESS | varchar(5) | NO | The default value is NULL. |
| RAISE_EVENTS | text | NO | The default value is NULL. |
| SYSTEM | varchar(5) | NO | The default value is NULL. |
| JOB_WEIGHT | bigint(0) | NO | The weight of the job, which specifies the priority of the job when it is scheduled. |
| NLS_ENV | text | YES | The national language support (NLS) environment settings of the job. |
...
| DEFERRED_DROP | varchar(5) | NO | Indicates whether the job is deleted after it is completed. Valid values include:<ul><li>TRUE  </li><li>FALSE </li></ul> |
| ALLOW_RUNS_IN_RESTRICTED_MODE | varchar(5) | NO | Indicates whether the job is allowed to run in restricted mode. Valid values include:<ul><li>TRUE  </li><li>FALSE </li></ul> |
| COMMENTS | text | YES | The comments or description of the job. |
| FLAGS | bigint(20) | NO | The flags of the job. |
| RESTARTABLE | varchar(5) | NO | Indicates whether the job is restartable. |
...
View the running status of all scheduled jobs in the tenant.
