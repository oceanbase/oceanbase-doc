...
|machine-translation||
|slug|oceanbase.dba_ob_backup_delete_tasks-o-dba_ob_backup_delete_tasks-of-mysql-mode|
...
This view displays the information of `backup_set`/`backup_piece` cleanup tasks being executed in the current tenant.
...
| FINISH_LS_COUNT | bigint(20) | YES | The number of log stream tasks that are successfully completed. |
...
Query the `backup_set` or `backup_piece` cleanup tasks being executed in the current user tenant.
