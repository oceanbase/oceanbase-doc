...
The `role_table_grants` view displays the table-level privileges granted to activated roles in the current session, including the table-level privileges granted to other roles.
...
| IS_GRANTABLE   | varchar(3)   |  NO     | Indicates whether the role's privileges can be granted to other users.     |
...
6. View the table-level privileges granted to activated roles in the current session.
