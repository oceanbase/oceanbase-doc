...
| CFS_QUOTA_US | bigint(20) | NO | The CPU time slice of the thread, in microseconds. This field comes from the `cpu.cfs_quota_us` file in the path directory. |
| CFS_PERIOD_US | bigint(20) | NO | The control cycle time of the cgroup, in microseconds. This field comes from the `cpu.cfs_period_us` file in the path directory. |
| SHARES | bigint(20) | NO | The CPU usage ratio of the thread (only effective when the CPU is fully loaded). This field comes from the `cpu.shares` file in the path directory. |
