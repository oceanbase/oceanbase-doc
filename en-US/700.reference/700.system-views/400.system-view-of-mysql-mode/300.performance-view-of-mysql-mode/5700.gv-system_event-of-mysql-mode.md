...
This view displays the wait events on all OBServer nodes for the current tenant, and counts the number of occurrences, wait time, timeout count, and other metrics for each type of wait event.
...
| WAIT_CLASS_ID | bigint(20) | NO | The class ID of the wait event. |
...
| TIME_WAITED | double | NO | The total wait time, in 10 ms. |
| AVERAGE_WAIT | double | NO | The average wait time, in 10 ms. |
