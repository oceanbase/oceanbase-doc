...
<h4>Note</h4>
<p>The view <code>V$MEMSTORE_INFO</code> was renamed to <code>V$OB_MEMSTORE_INFO</code> in V4.0.0. In V4.0.0 and later, you must use the new view name <code>V$OB_MEMSTORE_INFO</code> to query the view. Otherwise, an error will be returned. </p>
</main>

## Purpose

This view displays the details of MemTables in all partitions of the current tenant on the current server.
...
| FREEZE_TS | bigint(20) | No | The timestamp when the MemTable is frozen. |

## Sample query

The following example shows how to query the details of MemTables in all partitions of the current tenant on the current server.
