...
This view displays the cache task information of the current tenant on the current OBServer node.
...
| STATUS          | varchar(64)           | NO             |  The status of the cache task:<ul><li>`WAITING`: The task is being initialized.</li> <li>`DONIG`: The task is being executed.</li> <li>`FINISHED`: The task has been completed.</li> <li>`SUSPENDED`: The task is suspended.</li> <li>`CANCELED`: The task is canceled.</li> <li>`FAILED`: The task has failed.</li></ul>    |
...
| RESULT          | bigint(20)            | NO             |  If the task has failed, the corresponding error code is displayed.    |
| COMMENT         | varchar(4096)         | NO             |  The detailed error description after the task has failed, which contains the `trace_id` information.    |

## Sample query

In the SS mode, a user tenant queries the cache task information of the tenant on the current OBServer node.
