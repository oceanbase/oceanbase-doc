...
<h4>Note</h4>
<p>This view is introduced in OBServer V4.3.5 BP2. </p>
</main>

## Purpose

This view displays the cache tasks of the current tenant on the current OBServer node.

<main id="notice" type='explain'>
<h4>Note</h4>
<p>Applicable only to Shared-Storage (SS) mode. The query result is an empty set in Shared-Nothing (SN) mode. </p>
</main>
...
| RESULT          | bigint(20)            | NO             |  The error code if the task fails.    |
| COMMENT         | varchar(4096)         | NO             |  The detailed error message when the task fails, which contains the `trace_id`.    |

## Sample query

In SS mode, a user tenant queries the cache tasks of the tenant on the current OBServer node.
