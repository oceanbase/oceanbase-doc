...
The `V$OB_SQLSTAT` view displays basic performance statistics for SQL statements on the current node. Each SQL statement is represented as a row (i.e., each unique combination of `SQL_ID` and `Plan_Hash` is a row). The lists with `_DELTA` in their names indicate the incremental values from the last WR snapshot collection to the current time.
...
| SOURCE_IP                     | varchar(46)         | NO   |   The IP address from which the request was sent.     |
| SOURCE_PORT                   | bigint(20)          | NO   |   The port number from which the request was sent.      |
| ROUTE_MISS_TOTAL              | bigint(20)          | NO   |   The total number of times the request partition was not hit since the OBServer was started.<main id="notice" type='explain'><h4>Note</h4><ul><li>For V4.3.x, this field is available starting with V4.3.5. </li> <li>For V4.2.x, this field is available starting with V4.2.2. </li></ul></main>   |
| ROUTE_MISS_DELTA              | bigint(20)          | NO   |   The number of times the request partition was not hit since the last WR snapshot collection started.<main id="notice" type='explain'><h4>Note</h4><ul><li>For V4.3.x, this field is available starting with V4.3.5. </li> <li>For V4.2.x, this field is available starting with V4.2.2. </li></ul></main>   |
...
| MUTI_QUERY_BATCH_DELTA        | bigint(20)        | NO   | The number of times the SQL batch has been executed since the last WR snapshot was captured. <main id="notice" type='explain'><h4>Note</h4><p>For V4.6.x, this column is available starting from V4.6.0. </p></main>|
...
| FULL_TABLE_SCAN_DELTA         | bigint(20)        | NO   | The number of full table scans performed by the SQL since the last WR snapshot was captured. <main id="notice" type='explain'><h4>Note</h4><p>For V4.6.x, this column is available starting from V4.6.0. </p></main>|
...
| ERROR_COUNT_DELTA             | bigint(20)        | NO   | The number of errors reported by the SQL since the last WR snapshot was captured. <main id="notice" type='explain'><h4>Note</h4><p>For V4.6.x, this column is available starting from V4.6.0. </p></main>|
| LATEST_ACTIVE_TIME            | TIMESTAMP(6)  | NO   | The last active time of the plan corresponding to the SQL ID and plan hash on a node. <main id="notice" type='explain'><h4>Note</h4><p>For V4.6.x, this column is available starting from V4.6.0. </p></main> |
