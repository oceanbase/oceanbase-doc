...
The `V$OB_SQLSTAT` view displays basic performance statistics for SQL statements on the current node. Each SQL statement is represented by a row (i.e., each unique combination of `SQL_ID` and `Plan_Hash`). Columns with `_DELTA` in their names indicate the incremental statistics from the last WR snapshot collection to the current time.
...
| PLAN_TYPE                     | bigint(20)          | NO   |    The type of the SQL plan. The value can be `1`, `2`, or `3`, which indicates a local plan, a remote plan, or a distributed plan, respectively. <ul><li>`1`: local plan </li> <li>`2`: remote plan</li> <li>`3`: distributed plan</li></ul>  </br> For PL object caching, this column indicates the type of the PL object. The value can be `1`, `2`, `3`, or `4`, which indicates a procedure, a function, a package, or an anonymous block, respectively. |
| QUERY_SQL                     | longtext            | NO   | The complete SQL statement. |
| SQL_TYPE                      | bigint(20)          | NO   | The type of the statement. |
| MODULE                        | varchar(64)         | NO   |   The module of the application that first parses the statement.    |
| ACTION                        | varchar(64)         | NO   |   The action of the application that first parses the statement.      |
| PARSING_DB_ID                 | bigint(20)          | NO   |   The ID of the database in which the statement is parsed.    |
| PARSING_DB_NAME               | varchar(128)        | NO   |   The name of the database in which the statement is parsed.     |
| PARSING_USER_ID               | bigint(20)          | NO   |   The ID of the user who parses the statement.     |
...
| PHYSICAL_READ_BYTES_TOTAL     | bigint(20)          | NO   |   The total number of bytes read from physical storage.     |
| PHYSICAL_READ_BYTES_DELTA     | bigint(20)          | NO   |   The incremental number of bytes read from physical storage.   |
| WRITE_THROTTLE_TOTAL          | bigint(20)          | NO   |   The total time consumed by write throttling when data is written to the MemStore.    |
| WRITE_THROTTLE_DELTA          | bigint(20)          | NO   |   The incremental time consumed by write throttling when data is written to the MemStore.    |
...
| ROUTE_MISS_TOTAL              | bigint(20)          | NO   |   The total number of times the request partition was not hit since the OBServer was started.<main id="notice" type='explain'><h4>Note</h4><ul><li>For V4.3.x, this field was introduced in V4.3.5. </li> <li>For V4.2.x, this field was introduced in V4.2.2. </li></ul></main>   |
| ROUTE_MISS_DELTA              | bigint(20)          | NO   |   The number of times the request partition was not hit since the last WR snapshot collection began.<main id="notice" type='explain'><h4>Note</h4><ul><li>For V4.3.x, this field was introduced in V4.3.5. </li> <li>For V4.2.x, this field was introduced in V4.2.2. </li></ul></main>   |
...
| PLAN_CACHE_HIT_TOTAL          | bigint(20)          | NO   |   The total number of times the Plan Cache was hit since the OBServer was started.<main id="notice" type='explain'><h4>Note</h4><ul><li>For V4.3.x, this field was introduced in V4.3.5. </li> <li>For V4.2.x, this field was introduced in V4.2.5. </li></ul></main>   |
| PLAN_CACHE_HIT_DELTA          | bigint(20)          | NO   |   The number of times the Plan Cache was hit since the last WR snapshot collection began.<main id="notice" type='explain'><h4>Note</h4><ul><li>For V4.3.x, this field was introduced in V4.3.5. </li> <li>For V4.2.x, this field was introduced in V4.2.5. </li></ul></main>  |
| MUTI_QUERY_TOTAL              | bigint(20)        | NO   | The number of times the SQL statement was in a Multi Query since the execution plan was generated.<main id="notice" type='explain'><h4>Note</h4><p>For V4.6.x, this field was introduced in V4.6.0. </p></main>|
| MUTI_QUERY_DELTA              | bigint(20)        | NO   | The number of times the SQL statement was in a Multi Query since the last WR snapshot collection began.<main id="notice" type='explain'><h4>Note</h4><p>For V4.6.x, this field was introduced in V4.6.0. </p></main>|
| MUTI_QUERY_BATCH_TOTAL        | bigint(20)        | NO   | The number of times the SQL batch was executed since the execution plan was generated.<main id="notice" type='explain'><h4>Note</h4><p>For V4.6.x, this field was introduced in V4.6.0. </p></main>|
...
| LATEST_ACTIVE_TIME            | TIMESTAMP(6)  | NO   | The last active time of the plan corresponding to the SQL ID + Plan Hash pair on a node. <main id="notice" type='explain'><h4>Note</h4><p>This column is available starting with V4.6.0 for V4.6.x. </p></main> |
...
The following example displays the basic performance statistics of SQL statements on the current node, and shows the first record.
