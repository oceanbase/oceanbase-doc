...
|slug|v-ob_px_target_monitor-mysql-mode|
...
This view displays the PX thread usage of the current tenant on the current OBServer node.
...
| VERSION                      | bigint(20) unsigned | NO         | The number of times the statistics collection has been restarted.          |
...
| PEER_TARGET_USED             | bigint(20)          | NO         | The number of PX threads used on the peer server.          |
...
Query the PX thread usage of the current tenant on the current OBServer node in the user tenant.
