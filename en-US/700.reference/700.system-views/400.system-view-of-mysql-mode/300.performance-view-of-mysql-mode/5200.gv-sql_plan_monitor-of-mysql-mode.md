...
The `GV$SQL_PLAN_MONITOR` view displays statistics at the operation level for slow queries on all OBServer nodes in the tenant. For each slow query, statistics are provided for each operation, including the time consumed by the operator and the number of rows returned. The view also provides TRACE_ID and other identifiers.
...
| PLAN_POSITION           | bigint(0) unsigned   | NO | The position of the operator among the children of its parent. |
