...
The `GV$SQL_PLAN_MONITOR` view displays the operation-level statistics of slow queries on all OBServer nodes in the current tenant. The statistics of each operation of each slow query include the amount of time consumed by the operation, the number of rows returned, and the trace ID.
...
You can query the [V$SQL_MONITOR_STATNAME](../../../../700.reference/700.system-views/400.system-view-of-mysql-mode/300.performance-view-of-mysql-mode/35800.v-sql_monitor_statname-of-mysql-mode.md) view for the value of the `OTHERSTAT_X_ID` column.

## Sample query

Query the operation-level statistics of slow queries on all OBServer nodes in the current tenant.
