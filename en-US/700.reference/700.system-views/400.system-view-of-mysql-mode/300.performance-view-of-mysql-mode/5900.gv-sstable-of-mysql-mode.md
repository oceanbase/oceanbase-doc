...
|slug|gv-ob_sstables-mysql-compatible-mode|
...
Displays the information about the MemTable and SSTable under each partition of the current tenant on all OBServer nodes.
...
In the user tenant, view the MemTable and SSTable information of each partition for log stream 1001 on all OBServer nodes.

```shell
obclient [oceanbase]> SELECT * FROM oceanbase.GV$OB_SSTABLES WHERE LS_ID=1001;
```
