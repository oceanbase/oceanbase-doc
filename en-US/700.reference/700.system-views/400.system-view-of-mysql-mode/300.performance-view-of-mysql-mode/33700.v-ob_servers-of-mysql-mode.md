...
|slug|v-ob_servers-of-mysql-mode|

# V$OB_SERVERS

## Purpose

The `V$OB_SERVERS` view displays information about the current OBServer node.
...
| CPU_CAPACITY_MAX | double | NO | The overprovisioning value of the total CPU capacity for the OBServer node. It is subject to the `resource_hard_limit` parameter. The following rule applies: `CPU_CAPACITY_MAX = CPU_CAPACITY * resource_hard_limit`. |
...
| DATA_DISK_HEALTH_STATUS | varchar(20) | NO | The health status of the data disk. Valid values: <li> `NORMAL`: The normal state.   <li> `WARNING`: A warning occurred.   <li> `ERROR`: An error occurred. |
