...
|machine-translation||
|slug|v$ob_log_stat-v-ob_log_stat-of-mysql-mode|
...
| ACCESS_MODE | varchar(32) | NO | The access mode: <ul><li> `APPEND` indicates that the log stream replica is in a state where it can be written to by the primary tenant.  </li><li> `RAW_WRITE` indicates that the log stream replica is in a state where it can only be read from by the standby tenant. </li></ul>|
...
| DEGRADED_LIST | varchar(1024) | NO | The list of full-featured replicas that are downgraded in the arbitration scenario <main id="notice" type='explain'><h4>Note</h4><p>This column is available starting with V4.1.0. </p></main> |
| LEARNER_LIST | longtext| NO | The list of read-only replicas for the current log stream <main id="notice" type='explain'><h4>Note</h4><p>This column is available starting with V4.2.0. </p></main> |
