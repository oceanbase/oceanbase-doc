...
The `GV$OB_TENANT_RESOURCE_LIMIT_DETAIL` view displays the resource limits and the maximum values of each limit for the current tenant on all OBServer nodes.
...
Query the resource limits and the maximum values of each limit for the current tenant on all OBServer nodes.
