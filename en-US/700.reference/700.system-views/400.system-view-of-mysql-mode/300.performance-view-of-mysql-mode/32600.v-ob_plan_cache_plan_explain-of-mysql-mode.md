...
The `V$OB_PLAN_CACHE_PLAN_EXPLAIN` view displays the physical execution plans cached on the current OBServer node.
  ...
  <main id="notice" type='explain'>
    <h4>Note</h4>
    <p>This view supports only the GET operation. When you query this view, you must specify the <code>TENANT_ID</code> and <code>PLAN_ID</code> fields. </p>
  </main>
...
1. Query the [V$OB_PLAN_CACHE_PLAN_STAT](32700.v-ob_plan_cache_plan_stat-of-mysql-mode.md) view to obtain the `PLAN_ID` of the current OBServer node.
...
2. Query the `V$OB_PLAN_CACHE_PLAN_EXPLAIN` view to obtain the physical execution plan in the plan cache, for example, with `PLAN_ID=605`.
