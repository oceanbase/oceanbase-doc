...
The `GV$OB_SQLSTAT` view displays the basic performance statistics of SQL statements in the cluster. Each SQL statement is represented as a row (i.e., each unique combination of `SQL_ID` and `Plan_Hash` is a row). The lists with `_DELTA` in the name indicate the incremental statistics from the last WR snapshot collection to the current time.
...
| PLAN_TYPE                     | bigint(20)          | NO   |    The type of the execution plan. <ul><li>`1`: Local Plan </li> <li>`2`: Remote Plan</li> <li>`3`: Distributed Plan </li></ul> </br>For PL object caching, this column indicates the type of the PL object. <ul><li>`1`: Procedure</li> <li>`2`: Function</li> <li>`3`: Package</li> <li>`4`: Anonymous Block </li></ul> |
| QUERY_SQL                     | longtext            | NO   | The full text of the SQL statement. |
| SQL_TYPE                      | bigint(20)          | NO   | The type of the statement. |
| MODULE                        | varchar(64)         | YES  |   The application module that parses the statement for the first time.    |
| ACTION                        | varchar(64)         | YES  |   The application action that parses the statement for the first time.      |
| PARSING_DB_ID                 | bigint(20)          | NO   |   The ID of the database that parses the statement.    |
| PARSING_DB_NAME               | varchar(128)        | NO   |   The name of the database that parses the statement.     |
| PARSING_USER_ID               | bigint(20)          | NO   |   The ID of the user that parses the statement.     |
| EXECUTIONS_TOTAL              | bigint(20)          | NO   |   The total number of times the plan is executed in the plan cache.     |
| EXECUTIONS_DELTA              | bigint(20)          | NO   |   The incremental number of times the plan is executed in the plan cache.      |
...
| ELAPSED_TIME_TOTAL            | bigint(20)          | NO   |   The total time consumed by the statement. For parallel execution, this value is the sum of the time consumed by each execution thread.          |
...
| WRITE_THROTTLE_TOTAL          | bigint(20)          | NO   |   The total time consumed by throttling during write operations to MemStore.    |
| WRITE_THROTTLE_DELTA          | bigint(20)          | NO   |   The incremental time consumed by throttling during write operations to MemStore.    |
...
| ROUTE_MISS_TOTAL              | bigint(20)          | NO   |   The total number of times that no partition is hit since the OBServer was started.<main id="notice" type='explain'><h4>Note</h4><ul><li>For V4.3.x, this column is available starting with V4.3.5. </li> <li>For V4.2.x, this column is available starting with V4.2.2. </li></ul></main>   |
| ROUTE_MISS_DELTA              | bigint(20)          | NO   |   The number of times that no partition is hit since the last WR snapshot was collected.<main id="notice" type='explain'><h4>Note</h4><ul><li>For V4.3.x, this column is available starting with V4.3.5. </li> <li>For V4.2.x, this column is available starting with V4.2.2. </li></ul></main>   |
| FIRST_LOAD_TIME               | timestamp(6)        | YES  |   The time when the plan was generated.<main id="notice" type='explain'><h4>Note</h4><ul><li>For V4.3.x, this column is available starting with V4.3.5. </li> <li>For V4.2.x, this column is available starting with V4.2.5. </li></ul></main>   |
| PLAN_CACHE_HIT_TOTAL          | bigint(20)          | NO   |   The total number of times that the Plan Cache was hit since the OBServer was started.<main id="notice" type='explain'><h4>Note</h4><ul><li>For V4.3.x, this column is available starting with V4.3.5. </li> <li>For V4.2.x, this column is available starting with V4.2.5. </li></ul></main>   |
| PLAN_CACHE_HIT_DELTA          | bigint(20)          | NO   |   The number of times that the Plan Cache was hit since the last WR snapshot was collected.<main id="notice" type='explain'><h4>Note</h4><ul><li>For V4.3.x, this column is available starting with V4.3.5. </li> <li>For V4.2.x, this column is available starting with V4.2.5. </li></ul></main>  |
| MUTI_QUERY_TOTAL              | bigint(20)        | NO   | The number of times the SQL statement was in a multi-query since the execution plan was generated.<main id="notice" type='explain'><h4>Note</h4><p>For V4.4.x, this column is available starting with V4.4.2. </p></main>|
| MUTI_QUERY_DELTA              | bigint(20)        | NO   | The number of times the SQL statement was in a multi-query since the last WR snapshot was collected.<main id="notice" type='explain'><h4>Note</h4><p>For V4.4.x, this column is available starting with V4.4.2. </p></main>|
| MUTI_QUERY_BATCH_TOTAL        | bigint(20)        | NO   | The number of times the SQL batch was executed since the execution plan was generated.<main id="notice" type='explain'><h4>Note</h4><p>For V4.4.x, this column is available starting with V4.4.2. </p></main>|
| MUTI_QUERY_BATCH_DELTA        | bigint(20)        | NO   | The number of times the SQL batch has been executed since the last WR snapshot was collected. <main id="notice" type='explain'><h4>Note</h4><p>For V4.4.x, this column was introduced in V4.4.2. </p></main>|
| FULL_TABLE_SCAN_TOTAL         | bigint(20)        | NO   | The number of full table scans since the execution plan was generated. <main id="notice" type='explain'><h4>Note</h4><p>For V4.4.x, this column was introduced in V4.4.2. </p></main>|
| FULL_TABLE_SCAN_DELTA         | bigint(20)        | NO   | The number of full table scans since the last WR snapshot was collected. <main id="notice" type='explain'><h4>Note</h4><p>For V4.4.x, this column was introduced in V4.4.2. </p></main>|
| ERROR_COUNT_TOTAL             | bigint(20)        | NO   | The number of errors since the execution plan was generated. <main id="notice" type='explain'><h4>Note</h4><p>For V4.4.x, this column was introduced in V4.4.2. </p></main>|
| ERROR_COUNT_DELTA             | bigint(20)        | NO   | The number of errors since the last WR snapshot was collected. <main id="notice" type='explain'><h4>Note</h4><p>For V4.4.x, this column was introduced in V4.4.2. </p></main>|
| LATEST_ACTIVE_TIME            | timestamp(6)      | YES  | The last active time of the plan corresponding to the SQL ID and plan hash on a node. <main id="notice" type='explain'><h4>Note</h4><p>For V4.4.x, this column was introduced in V4.4.2. </p></main>     |
