...
The `GV$OB_SQLSTAT` view displays the basic performance statistics of SQL statements in the cluster. Each SQL statement is displayed as a row (that is, each unique combination of `SQL_ID` and `Plan_Hash` is displayed as a row). The statistics that contain `_DELTA` indicate the incremental statistics from the last WR snapshot to the current time.
...
| QUERY_SQL                     | longtext            | NO   | Full SQL statement. |
| SQL_TYPE                      | bigint(20)          | NO   |    Type of the statement. |
| MODULE                        | varchar(64)         | YES  |    Module of the application that first parsed the statement.    |
| ACTION                        | varchar(64)         | YES  |    Action of the application that first parsed the statement.      |
...
| ELAPSED_TIME_TOTAL            | bigint(20)          | NO   |    Total time consumed by the statement. For parallel execution scenarios, this value is the sum of the times consumed by each execution thread.          |
| ELAPSED_TIME_DELTA            | bigint(20)          | NO   |    Increment of the total time consumed by the statement.   |
...
| PHYSICAL_READ_BYTES_DELTA     | bigint(20)          | NO   |    Increment of the total number of bytes read physically.   |
| WRITE_THROTTLE_TOTAL          | bigint(20)          | NO   |    Total time when write throttling was triggered when data was written to the MemStore.    |
| WRITE_THROTTLE_DELTA          | bigint(20)          | NO   |    Increment of the total time when write throttling was triggered when data was written to the MemStore.    |
...
| MEMSTORE_READ_ROWS_TOTAL      | bigint(20)          | NO   |    Total number of rows read from the MemStore.    |
| MEMSTORE_READ_ROWS_DELTA      | bigint(20)          | NO   |    Increment of the total number of rows read from the MemStore.    |
| MINOR_SSSTORE_READ_ROWS_TOTAL | bigint(20)          | NO   |    Total number of rows read from the MINOR SSStore.   |
| MINOR_SSSTORE_READ_ROWS_DELTA | bigint(20)          | NO   |    Increment of the total number of rows read from the MINOR SSStore.  |
| MAJOR_SSSTORE_READ_ROWS_TOTAL | bigint(20)          | NO   |    Total number of rows read from the MAJOR SSStore.     |
| MAJOR_SSSTORE_READ_ROWS_DELTA | bigint(20)          | NO   |    Increment of the total number of rows read from the MAJOR SSStore.   |
| RPC_TOTAL                     | bigint(20)          | NO   |    Total number of RPCs.    |
| RPC_DELTA                     | bigint(20)          | NO   |    Increment of the total number of RPCs.   |
| FETCHES_TOTAL                 | bigint(20)          | NO   |    Total number of result set fetches.   |
| FETCHES_DELTA                 | bigint(20)          | NO   |    Increment of the total number of result set fetches.     |
| RETRY_TOTAL                   | bigint(20)          | NO   |    Total number of retries of the SQL statement.   |
| RETRY_DELTA                   | bigint(20)          | NO   |    Increment of the total number of retries of the SQL statement.      |
| PARTITION_TOTAL               | bigint(20)          | NO   |    Total number of partitions scanned by the SQL statement.      |
| PARTITION_DELTA               | bigint(20)          | NO   |    Increment of the total number of partitions scanned by the SQL statement.    |
...
| ROUTE_MISS_TOTAL              | bigint(20)          | NO   |    Total number of times that requests were not matched to partitions since the OBServer node started.<main id="notice" type='explain'><h4>Note</h4><ul><li>For V4.3.x, this field was introduced in V4.3.5. </li> <li>For V4.2.x, this field was introduced in V4.2.2. </li></ul></main>   |
| ROUTE_MISS_DELTA              | bigint(20)          | NO   |    Number of times that requests were not matched to partitions since the last WR snapshot was collected.<main id="notice" type='explain'><h4>Note</h4><ul><li>For V4.3.x, this field was introduced in V4.3.5. </li> <li>For V4.2.x, this field was introduced in V4.2.2. </li></ul></main>   |
| FIRST_LOAD_TIME               | timestamp(6)        | YES  |    Time when the plan was generated.<main id="notice" type='explain'><h4>Note</h4><ul><li>For V4.3.x, this field was introduced in V4.3.5. </li> <li>For V4.2.x, this field was introduced in V4.2.5. </li></ul></main>   |
| PLAN_CACHE_HIT_TOTAL          | bigint(20)          | NO   |    Total number of times that plans were hit in the plan cache since the OBServer node started.<main id="notice" type='explain'><h4>Note</h4><ul><li>For V4.3.x, this field was introduced in V4.3.5. </li> <li>For V4.2.x, this field was introduced in V4.2.5. </li></ul></main>   |
| PLAN_CACHE_HIT_DELTA          | bigint(20)          | NO   |    Number of times that plans were hit in the plan cache since the last WR snapshot was collected.<main id="notice" type='explain'><h4>Note</h4><ul><li>For V4.3.x, this field was introduced in V4.3.5. </li> <li>For V4.2.x, this field was introduced in V4.2.5. </li></ul></main>  |

The following table describes the values of the `SQL_TYPE` column in the view.
