...
The `GV$OB_SQLSTAT` view displays the basic performance statistics of SQL statements in the cluster. Each SQL statement is represented as a row (i.e., each unique combination of `SQL_ID` and `Plan_Hash` corresponds to a row). The columns with `_DELTA` in their names indicate the incremental statistics from the last WR snapshot collection to the current time.
...
| PLAN_TYPE                     | bigint(20)          | NO   |    The type of the SQL plan. <ul><li>`1`: Local Plan </li> <li>`2`: Remote Plan</li> <li>`3`: Distributed Plan </li></ul> </br>For PL object cache, this field indicates the type of the PL object. <ul><li>`1`: Procedure</li> <li>`2`: Function</li> <li>`3`: Package</li> <li>`4`: Anonymous Block </li></ul> |
...
| PARSING_DB_ID                 | bigint(20)          | NO   |   The ID of the database to which the statement was parsed.    |
| PARSING_DB_NAME               | varchar(128)        | NO   |   The name of the database to which the statement was parsed.     |
...
| WRITE_THROTTLE_TOTAL          | bigint(20)          | NO   |   The total time consumed by write throttling to MemStore.    |
| WRITE_THROTTLE_DELTA          | bigint(20)          | NO   |   The incremental time consumed by write throttling to MemStore.    |
...
| ROUTE_MISS_TOTAL              | bigint(20)          | NO   |   The total number of times that the request partition is not hit since the OBServer was started.<main id="notice" type='explain'><h4>Note</h4><ul><li>For V4.3.x, this column is available starting with V4.3.5. </li> <li>For V4.2.x, this column is available starting with V4.2.2. </li></ul></main>   |
| ROUTE_MISS_DELTA              | bigint(20)          | NO   |   The number of times that the request partition is not hit since the last WR snapshot collection started.<main id="notice" type='explain'><h4>Note</h4><ul><li>For V4.3.x, this column is available starting with V4.3.5. </li> <li>For V4.2.x, this column is available starting with V4.2.2. </li></ul></main>   |
...
| PLAN_CACHE_HIT_TOTAL          | bigint(20)          | NO   |   The total number of times that the plan cache is hit since the OBServer was started.<main id="notice" type='explain'><h4>Note</h4><ul><li>For V4.3.x, this column is available starting with V4.3.5. </li> <li>For V4.2.x, this column is available starting with V4.2.5. </li></ul></main>   |
| PLAN_CACHE_HIT_DELTA          | bigint(20)          | NO   |   The number of times that the plan cache is hit since the last WR snapshot collection started.<main id="notice" type='explain'><h4>Note</h4><ul><li>For V4.3.x, this column is available starting with V4.3.5. </li> <li>For V4.2.x, this column is available starting with V4.2.5. </li></ul></main>  |
| MUTI_QUERY_TOTAL              | bigint(20)        | NO   | The number of times that the SQL statement has been in a multi-query since the execution plan was generated.<main id="notice" type='explain'><h4>Note</h4><p>For V4.6.x, this column is available starting with V4.6.0. </p></main>|
| MUTI_QUERY_DELTA              | bigint(20)        | NO   | The number of times that the SQL statement has been in a multi-query since the last WR snapshot collection.<main id="notice" type='explain'><h4>Note</h4><p>For V4.6.x, this column is available starting with V4.6.0. </p></main>|
| MUTI_QUERY_BATCH_TOTAL        | bigint(20)        | NO   | The number of times that the SQL batch has been executed since the execution plan was generated.<main id="notice" type='explain'><h4>Note</h4><p>For V4.6.x, this column is available starting with V4.6.0. </p></main>|
...
| LATEST_ACTIVE_TIME            | TIMESTAMP(6)      | YES  | The last active time of the plan corresponding to the SQL ID + Plan Hash pair on a node.<main id="notice" type='explain'><h4>Note</h4><p>This column is available starting with V4.6.0 for V4.6.x. </p></main> |

The following table describes the values in the `SQL_TYPE` column and the corresponding statements.
