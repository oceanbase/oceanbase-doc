...
This view displays the overall local cache information of the current tenant on the current OBServer node.
...
| CACHE_HIT_COUNT      | bigint(20)    | NO   |  The number of cache hits. This field is not enabled and its value is `-1`.    |
| CACHE_MISS_COUNT     | bigint(20)    | NO   |  The number of cache misses. This field is not enabled and its value is `-1`.    |
| CACHE_HIT_SIZE       | bigint(20)    | NO   |  The size of data cached to the tablet, in bytes. This field is not enabled and its value is `-1`.   |
| CACHE_MISS_SIZE      | bigint(20)    | NO   |  The size of data cached to the tablet, in bytes. This field is not enabled and its value is `-1`.   |
| INFO                 | varchar(4096) | NO   |  Other information.    |

## Sample query

In SS mode, query the overall local cache information of the current tenant on the current OBServer node.
