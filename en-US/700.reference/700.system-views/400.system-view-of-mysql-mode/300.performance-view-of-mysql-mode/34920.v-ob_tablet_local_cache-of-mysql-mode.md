...
This view displays the overall cache usage of the current tenant on the current OBServer node.
...
In SS mode, a user of a tenant queries the overall cache usage of the tenant on the current OBServer node.

```shell
obclient> SELECT SVR_IP, SVR_POR, TENANT_ID, TABLET_ID, STORAGE_CACHE_POLICY, INFO FROM oceanbase.V$OB_TABLET_LOCAL_CACHE;
```
