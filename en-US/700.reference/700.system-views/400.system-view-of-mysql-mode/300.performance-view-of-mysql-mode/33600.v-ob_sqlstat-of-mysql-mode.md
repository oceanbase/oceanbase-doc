...
The `V$OB_SQLSTAT` view displays basic SQL performance statistics on the current node. Each SQL statement occupies one row, that is, each unique value of `SQL_ID` + `Plan_Hash` occupies one row. Columns that contain `_DELTA` indicate the increment of the statistic values from the last collected WR snapshot to the current time.
...
| PLAN_TYPE | bigint(20) | NO | For SQL plan caching, this field indicates the plan type: <ul><li>`1`: indicates Local Plan </li> <li>`2`: indicates Remote Plan</li> <li>`3`: indicates a Distributed Plan </li></ul> </br>For the PL object cache, this field indicates the type of PL object:<ul><li>`1`: indicates Procedure</li> <li>`2`: indicates Function</li> <li>`3`:indicates Package</li> <li>`4`: indicates Anonymous Block </li></ul> |
| QUERY_SQL | longtext | NO | Full SQL text |
| SQL_TYPE | bigint(20) | NO | Statement type |
| MODULE | varchar(64) | NO | The application Module when the statement was first parsed, through`DBMS_APPLICATION_INFO.SET_MODULE`Package settings |
| ACTION | varchar(64) | NO | The application Action when this statement was parsed for the first time, through`DBMS_APPLICATION_INFO.SET_ACTION`Package settings |
...
View the basic SQL performance statistics on the current node and display the first record.
