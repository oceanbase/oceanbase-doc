...
|TYPE              |varchar(64)   |NO   |The type of compaction. Valid values are as follows: <ul><li>`MDS_TABLE_MERGE`: The system metadata is persisted to disk in the SSTable format. </li> <li>`MAJOR_MERGE`: A tenant-level major compaction. </li> <li>`MEDIUM_MERGE`: A partition-level major compaction. </li> <li>`MINI_MERGE`: A mini compaction that converts MemTables into a Mini SSTable. </li> <li>`MINOR_MERGE`: A minor compaction that combines multiple Mini SSTables into one new Mini SSTable or combines multiple Mini SSTables and one Minor SSTable into one new Minor SSTable. </li> <li>`META_MAJOR_MERGE`: A special type of compaction that synthesizes data from the specified time point into one Meta Major SSTable. The data format of this SSTable is the same as that of a Major SSTable, but it does not contain multi-version data or uncommitted transactions. </li></ul>   |
...
|PROGRESSIVE_COMPACTION_ROUND     |bigint(20)    |NO   |The current round of progressive compaction (if this is a full compaction, the value of this field is -1).    |
...
|COMMENTS                         |varchar(1024) |NO   |Records the history of failed compactions and the collection time of this compaction.    |
...
| KEPT_SNAPSHOT                   | varchar(128) | NO   |  The information about the multi-version sites retained in this compaction<main id="notice" type='explain'><h4>Note</h4><p>This field is introduced in V4.3.0. </p></main>    |
| MERGE_LEVEL                     | varchar(64)  | NO   |  Specifies whether to perform a major compaction or reuse microblocks in this compaction<main id="notice" type='explain'><h4>Note</h4><p>This field is introduced in V4.3.0. </p></main>    |

## Query example

A user tenant queries the historical information about minor compactions on all OBServer nodes.
