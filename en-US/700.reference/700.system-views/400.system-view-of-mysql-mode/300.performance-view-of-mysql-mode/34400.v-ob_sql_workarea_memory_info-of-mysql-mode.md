...
|slug|v-ob_sql_workarea_memory_info-mysql-mode|
...
| WORKAREA_HOLD_SIZE     | bigint(20) | NO             | The current hold memory size of Workarea. The unit is bytes. <main id="notice" type='explain'><h4>Note</h4><p>>The actual usage cannot be obtained from the memory management module, so only the hold value can be obtained.</p></main>    |
