...
|slug|v-ob_merge_info-of-mysql-mode|

# V$OB_MERGE_INFO

<main id="notice" type='explain'>
<h4>Note</h4>
<p>Starting from V4.0.0, the view <code>V$MERGE_INFO</code> is renamed to <code>V$OB_MERGE_INFO</code>. In V4.0.0 and later, you must use the new view name <code>V$OB_MERGE_INFO</code> to query data. If you use the old view name <code>V$MERGE_INFO</code> to query data, an error will occur.</p>
</main>
...
| ACTION            | varchar(64)   | NO         | The specific action of the compaction. <ul><li>`MDS_TABLE_MERGE`: The system metadata is persisted to the disk in the SSTable format. </li> <li>`MAJOR_MERGE`: A tenant-level major compaction. </li> <li>`MEDIUM_MERGE`: A partition-level major compaction. </li> <li>`MINI_MERGE`: A mini compaction that transforms MemTables into a mini SSTable. </li> <li>`MINOR_MERGE`: A minor compaction that merges multiple mini SSTables into one new mini SSTable or merges multiple mini SSTables and one minor SSTable into one new minor SSTable. </li> <li>`META_MAJOR_MERGE`: A special type of compaction that synthesizes data generated before a specified time point into one meta major SSTable. The data format of this SSTable is the same as that of a major SSTable, but it does not contain multi-version data or uncommitted transactions. </li></ul>     |
...
* [Query information about major compactions](../../../200.system-management/500.manage-data-storage/100.dump-management/400.view-dump-information.md)
