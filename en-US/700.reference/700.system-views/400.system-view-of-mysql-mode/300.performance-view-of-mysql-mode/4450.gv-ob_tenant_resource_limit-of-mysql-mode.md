...
The `GV$OB_TENANT_RESOURCE_LIMIT` view displays the logical resource usage, upper limit, effective limit conditions, and maximum resource usage after a restart for the current tenant on all OBServer nodes.
...
| MAX_UTILIZATION      | bigint(20)   | NO   | The maximum resource usage since the last server startup.    |
...
Query the logical resource usage, upper limit, effective limit conditions, and maximum resource usage after a restart for the current tenant on all OBServer nodes.
