...
The `V$OB_PLAN_CACHE_PLAN_STAT` view displays the status of each cached object in the plan cache on the current OBServer node for the current tenant.
...
| OUTLINE_ID            | bigint(20)          | NO         | The ID of the outline. -1 indicates that the plan was not generated by using a bound outline.                                         |
...
| PS_STMT_ID            | bigint(20)          | NO         | The Prepare ID corresponding to the request. <ul><li>`-1`: indicates that the SQL statement does not use the PS protocol. </li><li>Non-`-1`: indicates that the SQL statement uses the PS protocol, and the value is the unique identifier returned by the PS protocol for the statement. </li></ul>   |
| SESSID                | bigint(20) unsigned | NO         | The session ID of the cache object. <br>For V4.3.x: <ul><li> V4.3.5 BP2 and earlier: <br> This field indicates the server session ID in all connection modes (direct mode and ODP mode). </li><li> V4.3.5 BP2 and later: <ul><li> Direct mode: <br> This field indicates the server session ID. </li> <li> ODP mode: </li><ul><li> If the <code>client_session_id_version</code> parameter in ODP is set to `2`, this field indicates the client session ID. </li> <li> If <code>client_session_id_version</code> is set to `1`, this field indicates the server session ID. </li></ul></ul></li></ul>                                                                |
...
| CREATE_REASON           | varchar(4096)     | NO         | The reason for which the current plan was added. <main id="notice" type='explain'><h4>Note</h4><p>This field is available starting with V4.6.0. </p></main>|
