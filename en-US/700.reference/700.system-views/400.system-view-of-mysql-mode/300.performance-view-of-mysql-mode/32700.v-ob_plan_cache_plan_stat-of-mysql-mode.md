...
<main id="notice" type='explain'>
  <h4>Note</h4>
  <p>This view was renamed from V$PLAN_CACHE_PLAN_STAT to V$OB_PLAN_CACHE_PLAN_STAT starting from V4.0.0. </p>
</main>
...
| TYPE                  | bigint(20)          | NO         | For a SQL plan cache, this field indicates the type of the plan.  <ul><li> 1: Local Plan  </li><li> 2: Remote Plan </li><li>3: Distribute Plan  </li></ul>For a PL object cache, this field indicates the type of the PL object:<ul><li> 1: Procedure    </li><li> 2: Function </li><li>3: Package  </li><li> 4: Anonymous Block   </li></ul>   |
| IS_BIND_SENSITIVE     | bigint(20)          | NO         | Indicates whether the plan needs to be opened with ACS.                                                                    |
| IS_BIND_AWARE         | bigint(20)          | NO         | Indicates whether the plan is opened with ACS.                                                                     |
| DB_ID                 | bigint(20) unsigned | NO         | The database ID.                                                                           |
| STATEMENT             | longtext            | NO         | For a SQL plan cache, this field contains the parameterized SQL statement. For an anonymous block cache, this field contains the parameterized anonymous block statement. This field is invalid for other types of cached objects.                     |
| QUERY_SQL             | longtext            | NO         | For a SQL plan cache, this field contains the original SQL statement queried when the plan was first loaded. For an anonymous block cache, this field contains the parameterized anonymous block statement.                        |
...
| PS_STMT_ID            | bigint(20)          | NO         | The Prepare ID corresponding to the request.<ul><li>`-1`: indicates that the PS protocol is not used by the SQL statement.</li><li> Non-`-1`: indicates that the PS protocol is used by the SQL statement, and the value uniquely identifies the statement as returned by the PS protocol.  </li></ul>   |
| SESSID                | bigint(20) unsigned | NO         | The ID of the session where the object is cached.<br>For V4.3.x:<ul><li> V4.3.5 BP2 or earlier:<br>This column displays the Server Session ID in both direct connection mode and ODP mode.</li><li> V4.3.5 BP2 or later:<ul><li>Direct connection mode:<br>This column displays the Server Session ID. </li> <li>ODP mode:</li><ul><li>When the <code>client_session_id_version = 2</code> parameter is specified in ODP, this column displays the Client Session ID.  </li> <li>When <code>client_session_id_version = 1</code> is specified, this column displays the Server Session ID. </li></ul></ul></li></ul>                                                                |
| TEMP_TABLES           | longtext            | NO         | The names of temporary tables in the SQL plan. If no temporary tables exist, the column is empty.                                                     |
| IS_USE_JIT            | tinyint(4)          | NO         | Indicates whether the expression compilation execution is enabled for the SQL plan.                                                               |
...
| PL_SCHEMA_ID          | bigint(20) unsigned | NO         | For a PL object that is not an anonymous block, the value of this column is the schema ID of the cached object; for an anonymous block, the value of this column is the same as that of the ps_stmt_id column; for an SQL plan, this column has no meaning.       |
| IS_BATCHED_MULTI_STMT | tinyint(4)          | NO         | Indicates whether the plan is optimized by the batched multistmt optimization.                                                    |
...
| FIRST_GET_PLAN_TIME     | bigint(20)        | NO         | The hard parsing time.|
| FIRST_EXE_USEC          | bigint(20)        | NO         | The execution time of the SQL statement for the first time.|
| FORMAT_SQL_ID           | varchar(32)       | NO         | The Format SQL ID of the current plan.<main id="notice" type='explain'><h4>Note</h4><p>This column is available starting with V4.4.0. </p></main>|
| CACHE_NODE_ID           | bigint(20)        | NO         | The ID of the Cache Node to which the object belongs.<main id="notice" type='explain'><h4>Note</h4><p>This column is available starting with V4.6.0. </p></main>|
| PCV_ID                  | bigint(20)        | NO         | The ID of the Plan Cache Value to which the object belongs.<main id="notice" type='explain'><h4>Note</h4><p>This column is available starting with V4.6.0. </p></main>|
| PLAN_SET_ID             | bigint(20)        | NO         | The ID of the Plan Set to which the object belongs.<main id="notice" type='explain'><h4>Note</h4><p>This column is available starting with V4.6.0. </p></main>|
| CREATE_REASON           | varchar(4096)     | NO         | The reason why the plan is added.<main id="notice" type='explain'><h4>Note</h4><p>This column is available starting with V4.6.0. </p></main>|

## Sample query

Query the status of each cached object in the plan cache of the current tenant on the current OBServer node.
