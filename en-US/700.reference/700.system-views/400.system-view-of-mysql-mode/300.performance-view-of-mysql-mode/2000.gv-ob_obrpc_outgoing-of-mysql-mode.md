...
This view displays the outgoing RPC statistics for the current tenant on all OBServer nodes.
...
| COUNT          | bigint(20)   | NO             | The number of times the RPC was used.                  |
...
In the user tenant, view the RPC requests sent by the tenant on all nodes, and display the first 10 records.
...
* View the RPC statistics sent by the current tenant on the current node: [V$OB_RPC_OUTGOING](32400.v-ob_obrpc_outgoing-of-mysql-mode.md)

* View the RPC statistics received by the current tenant on all nodes: [GV$OB_RPC_INCOMING](1900.gv-ob_obrpc_incoming-of-mysql-mode.md)