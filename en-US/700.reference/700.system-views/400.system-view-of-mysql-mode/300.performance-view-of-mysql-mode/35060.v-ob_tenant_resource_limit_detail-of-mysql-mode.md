...
The `V$OB_TENANT_RESOURCE_LIMIT_DETAIL` view displays the logical resource limits and the maximum values of each limit for the current tenant on the current OBServer.
...
Query the logical resource limits and the maximum values of each limit for the current tenant on the current OBServer.
