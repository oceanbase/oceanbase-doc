...
| null | tinyint(4) | NO | <ul><li> If a DAS execution plan is obtained and a local partition is hit, the value is `1`. If no local partition is hit, the value is `0`.   </li><li>  If no DAS execution plan is obtained, the value is `1`.  </li></ul> |
