...
| SESSION_ID | bigint(20) unsigned | NO | Server-side session ID |
| PS_CLIENT_STMT_ID | bigint(20) | NO | PS ID Returned to the Client After a Prepare Request Is Sent |
| PS_INNER_STMT_ID | bigint(20) | NO | Internal PS ID in OBServer (PS ID in PS Cache) |
| STMT_TYPE | varchar(256) | NO | Statement type of the prepared statement. Valid values:`T_SELECT`,`T_INSERT`,`T_REPLACE`,`T_DELETE`,`T_UPDATE`,`T_MERGE`,`T_EXPLAIN`,`T_CREATE_TABLE`,`T_DROP_TABLE`etc. |
...
The following table describes the parameter types and their corresponding values in the `PARAM_TYPES` field of the view.
