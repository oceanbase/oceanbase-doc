...
| STMT_TYPE | varchar(256) | NO | Statement type of the prepared statement. Valid values:`T_SELECT`,`T_INSERT`,`T_REPLACE`,`T_DELETE`,`T_UPDATE`,`T_MERGE`,`T_EXPLAIN`,`T_CREATE_TABLE`,`T_DROP_TABLE`and so on. |
...
| REF_COUNT | bigint(20) | NO | `ps_session_info_`The reference count of the object,`ps_session_info_`is an attribute on a session. After a session prepares a statement, a`ps_session_info_`. |
...
[GV$OB_SESSION_PS_INFO](3410.gv-ob_session_ps_info-of-mysql-mode.md)
