...
|slug|v-ob_tablet_compaction_history-of-mysql-mode|
...
The `V$OB_TABLET_COMPACTION_HISTORY` view displays the historical information about minor and major compactions at the tablet level.

## Field descriptions

|   Field name    |      Type      | **Allow NULL** |   Description    |
|--------------------------------------|--------------|----------------|--------|
...
|TYPE    |varchar(64)    |NO   |The type of compaction. Valid values are: <ul><li>`MDS_TABLE_MERGE`: The system metadata is persisted to disk in the SSTable format. </li> <li>`MAJOR_MERGE`: A tenant-level major compaction. </li> <li>`MEDIUM_MERGE`: A partition-level minor compaction. </li> <li>`MINI_MERGE`: A mini compaction that converts MemTables into a mini SSTable. </li> <li>`MINOR_MERGE`: A minor compaction that combines multiple mini SSTables into a new mini SSTable or combines multiple mini SSTables with one minor SSTable into a new minor SSTable. </li> <li>`META_MAJOR_MERGE`: A special compaction that synthesizes data from the start of the specified time point into a Meta Major SSTable. The data format of this SSTable is the same as that of a Major SSTable, but does not contain multi-version data or uncommitted transactions. </li></ul>    |
...
|COMPRESSION_RATIO    |double    |NO   |The compression ratio of new data = The size of compressed data after compression/The size of data before compression    |
|NEW_FLUSH_DATA_RATE    |bigint(20)    |NO   |The output speed of new data, in KB/s.    |
|PROGRESSIVE_COMPACTION_ROUND    |bigint(20)    |NO   |The current round of progressive compaction (if it is a full major compaction, the value of this column is -1).    |
...
|PARALLEL_INFO    |varchar(512)    |NO   |The status of parallel tasks, which displays the statistics (minimum value, maximum value, and average value) of the data scanned, runtime, and data output for each parallel task.    |
...
|COMMENTS    |varchar(256)    |NO   |Records the history of failed compactions and collects the duration of this compaction.    |
...
| KEPT_SNAPSHOT                   | varchar(128) | NO   |  The information about the multi-version snapshots retained during the compaction<main id="notice" type='explain'><h4>Note</h4><p>This field is introduced in V4.3.0. </p></main>    |
