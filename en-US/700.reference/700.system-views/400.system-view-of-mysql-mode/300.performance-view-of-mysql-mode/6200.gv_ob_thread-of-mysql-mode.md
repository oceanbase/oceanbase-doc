...
The `GV$OB_THREAD` view displays the status information about all threads in the observer process of each OBServer node. In a user tenant, it shows the relevant threads of the current tenant.
...
In a user tenant, view the status of all threads in the observer process on each OBServer node and display the first two records.
