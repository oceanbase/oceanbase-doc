...
|dir-name-en||
|tenant-type||
|slug|observer-memory-logs|

# Memory-related logs

OceanBase Database supports viewing memory usage through logs.

## Tenant statistics

Statistics of total memory per tenant. Each row represents one tenant.
...
## Tenant ctx statistics

Memory statistics for each ctx under a tenant. ctx is a coarse-grained division; a tenant has multiple ctx values.
...
## mod statistics under tenant ctx

Finer-grained memory statistics under tenant ctx. One ctx has multiple mods.
...
## PM statistics under tenant

PM memory is temporary memory during SQL execution and is released when SQL execution ends.
