...
The observer process is single-process. An observer process uses many threads, which can be divided into worker threads and background threads.
...
* dag thread: the thread that performs tasks such as dump, merge, and migration of partitions.
