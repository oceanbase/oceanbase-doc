...
Each statement (for the Read Committed isolation level) or transaction (for Repeatable Read and Serializable isolation levels) gets a read timestamp from GTS when it starts. Data whose transaction timestamp is later than the read timestamp is skipped when data is read. This provides a uniform global snapshot for data reads.
