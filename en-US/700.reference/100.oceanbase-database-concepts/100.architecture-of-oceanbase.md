...
Each statement (for the Read Committed isolation level) or transaction (for Repeatable Read and Serializable isolation levels) starts when a timestamp is generated by GTS as the read version number of the statement or transaction. When OceanBase Database reads data, it skips data whose transaction version number is greater than the read version number. This provides a uniform global snapshot for data reads.
