...
At the beginning of each statement (for the Read Committed isolation level) or each transaction (for the Repeatable Read and Serializable isolation levels), a timestamp is obtained from the GTS to serve as the read version for the statement or transaction. When reading data, any transaction versions with timestamps greater than the current read version are ignored. This ensures that read operations have a consistent global data snapshot.
