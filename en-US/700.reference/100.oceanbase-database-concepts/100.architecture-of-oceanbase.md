...
Data is stored in four layers in a tablet: MemTable, L0 SSTable, L1 SSTable, and major SSTable. Data related to DML operations, such as `INSERT`, `UPDATE`, and `DELETE`, is first written into a MemTable. After the size of the MemTable reaches the specified threshold, a minor compaction is performed to write its data to an L0 SSTable on the disk. When the number of L0 SSTables reaches the specified threshold, the L0 SSTables are merged into one L1 SSTable (also called major compaction). During the specified daily off-peak hours, the system performs a major compaction to merge all MemTables, L0 SSTables, and L1 SSTables into one major SSTable.
