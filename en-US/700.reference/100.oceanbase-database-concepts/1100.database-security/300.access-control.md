...
A well-developed database system requires an administrator to manage it and regular users to operate on database objects in it. To access and operate on database objects, the regular users must have the corresponding privileges.
...
* `RESOURCE` role: Users with this role can only create database objects in their own schema.

* `CONNECT` role: Users with this role have the privilege to connect to databases.
...
* When creating a view, you need privileges to access objects in the view.
...
* Global privileges: the privileges to manage the entire tenant, such as modifying system settings and accessing all tables in the tenant.

* Database privileges: the privileges to manage all objects in a database, such as creating, accessing, and dropping tables in the database.
...
When a user logs in to an OBServer node, the OBServer node matches the IP address of the user against the four values. If the IP address of the user matches none of them, the access is denied. If the IP address of the user matches any one of them, the access is allowed.
...
* An IP address that contains netmasks, such as 192.168.x.x/24 or 192.168.x.x/255.255.xxx.x. In this case, mask matching is used. Access is allowed only when the client IP address and netmask equals the specified value.

## Row-level access control

OceanBase Database is compatible with Oracle Label Security, which provides row-level access control to ensure the security of reading and writing data.
