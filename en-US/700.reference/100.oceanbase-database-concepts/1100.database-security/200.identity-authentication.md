...
* mysql_native_password: The traditional password authentication plugin that uses the SHA-1 hash algorithm.
* caching_sha2_password: A password authentication plugin based on SHA-256, providing a higher level of security. This plugin is supported starting from V4.4.2 BP2 and has the following features:
  * SHA-256 hash algorithm: Replaces SHA-1 with the stronger SHA-256 algorithm and appends a random value when generating the hash.
  ...
  * Secure transmission: Supports encrypted transmission of user passwords over SSL and RSA.
...
Starting from V4.4.2 BP2, OceanBase Database supports the password authentication plugin mechanism in Oracle-compatible mode. You can explicitly specify to use the `caching_sha2_password` (SHA-256-based) authentication plugin when creating or modifying a user, which provides a higher level of security protection.
