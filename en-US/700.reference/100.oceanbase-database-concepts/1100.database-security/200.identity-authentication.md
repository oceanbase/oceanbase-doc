...
In Oracle mode, usernames are unique within a tenant, but users in different tenants can have the same username. Therefore, a user is uniquely identified across the system by the `username@tenant_name` format.

### Password complexity

To prevent malicious password attacks, OceanBase Database allows you to set a password complexity function as needed to verify the login identity and improve database security.

In Oracle mode, you can use the `PASSWORD_VERIFY_FUNCTION` attribute in a Profile to verify whether the password complexity meets the requirements. You must first create a PL function for password complexity verification that meets the following interface:
...
Oracle mode uses User Profile to implement user locking. User Profile provides two password parameters to lock a user after consecutive failed login attempts:
