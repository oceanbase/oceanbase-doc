...
* caching_sha2_password: A password authentication plugin based on SHA-256, providing a higher level of security. This plugin is supported starting from V4.6.1 and has the following features:
...
Starting from V4.6.1, OceanBase Database supports the password authentication plugin mechanism in Oracle mode. You can explicitly specify to use the `caching_sha2_password` (SHA-256-based) authentication plugin when creating or modifying a user, which provides a higher level of security protection.
