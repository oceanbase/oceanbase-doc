...
The Physical Standby Database solution is an important part of the high availability capability of OceanBase Database. It ensures high availability, data protection, and disaster recovery for your key applications.
...
OceanBase Database V4.1.0 and later provide the Physical Standby Database solution at the tenant level. That is, the primary or standby role is assigned to a tenant but not a cluster. In this topic, a primary database is equivalent to a primary tenant, and a standby database is equivalent to a standby tenant, unless otherwise specified.
...
* Log replay service
  ...
  The log replay service applies the logs written to the log storage service in the standby tenant to the MemStore in real time or with a delay, so as to ensure data consistency between primary and standby tenants.
...
CDC is a solution that captures and records data changes in the database. When data is inserted, updated, or deleted in your database, CDC captures and records the changes based on their sequence. This way, other systems can promptly process and use the latest data in your database.
