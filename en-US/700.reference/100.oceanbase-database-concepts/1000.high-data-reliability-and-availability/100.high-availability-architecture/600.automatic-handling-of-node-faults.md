...
When a node in the cluster fails, one of the following two situations applies:

* The failed node can be restarted. In this case, regardless of the previous heartbeat status of the failed machine, after restart the OBServer node can resume services once the heartbeat connection to Root Service is restored.

* The failed node is damaged and cannot be restarted. In this case, after confirming that the node is damaged and cannot be restarted, the database administrator must perform cluster management operations to delete the node. For the deletion procedure, refer to the `ALTER SYSTEM DELETE SERVER` management operation.
