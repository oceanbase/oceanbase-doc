...
Single point of failure (SPOF) is one of the main issues to address in the design of distributed systems. If you want the system to work as expected after a node fails, you must deploy multiple replicas on the node in leader/follower mode. The leader replica is elected among multiple replicas by using the election protocol. If the leader replica fails, the system automatically switches over to a follower replica by using the election protocol.
...
When the leader cannot connect to the majority for more than one lease period, a new round of election will be initiated to elect the next leader. The default election lease period is 4s.
