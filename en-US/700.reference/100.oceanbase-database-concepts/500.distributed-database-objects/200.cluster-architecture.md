...
* Each log stream can have at most one Paxos replica and multiple non-Paxos replicas in each zone. You can specify non-Paxos replicas in the locality, as shown in the following example:
  ...
  * `locality = "F{1}@z1, R{1}@z1"`: indicates that one full-featured replica and one read-only replica are distributed in z1.

  * `locality = "F{1}@z1, R{ALL_SERVER}@z1"`: indicates that one full-featured replica is distributed in z1 and read-only replicas are created on other servers in the same zone. Read-only replicas are optional.
