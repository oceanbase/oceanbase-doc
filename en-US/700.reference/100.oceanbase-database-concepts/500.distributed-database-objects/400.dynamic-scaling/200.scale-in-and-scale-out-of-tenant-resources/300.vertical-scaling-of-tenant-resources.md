...
|dir-name-en||
|tenant-type||
|slug|vertical-scaling-of-tenant-resources|
...
* Example 1
  ...
  1. Create a unit config named `u_c0` and a resource pool named `pool1`, and then apply `u_c0` to `pool1`.
  ...
  2. Increase the values of the `MIN_CPU`, `MAX_CPU`, and `MEMORY_SIZE` parameters in `u_c0`.
...
* Example 2
  ...
  1. Create two unit configs named `u_c0` and `u_c1` and a resource pool named `pool1`, and then apply `u_c0` to `pool1`.
