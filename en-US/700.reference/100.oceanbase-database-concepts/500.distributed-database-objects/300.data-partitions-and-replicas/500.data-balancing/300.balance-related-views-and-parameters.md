...
| enable_ls_leader_balance | Tenant level | true  | Specifies whether to enable log stream leader balancing. If this parameter is set to `false` and <code>enable_rebalance</code> is set to `true`, log stream leader balancing is disabled. | LS balancing |
...
| zone_disk_balance_tolerance_percentage   | Tenant level | 10 | The tolerance for imbalance in the disk balancing algorithm for table groups across zones. | Table group disk balancing |
