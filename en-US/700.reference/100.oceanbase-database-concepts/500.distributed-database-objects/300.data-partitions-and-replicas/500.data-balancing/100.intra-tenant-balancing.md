...
## Scenarios of partition aggregation and then sharding

Partition balancing is a continuous process to maintain the system load balance. In actual business operations, the system aggregates and redistributes partitions based on the real-time load status to achieve the optimal balance between resource utilization efficiency and performance. Common scenarios of partition aggregation and redistribution include:

* A tenant performs horizontal scaling down and then scaling up. For example, the number of units (`UNIT_NUM`) of a tenant changes from N to 1 and then from 1 to N.

* The number of primary zones (`PRIMARY_ZONE`) of a tenant first decreases and then increases. For example, the value of `PRIMARY_ZONE` changes from `RANDOM` to `zone1` and then from `zone1` to `RANDOM`.

* After a large number of tables are added to a table group with the `NONE` attribute, the tables are removed from the table group.

Based on the existing log stream balancing and partition balancing algorithms, after partition aggregation and redistribution, continuous partitions may be grouped on the same log stream, or non-partitioned tables in the same database may be grouped on the same log stream. To solve this problem, OceanBase Database optimizes the partition balancing algorithm:

* For non-partitioned tables, after partition aggregation and redistribution, multiple non-partitioned tables in the same database are distributed across user log streams based on the number of partitions, avoiding the situation where non-partitioned tables in the same database are continuously grouped on the same log stream.

* For partitioned tables, after partition aggregation and redistribution, continuous partitions are distributed across user log streams using the Round Robin method based on the number of partitions, avoiding the situation where partitions of a partitioned table are continuously grouped on the same log stream.