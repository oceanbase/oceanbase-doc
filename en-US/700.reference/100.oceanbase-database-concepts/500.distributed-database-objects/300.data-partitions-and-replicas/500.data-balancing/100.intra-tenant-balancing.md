...
In OceanBase Database V4.3.5, the ability to change the `duplicate_scope` attribute of replicated tables was introduced in V4.3.5 BP1. A replicated table can only exist in a broadcast log stream. After you modify the `duplicate_scope` attribute, the system will automatically align the attribute based on the partition balancing status. Once the partition balancing is complete, you can use the table with the expected attribute. You can temporarily set the tenant-level parameter `partition_balance_schedule_interval` to a smaller value to speed up the alignment process.

For more information about how to change the `duplicate_scope` attribute, see [Change the attributes of a replicated table (MySQL mode)](../../../../300.database-object-management/100.manage-object-of-mysql-mode/200.manage-tables-of-mysql-mode/650.change-duplicate-scope-of-mysql-mode.md) and [Change the attributes of a replicated table (Oracle mode)](../../../../300.database-object-management/200.manage-object-of-oracle-mode/100.manage-tables-of-oracle-mode/650.change-duplicate-scope-of-oracle-mode.md).
