...
![Initial distribution of a primary partitioned table](https://obbusiness-private.oss-cn-shanghai.aliyuncs.com/doc/img/observer-enterprise/V4.2.5/700.reference/100.oceanbase-database-concepts/500.distributed-database-objects/balance-weight-with-partition-table1-new.png)
...
* Scenario 1: Scattering a few hotspots
  ...
  ![Application scenario 1 for a primary partitioned table](https://obbusiness-private.oss-cn-shanghai.aliyuncs.com/doc/img/observer-enterprise/V4.2.5/700.reference/100.oceanbase-database-concepts/500.distributed-database-objects/balance-weight-with-partition-table2-new.png)
...
* Scenario 2: Distributing all partitions by weight
  ...
  * Small weight = 1
  ...
  ![Application scenario 2 for a primary partitioned table](https://obbusiness-private.oss-cn-shanghai.aliyuncs.com/doc/img/observer-enterprise/V4.2.5/700.reference/100.oceanbase-database-concepts/500.distributed-database-objects/balance-weight-with-partition-table3-new.png)
...
* Scenario 3: Hotspot partitions exclusive within a balancing group
  ...
  * Small weight = 1
  ...
  ![Application scenario 3 for a primary partitioned table](https://obbusiness-private.oss-cn-shanghai.aliyuncs.com/doc/img/observer-enterprise/V4.2.5/700.reference/100.oceanbase-database-concepts/500.distributed-database-objects/balance-weight-with-partition-table4-new.png)
...
| PARTITION | Partitions with the same primary partition value are aggregated (same partition group); primary partitions are scattered. | A few weighted partitions affect the overall distribution of the table group. |
| ADAPTIVE | When all tables in the group are primary partitioned tables, partitions with the same primary partition value are aggregated (same partition group); primary partitions are scattered. When all tables are subpartitioned tables, subpartitions with the same primary and subpartition values are aggregated (same partition group); subpartitions are scattered. | When all tables are primary partitioned tables, a few weighted partitions affect the overall distribution. Partition weights for subpartitioned tables are not supported in the current version. |
