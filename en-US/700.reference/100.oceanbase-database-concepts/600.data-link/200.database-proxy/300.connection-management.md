...
MySQL has no timeout mechanism, but it kills a session that has been idle for a long period of time. If a client does not receive a response from the OBServer node, the client keeps waiting. In this case, a database administrator (DBA) will intervene in the process and call the kill session command of MySQL to close the session.
