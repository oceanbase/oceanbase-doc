...
### Forcible read-only routing

In the case of normal deployment with weak-consistency reads, set `proxy_route_policy` to `FORCE_READONLY_ZONE`. Then, all read requests are routed to nodes where read-only replicas reside. In this strategy, the routing rules are as follows :

* Weak-consistency read requests are routed to nodes where read-only replicas reside.
* If the request is not a weak-consistency read request, error `4007` is returned, indicating that non-read requests are not allowed in this routing strategy. To route non-read requests, change the value of `ob_route_policy` to try another strategy.
