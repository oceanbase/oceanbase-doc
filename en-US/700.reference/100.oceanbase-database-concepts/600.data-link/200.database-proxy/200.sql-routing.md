...
* Weak read requests: routed to the read-only replicas.
* Non-weak read requests: return error code `4007`. This rule does not allow non-weak read requests. If your business indeed needs to send non-weak read requests, you can set the `ob_route_policy` variable to another value.
