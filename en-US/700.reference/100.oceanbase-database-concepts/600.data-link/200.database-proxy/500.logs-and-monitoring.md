...
If the value of the `query_digest_time_threshold` parameter for a request exceeds the threshold or an error response is returned for the request, the request information is sent to the `obproxy_digest.log` file. The default value is 100 ms (2 ms for the primary site).

<main id="notice" type='explain'>
    <h4>Note</h4>
    <p><code>query_digest_time_threshold</code> specifies the threshold of request execution duration. If the execution duration exceeds the threshold, a log is recorded in the <code>obproxy_digest.log</code> file. For more information about this parameter, see <a href="https://en.oceanbase.com/docs/community-odp-en-10000000000864246">Dynamically modifiable parameters</a> of ODP. </p>
  </main>
...
If the value of the `slow_query_time_threshold` parameter for a request exceeds the threshold, the request information is sent to the `obproxy_slow.log` file. The default value of `slow_query_time_threshold` is 500 ms.
...
```sql
Throttling status (RUNNING/OBSERVE) SQL Throttling rule name
```
