...
* Append configurations to the `-o` option when you start ODP. Here is an example: `-o <var_name>=<var_value>`.
...
| refresh_rslist                          | false | Specifies whether to update the RootService list configuration from ConfigServer. |
| refresh_idc_list                        | false | Specifies whether to update the IDC configuration from ConfigServer.    |
| refresh_config                          | false | Specifies whether to update all configurations from MetaDB.              |
| partition_location_expire_relative_time | 0     | The validity period of all location caches in seconds.    |

For more information about other ODP configurations, run the `show proxyconfig` command or read topics about ODP.

## Driver support

Operations of the ODP account `root@proxysys` are fully compatible with the MySQL protocol, and support the `COM_QUERY` field of MySQL. You can also manage ODP configurations by using MySQL drivers such as Java Database Connectivity (JDBC).
