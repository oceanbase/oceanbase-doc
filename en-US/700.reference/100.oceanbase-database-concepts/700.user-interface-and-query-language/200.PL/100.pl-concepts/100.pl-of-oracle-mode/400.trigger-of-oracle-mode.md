...
* Compound DML trigger
   ...
   You can create a compound DML trigger on a table or a view. It supports firing at multiple timing points. A compound trigger is sectioned by each timing point, and each section has its own executable part and optional exception-handling part.

* System trigger

   System triggers can be classified into two types based on the triggering event: DDL event triggers and Database event triggers. A DDL event trigger is activated by executing DDL statements. A Database event trigger is activated by Database events. `logon` and `logoff` are Database events, so these two types of triggers are Database event triggers. They are activated after user login and before user logout, respectively.
...
* A row-level BEFORE trigger is executed before each row is affected by the triggering event.

* A row-level AFTER trigger is executed after each row is affected by the triggering event.

System triggers can be created on a schema (user) or on the Database to specify when they are activated: on user login or on any user login. You can use `ON user_name.SCHEMA` to specify that the trigger is activated when a specific user logs in. If you do not specify `user_name`, the trigger is activated when the user who created the trigger logs in.

The trigger activation order is as follows:
