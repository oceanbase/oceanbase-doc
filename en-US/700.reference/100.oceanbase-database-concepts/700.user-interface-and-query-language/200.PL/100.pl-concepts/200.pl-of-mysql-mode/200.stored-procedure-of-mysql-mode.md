...
You can create a stored procedure by using the `CREATE PROCEDURE` statement. The differences between MySQL mode and Oracle mode are as follows:
...
* Stored procedures in MySQL mode do not support overloading, so you do not need to consider parameter types when deleting a stored procedure.
