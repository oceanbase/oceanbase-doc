...
The following example creates a trigger named `test_trg`, associates it with a table named `test` to trigger an `INSERT` operation, and uses the trigger as an accumulator to return the sum of column values inserted into the table.
