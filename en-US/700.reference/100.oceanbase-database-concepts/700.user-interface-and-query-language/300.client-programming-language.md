...
* **ecoblib**: the runtime library provided by OceanBase Database for ECOB, which provides the function interfaces required by ECOB.

* **Pro*C**: the precompiler provided by Oracle to compile C language programs that are embedded with SQL statements.

* **sqllib**: the runtime library provided by Oracle for Pro\*C, which provides the function interfaces required by Pro\*C.

## Features

* Supports SQL syntax that is compatible with OceanBase Database in Oracle-compatible mode.
...
* **<:username>:**
   ...
   The username that is used to connect to the OceanBase tenant in Oracle-compatible mode. The username follows the **username@tenant name** format. For example, you can set this variable to `test@oracle`. If OceanBase Database Proxy (ODP) is used for connection, the cluster name must be added. In this case, the username follows the **username@tenant name#cluster name** format. For example, you can set this variable to `test@oracle#cluster1`.

* **<:password>:**

   The password that is used to connect to the OceanBase tenant in Oracle-compatible mode.

* **<:dbstring>:**

   The service name that is used to connect to the OceanBase tenant in Oracle-compatible mode. The service name is the name of the connection string in the `tnsnames.ora` file that is specified in the `TNS_ADMIN` environment variable. Here is an example of the connection string in the `tnsnames.ora` file:
...
* **<:user_password>:**
   ...
   The username and password used to connect to the OceanBase tenant in Oracle-compatible mode, for example, testUser@oracle/******. The username is separated from the password with a forward slash (/).
...
* You can execute the `UPDATE` statement to update table columns in a database with the values of external variables or update only the row at which the cursor is located.

* You can execute the `DELETE` statement to delete one or more rows in a database or delete only the row at which the cursor is located.
...
You can execute the `WHENEVER` statement to specify the method of handling errors and warning conditions.
