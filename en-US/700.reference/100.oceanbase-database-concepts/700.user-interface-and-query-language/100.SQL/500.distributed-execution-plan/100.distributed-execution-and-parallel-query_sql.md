...
If the system does not have sufficient resources for additional parallel processing, using parallel queries or increasing the parallel degree will not improve execution performance. On the contrary, when the system is overloaded, the operating system will be forced to perform more scheduling (for example, context switching), which may degrade performance.

In DSS systems, a large amount of data often needs to be accessed, and parallel execution can improve response time. For simple DML operations or queries involving a small amount of data, parallel queries cannot significantly reduce query response time.
...
To better understand this form of network communication, the concept of Data Transfer Layer (DTL) is introduced. That is, the communication connection between any two points is described using the concept of a channel.

A channel has a sender and a receiver. In the initial implementation, the sender was allowed to send unlimited data to the receiver. However, if the receiver could not consume the data immediately, the receiver's memory could be exhausted. Therefore, flow control logic was added. Each channel receiver reserves three slots. When the slots are occupied by data, the sender is notified to pause sending. When data is consumed and a slot becomes free, the sender is notified to continue sending.
