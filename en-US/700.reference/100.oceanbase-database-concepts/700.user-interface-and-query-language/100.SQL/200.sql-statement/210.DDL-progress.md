...
You can use the following statement to view the fields in the `GV$SESSION_LONGOPS` view:
...
During this phase, the `MESSAGE` field displays `WAIT TRANS END`, and the ID of the first unfinished transaction is displayed (for example, PENDING_TX_ID: 76). You can query the `__all_virtual_trans_stat` table for the corresponding `trans_id` to obtain detailed transaction information.
...
MESSAGE examples during the data completion phase:
