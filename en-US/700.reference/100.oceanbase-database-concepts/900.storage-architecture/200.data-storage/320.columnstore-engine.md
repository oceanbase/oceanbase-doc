...
* Incremental data
   ...
   All user data written after the latest version of baseline data is incremental data. Incremental data can be memory data just written to MemTable or disk data already dumped to SSTable. Replicas of incremental data are maintained independently and are not necessarily consistent. Unlike baseline data that is generated based on a specified version, incremental data contains multi-version data.
...
* In columnstore mode, the data of each column is stored as an independent SSTable, and the SSTables of all columns are combined into a virtual SSTable as the columnstore baseline data for users, as shown in the following figure.
...
* Adaptive compaction
   ...
   With the new columnstore storage mode, compaction behavior differs significantly from that of row-based data. Specifically, because incremental data is stored by row, it must be merged with baseline data before it is split and saved to a separate SSTable for each column. The compaction time and resource usage will significantly increase compared with those in row-based storage.

   To speed up compaction of columnstore tables, OceanBase Database has significantly optimized the compaction process. For columnstore tables, in addition to horizontal splitting and parallel compaction like rowstore tables, vertical splitting is also supported. Columnstore tables combine the compaction of multiple columns into a single compaction task, and the number of columns in a task can be adjusted based on system resources to achieve a better balance between compaction speed and memory usage.
...
* For information about creating columnstore tables, see [Create a table (MySQL-compatible mode)](../../../300.database-object-management/100.manage-object-of-mysql-mode/200.manage-tables-of-mysql-mode/200.create-a-table-for-mysql-tenant-of-mysql-mode.md) and [Create a table (Oracle-compatible mode)](../../../300.database-object-management/200.manage-object-of-oracle-mode/100.manage-tables-of-oracle-mode/200.create-a-table-for-oracle-tenant-of-oracle-mode.md).
* For information about Skip Index, see [Column Skip Index properties (MySQL-compatible mode)](../../../300.database-object-management/100.manage-object-of-mysql-mode/200.manage-tables-of-mysql-mode/250.identify-skip-index-properties-of-mysql-mode.md) and [Column Skip Index properties (Oracle-compatible mode)](../../../300.database-object-management/200.manage-object-of-oracle-mode/100.manage-tables-of-oracle-mode/250.identify-skip-index-properties-of-oracle-mode.md).
