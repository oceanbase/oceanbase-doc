...
* Dump
  ...
  Dump consists of two processes: mini compaction and minor compaction. When the size of the MemTable in memory exceeds a certain threshold, the data in the MemTable needs to be dumped to Mini SSTables on disk to free memory. This process is called mini compaction. As users write more data, the number of Mini SSTables increases. When the number of Mini SSTables exceeds a certain threshold, minor compaction is automatically triggered in the background.
  
* Merge

  Merge, also known as major compaction, is called daily compaction in OceanBase Database. The concept differs slightly from that in other LSM-tree-based databases. As the name suggests, this concept was originally intended to run as a cluster-wide compaction action around 02:00 every morning. Merge is typically scheduled by the RS (RootService) of each tenant based on write status or user settings. Each merge of each tenant selects a global snapshot point, and all partitions in the tenant perform a major compaction using the data at this snapshot point. In this way, each merge generates SSTables for all tenant data based on this unified snapshot point. This mechanism not only helps users regularly integrate incremental data and improve read performance, but also provides a natural data verification point. Through this global consistency point, OceanBase Database can perform multi-dimensional physical data verification on multiple replicas and between primary tables and index tables.
