...
## How OceanBase Database prevents disk silent errors

The data stored on disks of OceanBase Database has two parts: RedoLog (transaction logs) and SSTables. RedoLog is replayed to form MemTable in memory. SSTables store the static data in the log-structured merge-tree (LSM tree). A MemTable and an SSTable form a replica of user data.
...
## RedoLog verification mechanism

The checksum of each RedoLog is recorded at its header. The checksum of each log is forcibly verified during network transmission and log replay. This ensures that the logs synchronized to the three replicas are correct and consistent. If the data in a log encounters a silent corruption error, this log is not synchronized to other replicas.
