...
# Dump (Mini and minor compactions)

Dump consists of two processes: mini compaction and minor compaction.
...
Dump can be triggered automatically or manually.

When the size of the active MemStore memory for a tenant reaches `freeze_trigger_percentage * memstore_limit` (where `memstore_limit = tenant memory * memstore_limit_percentage`), the system automatically triggers the freeze (a prerequisite for dump). After that, the system internally schedules the dump.

You can also manually trigger dump using the following O&M command.
...
* Cluster-level dump
  ...
  * Dump initiated for the `sys` tenant

    ```sql
    obclient> ALTER SYSTEM MINOR FREEZE TENANT = sys;
    ```

  * Dump initiated for all user tenants

    ```sql
    obclient> ALTER SYSTEM MINOR FREEZE TENANT = all_user;
    ```

  * Dump initiated for all META tenants
...
For more information about dump operations, see [Dump management overview](../../../200.system-management/500.manage-data-storage/100.dump-management/100.dump-management-overview.md).