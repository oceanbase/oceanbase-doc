...
Compared with dump (mini and minor compactions), merge (major compaction) is typically a more important operation and is relatively time-consuming. Based on best practices, a major compaction is performed only once within a day and during off-peak hours. Therefore, a major compaction is also called a daily compaction.

A major compaction compacts all dynamic and static data, which is a time-consuming operation. When the incremental data produced by dump accumulates to a certain extent, merge is implemented through Major Freeze. The main difference from dump is that merge is the act of a tenant merging with its static data at a unified snapshot point, and finally produces a tenant-level snapshot.
...
| A mini compaction persists one or more MemTables into an SSTable.                | Combines multiple Mini SSTables into one Mini SSTable, or combines multiple Mini SSTables with one Minor SSTable to produce a new Minor SSTable. Only incremental data is involved. Rows that are ultimately deleted need special markers. | A major compaction compacts MemTables and SSTables of the current major version with the full static data of the previous major version and generates a new set of full data.  |
...
For more information about merge operations, see [Merge management overview](../../../200.system-management/500.manage-data-storage/200.merge-management/100.consolidation-management-overview.md).
