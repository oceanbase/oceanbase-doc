...
* `MIN_CPU`: the minimum CPU resources that can be provided by a resource unit that uses this resource unit configuration. This parameter defaults to the value of `MAX_CPU`.
...
* `MAX_IOPS` and `MIN_IOPS`: the maximum and minimum IOPS resources that can be provided by a resource unit that uses this resource unit configuration. The minimum values of the two parameters are both `1024`, and the value of `MAX_IOPS` must be greater than or equal to that of `MIN_IOPS`.
  ...
  * If neither `MIN_IOPS` nor `MAX_IOPS` is specified, the default value of the two parameters is `INT64_MAX`.
...
* IOPS resources (IOPS): The meta tenant shares IOPS resources with the corresponding user tenant. The IOPS resource specification of the meta tenant displayed in the `GV$OB_UNITS` view is `NULL`, which indicates that the meta tenant shares IOPS resources with the user tenant.
...
You must deploy each resource unit on a physical server that can accommodate the resource unit. A physical server can accommodate only one resource unit in the same resource pool. If `zone1` or `zone2` contains less than two physical servers or the resources on the physical server do not meet the specifications of `uc1`, the sample statement fails to create the resource pool.
...
In this example, resource pools `pool1` and `pool2` are created, and then tenant `tt` is created. The tenant `tt` has two resource units in both `z1`and `z2` and one resource unit in `z3`. The specifications of each resource unit comply with the resource configuration specified by `uc1`.
...
Sample statements for modifying the resource unit configuration of a resource pool are as follows:
...
In this example, the resource unit configuration specified for the resource pools `pool1` and `pool2` is `uc1`. The `ALTER RESOURCE UNIT uc1 MAX_CPU 6, MEMORY_SIZE '36G';` statement changes the `MAX_CPU` value of `uc1` to `6` and the `MEMORY_SIZE` value to `36G`. Other configurations remain unchanged. You can modify resource parameters to adjust the resource unit config of the resource pool for a tenant in the corresponding zone, so as to adjust the service capability of the tenant.
...
* Increase resource specifications
  ...
  In example 1, a resource unit configuration named `u_c0` and a resource pool named `pool1` are created. The resource pool `pool1` uses the resource unit configuration `u_c0`. Then, you can increase the value of `MIN_CPU`, `MAX_CPU`, or `MEMORY_SIZE` of `u_c0`. The adjustment is intended to upgrade the unit config of `pool1` and thereby enhance the service capability of the corresponding tenant.
...
When you modify the `UNIT_NUM` parameter, you can increase or decrease its value. The `ALTER RESOURCE POOL rp1 UNIT_NUM 3;` statement increases the value of `UNIT_NUM`, while the `ALTER RESOURCE POOL rp1 UNIT_NUM 2;` and `ALTER RESOURCE POOL rp1 UNIT_NUM 1 DELETE UNIT = (1001, 1003);` statements decrease the value of `UNIT_NUM`.
...
obclient> ALTER RESOURCE TENANT tt UNIT_NUM = 1;// Decrease the value of the UNIT_NUM parameter.
```
...
When you modify the `ZONE_LIST` parameter, you can expand or narrow the scope of zones. The `ALTER RESOURCE POOL pool1 ZONE_LIST=('z1','z2','z3','z4');` statement expands the scope of zones, while the `ALTER RESOURCE POOL pool2 ZONE_LIST=('z1','z2');` statement narrows the scope of zones.
