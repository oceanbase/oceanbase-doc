...
The current version supports homogeneous zone mode and heterogeneous zone mode for tenants:

* In homogeneous zone mode, the `UNIT_NUM` of each zone under a tenant must be the same.

* In heterogeneous zone mode, the `UNIT_NUM` of each zone under a tenant can differ, but a tenant can have at most two different `UNIT_NUM` values across all zones.

<main id="notice" type='notice'>
<h4>Notice</h4>
<p>By default, existing tenants and upgraded tenants use homogeneous zone mode. To enable heterogeneous zone mode, set the tenant-level parameter <code>zone_deploy_mode</code> to <code>hetero</code>. After the change, you cannot switch back to <code>homo</code> (homogeneous zone mode).</p>
</main>

#### Changes under homogeneous zone mode

* **Modify `UNIT_NUM` for unallocated resource pools**

  In homogeneous zone mode, when a resource pool is not assigned to a tenant, you can use the `ALTER RESOURCE POOL` statement to adjust the `UNIT_NUM` of the resource pool and the number of resource units in each zone.

  Assume that the unallocated resource pool is created as follows:

  ```sql
  obclient> CREATE RESOURCE POOL rp1 UNIT 'uc1', UNIT_NUM 2, ZONE_LIST ('zone1', 'zone2');
  ```

  Sample statements for modifying the `UNIT_NUM` of the resource pool are as follows:
  ...
  ```sql
  obclient> ALTER RESOURCE POOL rp1 UNIT_NUM 1 DELETE UNIT = (1001, 1003); // Decrease UNIT_NUM by specifying resource units.
  ```

  Modifying `UNIT_NUM` includes increasing and decreasing. The `ALTER RESOURCE POOL rp1 UNIT_NUM 3` statement increases `UNIT_NUM`. The `ALTER RESOURCE POOL rp1 UNIT_NUM 2` and `ALTER RESOURCE POOL rp1 UNIT_NUM 1 DELETE UNIT = (1001, 1003)` statements decrease `UNIT_NUM`.

* **Modify `UNIT_NUM` for tenant resource pools**

  In homogeneous zone mode, you can also modify `UNIT_NUM` after a resource pool is assigned to a tenant. Modifications must be performed at the tenant level. All resource pools of the tenant must be modified for the operation to succeed.

  Assume that the tenant resource pools are created as follows:
  ...
  Sample statements for modifying the `UNIT_NUM` of tenant resource pools are as follows:
  ...
  ```
   ...
   Note that in heterogeneous zone mode, you cannot decrease `UNIT_NUM` by specifying `UNIT_GROUP`.

   Additionally, in heterogeneous zone mode, you can adjust the `UNIT_NUM` for an individual resource pool. The commands are as follows:
   ...
   ```shell
   obclient> ALTER RESOURCE POOL pool2 UNIT_NUM 1 DELETE UNIT = (1001, 1003); // Decrease UNIT_NUM by specifying resource units
   ```
