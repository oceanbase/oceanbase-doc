...
* If only one tenant on an OBServer node is heavily loaded and other tenants are idle, the worker threads of the heavily loaded tenant can fully utilize the physical CPUs of the same number as the threads.
