...
* CPU resources are isolated based on user-mode scheduling.
  ...
  To achieve a better isolation effect, OceanBase Database starts to support the configuration of control groups (cgroups) to isolate CPU resources in V4.0.0 and later.
...
Many threads with different features start along with an OBServer node. For more information, see [Threads](../../1200.observer-node-architecture/300.observer-thread-model/100.thread-introduction.md). In this section, threads are classified into the following two types:
...
unit_status:"NORMAL", // The status of the resource unit. Valid values: NORMAL, MIGRATE_IN, MIGRATE_OUT, MARK_DELETING, WAIT_GC_IN_OBSERVER, DELETING_IN_OBSERVER, and ERROR_STAT.
...
replay_start_point:ObLogCursor{file_id=1, // The SSTable log (slog) replay point of the tenant.
log_id=440,
offset=343322},
