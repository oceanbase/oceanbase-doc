...
You can create user tables in a `sys` tenant. All user tables and system tables are served by the No.1 log stream. The data of a `sys` tenant is cluster-specific and does not support physical synchronization or backup and restore between the primary and secondary clusters.

<main id="notice" type='notice'>
<h4>Notice</h4>
<p>The <code>sys</code> tenant is intended for cluster and tenant management and does not provide full database functionality. It is not recommended for use in production or business testing environments. </p>
</main>
...
A meta tenant stores and manages private data of clusters of a user tenant, which includes parameters and information about locations, replicas, log stream status, backup and restore, and major compactions. This part of data does not require physical synchronization or physical backup and restore across databases.
