...
OceanBase Database uses multi-version two-phase locking to ensure the correctness of its concurrency control model. The lock mechanism is important for ensuring data concurrency and consistency.
...
<main id="notice" type='explain'>
  <h4>Note</h4>
  <p>The following SQL statement is for demonstration purposes only and cannot be executed temporarily. </p>
</main>
...
OceanBase Database uses multi-version two-phase locking. When a transaction is used to modify data, the system generates a new version of data instead of modifying the original data. Therefore, you can read data of an earlier version from a consistent snapshot. In this way, OceanBase Database can control concurrent operations without row locks and prevent mutual exclusion between reads and writes. This greatly improves the concurrent level of OceanBase Database. However, a `SELECT ... FOR UPDATE` statement still adds a row lock during execution and is mutually exclusive with other update statements or `SELECT ... FOR UPDATE` statements. An update operation, on the other hand, is mutually exclusive with all operations that require a row lock.
...
At the beginning of each LCL period, each node resets its public label to its private label.
