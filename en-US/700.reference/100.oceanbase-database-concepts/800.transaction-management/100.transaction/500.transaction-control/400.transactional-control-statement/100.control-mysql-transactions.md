...
* No statement is executed for a session in an active transaction within the idle timeout period.
  ...
  <main id="notice" type='explain'>
    ...
    <li>
    <p><code>ob_trx_idle_timeout</code> specifies the idle timeout duration for a transaction in a session, in microseconds. A timeout occurs when the execution interval between two statements in the transaction exceeds this value. </p>
    </li>
    </ul>
  </main>
...
If an internal error occurs during the execution of a transaction, such as a participant node failure or other reasons that prevent the transaction from continuing, the transaction cannot successfully execute statements and can only be rolled back. In this case, the transaction cannot execute a statement and must be rolled back.
