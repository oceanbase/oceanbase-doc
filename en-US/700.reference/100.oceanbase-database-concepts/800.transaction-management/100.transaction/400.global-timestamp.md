...
OceanBase Database runs a Global Timestamp Service (GTS) for each tenant. When OceanBase Database commits transactions, it uses GTS of each tenant to retrieve the transaction versions. This ensures that the transactions are committed in the order of their versions.
...
* Retrieval optimization of the statement snapshot
   ...
   When a transaction is committed, OceanBase Database updates the Global Committed Version on the server where it runs. If a statement can determine that its query targets a single server, it uses that server's Global Committed Version as the Read Version directly, reducing the request pressure on the global timestamp.

* Retrieval optimization of the transaction commit version

   Multiple transactions can obtain the global timestamp in batches. Timestamp requests can be sent in advance to shorten the transaction commit time.