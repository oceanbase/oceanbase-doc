...
When a transaction is started, OceanBase Database assigns an ID to uniquely identify the transaction. You can use `oceanbase.__all_virtual_trans_stat` to query the status of a transaction. In the following example, the `UPDATE` statement starts a transaction.
...
When a transaction ends, all the partitions affected by the transaction receive a transaction commit (`COMMIT`) or rollback (`ROLLBACK`) request, depending on the scenario. The commit or rollback of a transaction is triggered in any of the following scenarios:

* You explicitly initiate a commit or rollback. If you initiate a commit, OceanBase Database persists the changes made by the transaction to the commit log (clog) file.
