...
* For self-referencing foreign keys, where a row references itself and the behavior is `DELETE CASCADE`, OceanBase Database MySQL-compatible mode can successfully delete that row.

* For self-referencing foreign keys, if a parent key of a row is referenced, OceanBase Database MySQL-compatible mode allows cascading updates.
