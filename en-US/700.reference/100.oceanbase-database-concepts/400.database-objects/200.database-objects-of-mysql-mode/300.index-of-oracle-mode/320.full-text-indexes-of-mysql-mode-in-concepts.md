...
* Languages that use spaces as separators, such as English (for example, "apple watch series 9").
* Chinese text with manually added separators (for example, "南京 长江大桥").
...
* A Chinese tokenizer based on the open-source IK Analyzer. Supports two modes:

  * **Smart mode**: Prefers longer tokens and reduces the number of splits (for example, "南京市" is not split into "南京" and "市").
  * **Max Word mode**: Outputs all possible short tokens (for example, "南京市" is split into "南京" and "市").
...
* E-commerce product description search (for example, precise matching of "华为Mate60").
...
* **Precise mode**: Strictly splits according to the dictionary (for example, "不能" is not split into "不" and "能").
* **Full mode**: Lists all possible split combinations.
* **Search-engine mode**: Balances precision and recall (for example, "南京市长江大桥" → "南京", "市长", "长江大桥").
* Supports custom dictionaries and new word discovery. Compatible with multiple languages (Chinese, English, Japanese, and others).

**Use cases:**

* Medical and technology terminology analysis (for example, precise splitting of "人工智能").
