...
|slug|index-overview-mysql-mode|
...
When you truncate or delete a partition, OceanBase Database rebuilds all global indexes of the corresponding table. During the rebuild process, the indexes are temporarily unavailable.
...
A columnstore index stores data in an index table in columnar format. OceanBase Database supports specifying the table storage format as columnar when creating a table. An index is also a table, and therefore supports columnar storage for index table data.
