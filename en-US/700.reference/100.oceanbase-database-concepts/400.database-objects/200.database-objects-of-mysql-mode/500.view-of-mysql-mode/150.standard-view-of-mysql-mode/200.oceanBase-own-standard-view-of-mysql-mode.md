...
OceanBase-specific views of MySQL-compatible tenants and the `sys` tenant are stored in the `oceanbase` database, and those of Oracle-compatible tenants are stored in the `SYS` schema.
