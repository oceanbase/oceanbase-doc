...
1. **Table groups with SHARDING = PARTITION**:
   ...
   - In these table groups, data is partitioned at the first level, and all subpartitions of the same primary partition are aggregated. The partitioning definitions of all primary partitions must be the same to allow the coexistence of primary partitions and subpartitions.

   - **Partition alignment rules**: Partitions with the same primary partition values are aggregated on the same server. This applies to both primary partitions and subpartitions of the same primary partition.

2. **Table groups with SHARDING = ADAPTIVE**:

   - Tables in this type of table group are distributed based on the adaptive method. If the table group contains primary partitioned tables, data is distributed based on the primary partitions. If the table group contains secondary partitioned tables, data is distributed based on the subpartitions of each primary partition.

   - **Partitioning requirements and alignment rules**: All tables in the table group must be of the same partitioning type (primary or secondary), and have the same partitioning definition. Partitions with the same primary partition values are aggregated on the same server. For subpartitions, both the primary and secondary partition values must be the same.
...
- [Create a table group](../../../../300.database-object-management/100.manage-object-of-mysql-mode/400.manage-table-groups-of-mysql-mode/200.create-a-table-group-of-mysql-mode.md)
...
- [Add tables to a table group](../../../../300.database-object-management/100.manage-object-of-mysql-mode/400.manage-table-groups-of-mysql-mode/400.add-tables-to-a-table-group-of-mysql-mode.md)
- [Modify the SHARDING attribute of a table group](../../../../300.database-object-management/100.manage-object-of-mysql-mode/400.manage-table-groups-of-mysql-mode/500.modify-the-sharding-attribute-ot-a-table-group-of-mysql-mode.md)
...
- [Drop a table group](../../../../300.database-object-management/100.manage-object-of-mysql-mode/400.manage-table-groups-of-mysql-mode/700.delete-a-table-group-of-mysql-mode.md)