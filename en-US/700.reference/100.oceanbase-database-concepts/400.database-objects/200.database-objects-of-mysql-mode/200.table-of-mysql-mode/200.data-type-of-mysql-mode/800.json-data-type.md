...
The JSON data type is a special data type for storing and processing JSON (JavaScript Object Notation) data in a database. JSON values must be composed of objects (JSON objects), arrays, strings, numbers, boolean values (`false`/`true`), or `null`. The values `false`, `true`, and `null` can only be in lowercase.
