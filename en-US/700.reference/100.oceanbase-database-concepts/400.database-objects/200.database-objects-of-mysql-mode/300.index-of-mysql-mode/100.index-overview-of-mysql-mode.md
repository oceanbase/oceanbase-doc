...
A unique index requires that the values of the indexed columns are unique. Each index value corresponds to only one record. A unique index ensures the uniqueness of a field in a table.

#### Non-unique indexes

A non-unique index allows duplicate values in the indexed columns. Multiple index values can correspond to one or more records. A non-unique index accelerates queries on the indexed columns but does not ensure the uniqueness of the indexed columns.
