...
If a local index is partitioned on a left prefix of the index columns and the subpartitioning key is included in the index key, it is a local prefixed index. Local prefixed indexes can be unique or nonunique.
...
If a local partitioned index is not a local prefixed index, it is a local non-prefixed index. Likely, the partitioning key of the primary table of the index is not on a left prefix of the index columns, or the subpartitioning key is not included in the index key.
