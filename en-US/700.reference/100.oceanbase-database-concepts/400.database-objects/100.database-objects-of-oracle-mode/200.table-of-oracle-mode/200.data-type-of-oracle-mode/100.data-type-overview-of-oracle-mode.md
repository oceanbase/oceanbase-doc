...
|slug|data-type-overview-oracle-compatible-mode|
...
Other important data types include `RAW`, large object (LOB), set, and user-defined data types. In procedural language (PL), you can also specify the `BOOLEAN`, reference, composite (including `COLLECTION` and `RECORD`), and other custom data types for constants and variables.
