...
Granule (granule unit) is the smallest parallel work unit for data scanning:

* Each granule describes a continuous data scanning task range, and is strictly limited to a single table partition.
* By splitting partition data into multiple granules, multiple parallel threads can process different granules simultaneously.

OceanBase Database supports the rebalancing of parallel execution (PX) tasks. When the remaining number of granules is less than the number of PX threads, the remaining granules are further split to make the scanning tasks more balanced.