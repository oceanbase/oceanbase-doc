...
For a distributed system based on the Shared-Nothing architecture, data of a relational table is stored in partitions on various nodes of the system. So, to run a cross-partition query, the execution plan must be designed to access data on multiple nodes. Therefore, OceanBase Database is provided with capabilities to generate and execute distributed execution plans.
...
Parallel queries can effectively improve the processing performance of the system when the system meets all the following conditions at the same time:
...
Generally, for DSSs with a large number of partitions and data warehouses, parallel execution can reduce the response time. For simple DML operations, intra-partition queries, or queries across a small number of partitions, parallel queries cannot significantly reduce the response time.
