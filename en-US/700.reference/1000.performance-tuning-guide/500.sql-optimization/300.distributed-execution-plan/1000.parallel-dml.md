...
When parallel DML is disabled, even if the `PARALLEL` hint is used in an SQL statement, DML operations are executed in series.
If you enable parallel DML for a session, parallel execution applies to all DML statements in the session. If you use the `ENABLE_PARALLEL_DML` hint to enable parallel DML for an SQL statement, parallel execution applies only to the statement. However, if no table has parallelism or the limitations on parallel operations are violated, DML operations are executed in series even if parallel DML is enabled.
...
3. View the execution plan.
    ...
    The query plan involves the following operators:
    ...
    * Operators 4 and 5: These operators perform a full table scan on the `branch_sp_tbl_src` table. This table is the data source from which data is selected for insertion.
