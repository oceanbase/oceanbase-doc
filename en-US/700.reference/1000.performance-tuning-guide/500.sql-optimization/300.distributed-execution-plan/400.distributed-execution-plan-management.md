...
If the global DOP and table-level DOP are both specified, the table-level DOP does not take effect.
  ...
  <main id="notice" type='notice'>
    <h4>Notice</h4>
    <p>In complex queries, the scheduler can schedule the parallel execution of two data flow operations (DFOs). In this case, the number of activated workers is two times the degree of parallelism, which means <code>PARALLEL × 2</code>. </p>
  </main>
