...
<main id="notice" >
    <h4>Note</h4>
    <p>OceanBase Database does not support the estimation of the statistics collection time. Therefore, we recommend that you set [`ob_query_timeout`](../../../../../800.configuration-items-and-system-variables/200.system-variable/300.global-system-variable/9000.ob_query_timeout-global.md) long enough to complete the collection of statistics. Especially, if schema-level statistics collection is interrupted due to timeout, the collected statistics are retained. In addition, we recommend that you increase the value of the [`ob_query_timeout`](../../../../../800.configuration-items-and-system-variables/200.system-variable/300.global-system-variable/9000.ob_query_timeout-global.md) variable at the session level, which affects only the current session without interfering with other operations. </p>
  </main>
...
| degree           | The degree of parallelism (DOP) in statistics collection. The default value is `NULL`.    |
...
| force            | Specifies whether to collect statistics by force and ignore the lock status. The default value is `FALSE`. This parameter can be executed only in the `BEGIN ... END` clause of a PL statement.   |
...
|      Parameter      |                           Description                        |
|--------------|--------------------------------------------------------|
| SIZE integer | The number of histogram buckets for the column. The value range is \[1, 2048\].                       |
...
Statistics collection on all tables under a user is time-consuming. Therefore, we recommend that you use this feature during off-peak hours.
