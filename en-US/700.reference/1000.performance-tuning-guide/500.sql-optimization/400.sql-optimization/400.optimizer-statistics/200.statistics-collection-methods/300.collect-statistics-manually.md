...
<main id="notice" type='note'>
    <h4>Note</h4>
    <p>Currently, OceanBase Database does not support estimation of the time required for collecting statistical information. Therefore, we recommend that you set the value of <a herf="../../../../../800.configuration-items-and-system-variables/200.system-variable/300.global-system-variable/9000.ob_query_timeout-global.md">ob_query_timeout</a> large enough to allow for the completion of statistics collection. This is especially important when collecting schema-level statistics, as the statistics that have already been collected will be retained if the process is interrupted due to a timeout. At the same time, users are advised to increase the the value of <a herf="../../../../../800.configuration-items-and-system-variables/200.system-variable/300.global-system-variable/9000.ob_query_timeout-global.md)">ob_query_timeout</a> at the session level. This action only affects the current session and will not interfere with other operations.</p>
</main>
