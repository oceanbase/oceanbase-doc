...
| granularity      | the partition level for collecting statistics. Valid values: <ul><li> `'GLOBAL'`: collect global-level statistics.</li> <li>`'PARTITION'`: collect partition-level statistics.</li> <li> `'SUBPARTITION'`: collect subpartition-level statistics.</li>   <li>`'ALL'` : collect statistics at all levels, including `GLOBAL`, `PARTITION`, and `SUBPARTITION` levels.</li>   <li> `'AUTO'` : collect statistics at all levels, including `GLOBAL`, `PARTITION`, and `SUBPARTITION` levels, by using the default method. If this parameter is not specified, the default value is `'AUTO'`. </li>   <li> `'DEFAULT'`: collect statistics at `GLOBAL` and `PATITION` levels.</li>   <li> `'GLOBAL AND PARTITION'`: collect statistics at `GLOBAL` and `PATITION` levels.</li>   <li> `'APPROX_GLOBAL AND PARTITION'`: collect partition-level statistics and infer global-level statistics based on the partition information.</li>   </ul>    |
...
In Oracle mode and MySQL mode, OceanBase Database allows you to collect statistics by using the `ANALYZE` statement.
...
* Use the Oracle mode syntax to collect statistics for the `tbl1` table of the `test` user in MySQL mode.
