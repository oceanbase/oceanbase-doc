...
`DBA_OB_OUTLINES` is an outline view. The following table describes the columns in the outline view.
...
* Use MD5 to generate the SQL ID based on the parameterized original SQL statement. You can use the following script to generate the corresponding `SQL_ID`.
...
```
  ...
  <main id="notice" type='notice'>
    ...
    <li>An outline created based on <code>SQL_TEXT</code> overrides an outline created based on <code>SQL_ID</code> because the former has a higher priority. </li>
...
t1:table_rows:3, physical_range_rows:1, logical_range_rows:1, index_back_rows:1, output_rows:1, est_method:local_storage, optimization_method=cost_based, available_index_name[idx_c2], pruned_index_name[t1]
level 0:
***********
...
* After an outline is created or dropped, a new query of the corresponding SQL statement triggers the invalidation of the corresponding execution plan in the plan cache and updates it to the execution plan generated based on the bound outline.

## Relationship between plan binding and throttling rules

In OceanBase Database V4.2.2 and later, the functionality of the `CREATE OUTLINE` and `ALTER OUTLINE` statements is extended. In addition to binding an execution plan for a specific query, these statements also allow you to limit the maximum number of parallel executions for the query. Throttling can help you effectively manage the database load, thereby avoiding performance issues caused by highly concurrent queries.
...
- Execute the `CREATE OUTLINE` statement to create an outline that contains the `USE_NL(tbl2)` hint. This hint tells the optimizer to use a nested loop join for the `tbl2` table. The `MAX_CONCURRENT(1)` hint in the following example specifies to use only one thread for executing the query at a time:
  ...
  ```
- Execute the `ALTER OUTLINE` statement to modify an existing outline. Add a hint that tells the optimizer to use a nested loop join and a hint that specifies to use only one thread for executing the query.
