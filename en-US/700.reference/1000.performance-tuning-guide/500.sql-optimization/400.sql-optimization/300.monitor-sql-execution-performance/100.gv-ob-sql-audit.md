...
| WAIT_TIME_MICRO         | bigint(20)          | number(38)     |The wait time of the wait event, in microseconds.      |
| TOTAL_WAIT_TIME_MICRO   | bigint(20)          | number(38)     |The total wait time during execution, in microseconds.     |
...
| APPLICATION_WAIT_TIME   | bigint(20) unsigned | number(38)     |The total wait time of application wait events, in microseconds.      |
| CONCURRENCY_WAIT_TIME   | bigint(20) unsigned | number(38)     |The total wait time of concurrency wait events, in microseconds.     |
| USER_IO_WAIT_TIME       | bigint(20) unsigned | number(38)     |The total wait time of user I/O wait events, in microseconds.    |
| SCHEDULE_TIME           | bigint(20) unsigned | number(38)     |The total wait time of schedule wait events, in microseconds.      |
...
| DATA_BLOCK_READ_CNT             | bigint(20)          | bigint(20)   |  The number of data microblocks accessed.   |
| DATA_BLOCK_CACHE_HIT            | bigint(20)          | bigint(20)   |  The number of data microblock cache hits.   |
| INDEX_BLOCK_READ_CNT            | bigint(20)          | bigint(20)   |  The number of intermediate-layer microblocks accessed.  |
| INDEX_BLOCK_CACHE_HIT           | bigint(20)          | bigint(20)   |  The number of intermediate-layer microblock cache hits.  |
| BLOCKSCAN_BLOCK_CNT             | bigint(20)          | bigint(20)   |  The number of data microblocks scanned during a unilateral scan.   |
| BLOCKSCAN_ROW_CNT               | bigint(20)          | bigint(20)   |  The number of data rows scanned during a unilateral scan.   |
| PUSHDOWN_STORAGE_FILTER_ROW_CNT | bigint(20)          | bigint(20)   |  The number of rows that remain after the filter condition is pushed down to and applied in the storage layer.  |
...
| PS_CLIENT_STMT_ID       | bigint(20)          | number(38)   | The ID of the prepared statement on the client corresponding to the request. Valid values: <ul><li> `-1`: indicates that the prepared statement (PS) protocol is not used in the SQL statement. That is, the SQL statement is directly executed without being prepared. </li><li> Value other than `-1`: indicates that the PS protocol is used in the SQL statement, and indicates the unique identifier returned by the PS protocol on the client for the statement. Such a value is used to reference prepared SQL statements in subsequent operations. </li></ul>    |
| PS_INNER_STMT_ID        | bigint(20)          | number(38)   | The ID of the prepared statement in the database corresponding to the request. Valid values: <ul><li> `-1`: indicates that the PS protocol is not used in the SQL statement. That is, the SQL statement is directly executed without being prepared. </li><li> Value other than `-1`: indicates that the PS protocol is used in the SQL statement, and indicates the unique identifier returned by the PS protocol on the client for the statement. Such a value is used to reference prepared SQL statements in subsequent operations. </li></ul>    |
|  TX_ID       | bigint(20)  | bigint(20)             | The hash value of the transaction corresponding to the request.     |
|  SNAPSHOT_VERSION       | bigint(20) unsigned  | bigint(20)             | The snapshot read version of the current statement.     |
...
| TX_INTERNAL_ROUTING           | bigint(20)            | NUMBER                  | Whether transaction routing is enabled for the current transaction              |
| TX_STATE_VERSION               | bigint(20) unsigned    | NUMBER(38)              | Version number of the current transaction status                    |
| FLT_TRACE_ID    | varchar(1024) | varchar(1024)| Trace ID for full-link tracing. If this field is empty, it indicates that full-link tracing is not enabled. This field is a UUID, which is different from a trace ID. For example: 000600d6-a5de-038c-6c80-df07e4e79149|
| PL_TRACE_ID                   | varchar(128)          | VARCHAR2(128)           | Trace ID of the outer PL of the current SQL statement       |
| PLSQL_EXEC_TIME               | bigint(20)            | NUMBER(38)              | Execution time of PL, in microseconds                           |
| TOTAL_MEMSTORE_READ_ROW_COUNT  | bigint(20)            | NUMBER(38)              | Total number of rows read from the memstore in the entire process |
| TOTAL_SSSTORE_READ_ROW_COUNT   | bigint(20)            | NUMBER(38)              | Total number of rows read from the SSSTORE in the entire process   |
| PROXY_USER                    | varchar(128)          | VARCHAR2(128)           | Name of the proxy user in the proxy login scenario   |
| FUSE_ROW_CACHE_HIT      | bigint(20)          | number(38)     |Not supported. The default value is `NULL`.        |
| LOCK_FOR_READ_TIME      | bigint(20)          | number(38)     |Time waited for a lock when data is read, in microseconds       |
| PARTITION_HIT                 | tinyint(4)            | -              | Whether the local partition is hit                  |
