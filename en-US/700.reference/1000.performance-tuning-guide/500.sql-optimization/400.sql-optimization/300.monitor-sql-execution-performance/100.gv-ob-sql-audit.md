...
* Set the percentage of memory for SQL audit in the tenant memory. The default value is 3, and the value range is \[0,80\].
...
| PS_CLIENT_STMT_ID       | bigint(20)          | NUMBER(38)   |The value of this column records the client prepare ID corresponding to the request: <ul><li> `-1`: indicates that the current SQL statement does not use the prepared statement (PS) protocol. In other words, the statement is directly executed without being preprocessed. </li><li> A value other than `-1`: indicates that the current SQL statement uses the PS protocol, and this value is the unique identifier returned by the preprocessing step of the statement in the client. This value can be referenced to execute the previously preprocessed SQL statement. </li></ul>    |
| PS_INNER_STMT_ID        | bigint(20)          | NUMBER(38)   |The value of this column records the internal prepare ID corresponding to the request: <ul><li> `-1`: indicates that the current SQL statement does not use the prepared statement (PS) protocol. In other words, the statement is directly executed without being preprocessed. </li><li> A value other than `-1`: indicates that the current SQL statement uses the PS protocol, and this value is the unique identifier returned by the preprocessing step of the statement in the client. This value can be referenced to execute the previously preprocessed SQL statement. </li></ul>    |
...
| LOCK_FOR_READ_TIME      | bigint(20)          | <main id="notice" type='notice'><h4>Notice</h4><p>This view does not contain the <code>LOCK_FOR_READ_TIME</code> field in the Oracle mode of OceanBase Database.</p></main> |Time, in microseconds, spent waiting for a read lock when data is read, |
