...
In OceanBase Database V4.1.0, distribute access service (DAS) group rescan is applicable to single-level nested loop join (NLJ) execution. This optimization performs batching on the left side of the NLJ to reduce the number of scans on its right side. This optimization is enabled by default, and it can be disabled through the internal system variable `_NLJ_BATCHING_ENABLED`. (In Oracle mode, this variable needs to be enclosed in double quotes.) The syntax is as follows:
...
In the execution plan results for NLJ, you can see the `use_batch=true` flag at the operator number 4 (`4 - output`). This means that the DAS group rescan optimization is enabled for the NLJ operator. Here is an example:
