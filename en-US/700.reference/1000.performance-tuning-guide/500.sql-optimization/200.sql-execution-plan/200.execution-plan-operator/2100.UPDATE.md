...
* Q3 updates data rows in the partitioned `t2` table that satisfy `c1='100'` and sets the value of the `c2` column to 150.

* Q4 updates data rows in the partitioned `t2` table that satisfy `c2 ='100'` and sets the value of the `c2` column to `rpad(t2.c2, 10, '9')`. As shown in the execution plan, the `UPDATE` operator is placed under the `EXCHANGE` operator, indicating that the 2nd and 3rd operators will be scheduled as a task at the partition level. During execution, the 4th operator scans data in the `t2` table that meets the `c2 = '100'` condition in one partition, and the 3rd `UPDATE` operator updates only the data scanned in the corresponding partition.
...
* Q6 updates the data rows in the `t2` partitioned table that meet the condition `c1 = 100` and sets the value of the `c1` column to 101. Since the updated column is a primary key column, the updated data rows may be in different partitions from the original data rows. Therefore, the `DISTRIBUTED UPDATE` operator is used for the update.
