...
* Q3 updates the data rows in the `t2` partition table that meet the condition `c1='100'` and sets the value of the `c2` column to 150.

* Q4 updates the data rows in the `t2` partition table that meet the condition `c2 ='100'` and sets the value of the `c2` column to `rpad(t2.c2, 10, '9')`. As shown in the execution plan, the `UPDATE` operator is placed under the `EXCHANGE` operator, so operators 2 and 3 will be scheduled as a task at the partition level. When executing, operator 4 scans the data in the `t2` partition that meets the condition `c2 = '100'`, and operator 3, the `UPDATE` operator, will only update the data it scanned in the corresponding partition.
...
* Q6 updates the data rows in the `t2` partitioned table that meet the condition `c1 = 100` and sets the value of the `c1` column to 101. Since the updated column is a primary key column, the updated data rows may be located in different partitions from the original data rows. Therefore, the `DISTRIBUTED UPDATE` operator is used for the update.
