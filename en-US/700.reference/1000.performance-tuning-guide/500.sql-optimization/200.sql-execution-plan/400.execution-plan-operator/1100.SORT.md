...
If the `ORDER BY` block is followed by a `LIMIT` clause (applicable only to the MySQL-compatible mode), the optimizer further optimizes the execution plan and generates the `TOP-N SORT` operator. That is, the optimizer uses heap sorting to select TOP-N data. Here is an example:
