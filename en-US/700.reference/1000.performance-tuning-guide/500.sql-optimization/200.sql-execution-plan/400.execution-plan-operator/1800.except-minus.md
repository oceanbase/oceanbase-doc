...
Generally, `MINUS` operators are used in Oracle-compatible mode to perform difference set operations, whereas `EXCEPT` operators are used in MySQL-compatible mode to perform difference set operations. In the MySQL-compatible mode of OceanBase Database, however, the `EXCEPT` and `MINUS` operators are not distinguished and both of them can be used for difference set operations.
...
| **Field** | **Description** |
|----------|-----------------------------------------------------------------------------------------------------------------------------------------|
| output | The output expressions of the operator. The output of the two suboperators are joined by `EXCEPT/MINUS` (`MINUS` for the Oracle-compatible mode and `EXCEPT` for the MySQL-compatible mode) to generate the difference set. Within the brackets are the output columns of the left and right suboperators.  |
