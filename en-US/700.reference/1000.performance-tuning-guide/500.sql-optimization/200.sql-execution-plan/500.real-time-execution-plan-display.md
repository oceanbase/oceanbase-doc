...
When a user executes a large SQL statement and the current connection takes a long time to respond, the user may want to know the execution status of the SQL statement, such as the execution plan and the execution process. In this case, the user can establish a new connection, use the `SHOW PROCESSLIST` command to locate the session ID of the SQL statement, and use the `DBMS_XPLAN` package and the `DISPLAY_ACTIVE_SESSION_PLAN` function to display the execution plan of the large SQL statement.
...
2. Connect to another session and use the `SHOW PROCESSLIST` command to query the session ID of the SQL statement.
...
3. Use the `DBMS_XPLAN` package to display the plan details of the session.

   ```shell
   /* Display the execution plan of a session in MySQL mode */
   ...
   /* Display the execution plan of a session in Oracle mode */
...
Additionally, the `GV$OB_SQL_PLAN` (or `V$OB_SQL_PLAN`) view displays the execution plans of all (or current) nodes of the current tenant, but we recommend that you use the `DBMS_XPLAN` package to view execution plans.
...
| **Column**  |  **Description**  |
|-------------|--------------|
| TENANT_ID    | The tenant ID. 1 indicates the system tenant ID, and other values indicate the user tenant ID or meta tenant ID. |
| PLAN_ID      | The plan ID. |
| SVR_IP       | The IP address of the server where the replica resides. |
| SVR_PORT     | The port number of the server where the replica resides. |
| SQL_ID       | The query ID. |
| DB_ID        | The ID of the schema to which the query belongs. |
| PLAN_HASH_VALUE | The hash value of the plan. |
| GMT_CREATE   | The record creation time. |
| OPERATOR     | The operator name (for example, `TABLE SCAN`, `SORT`). |
| OPTIONS      | Reserved. |
...
| OBJECT_TYPE  | The type of the scanned object (for example, synonym, dblink, basic table). |
| OPTIMIZER    | Index-related information (for example, physical rows, logical rows, tables to be joined, the type of statistics used, and the version of the statistics). |
| ID           | The ID of the logical operator. |
| PARENT_ID    | The ID of the parent operator. |
| DEPTH        | The depth of the operator in the current plan, that is, the layer of the operator in the binary plan tree. |
| POSITION     | The position of the operator among its siblings. |
...
| REAL_COST    | The actual cost when the plan is executed for the first time (feedback information). |
| CARDINALITY  | The number of output rows estimated by the optimizer for the operator. |
| REAL_CARDINALITY | The actual number of output rows when the operator is executed for the first time (feedback information). |
| BYTES        | The width of data estimated by the optimizer for the operator. |
| ROWSET       | The vectorized size of the operator. |
| OTHER_TAG    | Oracle-compatible field. The OceanBase database saves in this field the hints used in the current query. This field is recorded only in the first row of a plan. |
| PARTITION_START | The partition information scanned by the `TABLE SCAN` operator. |
| PARTITION_STOP  | Reserved. |
| PARTITION_ID | Reserved. |
| OTHER        | Oracle-compatible field. The OceanBase database saves in this field optimization information such as the plan type, the parameters of Fast Parser, Plan Note, and the constraints that the plan hits. |
| DISTRIBUTION | Reserved. |
| CPU_COST     | The actual CPU overhead of the operator when the plan is executed for the first time (feedback information). |
| IO_COST      | The actual I/O overhead of the operator when the plan is executed for the first time (feedback information). |
| TEMP_SPACE   | Reserved. |
| ACCESS_PREDICATES | The expression information that the operator needs to access. |
| FILTER_PREDICATES | The filtering conditions of the operator. |
| STARTUP_PREDICATES | The startup conditions of the operator. |
...
You can execute the `EXPLAIN` statement to display the execution plan generated by the optimizer. However, the actual execution plan used by the SQL query in the system may be different from the one displayed in the result of the `EXPLAIN` statement, as the actual execution plan can change due to statistics changes or user session variables changes. To determine the actual execution plan used by the SQL query in the system, you need to analyze the physical execution plans in the plan cache.

The plan cache of each server of OceanBase Database is independent. You can directly query the `V$OB_PLAN_CACHE_PLAN_STAT` view on the server to query the plan cache and provide the `tenant_id` and the SQL string (fuzzy matching is supported) to find the `plan_id` of the SQL query in the plan cache. Then, you can display the physical execution plan of the SQL query in the plan cache.

You can query the `V$OB_PLAN_CACHE_PLAN_EXPLAIN` view for the execution plan information.
...
1. Query the `V$OB_PLAN_CACHE_PLAN_STAT` view for the `plan_id` of the SQL query in the plan cache.
...
2. Use the `tenant_id` and the `plan_id` to query the physical execution plan from the `V$OB_PLAN_CACHE_PLAN_EXPLAIN` view.

   <main id="notice" type='notice'>
     <h4>Notice</h4>
     <p>The plans displayed in the view are physical execution plans. The operator names in the plans differ from those displayed in the result of the <code>EXPLAIN</code> statement.</p>
   </main>
...
Starting from OceanBase Database V4.0, the Real-time SQL Plan Monitor feature is supported. It tracks detailed performance metrics of each execution operator, such as the first-row return time, output row count, hash collision rate, and disk I/Os. You can query the `GV$SQL_PLAN_MONITOR` tenant-level view for the execution plan information, including the number of rows produced by each operator and the start and end times of each operator.

### Enable the SQL Plan Monitor feature

Set the [`enable_perf_event`](../../../800.configuration-items-and-system-variables/100.system-configuration-items/300.cluster-level-configuration-items/7700.enable_perf_event.md) parameter to true to enable the SQL Plan Monitor feature.

After the SQL Plan Monitor feature is enabled, the following SQL statements are recorded in the SQL Plan Monitor:

- Statements with the `/*+ MONITOR */` hint.
- Statements that execute parallel data modification language (PDML) statements.
- Statements whose first expensive operator takes more than three seconds to execute.
...
This topic explains how to query information about execution plans using the `GV$SQL_PLAN_MONITOR` and `V$SQL_MONITOR_STATNAME` views.

1. Query the `GV$OB_SQL_AUDIT` view for the `trace_id`.
...
2. Query the `GV$SQL_PLAN_MONITOR` view for a summary or details of the execution plan.
   ...
   You can also query the `GV$SQL_PLAN_MONITOR` view for details of the execution plan.
...
3. Query the `V$SQL_MONITOR_STATNAME` view for the meaning of the ID in the `OTHERSTAT_?_ID` column of the `GV$SQL_PLAN_MONITOR` view.
   ...
   -- A snippet of the query result is as follows:
