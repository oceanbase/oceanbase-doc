...
* Parallel execution control based on real-time load
...
### Tuning of parallel DML parameters
...
* In OceanBase Database of a version earlier than V4.1, if the execution time of a single parallel DML statement exceeds 30 minutes, you must set the `undo_retention` parameter. For more information, see the "Transaction processing" section in the [Classification and optimization](../150.classification-of-parallel-execution.md) topic.

## Adaptive parallel control based on real-time load

Starting from OceanBase Database V4.6.0, the system supports using the real-time number of PX tasks on a node as the basis for determining whether there are sufficient resources to execute parallel queries. The following configuration items are introduced:

|        **Parameter name**        | **Default value** | **Level** | **Description** |
| -------------------------- | --------- | -------- | -------- |
| `px_target_workers_per_cpu` | 8 | Tenant | Controls the ratio between the expected number of parallel execution threads and the number of CPU cores in a tenant. |
| `px_target_low_watermark` | 60 | Tenant | Sets the low watermark percentage threshold for the number of parallel threads. When the threshold is exceeded, the parallelism of new DFOs is downgraded to prevent CPU resource exhaustion. |
| `px_target_high_watermark` | 80 | Tenant | Controls the high watermark percentage threshold for the number of parallel threads. The closer the real-time load is to this watermark, the greater the degree of parallelism downgrade. When the threshold is exceeded, the parallelism is reduced to 1. |

<main id="notice" type='notice'>
  <h4>Notice</h4>
  <p>These three configuration items must satisfy: <code>px_target_low_watermark</code> <= <code>px_target_high_watermark</code> <= <code>px_target_workers_per_cpu</code>. </p>
</main>

### Considerations

* In V4.6.0 and later, the adaptive parallel control based on real-time load is enabled by default for new tenants. The parallel control based on the `parallel_servers_target` parameter is disabled.
* If you upgrade to V4.6.0, the parallel control based on the `parallel_servers_target` parameter is enabled by default. To use the adaptive parallel control based on real-time load, set the cluster-level configuration item `_enable_px_adaptive_dop` to `true`, as shown in the following example:

    ```shell
    obclient> ALTER SYSTEM SET _enable_px_adaptive_dop = true;
    ```

## References

* [px_target_low_watermark](../../../../800.configuration-items-and-system-variables/100.system-configuration-items/400.tenant-level-configuration-items/6870.px_target_low_watermark.md)
* [px_target_high_watermark](../../../../800.configuration-items-and-system-variables/100.system-configuration-items/400.tenant-level-configuration-items/6860.px_target_high_watermark.md)
* [px_target_workers_per_cpu](../../../../800.configuration-items-and-system-variables/100.system-configuration-items/400.tenant-level-configuration-items/6880.px_target_workers_per_cpu.md)