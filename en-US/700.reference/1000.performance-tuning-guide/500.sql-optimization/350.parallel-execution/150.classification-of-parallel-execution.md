...
2. **Determine the degree of parallelism (DOP)**
   ...
   * If it is detected that the data accessed by a DFO is less than a macroblock, the DOP partially decreases during running.
...
In most scenarios, parallel DML can significantly improve the data import, update, and deletion performance.

### DOP

The DOP for DML is consistent with that of the query part. If parallel DML is enabled, parallel execution is automatically enabled for the query part. The data read is redistributed based on the partition locations in the table to be updated. The DML operations are executed in parallel by multiple threads. Each thread processes the data of multiple partitions.
...
### Strategy for processing index tables

Automatic maintenance is supported for index tables during parallel DML.

- The storage layer automatically processes local indexes when the primary table is updated by using parallel DML.
- If the parallel DML framework is used, a corresponding plan is generated to maintain global indexes.
...
<!-- ![Parallel DML index](https://obbusiness-private.oss-cn-shanghai.aliyuncs.com/doc/img/observer/V4.2.0/px-index-table.jpg) -->
...
<!-- ![Parallel DML index 2](https://obbusiness-private.oss-cn-shanghai.aliyuncs.com/doc/img/observer/V4.2.0/px-index-table-2.png) -->
...
Like regular DML statements in OceanBase Database, parallel DML statements also support transaction processing. A PDML statement can appear in the same transaction with another query statement. After the parallel DML statement is executed, the transaction does not need to be immediately committed. The execution result of the parallel DML statement can be read in the subsequent query statement.

In OceanBase Database of a version earlier than V4.1.0, if the execution time of a parallel DML statement is excessively long, you must set an appropriate value for the tenant-level parameter `undo_retention`. Otherwise, the error `-4138 (OB_SNAPSHOT_DISCARDED)` may occur and the SQL statements are repeatedly retried until a timeout.
* The `undo_retention` parameter specifies the checkpoint for retaining undo logs, namely the time range of undo logs to be retained counting from the current time. In OceanBase Database, all multiversion data within the specified period is retained. If the execution time of a parallel DML statement exceeds the value of the `undo_retention` parameter, the multiversion data may be evicted. If any subsequent operation tries to access the evicted multiversion data, the error `OB_SNAPSHOT_DISCARDED` will be returned.
* The default value of the `undo_retention` parameter is 30 minutes. To be specific, if a parallel DML statement is not completed within 30 minutes, it can time out with an error returned, regardless of the timeout value specified for the statement. Generally, if the maximum parallel DML execution time is 2 hours, you can set `undo_retention` to 2.5 hours. You cannot set `undo_retention` to a very large value. Otherwise, the multiversion data cannot be recycled and the disk space will be used up.

Since OceanBase Database V4.1.0, the execution time of parallel DML statements is no longer controlled by the `undo_retention` parameter. Multiversion data is recycled based on the transaction version. The data that can be read based on the version of a transaction will not be recycled as long as this transaction is active. However, it makes an exception if the data disk is full. In this case, the multiversion data is forcibly recycled. The system returns the error `OB_SNAPSHOT_DISCARDED` for the parallel DML statement and automatically retries the whole statement.

### Direct load

Insufficient memory space will easily lead to an out of memory (OOM) error for parallel DML statements. A parallel DML statement that does not use a direct load path first writes data to the MemTable, and then the data is written to the disk through minor and major compactions. If the parallel DML statement writes data to the MemTable at a speed higher than the minor compaction speed, the memory usage keeps increasing until an OOM error is returned.

To resolve this issue, OceanBase Database introduces the direct load feature at the storage layer. If a parallel DML statement executes an INSERT operation in direct load mode, data will be directly written to the disk without the need to be written to the MemTable first. This avoids the OOM error and improves the data import performance.

You can use an `APPEND` hint to enable the direct load feature. Before you use direct load, you must commit the previous transaction and set `AUTOCOMMIT` to `1`. In OceanBase Database V4.2.0, direct load must be used in combination with parallel DML. If parallel DML is not enabled by using a hint or for the session, the direct load hint is automatically ignored. Here is a sample syntax:
...
* The `INSERT ALL`, `UPDATE`, `DELETE`, or `MERGE INTO` statement is executed on multiple tables.
* Batch execution is performed using the ArrayBinding feature.
* A DBLink is used.
* The `INSERT...VALUES...`, `INSERT UPDATE`, or `REPLACE` statement is used.
* The `ON UPDATE CURRENT_TIMESTAMP` clause is used to define a column.
...
* The sample syntax for using the `ALTER SESSION` statement to modify the DOP in Oracle mode is as follows:
  ...
  In OceanBase Database V4.2.0, you can use the `ALTER SESSION FORCE PARALLEL DML PARALLEL` statement rather than the `ALTER SESSION FORCE PARALLEL DDL PARALLEL` statement to enable parallel execution for the `CREATE TABLE AS SELECT` statement in Oracle mode.
...
<main id="notice" type='notice'>
      <h4>Notice</h4>
      <p>In OceanBase Database V4.2.0, the `CREATE TABLE AS SELECT` statement does not support direct load. If the amount of data is large, we recommend that you first create an empty table and then insert data into the table in parallel through direct load by using parallel DML. </p>
</main>

## Parallel LOAD DATA

`LOAD DATA` is not implemented based on parallel DML. Specifically, the system uses multiple threads to split the CSV file into multiple `INSERT` statements and then distributes the `INSERT` statements based on the specified DOP.
