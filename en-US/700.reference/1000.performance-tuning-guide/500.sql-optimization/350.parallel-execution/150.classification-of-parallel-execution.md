|slug|classification-of-parallel-execution|
|---|---|
...
When you update the partitioning key of a partitioned table, data may be migrated from one partition to another. In Oracle-compatible mode, you can execute the following statement to disable cross-partition data migration:
...
* The sample syntax for using the `ALTER SESSION` statement to modify the DOP in Oracle-compatible mode is as follows:
...
* In MySQL-compatible mode, the syntax varies slightly. The `SET` statement is used to specify the DOP by using a session variable. Here is an example:
  ...
  In OceanBase Database V4.2.0, you can use the `SET _FORCE_PARALLEL_DML_DOP` statement rather than the `SET _FORCE_PARALLEL_DDL_DOP` statement to enable parallel execution for the `CREATE TABLE AS SELECT` statement in MySQL-compatible mode.
...
* In Oracle-compatible mode, execute the `ALTER SESSION` statement to enable parallel execution. The syntax is as follows:
...
* In MySQL-compatible mode, use the `SET` statement to set a session variable to enable parallel execution. Here is an example:
  ...
  In OceanBase Database V4.2.0, you can use the `SET _ENABLE_PARALLEL_DML` statement rather than the `SET _ENABLE_PARALLEL_DDL` statement to enable parallel execution for the `CREATE TABLE AS SELECT` statement in MySQL-compatible mode.
