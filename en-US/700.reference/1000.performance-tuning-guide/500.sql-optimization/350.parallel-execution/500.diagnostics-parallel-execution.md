...
Query the `oceanbase.GV$SQL_PLAN_MONITOR` view in MySQL-compatible mode or the `SYS.GV$SQL_PLAN_MONITOR` view in Oracle-compatible mode for the execution status of each operator in each PX thread. Since OceanBase Database V4.1, the `GV$SQL_PLAN_MONITOR` view records two parts of data:
