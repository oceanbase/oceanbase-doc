...
* At most N threads can be requested from the resource pool at a time, where N = Value of `MIN_CPU` of the unit config for the resource units of the tenant × Value of `px_workers_per_cpu_quota`. At most N threads are allocated even if more than N threads are requested. `px_workers_per_cpu_quota` is a cluster-level parameter and is `10` by default. For example, if the DOP of a DFO is 100, the DFO needs to request 30 threads from Node A and 70 threads from Node B. Assume that the value of `MIN_CPU` of the unit config is `4` and the value of `px_workers_per_cpu_quota` is `10`, N = 4 ×10 = 40. The DFO can actually request 30 threads from Node A and 40 threads from Node B. Its actual DOP is 70.
