...
|dir-name-en||
|tenant-type||
|slug|clean-up-workload-repo-data|

# Clean up WR data

You can use SQL statements to check the storage usage by Workload Repository (WR). If WR occupies too much storage space, you can use the database management system (DBMS) package to delete data.
...
2. Clean up the space occupied by WR.
   After you use the DBMS package to clean up the space occupied by WR, you must initiate a major compaction.
    ...
    2. Delete snapshot data within the specified snapshot ID range. The following example deletes data whose snapshot IDs fall in the range of [47,275]:
    ...
    3. Query whether the snapshots are deleted.
...
3. Initiate a major compaction to release the space. For more information about major compactions, see [Major compaction](../../../100.oceanbase-database-concepts/900.storage-architecture/300.dump-and-merge/300.about-merge.md).
   ...
   2. Query the `oceanbase.CDB_OB_ZONE_MAJOR_COMPACTION` view to check whether the major compaction is completed. If the value of the `STATUS` column changes to `IDLE`, the major compaction is completed.

        ```sql
        obclient [oceanbase]> SELECT * FROM oceanbase.CDB_OB_ZONE_MAJOR_COMPACTION where tenant_id = 1;
        ```

        The value of the `STATUS` column is `COMPACTING`, indicating that the major compaction is in progress.
