...
* **Use the `oceanbase.CDB_WR_ACTIVE_SESSION_HISTORY` view to view ASH data of all tenants after it is persisted.**
  ...
  | GROUP_ID                    | The ID of the resource group to which the session belongs.     |
...
* **Use the `GV$OB_ACTIVE_SESSION_HISTORY` view to view the history of active sessions.**
  ...
  | THREAD_ID                   | The ID of the thread on which the active session resides.    |
  | STMT_TYPE                   | The type of the SQL statement of the active session.     |
  | TIME_MODEL                  | bigint(20)          | NO   | time model  information, which is a set of data in all IN_XXX fields (such as IN_PARSE and IN_PL_PARSE).     |
  | IN_PARSE                       | Indicates whether the session is parsing an SQL statement at the sampling time. |
  | IN_PL_PARSE                       | Indicates whether the session is parsing an SQL PL statement at the sampling time. |
  | IN_PLAN_CACHE            | Indicates whether the session is matching a plan in the plan cache at the sampling time. |
  | IN_SQL_OPTIMIZE            | Indicates whether the session is optimizing an SQL statement at the sampling time. |
...
* **Use the `V$OB_ACTIVE_SESSION_HISTORY` view to view the history of active sessions.**
  ...
  | THREAD_ID                   | The ID of the thread on which the active session resides.    |
  | STMT_TYPE                   | The type of the SQL statement of the active session.     |
  | TIME_MODEL                  | time model  information, which is a set of data in all IN_XXX fields (such as IN_PARSE and IN_PL_PARSE).     |
  | IN_PARSE                    | Indicates whether the session is parsing an SQL statement at the sampling time. |
  | IN_PL_PARSE                 | Indicates whether the session is parsing an SQL PL statement at the sampling time. |
  | IN_PLAN_CACHE               | Indicates whether the session is matching a plan in the plan cache at the sampling time. |
  | IN_SQL_OPTIMIZE             | Indicates whether the session is optimizing an SQL statement at the sampling time. |
...
* **Use the `GV$OB_SQLSTAT` view to view the basic performance statistics of SQL statements in the cluster.**
  ...
  *************************** 1. row ***************************
  ...
  | ROUTE_MISS_TOTAL              | The total number of times requests were not matched to the requested partitions since the OBServer node started.   |
  | ROUTE_MISS_DELTA              | The number of times requests were not matched to the requested partitions since the last WR snapshot was collected.   |
  | FIRST_LOAD_TIME               | The time when the plan was generated. |
  | PLAN_CACHE_HIT_TOTAL          | The total number of times the plan cache was hit since the OBServer node started.   |
...
* **You can query the statistics of resource groups in all OBServers by using the `GV$OB_RES_MGR_SYSSTAT` view.**
  ...
  | ---- | ---- |
  | CON_ID     | Tenant ID     |
  | GROUP_ID   | ID of the resource group     |
