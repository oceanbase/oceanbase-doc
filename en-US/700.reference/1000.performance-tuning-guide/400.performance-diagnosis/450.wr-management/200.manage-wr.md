...
| topnsql | The number of SQLSTAT records to be written to disk. The value range is [30, 50000]. |
| sqlstat_interval| The time interval between SQLSTAT snapshots, in minutes. |

Example: Set the snapshot retention period to 2,265 minutes (about 1 day and 12 hours) and the interval between snapshots to 16 minutes. Write 30 SQLSTAT records to disk and set the time interval between SQLSTAT snapshots to 5 minutes.
...
* Query the `oceanbase.CDB_WR_CONTROL` view for WR related configurations in all tenants.
  ...
  | SNAP_INTERVAL | The time interval between snapshots, in the `DD HH:mm:SS` format for easy reading. |
  | RETENTION | The retention period of snapshots, in the `DD HH:mm:SS` format for easy reading. |
...
* Query the `oceanbase.CDB_WR_SNAPSHOT` view for snapshots of all tenants.
  ...
  | **Field** | **Description** |
  | --- |--- |
  | CLUSTER_ID | The ID of the cluster. <ul><li>-1: indicates that the snapshot is pre-written to disk. </li><li>Normal ID: indicates that the snapshot is written to disk at regular intervals. </li></ul> |
