...
<main id="notice" type='notice'>
  <h4>Notice</h4>
  <p>This feature is supported only in the <code>sys</code> tenant, and is unavailable during upgrade. </p>
</main>
...
In the syntax, the value of `flush_level` can be only `TYPICAL`, which indicates the periodic snapshot mode. This means WR collects most statistics only in periodic snapshot mode. Here is an example:
...
Example: Set the snapshot retention period to 2,265 minutes (about 1 day and 12 hours) and the interval between snapshots to 16 minutes. Here is an example:
...
* Query the `oceanbase.DBA_WR_CONTROL` view for WR related configurations in the current tenant.
...
* Query the `oceanbase.CDB_WR_SNAPSHOT` view for snapshots of all tenants.
  ...
  | STARTUP_TIME  | The time when the process was started. The value is a timestamp accurate to milliseconds. |
    ...
    For more information about the `oceanbase.CDB_WR_SNAPSHOT` view, see [oceanbase.CDB_WR_SNAPSHOT](../../../700.system-views/300.system-view-of-sys-tenant/200.dictionary-view-of-sys-tenant/13500.o-cdb_wr_snapshot-of-sys-tenant.md).
