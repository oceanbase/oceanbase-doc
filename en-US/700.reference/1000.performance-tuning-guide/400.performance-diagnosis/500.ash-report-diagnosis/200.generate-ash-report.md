...
<main id="notice" type='notice'>
  ...
  <li>In MySQL-compatible mode, if you explicitly specify a value for a non-required parameter, you must explicitly set the other non-required parameters to `NULL`. Otherwise, the parameters will be ignored. </li></ul>
</main>

You can query the `GV$OB_ACTIVE_SESSION_HISTORY` view to obtain information about the ASH PL package. The `GV$OB_ACTIVE_SESSION_HISTORY` view provides session activity information. Any session that is connected to the database and waiting in the queue is considered an active session. Each session is sampled by a set of rows returned by the `GV$OB_ACTIVE_SESSION_HISTORY` view, and each active session is displayed in the view during the sampling period. For more information about this view, see [GV$OB_ACTIVE_SESSION_HISTORY (MySQL-compatible mode)](../../../700.system-views/400.system-view-of-mysql-mode/300.performance-view-of-mysql-mode/400.gv-ob_active_session_history-of-mysql-mode.md) and [GV$OB_ACTIVE_SESSION_HISTORY (Oracle-compatible mode)](../../../700.system-views/500.system-view-of-oracle-mode/300.performance-view-of-oracle-mode/600.gv-ob_active_session_history-of-oracle-mode.md).
...
1. Log in to a MySQL-compatible tenant of the cluster as a tenant administrator and enter OceanBase Database.
...
2. Display the report information on the console.
    ...
    * Display the report content on the console in TEXT format.
      ...
      Set the diagnostic start time to `2026-01-25 10:26:47`, the diagnostic end time to `2026-01-25 20:27:07`, and the report display format to `TEXT`. Set other parameters as needed.
    ...
    * Display the report content on the console in HTML format.
      ...
      Set the diagnostic start time to `2026-01-25 10:26:47`, the diagnostic end time to `2026-01-25 20:27:07`, and the report display format to `HTML`. Set other parameters as needed.
...
1. Log in to an Oracle-compatible tenant of the cluster as the `SYS` user and enter OceanBase Database.
