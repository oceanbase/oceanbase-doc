...
You can use the latency information to evaluate whether the response time (rt) of a single SQL statement meets your expectations. The latency information of a cluster includes:

* Latency between IDCs

* Latency between zones

* Latency between ODP and an OBServer node

* Latency between the client and ODP
...
You can use commands such as `ping`, `tsar`, `ethtool xxx`, and `ifconfig` to obtain the information. The following section uses deployment with three full replicas (FFF) in three IDCs in the same region as an example.
...
Throughout the process from the application server sending a request to an OBServer node receiving it, any involved components deserve our attention, as any bottlenecks on these components can significantly impact performance. Pay attention to:

* Physical resources: Check whether resources on each component in the intermediate link have reached bottlenecks, such as JVM memory, CPU utilization of the application server and ODP, and software interrupts.

* Request routing: Check whether ODP can correctly route SQL requests and whether incorrect forwarding occurs.
