...
-- HASH partitioning (MySQL-compatible mode)
...
-- HASH partitioning (Oracle-compatible mode)
create tablegroup tg2  partition by hash partitions 2;
create table pg_trans_test2_1(id1 int, id2 int)
...
-- RANGE partitioning (MySQL-compatible mode)
...
-- RANGE partitioning (Oracle-compatible mode)
create tablegroup tg3  
  partition by range columns 1
      (partition p0 values less than (10),
         partition p1 values less than(20));
...
-- LIST partitioning (MySQL-compatible mode)
...
-- LIST partitioning (Oracle-compatible mode)
create tablegroup tg4 partition by list columns 1 (
