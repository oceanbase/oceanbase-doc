...
`sar` is used for System Activity Reporter system activity reports. It includes file read/write status, system call usage, disk I/O, CPU efficiency, memory usage, process activities, and IPC-related activities. You can use `sar` to monitor network conditions. 
