...
| QUERY_SQL               | longtext            | NO             | The actual SQL statement. <main id="notice" type='explain'> <h4>Note</h4> <p>This field can be queried only by the SYS tenant starting from V4.3.5 BP4.</p> </main>   |
...
| INSERT_DUPLICATE_ROW_COUNT | bigint(20) | YES |Records the number of duplicate rows for `insertup` or `replace into`.<main id="notice" type='explain'><ul><h4>Note</h4> <li>This field was introduced in V4.3.5 BP2.</li>|
