...
| RET_CODE                | bigint(20)          | No             | The return code of the execution. Valid values:<li> `0`: indicates that the statement is successfully executed without errors.</li>  <li> Other return codes: indicates that OceanBase Database has encountered an error. For more information about the error codes, see [Error codes](../../../900.error-code/600.error-code-of-mysql-mode/100.use-error-information-of-mysql-mode.md).  </li>  |
...
| CONSISTENCY_LEVEL       | bigint(20)          | No             |The consistency level. Valid values:</li><li>`-1`: indicates that the consistency level is invalid.</li> <li>`1`: indicates to read data stored in SSTables.</li><li>`2`: indicates weak-consistency read.</li><li>`3`: indicates strong-consistency read.</li></ul>       |
...
| PARTITION_HIT | tinyint(4) | No | </li><li> If a data access service (DAS) execution plan is obtained and a local partition is hit, the value is `1`. If no local partition is hit, the value is `0`.  </li><li>  If no DAS execution plan is obtained, the value is `1` by default. </li></ul> |
| TX_INTERNAL_ROUTING     | bigint(20)           | No            | Indicates whether transaction routing is enabled for the current transaction. Valid values:</li><li>`0`: indicates that transaction routing is disabled, and statements can only be routed to the OBServer node where the transaction starts.  </li><li>`1`: indicates that transaction routing is enabled, and statements in a transaction can be routed to the most appropriate OBServer node by using ODP. </li></ul><main id="notice" type='explain'><h4>Note</h4><p>ODP is available for this column in OceanBase Database V4.1.0 and later. </p></main> |
| TX_STATE_VERSION        | bigint(20) unsigned  | No            | The version of the transaction status. When transaction routing is enabled, the value increments if the transaction status changes after the statement is executed, and remains unchanged if the transaction status is not changed after the statement is executed. <main id="notice" type='explain'><h4>Note</h4><p>ODP is available for this column in OceanBase Database V4.1.0 and later. </p></main> |
| FLT_TRACE_ID | varchar(1024) | No | The trace ID of this record in end-to-end diagnostics. If this column is empty, end-to-end diagnostics is not enabled. The value is a universally unique identifier (UUID), such as `000600d6-a5de-038c-6c80-df07e4e79149`, which is different from the value of the `TRACE_ID` column. |
...
| STMT_TYPE                       | varchar(128)        | Yes  |  The type of the SQL statement. Valid values:</li><li>An on-demand value, indicating a specific DML statement.<code>DDL_STMT</code>, indicating a DDL statement. </li><li><code>DCL_STMT</code>, indicating a DCL statement. </li><li><code>TCL_STMT</code>, indicating a TCL statement. </li><li><code>OTHER_STMT</code>, indicating a statement of other types. </li></ul>    |
| TOTAL_MEMSTORE_READ_ROW_COUNT   | bigint(20)          | No   | The number of all rows read in the MemStore in the whole working process. This column is displayed only in a thread that contains `query_text`.     |
| TOTAL_SSSTORE_READ_ROW_COUNT    | bigint(20)          | No   | The number of all rows read in the SSStore in the whole working process. This column is displayed only in a thread that contains `query_text`.     |
| PROXY_USER                      | varchar(128)        | Yes  |</li><li>The value of this column is the username of a proxy user when the proxy user is used for login. </li><li>The value of this column is empty when a non-proxy user is used for login. </li></ul> |
