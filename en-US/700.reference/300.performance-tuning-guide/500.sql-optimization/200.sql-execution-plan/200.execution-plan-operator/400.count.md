...
=======
The output of the preceding example shows that in the new SQL query, you can still find `ROWNUM`. However, the expression that contains `ROWNUM` is already placed with the equivalent `LIMIT` expression. This conversion allows you to optimize the query further. For more information, see [LIMIT](../../../../500.system-reference/100.system-configuration-items/300.cluster-level-configuration-items/17600.row_compaction_update_limit.md).
>>>>>>> V4.2.0
