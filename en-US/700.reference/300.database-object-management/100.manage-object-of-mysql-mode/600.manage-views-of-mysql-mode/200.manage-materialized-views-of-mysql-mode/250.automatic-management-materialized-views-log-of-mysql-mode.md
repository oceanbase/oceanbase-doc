...
When you create a materialized view, an mlog is automatically created or its definition is automatically updated only when the materialized view is explicitly declared as an incremental refresh materialized view (by specifying `REFRESH FAST`) or a real-time materialized view (by specifying `ENABLE ON QUERY COMPUTATION`).
