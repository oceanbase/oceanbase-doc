...
* OceanBase Database does not allow you to modify the properties of a materialized view, such as the refresh method, refresh time, and name.
* DDL operations on the base table of a materialized view may cause refresh failures of the materialized view.

  * To perform a full refresh for a materialized view, make sure that the column types of the materialized view match those of the base table.
  * To perform an incremental refresh for a materialized view, you must create a materialized view log in the base table. Some DDL operations on such base tables are blocked.
...
* When you maintain or update data of a materialized view with the `PRIMARY KEY` constraint, the maintenance or update fails if the data does not meet the constraint. For example, when you execute the `CREATE MATERIALIZED VIEW mv1(PRIMARY KEY(c1)) AS SELECT c1 FROM t1;` statement to create a materialized view named `mv1`, the system returns an error if the `c1` column in the `t1` table contains null values.
...
* You need the `SELECT` privilege on all base tables and the corresponding materialized view logs to perform an incremental refresh for a materialized view.
