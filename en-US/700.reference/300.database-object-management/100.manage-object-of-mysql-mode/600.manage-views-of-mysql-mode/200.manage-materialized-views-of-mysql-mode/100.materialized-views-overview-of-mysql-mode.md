...
You can create materialized views that contain aggregate, join, and subquery operations, and index and partition them to improve performance.
...
* You cannot create a materialized view on an ordinary view or external table.
* You cannot perform data insertion (insert), data deletion (delete), or materialized view definition modification (alter) operations on a materialized view.
* You cannot modify materialized view attributes, such as the refresh method, materialized view name, and refresh timing.
* DDL operations on the base table of a materialized view may prevent the materialized view from being refreshed in the expected mode.

  * For a full refresh, the data types of the corresponding columns in the materialized view and the base table must match.
  * For a quick refresh, you must create an mlog on the base table. Some DDL operations on the base table are intercepted. If you do not delete the mlog, you can still perform a quick refresh.
