...
* `refresh_clause`: optional. The refresh method and refresh parallelism of the materialized view.

    <main id="notice" type='explain'>
      <h4>Note</h4>
      <p>For V4.4.2, starting from V4.4.2 BP2, you can specify the refresh parallelism in the <code>REFRESH ... PARALLEL n</code> clause of the <code>REFRESH</code> clause.</p>
    </main>
...
* Before you create a real-time materialized view, you must create a materialized view log for each base table on which the real-time materialized view depends. OceanBase Database supports automatic management of materialized view logs. By default, the system automatically creates materialized view logs. For more information, see [Materialized view logs](200.materialized-views-log-of-mysql-mode.md).
...
2. Create a real-time materialized view named `mv_tbl2` based on the `tbl2` table.
...
3. After you create the real-time materialized view, you can query the [DBA_MVIEWS](../../../../700.system-views/400.system-view-of-mysql-mode/200.dictionary-view-of-mysql-mode/5000.o-dba_mviews-of-mysql-mode.md) view to check whether the materialized view is a real-time materialized view.
...
4. Query the execution plan of the real-time materialized view.
...
* At present, incremental refresh is supported for materialized views that are created by using SQL statements in the following five categories: non-aggregated single-table statements, aggregated single-table statements, multi-table join statements, aggregated multi-table join statements, and `UNION ALL` statements. For SQL statements that do not fall into these five categories, incremental refresh is not supported. For more information about the requirements for incremental refresh, see the **Incremental refresh** section in [Refresh a materialized view](400.refresh-materialized-views-of-mysql-mode.md).
    ...
    <main id="notice" type='notice'>
      <h4>Notice</h4>
      <p>For OceanBase Database V4.4.2, starting from V4.4.2 BP2, when you create an incrementally refreshed materialized view, if the materialized view definition contains aggregate functions or the <code>UNION ALL</code> operator, you do not need to manually specify the dependent columns.</p>
    </main>

* The `REFRESH FAST` method uses the records in the materialized view log to determine the content to be incrementally refreshed. OceanBase Database supports automatic management of materialized view logs. By default, the system automatically creates materialized view logs. For more information, see [Materialized view log](200.materialized-views-log-of-mysql-mode.md).
...
2. Create a materialized view named `mv_tbl5` based on the `tbl5` table, and set the refresh strategy of the materialized view to incremental refresh. In the query part, group the records in the `tbl5` table by the `col2` column and calculate the number of records in each group (`cnt`), the number of non-null records in the `col3` column (`cnt_col3`), and the sum of the `col3` column (`sum_col3`) as the results of the materialized view.
    ...
    For V4.4.2, starting from V4.4.2 BP2, when creating an incremental refreshed materialized view, if the materialized view definition contains aggregate functions or the `UNION ALL` operator, you do not need to manually specify the corresponding dependent columns. You can use the following statement to create the materialized view:
...
4. Create an incrementally refreshed materialized view based on a regular view that is declared as a dimension table (that is, with the `AS OF PROCTIME()` clause).
...
### Specify the refresh parallelism when creating a materialized view

When creating a materialized view, you can specify the refresh parallelism using the `REFRESH ... PARALLEL n` clause.

<main id="notice" type='explain'>
  <h4>Note</h4>
  <p>For V4.4.2, starting from V4.4.2 BP2, you can specify the refresh parallelism using the <code>REFRESH ... PARALLEL n</code> clause in the <code>REFRESH</code> clause.</p>
</main>

**Example:**

Create a materialized view named `mv_rcp3_tbl1` based on the `tbl1` table. Set the refresh strategy to complete refresh (`REFRESH COMPLETE`) and specify the query parallelism to 16 and the refresh parallelism to 5. Select the `col1` and `col2` columns that meet the `col3 >= 20` condition from the `tbl1` table as the data source of the materialized view.
