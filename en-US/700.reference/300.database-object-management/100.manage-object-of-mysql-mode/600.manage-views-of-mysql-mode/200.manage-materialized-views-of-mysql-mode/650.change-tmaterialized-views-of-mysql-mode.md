...
<main id="notice" type='explain'>
  <h4>Note</h4>
  <p>In OceanBase Database V4.3.5, the feature to modify the attributes of materialized views is supported in the V4.3.5 BP1 and later versions. </p>
</main>
...
* `database.`: the database where the materialized view to be modified resides. If you omit `database.`, the default database of the current session is used.
* `view_name`: the name of the materialized view to be modified.
* `alter_mview_action_list`: the list of actions that can be performed on the materialized view. You can specify multiple actions at a time, separated by commas (`,`).
