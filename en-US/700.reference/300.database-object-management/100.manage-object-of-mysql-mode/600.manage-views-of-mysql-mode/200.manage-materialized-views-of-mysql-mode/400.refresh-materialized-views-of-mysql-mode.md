...
3. Create a materialized view with an incremental refresh method.
    ...
    * Create a materialized view using the `MAX` and `MIN` aggregate functions. The query part selects the `col1` and `col2` columns from the `test_tbl1` table and calculates the aggregate results of `count(*)`, the sum of the minimum value of `col3` and the maximum value of `col4` in each group, grouping by the combination of `col1` and `col2`. The query is as follows: `SELECT col1, col2, count(*), sum(MIN(col3) + MAX(col4)) FROM test_tbl1 GROUP BY col1, col2;`.
      ...
      2. Create a materialized view named `mv7_test_tbl1`. Set the refresh method to incremental and allow manual refresh triggers as needed. The query part selects the `col1` and `col2` columns from the `test_tbl1` table and calculates the aggregate results of `count(*)`, the sum of the minimum value of `col3` and the maximum value of `col4` in each group, grouping by the combination of `col1` and `col2`. The query is as follows: `SELECT col1, col2, count(*), sum(MIN(col3) + MAX(col4)) FROM test_tbl1 GROUP BY col1, col2;`.
    ...
    * Use non-basic columns in the `GROUP BY` clause. The query part selects the `col3` and `(DATE(col2))` columns from the `m_tbl1` table and calculates the aggregate results of `count(*)`, the sum of the minimum value of `col3` and the maximum value of `col4` in each group, grouping by the combination of `col3` and `(DATE(col2))`. The query is as follows: `SELECT col3, DATE(col2), count(*), sum(MIN(col3) + MAX(col4)) FROM m_tbl1 GROUP BY col3, DATE(col2);`.

      1. Create a table named `m_tbl1`. The query is as follows: `CREATE TABLE m_tbl1 (col1 INT, col2 DATE, col3 INT, col4 INT);`.

          ```shell
          obclient> CREATE TABLE m_tbl1(col1 INT PRIMARY KEY, col2 DATETIME, col3 INT, col4 INT, col5 INT, col6 INT);
          ```

      2. Create a materialized view log on the `m_tbl1` table. The query is as follows: `CREATE MATERIALIZED VIEW LOG ON m_tbl1;`.
      ...
      3. Create an index named `idx1_m_tbl1` on the `m_tbl1` table based on the `col3` and `(DATE(col2))` columns. The query is as follows: `CREATE INDEX idx1_m_tbl1 ON m_tbl1 (col3, DATE(col2));`.

          ```shell
          obclient> CREATE INDEX idx1_m_tbl1 ON m_tbl1(col3, (DATE(col2)));
          ```

      4. Create a materialized view named `m_tbl1_mv1` for single-table aggregate incremental refresh. The query part selects the `col3` and `(DATE(col2))` columns from the `m_tbl1` table and calculates the aggregate results of `count(*)`, the sum of the minimum value of `col3` and the maximum value of `col4` in each group, grouping by the combination of `col3` and `(DATE(col2))`. The query is as follows: `SELECT col3, DATE(col2), count(*), sum(MIN(col3) + MAX(col4)) FROM m_tbl1 GROUP BY col3, DATE(col2);`.
...
A set materialized view supports the `UNION ALL` clause and allows incremental refreshes. An incremental refresh of a set materialized view allows the use of all materialized views that support incremental refreshes except for excepted join materialized views in the set branches.
...
3. Create a materialized view `mv_ua_tbl1_tbl2` that supports incremental refreshes of set queries. The materialized view is based on the `UNION ALL` clause. The materialized view is created by using the `INCREMENTAL_REFRESH` option.
...
OceanBase Database collects and stores statistics about view refresh operations. You can query these statistics from specific views. The statistics about current and historical view refresh operations are stored in the database. By analyzing the historical statistics, you can understand and analyze the performance of view refresh operations in the database.
...
* Diagnostics: By analyzing the detailed current and historical statistics, you can effectively analyze the performance of view refresh operations. For example, if a view refresh operation takes a long time, the statistics can help you identify whether the performance degradation is caused by an increase in system load or a large amount of data changes.
