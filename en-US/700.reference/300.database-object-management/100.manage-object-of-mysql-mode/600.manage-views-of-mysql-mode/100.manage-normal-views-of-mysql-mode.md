...
* The `WITH CHECK OPTION` clause prevents the system from inserting or updating rows except for the rows that meet the query condition defined by the `WHERE` clause in `select_statement`. When you create a view by using the `WITH CHECK OPTION` clause, you can add the `CASCADED` or `LOCAL` keyword to clarify the check scope. 
  ...
  * OceanBase Database also supports the use of the `WITH CHECK OPTION` clause when a subquery is specified in the `where_clause` of a view filter condition.

    <main id="notice" type='notice'>
      <h4>Notice</h4>
      <p>When the <code>WITH CHECK OPTION</code> clause is used in the filter condition, make sure that the following conditions are met: <ul><li>The CREATE statement cannot create an updatable view in MySQL mode.</li><li>The <code>JOIN</code> operation is not supported in the view definition.</li></ul></p>
    </main>
...
* If `view_name_list` contains views that exist and views that do not exist, an error may be reported during execution but the views that exist are still dropped. 
