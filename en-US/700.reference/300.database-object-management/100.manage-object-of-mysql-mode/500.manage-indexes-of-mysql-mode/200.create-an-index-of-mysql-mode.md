...
* `column_definition`: the data type of the column in the table. The column on which a spatial index is to be created must be of a spatial data type. 
...
In OceanBase Database V4.3.3, you can create a multi-valued index only when you execute the `CREATE TABLE` statement to create a table. You cannot create a multi-valued index on an existing table. However, you can execute the `ALTER TABLE` statement to drop an existing multi-valued index.

<main id="notice" type='notice'>
  <h4>Notice</h4>
  <p>The multi-valued index feature is still under experiment. We recommend that you do not use it in a production environment, to avoid affecting system stability. </p>
</main>
...
* Multi-valued indexes support only the following two combinations of character sets and collations:
  ...
  * The `utf8mb4` character set and its default `utf8mb4_0900_as_cs` collation
...
<main id="notice" type='notice'>
  <h4>Notice</h4>
  <p>The full-text index feature is still under experiment. We recommend that you do not use it in a production environment, to avoid impact on system stability. </p>
</main>

In MySQL mode of OceanBase Database V4.3.3, you can use the `CREATE TABLE` statement to create a full-text index.
...
* `FULLTEXT`: specifies to create a full-text index. <main id="notice" type='notice'><h4>Notice</h4><p><ul><li>OceanBase Database V4.3.3 supports only local full-text indexes. </li><li>The full-text index feature is still under experiment. We recommend that you do not use it in a production environment, to avoid affecting system stability. </li></ul></p></main>

* `INDEX | KEY`: specifies that either `INDEX` or `KEY` can be used as the index keyword in the statement.

* `index_name`: the name of the index to be created. This parameter is optional. If you do not specify a value for this parameter, the index name is the same as the column name by default.

* `WITH PARSER {SPACE | NGRAM | BENG}`: the full-text parser. This keyword is optional. OceanBase Database V4.3.3 supports three built-in parsers: `SPACE`, `NGRAM`, and `BENG`. If you do not specify this keyword, the `SPACE` parser is used by default.

* `LOCAL`: specifies to create a local index. This keyword is optional.

   <main id="notice" type='explain'>
     <h4>Note</h4>
     <p>If you do not specify the <code>LOCAL</code> or <code>GLOBAL</code> keyword when you create an index on a partitioned table, a local index is by default. </p>
   </main>
