...
OceanBase Database in MySQL mode imposes limitations on the expressions of function-based indexes. The expressions of some system functions cannot be used as function-based indexes. For more information, see [System functions supported for function-based indexes](../500.manage-indexes-of-mysql-mode/500.function-index-list-of-supported-functions-of-mysql-mode.md) and [System functions not supported for function-based indexes](../500.manage-indexes-of-mysql-mode/600.function-index-list-of-not-supported-functions-of-mysql-mode.md).
...
* `[UNIQUE | SPATIAL]`: `UNIQUE` specifies to create a unique index, and `SPATIAL` specifies to create a spatial index. This parameter is optional. When you create a unique index or a spatial index, you must add the corresponding keyword.

* `INDEX|KEY`: specifies that either `INDEX` or `KEY` can be used as the index keyword in the statement.

* `index_name`: the name of the function-based index to be created. This parameter is optional. If this parameter is not specified, the system automatically generates a name in the `functional_index_xx` format, in which `xx` is the index ID.
...
* `index_name`: the name of the function-based index to be created. This parameter is optional. If you do not specify a value for this parameter, the system automatically generates an index name in the `functional_index_xx` format, in which `xx` is the index ID.
...
* `FULLTEXT`: specifies to create a full-text index.
   ...
   <main id="notice" type='notice'>
     <h4>Notice</h4>
     <p><ul><li>OceanBase Database V4.3.3 supports only local full-text indexes. </li><li>The full-text index feature is still under experiment. We recommend that you do not use it in a production environment, to avoid affecting system stability. </li></ul></p>
   </main>
...
* `WITH PARSER {SPACE | NGRAM | BENG}`: the full-text parser. This keyword is optional. OceanBase Database V4.3.3 supports three built-in parsers: `SPACE`, `NGRAM`, and `BENG`. If you do not specify this keyword, the `SPACE` parser is used by default. You can use the [TOKENIZE](../../../500.sql-reference/100.sql-syntax/200.common-tenant-of-mysql-mode/400.functions-of-mysql-mode/960.text-processing-functions-of-mysql-mode/200.tokenize-of-mysql-mode.md) function to view the tokenization results of the specified text by using the specified parser and JSON-formatted parameters.
...
```sql
CREATE FULLTEXT INDEX index_name ON table_name (column_name, [column_name ...]) [WITH PARSER {SPACE | NGRAM | BENG}] [LOCAL];
```
...
* `WITH PARSER {SPACE | NGRAM | BENG}`: specifies the parser to be used for the full-text index. The current version supports only the `SPACE` (space), `NGRAM`, and `BENG` (basic English) built-in parsers. If this parameter is not specified, the `SPACE` parser is used by default. You can use the [TOKENIZE](../../../500.sql-reference/100.sql-syntax/200.common-tenant-of-mysql-mode/400.functions-of-mysql-mode/960.text-processing-functions-of-mysql-mode/200.tokenize-of-mysql-mode.md) function to view the tokenization results of the specified text by using the specified parser and JSON-formatted parameters.

* `LOCAL`: specifies to create a local full-text index.

**Here are some examples:**

Create a table named `tbl3` and then create a full-text index named `ft_idx1_tbl3` on the table.
...
2. Create a full-text index named `ft_idx1_tbl3` on the `tbl3` table and specify the `NGRAM` parser for the full-text index.

   ```sql
   CREATE FULLTEXT INDEX ft_idx1_tbl3 ON tbl3(col2) WITH PARSER NGRAM;
   ```
...
```sql
ALTER TABLE table_name ADD FULLTEXT [INDEX | KEY] [index_name](column_name, [column_name ...]) [WITH PARSER {SPACE | NGRAM | BENG}] [LOCAL];
```
...
1. Create a table named `tbl4`.
   ...
   ```sql
   CREATE TABLE tbl4(col1 INT, col2 VARCHAR(20), col3 VARCHAR(50));
   ```

2. Add a full-text index named `ft_idx1_tbl4` to the `tbl4` table and specify the `BENG` parser for the full-text index.

   ```sql
   ALTER TABLE tbl4 ADD FULLTEXT INDEX ft_idx1_tbl4(col2, col3) WITH PARSER BENG;
   ```
