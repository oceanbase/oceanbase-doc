...
|slug|create-indexes-mysql-compatible-mode|
...
OceanBase Database in MySQL-compatible mode supports spatial indexes. A spatial index describes the location of data stored on a medium and is used to improve the efficiency of data acquisition by the system.
...
OceanBase Database in MySQL-compatible mode imposes limitations on the expressions of function-based indexes. The expressions of some system functions cannot be used as function-based indexes. For more information, see [System functions supported for function-based indexes](../500.manage-indexes-of-mysql-mode/500.function-index-list-of-supported-functions-of-mysql-mode.md) and [System functions not supported for function-based indexes](../500.manage-indexes-of-mysql-mode/600.function-index-list-of-not-supported-functions-of-mysql-mode.md).
...
In MySQL-compatible mode of OceanBase Database V4.3.3, you can use the `CREATE TABLE`, `CREATE FULLTEXT INDEX`, or `ALTER TABLE` statement to create a full-text index.
...
* `PARSER_PROPERTIES[=](parser_properties_list)`: Optional. Specifies the properties of the tokenizer. The available options are as follows:

  * `min_token_size`: Specifies the minimum token length. The default value is 3, and the range is from 1 to 16.
  * `max_token_size`: Specifies the maximum token length. The default value is 84, and the range is from 10 to 84.
  * `ngram_token_size`: Specifies the token length for the `NGRAM` tokenizer. This is only effective for the `NGRAM` tokenizer. The default value is 2, and the range is from 1 to 10.
  * `ik_mode`: Specifies the tokenization mode for the `IK` tokenizer. The available values are:
