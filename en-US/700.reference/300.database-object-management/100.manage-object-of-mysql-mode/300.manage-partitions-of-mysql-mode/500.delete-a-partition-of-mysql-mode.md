...
|----------|-----------------------|-------------------------|
| True | Only the `TRUNCATE`/`DROP` partition is DML-inhibited; other partitions can perform DML operations. | Global indexes remain valid: DML operations can be performed on the main table and all index tables, and global indexes related to the main table remain valid. |
| False | Only the `TRUNCATE`/`DROP` partition is DML-inhibited; other partitions can perform DML operations. | Global indexes become invalid: DML operations can be performed on the main table and all index tables, but global indexes related to the main table become invalid or are rebuilt. |
...
| Concurrent DML | <ul><li>On the target partition: Completely blocked. </li><li>On other partitions: Generally allowed, provided the statement can clearly access other partitions. </li></ul> |
| Concurrent queries (`SELECT`) | Unaffected and can proceed normally. |
