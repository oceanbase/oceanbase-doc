...
For tables partitioned by RANGE/RANGE COLUMNS or LIST/LIST COLUMNS (including both partitions and subpartitions) that support partition dropping, the system does not rebuild global indexes. Instead, it uses a lazy maintenance strategy to keep indexes valid by background data merging. This prevents query interruptions or performance degradation due to index invalidation. Note that this optimization applies only to regular global indexes. The following scenarios cannot trigger this optimization:

* When the primary partition is a HASH partition (even if the subpartition is RANGE/RANGE COLUMNS or LIST/LIST COLUMNS), and you drop a subpartition.
* When the partitioning key is an expression, not a regular column.
