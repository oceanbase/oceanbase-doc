...
For tables that use RANGE/RANGE COLUMNS or LIST/LIST COLUMNS partitioning (including both partitions and subpartitions) and support the `TRUNCATE` partition operation, the system does not rebuild global indexes. Instead, it uses a background compaction process to lazily maintain the indexes. This approach keeps the indexes valid without causing query interruptions or performance degradation due to index invalidation. Note that this optimization applies only to regular global indexes. The following scenarios do not trigger this optimization:

* When the primary partition is a HASH partition (even if the subpartition is a RANGE/RANGE COLUMNS or LIST/LIST COLUMNS partition), truncating the subpartition.
* When the partitioning key is an expression rather than a regular column.
* For special global indexes such as full-text indexes or non-structured types, the indexes will be unavailable.
