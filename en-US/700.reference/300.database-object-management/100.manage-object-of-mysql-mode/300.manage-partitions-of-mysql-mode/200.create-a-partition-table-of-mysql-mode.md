...
* In MySQL-compatible mode, the result of the `expr` expression in the `PARTITION BY RANGE ( expr )` clause must be an integer. To use a timestamp column as the partitioning key for RANGE partitioning, you must convert the values of the timestamp type column into numeric values.
...
The MySQL-compatible mode of OceanBase Database supports the HASH, RANGE, LIST, KEY, RANGE COLUMNS, and LIST COLUMNS partitioning types, and a combination of any two partitioning types as the subpartitioning type.
...
<main id="notice" type='explain'>
  ...
  <p>
    <ul>
      <li>When you use a template to create a subpartitioned table, the template defines subpartitions under each partition. In this case, subpartitions under each partition share the same definition.
          <ul>
              <li>V4.4.2 supports defining a subpartitioning strategy for a specific partition that differs from the template when creating a template-based subpartitioned table. This allows you to use both template-based and custom subpartitioning. This feature is available starting with V4.4.2 BP2. </li>
          </ul>
      </li>
...
### Create a template-based or user-defined subpartitioned table with RANGE+RANGE partitioning
