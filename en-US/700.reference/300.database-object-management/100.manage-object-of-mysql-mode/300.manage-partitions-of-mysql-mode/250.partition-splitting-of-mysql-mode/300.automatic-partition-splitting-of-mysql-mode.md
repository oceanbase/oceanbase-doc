...
| SIZE('size_value') | (Optional) The threshold for partition splitting. The value ranges from 1 MB to +∞. If the `SIZE()` clause is not specified, the value of the `auto_split_tablet_size` parameter at the tenant level is used as the default value. `size_value` can be set to `unlimited`, which indicates that partition splitting is not triggered and no new automatic partition splitting task is scheduled. <main id="notice" type='explain'><h4>Note</h4><p>Starting from V4.4.2 BP1, the minimum value of the automatic partition splitting threshold in V4.4.2 is adjusted from 128 MB to 1 MB. </p></main> <main id="notice" type='notice'><h4>Notice</h4><p>For tables with large amounts of data, do not set <code>SIZE</code> to a value that is too small. Otherwise, a large number of partitions will be generated, which may affect the stability of the system. </p></main> |
...
| SIZE('size_value')   | (Optional) The threshold for partition splitting. When `size_value` is set to `unlimited`, partition splitting is not triggered, and no new automatic partition splitting task is scheduled. <main id="notice" type='explain'><h4>Note</h4><p>For V4.4.2, the lower limit of the automatic partition splitting threshold has been adjusted from 128 MB to 1 MB starting from V4.4.2 BP1. </p></main> <main id="notice" type='notice'><h4>Notice</h4><p>Do not set the <code>SIZE</code> parameter to a very small value in scenarios where the table contains a large amount of data. Otherwise, a large number of partitions may be generated, which may affect the performance of subsequent partition operations and the stability of the system. </p></main>|
