...
* For tenants upgraded from versions earlier than V4.3.5 BP2, performing a `DROP` partition operation will invalidate global indexes. For tables with global indexes, use the expired partition deletion feature with caution.

    <main id="notice" type='explain'>
      <h4>Note</h4>
      <p>Starting from V4.3.5 BP2, OceanBase Database introduces a tenant-level hidden parameter <code>_ob_enable_truncate_partition_preserve_global_index</code> in V4.3.5 to control whether to allow global indexes to remain valid when performing <code>TRUNCATE</code>/<code>DROP</code> operations on partitions (referred to as partition DDL operations).<ul><li>For tenants upgraded from earlier versions, the default value of this parameter is <code>False</code>, indicating that global indexes will become invalid when <code>TRUNCATE</code>/<code>DROP</code> operations are performed on partitions.</li><li>For newly created tenants, the default value is <code>True</code>, indicating that global indexes will remain valid when <code>TRUNCATE</code>/<code>DROP</code> operations are performed on partitions.</li></ul></p>
    </main>
...
* If you modify the partitioning key column type or partition rule, and the new table schema does not meet the requirements for dynamic partition management, subsequent dynamic partition management scheduling on this table will fail.
