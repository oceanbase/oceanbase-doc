...
* For tenants upgraded from versions earlier than V4.3.5 BP2, performing a `DROP` partition operation will invalidate global indexes. For tables with global indexes, use the expired partition deletion feature with caution.
    ...
    <main id="notice" type='explain'>
      <h4>Note</h4>
      <p>Starting from V4.3.5 BP2, OceanBase Database introduces a tenant-level hidden parameter <code>_ob_enable_truncate_partition_preserve_global_index</code> to control whether to allow global indexes to remain valid during <code>TRUNCATE</code>/<code>DROP</code> operations on partitions (referred to as partition DDL operations).<ul><li>For tenants upgraded from earlier versions, the default value of this parameter is <code>False</code>, meaning that global indexes will be invalidated during <code>TRUNCATE</code>/<code>DROP</code> operations on partitions.</li><li>For newly created tenants, the default value is <code>True</code>, meaning that global indexes will remain valid during <code>TRUNCATE</code>/<code>DROP</code> operations on partitions.</li></ul></p>
    </main>
