...
OceanBase Database can split the data of a table into different groups based on some rules. Data in the same group is stored in the same physical area. This area is called a partition. A table split in this way is called a partitioned table. In the MySQL-compatible mode of OceanBase Database, the tenant-level parameter `max_partition_num` specifies the maximum number of partitions supported for a single table. The default value is `8192`.
...
In a table row, one or more columns determine which partition the row belongs to. The set of data in the one or more columns is called the partitioning key. The partitioning key must be a subset of a primary key or a unique key. If a primary key exists, the partitioning key must be a subset of the primary key. An expression contains the partitioning key and determines which partitions the rows belong to. This expression is called a partitioning expression.

## Partitioning types

The MySQL-compatible mode of OceanBase Database supports the following partitioning types:
...
You can specify an expression as a partitioning key for RANGE, LIST, and HASH partitioning. MySQL-compatible tenants of OceanBase Database support only the following functions in partitioning expressions:
