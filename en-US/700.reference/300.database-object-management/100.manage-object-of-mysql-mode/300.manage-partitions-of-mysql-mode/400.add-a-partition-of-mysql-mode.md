...
<main id="notice" type='notice'>
  <h4>Attention</h4>
  <p><ul><li>Does not support adding secondary partitions in scenarios where the secondary partition type is Hash or Key type.</li><li>For OceanBase Database V4.3.5, adding secondary partitions is supported starting from V4.3.5 BP1.</li><li>The added secondary partition cannot have the same name as existing primary or secondary partitions in the table.</li><li>When adding a Range secondary partition, the upper bound of the Range must be greater than the upper bound of the largest partition under the current primary partition; when adding multiple Range secondary partitions, the upper bounds must be arranged in ascending order, meaning the Range partition with the larger upper bound should be placed afterward.</li><li>When adding a List secondary partition, it cannot duplicate the values of existing secondary partitions under the current primary partition.</li><li>The operation of adding multiple secondary partitions is atomic, meaning either all succeed or all fail.</li></ul></p>
</main>
...
| partition_name       | Specify the name of the primary partition to which the secondary partition to be added belongs. |
| subpartition_name    | Specify the name of the secondary partition to be added. |
| subpartition_option  | Indicates the newly added secondary partition definition. |
...
* Add a secondary partition table to the Range + Range templated partition table.

  1. Create a templated secondary partition table `tbl1_m_rr` with Range + Range partitioning.
  ...
  2. Add the second-level partition `p1sp2` to the first-level partition `p1` of table `tbl1_m_rr`.
...
* Add a secondary partition table to the non-templated partition table in the Range + Range direction.

  1. Create a non-templated secondary partition table `tbl2_f_rr` with Range + Range partitioning.
  ...
  2. Add secondary partitions `sp5` and `sp6` to the first-level partition `p1` of table `tbl2_f_rr`.
