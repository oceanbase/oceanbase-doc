...
* `PARTITIONING_TYPE`, `SUBPARTITIONING_TYPE`, `PARTITION_COUNT`, `DEF_SUBPARTITION_COUNT`, `PARTITIONING_KEY_COUNT`, and `SUBPARTITIONING_KEY_COUNT`: the attributes of the table group with partition definitions in OceanBase Database V3.x. In the current OceanBase Database version, the value of these attributes is `NONE` or `NULL`.

* `SHARDING`: the `SHARDING` attribute of the table group.
