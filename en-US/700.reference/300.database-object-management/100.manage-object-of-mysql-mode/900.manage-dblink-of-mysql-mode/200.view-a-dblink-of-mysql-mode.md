...
* `REVERSE_TENANT_NAME`: the tenant name used to connect to the local database from the remote database by using the DBLink. At present, the reverse link feature is not supported in MySQL mode.
