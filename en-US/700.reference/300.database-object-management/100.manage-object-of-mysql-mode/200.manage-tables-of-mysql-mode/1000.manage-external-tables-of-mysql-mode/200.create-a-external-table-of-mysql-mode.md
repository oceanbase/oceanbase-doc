...
* `col_name col_type [AS (metadata$filecol{N} | external$filepos{N})]`: defines the columns. `AS (metadata$filecol{N})` is used to manually define column mapping; `AS (external$filepos{N})` is used to index columns by position.
  ...
  The column types supported by external tables are the same as those of regular tables. For more information about the data types supported by OceanBase Database in MySQL mode, see [Overview of data types](../../../../500.sql-reference/100.sql-syntax/200.common-tenant-of-mysql-mode/100.basic-elements-of-mysql-mode/100.data-type-of-mysql-mode/100.data-type-overview-of-mysql-mode.md).
