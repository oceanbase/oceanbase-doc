...
* `col_name col_type [AS (metadata$filecol{N})]` specifies a column, where `AS (metadata$filecol{N})` specifies the column mapping.
   ...
   The column types supported by an external table are the same as those supported by a normal table. For more information about the data types supported in the MySQL mode of OceanBase Database, see [Overview](../../../../500.sql-reference/100.sql-syntax/200.common-tenant-of-mysql-mode/100.basic-elements-of-mysql-mode/100.data-type-of-mysql-mode/100.data-type-overview-of-mysql-mode.md).
