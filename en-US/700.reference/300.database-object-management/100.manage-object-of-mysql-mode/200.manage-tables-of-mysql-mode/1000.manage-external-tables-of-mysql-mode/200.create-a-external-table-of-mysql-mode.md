...
* `LOCATION = '<string>'`: specifies the path where the external file is stored. In most cases, the data files of an external table are stored in a dedicated directory, which can contain subdirectories. When you create an external table, the database automatically collects all files in the directory.
...
*  `FORMAT = ( TYPE = 'CSV'...)`: specifies the format of CSV external files.
...
* `FORMAT = ( TYPE = 'PARQUET'...)`: specifies the format of external files as Parquet.
