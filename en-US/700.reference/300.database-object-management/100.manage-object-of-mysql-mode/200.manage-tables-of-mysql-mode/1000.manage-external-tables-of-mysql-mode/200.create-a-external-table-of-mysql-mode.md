...
* `col_name col_type [AS (metadata$filecol{N})]`: defines a column. `AS (metadata$filecol{N})` specifies a column mapping.
  ...
  The column types supported by an external table are the same as those supported by a normal table. 
...
*  `FORMAT = ( TYPE = 'CSV'...)`: specifies CSV as the format of external files.
  ...
  * `ENCODING`: the character set encoding used by the file. If this parameter is not specified, the default value `UTF8MB4` takes effect.
