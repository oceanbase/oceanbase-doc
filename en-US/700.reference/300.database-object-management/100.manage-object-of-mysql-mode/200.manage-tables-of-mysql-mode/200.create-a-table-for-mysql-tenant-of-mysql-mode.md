...
Here, the `MERGE_ENGINE` parameter specifies the update model of the table. Valid values:
...
For scenarios other than OLAP, we recommend that you use the `partial_update` update model. For more information about the scenarios, see [Configuration best practices](../../../../400.deploy/700.best-practices.md).

**Here are some examples:**

* Create a columnstore table named `mer_tbl1` and set the update model to `merge on write`.
...
* Create a hybrid row-column table named `mer_tbl2` and set the update model to `merge on write`.
