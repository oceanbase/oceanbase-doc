...
OceanBase database supports two types of table organization formats: Index Organized Table and Heap Organized Table.

<main id="notice" type='explain'>
  <h4>Instructions</h4>
  For OceanBase Database V4.3.5, heap-organized tables are supported starting from the V4.3.5 BP1 version.
</main>

### Usage Restrictions

The following DDL operations are not supported for OceanBase database heap organized tables:

* Primary key constraint operations
* Modify column to primary key
* The length of the primary key column type is changed from large to small.
* Add primary key column
* Modify the primary key to an auto-increment column
* Delete primary key column
* Regional division

### Syntax for specifying table organization form

The syntax for specifying the table organization when creating a table is to add the table option `ORGANIZATION` after the `CREATE TABLE` statement, and the SQL statement is as follows:
...
<main id="notice" type='explain'>
  <h4>Instructions</h4>
  For OceanBase Database V4.3.5, the `CREATE TABLE` statement supports the `ORGANIZATION` option starting from V4.3.5 BP1.
</main>

Among them, the `ORGANIZATION` parameter is used to specify the storage order of data rows in the table, with the following possible values:
...
If the `ORGANIZATION` option is not specified, the value of this option will be the same as the value of the configuration item [default_table_organization](../../../800.configuration-items-and-system-variables/100.system-configuration-items/400.tenant-level-configuration-items/2150.default_table_organization.md).

**The example is as follows:**

* Specify the value of the `ORGANIZATION` attribute as `HEAP` when creating the table.
...
* When creating a table, if `ORGANIZATION` is not specified.
  ...
  After modifying the tenant-level configuration item `default_table_organization`, create a table. For a detailed introduction to the configuration item `default_table_organization`, see [default_table_organization](../../../800.configuration-items-and-system-variables/100.system-configuration-items/400.tenant-level-configuration-items/2150.default_table_organization.md).

  <main id="notice" type='explain'>
    <h4>Instructions</h4>
    The current version configuration item `default_table_organization` is only applicable to OceanBase Database MySQL mode user tenants. Sys (system) tenants and Oracle mode user tenants are not supported.
  </main>

  1. Set the value of the configuration item `default_table_organization` to `HEAP`.

      ```sql
      ALTER SYSTEM SET default_table_organization = 'HEAP';
      ```

  2. Create table `ora_tbl2`.
