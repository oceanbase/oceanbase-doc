...
* Change the data type of a column
   ...
   OceanBase Database currently does not support modifying the column type of columns with foreign key dependencies or the columns they depend on.

   For more information about rules for changing the data types of columns in the MySQL mode of OceanBase Database, see [Column type change rules](../../../500.sql-reference/100.sql-syntax/200.common-tenant-of-mysql-mode/700.ddl-function-of-mysql-mode/900.column-type-change-rule-of-mysql-mode.md).

   Starting from OceanBase Database V4.2.2, only online DDL operations are supported for changing the data type of a primary key column of the INTEGER type.
   ...
   The following sample code changes the data type of the `c1` column of the `test` table to `BIGINT`.

   ```sql
   obclient> ALTER TABLE test MODIFY c1 BIGINT(60);
   ```
