...
* If a load balancing task is triggered for scaling up or down, changing the primary zone, or modifying the locality of the current tenant, the task of converting a table into a replica table must wait until the balancing task is completed.
    ...
    <main id="notice" type='notice'>
      <h4>Notice</h4>
      <p>After you convert a table into a replica table, you must wait for the background partition balancing task to complete before you use the table as the target attribute. For more information about partition balancing, see <a href="../../../100.oceanbase-database-concepts/500.distributed-database-objects/300.data-partitions-and-replicas/500.data-balancing/100.intra-tenant-balancing.md">Intra-tenant balancing</a>. </p>
    </main>

* A table cannot be converted into a replica table if it belongs to a table group. In this case, you can convert the table into a global table instead.

* The conversion of a table into a replica table depends on the load balancing and transfer features of the tenant. If any of the tenant-level parameters `enable_rebalance`, `enable_transfer`, and `enable_rereplication` is set to `false`, you cannot convert a table into a replica table. If you set the `enable_rebalance` or `enable_transfer` parameter to `false` and there are ongoing tasks of converting tables into replica tables, the tasks will be canceled.
...
* `cluster`: specifies to convert the current table into a replica table.
...
2. Convert the replica table into a normal table.
    ...
    <main id="notice" type='notice'>
      <h4>Note</h4>
      <p>The preceding SQL statement will immediately return success after modifying the table attribute. The actual conversion of the replica table is asynchronously performed in the background by a thread. </p>
    </main>

3. View the attributes of the replica table.

    You can query the `oceanbase.DBA_OB_TABLE_LOCATIONS` view for the attributes of the replica table and the log stream ID where the replica table is located. If the `DUPLICATE_SCOPE` column displays `CLUSTER`, the table is a replica table. If it displays `NONE`, the table is a normal table.
...
4. View the conversion progress.
   ...
   1. View the attribute of the log stream.
       ...
       All replica tables in a tenant must be placed on a special log stream (broadcast log stream) for replica tables. You can query the `oceanbase.DBA_OB_LS` view for the attribute of the log stream. If the `FLAG` column contains `DUPLICATE`, the log stream is a broadcast log stream. Otherwise, it is a normal log stream.
   ...
   2. Determine whether a table conversion task needs to be generated.
       ...
       * `DUPLICATE_SCOPE` is `NONE`, and `FLAG` contains `DUPLICATE`: The non-replica table is placed on a broadcast log stream. A table conversion task needs to be generated to transfer the non-replica table from the broadcast log stream to a normal log stream.
       * `DUPLICATE_SCOPE` is `NONE`, and `FLAG` does not contain `DUPLICATE`: The replica table is placed on a normal log stream. A table conversion task needs to be generated to transfer the replica table from a normal log stream to a broadcast log stream.
