...
For more information about system variables, see [Overview](../../../../700.reference/800.configuration-items-and-system-variables/000.configuration-items-and-system-variables-overview.md).
...
OceanBase Database supports two auto-increment modes for auto-increment columns: NOORDER and ORDER. The default mode is ORDER. The two modes are described as follows:
...
* `NOT NULL` constraint: The constrained column cannot contain `NULL` values.
...
`10` is greater than the current maximum value `4` of the auto-increment column. If you insert a data record with the `name` column value `G` but no `id` column value into the `t1` table, OceanBase Database automatically populates the next value of the auto-increment column, namely, the value following `10`, in the `id` column.
...
* `data_type`: the data type of the column to be modified.
  ...
  For more information about data types, see [Overview](../../../500.sql-reference/100.sql-syntax/200.common-tenant-of-mysql-mode/100.basic-elements-of-mysql-mode/100.data-type-of-mysql-mode/100.data-type-overview-of-mysql-mode.md).
