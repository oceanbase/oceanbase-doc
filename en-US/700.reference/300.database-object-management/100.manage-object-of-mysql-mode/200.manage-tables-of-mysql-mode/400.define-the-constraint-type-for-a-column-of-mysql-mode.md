...
MySQL tenants of OceanBase Database support `NOT NULL`, `UNIQUE`, `PRIMARY KEY`, `FOREIGN KEY`, and `CHECK` constraints.

### NOT NULL constraint

A `NOT NULL` constraint requires that the constrained column not contain a `NULL` value.
...
The following sample code creates a table named `tbl1` and defines a `NOT NULL` constraint on the `col1` column.
...
A `UNIQUE` constraint requires that the constrained column not contain duplicate values. However, it can contain multiple `NULL` values.

The following sample code creates a table named `tbl6` and defines a `UNIQUE` constraint on the `col1` column.
...
The following sample code creates a table named `tbl2` and defines a `PRIMARY KEY` constraint on the `col1` column.
...
A `FOREIGN KEY` constraint maintains the integrity of correlations between tables and points to the primary key columns of the parent table.

* To create a table and define a `FOREIGN KEY` constraint on the table, make sure that you have the `CREATE` privilege, as well as the `REFERENCES` privilege on the parent table.
* To define a `FOREIGN KEY` constraint on an existing table, make sure that you have the `ALTER` privilege, as well as the `REFERENCES` privilege on the parent table.
...
* `RESTRICT`: If the referenced column of the parent table is dropped or updated, the `DELETE` or `UPDATE` operation on the child table is rejected.
...
* `NO ACTION`: This keyword is equivalent to `RESTRICT`. If the referenced column of the parent table is dropped or updated, the `DELETE` or `UPDATE` operation on the child table is rejected.
...
The following sample code creates a table named `tbl8` and defines a constraint where the value of the `col1` column must be greater than `0`.
...
You can view the constraint that you created by querying the `information_schema.TABLE_CONSTRAINTS` view. Here is an example:
...
The following sample code creates a table named `tbl7` and adds a `UNIQUE` constraint on the `col2` column in the `tbl7` table.
...
The following sample code creates a table named `tbl9` and change the attribute of a column in the table to `NOT NULL`.
...
* Add a `CHECK` constraint on the `col2` column of the `tbl8` table.
...
* Modify the `CHECK` constraint for the table.
  ...
  By default, a `CHECK` constraint is enabled after it is created. If you want to invalidate the constraint, you can disable it.
...
We recommend that you specify a default value for a column with the `NOT NULL` constraint. If the column is of a date or time data type, you can set the default value to the current time of the database.
