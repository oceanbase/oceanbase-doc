...
MySQL-compatible tenants of OceanBase Database support the `NOT NULL`, `UNIQUE`, `PRIMARY KEY`, `FOREIGN KEY`, and `CHECK` constraints.
...
* `RESTRICT`: If the referenced column of the parent table is dropped or updated, the DROP or UPDATE operation on the parent table is rejected.
...
* `NO ACTION`: This keyword is equivalent to `RESTRICT`. If the referenced column of the parent table is dropped or updated, the DROP or UPDATE operation on the parent table is rejected.
