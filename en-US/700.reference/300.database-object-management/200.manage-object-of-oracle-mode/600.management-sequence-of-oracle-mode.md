...
This topic describes how to create, query, modify, and drop sequences in Oracle-compatible mode.

In OceanBase Database, a sequence is a database object of Oracle-compatible tenants. You can use it to generate unique values. It is useful when a table needs a unique column as the primary key.

A sequence provides two pseudo-columns `CURRVAL` and `NEXTVAL`, which return the current sequence value and the next sequence value respectively. Each reference to `NEXTVAL` advances the value of `CURRVAL`.

## Limitations of sequences

* A sequence cannot be used together with `HAVING`, `ORDER BY`, or `GROUP BY` clauses.
* A sequence cannot appear in subqueries (except in `INSERT INTO SELECT`).
* A sequence cannot appear in `WHERE` expressions.
...
After a sequence is created, you can use the `USER_SEQUENCES`, `ALL_SEQUENCES`, and `DBA_SEQUENCES` views to query the created sequences.
...
To prevent cache expiration issues with other sequences, OceanBase Database introduces the `ALTER SYSTEM FLUSH SEQUENCE CACHE` statement starting from V4.4.1. You can use this statement to proactively clear the sequence cache for quick recovery when issues occur. The syntax is as follows:
...
For more information about the statement for clearing the sequence cache, see [ALTER SYSTEM FLUSH SEQUENCE CACHE](../../500.sql-reference/100.sql-syntax/300.common-tenant-of-oracle-mode/900.sql-statement-of-oracle-mode/100.ddl-of-oracle-mode/850.alter-system-flush-sequence-cache-of-oracle-mode.md).

**Example:**

Clear the cache of the sequence named `test_seq1` on the current node within the current tenant and database.
