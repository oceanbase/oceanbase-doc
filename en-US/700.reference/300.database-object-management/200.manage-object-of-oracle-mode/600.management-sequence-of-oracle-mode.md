...
A sequence provides two pseudo-columns: `CURRVAL` and `NEXTVAL`, which return the current sequence value and the next sequence value respectively. Each reference to `NEXTVAL` updates the value of the `CURRVAL` column.
...
* `START WITH`: the start value of the sequence.

* `INCREMENT BY`: the increment step. It can be negative. The default value is `1`.

* `CACHE`: the number of sequence values to be cached for faster access. We recommend that you specify this parameter in high concurrency scenarios.

  <main id="notice" type='explain'>
  <h4>Note</h4>
  <p>When you use sequences in a distributed system, if you use the <code>CACHE</code> and <code>NOORDER</code> parameters at the same time, the sequence numbers that you obtain from connecting to different nodes are unique, but do not increase orderly. </p>
  </main>
...
You can query the `USER_SEQUENCES`, `ALL_SEQUENCES`, or `DBA_SEQUENCES` view for sequences that you created.
...
1. Modify the maximum value of the `seq2` sequence and enable cyclic generation of sequence values.

   ```sql
   obclient> ALTER SEQUENCE seq2 MAXVALUE 1024 CYCLE;
   ```

2. Modify the start value of the sequence.

   * If the value of `INCREMENT BY` is greater than `0`, you can reset the sequence to make the sequence start from `MINVALUE`. If the value of `INCREMENT BY` is less than `0`, you can execute the following statement to reset the sequence and make the sequence start from `MAXVALUE`:
