...
In Oracle-compatible mode of OceanBase Database, you can create a DBLink between two Oracle-compatible tenants of OceanBase Database or between an Oracle-compatible tenant of OceanBase Database and an Oracle database.
...
* We recommend that you set the `local_ip:local_port` parameter following the `MY_NAME` parameter to the IP address and port number of an OceanBase Database Proxy (ODP) node in the local cluster. This way, the ODP node automatically routes the reverse link to the correct node to obtain local data.
  ...
  Assume that you set the `local_ip:local_port` parameter following the `MY_NAME` parameter to the IP address and port number of OBServer node A in the local cluster when you create a DBLink. In Oracle-compatible mode of OceanBase Database, when you execute the `INSERT`, `UPDATE`, `DELETE`, or `MERGE INTO` statement to write data of a local table to a remote table, the following cases occur:
...
To access a remote OceanBase database, you can create a DBLink from an Oracle-compatible tenant in your local OceanBase database to an Oracle-compatible tenant in the remote OceanBase database.
...
The SQL syntax for creating a DBLink from one Oracle-compatible tenant of OceanBase Database to another is as follows:
...
To access a remote Oracle database, you can create a DBLink from an Oracle-compatible tenant in your local OceanBase database to the remote Oracle database.
