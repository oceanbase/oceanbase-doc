...
* In SQL, when writing data to a remote database through DBLink, it is necessary to explicitly use the `COMMIT` statement to submit both local and remote XA transactions. Auto-committing transactions by setting `autocommit = True` is not currently supported. When writing data via DBLink in SQL, if an XA transaction is initiated, the system will forcibly set the `autocommit` value for the current session to `False`, and it will not revert to its original value until the XA transaction is committed or rolled back.

* In anonymous blocks, when writing data to a remote database through DBLink, the transaction commit method is similar to that of ordinary transactions. Auto-committing transactions by setting `autocommit = True` is supported. When `autocommit = False` is set, an explicit `COMMIT` statement is required to commit the transaction.
