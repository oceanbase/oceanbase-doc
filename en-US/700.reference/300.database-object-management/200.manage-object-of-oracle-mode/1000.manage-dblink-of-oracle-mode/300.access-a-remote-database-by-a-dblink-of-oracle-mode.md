...
You can use a DBLink to access objects, such as tables, views, synonyms, sequences, stored procedures, and user-defined functions (UDFs), in a remote database.
...
* `table_name@dblink_name`: the tables to be queried in the remote database. Separate the table names with commas (,).
...
Example: Use a DBLink named `my_link` to query data from the `tbl1` table in the remote database:
...
In OceanBase Database V4.2.1 and later, you can access sequence values in a remote OceanBase database or Oracle database by using a DBLink.
...
OceanBase Database allows you to call a stored procedure in a remote Oracle database or in a remote Oracle tenant of OceanBase Database by using a DBLink.
In the current version of OceanBase Database in Oracle mode, you can use a DBLink to transmit data of complex data types, including basic data types such as integers and strings, as well as nested complex structures. Supported nesting types are as follows:

* record(basic): a record type that contains a field of a basic data type.
* array(basic): an array that consists of elements of a basic data type.
* array(record(basic)): an array that consists of elements of a record type and the record fields are of a basic data type.

When you use a DBLink to call a stored procedure with complex parameter types in a remote Oracle tenant of OceanBase Database, take note of the following considerations:

* Only the `record(basic)`, `array(basic)`, and `array(record(basic))` complex types are supported and must be defined in packages that are directly called.
* At present, the array(basic) and array(record(basic)) complex types do not support the `BINARY_DOUBLE` type.
* At present, the `FLOAT`, `TIMESTAMP`, `TIMESTAMP WITH TIME ZONE`, and `TIMESTAMP WITH LOCAL TIME ZONE` types are not supported as output parameters of an array.
* Parameters of the `CHAR` type are supported when you call a stored procedure in a remote native OceanBase database.

When you use a DBLink to call a stored procedure in a remote Oracle database, take note of the following considerations:

* Parameters of the `CHAR` type are not supported when you call a stored procedure in a remote native Oracle database.
...
* In the call of a package of stored procedures, inout parameters of the following composite data types are supported:
  ...
  * [Records](../../../500.sql-reference/300.pl-reference/300.pl-oracle/400.collections-and-records-oracle/800.record-oracle.md)
  ...
  * [Nested tables](../../../500.sql-reference/300.pl-reference/300.pl-oracle/400.collections-and-records-oracle/500.nested-table-oracle.md) and [Associative arrays](../../../500.sql-reference/300.pl-reference/300.pl-oracle/400.collections-and-records-oracle/300.associative-array-oracle.md)
...
* You can use a DBLink to call the synonyms of stored procedures or packages in a remote Oracle database, and call the synonyms created in a local database for remote stored procedures or packages.

* You cannot use a DBLink to call remote packages of the constructor type.

Here are some examples: Assume that you have created, in a local OceanBase database, a DBLink named `orcl_link` to a remote Oracle database.
...
* Call a simple stored procedure.
  ...
  1. Prepare the data environment.
     ...
     Execute the following statement to insert data into the table:
  ...
  * Create a synonym named `syn_local_customer_id`for the stored procedure `get_customer_id` in the local OceanBase database by using the DBLink
...
* Call a stored procedure that contains inout parameters
  ...
  Create a stored procedure named `get_employee_info` to receive an input parameter `p_employee_id` and returns the name and ID of the employee. If the employee is not found, the stored procedure returns `Employee not found`. If other exceptions occur, the stored procedure returns `Error occurred`.
...
* Call a package of stored procedures with inout parameters of composite data types
  ...
  In the remote Oracle database, create a table named `tbl1_v` and a package named `pkg1`. The package type is nested table.
...
In OceanBase Database V4.2.2 and later, you can call a user-defined function (UDF) or a package function in a remote Oracle database by using a DBLink. At present, you cannot call UDFs or package functions in a remote OceanBase database.
...
* You can call synonyms of UDFs or packages in a remote Oracle database, and call synonyms created in a local database for remote UDFs or packages.
...
* Call a simple UDF.
  ...
  If you create a synonym for a UDF or package in the remote Oracle database, you can call the synonym in the local OceanBase database. Here is an example:
  ...
  * Call the synonym created in the local OceanBase database
  ...
  For more information about UDFs, see [Overview](../../../500.sql-reference/300.pl-reference/300.pl-oracle/700.stored-procedure-and-functions-oracle/700.user-defined-aggregation-functions-oracle/100.user-defined-func-overview-oracle.md).
...
* Call a package function.
  ...
  2. Call this package function from the local OceanBase database.
