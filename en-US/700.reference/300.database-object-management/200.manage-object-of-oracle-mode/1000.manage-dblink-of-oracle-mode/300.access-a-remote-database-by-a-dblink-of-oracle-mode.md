...
1. In the local database, create a table named `tbl1` and a DBLink named `my_link` that connects to an Oracle-compatible tenant of a remote OceanBase database.
...
OceanBase Database allows you to call a stored procedure in a remote Oracle database or OceanBase's Oracle-compatible tenant by using a DBLink.
In the current version, DBLink in Oracle-compatible mode of OceanBase Database supports the transmission of complex data types. These data types are not only basic types (such as integers and strings), but also nested complex structures. The supported nested data types are as follows:
...
When you call a stored procedure in a remote OceanBase's Oracle-compatible tenant that contains parameters of complex data types by using a DBLink, note the following:
