...
We recommend that you use the partition exchange feature as an alternative to direct load. For example, you can use the partition exchange feature to import incremental data from a partitioned table named A to another partitioned table. The steps are as follows:

1. Create an empty partition P in table A.
2. Create a non-partitioned table B with the same schema as table A, and import incremental data to table B through direct load.
...
This feature supports only RANGE partitioning.
  ...
  * If you want to exchange the data of a partition, the partitioning type must be RANGE partitioning.
  * If you want to exchange the data of a subpartition of a subpartitioned table, the subpartitioning type must be RANGE partitioning, but the partitioning type is not limited.
...
* Constraints on the partitioned table must be the same as those on the non-partitioned table. OceanBase Database V4.3.3 does not support `FOREIGN KEY` constraints on either table.
* The local index table of the partitioned table must be consistent with the index table of the non-partitioned table.
* Only the `INCLUDING INDEXES` option is supported (you must explicitly specify `INCLUDING INDEXES` in Oracle mode). The data of the local indexes of the partitions to be exchanged will be exchanged. After the exchange, the local indexes remain available.
* This feature does not support the `UPDATE GLOBAL INDEXES` clause. Therefore, global indexes remain in the unusable state after the partition exchange is successful.
...
* [SELECT statements](../../../500.sql-reference/100.sql-syntax/200.common-tenant-of-mysql-mode/600.sql-statement-of-mysql-mode/8100.select-of-mysql-mode/100.select-of-mysql-mode.md)
