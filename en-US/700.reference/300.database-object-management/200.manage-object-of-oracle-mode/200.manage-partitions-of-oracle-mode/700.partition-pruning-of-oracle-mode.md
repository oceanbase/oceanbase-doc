...
## Principles of partition pruning for partitions
...
For example, when creating a partitioned table with the partitioning key as `c1` and the query condition as an equality condition, the following actions can be observed:
...
Partition pruning determines the partitions that need to be accessed by intersecting the partitioning key in the `WHERE` clause with the partition range defined in the table. Partition pruning can be performed regardless of whether the query condition is an equality condition or a non-equality condition.

For example, when the partitioning key of the following partitioned table is `c1`, and the query condition is a non-equality condition `c1 < 150 and c1 > 110`, partition pruning can be performed:
...
## Principles of partition pruning for subpartitions

For subpartitions, the partitions to access are first determined based on the partitioning key, and then the subpartitions to access are determined based on the subpartitioning key. The final result is the product of the two to determine all physical partitions to access.
...
In some cases, partition pruning may have a certain degree of amplification, but the optimizer can ensure that the pruning result is a superset of the data to be accessed and no data is lost.
