...
The Oracle mode of OceanBase Database supports the following primary partitioned tables:
...
* Only the top-level partition can be an interval partition. That is, you can specify an interval partition for a top-level partition table or a subpartition table, but not for a subpartition of a subpartition table.
* At least one range partition must be specified when you create a table.
* You cannot specify the `DYNAMIC_PARTITION_POLICY` attribute when you specify the `INTERVAL` attribute for a table.
* The partitioning key must be a single column of the `NUMBER`, `DATE`, `FLOAT`, or `TIMESTAMP` type.
...
* Create a top-level interval-partitioned table with the partitioning key column of the `INT` type and a step of 10.
...
* Create a top-level interval-partitioned table with the partitioning key column of the `DATE` type and a step of one month.
...
* Create a subpartitioned interval-partitioned table, where the top-level partition is an interval partition with a step of 10, and the subpartition is a range partition.
