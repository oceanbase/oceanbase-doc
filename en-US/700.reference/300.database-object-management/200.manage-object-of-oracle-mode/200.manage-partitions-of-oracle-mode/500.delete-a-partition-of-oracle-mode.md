...
You can delete a partition in a partitioned table as needed. When you delete a partition, the definition and data in the partition are also deleted.

## Support for deleting a partition in a partitioned table

In Oracle mode of OceanBase Database:

* For a partitioned table, you can delete a RANGE partition or a LIST partition, but you cannot delete a HASH partition.

* For a subpartitioned table, you can delete a RANGE or LIST subpartition.

The following table describes the support for deleting a partition in a partitioned table in Oracle mode of OceanBase Database.

|   Partitioned table  |        Partition type                           | Delete a partition | Delete a subpartition |
|-----------|------------------------------------------|-------------|------------|
...
Before you delete a partition, note the following:

* When you delete a partition, make sure that no active transaction or query is running on the partition. Otherwise, an error may be returned when you execute an SQL statement, or an unexpected situation may occur. You can query the `oceanbase.GV$OB_TRANSACTION_PARTICIPANTS` view in the `sys` tenant to obtain the status of transactions that are not yet completed.

* In Oracle mode, when you delete a partition in a partitioned table that has global indexes, you must specify the `UPDATE GLOBAL INDEXES` clause in the `ALTER TABLE` statement. If you do not specify this clause, the global indexes on the partitioned table will be unavailable after the partition is deleted.
  ...
  * For a table that is partitioned by RANGE or LIST (including partitioned and subpartitioned tables) and supports partition deletion, when you specify the `UPDATE GLOBAL INDEXES` clause, the system does not rebuild the global indexes. Instead, it uses a lazy maintenance strategy to keep the indexes valid by deleting data in the background. This avoids query interruptions or performance degradation caused by index invalidation. Note that this optimization applies only to regular global indexes. The following scenarios cannot trigger this optimization:
    ...
    * For special global indexes such as full-text indexes and non-structured types, even if you specify the `UPDATE GLOBAL INDEXES` clause, the indexes will be unavailable.

## Delete a partition in a partitioned table

You can delete a partition in a partitioned table as needed.
...
```
  ...
  <main id="notice" type='explain'>
    ...
    <ul>
    <li>
    <p>You can delete one or more partitions, but not all partitions, in a partitioned table. </p>
    </li>
    <li>
    <p>Deleting a partition also deletes the data in the partition. If you want to delete only the data, you can use the <code>TRUNCATE</code> statement. </p>
    </li>
    </ul>
  </main>
...
* Delete the `M202005` and `M202006` partitions in the `tbl1_r` partitioned table.
...
* Delete the `p0` partition in the `t2_f_rr` partitioned table.
...
## Delete a subpartition in a subpartitioned table

You can delete a subpartition in a subpartitioned table as needed. When you delete a subpartition, the definition and data in the subpartition are also deleted.
...
Delete the `sp5` and `sp6` subpartitions in the `p1` partition of the `t2_f_rr` subpartitioned table and update the global indexes.
