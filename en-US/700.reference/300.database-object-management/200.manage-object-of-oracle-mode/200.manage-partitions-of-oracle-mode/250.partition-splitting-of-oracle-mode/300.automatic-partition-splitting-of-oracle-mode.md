...
* Automatic subpartition splitting is not supported for tables that are list-partitioned or hash-partitioned.
* Automatic subpartition splitting is not supported for tables that are subpartitioned.
* The partitioning key of a table that is automatically subpartitioned cannot be different from the primary key prefix.
* Automatic subpartition splitting is not supported for tables without a primary key.
* Automatic subpartition splitting is not supported for columnstore tables.
* Automatic subpartition splitting is not supported for tables in a table group that contains more than one table.
* Automatic subpartition splitting is not supported for materialized views.
* Automatic subpartition splitting is not supported for full-text indexes.
* Automatic subpartition splitting is not supported for GIS indexes.
* Automatic subpartition splitting is not supported for vector indexes.
* In the current version, you can enable automatic subpartition splitting for both the primary table and global indexes. However, the following limitations apply to global indexes:

  1. You cannot enable automatic subpartition splitting for global indexes alone. The global indexes will be automatically subpartitioned if the primary table is automatically subpartitioned and the global indexes meet the automatic subpartitioning rules (no partitioning key or the partitioning key is the primary key prefix).
  2. For a non-partitioned global index, when automatic subpartition splitting is triggered, the index key is used as the partitioning key instead of the primary key of the index table.
...
### Example 1: Create an auto-partitioning table
...
2. Create an auto-partitioning table named `test1_tbl1` without specifying a pre-partitioning key and a split threshold of 10 GB.
...
3. Create an auto-partitioning table named `test1_tbl2`, specify the pre-partitioning key as `col1`, and set the split threshold to 10 GB.
...
4. Create an auto-partitioning table named `test1_tbl3`, without specifying a pre-partitioning key and a split threshold (the split threshold will be set to the value of `auto_split_tablet_size` by default).
...
5. Create an auto-partitioning table named `test1_tbl4`, specify the pre-partitioning key as `col1, col2`, set the split threshold to 10 GB, and create partitioning points `P0`, `P1`, `P2`, and `P3`.
...
1. Create an auto-partitioning table named `test2_tbl1`, specifying the pre-partitioning key as `col1` and the split threshold as 10 GB.
...
3. Create an auto-partitioning table named `test2_tbl2`, specifying the pre-partitioning key as `col1` and the split threshold as 10 GB.
...
5. Create an auto-partitioning table named `test2_tbl3`, specifying the pre-partitioning key as `col1` and the split threshold as 10 GB.
...
7. Create an auto-partitioning table named `test2_tbl4`, specifying the pre-partitioning key as `col1` and the split threshold as 10 GB.
...
9. Create a non-auto-partitioning table named `test2_tbl5`.

    ```sql
    CREATE TABLE test2_tbl5 (col1 NUMBER, col2 NUMBER, col3 VARCHAR2(30), PRIMARY KEY(col1, col2));
    ```

10. Change the `test2_tbl5` table to an auto-partitioning table without specifying the `SIZE` clause. The split threshold will be set to the value of `auto_split_tablet_size` by default.
...
5. Create a non-auto-partitioning table named `test3_tbl4`.
