...
OceanBase Database supports automatic subpartition splitting for both rowstore and columnstore tables. The current version supports automatic subpartition splitting only for a table with a primary key that is range-partitioned at the subpartition level.

<main id="notice" type='explain'>
  <h4>Note</h4>
  <p>Starting from V4.6.0, OceanBase Database supports automatic subpartition splitting for columnstore tables.</p>
</main>
...
* Automatic partition splitting is not supported for tables containing columnstore local indexes.
...
## Table-level automatic subpartition splitting
...
| SIZE('size_value')   | Optional. The threshold for triggering subpartition splitting. The value of `size_value` ranges from 1 MB to +∞. If the `SIZE()` clause is not specified, the value of the `auto_split_tablet_size` parameter at the tenant level is used. `size_value` can be `unlimited`, indicating that there is no limit on the subpartition splitting threshold. In this case, no subpartition splitting will be triggered, and no new automatic subpartition splitting task will be scheduled. <main id="notice" type='notice'><h4>Notice</h4><p>In scenarios where the amount of table data is large, do not set the <code>SIZE</code> parameter to a small value. Otherwise, a large number of partitions will be generated, which will affect the performance of subsequent partitioning operations and the system stability. </p></main>|
...
| table_column_group_option | Optional. The storage format option for the table. The following options describes the formats. <ul><li>`WITH COLUMN GROUP(all columns)` : specifies to create a rowstore table. </li><li>`WITH COLUMN GROUP(each column)` : specifies to create a columnstore table. </li><li>`WITH COLUMN GROUP(all columns, each column)` : specifies to create a redundant table that stores data in both rowstore and columnstore formats. </li></ul>|

### Syntax for modifying a table

<main id="notice" type='notice'>
  <h4>Notice</h4>
  <p>Starting from V4.6.0, OceanBase Database supports automatic subpartition splitting for columnstore tables. If you create a columnstore table in a version earlier than V4.6.0 and upgrade it to V4.6.0 or later, you can use the <code>ALTER TABLE</code> statement to enable automatic subpartition splitting for the table. </p>
</main>
...
| SIZE('size_value')   | Optional. The threshold for triggering subpartition splitting. The value range is [1MB, +∞). If `size_value` is set to `unlimited`, no subpartition splitting will be triggered for existing partitions, and no new automatic subpartition splitting task will be scheduled. <main id="notice" type='notice'><h4>Notice</h4><p>In scenarios where the table contains a large amount of data, do not set <code>SIZE</code> to a value that is too small, as this may result in a large number of partitions, which may affect the stability of subsequent partitioning operations and the system. </p></main>|
...
## Tenant-level automatic subpartition splitting
...
#### Create an auto-partitioned rowstore table
...
#### Create an auto-partitioned columnstore table

* Create an auto-partitioned columnstore table `test1_cg1` without specifying a pre-partitioning key and with a split threshold of 10 GB.

    ```shell
    obclient> CREATE TABLE test1_cg1 (col1 NUMBER, col2 NUMBER, col3 VARCHAR2(30), PRIMARY KEY(col1))
        PARTITION BY RANGE() SIZE('10GB')
        WITH COLUMN GROUP (each column);
    ```

* Create an auto-partitioned columnstore table `test1_cg2` without specifying a pre-partitioning key and with a split threshold of 10 GB.

    ```shell
    obclient> CREATE TABLE test1_cg2 (col1 NUMBER, col2 NUMBER, col3 VARCHAR2(30), PRIMARY KEY(col1))
        PARTITION BY RANGE() SIZE('10GB')
        WITH COLUMN GROUP (all columns, each column);
    ```
