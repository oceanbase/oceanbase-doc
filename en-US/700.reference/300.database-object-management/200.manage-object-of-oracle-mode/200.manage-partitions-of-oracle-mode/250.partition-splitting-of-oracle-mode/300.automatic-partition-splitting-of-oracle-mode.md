...
| SIZE('size_value')   | Optional. The threshold for triggering partition splitting. If the `SIZE()` clause is not specified, the value of the tenant-level `auto_split_tablet_size` parameter is used. `size_value` can be `unlimited`, indicating that there is no limit on the splitting threshold and no new automatic partition splitting will be scheduled.|
...
| SIZE('size_value')   | Optional. The threshold for triggering partition splitting. If `size_value` is set to `unlimited`, no partition splitting will be triggered for existing partitions, and no new automatic partition splitting task will be scheduled.|
