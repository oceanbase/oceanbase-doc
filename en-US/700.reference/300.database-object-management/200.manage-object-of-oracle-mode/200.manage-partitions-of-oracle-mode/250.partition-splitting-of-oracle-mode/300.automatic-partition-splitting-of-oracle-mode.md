...
You can set automatic partition splitting rules in OceanBase Database. When the data volume reaches a certain threshold, the database automatically splits partitions. This way, the database can automatically scale out as the data volume grows, avoiding issues such as load balancing and performance degradation caused by large data volumes in a single partition.

## Support for automatic partition splitting

The current version supports automatic partition splitting only for Range partitioned tables with a primary key.

## Limitations and considerations

* Automatic partition splitting is not supported for List or Hash partitioned tables.
* Automatic partition splitting is not supported for subpartitioned tables.
* Automatic partition splitting is not supported if the partitioning key of the table being automatically split does not match the prefix of the primary key.
...
* OceanBase Database supports the [global_index_auto_split_policy](../../../../800.configuration-items-and-system-variables/100.system-configuration-items/400.tenant-level-configuration-items/3350.global_index_auto_split_policy.md) parameter to control whether the automatic partitioning feature is enabled for newly created global indexes. If automatic partitioning is enabled for a newly created global index, the partitioning rules will be regenerated based on the data volume, that is, pre-splitting will occur.

## Table-level automatic partition configuration

You can create a single automatic partition table by specifying the `SIZE` clause when you create a table by using the `CREATE TABLE` statement.

### Syntax for creating a table with automatic partitioning
...
| PARTITION BY RANGE([column_name_list]) | The prepartitioning key. When automatic partition splitting is triggered, the specified prepartitioning key will be used as the actual partitioning key (if `column_name_list` is not specified, the primary key will be used as the partitioning key).|
| SIZE('size_value')   | Optional. The threshold for triggering partition splitting. The value of `size_value` ranges from 1 MB to +∞. If the `SIZE()` clause is not specified, the value of the tenant-level `auto_split_tablet_size` parameter is used. `size_value` can be `unlimited`, indicating that there is no limit on the splitting threshold and no new automatic partition splitting will be scheduled. <main id="notice" type='explain'><h4>Note</h4><p>For V4.4.2, the lower limit of the automatic partition splitting threshold was adjusted from 128 MB to 1 MB starting from V4.4.2 BP1. </p></main> <main id="notice" type='notice'><h4>Notice</h4><p>For large tables, do not set the <code>SIZE</code> parameter to a value that is too small. Otherwise, a large number of partitions will be generated, which may affect the performance of subsequent partition operations and system stability. </p></main>|
...
### Syntax for modifying automatic partition attributes
...
<main id="notice" type='explain'>
  <h4>Note</h4>
  <p>If you do not specify partitioning rules in the <code>ALTER TABLE ... PARTITION BY ...</code> statement, the automatic partition attributes of the table will be modified in an online DDL manner, and the existing partition definition will not be affected. </p>
</main>
...
| **Parameter** | **Description** |
|----------------------|----------|
| column_name_list     | The prepartitioning key of a non-partitioned table. When you execute the `ALTER TABLE` statement to modify the automatic partition attributes of a non-partitioned table, if the `RANGE()` clause contains the prepartitioning key, the corresponding record in the schema will be modified; otherwise, the original setting will be retained.|
| SIZE('size_value')   | Optional. The threshold for triggering partition splitting. If `size_value` is set to `unlimited`, no partition splitting will be triggered for existing partitions, and no new automatic partition splitting task will be scheduled. <main id="notice" type='explain'><h4>Note</h4><p>For V4.4.2, the lower limit of the automatic partition splitting threshold has been adjusted from 128 MB to 1 MB starting from V4.4.2 BP1. </p></main> <main id="notice" type='notice'><h4>Notice</h4><p>Do not set the <code>SIZE</code> parameter to a value that is too small for tables with a large amount of data. This may result in a large number of partitions, which can negatively impact subsequent partition operations and system stability. </p></main>|
| range_partition_list | Optional. The partitioning rules. If you do not specify the `SIZE` clause but specify partitioning rules, only the partitioning rules will be modified, and the existing automatic partition definition will not be affected. <main id="notice" type='notice'><h4>Notice</h4><p>At present, OceanBase Database in Oracle-compatible mode does not support modifying the partition definition of a partitioned table. </p></main>|

## Tenant-level automatic partition configuration

You can set the `enable_auto_split` and `auto_split_tablet_size` parameters at the tenant level to control automatic partition splitting for tables that meet the conditions in the tenant. If you do not specify the `SIZE` clause when you create a table by using the `CREATE TABLE` statement, whether to create an automatic partition table is determined based on the following tenant-level parameters (the primary key column is used as the partitioning key of the automatic partition table by default).

* [enable_auto_split](../../../../800.configuration-items-and-system-variables/100.system-configuration-items/400.tenant-level-configuration-items/2350.enable_auto_split.md): a tenant-level parameter that specifies whether to enable automatic partition splitting for a tenant.
* [auto_split_tablet_size](../../../../800.configuration-items-and-system-variables/100.system-configuration-items/400.tenant-level-configuration-items/450.auto_split_tablet_size.md): a tenant-level parameter that specifies the threshold for triggering automatic partition splitting.

If you do not specify the `SIZE` clause and set `enable_auto_split` to `true`, automatic partition splitting will be enabled only for tables that meet the partition splitting conditions, and will not be enabled for tables that do not meet the conditions. For example, if you create a subpartitioned table, a non-Range partitioned table, a table without a primary key, or a table whose partitioning key is different from the primary key prefix, the table will be created as a non-automatic partition table (if you specify special syntax for automatic partition splitting, an error will be returned).

<main id="notice" type='explain'>
  <h4>Note</h4>
  <p>You can query the automatic partition attributes of a table by using the <code>sys.ALL_TABLES/sys.DBA_TABLES/sys.USER_TABLES</code> view or the <code>oceanbase.CDB_TABLES</code> view in the system tenant. </p>
</main>

## Automatic partition behavior

### Automatic partition DDL operations

* Modify partitioning rules: Only modifying to Range partitioning by primary key is supported. Other partitioning rules cannot be modified.
* Modify the primary key: The automatic partitioning key must be the primary key prefix after the primary key is modified.
* Column operations: The restrictions on the prepartitioning key are the same as those on the partitioning key.

<main id="notice" type='explain'>
  <h4>Note</h4>
  <p>The restrictions on other DDL operations are the same as those for non-automatic partition tables. </p>
</main>

### DML behavior

During partition splitting, two schemas coexist. DML operations on the source and target partitions are equivalent to operations on the same data. For example, write and query operations on the source partition will be converted to corresponding query and write operations on the target partition, and write and query operations on the target partition will be aware of write operations forwarded from the source partition. The following table describes the DML behavior in different scenarios:

| N/A | DML operation on the source partition (INSERT INTO t1 PARTITION(p0) ...) | DML operation involving the source partition (SELECT * FROM t1) | DML operation on the target partition | DML operation involving the target partition |
|-----|---------------------------------------------------|------------------------------------|-------------------|-------------------|
| Before the splitting transaction starts | Normal | Normal | Error. The behavior is the same as that for operations on non-existent partitions. | Not applicable |
| During splitting | Error may occur. The behavior is the same as that for operations on non-existent partitions when an error occurs. | No error. The system performs write stream switching and query statement retry internally. | Normal | Normal |
| After the splitting transaction commits | Error. The behavior is the same as that for operations on non-existent partitions. | Not applicable | Normal | Normal |
...
2. Create a non-partitioned table named `test3_tbl1`. Since all non-partitioned tables in the tenant are automatically SIZE-partitioned, the following syntax will automatically partition the table and read the tenant-level parameter (the default split threshold is 128 MB).
