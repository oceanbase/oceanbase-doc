...
The current version supports automatic subpartition splitting only for a table with a primary key that is range-partitioned at the subpartition level. OceanBase Database supports automatic subpartition splitting for both rowstore and columnstore tables. Starting from OceanBase Database V4.6.0, columnstore tables support automatic subpartition splitting.

<main id="notice" type='explain'>
  <h4>Note</h4>
  <p>Starting from OceanBase Database V4.6.0, columnstore tables support automatic subpartition splitting.</p>
</main>
...
| table_column_group_option | Optional. The storage format option for the table. The following table describes the options. <ul><li>`WITH COLUMN GROUP(all columns)` : specifies to create a rowstore table. </li><li>`WITH COLUMN GROUP(each column)` : specifies to create a columnstore table. </li><li>`WITH COLUMN GROUP(all columns, each column)` : specifies to create a redundant table that stores data in both rowstore and columnstore formats. </li></ul>|

### Syntax for modifying automatic subpartitioning attributes

<main id="notice" type='notice'>
  <h4>Notice</h4>
  <p>Starting from OceanBase Database V4.6.0, columnstore tables support automatic subpartition splitting. If you create a columnstore table in a version earlier than V4.6.0 and upgrade it to V4.6.0 or later, you can use the <code>ALTER TABLE</code> statement to enable automatic subpartition splitting for the table. </p>
</main>
