...
* In the Oracle mode of OceanBase Database, when you truncate a partition from a partitioned or subpartitioned table with global indexes, you must add the `UPDATE GLOBAL INDEXES` keyword to the `ALTER TABLE` statement to update global indexes. If the `UPDATE GLOBAL INDEXES` keyword is not added, the global indexes of this partitioned table become unavailable after the partition is truncated.
  ...
  * For tables with Range or List partitioning (including partitions and subpartitions) that support the `TRUNCATE` partition operation, when you specify `UPDATE GLOBAL INDEXES`, the system does not rebuild global indexes. Instead, it removes data in the background during major compactions to maintain index validity using a lazy (delayed) maintenance strategy. This avoids query interruptions or performance degradation due to index invalidation. Note that this optimization applies only to regular global indexes. The following scenarios do not trigger this optimization:

    * When the partition is a Hash partition (even if the subpartition is a Range or List partition), and you truncate the subpartition.
    * When the partitioning key is an expression rather than a regular column.
    * For special global indexes such as full-text indexes and non-structured types, even if you use `UPDATE GLOBAL INDEXES`, the indexes remain unavailable.
