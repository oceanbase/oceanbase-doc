...
<main id="notice" type='explain'>
  <h4>Note</h4>
  <p>For OceanBase Database V4.4.2, the <code>INTERVAL</code> partitioning syntax is supported starting from V4.4.2 BP1. </p>
</main>
...
|----------|-----------------------|-------------------------|
| True | Only the `TRUNCATE`/`DROP` partition is blocked from DML operations, while other partitions of the main table can still perform DML operations. | Global indexes remain valid: DML operations can be performed on the main table and all index tables, and global indexes related to the main table remain valid. |
| False | Only the `TRUNCATE`/`DROP` partition is blocked from DML operations, while other partitions of the main table can still perform DML operations. | Global indexes become invalid: DML operations can be performed on the main table and all index tables, but global indexes related to the main table become invalid or are rebuilt. |
...
| Operation type | Impact during the truncation |
|----------|------------------------------|
| Concurrent DDL | Prohibited or blocked. Any other DDL operations on the same table cannot be executed concurrently. |
| Concurrent DML | <ul><li>On the target partition: Completely blocked.</li><li>On other partitions: Usually allowed, provided the statement can clearly access other partitions. </li></ul>|
| Concurrent queries (`SELECT`) | Not affected. They can be executed normally. |
