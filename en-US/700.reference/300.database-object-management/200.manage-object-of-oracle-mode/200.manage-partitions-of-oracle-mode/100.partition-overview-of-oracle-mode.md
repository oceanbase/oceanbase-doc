...
![Partition](https://obbusiness-private.oss-cn-shanghai.aliyuncs.com/doc/img/observer-enterprise/V4.2.1/database-object-management/partition.png)
...
In range partitioning, the partitioning key must be a column, not an expression, and must have a data type in the following list: `NUMBER`, `CHAR`, `VARCHAR2`, `DATE`, `TIMESTAMP`, `INTERVAL`, or `RAW`. It cannot be `BLOB` or `CLOB`.
...
* An interval partition can only be used as a top-level partition. Interval partitioning is not supported for subpartitions.
...
In list partitioning, the partitioning key must be a column, not an expression, and must have a data type in the following list: `NUMBER`, `CHAR`, `VARCHAR2`, `DATE`, `TIMESTAMP`, `INTERVAL`, or `RAW`. It cannot be `BLOB` or `CLOB`.
...
In hash partitioning, the partitioning key must be a column, not an expression, and must have a data type in the following list: `NUMBER`, `CHAR`, `VARCHAR2`, `DATE`, `TIMESTAMP`, `INTERVAL`, or `RAW`. It cannot be `BLOB` or `CLOB`.
