...
* `select_stmt`: the `SELECT` statement that defines the view. The information in this statement can be retrieved from a base table or another view.
  ...
  <main id="notice" type='explain'>
    <h4>Note</h4>
    <p><ul><li>In Oracle mode of OceanBase Database, you can use the <code>--string</code> option to add comments to a view when you create it. In this case, the comments are stored in the view definition. <ul><li>You can add comments to <code>select_stmt</code> at any position.</li></ul></p>
  </main>
...
* `WITH CHECK OPTION`: a syntax for creating a view. This syntax ensures that the data that you insert or update through the view meets the definition conditions of the view. In other words, you can insert or update data through the view, but the data must meet the definition conditions of the view.
    ...
    <main id="notice" type='explain'>
      <h4>Note</h4>
      <p>In Oracle mode, the <code>WITH CHECK OPTION</code> syntax does not support the <code>LOCAL</code> or <code>CASCADED</code> options. By default, the <code>CASCADED</code> option is used.</p>
    </main>
