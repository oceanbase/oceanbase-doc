...
* `WITH READ ONLY`: This option is used when creating or replacing a view. It specifies that the view can only be used for reading data and cannot be used for DML operations (`INSERT`, `UPDATE`, `DELETE`). This means that you cannot insert, update, or delete data through the view.

* `WITH CHECK OPTION`: This syntax is used when creating a view. It ensures that the data inserted or updated through the view meets the conditions defined in the view. This means that you can perform DML operations through the view, but the data must meet the conditions defined in the view.
    ...
    <main id="notice" type='notice'>
      <h4>Notice</h4>
      <p>When the <code>WITH CHECK OPTION</code> clause is used in the filter condition, the view definition does not support the <code>JOIN</code> operation.</p>
    </main>
