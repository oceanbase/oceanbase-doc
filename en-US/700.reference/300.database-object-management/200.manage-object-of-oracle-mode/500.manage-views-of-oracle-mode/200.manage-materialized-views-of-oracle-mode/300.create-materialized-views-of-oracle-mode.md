...
When you create a materialized view, you can omit or specify the `DISABLE ON QUERY COMPUTATION` clause to create a regular materialized view. 
...
4. After the real-time materialized view is created, you can query the [DBA_MVIEWS](../../../../700.system-views/500.system-view-of-oracle-mode/200.dictionary-view-of-oracle-mode/9400.dba_mviews-of-oracle-mode.md) view to check whether the materialized view is a real-time materialized view.
    ...
    <main id="notice" type='notice'>
      <h4>Notice</h4>
      <p>In Oracle mode, if the <code>MVIEW_NAME</code> field in the <code>sys.DBA_MVIEWS</code> view matches the table name, the table name must be in uppercase letters. </p>
    </main>
...
* When you create a materialized view, you can add the `AS OF PROCTIME()` clause to the base table. If you use `AS OF PROCTIME()` outside the base table, an error is returned.
  ...
  * When you use a regular view declared as a dimension table with the <code>AS OF PROCTIME()</code> clause as the base table for an incrementally refreshed materialized view, the following limitations apply:
...
5. Create an incrementally refreshed materialized view based on a regular view declared with the `AS OF PROCTIME()` clause.
...
* [Query Rewriting with Materialized Views](500.materialized-views-rewrite-of-oracle-mode.md)
