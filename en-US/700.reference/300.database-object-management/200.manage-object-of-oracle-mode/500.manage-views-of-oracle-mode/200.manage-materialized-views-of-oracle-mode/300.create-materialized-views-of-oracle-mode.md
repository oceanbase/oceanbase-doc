...
* `refresh_clause [PARALLEL n] [query_rewrite_clause] [on_query_computation_clause]`: optional. The details are as follows:

  * `refresh_clause`: the refresh method of the materialized view.
  * `PARALLEL n`: the number of parallel threads used for the refresh operation.

    <main id="notice" type='explain'>
      <h4>Note</h4>
      <p>For V4.4.2, starting from V4.4.2 BP2, you can specify the number of parallel threads for refresh in the <code>REFRESH ... PARALLEL n</code> clause of the <code>REFRESH</code> clause. </p>
    </main')
...
* Before you create a real-time materialized view, you must create a materialized view log for each base table on which the materialized view depends. OceanBase Database supports automatic management of materialized view logs. By default, the system automatically creates materialized view logs. For more information, see [Materialized view logs](200.materialized-views-log-of-oracle-mode.md).
...
2. Create a real-time materialized view `mv_tbl2` based on the `tbl2` table.
...
3. After the real-time materialized view is created, you can query the [DBA_MVIEWS](../../../../700.system-views/500.system-view-of-oracle-mode/200.dictionary-view-of-oracle-mode/9400.dba_mviews-of-oracle-mode.md) view to check whether the materialized view is a real-time materialized view.
...
4. View the execution plan of the real-time materialized view.
...
* Currently, incremental refresh is supported for materialized views that are based on the following types of SQL statements: non-aggregated single-table statements, aggregated single-table statements, multi-table join statements, aggregated multi-table join statements, and `UNION ALL` statements. For other types of SQL statements, incremental refresh is not supported. For more information about the requirements for incremental refresh, see the **Incremental refresh** section in [Refresh a materialized view](400.refresh-materialized-views-of-oracle-mode.md).
    ...
    <main id="notice" type='notice'>
      <h4>Notice</h4>
      <p>For OceanBase Database V4.4.2, starting from V4.4.2 BP2, when you create an incrementally refreshed materialized view, you do not need to manually specify the dependent columns if the materialized view definition contains aggregate functions or the <code>UNION ALL</code> operator.</p>
    </main>
...
4. Create an incrementally refreshed materialized view based on a standard view declared with the `AS OF PROCTIME()` clause.
...
Create a materialized view named `mv_rcp3_tbl1` based on the `tbl1` table. Specify the full refresh strategy (`REFRESH COMPLETE`) for the materialized view, set the query parallelism to 16 and the refresh parallelism to 5. Select the `col1` and `col2` columns from the `tbl1` table where `col3` is greater than or equal to `20` as the data source of the materialized view.
