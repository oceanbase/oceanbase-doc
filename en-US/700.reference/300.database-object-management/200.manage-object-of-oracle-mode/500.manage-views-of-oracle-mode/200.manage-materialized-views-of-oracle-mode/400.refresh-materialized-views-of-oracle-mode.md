...
After the data of the base tables is updated, the data of the materialized view may be inconsistent with that of the base tables. To maintain the data of the materialized view, OceanBase Database refreshes the materialized view.

OceanBase Database supports full refreshes and incremental refreshes for materialized views, and supports manually scheduled and automatically scheduled refreshes.
...
OceanBase Database performs a full refresh by using a remote refresh. Specifically, a hidden table is created, a refresh statement is executed on the hidden table, and then the original table and the hidden table are switched. Therefore, a full refresh requires additional space and will rebuild the indexes (if any) in full.

### Considerations

* A full refresh can be a very time-consuming process, especially when a large amount of data needs to be read and processed. Therefore, you should consider the time required for a full refresh before you perform it.
* A full refresh is allowed only if the column types of the base table match those of the materialized view. Otherwise, a full refresh is not allowed.
