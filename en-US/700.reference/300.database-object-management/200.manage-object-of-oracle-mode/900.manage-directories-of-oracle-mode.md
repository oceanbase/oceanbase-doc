...
The Oracle-compatible mode of OceanBase Database supports mapping a folder on a shared file system (NFS) to a database directory object. Directory objects are mainly used in the `UTL_FILE` system package.
...
* When you create a directory object, you must use the global variable `secure_file_priv` to specify the system path to the files to be accessed.
   ...
   1. Log in as the administrator to an Oracle-compatible tenant in the cluster by using a local Unix socket on the OBServer node.
