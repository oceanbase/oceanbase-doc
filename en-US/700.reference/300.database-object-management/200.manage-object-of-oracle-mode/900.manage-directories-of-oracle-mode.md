...
You can use the `CREATE DIRECTORY` statement to create a directory object in OceanBase Database. A created directory object specifies an alias for a folder on the shared file system.

### Considerations

Take note of the following considerations when you create a directory object:
...
* When you create a directory object, you must use the global variable `secure_file_priv` to specify the system path to the files to be accessed.
   ...
   You can specify the accessible file system path as follows:
...
You can use the `DROP DIRECTORY` statement to drop a directory object from OceanBase Database. To drop a directory object, you must have the `DROP ANY DIRECTORY` privilege.
...
Views related to directory objects
