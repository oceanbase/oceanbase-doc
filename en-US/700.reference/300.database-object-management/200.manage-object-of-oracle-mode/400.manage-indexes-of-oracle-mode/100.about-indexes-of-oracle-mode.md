...
|slug|indexes-oracle-compatible-mode|
...
* Create indexes on fields that are frequently used for queries, but avoid creating too many indexes on tables that are frequently updated.

* Do not use indexes for tables with a small data volume. The time to query all data may be shorter than the time to traverse the indexes, so indexes may not improve performance.
