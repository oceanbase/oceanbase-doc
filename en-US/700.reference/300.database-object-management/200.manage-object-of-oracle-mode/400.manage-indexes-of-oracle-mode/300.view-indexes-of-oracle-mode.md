...
|machine-translation||
|slug|view-indexes-oracle-compatible-mode|
...
## Check whether an index is a global or local index

You can use the [sys.DBA_OB_TABLE_LOCATIONS](../../../700.system-views/500.system-view-of-oracle-mode/200.dictionary-view-of-oracle-mode/14300.dba_ob_table_locations-of-oracle-mode.md) view to check whether an index is a global index (`GLOBAL`) or a local index (`LOCAL`).

**Here is an example:**

1. Query the views under the `TBL1` table.
...
2. Check whether the `TBL1_IDX1` index is a global index (`GLOBAL`) or a local index (`LOCAL`).
...
3. Obtain the index information of the `TBL1` table from the `sys.DBA_INDEXES` and `sys.DBA_OB_TABLE_LOCATIONS` views.
