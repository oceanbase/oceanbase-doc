...
|---------------------------|------------|----------|
| _iut_enable               | true       | Specifies whether to enable index monitoring. The default value is `true`, which indicates that index monitoring is enabled. The value `false` indicates that index monitoring is disabled. |
...
4. Execute the `COMMIT;` statement.
...
6. Query the `DBA_INDEX_USAGE` view.
    ...
    3. Query the `DBA_INDEX_USAGE` view again.
       ...
       **The query result shows the following information:**
       ...
       The `TOTAL_ACCESS_COUNT` column indicates that during the `SELECT` operation, the `idx1_tbl1` index in the `SYS` schema was accessed 1 time at `24-JAN-24 02.28.38.264523 PM`.
       ...
       * `LAST_USED`: the time when the index table was last accessed.
       ...
       For more information about the columns in the `DBA_INDEX_USAGE` view, see [DBA_INDEX_USAGE](../../../700.system-views/500.system-view-of-oracle-mode/200.dictionary-view-of-oracle-mode/8600.dba_index_usage-of-oracle-mode.md).
