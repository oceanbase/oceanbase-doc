...
* `SHARDING`: the table group attribute. The requirements on tables in a table group vary based on the attribute value. If the `SHARDING` attribute is not specified, a table group with `SHARDING = 'ADAPTIVE'` is created by default.

   * `SHARDING = 'NONE'`: no limitations on tables to be added to the table group.

   * `SHARDING = 'PARTITION'`: requires that the first-level partitions of all tables to be added to the table group have the same partition definition as all tables in the table group, including the partitioning type, partition count, and partition value.
   ...
   * `SHARDING = 'ADAPTIVE'`: requires that the partitions and subpartitions of all tables to be added to the table group have the same partition definition as all tables in the table group, including the partitioning type, partition count, and partition value.
...
* [About table groups](../300.manage-table-groups-of-oracle-mode/100.about-table-groups-of-oracle-mode.md)
