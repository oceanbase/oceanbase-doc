...
## Convert the storage format of a table

When you create a table in OceanBase Database, the table is a rowstore table by default. You can use the `WITH COLUMN GROUP` option to explicitly specify to create a columnstore table or a hybrid rowstore-columnstore table. You can convert the storage format of a table by using the `ALTER TABLE` statement.
