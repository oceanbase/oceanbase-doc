...
|slug|define-column-constraints-oracle-mode|
...
Oracle-compatible tenants of OceanBase Database support the `NOT NULL`, `UNIQUE`, `PRIMARY KEY`, `FOREIGN KEY`, and `CHECK` constraints.
...
* `CASCADE`: When a row in the parent table is deleted, the corresponding rows in the child table are automatically deleted.

* `SET NULL`: When a row in the parent table is deleted, values of the corresponding constrained column of the child table are set to `NULL`.

   If you specify the `SET NULL` keyword, the constrained column cannot be a generated column.

If you do not specify the `ON DELETE` clause when you create a `FOREIGN KEY` constraint, the operation to delete a row in the parent table that is referenced by the child table will be rejected due to the foreign key constraint violation.
...
In this example, when a `DELETE` operation is performed on the `col1` column of the parent table `tbl3`, the system sets the values of the corresponding foreign key column in the child table to `NULL`.
