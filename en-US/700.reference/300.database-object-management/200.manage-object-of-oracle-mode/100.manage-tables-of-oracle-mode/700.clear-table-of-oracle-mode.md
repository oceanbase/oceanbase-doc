...
Using `DELETE FROM` to empty a table that has many rows consumes a great deal of system resources.
...
* You cannot perform the `TRUNCATE TABLE` operation on a table that is involved in a transaction or is locked.
