...
* `partial_update`: the default value, indicating that only the changed columns are updated. This mode is suitable for OLTP scenarios where the number of updates is high but the number of queries is low.
* `delete_insert`: indicating that the entire row is updated by deleting the old row and inserting a new row. This mode prioritizes query performance and supports Skip Indexes for incremental data (Memtables and Delta SSTables). It enables efficient filtering during queries, and if the filtering result does not involve updates to baseline data, it allows separate batch processing of baseline and incremental data, reducing read amplification. This mode is suitable for OLAP scenarios where the ratio of incremental data is high and frequent complex queries or batch analyses are required.

<main id="notice" type='explain'>
  <h4>Note</h4>
  <p>The core difference between <code>delete_insert</code> and <code>partial_update</code> lies in the granularity of updates: <code>delete_insert</code> updates the entire row during each update, while <code>partial_update</code> only updates the changed columns. This full-row update enables <code>delete_insert</code> to support Skip Indexes for Delta SSTables, enhancing filtering pushdown performance in analytical queries.</p>
</main>
...
For more information about the configuration scenarios, see [Configuration best practices](../../../../400.deploy/700.best-practices.md).
