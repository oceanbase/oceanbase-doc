...
The current version supports automatic partition splitting only for primary key tables with partitions partitioned by RANGE.
...
* Currently, automatic partitioning is allowed for both the primary table and global indexes, but there are the following restrictions on global indexes:

  1. Global indexes cannot have automatic partitioning enabled independently. Whether global indexes enable automatic partitioning depends on the primary table. If the primary table has automatic partitioning enabled and the global index meets the automatic partitioning rules (that is, no partition key or the partition key matches the prefix of the primary key), then automatic partitioning will also be enabled for the global index.
  2. Non-partitioned global indexes will use their own index keys as the partition key when automatic splitting is triggered, rather than using the primary key of the index table.
...
| range_partition_list | (Optional) The partitioning rules. If you do not specify the `SIZE` clause but specify partitioning definitions, only the partitioning rules are modified, and the existing automatic partition splitting definitions are retained. <main id="notice" type='notice'><h4>Notice</h4><p>At present, OceanBase Database in Oracle mode does not support modifying the partitioning definition of a partitioned table. </p></main> |
...
<main id="notice" type='explain'>
  <h4>Note</h4>
  <p>You can query the automatic partition splitting attributes of a table by using the <code>sys.ALL_TABLES/sys.DBA_TABLES/sys.USER_TABLES</code> view or the <code>oceanbase.CDB_TABLES</code> view in the system tenant. </p>
</main>
...
3. Create an automatically partitioned table named `test2_tbl2` with a pre-partitioning key of `col1` and a split threshold of 10 GB.
...
5. Create an auto-partitioning table named `test2_tbl3` with a pre-partitioning key of `col1` and a split threshold of 10 GB.
...
6. Disable automatic partitioning for the table `test2_tbl3`. This modification does not affect existing partitions, and no new automatic partition splits will be scheduled in the future.
