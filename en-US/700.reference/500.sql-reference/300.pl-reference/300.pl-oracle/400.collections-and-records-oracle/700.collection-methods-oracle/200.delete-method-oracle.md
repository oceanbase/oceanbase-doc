...
* For associative arrays or nested tables (excluding variable-length arrays):
  ...
  * DELETE(n) : removes the element at index n. If the element does not exist, the `DELETE` method does not take effect.
  * DELETE(m,n) : removes all elements with indices between m and n, inclusive, if both m and n exist and m is less than or equal to n. Otherwise, the `DELETE` method does not take effect.
