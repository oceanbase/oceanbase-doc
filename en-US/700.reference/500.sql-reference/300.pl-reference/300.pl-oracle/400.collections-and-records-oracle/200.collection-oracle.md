...
|------|------|---------------------|-------------|--------|-------------------|
| Associative array | Not specified | String or `PLS_INTEGER` | Either | Empty | PL block or package |
...
## Comparing sets
To determine whether one set variable is less than another, you must first define the meaning of "less than" in this context and write a function that returns `TRUE` or `FALSE`.
You cannot compare associative array variables with `NULL` values or with each other. You also cannot use relational operators to compare two set variables. This restriction also applies to implicit comparisons. For example, you cannot use set variables in the `DISTINCT`, `GROUP BY`, or `ORDER BY` clauses.

The comparison of sets is mainly divided into the following three typical cases:
* Comparing a variable of a varray or nested table type with a `NULL` value
