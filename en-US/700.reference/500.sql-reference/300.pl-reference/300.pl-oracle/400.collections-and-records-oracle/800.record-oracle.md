...
In OceanBase Database V4.2.4 and later, you can use a constructor function to directly assign values to a record variable, without the need to separately assign a value to each field. To separately assign a value to each field, you can assign the values of one record variable to another only when the following conditions are met:
