...
The `GET_LINE` stored procedure reads data of a specified length starting from the offset position where the last read ends in the file based on the input file handle and read length. The read will be stopped by a line break.
