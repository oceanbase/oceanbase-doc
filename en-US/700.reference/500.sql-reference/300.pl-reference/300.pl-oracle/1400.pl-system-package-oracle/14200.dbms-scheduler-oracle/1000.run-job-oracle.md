...
|--------------------|------------------|
| job_name            | The name of a job or a list of names that are separated with commas (,). Each name represents an existing job. You can add a schema name and dot separator before the name. If you specify a job that involves multiple destinations, the job runs on all destinations.    |
| force | Specifies whether to forcibly run the job. The value `TRUE` specifies to forcibly run the job.  |
...
* All parameters are specified or have default values.
