...
|value|The target variable to store the return value.

## Examples

Execute the following command to create a table named `tab` with four columns, and use a stored procedure named `single_Row_insert` to insert data into the table using dynamic SQL. The `RETURNING` clause is used to return the calculated value `(c1*c2)` and the inserted date value to the output parameters, and the results are then output.
