| Description   |                 |
|---------------|-----------------|
| keywords      |                 |
| dir-name      |                 |
| dir-name-en   |                 |
| tenant-type   | Oracle Mode     |
|machine-translation|en,ja-marker|
|slug|comma_to_table-oracle-compatible-mode|

# COMMA_TO_TABLE

The `COMMA_TO_TABLE` procedure converts a comma-separated string into a PL/SQL table.

<main id="notice" >
  <h4>Applicability</h4>
  <p>This topic applies only to OceanBase Database in Oracle mode. </p>
</main>

## Syntax

```sql
PROCEDURE COMMA_TO_TABLE( LIST   IN  VARCHAR2,
                         TABLEN OUT BINARY_INTEGER,
                         TAB    OUT UNCL_ARRAY );
PROCEDURE COMMA_TO_TABLE( LIST   IN  VARCHAR2,
                         TABLEN OUT BINARY_INTEGER,
                         TAB    OUT LNAME_ARRAY );
```

## Parameters

|   Parameter   |      Description       |
|--------|---------------|
|list|The comma-separated list of names.|
|tablen|The number of tables in the PL/SQL table.|
|tab|The PL/SQL table containing the list of names.|

## Examples

```sql
obclient> DECLARE
    ->   t_Vararray    DBMS_UTILITY.UNCL_ARRAY;
    ->   Vc_Stringlist VARCHAR2(32767);
    ->   n_Idx         BINARY_INTEGER;
    -> BEGIN
    ->   Vc_Stringlist := 'dMl,dDl';
    ->   DBMS_UTILITY.Comma_To_Table(Vc_Stringlist, n_Idx, t_Vararray);
    ->   DBMS_OUTPUT.Put_Line(n_Idx);
    ->   FOR i IN 1 .. n_Idx LOOP
    ->      t_Vararray(i) := '[' || t_Vararray(i) || ']';
    ->      DBMS_OUTPUT.Put_Line(t_Vararray(i));
    ->   END LOOP;
    -> END;
    -> /
Query OK, 1 row affected (1.895 sec)
```