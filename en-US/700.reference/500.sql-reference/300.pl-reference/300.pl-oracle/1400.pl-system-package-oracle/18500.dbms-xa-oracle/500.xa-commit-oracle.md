...
For more information, see [DBMS_XA constants](../18500.dbms-xa-oracle/200.dbms-xa-constant-oracle.md).
...
* If the RM did not commit the transaction and `onePhase` is set to `TRUE`, the RM may return one of the `XA_RB*` codes. At this point, the RM has rolled back the work of the branch and has released all resources held.
