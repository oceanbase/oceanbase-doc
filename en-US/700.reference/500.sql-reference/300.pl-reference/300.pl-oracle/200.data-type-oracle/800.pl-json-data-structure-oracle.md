...
To facilitate manipulation of JSON data, OceanBase Database supports the following JSON data types in procedural language (PL) statements: `JSON_OBJECT_T`, `JSON_ELEMENT_T`, and `JSON_ARRAY_T`.
...
Here is an example of `JSON_OBJECT_T`:
...
Here is an example of `JSON_ARRAY_T`:
