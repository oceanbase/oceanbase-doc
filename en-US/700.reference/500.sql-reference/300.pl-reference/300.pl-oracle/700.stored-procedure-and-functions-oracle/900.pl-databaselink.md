...
OceanBase Database supports remote calls to stored procedures (including independent stored procedures and those in program packages) and functions in program packages, allowing you to execute PL stored procedures and functions in a remote database. OceanBase Database supports the following types of DBLinks for remote calls:
...
In OceanBase Database in Oracle mode, you can use a DBLink to transmit data of complex data types, including basic types such as integers and strings, as well as nested complex structures. Supported nesting methods are as follows:

* record(basic): indicates a record type that contains a field of a basic type.
* array(basic): indicates an array that consists of elements of a basic type.
* array(record(basic)): indicates an array that consists of elements of a record type and the record fields are of a basic type.
