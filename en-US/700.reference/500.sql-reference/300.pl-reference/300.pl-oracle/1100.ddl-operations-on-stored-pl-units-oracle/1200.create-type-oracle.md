...
You can use the `CREATE TYPE` statement to create a user-defined type (UDT). You can specify the name, method, and other attributes of a type during creation.
...
* Abstract data type (ADT)
  ...
  + If you want to store complex structured data that may need to be encapsulated as methods in a database, you can create an ADT. For example, you can create an ADT to indicate addresses or employees.

* Independent variable array (VARRAY) type

  + If you want to express an array that has a fixed upper limit and needs to be stored and retrieved as a whole, you can define a VARRAY type.

* Independent nested table type

  + If you want to store and access a dataset with an unlimited number of items of the same type, and each item in the dataset is unordered, you can create an independent nested table type.

* Incomplete object type

  An incomplete object type is a type created by using a forward type definition. An incomplete object type has a name but no attributes or methods. It can be referenced by other types and can be used to define types that reference each other. However, you must first create a complete type before you use it to create table columns, object columns, or nested table columns. If you can predict the mutual references among types, you can create an incomplete object type. Specifically, declare an incomplete type as a placeholder, define reference types, and further define the incomplete type.

The `CREATE TYPE BODY` statement is built in with the code for implementing the type. If the created type contains methods, you also need to use the `CREATE TYPE BODY` statement to provide the implementation code of these methods.
...
| plsql_type_spec_source | FORCE                           | If the type specified by `type_name` exists and is referenced by another type but is not used by a table, `FORCE` will force this statement to replace the type. If the type specified by `type_name` is used by a table, the statement will fail with or without `FORCE`. <p><br>Note that if Type `t1` depends on Type `t2` and Type `t2` depends on a table, Type `t1` also depends on the table. </br></p>                 |
