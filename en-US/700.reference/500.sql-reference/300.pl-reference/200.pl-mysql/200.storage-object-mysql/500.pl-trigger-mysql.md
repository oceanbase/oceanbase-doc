...
Example 2: Create two triggers named `trg2_t` and `trg3_t` that fire after the `trg1_t` trigger, and create a trigger named `trg4_t` that fires before the `trg1_t` trigger.
