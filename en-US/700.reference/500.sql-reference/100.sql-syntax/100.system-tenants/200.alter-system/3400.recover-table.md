...
|----------------------|----------------------|
|  table_name_list     | The tables to be restored, in the format of `database_name.table_name1,database_name.table_name2,...`. Separate multiple tables with commas (,). <ul><li>The specified table name must be consistent with that stored in the system. For example, assume that you have created a table named `test` in an Oracle tenant, which is stored by the name of `TEST` in the system. To restore this table, you must specify the table name `TEST`. Otherwise, the system will return an error indicating that the table does not exist. </li> <li>To restore all tables in a database, you can replace `table_name` with `*`, namely, specify `database_name.*`. </li> <li>To restore all tables in all databases, replace both `database_name` and `table_name` with `*`, namely, specify `*.*`. </li><li>If either `database_name` or `table_name` contains a special character, enclose the special character in backticks (``). </li></ul> |
...
| UNTIL | The timestamp or system change number (SCN) to which data is restored. To restore to a specified TIME or SCN value, join the parameter name and value with an equal sign (`=`). By default, if you do not specify the `UNTIL` clause, the tenant is restored to the latest timestamp or SCN. <ul><li>`TIME ='timestamp'` specifies the timestamp to restore to. </li>  <li>`SCN = scn` specifies the SCN to restore to. </li></ul> |
| restore_option     | The restore options of the auxiliary tenant, including `pool_list`, `locality`, `primary_zone`, and `concurrency`. Separate the options with ampersands (`&`). <ul><li>`pool_list` is required and specifies the resource pool. Multiple resource pools must be separated with commas (,). </li>  <li>`locality` is optional and specifies the locality of replicas of the auxiliary tenant, which must match the zone information of `pool_list` of the cluster where the auxiliary tenant belongs. By default, if this option is not specified, a full-featured replica is set for each zone in `zone_list` of the resource pools specified by `pool_list`. </br>Example: `locality='F@z1,F@z2,F@z3'`. </li>  <li> `primary_zone` is optional and specifies the preferred position of the leader of the auxiliary tenant, which must match the settings of `pool_list` and `locality`. In other words, the value of this parameter must match the zone information and meet the constraint that the primary region has at least two Paxos members. If this parameter is not specified, the system randomly distributes leaders in the zones specified by `zone_list` in `locality`. </br>Example: `primary_zone='z1'`. </li> <li>`concurrency` is optional and specifies the concurrency for restore. If `concurrency` is not specified, the `MAX_CPU` value assigned to the auxiliary tenant is used as the default value. </br>Example: `concurrency=50`.</li></ul> |
| WITH KEY FROM 'backup_key_path' ENCRYPTED BY 'password' | Optional. The key backup information of the tenant. You need to specify the key backup information for a tenant restore only when the tenant is configured with transparent encryption. <ul><li>`backup_key_path` specifies the backup path of the key. </li> <li>`password` specifies the encryption password specified during key backup. </li></ul> |
...
1. Create resources required by the auxiliary tenant in the `sys` tenant. This example creates a resource unit named `recover_4c5g` and uses it to create a resource pool.
...
3. After the table restore statement is executed, you can check the restore progress in the `CDB_OB_RECOVER_TABLE_JOBS` view. The `STATUS` column indicates the restore stage. In this example, the stage is `RESTORE_AUX_TENANT`, which means that the auxiliary tenant is being restored.
