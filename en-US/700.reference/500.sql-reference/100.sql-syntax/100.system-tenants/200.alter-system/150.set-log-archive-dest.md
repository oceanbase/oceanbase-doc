...
|-----------------------|--------------------------------------------------------------------------------------------------------------------------------------------|
| archive_path          | The path where the archive is stored. Currently, OceanBase Database supports NFS, Alibaba Cloud OSS, AWS S3, and object storage services compatible with the S3 protocol, such as Huawei OBS, Google GCS, and Tencent Cloud COS. For more information about this parameter, see [archive_path](#archive_path).   |
...
* `delete_mode`: the archive cleanup mode. Valid only for Alibaba Cloud OSS, AWS S3, and Tencent Cloud COS. The value of this parameter is connected to other parameters by using the `&` character. This parameter supports the following values:
...
* `checksum_type`: the check algorithm for checking the integrity of the archive file. This parameter is used to check the integrity of the archive file. If this parameter is not specified, MD5 check is performed by default for all object storage types. The following values are supported based on the object storage type:
  ...
  * `no_checksum`: specifies not to check the integrity of the archive file. This value is supported only for Alibaba Cloud OSS. For object storage services that use the S3 protocol, including AWS S3, OBS, GCS, and Tencent Cloud COS, this value is not supported due to the limitations of the S3 SDK.
