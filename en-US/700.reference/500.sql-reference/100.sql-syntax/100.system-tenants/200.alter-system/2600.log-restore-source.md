...
|slug|sql-tenant-log-restore-source|
...
* When you switch the log restore source of a standby tenant to another, you must make sure that the source tenant is consistent with that specified when you created the standby tenant. In other words, the standby tenant can obtain logs from the archive logs of the source tenant or directly read logs from the source tenant.
...
| archive_path           | The archive path of the log restore source if the standby tenant obtains logs from archive logs. You can query the `CDB_OB_ARCHIVE_DEST` view from the sys tenant or the `DBA_OB_ARCHIVE_DEST` view from a user tenant for the archive path of the primary tenant or source standby tenant.                        |
...
1. Query the archive path of the primary tenant.
   ...
   * Query the archive path of the primary tenant from the current tenant
   ...
   The value corresponding to `path` in the `NAME` column is the log archive path of the current tenant.

2. Change the log restore source.

   * Change the log restore source of a standby tenant from the `sys` tenant of the cluster where the standby tenant `standby_tenant1` resides
