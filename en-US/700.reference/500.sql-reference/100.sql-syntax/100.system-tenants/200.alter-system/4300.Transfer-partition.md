...
* You can transfer a partition only in the primary tenant, and you can transfer only partitions in user tenants.

* If the Transfer feature is disabled for a tenant (that is, the value of the `enable_transfer` parameter in the tenant is `false`), an error will be returned when you execute the `TRANSFER PARTITION` statement for the tenant. If a `TRANSFER PARTITION` statement has been executed successfully, the corresponding Transfer Partition task may be canceled.

* You cannot transfer a partition in a system table.

* You cannot transfer a partition in a user table in a system tenant.

* You cannot transfer a regular table to a broadcast log stream or a replicated table to a regular log stream.

* You cannot transfer non-independent partitions. For example, partitions of a local index table or a LOB table cannot be transferred.

* You cannot initiate a Transfer Partition operation for the same partition until the previous Transfer Partition operation is completed.

* If a balance job (BALANCE_JOB) is being processed in the current cluster, a manually triggered Transfer Partition task will not start immediately.
...
3. Confirm the partition information.
   ...
   You can query the `CDB_OB_TABLE_LOCATIONS` view for `TABLE_ID`, `OBJECT_ID`, and `LS_ID` values of the specified partition.

   * Query `TABLE_ID`, `OBJECT_ID`, and `LS_ID` values of a non-partitioned table
   ...
   * Query `TABLE_ID`, `OBJECT_ID`, and `LS_ID` values of a partitioned table
   ...
   * Query `TABLE_ID`, `OBJECT_ID`, and `LS_ID` values of a subpartitioned table
