...
|---------------------------|------------------|
| MAJOR FREEZE              | Initiates a major compaction on a daily basis. <br>You can use the `TENANT=all` or `TENANT=all_user` parameter to specify to initiate a major compaction on all user tenants (excluding system and meta tenants). We recommend that you use the `all_user` parameter. The `all` parameter will be deprecated in the future; use the `TENANT=all_meta` parameter to specify to initiate a major compaction on all meta tenants. You can also use the `TENANT=tenant_name [, tenant_name ...]` parameter to specify to initiate a major compaction on the specified tenants. </br>       |
| MINOR FREEZE              | Initiates a minor compaction.            |
| {SUSPEND \| RESUME} MERGE | Suspends or resumes a major compaction on a daily basis. <br>You can use the `TENANT=all` or `TENANT=all_user` parameter to specify to suspend or resume major compactions on all tenants (excluding system and meta tenants). We recommend that you use the `all_user` parameter. The `all` parameter will be deprecated in the future; use the `TENANT=all_meta` parameter to specify to suspend or resume major compactions on all meta tenants. You can also use the `TENANT=tenant_name [, tenant_name ...]` parameter to specify to suspend or resume a major compaction on the specified tenants. </br>      |
| CLEAR MERGE ERROR         | Clears the error flags of major compactions. <br>You can use the `TENANT=all` or `TENANT=all_user` parameter to specify to clear the error flags of major compactions on all tenants (excluding system and meta tenants). We recommend that you use the `all_user` parameter. The `all` parameter will be deprecated in the future; use the `TENANT=all_meta` parameter to specify to clear the error flags of major compactions on all meta tenants. You can also use the `TENANT=tenant_name [, tenant_name ...]` parameter to specify to clear the error flags of major compactions on the specified tenants. </br>       |
| MAJOR FREEZE TENANT [=] tenant_name TABLET_ID = tablet_id | Specifies a tablet ID for a major compaction, namely, a partition-level major compaction. <main id="notice" type='explain'><h4>Note</h4><p> This statement can be executed only in the sys tenant. </p></main>|
...
* Initiate a major compaction of all user tenants (excluding the system and metadata tenants).
...
* Initiate a major compaction of all metadata tenants.
...
* Initiate major compactions of the `tenant1` and `tenant2` tenants.
...
* Initiate a minor compaction of all user tenants (excluding the system and metadata tenants).
...
* Initiate a minor compaction of all metadata tenants.
...
* Initiate minor compactions of the `tenant1` and `tenant2` tenants.
...
* Initiate a minor compaction of a specified partition in the `tenant1` tenant.
...
* Initiate a minor compaction of a specified OBServer node.
...
* Initiate a minor compaction of a specified log stream in the `tenant1` tenant.
...
* Initiate a minor compaction of a specified partition in a specified log stream in the `tenant1` tenant.
...
* Suspend daily major compactions of the `tenant1` and `tenant2` tenants.
...
* Suspend daily major compactions of all metadata tenants.
...
* Resume daily major compactions of all user tenants (excluding the system and metadata tenants).
...
* Resume daily major compactions of the `tenant1` and `tenant2` tenants.
...
* Resume daily major compactions of all metadata tenants.
...
* Purge error markers of minor compactions of the `tenant1` and `tenant2` tenants.
...
* Purge error markers of minor compactions of all metadata tenants.
...
1. Query the TABLET ID of the table.
    ...
    **Here is an example:**
    ...
    For more information about the view columns, see [oceanbase.CDB_OB_TABLE_LOCATIONS](../../../../700.system-views/300.system-view-of-sys-tenant/200.dictionary-view-of-sys-tenant/17700.oceanbase-cdb_ob_table_locations-of-sys-tenant.md).
