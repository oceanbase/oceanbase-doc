...
| tenant_name             | The name of the tenant to execute the operation. <main id="notice" type='notice'><h4>Notice</h4><p>Only the system tenant needs to specify the tenant to operate on using the <code>TENANT tenant_name</code> clause. User tenants can only execute this command on themselves, so this clause is not required.</p></main> |
