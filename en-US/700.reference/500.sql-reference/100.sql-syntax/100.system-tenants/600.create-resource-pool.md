...
| ZONE_LIST \[=\] ('zone' \[, 'zone' ...\])  | The zones in which the resource pool to be created is distributed.                                    |

## Examples

Create a resource pool named `pool1`. Specifically, the unit config is `unit1`, the number of units is 1, and the resources are distributed in `zone1`.
