...
* `read_only`: specifies whether the tenant is in read-only mode or read/write mode. If this parameter is not specified, the tenant is in read/write mode by default.
