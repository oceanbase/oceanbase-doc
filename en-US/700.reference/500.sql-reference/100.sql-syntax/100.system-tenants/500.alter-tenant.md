...
You can use the `ALTER TENANT` statement to modify the attributes of a user tenant, including the locality, primary zone setting, resource pool list, and system variable values.
...
You can modify a tenant only as the `root` user of the `sys` tenant (namely `root@sys`).
...
| LOCALITY                 | Modifies the locality of the tenant. Locality describes the distribution of replicas across zones. For example, `F@z1,F@z2,F@z3` indicates that replicas in zones `z1`, `z2`, and `z3` are full-featured replicas. </br>If you want to add a zone, make sure that this zone is included in the `ZONE_LIST` setting of the resource pool of the tenant. |
| PRIMARY_ZONE             | Modifies the primary zone setting of the tenant. Valid values: <ul><li>`RANDOM`: specifies all zones where the tenant resides as the primary zones, which means that all the zones have the same priority. </li> <li>`zone_name`: specifies a specific zone as the primary zone. </li></ul> </br>If you do not specify this parameter when you create a tenant, the default value `RANDOM` is used.                     |
...
| ENABLE_EXTENDED_ROWID      | Specifies whether to enable the Extended ROWID mode for all tables in the tenant. After you enable this mode, it cannot be disabled. However, you can use the tenant-level parameter `default_enable_extended_rowid` to specify whether to enable the Extended ROWID mode for tables created later. |
...
* Change the primary zone setting to `"zone2"` for the `tenant1` tenant.

  ```sql
  obclient [oceanbase]> ALTER TENANT tenant1 primary_zone='zone2';
  ```

* Modify the resource pool list for the `tenant1` tenant. You need to modify the resource pool list generally when you increase or decrease the number of replicas for a tenant. When you modify the resource pool list for a tenant, you can add or remove only one resource pool at a time. You cannot directly replace one resource pool with another.
...
* Modify the locality of `tenant1` to increase the number of replicas. `F` indicates a full-featured replica, and `B_4` is the name of the zone to be added.
...
* If the arbitration service is deployed for the `tenant1` tenant, enable the arbitration service.
