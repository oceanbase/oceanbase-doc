...
The `CREATE TABLE` and `ALTER TABLE` statements allow you to specify the character set and collation for a table. The syntax is as follows:
...
// Change the character set and collation of the table's existing data and update the corresponding settings.
obclient> ALTER TABLE test_collation CONVERT TO CHARACTER SET utf8mb4 COLLATE utf8mb4_bin;
Query OK, 0 rows affected
```

## Choose the character set and collation for a table

OceanBase Database selects the character set and collation for a table based on the following rules:
...
If the character set and collation are not specified for individual columns, the table's character set and collation will be used as the default for the column definitions.