...
| PUSH_SUBQ                      | Indicates the optimizer to execute subqueries that are not rewritten as joins as early as possible.<main id="notice" type='explain'><h4>Note</h4><p>For OceanBase Database V4.3.5, the <code>PUSH_SUBQ</code> hint is supported starting from V4.3.5 BP2. </p></main>|
| NO_PUSH_SUBQ                   | Is the opposite of `PUSH_SUBQ`, instructing the optimizer to execute subqueries that are not rewritten as joins at the end.<main id="notice" type='explain'><h4>Note</h4><p>For OceanBase Database V4.3.5, the <code>NO_PUSH_SUBQ</code> hint is supported starting from V4.3.5 BP2. </p></main>|
...
The `PQ_SUBQUERY` hint includes two main control methods: non-precise control and precise control.
...
- `table_list`: Optional. Specifies the list of tables. It is used only in the subplan filter method for joins.
...
- `method_list`: The distributed data distribution method. The specific distribution methods are as follows:
  ...
  - `DIST_NONE_ALL`: No partitioning is performed, and data is broadcast to all nodes using `NONE ALL`.
...
- `win_group`: (Index list) Declares the grouping and internal behavior of window functions.
  - `dist_method`: Specifies the distributed execution method. The options are the same as above.
  - `PARTITION_SORT`: Whether to apply the partition sort algorithm when using hash distributed execution (`HASH`) or no distributed execution (`NONE`). By default, this is not applied.
...
* `@qb_name`: Optional. Specifies the alias for the subquery (for more information, see the **QB_NAME parameter** section above). It helps clarify the target of the hint. If omitted, the hint applies to the subquery where it is placed.

**Example:**

Use the `PUSH_SUBQ` hint in a query to instruct the optimizer to execute the subquery early, filtering data from the `tbl1` table in advance.
...
The `NO_PUSH_SUBQ` hint is the opposite of the `PUSH_SUBQ` hint. It instructs the optimizer to execute subqueries that have not been rewritten as joins at the end. It is applicable when the subquery has high execution cost or cannot significantly reduce the number of rows. It is applicable in the following scenarios:

* The subquery has high execution cost or its results do not significantly reduce the data volume.
