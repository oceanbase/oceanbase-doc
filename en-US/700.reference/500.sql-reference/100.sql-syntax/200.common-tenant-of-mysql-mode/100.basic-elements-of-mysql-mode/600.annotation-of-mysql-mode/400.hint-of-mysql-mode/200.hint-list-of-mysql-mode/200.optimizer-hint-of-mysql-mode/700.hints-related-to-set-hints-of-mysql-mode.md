...
Since there is no direct place to add hints for set operation clauses, the hints described in this topic must be specified using qb name to indicate the set operation object they affect. For detailed usage, see the examples for each hint.

| Hint type                        | Description                                                  |
|----------------------------------|-------------------------------------------------------|
|`PQ_SET`                          | Controls the distributed execution method for set operations.                             |
|`USE_HASH_SET`                    | Enables the optimizer to use the HASH method in set operations. Its reverse operation is `NO_USE_HASH_SET`.   |
|`NO_USE_HASH_SET`                 | Disables the optimizer from using the HASH method in set operations. Its reverse operation is `USE_HASH_SET`.     |
...
- `left_branch_qb_name`: specifies the left branch of the `UNION` or `INTERSECT` operation.
...
|DIST_HASH_HASH        | PQ_SET ( @SET$1 SEL$1 HASH HASH)|  Only applicable to two-branch `UNION` or `INTERSECT` operations, with SEL$1 as the left branch.  |
|DIST_NONE_PARTITION   | PQ_SET ( @SET$1 SEL$2 NONE PARTITION )| Only applicable to two-branch `UNION` or `INTERSECT` operations, with SEL$2 as the left branch.|
|DIST_PARTITION_NONE   | PQ_SET ( @SET$1 SEL$1 PARTITION NONE )|Only applicable to two-branch `UNION` or `INTERSECT` operations, with SEL$1 as the left branch|

### Examples

In the following queries, the `pq_set(@set$1 local local)` clause specifies that data is pulled directly back to the local node in distributed execution, the `pq_set(@set$1 sel$2 hash hash)` clause specifies that the `HASH` redistribution method is used to perform the set operation, and the `sel$2` clause is used as the left operand in the set operation.

```sql
--Specify a specific distributed execution method by using the PQ_SET hint for collection operations.
...
The `USE_HASH_SET` hint specifies the algorithm to be used for set operations. Its opposite is `NO_USE_HASH_SET`.
...
The following example shows how to use the `USE_HASH_SET` hint to force the use of the `HASH` algorithm.

```sql
-- Use the USE_HASH_SET hint to force the use of the HASH algorithm for set operations.
EXPLAIN BASIC
...
The `NO_USE_HASH_SET` hint specifies the algorithm to be used for set operations. Its opposite is `USE_HASH_SET`.
...
The following query example shows how to use the `NO_USE_HASH_SET` hint to instruct the optimizer to avoid using the hash method for set operations. Since OceanBase Database supports both the hash and merge algorithms for set operations, using the `NO_USE_HASH_SET` hint results in the use of the `MERGE` algorithm for set operations. The `UNION ALL` set operation does not have a deduplication semantics and is not affected by this hint.
