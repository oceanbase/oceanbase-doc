...
|slug|optimized-groupby-hints-mysql-mode|
...
The `USE_HASH_AGGREGATION` hint instructs the optimizer to force the use of the `HASH` aggregation algorithm when generating the execution plan for the SQL statement. It sets `GROUP BY`/`DISTINCT` to use HASH AGGREGATE, such as HASH `GROUP BY` and HASH `DISTINCT`. Its reverse operation is `NO_USE_HASH_AGGREGATION`.
...
-- The optimizer will choose other aggregation methods, such as sort aggregation, to calculate the sum of salaries after grouping by e.department_id.
...
The `NO_GBY_PUSHDOWN` hint is the opposite of the `GBY_PUSHDOWN` hint. It instructs the optimizer not to push the `GROUP BY` operation down to the data nodes but to perform the entire grouping and aggregation process at the central node. Its reverse operation is `GBY_PUSHDOWN`.
