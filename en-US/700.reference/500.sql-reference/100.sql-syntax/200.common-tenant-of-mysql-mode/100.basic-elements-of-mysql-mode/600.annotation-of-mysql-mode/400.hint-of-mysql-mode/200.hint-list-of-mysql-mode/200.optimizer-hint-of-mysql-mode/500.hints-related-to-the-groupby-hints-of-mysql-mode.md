...
| `NO_USE_HASH_AGGREGATION`    | Avoids using hash aggregation. Its reverse operation is `USE_HASH_AGGREGATION`.                             |
...
|`DISABLE_OP_RICH_FORMAT`|In the plan, the specified physical operators in `op_list` are rolled back to vectorized 1.0.|
...
The `NO_USE_HASH_AGGREGATION` hint instructs the optimizer to avoid using the hash aggregation algorithm when executing the SQL statement. It sets groupby/distinct to not use HASH AGGREGATE, using MERGE GROUP BY and MERGE DISTINCT instead. Its reverse operation is `USE_HASH_AGGREGATION`.
...
-- The optimizer will choose other aggregation methods, such as sort aggregation, to calculate the sum of salaries grouped by e.department_id.
...
The `NO_GBY_PUSHDOWN` hint is the opposite of the `GBY_PUSHDOWN` hint. It instructs the optimizer not to push the `GROUP BY` operation down to the data nodes but to complete the entire grouping and aggregation process at the central node. Its reverse operation is `GBY_PUSHDOWN`.
...
The `DISABLE_OP_RICH_FORMAT` hint is used to roll back the specified physical operators in `op_list` to vectorized 1.0 in the plan.
