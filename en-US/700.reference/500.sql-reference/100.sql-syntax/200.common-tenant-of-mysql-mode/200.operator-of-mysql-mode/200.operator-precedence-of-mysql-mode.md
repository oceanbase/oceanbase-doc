...
The following table lists operators from the highest to the lowest precedence. Operators on the same line have the same precedence.
...
* When operators with the same precedence appear in an expression, the operators are evaluated from left to right, except for assignment operators, which are evaluated from right to left.
...
* The precedence and meaning of the following operators depend on the SQL mode:
  ...
  * By default, `!` has a higher precedence than `NOT`. When `HIGH_NOT_PRECEDENCE` is enabled, `!` and `NOT` have the same precedence.