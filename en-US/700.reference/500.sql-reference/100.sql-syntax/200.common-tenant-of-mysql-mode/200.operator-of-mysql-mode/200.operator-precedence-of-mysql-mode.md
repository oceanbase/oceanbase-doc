...
The following table lists operators from the highest to the lowest precedence. Operators in the same row have the same precedence.
...
* When operators of the same precedence appear in an expression, they are evaluated from left to right, except for assignment operators, which are evaluated from right to left.
