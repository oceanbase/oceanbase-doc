...
| Value (used in SQL syntax) | Encryption algorithm used | Key length | Mode | Description |
|---|----------|----------|------|------|
...
* The rule name is globally unique within the tenant, but only for sensitive rules, not for objects. In other words, a table named `R1` and a sensitive rule named `R1` can coexist, but two sensitive rules named `R1` cannot.
* Data protection for sensitive rules is only at the **column level**, not at the table or database level. Therefore, you cannot create data protection rules for entire tables or databases. If you want to encrypt all columns in a table, you must explicitly specify all column names.
