...
* `TIME_ZONE = {'default' | 'time_zone'}`: Optional. Indicates the time zone for determining the current time and the size of the partition key of the `date`, `datetime`, and `year` types. This parameter cannot be modified. Valid values:
...
* `BIGINT_PRECISION = {'none'| 'ms' | 's'}`: Optional. Indicates the timestamp precision of the `bigint` type partition key. This parameter cannot be modified. Valid values:
