...
| WITH PARSER tokenizer_option                 | Optional. The full-text parser. OceanBase Database V4.3.3 supports three built-in parsers: `SPACE`, `NGRAM`, and `BENG`. If you do not specify this keyword, the `SPACE` parser is used by default. |
| PARSER_PROPERTIES\[=\](parser_properties_list) | Optional. The attributes of the parser. For more information, see [parser_properties](#parser_properties). <main id="notice" type='explain'><h4>Note</h4><p>For OceanBase Database V4.3.5, the <code>PARSER_PROPERTIES</code> parameter is supported in a bug fix patch (V4.3.5 BP1) or later. </p></main>|
...
* `SPACE`: the default value. It specifies to tokenize the data by space. Other attributes can be specified for this value. The following table describes the other attributes.
...
* `NGRAM`: specifies to tokenize the data based on N-Gram (Chinese). Other attributes can be specified for this value. The following table describes the other attributes.

  |        Attribute      | Value range |
  |------------------|---------|
  | ngram_token_size | [1, 10] |

* `BENG`: specifies to tokenize the data based on Beng (basic English). Other attributes can be specified for this value. The following table describes the other attributes.
...
* `IK`: specifies to tokenize the data based on IK (Chinese). Currently, only the `utf-8` character set is supported. Other attributes can be specified for this value. The following table describes the other attributes.
  ...
  <main id="notice" type='explain'>
    <h4>Note</h4>
    <p>In OceanBase Database V4.3.5, the <code>IK</code> tokenizer is supported from V4.3.5 BP1. </p>
  </main>
