...
| ORGANIZATION [=] {INDEX \| HEAP} | Specifies the storage order of data rows in the table, namely, the organization mode of the table. The available values are as follows: <ul><li>`INDEX`: Specifies to create an index-organized table.</li><li>`HEAP`: Specifies to create a heap-organized table.</li></ul> If the `ORGANIZATION` option is not specified, its value defaults to the value of the [default_table_organization](../../../../800.configuration-items-and-system-variables/100.system-configuration-items/400.tenant-level-configuration-items/2150.default_table_organization.md) parameter. <main id="notice" type='explain'><h4>Note</h4><p>The <code>ORGANIZATION</code> option is supported in the <code>CREATE TABLE</code> statement of OceanBase Database V4.3.5 and later. </p></main> |
