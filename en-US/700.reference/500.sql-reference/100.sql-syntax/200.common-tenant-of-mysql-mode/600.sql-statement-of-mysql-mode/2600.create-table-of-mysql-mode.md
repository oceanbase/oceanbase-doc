...
| ORGANIZATION [=] {INDEX \| HEAP} | Specifies the storage order of data rows in the table, that is, the table organization method. Valid values: <ul><li>`INDEX`: Indicates an index-organized table.</li><li>`HEAP`: Indicates a heap-organized table.</li></ul> If the `ORGANIZATION` option is not specified, its value defaults to the value of the [default_table_organization](../../../../800.configuration-items-and-system-variables/100.system-configuration-items/400.tenant-level-configuration-items/2150.default_table_organization.md) parameter. <main id="notice" type='explain'><h4>Note</h4><p>For OceanBase Database V4.3.5, the <code>CREATE TABLE</code> statement supports the <code>ORGANIZATION</code> option starting from V4.3.5 BP1. </p></main> |
