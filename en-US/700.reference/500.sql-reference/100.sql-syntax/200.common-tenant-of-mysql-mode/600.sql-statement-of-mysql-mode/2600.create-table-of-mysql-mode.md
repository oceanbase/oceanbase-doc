...
| FULLTEXT | Optional. Specifies to create a full-text index. For more information about how to create a full-text index, see **Create a full-text index** in [Create an index](../../../../300.database-object-management/100.manage-object-of-mysql-mode/500.manage-indexes-of-mysql-mode/200.create-an-index-of-mysql-mode.md). <main id="notice" type='notice'><h4>Notice</h4><p><ul><li>In the current version, you can create only local full-text indexes. </li><li>The full-text indexing feature is currently in the experimental stage. To ensure system stability, we recommend that you do not use it in a production environment. </li></ul></p></main> |
| WITH PARSER {SPACE \| NGRAM \| BENG} | Optional. The full-text parser. The current version supports three built-in parsers, including `SPACE` (white space), `NGRAM`, and `BENG` (basic English). `SPACE` is the default parser.  |
...
| COMMENT | The comment,which is case-insensitive.  |
...
| index_json_clause | Specifies to create a multi-valued index. For more information about multi-valued indexes, see **Create a multi-valued index** in [Create an index](../../../../300.database-object-management/100.manage-object-of-mysql-mode/500.manage-indexes-of-mysql-mode/200.create-an-index-of-mysql-mode.md). <main id="notice" type='notice'><h4>Notice</h4><p>The multi-valued index feature is currently in the experimental stage. To ensure system stability, we recommend that you do not use it in a production environment. </p></main> |
