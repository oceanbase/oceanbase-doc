...
| view_select_stmt      | The `SELECT` statement that defines data in the materialized view. You use this statement to retrieve data from a base table (a regular table or a materialized view) and store the results in the materialized view. The syntax of `view_select_stmt` is the same as that of a regular `SELECT` statement. For more information about the syntax, see [SELECT statement](8100.select-of-mysql-mode/100.select-of-mysql-mode.md). |
