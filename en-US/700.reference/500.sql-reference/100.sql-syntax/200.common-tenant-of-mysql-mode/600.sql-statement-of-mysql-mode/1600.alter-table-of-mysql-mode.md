...
| enable_macro_block_bloom_filter [=] {True \| False} | Specifies whether to persist the macroblock-level Bloom filter. Valid values: <ul><li><code>True</code>: specifies to enable the persistent macroblock-level Bloom filter. </li></ul><ul></li></li><li><code>False</code>: specifies to disable the persistent macroblock-level Bloom filter. </li></ul><main id="notice" type='explain'><h4>Note</h4><p>For OceanBase Database V4.3.5, the <code>enable_macro_block_bloom_filter</code> option is supported since V4.3.5 BP2. </p></main> |
