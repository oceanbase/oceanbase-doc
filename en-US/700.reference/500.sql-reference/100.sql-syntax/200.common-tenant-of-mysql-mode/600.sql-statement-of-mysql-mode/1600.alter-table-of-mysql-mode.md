...
| index_column_group_option | The index options. The following options are supported:<ul><li><code>WITH COLUMN GROUP(all columns, each column)</code>: adds an rowstore-columnstore redundant index. </li><li><code>WITH COLUMN GROUP(all columns)</code>: adds a rowstore index. </li><li><code>WITH COLUMN GROUP(each column)</code>: adds an columnstore index. </li></ul> |
