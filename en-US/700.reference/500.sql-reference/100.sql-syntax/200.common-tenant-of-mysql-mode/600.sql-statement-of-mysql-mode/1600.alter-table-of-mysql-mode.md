...
| PARSER_PROPERTIES\[=\](parser_properties_list) | Optional. Specifies the properties of the tokenizer. For more information, see [parser_properties](#parser_properties) below. <main id="notice" type='explain'><h4>Note</h4><p>In OceanBase Database V4.3.5, support for specifying tokenizer properties (<code>PARSER_PROPERTIES</code>) is available starting from V4.3.5 BP1.</p></main> |
...
| DUPLICATE_SCOPE | Used to change the properties of a replicated table. <ul><li>`none`: indicates converting the current table into a regular table.</li><li>`cluster`: indicates converting the current table into a replicated table.</li></ul> For more information about changing the properties of a replicated table, see [Change Replicated Table Properties](../../../../300.database-object-management/100.manage-object-of-mysql-mode/200.manage-tables-of-mysql-mode/650.change-duplicate-scope-of-mysql-mode.md). |
...
| MODIFY PARTITION partition_name ADD SUBPARTITION (add_subpartition_option) | Indicates adding a new subpartition. <main id="notice" type='notice'><h4>Notice</h4><p><ul><li>Adding new subpartitions is not supported for scenarios where the subpartition type is Hash or Key.</li><li>In OceanBase Database V4.3.5, support for adding new subpartitions is available starting from V4.3.5 BP1.</li></ul></p></main> |
| add_subpartition_option | Represents the definition of the new subpartition. |
...
* `SPACE`: Default value, indicating tokenization by spaces. The following properties can be specified:
...
* `NGRAM`: Indicates tokenization based on N-Gram (Chinese). The following properties can be specified:

  |        Property      | Value range |
  |----------------------|-------------|
  | ngram_token_size     | [1, 10]     |

* `BENG`: Tokenization based on Beng (basic English). The following properties can be specified:
...
* `IK`: Indicates tokenization based on IK (Chinese). Currently supports only the `utf-8` character set. The following properties can be specified:
  ...
  <main id="notice" type='explain'>
    <h4>Note</h4>
    <p>In OceanBase Database V4.3.5, support for the <code>IK</code> tokenizer is available starting from V4.3.5 BP1.</p>
  </main>

You can use the [TOKENIZE](../400.functions-of-mysql-mode/960.text-processing-functions-of-mysql-mode/200.tokenize-of-mysql-mode.md) function to view the tokenization results of text according to the specified tokenizer and JSON format parameters.

### parser_properties

* `min_token_size`: Specifies the minimum token length, with a default value of 3. The range is from 1 to 16.
* `max_token_size`: Specifies the maximum token length, with a default value of 84. The range is from 10 to 84.
* `ngram_token_size`: Specifies the token length for `NGRAM`, effective only for the `NGRAM` tokenizer, with a default value of 2. The range is from 1 to 10.
* `ik_mode`: Specifies the tokenization mode for the `IK` tokenizer. Values are as follows:

  * `smart`: Default value, indicating that words in the dictionary are used to improve tokenization accuracy. Dictionary word boundaries are prioritized, potentially reducing unnecessary expansion.
  * `max_word`: Indicates that words defined in the dictionary are recognized, but do not affect maximum tokenization expansion. Even if defined in the dictionary, `max_word` mode will attempt to split the text into more words.
...
2. Add full-text index `fidx` to table `tbl1`.
