...
| WITH PARSER {SPACE \| NGRAM \| BENG} | Optional. Specifies the parser for the full-text index. The current version supports only three built-in parsers, `SPACE` (space), `NGRAM`, and `BENG` (basic English). If you do not specify a parser, the `SPACE` parser is used by default. You can use the [TOKENIZE](../400.functions-of-mysql-mode/960.text-processing-functions-of-mysql-mode/200.tokenize-of-mysql-mode.md) function to view the tokenization result of the specified parser and the specified JSON parameters. |
