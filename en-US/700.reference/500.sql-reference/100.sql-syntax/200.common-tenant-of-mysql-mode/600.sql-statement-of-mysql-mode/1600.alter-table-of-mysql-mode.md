...
| \[FIRST  \| BEFORE \| AFTER column_name\] | Specifies the position of the new column as the first column of the table or before or after the `column_name` column. Currently, OceanBase Database allows you to specify the position of a column only in the `ADD COLUMN` statement. |
...
| DROP \[COLUMN\]                                            | Deletes a column. <main id="notice" type='notice'><h4>Notice</h4><p>Deleting a primary key column is not allowed.</p></main>|
...
| ADD FULLTEXT \[INDEX \| KEY\] | Adds a full-text index. For more information about how to create a full-text index, see [Create an index](../../../../300.database-object-management/100.manage-object-of-mysql-mode/500.manage-indexes-of-mysql-mode/200.create-an-index-of-mysql-mode.md). |
| WITH PARSER tokenizer_option                 | Optional. Specifies the parser for the full-text index. For more information, see [tokenizer_option](#tokenizer_option).|
| PARSER_PROPERTIES\[=\](parser_properties_list) | Optional. Specifies the properties of the parser. For more information, see [parser_properties](#parser_properties). <main id="notice" type='explain'><h4>Note</h4><p>In OceanBase Database V4.3.5, the <code>PARSER_PROPERTIES</code> parameter is supported from V4.3.5 BP1. </p></main>|
...
| \[NOT\] ENFORCED                                           | Specifies whether to enforce the `CHECK` constraint named `constraint_name`. <ul><li>If omitted or specified as `ENFORCED`, the constraint is created and enforced (default behavior).</li><li>If specified as `NOT ENFORCED`, the constraint is created but not enforced.</li></ul>   |
| ADD COLUMN GROUP([all columns, ]each column) [DELAYED] | Changes a table to a columnstore table. The parameter values are described as follows:<ul><li><code>ADD COLUMN GROUP(all columns, each column)</code>: changes the table to a rowstore-columnstore redundant table. </li><li><code>ADD COLUMN GROUP(each column)</code>: changes the table to a columnstore table. </li><li>`DELAYED`: specifies that the rowstore-to-columnstore conversion command is executed asynchronously. After the command is executed, the storage format in the table definition is modified, but the rowstore-to-columnstore conversion is performed when a major compaction task is executed. This operation does not block current DML operations. It is an online DDL. If you do not specify `DELAYED`, the rowstore-to-columnstore conversion is performed synchronously. <main id="notice" type='notice'><h4>Notice</h4><p><ul><li>The `DELAYED` option is supported only when you change a rowstore table to a columnstore table (`each column`) or a rowstore-columnstore redundant table (`all columns, each column`). </li><li>After you execute the `DELAYED` rowstore-to-columnstore conversion command, the query performance may not meet expectations before the baseline data is actually compacted. </li></ul></p></main></li></ul>|
...
| index_column_group_option | An index option. The following options are supported: <ul><li><code>WITH COLUMN GROUP(all columns, each column)</code>: adds a hybrid rowstore-columnstore index. </li><li><code>WITH COLUMN GROUP(all columns)</code>: adds a rowstore index. </li><li><code>WITH COLUMN GROUP(each column)</code>: adds a columnstore index. </li></ul>|
...
| DUPLICATE_SCOPE | Modifies the duplicate table properties. <ul><li>`none`: Converts the current table into a regular table.</li><li>`cluster`: Converts the current table into a duplicate table.</li></ul> For more information on modifying duplicate table properties, refer to [Modify duplicate table properties](../../../../300.database-object-management/100.manage-object-of-mysql-mode/200.manage-tables-of-mysql-mode/650.change-duplicate-scope-of-mysql-mode.md). |
| enable_macro_block_bloom_filter [=] {True \| False} | Specifies whether to persist the macro block-level bloom filter. The values are as follows: <ul><li><code>True</code>: Enables persistence of the macro block-level bloom filter.</li><li><code>False</code>: Disables persistence of the macro block-level bloom filter.</li></ul><main id="notice" type='explain'><h4>Note</h4><p>For OceanBase Database V4.3.5, the <code>enable_macro_block_bloom_filter</code> option is supported in the <code>CREATE TABLE</code> statement starting from V4.3.5 BP2.</p></main> |
| DYNAMIC_PARTITION_POLICY \[=\] \(dynamic_partition_policy_list\) | Modifies the dynamic partition management properties of a table. `dynamic_partition_policy_list` is a list of configurable parameters for dynamic partition policies, separated by commas. For detailed information, see [dynamic_partition_policy_option](#dynamic_partition_policy_option) below. <main id="notice" type='explain'><h4>Note</h4><p>For OceanBase Database V4.3.5, specifying dynamic partition management properties for tables is supported starting from V4.3.5 BP2.</p></main> |
| REORGANIZE PARTITION | Manually splits a partition. The parameters are described as follows:<ul><li>`partition_name`: the name of the partition to be split. </li><li>`split_partition_name`: the name of the new partition to be created. </li><li>`value`: the boundary value (value range) for splitting the partition. The value range defined for partition splitting must be the same as that of the source partition, and the values must be in ascending order. </li><li>`source_value`: the original boundary value (value range) of the partition to be split. It is the definition of the last split partition, and its value is the same as that of the source partition. </li></ul> For more information about manual partition splitting, see [Manual partition splitting](../../../../300.database-object-management/100.manage-object-of-mysql-mode/300.manage-partitions-of-mysql-mode/250.partition-splitting-of-mysql-mode/200.manual-partition-splitting-of-mysql-mode.md). <main id="notice" type='notice'><h4>Notice</h4><p>In MySQL mode of OceanBase Database, you can split only one partition into multiple partitions at a time, but not multiple partitions into multiple partitions. </p></main> |
| PARTITION BY | Modifies the automatic partitioning attribute or partitioning rule of a table. The parameters are described as follows:<ul><li> `partition_option`: the option for modifying the partitioning rule. For more information, see [Modify the partitioning rule](../../../../300.database-object-management/100.manage-object-of-mysql-mode/300.manage-partitions-of-mysql-mode/300.modify-partition-rules-of-mysql-mode.md). </li><li> `RANGE [COLUMNS]([column_name_list]) [SIZE('size_value')]`: the syntax for modifying the automatic partitioning attribute of a table. For more information, see [Automatic partition splitting](../../../../300.database-object-management/100.manage-object-of-mysql-mode/300.manage-partitions-of-mysql-mode/250.partition-splitting-of-mysql-mode/300.automatic-partition-splitting-of-mysql-mode.md). </li></ul>|
| MODIFY PARTITION partition_name ADD SUBPARTITION (add_subpartition_option) | Adds a new subpartition. <main id="notice" type='notice'><h4>Note</h4><p><ul><li>Adding subpartitions is not supported for scenarios where the subpartition type is Hash or Key.</li><li>For OceanBase Database V4.3.5, adding subpartitions is supported starting from V4.3.5 BP1.</li></ul></p></main> |
| add_subpartition_option | Defines the newly added subpartition. |
| range_partition_expr    | Represents the Range/Range Columns partition expression. |
| list_partition_expr     | Represents the List/List Columns partition expression. |
| FORCE | Clears obsolete columns. <main id="notice" type='explain'><h4>Note</h4><p><ul><li>For OceanBase Database V4.3.5, clearing obsolete columns is supported starting from V4.3.5 BP2.</li><li>When certain columns are deleted, even if they are no longer in use, they still occupy physical storage space. To remove these obsolete columns and reclaim the associated space, clearing them is required.</li></ul></p></main> |
| SEMISTRUCT_ENCODING_TYPE | Optional. Specifies the type of semi-structured encoding. The details are as follows: <ul><li>`''`: Indicates that semi-structured encoding is not enabled.</li><li>`'encoding'`: Indicates that semi-structured encoding is enabled.</li></ul> For more information, see [Use semi-structured encoding](../100.basic-elements-of-mysql-mode/100.data-type-of-mysql-mode/800.json-formatted-data-type-of-mysql-mode/200.create-a-json-value-of-mysql-mode.md). |
...
* `SPACE`: the default value, which specifies to tokenize the text by space. You can specify the following attributes:
...
* `NGRAM`: specifies to tokenize the text based on N-Gram (Chinese). You can specify the following attribute:

  |        Attribute      | Value range |
  |------------------|---------|
  | ngram_token_size | [1, 10] |

* `NGRAM2`: specifies to split text into continuous characters within the range of `min_ngram_size` to `max_ngram_size`. You can specify the following attributes:
...
* `BENG`: specifies to tokenize the text based on Beng (basic English). You can specify the following attributes:
...
* `IK`: specifies to tokenize the text based on IK (Chinese). Currently, only the `utf-8` character set is supported. You can specify the following attribute:
...
You can use the [TOKENIZE](../400.functions-of-mysql-mode/960.text-processing-functions-of-mysql-mode/200.tokenize-of-mysql-mode.md) function to view the results of tokenizing the text based on the specified tokenizer and JSON attributes.
...
* `ngram_token_size`: the token size for the `NGRAM` tokenizer. Only the `NGRAM` tokenizer is effective. The default value is 2. The value range is 1 to 10.
* `ik_mode`: the tokenization mode for the `IK` tokenizer. Valid values include:

  * `smart`: the default value, which specifies to use words in the dictionary to improve tokenization accuracy and prioritize dictionary boundaries to possibly reduce unnecessary expansion.
  * `max_word`: specifies to identify words defined in the dictionary but allows maximum expansion. The `max_word` mode still attempts to divide the text into more words even if they are not defined in the dictionary.
...
* Change the DOP of the `tbl3` table to `2`.
  ...
  Apart from the `ALTER TABLE table_name PARALLEL int;` statement, you can also use the `PARALLEL` hint to change the DOP of a table. The syntax is `alter /*+ parallel(int) */ table`.
...
### Modify the skip index attribute of columns

Create a table named `test_skidx`.
...
* Add the skip index attribute for a column after the table is created. Add a skip index of the `MIN_MAX` type for the `col4` column in the `test_skidx` table.
