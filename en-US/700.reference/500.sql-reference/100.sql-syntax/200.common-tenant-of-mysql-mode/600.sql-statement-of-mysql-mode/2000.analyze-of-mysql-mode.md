...
When the tenant-level parameter `enable_sql_extension` is set to `TRUE`, you can use the following syntax in Oracle-compatible mode:
...
* When the tenant-level parameter `enable_sql_extension` is set to `TRUE`, use the syntax in Oracle-compatible mode to collect the statistics of the `test` table of the `user01` user, with the total number of buckets being 128 for each column.
