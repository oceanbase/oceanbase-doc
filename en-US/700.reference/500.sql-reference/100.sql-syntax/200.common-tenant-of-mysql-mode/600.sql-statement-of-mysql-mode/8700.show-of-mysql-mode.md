...
| PARAMETERS \[like_or_where_clause\] tenant_name                                 | Displays system parameters.                   |
...
| \[FULL\] PROCESSLIST                                                           | Displays the process list of the current tenant. Specifically: <ul><li><code>SHOW PROCESSLIST</code> displays a brief process list, as follows:  <ul><li><code>Id</code>: the process ID, which is the Client Session ID of the current session. This ID uniquely identifies the session on the client side.</li><li><code>User</code>: the username of the user who has connected to the database.</li><li><code>Host</code>: the IP address and port of the client. If the client connects through ODP, this field displays the IP address and port of ODP.</li><li><code>db</code>: the name of the database being accessed.</li><li><code>Command</code>: the type of the command being executed. Valid values: <code>Query</code>, <code>Sleep</code>, and so on.</li><li><code>Time</code>: the execution time of the current command, in seconds. If the command is retried, the value is reset to 0 and recalculated.</li><li><code>State</code>: the state of the current session. Valid values: <code>SLEEP</code>, <code>ACTIVE</code>, and so on.</li><li><code>Info</code>: the command being executed. The length is limited to 100 characters. If the length exceeds 100 characters, the excess part is truncated.</li></ul>  </li><li><code>SHOW FULL PROCESSLIST</code> displays the complete process list, including detailed information about each process, as follows:  <ul><li><code>Id</code>: the process ID, which is the Client Session ID of the current session. This ID uniquely identifies the session on the client side.</li><li><code>User</code>: the username of the user who has connected to the database.</li><li><code>Tenant</code>: the tenant to which the user has connected.</li><li><code>Host</code>: the IP address and port of the client. If the client connects through ODP, this field displays the IP address and port of ODP.</li><li><code>db</code>: the name of the database being accessed.</li><li><code>Command</code>: the type of the command being executed. Valid values: <code>Query</code>, <code>Sleep</code>, and so on.</li><li><code>Time</code>: the execution time of the current command, in seconds. If the command is retried, the value is reset to 0 and recalculated.</li><li><code>State</code>: the state of the current session. Valid values: <code>SLEEP</code>, <code>ACTIVE</code>, and so on.</li><li><code>Info</code>: the command being executed.</li><li><code>Ip</code>: the IP address of the server.</li><li><code>Port</code>: the SQL port number.</li></ul>  </li></ul>  <main id="notice" type='explain'><h4>Note</h4><p>To query the number of sessions and the session IDs in the current database, see <a href="../../../../1200.database-proxy/1500.view-tenant-sessions.md">View tenant sessions</a>.</p></main>   |
...
| CREATE TENANT tenant_name                                                       | Displays the statement for creating a tenant. The sys tenant can view the statements for creating all tenants. A business tenant can only view its own creation statement.  |
...
| GRANTS opt_for_grant_user                                                       | Displays the privileges of the specified user. If no username is specified, it displays the privileges of the current user. The current user can view its own privileges. If you need to view the privileges of other specified users, you must have the `SELECT` privilege on `mysql.user`. |
...
| PROCEDURE CODE proc_name | Displays the internal implementation of the stored procedure <code>proc_name</code>.<main id="notice" type='notice'><h4>Notice</h4><p>This keyword is supported in the current version, but the feature is not available. </p></main>|
| FUNCTION CODE func_name | Displays the internal implementation of the stored function <code>func_name</code>.<main id="notice" type='notice'><h4>Notice</h4><p>This keyword is supported in the current version, but the feature is not available. </p></main>|
| ENGIN engine_name | Displays the operation information of the storage engine.<main id="notice" type='notice'><h4>Notice</h4><p>This keyword is supported in the current version, but the feature is not available. </p></main>|
| OPEN TABLES | Displays the tables and their status information that are currently opened on the server.<main id="notice" type='notice'><h4>Notice</h4><p>This keyword is supported in the current version, but the feature is not available. </p></main> |
...
| CREATE LOCATION location_name | Displays the statement for creating a location object.<main id="notice" type='explain'><h4>Note</h4><p>For OceanBase Database V4.4.x, this syntax is supported starting from V4.4.1. </p></main>|
| LOCATIONS | Displays the location objects of the current tenant.<main id="notice" type='explain'><h4>Note</h4><p>For OceanBase Database V4.4.x, this syntax is supported starting from V4.4.1. </p></main>|
| FILES IN LOCATION @location_name['/path'] [PATTERN = 'regex_pattern'] | Displays the files that exist in the location.<ul><li>`['/path']` is optional and indicates a subdirectory. </li><li>`PATTERN = 'regex_pattern'` is optional and specifies a regular expression to filter files in the location directory. </li></ul> <main id="notice" type='explain'><h4>Note</h4><p><ul><li>To execute the <code>SHOW FILES IN LOCATION</code> statement, the current user must have the <code>READ</code> privilege on the location object. </li><li>For OceanBase Database V4.4.x, this syntax is supported starting from V4.4.1. </li></ul></p></main>|
| SENSITIVE RULES show_sensitive_rule_option | Displays the sensitive column protection rules of the current tenant. Optional: You can use `LIKE` or `WHERE` to filter the rules; or you can use `FROM` or `IN` to specify `relation_factor` (table) and optionally use `FROM` or `IN` `database_name` to specify a database; or you can use `FROM` or `IN` `DATABASE` `database_name` to filter the rules by database. For more information, see [CREATE SENSITIVE RULE](2470.create-sensitive-rule-of-mysql-mode.md). |

## Examples

* View the sensitive column protection rules of the current tenant.

  ```sql
  obclient> SHOW SENSITIVE RULES;
  ```

  ```sql
  obclient> SHOW SENSITIVE RULES FROM db1.t1;
  ```

  ```sql
  obclient> SHOW SENSITIVE RULES IN DATABASE db1 LIKE 'r%';
  ```
...
* View the meaning of each privilege.
...
* View the sequences that you have created.
...
* Use the `SHOW COLUMNS` statement to query the column information of the `tbl2` table.
...
* Use the `SHOW EXTENDED COLUMNS` statement to query the hidden column information of the `tbl2` table.
...
* Use the `SHOW INDEX` statement to query the index information of the `tbl2` table.
...
* Use the `SHOW EXTENDED INDEX` statement to query the hidden index information of the `tbl2` table.
...
* View the Location object of the current tenant.

    ```sql
    obclient> SHOW LOCATIONS;
    ```
...
* View the statement used to create the Location object.

    ```sql
    obclient> SHOW CREATE LOCATION test_location_local;
    ```
