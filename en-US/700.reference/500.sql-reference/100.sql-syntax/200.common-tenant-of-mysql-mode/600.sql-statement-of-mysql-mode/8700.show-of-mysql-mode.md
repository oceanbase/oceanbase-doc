...
|TRACE [FORMAT='JSON']                                                             | Queries the execution status of SQL statements. You can choose to output the results in the JSON format. |
...
|\[FULL\] PROCESSLIST  | Queries the list of processes in the current tenant. The following options are supported: <ul><li><code>SHOW PROCESSLIST</code> displays the brief list of processes with the following information: <ul><li><code>Id</code>: the ID of the process, namely, the client session ID of the current session. This ID is the unique identifier of the session in the client. </li><li><code>User</code>: the username used for database connection. </li><li><code>Host</code>: the IP address and port number of the client. If the database is connected through OceanBase Database Proxy (ODP), the IP address and port number of ODP are displayed here. </li><li><code>db</code>: the name of the database accessed. </li><li><code>Command</code>: the type of the command being executed, such as <code>Query</code> or <code>Sleep</code>. </li><li><code>Time</code>: the execution duration of the current command, in seconds. If the command is retried, the time is reset and recalculated. </li><li><code>State</code>: the status of the current session, such as <code>SLEEP</code> or <code>ACTIVE</code>. </li><li><code>Info</code>: the command being executed. A maximum of 100 characters can be displayed and the exceeding part is discarded. </li></ul>  </li><li><code>SHOW FULL PROCESSLIST</code> displays the complete list of processes with the following information:   <ul><li><code>Id</code>: the ID of the process, namely, the client session ID of the current session. This ID is the unique identifier of the session in the client. </li><li><code>User</code>: the username used for database connection. </li><li><code>Tenant</code>: the connected tenant. </li><li><code>Host</code>: the IP address and port number of the client. If the database is connected through ODP, the IP address and port number of ODP are displayed here. </li><li><code>db</code>: the name of the database accessed. </li><li><code>Command</code>: the type of the command being executed, such as <code>Query</code> or <code>Sleep</code>. </li><li><code>Time</code>: the execution duration of the current command, in seconds. If the command is retried, the time is reset and recalculated. </li><li><code>State</code>: the status of the current session, such as <code>SLEEP</code> or <code>ACTIVE</code>. </li><li><code>Info</code>: the command being executed. </li><li><code>Ip</code>: the IP address of the OBServer node. </li><li><code>Port</code>: the port number of the OBServer node. </li></ul>  </li></ul>  <main id="notice" type='explain'><h4>Note</h4><p>You can execute the <code>SHOW PROCESSLIST</code> statement to query the quantity and IDs of sessions in the current database. For more information, see <a href="../../../../1200.database-proxy/1500.view-tenant-sessions.md">View tenant sessions</a>. </p></main>|
...
|CREATE TENANT tenant_name                                                       | Queries the `CREATE TENANT` statement of the specified tenant. The sys tenant can query the `CREATE TENANT` statements of all tenants. A user tenant can query only its own `CREATE TENANT` statement.  |
|CREATE TABLEGROUP tablegroup_name                                               | Queries the `CREATE TABLEGROUP` statement of the specified tablegroup.  |
|CREATE {DATABASE \| SCHEMA} \[IF NOT EXISTS\] database_name                     | Queries the `CREATE DATABASE` statement of the specified database. The `IF NOT EXISTS` keyword is used to add `IF NOT EXISTS` to the `CREATE` statement.  |
| CREATE TABLE table_name                                                         | Queries the `CREATE TABLE` statement of the specified table.  |
| CREATE VIEW view_name                                                           | Queries the `CREATE VIEW` statement of the specified view.       |
| CREATE PROCEDURE proc_name                                                      | Queries the `CREATE PROCEDURE` statement of the specified stored procedure.   |
| CREATE FUNCTION func_name                                                       | Queries the `CREATE FUNCTION` statement of the specified function.    |
| CREATE USER user_name                                                           | Queries the `CREATE USER` statement of the specified user. |
...
| PROFILE | Queries the performance details about the previous query statement. In the current version, only the keyword syntax is supported. The functionality does not actually take effect. |
| PROFILES | Queries the profiling information of all queries executed since analysis is enabled. In the current version, only the keyword syntax is supported. The functionality does not actually take effect. |
| PLUGINS | Queries information about server plug-ins. In the current version, only the keyword syntax is supported. The functionality does not actually take effect. |
