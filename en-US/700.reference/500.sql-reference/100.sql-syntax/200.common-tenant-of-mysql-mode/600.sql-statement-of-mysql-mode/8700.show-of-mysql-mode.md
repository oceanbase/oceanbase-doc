...
| \[FULL\] PROCESSLIST                                                           | Displays the process list of the current tenant. Details are as follows: <ul><li><code>SHOW PROCESSLIST</code> displays a brief process list, details are as follows:  <ul><li><code>Id</code>: the process ID, which is the Client Session ID of the current session. This ID uniquely identifies the session in the client.</li><li><code>User</code>: the username of the connected user.</li><li><code>Host</code>: the IP address and port of the client (or the IP address and port of the ODP if the connection is made through ODP).</li><li><code>db</code>: the name of the database being accessed.</li><li><code>Command</code>: the type of the current command, which can be <code>Query</code>, <code>Sleep</code>, etc.</li><li><code>Time</code>: the execution time of the current command in seconds. If the command is retried, the time is reset to zero and recalculated.</li><li><code>State</code>: the current session state, which can be <code>SLEEP</code>, <code>ACTIVE</code>, etc.</li><li><code>Info</code>: the command being executed, with a maximum length of 100 characters. Any excess characters are truncated.</li></ul>  </li><li><code>SHOW FULL PROCESSLIST</code>: displays the full process list, including detailed information for each process, as follows:  <ul><li><code>Id</code>: the process ID, which is the Client Session ID of the current session. This ID uniquely identifies the session in the client.</li><li><code>User</code>: the username of the connected user.</li><li><code>Tenant</code>: the connected tenant.</li><li><code>Host</code>: the IP address and port of the client (or the IP address and port of the ODP if the connection is made through ODP).</li><li><code>db</code>: the name of the database being accessed.</li><li><code>Command</code>: the type of the current command, which can be <code>Query</code>, <code>Sleep</code>, etc.</li><li><code>Time</code>: the execution time of the current command in seconds. If the command is retried, the time is reset to zero and recalculated.</li><li><code>State</code>: the current session state, which can be <code>SLEEP</code>, <code>ACTIVE</code>, etc.</li><li><code>Info</code>: the command being executed.</li><li><code>Ip</code>: the IP address of the server.</li><li><code>Port</code>: the SQL port number.</li></ul>  </li></ul>  <main id="notice" type='explain'><h4>Note</h4><p>To query the number of sessions and their IDs in the current database, see <a href="../../../../1200.database-proxy/1500.view-tenant-sessions.md">View tenant sessions</a>.</p></main>   |
...
| PROCEDURE CODE proc_name | Displays the internal implementation of the stored procedure <code>proc_name</code>.<main id="notice" type='notice'><h4>Notice</h4><p>This keyword is supported only in the current version, and the feature is not effective. </p></main>|
| FUNCTION CODE func_name | Displays the internal implementation of the stored function <code>func_name</code>.<main id="notice" type='notice'><h4>Notice</h4><p>This keyword is supported only in the current version, and the feature is not effective. </p></main>|
| ENGIN engine_name | Displays the operation information of the storage engine.<main id="notice" type='notice'><h4>Notice</h4><p>This keyword is supported only in the current version, and the feature is not effective. </p></main>|
| OPEN TABLES | Displays the tables and their status information that are currently open on the server.<main id="notice" type='notice'><h4>Notice</h4><p>This keyword is supported only in the current version, and the feature is not effective. </p></main> |
...
| CREATE LOCATION location_name | Displays the statement for creating a location object.<main id="notice" type='explain'><h4>Note</h4><p>This syntax is supported in OceanBase Database V4.4.x starting from V4.4.1. </p></main>|
| LOCATIONS | Displays the location objects of the current tenant.<main id="notice" type='explain'><h4>Note</h4><p>This syntax is supported in OceanBase Database V4.4.x starting from V4.4.1. </p></main>|
| FILES IN LOCATION @location_name['/path'] [PATTERN = 'regex_pattern'] | Displays the files in the location.<ul><li>`['/path']` is an optional parameter that specifies a subdirectory. </li><li>`PATTERN = 'regex_pattern'` is an optional parameter that specifies a regular expression to filter the files in the location directory. </li></ul> <main id="notice" type='explain'><h4>Note</h4><p><ul><li>To execute the <code>SHOW FILES IN LOCATION</code> statement, the current user must have the <code>READ</code> privilege on the location object. </li><li>This syntax is supported in OceanBase Database V4.4.x starting from V4.4.1. </li></ul></p></main>|
...
* Use `SHOW COLUMNS` to view the column information of the `tbl2` table.
...
* Use `SHOW EXTENDED COLUMNS` to view the hidden column information of the `tbl2` table.
...
* Use `SHOW INDEX` to view the index information of the `tbl2` table.
...
* Use `SHOW EXTENDED INDEX` to view the hidden index information of the `tbl2` table.
...
* View the statement used to create the Location objects.
