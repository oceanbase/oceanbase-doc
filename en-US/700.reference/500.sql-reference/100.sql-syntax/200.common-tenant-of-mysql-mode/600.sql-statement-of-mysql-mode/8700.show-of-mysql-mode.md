...
|--------------------------------------------------|-------------------------------------------------------------------|
|\[EXTENDED\] \[FULL\] TABLES \{FROM \| IN} database_name                                    | Displays all tables in the `database_name` database. <ul><li>The `EXTENDED` keyword is used to display hidden tables. Since OceanBase Database does not generate hidden tables in the current version, this parameter has no practical effect.</li><li>The `FULL` keyword is used to display table types.</li></ul>|
...
| \[FULL\] PROCESSLIST                                                           | Displays the process list of the current tenant. Specifically: <ul><li><code>SHOW PROCESSLIST</code> displays a brief process list, as follows:  <ul><li><code>Id</code>: the process ID, which is the Client Session ID of the current session. This ID uniquely identifies the session in the client.</li><li><code>User</code>: the username of the user who has connected to the database.</li><li><code>Host</code>: the IP address and port of the client (or the IP address and port of the ODP if the client connects through ODP). </li><li><code>db</code>: the name of the database being accessed.</li><li><code>Command</code>: the type of the command being executed. Valid values: <code>Query</code>, <code>Sleep</code>, and so on.</li><li><code>Time</code>: the execution time of the current command, in seconds. If the command is retried, this value is reset to zero and recalculated.</li><li><code>State</code>: the state of the current session. Valid values: <code>SLEEP</code>, <code>ACTIVE</code>, and so on.</li><li><code>Info</code>: the command being executed. The maximum length is 100 characters, and the part exceeding this limit is truncated.</li></ul>  </li><li><code>SHOW FULL PROCESSLIST</code> displays a complete process list, including detailed information about each process, as follows:  <ul><li><code>Id</code>: the process ID, which is the Client Session ID of the current session. This ID uniquely identifies the session in the client.</li><li><code>User</code>: the username of the user who has connected to the database.</li><li><code>Tenant</code>: the tenant to which the user has connected.</li><li><code>Host</code>: the IP address and port of the client (or the IP address and port of the ODP if the client connects through ODP). </li><li><code>db</code>: the name of the database being accessed.</li><li><code>Command</code>: the type of the command being executed. Valid values: <code>Query</code>, <code>Sleep</code>, and so on.</li><li><code>Time</code>: the execution time of the current command, in seconds. If the command is retried, this value is reset to zero and recalculated.</li><li><code>State</code>: the state of the current session. Valid values: <code>SLEEP</code>, <code>ACTIVE</code>, and so on.</li><li><code>Info</code>: the command being executed.</li><li><code>Ip</code>: the IP address of the server.</li><li><code>Port</code>: the SQL port number.</li></ul>  </li></ul>  <main id="notice" type='explain'><h4>Note</h4><p>To query the number of sessions and the session IDs in the current database, see <a href="../../../../1200.database-proxy/1500.view-tenant-sessions.md">View tenant sessions</a>.</p></main>   |
...
| GRANTS opt_for_grant_user                                                       | Displays the privileges of the specified user. If no username is specified, it displays the privileges of the current user. The current user can view their own privileges. If you need to view the privileges of another specified user, you must have the `SELECT` privilege on `mysql.user`. |
...
| PROCEDURE CODE proc_name | Displays the internal implementation of the stored procedure <code>proc_name</code>.<main id="notice" type='notice'><h4>Notice</h4><p>This keyword is supported in the current version, but the feature is not effective. </p></main>|
| FUNCTION CODE func_name | Displays the internal implementation of the stored function <code>func_name</code>.<main id="notice" type='notice'><h4>Notice</h4><p>This keyword is supported in the current version, but the feature is not effective. </p></main>|
| ENGIN engine_name | Displays the operation information of the storage engine.<main id="notice" type='notice'><h4>Notice</h4><p>This keyword is supported in the current version, but the feature is not effective. </p></main>|
| OPEN TABLES | Displays the currently open tables and their status information on the server.<main id="notice" type='notice'><h4>Notice</h4><p>This keyword is supported in the current version, but the feature is not effective. </p></main> |
...
| FILES IN LOCATION @location_name['/path'] [PATTERN = 'regex_pattern'] | Displays the files existing in the location.<ul><li>`['/path']` is an optional parameter indicating a subdirectory. </li><li>`PATTERN = 'regex_pattern'` is an optional parameter used to specify a regular expression for filtering files in the location directory. </li></ul> <main id="notice" type='explain'><h4>Note</h4><p><ul><li>To execute the <code>SHOW FILES IN LOCATION</code> statement, the current user must have the <code>READ</code> privilege on the location object. </li><li>For OceanBase Database V4.4.x, this syntax is supported starting from V4.4.1. </li></ul></p></main>|
...
* View the meanings of each privilege.
...
* Use the `SHOW TRACE` statement to view the execution information of a specific SQL statement and output the information in JSON format.
...
* Use the `SHOW COLUMNS` statement to view the column information of the `tbl2` table.
...
* Use the `SHOW EXTENDED COLUMNS` statement to view the hidden column information of the `tbl2` table.
...
* Use the `SHOW INDEX` statement to view the index information of the `tbl2` table.
...
* Use the `SHOW EXTENDED INDEX` statement to view the hidden index information of the `tbl2` table.
...
* View the Location objects in the current tenant.
...
* View the statement used to create a Location object.
