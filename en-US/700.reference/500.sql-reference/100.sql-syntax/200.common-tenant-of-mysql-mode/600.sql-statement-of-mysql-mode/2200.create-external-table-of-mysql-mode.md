...
| PARTITION_TYPE = USER_SPECIFIED | Specifies to manually add or drop partitions. When <code>PARTITION_TYPE = USER_SPECIFIED</code> is configured, the external table will not automatically manage its partitions.  |
