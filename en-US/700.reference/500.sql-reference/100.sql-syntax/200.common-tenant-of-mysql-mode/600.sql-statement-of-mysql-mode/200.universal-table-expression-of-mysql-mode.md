...
CTEs are optional in the syntax of DML statements and are defined by using the `WITH` clause. You can separate multiple subclauses in a `WITH` clause with commas (,). Each subclause provides a subquery to generate a result set and associates the subquery with a name. The syntax is as follows:
...
* A recursive CTE subquery comprises two parts separated with `UNION [ALL | DISTINCT]`.

  <main id="notice" type='explain'>
    <h4>Note</h4>
    <p><ul><li><code>UNION</code> or <code>UNION DISTINCT</code>: combines the result sets of two or more <code>SELECT</code> statements into one set with the duplicate rows removed. </li><li><code>UNION ALL</code>: combines the result sets of two or more <code>SELECT</code> statements into one set without removing duplicate rows. </li></ul></p>
  </main>
...
* `LIMIT` is not allowed between the recursive `SELECT` part and the non-recursive `SELECT` part.
...
In preceding examples, the recursive CTE traverses the hierarchy of students, while the non-recursive CTE selects only students of a fixed level.
