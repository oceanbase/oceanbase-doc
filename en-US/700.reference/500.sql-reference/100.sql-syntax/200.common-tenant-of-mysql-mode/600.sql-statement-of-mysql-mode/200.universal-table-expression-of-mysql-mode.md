...
If a list of names in parentheses follows the CTE name, these names are column names and must match the number of columns in the `SELECT` statement of the CTE. If no column names are specified, the column names will be taken from the `AS(subquery)` part, specifically from the first `SELECT` list.
...
* The data types of the result columns of the CTE are inferred only from the column types of the non-recursive `SELECT` part, and all columns can be nullable. The types of the recursive `SELECT` part are ignored.
...
(
  ...
  UNION ALL
  SELECT n + 2 FROM cte1 WHERE n < 10     /*Recursive part that generates a new value that is 2 greater than the n value in the previous row set, until n is not less than 10*/
)
SELECT * FROM cte1;
```
...
* The recursive `SELECT` part cannot contain the `LIMIT` structure.

## Examples

The following example shows the difference between a recursive and a nonrecursive CTE. We create a student table that contains the student ID, name, and mentor ID for each student.
...
A nonrecursive CTE does not reference itself. For example, if we want to select all of Alice's direct students (the first-level students), we can use a nonrecursive CTE like this:
