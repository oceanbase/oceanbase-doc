...
| HIGH_PRIORITY | Optional. Specifies that this statement has a higher priority than the table update operation. In the current version, only the keyword syntax is supported. The functionality does not actually take effect. |
...
| SQL_SMALL_RESULT \| SQL_BIG_RESULT  | Optional. Notifies the optimizer of the size of the result set when `GROUP BY` or `DISTINCT` is used. In the current version, only the keyword syntax is supported. The functionality does not actually take effect. |
| SQL_BUFFER_RESULT | Optional. Forcibly stores the result set in a temporary table. In the current version, only the keyword syntax is supported. The functionality does not actually take effect. |
...
| FOR UPDATE                                                                                  | Imposes an exclusive lock on all the rows in the query result to prevent other concurrent transactions from changing or reading the rows in some transaction isolation levels. For more information about lock options, see [opt_for_update_wait](#opt_for_update_wait). |
...
* `NOWAIT` or `NO_WAIT`: specifies not to wait for other transactions to release resources, but to directly return a resource acquisition failure.
* `SKIP LOCKED`: specifies not to wait for a row lock but to immediately execute the query and remove locked rows from the result set.

For more information about `FOR UPDATE`, see [Use the SELECT FOR UPDATE statement to lock query results](../../../../../../300.develop/100.application-development-of-mysql-mode/500.read-data-of-mysql-mode/300.use-operators-and-functions-in-query-of-mysql-mode/1000.lock-query-results-select-for-update-of-mysql-mode.md).
