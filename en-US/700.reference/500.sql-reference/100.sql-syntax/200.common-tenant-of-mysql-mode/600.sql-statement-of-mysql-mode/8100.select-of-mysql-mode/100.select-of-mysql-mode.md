...
| GROUP BY [group_by_condition_list] group_by_summary_option (expression_list) | Optional. Specifies the columns to group the result by. <ul><li> `group_by_condition_list`: Optional. Specifies the columns or expressions to group by. For more information, see [group_by_condition](#group_by_condition). <ul><li>When `group_by_condition_list` is explicitly specified, the columns listed in it are considered as fixed grouping conditions. They will be combined with the subsequent `group_by_summary_option` to form the final grouping logic. For example, `GROUP BY col1, GROUPING SETS ((col2), (col3))` is equivalent to `GROUP BY GROUPING SETS ((col1, col2), (col1, col3))`. </li><li>If `group_by_condition_list` is not specified, all grouping logic is decided by `group_by_summary_option`, namely, the columns or expressions in `expression_list`. </li></ul> </li><li> `group_by_summary_option`: Specifies advanced grouping options. For more information, see [group_by_summary_option](#group_by_summary_option). </li><li>`expression_list`: A list of column names, aliases, or expressions. It specifies the dimensions for summarization. </li></ul> <main id="notice" type='explain'><h4>Note</h4><p> In OceanBase Database V4.3.5, advanced grouping summary is supported from V4.3.5 BP1. </p></main>|
...
<main id="notice" type='explain'>
  <h4>Note</h4>
  <p>Starting from Build 1 (BP1) of OceanBase Database V4.3.5, you can perform advanced grouping and summarization operations. </p>
</main>

* `GROUPING SETS`: specifies the custom grouping methods that allow multiple groupings. It has the following characteristics:

  * You can flexibly define the groupings.
  * The result set contains only the specified groupings.
...
* `ROLLUP`: specifies the hierarchical summarization method. It generates multi-level summaries by reducing the number of grouping columns in sequence according to the order of columns in `expression_list`. It has the following characteristics:

  * It generates a summary for each possible subset of grouping columns.
  * The result set contains:
  ...
  You can view `ROLLUP` as a special case of `GROUPING SETS`. For example:
  ...
  You can also combine `ROLLUP` with `GROUPING SETS`. For example:
...
* `CUBE`: specifies the multidimensional summarization method. It generates summaries of all possible groupings. It has the following characteristics:
  ...
  * The total row.
  ...
  You can view `CUBE` as a special case of `GROUPING SETS`. For example:
  ...
  You can also combine `CUBE` with `GROUPING SETS`. For example:
