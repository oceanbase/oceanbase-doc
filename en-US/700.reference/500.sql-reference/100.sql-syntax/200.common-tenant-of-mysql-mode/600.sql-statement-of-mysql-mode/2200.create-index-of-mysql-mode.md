...
* The `WITH PARSER {SPACE | NGRAM | BENG}` clause can be specified only when you create a full-text index.
...
| WITH PARSER {SPACE \| NGRAM \| BENG} | Specifies the parser for the full-text index. Currently, three built-in parsers, `SPACE` (space), `NGRAM`, and `BENG` (basic English), are supported. If this clause is not specified, the `SPACE` parser is used by default. You can call the [TOKENIZE](../400.functions-of-mysql-mode/960.text-processing-functions-of-mysql-mode/200.tokenize-of-mysql-mode.md) function to view the tokenization results of text based on the specified parser and JSON parameters.|
