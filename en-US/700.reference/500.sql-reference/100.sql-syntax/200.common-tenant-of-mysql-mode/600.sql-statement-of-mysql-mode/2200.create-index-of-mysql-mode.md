...
| WITH PARSER tokenizer_option                 | Optional, specifies the tokenizer for the full-text index. For more details, see [tokenizer_option](#tokenizer_option) below. |
| PARSER_PROPERTIES\[=\](parser_properties_list) | Optional, specifies the tokenizer properties. For more details, see [parser_properties](#parser_properties) below. <main id="notice" type='explain'><h4>Note</h4><p>For OceanBase Database V4.3.5, specifying tokenizer properties (<code>PARSER_PROPERTIES</code>) is supported starting from V4.3.5 BP1.</p></main>|
...
* `SPACE`: specifies to tokenize the text by space. The following attributes can be specified:
...
* `IK`: specifies to tokenize the text based on IK (Chinese). Currently, only the `utf-8` character set is supported. The following attribute can be specified:
  ...
  <main id="notice" type='explain'>
    <h4>Note</h4>
    <p>In OceanBase Database V4.3.5, the <code>IK</code> tokenizer is supported starting from V4.3.5 BP1. </p>
  </main>
