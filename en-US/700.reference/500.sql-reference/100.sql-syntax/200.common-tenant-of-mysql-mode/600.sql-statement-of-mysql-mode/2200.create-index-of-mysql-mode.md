...
* `column_name [(integer)] [ASC | DESC] [ID id]`: specifies a column as the sort key.

  * `column_name`: the name of the column to sort.

  * `integer`: the length limit of the sort key. This option is optional.

  * `ASC`: specifies to sort column values in ascending order.

  * `DESC`: specifies to sort column values in descending order. In the current version, only the descending syntax is supported. The functionality does not actually take effect.
...
* `(index_expr) [ASC | DESC] [ID id]`: specifies to use an index expression as the sort key. You can define an index expression by using expressions or functions. The index expression setting contains the following options:
