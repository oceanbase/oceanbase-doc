...
The recycle bin is disabled by default. The `recyclebin` parameter specifies whether to enable the recycle bin. You can use the `SHOW VARIABLES LIKE 'recyclebin'` statement to check whether the recycle bin is enabled. For more information about the `recyclebin` parameter, see **System variables** in the Reference Guide.
