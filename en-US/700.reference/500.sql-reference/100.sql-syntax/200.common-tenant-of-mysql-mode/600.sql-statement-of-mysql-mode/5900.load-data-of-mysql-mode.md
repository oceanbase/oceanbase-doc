...
* **Supported input files for the `LOAD DATA` statement in OceanBase Database:**

  * **Server-side files**: files stored on an OBServer node of OceanBase Database. You can use the `LOAD DATA INFILE` or `LOAD DATA FROM URL` statement to load data from server-side files to a database table.

  * **Local client files**: files stored in the local file system of the client. You can use the `LOAD DATA LOCAL INFILE` or `LOAD DATA FROM URL` statement to load data from local client files to a database table.
...
* **The current implementation of the LOAD DATA statement allows you to import data from CSV files. The entire import process comprises the following steps:**

  1. **Parse the file**: OceanBase Database reads data from the file based on the specified file name and parses the data in the file in parallel or serially based on the specified degree of parallelism.

  2. **Distribute the data**: As OceanBase Database is a distributed database, the data of each partition may be stored on a different OBServer node. The `LOAD DATA` statement calculates the destination OBServer node for each parsed data record.

  3. **Insert the data**: After a data record is received by the destination OBServer node, the `INSERT` statement is executed to insert the data record into the corresponding partition.
...
* To import data from an external file, you must have the <code>FILE</code> privilege and meet the following requirements:
  ...
  * When loading a file on the server, you must set the system variable <a href="../../../../800.configuration-items-and-system-variables/200.system-variable/300.global-system-variable/12000.secure_file_priv-global.md">secure_file_priv</a> in advance to specify the path that is accessible when importing or exporting files.
  * When loading a local file on the client, you must add the <code>--local-infile[=1]</code> option when starting the MySQL/[OBClient](https://www.oceanbase.com/softwarecenter-cloud) client to enable the data loading feature from the local file system.

* When importing data to a specified partition, the target table must not be a replica table and must not have auto-increment columns, ID columns, or global indexes.
...
| REPLACE \| IGNORE                | The action to take when a unique key conflict occurs. The <code>REPLACE</code> action overwrites the conflicting row, and the <code>IGNORE</code> action ignores the conflicting row. The <code>LOAD DATA</code> statement determines whether data is duplicated based on the primary key of the table. If the table has no primary key, the <code>REPLACE</code> and <code>IGNORE</code> options are the same. By default, when a data conflict occurs, the <code>LOAD DATA</code> statement logs the error data in a log file. <main id="notice" type='notice'><h4>Notice</h4><p><ul><li>When you execute the <code>LOAD DATA LOCAL INFILE</code> statement in MySQL mode, the system automatically adds the <code>IGNORE</code> option. This provides better compatibility with MySQL databases. </li><li>If the parallelism is greater than <code>1</code> and you specify the <code>REPLACE</code> or <code>IGNORE</code> option, the records of conflicting rows inserted in the last execution may be different from those inserted in the execution in serial mode. To strictly ensure the insertion results of conflicting records, do not specify the parallelism for this statement or set the parallelism to <code>1</code>. </li></ul></p></main> |
...
* The `LOCATION` clause specifies the path where external table files are stored. Generally, the data files of an external table are stored in a separate directory. Subdirectories can be contained in the directory. When you create the table, the external table automatically collects all files in the directory.

  * The local `LOCATION` clause is in the format of `LOCATION = '[file://] local_file_path'`. `local_file_path` can be a relative path or an absolute path. If a relative path is specified, the current directory must be the installation directory of OceanBase Database. `secure_file_priv` specifies the file path that the OBServer node has the permission to access. `local_file_path` must be a subpath of `secure_file_priv`.

  * The remote `LOCATION` can be in one of the following formats:
    * `LOCATION = '{oss|cos|S3}://$ACCESS_ID:$ACCESS_KEY@$HOST:s3_region/remote_file_path'`, where `$ACCESS_ID`, `$ACCESS_KEY`, and `$HOST` are the access information required for accessing OSS, COS, and S3 respectively, and `s3_region` is the region information selected when accessing S3. These sensitive access information is stored in encrypted form in the system tables of the database.
    * `LOCATION = 'hdfs://$ {hdfs_namenode_address}:${port}/PATH.localhost'`, where `port` is the port number of HDFS, and `PATH` is the directory path in HDFS.
      * With Kerberos authentication: `LOCATION = 'hdfs://localhost:port/user?principal=xxx&keytab=xxx&krb5conf=xxx&configs=xxx'`.
        ...
        * `keytab`: the filename of the key file for user authentication.
        * `krb5conf`: the filename of the description file for the Kerberos environment.
        * `configs`: the additional HDFS configuration. By default, it is left empty. However, in a Kerberos environment, this parameter must be configured, for example, `dfs.data.transfer.protection=authentication,privacy`, which specifies the data transfer protection level as `authentication` and `privacy`.
...
* The `FORMAT` clause specifies attributes related to the file format. Valid values: CSV, PARQUET, and ORC.
  ...
  * When **TYPE = 'CSV'**, the following options are supported:
    ...
    * `ESCAPE`: specifies the escape character for the CSV file. The value can be only 1 byte. The default value is `ESCAPE ='\'`.
    * `FIELD_OPTIONALLY_ENCLOSED_BY`: specifies the characters that enclose the field values in the CSV file. The default value is an empty string. This option is supported only for columns of the CHAR, VARCHAR, TEXT, and JSON data types.
    * `ENCODING`: specifies the character set encoding. If this parameter is not specified, the default value `UTF8MB4` takes effect.
    * `NULL_IF`: specifies the strings that are to be treated as `NULL` values. The default value is an empty string.
...
* For more information about examples that use direct load, see [Import data by using the LOAD DATA statement](../../../../../500.data-migration/1100.bypass-import/200.full-bypass-import.md).