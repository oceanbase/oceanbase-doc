...
**Bypass mode and non-bypass mode**: You can use the Hint to enable bypass import to improve performance. For more information, see [Use the LOAD DATA statement to bypass import data or files](../../../../../620.obap/300.obap-data-collection/300.obap-import-data/20.bypass-import/200.full-bypass-import.md).
...
<main id="notice" type='explain'>
  <h4>Note</h4>
  <p>When you execute this form, it is equivalent to <code>INSERT INTO table_name SELECT ... FROM url_table_function_expr</code>. Therefore, the hints in this statement are consistent with those in the <a href="https://www.oceanbase.com/docs/common-oceanbase-database-cn-1000000004480988">INSERT statement</a> (such as <code>direct</code> and <code>parallel</code>). The <code>load_batch_size</code> parameter, which is specific to the INFILE form, is no longer supported. </p>
</main>
...
| LOG ERRORS | Optional. Enables error diagnostics during the import. For more information, see [log_errors](#log_errors). |

**Supported formats**: CSV, PARQUET, and ORC (by using FILES); and ODPS (by using SOURCE).

**Bypass and non-bypass**: You can enable bypass import by using a hint. For more information, see [Use the LOAD DATA statement to bypass import data/files](../../../../../620.obap/300.obap-data-collection/300.obap-import-data/20.bypass-import/200.full-bypass-import.md).
...
* For more information about how to use the `LOAD DATA` statement for bypass import, see [Bypass import by using the LOAD DATA statement](../../../../../620.obap/300.obap-data-collection/300.obap-import-data/20.bypass-import/200.full-bypass-import.md)