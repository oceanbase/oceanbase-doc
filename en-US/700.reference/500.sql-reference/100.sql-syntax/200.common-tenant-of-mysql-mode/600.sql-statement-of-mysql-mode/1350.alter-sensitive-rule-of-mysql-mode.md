...
| sensitive_field_list | String |  | The columns to be encrypted. The format is `db.table(col1, col2), db2.table2(col3, col4), ...`. Each sensitive item specifies one or more columns in a table, with columns separated by commas. Sensitive items are separated by commas. If `db` is not specified, the current database is used. |
| encryption_method | String |  | The encryption method. The default value is `aes-256`, which refers to the AES-256-ECB encryption algorithm. Valid values are described below. |
