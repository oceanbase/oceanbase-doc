...
| DEFAULT TABLEGROUP table_group_name                   | The default table group of the database. When you set it to `NULL`, the system disables the default table group. After you change the default table group of the database, existing tables will not be automatically moved to the new default table group. Only new tables for which no table group is specified during creation will be added to the new default table group.      |

## Examples

Change the character set of the `test2` database to `UTF8MB4`, the database collation to `UTF8MB4_BIN`, and the database read/write attribute to `READ WRITE`.
