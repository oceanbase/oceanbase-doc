...
* `xid`: indicates the globally unique identifier of the XA transaction.

### xa_end_stmt

`XA END xid [SUSPEND [FOR MIGRATE]]`: This statement is used to end an XA transaction and optionally suspend it for migration.

* `xid`: indicates the globally unique identifier of the XA transaction to be ended.

  * `SUSPEND [FOR MIGRATE]`: optional. Only supported in the specified syntax, but the feature does not take effect.

### xa_prepare_stmt

`XA PREPARE xid`: This statement is used to notify the resource manager to prepare to commit the XA transaction.

* `xid`: indicates the globally unique identifier of the XA transaction to be prepared for commit.

### xa_commit_stmt

`XA COMMIT xid [ONE PHASE]`: This statement is used to commit a prepared XA transaction.

* `xid`: indicates the globally unique identifier of the XA transaction to be committed.
...
* `xid`: indicates the globally unique identifier of the XA transaction to be rolled back.
