...
The MySQL-compatible mode of OceanBase Database allows you to add a `STORED` column, modify a `STORED` column, drop a `STORED` column, add a `VIRTUAL` column, and drop a `VIRTUAL` column.
