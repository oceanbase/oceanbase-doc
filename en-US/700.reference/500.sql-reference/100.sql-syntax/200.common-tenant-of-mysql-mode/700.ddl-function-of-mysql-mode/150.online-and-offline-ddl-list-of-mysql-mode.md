...
The following table describes the online DDL operations supported by the MySQL-compatible mode of OceanBase Database V4.x.

<main id="notice" type='notice'>
  <h4>Notice</h4>
  <p>In the current version of OceanBase Database, online DDL operations do not support hybrid type operations. For example, adding a column and adding an index separately are both supported online DDL operations. However, adding a column and adding an index at the same time is not supported as an online DDL operation and only supports Offline DDL operations.</p>
</main>
...
| Index operation | Hybrid index operations | Related to the data volume, because index data needs to be completed | For example, `ALTER TABLE t1 ADD INDEX i4(c1), DROP INDEX i2, RENAME INDEX i1 TO i1x` involves hybrid index operations.  |
...
The following table describes the offline DDL operations supported by the MySQL-compatible mode of OceanBase Database V4.x.
...
| Column operation | Reorder columns (`BEFORE`/`AFTER`/`FIRST`) | Related to the data volume, because data is reorganized | This operation is not supported in Oracle mode. |
...
| Column operation | Change the type of a column | Related to the data volume, because data is reorganized | This operation is not supported in Oracle mode. |
| Column operation | Set a column as the primary key | Related to the data volume, because data is reorganized | N/A |
...
| Column operation | Drop a `VIRTUAL` column | Related to the data volume, because data is reorganized | N/A |
...
| Table operation | Change the character set | Related to the data volume, because data is reorganized | This operation is not supported in Oracle mode. |
