...
|  Column operation | Add a column with the `BEFORE`, `AFTER`, or `FIRST` keyword specified | Only for metadata modification | This operation is not supported in Oracle mode. |
...
|     Type     |       Operation                |         Time spent       |      Remarks     |
|-------------|--------------------------|--------------------------|--------------------|
| Column operation | Relocate a column with the `BEFORE`, `AFTER`, `FIRST` keyword specified | Related to the data volume, because data is reorganized (or rewritten) | This operation is not supported in Oracle mode. |
...
|  Column operation | Set a column as the primary key | Related to the data volume, because data is reorganized (or rewritten) | N/A |
|  Column operation | Add or drop a `STORED` generated column | Related to the data volume, because data is reorganized (or rewritten) | N/A |
