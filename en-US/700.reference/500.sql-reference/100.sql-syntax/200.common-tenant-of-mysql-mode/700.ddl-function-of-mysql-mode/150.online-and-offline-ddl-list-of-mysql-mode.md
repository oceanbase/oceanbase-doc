...
|   Column operation    | Add a column to the end of a table  | Only for metadata modification | For example, add a LOB (TEXT) column by executing a statement such as `ALTER TABLE tbl1 ADD c3 LOB`. | Supported <main id="notice" type='notice'><h4>Notice</h4><p>Starting from V4.3.5 BP1, OceanBase Database V4.3.5 supports adding columns to the base table of an mlog-ctl-based materialized view. </p></main>|
|  Column operations | Add a column in the middle (`BEFORE`, `AFTER`, or `FIRST`) | Data volume related. Data needs to be reorganized (that is, the original data needs to be rewritten). | Not supported in Oracle mode. | Supported <main id="notice" type='notice'><h4>Notice</h4><p>Starting from V4.3.5 BP1, OceanBase Database V4.3.5 supports adding columns to the base table of an mlog-ctl-based materialized view. </p></main>|
