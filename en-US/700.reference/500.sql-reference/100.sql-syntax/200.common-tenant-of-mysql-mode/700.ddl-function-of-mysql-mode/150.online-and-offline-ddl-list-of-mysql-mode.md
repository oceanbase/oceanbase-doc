...
- **Definition**: During an online DDL operation, users can still access the database, including reading and writing data.
- **Advantages**: Online DDL operations have minimal impact on business and can be performed while the system is running. They are typically used in high-availability scenarios.
- **Characteristics**: Online DDL operations usually support concurrent access and cause fewer locks, thereby reducing the downtime.

## Offline DDL (Offline DDL)

- **Definition**: During an offline DDL operation, the related tables are locked and users cannot access the corresponding data in the database.
- **Advantages**: Simple and easy to implement. It is suitable for scenarios with slight impact on business.
- **Characteristics**: Offline DDL operations will cause some downtime, which affects normal queries and transaction processing.

## Transaction correlation

- **Transaction**: A transaction is a group of operations. All operations in a transaction must succeed or fail together. A transaction ensures data consistency during its operation.
- Online DDL and offline DDL operations are usually performed separately. In some database management systems, DDL operations cannot be controlled by transactions (especially offline DDL).

## Transaction wait strategies

- **Online DDL**: Typically, online DDL does not need to wait for the current transaction to end and can be executed concurrently.
- **Offline DDL**: Typically, offline DDL needs to wait for the current transaction to end since the related table is locked during the transaction.
...
|  Column operations | Add a column in the middle (`BEFORE`, `AFTER`, or `FIRST`) | Data volume related. Data needs to be reorganized (that is, the original data needs to be rewritten). | Not supported in Oracle mode. | Not supported |
...
|  Column operations | Drop a `VIRTUAL` column |  Data volume related. Data needs to be reorganized.  | N/A| Not supported |
...
| Table operations   |	Convert row-based storage to columnar storage <main id="notice" type='notice'><h4>Notice</h4><p>When you execute <code>ALTER TABLE</code> without the <code>DELAYED</code> identifier, the operation is offline DDL. </p></main>|	Data volume related. Data needs to be reorganized. |	 N/A| Not supported |
| Table operations   |	Convert row-based storage to hybrid row-column-based storage <main id="notice" type='notice'><h4>Notice</h4><p>When you execute <code>ALTER TABLE</code> without the <code>DELAYED</code> identifier, the operation is offline DDL. </p></main>|	 Data volume related. Data needs to be reorganized. |	 N/A| Not supported |
...
|  Partition operations | Manual partition split              | Data volume related. Data needs to be reorganized. |  N/A  | Not supported |
|  Partition operations | Modify the auto-partitioning attributes and the partitioning rule | Data volume related. | For example: `ALTER TABLE t1 PARTITION BY RANGE(xxx) SIZE('xxx') (PARTITION...);`| Not supported |
