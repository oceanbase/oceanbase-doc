...
A lateral derived table can provide higher flexibility and efficiency for subqueries, especially for subqueries that depend on columns in the outer query. Starting from V4.2.2, OceanBase Database in MySQL-compatible mode introduces the `LATERAL` keyword to support this feature.
