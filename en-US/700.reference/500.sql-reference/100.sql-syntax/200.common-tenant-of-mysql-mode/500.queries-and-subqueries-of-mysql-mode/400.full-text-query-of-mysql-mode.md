...
* `search_modifier`: the search modifier that determines the search mode. This parameter is optional. Valid value:
  ...
  * `IN BOOLEAN MODE`: specifies to use the boolean search mode. Currently, the database supports three commonly used Boolean operators and nested Boolean expressions. The operators are described as follows:

    * `+`: indicates `AND`, representing the intersection of sets.
    * `-`: indicates `NOT`, representing the difference between sets.
    * No operator: when used alone, it indicates `OR`, representing the union of sets, such as `A B` indicating `A OR B`. The relevance between the words A and B in a sentence is higher when an ampersand is used together with the operator. However, the operator alone loses the meaning of `OR`.
    * `()`: indicates nested Boolean expressions. When an operator is not added before a set of nested clauses, the `OR` operator is implied before the set, such as `+A (nested clauses)`, indicating that A or the set of nested clauses must be included.

      <main id="notice" type='explain'>
        <h4>Note</h4>
        <p>Starting from V4.3.5 BP1, OceanBase Database V4.3.5 supports <code>IN BOOLEAN MODE</code>. </p>
      </main>
