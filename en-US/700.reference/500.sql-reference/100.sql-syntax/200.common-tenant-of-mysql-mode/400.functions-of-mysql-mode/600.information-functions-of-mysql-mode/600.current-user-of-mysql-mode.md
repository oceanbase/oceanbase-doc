...
Log in to OceanBase Database as the `test` user, and execute `CURRENT_USER()`. The result is as follows:
...
Delete the `test@192.168.%.%` user, connect to OceanBase Database by using the `test` user, and execute `CURRENT_USER()`. The result is as follows:
