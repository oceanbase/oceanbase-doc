...
If `expr1` is `TRUE` (that is, `expr1` is not equal to 0 and is not equal to `NULL`), returns `expr2`; otherwise:

* If `IF(expr1, expr2)` is used, returns `NULL`.
* If `IF(expr1, expr2, expr3)` is used, returns `expr3`.
