...
* In MySQL-compatible mode, if `min` or `max` is an integer, the output of the `UNIFORM()` function is an integer; otherwise, the output of the `UNIFORM()` function is a floating-point number. 
