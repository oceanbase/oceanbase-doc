...
* If any parameter of the expression is `NULL`, the return value is `NULL`.
* If the `delimiter` does not exist in `str` and `start_part` is `1`, the original string of `str` is returned.
