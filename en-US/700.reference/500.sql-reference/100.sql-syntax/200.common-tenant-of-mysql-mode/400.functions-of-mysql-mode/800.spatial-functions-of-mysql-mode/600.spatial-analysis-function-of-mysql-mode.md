...
The unit conversion takes effect only when the geometry `ls` has an associated spatial reference ID (SRID), and the SRID is not 0. An SRID of 0 usually indicates no SRS.
...
- `ls`: the input parameter that represents the linear geometry.
- `unit`: the unit of length, which is optional. If this parameter is not specified, the default unit `metre` is used. Various length units such as `foot` and `centimetre` are supported, depending on function implementation and the spatial database system.
...
Then, `ST_Length()` calculates the length of this LineString object. In the first function call, the length unit `foot` is specified. The length of 1029205.9131247795 feet is returned. In the second call of `ST_Length()`, no unit is specified and the default unit `metre` is used. The length of 313701.9623204328 meters is returned.
...
In this example, a polygon with a complex shape is defined and two kinds of spatial analysis operations are performed on the polygon.
...
`ST_Centroid()` calculates the centroid of the polygon, which is the equilibrium point of each part of the polygon but is not necessarily located in the interior of the polygon.
The result is displayed in two columns. The first column `pt_on_surf` records the coordinates of a point on the surface of the polygon. The second column `centroid` records the coordinates of the centroid of the polygon. The query result shows that the point on the polygon surface is (1 5), and the centroid is (4.076923076923077 5).
...
`_ST_PointOnSurface(geometry a)` returns a point on the surface of the input geometry `a`. The function returns a point that lies in the interior of a geometry (such as a polygon or another planar geometry), rather than the geometric or arithmetic center of the geometry.
...
The function may not return any of the interior points, but return a specific point that may represent the entire geometry, for example, a vertex or a boundary point of a polygon.
...
In this example, a polygon with a specific shape is created from a WKT string. Then, the polygon is analyzed by using the `_ST_PointOnSurface` and `ST_Centroid` functions.
