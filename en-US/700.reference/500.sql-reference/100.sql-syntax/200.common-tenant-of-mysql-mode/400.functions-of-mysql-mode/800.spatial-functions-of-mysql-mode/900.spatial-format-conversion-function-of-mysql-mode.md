...
`ST_AsGeoJSON()` converts a geometry object into a GeoJSON string. GeoJSON is a geospatial data interchange format based on JSON. GeoJSON supports the following geometry types: Point, MultiPoint, LineString, MultiLineString, Polygon, and MultiPolygon.
...
* `options`: a three-bit switch that specifies whether to include the bounding box and coordinate reference system (CRS) information in the JSON output. This parameter is optional.
...
- `boolean clip_geom=true`: a Boolean value that indicates whether to clip off the geometry data outside tile bounds but in the buffer to reduce the size of the output tile. This parameter is optional. The default value is `true`, which specifies to clip the part beyond tile bounds.
