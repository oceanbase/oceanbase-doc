...
If both geometry arguments are valid Point or MultiPoint values in an SRS, the return value is the shortest distance between the two geometry objects on the sphere with a radius specified by `radius`. If the `radius` argument is omitted, the average radius is used by default, which is (2a+b)/3, where `a` is the semi-major axis of the SRS and `b` is the semi-minor axis.
...
`ST_Crosses` tests whether geometry objects `g1` and `g2` spatially cross. The function returns `1` if `g1` crosses `g2`, and returns `0` if otherwise. The `ST_Crosses()` function supports spatial indexes.
...
Specifically, if two LineString objects cross each other at more than two points in their interiors, `ST_Crosses(g1, g2)` returns `1`. If a LineString object passes through the interior of a surface object, `ST_Crosses(g1, g2)` returns `1`.
...
In this example, `ST_Crosses()` tests whether a linear geometry object, a straight line from point (1,1) to point (11,11), crosses a Polygon object, a square with vertices of (0,0), (0,10), (10,10), and (10,0). The return result is 1, indicating that the straight line crosses the polygon.
...
If two LineString objects share a segment but do not contain each other, `ST_Overlaps(g1,g2)` returns `1`.
...
Note that if both input geometry objects are Point objects, `_ST_Touches(geometry A, geometry B)` returns `0`. This is because points do not have boundaries, and two points therefore cannot be considered "touching". They are either identical (that is, at the same position) or completely independent.
...
In this example, `_ST_Touches` tests whether a LineString object touches a Point object in space. The Point object (0 2) is at the end of the LineString object (0 0, 1 1, 0 2), so the function returns 1, indicating that the two objects spatially touch.
...
`ST_Equals(geometry A, geometry B)` tests whether two geometry objects `A` and `B` are spatially equal. Specifically, the function tests whether the two objects contain the same set of points that are at the same relative positions, but the function does not require the same sequence of points.
