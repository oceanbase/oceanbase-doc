...
* `timeout`: the timeout period for waiting for a lock to be released, in seconds. Valid values:
...
1. In session 1, execute the following statement to obtain the lock named `my_lock` with a timeout period of 10s:
...
2. In session 2, execute the following statement to obtain the lock named `my_lock` with a timeout period of 10s:
