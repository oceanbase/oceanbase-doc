...
`3221487701` is returned, indicating that the lock `my_lock` is held by the session with the ID of `3221487701`. You can query the `oceanbase.GV$OB_PROCESSLIST` view for information about the session. Here is an example:
