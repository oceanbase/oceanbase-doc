...
For any given column in the current row, `LAG()` retrieves data of the column in a row at a given physical offset after the current row. A self join on the same table achieves the same result, but the `LAG()` function is more efficient.
