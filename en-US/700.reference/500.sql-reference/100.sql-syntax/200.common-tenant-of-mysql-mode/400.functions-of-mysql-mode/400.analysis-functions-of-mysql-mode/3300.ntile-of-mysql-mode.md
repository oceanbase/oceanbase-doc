...
Divides the sorted rows in a partition into a specified number of groups of as equal size as possible and returns the group number to each row. If `expr` is `NULL`, it returns `NULL`.
