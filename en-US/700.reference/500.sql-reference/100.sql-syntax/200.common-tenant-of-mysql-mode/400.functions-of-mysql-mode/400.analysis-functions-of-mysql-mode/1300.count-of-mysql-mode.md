...
`COUNT()` returns the number of non-`NULL` values in rows retrieved by using the `SELECT` statement. If no matching rows are found, `COUNT()` returns `0`. You can use the `DISTINCT` option to return the number of distinct values of `expr`.
