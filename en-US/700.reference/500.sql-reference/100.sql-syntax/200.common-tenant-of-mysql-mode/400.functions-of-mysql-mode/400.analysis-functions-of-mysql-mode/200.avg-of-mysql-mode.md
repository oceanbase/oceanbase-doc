...
|slug|avg-mysql-compatible-mode|
...
`AVG()` returns the average value of a specified group. NULLs are ignored. The `DISTINCT` option can be used to return the average of distinct values of `expr`. If no matching rows are found, `AVG()` returns `NULL`.
