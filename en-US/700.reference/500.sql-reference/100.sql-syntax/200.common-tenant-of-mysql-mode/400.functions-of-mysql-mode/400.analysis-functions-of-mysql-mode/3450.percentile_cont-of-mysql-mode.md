...
<main id="notice" type='explain'>
  <h4>Note</h4>
  <p>This function is supported starting from OceanBase Database V4.2.5 BP4. </p>
</main>
...
* `expr`: the target column for which to calculate the percentile value. It must be a numeric type. For more information about the supported types, see **Supported types**.
...
* `percentile`: the specified percentile value, which indicates the position of the target value in the data distribution. It is a floating-point constant in the range [0, 1].
...
3. Use `PERCENTILE_CONT` to calculate the median (50th percentile) for each category in the `col1` column.
