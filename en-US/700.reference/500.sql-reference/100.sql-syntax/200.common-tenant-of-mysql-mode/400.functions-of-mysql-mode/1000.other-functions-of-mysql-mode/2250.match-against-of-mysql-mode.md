...
* `IN BOOLEAN MODE`: specifies to use the boolean search mode. The current version supports three commonly used Boolean operators and nested Boolean expressions. The operators are described as follows:

  * `+`: indicates `AND`, representing the intersection of sets.
  * `-`: indicates `NOT`, representing the difference between sets.
  * No operator: when used alone, it indicates `OR`, representing the union of sets. For example, `A B` is equivalent to `A OR B`. The `AND` relevance between the words in the sentence can be enhanced by using the `+` operator, but the `OR` meaning is lost. For example, in the query `+A B`, the sentence must contain the word A, and the relevance between A and B is calculated.
  * `()`: represents nested Boolean expressions. If no operator precedes the parentheses, the `OR` operator is implied, such as `+A (nested clause)`, which means that the sentence must contain A or the nested clause.

    <main id="notice" type='explain'>
      <h4>Note</h4>
      <p>Starting from Build 1 (BP1) of OceanBase Database V4.3.5, the <code>IN BOOLEAN MODE</code> clause is supported. </p>
    </main>
...
* In a query that contains `OR`-connected `MATCH AGAINST` predicates, an index merge plan is automatically attempted. The optimizer ultimately chooses the index merge plan based on the cost competition.

    <main id="notice" type='explain'>
      <h4>Note</h4>
      <p>Starting from Build Package (BP) 1 of OceanBase Database V4.3.5, the system supports index merge plans. </p>
    </main>
