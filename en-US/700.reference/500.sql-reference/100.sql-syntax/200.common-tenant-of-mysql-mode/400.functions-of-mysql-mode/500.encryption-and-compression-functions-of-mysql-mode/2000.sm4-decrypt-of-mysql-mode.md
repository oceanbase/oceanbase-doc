...
|slug|sm4_decrypt-mysql-compatible-mode|
...
For more information about the `init_vector` parameter, see [SM4_ENCRYPT](1900.sm4-encrypt-of-mysql-mode.md).

## Examples

1. In `sm4-ecb` mode, `SM4_DECRYPT()` accepts two arguments in the format of `{str, key_str}`. If three values are passed in, the third value is ignored. Here is an example:
