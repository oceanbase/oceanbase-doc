...
<h4>Note</h4>
<p>When the tenant is configured with <code>sql_func_extension_mode='ClickHouse'</code>, the <code>DATE_ADD</code>/<code>DATE_SUB</code> functions in the preceding table use the ClickHouse-compatible implementation (including truncation of intervals, subsecond precision, and restrictions on combining <code>DATE</code> with subsecond units). For more information, see <b>DATE_ADD / DATE_SUB (ClickHouse mode)</b>.</p>
</main>
