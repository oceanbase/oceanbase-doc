...
* If a hierarchical query contains the `CONNECT BY` clause, you can use the `USE_HASH` or `USE_NL` hint to select the optimal execution plan in scenarios with large amounts of data. For more information about the `USE_HASH` and `USE_NL` hints, see [Hints related to joins](../../300.common-tenant-of-oracle-mode/300.basic-elements-of-oracle-mode/600.annotation-of-oracle-mode/400.hint-of-oracle-mode/200.hint-list-of-oracle-mode/300.hints-related-to-join-operations-of-oracle-mode.md).
...
Before you use a hierarchical query, you must understand its execution process. The following describes the general execution process of a hierarchical query:
...
4. Use the `CONNECT BY` clause for a recursive query and use the `USE_HASH` hint to instruct the optimizer to use the HASH-JOIN algorithm.
    ...
    |      2 | Manager of Europe region         |      1 |
    |      5 | Manager of Italy region       |      2 |
    |      6 | Manager of France region         |      2 |
    |      3 | Manager of Asia-Pacific region         |      1 |
    |      7 | Manager of China region         |      3 |
    |     12 | Manager of Beijing, China region         |      7 |
    |      8 | Manager of South Korea region         |      3 |
    |      9 | Manager of Japan region         |      3 |
    |      4 | Manager of Americas region         |      1 |
    |     10 | Manager of US region         |      4 |
    |     11 | Manager of Canada region       |      4 |
    +--------+--------------------+--------+
