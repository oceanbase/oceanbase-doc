| Description   |                 |
|---------------|-----------------|
| machine-translation ||
| keywords      |                 |
| dir-name      |                 |
| dir-name-en   |                 |
| tenant-type   | Oracle Mode     |
|slug|drop-sequence-oracle-mode|

# DROP SEQUENCE

## Purpose

This statement is used to drop a sequence.

## Syntax

```sql
DROP SEQUENCE [schema. ] sequence_name
```

## Parameters

|      Parameter       |                            Description                             |
|---------------|-----------------------------------------------------------|
| schema.       | The user who owns the sequence. If you do not specify `Schema`, the sequence of the current login user is used by default. |
| sequence_name | The name of the sequence to be dropped.                                               |

## Examples

Example of creating and dropping a sequence.

```sql
obclient> CREATE SEQUENCE employee_id_seq
    START WITH 1000
    INCREMENT BY 1;

obclient> DROP SEQUENCE employee_id_seq;
```

## References

- [CREATE SEQUENCE](2200.create-sequence-of-oracle-mode.md)
- [ALTER SEQUENCE](600.alter-sequence-of-oracle-mode.md)