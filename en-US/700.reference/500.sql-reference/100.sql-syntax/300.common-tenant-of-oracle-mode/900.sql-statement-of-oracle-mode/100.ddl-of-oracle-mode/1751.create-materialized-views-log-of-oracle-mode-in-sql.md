...
A materialized view log (mlog) records incremental update data of a base table (a regular table or a materialized view) to support fast refreshes of materialized views. An mlog is a record table that tracks changes of the base table and applies the changes to the corresponding materialized view, achieving fast refreshes.
...
|---------------------|--------------------------|
| OR REPLACE              | Optional. Specifies to create a new `mlog` if the corresponding `mlog` already exists, and replace the existing `mlog` with the new one. The new `mlog` will be created based on the definition. The normal reading and writing of materialized views and base tables are not affected during the recreation of the `mlog`. <main id="notice" type='explain'><h4>Note</h4><p>For OceanBase Database V4.3.5, the <code>OR REPLACE</code> option is supported starting from V4.3.5 BP3. </p></main> |
...
2. Create a materialized view log in the `test_tbl1` table. Set the DOP of the materialized view log to `5`. Specify to record data changes to the `col2` columns in the base table to the materialized view, including the old and new values before and after the change. Specify to clear expired log data once a day, starting from the current day.
...
5. View the materialized view log information on the `test_tbl1` table.
