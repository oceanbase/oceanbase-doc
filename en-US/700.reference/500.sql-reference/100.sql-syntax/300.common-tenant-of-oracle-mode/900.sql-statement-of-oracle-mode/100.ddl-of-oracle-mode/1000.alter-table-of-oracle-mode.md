...
| `enable_macro_block_bloom_filter`| Specifies whether to persist the bloom filter at the macroblock level. Valid values are as follows:<ul><li><code>True</code>: Enables the persistence of the bloom filter at the macroblock level.</li></ul><ul></li></li><li><code>False</code>: Disables the persistence of the bloom filter at the macroblock level.</li></ul>|
| `DROP PARTITION partition_name` | Drops the specified partition. |
| `RENAME PARTITION old_name TO new_name` | Renames a partition. When renaming a partition, `new_name` specifies the new partition name (case-insensitive). Renaming a partition modifies the corresponding partition in the main table but does not affect the partition name of a local index. You can view the `USER_TAB_PARTITIONS` and `USER_TAB_SUBPARTITIONS` views to confirm the result of the partition name modification. For more information, see [Rename a partition](../../../../../300.database-object-management/200.manage-object-of-oracle-mode/200.manage-partitions-of-oracle-mode/310.rename-a-partition-of-oracle-mode.md). During the partition renaming process, if a DML operation holds a lock on the partition, the partition name modification is blocked until the DML operation releases the lock. |
| `SPLIT PARTITION partition_name split_actions` | Splits a partition. This operation is used for manual partition splitting. It supports two formats: `split_at_format` and `split_into_format`. For more information, see [split_partition_option](#split_partition_option). |
| `TRUNCATE PARTITION partition_name` | Truncates the data in the specified partition. |
| `MODIFY PARTITION partition_name ...` | Modifies the partition attributes. This operation is used for adding a subpartition.<main id="notice" type='notice'><h4>Notice</h4><p>Adding a subpartition is not supported when the subpartition type is Hash. </p></main> |
| `ADD COLUMN GROUP (column_list) [DELAYED]` | Adds a column group. This operation converts the table to a columnar table. The following table describes the syntax. <ul><li><code>ADD COLUMN GROUP(all columns, each column)</code>: Converts the table to a rowstore-columnar table. </li><li><code>ADD COLUMN GROUP(each column)</code>: Converts the table to a columnar table. </li><li>`DELAYED`: An optional parameter. If you specify this parameter, the conversion from a rowstore table to a columnar table is performed asynchronously. After the conversion is performed, the storage format in the table definition is modified. However, the conversion is executed during a major compaction. This operation does not block current DML operations, making it an online DDL operation. If you do not specify `DELAYED`, the default is to perform the conversion synchronously, making it an offline DDL operation. <main id="notice" type='notice'><h4>Notice</h4><p><ul><li>You can specify <code>DELAYED</code> only when you convert a rowstore table to a columnar table (<code>each column</code>) or a rowstore-columnar table (<code>all columns, each column</code>). </li><li>After you execute the <code>DELAYED</code> command to convert a rowstore table to a columnar table, the query performance may not meet expectations until the baseline data is merged. This is because the data storage format has not been converted. </li></ul></p></main></li></ul> |
...
| `PARTITION BY RANGE (...) opt_auto_split_tablet_size_option` | Sets the automatic splitting of the range partition. |
...
* `SPLIT PARTITION partition_name INTO (PARTITION split_partition_name VALUES LESS THAN (value) [, PARTITION split_partition_name VALUES LESS THAN (value) ...], PARTITION split_partition_name)`：Use this syntax to split a partition into multiple partitions. The `value` ranges defined for the new partitions must be the same as the `value` range of the source partition, and the `value` ranges must be defined in ascending order. The `value` of the last new partition must be different from that of the source partition.
...
* `PRECREATE_TIME = {'-1' | '0' | 'n {hour | day | week | month | year}'}`：Specifies the precreation time. When dynamic partition management is scheduled, partitions are precreated so that the **upper bound of the maximum partition > now() + precreate_time**. Valid values:
  ...
  * `n {hour | day | week | month | year}`: Partitions are precreated for the specified time span. For example, `3 hour` indicates that partitions are precreated for the past 3 hours.
  ...
  <main id="notice" type='explain'>
    <h4>Note</h4>
    <p><ul><li>When multiple partitions are precreated, the intervals between their boundaries are <code>TIME_UNIT</code>. </li><li>The boundary of the first precreated partition is the ceiling of the maximum existing partition boundary, based on <code>TIME_UNIT</code>. </li></ul></p>
  </main>

* `EXPIRE_TIME = {'-1' | '0' | 'n {hour | day | week | month | year}'}`：Optional. Specifies the expiration time of a partition. When dynamic partition management is scheduled, all **partitions whose upper bound < now() - expire_time** are deleted. Valid values:

  * `-1`: The default value. Partitions never expire.
  * `0`: All partitions except the current partition expire.
  * `n {hour | day | week | month | year}`: The expiration time of a partition. For example, `1 day` indicates that a partition expires after 1 day.
...
* Add the `p4` partition to the non-templated subpartitioned table `tbl3`. You must specify the definitions of the partition and the subpartitions under it.
...
* Add the `p3` partition to the templated subpartitioned table `tbl4`. You need only specify the definition of the partition. The definitions of the subpartitions are automatically filled in based on the template.
...
* Modify the column store attribute of the table.
  ...
  2. Change the `tbl1` table to a rowstore and columnstore redundant table, and then remove the rowstore and columnstore redundant attributes.
  ...
  3. Change the `tbl1` table to a columnstore table, and then remove the columnstore attribute.
...
* Modify the Skip Index attribute of a column in the table.
  ...
  4. Remove the Skip Index attribute of the `col1` column in the `test_skidx` table after the table is created.

     ```shell
     ALTER TABLE test_skidx MODIFY col1 NUMBER SKIP_INDEX();
     ```

* Modify the table attribute and run the following command to disable the bloom filter at the macroblock level for the `tb` table.
