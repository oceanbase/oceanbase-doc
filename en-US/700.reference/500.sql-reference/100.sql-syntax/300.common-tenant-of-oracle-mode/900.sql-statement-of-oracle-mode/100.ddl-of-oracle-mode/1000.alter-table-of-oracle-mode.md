...
| `alter_column_option` | Modifies column definitions. |
...
| `enable_macro_block_bloom_filter`| Specifies whether to persist the macro block-level bloom filter. Valid values:<ul><li><code>True</code>: enables persistence of the macro block-level bloom filter.</li></ul><ul></li></li><li><code>False</code>: disables persistence of the macro block-level bloom filter.</li></ul>|
| `DROP PARTITION partition_name` | Drops the specified partition. |
| `RENAME PARTITION old_name TO new_name` | Renames a partition. When renaming a partition, `new_name` specifies the new partition name (case-insensitive). Renaming a partition modifies the corresponding partition in the main table but does not affect the partition names in local indexes. You can use the `USER_TAB_PARTITIONS` and `USER_TAB_SUBPARTITIONS` views to confirm the results of the partition name modification. For more information, see [Rename a partition](../../../../../300.database-object-management/200.manage-object-of-oracle-mode/200.manage-partitions-of-oracle-mode/310.rename-a-partition-of-oracle-mode.md). During partition renaming, if a DML operation holds a lock on the related partition, the partition name modification operation is blocked until the DML operation releases the lock. |
| `SPLIT PARTITION partition_name split_actions` | Splits a partition. This statement is used to manually split a partition. It supports two formats: `split_at_format` and `split_into_format`. For more information, see [split_partition_option](#split_partition_option). |
...
| `PARTITION BY RANGE (...) opt_auto_split_tablet_size_option` | Sets automatic partition splitting for a range partition. |
...
| `DROP PRIMARY KEY` | Drops the primary key. <main id="notice" type='explain'><h4>Note</h4><p>In Oracle mode, you cannot drop the primary key of a table if it is the parent table of a foreign key constraint. </p></main> |
| `EXCHANGE {PARTITION partition_name \| SUBPARTITION subpartition_name} WITH TABLE origin_table_name INCLUDING INDEXES WITHOUT VALIDATION` | Specifies a partition exchange. The parameters are described as follows:<ul><li>`PARTITION partition_name`: specifies the name of the partition in the target partitioned table for the partition exchange.</li><li>`SUBPARTITION subpartition_name`: specifies the name of the subpartition in the target partitioned table for the partition exchange.</li><li>`origin_table_name`: specifies the name of the source table for the partition exchange, which is a non-partitioned table or a partitioned table without subpartitions.</li></ul> For more information about partition exchange, see [Partition exchange](../../../../../300.database-object-management/200.manage-object-of-oracle-mode/200.manage-partitions-of-oracle-mode/1000.exchange-partition-of-oracle-mode.md).|
| DYNAMIC_PARTITION_POLICY \[=\] \(dynamic_partition_policy_list\) | Modifies the dynamic partition management attribute of a table. `dynamic_partition_policy_list` specifies the list of configurable parameters for the dynamic partition strategy, with each parameter separated by an English comma. For more information about the parameters, see [dynamic_partition_policy_option](#dynamic_partition_policy_option).|
...
* `SPLIT PARTITION partition_name INTO (PARTITION split_partition_name VALUES LESS THAN (value) [, PARTITION split_partition_name VALUES LESS THAN (value) ...], PARTITION split_partition_name)`：Use this syntax to split a partition into multiple partitions. The `value` ranges defined in the `SPLIT PARTITION` clause must be the same as the `value` range of the source partition, and the `value` ranges must be defined in ascending order (you cannot define the last split partition with a `value` that is the same as the source partition).
...
* Add the `p4` partition to the non-templated subpartitioned table `tbl3`. You must specify the definitions of the partition and its subpartitions.
...
* Rename the partitions.
  ...
  /* Rename the partition, but the modification does not affect the names of the partitions of the local index. */
  ```
...
* Modify the column storage attribute of the table.
...
* Modify the Skip Index attribute of a column in the table.
  ...
  1. Use the following SQL statement to create the `test_skidx` table.
...
* Modify the table attribute, and execute the following command to disable the bloom filter at the macroblock level for the `tb` table.
...
<h4>Notice</h4>
<p>Users with the <code>SUPER</code> privilege cannot successfully perform the related operations. We recommend that you use a regular user. </p>
</main>
