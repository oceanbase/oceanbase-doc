...
| `opt_alter_compress_option` | Modifies the compression option for the table. |
| `alter_column_option` | Modifies the column definition. |
| `alter_tablegroup_option` | Modifies the table group option. |
...
| `RENAME PARTITION old_name TO new_name` | Renames the partition. When renaming a partition or a subpartition, `new_name` specifies the new partition name (case-insensitive). The partition renaming operation modifies the corresponding partitions in the main table but does not affect the partition names of local indexes. You can check the `USER_TAB_PARTITIONS` and `USER_TAB_SUBPARTITIONS` views to confirm the results after the partition name is modified. For more information, see [Rename a partition](../../../../../300.database-object-management/200.manage-object-of-oracle-mode/200.manage-partitions-of-oracle-mode/310.rename-a-partition-of-oracle-mode.md). During the partition renaming process, if a DML operation holds a lock on the partition, the partition name modification will be blocked until the DML operation releases the lock. |
| `SPLIT PARTITION partition_name split_actions` | Splits the partition. This operation is used to manually split a partition. It supports two formats: `split_at_format` and `split_into_format`. For more information, see [split_partition_option](#split_partition_option). |
...
| `MODIFY CONSTRAINT` | Modifies the status of the constraint to enabled or disabled. This operation supports only foreign key constraints and `CHECK` constraints. |
| `DROP PRIMARY KEY` | Drops the primary key.<main id="notice" type='explain'><h4>Note</h4><p>For Oracle mode, you cannot drop the primary key of a table that is a parent table of a foreign key. </p></main> |
| `EXCHANGE {PARTITION partition_name \| SUBPARTITION subpartition_name} WITH TABLE origin_table_name INCLUDING INDEXES WITHOUT VALIDATION` | Specifies the partition exchange. <ul><li>`PARTITION partition_name`: specifies the name of the target partition in the partition exchange. </li><li>`SUBPARTITION subpartition_name`: specifies the name of the target subpartition in the partition exchange. </li><li>`origin_table_name`: specifies the name of the source table in the partition exchange. The source table must be a non-partitioned table or a partitioned table. </li></ul> For more information about partition exchange, see [Partition exchange](../../../../../300.database-object-management/200.manage-object-of-oracle-mode/200.manage-partitions-of-oracle-mode/1000.exchange-partition-of-oracle-mode.md). |
| DYNAMIC_PARTITION_POLICY \[=\] $dynamic_partition_policy_list$ | Modifies the dynamic partition management attribute of a table. `dynamic_partition_policy_list` specifies the configurable parameters of the dynamic partition strategy. The parameters are separated by commas. For more information, see [dynamic_partition_policy_option](#dynamic_partition_policy_option) below. |
| SET INTERVAL(expr) | Converts a RANGE partitioned table to an INTERVAL partitioned table or vice versa. For more information, see [Modify the partitioning rule](../../../../../300.database-object-management/200.manage-object-of-oracle-mode/200.manage-partitions-of-oracle-mode/300.modify-partition-rules-of-oracle-mode.md) in the Modify the partition type section. |
|COLUMN_NAME_CASE_SENSITIVE [=] {True \| False} | Specifies whether to enable case-sensitive column names for generated columns. <ul><li><code>True</code>: enables case-sensitive column names. </li><li><code>False</code>: disables case-sensitive column names. <main id="notice" type='explain'><h4>Note</h4><p> In versions earlier than V4.5.0, the default is to enable case-sensitive column names. In V4.5.0 and later, the default is to disable case-sensitive column names. </p></main>|
...
* `ENABLE = {true | false}`: specifies whether to enable dynamic partition management. Valid values:
  ...
  * `true`: the default value, which indicates that dynamic partition management is enabled.
...
* `PRECREATE_TIME = {'-1' | '0' | 'n {hour | day | week | month | year}'}`: specifies the time to precreate partitions. When dynamic partition management is scheduled, partitions are precreated so that the upper bound of the latest partition is greater than the current time plus the precreate time. Valid values:
  ...
  * `-1`: the default value, which indicates that no partitions are precreated.
  ...
  * `n {hour | day | week | month | year}`: indicates the time span for precreating partitions. For example, `3 hour` indicates that partitions are precreated for the last 3 hours.
  ...
  <main id="notice" type='explain'>
    <h4>Note</h4>
    <p><ul><li>When multiple partitions are to be precreated, the interval between the boundaries of adjacent partitions is <code>TIME_UNIT</code>. </li><li>The boundary of the first precreated partition is the smallest integer greater than or equal to the upper bound of the latest existing partition, rounded up to the nearest multiple of <code>TIME_UNIT</code>. </li></ul></p>
  </main>

* `EXPIRE_TIME = {'-1' | '0' | 'n {hour | day | week | month | year}'}`: specifies the expiration time of a partition. When dynamic partition management is scheduled, all partitions whose upper bounds are earlier than the current time minus the expiration time are deleted. Valid values:

  * `-1`: the default value, which indicates that partitions never expire.
  * `0`: indicates that all partitions except the current one are expired.
  * `n {hour | day | week | month | year}`: indicates the expiration time of a partition. For example, `1 day` indicates that the expiration time of a partition is 1 day.
...
* Modify the data type of the `col1` column in the `tbl1` table.
...
* Add the `p3` partition to the templated subpartitioned table `tbl4`. You only need to specify the partition definition. The subpartition definitions are automatically filled in based on the template.
...
* Modify the primary key of the `tbl9` table to the `col2` column.
...
* Rename a partition and a subpartition.
  ...
  /* Rename a partition. The modification does not affect the names of the partitions of the local index. */
  ```
  ...
  /* Rename a subpartition. The modification does not affect the names of the subpartitions of the local index. */
...
* Modify the Skip Index attribute of a column in the table.
  ...
  4. Delete the Skip Index attribute of the `col1` column in the `test_skidx` table after the table is created.

     ```shell
     ALTER TABLE test_skidx MODIFY col1 NUMBER SKIP_INDEX();
     ```

* Modify the table attribute and run the following command to disable the persistent macroblock-level Bloom filter for the `tb` table.
