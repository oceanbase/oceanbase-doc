...
| DROP COLUMN GROUP([all columns,]each column) | Drops the columnstore attribute of a table. The following options are supported:<ul><li><code>DROP COLUMN GROUP(all columns, each column)</code>: drops the rowstore-columnstore redundancy attribute of the table. </li><li><code>DROP COLUMN GROUP(each column)</code>: drops the columnstore attribute of the table. </li></ul> |
