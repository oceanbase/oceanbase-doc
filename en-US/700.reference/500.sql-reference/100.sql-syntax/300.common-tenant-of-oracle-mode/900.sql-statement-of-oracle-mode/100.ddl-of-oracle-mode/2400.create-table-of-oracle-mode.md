...
| comment | The comment. The maximum length of a comment is 4,000 bytes. The comment is case-insensitive.  |
...
Data may not be fully cleaned up due to a fault. Currently, OceanBase Database does not support automatic cleanup in this case. Generally, this problem does not affect the use of the database. If there is too much residual data, you can drop the temporary table and rebuild it.
