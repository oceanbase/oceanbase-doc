...
| URL = 'file_path' | Specifies the external storage path. Supported storage types include: local file, HDFS, OSS, and S3. For more information, see [file_path](#file_path). |
...
| CONFIGS       | Optional. Specifies additional Hadoop client configuration parameters in the `key=value` format, separated by `#`, when the target HDFS cluster is configured with Kerberos. For more information, see [configs_option](#configs_option). |
...
* Create a Location object with the authentication mode set to user authentication.
    ...
    <main id="notice" type='explain'>
      <h4>Note</h4>
      <p>This applies to clusters without Kerberos authentication enabled, but where specific HDFS paths require access by a specific user.</p>
    </main>
