...
| FAILED_LOGIN_ATTEMPTS | The number of consecutive failed login attempts of a user account.  |
...
* Modify the number of failed login attempts and the password locking period. Specifically, set `FAILED_LOGIN_ATTEMPTS` to `5` and `PASSWORD_LOCK_TIME` to `1` in the profile named `profile1`.
