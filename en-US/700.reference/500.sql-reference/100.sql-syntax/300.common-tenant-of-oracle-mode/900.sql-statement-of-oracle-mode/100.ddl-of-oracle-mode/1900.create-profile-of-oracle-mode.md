...
| PASSWORD_ROLLOVER_TIME | Specifies the password rotation validity period in microseconds. During the rotation validity period, both the old and new passwords can be used for authentication.<main id="notice" type="explain"><h4>Note</h4><p>This parameter is supported starting from V4.6.1.</p></main> |
...
Create a password-restricted profile named `profile1`, and set `FAILED_LOGIN_ATTEMPTS` to `5`, `PASSWORD_LOCK_TIME` to `1` day, `PASSWORD_LIFE_TIME` to `60` days, `PASSWORD_VERIFY_FUNCTION` to `NULL`, `PASSWORD_GRACE_TIME` to the default limit, and `PASSWORD_ROLLOVER_TIME` to `86400000000` microseconds (example value).
