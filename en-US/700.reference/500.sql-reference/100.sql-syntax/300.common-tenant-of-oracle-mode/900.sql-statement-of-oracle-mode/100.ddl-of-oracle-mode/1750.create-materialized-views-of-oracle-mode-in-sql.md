...
| view_select_stmt      | Specifies the query (`SELECT`) statement for the data in the materialized view. This statement retrieves data from the base tables (which can be regular tables or materialized views) and stores the results in the materialized view. The syntax of `view_select_stmt` is the same as a standard `SELECT` statement. For syntax details, see [SIMPLE SELECT](../200.dml-of-oracle-mode/500.select-of-oracle-mode/100.simple-select-of-oracle-mode.md).  <main id="notice" type='explain'><h4>Note</h4><p>For OceanBase Database V4.3.5, you can use external tables as base tables when creating full-refresh materialized views starting from V4.3.5 BP2.</p></main> |
