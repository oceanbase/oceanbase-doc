...
| Value (used in SQL statements) | Actual encryption algorithm | Key length | Mode | Description |
|---|----------|----------|------|------|
...
* The `DISABLE` clause temporarily disables the sensitive rule but does not delete it. By default, a sensitive rule is enabled after it is created.

When you modify a sensitive rule, note the following restrictions:

* The columns added by using the `ADD COLUMN` clause cannot be protected by any existing rule.
* The columns to be dropped by using the `DROP COLUMN` clause must be protected by the specified rule.
