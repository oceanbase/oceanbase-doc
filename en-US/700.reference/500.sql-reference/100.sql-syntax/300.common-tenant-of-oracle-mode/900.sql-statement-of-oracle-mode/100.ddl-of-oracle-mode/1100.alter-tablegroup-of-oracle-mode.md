...
| schema_name.      | Specifies the schema to which the table belongs. This parameter is optional. If you do not specify `schema_name`, the table to be added belongs to the current schema by default. |
| table_name        | The name of the table to be added to the table group. If multiple tables are to be added, separate the tables with a comma (,). When multiple tables are added, the table names can be duplicate. If a table to be added already exists in the table group, OceanBase Database does not return an error. |
