...
<main id="notice" type='explain'>
  ...
  <ul>
    ...
    <li>You must enable the audit feature by setting the system parameter audit_trail. Otherwise, the audit rules configured by this statement will not take effect.</li>
    </ul>
  </main>
...
To unlock the `ORAAUDITOR` user, execute the following statement:
...
|------|------|
| statement_operation_list | A list of SQL statement operations to be audited. |
| ALL | Audits all auditable statements. |
| ALL STATEMENTS | Audits all statements. |
| user_name | A list of usernames to be audited, separated by commas. |
...
| object_operation_list | A list of object operations to be audited. |
...
| NOT EXISTS   | Audits operations that fail due to the absence of the object. |
...
| SELECT SEQUENCE   | Audits all statements containing `sequence.CURRVAL` or `sequence.NEXTVAL`. |
...
2. Audit the SELECT table operations of user `SYS`:

   ```sql
   obclient> AUDIT SELECT TABLE BY SYS BY ACCESS;
   ```

3. Audit only the failed table operations of user `SYS`:
...
1. Disable the audit for the table operations of user `SYS`:

   ```sql
   obclient> NOAUDIT TABLE BY SYS;
   ```

2. Disable the audit for the SELECT table operations of user `SYS`:
...
1. View the statement audit configurations:

   ```sql
   obclient> SELECT * FROM DBA_STMT_AUDIT_OPTS;
   ```

2. View the object audit configurations:
...
1. Enable system operation audit:
