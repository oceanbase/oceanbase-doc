...
|-----------------|------------------------------------------------------------|
| session_id      | The client session ID of the current session, which is the unique identifier of the session in the client. <main id="notice" type='explain'><h4>Note</h4><p> You can execute the <code>SHOW PROCESSLIST;</code> SQL statement to query session IDs in the database. </p></main> |
