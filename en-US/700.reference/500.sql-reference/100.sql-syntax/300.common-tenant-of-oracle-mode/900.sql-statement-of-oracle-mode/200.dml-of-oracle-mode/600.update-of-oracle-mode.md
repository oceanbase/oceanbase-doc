...
| dml_table_clause | Specifies the name of the table, view, or column returned by a special subquery to be updated.<br> **Note**  If you specify `view`, the database updates the base table of the view. However, you cannot update multiple base tables through a view. |
...
| ref_name         | The name of the variable.                                                                                                        |
  ...
  <main id="notice" type='notice'>
    <h4>Notice</h4>
    <p>A special subquery refers to a subquery similar to the one corresponding to an updatable view. This type of subquery should not contain complex operators, such as <code>GROUP BY</code>, <code>DISTINCT</code>, and <code>WINDOW FUNCTION</code>. </p>
  </main>
...
* Update a single table: Modify the `col2` column value of the row in the `tbl1` table where `tbl1.col1=1` to `100`.
...
* Update a single table: Modify the `col2` column value of the row in the subquery where `v.col1=1` to `10`.
...
* Update a single table: Modify the `col2` column value of the rows in the `tbl1` table where `tbl1.col1<3` to `100`.
...
* Update a single table: Use the `RETURNING` clause to return the modified data.
...
3. Perform a table update operation using variables.
   ...
   ```shell
    ...
    BEGIN
        -- Get data from employees1 and store it in the variable
        SELECT * INTO var_emp
        FROM employees1
        WHERE id = 1;
