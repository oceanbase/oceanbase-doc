...
* If you specify the partitions of the target table, the update operation affects only the data in the target table that belongs to the specified partitions.

* If you specify the partitions of the target table and the source table, the matching operation matches the data in the specified partitions of the target table with that in the specified partitions of the source table.

* If no matching data is found in the target table and source table, the update operation is not performed.
...
| PARTITION(partition_name_list) | Optional. The list of partition names of the target or source table. Separate multiple partition names with commas (,).<main id="notice" type='explain'><h4>Note</h4><p>In OceanBase Database V4.3.5, specifying partitions in the <code>MERGE INTO</code> statement is supported in the target and source tables (using the USING clause) since BP2 of V4.3.5. </p></main>|
...
code-placeholder/0ad671b4-574a-4476-8317-e454aae6d640

  code-placeholder/0ad671b4-574a-4476-8317-e454aae6d640

      * Update data in partitions.
  ...
  ```sql
      ...
      3. Query data in partition `p0` of `test_tbl3`.
  ...
  The return result is as follows:
      ...
      4. Query data in partitions `p0` and `p1` of `test_tbl4`.
  ...
  5. Match the data in partitions `p0` and `p1` of `test_tbl4` with that in partition `p0` of `test_tbl3`. If the data in the `col1` columns of the two tables is the same, update the values in the `col2` column of `test_tbl3` to 10000.
