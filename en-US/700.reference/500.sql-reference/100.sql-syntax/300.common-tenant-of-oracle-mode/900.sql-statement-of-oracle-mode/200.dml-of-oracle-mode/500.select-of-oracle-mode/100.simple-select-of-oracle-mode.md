...
| WHERE where_conditions | Optional. The filter conditions. Only data that meets the conditions is returned in the query results. `where_conditions` is an expression.  |
| GROUP BY group_by_list | Groups data by one or multiple columns. It is usually used together with aggregate functions.  <br>**Note**: If no column following the `SELECT` clause uses any aggregate function, in a GROUP BY query, the columns following the `SELECT` clause must be specified in the `GROUP BY` clause.  |
