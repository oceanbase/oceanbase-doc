...
| GROUP BY group_by_list | The grouping field, which is usually used together with aggregate functions.  <br>**Note**: If no column following the `SELECT` clause uses any aggregate function, in a GROUP BY query, the columns following the `SELECT` clause must be specified in the `GROUP BY` clause.  |
