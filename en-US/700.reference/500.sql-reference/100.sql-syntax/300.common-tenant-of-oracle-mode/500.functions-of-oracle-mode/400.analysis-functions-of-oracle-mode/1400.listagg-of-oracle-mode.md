...
This function is used to convert rows to columns. The `LISTAGG` function sorts the data in each group specified by the `ORDER BY` clause and then concatenates the values of the measure column. It can be used as an aggregate or analytic function.
  ...
  <main id="notice" type='explain'>
    <h4>Note</h4>
    <ul>
    <li>When used as an analytic function, you must use the <code>OVER</code> clause to define the window for calculation. This function partitions the query result set into groups based on one or more expressions in the <code>query_partition_clause</code>.</li>
