...
|slug|sdo-geometry-format-conversion-functions-oracle-mode|

# SDO_GEOMETRY format conversion functions

The Oracle-compatible mode of OceanBase Database allows you to use the `GET_WKB()`, `GET_WKT()`, or `GET_GEOJSON()` member function of `SDO_GEOMETRY` to convert a `SDO_GEOMETRY` spatial object into the well-known binary (WKB), well-known text (WKT), or JavaScript Object Notation (JSON) format.
