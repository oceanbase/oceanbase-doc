...
| AS tag | Specifies the tag name.<br> If the expression is a column name, the AS clause is optional. The default tag name is the column name in uppercase letters.<br> If the expression is not a column name (e.g., an aggregate function, a literal, or the concatenation of two columns), the AS clause is required. |
| tag | The tag name, which must conform to XML naming standards. It can be specified as a string. <code>EVALNAME</code> followed by an expression that evaluates to a string literal. |
