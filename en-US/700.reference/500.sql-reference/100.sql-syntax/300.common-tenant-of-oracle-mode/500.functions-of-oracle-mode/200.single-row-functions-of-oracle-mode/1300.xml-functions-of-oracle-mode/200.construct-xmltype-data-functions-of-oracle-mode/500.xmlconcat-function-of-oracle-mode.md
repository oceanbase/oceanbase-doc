| Description   |                 |
|---------------|-----------------|
| machine-translation ||
| keywords      |                 |
| dir-name      |                 |
| dir-name-en   |                 |
| tenant-type   | Oracle Mode     |
|slug|xmlconcat-function-oracle-mode|

# XMLCONCAT

## Purpose

The `XMLCONCAT` function concatenates a sequence of `XMLType` instances and returns the concatenated `XMLType` sequence.

## Syntax

```sql
XMLCONCAT (XMLType_instance [, XMLType_instance ]...)
```

## Parameters

| Parameter | Description |
| --- | --- |
| XMLType_instance | One or more XML instances to be concatenated. |

## Return type

The concatenated `XMLType` sequence.

## Examples

```sql
# Concatenate two XMLType instances.
obclient> select XMLCONCAT(xmltype('<testinfo a="0"> <e1>111</e1> </testinfo>'), xmltype('<testinfo a="1"> <e1>111</e1> </testinfo>')) from dual;

<testinfo a="0"> <e1>111</e1> </testinfo><testinfo a="1"> <e1>111</e1> </testinfo>
```