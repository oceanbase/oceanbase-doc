...
|slug|uniform-oracle-compatible-mode|
...
* In Oracle-compatible mode, `min` and `max` are treated as the `NUMBER` type, regardless of whether they are integers. The output of the `UNIFORM()` function is a floating-point number. To generate an integer, you can use `UNIFORM()` together with the `FLOOR()` function. 
