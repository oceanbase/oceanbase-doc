...
- `FORMAT JSON` clause: Optional. Required when the data type of `expr` is `BLOB`.
- `json_path_literal`: Required. Specifies the path for the query. You can use a `json_path` statement with a filter condition here.
- `ERROR` clause:
...
- `EMPTY` clause: Returns `FALSE` if there are no results that are `TRUE`.
- `PASSING` clause: Passes external SQL variables to `json_path` for comparison with values at the path specified in the filter expression.
