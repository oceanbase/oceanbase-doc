...
- `PRETTY`: Specifies whether to use pretty-print for character-type output.
...
- `WITHOUT WRAPPER`: Ignores the `ARRAY WRAPPER` feature.

- `WITH WRAPPER`: Uses the `ARRAY WRAPPER` feature.

- `WITH UNCONDITIONAL WRAPPER`: Equivalent to `WITH WRAPPER`.

- `WITH CONDITIONAL WRAPPER`: Uses the `ARRAY WRAPPER` feature when multiple values are returned. If only one value is returned, the `ARRAY WRAPPER` feature is ignored.
...
- `EMPTY` clause:
  ...
  - `NULL ON EMPTY`: Returns `NULL` when no matching items are found. This is the default setting.
  - `ERROR ON EMPTY`: Returns the corresponding Oracle error when no matching items are found.
  - `EMPTY ON EMPTY`: Equivalent to specifying `EMPTY ARRAY ON EMPTY`.
  - `EMPTY ARRAY ON EMPTY`: Returns an empty JSON array when no matching items are found.
  - `EMPTY OBJECT ON EMPTY`: Returns an empty JSON object when no matching items are found.
...
# Use the WRAPPER clause and ERROR clause.
