...
The `JSON_OBJECTAGG()` function takes input parameters as key-value pairs, where both the `key` and `value` are list expressions, and combines all the key-value pairs into a single JSON object, which is then returned.
...
- `NULL|ABSENT ON NULL`: Specifies that the clause is only effective when the parameter expression evaluates to `NULL`.
