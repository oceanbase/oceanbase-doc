...
The `JSON_OBJECTAGG()` function takes input parameters as key-value pairs, where both the `key` and `value` are expressions. It combines all key-value pairs into a single JSON object and returns that object.
...
The syntax for the `JSON_OBJECT()` function is as follows:

- `WITH UNIQUE KEYS`: Ensures that the result does not contain duplicate keys.
- `NULL|ABSENT ON NULL`: Specifies that this clause takes effect only when the parameter expression evaluates to `NULL`.
  - `NULL ON NULL`: Uses the JSON `NULL` value as the current array element.
  - `ABSENT ON NULL`: Ignores the array element.
- `STRICT`: Validates that the output is a valid JSON-formatted data.
