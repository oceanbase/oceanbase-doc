...
The `JSON_TABLE()` function provides a way to link relational tables with JSON structured data. You can use the output of `JSON_TABLE()` in SQL just like a virtual relational table. `JSON_TABLE()` provides column output for each JSON value and multi-row (column) output for arrays within the JSON data.
...
- The `FOR ORDINALITY` clause provides a row number for the current output row, which is an `INT` column. The output value types are four in total. Except for the `ORDINALITY` column, the other three types are similar in functionality and behavior to `JSON_VALUE()`, `JSON_QUERY()`, and `JSON_EXISTS()`.
- The `[NESTED PATH ...]` clause is a repeated nested structure.
- The `ERROR` clause specifies the error handling for the initial input `expr`.
