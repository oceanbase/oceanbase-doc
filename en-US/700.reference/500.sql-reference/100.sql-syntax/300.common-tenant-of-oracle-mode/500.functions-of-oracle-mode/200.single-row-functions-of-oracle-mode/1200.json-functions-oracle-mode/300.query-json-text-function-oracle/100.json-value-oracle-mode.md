...
- `[RETURNING NUMBER | DATE | TIMESTAMP | VARCHAR2 | VARCHAR2[size]` : Optional. Specifies the return value type. `VARCHAR2[size]` specifies the maximum length of the return value. The default value is `VARCHAR2(4000)`.
...
- `EMPTY` : Specifies the processed result when a null value is found during the search.
