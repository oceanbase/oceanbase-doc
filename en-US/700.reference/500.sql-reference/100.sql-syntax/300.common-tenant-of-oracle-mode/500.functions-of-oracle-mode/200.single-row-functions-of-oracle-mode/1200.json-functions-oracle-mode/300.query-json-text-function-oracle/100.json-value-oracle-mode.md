...
The `JSON_VALUE()` function is used to query specific parts of JSON data and returns an SQL value.
...
- `ERROR` : Controls the output of errors other than type conversion errors during expression execution and returns the processed result.
- `EMPTY` : Specifies the processed result when a null value is encountered during the search.
