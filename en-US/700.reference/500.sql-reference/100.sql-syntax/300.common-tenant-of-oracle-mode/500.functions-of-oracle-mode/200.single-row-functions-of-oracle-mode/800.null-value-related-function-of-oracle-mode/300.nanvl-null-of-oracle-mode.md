...
* If `n1` is `NaN`, it returns the value of parameter `n2`.

* If `n1` is not `NaN`, it returns the value of parameter `n1`.

  <main id="notice" type='notice'>
    <h4>Notice</h4>
    <p>This function applies only to the <code>BINARY_FLOAT</code> or <code>BINARY_DOUBLE</code> data types. </p>
  </main>
...
* If any parameter is of the `BINARY_DOUBLE` data type, the return type is `BINARY_DOUBLE`.

* If all parameters are of the `BINARY_FLOAT` data type, the return type is `BINARY_FLOAT`.

* If any parameter is `NULL`, the return value is `NULL`.

## Examples

* Create a table named `tbl1` and insert test data. If the value in column `col2` is numeric, it returns the original value; otherwise, it returns `0`.
