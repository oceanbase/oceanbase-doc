...
| Function name | Feature description |
| -------- | ---------- |
| SYS_CONNECT_BY_PATH | Returns the path of column values from the root to the node, consisting of`CONNECT BY`The column values for each row returned under the condition are separated by a specified delimiter. |
