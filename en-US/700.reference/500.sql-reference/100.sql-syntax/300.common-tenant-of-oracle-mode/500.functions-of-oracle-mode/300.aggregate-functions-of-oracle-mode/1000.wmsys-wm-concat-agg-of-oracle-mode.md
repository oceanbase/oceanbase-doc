...
|slug|wmsys-wm-concat-agg-oracle-mode|
...
This function is used to operate on values in a set of rows, sort the rows according to specified rules, and return the value that is either the first or last in the sorted set. It can be used as an aggregate or analytic function.
  ...
  <main id="notice" type='explain'>
    <h4>Note</h4>
    <ul>
    <li>When used as an analytic function, you must use the <code>OVER</code> clause to define the window for computation. This function partitions the query result set into groups based on one or more expressions specified in <code>query_partition_clause</code>.</li>
...
| expr                    | Any expression.                                                                                                                                                                                                            |
| FIRST \| LAST           | Specifies the position of the value to return after sorting `expr`. <ul><li> `FIRST` specifies the value at the top of the sorted list.   </li><li> `LAST` specifies the value at the bottom of the sorted list.</li></ul>                                               |
...
| NULLS { FIRST \| LAST } | Specifies the position of `NULL` values in the sorted list. This is an optional parameter. <ul><li> `NULLS FIRST` specifies that `NULL` values appear before non-`NULL` values.  </li><li> `NULLS LAST` specifies that `NULL` values appear after non-`NULL` values, which is the default.</li></ul>    |
