...
- `GROUP_ID` cannot be used in the `WHERE` clause, join conditions, inside aggregate functions, or in the definition of grouping attributes.
...
The `GROUP_ID()` function is used to identify duplicate records returned by the `GROUP BY` clause. For example, for two identical aggregated records with `c1=1`, `GROUP_ID()` returns 0 and 1 respectively.
