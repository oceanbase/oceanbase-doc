...
|slug|variance-agg-oracle-compatible-mode|
...
|---------------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| DISTINCT \| UNIQUE \| ALL | Specifies whether to remove duplicates during the execution of the query. This parameter is optional. Default value: `ALL`. <ul><li> `ALL`: specifies to accept duplicate values and ignore null values in the data.   </li><li> `DISTINCT`: specifies to remove duplicates and ignore null values in the data.   </li><li> `UNIQUE`: specifies to remove duplicates and ignore null values in the data. </li></ul> |
...
| OVER                      | You can use the `OVER` clause to define a window over the data on which the function operates. For more information, see [Analytic functions](../400.analysis-functions-of-oracle-mode/100.window-function-description-of-oracle-mode.md).                                                                                                                                                                                                          |
  ...
  <main id="notice" type='notice'>
    <h4>Notice</h4>
    <p>If you specify the <code>DISTINCT</code> or <code>UNIQUE</code> keyword, <code>analytic_clause</code> cannot contain <code>order_by_clause</code> or <code>windowing_clause</code>. </p>
  </main>
...
Assume that the table `employees` has been created.
