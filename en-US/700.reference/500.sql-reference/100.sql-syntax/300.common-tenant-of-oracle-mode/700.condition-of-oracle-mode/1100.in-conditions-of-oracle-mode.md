...
* `IN` is equivalent to `=ANY`, indicating all members in the set. Here is an example:
...
* `NOT IN` is equivalent to `!= ALL`. If any member in the set is `NULL`, the evaluation result is `FALSE`. Here is an example:
  ...
  If the evaluation result of any item in the list following the `NOT IN` operation is `NULL`, the evaluation results of all rows are `FALSE` or `UNKNOWN`, and no row is returned.
