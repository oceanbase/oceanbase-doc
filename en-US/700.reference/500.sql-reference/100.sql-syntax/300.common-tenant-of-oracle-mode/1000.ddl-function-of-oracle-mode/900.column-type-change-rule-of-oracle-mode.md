...
When you change the data type of a column in the Oracle-compatible mode of OceanBase Database, you must first follow the table-level column type change rules, especially in scenarios where some data types are forbidden, and then follow the change rules of the source data type and target data type.
...
In the Oracle-compatible mode of OceanBase Database, conversions among data types of different categories are not allowed. The following sections describe the conversion between data types of the same category.
