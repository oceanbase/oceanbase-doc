...
The following table describes the online DDL operations supported by the Oracle-compatible mode of OceanBase Database V4.x.
...
| Index operation | Hybrid index operations | Related to the operation with the longest execution time | For example, the syntax `ALTER TABLE t1 ADD INDEX i4(c1), DROP INDEX i2, RENAME INDEX i1 TO i1x` is not supported in Oracle-compatible mode.  |
...
| Column operation | Drop a column | Only for metadata modification | N/A |
...
The following table describes the offline DDL operations supported by OceanBase Database V4.x in Oracle-compatible mode.
...
| Column operation | Clear an unused column | Related to the data volume, because data is reorganized | N/A |
