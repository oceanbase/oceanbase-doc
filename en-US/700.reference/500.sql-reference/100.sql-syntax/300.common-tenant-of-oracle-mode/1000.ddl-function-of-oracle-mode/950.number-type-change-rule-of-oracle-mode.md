...
| Storage type | Default storage rule for syntax in MySQL-compatible mode | Default storage rule for syntax in Oracle-compatible mode | Description |
| --- | --- | --- | --- |
...
| Floating-point NUMBER | Not supported | NUMBER | In Oracle-compatible mode of OceanBase Database, floating-point `NUMBER` indicates floating-point numbers with a precision of 38 bits and must not be a subset of `NUMBER(M, D)`. If and only if `M-D <= 38`, `NUMBER(M, D)` is its subset.  |
...
<main id="notice" type='notice'>
  <h4>Notice</h4>
  <ul><li><p>Floating-point <code>NUMBER</code> and Oracle <code>FLOAT</code> exist only in Oracle-compatible mode, while unsigned fixed-point <code>DECIMAL(M,D)</code> exists only in MySQL-compatible mode. </p></li>
  <li><p>In Oracle-compatible mode, not allowing instant conversion (that modifies only the schema but does not convert data) means that conversion is not supported for non-empty tables. In MySQL-compatible mode, not allowing instant conversion means that data must be rewritten. Even if instant conversion is allowed in MySQL-compatible mode, the database does not ensure that data of all source types can be inserted into the destination type without loss. For example, lossless conversion cannot be guaranteed if you convert fixed-point numbers to Oracle <code>FLOAT</code> numbers. </p></li></ul>
</main>
