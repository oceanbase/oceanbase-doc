...
A NULL in an SQL function means that an argument of the SQL function is NULL. Given a NULL argument, most scalar functions return `NULL`, while aggregate functions (except for `COUNT()` and `GROUPING()`) ignore the `NULL`.

When you use the `ORDER BY` clause to sort data, NULLs come after other values in ascending order and before other values in descending order.
...
NULLs may appear in comparison conditions. To test for NULLs, you can only use the `IS NULL` and `IS NOT NULL` operators. `NULL` means a lack of data, so a `NULL` value cannot be equal to, unequal to, greater than, or less than any other value. It cannot be compared with any values, including itself.
