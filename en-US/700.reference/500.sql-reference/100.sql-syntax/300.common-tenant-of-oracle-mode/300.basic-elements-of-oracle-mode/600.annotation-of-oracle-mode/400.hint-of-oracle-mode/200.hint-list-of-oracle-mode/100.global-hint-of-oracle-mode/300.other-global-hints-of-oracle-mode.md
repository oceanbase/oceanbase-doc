...
Hints under the Global hint type allow developers and database administrators to fine-tune the execution of SQL statements. The following table describes other hint types.

| Hint name                               | Description                            |
|---------------------------------------|-------------------------------|
| `APPEND`                              | Collect statistics during INSERT operations and enable direct load mode (`DIRECT INSERT`) in `INSERT INTO SELECT`.  |
...
| `DISABLE_PARALLEL_DAS_DML`                 | Disables Distributed Data Access Service (DAS) concurrency optimization for the current DML statement.                             |
...
In this example, `direct(true, 0, 'full')` enables full direct load, sorts the data, and allows a maximum of 0 error rows; `enable_parallel_dml` enables parallel DML; and `PARALLEL(4)` sets the parallelism level to 4.
...
The following table describes the parameters of the `DYNAMIC_SAMPLING` hint:
...
code-placeholder-4847d6bc-7c88-49b4-aa41-951ce01df155
