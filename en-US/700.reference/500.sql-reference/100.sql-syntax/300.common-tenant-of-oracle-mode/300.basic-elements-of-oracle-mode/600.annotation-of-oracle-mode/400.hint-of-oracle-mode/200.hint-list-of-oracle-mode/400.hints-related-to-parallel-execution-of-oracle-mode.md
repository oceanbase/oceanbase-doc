...
The `PARALLEL` hint is a statement-level hint that instructs the optimizer to use the specified number of threads for parallel execution. The hint overrides the value of `PARALLEL_DEGREE_ POLICY`. The hint can be used as a part of the `SELECT`, `INSERT`, `UPDATE`, and `DELETE` statements, or as a part of a table scan. The `PARALLEL` hint is ignored if any parallel limit is violated.
...
The `USE_PX` hint forces the server to execute SQL statements in parallel execution mode. In parallel execution mode, an SQL statement can be executed by using multiple threads to improve the query performance. Generally, the `USE_PX` and `PARALLEL` hints are used to jointly specify the number of threads used for parallel execution. By default, the `USE_PX` hint is used.
