...
Since there is no direct place to add hints for set operations, the hints described in this section must be specified using qb name to indicate the set operation object they apply to. For more information, see the examples for each hint.
...
The following query example shows how to use the `pq_set(@set$1 local local)` hint to specify that data is directly pulled back to the local node in distributed execution, the `pq_set(@set$1 sel$2 hash hash)` hint to specify that data is redistributed using the `HASH` method for set operations, and `sel$2` as the left operand of the set operation.

```sql
-- Use the PQ_SET hint to specify the distributed execution method for set operations.
...
The `USE_HASH_SET` hint specifies the algorithm to be used for set operations. The reverse of this hint is `NO_USE_HASH_SET`.
