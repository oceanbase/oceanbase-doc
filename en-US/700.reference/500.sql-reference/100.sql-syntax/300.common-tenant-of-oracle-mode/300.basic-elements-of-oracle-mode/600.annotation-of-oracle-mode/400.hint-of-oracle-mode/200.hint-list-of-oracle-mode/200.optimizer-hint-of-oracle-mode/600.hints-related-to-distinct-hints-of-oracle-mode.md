...
|-----------------------------|--------------------------------------------------------|
| `USE_HASH_DISTINCT`           |  Enables the optimizer to use the `HASH` algorithm to execute the `DISTINCT` operation. Its reverse operation is the `NO_USE_HASH_DISTINCT` hint.  |
| `NO_USE_HASH_DISTINCT`        |  Disables the optimizer from using the `HASH` algorithm when executing the `DISTINCT` operation. Its reverse operation is the `USE_HASH_DISTINCT` hint.  |
| `DISTINCT_PUSHDOWN`           |  Indicates that the optimizer can perform `DISTINCT` pushdown in a distributed environment. Its reverse operation is the `NO_DISTINCT_PUSHDOWN` hint.  |
| `NO_DISTINCT_PUSHDOWN`        |  Indicates that the optimizer cannot perform `DISTINCT` pushdown in a distributed environment. Its reverse operation is the `DISTINCT_PUSHDOWN` hint.  |
