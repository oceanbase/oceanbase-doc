...
<main id="notice" type='notice'>
  <h4>Notice</h4>
  <p>OceanBase Database is not compatible with Oracle in creating spatial indexes. In Oracle, you must manually maintain the mapping between spatial index columns and index information in the `USER_SDO_GEOM_METADATA` system table. However, OceanBase Database uses the MySQL style to be compatible with Oracle mode and does not maintain the `USER_SDO_GEOM_METADATA` table. Therefore, the syntax is different from that in Oracle. </p>
</main>
