...
Spatial data types are used to store and process geospatial data. OceanBase Database in Oracle mode supports the `SDO_GEOMETRY` spatial data type. `SDO_GEOMETRY` stores and processes geometric data. It is a composite data type used to represent a two-dimensional or three-dimensional geometric figure.
...
* Construction: Spatial objects of the `SDO_GEOMETRY` type can be constructed by using the default constructor function `SDO_GEOMETRY()` from well-known text (WKT) or well-known binary (WKB) data.
...
`SDO_GEOMETRY` is a composite data type that contains multiple intrinsic attributes. You can access these attributes in PL/SQL.
...
`SDO_GTYPE` indicates the type code of a geometry. The value is a `NUMBER` value in the **DLTT** format.

**The meaning of DLTT is described as follows:**
...
<main id="notice" type='explain'>
  <h4>Note</h4>
  <p><code>SDO_SRID</code> has the same meaning as <code>SRID</code> in MySQL tenants, which specifies the <code>ID</code> of an SRS. For more information about the SRS in MySQL tenants, see <a href="../../../../200.common-tenant-of-mysql-mode/100.basic-elements-of-mysql-mode/100.data-type-of-mysql-mode/900.spatial-data-type-of-mysql-mode/200.spacial-reference-system-of-mysql-mode.md">SRS</a>. </p>
</main>

#### SDO_POINT

`SDO_POINT` defines the point coordinates of a geometry. The attribute contains the X, Y, and optional Z coordinates of the geometry.
...
`SDO_ELEM_INFO` defines geometric element information of a geometry. `SDO_ELEM_INFO` is a variable-length array consisting of `NUMBER` values, which can be divided into multiple triplets that explain the `SDO_ORDINATES` attribute. Each triplet set of numbers is interpreted as follows:
...
* `SDO_ETYPE`: the element type. `SDO_ETYPE` values 1, 2, 1003 (indicates an exterior polygon ring, which must be specified in counterclockwise order), and 2003 (indicates an interior polygon ring, which must be specified in clockwise order) are considered simple elements. They are defined by a single triplet entry in the `SDO_ELEM_INFO` array.
...
| 1003 or 2003 | 1               | A simple polygon whose vertices are connected by straight line segments. The last point specified must be exactly the same point as the first. 1003 indicates an exterior ring and `2003` indicates an interior ring. |
| 1003 or 2003 | 3               | A rectangle (sometimes called an optimized rectangle). Only the coordinates of the lower-left vertex and upper-right vertex need to be recorded. |
...
In the above example, the first number `1` indicates to start from the first member of `SDO_ORDINATE_ARRAY`, the second number `1003` indicates an exterior polygon ring, and the third number `1` indicates connection by straight line segments.
...
Create a 2D point with the type code 2001 and coordinates X = 10, Y = 20, and Z = NULL. Do not specify the `SRID`, `SDO_ELEM_INFO`, or `SDO_ORDINATE_ARRAY` attribute.
