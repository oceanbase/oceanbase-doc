...
| latin1_german1_ci  | latin1  | Uses the collation for the latin1 character set in German environments. |
| latin1_danish_ci   | latin1  | Uses the collation for the latin1 character set in Danish environments. |
| latin1_german2_ci  | latin1  | Uses the collation for German environments where dictionary order is required for character comparison. |
| latin1_general_ci  | latin1  | Uses the case-insensitive collation that supports diacritic marks, which is suitable for database designs that involve some European languages. |
| latin1_general_cs  | latin1  | Uses the case-sensitive general collation that supports multiple languages (such as Western European languages). |
| latin1_spanish_ci  | latin1  | Uses the collation for Spanish (Spanish) environments. |
...
| ascii_general_ci | ascii  | Uses the case-insensitive collation based on alphabetical sorting. It ignores case differences and treats uppercase and lowercase letters as the same character. |
