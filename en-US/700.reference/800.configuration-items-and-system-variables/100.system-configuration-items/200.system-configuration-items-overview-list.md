...
| [tenant_sql_login_thread_count](400.tenant-level-configuration-items/7900.tenant_sql_login_thread_count.md) | The number of logon threads of a MySQL tenant, namely the number of `mysql_queue` threads. The default value `0` indicates that the value of the parameter is the same as that of `unit_min_cpu`. |
