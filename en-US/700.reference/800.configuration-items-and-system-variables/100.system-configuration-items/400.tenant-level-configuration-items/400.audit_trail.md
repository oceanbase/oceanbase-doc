...
| Type           | String     |
| Default value             | NONE        |
| Value range           | <ul><li>NONE: disables auditing.</li><li>OS: writes audit records to local files.</li><li>LOG: writes audit records to the audit log. Management policies such as log archiving, compression, rotation, cleanup, and encryption are supported.</li><li>DB: writes audit records to internal tables.</li><li>DB,EXTENDED: writes audit records to internal tables and includes executed SQL statements in the records.</li><li>DB_EXTENDED: writes audit records to internal tables and includes executed SQL statements in the records.</li></ul><main id="notice" type='explain'><h4>Note</h4><p>The <code>LOG</code> value is available starting with V4.4.2 BP2. </p></main>   |
