...
<main id="notice" type='explain'>
  <h4>Note</h4>
  <p>Currently, the <code>default_table_organization</code> parameter is applicable only to MySQL user tenants in OceanBase Database. It is not supported for the sys tenant and Oracle user tenants.</p>
</main>
...
* `HEAP` (heap-organized tables):
  ...
  * When you create a table with a primary key in this mode, the data is not sorted based on the order of the primary key column.
...
* `INDEX` (index-organized tables):
  ...
  * When you create a table with a primary key in this mode, the data is sorted based on the order of the primary key column.
...
1. View the current value of the `default_table_organization` parameter.
...
3. View the current value of the `default_table_organization` parameter.
