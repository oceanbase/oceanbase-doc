...
| Value range   | <ul><li>`DISTRIBUTED` (default): When multiple nodes are available in the tenant (for example, the number of primary zones or units in the tenant is greater than 1), automatic partition splitting is enabled by default for newly created global indexes. </li><li>`ALL`: Automatic partition splitting is enabled by default for newly created global indexes. </li><li>`OFF`: Automatic partition splitting is disabled for all global indexes, including existing ones. </li></ul>|
