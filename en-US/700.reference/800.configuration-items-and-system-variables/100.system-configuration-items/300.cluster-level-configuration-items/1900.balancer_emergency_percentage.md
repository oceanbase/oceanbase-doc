|description||
|---|---|
| machine-translation ||
|keywords||
|dir-name||
|dir-name-en||
|tenant-type||
|slug|balancer_emergency_percentage|

# balancer_emergency_percentage

<main id="notice" type='explain'>
<h4>Note</h4>
<p>Starting from V4.0.0, this parameter is no longer applicable because the storage architecture was upgraded in V4.x. It has been removed starting from V4.0.0. </p>
</main>

## Description

`balancer_emergency_percentage` specifies the unit load threshold. When the unit load exceeds this threshold, replica migration is allowed, and load balancing is performed even during major compactions.

## Attributes

|      **Attribute**      |   **Description**   |
|------------------|------------|
| Parameter type                     | Int         |
| Default value              | 80         |
| Value range             | \[1, 100\] |
| Effective upon OBServer node restart | No          |