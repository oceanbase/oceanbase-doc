...
`rootservice_ready_check_interval` specifies the interval between the time when the Root Service starts and the time when the system checks the cluster status.
...
Set the interval between the time when the Root Service starts and the time when the system checks the cluster status to 3s.
