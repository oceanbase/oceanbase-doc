|description||
|---|---|
|keywords||
|dir-name||
|dir-name-en||
|tenant-type||
|machine-translation| en |
|slug|ob_enable_show_trace-session|

# ob_enable_show_trace

<main id="notice" type='explain'>
  <h4>Note</h4>
  <p>This variable was introduced in OceanBase Database V4.1.0. </p>
</main>

## Description

`ob_enable_show_trace` specifies whether to enable trace logs.

## Limitations

* Query the variable

  In the `sys` tenant or a user tenant, you can execute the `SHOW VARIABLES` statement to query the variable at the session level or query the `SYS.TENANT_VIRTUAL_SESSION_VARIABLE` view (in Oracle-compatible mode) or `information_schema.SESSION_VARIABLES` view (in MySQL-compatible mode) for the variable at the session level.

* Modify the variable

  In the `sys` tenant or a user tenant, you can directly change the value of the variable at the session level for the current tenant.

## Attributes

| **Attribute** | **Description** |
|-----------|-----------------|
| Type | Boolean |
| Default value | 0 |
| Value range | <ul><li>`0`: disable</li>   <li>`1`: enable</li></ul>        |
| Effective scope | Session |
| Modifiable | Yes. You can use the `SET` or `ALTER SESSION SET` statement to modify the variable. |

## Examples

Enable trace logs.

```shell
obclient> SET ob_enable_show_trace = 1;
```

## References

[SQL Trace](../../../../700.reference/1000.performance-tuning-guide/500.sql-optimization/400.sql-optimization/300.monitor-sql-execution-performance/200.sql-trace.md)