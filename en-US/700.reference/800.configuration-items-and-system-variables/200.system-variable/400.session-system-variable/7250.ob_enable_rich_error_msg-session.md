|description||
|---|---|
|keywords||
|dir-name||
|dir-name-en||
|tenant-type||
|machine-translation||
|slug|ob_enable_rich_error_msg-session|

# ob_enable_rich_error_msg

`ob_enable_rich_error_msg` specifies whether to display information such as `svr_ip`, `time`, and `trace_id` when an error occurs during SQL statement execution.

<main id="notice" type='explain'>
  <h4>Note</h4>
  <p>This variable was introduced in OceanBase Database V4.0.0. </p>
</main>

| **Attribute** | **Description** |
|---------|------------------|
| Type | Boolean |
| Default value | False |
| Value range | <li>`False`: specifies not to display the related information.<li>`True`: specifies to display the related information. |
| Effective scope | <li>Global<li>Session |
