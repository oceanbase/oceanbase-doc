...
* Query a variable
  ...
  * Global level
    ...
    `sys` tenants and user tenants can execute the `SHOW VARIABLES` statement or view the `information_schema.GLOBAL_VARIABLES` view (MySQL-compatible mode) to query the value of a global system variable.

  * Session level

    `sys` tenants and user tenants can execute the `SHOW VARIABLES` statement or view the `information_schema.SESSION_VARIABLES` view (MySQL-compatible mode) to query the value of a session system variable.
