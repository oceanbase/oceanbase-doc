...
* Query variables
  ...
  * Global level
    ...
    `sys` tenants and user tenants can execute the `SHOW VARIABLES` statement or view `SYS.TENANT_VIRTUAL_GLOBAL_VARIABLE` (Oracle-compatible mode) and `information_schema.GLOBAL_VARIABLES` (MySQL-compatible mode) to view the value of a global system variable.

  * Session level

    `sys` tenants and user tenants can execute the `SHOW VARIABLES` statement or view `SYS.TENANT_VIRTUAL_SESSION_VARIABLE` (Oracle-compatible mode) and `information_schema.SESSION_VARIABLES` (MySQL-compatible mode) to view the value of a session system variable.
