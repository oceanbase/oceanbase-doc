...
<main id="notice">
  <h4>Applicability</h4>
  <p>This variable is available only in the MySQL mode of OceanBase Database. </p>
</main>
...
* Query variables
  ...
  The `sys` tenant and all user tenants can query the value of a global system variable by using the `SHOW VARIABLES` statement or the `information_schema.GLOBAL_VARIABLES` view (MySQL mode).
