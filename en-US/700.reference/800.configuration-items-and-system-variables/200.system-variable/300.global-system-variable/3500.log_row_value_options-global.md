...
| Value range | ['', 'partial_json']''<ul><li><code>''</code>: an empty string, which specifies to disable partial updates. </li><li>`partial_json`: specifies to enable partial updates for JSON data. </li></ul> |
