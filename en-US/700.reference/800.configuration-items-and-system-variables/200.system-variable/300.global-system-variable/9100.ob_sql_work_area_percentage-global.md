...
* Query variables
  ...
  Users in the `sys` tenant and all users in other tenants can execute the `SHOW VARIABLES` statement or view the `SYS.TENANT_VIRTUAL_GLOBAL_VARIABLE` view (Oracle-compatible mode) or the `information_schema.GLOBAL_VARIABLES` view (MySQL-compatible mode) to query the value of a global system variable.
...
* Modify variables
  ...
  * For V4.x, Oracle users must have the `ALTER SYSTEM` privilege to modify the value of a global system variable starting from V4.2.0.
