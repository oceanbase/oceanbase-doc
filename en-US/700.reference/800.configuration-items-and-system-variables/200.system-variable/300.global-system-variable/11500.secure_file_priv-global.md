...
|-----------|---------|
| Type | Varchar |
| Default value | "", which indicates an empty string. <main id="notice" type='explain'><h4>Note</h4><p>The default value has been changed from <code>NULL</code> to <code>""</code> since OceanBase Database V4.2.1. </p></main> |
| Value range | <ul><li>Empty string: File import and export are disabled. </li><li> `NULL`: File import and export are disabled. </li><li> `"NULL"`: File import and export are disabled. </li><li>A path name: Only files in the specified path can be imported and exported. </li><li>`"/"`: No limitation is imposed, and any path can be accessed.</li></ul><main id="notice" type='explain'><h4>Note</h4><p>The meanings of an empty string and the value <code>NULL</code> have been changed from "no limitation imposed" to "file import and export are disabled" since OceanBase Database V4.2.0. </p></main> |
