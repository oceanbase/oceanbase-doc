...
| Value range | <ul><li>`INDEPENDENT`: assumes that predicates are completely independent. </li><li>`PARTIAL`: assumes that predicates are partially correlated. </li><li>`FULL`: assumes that predicates are completely correlated. </li></ul><main id="notice" type='explain'><h4>Note</h4><p>In most scenarios, the row estimation results based on the three correlation assumptions sequentially increase: `INDEPENDENT` < `PARTIAL` < `FULL`. </p></main> |
