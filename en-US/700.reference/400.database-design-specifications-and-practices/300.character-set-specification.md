...
Using the gbk character set as an example:
...
* Set the character set when you create a tenant
  ...
  * Select gbk as the character set when you create a tenant in OceanBase Cloud Platform (OCP).
  ...
  <main id="notice" type='notice'>
    <h4>Notice</h4>
    <ul>
    <li>In Oracle-compatible mode, the character set is specified at the tenant level. In a tenant whose character set is set to gbk, the gbk character set is used for the char, varchar2, and clob fields in all user tables. However, the utf8 character set is still used for the char, varchar2, and other fields in system tables. </li>
    <li>In Oracle-compatible mode, the character set of a tenant cannot be modified. In other words, you cannot use the <code>ALTER</code> statement to modify the character set of a tenant, database, table, or column. </li>
    </ul>
  </main>
...
* Set the client (link) character set
  ...
  * Configuration method:
    ...
    * Permanent configuration
      ...
      * `character_set_connection`: the connection character set. In an Oracle-compatible tenant, set this parameter to the same value as that of `character_set_client`.

      * `character_set_results`: the character set of the results returned by the server to the client.

        Generally, the character set of the strings that the client sends to the server and the character set of the strings that the server returns to the client are the same. Therefore, in Oracle-compatible mode, the values of these three variables must be the same. In MySQL-compatible mode, these three variables are provided for flexible configuration. In general scenarios, you can set the three variables to the client character set.
...
* Set the client character set
  ...
  * When you use OBClient to connect to OceanBase Database, we recommend that you use the `zh_CN.GB18030` superset of `zh_CN.GBK` for the bash environment variables of the GBK link.
  ...
  <main id="notice" type='notice'>
    <h4>Notice</h4>
    <p>In addition to configuring the database (observer process) for the GBK link, you must also configure the client and driver accordingly. Garbled characters may be displayed if the environment is misconfigured.</p>
  </main>
