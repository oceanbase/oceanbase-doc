...
# Overview of object naming conventions
...
| Item | Upper limit |
|-----------------|---------------------------------------------------|
| Connections per ODP | Controlled by the `client_max_connections` parameter of ODP. The default value is 8192.  <blockquote><b>Note</b></br>You can increase the number of ODP nodes or modify the `client_max_connections` parameter to increase the maximum number of connections to a cluster. </blockquote> |

## Maximum number of partition replicas

| Item | Upper limit |
|--------------------|-----------------------------|
| Partition replicas per OBServer node | No strict limit.  <blockquote>**Note** </br>The number of partition replicas per OBServer node can be estimated based on the tenant memory size. About 20,000 tablets are supported per 1 GB of memory. </blockquote> |
...
| Number of partitions | <ul><li>Oracle-compatible mode: 65,536 </li><li>MySQL-compatible mode: 8,192 </li></ul> |
