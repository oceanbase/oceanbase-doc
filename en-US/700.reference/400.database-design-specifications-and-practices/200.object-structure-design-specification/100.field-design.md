...
* Recommendation on data field selection
  ...
  * For large tables that store IP addresses, we recommend that you use the `NUMBER` data type to save storage space. Implement conversion at the frontend. Keep value ranges consistent with network segment data in size.
