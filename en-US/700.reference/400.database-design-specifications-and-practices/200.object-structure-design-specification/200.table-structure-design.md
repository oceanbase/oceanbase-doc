...
During database operations, developers create various tables to perform database operations. This topic aims to help developers standardize table structure design in the database.
...
The first paradigm is the most basic paradigm. If all the field values in a database table are indivisible atomic values, the database table satisfies the first paradigm.
...
The primary key in this table is (`sno`,`tno`). Each field is an unsplittable atomic value, which meets the first paradigm. However, `sname` depends on `sno`, while `tname` depends on `tno`. Therefore, the table does not meet the second paradigm and needs to be split. The table is split into the following three tables.
...
All fields in the table are atomic values and have no partial dependency, which meets the first and second paradigms. However, `cname` depends on `cno`, while `cno` depends on `sno`. Therefore, the table does not meet the third paradigm. Split this table into two tables as follows:
...
* For yes-or-no fields, we recommend that you use the `unsigned tinyint` data type (1 indicates yes, 0 indicates no). The values must be consistent across all applications. For example, for the `is_deleted` field that indicates logical deletion, 1 indicates deleted, and 0 indicates not deleted.
