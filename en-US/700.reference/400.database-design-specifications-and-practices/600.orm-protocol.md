...
* Avoid creating a large and comprehensive data update API that takes the POJO class as input. It is not advisable to run a generic `update table set c1=value1,c2=value2,c3=value3;` statement without considering the specific fields to be updated. When executing an SQL statement, do not update fields that are not modified. Updating unmodified fields can lead to errors, hinder efficiency, and increase the storage size of binlogs.
