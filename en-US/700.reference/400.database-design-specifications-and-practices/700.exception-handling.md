...
* An exception is expected and you want to handle it.

   For example, you expect that a `SELECT INTO` statement may return no rows, causing OceanBase Database to report the predefined exception `NO_DATA_FOUND`. You wish to use a subprogram or a `STORED PROCEDURE` block to handle the exception (which is not an error) so that the program can continue execution. For example:
...
* You must discard or close a resource. For example:
...
* You need to record error logs at the top level of the subprogram code. For example:
...
OceanBase Database provides two exception handling mechanisms. One is to capture exceptions by using the exception block in PL when writing SQL statements. The other is to use the corresponding exception handling methods when different drivers are used for application layer code development.
...
### OceanBase JDBC driver example

For SQL statements that directly depend on the underlying driver error types, we recommend that the ORM framework use standard template classes such as `JDBCTemplate`, `SqlMapClientTemplate`, or `SqlMapClientDaoSupport`. These implementations can convert underlying errors into the standard exception types of the Spring Framework. If you use a standard template class, OceanBase Database does not need to directly perceive the underlying driver type `com.alipay.oceanbase.obproxy.mysql.exceptions.jdbc4.MySQLIntegrityConstraintViolationException`, as shown in the following example:
...
Also check for other driver exceptions under `com.alipay.oceanbase.obproxy.mysql.exceptions`.\*. If any of these are used, convert them to the corresponding Spring Framework standard exceptions.

  <main id="notice" type='explain'>
    <h4>Note</h4>
    <p>In the MySQL-compatible mode of OceanBase Database, we recommend that you use the native Java Database Connectivity (JDBC) driver of MySQL Database. </p>
  </main>

This topic uses the OceanBase JDBC driver as an example only. For more information about how other drivers handle exceptions, refer to the official documentation of the corresponding driver.
