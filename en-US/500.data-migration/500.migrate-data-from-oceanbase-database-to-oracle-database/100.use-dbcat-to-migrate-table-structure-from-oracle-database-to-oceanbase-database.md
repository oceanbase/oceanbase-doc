...
## Export schemas from an Oracle-compatible tenant of OceanBase Database
...
The following example shows how to migrate all schemas from the test database in an Oracle-compatible tenant of OceanBase Database V4.0.0 to an Oracle 12c database.
...
## Import schemas to the Oracle database

The file exported by using DBCAT is an SQL file. To import the SQL file to the Oracle database, access the Oracle database and then run the `@filename` command to import the SQL file. For example:
