...
|-----------|-----------------------|-----------------------|
| MySQL-compatible mode  | V2.2.40 and later           | MySQL Client or OBClient |
| Oracle-compatible mode | V2.2.40 and later           | OBClient              |
...
* To execute the `SELECT INTO` statement in a MySQL-compatible tenant, you must have the `FILE` privilege and the `SELECT` privilege on the table. You can use the following command to grant the `FILE` privilege to a user:

   ```sql
    GRANT FILE ON *.* TO user_name;
   ```

   In the statement, `user_name` is the username of the account that executes the `SELECT INTO` statement. For more information about privileges in OceanBase Database in MySQL-compatible mode, see [Privilege types in MySQL-compatible mode](../../600.manage/500.security-and-permissions/300.access-control/200.user-and-permission/200.permission-of-mysql-mode/100.permission-classification-of-mysql.md).

* To execute the `SELECT INTO` statement in an Oracle-compatible tenant, you must have the `SELECT` privilege on the table. For more information about OceanBase Database privileges, see [Privilege types in Oracle-compatible mode](../../600.manage/500.security-and-permissions/300.access-control/200.user-and-permission/300.permission-of-oracle-mode/000.permission-classification-of-oracle-mode.md).
...
* When an exported file is saved to OSS, the file path is in the `oss://\$PATH/\$FILENAME/?host=\$HOST&access_id=\$ACCESS_ID&access_key=\$ACCESSKEY` format, where:
  ...
  * `\$PATH`: the path to save the exported file, that is, the file path in the bucket.
  * `\$FILENAME`: the name of the exported file. When `SINGLE = FALSE`, it specifies the prefix of the exported file. If you do not specify this parameter, the system uses the default prefix `data`. The system automatically generates the suffix.
  * `\$HOST`: the hostname or CDN acceleration domain of the OSS service, that is, the address of the OSS service to access.
  * `\$ACCESS_ID`: the Access Key ID required to access the OSS service for identity verification.
  * `\$ACCESSKEY`: the Access Key Secret required to access the OSS service for identity verification.

    <main id="notice" type='explain'>
      <h4>Note</h4>
      <p><ul><li>OceanBase Database supports S3 and S3 protocol-based object storage as the destination for data export.</li><li>Due to file size limitations in Alibaba Cloud OSS, files larger than 5 GB will be split into multiple files, with each file being smaller than 5 GB when exported to OSS.</li></ul></p>
    </main>

* When the export file is saved to HDFS, the format is as follows:

  <main id="notice" type='notice'>
    <h4>Notice</h4>
    <p>To use the HDFS external table write capability, OceanBase Database must run in an environment with the JAVA SDK. For more information about deploying the JAVA SDK environment, see <a href="../../400.deploy/300.deploy-oceanbase-enterprise-edition/500.deploy-the-ob-java-sdk-environment.md">Deploy the OceanBase Database JAVA SDK environment</a>.</p>
  </main>

  * Export format with SIMPLE authentication:
    ...
    * Note:
      ...
      * When using basic authentication (SIMPLE), you can specify the HDFS username by using the `username` parameter.
      * If the `username` parameter is not specified, the HDFS username defined in the OceanBase Database configuration is used by default.

  * Export format with KERBEROS authentication:
    ...
    * Kerberos authentication requires the Kerberos principal (`principal`), keytab file (`keytab`), and Kerberos configuration file (`krb5conf`).
    * The `username` parameter does not apply in this scenario because Kerberos authentication identifies the identity through the `principal`.

  **Parameter description:**

  * Common parameters:

    * `\$NAMENODE`: the IP address of the HDFS NameNode.
    * `\$PORT`: the port of the HDFS NameNode.
    * `\$PATH`: the storage path of the file in HDFS.
    * `\$FILENAME`: the name of the exported file.

  * SIMPLE authentication parameters:

    * `?username=\$USERNAME`: optional. The HDFS username. If `username` is not specified, the HDFS username defined in the OceanBase Database configuration is used by default.

  * KERBEROS authentication parameters:

    * `\$PRINCIPAL`: the Kerberos principal name. For example, `principal=hdfs/hadoop@EXAMPLE.COM`.
    * `\$KEYTAB`: the path to the Kerberos keytab file. For example, `keytab=/path/to/hdfs.keytab`.
    * `\$KRB5CONF`: the path to the Kerberos configuration file. For example, `krb5conf=/path/to/krb5.conf`.
    * `\$CONFIGS`: additional HDFS configuration parameters in key-value pairs. For example, `configs=dfs.data.transfer.protection=authentication,privacy`.

  **Example file_name values when the export file is saved to HDFS:**

  * SIMPLE authentication (basic authentication):

    * Applicable to HDFS clusters without Kerberos authentication.
  ...
  * KERBEROS authentication:
    ...
    * Applicable to HDFS clusters that require Kerberos authentication.
    * Query parameters are filled as needed. Multiple configuration items (such as `configs`) are supported.
    * Example:
...
* `BUFFER_SIZE [=] {int | string}`: specifies the size of memory applied for each thread for each partition when data is exported. This parameter is not effective for non-partitioned tables. The default value is 1 MB.
  ...
  <main id="notice" type='explain'>
    <h4>Note</h4>
    <p><code>BUFFER_SIZE</code> is used for export performance tuning. If sufficient memory is available on the server and you want to improve the export efficiency, you can set a large value (for example, 4 MB). If the memory is insufficient, you can set a small value (for example, 4 KB). If you set it to 0, all partitions of a thread share a public memory.</p>
  </main>
...
* When **TYPE = 'PARQUET'**, the following fields are included:
  ...
  * `ROW_GROUP_SIZE`: specifies the size of a row group in the PARQUET file, in bytes. You can specify a numeric value or a string in the format of `'64MB'`. If this parameter is not specified, the default value `256MB` takes effect. We recommend that you use the default value.
...
* When **TYPE = 'ORC'**, the following fields are included:
  ...
  * `ROW_INDEX_STRIDE`: specifies the frequency at which index records are written. It defines how many rows are recorded per index entry. If this parameter is not specified, the default value `10000` takes effect. We recommend that you use the default value.
...
**Export with SIMPLE authentication:**

Export data from the OceanBase Database table `lineitem_external` to HDFS in CSV format.
...
**Export with KERBEROS authentication:**

Unlike SIMPLE authentication, KERBEROS authentication requires additional configuration files and key files. Export data from the OceanBase Database table `lineitem_external` to HDFS in CSV format.
