...
DBCAT can run on CentOS, macOS, and Windows operating systems. You must first install `JDK 1.8` or later. You can also use `OpenJDK`. Then, specify the environment variable `JAVA_HOME`.
...
| conf | The directory where log configuration files are located.  |
...
The following example shows how to migrate the schemas of all objects from the test database of Oracle Database 12c to an Oracle-compatible tenant of OceanBase Database V4.0.0.
...
* In the command, the `--from` and `--to` options respectively specify the source and destination database types, which must contain the database version number. The following table describes the source and destination databases supported by DBCAT.
  ...
  |OBORACLE | ORACLE |
   ...
   In the preceding table, OBMYSQL indicates MySQL-compatible tenants of OceanBase Database, and OBORACLE indicates Oracle-compatible tenants of OceanBase Database.
...
## Import schemas to the OceanBase database

The file exported by using DBCAT is an SQL file. Here you can use the import function of OceanBase Developer Center (ODC) to import the schemas to the OceanBase database. For more information, see [Batch export and import](https://en.oceanbase.com/docs/community-odc-en-10000000000842077).
...
## Verify the schema import result
