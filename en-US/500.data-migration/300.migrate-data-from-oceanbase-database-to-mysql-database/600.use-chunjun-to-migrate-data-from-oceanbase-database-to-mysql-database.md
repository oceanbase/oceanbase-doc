...
This guide explains how to use ChunJun in Local mode to migrate data from OceanBase Database (MySQL-compatible mode) to a MySQL database.
...
| Business tenant name (MySQL-compatible mode) | mysql001       |
| Username                 | root           |
| Password                | ******    |
...
Log in to OceanBase Database (MySQL-compatible mode) and insert a row into the `test_tbl1` table.
...
* For more information about the JSON file configuration for a MySQL database as the target, see [MySQL Sink](https://dtstack.github.io/chunjun/documents/zh/ChunJun%E8%BF%9E%E6%8E%A5%E5%99%A8@mysql@mysql-sink?lang=zh).
* For more script examples, see the files in the `/chunjun-dist/chunjun-examples` folder of the project. -->
