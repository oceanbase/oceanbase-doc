...
ChunJun (formerly known as FlinkX) is a stable, user-friendly, and efficient batch and stream integrated data integration framework. Currently, it leverages the Flink real-time computing engine to synchronize data and perform computations among various heterogeneous data sources. OceanBase Binlog Service (OceanBase Binlog Service, shortly Binlog Service) collects transaction logs from OceanBase Database and converts them into MySQL Binlog. It is mainly used in real-time data subscription scenarios.
chunjun-connector-oceanbasecdc integrates oblogclient internally to connect to the Binlog Service to obtain related log data. This enables the migration of incremental data from OceanBase Database to another database.

This topic will guide you through the process of migrating data from a MySQL database in OceanBase Database to a MySQL database using ChunJun in Local mode.
...
| OceanBase Database information (source database) | Example value |
|--------------------------|----------------|
...
|   MySQL Database information (target database) | Example value |
|---------------|----------------|
...
* The Binlog service has been installed and deployed. For more information, see [Create a Binlog cluster](https://www.oceanbase.com/docs/common-ocp-1000000001739745).
