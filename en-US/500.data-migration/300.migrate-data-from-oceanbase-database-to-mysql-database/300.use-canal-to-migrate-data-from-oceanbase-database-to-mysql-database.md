...
Follow the instructions in [Create a Binlog cluster](https://en.oceanbase.com/docs/common-ocp-10000000001899110) to install and deploy OceanBase Binlog Service.
...
4. Configure a Canal instance.
    ...
    # Whether to remove the tenant prefix from database names. By default, the log proxy outputs logs with database names in the [tenant].[db] format.
    canal.instance.oceanbase.tenant=mysql001
    canal.instance.parser.excludeTenantInDbName=true

    # Log filtering. Specify the filter in the [tenant].[database].[table] format. Regular expressions are supported.
