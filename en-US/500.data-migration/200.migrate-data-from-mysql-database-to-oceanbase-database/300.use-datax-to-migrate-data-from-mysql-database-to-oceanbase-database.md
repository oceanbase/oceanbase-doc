...
<!-- For more information about the features and parameters, see [`mysqlreader` plug-in](https://github.com/alibaba/DataX/blob/master/mysqlreader/doc/mysqlreader.md). -->

### `oceanbasev10writer` plug-in

The `oceanbasev10writer` plugin allows you to write data to target tables in the OceanBase database.
Under the hood, `oceanbasev10writer` connects to OceanBase remotely via obproxy using a Java client (either MySQL JDBC or OceanBase Client), and executes insert SQL statements to write data. OceanBase handles these inserts internally by batching and committing them in groups.

In terms of its operation, `oceanbasev10writer` uses the DataX framework to retrieve protocol data produced by the Reader and generates `insert` statements accordingly. When writing data, if a primary key or unique key conflict occurs, OceanBase Database’s MySQL-compatible tenant supports the `replace` mode to update all fields in the table, while the Oracle-compatible tenant currently only supports the Insert mode. For better performance, data is written in batches; the write request is only triggered once the number of rows reaches a preset threshold.
...
Example: Migrate the data of the `tpccdb.bmsql_order` table in a MySQL database to the `tpcc.bmsql_order` table in a MySQL-compatible tenant of OceanBase Database. 
