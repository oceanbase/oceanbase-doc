...
This topic describes how to migrate data from a MySQL database to OceanBase Database (MySQL-compatible mode) by using mydumper and myloader. 
...
<h4>Notice</h4>
<p>You can use mydumper to export data only from MySQL-compatible tenants of OceanBase Database. </p>
</main>
...
Statements in the backup table schema may contain `FOREIGN KEY` constraints. When you import the table schema to a MySQL-compatible tenant of OceanBase Database, an error occurs if the table referenced by a foreign key does not exist. Therefore, you must disable the `FOREIGN KEY` constraint check before the import. 
