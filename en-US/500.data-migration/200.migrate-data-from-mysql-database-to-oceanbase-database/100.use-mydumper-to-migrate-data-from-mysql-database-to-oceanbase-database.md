...
2. Perform backup operations.
   ...
   Here is a sample mydumper command used to back up data in a MySQL database:
   ...
   |-----------|-------------------|----------|
   |-h|--host|The name or IP address of the host to connect.|
   ...
   |-B|--database|The name of the database to back up. If you do not specify this parameter, all databases on the connected host are backed up by default.|
   ...
   |-t| --threads|The number of backup threads executed in parallel. The default value is `4`.|
...
3. Perform restore operations.
   ...
   * Use the `source` command to restore the data.
      ...
      If you cannot directly restore a table, you can find the SQL file of the table, and run the `source` command on the CLI to restore the table. The syntax is as follows:
   ...
   * Here is a sample `myloader` command used to restore data in an OceanBase database:
...
1. Download the installation package.
   ...
   Go to the [download page](https://github.com/mydumper/mydumper/tags), and download the installation package as needed.

2. Decompress the installation package to the host where the database is deployed.

   Here is a sample command to decompress the `mydumper-0.12.7-2-zstd.el7.x86_64.rpm` package:
...
#### Back up the `t1` and `t2` tables in the `test` database
...
For more information about mydumper and myloader, visit <https://github.com/mydumper/mydumper>.
