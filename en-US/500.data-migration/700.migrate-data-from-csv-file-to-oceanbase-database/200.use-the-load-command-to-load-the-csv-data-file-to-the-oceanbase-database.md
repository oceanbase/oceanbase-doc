...
2. Grant other necessary privileges.
   ...
   * The `INSERT` privilege on the corresponding table is required in MySQL-compatible mode.
   ...
   * The `CREATE SESSION` privilege is required in Oracle-compatible mode.
...
<main id="notice" type='explain'>
  <h4>Note</h4>
  <p>OceanBase Database supports two modes: MySQL and Oracle. The following example shows how to use the <code>LOAD DATA</code> statement in MySQL-compatible mode. </p>
</main>
...
OceanBase Database supports specifying an error diagnostics mode for the `LOAD DATA` statement when importing data from a URL. This mode allows failed rows to be recorded instead of terminating the entire operation due to the first error. Combined with the `REJECT LIMIT` clause, you can control the maximum number of allowable error rows.

For more information about the `LOAD DATA` statement for importing data from a URL with error diagnostics, see [LOAD DATA (MySQL-compatible mode)](../../700.reference/500.sql-reference/100.sql-syntax/200.common-tenant-of-mysql-mode/600.sql-statement-of-mysql-mode/5900.load-data-of-mysql-mode.md) or [LOAD DATA (Oracle-compatible mode)](../../700.reference/500.sql-reference/100.sql-syntax/300.common-tenant-of-oracle-mode/900.sql-statement-of-oracle-mode/300.dcl-of-oracle-mode/1900.load-data-of-oracle-mode.md).

## References

* For more information about how to use the `LOAD DATA` statement to import data through direct load, see [Direct load](../../620.obap/300.obap-data-collection/300.obap-import-data/20.bypass-import/100.overview-of-bypass-import.md).
