...
3. Insert data.
   ...
   After the target OBServer node receives the data, it executes the `INSERT` statement to insert the data into the corresponding partition.
...
1. Grant the `FILE` privilege.
   ...
   Execute the following statement to grant the `FILE` privilege to a user:
...
2. Grant other necessary privileges.
   ...
   * The `INSERT` privilege on the corresponding table is required in MySQL mode.
      ...
      Execute the following statement to grant the `INSERT` privilege to a user:
   ...
   * The `CREATE SESSION` privilege is required in Oracle mode.
      ...
      Execute the following statement to grant the `CREATE SESSION` privilege to a user:
...
4. Specify the path of the file to import.
...
7. Use the `LOAD DATA` statement to import data.
   ...
   * Set the name of the target table to which the data is to be loaded to `student`.
   ...
   * Specify the mappings between columns in the source data file and those in the target table. The first column in the data file will be mapped to the `id` column in the target table, the second column to the `name` column, and the third column to the `score` column.
...
8. Query data in the target table.
...
3. Start the client.
   ...
   Run the following command to use OceanBase Command-Line Client (OBClient) to connect to OceanBase Database. Add the `--local-infile` parameter to the command to enable the feature for loading data from local files.
...
6. Query data in the target table.
...
* For more information about how to use the `LOAD DATA` statement to import data in direct load mode, see [Use the LOAD DATA statement to import data in direct load mode](../1100.bypass-import/200.use-load-data-statement-to-bypass-import-data.md).
