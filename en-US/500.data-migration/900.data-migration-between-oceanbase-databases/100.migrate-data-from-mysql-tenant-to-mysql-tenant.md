...
# Use obloader & obdumper to migrate data between MySQL-compatible tenants in OceanBase Database

This topic describes how to use obloader & obdumper to migrate data from a MySQL-compatible tenant to another MySQL-compatible tenant in OceanBase Database. 
...
| Destination tenant name (MySQL-compatible mode) | mysql002 |
| User account of the `mysql002` tenant (with the read and write privileges) | obloader_user01 |
| Password of the `obloader_user01` user in the `mysql002` tenant | ****** |
