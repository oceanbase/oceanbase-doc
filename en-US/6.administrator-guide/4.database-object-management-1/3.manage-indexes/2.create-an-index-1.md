...
This topic describes how to create an index on a non-partitioned table by using the `CREATE INDEX` statement. For more information about creating an index on a partitioned table, see [Local index](/en-US/6.administrator-guide/5.data-distribution-and-link-management/1.partition-table-and-partitioned-index-management/6.create-an-index-on-a-partition-table/1.local-index.md). 
