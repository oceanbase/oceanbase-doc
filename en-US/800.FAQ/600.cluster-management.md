...
| The clog disk is full | During the clog reconfirm process, logs such as `prepare` and `start_working` are written and the confirmation by the majority of replicas is required. If these logs cannot be written on a server because the clog disk of the server is full, and the remaining normal servers are not the majority, the reconfirm process fails.  | If the clog disk is full, the clog module reports the [ERROR] level code -4264 in the observer.log file. The error keywords are as follows:<br>`log outof disk space(partition_key={tid:1102810162709414, partition_id:46, part_cnt:0}, server="xx.xx.xx.xx:29432", ret=-4264, free_quota=-3381817344)` |
