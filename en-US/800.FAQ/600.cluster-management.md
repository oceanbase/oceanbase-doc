...
In the V4.x series, OceanBase Database supports the following types of replica: full-featured replica (FULL), read-only replica (ReadOnly), and columnstore replica (COLUMNSTORE). Only the full-featured replicas can be a candidate for leader in an election and become the primary to provide database services.  
...
If there are no bottlenecks in the IO or network layers (infrastructure layer), but issues persist on the minority nodes, it is advisable to promptly reach out to technical support for comprehensive troubleshooting assistance.
