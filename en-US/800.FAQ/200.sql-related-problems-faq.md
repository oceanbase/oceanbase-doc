...
| useServerPrepStmts | FALSE | Specifies whether to use prepared statements on the server. This attribute is valid only for prepared statements. Valid values: <ul><li>TRUE: specifies to use prepared statements on the server. In this case, the `_ob_enable_prepared_statement` must be set to TRUE on the OBServer node, and the binary protocol is used for communication.</li><li>FALSE: specifies to use prepared statements on the client. In this case, the text protocol is used for communication. </ul></li> |
