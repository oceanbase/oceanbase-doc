...
You can execute the `SHOW ERRORS` statement to view the information about errors that occur when stored procedures are created.
...
### How do I query the routing information of an SQL statement in a PL object?

You can query the `sys.all_virtual_routine_agent` table for the routing information of an SQL statement in a PL object.
Related fields in the table are described as follows:
...
### Is the `INSERT ALL INTO` syntax supported in MySQL mode?

No. At present, the `INSERT ALL INTO` syntax is supported only in Oracle mode.

### How do the SQL engine and transaction engine allocate and isolate resources for tenants?

Both the SQL engine and transaction engine isolate resources among tenants. Specifically:

- The plan cache of the SQL engine and the locks of the transaction engine are independent of each other.
...
- Thread: The threads of the SQL engine and transaction engine of different tenants are independent of each other. If the threads of a tenant are suspended, other tenants are not affected.

### What is batch execution in OceanBase Database?

When you connect to OceanBase Database through Java Database Connectivity (JDBC), you can place multiple requests in the same group and transmit the requests at the same time. This is called batch execution or batch processing.
...
### Which classes and objects in JDBC are suitable for batch execution?
...
- In terms of performance, `cachePrepStmt`, `prepStmtCacheSize`, `prepStmtCacheSqlLimit`, and `maxBatchTotalParamsNum` can improve the performance.

The following table describes the attributes.

| Attribute | Default value | Description |
| ----- | ----- | ----- |
| allowMultiQueries | FALSE | Specifies whether the semicolon (;) can be used to join requests in a statement. Batch execution does not depend on this attribute but depends on the `rewriteBatchedStatements` attribute. <main id="notice" type='explain'><h4>Note</h4><ul><li>If the JDBC version is 1.1.9 or earlier, <code>allowMultiQueries</code> must be set to <code>TRUE</code> so that multiple <code>UPDATE</code> statements can be provided and separated with semicolons (;). Otherwise, the error `Not supported feature or function` is returned. </li><li>If the JDBC version is 2.2.6 or later, the `allowMultiQueries` attribute is optional. </li></ul></main>|
| rewriteBatchedStatements | FALSE | Specifies whether to rewrite the `INSERT` statement in batch execution. <ul><li> For a prepared statement, multiple `INSERT` statements are concatenated into one `INSERT` statement by using a `VALUES` clause, with each `INSERT` statement represented as a parameter of the `VALUES` clause.</li><li> For a statement, multiple `INSERT` statements are concatenated with semicolons (;). </li></ul> |
...
| cachePrepStmts | FALSE/TRUE | Specifies whether the JDBC driver caches prepared statements. The cached content for a prepared statement on the server is different from that for a prepared statement on the client. <main id="notice" type='explain'><h4>Note</h4><p>The default value is `FALSE` for OceanBase Connector/J 1.x, and `TRUE` for OceanBase Connector/J 2.x. </p></main> |
| prepStmtCacheSize | 25/250 | The number of prepared statements that can be cached. This attribute is valid only when `cachePrepStmts` is set to `TRUE`. <main id="notice" type='explain'><h4>Note</h4><p>The default value is `25` for OceanBase Connector/J 1.x, and `250` for OceanBase Connector/J 2.x.</p></main> |
| prepStmtCacheSqlLimit | 256/2048 | The maximum size of SQL statements that can be cached. This attribute is valid only when `cachePrepStmts` is set to `TRUE`. <main id="notice" type='explain'><h4>Note</h4><p>The default value is `256` for OceanBase Connector/J 1.x, and `2048` for OceanBase Connector/J 2.x. </p></main> |
...
| --- | --- | --- | --- |
| TRUE | Multiple `INSERT` statements are concatenated into one `INSERT` statement in the format of `INSERT INTO TEST1 VALUES (?), (?),..., (?)`, with the `VALUES` clause of each `INSERT` statement represented as a question mark (?). | Multiple independent `UPDATE` statements are executed separately, with the variables replaced with question marks (?). | Scenario 1 |
| FALSE | Multiple `INSERT` statements are concatenated into one `INSERT` statement in the format of `INSERT INTO TEST1 VALUES (1), (2), ...,(10)`, with the `VALUES` clause of each `INSERT` statement represented as a specific value. | Multiple independent `UPDATE` statements are concatenated with semicolons (;). | Scenario 2 |
...
In OceanBase Database, the processing of batch execution is different for `INSERT`, `UPDATE`, and `DELETE` statements.
...
- Scenario 1
  ...
  | ------ | ------ |
  | TRUE | Multiple `INSERT` statements are concatenated into one `INSERT` statement in the format of `INSERT INTO TEST1 VALUES (?), (?),..., (?)`, with the `VALUES` clause of each `INSERT` statement represented as a question mark (?). |

  In scenario 1, an OBServer node receives a `COM_STMT_PREPARE` (`request_type=5`) request and a `COM_STMT_EXECUTE` (`request_type=6`) request for the `INSERT` statement. The benefits are as follows:

  - The batch execution of the `INSERT` statement is completed in two communications.
  - The inherent attributes of a prepared statement help reduce the compilation time.
...
- Scenario 2
  ...
  | useServerPrepStmts | INSERT |
  | ------ | ------ |
  | FALSE | Multiple `INSERT` statements are concatenated into one `INSERT` statement in the format of `INSERT INTO TEST1 VALUES (1), (2), ...,(10)`, with the `VALUES` clause of each `INSERT` statement represented as a specific value. |
...
- Scenario 3/4
  ...
  - The batch execution of the `INSERT` statements is completed in only one communication.
...
| --- | --- |  --- |
| TRUE | Multiple independent `UPDATE` statements are executed separately, with the variables replaced with question marks (?). | Scenario 1 |
...
- If `ob_enable_batched_multi_statement` is set to `TRUE`, for the batch execution of `UPDATE` statements in scenarios 2, 3, and 4, the OBServer node parses multiple statements in the same format as one statement and generates a batch physical plan based on the corresponding parameters and distribution of data. This effectively improves the batch execution efficiency. To use this feature, you must enable explicit transaction commit.
...
In scenarios 2, 3, and 4, the text protocol is used for communication and batch execution of `UPDATE` statements can be implemented. Therefore, the configurations of these scenarios are recommended.
...
- In scenario 1, if batch `INSERT` takes effect, you will see records similar to the following ones in the `gv$sql_audit` view:
...
- In scenario 2, if batch `INSERT` takes effect, you will see records similar to the following ones in the `gv$sql_audit` view:
...
- In scenario 2, if batch `UPDATE` takes effect, you will see records similar to the following ones in the `gv$sql_audit` view:
...
- If the statements are executed one by one on the OBClient, this array will return the number of rows updated by each operation in the batch.
- If the statements are executed as a whole on the OBClient, for example, the JDBC driver rewrites multiple `INSERT` statements into one `INSERT` statement with multiple values (scenarios 1 and 2), or the `UPDATE` statements are executed as a batch physical plan (scenario 2), each element in this array returns `-2`, indicating that the execution is successful but the number of updated rows is unknown.

## SQL tuning

### What do I do when the data statistics are inaccurate?

Query optimization relies on accurate data statistics. By default, the OceanBase optimizer collects some statistics during the major compaction of data. However, the statistics may be inaccurate and fail to reflect the actual data characteristics if you extensively modify the data. You can actively update the statistics through daily major compaction.
In addition to collecting statistics, the optimizer also samples data from the storage layer based on query conditions for subsequent optimization and selection. At present, OceanBase Database only supports sampling from local storage. If the data partitions are stored on remote nodes, the optimizer can only estimate the cost based on the collected statistics, which may lead to inaccurate cost estimation.

### What do I do when the query performance deteriorates due to physical design of the database
...
### What do I do when the response time of individual SQL queries is affected by the system workload?

The overall load of the system not only affects the overall throughput of the system but also causes changes in the response time of individual SQL queries. In OceanBase Database, the SQL engine processes queries linearly, which means that new queries are put in a queue if all threads are occupied until a thread completes the current query. You can find the queue time of a query in the `(G)V$OB_SQL_AUDIT` view.

### What do I do when an incorrect execution plan is selected due to cost model defects?

OceanBase Database uses its built-in cost model. The selection of the optimal execution plan depends on this cost model. Therefore, if an incorrect plan is selected due to possible defects in this cost model, you can only choose to bind the "correct" execution plan.

### What do I do when a routing feedback logic error occurs between the client and the server?

One of the main features of OceanBase Database Proxy (ODP) is to route SQL queries to the right OBServer node. Specifically, if you do not specify week-consistency read in a query, ODP routes the query to the OBServer node where the leader replica of a partition is located to avoid secondary forwarding between OBServer nodes. Otherwise, ODP forwards the query to the right OBServer node based on the preset rules.
The loose coupling between ODP and OBServer nodes may cause latencies in refreshing the physical distribution of data cached on ODP, resulting in routing errors. The routing information may be changed in the following circumstances:

- Re-election of the leader due to load balancing
- Re-election of the leader between OBServer nodes due to network instability
- Re-election of the leader due to the addition or removal of an OBServer node or rotating compactions

You need to consider these circumstances when a large number of remote execution queries are found in the SQL Audit records or plan cache. The routing feedback logic exists between the client and the server, so the client refreshes the physical distribution of data when an error occurs. Then, the routing feature is restored.

## Index monitoring

### Index monitoring is enabled by default. Will it affect the performance?

Yes, index monitoring will affect the performance. However, the impact of index monitoring in default mode (SAMPLED) is slight and can be ignored.
