...
- **What is the "large account" issue?**
  ...
  The "large account" issue refers to significant performance differences between different queries with the same SQL ID due to different execution plans for the same SQL ID. For example, when a parameterized SQL statement is executed for the first time, the execution plan (p1) may be a full table scan, which is suitable for a scenario with a very low selectivity. If another parameterized SQL statement (such as `select * from items where store = 'xiaomaibu';`) is executed later, and the selectivity is high, the original plan p1 may not be the optimal plan, which causes performance degradation.
...
- **How to address the "large account" issue?**

   + Disable the plan cache

     1. **Disable the plan cache for a single SQL statement**: You can use the `USE_PLAN_CACHE` hint or set the session variable `ob_enable_plan_cache` to disable the plan cache for a single SQL statement.
       ...
       **Disabling the plan cache** for a single SQL statement requires modification of the SQL statement in the application. If the issue is discovered after the application is released, you must release a new version of the application to address the issue. You can also have a database administrator add hints to the SQL statement by using SQL outlines.

     2. **Use SQL outlines**: You can bind hints to SQL statements by using SQL outlines to control the execution plans of SQL statements.

        ```sql
        CREATE OUTLINE otl_no_plan_cache1
        ON select /*+ USE_PLAN_CACHE(NONE) */ * from items where store = 'taobao';
        # Or you can specify the SQL_ID. The SQL_ID can be obtained from the V$OB_SQL_AUDIT view.
