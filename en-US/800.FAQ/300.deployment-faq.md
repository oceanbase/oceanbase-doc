...
| Ubuntu                    | 22.04 LTS and 24.04 LTS | x86_64 (including Hygon)|
...
That is, can an OBServer node use IPv4 for RPC communication with other OBServer nodes, and use IPv6 for external SQL communication, or use IPv4 internally and IPv6 externally for SQL?

Not supported, because:

* OBServer will not listen on IPv6 if IPv6 is not enabled
* When IPv6 is enabled, OBServer sets IPV6_V6ONLY when listening on the SQL port, so IPv4 connections cannot be established.

## Can an OceanBase cluster use both IPv4 and IPv6 together?

That is, is it allowed to deploy IPv4 and IPv6 addresses mixed within a cluster, with some OBServer nodes started using IPv4 addresses and others using IPv6 addresses?

Yes.

<main id="notice" type='notice'>
  <h4>Notice</h4>
  <p>OBServer nodes started with IPv4 require their operating system and underlying network to support IPv6; otherwise, they will not be able to communicate properly with other OBServer nodes using IPv6.</p>
</main>