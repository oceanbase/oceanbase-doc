...
8. Specify a user label. 
   ...
   1. Log in to the `Oracle` tenant as the `sys` user, create a user named `user1`, and then grant `user1` the `sys` user's all privileges except `GRANT OPTION`. 
   ...
   2. Log in to as the security administrator `LBACSYS` and specify a label for `user1`. 
