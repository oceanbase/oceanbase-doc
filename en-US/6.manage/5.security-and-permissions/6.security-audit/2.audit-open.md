...
4. Enable AuditTrail and set the storage location for the audit records. 
   ...
   * `DB,EXTENDED / DB_EXTENDED`: writes audit records and executed SQL statements into an internal table. 
   ...
   <main id="notice" type='explain'>
      ...
      <ul>
      <li>If you choose to write audit records into a local disk file, the audit records are written into the <code><安装目录>/audit/</code> directory in asynchronous write mode. The default installation directory is <code>/home/admin/oceanbase/audit/observer_xx_xxxxxx.aud</code>. </li>
