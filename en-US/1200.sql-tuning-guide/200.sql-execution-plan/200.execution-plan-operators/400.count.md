...
The output of the preceding example shows that in the new SQL query, you can still find `ROWNUM`. However, the expression that contains `ROWNUM` is already placed with the equivalent `LIMIT` expression. This conversion enables you to optimize the query further. For more information, see [LIMIT](../2.execution-plan-operators/12.limit.md).
