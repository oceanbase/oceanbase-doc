...
* For more information about vector indexes, see [Vector index overview](200.ob-vector-index/100.ob-vector-index-overview.md).
* To perform large-scale performance tests, we recommend using the [VectorDBBenchmark tool](700.ob-vector-search-bench-test.md) to generate a test dataset to better compare the performance differences between exact search and approximate search.