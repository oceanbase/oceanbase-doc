...
| Second layer | Partitions | ≈Tablet | Milvus enhances read performance by chunking the same shard (shards are usually partitioned by primary key) based on other columns.<br>OceanBase implements this by sorting keys within a partition. |
...
For more information about pyobvector's APIs, see [pyobvector Python SDK](900.ob-vector-search-supported-clients-and-languages/200.ob-vector-pyobvector.md).
...
| JSON | JSON | For more information, see [pyobvector Python SDK](900.ob-vector-search-supported-clients-and-languages/200.ob-vector-pyobvector.md). |
