...
* You can call only some distance functions during vector index search. For more information, see [Use SQL functions](250.ob-vector-function.md).
...
* **Before you use the HNSW/HNSW_SQ/HNSW_BQ vector indexes in versions earlier than V4.3.5 BP3**, you must manually set the `ob_vector_memory_limit_percentage` parameter to enable the vector feature. We recommend that you set the parameter to `30` for optimal search performance. If you do not set the parameter, the system does not allocate memory for vector indexes and an error is returned when you create an index. The IVF/IVF_SQ/IVF_PQ indexes do not need to be kept in memory, so you do not need to set this parameter. Example:
    ```sql
    ALTER SYSTEM SET ob_vector_memory_limit_percentage = 30;
    ```

* **Starting from V4.3.5 BP3**, the vector search feature is enabled by default. The default value of `0` indicates the adaptive mode, where the system automatically adjusts the memory usage ratio of vector index data in the tenant. You do not need to manually adjust this parameter:
