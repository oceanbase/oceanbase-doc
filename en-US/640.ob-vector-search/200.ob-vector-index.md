...
|-----------|------|----------|
| HNSW      | The maximum dimension of an index column is 4096. An HNSW index is an in-memory index that needs to be fully loaded into memory. It supports DML operations and real-time search. |  |
...
* Dense vector indexes support the L2, inner product (IP), and cosine distance algorithms. Sparse vector indexes support only the IP algorithm.
...
* You can create a vector index and a full-text index in the same table. A vector index can be a dense or sparse index.
...
The syntax for dropping a sparse vector index and a dense vector index is the same.
