...
* For dense vector indexes, the supported index distance algorithms are L2, inner product, and cosine distance. For sparse vector indexes, the supported index distance algorithms are inner product and negative inner product. For more information, see [Create a vector index](200.ob-vector-index.md). The support of sparse vector index distance algorithms was added in V4.3.5 BP2.
...
* For sparse vectors, one parameter can be a string in the sparse vector format, such as `c2,'{1:2.4}'`. However, if one parameter is a string in the sparse vector format, the other parameter must not be a string.
...
* Apart from the vector type, other types of parameters can be forcibly converted to the vector type. For example, the parameters can be a one-level array such as `[1,2,3,...]`, or a string such as `'[1,2,3]'`.

* The dimensions of the two parameters must be the same.

* If any of the parameters is of the one-level array type, the elements in the parameter cannot be `NULL`.
...
* If a single-element array type parameter exists, the parameter must not contain `NULL` elements.
...
* If both parameters are of single-element array types, the return value is of the `array(float)` type.
