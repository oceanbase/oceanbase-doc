...
* You cannot perform the following operations on vector data with different dimensions. An error `different vector dimensions %d and %d` is returned.
* If the result exceeds the range of a floating-point number, an error `value out of range: overflow / underflow` is returned.
* For dense vector indexes, you can use L2, inner product, and cosine distance as the index distance algorithms. For more information, see [HNSW series indexes](200.ob-vector-index/300.ob-dense-vector-index/200.ob-hnsw-index.md) and [IVF series indexes](200.ob-vector-index/300.ob-dense-vector-index/300.ob-ivf-index.md).
* You can call the `L2_distance`, `Cosine_distance`, `Inner_product`, and `Negative_inner_product` distance functions in this topic for vector index search.
...
* **Before you use the HNSW, HNSW_SQ, or HNSW_BQ vector index in versions earlier than V4.4.1**, you must manually set the `ob_vector_memory_limit_percentage` parameter to enable vector search. We recommend that you set this parameter to `30` to achieve the best search performance. If you do not set this parameter, no memory is allocated to the vector index, and an error is returned when you create the index. The IVF and IVF_PQ indexes do not need to be kept in memory, so you do not need to pay attention to this parameter. Here is an example of how to configure this parameter:
    ```sql
    ALTER SYSTEM SET ob_vector_memory_limit_percentage = 30;
    ```

* **Starting from V4.4.1**, vector search is enabled by default. The default value of `0` indicates adaptive mode, where the system automatically adjusts the memory usage ratio of vector index data in the tenant. You do not need to manually adjust this parameter:
...
* In addition to vector types, other types that can be explicitly converted to vectors are also supported, such as string types (e.g., `'[1,2,3]'`).
* The dimensions of the two parameters must be the same.
* When a parameter is of the array type, the elements cannot be `NULL`.
* When a parameter is a sparse vector, the parameter can be a string in the sparse vector format, such as `c2,'{1:2.4}'`. However, both parameters cannot be strings.
...
vector_distance calculates the distance between two vectors and supports different distance algorithms based on the specified parameters.
...
The `metric` parameter specifies the distance algorithm. The supported values are:

* If not specified, the default algorithm is `euclidean`.

* If specified, the supported value is

  * `euclidean`. This specifies the Euclidean distance, which is the same as L2_distance.

  * `manhattan`. This specifies the Manhattan distance, which is the same as L1_distance.

  * `cosine`. This specifies the cosine distance, which is the same as Cosine_distance.

  * `dot`. This specifies the inner product, which is the same as Inner_product.
...
* In addition to vector types, other types that can be explicitly cast to vector types are also supported, such as string types (e.g., `'[1,2,3]'`). **Note:** Both parameters cannot be string types. At least one parameter must be a vector or single-level array type.
...
* In addition to vector types, other types that can be strongly converted to vector types are also accepted, such as string types (e.g., `'[1,2,3]'`).
