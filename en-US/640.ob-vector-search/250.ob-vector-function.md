...
* Dense vector indexes support L2, inner product, and cosine distance as index distance algorithms. Sparse vector indexes support inner product and negative inner product as index distance algorithms. For more information, see [Create a vector index](200.ob-vector-index.md). The negative inner product algorithm for sparse vector indexes is supported starting from V4.3.5 BP2.
* The `L2_distance`, `Cosine_distance`, `Inner_product`, and `Negative_inner_product` functions can be used for vector index search.
