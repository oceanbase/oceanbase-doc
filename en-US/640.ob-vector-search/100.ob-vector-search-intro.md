...
OceanBase Database supports dense vectors of up to 16,000 dimensions in the Float data type. It provides support for calculating various types of vector distances, including Manhattan distance, Euclidean distance, inner product, and cosine similarity. It also supports the creation of HNSW-based vector indexes, as well as incremental updates and deletions, ensuring that recall rates are not affected by these operations. OceanBase vector search offers integrated query capabilities with scalar filtering. Additionally, it provides flexible access interfaces, supporting SQL access through MySQL protocol clients in various programming languages, as well as access via a Python SDK. Furthermore, OceanBase Database has been adapted to AI application development frameworks such as LlamaIndex, DB-GPT, and the AI application development platform Dify, enhancing its service capabilities for AI application development.
