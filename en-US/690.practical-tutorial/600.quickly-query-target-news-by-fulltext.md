...
```sql
-- Query using Boolean mode to find news containing both "乌龙球" (own goal) and "拜仁" (Bayern)
...
1. Use the `TOKENIZE` function to verify the tokenization effects of tokenizers:
   ...
   The above result does not recognize the names "博阿滕" (Boateng) and "格策" (Götze).
...
3. Update the system dictionary. You can first insert the Chinese names into the system dictionary table. After updating the system dictionary, you need to refresh the cache.
...
5. Verify the optimization effect of the tokenizer:
   ...
   ```sql
   -- Execute the tokenizer test again (same input)
   SELECT TOKENIZE('博阿滕右路反击人球分过传中，格策后点停球转身闪开角度，在门前8米处低射从皮亚托夫裆下钻进门内', 'ik', '[{"additional_args": [{"ik_mode": "smart"}]}]');
   ```
   ...
   The above result indicates that the professional names (博阿滕, 格策, 皮亚托夫) are correctly identified.
