...
- **Semantic complexity**: Address challenges in Chinese word segmentation and synonym handling in natural language processing.
...
1. **Environment requirements**: You have deployed an OceanBase cluster of V4.3.5 BP1 or later and created a MySQL tenant. For more information about how to deploy an OceanBase cluster, see [Overview](../400.deploy/100.deploy-overview.md). After the deployment is complete, run the following SQL statements to verify the cluster and tenant information:
...
OceanBase Database is pre-installed with the IK Chinese tokenizer and the boolean mode, which is more efficient than traditional natural language processing. We will use the [Chinese football sports news dataset](https://obbusiness-private.oss-cn-shanghai.aliyuncs.com/online-experience/distributed-sql/sports_data_whole.csv) to import data into OceanBase Database. We will create a partitioned table named `sport_data_whole` with three columns of variable-length characters (`event`, `date`, and `news`). We will also create a full-text index on the `news` column using the IK Chinese tokenizer in `max_word` mode.

<main id="notice" type='explain'>
<h4>Note</h4>
<p>The <code>smart</code> mode and <code>max_word</code> mode of the IK tokenizer differ in that the former stops matching after the longest word is found, while the latter continues to match shorter words. <br>OceanBase Database also includes other built-in tokenizers, such as space and beng for English, and ngram for splitting by character length.</p>
</main>

```sql
-- Create a table and create a full-text index using the IK tokenizer
...
After importing the data, the table contains a total of 5,268 news articles, with an average length of approximately 2,700 Chinese characters. The original data size is about 57 MB. After efficient compression, the total storage space required is less than 30 MB. This is mainly due to the large amount of data stored in the inverted and forward auxiliary tables of the full-text index, which contain a significant number of tokenization records.
...
With the stored news dataset and index, we can perform multi-condition combination or highly filtered searches. For example, as a fan, if I want to search for news containing both "Bayern" and "own goal," I can use the Boolean mode.

Compared to string `LIKE` matching without an index, the Boolean mode syntax is more concise and queries faster.

```sql
-- Query using Boolean mode to find news containing both "own goal" and "Bayern"
...
We can further rank the returned two news articles based on the scores in the output to determine which one is more relevant to the query.
...
Additionally, Boolean mode allows us to exclude certain keywords. For example, fouls are almost always present in football matches. If I want to know which matches are very intense but without yellow cards, red cards, or even fouls, I can use the `-` operator in Boolean mode.
...
For example, the following manual tokenization result shows that the dictionary does not support the names of foreign sports stars (such as Boateng and Götze), which may lead to poor search results when searching by name.

1. Use the `TOKENIZE` function to verify the tokenization effects of tokenizers:

   ```sql
   -- Verify the tokenization effects of Chinese sports news using the ik_smart tokenizer.
   SELECT TOKENIZE('Boateng launched a counterattack from the right flank, passing the ball to Götze, who then turned and shot from 8 meters out, scoring past Piatov', 'ik', '[{"additional_args": [{"ik_mode": "smart"}]}]');
   ```
   ...
   ```sql
   +---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
   | TOKENIZE('Boateng launched a counterattack from the right flank, passing the ball to Götze, who then turned and shot from 8 meters out, scoring past Piatov', 'ik', '[{"additional_args": [{"ik_mode": "smart"}]}]')                                                                                          |
   +---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
   | ["Boateng", "launched", "a", "counterattack", "from", "the", "right", "flank", "passing", "the", "ball", "to", "Götze", "who", "then", "turned", "and", "shot", "from", "8", "meters", "out", "scoring", "past", "Piatov"]                                           |
   +---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
   1 row in set
   ```

   The result does not recognize the names "Boateng" and "Götze".

2. Next, execute the following statement to query whether the target documents are hit:

   ```sql
   -- Boolean mode search for news about specific players.
   SELECT COUNT(*)
   FROM sport_data_whole
...
The above result indicates that the target record is not matched.

3. To update the system dictionary, you can first insert the Chinese name into the system dictionary table. After updating the system dictionary, you need to refresh the cache.
...
5. Verify the optimization effect of the tokenizer:

   ```sql
   -- Execute the tokenizer test again (with the same input).
   SELECT TOKENIZE('Boateng launched a counterattack on the right flank, passing the ball to Götze, who then turned and shot from 8 meters out, scoring from under Piatov', 'ik', '[{"additional_args": [{"ik_mode": "smart"}]}]');
   ```
   ...
   ```sql
   +---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
   | tokenize('Boateng launched a counterattack on the right flank, passing the ball to Götze, who then turned and shot from 8 meters out, scoring from under Piatov', 'ik', '[{"additional_args": [{"ik_mode": "smart"}]}]')                                                                  |
   +---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
   | ["Piatov", "scoring", "from", "under", "8", "meters", "out", "shot", "turned", "ball", "Götze", "passed", "right", "flank", "counterattack", "launched", "Boateng"]                                           |
   +---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
   1 row in set
   ```

   The above result indicates that the professional names are correctly identified.

6. Execute the following statement to verify the retrieval accuracy:
   ...
   The above result indicates that 79 relevant records are correctly matched.

## Performance comparison between OceanBase Database and MySQL

To compare the full-text search performance of OceanBase Database with that of MySQL, we use the full-text search feature of MySQL as a reference. However, since MySQL's Chinese text segmentation capability is relatively weak, we selected the `wikir1k` dataset, which contains 369,721 rows with an average of 100 words per row, for the performance comparison.
