...
2. Set tenant parameters.
   ...
   For example, if the resource unit of the test tenant is CREATE RESOURCE UNIT $unit_name max_cpu 26,
...
<main id="notice" type='explain'>
  <h4>Note</h4>
  <p> You can perform the following steps to generate an SQL query statement and then adjust it, or directly use the SQL query statement provided in [GitHub](https://github.com/oceanbase/obdeploy/tree/master/plugins/tpch/3.1.0/queries).  </p>
  <p>To use an SQL query statement provided in GitHub, you must change the value of the <code>cpu_num</code> parameter in the statement to the actual number of CPU cores. </p>
</main>
...
4. Perform a major compaction.
   ...
   <main id="notice" type='notice'>
     <h4>Note</h4>
     <p>To perform a major compaction, you must log on to the sys tenant of the OceanBase cluster as the <code>root</code> user. </p>
   </main>
...
5. Check whether the major compaction is completed.
   ...
   <main id="notice" type='explain'>
    <h4>Note</h4>
    <p> The major compaction task is completed when the value of <code>frozen_version</code> equals that of <code>last_merged_version</code> of all zones.   </p>
   </main>
...
* Q: What do I do when an error occurred when I was viewing the data? Error message:
...
In the test environment prepared for automatically running the TPC-H benchmark, you can manually run the TPC-H benchmark to verify the OLAP capabilities and features of OceanBase Database.
...
Then, run the following command to start running the TPC-H benchmark with a dataset size of 1 GB. The entire process contains dataset generation, schema import, and automatic benchmark running. In this example, it is assumed that your test environment is deployed in the same way as that in [Get started with OceanBase Database](../1.quickly-experience-oceanbase/1.quickly-experience-oceanbase-for-enterprise.md). You can modify the cluster name, password, or installation directory as needed in the case of any differences.

  <main id="notice" type='notice'>
    <h4>Note</h4>
    <p>Make sure that your disk space is sufficient to store the dataset files. Otherwise, system exceptions will occur if your disk space are fully occupied by these files. </p>
  </main>
