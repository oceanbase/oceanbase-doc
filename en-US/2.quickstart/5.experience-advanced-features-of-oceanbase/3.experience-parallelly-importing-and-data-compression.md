...
<main id="notice" type='notice'>
    <h4>Notice</h4>
    <p>For security reasons, the following authorization SQL statement can only be executed locally but cannot be executed on a remote OBClient. In other words, you must log on to an OBClient (or a MySQL client) on the server where the observer process runs and execute the authorization statement. </p>
  </main>
