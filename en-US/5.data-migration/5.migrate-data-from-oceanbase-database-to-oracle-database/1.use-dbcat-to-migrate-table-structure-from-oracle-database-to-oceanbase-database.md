...
Run the following command to migrate all schemas from the test database in an Oracle tenant of OceanBase Database V4.0.0 to an Oracle 12c database. 
...
The data exported by using DBCAT is an SQL file. To import the SQL file to the Oracle database, access the Oracle database and then run the `@File name` command to import the SQL file. Example:
