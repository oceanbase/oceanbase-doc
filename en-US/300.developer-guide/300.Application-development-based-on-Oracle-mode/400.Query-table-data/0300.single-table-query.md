...
Note the difference between the `WHERE` and `HAVING` clauses. The `WHERE` clause filters data before grouping (aggregation), whereas the `HAVING` clause filters data after grouping and returns the entire SQL query result.
...
* You can remove duplicate rows in a query. Examples:
...
* You can use the `ROWNUM` clause to limit the number of rows returned per page. This feature is often used in pagination. Examples:
...
In addition, you can use the `WHERE` clause to query specified columns. For more information, see [WHERE queries](../400.Query-table-data/0400.condition-query.md).
For more information, see [Use operators and functions in queries](0900.Use-operators-and-functions-in-queries/0100.use-arithmetic-operator-in-query.md).
