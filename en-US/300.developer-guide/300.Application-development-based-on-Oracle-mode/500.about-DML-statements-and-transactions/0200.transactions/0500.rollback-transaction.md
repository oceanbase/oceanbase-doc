...
In the following example, `ROLLBACK` is used to revoke all changes made by the current transaction. To roll back the transaction to a savepoint, see [Example: Roll back a transaction to a savepoint](../0200.transactions/0300.transaction-savepoint.md).
