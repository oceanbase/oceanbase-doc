...
In scenarios with multiple concurrencies, the same row of data may be operated by two transactions. For query reads, you can use the `SELECT FOR UPDATE` statement to lock the query results to prevent other DML statements from modifying this record. For information about how to use the `SELECT FOR UPDATE` statement, see [Use the SELECT FOR UPDATE statement to lock query results](4.read-data-for-oracle-mode/8.use-operators-and-functions-in-query-for-oracle-mode/10.lock-query-result-select-for-update.md). 
...
For more information about fields in the `$OB_TRANSACTION_PARTICIPANTS` view, see [$OB_TRANSACTION_PARTICIPANTS](../../7.reference/2.manage-guide/8.system-reference/5.system-view-for-oracle/3.performance-view-6/49.v-ob_transaction_participants-1.md). 
