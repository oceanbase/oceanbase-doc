...
* Enhanced full-text index capabilities
  ...
  Starting from V4.3.1, OceanBase Database has supported full-text indexing in MySQL mode. As the version evolves, additional features and extensions have been gradually added. The new version of V4.3.5 BP1 introduces the built-in Chinese `IK` full-text indexing engine, supporting two modes: `SMART` and `MAX_WORD`. It also allows users to add custom full-text indexing engines in plugin form. The new version also adds a new attribute, `parser_properties`, for full-text indexing indexes, allowing configuration at the table level. Apart from the existing natural language mode for full-text indexing, the new version also supports boolean mode, which can be activated using the phrase `IN BOOLEAN MODE` in the `MATCH AGAINST` clause. Boolean mode supports boolean operators such as `+`, `-`, `()`, and no operation. Additionally, this release supports merging full-text indexes, using the index union merge computation to enhance query performance. The update/ deletion of full-text indexed columns in the primary table is optimized to improve update/delete performance.
...
* Support for PL/SQL wrap in Oracle mode
  ...
  The PL/SQL wrap feature allows users to encrypt and obfuscate the source code of PL/SQL objects, preventing anyone from viewing the plaintext source code of encrypted objects from the `*_SOURCE` views. This helps meet business needs for protecting core algorithms and processes. OceanBase provides two methods for PL/SQL source code encryption: the Wrap Utility and the DBMS_DDL system package.
...
* Support for fast column deletion in Oracle mode
  ...
  In V4.3.5 BP1, the column deletion operation was changed from actual deletion to marking as deleted, significantly improving deletion performance. However, columns marked as deleted are stored as hidden columns. If a table accumulates 128 hidden columns due to frequent deletions, it will block future column additions and deletions. To clear hidden columns, users need to execute the `ALTER TABLE FORCE` command, which locks the table and reorganizes the data, blocking other transactions. It's worth noting that in the Oracle mode of OceanBase Database, only `ALTER TABLE TABLE_NAME DROP COLUMN C1` or `DROP (C1, C2, C...)` will trigger fast column deletion. If a single SQL statement contains DDL operations in addition to column deletion, it will continue to follow the offline path.
...
* Fixed the memory leak issue in some specific scenarios.
...
* Fixed the issue of failed memory reuse for SQL audit.
...
* **Support for nested materialized views**
    ...
    For more information, see the section on creating nested materialized views in [Create a materialized view (MySQL mode)](https://en.oceanbase.com/docs/common-oceanbase-database-10000000001976540) or [Create a materialized view (Oracle mode)](https://en.oceanbase.com/docs/common-oceanbase-database-10000000001976550).
...
* **Specific partition-based direct load**
    ...
    For more information, see [Full direct load](https://en.oceanbase.com/docs/common-oceanbase-database-10000000001971205) or [Incremental direct load](https://en.oceanbase.com/docs/common-oceanbase-database-10000000001971203).
...
* **Enhancements to full-text indexing functionality**
    ...
    * Supports Functional Lookup:
    ...
    For more information, see [MATCH AGAINST](https://en.oceanbase.com/docs/common-oceanbase-database-10000000001976556).
...
* **Enhancements to SELECT INTO OUTFILE functionality**
    ...
    * You can export data in the ORC format.
    ...
    For more information, see [Export data by using the OUTFILE statement](https://en.oceanbase.com/docs/common-oceanbase-database-10000000001971212).

* **Enhancements to external table functionality**

    In V4.3.5, external tables can read data in the ORC format.

    For more information, see [Create an external table (MySQL mode)](https://en.oceanbase.com/docs/common-oceanbase-database-10000000001971659) or [Create an external table (Oracle mode)](https://en.oceanbase.com/docs/common-oceanbase-database-10000000001971694).
...
* **Support for the SELECT ... FOR UPDATE SKIP LOCKED clause**
    ...
    For more information, see [Lock query results with SELECT FOR UPDATE](https://en.oceanbase.com/docs/common-oceanbase-database-10000000001974038).
...
* **Support for asynchronous online DDL in row-to-column conversion operations**
    ...
    For more information, see [Online and offline DDL operations](https://en.oceanbase.com/docs/common-oceanbase-database-10000000001974221) and [Modify tables](https://en.oceanbase.com/docs/common-oceanbase-database-10000000001973677).
...
* **Expansion of parallel DDL features**
    ...
    In the current version of OceanBase Database, parallel DDL and serial DDL are mutually exclusive, resulting in suboptimal performance when they are executed alternately. In scenarios where OMS is used for schema migration, OceanBase Database already supports parallel execution of `CREATE TABLE`. However, other DDL operations that alternate with `CREATE TABLE` during the migration process are partially serial, affecting migration efficiency. V4.3.5 adds support for the following parallel DDL operations: setting `COMMENT` on tables or columns and executing `CREATE INDEX` in parallel in Oracle compatiable mode.
...
* **Optimization of parallel migration of log stream replicas**
    ...
    For more information, see [replica_parallel_migration_mode](https://en.oceanbase.com/docs/common-oceanbase-database-10000000001976675).
...
* **Support for more ARRAY expressions**
    ...
    For more information, see [Overview of array functions](https://en.oceanbase.com/docs/common-oceanbase-database-10000000001974941).
...
| ODC         | ODC V4.3.2 BP2      | - |
| OBCDC       | OBCDC V4.3.5        | - |
| OMS         | <ul><li>If the destination is OceanBase Database V4.3.5, OMS V4.2.4 BP1 is recommended.</li><li>If the destination is OceanBase Database V4.3.5, contact OceanBase Technical Support.</li></ul> | - |
| OCCI        | OCCI V1.0.4         | - |
| OBCI        | OBCI V2.0.11        | - |
