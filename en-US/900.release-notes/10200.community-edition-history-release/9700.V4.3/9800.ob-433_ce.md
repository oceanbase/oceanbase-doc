...
* **ARRAY data type**
   ...
   `ARRAY` is a complex data type commonly used in AP business scenarios to store multiple elements of the same type. It is an ideal choice for managing and querying multi-valued attributes that cannot be effectively represented by relational data. Starting from OceanBase Database V4.3.3, the `ARRAY` data type is supported in MySQL-compatible mode. You can define a column as either a numeric or character array, including nested arrays, when creating a table. It supports expressions for querying or writing to array objects, `array_contains` expressions, and the `ANY` operator to check if an element is present in the array. Additionally, it supports operators like `+`, `-`, `=`, and `!=` for calculations and comparisons of array elements.
...
* **[Patch from V4.2] Support for more character sets and collations**
...
* **[Patch from V4.2] Memory usage optimization for SQL_AUDIT**
...
* **[Patch from V4.2] Compatibility with the REFERENCES, CREATE ROLE, DROP ROLE, and TRIGGER privileges of MySQL**
...
| The meaning of the `tenant_id` column in the `[G]V$OB_SQL_AUDIT` view is modified. | Some internal requests generated in a user tenant are initialized in the sys tenant. When such an internal request needs to read data, it reads data from the user tenant. In this case, the value of `tenant_id` is `1`, and that of `effective_tenant_id` is the ID of the current tenant. For sequential collection requirements based on the platform within a tenant, the new version adjusts the `tenant_id` column, which can be used as an index, to be equivalent with the `effective_tenant_id` column. |
...
| information_schema.tables | Modified | <ul><li>The `DATA_LENGTH` column is modified to indicate the size of storage space occupied by tables. </li><li>The `INDEX_LENGTH` column is modified to indicate the size of storage space occupied by non-primary key indexes. </li><li>The values of the columns are calculated based on macroblocks, in bytes.</li></ul> |
| information_schema.partitions | Modified | <ul><li>The `DATA_LENGTH` column is modified to indicate the size of storage space occupied by partitions. </li><li>The `INDEX_LENGTH` column is modified to indicate the size of storage space occupied by partition indexes. </li><li>The values of the columns are calculated based on macroblocks, in bytes.</li></ul> |
| information_schema.role_column_grants | New | Displays the table-level privileges of activated roles granted to the user in the current session, and the table-level privileges of other roles granted to these roles. This view is available only in MySQL-compatible mode. |
| information_schema.role_routine_grants | New | Displays the routine privileges of activated roles granted to the user in the current session, and the routine privileges of other roles granted to these roles. This view is available only in MySQL-compatible mode. |
| information_schema.role_table_grants | New | Displays the column privileges of activated roles granted to the user in the current session, and the column privileges of other roles granted to these roles. This view is available only in MySQL-compatible mode. |
...
|----------------------------------|------------|---------|
| decode_trace_id | New | Parses `trace_id` to obtain the IP address and port number of the OBServer node that initiates the SQL request. This function is applicable in both MySQL and Oracle modes. |
...
The following table lists the recommended versions of platforms and tools for OceanBase Database V4.3.3_CE.
