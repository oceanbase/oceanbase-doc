...
* Fixed the issue where the return result of `DBMS_UTILITY.GET_HASH_VALUE(ROWID,1,10000)` was incompatible in Oracle-compatible mode. 
...
* Fixed the issue where in Oracle-compatible mode, when a package variable was used as the output parameter for internal calls, the stored procedure execution result was incompatible with that of the native Oracle Database. 
...
* The system package UTL_RAW in Oracle-compatible mode supports the `CAST_TO_NVARCHAR2()` function, which converts RAW values into NVARCHAR2 values. 
* DBLinks support DNS resolution in Oracle-compatible mode. 
* The system functions `XMLAGG()`, `XMLPARSE()`, and `XMLELEMENT()` are supported in Oracle-compatible mode. 
* The `FROM_BASE64()` and `TO_BASE64()` functions are supported in MySQL-compatible mode. 
* The compatibility with the system variables `DEFAULT_STORAGE_ENGINE` and `EVENT_SCHEDULER` is improved in MySQL-compatible mode. 
...
* Fixed the issue where the execution result was wrong in MySQL-compatible mode because the filters applied by the `NEST LOOP JOIN` operator was abnormal. 
...
* Fixed the issue where in Oracle-compatible mode, when `AUTO COMMIT` was disabled, after some data was inserted, the `SELECT` statement in a user-defined function (UDF) could not read uncommitted data. 
* Fixed the issue where in Oracle-compatible mode, when `AUTO COMMIT` was enabled, after a transaction was manually started and the executed PL package contained DML statements, the transaction was automatically committed. 
...
* Fixed the issue where in Oracle-compatible mode, Error 4179 was returned when a cursor for looping operations was used in a package.
