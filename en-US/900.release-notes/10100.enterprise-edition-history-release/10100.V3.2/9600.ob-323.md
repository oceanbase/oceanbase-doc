...
* Fixed the issue of physical restore failure when using the `TIMESTAMP` or `TIMESTAMP WITH LOCAL TIME ZONE` data types as primary keys in a table in Oracle-compatible mode, after enabling TDE encryption, writing data, and archiving the generated clogs.
* Fixed the issue of stack overflow caused by excessive loop iterations in stored procedures with `GOTO` (Oracle-compatible mode) or `ITERATE` (MySQL-compatible mode) statements.
* Fixed the issue where when the `getBigDecimal` API of JDBC V1.1.10.6 was used in Oracle-compatible mode, a loss of significance occurred after `Scale` was set to `0`. 
...
* Fixed the issue where in Oracle-compatible mode, the scale and precision of the data returned by the `ResultSetMetaData` API were inconsistent with those returned in Oracle Database. 
* Fixed the issue where an error might return during cross-user schema identification if no database was specified for public synonyms in Oracle-compatible mode. 
Fixed the issue where executing an `insert into select` statement in Oracle-compatible mode, without hitting any partition, would result in an Oracle-compatible tenant executing the insertion of 0 rows successfully, but the OBServer node would throw an error.
* Fixed the issue where in MySQL-compatible mode, the output sequence of the `WM_CONCAT` function was inconsistent with that in Oracle-compatible mode. 
...
* Fixed the issue where an error was returned for a ROWNUM query in Oracle-compatible mode. 
...
* Fixed the issue where in Oracle-compatible mode, information about DBLink-related views could not be displayed by running the `DESC` command.
* Fixed the issue where in Oracle-compatible mode, when you use the `TO_NUMBER` function to convert a value of the NCHAR data type, the `Internal error` message was returned.
...
* The `FROM_BASE64()` and `TO_BASE64()` functions are supported in MySQL-compatible mode.
...
* Fixed the issue where in Oracle-compatible mode, Error 4016 was returned during the execution of a hierarchical query due to different parameter values.
...
* Fixed the issue where when an SQL statement was executed in PS mode in an Oracle-compatible tenant, the following error message was returned because the value of the bound variable was not real: `field list ambiguously defined`.
* Fixed the issue where when an SQL statement was executed in an Oracle-compatible tenant, the `ORDER BY` clause supported only valid columns. Constants or aliases were unsupported.
...
* In MySQL-compatible mode, the ADDTIME() function is supported to add the specified time interval to the given date and time. For example, `SELECT ADDTIME('2007-12-31 23:59:59.999999', '1 1:1:1.000002');`.
* In MySQL-compatible mode, the DAYNAME() function is supported to return the name of the workday of the given date.   For example, `SELECT DAYNAME('2018-01-8')`
...
* The upper limit of window functions is changed. A maximum of 32 window functions is allowed in OceanBase Database in both Oracle and MySQL modes. In Oracle-compatible mode, the number of allowed window functions is no longer limited and is consistent with that in Oracle. In MySQL-compatible mode, the number of named window functions cannot exceed 127.
...
* Fixed the issue where in Oracle-compatible mode, if the character set of the tenant was set to GBK, Error 4285 was returned when the `UTL_I18N.STRING_TO_RAW` function was executed because the character set specified for parameters is invalid.
...
* Fixed the issue where in MySQL-compatible mode, when the SQL mode was set to `NO_ZERO_DATE` or `STRICT_ALL_TABLES`, the query results returned by using the `STR_TO_DATE` function were different from those in MySQL.
...
* Fixed the issue where in Oracle-compatible mode, the system returned an error indicating PL primary key conflicts when the `OCI_BATCH_ERRORS` handle was executed by using OBCI.
* Fixed the issue where in MySQL-compatible mode, a large number of CPU resources were consumed when ArcGIS expressions were executed in parallel. As a result, the client terminated the execution of the SQL statement. However, the background thread continued the execution.
...
* In MySQL-compatible mode, the definition and acquisition of the system variable `have_query_cache` are supported. The value of this system variable is always `NO`.
* In MySQL-compatible mode, the `NO_ZERO_DATE` and `ERROR_FOR_DIVISION_BY_ZERO` modes are added, and the `NO_AUTO_CREATE_USER`, and `NO_ENGINE_SUBSTITUTION` modes are supported.
* In Oracle-compatible mode, the value of the `data_length` field in the `ALL_TAB_COLUMNS` view is compatible with the field length in Oracle.
* In Oracle-compatible mode, the length of a created variable array must be a positive integer.
...
The MySQL-compatible version numbers in MySQL-compatible mode are displayed since OceanBase Database V3.2.3 BP4. The following table lists the versions of the required components.
|Component|Version|
|---|----|
|ODP| V3.2.3.2|
|OCP |V3.3.2|
...
* Fixed the issue where the observer process unexpectedly exited in Oracle-compatible mode while processing a query that used the `ORA_ROWSCN` pseudocolumn.
...
* Fixed the issue where when you create a DBLink to query data in a table in Oracle-compatible mode, an error occurs if the Oracle character set is set to `WE8ISO8859P1`.
...
* In Oracle-compatible mode, default values of column fields can be defined by using the Sequence function.
...
* Fixed the issue where when a prepared statement was enabled in Oracle-compatible mode, the return value of the` NUMTODSINTERVAL()` function was truncated to a specific precision.
* Fixed the issue where after deadlock detection was enabled, memory usage of LockWaitMgr in the sys tenant soared.
* Fixed the issue where a character set conversion error occurred in Oracle-compatible mode when a DBLink was created to query data in the Oracle database.
...
* Fixed the issue where an error occurred when the `DBMS_JOB` package was used to create a scheduled task in Oracle-compatible mode, and a value was assigned to the `INTERVAL` parameter to extract the date.
* Fixed the issue where a compilation error was returned when the `SELECT INFO` statement was used in a stored procedure in Oracle-compatible mode to define a cursor.
...
* Fixed the issue where an error occurred when a trigger was executed in MySQL-compatible mode by using an `INSERT` statement to insert an object into a column of the SET data type.
...
* The `DETERMINISTIC` attribute is supported for stored procedures and methods in MySQL-compatible mode.
...
* Fixed the issue where the query of the `information_schema.columns` view in MySQL-compatible mode by using a null condition returned incorrect column values.
...
* The keyword `DISTINCTROW` is supported in MySQL-compatible mode.
...
* In MySQL-compatible mode, the behavior of CHECK constraints is changed to be compatible with MySQL 8.0. The earlier versions are compatible with MySQL 5.7.
...
* In Oracle-compatible mode, OceanBase Database returns an error if you update a field in the UPDATE statement multiple times, which is the same in Oracle.
...
* Fixed the issue where an error occurred if you use the `CREATE TABLE AS` statement to create a partitioned table in MySQL-compatible mode.
...
* Fixed the issue where the length of a table name in MySQL-compatible mode was inconsistent with the maximum length in a MySQL database.
