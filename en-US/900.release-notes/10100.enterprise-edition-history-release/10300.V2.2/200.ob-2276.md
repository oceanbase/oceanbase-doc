...
* In Oracle-compatible mode, the observer process unexpectedly exits if you drop a user and then drop the trigger of the user.
...
* In Oracle-compatible mode, an error occurs because a special character contained in the CHAR type column is smaller than a space.

* The XA_ROLLBACK function can be hung.

* In Oracle-compatible mode, the parallel execution returns error 4016: dop of child dfo is not right.

* In Oracle-compatible mode, an error occurs during the conversion from the DOUBLE type to the NUMBER type when PS mode is enabled.
...
* In MySQL-compatible mode, an error may occur if a PL block contains multiple SQL statements.