...
* Fixed an issue that if a `PACKAGE` variable is used as the output parameter for internal calls in Oracle-compatible mode, the execution results of stored procedures are incompatible with that of an Oracle database.
...
* Fixed an issue that the binary comparison of the `FIND_IN_SET(binary 'b','a,B,c,d')` function does not take effect in MySQL-compatible mode.
* Fixed an issue that the execution of the `FIND_IN_SET` function is slow in MySQL-compatible mode.
...
In MySQL-compatible mode, the `ORDER BY` or `LIMIT` operator cannot be used for multi-table updates, which is consistent with MySQL. 
...
* Fixed an issue that in Oracle-compatible mode, when a package uses a cursor for loop operations, Error 4179 is returned.
...
* Fixed an issue that in MySQL-compatible mode, an error is returned for invalid characters generated during the conversion of the GBK character set by calling the `CONVERT()` function. In this version, the invalid characters are converted to 0x3F, which is compatible with MySQL.
...
* Fixed an issue that in Oracle-compatible mode, when a prepared statement is enabled, the return value of the `NUMTODSINTERVAL()` function is truncated to a specific precision.
...
* Fixed an issue that in Oracle-compatible mode, when you create a stored procedure with the specified character set GBK, the OBServer node becomes suspended unexpectedly in the SQL prepare phase.
* Fixed an issue that in MySQL-compatible mode, an error is returned because the Limit operator does not support character type conversion.
...
* Fixed an issue that Error 4007 is returned when the `DBMS_LOB.SUBSTR` system package is executed in Oracle-compatible mode.
...
* Fixed an issue that in MySQL-compatible mode, an error is returned during the parsing of the `GROUP BY expression` because of unclear semantics. In this version, the `expression` from the selection list is used to avoid errors.
...
* The `USERENV('SID')` and `USERENV('LANGUAGE')` functions are supported in Oracle-compatible mode.
...
* Fixed an issue that in Oracle-compatible mode, a false positive error is returned, which indicates that the total row length of all columns exceeds the value of the `OB_MAX_USER_ROW_LENGTH` parameter.
...
* Fixed an issue that in Oracle-compatible mode, no privilege is provided to control the `TRUNCATE DLL` operation.
...
* Fixed an issue that in Oracle-compatible mode, the `TO_CHAR()` function cannot recognize the date format.

* Fixed an issue that an error occurs when checking is conducted for the index created on a column of the `CHAR` type in Oracle-compatible mode.
...
* Fixed an issue that in Oracle-compatible mode, the `ALL_TAB_PARTITIONS` view displays partitions in an unordered way.
...
* Fixed an issue that in Oracle-compatible mode, the `pivot` and `unpivot` clauses are incompatible with Oracle.
...
* Fixed an issue that in Oracle-compatible mode, the error `unknown timezone` is returned for the OBClient connection after you specify the `global time_zone` system variable.

* Fixed an issue that in MySQL-compatible mode, the user privileges on a table remain valid after the table is deleted.

* Fixed an issue that some IP addresses in the `gv$sql_audit` view are parsed into `0.0.0.0`.

* Fixed an issue that in Oracle-compatible mode, an error is returned when the `SELECT * FROM (GENERATED TABLE)` statement is executed due to duplicate column names.
...
* Fixed an issue that in Oracle-compatible mode, the value of the `PARTITION KEY COUNT` field in the `ALL_PART_TABLES` table is not displayed.

* Fixed an issue that the tenant restore operation fails when the tenant is deleted.

* Fixed an issue that in MySQL-compatible mode, the execution of the `SET PASSWORD` statement in stored procedures does not take effect.

* Fixed an issue that the creation of `views` in PL fails in MySQL-compatible mode.
...
* Fixed an issue that in Oracle-compatible mode, the use of `TRIGGER` causes the `SQL EXECUTOR` module to have memory leaks.
...
* The `LOAD DATA REPLACE` syntax is supported in Oracle-compatible mode.

* The user login and logoff statistics are collected.

* The `ORDER BY` expression is supported in MySQL-compatible mode.
...
* Fixed an issue that the observer process unexpectedly exits in Oracle-compatible mode because of the execution of the `SELECT FETCH UNION SELECT FETCH` syntax.
...
You can set the `SQL_MODE` parameter to `NO_ZERO_IN_DATE` in MySQL-compatible mode.

### Compatibility changes

The following default actions of the `SQL_MODE` parameter are changed in MySQL-compatible mode:
...
* The `MAKETIME()` function is supported in MySQL-compatible mode.

* The `UTC_DATE(), UTC_TIME(), and GET_FORMAT()` functions are supported in MySQL-compatible mode.

### Bug fixes

* Fixed an issue that `XA transactions` in the `Active` status cannot be rolled back.

* Fixed an issue that in Oracle-compatible mode, if the index column contains the `CHAR` data type and values that are less than or equal to 0, the index column cannot be queried.
...
* The `LOG(), LN(), PI(), RADIANS(), ATAN(), ASIN(), ACOS(), and COT()` functions are supported in MySQL-compatible mode.

* The `MONTHNAME()` function is supported in MySQL-compatible mode.

* The `LAST_DAY()` function is supported in MySQL-compatible mode.

* The `QUARTER()` function is supported in MySQL-compatible mode.

* The `BIT_LENGTH()` function is supported in MySQL-compatible mode.
...
* In MySQL-compatible mode, using escape characters such as single quotation marks (' ') in a `LIKE` statement is supported.
...
* Fixed an issue that in Oracle-compatible mode, the `WIDTH_BUCKET()` function is incompatible with Oracle when `min_value` is less than `expression`.

* Fixed an issue that in Oracle-compatible mode, the `RPAD()` function is incompatible with Oracle.
