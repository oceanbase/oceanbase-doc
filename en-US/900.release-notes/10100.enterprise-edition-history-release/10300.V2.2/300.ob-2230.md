...
* When the Perl driver is used for the MySQL-compatible mode of OceanBase Database, the INSERT ... ON DUPLICATE statement returns the number of affected rows that is inconsistent with the number of the affected rows of the MySQL database.
...
* We recommend that you do not use the NVARCHAR2 data type in Oracle-compatible mode. Otherwise, the thread that executes the SQL query may not respond.

* In Oracle-compatible mode, you must not set sql_mode to PAD_CHAR_TO_FULL_LENGTH. Otherwise, a query error may occur.

* In Oracle-compatible mode, you must not specify the alias of a system view in the statement when you query the system view. Otherwise, an error is returned.
...
* Number system conversion by using the to_number function is supported in Oracle-compatible mode.

* The query performance of the all_tab_columns view in Oracle-compatible mode is optimized.

### Fixed issues

* If a PL package is used in Oracle-compatible mode when the tenant memory is running out, the plan cache may access an invalid address, which leads to the unexpected exit of the observer process.
...
* An error occurs during the comparison between the CHAR and VARCHAR data types of OceanBase Database in Oracle-compatible mode.

* The execution of the SUBTIME() function in MySQL-compatible mode may cause an out-of-bounds error and returns the error code 4016.
...
* You add columns to a table in MySQL-compatible mode and specify the column order, but the results are not as expected.
...
* In MySQL-compatible mode, an error is returned during the conversion from the STRING data type to the INT data type.
...
* In Oracle-compatible mode, an error occurs due to incorrect processing of the null values in the `UPDATE SET` statement.
...
* We recommend that you do not use the NVARCHAR2 data type in Oracle-compatible mode.
...
* In Oracle-compatible mode, an error occurs during the execution of the INSERT (SELECT FROM DUAL) UNION (SELECT FROM DUAL) statement.
...
* The connection with a MySQL-compatible tenant by using UNIX domain sockets is supported.
...
* In Oracle-compatible mode, the TO_TIMESTAMP() function always populates the date in UTC.
