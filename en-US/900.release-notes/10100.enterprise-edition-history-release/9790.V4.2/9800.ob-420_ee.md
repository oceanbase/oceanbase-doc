...
* Fixed the issue where, in the Oracle-compatible mode, when the RS tenant and the regular tenant are not on the same server, Error 4013 may occur during SQL execution if memory allocation using the tenant ID is attempted on the RS server.
...
* **MySQL compatibility**
   * **Function index**: OceanBase Database already supports function-based indexes in earlier versions in Oracle-compatible mode. The MySQL-compatible mode of V4.2.0 now supports MySQL 8.0-compatible function-based indexes, further enhancing feature stability. 
...
* **Oracle compatibility**
    ...
    * **LOCK TABLE**: OceanBase Database provides LOCK TABLE syntaxes for Oracle-compatible tenants in V4.1.0 and earlier versions. However, many limitations on using the syntaxes exist. OceanBase Database V4.2.0 provides higher compatibility with LOCK TABLE syntaxes and enhances the features that are unavailable in earlier versions. Specifically, you can lock multiple tables, partitions, and subpartitions, and make the WAIT N and NOWAIT keywords take effect. 

* **Multi-model support**

    * **Oracle XML**: The XMLType data type is supported in Oracle-compatible mode. Nine XML expressions and basic XML creation, query, and modification capabilities are provided. 
...
GB18030-2022 character set was released on July 19, 2022, and formally implemented on August 1, 2023, becoming a mandatory standard for full-text execution in information systems. Compared with the GB18030-2005 standard, the new standard supports more rare Chinese characters and minority languages. OceanBase Database V4.2.0 supports the new GB18030_2022 character set, along with the corresponding sorting methods. The character set name is GB18030_2022 in MySQL-compatible mode and ZHS32GB18030_2022 in Oracle-compatible mode. 
...
| --- | --- | --- | --- |
| `DBMS_SQL.LAST_ERROR_POSITION` | 4.2 | New | Returns the position where the syntax error occurs during the last call to DBMS_SQL.PARSE. This package applies to the Oracle-compatible mode.  |
| `DBMS_SESSION.RESET_PACKAGE` | 4.2 | New | Clears the status of all package variables in the current session. This package applies to the Oracle-compatible mode.  |
