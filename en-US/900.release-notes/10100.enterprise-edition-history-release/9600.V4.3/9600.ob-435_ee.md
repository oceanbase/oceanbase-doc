...
* Fixed the issue where the `PERCENTILE_CONT` expression is incorrectly unquoted in MySQL-compatible mode.
...
* Support for the STRING data type in MySQL-compatible mode
...
* Invisible columns in MySQL-compatible mode

  The new version supports invisible columns in MySQL-compatible mode. When a column is marked as invisible, it is ignored unless explicitly specified in an SQL statement.

* Optimization of foreign key functionality in MySQL-compatible mode

  The new version allows foreign keys to reference columns with non-unique indexes in MySQL-compatible mode. It also relaxes the type constraints for parent and child keys and disables circular self-referencing foreign key cascading updates.
...
* Enhanced full-text index capabilities
  ...
  Starting from V4.3.1, OceanBase Database has supported full-text indexing in MySQL-compatible mode. As the version evolves, additional features and extensions have been gradually added. The new version of V4.3.5 BP1 introduces the built-in Chinese `IK` full-text indexing engine, supporting two modes: `SMART` and `MAX_WORD`. It also allows users to add custom full-text indexing engines in plugin form. The new version also adds a new attribute, `parser_properties`, for full-text indexing indexes, allowing configuration at the table level. Apart from the existing natural language mode for full-text indexing, the new version also supports boolean mode, which can be activated using the phrase `IN BOOLEAN MODE` in the `MATCH AGAINST` clause. Boolean mode supports boolean operators such as `+`, `-`, `()`, and no operation. Additionally, this release supports merging full-text indexes, using the index union merge computation to enhance query performance. The update/ deletion of full-text indexed columns in the primary table is optimized to improve update/delete performance.
...
* Support for PL/SQL wrap in Oracle-compatible mode
...
* Enhanced PL functionality
  ...
  The new version supports After Logon and Before Logoff triggers. It optimizes the performance of using index tables within anonymous blocks and query performance for the `mysql.proc` system table. Oracle-compatible tenants can now use the `UTL_SMTP`, `UTL_TCP`, and `DBMS_UTILITY.COMMA_TO_TABLE` system packages. MySQL-compatible tenants can use the new `alter procedure` and `function compile` syntaxes to manually compile stored procedures or functions.
...
* Maximum availability mode for dual IDCs in the same city
...
|----------|-------------|-------------|
| TO_CHAR | New | A new function in MySQL-compatible mode, used to convert data of other types into string type. |
| KEYVALUE | New | A new function in MySQL-compatible mode, used to parse key-value pairs stored in a data table and extract the value corresponding to a given key. |
| URL_DECODE | New | A new function in MySQL-compatible mode, used to decode a string from the `application/x-www-form-urlencoded MIME` format into a normal character string. |
| SPLIT_PART | New | A new function in MySQL-compatible mode, used to split a string `str` by a specified delimiter `separator` and return the substring from the `start` part to the `end` part. |
| PERCENTILE_CONT | New | A new function in MySQL-compatible mode, used to return an interpolated value at a specified percentile based on the sorting specification and percentile value. |

### System package changes

| System package | Change type | Description |
|----------------|-------------|-------------|
| UTL_TCP | New | A new system package in Oracle-compatible mode, used to support communication with external servers through the TCP/IP protocol. |
| UTL_SMTP | New | A new system package in Oracle-compatible mode, used to support sending emails through Simple Mail Transfer Protocol (SMTP). |
...
| DBMS_DDL | New | A new system package in Oracle-compatible mode, used to support encryption of PL/SQL source code. |
...
| Added syntax for subpartition operations in MySQL-compatible mode | Extended the `RANGE [COLUMNS]`/`LIST [COLUMNS]` syntax for adding and dropping subpartitions in MySQL-compatible mode. |
...
| Added syntax such as GROUPING SETS, CUBE, and QUALIFY in MySQL-compatible mode | Added syntax such as `GROUPING SETS`, `CUBE`, and `QUALIFY` in MySQL-compatible mode. |
...
* Fixed the issue where the `UNIQUE` constraint is mistakenly dropped in Oracle-compatible mode, which prevents tables from being dropped.
...
In terms of compatibility, XA transaction functionality is compatible for MySQL-compatible tenants, and the `Latin1` character set is refined. Oracle-compatible tenants support remote procedure calls between tenants with complex type parameters, facilitating smooth migration between Oracle and MySQL businesses.
...
* **Support for nested materialized views**
    ...
    For more information, see the section on creating nested materialized views in [Create a materialized view (MySQL-compatible mode)](https://en.oceanbase.com/docs/common-oceanbase-database-10000000001976540) or [Create a materialized view (Oracle-compatible mode)](https://en.oceanbase.com/docs/common-oceanbase-database-10000000001976550).
...
* **Enhancements to external table functionality**
    ...
    For more information, see [Create an external table (MySQL-compatible mode)](https://en.oceanbase.com/docs/common-oceanbase-database-10000000001971659) or [Create an external table (Oracle-compatible mode)](https://en.oceanbase.com/docs/common-oceanbase-database-10000000001971694).
...
| V$STATNAME                         | New | Displays the names of statistics in Oracle-compatible mode. |
...
| Modified the syntax for changing automatic partitioning attributes in Oracle-compatible mode: [MODIFY] PARTITION BY | V4.3.4 supports only `PARTITION BY`. To ensure consistency with the syntax for modifying partitioning rules in Oracle-compatible mode, the syntax for changing automatic partitioning attributes supports `[MODIFY] PARTITION BY`. |
