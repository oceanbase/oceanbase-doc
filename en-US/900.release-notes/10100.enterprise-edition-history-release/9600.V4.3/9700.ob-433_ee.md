...
* **PL syntax details**
  ...
  OceanBase Database V4.3.3 in Oracle-compatible mode allows you to use the `=>` symbol to pass values to parameters in a custom type.

* **Remote call of stored procedures between Oracle-compatible tenants through a DBLink**

  In OceanBase Database of earlier versions, an Oracle-compatible tenant can call stored procedures from a native Oracle database through a DBLink. In OceanBase Database V4.3.3, one Oracle-compatible tenant can call stored procedures from another through a DBLink. At present, only basic types of parameters are supported.
...
* **Extension of parallel DDL operations**
  ...
  * Setting comments on tables or columns in Oracle-compatible mode.
...
| Change | Description |
|-------------------------------------------------------------|---------|
| The `SHOW CREATE TABLE/VIEW` statement in MySQL-compatible mode and the `DBMS_METADATA.GET_DDL` function in Oracle-compatible mode can display more attributes of materialized views. | In earlier versions, the `SHOW CREATE TABLE/VIEW` statement and `DBMS_METADATA.GET_DDL` function can display only part of the attributes in materialized view definition. The attributes that specify the data refresh type, refresh subtask details, whether to enable query rewriting, whether to use columnar storage, and whether the materialized view is a real-time one, cannot be displayed. In the new version, these attributes of materialized views are displayed by default. |
| An error is returned when a value is assigned to a const variable in a trigger in Oracle-compatible mode. | In earlier versions, no error is returned when a value is assigned to a const variable in a trigger, which is not as expected. In the new version, an error is returned in such cases. |
