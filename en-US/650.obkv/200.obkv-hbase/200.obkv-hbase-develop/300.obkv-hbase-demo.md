|description||
|---|---|
|slug|obkv-hbase-operation-examples|
...
OBKV-HBase supports the Reverse Scan feature, which allows users to scan table data in reverse order by row key (RowKey), from largest to smallest. This is the opposite of the regular forward scan, which scans data from smallest to largest. An example of reverse scanning is shown below:
