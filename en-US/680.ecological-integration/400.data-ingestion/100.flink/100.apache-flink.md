...
This guide is for developers who want to build efficient data pipelines using Apache Flink and OceanBase Database. Whether you are a beginner or an experienced user, this guide will help you: 

* Get up to speed with the core features of Flink.
* Understand the various Flink connectors available for OceanBase Database.
* Choose the best integration solution for your specific business needs.

## Applicable versions

* Flink V1.15 and later.
* The JDBC connector is applicable to OceanBase Database V3.x, V4.x, and later.
* The Flink Connector OceanBase Direct Load, OBKV-HBase Connector, and OBKV-HBase2 Connector are applicable to OceanBase Database V4.2.x and later.
...
Flink SQL is the simplest and most efficient way to use Flink. With a SQL-like syntax, you can:
...
* Write the results to a target system.
...
<main id="notice" type='notice'>
  <h4>Notice</h4>
  <p><ul><li>Connectors cannot be interchanged. Choose the appropriate one based on your specific scenario.</li><li>OceanBase Database MySQL tenants are compatible with Flink MySQL Connector. For Oracle tenants, you must use Flink OceanBase Connector.</li></ul></p>
</main>
...
| Lookup dimension table association | Flink Connector JDBC (Lookup Mode) | Standard JDBC | [Flink Connector JDBC Documentation](https://nightlies.apache.org/flink/flink-docs-release-1.19/zh/docs/connectors/table/jdbc/) |
| Batch read of the entire table | Flink Connector JDBC (Batch Read, Single Parallelism) | Standard JDBC | [Flink Connector JDBC Documentation](https://nightlies.apache.org/flink/flink-docs-release-1.19/zh/docs/connectors/table/jdbc/) |
| CDC data synchronization <main id="notice" type='notice'><h4>Notice</h4><p>Only supported for MySQL mode tenants in OceanBase Database. Oracle mode tenants are not supported. </p></main> | Flink CDC (OceanBase CDC) | Parallel full + incremental read | [OceanBase CDC Documentation](https://nightlies.apache.org/flink/flink-cdc-docs-master/zh/docs/connectors/flink-sources/oceanbase-cdc/) |
| Large-scale data import, TB-level batch data migration | Flink Connector OceanBase Direct Load | Based on direct load, high throughput | [Flink Connector OceanBase Direct Load](https://github.com/oceanbase/flink-connector-oceanbase/blob/main/docs/sink/flink-connector-oceanbase-directload_cn.md) |
| High-performance write of fixed columns in KV format (simple) | Flink Connector OBKV HBase | Based on OBKV API, nested structure | [Flink Connector OBKV HBase](https://github.com/oceanbase/flink-connector-oceanbase/blob/main/docs/sink/flink-connector-obkv-hbase_cn.md) |
| High-performance KV write (advanced feature) | Flink Connector OBKV HBase2 | Flat structure, supports dynamic columns and partial updates | [Flink Connector OBKV HBase2](https://github.com/oceanbase/flink-connector-oceanbase/blob/main/docs/sink/flink-connector-obkv-hbase2_cn.md) |
...
Use the Flink Connector OceanBase. For more information, see [Flink Connector OceanBase](https://github.com/oceanbase/flink-connector-oceanbase/blob/main/docs/sink/flink-connector-oceanbase_cn.md).
...
Use the Flink Connector OceanBase Direct Load. For more information, see [Flink Connector OceanBase Direct Load](https://github.com/oceanbase/flink-connector-oceanbase/blob/main/docs/sink/flink-connector-oceanbase-directload_cn.md).
...
Use the Flink Connector OBKV HBase. For more information, see [Flink Connector OBKV HBase](https://github.com/oceanbase/flink-connector-oceanbase/blob/main/docs/sink/flink-connector-obkv-hbase_cn.md).
...
Flink Connector OBKV HBase2. For more information, see [Flink Connector OBKV HBase2](https://github.com/oceanbase/flink-connector-oceanbase/blob/main/docs/sink/flink-connector-obkv-hbase2_cn.md).
...
Flink CDC (OceanBase CDC). For more information, see [OceanBase CDC official documentation](https://nightlies.apache.org/flink/flink-cdc-docs-master/zh/docs/connectors/flink-sources/oceanbase-cdc/).
...
Flink JDBC Connector (as a lookup source). For more information, see [Flink JDBC Connector official documentation](https://nightlies.apache.org/flink/flink-docs-release-1.19/zh/docs/connectors/table/jdbc/).
...
<main id="notice" type='explain'>
  <h4>Note</h4>
  <p>Here are some recommendations for selection:
  <ul><li>CDC scenarios (full + incremental synchronization): recommend OceanBase CDC (only for MySQL tenants)</li>
  <li>Lookup join scenarios (dimension table join): recommend JDBC Connector.</li>
  <li>Batch full-table scan: recommend OceanBase CDC (MySQL tenant) for performance; for Oracle tenants, JDBC Connector is the only option, but it is slower.</li></ul></p>
</main>
...
* OceanBase CDC: Supports parallel full-table reads and binlog incremental reads, offering high performance and is ideal for CDC scenarios (only applicable to MySQL tenants).
...
* For CDC full-table + incremental reads (MySQL tenant): Use **OceanBase CDC** (recommended).
* For batch reading of full tables: Prefer **OceanBase CDC**. If not supported (e.g., Oracle tenant), use the **JDBC Source**.

### Question 4: Why does OceanBase CDC only support MySQL tenants? What about Oracle tenants?

OceanBase CDC supports the binlog service compatible with MySQL. For Oracle tenants, use the OMS tool to implement CDC incremental reads.
