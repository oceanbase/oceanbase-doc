...
<main id="notice" >
  <h4>Applicability</h4>
  <p>MyBatis is applicable to OceanBase Database in MySQL and Oracle modes. This topic provides an example for the Oracle mode only. </p>
</main>

This topic describes how to build an application based on MyBatis and OceanBase Database to perform basic operations such as table creation, data insertion, and data query.
...
<main id="notice" type='explain'>
  <h4>Note</h4>
  <p>The steps in this topic are based on the Windows environment. If you are using a different operating system or compiler, the steps may vary slightly. </p>
</main>
...
2. Fill in the following URL with the information of the deployed OceanBase Database.
    ...
    * `user_name`: specified by the `-u` parameter. The format is **username@tenant name#cluster name** or **username@SERVICE:service name**. If the **username@tenant name#cluster name** format is used, the default tenant is `sys`, and the administrator is `root`. The cluster name can be omitted for direct connection, but it is required for ODP connection.
...
* `pom.xml`: the configuration file of the Maven project, which contains information such as project dependencies, plugins, and build settings.
* `.idea`: the directory used by the IDE (Integrated Development Environment) to store project-related configuration information.
* `src`: the directory used to store source code in the project.
* `main`: the directory used to store the main source code and resource files.
* `java`: the directory used to store Java source code.
* `com`: the root directory for storing Java packages.
* `oceanbase`: the root directory for storing the project.
* `mapper`: the directory for storing MyBatis Mapper interfaces and XML files.
* `IAppMapper.java`: the interface for the application data access layer.
* `IUserMapper.java`: the interface for the user data access layer.
* `pojo`: the directory for storing JavaBeans or entity classes.
* `App.java`: the entity class for applications.
* `User.java`: the entity class for users.
* `resources`: the directory for storing resource files such as configuration files and SQL files.
* `jdbc.properties`: the configuration file for storing database connection information.
* `mybatis-config.xml`: the configuration file for storing MyBatis settings.
...
* `TestMybatis.java`: the Java class for testing MyBatis.
...
<main id="notice" type='explain'>
  <h4>Note</h4>
  <p>If you just want to verify the example, you can use the default code without any modifications. Alternatively, you can modify the <code>pom.xml</code> file based on your specific requirements as explained below.</p>
</main>
...
2. Configure the namespaces and model version for the POM.
...
4. Define the project's build process using `<build>`.
...
5. Define the components on which the project depends using `<dependencies>`.
   ...
   2. Specify the test framework of the dependency as `junit`, the name as `junit`, and the version as `4.10`.
   ...
   3. Specify the framework of the dependency as `org.mybatis`, the name as `mybatis`, and the version as `3.5.9`.
   ...
   4. Specify the plugin of the dependency as `com.github.pagehelper`, the name as `pagehelper`, and the version as `5.3.0`.
...
1. File declaration.
    ...
    This declaration indicates that the file is an XML file and specifies that the current document is a MyBatis configuration file. It uses XML version `1.0` and character encoding `UTF-8`. The version number is 3.0, the language is English, and the DTD (Document Type Definition) file provided by the official MyBatis website is used for validation.
...
3. Configure MyBatis global settings.

    The content of MyBatis global settings includes the following parts:
    ...
    * `autoMappingBehavior`: used to configure the handling behavior for automatic mapping, set to `PARTIAL`.
    * `defaultExecutorType`: used to configure the default executor type, set to `SIMPLE`.
    * `mapUnderscoreToCamelCase`: used to configure whether to convert database column names with underscores to camel case in Java object properties, set to `true`.
...
4. Configure MyBatis plugins.

    The content of MyBatis plugins includes the following parts:
    ...
    * `supportMethodsArguments`: indicates whether to support passing pagination parameters using method parameters.
...
5. Configure the environment of MyBatis.
   The content of MyBatis environment includes the following parts:
    ...
    * `property`: specifies the class name of the database driver.
    ...
    <main id="notice" type='explain'>
     <h4>Note</h4>
     <p>In other configuration elements of MyBatis, you can use <code>${}</code> placeholders to reference configured data sources, transaction managers, and executors in the environment.</p>
    </main>
...
6. Configure the mapper of MyBatis.
    ...
    The common elements of MyBatis mapper are as follows:
...
The `IUserMapper.xml` file is a mapping file that defines SQL statements related to the user object. This file defines a namespace named `IUserMapper` to store SQL statements related to the user object.
...
2. Configure the mapping relationship with `IUserMapper.java`.
    ...
    6. Create an SQL statement named `selectUserByPage` to query user objects from the `test_user` table for a specified page. This uses subqueries and the `rownum` field for pagination, where `#{pageNum}` and `#{pageSize}` represent the current page number and the number of records per page, respectively.
...
2. Define the `IAppMapper` interface.
   The `IAppMapper` interface defines the MyBatis `Mapper` interface. The `Mapper` interface is used to define SQL mapping relationships and perform CRUD operations on the database. Specifically, the `IAppMapper` interface defines methods for inserting, updating, deleting, and querying data from the `test_app` table, including:
   ...
   * `@Results` method: Represents the mapping relationship of query results, used to map fields in the query result to properties of Java objects.
   **CRUD operations on the database are as follows:**
   1. Insert data
    Map the App object to a record in the `test_app` table. Use `#{property name}` to represent the attribute values of the App object. The `Integer` return value indicates the automatically generated ID returned by the SQL statement after inserting data, which corresponds to the ID field in the `test_app` table.
   ...
   3. Update data
    To modify records in the `test_app` table, perform an Update operation.
...
The `IUserMapper.java` file is used to define methods for database operations.
...
1. Reference other classes and interfaces.
   ...
   * `List` interface: Represents the list type.
   **Code:**
...
2. Define the `App` class.
   ...
   3. Define `getId`, `setId`, `getName`, and `setName` methods for getting and setting the application name. The `setName` method returns the name of the application.
...
The TestMyBatis.java file demonstrates how to use MyBatis for data operations. It tests the basic functionality of the MyBatis framework, the execution of SQL statements, and the calling methods of Mapper interfaces, as well as the parameters and return values of SQL statements.
...
1. Import other classes and interfaces.
    ...
    * `PageInfo` plugin: Packages pagination query results.
    * `Resources` class: Loads MyBatis configuration files.
...
2. Define the `testUserMapper` method.
    ...
    6. Use the `execute()` method of the `Statement` object to create the `test_user` table. This table contains two fields: one is `id`, of type `number(20)`, serving as the primary key; the other is `name`, of type `varchar2(100)`.
    ...
    8. Use a `for` loop to insert 10 records into the `test_user` table. In each iteration, create a new User object and use the `insertUser()` method of the `Mapper` interface to execute the insert operation. The result of the insert operation is stored in the `insertResult` variable.
    9. Perform delete, update, and query operations on the data in the `test_user` table. Finally, use the `forEach()` method to print the information of each user in the user list.
    10. Create a user named `insert` and use the `selectUserByPage()` method to query all user data with the name "insert" in the `test_user` table. Return the user list for the specified page. Query the second page, displaying 3 records per page. Print these user data to the console. Close the `Statement` and `SqlSession` instances, commit the transaction, and release resources.
...
3. Define the `testSqlSession` method.
    ...
    4. Use the `createStatement()` method of the `Connection` instance to create an instance of `Statement`. This allows you to execute SQL statements sequentially.
    ...
    8. Use the `delete()` method of the `sqlSession` object to execute the delete operation. Set the delete condition by passing the parameter 1L. Store the result of the delete operation in the `deleteResult` variable.
    9. Use the `sqlSession` object to execute a database update operation. Create a User object, call the `update` method by passing the SQL statement identifier and parameter object, and complete the update operation. The specific SQL statements and parameter mappings can be found in the XML configuration file of the "com.oceanbase.mapper.IUserMapper" interface. Store the result of the update operation in the `updateResult` variable.
    10. Use the `openSession()` method of the `SqlSessionFactory` to create an instance of `SqlSession`. Use this instance to execute a query operation and store the query results in the `userList` variable. Finally, use the `forEach` method to traverse `userList` and output it to the console.
...
4. Define the `testAppMapper` method.
    ...
    9. Use the `delete()` method of the `mapper` object to execute the delete operation. Set the delete condition by passing the parameter `1L`. Store the result of the delete operation in the `deleteResult` variable.
