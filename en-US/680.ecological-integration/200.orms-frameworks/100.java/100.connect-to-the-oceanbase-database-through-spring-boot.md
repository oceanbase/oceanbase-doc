...
This topic describes how to use the Spring Boot framework and OceanBase Database to build an application and perform basic database operations such as creating tables, inserting data, and querying data.
...
* The tenant account is `root@mysq001`. `mysql001` is a MySQL-compatible tenant created in OceanBase Database, and `root` is the username of a user in the `mysql001` tenant.
* The password is `******`.
