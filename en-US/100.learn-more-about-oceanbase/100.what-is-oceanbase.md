...
In a shared storage architecture, compute nodes and data storage are fully decoupled. Storage can be purchased on demand, and compute nodes can be scaled up or down flexibly, delivering exceptional resource elasticity and cost efficiency. This approach supports all product types, including analytical processing (AP), transactional processing (TP), and key-value (KV).
