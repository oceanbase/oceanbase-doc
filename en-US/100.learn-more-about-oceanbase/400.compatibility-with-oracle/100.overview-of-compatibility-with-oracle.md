...
This topic describes the compatibility between OceanBase Database's Oracle-compatible mode and native Oracle Database.
...
However, due to differences in underlying architecture and product structure, there are some features that OceanBase Database does not support or where it behaves differently from Oracle Database. This topic covers the compatibility between OceanBase Database's Oracle-compatible mode and Oracle Database in the following areas:
...
The Oracle-compatible mode of OceanBase Database supports the following character sets:
...
The following table describes the collations supported by the Oracle-compatible mode of OceanBase Database.
  ...
  | gb18030_2022_bin        | gb18030_2022 | Uses binary collation. Default collation for this character set in Oracle-compatible mode. |
...
* Modify base tables: You can use the `ALTER TABLE` statement to add, drop, and modify columns; add and drop constraints; and add, drop, and modify partitions. For more information about the syntax for modifying tables, see [ALTER TABLE](../../700.reference/500.sql-reference/100.sql-syntax/300.common-tenant-of-oracle-mode/900.sql-statement-of-oracle-mode/100.ddl-of-oracle-mode/1000.alter-table-of-oracle-mode.md).
...
* Supports only B-tree indexes, function indexes, and spatial indexes.
...
* During index rebuild using `UPDATE GLOBAL INDEX`, the index is unavailable.
...
* Compatible with most system privileges in Oracle Database. For more information about the privileges supported by the Oracle-compatible mode of OceanBase Database, see [Privilege types in Oracle-compatible mode](../../600.manage/500.security-and-permissions/300.access-control/200.user-and-permission/300.permission-of-oracle-mode/000.permission-classification-of-oracle-mode.md).
...
* Does not allow you to modify, overwrite, or delete audit data.
...
* Allows you to use Network File System (NFS), Alibaba Cloud Object Storage Service (OSS), Amazon Simple Storage Service (S3), and object storage services compatible with the S3 protocol (for example, Huawei OBS, Google GCS, Tencent Cloud COS) as the backup media.

* Supports automatic and manual cleanup of backups.

* Supports cleanup of specified backups or archives.
...
* Supports log archive and backup data verification.
